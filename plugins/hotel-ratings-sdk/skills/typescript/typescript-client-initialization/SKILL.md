...
- `hotel-ratingslib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'hotel-ratingslib';
...
```ts
import { {Resource}Controller } from 'hotel-ratingslib';
