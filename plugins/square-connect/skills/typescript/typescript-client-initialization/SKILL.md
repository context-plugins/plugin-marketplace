...
- `square-connect-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'square-connect-apilib';
...
```ts
import { {Resource}Controller } from 'square-connect-apilib';
