...
| Project | `SubscriptionClient.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `SubscriptionClient` (the `using` namespace) |
| Client class | `SubscriptionClientClient` |
| Options class | `SubscriptionClientClientOptions` |
...
namespaces transitively, so `using SubscriptionClient.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/subscriptionclient-csharp
dotnet add reference subscriptionclient-csharp/SubscriptionClient.csproj
```

```csharp
using SubscriptionClient;          // client + options + ServerEnvironment
using SubscriptionClient.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/subscriptionclient-csharp /tmp/subscriptionclient-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/subscriptionclient-csharp "$env:TEMP\subscriptionclient-dotnet-src"
```
...
- `SubscriptionClientClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/subscriptionclient-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\subscriptionclient-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `SubscriptionClientClient`, build its
   `SubscriptionClientClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
