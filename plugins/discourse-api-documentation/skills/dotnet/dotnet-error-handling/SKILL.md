...
> `{ApiGroup}`, `DiscourseApiDocumentation`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `DiscourseApiDocumentation.Core.Exceptions`
- `ApiError` **and** `RawError` → `DiscourseApiDocumentation.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `DiscourseApiDocumentation.Errors`
...
using DiscourseApiDocumentation.Core.ErrorResponse;  // ApiError, RawError
using DiscourseApiDocumentation.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`DiscourseApiDocumentation.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using DiscourseApiDocumentation.Core.Exceptions;     // SdkException<TError>
using DiscourseApiDocumentation.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `DiscourseApiDocumentation.Core.ErrorResponse`. Its public
...
`readonly struct` under `DiscourseApiDocumentation.Core.Models`. If the controller has no `{Operation}Result`
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `DiscourseApiDocumentation.Core.Exceptions`) — an auth *application* failure, not an API error — when the
