...
description: "Square TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `square`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any Square TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the Square TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Square TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-square` for the workflow that wraps this skill.
...
Verified against `package.json` and `sdk-map.md` of the generated package at version `2.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Square |
| Package name (what you install, and what you import) | `square` — **not on npm**; built from source (see *Install*) |
| Import specifier | `square` — the package root is the **only** entry; deep imports do not resolve |
| Version | `2.0` (API spec version `2.0`) |
| Client class | `SquareClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new SquareClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new SquareClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `oauth2` · `oauth2Strategy` · `oauth2ClientSecret`. `timeout` defaults to `60_000` ms |
| Auth | **OAuth 2 authorization code** — set `ClientOptions.oauth2`<br>**API key (header)** — set `ClientOptions.oauth2ClientSecret` |
| Environments | 3 environments (`ServerEnvironment.Production` *(default)*, `ServerEnvironment.Sandbox`, `ServerEnvironment.Custom`) × 1 server group |
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://connect.squareup.com` |
...
| Surface | 334 operations · 43 resources · 1271 models · 203 open enums · 0 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/square-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 1474 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { SquareClient, ServerEnvironment, ResponseError, SquareError } from "square";
import type { ClientOptions, AchDetails } from "square";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `square/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **⚠ The SDK exports a model type literally named `Error`** (`src/models/error.ts`, schema `errorSchema`). Import it unaliased and it **shadows the global `Error`** for the rest of the file. Alias it: `import type { Error as SdkError } from "square"`.
- **⚠ The SDK exports a model type literally named `Event`** (`src/models/event.ts`, schema `eventSchema`). Import it unaliased and it **shadows the global `Event`** for the rest of the file. Alias it: `import type { Event as SdkEvent } from "square"`.
- **From CommonJS**, the typed spelling is `import sdk = require("square")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://connect.squareup.com` | `serverOptions.default.production.baseUrl` |
| `default` | `sandbox` | `https://connect.squareupsandbox.com` | `serverOptions.default.sandbox.baseUrl` |
| `default` | `custom` | `{custom_url}` | `serverOptions.default.custom.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 332 of the 334 operations require a credential and 2 are public.

| `ClientOptions` field | Scheme kind | What the SDK sends |
| --- | --- | --- |
| `oauth2` | OAuth 2 authorization code | `Authorization: Bearer <access token>` |
| `oauth2ClientSecret` | API key (header) | header `Authorization: <key>` |

```ts
const client = new SquareClient({
  oauth2: { clientId: process.env.CLIENT_ID!, redirectUri: process.env.REDIRECT_URI!, promptForAuthorizationCode: async (url) => { /* send the user to url */ return "YOUR_AUTHORIZATION_CODE"; } },
  oauth2ClientSecret: process.env.API_KEY!,
});
```
...
| --- | --- | --- | --- |
| `client.oAuth` | `OAuth` | 3 | `obtainToken` · `retrieveTokenStatus` · `revokeToken` |
| `client.v1Transactions` | `V1Transactions` | 3 | `v1ListOrders` · `v1RetrieveOrder` · `v1UpdateOrder` |
| `client.applePay` | `ApplePay` | 1 | `registerDomain` |
| `client.bankAccounts` | `BankAccounts` | 5 | `createBankAccount` · `disableBankAccount` · `getBankAccount` · `getBankAccountByV1Id` · `listBankAccounts` |
| `client.bookings` | `Bookings` | 13 | `bulkRetrieveBookings` · `bulkRetrieveTeamMemberBookingProfiles` · `cancelBooking` · `createBooking` · `listBookings` · `listLocationBookingProfiles` · `listTeamMemberBookingProfiles` · `retrieveBooking` · `retrieveBusinessBookingProfile` · `retrieveLocationBookingProfile` · `retrieveTeamMemberBookingProfile` · `searchAvailability` · `updateBooking` |
| `client.bookingCustomAttributes` | `BookingCustomAttributes` | 11 | `bulkDeleteBookingCustomAttributes` · `bulkUpsertBookingCustomAttributes` · `createBookingCustomAttributeDefinition` · `deleteBookingCustomAttribute` · `deleteBookingCustomAttributeDefinition` · `listBookingCustomAttributeDefinitions` · `listBookingCustomAttributes` · `retrieveBookingCustomAttribute` · `retrieveBookingCustomAttributeDefinition` · `updateBookingCustomAttributeDefinition` · `upsertBookingCustomAttribute` |
| `client.cards` | `Cards` | 4 | `createCard` · `disableCard` · `listCards` · `retrieveCard` |
| `client.cashDrawers` | `CashDrawers` | 3 | `listCashDrawerShiftEvents` · `listCashDrawerShifts` · `retrieveCashDrawerShift` |
| `client.catalog` | `Catalog` | 14 | `batchDeleteCatalogObjects` · `batchRetrieveCatalogObjects` · `batchUpsertCatalogObjects` · `catalogInfo` · `createCatalogImage` · `deleteCatalogObject` · `listCatalog` · `retrieveCatalogObject` · `searchCatalogItems` · `searchCatalogObjects` · `updateCatalogImage` · `updateItemModifierLists` · `updateItemTaxes` · `upsertCatalogObject` |
| `client.channels` | `Channels` | 3 | `bulkRetrieveChannels` · `listChannels` · `retrieveChannel` |
| `client.customers` | `Customers` | 14 | `addGroupToCustomer` · `bulkCreateCustomers` · `bulkDeleteCustomers` · `bulkRetrieveCustomers` · `bulkUpdateCustomers` · `createCustomer` · `createCustomerCard` · `deleteCustomer` · `deleteCustomerCard` · `listCustomers` · `removeGroupFromCustomer` · `retrieveCustomer` · `searchCustomers` · `updateCustomer` |
| `client.customerCustomAttributes` | `CustomerCustomAttributes` | 10 | `bulkUpsertCustomerCustomAttributes` · `createCustomerCustomAttributeDefinition` · `deleteCustomerCustomAttribute` · `deleteCustomerCustomAttributeDefinition` · `listCustomerCustomAttributeDefinitions` · `listCustomerCustomAttributes` · `retrieveCustomerCustomAttribute` · `retrieveCustomerCustomAttributeDefinition` · `updateCustomerCustomAttributeDefinition` · `upsertCustomerCustomAttribute` |
| `client.customerGroups` | `CustomerGroups` | 5 | `createCustomerGroup` · `deleteCustomerGroup` · `listCustomerGroups` · `retrieveCustomerGroup` · `updateCustomerGroup` |
| `client.customerSegments` | `CustomerSegments` | 2 | `listCustomerSegments` · `retrieveCustomerSegment` |
| `client.devices` | `Devices` | 5 | `createDeviceCode` · `getDevice` · `getDeviceCode` · `listDeviceCodes` · `listDevices` |
| `client.disputes` | `Disputes` | 9 | `acceptDispute` · `createDisputeEvidenceFile` · `createDisputeEvidenceText` · `deleteDisputeEvidence` · `listDisputeEvidence` · `listDisputes` · `retrieveDispute` · `retrieveDisputeEvidence` · `submitEvidence` |
| `client.employees` | `Employees` | 2 | `listEmployees` · `retrieveEmployee` |
| `client.events` | `Events` | 4 | `disableEvents` · `enableEvents` · `listEventTypes` · `searchEvents` |
| `client.giftCards` | `GiftCards` | 7 | `createGiftCard` · `linkCustomerToGiftCard` · `listGiftCards` · `retrieveGiftCard` · `retrieveGiftCardFromGan` · `retrieveGiftCardFromNonce` · `unlinkCustomerFromGiftCard` |
| `client.giftCardActivities` | `GiftCardActivities` | 2 | `createGiftCardActivity` · `listGiftCardActivities` |
| `client.inventory` | `Inventory` | 19 | `batchChangeInventory` · `batchRetrieveInventoryChanges` · `batchRetrieveInventoryCounts` · `createInventoryAdjustmentReason` · `deleteInventoryAdjustmentReason` · `deprecatedBatchChangeInventory` · `deprecatedBatchRetrieveInventoryChanges` · `deprecatedBatchRetrieveInventoryCounts` · `deprecatedRetrieveInventoryAdjustment` · `deprecatedRetrieveInventoryPhysicalCount` · `listInventoryAdjustmentReasons` · `restoreInventoryAdjustmentReason` · `retrieveInventoryAdjustment` · `retrieveInventoryAdjustmentReason` · `retrieveInventoryChanges` · `retrieveInventoryCount` · `retrieveInventoryPhysicalCount` · `updateInventoryAdjustment` · `updateInventoryAdjustmentReason` |
| `client.invoices` | `Invoices` | 10 | `cancelInvoice` · `createInvoice` · `createInvoiceAttachment` · `deleteInvoice` · `deleteInvoiceAttachment` · `getInvoice` · `listInvoices` · `publishInvoice` · `searchInvoices` · `updateInvoice` |
| `client.labor` | `Labor` | 27 | `bulkPublishScheduledShifts` · `createBreakType` · `createScheduledShift` · `createShift` · `createTimecard` · `deleteBreakType` · `deleteShift` · `deleteTimecard` · `getBreakType` · `getEmployeeWage` · `getShift` · `getTeamMemberWage` · `listBreakTypes` · `listEmployeeWages` · `listTeamMemberWages` · `listWorkweekConfigs` · `publishScheduledShift` · `retrieveScheduledShift` · `retrieveTimecard` · `searchScheduledShifts` · `searchShifts` · `searchTimecards` · `updateBreakType` · `updateScheduledShift` · `updateShift` · `updateTimecard` · `updateWorkweekConfig` |
| `client.locations` | `Locations` | 4 | `createLocation` · `listLocations` · `retrieveLocation` · `updateLocation` |
| `client.locationCustomAttributes` | `LocationCustomAttributes` | 11 | `bulkDeleteLocationCustomAttributes` · `bulkUpsertLocationCustomAttributes` · `createLocationCustomAttributeDefinition` · `deleteLocationCustomAttribute` · `deleteLocationCustomAttributeDefinition` · `listLocationCustomAttributeDefinitions` · `listLocationCustomAttributes` · `retrieveLocationCustomAttribute` · `retrieveLocationCustomAttributeDefinition` · `updateLocationCustomAttributeDefinition` · `upsertLocationCustomAttribute` |
| `client.checkoutApi` | `CheckoutApi` | 10 | `createCheckout` · `createPaymentLink` · `deletePaymentLink` · `listPaymentLinks` · `retrieveLocationSettings` · `retrieveMerchantSettings` · `retrievePaymentLink` · `updateLocationSettings` · `updateMerchantSettings` · `updatePaymentLink` |
| `client.transactions` | `Transactions` | 4 | `captureTransaction` · `listTransactions` · `retrieveTransaction` · `voidTransaction` |
| `client.loyalty` | `Loyalty` | 18 | `accumulateLoyaltyPoints` · `adjustLoyaltyPoints` · `calculateLoyaltyPoints` · `cancelLoyaltyPromotion` · `createLoyaltyAccount` · `createLoyaltyPromotion` · `createLoyaltyReward` · `deleteLoyaltyReward` · `listLoyaltyPrograms` · `listLoyaltyPromotions` · `redeemLoyaltyReward` · `retrieveLoyaltyAccount` · `retrieveLoyaltyProgram` · `retrieveLoyaltyPromotion` · `retrieveLoyaltyReward` · `searchLoyaltyAccounts` · `searchLoyaltyEvents` · `searchLoyaltyRewards` |
| `client.merchants` | `Merchants` | 2 | `listMerchants` · `retrieveMerchant` |
| `client.merchantCustomAttributes` | `MerchantCustomAttributes` | 11 | `bulkDeleteMerchantCustomAttributes` · `bulkUpsertMerchantCustomAttributes` · `createMerchantCustomAttributeDefinition` · `deleteMerchantCustomAttribute` · `deleteMerchantCustomAttributeDefinition` · `listMerchantCustomAttributeDefinitions` · `listMerchantCustomAttributes` · `retrieveMerchantCustomAttribute` · `retrieveMerchantCustomAttributeDefinition` · `updateMerchantCustomAttributeDefinition` · `upsertMerchantCustomAttribute` |
| `client.orders` | `Orders` | 8 | `batchRetrieveOrders` · `calculateOrder` · `cloneOrder` · `createOrder` · `payOrder` · `retrieveOrder` · `searchOrders` · `updateOrder` |
| `client.orderCustomAttributes` | `OrderCustomAttributes` | 11 | `bulkDeleteOrderCustomAttributes` · `bulkUpsertOrderCustomAttributes` · `createOrderCustomAttributeDefinition` · `deleteOrderCustomAttribute` · `deleteOrderCustomAttributeDefinition` · `listOrderCustomAttributeDefinitions` · `listOrderCustomAttributes` · `retrieveOrderCustomAttribute` · `retrieveOrderCustomAttributeDefinition` · `updateOrderCustomAttributeDefinition` · `upsertOrderCustomAttribute` |
| `client.payments` | `Payments` | 7 | `cancelPayment` · `cancelPaymentByIdempotencyKey` · `completePayment` · `createPayment` · `getPayment` · `listPayments` · `updatePayment` |
| `client.payouts` | `Payouts` | 3 | `getPayout` · `listPayoutEntries` · `listPayouts` |
| `client.refunds` | `Refunds` | 3 | `getPaymentRefund` · `listPaymentRefunds` · `refundPayment` |
| `client.sites` | `Sites` | 1 | `listSites` |
| `client.snippets` | `Snippets` | 3 | `deleteSnippet` · `retrieveSnippet` · `upsertSnippet` |
| `client.subscriptions` | `Subscriptions` | 12 | `bulkSwapPlan` · `cancelSubscription` · `changeBillingAnchorDate` · `createSubscription` · `deleteSubscriptionAction` · `listSubscriptionEvents` · `pauseSubscription` · `resumeSubscription` · `retrieveSubscription` · `searchSubscriptions` · `swapPlan` · `updateSubscription` |
| `client.team` | `Team` | 12 | `bulkCreateTeamMembers` · `bulkUpdateTeamMembers` · `createJob` · `createTeamMember` · `listJobs` · `retrieveJob` · `retrieveTeamMember` · `retrieveWageSetting` · `searchTeamMembers` · `updateJob` · `updateTeamMember` · `updateWageSetting` |
| `client.terminal` | `Terminal` | 15 | `cancelTerminalAction` · `cancelTerminalCheckout` · `cancelTerminalRefund` · `createTerminalAction` · `createTerminalCheckout` · `createTerminalRefund` · `dismissTerminalAction` · `dismissTerminalCheckout` · `dismissTerminalRefund` · `getTerminalAction` · `getTerminalCheckout` · `getTerminalRefund` · `searchTerminalActions` · `searchTerminalCheckouts` · `searchTerminalRefunds` |
| `client.transferOrderApi` | `TransferOrderApi` | 8 | `cancelTransferOrder` · `createTransferOrder` · `deleteTransferOrder` · `receiveTransferOrder` · `retrieveTransferOrder` · `searchTransferOrders` · `startTransferOrder` · `updateTransferOrder` |
| `client.vendors` | `Vendors` | 7 | `bulkCreateVendors` · `bulkRetrieveVendors` · `bulkUpdateVendors` · `createVendor` · `retrieveVendor` · `searchVendors` · `updateVendor` |
| `client.webhookSubscriptions` | `WebhookSubscriptions` | 8 | `createWebhookSubscription` · `deleteWebhookSubscription` · `listWebhookEventTypes` · `listWebhookSubscriptions` · `retrieveWebhookSubscription` · `testWebhookSubscription` · `updateWebhookSubscription` · `updateWebhookSubscriptionSignatureKey` |
...
⚠ **The request type name is not uniformly `<Operation>Request`.** 163 operations take `<Operation>RequestParams` instead: `obtainToken`, `revokeToken`, `v1UpdateOrder`, `registerDomain`, `createBankAccount`, `bulkRetrieveBookings`, `bulkRetrieveTeamMemberBookingProfiles`, `cancelBooking`, `createBooking`, `searchAvailability`, `updateBooking`, `bulkDeleteBookingCustomAttributes`, `bulkUpsertBookingCustomAttributes`, `createBookingCustomAttributeDefinition`, `updateBookingCustomAttributeDefinition`, `upsertBookingCustomAttribute`, `createCard`, `batchDeleteCatalogObjects`, `batchRetrieveCatalogObjects`, `batchUpsertCatalogObjects`, `searchCatalogItems`, `searchCatalogObjects`, `updateCatalogImage`, `updateItemModifierLists`, `updateItemTaxes`, `upsertCatalogObject`, `bulkRetrieveChannels`, `bulkCreateCustomers`, `bulkDeleteCustomers`, `bulkRetrieveCustomers`, `bulkUpdateCustomers`, `createCustomer`, `createCustomerCard`, `searchCustomers`, `updateCustomer`, `bulkUpsertCustomerCustomAttributes`, `createCustomerCustomAttributeDefinition`, `updateCustomerCustomAttributeDefinition`, `upsertCustomerCustomAttribute`, `createCustomerGroup`, `updateCustomerGroup`, `createDeviceCode`, `createDisputeEvidenceFile`, `createDisputeEvidenceText`, `searchEvents`, `createGiftCard`, `linkCustomerToGiftCard`, `retrieveGiftCardFromGan`, `retrieveGiftCardFromNonce`, `unlinkCustomerFromGiftCard`, `createGiftCardActivity`, `batchChangeInventory`, `batchRetrieveInventoryChanges`, `batchRetrieveInventoryCounts`, `createInventoryAdjustmentReason`, `deleteInventoryAdjustmentReason`, `restoreInventoryAdjustmentReason`, `retrieveInventoryAdjustmentReason`, `updateInventoryAdjustment`, `updateInventoryAdjustmentReason`, `cancelInvoice`, `createInvoice`, `createInvoiceAttachment`, `publishInvoice`, `searchInvoices`, `updateInvoice`, `bulkPublishScheduledShifts`, `createBreakType`, `createScheduledShift`, `createShift`, `createTimecard`, `publishScheduledShift`, `searchScheduledShifts`, `searchShifts`, `searchTimecards`, `updateBreakType`, `updateScheduledShift`, `updateShift`, `updateTimecard`, `updateWorkweekConfig`, `createLocation`, `updateLocation`, `bulkDeleteLocationCustomAttributes`, `bulkUpsertLocationCustomAttributes`, `createLocationCustomAttributeDefinition`, `updateLocationCustomAttributeDefinition`, `upsertLocationCustomAttribute`, `createCheckout`, `createPaymentLink`, `updateLocationSettings`, `updateMerchantSettings`, `updatePaymentLink`, `accumulateLoyaltyPoints`, `adjustLoyaltyPoints`, `calculateLoyaltyPoints`, `createLoyaltyAccount`, `createLoyaltyPromotion`, `createLoyaltyReward`, `redeemLoyaltyReward`, `searchLoyaltyAccounts`, `searchLoyaltyEvents`, `searchLoyaltyRewards`, `bulkDeleteMerchantCustomAttributes`, `bulkUpsertMerchantCustomAttributes`, `createMerchantCustomAttributeDefinition`, `updateMerchantCustomAttributeDefinition`, `upsertMerchantCustomAttribute`, `batchRetrieveOrders`, `calculateOrder`, `cloneOrder`, `createOrder`, `payOrder`, `searchOrders`, `updateOrder`, `bulkDeleteOrderCustomAttributes`, `bulkUpsertOrderCustomAttributes`, `createOrderCustomAttributeDefinition`, `updateOrderCustomAttributeDefinition`, `upsertOrderCustomAttribute`, `cancelPaymentByIdempotencyKey`, `completePayment`, `createPayment`, `updatePayment`, `refundPayment`, `upsertSnippet`, `bulkSwapPlan`, `changeBillingAnchorDate`, `createSubscription`, `pauseSubscription`, `resumeSubscription`, `searchSubscriptions`, `swapPlan`, `updateSubscription`, `bulkCreateTeamMembers`, `bulkUpdateTeamMembers`, `createJob`, `createTeamMember`, `searchTeamMembers`, `updateJob`, `updateTeamMember`, `updateWageSetting`, `createTerminalAction`, `createTerminalCheckout`, `createTerminalRefund`, `searchTerminalActions`, `searchTerminalCheckouts`, `searchTerminalRefunds`, `cancelTransferOrder`, `createTransferOrder`, `receiveTransferOrder`, `searchTransferOrders`, `startTransferOrder`, `updateTransferOrder`, `bulkCreateVendors`, `bulkRetrieveVendors`, `bulkUpdateVendors`, `createVendor`, `searchVendors`, `updateVendor`, `createWebhookSubscription`, `testWebhookSubscription`, `updateWebhookSubscription`, `updateWebhookSubscriptionSignatureKey`. Take the name from the operation's **Signature** bullet on its map page; never construct it from the method name.
...
```bash
node -e "console.log(require.resolve('square/package.json'))"
```

Failing that it is at `node_modules/square/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/square/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/square/`:
...
Beyond the usual signatures and model members, a contract sheet for the Square TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.

1. **Which host each deployment talks to**, and where that is set. The members are `ServerEnvironment.Production`, `ServerEnvironment.Sandbox`, `ServerEnvironment.Custom`, defaulting to `ServerEnvironment.Production` when the field is unset.
...
3. **The exact request type name per operation**, taken from the **Signature** bullet — 163 operations take `<Operation>RequestParams`, not `<Operation>Request`.
...
9. **That `Error`, `Event` imported from this package are model types, not the globals of those names** — every sheet that references one should carry the alias it will be imported under. The error base is re-exported as `SquareError` for the same reason.
