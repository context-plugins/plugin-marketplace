...
```ruby
client = ShellCardmanagement::Client.new(
  max_retries:    3,
...
```ruby
client = ShellCardmanagement::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ShellCardmanagement::Client.new(adapter: :net_http)
```
...
client = ShellCardmanagement::Client.new(connection: conn)
```
...
client = ShellCardmanagement::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = ShellCardmanagement::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ShellCardmanagement::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ShellCardmanagement::ProxySettings.from_env
client = ShellCardmanagement::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ShellCardmanagement::HttpCallBack
...
client = ShellCardmanagement::Client.new(http_callback: LogCallback.new)
```
...
client = ShellCardmanagement::Client.from_env(timeout: 30, max_retries: 3)
```
