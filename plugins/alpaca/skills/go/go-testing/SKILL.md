...
`testing` + `testify/require` for reference; substitute the real `alpaca` and names.
...
import (
    ...
    "github.com/context-plugins/alpaca-go-sdk"   // root package alpaca
)
...
func clientReturning(status int, body string) (alpaca.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := alpaca.NewClient(
        alpaca.CreateConfiguration(
            alpaca.WithHttpConfiguration(
                alpaca.CreateHttpConfiguration(alpaca.WithTransport(stub)),
            ),
...
client := alpaca.NewClient(
    alpaca.CreateConfiguration(
        alpaca.WithHttpConfiguration(
            alpaca.CreateHttpConfiguration(alpaca.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
