...
kwargs to the `SpotifyWebAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `spotifywebapi/spotifywebapi_client.py` and `spotifywebapi/http/auth/` in
the cloned source.

## Basic auth

```python
from spotifywebapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from spotifywebapi.http.auth.api_key import ApiKeyCredentials
...
```python
from spotifywebapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from spotifywebapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from spotifywebapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from spotifywebapi.http.auth.o_auth_acg import OAuthACGCredentials
from spotifywebapi.models.o_auth_scope_spotifywebapi_enum import OAuthScope{Pkg}Enum
...
client = SpotifyWebAPIClient(o_auth_acg_credentials=creds)
...
```python
from spotifywebapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SpotifyWebAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SpotifyWebAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `spotifywebapi/configuration.py` and each `spotifywebapi/http/auth/*.py` for the exact
