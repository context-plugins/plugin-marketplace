...
`deploymentadminclient/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `deploymentadminclient/exceptions/` and operation docstrings.
...
```python
from deploymentadminclient.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `deploymentadminclient/exceptions/`.
...
```python
from deploymentadminclient.exceptions.api_exception import APIException
from deploymentadminclient.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`deploymentadminclient/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`deploymentadminclient/exceptions/` for the class.
...
`deploymentadminclient/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `deploymentadminclient/exceptions/`
  or a `ValueError` — check the auth handler in `deploymentadminclient/http/auth/` for the exact type.
