...
│   └── net.newstore.p.dodicidemo.exceptions.ApiException          ← base for all non-2xx API responses
│       └── net.newstore.p.dodicidemo.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `net.newstore.p.dodicidemo.http.client.HttpContext`, which carries:
