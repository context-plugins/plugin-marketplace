...
| --- | --- |
| API | mindbody |
| Source repo | https://github.com/context-plugins/mindbody-csharp-sdk (branch `main`) |
| Root namespace | `MindbodyPushApiApi` (the `using` namespace) |
| Client class | `MindbodyPushApiApiClient` |
| Options class | `MindbodyPushApiApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `MindbodyPushApiApiClientOptions`: `ApiKey: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using MindbodyPushApiApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/mindbody-csharp-sdk
dotnet add reference mindbody-csharp-sdk/MindbodyPushApiApi.csproj
```
...
temp directory** (`<temp>/mindbody-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/mindbody-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/mindbody-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/mindbody-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\mindbody-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/mindbody-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/mindbody-dotnet-src/`, **never** in the project repo, and its path never goes into
`mindbody-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `MindbodyPushApiApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new MindbodyPushApiApiClient(...)`, build its options, or DI-register via
   `AddMindbodyPushApiApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `MindbodyPushApiApiClientOptions` — the map's *Servers & auth* section
