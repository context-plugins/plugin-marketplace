...
> `MailboxValidatorFreeEmailCheckerClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): MailboxValidatorFreeEmailCheckerClient
{
...
```php
use MailboxValidatorFreeEmailCheckerLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use MailboxValidatorFreeEmailCheckerLib\Exceptions\{OperationException};
use MailboxValidatorFreeEmailCheckerLib\Exceptions\ApiException;
...
app()->instance(MailboxValidatorFreeEmailCheckerClient::class, clientWithResponses([
