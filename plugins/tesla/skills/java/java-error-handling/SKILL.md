...
│   └── com.tesla.cloud.vn.na.prd.fleetapi.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.tesla.cloud.vn.na.prd.fleetapi.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.tesla.cloud.vn.na.prd.fleetapi.http.client.HttpContext`, which carries:
