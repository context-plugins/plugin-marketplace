...
```ruby
client = PayPalAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = PayPalAPI::Client.new(adapter: :net_http)
```
...
client = PayPalAPI::Client.new(connection: conn)
```
...
client = PayPalAPI::Client.new(environment: Environment::PRODUCTION)
client = PayPalAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = PayPalAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = PayPalAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = PayPalAPI::ProxySettings.from_env
client = PayPalAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < PayPalAPI::HttpCallBack
...
client = PayPalAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = PayPalAPI::Client.from_env
# Overrides take precedence over env vars:
client = PayPalAPI::Client.from_env(timeout: 30, max_retries: 3)
```
