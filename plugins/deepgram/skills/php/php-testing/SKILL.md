...
> `DeepgramClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): DeepgramClient
{
...
```php
use DeepgramLib\Exceptions\{OperationException};
use DeepgramLib\Exceptions\ApiException;
...
app()->instance(DeepgramClient::class, clientWithResponses([
