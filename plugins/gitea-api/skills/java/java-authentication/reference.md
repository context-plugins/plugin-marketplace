...
against the `GiteaAPIClient.Builder` setters, the `com.example.www.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GiteaAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
GiteaAPIClient client = new GiteaAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeGiteaAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
