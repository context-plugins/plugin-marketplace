...
- `locationscore` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/location-score-go` — the Go import path for the root package; `github.com/context-plugins/location-score-go/models` for request/response types.
...
```go
import "github.com/context-plugins/location-score-go"

client := locationscore.NewClient(
    locationscore.CreateConfiguration(
        locationscore.WithEnvironment(locationscore.PRODUCTION),
...
from environment variables named `LOCATIONSCORE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := locationscore.NewClient(locationscore.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `locationscore.PRODUCTION`,
`locationscore.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := locationscore.NewClient(
    locationscore.CreateConfiguration(locationscore.WithEnvironment(locationscore.PRODUCTION)),
)
```
...
client := locationscore.NewClient(
    locationscore.CreateConfiguration(
        locationscore.WithHttpConfiguration(
            locationscore.CreateHttpConfiguration(
                locationscore.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                locationscore.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = locationscore.NewClient(locationscore.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api locationscore.ClientInterface
}

func NewService(api locationscore.ClientInterface) *Service {
