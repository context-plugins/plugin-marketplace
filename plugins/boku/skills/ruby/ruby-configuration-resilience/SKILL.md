...
```ruby
client = Boku::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Boku::Client.new(adapter: :net_http)
```
...
client = Boku::Client.new(connection: conn)
```
...
client = Boku::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Boku::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Boku::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Boku::ProxySettings.from_env
client = Boku::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Boku::HttpCallBack
...
client = Boku::Client.new(http_callback: LogCallback.new)
```
...
client = Boku::Client.from_env(timeout: 30, max_retries: 3)
```
