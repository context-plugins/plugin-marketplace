...
your SDK source (`finnhub/models/`).
...
```python
from finnhub.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `finnhub/utilities/union_type_lookup.py`) and each candidate model
...
```python
from finnhub.models.animal import Cat, Dog
...
```python
from finnhub.models.animal import Animal, Cat, Dog
