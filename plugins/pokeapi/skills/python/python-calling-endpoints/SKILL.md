...
Open `pokeapi/pokeapi_client.py` for the controller property names, then the relevant
`pokeapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from pokeapi.models.{request_model} import {RequestModel}
...
Open `pokeapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from pokeapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from pokeapi.http.http_call_back import HttpCallBack
...
client = PokapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `pokeapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `PokapiClient` in `pokeapi/pokeapi_client.py`.
- Operation method signatures are in `pokeapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `pokeapi/models/`; enum types have `from_value()`.
