...
```typescript
import { {Operation}Error } from 'maxio-advanced-billinglib/errors';
...
```typescript
import { ApiError } from 'maxio-advanced-billinglib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(MaxioAdvancedBillingClient)
    .useValue(new MaxioAdvancedBillingClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
