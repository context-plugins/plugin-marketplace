...
`StaxFattMerchantApiClient`/`StaxFattMerchantApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `StaxFattMerchantApiClient`,
...
static StaxFattMerchantApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new StaxFattMerchantApiClient(new HttpClient(handler), new StaxFattMerchantApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using StaxFattMerchantApi.Errors;              // {Operation}Error types
...
using StaxFattMerchantApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new StaxFattMerchantApiClient(new HttpClient(handler), new StaxFattMerchantApiClientOptions());
...
- For DI-based code, the SDK's `AddStaxFattMerchantApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `StaxFattMerchantApiClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<StaxFattMerchantApiClient>();
  ```
