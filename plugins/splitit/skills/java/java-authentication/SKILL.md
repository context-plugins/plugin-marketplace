...
To see which schemes a specific SDK accepts, read the credential setter methods on `SplititClient.Builder`
...
- Set credentials **on the `SplititClient.Builder`** before calling `.build()` — the client is immutable
