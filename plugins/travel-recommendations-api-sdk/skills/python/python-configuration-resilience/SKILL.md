...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `TravelRecommendationsAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`travelrecommendationsapi/configuration.py` in the cloned source.
...
```python
import requests
from travelrecommendationsapi.configuration import Configuration
...
`travelrecommendationsapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`travelrecommendationsapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `travelrecommendationsapi/configuration.py`
...
`NumberPagedResponse`) live in `travelrecommendationsapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `travelrecommendationsapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
