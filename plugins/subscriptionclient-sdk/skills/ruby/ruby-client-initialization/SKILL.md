...
- `SubscriptionClient` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `subscriptionclient` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'subscriptionclient'
include SubscriptionClient
...
The authoritative list is the `initialize` signature in `lib/subscriptionclient/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/subscriptionclient/configuration.rb`
(`build_default_config_from_env`) and `lib/subscriptionclient/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/subscriptionclient.rb`) and store it in a
