...
All types below live under `DiscourseApiDocumentation.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using DiscourseApiDocumentation.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new DiscourseApiDocumentationClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `DiscourseApiDocumentation.Core.Exceptions`):
...
With DI, the SDK's `AddDiscourseApiDocumentationClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddDiscourseApiDocumentationClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
