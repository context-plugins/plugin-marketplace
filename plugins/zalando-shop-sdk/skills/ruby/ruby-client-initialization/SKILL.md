...
- `ZalandoShop` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `zalando_shop` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/zalando_shop/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/zalando_shop/configuration.rb`
(`build_default_config_from_env`) and `lib/zalando_shop/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/zalando_shop.rb`) and store it in a
