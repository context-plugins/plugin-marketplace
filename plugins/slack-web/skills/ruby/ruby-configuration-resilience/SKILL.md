...
```ruby
client = SlackWebAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = SlackWebAPI::Client.new(adapter: :net_http)
```
...
client = SlackWebAPI::Client.new(connection: conn)
```
...
client = SlackWebAPI::Client.new(environment: Environment::PRODUCTION)
client = SlackWebAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = SlackWebAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = SlackWebAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = SlackWebAPI::ProxySettings.from_env
client = SlackWebAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < SlackWebAPI::HttpCallBack
...
client = SlackWebAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = SlackWebAPI::Client.from_env
# Overrides take precedence over env vars:
client = SlackWebAPI::Client.from_env(timeout: 30, max_retries: 3)
```
