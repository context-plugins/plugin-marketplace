...
- `1PasswordConnect` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `1password_connect` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require '1password_connect'
include 1PasswordConnect
...
The authoritative list is the `initialize` signature in `lib/1password_connect/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/1password_connect/configuration.rb`
(`build_default_config_from_env`) and `lib/1password_connect/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/1password_connect.rb`) and store it in a
