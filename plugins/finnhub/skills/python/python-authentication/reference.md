...
kwarg names are generated per-API — confirm from `finnhub/finnhub_client.py` and `finnhub/http/auth/` in
the cloned source.

## Basic auth

```python
from finnhub.http.auth.basic_auth import BasicAuthCredentials
...
```python
from finnhub.http.auth.api_key import ApiKeyCredentials
...
```python
from finnhub.http.auth.api_header import ApiHeaderCredentials
...
```python
from finnhub.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from finnhub.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from finnhub.http.auth.o_auth_acg import OAuthACGCredentials
from finnhub.models.o_auth_scope_finnhub_enum import OAuthScope{Pkg}Enum
...
```python
from finnhub.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `finnhub/configuration.py` and each `finnhub/http/auth/*.py` for the exact
