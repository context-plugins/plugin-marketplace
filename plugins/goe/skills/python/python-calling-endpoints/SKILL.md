...
Open `goe/goe_client.py` for the controller property names, then the relevant
`goe/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from goe.models.{request_model} import {RequestModel}
...
Open `goe/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from goe.models.suite_code_enum import SuiteCodeEnum
...
```python
from goe.http.http_call_back import HttpCallBack
...
client = GoeapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `goe/pagination/` in the source.

## Finding the right method

- Controller property names are on `GoeapiClient` in `goe/goe_client.py`.
- Operation method signatures are in `goe/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `goe/models/`; enum types have `from_value()`.
