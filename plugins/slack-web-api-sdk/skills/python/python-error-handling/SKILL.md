...
`slackwebapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `slackwebapi/exceptions/` and operation docstrings.
...
```python
from slackwebapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `slackwebapi/exceptions/`.
...
file (`slackwebapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`slackwebapi/exceptions/` for the class.
...
`slackwebapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `slackwebapi/exceptions/`
  or a `ValueError` — check the auth handler in `slackwebapi/http/auth/` for the exact type.
