...
`vimeo/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `vimeo/exceptions/` and operation docstrings.
...
```python
from vimeo.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `vimeo/exceptions/`.
...
```python
from vimeo.exceptions.api_exception import APIException
from vimeo.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`vimeo/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`vimeo/exceptions/` for the class.
...
`vimeo/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `vimeo/exceptions/`
  or a `ValueError` — check the auth handler in `vimeo/http/auth/` for the exact type.
