...
your SDK source (`sportsdata/models/`).
...
```python
from sportsdata.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `sportsdata/utilities/union_type_lookup.py`) and each candidate model
...
```python
from sportsdata.models.animal import Cat, Dog
...
```python
from sportsdata.models.animal import Animal, Cat, Dog
