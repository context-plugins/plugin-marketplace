...
> `XeroAccountingAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): XeroAccountingAPIClient
{
...
```php
use XeroAccountingAPILib\Exceptions\{OperationException};
use XeroAccountingAPILib\Exceptions\ApiException;
...
app()->instance(XeroAccountingAPIClient::class, clientWithResponses([
