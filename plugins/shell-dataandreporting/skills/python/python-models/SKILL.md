...
your SDK source (`shelldataandreporting/models/`).
...
```python
from shelldataandreporting.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `shelldataandreporting/utilities/union_type_lookup.py`) and each candidate model
...
```python
from shelldataandreporting.models.animal import Cat, Dog
...
```python
from shelldataandreporting.models.animal import Animal, Cat, Dog
