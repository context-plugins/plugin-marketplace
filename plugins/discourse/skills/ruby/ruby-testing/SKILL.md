...
client = stub_client(stubs)
assert_raises Discourse::APIException do
...
BASE = Discourse::Configuration.new(environment: Discourse::Environment::PRODUCTION).get_base_uri

def build_test_client
  Discourse::Client.new(
    environment: Discourse::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Discourse::APIException
```
...
```ruby
expect { call }.to raise_error(Discourse::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Discourse::HttpCallBack
...
client  = Discourse::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
