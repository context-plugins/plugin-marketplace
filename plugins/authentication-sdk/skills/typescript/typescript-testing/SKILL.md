...
```typescript
import { {Operation}Error } from 'authenticationlib/errors';
...
```typescript
import { ApiError } from 'authenticationlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AuthenticationClient)
    .useValue(new AuthenticationClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
