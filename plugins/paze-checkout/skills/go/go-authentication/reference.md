...
```go
pazecheckoutapi.WithOAuthCCGCredentials(
    pazecheckoutapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr pazecheckoutapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := pazecheckoutapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr pazecheckoutapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := pazecheckoutapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := pazecheckoutapi.NewClient(pazecheckoutapi.CreateConfiguration(pazecheckoutapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    pazecheckoutapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := pazecheckoutapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := pazecheckoutapi.NewClient(pazecheckoutapi.CreateConfiguration(pazecheckoutapi.WithOAuthROPCGCredentials(creds)))
...
```go
pazecheckoutapi.WithOAuthBearerTokenCredentials(pazecheckoutapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
pazecheckoutapi.WithBasicAuthCredentials(pazecheckoutapi.NewBasicAuthCredentials(username, password))
pazecheckoutapi.WithApiHeaderCredentials(pazecheckoutapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
pazecheckoutapi.WithApiKeyCredentials(pazecheckoutapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `PAZECHECKOUTAPI_...` variables — e.g.
`PAZECHECKOUTAPI_USERNAME`, `PAZECHECKOUTAPI_PASSWORD`, `PAZECHECKOUTAPI_TOKEN`, `PAZECHECKOUTAPI_API_KEY`,
`PAZECHECKOUTAPI_O_AUTH_CLIENT_ID`, `PAZECHECKOUTAPI_O_AUTH_CLIENT_SECRET`, `PAZECHECKOUTAPI_ACCESS_TOKEN`. Grep the
