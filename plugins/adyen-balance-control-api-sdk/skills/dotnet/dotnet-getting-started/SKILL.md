...
| Project | `AdyenBalanceControlApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `AdyenBalanceControlApi` (the `using` namespace) |
| Client class | `AdyenBalanceControlApiClient` |
| Options class | `AdyenBalanceControlApiClientOptions` |
...
namespaces transitively, so `using AdyenBalanceControlApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/adyen-balance-control-api-csharp
dotnet add reference adyen-balance-control-api-csharp/AdyenBalanceControlApi.csproj
```

```csharp
using AdyenBalanceControlApi;          // client + options + ServerEnvironment
using AdyenBalanceControlApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/adyen-balance-control-api-csharp /tmp/adyen-balance-control-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/adyen-balance-control-api-csharp "$env:TEMP\adyen-balance-control-api-dotnet-src"
```
...
- `AdyenBalanceControlApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/adyen-balance-control-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\adyen-balance-control-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AdyenBalanceControlApiClient`, build its
   `AdyenBalanceControlApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
