...
> `FlightPriceAnalysisAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FlightPriceAnalysisAPIClient
{
...
```php
use FlightPriceAnalysisAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use FlightPriceAnalysisAPILib\Exceptions\{OperationException};
use FlightPriceAnalysisAPILib\Exceptions\ApiException;
...
app()->instance(FlightPriceAnalysisAPIClient::class, clientWithResponses([
