...
`FirecrawlApiClient`/`FirecrawlApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `FirecrawlApiClient`,
...
static FirecrawlApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new FirecrawlApiClient(new HttpClient(handler), new FirecrawlApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using FirecrawlApi.Errors;              // {Operation}Error types
...
using FirecrawlApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new FirecrawlApiClient(new HttpClient(handler), new FirecrawlApiClientOptions());
...
- For DI-based code, the SDK's `AddFirecrawlApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `FirecrawlApiClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<FirecrawlApiClient>();
  ```
