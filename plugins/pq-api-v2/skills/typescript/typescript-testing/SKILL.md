...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new PQAPIV2Client({
...
```typescript
import { {Operation}Error } from 'pq-api-v-2-lib/errors';
...
```typescript
import { ApiError } from 'pq-api-v-2-lib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(PQAPIV2Client)
    .useValue(new PQAPIV2Client({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
