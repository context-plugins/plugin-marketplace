...
- `flight-inspiration-searchlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'flight-inspiration-searchlib';
...
```ts
import { {Resource}Controller } from 'flight-inspiration-searchlib';
