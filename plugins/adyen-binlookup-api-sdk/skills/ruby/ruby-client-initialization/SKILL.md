...
- `AdyenBinLookupAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `adyen_binlookup_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'adyen_binlookup_api'
include AdyenBinLookupAPI
...
The authoritative list is the `initialize` signature in `lib/adyen_binlookup_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/adyen_binlookup_api/configuration.rb`
(`build_default_config_from_env`) and `lib/adyen_binlookup_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/adyen_binlookup_api.rb`) and store it in a
