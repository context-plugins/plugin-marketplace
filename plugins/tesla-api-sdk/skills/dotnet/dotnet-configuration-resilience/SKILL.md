...
All types below live under `TeslaApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using TeslaApi.Core.Configuration;
...
```csharp
using TeslaApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new TeslaApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `TeslaApi.Core.Exceptions`):
...
With DI, the SDK's `AddTeslaApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddTeslaApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
