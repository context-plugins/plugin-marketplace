...
```go
shutterstock.WithOAuthCCGCredentials(
    shutterstock.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr shutterstock.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := shutterstock.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr shutterstock.OAuthCCGManager) models.OAuthToken {
...
```go
creds := shutterstock.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := shutterstock.NewClient(shutterstock.CreateConfiguration(shutterstock.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    shutterstock.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := shutterstock.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := shutterstock.NewClient(shutterstock.CreateConfiguration(shutterstock.WithOAuthROPCGCredentials(creds)))
...
```go
shutterstock.WithOAuthBearerTokenCredentials(shutterstock.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
shutterstock.WithBasicAuthCredentials(shutterstock.NewBasicAuthCredentials(username, password))
shutterstock.WithApiHeaderCredentials(shutterstock.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
shutterstock.WithApiKeyCredentials(shutterstock.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SHUTTERSTOCK_...` variables — e.g.
`SHUTTERSTOCK_USERNAME`, `SHUTTERSTOCK_PASSWORD`, `SHUTTERSTOCK_TOKEN`, `SHUTTERSTOCK_API_KEY`,
`SHUTTERSTOCK_O_AUTH_CLIENT_ID`, `SHUTTERSTOCK_O_AUTH_CLIENT_SECRET`, `SHUTTERSTOCK_ACCESS_TOKEN`. Grep the
