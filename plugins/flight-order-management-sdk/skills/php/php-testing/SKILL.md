...
> `FlightOrderManagementClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FlightOrderManagementClient
{
...
```php
use FlightOrderManagementLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use FlightOrderManagementLib\Exceptions\{OperationException};
use FlightOrderManagementLib\Exceptions\ApiException;
...
app()->instance(FlightOrderManagementClient::class, clientWithResponses([
