...
Open `lib/deploymentadminclient/client.rb` for the accessor names, then the relevant
`lib/deploymentadminclient/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = DeploymentAdminClient::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: DeploymentAdminClient::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/deploymentadminclient/models/`. Field names are defined as
...
Read the enum module in `lib/deploymentadminclient/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/deploymentadminclient/client.rb`.
- Operation methods are in `lib/deploymentadminclient/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/deploymentadminclient/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/deploymentadminclient/exceptions/`.
