...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddForgeFinanceApis1Client extension. Use the moment you write `new ForgeFinanceApis1Client(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `ForgeFinanceApis1Client` — the single public client class (e.g. `FooClient`).
- `ForgeFinanceApis1ClientOptions` — its options class.
- `ForgeFinanceApis1` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddForgeFinanceApis1Client`, which registers the
...
builder.Services.AddForgeFinanceApis1Client(options =>
{
...
```csharp
public sealed class MyService(ForgeFinanceApis1Client client)
{
