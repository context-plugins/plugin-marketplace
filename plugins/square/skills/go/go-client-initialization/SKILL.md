...
- `square` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/square-go-sdk` — the Go import path for the root package; `github.com/context-plugins/square-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/square-go-sdk"

client := square.NewClient(
    square.CreateConfiguration(
        square.WithEnvironment(square.PRODUCTION),
...
from environment variables named `SQUARE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := square.NewClient(square.CreateConfigurationFromEnvironment())
```
...
```go
client := square.NewClient(
    square.CreateConfiguration(square.WithEnvironment(square.PRODUCTION)),
)
```
...
client := square.NewClient(
    square.CreateConfiguration(
        square.WithHttpConfiguration(
            square.CreateHttpConfiguration(
                square.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                square.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = square.NewClient(square.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api square.ClientInterface
}

func NewService(api square.ClientInterface) *Service {
