...
```go
sqlmanagementclient.WithOAuthCCGCredentials(
    sqlmanagementclient.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr sqlmanagementclient.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := sqlmanagementclient.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr sqlmanagementclient.OAuthCCGManager) models.OAuthToken {
...
```go
creds := sqlmanagementclient.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := sqlmanagementclient.NewClient(sqlmanagementclient.CreateConfiguration(sqlmanagementclient.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    sqlmanagementclient.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := sqlmanagementclient.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := sqlmanagementclient.NewClient(sqlmanagementclient.CreateConfiguration(sqlmanagementclient.WithOAuthROPCGCredentials(creds)))
...
```go
sqlmanagementclient.WithOAuthBearerTokenCredentials(sqlmanagementclient.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
sqlmanagementclient.WithBasicAuthCredentials(sqlmanagementclient.NewBasicAuthCredentials(username, password))
sqlmanagementclient.WithApiHeaderCredentials(sqlmanagementclient.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
sqlmanagementclient.WithApiKeyCredentials(sqlmanagementclient.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SQLMANAGEMENTCLIENT_...` variables — e.g.
`SQLMANAGEMENTCLIENT_USERNAME`, `SQLMANAGEMENTCLIENT_PASSWORD`, `SQLMANAGEMENTCLIENT_TOKEN`, `SQLMANAGEMENTCLIENT_API_KEY`,
`SQLMANAGEMENTCLIENT_O_AUTH_CLIENT_ID`, `SQLMANAGEMENTCLIENT_O_AUTH_CLIENT_SECRET`, `SQLMANAGEMENTCLIENT_ACCESS_TOKEN`. Grep the
