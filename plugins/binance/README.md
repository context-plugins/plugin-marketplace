...
Integrate and consume the **APIMatic-generated Binance SDK** (the "APIMATIC v3.0" generic
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, TypeScript.
...
(`dotnet-binance-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-binance`** router skill routes all
C#/.NET SDK work to the single **`dotnet-binance-sdk`** agent (.NET-only), which answers
