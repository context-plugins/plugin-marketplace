...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `paypallib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'paypallib';
import { ApiError } from 'paypallib';
import { {Operation}Error } from 'paypallib/errors';
