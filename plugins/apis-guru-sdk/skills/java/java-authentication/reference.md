...
against the `APIsGuruClient.Builder` setters, the `guru.apis.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `APIsGuruClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
APIsGuruClient client = new APIsGuruClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAPIsGuruEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
