...
```go
adyenbalancecontrolapi.WithOAuthCCGCredentials(
    adyenbalancecontrolapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr adyenbalancecontrolapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := adyenbalancecontrolapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr adyenbalancecontrolapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := adyenbalancecontrolapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := adyenbalancecontrolapi.NewClient(adyenbalancecontrolapi.CreateConfiguration(adyenbalancecontrolapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    adyenbalancecontrolapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := adyenbalancecontrolapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := adyenbalancecontrolapi.NewClient(adyenbalancecontrolapi.CreateConfiguration(adyenbalancecontrolapi.WithOAuthROPCGCredentials(creds)))
...
```go
adyenbalancecontrolapi.WithOAuthBearerTokenCredentials(adyenbalancecontrolapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
adyenbalancecontrolapi.WithBasicAuthCredentials(adyenbalancecontrolapi.NewBasicAuthCredentials(username, password))
adyenbalancecontrolapi.WithApiHeaderCredentials(adyenbalancecontrolapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
adyenbalancecontrolapi.WithApiKeyCredentials(adyenbalancecontrolapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `ADYENBALANCECONTROLAPI_...` variables — e.g.
`ADYENBALANCECONTROLAPI_USERNAME`, `ADYENBALANCECONTROLAPI_PASSWORD`, `ADYENBALANCECONTROLAPI_TOKEN`, `ADYENBALANCECONTROLAPI_API_KEY`,
`ADYENBALANCECONTROLAPI_O_AUTH_CLIENT_ID`, `ADYENBALANCECONTROLAPI_O_AUTH_CLIENT_SECRET`, `ADYENBALANCECONTROLAPI_ACCESS_TOKEN`. Grep the
