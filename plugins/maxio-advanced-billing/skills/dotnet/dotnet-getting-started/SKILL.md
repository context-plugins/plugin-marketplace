...
| Project | `MaxioAdvancedBilling.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using MaxioAdvancedBilling.Models;` alone does **not** make enums, union types, or
...
```csharp
using MaxioAdvancedBilling;          // client + options + ServerEnvironment
using MaxioAdvancedBilling.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/maxio-advanced-billing-csharp-sdk /tmp/maxio-advanced-billing-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/maxio-advanced-billing-csharp-sdk "$env:TEMP\maxio-advanced-billing-dotnet-src"
```
...
- `MaxioAdvancedBillingClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/maxio-advanced-billing-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\maxio-advanced-billing-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `MaxioAdvancedBillingClient`, build its
   `MaxioAdvancedBillingClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
