...
against the `VerizonClient.Builder` setters, the `com.verizon.thingspace.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `VerizonClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
VerizonClient client = new VerizonClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeVerizonEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
