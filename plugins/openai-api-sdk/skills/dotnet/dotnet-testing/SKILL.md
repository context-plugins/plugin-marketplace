...
`OpenAiApiClient`/`OpenAiApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `OpenAiApiClient`,
...
static OpenAiApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new OpenAiApiClient(new HttpClient(handler), new OpenAiApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using OpenAiApi.Errors;              // {Operation}Error types
...
```csharp
using OpenAiApi.Core.Exceptions;
using OpenAiApi.Core.ErrorResponse;
...
using OpenAiApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new OpenAiApiClient(new HttpClient(handler), new OpenAiApiClientOptions());
...
- For DI-based code, the SDK's `AddOpenAiApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `OpenAiApiClient` from the provider:
  ```csharp
  services.AddOpenAiApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<OpenAiApiClient>();
  ```
