...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ShellAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`shellapi/configuration.py` in the cloned source.
...
```python
import requests
from shellapi.configuration import Configuration
...
`shellapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`shellapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `shellapi/configuration.py`
...
`NumberPagedResponse`) live in `shellapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `shellapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
