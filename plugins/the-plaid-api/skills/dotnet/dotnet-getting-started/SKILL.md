...
| --- | --- |
| API | `The Plaid API` |
| Project | `ThePlaidApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using ThePlaidApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/the-plaid-api-csharp-sdk
dotnet add reference the-plaid-api-csharp-sdk/ThePlaidApi.csproj
```

```csharp
using ThePlaidApi;          // client + options + ServerEnvironment
using ThePlaidApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/the-plaid-api-csharp-sdk /tmp/the-plaid-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/the-plaid-api-csharp-sdk "$env:TEMP\the-plaid-api-dotnet-src"
```
...
- `ThePlaidApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/the-plaid-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\the-plaid-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `ThePlaidApiClient`, build its
   `ThePlaidApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
