...
kwargs to the `GoogleMapsPlatformClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `googlemapsplatform/googlemapsplatform_client.py` and `googlemapsplatform/http/auth/` in
the cloned source.

## Basic auth

```python
from googlemapsplatform.http.auth.basic_auth import BasicAuthCredentials
...
```python
from googlemapsplatform.http.auth.api_key import ApiKeyCredentials
...
```python
from googlemapsplatform.http.auth.api_header import ApiHeaderCredentials
...
```python
from googlemapsplatform.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from googlemapsplatform.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from googlemapsplatform.http.auth.o_auth_acg import OAuthACGCredentials
from googlemapsplatform.models.o_auth_scope_googlemapsplatform_enum import OAuthScope{Pkg}Enum
...
client = GoogleMapsPlatformClient(o_auth_acg_credentials=creds)
...
```python
from googlemapsplatform.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = GoogleMapsPlatformClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GoogleMapsPlatformClient.from_environment()`) read each credential
...
Grep `from_environment` in `googlemapsplatform/configuration.py` and each `googlemapsplatform/http/auth/*.py` for the exact
