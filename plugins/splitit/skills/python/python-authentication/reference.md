...
kwargs to the `SplititClient` constructor (or to `Configuration`). Exact credential class names and
...
client = SplititClient(o_auth_acg_credentials=creds)
...
client = SplititClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SplititClient.from_environment()`) read each credential
