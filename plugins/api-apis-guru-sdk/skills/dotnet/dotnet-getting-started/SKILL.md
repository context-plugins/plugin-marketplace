...
| --- | --- |
| API | `api.apis.guru` |
| Project | `TripPurposePrediction.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `TripPurposePrediction` (the `using` namespace) |
| Client class | `TripPurposePredictionClient` |
| Options class | `TripPurposePredictionClientOptions` |
...
namespaces transitively, so `using TripPurposePrediction.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/api-apis-guru-csharp-832e4b
dotnet add reference api-apis-guru-csharp-832e4b/TripPurposePrediction.csproj
```

```csharp
using TripPurposePrediction;          // client + options + ServerEnvironment
using TripPurposePrediction.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-832e4b /tmp/api-apis-guru-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-832e4b "$env:TEMP\api-apis-guru-dotnet-src"
```
...
- `TripPurposePredictionClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `TripPurposePredictionClient`, build its
   `TripPurposePredictionClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
