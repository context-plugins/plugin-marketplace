...
client = stub_client(stubs)
assert_raises Vimeo::APIException do
...
BASE = Vimeo::Configuration.new(environment: Vimeo::Environment::PRODUCTION).get_base_uri

def build_test_client
  Vimeo::Client.new(
    environment: Vimeo::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Vimeo::APIException
```
...
```ruby
expect { call }.to raise_error(Vimeo::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Vimeo::HttpCallBack
...
client  = Vimeo::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
