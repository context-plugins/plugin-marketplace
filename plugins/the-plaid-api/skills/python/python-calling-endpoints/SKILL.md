...
Open `theplaidapi/theplaidapi_client.py` for the controller property names, then the relevant
`theplaidapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from theplaidapi.models.{request_model} import {RequestModel}
...
Open `theplaidapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from theplaidapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from theplaidapi.http.http_call_back import HttpCallBack
...
client = ThePlaidAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `theplaidapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `ThePlaidAPIClient` in `theplaidapi/theplaidapi_client.py`.
- Operation method signatures are in `theplaidapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `theplaidapi/models/`; enum types have `from_value()`.
