...
- `gitea-api-lib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'gitea-api-lib';
...
```ts
import { {Resource}Controller } from 'gitea-api-lib';
