...
| --- | --- |
| API | `CellPoint API` |
| Project | `CellPointApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `CellPointApi` (the `using` namespace) |
| Client class | `CellPointApiClient` |
| Options class | `CellPointApiClientOptions` |
...
namespaces transitively, so `using CellPointApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/cellpoint-api-csharp
dotnet add reference cellpoint-api-csharp/CellPointApi.csproj
```

```csharp
using CellPointApi;          // client + options + ServerEnvironment
using CellPointApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/cellpoint-api-csharp /tmp/cellpoint-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/cellpoint-api-csharp "$env:TEMP\cellpoint-api-dotnet-src"
```
...
- `CellPointApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/cellpoint-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\cellpoint-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `CellPointApiClient`, build its
   `CellPointApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
