...
Confirm every name against `lib/flight_create_orders/client.rb`, `lib/flight_create_orders/configuration.rb`, the
`lib/flight_create_orders/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
