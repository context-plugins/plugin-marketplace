...
Open `FlightOffersPriceClient.java` for the full list of `get*Controller()` methods. Controllers are stateless;
...
- `FlightOffersPriceClient.java` — controller accessor methods; each `get{Resource}Controller()` returns a
  `{Resource}Controller`.
