...
- `on-demand-flight-statuslib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'on-demand-flight-statuslib';
...
```ts
import { {Resource}Controller } from 'on-demand-flight-statuslib';
