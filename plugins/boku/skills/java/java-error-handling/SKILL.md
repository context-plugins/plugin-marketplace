...
│   └── com.boku.gbapi4stage.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.boku.gbapi4stage.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.boku.gbapi4stage.http.client.HttpContext`, which carries:
