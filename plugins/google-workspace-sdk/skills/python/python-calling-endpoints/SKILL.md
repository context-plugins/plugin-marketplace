...
Open `googleworkspace/googleworkspace_client.py` for the controller property names, then the relevant
`googleworkspace/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from googleworkspace.models.{request_model} import {RequestModel}
...
Open `googleworkspace/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from googleworkspace.models.suite_code_enum import SuiteCodeEnum
...
```python
from googleworkspace.http.http_call_back import HttpCallBack
...
client = GoogleWorkspaceClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `googleworkspace/pagination/` in the source.

## Finding the right method

- Controller property names are on `GoogleWorkspaceClient` in `googleworkspace/googleworkspace_client.py`.
- Operation method signatures are in `googleworkspace/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `googleworkspace/models/`; enum types have `from_value()`.
