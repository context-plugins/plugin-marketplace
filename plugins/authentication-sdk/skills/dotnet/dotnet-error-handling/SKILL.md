...
> `{ApiGroup}`, `Authentication`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `Authentication.Core.Exceptions`
- `ApiError` **and** `RawError` → `Authentication.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `Authentication.Errors`
...
```csharp
using Authentication.Core.Exceptions;     // SdkException<TError>
using Authentication.Core.ErrorResponse;  // ApiError, RawError
using Authentication.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`Authentication.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using Authentication.Core.Exceptions;     // SdkException<TError>
using Authentication.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `Authentication.Core.ErrorResponse`. Its public
...
`readonly struct` under `Authentication.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using Authentication.Core.Models;        // ApiResult<TResponse, TError>
using Authentication.Core.ErrorResponse; // RawError
using Authentication.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `Authentication.Core.Exceptions`) — an auth *application* failure, not an API error — when the
