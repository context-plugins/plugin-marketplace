...
> Throughout, `mindbody` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := mindbody.NewClient(
    mindbody.CreateConfiguration(
        mindbody.WithBasicAuthCredentials(
            mindbody.NewBasicAuthCredentials(
...
mindbody.WithApiKeyCredentials(mindbody.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
mindbody.WithApiHeaderCredentials(mindbody.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
mindbody.WithOAuthBearerTokenCredentials(
    mindbody.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := mindbody.NewClient(
    mindbody.CreateConfiguration(
        mindbody.WithOAuthCCGCredentials(
            mindbody.NewOAuthCCGCredentials(
