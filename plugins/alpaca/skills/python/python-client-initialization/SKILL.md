...
> `TraderApiClient` is the SDK's client class — **read the real name** from `alpaca/alpaca_client.py`
...
- `alpaca` — the root package name (e.g. `multiauthsample`, `batester`).
- `TraderApiClient` — the generated client class (read it from the `class …Client` declaration in `alpaca/alpaca_client.py`).
...
```python
from alpaca.alpaca_client import TraderApiClient
from alpaca.configuration import Environment
from alpaca.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from alpaca.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `alpaca/alpaca_client.py` in the cloned source.
...
`from_environment` in `alpaca/configuration.py` for the exact names.
...
Open `alpaca/alpaca_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `alpaca/configuration.py`. **Read the enum in `configuration.py`
...
`alpaca/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
