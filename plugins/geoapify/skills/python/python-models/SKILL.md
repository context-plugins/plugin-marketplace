...
your SDK source (`geoapify/models/`).
...
```python
from geoapify.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `geoapify/utilities/union_type_lookup.py`) and each candidate model
...
```python
from geoapify.models.animal import Cat, Dog
...
```python
from geoapify.models.animal import Animal, Cat, Dog
