...
> `FlightCreateOrdersClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FlightCreateOrdersClient
{
...
```php
use FlightCreateOrdersLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use FlightCreateOrdersLib\Exceptions\{OperationException};
use FlightCreateOrdersLib\Exceptions\ApiException;
...
app()->instance(FlightCreateOrdersClient::class, clientWithResponses([
