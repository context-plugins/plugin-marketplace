...
All types below live under `HotelRatings.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using HotelRatings.Core.Configuration;
...
```csharp
using HotelRatings.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new HotelRatingsClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `HotelRatings.Core.Exceptions`):
...
With DI, the SDK's `AddHotelRatingsClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddHotelRatingsClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
