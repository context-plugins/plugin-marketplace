...
`testing` + `testify/require` for reference; substitute the real `finnhub` and names.
...
import (
    ...
    "github.com/context-plugins/finnhub-go-sdk"   // root package finnhub
)
...
func clientReturning(status int, body string) (finnhub.ClientInterface, *stubTransport) {
    ...
    client := finnhub.NewClient(
        finnhub.CreateConfiguration(
            finnhub.WithHttpConfiguration(
                finnhub.CreateHttpConfiguration(finnhub.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/finnhub-go-sdk/errors"
...
client := finnhub.NewClient(
    finnhub.CreateConfiguration(
        finnhub.WithHttpConfiguration(
            finnhub.CreateHttpConfiguration(finnhub.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
