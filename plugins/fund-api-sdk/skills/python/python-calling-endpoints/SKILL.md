...
Open `fundapi/fundapi_client.py` for the controller property names, then the relevant
`fundapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from fundapi.models.{request_model} import {RequestModel}
...
Open `fundapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from fundapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from fundapi.http.http_call_back import HttpCallBack
...
client = FundAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `fundapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `FundAPIClient` in `fundapi/fundapi_client.py`.
- Operation method signatures are in `fundapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `fundapi/models/`; enum types have `from_value()`.
