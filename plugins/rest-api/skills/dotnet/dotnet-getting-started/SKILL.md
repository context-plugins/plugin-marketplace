...
| --- | --- |
| API | `REST API` |
| Project | `RestApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `RestApi` (the `using` namespace) |
| Client class | `RestApiClient` |
| Options class | `RestApiClientOptions` |
...
namespaces transitively, so `using RestApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/rest-api-csharp-sdk
dotnet add reference rest-api-csharp-sdk/RestApi.csproj
```

```csharp
using RestApi;          // client + options + ServerEnvironment
using RestApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/rest-api-csharp-sdk /tmp/rest-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/rest-api-csharp-sdk "$env:TEMP\rest-api-dotnet-src"
```
...
- `RestApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/rest-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\rest-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `RestApiClient`, build its
   `RestApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
