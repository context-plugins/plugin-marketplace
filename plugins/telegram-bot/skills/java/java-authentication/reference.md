...
against the `TelegramBotAPIClient.Builder` setters, the `org.telegram.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `TelegramBotAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
TelegramBotAPIClient client = new TelegramBotAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeTelegramBotAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new TelegramBotAPIClient.Builder()
