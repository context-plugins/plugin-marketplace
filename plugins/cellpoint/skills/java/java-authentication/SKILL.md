...
To see which schemes a specific SDK accepts, read the credential setter methods on `CellpointClient.Builder`
...
- Set credentials **on the `CellpointClient.Builder`** before calling `.build()` — the client is immutable
