...
client = stub_client(stubs)
assert_raises ShellAPI::APIException do
...
def build_test_client
  ShellAPI::Client.new(
    environment: ShellAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ShellAPI::APIException
```
...
```ruby
expect { call }.to raise_error(ShellAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ShellAPI::HttpCallBack
...
client  = ShellAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
