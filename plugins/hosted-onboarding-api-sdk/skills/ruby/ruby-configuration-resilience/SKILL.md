...
```ruby
client = HostedOnboardingAPI::Client.new(
  max_retries:    3,
...
```ruby
client = HostedOnboardingAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = HostedOnboardingAPI::Client.new(adapter: :net_http)
```
...
client = HostedOnboardingAPI::Client.new(connection: conn)
```
...
client = HostedOnboardingAPI::Client.new(environment: Environment::PRODUCTION)
client = HostedOnboardingAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = HostedOnboardingAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = HostedOnboardingAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = HostedOnboardingAPI::ProxySettings.from_env
client = HostedOnboardingAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < HostedOnboardingAPI::HttpCallBack
...
client = HostedOnboardingAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = HostedOnboardingAPI::Client.from_env
# Overrides take precedence over env vars:
client = HostedOnboardingAPI::Client.from_env(timeout: 30, max_retries: 3)
```
