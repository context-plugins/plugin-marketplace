...
│   └── dev.firecrawl.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── dev.firecrawl.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `dev.firecrawl.api.http.client.HttpContext`, which carries:
