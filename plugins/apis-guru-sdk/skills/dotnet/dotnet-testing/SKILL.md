...
`ApisGuruClient`/`ApisGuruClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `ApisGuruClient`,
...
static ApisGuruClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new ApisGuruClient(new HttpClient(handler), new ApisGuruClientOptions { /* auth not needed for stubs */ });
}
```
...
using ApisGuru.Errors;              // {Operation}Error types
...
```csharp
using ApisGuru.Core.Exceptions;
using ApisGuru.Core.ErrorResponse;
...
using ApisGuru.Core.ErrorResponse; // RawError (Case B)
...
var client = new ApisGuruClient(new HttpClient(handler), new ApisGuruClientOptions());
...
- For DI-based code, the SDK's `AddApisGuruClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `ApisGuruClient` from the provider:
  ```csharp
  services.AddApisGuruClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<ApisGuruClient>();
  ```
