...
- `slackwebapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `SlackWebAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
...
from slackwebapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from slackwebapi.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `slackwebapi/slackwebapi_client.py` in the cloned source.
...
`from_environment` in `slackwebapi/configuration.py` for the exact names.
...
Open `slackwebapi/slackwebapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `slackwebapi/configuration.py`. Use the constant for the desired
...
`slackwebapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
