...
- `AutomationManagementClient` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `automationmanagementclient` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'automationmanagementclient'
include AutomationManagementClient
...
The authoritative list is the `initialize` signature in `lib/automationmanagementclient/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/automationmanagementclient/configuration.rb`
(`build_default_config_from_env`) and `lib/automationmanagementclient/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/automationmanagementclient.rb`) and store it in a
