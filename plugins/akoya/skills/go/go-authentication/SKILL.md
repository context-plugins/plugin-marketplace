...
> Throughout, `akoya` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := akoya.NewClient(
    akoya.CreateConfiguration(
        akoya.WithBasicAuthCredentials(
            akoya.NewBasicAuthCredentials(
...
akoya.WithApiKeyCredentials(akoya.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
akoya.WithApiHeaderCredentials(akoya.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
akoya.WithOAuthBearerTokenCredentials(
    akoya.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
