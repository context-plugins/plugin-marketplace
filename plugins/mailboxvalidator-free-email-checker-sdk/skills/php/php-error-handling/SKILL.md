...
> `{operation}`, `{apiGroup}`, `MailboxValidatorFreeEmailCheckerLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `MailboxValidatorFreeEmailCheckerLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `MailboxValidatorFreeEmailCheckerLib\Exceptions\` — read the source to
