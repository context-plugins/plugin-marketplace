...
Open `pqapiv2/pqapiv2_client.py` for the controller property names, then the relevant
`pqapiv2/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from pqapiv2.models.{request_model} import {RequestModel}
...
Open `pqapiv2/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from pqapiv2.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `pqapiv2/pagination/` in the source.

## Finding the right method

- Controller property names are on `PQAPIV2Client` in `pqapiv2/pqapiv2_client.py`.
- Operation method signatures are in `pqapiv2/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `pqapiv2/models/`; enum types have `from_value()`.
