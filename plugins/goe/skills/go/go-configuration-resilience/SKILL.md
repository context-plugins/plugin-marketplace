    ...
    goe.CreateConfiguration(
        goe.WithHttpConfiguration(
            goe.CreateHttpConfiguration(
                goe.WithTimeout(30),
                goe.WithRetryConfiguration(
                    goe.CreateRetryConfiguration(
                        goe.WithMaxRetryAttempts(3),
                        goe.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := goe.CreateRetryConfiguration(
    goe.WithMaxRetryAttempts(5),
    goe.WithRetryInterval(1),
    goe.WithBackoffFactor(2),
    goe.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    goe.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := goe.NewClient(
    goe.CreateConfiguration(
        goe.WithHttpConfiguration(
            goe.CreateHttpConfiguration(
                goe.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
