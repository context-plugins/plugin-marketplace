...
- `flight-create-orderslib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'flight-create-orderslib';
...
```ts
import { {Resource}Controller } from 'flight-create-orderslib';
