...
- `ebayfinancesapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/ebay-finances-api-go` — the Go import path for the root package; `github.com/context-plugins/ebay-finances-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/ebay-finances-api-go"

client := ebayfinancesapi.NewClient(
    ebayfinancesapi.CreateConfiguration(
        ebayfinancesapi.WithEnvironment(ebayfinancesapi.PRODUCTION),
...
from environment variables named `EBAYFINANCESAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := ebayfinancesapi.NewClient(ebayfinancesapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `ebayfinancesapi.PRODUCTION`,
`ebayfinancesapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := ebayfinancesapi.NewClient(
    ebayfinancesapi.CreateConfiguration(ebayfinancesapi.WithEnvironment(ebayfinancesapi.PRODUCTION)),
)
```
...
client := ebayfinancesapi.NewClient(
    ebayfinancesapi.CreateConfiguration(
        ebayfinancesapi.WithHttpConfiguration(
            ebayfinancesapi.CreateHttpConfiguration(
                ebayfinancesapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                ebayfinancesapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = ebayfinancesapi.NewClient(ebayfinancesapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api ebayfinancesapi.ClientInterface
}

func NewService(api ebayfinancesapi.ClientInterface) *Service {
