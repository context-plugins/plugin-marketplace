...
kwargs to the `MediumAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `mediumapi/mediumapi_client.py` and `mediumapi/http/auth/` in
the cloned source.

## Basic auth

```python
from mediumapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from mediumapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from mediumapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from mediumapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from mediumapi.http.auth.o_auth_acg import OAuthACGCredentials
from mediumapi.models.o_auth_scope_mediumapi_enum import OAuthScope{Pkg}Enum
...
client = MediumAPIClient(o_auth_acg_credentials=creds)
...
```python
from mediumapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = MediumAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `MediumAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `mediumapi/configuration.py` and each `mediumapi/http/auth/*.py` for the exact
