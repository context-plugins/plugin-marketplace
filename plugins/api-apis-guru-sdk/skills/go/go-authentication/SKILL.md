...
> Throughout, `apiapisguru` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := apiapisguru.NewClient(
    apiapisguru.CreateConfiguration(
        apiapisguru.WithBasicAuthCredentials(
            apiapisguru.NewBasicAuthCredentials(
...
apiapisguru.WithApiKeyCredentials(apiapisguru.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
apiapisguru.WithApiHeaderCredentials(apiapisguru.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
apiapisguru.WithOAuthBearerTokenCredentials(
    apiapisguru.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := apiapisguru.NewClient(
    apiapisguru.CreateConfiguration(
        apiapisguru.WithOAuthCCGCredentials(
            apiapisguru.NewOAuthCCGCredentials(
