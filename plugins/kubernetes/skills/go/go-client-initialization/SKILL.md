...
- `kubernetes` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/kubernetes-go-sdk` — the Go import path for the root package; `github.com/context-plugins/kubernetes-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/kubernetes-go-sdk"

client := kubernetes.NewClient(
    kubernetes.CreateConfiguration(
        kubernetes.WithEnvironment(kubernetes.PRODUCTION),
...
from environment variables named `KUBERNETES_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := kubernetes.NewClient(kubernetes.CreateConfigurationFromEnvironment())
```
...
```go
client := kubernetes.NewClient(
    kubernetes.CreateConfiguration(kubernetes.WithEnvironment(kubernetes.PRODUCTION)),
)
```
...
client := kubernetes.NewClient(
    kubernetes.CreateConfiguration(
        kubernetes.WithHttpConfiguration(
            kubernetes.CreateHttpConfiguration(
                kubernetes.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                kubernetes.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = kubernetes.NewClient(kubernetes.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api kubernetes.ClientInterface
}

func NewService(api kubernetes.ClientInterface) *Service {
