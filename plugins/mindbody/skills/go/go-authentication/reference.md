...
```go
mindbody.WithOAuthCCGCredentials(
    mindbody.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr mindbody.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := mindbody.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr mindbody.OAuthCCGManager) models.OAuthToken {
...
```go
creds := mindbody.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := mindbody.NewClient(mindbody.CreateConfiguration(mindbody.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    mindbody.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := mindbody.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := mindbody.NewClient(mindbody.CreateConfiguration(mindbody.WithOAuthROPCGCredentials(creds)))
...
```go
mindbody.WithOAuthBearerTokenCredentials(mindbody.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
mindbody.WithBasicAuthCredentials(mindbody.NewBasicAuthCredentials(username, password))
mindbody.WithApiHeaderCredentials(mindbody.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
mindbody.WithApiKeyCredentials(mindbody.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `MINDBODY_...` variables — e.g.
`MINDBODY_USERNAME`, `MINDBODY_PASSWORD`, `MINDBODY_TOKEN`, `MINDBODY_API_KEY`,
`MINDBODY_O_AUTH_CLIENT_ID`, `MINDBODY_O_AUTH_CLIENT_SECRET`, `MINDBODY_ACCESS_TOKEN`. Grep the
