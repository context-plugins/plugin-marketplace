...
Open `adyenbinlookupapi/adyenbinlookupapi_client.py` for the controller property names, then the relevant
`adyenbinlookupapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from adyenbinlookupapi.models.{request_model} import {RequestModel}
...
Open `adyenbinlookupapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from adyenbinlookupapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from adyenbinlookupapi.http.http_call_back import HttpCallBack
...
client = AdyenBinLookupAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `adyenbinlookupapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `AdyenBinLookupAPIClient` in `adyenbinlookupapi/adyenbinlookupapi_client.py`.
- Operation method signatures are in `adyenbinlookupapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `adyenbinlookupapi/models/`; enum types have `from_value()`.
