...
To see which schemes a specific SDK accepts, read the credential setter methods on `GeoapifyApiClient.Builder`
...
- Set credentials **on the `GeoapifyApiClient.Builder`** before calling `.build()` — the client is immutable
