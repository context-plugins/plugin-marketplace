...
`ThePlaidApiClient`/`ThePlaidApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `ThePlaidApiClient`,
...
static ThePlaidApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new ThePlaidApiClient(new HttpClient(handler), new ThePlaidApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using ThePlaidApi.Errors;              // {Operation}Error types
...
using ThePlaidApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new ThePlaidApiClient(new HttpClient(handler), new ThePlaidApiClientOptions());
...
- For DI-based code, the SDK's `AddThePlaidApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `ThePlaidApiClient` from the provider:
  ```csharp
  services.AddThePlaidApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<ThePlaidApiClient>();
  ```
