...
> Throughout, `telegrambotapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := telegrambotapi.NewClient(
    telegrambotapi.CreateConfiguration(
        telegrambotapi.WithBasicAuthCredentials(
            telegrambotapi.NewBasicAuthCredentials(
...
telegrambotapi.WithApiKeyCredentials(telegrambotapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
telegrambotapi.WithApiHeaderCredentials(telegrambotapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
telegrambotapi.WithOAuthBearerTokenCredentials(
    telegrambotapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := telegrambotapi.NewClient(
    telegrambotapi.CreateConfiguration(
        telegrambotapi.WithOAuthCCGCredentials(
            telegrambotapi.NewOAuthCCGCredentials(
