---
name: dotnet-goe-getting-started
description: GOE .NET SDK identity and lookup layer for the dotnet-goe-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the GOE .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-goe-sdk` helper
...
| --- | --- |
| API | GOE |
| Source repo | https://github.com/context-plugins/goe-csharp-sdk (branch `main`) |
| Root namespace | `Goeapi` (the `using` namespace) |
| Client class | `GoeapiClient` |
| Options class | `GoeapiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `GoeapiClientOptions`: `Jwtbearer: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using Goeapi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/goe-csharp-sdk
dotnet add reference goe-csharp-sdk/Goeapi.csproj
```
...
temp directory** (`<temp>/goe-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/goe-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/goe-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/goe-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\goe-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/goe-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/goe-dotnet-src/`, **never** in the project repo, and its path never goes into
`goe-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `GoeapiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new GoeapiClient(...)`, build its options, or DI-register via
   `AddGoeapiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `GoeapiClientOptions` — the map's *Servers & auth* section
