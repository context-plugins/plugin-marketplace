...
| --- | --- |
| API | `sportsdata` |
| Project | `Sportsdata.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using Sportsdata.Models;` alone does **not** make enums, union types, or
...
```csharp
using Sportsdata;          // client + options + ServerEnvironment
using Sportsdata.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/sportsdata-csharp-sdk /tmp/sportsdata-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/sportsdata-csharp-sdk "$env:TEMP\sportsdata-dotnet-src"
```
...
- `SportsdataClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/sportsdata-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\sportsdata-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `SportsdataClient`, build its
   `SportsdataClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
