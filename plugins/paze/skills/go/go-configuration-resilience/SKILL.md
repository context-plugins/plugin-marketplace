    ...
    paze.CreateConfiguration(
        paze.WithHttpConfiguration(
            paze.CreateHttpConfiguration(
                paze.WithTimeout(30),
                paze.WithRetryConfiguration(
                    paze.CreateRetryConfiguration(
                        paze.WithMaxRetryAttempts(3),
                        paze.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := paze.CreateRetryConfiguration(
    paze.WithMaxRetryAttempts(5),
    paze.WithRetryInterval(1),
    paze.WithBackoffFactor(2),
    paze.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    paze.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := paze.NewClient(
    paze.CreateConfiguration(
        paze.WithHttpConfiguration(
            paze.CreateHttpConfiguration(
                paze.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
