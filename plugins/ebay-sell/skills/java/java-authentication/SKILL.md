...
To see which schemes a specific SDK accepts, read the credential setter methods on `EbaySellClient.Builder`
...
- Set credentials **on the `EbaySellClient.Builder`** before calling `.build()` — the client is immutable
