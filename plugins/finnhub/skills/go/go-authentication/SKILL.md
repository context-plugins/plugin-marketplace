...
> Throughout, `finnhub` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := finnhub.NewClient(
    finnhub.CreateConfiguration(
        finnhub.WithBasicAuthCredentials(
            finnhub.NewBasicAuthCredentials(
...
finnhub.WithApiKeyCredentials(finnhub.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
finnhub.WithApiHeaderCredentials(finnhub.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
finnhub.WithOAuthBearerTokenCredentials(
    finnhub.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := finnhub.NewClient(
    finnhub.CreateConfiguration(
        finnhub.WithOAuthCCGCredentials(
            finnhub.NewOAuthCCGCredentials(
