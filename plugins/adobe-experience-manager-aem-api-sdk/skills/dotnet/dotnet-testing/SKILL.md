...
`AdobeExperienceManagerAemApiClient`/`AdobeExperienceManagerAemApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `AdobeExperienceManagerAemApiClient`,
...
static AdobeExperienceManagerAemApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new AdobeExperienceManagerAemApiClient(new HttpClient(handler), new AdobeExperienceManagerAemApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using AdobeExperienceManagerAemApi.Errors;              // {Operation}Error types
...
```csharp
using AdobeExperienceManagerAemApi.Core.Exceptions;
using AdobeExperienceManagerAemApi.Core.ErrorResponse;
...
using AdobeExperienceManagerAemApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new AdobeExperienceManagerAemApiClient(new HttpClient(handler), new AdobeExperienceManagerAemApiClientOptions());
...
- For DI-based code, the SDK's `AddAdobeExperienceManagerAemApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `AdobeExperienceManagerAemApiClient` from the provider:
  ```csharp
  services.AddAdobeExperienceManagerAemApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<AdobeExperienceManagerAemApiClient>();
  ```
