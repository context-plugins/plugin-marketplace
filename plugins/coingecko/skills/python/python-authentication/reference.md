...
kwarg names are generated per-API — confirm from `coingecko/coingecko_client.py` and `coingecko/http/auth/` in
the cloned source.

## Basic auth

```python
from coingecko.http.auth.basic_auth import BasicAuthCredentials
...
```python
from coingecko.http.auth.api_key import ApiKeyCredentials
...
```python
from coingecko.http.auth.api_header import ApiHeaderCredentials
...
```python
from coingecko.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from coingecko.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from coingecko.http.auth.o_auth_acg import OAuthACGCredentials
from coingecko.models.o_auth_scope_coingecko_enum import OAuthScope{Pkg}Enum
...
```python
from coingecko.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `coingecko/configuration.py` and each `coingecko/http/auth/*.py` for the exact
