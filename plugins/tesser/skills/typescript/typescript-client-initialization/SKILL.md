...
- `tesser-api-v-1-lib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'tesser-api-v-1-lib';
...
```ts
import { {Resource}Controller } from 'tesser-api-v-1-lib';
