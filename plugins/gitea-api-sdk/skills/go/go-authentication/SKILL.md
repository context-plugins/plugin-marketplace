...
> Throughout, `giteaapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := giteaapi.NewClient(
    giteaapi.CreateConfiguration(
        giteaapi.WithBasicAuthCredentials(
            giteaapi.NewBasicAuthCredentials(
...
giteaapi.WithApiKeyCredentials(giteaapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
giteaapi.WithApiHeaderCredentials(giteaapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
giteaapi.WithOAuthBearerTokenCredentials(
    giteaapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := giteaapi.NewClient(
    giteaapi.CreateConfiguration(
        giteaapi.WithOAuthCCGCredentials(
            giteaapi.NewOAuthCCGCredentials(
