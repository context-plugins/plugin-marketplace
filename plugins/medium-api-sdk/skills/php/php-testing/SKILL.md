...
> `MediumAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): MediumAPIClient
{
...
```php
use MediumAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use MediumAPILib\Exceptions\{OperationException};
use MediumAPILib\Exceptions\ApiException;
...
app()->instance(MediumAPIClient::class, clientWithResponses([
