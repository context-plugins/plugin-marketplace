...
`testing` + `testify/require` for reference; substitute the real `firecrawlapi` and names.
...
import (
    ...
    "github.com/context-plugins/firecrawl-api-go-sdk"   // root package firecrawlapi
)
...
func clientReturning(status int, body string) (firecrawlapi.ClientInterface, *stubTransport) {
    ...
    client := firecrawlapi.NewClient(
        firecrawlapi.CreateConfiguration(
            firecrawlapi.WithHttpConfiguration(
                firecrawlapi.CreateHttpConfiguration(firecrawlapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/firecrawl-api-go-sdk/errors"
...
client := firecrawlapi.NewClient(
    firecrawlapi.CreateConfiguration(
        firecrawlapi.WithHttpConfiguration(
            firecrawlapi.CreateHttpConfiguration(firecrawlapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
