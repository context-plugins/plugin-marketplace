...
`flightcreateorders/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `flightcreateorders/exceptions/` and operation docstrings.
...
```python
from flightcreateorders.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `flightcreateorders/exceptions/`.
...
```python
from flightcreateorders.exceptions.api_exception import APIException
from flightcreateorders.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`flightcreateorders/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`flightcreateorders/exceptions/` for the class.
...
`flightcreateorders/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `flightcreateorders/exceptions/`
  or a `ValueError` — check the auth handler in `flightcreateorders/http/auth/` for the exact type.
