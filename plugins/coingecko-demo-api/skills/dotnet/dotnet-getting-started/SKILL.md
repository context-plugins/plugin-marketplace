...
| Project | `CoinGeckoDemoApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using CoinGeckoDemoApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/coingecko-demo-api-csharp-sdk
dotnet add reference coingecko-demo-api-csharp-sdk/CoinGeckoDemoApi.csproj
```

```csharp
using CoinGeckoDemoApi;          // client + options + ServerEnvironment
using CoinGeckoDemoApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/coingecko-demo-api-csharp-sdk /tmp/coingecko-demo-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/coingecko-demo-api-csharp-sdk "$env:TEMP\coingecko-demo-api-dotnet-src"
```
...
- `CoinGeckoDemoApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/coingecko-demo-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\coingecko-demo-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `CoinGeckoDemoApiClient`, build its
   `CoinGeckoDemoApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
