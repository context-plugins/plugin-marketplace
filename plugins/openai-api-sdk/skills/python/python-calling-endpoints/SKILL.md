...
Open `openaiapi/openaiapi_client.py` for the controller property names, then the relevant
`openaiapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from openaiapi.models.{request_model} import {RequestModel}
...
Open `openaiapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from openaiapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from openaiapi.http.http_call_back import HttpCallBack
...
client = OpenAIAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `openaiapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `OpenAIAPIClient` in `openaiapi/openaiapi_client.py`.
- Operation method signatures are in `openaiapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `openaiapi/models/`; enum types have `from_value()`.
