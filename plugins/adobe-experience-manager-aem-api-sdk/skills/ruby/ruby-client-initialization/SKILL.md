...
- `AdobeExperienceManagerAEMAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `adobe_experience_manager_aem_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'adobe_experience_manager_aem_api'
include AdobeExperienceManagerAEMAPI
...
The authoritative list is the `initialize` signature in `lib/adobe_experience_manager_aem_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/adobe_experience_manager_aem_api/configuration.rb`
(`build_default_config_from_env`) and `lib/adobe_experience_manager_aem_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/adobe_experience_manager_aem_api.rb`) and store it in a
