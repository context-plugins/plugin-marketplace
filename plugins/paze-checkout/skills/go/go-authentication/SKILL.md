...
> Throughout, `pazecheckoutapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := pazecheckoutapi.NewClient(
    pazecheckoutapi.CreateConfiguration(
        pazecheckoutapi.WithBasicAuthCredentials(
            pazecheckoutapi.NewBasicAuthCredentials(
...
pazecheckoutapi.WithApiKeyCredentials(pazecheckoutapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
pazecheckoutapi.WithApiHeaderCredentials(pazecheckoutapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
pazecheckoutapi.WithOAuthBearerTokenCredentials(
    pazecheckoutapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := pazecheckoutapi.NewClient(
    pazecheckoutapi.CreateConfiguration(
        pazecheckoutapi.WithOAuthCCGCredentials(
            pazecheckoutapi.NewOAuthCCGCredentials(
