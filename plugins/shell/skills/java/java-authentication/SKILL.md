...
To see which schemes a specific SDK accepts, read the credential setter methods on `ShellAPIClient.Builder`
...
- Set credentials **on the `ShellAPIClient.Builder`** before calling `.build()` — the client is immutable
