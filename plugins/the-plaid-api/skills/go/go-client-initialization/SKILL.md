...
- `theplaidapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/the-plaid-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/the-plaid-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/the-plaid-api-go-sdk"

client := theplaidapi.NewClient(
    theplaidapi.CreateConfiguration(
        theplaidapi.WithEnvironment(theplaidapi.PRODUCTION),
...
from environment variables named `THEPLAIDAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := theplaidapi.NewClient(theplaidapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `theplaidapi.PRODUCTION`,
`theplaidapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := theplaidapi.NewClient(
    theplaidapi.CreateConfiguration(theplaidapi.WithEnvironment(theplaidapi.PRODUCTION)),
)
```
...
client := theplaidapi.NewClient(
    theplaidapi.CreateConfiguration(
        theplaidapi.WithHttpConfiguration(
            theplaidapi.CreateHttpConfiguration(
                theplaidapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                theplaidapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = theplaidapi.NewClient(theplaidapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api theplaidapi.ClientInterface
}

func NewService(api theplaidapi.ClientInterface) *Service {
