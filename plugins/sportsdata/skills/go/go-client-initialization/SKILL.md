...
- `sportsdata` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/sportsdata-go-sdk` — the Go import path for the root package; `github.com/context-plugins/sportsdata-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/sportsdata-go-sdk"

client := sportsdata.NewClient(
    sportsdata.CreateConfiguration(
        sportsdata.WithEnvironment(sportsdata.PRODUCTION),
...
from environment variables named `SPORTSDATA_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := sportsdata.NewClient(sportsdata.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `sportsdata.PRODUCTION`,
`sportsdata.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := sportsdata.NewClient(
    sportsdata.CreateConfiguration(sportsdata.WithEnvironment(sportsdata.PRODUCTION)),
)
```
...
client := sportsdata.NewClient(
    sportsdata.CreateConfiguration(
        sportsdata.WithHttpConfiguration(
            sportsdata.CreateHttpConfiguration(
                sportsdata.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                sportsdata.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = sportsdata.NewClient(sportsdata.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api sportsdata.ClientInterface
}

func NewService(api sportsdata.ClientInterface) *Service {
