...
MultiAuthSampleClient client = new MultiAuthSampleClient.Builder()
    .environment(Environment.TESTING)   // switch environments
...
```java
// Confirmed from MultiAuth-Sample: MultiAuthSampleClient → getHttpClientConfig()
...
Confirmed from `MultiAuthSampleClient.shutdown()` in the generated source.
