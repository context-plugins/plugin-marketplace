...
| --- | --- |
| API | `Events API` |
| Project | `EventsApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `EventsApi` (the `using` namespace) |
| Client class | `EventsApiClient` |
| Options class | `EventsApiClientOptions` |
...
namespaces transitively, so `using EventsApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/events-api-csharp
dotnet add reference events-api-csharp/EventsApi.csproj
```

```csharp
using EventsApi;          // client + options + ServerEnvironment
using EventsApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/events-api-csharp /tmp/events-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/events-api-csharp "$env:TEMP\events-api-dotnet-src"
```
...
- `EventsApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/events-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\events-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `EventsApiClient`, build its
   `EventsApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
