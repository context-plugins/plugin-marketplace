...
Open `newstore/newstore_client.py` for the controller property names, then the relevant
`newstore/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from newstore.models.{request_model} import {RequestModel}
...
Open `newstore/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from newstore.models.suite_code_enum import SuiteCodeEnum
...
```python
from newstore.http.http_call_back import HttpCallBack
...
client = NewstoreapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `newstore/pagination/` in the source.

## Finding the right method

- Controller property names are on `NewstoreapiClient` in `newstore/newstore_client.py`.
- Operation method signatures are in `newstore/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `newstore/models/`; enum types have `from_value()`.
