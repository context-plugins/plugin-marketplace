...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new AuthentiqAPIClient({
...
```typescript
import { {Operation}Error } from 'authentiq-apilib/errors';
...
```typescript
import { ApiError } from 'authentiq-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AuthentiqAPIClient)
    .useValue(new AuthentiqAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
