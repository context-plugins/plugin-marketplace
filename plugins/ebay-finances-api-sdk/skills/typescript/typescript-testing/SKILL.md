...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new EBayFinancesAPIClient({
...
```typescript
import { {Operation}Error } from 'ebay-finances-apilib/errors';
...
```typescript
import { ApiError } from 'ebay-finances-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(EBayFinancesAPIClient)
    .useValue(new EBayFinancesAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
