...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the FlightOrderManagementClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write FlightOrderManagementClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `flightordermanagement` — the root package name (e.g. `multiauthsample`, `batester`).
- `FlightOrderManagementClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `FlightOrderManagementClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from flightordermanagement.flightordermanagement_client import FlightOrderManagementClient
from flightordermanagement.configuration import Environment
from flightordermanagement.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = FlightOrderManagementClient(
...
```python
from flightordermanagement.configuration import Configuration, Environment
...
client = FlightOrderManagementClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `flightordermanagement/flightordermanagement_client.py` in the cloned source.
...
`from_environment()` is a class method on both `FlightOrderManagementClient` and `Configuration` that reads
...
client = FlightOrderManagementClient.from_environment()
...
`from_environment` in `flightordermanagement/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `FlightOrderManagementClient`. Access them as attributes —
...
Open `flightordermanagement/flightordermanagement_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `flightordermanagement/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from flightordermanagement.configuration import Environment

client = FlightOrderManagementClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`flightordermanagement/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = FlightOrderManagementClient(config=new_config)
```
