...
`testing` + `testify/require` for reference; substitute the real `shellapi` and names.
...
import (
    ...
    "github.com/context-plugins/shell-api-go"   // root package shellapi
)
...
func clientReturning(status int, body string) (shellapi.ClientInterface, *stubTransport) {
    ...
    client := shellapi.NewClient(
        shellapi.CreateConfiguration(
            shellapi.WithHttpConfiguration(
                shellapi.CreateHttpConfiguration(shellapi.WithTransport(stub)),
            ),
...
client := shellapi.NewClient(
    shellapi.CreateConfiguration(
        shellapi.WithHttpConfiguration(
            shellapi.CreateHttpConfiguration(shellapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
