...
`testing` + `testify/require` for reference; substitute the real `mediumapi` and names.
...
import (
    ...
    "github.com/context-plugins/medium-api-go"   // root package mediumapi
)
...
func clientReturning(status int, body string) (mediumapi.ClientInterface, *stubTransport) {
    ...
    client := mediumapi.NewClient(
        mediumapi.CreateConfiguration(
            mediumapi.WithHttpConfiguration(
                mediumapi.CreateHttpConfiguration(mediumapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/medium-api-go/errors"
...
client := mediumapi.NewClient(
    mediumapi.CreateConfiguration(
        mediumapi.WithHttpConfiguration(
            mediumapi.CreateHttpConfiguration(mediumapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
