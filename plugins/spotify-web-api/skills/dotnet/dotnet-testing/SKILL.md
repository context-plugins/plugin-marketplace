...
`SpotifyWebApiClient`/`SpotifyWebApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `SpotifyWebApiClient`,
...
static SpotifyWebApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new SpotifyWebApiClient(new HttpClient(handler), new SpotifyWebApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using SpotifyWebApi.Errors;              // {Operation}Error types
...
using SpotifyWebApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new SpotifyWebApiClient(new HttpClient(handler), new SpotifyWebApiClientOptions());
...
- For DI-based code, the SDK's `AddSpotifyWebApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `SpotifyWebApiClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<SpotifyWebApiClient>();
  ```
