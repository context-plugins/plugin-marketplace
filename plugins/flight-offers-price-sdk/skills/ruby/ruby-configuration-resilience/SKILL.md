...
```ruby
client = FlightOffersPrice::Client.new(
  max_retries:    3,
...
```ruby
client = FlightOffersPrice::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = FlightOffersPrice::Client.new(adapter: :net_http)
```
...
client = FlightOffersPrice::Client.new(connection: conn)
```
...
client = FlightOffersPrice::Client.new(environment: Environment::PRODUCTION)
client = FlightOffersPrice::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = FlightOffersPrice::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = FlightOffersPrice::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = FlightOffersPrice::ProxySettings.from_env
client = FlightOffersPrice::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < FlightOffersPrice::HttpCallBack
...
client = FlightOffersPrice::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = FlightOffersPrice::Client.from_env
# Overrides take precedence over env vars:
client = FlightOffersPrice::Client.from_env(timeout: 30, max_retries: 3)
```
