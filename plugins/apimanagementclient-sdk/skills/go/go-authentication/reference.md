...
```go
apimanagementclient.WithOAuthCCGCredentials(
    apimanagementclient.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr apimanagementclient.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := apimanagementclient.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr apimanagementclient.OAuthCCGManager) models.OAuthToken {
...
```go
creds := apimanagementclient.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := apimanagementclient.NewClient(apimanagementclient.CreateConfiguration(apimanagementclient.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    apimanagementclient.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := apimanagementclient.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := apimanagementclient.NewClient(apimanagementclient.CreateConfiguration(apimanagementclient.WithOAuthROPCGCredentials(creds)))
...
```go
apimanagementclient.WithOAuthBearerTokenCredentials(apimanagementclient.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
apimanagementclient.WithBasicAuthCredentials(apimanagementclient.NewBasicAuthCredentials(username, password))
apimanagementclient.WithApiHeaderCredentials(apimanagementclient.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
apimanagementclient.WithApiKeyCredentials(apimanagementclient.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `APIMANAGEMENTCLIENT_...` variables — e.g.
`APIMANAGEMENTCLIENT_USERNAME`, `APIMANAGEMENTCLIENT_PASSWORD`, `APIMANAGEMENTCLIENT_TOKEN`, `APIMANAGEMENTCLIENT_API_KEY`,
`APIMANAGEMENTCLIENT_O_AUTH_CLIENT_ID`, `APIMANAGEMENTCLIENT_O_AUTH_CLIENT_SECRET`, `APIMANAGEMENTCLIENT_ACCESS_TOKEN`. Grep the
