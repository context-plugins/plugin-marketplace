---
name: "typescript-coin-gecko-getting-started"
description: "CoinGecko TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `coin-gecko`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any CoinGecko TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the CoinGecko TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing CoinGecko TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-coin-gecko` for the workflow that wraps this skill.
...
Verified against `package.json` and `sdk-map.md` of the generated package at version `3.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | CoinGecko |
| Package name (what you install, and what you import) | `coin-gecko` — **not on npm**; built from source (see *Install*) |
| Import specifier | `coin-gecko` — the package root is the **only** entry; deep imports do not resolve |
| Version | `3.0.0` (API spec version `3.0.0`) |
| Client class | `CoinGeckoClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new CoinGeckoClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new CoinGeckoClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `headerAuth` · `queryAuth`. `timeout` defaults to `60_000` ms |
| Auth | **API key (header)** — set `ClientOptions.headerAuth`<br>**API key (query)** — set `ClientOptions.queryAuth` |
| Environments | 1 environment (`ServerEnvironment.Production` *(default)*) × 1 server group |
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://api.coingecko.com/api/v3` |
...
| Surface | 61 operations · 11 resources · 192 models · 28 open enums · 3 unions · 0 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/coingecko-typescript-sdk#main
```

That fetches everything the package's `files` list packs — `src/`, `sdk-map.md` and the pages under `map/operations/` — so **every lookup on this page works as soon as the install finishes**. What it does *not* do is build `dist/`: the generated manifest declares no `prepare` script, and the `exports` map points into `dist/`, so the first `import` fails until something builds it. Run the SDK's own build once inside the installed package (`npm install && npm run build` in `node_modules/coin-gecko/`), or clone the repository, build there, and depend on that directory by path instead.
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 223 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { CoinGeckoClient, ServerEnvironment, ResponseError, CoinGeckoError } from "coin-gecko";
import type { ClientOptions, AssetPlatform } from "coin-gecko";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `coin-gecko/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **From CommonJS**, the typed spelling is `import sdk = require("coin-gecko")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://api.coingecko.com/api/v3` | `serverOptions.default.production.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 61 of the 61 operations require a credential and 0 are public.

| `ClientOptions` field | Scheme kind | What the SDK sends |
| --- | --- | --- |
| `headerAuth` | API key (header) | header `x-cg-demo-api-key: <key>` |
| `queryAuth` | API key (query) | query `x_cg_demo_api_key=<key>` |

```ts
const client = new CoinGeckoClient({
  headerAuth: process.env.API_KEY!,
  queryAuth: process.env.API_KEY!,
});
```
...
| --- | --- | --- | --- |
| `client.coins` | `Coins` | 13 | `coinsCategories` · `coinsCategoriesList` · `coinsContractAddress` · `coinsId` · `coinsIdHistory` · `coinsIdMarketChart` · `coinsIdMarketChartRange` · `coinsIdOhlc` · `coinsIdTickers` · `coinsList` · `coinsMarkets` · `contractAddressMarketChart` · `contractAddressMarketChartRange` |
| `client.derivatives` | `Derivatives` | 4 | `derivativesExchanges` · `derivativesExchangesId` · `derivativesExchangesList` · `derivativesTickers` |
| `client.entities` | `Entities` | 2 | `companiesPublicTreasury` · `entitiesList` |
| `client.exchanges` | `Exchanges` | 6 | `exchangeRates` · `exchanges` · `exchangesId` · `exchangesIdTickers` · `exchangesIdVolumeChart` · `exchangesList` |
| `client.globalApi` | `GlobalApi` | 2 | `cryptoGlobal` · `globalDefi` |
| `client.misc` | `Misc` | 3 | `assetPlatformsList` · `pingServer` · `tokenLists` |
| `client.nfts` | `Nfts` | 3 | `nftsContractAddress` · `nftsId` · `nftsList` |
| `client.onchain` | `Onchain` | 20 | `dexesList` · `latestPoolsList` · `latestPoolsNetwork` · `networksList` · `onchainSimplePrice` · `poolAddress` · `poolOhlcvContractAddress` · `poolTokenInfoContractAddress` · `poolTradesContractAddress` · `poolsAddresses` · `searchPools` · `tokenDataContractAddress` · `tokenInfoContractAddress` · `tokensDataContractAddresses` · `tokensInfoRecentUpdated` · `topPoolsContractAddress` · `topPoolsDex` · `topPoolsNetwork` · `trendingPoolsList` · `trendingPoolsNetwork` |
| `client.publicTreasuryApi` | `PublicTreasuryApi` | 3 | `publicTreasuryEntity` · `publicTreasuryEntityChart` · `publicTreasuryTransactionHistory` |
| `client.searchApi` | `SearchApi` | 2 | `searchData` · `trendingSearch` |
| `client.simple` | `Simple` | 3 | `simplePrice` · `simpleSupportedCurrencies` · `simpleTokenPrice` |
...
```bash
node -e "console.log(require.resolve('coin-gecko/package.json'))"
```

Failing that it is at `node_modules/coin-gecko/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/coin-gecko/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/coin-gecko/`:
...
Beyond the usual signatures and model members, a contract sheet for the CoinGecko TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
