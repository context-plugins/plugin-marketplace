...
To see which schemes a specific SDK accepts, read the credential setter methods on `AdyenAPIClient.Builder`
...
- Set credentials **on the `AdyenAPIClient.Builder`** before calling `.build()` — the client is immutable
