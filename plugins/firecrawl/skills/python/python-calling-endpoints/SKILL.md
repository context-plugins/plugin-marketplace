...
Open `firecrawl/firecrawl_client.py` for the controller property names, then the relevant
`firecrawl/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from firecrawl.models.{request_model} import {RequestModel}
...
Open `firecrawl/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
an iterator over the items on that page. Inspect `firecrawl/pagination/` in the source.
