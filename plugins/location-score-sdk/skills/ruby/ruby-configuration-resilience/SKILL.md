...
```ruby
client = LocationScore::Client.new(
  max_retries:    3,
...
```ruby
client = LocationScore::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = LocationScore::Client.new(adapter: :net_http)
```
...
client = LocationScore::Client.new(connection: conn)
```
...
client = LocationScore::Client.new(environment: Environment::PRODUCTION)
client = LocationScore::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = LocationScore::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = LocationScore::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = LocationScore::ProxySettings.from_env
client = LocationScore::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < LocationScore::HttpCallBack
...
client = LocationScore::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = LocationScore::Client.from_env
# Overrides take precedence over env vars:
client = LocationScore::Client.from_env(timeout: 30, max_retries: 3)
```
