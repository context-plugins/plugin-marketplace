...
> `{operation}`, `{apiGroup}`, `FlightDelayPredictionLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `FlightDelayPredictionLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use FlightDelayPredictionLib\Exceptions\ApiException;
...
```php
use FlightDelayPredictionLib\Exceptions\ApiException;
use FlightDelayPredictionLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `FlightDelayPredictionLib\Exceptions\` — read the source to
