...
def stub_client(stubs)
  ...
  TravelRecommendationsAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises TravelRecommendationsAPI::APIException do
...
def build_test_client
  TravelRecommendationsAPI::Client.new(
    environment: TravelRecommendationsAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error TravelRecommendationsAPI::APIException
```
...
```ruby
expect { call }.to raise_error(TravelRecommendationsAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < TravelRecommendationsAPI::HttpCallBack
...
client  = TravelRecommendationsAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
