...
<!-- crawler:auth -->
| Auth | Credentials properties on `TeslaFleetManagementApiClientOptions`: `BearerAuth: string?`, `ThirdpartytokenAuthorizationCode: OAuth2AuthorizationCodeCredentials?`, `ThirdpartytokenAuthorizationCodeTokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?`, `ThirdpartytokenClientCredentials: OAuth2ClientCredentials?`, `ThirdpartytokenClientCredentialsTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
git clone --branch main https://github.com/context-plugins/tesla-csharp-sdk
dotnet add reference tesla-csharp-sdk/TeslaFleetManagementApi.csproj
```
...
temp directory** (`<temp>/tesla-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/tesla-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/tesla-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/tesla-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\tesla-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/tesla-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/tesla-dotnet-src/`, **never** in the project repo, and its path never goes into
`tesla-dotnet-plan.md` — the main agent must never see the clone or its path.
