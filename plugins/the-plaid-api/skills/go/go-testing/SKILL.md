...
`testing` + `testify/require` for reference; substitute the real `theplaidapi` and names.
...
import (
    ...
    "github.com/context-plugins/the-plaid-api-go-sdk"   // root package theplaidapi
)
...
func clientReturning(status int, body string) (theplaidapi.ClientInterface, *stubTransport) {
    ...
    client := theplaidapi.NewClient(
        theplaidapi.CreateConfiguration(
            theplaidapi.WithHttpConfiguration(
                theplaidapi.CreateHttpConfiguration(theplaidapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/the-plaid-api-go-sdk/errors"
...
client := theplaidapi.NewClient(
    theplaidapi.CreateConfiguration(
        theplaidapi.WithHttpConfiguration(
            theplaidapi.CreateHttpConfiguration(theplaidapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
