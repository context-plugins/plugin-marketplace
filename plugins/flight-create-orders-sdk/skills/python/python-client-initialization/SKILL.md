...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the FlightCreateOrdersClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write FlightCreateOrdersClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `flightcreateorders` — the root package name (e.g. `multiauthsample`, `batester`).
- `FlightCreateOrdersClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `FlightCreateOrdersClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from flightcreateorders.flightcreateorders_client import FlightCreateOrdersClient
from flightcreateorders.configuration import Environment
from flightcreateorders.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = FlightCreateOrdersClient(
...
```python
from flightcreateorders.configuration import Configuration, Environment
...
client = FlightCreateOrdersClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `flightcreateorders/flightcreateorders_client.py` in the cloned source.
...
`from_environment()` is a class method on both `FlightCreateOrdersClient` and `Configuration` that reads
...
client = FlightCreateOrdersClient.from_environment()
...
`from_environment` in `flightcreateorders/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `FlightCreateOrdersClient`. Access them as attributes —
...
Open `flightcreateorders/flightcreateorders_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `flightcreateorders/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from flightcreateorders.configuration import Environment

client = FlightCreateOrdersClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`flightcreateorders/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = FlightCreateOrdersClient(config=new_config)
```
