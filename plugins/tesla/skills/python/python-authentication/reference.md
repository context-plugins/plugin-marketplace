...
kwargs to the `TeslaClient` constructor (or to `Configuration`). Exact credential class names and
...
client = TeslaClient(o_auth_acg_credentials=creds)
...
client = TeslaClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `TeslaClient.from_environment()`) read each credential
