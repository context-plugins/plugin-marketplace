...
> Throughout, `splitit` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := splitit.NewClient(
    splitit.CreateConfiguration(
        splitit.WithBasicAuthCredentials(
            splitit.NewBasicAuthCredentials(
...
splitit.WithApiKeyCredentials(splitit.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
splitit.WithApiHeaderCredentials(splitit.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
splitit.WithOAuthBearerTokenCredentials(
    splitit.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := splitit.NewClient(
    splitit.CreateConfiguration(
        splitit.WithOAuthCCGCredentials(
            splitit.NewOAuthCCGCredentials(
