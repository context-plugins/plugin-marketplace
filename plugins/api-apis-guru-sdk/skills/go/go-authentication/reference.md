...
```go
apiapisguru.WithOAuthCCGCredentials(
    apiapisguru.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr apiapisguru.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := apiapisguru.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr apiapisguru.OAuthCCGManager) models.OAuthToken {
...
```go
creds := apiapisguru.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := apiapisguru.NewClient(apiapisguru.CreateConfiguration(apiapisguru.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    apiapisguru.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := apiapisguru.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := apiapisguru.NewClient(apiapisguru.CreateConfiguration(apiapisguru.WithOAuthROPCGCredentials(creds)))
...
```go
apiapisguru.WithOAuthBearerTokenCredentials(apiapisguru.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
apiapisguru.WithBasicAuthCredentials(apiapisguru.NewBasicAuthCredentials(username, password))
apiapisguru.WithApiHeaderCredentials(apiapisguru.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
apiapisguru.WithApiKeyCredentials(apiapisguru.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `APIAPISGURU_...` variables — e.g.
`APIAPISGURU_USERNAME`, `APIAPISGURU_PASSWORD`, `APIAPISGURU_TOKEN`, `APIAPISGURU_API_KEY`,
`APIAPISGURU_O_AUTH_CLIENT_ID`, `APIAPISGURU_O_AUTH_CLIENT_SECRET`, `APIAPISGURU_ACCESS_TOKEN`. Grep the
