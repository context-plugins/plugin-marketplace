...
Open `lib/shell_ev/client.rb` for the accessor names, then the relevant
`lib/shell_ev/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = ShellEv::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: ShellEv::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/shell_ev/models/`. Field names are defined as
...
Read the enum module in `lib/shell_ev/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/shell_ev/client.rb`.
- Operation methods are in `lib/shell_ev/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/shell_ev/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/shell_ev/exceptions/`.
