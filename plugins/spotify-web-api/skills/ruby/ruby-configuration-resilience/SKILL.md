...
```ruby
client = SpotifyWebAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = SpotifyWebAPI::Client.new(adapter: :net_http)
```
...
client = SpotifyWebAPI::Client.new(connection: conn)
```
...
client = SpotifyWebAPI::Client.new(environment: Environment::PRODUCTION)
client = SpotifyWebAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = SpotifyWebAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = SpotifyWebAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = SpotifyWebAPI::ProxySettings.from_env
client = SpotifyWebAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < SpotifyWebAPI::HttpCallBack
...
client = SpotifyWebAPI::Client.new(http_callback: LogCallback.new)
```
...
client = SpotifyWebAPI::Client.from_env(timeout: 30, max_retries: 3)
```
