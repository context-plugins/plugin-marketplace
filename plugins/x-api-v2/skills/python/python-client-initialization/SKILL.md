...
- `xapiv2` — the root package name (e.g. `multiauthsample`, `batester`).
- `Xapiv2Client` — the generated client class (read it from the `class …Client` declaration in `xapiv2/xapiv2_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `Xapiv2Client` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from xapiv2.xapiv2_client import Xapiv2Client
from xapiv2.configuration import Environment
from xapiv2.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from xapiv2.configuration import Configuration, Environment
...
client = Xapiv2Client(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `xapiv2/xapiv2_client.py` in the cloned source.
...
`from_environment()` is a class method on both `Xapiv2Client` and `Configuration` that reads
...
client = Xapiv2Client.from_environment()
...
`from_environment` in `xapiv2/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `Xapiv2Client`. Access them as attributes —
...
Open `xapiv2/xapiv2_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `xapiv2/configuration.py`. **Read the enum in `configuration.py`
...
client = Xapiv2Client(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`xapiv2/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = Xapiv2Client(config=new_config)
```
