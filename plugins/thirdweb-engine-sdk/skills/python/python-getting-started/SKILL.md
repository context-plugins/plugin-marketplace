...
| Install | `pip install "git+https://github.com/context-plugins/thirdweb-engine-python@main"` |
| Client class | `ThirdwebEngineClient` in `thirdwebengine/thirdwebengine_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `thirdwebengine/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `ThirdwebEngineClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `thirdwebengine/exceptions/api_exception.py` — raised on non-2xx |
...
- `thirdwebengine/thirdwebengine_client.py` — the `ThirdwebEngineClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `thirdwebengine/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `thirdwebengine/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `thirdwebengine/controllers/` — one `{Resource}Controller` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `thirdwebengine/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `thirdwebengine/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `thirdwebengine/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `thirdwebengine/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `thirdwebengine/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/thirdweb-engine-python@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/thirdweb-engine-python /tmp/thirdweb-engine-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/thirdweb-engine-python "$env:TEMP\thirdweb-engine-python-src"
```
...
- `thirdwebengine/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `thirdwebengine/thirdwebengine_client.py` — `ThirdwebEngineClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `thirdwebengine/controllers/*.py` — operation method signatures, parameter names, and return types.
- `thirdwebengine/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `thirdwebengine/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/thirdweb-engine-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\thirdweb-engine-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `ThirdwebEngineClient(...)`.
