...
reference; substitute your `shellcardmanagement` and `ShellcardmanagementapisClient` names.
...
- **Get the stub URL from the SDK, not from this page.** Every environment resolves to a real remote
  host — read the `environments` dict in `shellcardmanagement/configuration.py`, or call
