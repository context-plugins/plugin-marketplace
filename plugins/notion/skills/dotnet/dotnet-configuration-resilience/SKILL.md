...
All types below live under `Notion.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using Notion.Core.Configuration;
...
```csharp
using Notion.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new NotionClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `Notion.Core.Exceptions`):
...
With DI, the SDK's `AddNotionClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddNotionClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
