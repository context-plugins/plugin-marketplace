...
```go
hostedonboardingapi.WithOAuthCCGCredentials(
    hostedonboardingapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr hostedonboardingapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := hostedonboardingapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr hostedonboardingapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := hostedonboardingapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := hostedonboardingapi.NewClient(hostedonboardingapi.CreateConfiguration(hostedonboardingapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    hostedonboardingapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := hostedonboardingapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := hostedonboardingapi.NewClient(hostedonboardingapi.CreateConfiguration(hostedonboardingapi.WithOAuthROPCGCredentials(creds)))
...
```go
hostedonboardingapi.WithOAuthBearerTokenCredentials(hostedonboardingapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
hostedonboardingapi.WithBasicAuthCredentials(hostedonboardingapi.NewBasicAuthCredentials(username, password))
hostedonboardingapi.WithApiHeaderCredentials(hostedonboardingapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
hostedonboardingapi.WithApiKeyCredentials(hostedonboardingapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `HOSTEDONBOARDINGAPI_...` variables — e.g.
`HOSTEDONBOARDINGAPI_USERNAME`, `HOSTEDONBOARDINGAPI_PASSWORD`, `HOSTEDONBOARDINGAPI_TOKEN`, `HOSTEDONBOARDINGAPI_API_KEY`,
`HOSTEDONBOARDINGAPI_O_AUTH_CLIENT_ID`, `HOSTEDONBOARDINGAPI_O_AUTH_CLIENT_SECRET`, `HOSTEDONBOARDINGAPI_ACCESS_TOKEN`. Grep the
