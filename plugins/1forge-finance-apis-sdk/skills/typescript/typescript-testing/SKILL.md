...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new 1ForgeFinanceAPIsClient({
...
```typescript
import { {Operation}Error } from 'm1-forge-finance-apislib/errors';
...
```typescript
import { ApiError } from 'm1-forge-finance-apislib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(1ForgeFinanceAPIsClient)
    .useValue(new 1ForgeFinanceAPIsClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
