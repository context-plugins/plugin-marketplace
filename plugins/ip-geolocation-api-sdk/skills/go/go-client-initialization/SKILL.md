...
- `ipgeolocationapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/ip-geolocation-api-go` — the Go import path for the root package; `github.com/context-plugins/ip-geolocation-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/ip-geolocation-api-go"

client := ipgeolocationapi.NewClient(
    ipgeolocationapi.CreateConfiguration(
        ipgeolocationapi.WithEnvironment(ipgeolocationapi.PRODUCTION),
...
from environment variables named `IPGEOLOCATIONAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := ipgeolocationapi.NewClient(ipgeolocationapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `ipgeolocationapi.PRODUCTION`,
`ipgeolocationapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := ipgeolocationapi.NewClient(
    ipgeolocationapi.CreateConfiguration(ipgeolocationapi.WithEnvironment(ipgeolocationapi.PRODUCTION)),
)
```
...
client := ipgeolocationapi.NewClient(
    ipgeolocationapi.CreateConfiguration(
        ipgeolocationapi.WithHttpConfiguration(
            ipgeolocationapi.CreateHttpConfiguration(
                ipgeolocationapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                ipgeolocationapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = ipgeolocationapi.NewClient(ipgeolocationapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api ipgeolocationapi.ClientInterface
}

func NewService(api ipgeolocationapi.ClientInterface) *Service {
