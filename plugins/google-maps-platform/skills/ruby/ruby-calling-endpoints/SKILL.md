...
Open `lib/google_maps_platform/client.rb` for the accessor names, then the relevant
`lib/google_maps_platform/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = GoogleMapsPlatform::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: GoogleMapsPlatform::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/google_maps_platform/models/`. Field names are defined as
...
Read the enum module in `lib/google_maps_platform/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/google_maps_platform/client.rb`.
- Operation methods are in `lib/google_maps_platform/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/google_maps_platform/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/google_maps_platform/exceptions/`.
