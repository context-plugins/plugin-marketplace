...
Integrate and consume the **APIMatic-generated Square SDK** (the "APIMATIC v3.0" generic
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, TypeScript, Java, PHP, Ruby, Go.
...
(`dotnet-square-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-square`** router skill routes all
C#/.NET SDK work to the single **`dotnet-square-sdk`** agent (.NET-only), which answers
