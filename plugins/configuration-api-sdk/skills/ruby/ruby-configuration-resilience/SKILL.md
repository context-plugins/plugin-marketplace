...
```ruby
client = ConfigurationAPI::Client.new(
  max_retries:    3,
...
```ruby
client = ConfigurationAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ConfigurationAPI::Client.new(adapter: :net_http)
```
...
client = ConfigurationAPI::Client.new(connection: conn)
```
...
client = ConfigurationAPI::Client.new(environment: Environment::PRODUCTION)
client = ConfigurationAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ConfigurationAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ConfigurationAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ConfigurationAPI::ProxySettings.from_env
client = ConfigurationAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ConfigurationAPI::HttpCallBack
...
client = ConfigurationAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ConfigurationAPI::Client.from_env
# Overrides take precedence over env vars:
client = ConfigurationAPI::Client.from_env(timeout: 30, max_retries: 3)
```
