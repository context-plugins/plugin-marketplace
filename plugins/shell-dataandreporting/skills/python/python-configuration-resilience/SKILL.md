...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ShelldatareportingapisClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`shelldataandreporting/configuration.py` in the cloned source.
...
```python
import requests
from shelldataandreporting.configuration import Configuration
...
`shelldataandreporting/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`shelldataandreporting/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `shelldataandreporting/configuration.py`
...
`NumberPagedResponse`) live in `shelldataandreporting/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `shelldataandreporting/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
