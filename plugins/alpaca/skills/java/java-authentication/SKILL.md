...
To see which schemes a specific SDK accepts, read the credential setter methods on `TraderApiClient.Builder`
...
- Set credentials **on the `TraderApiClient.Builder`** before calling `.build()` — the client is immutable
