...
> `ShellAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ShellAPIClient
{
...
```php
use ShellAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use ShellAPILib\Exceptions\{OperationException};
use ShellAPILib\Exceptions\ApiException;
...
app()->instance(ShellAPIClient::class, clientWithResponses([
