...
`postnl/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `postnl/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `postnl/exceptions/`.
...
```python
from postnl.exceptions.api_exception import APIException
from postnl.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`postnl/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`postnl/exceptions/` for the class.
...
`postnl/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `postnl/exceptions/`
  or a `ValueError` — check the auth handler in `postnl/http/auth/` for the exact type.
