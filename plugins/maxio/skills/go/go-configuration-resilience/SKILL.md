    ...
    maxio.CreateConfiguration(
        maxio.WithHttpConfiguration(
            maxio.CreateHttpConfiguration(
                maxio.WithTimeout(30),
                maxio.WithRetryConfiguration(
                    maxio.CreateRetryConfiguration(
                        maxio.WithMaxRetryAttempts(3),
                        maxio.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := maxio.CreateRetryConfiguration(
    maxio.WithMaxRetryAttempts(5),
    maxio.WithRetryInterval(1),
    maxio.WithBackoffFactor(2),
    maxio.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    maxio.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := maxio.NewClient(
    maxio.CreateConfiguration(
        maxio.WithHttpConfiguration(
            maxio.CreateHttpConfiguration(
                maxio.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
