...
`testing` + `testify/require` for reference; substitute the real `cellpointapi` and names.
...
import (
    ...
    "github.com/context-plugins/cellpoint-api-go"   // root package cellpointapi
)
...
func clientReturning(status int, body string) (cellpointapi.ClientInterface, *stubTransport) {
    ...
    client := cellpointapi.NewClient(
        cellpointapi.CreateConfiguration(
            cellpointapi.WithHttpConfiguration(
                cellpointapi.CreateHttpConfiguration(cellpointapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/cellpoint-api-go/errors"
...
client := cellpointapi.NewClient(
    cellpointapi.CreateConfiguration(
        cellpointapi.WithHttpConfiguration(
            cellpointapi.CreateHttpConfiguration(cellpointapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
