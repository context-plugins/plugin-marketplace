...
`testing` + `testify/require` for reference; substitute the real `hotelnameautocomplete` and names.
...
import (
    ...
    "github.com/context-plugins/hotel-name-autocomplete-go"   // root package hotelnameautocomplete
)
...
func clientReturning(status int, body string) (hotelnameautocomplete.ClientInterface, *stubTransport) {
    ...
    client := hotelnameautocomplete.NewClient(
        hotelnameautocomplete.CreateConfiguration(
            hotelnameautocomplete.WithHttpConfiguration(
                hotelnameautocomplete.CreateHttpConfiguration(hotelnameautocomplete.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/hotel-name-autocomplete-go/errors"
...
client := hotelnameautocomplete.NewClient(
    hotelnameautocomplete.CreateConfiguration(
        hotelnameautocomplete.WithHttpConfiguration(
            hotelnameautocomplete.CreateHttpConfiguration(hotelnameautocomplete.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
