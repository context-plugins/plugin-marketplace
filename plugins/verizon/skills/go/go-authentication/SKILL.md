...
> Throughout, `verizonapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := verizonapi.NewClient(
    verizonapi.CreateConfiguration(
        verizonapi.WithBasicAuthCredentials(
            verizonapi.NewBasicAuthCredentials(
...
verizonapi.WithApiKeyCredentials(verizonapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
verizonapi.WithApiHeaderCredentials(verizonapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
verizonapi.WithOAuthBearerTokenCredentials(
    verizonapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := verizonapi.NewClient(
    verizonapi.CreateConfiguration(
        verizonapi.WithOAuthCCGCredentials(
            verizonapi.NewOAuthCCGCredentials(
