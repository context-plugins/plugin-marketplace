...
- `adyenbinlookupapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/adyen-binlookup-api-go` — the Go import path for the root package; `github.com/context-plugins/adyen-binlookup-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/adyen-binlookup-api-go"

client := adyenbinlookupapi.NewClient(
    adyenbinlookupapi.CreateConfiguration(
        adyenbinlookupapi.WithEnvironment(adyenbinlookupapi.PRODUCTION),
...
from environment variables named `ADYENBINLOOKUPAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := adyenbinlookupapi.NewClient(adyenbinlookupapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `adyenbinlookupapi.PRODUCTION`,
`adyenbinlookupapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := adyenbinlookupapi.NewClient(
    adyenbinlookupapi.CreateConfiguration(adyenbinlookupapi.WithEnvironment(adyenbinlookupapi.PRODUCTION)),
)
```
...
client := adyenbinlookupapi.NewClient(
    adyenbinlookupapi.CreateConfiguration(
        adyenbinlookupapi.WithHttpConfiguration(
            adyenbinlookupapi.CreateHttpConfiguration(
                adyenbinlookupapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                adyenbinlookupapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = adyenbinlookupapi.NewClient(adyenbinlookupapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api adyenbinlookupapi.ClientInterface
}

func NewService(api adyenbinlookupapi.ClientInterface) *Service {
