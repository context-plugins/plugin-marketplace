...
- `XeroClient` — the single public client class (e.g. `FooClient`).
- `XeroClientOptions` — its options class.
- `Xero` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public XeroClient(HttpClient httpClient, XeroClientOptions options)
```
...
```csharp
public class XeroClientOptions
{
...
var client = new XeroClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddXeroClient`, which registers the
...
builder.Services.AddXeroClient(options =>
{
...
```csharp
public sealed class MyService(XeroClient client)
{
