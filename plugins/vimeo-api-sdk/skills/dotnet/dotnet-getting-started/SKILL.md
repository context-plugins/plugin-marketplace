...
| --- | --- |
| API | `Vimeo API` |
| Project | `VimeoApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using VimeoApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/vimeo-api-csharp
dotnet add reference vimeo-api-csharp/VimeoApi.csproj
```

```csharp
using VimeoApi;          // client + options + ServerEnvironment
using VimeoApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-api-csharp /tmp/vimeo-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-api-csharp "$env:TEMP\vimeo-api-dotnet-src"
```
...
- `VimeoApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/vimeo-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\vimeo-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `VimeoApiClient`, build its
   `VimeoApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
