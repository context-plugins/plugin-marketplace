...
> `TeslaAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): TeslaAPIClient
{
...
```php
use TeslaAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use TeslaAPILib\Exceptions\{OperationException};
use TeslaAPILib\Exceptions\ApiException;
...
app()->instance(TeslaAPIClient::class, clientWithResponses([
