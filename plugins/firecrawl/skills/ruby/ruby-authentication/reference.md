...
Confirm every name against `lib/firecrawl/client.rb`, `lib/firecrawl/configuration.rb`, the
`lib/firecrawl/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
