...
| Project | `ShutterstockApiExplorer.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using ShutterstockApiExplorer.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/shutterstock-api-explorer-csharp-sdk
dotnet add reference shutterstock-api-explorer-csharp-sdk/ShutterstockApiExplorer.csproj
```

```csharp
using ShutterstockApiExplorer;          // client + options + ServerEnvironment
using ShutterstockApiExplorer.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/shutterstock-api-explorer-csharp-sdk /tmp/shutterstock-api-explorer-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/shutterstock-api-explorer-csharp-sdk "$env:TEMP\shutterstock-api-explorer-dotnet-src"
```
...
- `ShutterstockApiExplorerClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/shutterstock-api-explorer-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\shutterstock-api-explorer-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `ShutterstockApiExplorerClient`, build its
   `ShutterstockApiExplorerClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
