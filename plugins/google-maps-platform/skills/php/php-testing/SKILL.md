...
> `GoogleMapsPlatformClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): GoogleMapsPlatformClient
{
...
```php
use GoogleMapsPlatformLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use GoogleMapsPlatformLib\Exceptions\{OperationException};
use GoogleMapsPlatformLib\Exceptions\ApiException;
...
app()->instance(GoogleMapsPlatformClient::class, clientWithResponses([
