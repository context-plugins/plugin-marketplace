...
Confirm every name against `lib/authentiq_api/client.rb`, `lib/authentiq_api/configuration.rb`, the
`lib/authentiq_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
