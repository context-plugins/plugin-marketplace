...
```go
klarna.WithOAuthCCGCredentials(
    klarna.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr klarna.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := klarna.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr klarna.OAuthCCGManager) models.OAuthToken {
...
```go
creds := klarna.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := klarna.NewClient(klarna.CreateConfiguration(klarna.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    klarna.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := klarna.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := klarna.NewClient(klarna.CreateConfiguration(klarna.WithOAuthROPCGCredentials(creds)))
...
```go
klarna.WithOAuthBearerTokenCredentials(klarna.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
klarna.WithBasicAuthCredentials(klarna.NewBasicAuthCredentials(username, password))
klarna.WithApiHeaderCredentials(klarna.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
klarna.WithApiKeyCredentials(klarna.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `KLARNA_...` variables — e.g.
`KLARNA_USERNAME`, `KLARNA_PASSWORD`, `KLARNA_TOKEN`, `KLARNA_API_KEY`,
`KLARNA_O_AUTH_CLIENT_ID`, `KLARNA_O_AUTH_CLIENT_SECRET`, `KLARNA_ACCESS_TOKEN`. Grep the
