...
- `access-analyzerlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'access-analyzerlib';
...
```ts
import { {Resource}Controller } from 'access-analyzerlib';
