...
> `SubscriptionClientClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SubscriptionClientClient
{
...
```php
use SubscriptionClientLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SubscriptionClientLib\Exceptions\{OperationException};
use SubscriptionClientLib\Exceptions\ApiException;
...
app()->instance(SubscriptionClientClient::class, clientWithResponses([
