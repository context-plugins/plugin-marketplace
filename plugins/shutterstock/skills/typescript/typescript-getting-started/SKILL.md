...
description: "Shutterstock API Explorer TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `shutterstock-api-explorer`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any Shutterstock API Explorer TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the Shutterstock API Explorer TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Shutterstock API Explorer TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-shutterstock-api-explorer` for the workflow that wraps this skill.
...
| Package name (what you install, and what you import) | `shutterstock-api-explorer` — **not on npm**; built from source (see *Install*) |
| Import specifier | `shutterstock-api-explorer` — the package root is the **only** entry; deep imports do not resolve |
| Version | `1.2.0` (API spec version `1.2.0`) |
| Client class | `ShutterstockApiExplorerClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new ShutterstockApiExplorerClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new ShutterstockApiExplorerClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `basic` · `customerAccessCode` · `customerAccessCodeStrategy`. `timeout` defaults to `60_000` ms |
...
```ts
import { ShutterstockApiExplorerClient, ServerEnvironment, ResponseError, ShutterstockApiExplorerError } from "shutterstock-api-explorer";
import type { ClientOptions, AccessTokenDetails } from "shutterstock-api-explorer";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `shutterstock-api-explorer/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **⚠ The SDK exports a model type literally named `Error`** (`src/models/error.ts`, schema `errorSchema`). Import it unaliased and it **shadows the global `Error`** for the rest of the file. Alias it: `import type { Error as SdkError } from "shutterstock-api-explorer"`.
- **From CommonJS**, the typed spelling is `import sdk = require("shutterstock-api-explorer")`. A plain `require` destructure runs but yields `any`.
...
```ts
const client = new ShutterstockApiExplorerClient({
...
```bash
node -e "console.log(require.resolve('shutterstock-api-explorer/package.json'))"
```

Failing that it is at `node_modules/shutterstock-api-explorer/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/shutterstock-api-explorer/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/shutterstock-api-explorer/`:
...
Beyond the usual signatures and model members, a contract sheet for the Shutterstock API Explorer TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
...
9. **That `Error` imported from this package is a model type, not the global of that name** — every sheet that references one should carry the alias it will be imported under. The error base is re-exported as `ShutterstockApiExplorerError` for the same reason.
