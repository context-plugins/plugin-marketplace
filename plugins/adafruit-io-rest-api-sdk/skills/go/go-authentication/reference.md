...
```go
adafruitiorestapi.WithOAuthCCGCredentials(
    adafruitiorestapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr adafruitiorestapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := adafruitiorestapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr adafruitiorestapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := adafruitiorestapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := adafruitiorestapi.NewClient(adafruitiorestapi.CreateConfiguration(adafruitiorestapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    adafruitiorestapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := adafruitiorestapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := adafruitiorestapi.NewClient(adafruitiorestapi.CreateConfiguration(adafruitiorestapi.WithOAuthROPCGCredentials(creds)))
...
```go
adafruitiorestapi.WithOAuthBearerTokenCredentials(adafruitiorestapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
adafruitiorestapi.WithBasicAuthCredentials(adafruitiorestapi.NewBasicAuthCredentials(username, password))
adafruitiorestapi.WithApiHeaderCredentials(adafruitiorestapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
adafruitiorestapi.WithApiKeyCredentials(adafruitiorestapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `ADAFRUITIORESTAPI_...` variables — e.g.
`ADAFRUITIORESTAPI_USERNAME`, `ADAFRUITIORESTAPI_PASSWORD`, `ADAFRUITIORESTAPI_TOKEN`, `ADAFRUITIORESTAPI_API_KEY`,
`ADAFRUITIORESTAPI_O_AUTH_CLIENT_ID`, `ADAFRUITIORESTAPI_O_AUTH_CLIENT_SECRET`, `ADAFRUITIORESTAPI_ACCESS_TOKEN`. Grep the
