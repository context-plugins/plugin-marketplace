...
`shutterstock/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `shutterstock/exceptions/` and operation docstrings.
...
```python
from shutterstock.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `shutterstock/exceptions/`.
...
```python
from shutterstock.exceptions.api_exception import APIException
from shutterstock.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`shutterstock/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`shutterstock/exceptions/` for the class.
...
`shutterstock/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `shutterstock/exceptions/`
  or a `ValueError` — check the auth handler in `shutterstock/http/auth/` for the exact type.
