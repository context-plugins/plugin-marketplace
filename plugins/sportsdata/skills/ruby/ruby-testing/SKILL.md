...
client = stub_client(stubs)
assert_raises Sportsdata::APIException do
...
BASE = Sportsdata::Configuration.new(environment: Sportsdata::Environment::PRODUCTION).get_base_uri

def build_test_client
  Sportsdata::Client.new(
    environment: Sportsdata::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Sportsdata::APIException
```
...
```ruby
expect { call }.to raise_error(Sportsdata::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Sportsdata::HttpCallBack
...
client  = Sportsdata::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
