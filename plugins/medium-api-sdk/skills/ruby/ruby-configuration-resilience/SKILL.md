...
```ruby
client = MediumAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = MediumAPI::Client.new(adapter: :net_http)
```
...
client = MediumAPI::Client.new(connection: conn)
```
...
client = MediumAPI::Client.new(environment: Environment::PRODUCTION)
client = MediumAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = MediumAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = MediumAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = MediumAPI::ProxySettings.from_env
client = MediumAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < MediumAPI::HttpCallBack
...
client = MediumAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = MediumAPI::Client.from_env
# Overrides take precedence over env vars:
client = MediumAPI::Client.from_env(timeout: 30, max_retries: 3)
```
