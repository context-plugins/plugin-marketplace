...
```ruby
client = DeploymentAdminClient::Client.new(
  max_retries:    3,
...
```ruby
client = DeploymentAdminClient::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = DeploymentAdminClient::Client.new(adapter: :net_http)
```
...
client = DeploymentAdminClient::Client.new(connection: conn)
```
...
client = DeploymentAdminClient::Client.new(environment: Environment::PRODUCTION)
client = DeploymentAdminClient::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = DeploymentAdminClient::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = DeploymentAdminClient::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = DeploymentAdminClient::ProxySettings.from_env
client = DeploymentAdminClient::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < DeploymentAdminClient::HttpCallBack
...
client = DeploymentAdminClient::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = DeploymentAdminClient::Client.from_env
# Overrides take precedence over env vars:
client = DeploymentAdminClient::Client.from_env(timeout: 30, max_retries: 3)
```
