...
> `FinnhubApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FinnhubApiClient
{
...
```php
use FinnhubApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use FinnhubApiLib\Exceptions\{OperationException};
use FinnhubApiLib\Exceptions\ApiException;
...
app()->instance(FinnhubApiClient::class, clientWithResponses([
