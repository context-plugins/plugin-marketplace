...
> Throughout, `ZalandoShop` and other `{...}` tokens are placeholders for names you take from your SDK —
...
`lib/zalando_shop/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/zalando_shop/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/zalando_shop/http/auth/api_header.rb` and
