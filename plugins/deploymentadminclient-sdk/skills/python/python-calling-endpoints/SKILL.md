...
Open `deploymentadminclient/deploymentadminclient_client.py` for the controller property names, then the relevant
`deploymentadminclient/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from deploymentadminclient.models.{request_model} import {RequestModel}
...
Open `deploymentadminclient/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from deploymentadminclient.models.suite_code_enum import SuiteCodeEnum
...
```python
from deploymentadminclient.http.http_call_back import HttpCallBack
...
client = DeploymentAdminClientClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `deploymentadminclient/pagination/` in the source.

## Finding the right method

- Controller property names are on `DeploymentAdminClientClient` in `deploymentadminclient/deploymentadminclient_client.py`.
- Operation method signatures are in `deploymentadminclient/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `deploymentadminclient/models/`; enum types have `from_value()`.
