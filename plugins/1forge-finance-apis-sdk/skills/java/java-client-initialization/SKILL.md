...
- `1ForgeFinanceAPIsClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.m1forge` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`1ForgeFinanceAPIsClient.Builder` class. A minimal initialization:

```java
import com.m1forge.1ForgeFinanceAPIsClient;
import com.m1forge.Environment;

1ForgeFinanceAPIsClient client = new 1ForgeFinanceAPIsClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `1ForgeFinanceAPIsClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `1ForgeFinanceAPIsClient.java` |

Call `.build()` to produce an immutable `1ForgeFinanceAPIsClient` instance.
...
public class ApiConfig {
    @Bean
    public 1ForgeFinanceAPIsClient apiClient() {
        return new 1ForgeFinanceAPIsClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired 1ForgeFinanceAPIsClient client) {
        client.shutdown();
    }
}
```

Inject `1ForgeFinanceAPIsClient` into your services as a normal Spring dependency. Build only one bean.
