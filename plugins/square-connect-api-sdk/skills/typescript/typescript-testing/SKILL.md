...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new SquareConnectAPIClient({
...
```typescript
import { {Operation}Error } from 'square-connect-apilib/errors';
...
```typescript
import { ApiError } from 'square-connect-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(SquareConnectAPIClient)
    .useValue(new SquareConnectAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
