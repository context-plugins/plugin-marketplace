...
```ruby
client = AdobeExperienceManagerAEMAPI::Client.new(
  max_retries:    3,
...
```ruby
client = AdobeExperienceManagerAEMAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = AdobeExperienceManagerAEMAPI::Client.new(adapter: :net_http)
```
...
client = AdobeExperienceManagerAEMAPI::Client.new(connection: conn)
```
...
client = AdobeExperienceManagerAEMAPI::Client.new(environment: Environment::PRODUCTION)
client = AdobeExperienceManagerAEMAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = AdobeExperienceManagerAEMAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = AdobeExperienceManagerAEMAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = AdobeExperienceManagerAEMAPI::ProxySettings.from_env
client = AdobeExperienceManagerAEMAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < AdobeExperienceManagerAEMAPI::HttpCallBack
...
client = AdobeExperienceManagerAEMAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = AdobeExperienceManagerAEMAPI::Client.from_env
# Overrides take precedence over env vars:
client = AdobeExperienceManagerAEMAPI::Client.from_env(timeout: 30, max_retries: 3)
```
