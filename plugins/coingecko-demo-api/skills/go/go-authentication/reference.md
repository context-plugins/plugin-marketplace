...
```go
coingeckodemoapi.WithOAuthCCGCredentials(
    coingeckodemoapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr coingeckodemoapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := coingeckodemoapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr coingeckodemoapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := coingeckodemoapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := coingeckodemoapi.NewClient(coingeckodemoapi.CreateConfiguration(coingeckodemoapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    coingeckodemoapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := coingeckodemoapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := coingeckodemoapi.NewClient(coingeckodemoapi.CreateConfiguration(coingeckodemoapi.WithOAuthROPCGCredentials(creds)))
...
```go
coingeckodemoapi.WithOAuthBearerTokenCredentials(coingeckodemoapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
coingeckodemoapi.WithBasicAuthCredentials(coingeckodemoapi.NewBasicAuthCredentials(username, password))
coingeckodemoapi.WithApiHeaderCredentials(coingeckodemoapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
coingeckodemoapi.WithApiKeyCredentials(coingeckodemoapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `COINGECKODEMOAPI_...` variables — e.g.
`COINGECKODEMOAPI_USERNAME`, `COINGECKODEMOAPI_PASSWORD`, `COINGECKODEMOAPI_TOKEN`, `COINGECKODEMOAPI_API_KEY`,
`COINGECKODEMOAPI_O_AUTH_CLIENT_ID`, `COINGECKODEMOAPI_O_AUTH_CLIENT_SECRET`, `COINGECKODEMOAPI_ACCESS_TOKEN`. Grep the
