...
> Throughout, `squareconnectapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := squareconnectapi.NewClient(
    squareconnectapi.CreateConfiguration(
        squareconnectapi.WithBasicAuthCredentials(
            squareconnectapi.NewBasicAuthCredentials(
...
squareconnectapi.WithApiKeyCredentials(squareconnectapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
squareconnectapi.WithApiHeaderCredentials(squareconnectapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
squareconnectapi.WithOAuthBearerTokenCredentials(
    squareconnectapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := squareconnectapi.NewClient(
    squareconnectapi.CreateConfiguration(
        squareconnectapi.WithOAuthCCGCredentials(
            squareconnectapi.NewOAuthCCGCredentials(
