...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ApiApisGuruClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`apiapisguru/configuration.py` in the cloned source.
...
```python
import requests
from apiapisguru.configuration import Configuration
...
`apiapisguru/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`apiapisguru/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `apiapisguru/configuration.py`
...
`NumberPagedResponse`) live in `apiapisguru/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `apiapisguru/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
