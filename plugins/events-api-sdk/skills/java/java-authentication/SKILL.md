...
To see which schemes a specific SDK accepts, read the credential setter methods on `EventsAPIClient.Builder`
...
- Set credentials **on the `EventsAPIClient.Builder`** before calling `.build()` — the client is immutable
