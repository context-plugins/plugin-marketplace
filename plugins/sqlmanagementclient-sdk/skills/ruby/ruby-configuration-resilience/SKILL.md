...
```ruby
client = SqlManagementClient::Client.new(
  max_retries:    3,
...
```ruby
client = SqlManagementClient::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = SqlManagementClient::Client.new(adapter: :net_http)
```
...
client = SqlManagementClient::Client.new(connection: conn)
```
...
client = SqlManagementClient::Client.new(environment: Environment::PRODUCTION)
client = SqlManagementClient::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = SqlManagementClient::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = SqlManagementClient::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = SqlManagementClient::ProxySettings.from_env
client = SqlManagementClient::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < SqlManagementClient::HttpCallBack
...
client = SqlManagementClient::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = SqlManagementClient::Client.from_env
# Overrides take precedence over env vars:
client = SqlManagementClient::Client.from_env(timeout: 30, max_retries: 3)
```
