...
client = stub_client(stubs)
assert_raises SeatmapDisplay::APIException do
...
def build_test_client
  SeatmapDisplay::Client.new(
    environment: SeatmapDisplay::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error SeatmapDisplay::APIException
```
...
```ruby
expect { call }.to raise_error(SeatmapDisplay::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < SeatmapDisplay::HttpCallBack
...
client  = SeatmapDisplay::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
