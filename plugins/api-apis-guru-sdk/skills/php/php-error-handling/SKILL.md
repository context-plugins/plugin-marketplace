...
> `{operation}`, `{apiGroup}`, `FlightCheckInLinksLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `FlightCheckInLinksLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use FlightCheckInLinksLib\Exceptions\ApiException;
...
```php
use FlightCheckInLinksLib\Exceptions\ApiException;
use FlightCheckInLinksLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `FlightCheckInLinksLib\Exceptions\` — read the source to
