...
> `SeatmapDisplayClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SeatmapDisplayClient
{
...
```php
use SeatmapDisplayLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SeatmapDisplayLib\Exceptions\{OperationException};
use SeatmapDisplayLib\Exceptions\ApiException;
...
app()->instance(SeatmapDisplayClient::class, clientWithResponses([
