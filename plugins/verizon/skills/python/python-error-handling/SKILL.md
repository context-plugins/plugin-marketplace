...
`verizon/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `verizon/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `verizon/exceptions/`.
...
file (`verizon/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`verizon/exceptions/` for the class.
...
`verizon/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `verizon/exceptions/`
  or a `ValueError` — check the auth handler in `verizon/http/auth/` for the exact type.
