...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddMediumApiClient extension. Use the moment you write `new MediumApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `MediumApiClientOptions` — its options class.
- `MediumApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public MediumApiClient(HttpClient httpClient, MediumApiClientOptions options)
```
...
```csharp
public class MediumApiClientOptions
{
...
```csharp
using MediumApi;
using MediumApi.Servers;

var options = new MediumApiClientOptions
{
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddMediumApiClient`, which registers the
...
builder.Services.AddMediumApiClient(options =>
{
...
```csharp
public sealed class MyService(MediumApiClient client)
{
