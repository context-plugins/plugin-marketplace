...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `Pqapiv2Client`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`payquicker/configuration.py` in the cloned source.
...
```python
import requests
from payquicker.configuration import Configuration
...
`payquicker/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`payquicker/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `payquicker/configuration.py`
...
`NumberPagedResponse`) live in `payquicker/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `payquicker/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
