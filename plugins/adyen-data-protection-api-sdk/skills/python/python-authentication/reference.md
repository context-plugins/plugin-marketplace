...
kwargs to the `AdyenDataProtectionAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `adyendataprotectionapi/adyendataprotectionapi_client.py` and `adyendataprotectionapi/http/auth/` in
the cloned source.

## Basic auth

```python
from adyendataprotectionapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from adyendataprotectionapi.http.auth.api_key import ApiKeyCredentials
...
```python
from adyendataprotectionapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from adyendataprotectionapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from adyendataprotectionapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from adyendataprotectionapi.http.auth.o_auth_acg import OAuthACGCredentials
from adyendataprotectionapi.models.o_auth_scope_adyendataprotectionapi_enum import OAuthScope{Pkg}Enum
...
client = AdyenDataProtectionAPIClient(o_auth_acg_credentials=creds)
...
```python
from adyendataprotectionapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AdyenDataProtectionAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AdyenDataProtectionAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `adyendataprotectionapi/configuration.py` and each `adyendataprotectionapi/http/auth/*.py` for the exact
