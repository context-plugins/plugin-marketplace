...
| Project | `SqlManagementClient.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `SqlManagementClient` (the `using` namespace) |
| Client class | `SqlManagementClientClient` |
| Options class | `SqlManagementClientClientOptions` |
...
namespaces transitively, so `using SqlManagementClient.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/sqlmanagementclient-csharp
dotnet add reference sqlmanagementclient-csharp/SqlManagementClient.csproj
```

```csharp
using SqlManagementClient;          // client + options + ServerEnvironment
using SqlManagementClient.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/sqlmanagementclient-csharp /tmp/sqlmanagementclient-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/sqlmanagementclient-csharp "$env:TEMP\sqlmanagementclient-dotnet-src"
```
...
- `SqlManagementClientClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/sqlmanagementclient-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\sqlmanagementclient-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `SqlManagementClientClient`, build its
   `SqlManagementClientClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
