...
- `googlemapsplatform` — the root package name (e.g. `multiauthsample`, `batester`).
- `GoogleMapsPlatformClient` — the generated client class (read it from the `class …Client` declaration in `googlemapsplatform/googlemapsplatform_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `GoogleMapsPlatformClient` constructor accepts all configuration as **keyword arguments** — there is no
...
from googlemapsplatform.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from googlemapsplatform.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `googlemapsplatform/googlemapsplatform_client.py` in the cloned source.
...
`from_environment()` is a class method on both `GoogleMapsPlatformClient` and `Configuration` that reads
...
client = GoogleMapsPlatformClient.from_environment()
...
`from_environment` in `googlemapsplatform/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `GoogleMapsPlatformClient`. Access them as attributes —
...
Open `googlemapsplatform/googlemapsplatform_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `googlemapsplatform/configuration.py`. **Read the enum in `configuration.py`
...
client = GoogleMapsPlatformClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`googlemapsplatform/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = GoogleMapsPlatformClient(config=new_config)
```
