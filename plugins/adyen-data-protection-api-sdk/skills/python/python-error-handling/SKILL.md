...
`adyendataprotectionapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `adyendataprotectionapi/exceptions/` and operation docstrings.
...
```python
from adyendataprotectionapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `adyendataprotectionapi/exceptions/`.
...
```python
from adyendataprotectionapi.exceptions.api_exception import APIException
from adyendataprotectionapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`adyendataprotectionapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`adyendataprotectionapi/exceptions/` for the class.
...
`adyendataprotectionapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `adyendataprotectionapi/exceptions/`
  or a `ValueError` — check the auth handler in `adyendataprotectionapi/http/auth/` for the exact type.
