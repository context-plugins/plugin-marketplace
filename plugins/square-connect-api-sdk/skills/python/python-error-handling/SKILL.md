...
`squareconnectapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `squareconnectapi/exceptions/` and operation docstrings.
...
```python
from squareconnectapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `squareconnectapi/exceptions/`.
...
```python
from squareconnectapi.exceptions.api_exception import APIException
from squareconnectapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`squareconnectapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`squareconnectapi/exceptions/` for the class.
...
`squareconnectapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `squareconnectapi/exceptions/`
  or a `ValueError` — check the auth handler in `squareconnectapi/http/auth/` for the exact type.
