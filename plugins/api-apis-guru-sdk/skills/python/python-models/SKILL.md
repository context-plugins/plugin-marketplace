...
your SDK source (`apiapisguru/models/`).
...
```python
from apiapisguru.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `apiapisguru/utilities/union_type_lookup.py`) and each candidate model
...
```python
from apiapisguru.models.animal import Cat, Dog
...
```python
from apiapisguru.models.animal import Animal, Cat, Dog
