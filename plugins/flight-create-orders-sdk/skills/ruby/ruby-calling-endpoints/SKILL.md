...
Open `lib/flight_create_orders/client.rb` for the accessor names, then the relevant
`lib/flight_create_orders/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = FlightCreateOrders::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: FlightCreateOrders::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/flight_create_orders/models/`. Field names are defined as
...
Read the enum module in `lib/flight_create_orders/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/flight_create_orders/client.rb`.
- Operation methods are in `lib/flight_create_orders/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/flight_create_orders/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/flight_create_orders/exceptions/`.
