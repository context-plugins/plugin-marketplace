...
To see which schemes a specific SDK accepts, read the credential setter methods on `VimeoAPIClient.Builder`
...
- Set credentials **on the `VimeoAPIClient.Builder`** before calling `.build()` — the client is immutable
