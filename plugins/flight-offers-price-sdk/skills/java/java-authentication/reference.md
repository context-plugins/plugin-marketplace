...
against the `FlightOffersPriceClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FlightOffersPriceClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FlightOffersPriceClient client = new FlightOffersPriceClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFlightOffersPriceEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new FlightOffersPriceClient.Builder()
