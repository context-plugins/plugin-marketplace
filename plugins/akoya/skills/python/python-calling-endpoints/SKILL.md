...
Open `akoya/akoya_client.py` for the controller property names, then the relevant
`akoya/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from akoya.models.{request_model} import {RequestModel}
...
Open `akoya/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from akoya.models.suite_code_enum import SuiteCodeEnum
...
```python
from akoya.http.http_call_back import HttpCallBack
...
client = AkoyaClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `akoya/pagination/` in the source.

## Finding the right method

- Controller property names are on `AkoyaClient` in `akoya/akoya_client.py`.
- Operation method signatures are in `akoya/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `akoya/models/`; enum types have `from_value()`.
