...
- `m3-commas-iolib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'm3-commas-iolib';
...
```ts
import { {Resource}Controller } from 'm3-commas-iolib';
