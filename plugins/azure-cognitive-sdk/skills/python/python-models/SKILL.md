...
your SDK source (`azurecognitive/models/`).
...
```python
from azurecognitive.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `azurecognitive/utilities/union_type_lookup.py`) and each candidate model
...
```python
from azurecognitive.models.animal import Cat, Dog
...
```python
from azurecognitive.models.animal import Animal, Cat, Dog
