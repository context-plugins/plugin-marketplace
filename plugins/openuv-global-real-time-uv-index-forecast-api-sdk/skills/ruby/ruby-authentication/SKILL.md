...
> Throughout, `OpenUVGlobalRealTimeUVIndexForecastAPI` and other `{...}` tokens are placeholders for names you take from your SDK —
...
`lib/openuv_global_real_time_uv_index_forecast_api/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/openuv_global_real_time_uv_index_forecast_api/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/openuv_global_real_time_uv_index_forecast_api/http/auth/api_header.rb` and
