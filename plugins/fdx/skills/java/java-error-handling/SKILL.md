...
│   └── net.apigee.financialdataexchangeprod.exceptions.ApiException          ← base for all non-2xx API responses
│       └── net.apigee.financialdataexchangeprod.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `net.apigee.financialdataexchangeprod.http.client.HttpContext`, which carries:
