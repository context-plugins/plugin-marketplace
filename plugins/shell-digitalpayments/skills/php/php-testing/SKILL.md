...
> `ShellSmartPayAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ShellSmartPayAPIClient
{
...
```php
use ShellSmartPayAPILib\Exceptions\{OperationException};
use ShellSmartPayAPILib\Exceptions\ApiException;
...
app()->instance(ShellSmartPayAPIClient::class, clientWithResponses([
