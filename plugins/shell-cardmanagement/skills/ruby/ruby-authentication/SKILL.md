...
> Throughout, `ShellCardmanagement` and other `{...}` tokens are placeholders for names you take from your SDK —
...
`lib/shell_cardmanagement/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/shell_cardmanagement/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/shell_cardmanagement/http/auth/api_header.rb` and
