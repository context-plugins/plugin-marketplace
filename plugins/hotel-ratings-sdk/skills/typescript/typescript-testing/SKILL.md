...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new HotelRatingsClient({
...
```typescript
import { {Operation}Error } from 'hotel-ratingslib/errors';
...
```typescript
import { ApiError } from 'hotel-ratingslib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(HotelRatingsClient)
    .useValue(new HotelRatingsClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
