...
- `alpaca` — the root package name (e.g. `multiauthsample`).
...
client := alpaca.NewClient(
    alpaca.CreateConfiguration(
        alpaca.WithEnvironment(alpaca.PRODUCTION),
...
from environment variables named `ALPACA_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := alpaca.NewClient(alpaca.CreateConfigurationFromEnvironment())
```
...
```go
client := alpaca.NewClient(
    alpaca.CreateConfiguration(alpaca.WithEnvironment(alpaca.PRODUCTION)),
)
```
...
client := alpaca.NewClient(
    alpaca.CreateConfiguration(
        alpaca.WithHttpConfiguration(
            alpaca.CreateHttpConfiguration(
                alpaca.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                alpaca.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = alpaca.NewClient(alpaca.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api alpaca.ClientInterface
}

func NewService(api alpaca.ClientInterface) *Service {
