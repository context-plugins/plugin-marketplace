...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `GeoapifyapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`geoapify/configuration.py` in the cloned source.
...
```python
import requests
from geoapify.configuration import Configuration
...
`geoapify/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`geoapify/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `geoapify/configuration.py`
...
`NumberPagedResponse`) live in `geoapify/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `geoapify/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
