...
> Throughout, `authentication` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := authentication.NewClient(
    authentication.CreateConfiguration(
        authentication.WithBasicAuthCredentials(
            authentication.NewBasicAuthCredentials(
...
authentication.WithApiKeyCredentials(authentication.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
authentication.WithApiHeaderCredentials(authentication.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
authentication.WithOAuthBearerTokenCredentials(
    authentication.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := authentication.NewClient(
    authentication.CreateConfiguration(
        authentication.WithOAuthCCGCredentials(
            authentication.NewOAuthCCGCredentials(
