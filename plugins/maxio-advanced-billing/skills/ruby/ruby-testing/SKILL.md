...
def stub_client(stubs)
  ...
  MaxioAdvancedBilling::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises MaxioAdvancedBilling::APIException do
...
BASE = MaxioAdvancedBilling::Configuration.new(environment: MaxioAdvancedBilling::Environment::PRODUCTION).get_base_uri

def build_test_client
  MaxioAdvancedBilling::Client.new(
    environment: MaxioAdvancedBilling::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error MaxioAdvancedBilling::APIException
```
...
```ruby
expect { call }.to raise_error(MaxioAdvancedBilling::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < MaxioAdvancedBilling::HttpCallBack
...
client  = MaxioAdvancedBilling::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
