...
| Source repo | https://github.com/context-plugins/notion-csharp-sdk (branch `main` — the ref this map documents) |
| Root namespace | `NotionApi` (the `using` namespace) |
| Client class | `NotionApiClient` |
| Options class | `NotionApiClientOptions` |
| Auth | **Bearer** token — set `BearerAuth` |
| Environments | `ServerEnvironment.Production` *(default)* → `https://api.notion.com/v1` |
| Log environment variable | `NOTIONAPICLIENT_LOG` |
| Target framework | `netstandard2.0` |

The table above is **orientation, not a copy-paste recipe** — it gives you the names and facts (install, namespaces, the auth *pattern*, the environments), while the actual integration code comes from the companion skills. Load every one the contract sheet's REQUIRED READING names before you start implementing (`dotnet-integrate-notion-api` makes that binding); the **Integration workflow** below says which governs which step.

## Namespaces (using-directives)

The SDK splits its public types across **separate child namespaces**. C# does **not** import child namespaces transitively, so `using NotionApi.Models;` alone does **not** make enums, union types, or error types visible — you get `CS0103`/`CS0246` ("name/type does not exist") on build. Add a separate `using` for each kind of type you reference:

| Type | Namespace |
| --- | --- |
| `NotionApiClient` | `NotionApi` |
| `NotionApiClientOptions` | `NotionApi` |
| `ServerEnvironment` | `NotionApi.Servers` |
...
**Clone once, up front**, into a fresh timestamped folder under `<temp>/notion-api-sdk-src/`, and reuse that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/notion-api-sdk-src/$(date +%Y%m%d-%H%M%S)"
git clone --filter=blob:none --branch main https://github.com/context-plugins/notion-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\notion-api-sdk-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --filter=blob:none --branch main https://github.com/context-plugins/notion-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```
...
The clone lives in `<temp>/notion-api-sdk-src/`, **never** in the project repo, and its path never goes into `notion-api-plan.md` — the plan must stay portable, and the clone is a session-local reference, not part of the project.
...
- `Servers/`, `NotionApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
