...
```ruby
client = FundAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = FundAPI::Client.new(adapter: :net_http)
```
...
client = FundAPI::Client.new(connection: conn)
```
...
client = FundAPI::Client.new(environment: Environment::PRODUCTION)
client = FundAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = FundAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = FundAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = FundAPI::ProxySettings.from_env
client = FundAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < FundAPI::HttpCallBack
...
client = FundAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = FundAPI::Client.from_env
# Overrides take precedence over env vars:
client = FundAPI::Client.from_env(timeout: 30, max_retries: 3)
```
