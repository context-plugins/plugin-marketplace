...
```go
tesla.WithOAuthCCGCredentials(
    tesla.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr tesla.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := tesla.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr tesla.OAuthCCGManager) models.OAuthToken {
...
```go
creds := tesla.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := tesla.NewClient(tesla.CreateConfiguration(tesla.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    tesla.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := tesla.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := tesla.NewClient(tesla.CreateConfiguration(tesla.WithOAuthROPCGCredentials(creds)))
...
```go
tesla.WithOAuthBearerTokenCredentials(tesla.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
tesla.WithBasicAuthCredentials(tesla.NewBasicAuthCredentials(username, password))
tesla.WithApiHeaderCredentials(tesla.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
tesla.WithApiKeyCredentials(tesla.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `TESLA_...` variables — e.g.
`TESLA_USERNAME`, `TESLA_PASSWORD`, `TESLA_TOKEN`, `TESLA_API_KEY`,
`TESLA_O_AUTH_CLIENT_ID`, `TESLA_O_AUTH_CLIENT_SECRET`, `TESLA_ACCESS_TOKEN`. Grep the
