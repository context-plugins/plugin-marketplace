...
- `vimeo-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'vimeo-apilib';
...
```ts
import { {Resource}Controller } from 'vimeo-apilib';
