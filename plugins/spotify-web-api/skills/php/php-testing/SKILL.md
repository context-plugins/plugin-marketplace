...
> `SpotifyWebApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SpotifyWebApiClient
{
...
```php
use SpotifyWebApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SpotifyWebApiLib\Exceptions\{OperationException};
use SpotifyWebApiLib\Exceptions\ApiException;
...
app()->instance(SpotifyWebApiClient::class, clientWithResponses([
