...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AzureCognitiveClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`azurecognitive/configuration.py` in the cloned source.
...
```python
import requests
from azurecognitive.configuration import Configuration
...
`azurecognitive/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`azurecognitive/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `azurecognitive/configuration.py`
...
`NumberPagedResponse`) live in `azurecognitive/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `azurecognitive/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
