...
description: APIMatic-generated Python SDK (APIMATIC v3.0, built on apimatic-core + apimatic-requests-client-adapter) reference and grounding layer — the PyPI package name from pyproject.toml, the single 3commasIoClient class in 3commasio_client.py constructed with keyword args (credentials objects + environment= + http/config kwargs), the Configuration class with Environment/Server enums, controllers accessed via @LazyProperty on the client, the APIException base, the HttpCallBack test seam, the from_environment() class method, and how to navigate the generated source. Entry point into the companion python-* skills (client-initialization, authentication, calling-endpoints, models, error-handling, configuration-resilience, testing) — gates loading each at its integration step, since the source shows signatures but not the usage gotchas these skills carry.
---
...
| Install | `pip install "git+https://github.com/context-plugins/3commas-io-python@main"` |
| Client class | `3commasIoClient` in `3commasio/3commasio_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `3commasio/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `3commasIoClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `3commasio/exceptions/api_exception.py` — raised on non-2xx |
...
- `3commasio/3commasio_client.py` — the `3commasIoClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `3commasio/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `3commasio/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `3commasio/controllers/` — one `{Resource}Controller` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `3commasio/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `3commasio/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `3commasio/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `3commasio/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `3commasio/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/3commas-io-python@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/3commas-io-python /tmp/3commas-io-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/3commas-io-python "$env:TEMP\3commas-io-python-src"
```
...
- `3commasio/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `3commasio/3commasio_client.py` — `3commasIoClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `3commasio/controllers/*.py` — operation method signatures, parameter names, and return types.
- `3commasio/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `3commasio/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/3commas-io-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\3commas-io-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `3commasIoClient(...)`.
