...
| Project | `TravelRecommendationsApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `TravelRecommendationsApi` (the `using` namespace) |
| Client class | `TravelRecommendationsApiClient` |
| Options class | `TravelRecommendationsApiClientOptions` |
...
namespaces transitively, so `using TravelRecommendationsApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/travel-recommendations-api-csharp
dotnet add reference travel-recommendations-api-csharp/TravelRecommendationsApi.csproj
```

```csharp
using TravelRecommendationsApi;          // client + options + ServerEnvironment
using TravelRecommendationsApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/travel-recommendations-api-csharp /tmp/travel-recommendations-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/travel-recommendations-api-csharp "$env:TEMP\travel-recommendations-api-dotnet-src"
```
...
- `TravelRecommendationsApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/travel-recommendations-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\travel-recommendations-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `TravelRecommendationsApiClient`, build its
   `TravelRecommendationsApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
