...
`testing` + `testify/require` for reference; substitute the real `vimeoapi` and names.
...
import (
    ...
    "github.com/context-plugins/vimeo-api-go-sdk"   // root package vimeoapi
)
...
func clientReturning(status int, body string) (vimeoapi.ClientInterface, *stubTransport) {
    ...
    client := vimeoapi.NewClient(
        vimeoapi.CreateConfiguration(
            vimeoapi.WithHttpConfiguration(
                vimeoapi.CreateHttpConfiguration(vimeoapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/vimeo-api-go-sdk/errors"
...
client := vimeoapi.NewClient(
    vimeoapi.CreateConfiguration(
        vimeoapi.WithHttpConfiguration(
            vimeoapi.CreateHttpConfiguration(vimeoapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
