...
> `{ApiGroup}`, `AdyenBinLookupApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `AdyenBinLookupApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `AdyenBinLookupApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `AdyenBinLookupApi.Errors`
...
```csharp
using AdyenBinLookupApi.Core.Exceptions;     // SdkException<TError>
using AdyenBinLookupApi.Core.ErrorResponse;  // ApiError, RawError
using AdyenBinLookupApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`AdyenBinLookupApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using AdyenBinLookupApi.Core.Exceptions;     // SdkException<TError>
using AdyenBinLookupApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `AdyenBinLookupApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `AdyenBinLookupApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using AdyenBinLookupApi.Core.Models;        // ApiResult<TResponse, TError>
using AdyenBinLookupApi.Core.ErrorResponse; // RawError
using AdyenBinLookupApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `AdyenBinLookupApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
