...
`discourse/configuration.py` in the cloned source.
...
```python
import requests
from discourse.configuration import Configuration
...
`discourse/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`discourse/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `discourse/configuration.py`
...
`NumberPagedResponse`) live in `discourse/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `discourse/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
