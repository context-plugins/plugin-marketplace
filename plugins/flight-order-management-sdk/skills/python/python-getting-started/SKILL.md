...
| Install | `pip install "git+https://github.com/context-plugins/flight-order-management-python@main"` |
| Client class | `FlightOrderManagementClient` in `flightordermanagement/flightordermanagement_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `flightordermanagement/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `FlightOrderManagementClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `flightordermanagement/exceptions/api_exception.py` — raised on non-2xx |
...
- `flightordermanagement/flightordermanagement_client.py` — the `FlightOrderManagementClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `flightordermanagement/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `flightordermanagement/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `flightordermanagement/controllers/` — one `{Resource}Controller` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `flightordermanagement/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `flightordermanagement/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `flightordermanagement/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `flightordermanagement/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `flightordermanagement/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/flight-order-management-python@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/flight-order-management-python /tmp/flight-order-management-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/flight-order-management-python "$env:TEMP\flight-order-management-python-src"
```
...
- `flightordermanagement/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `flightordermanagement/flightordermanagement_client.py` — `FlightOrderManagementClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `flightordermanagement/controllers/*.py` — operation method signatures, parameter names, and return types.
- `flightordermanagement/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `flightordermanagement/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/flight-order-management-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\flight-order-management-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `FlightOrderManagementClient(...)`.
