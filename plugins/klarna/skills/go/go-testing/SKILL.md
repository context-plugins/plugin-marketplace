...
`testing` + `testify/require` for reference; substitute the real `klarna` and names.
...
import (
    ...
    "github.com/context-plugins/klarna-go-sdk"   // root package klarna
)
...
func clientReturning(status int, body string) (klarna.ClientInterface, *stubTransport) {
    ...
    client := klarna.NewClient(
        klarna.CreateConfiguration(
            klarna.WithHttpConfiguration(
                klarna.CreateHttpConfiguration(klarna.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/klarna-go-sdk/errors"
...
client := klarna.NewClient(
    klarna.CreateConfiguration(
        klarna.WithHttpConfiguration(
            klarna.CreateHttpConfiguration(klarna.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
