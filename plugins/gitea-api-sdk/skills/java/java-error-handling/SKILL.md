...
│   └── v1.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── v1.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `v1.api.http.client.HttpContext`, which carries:
