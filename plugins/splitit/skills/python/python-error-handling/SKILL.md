...
`splitit/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `splitit/exceptions/` and operation docstrings.
...
```python
from splitit.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `splitit/exceptions/`.
...
```python
from splitit.exceptions.api_exception import APIException
from splitit.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`splitit/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`splitit/exceptions/` for the class.
...
`splitit/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `splitit/exceptions/`
  or a `ValueError` — check the auth handler in `splitit/http/auth/` for the exact type.
