...
Confirm every name against `lib/apis_guru/client.rb`, `lib/apis_guru/configuration.rb`, the
`lib/apis_guru/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
