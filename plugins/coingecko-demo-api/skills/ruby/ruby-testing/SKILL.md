...
client = stub_client(stubs)
assert_raises CoinGeckoDemoAPI::APIException do
...
def build_test_client
  CoinGeckoDemoAPI::Client.new(
    environment: CoinGeckoDemoAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error CoinGeckoDemoAPI::APIException
```
...
```ruby
expect { call }.to raise_error(CoinGeckoDemoAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < CoinGeckoDemoAPI::HttpCallBack
...
client  = CoinGeckoDemoAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
