...
`goe/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `goe/exceptions/` and operation docstrings.
...
```python
from goe.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `goe/exceptions/`.
...
```python
from goe.exceptions.api_exception import APIException
from goe.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`goe/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`goe/exceptions/` for the class.
...
`goe/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `goe/exceptions/`
  or a `ValueError` — check the auth handler in `goe/http/auth/` for the exact type.
