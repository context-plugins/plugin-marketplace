...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddPqApiV2Client extension. Use the moment you write `new PqApiV2Client(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `PqApiV2ClientOptions` — its options class.
- `PqApiV2` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public PqApiV2Client(HttpClient httpClient, PqApiV2ClientOptions options)
```
...
```csharp
public class PqApiV2ClientOptions
{
...
var client = new PqApiV2Client(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddPqApiV2Client`, which registers the
...
builder.Services.AddPqApiV2Client(options =>
{
...
```csharp
public sealed class MyService(PqApiV2Client client)
{
