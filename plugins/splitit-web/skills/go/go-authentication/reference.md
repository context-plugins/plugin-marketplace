...
```go
splititwebapiv3.WithOAuthCCGCredentials(
    splititwebapiv3.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr splititwebapiv3.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := splititwebapiv3.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr splititwebapiv3.OAuthCCGManager) models.OAuthToken {
...
```go
creds := splititwebapiv3.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := splititwebapiv3.NewClient(splititwebapiv3.CreateConfiguration(splititwebapiv3.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    splititwebapiv3.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := splititwebapiv3.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := splititwebapiv3.NewClient(splititwebapiv3.CreateConfiguration(splititwebapiv3.WithOAuthROPCGCredentials(creds)))
...
```go
splititwebapiv3.WithOAuthBearerTokenCredentials(splititwebapiv3.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
splititwebapiv3.WithBasicAuthCredentials(splititwebapiv3.NewBasicAuthCredentials(username, password))
splititwebapiv3.WithApiHeaderCredentials(splititwebapiv3.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
splititwebapiv3.WithApiKeyCredentials(splititwebapiv3.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SPLITITWEBAPIV3_...` variables — e.g.
`SPLITITWEBAPIV3_USERNAME`, `SPLITITWEBAPIV3_PASSWORD`, `SPLITITWEBAPIV3_TOKEN`, `SPLITITWEBAPIV3_API_KEY`,
`SPLITITWEBAPIV3_O_AUTH_CLIENT_ID`, `SPLITITWEBAPIV3_O_AUTH_CLIENT_SECRET`, `SPLITITWEBAPIV3_ACCESS_TOKEN`. Grep the
