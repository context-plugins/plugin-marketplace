...
`testing` + `testify/require` for reference; substitute the real `maxioadvancedbilling` and names.
...
import (
    ...
    "github.com/context-plugins/maxio-advanced-billing-go-sdk"   // root package maxioadvancedbilling
)
...
func clientReturning(status int, body string) (maxioadvancedbilling.ClientInterface, *stubTransport) {
    ...
    client := maxioadvancedbilling.NewClient(
        maxioadvancedbilling.CreateConfiguration(
            maxioadvancedbilling.WithHttpConfiguration(
                maxioadvancedbilling.CreateHttpConfiguration(maxioadvancedbilling.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/maxio-advanced-billing-go-sdk/errors"
...
client := maxioadvancedbilling.NewClient(
    maxioadvancedbilling.CreateConfiguration(
        maxioadvancedbilling.WithHttpConfiguration(
            maxioadvancedbilling.CreateHttpConfiguration(maxioadvancedbilling.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
