---
name: dotnet-shell-digitalpayments-getting-started
description: shell-digitalpayments .NET SDK identity and lookup layer for the dotnet-shell-digitalpayments-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the shell-digitalpayments .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-shell-digitalpayments-sdk` helper
...
| | |
| --- | --- |
| API | shell-digitalpayments |
| Source repo | https://github.com/context-plugins/shell-digitalpayments-csharp-sdk (branch `main`) |
| Root namespace | `ShellSmartPayApi` (the `using` namespace) |
| Client class | `ShellSmartPayApiClient` |
| Options class | `ShellSmartPayApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `ShellSmartPayApiClientOptions`: `MppToken: string?`, `OAuthTokenPost: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `Test`, `Production` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using ShellSmartPayApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/shell-digitalpayments-csharp-sdk
dotnet add reference shell-digitalpayments-csharp-sdk/ShellSmartPayApi.csproj
```
...
temp directory** (`<temp>/shell-digitalpayments-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/shell-digitalpayments-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/shell-digitalpayments-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/shell-digitalpayments-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\shell-digitalpayments-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/shell-digitalpayments-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/shell-digitalpayments-dotnet-src/`, **never** in the project repo, and its path never goes into
`shell-digitalpayments-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `ShellSmartPayApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new ShellSmartPayApiClient(...)`, build its options, or DI-register via
   `AddShellSmartPayApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `ShellSmartPayApiClientOptions` — the map's *Servers & auth* section
