...
```go
payquicker.WithOAuthCCGCredentials(
    payquicker.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr payquicker.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := payquicker.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr payquicker.OAuthCCGManager) models.OAuthToken {
...
```go
creds := payquicker.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := payquicker.NewClient(payquicker.CreateConfiguration(payquicker.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    payquicker.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := payquicker.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := payquicker.NewClient(payquicker.CreateConfiguration(payquicker.WithOAuthROPCGCredentials(creds)))
...
```go
payquicker.WithOAuthBearerTokenCredentials(payquicker.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
payquicker.WithBasicAuthCredentials(payquicker.NewBasicAuthCredentials(username, password))
payquicker.WithApiHeaderCredentials(payquicker.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
payquicker.WithApiKeyCredentials(payquicker.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `PAYQUICKER_...` variables — e.g.
`PAYQUICKER_USERNAME`, `PAYQUICKER_PASSWORD`, `PAYQUICKER_TOKEN`, `PAYQUICKER_API_KEY`,
`PAYQUICKER_O_AUTH_CLIENT_ID`, `PAYQUICKER_O_AUTH_CLIENT_SECRET`, `PAYQUICKER_ACCESS_TOKEN`. Grep the
