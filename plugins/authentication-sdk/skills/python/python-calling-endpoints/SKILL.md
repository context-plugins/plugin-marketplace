...
Open `authentication/authentication_client.py` for the controller property names, then the relevant
`authentication/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from authentication.models.{request_model} import {RequestModel}
...
Open `authentication/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from authentication.models.suite_code_enum import SuiteCodeEnum
...
```python
from authentication.http.http_call_back import HttpCallBack
...
client = AuthenticationClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `authentication/pagination/` in the source.

## Finding the right method

- Controller property names are on `AuthenticationClient` in `authentication/authentication_client.py`.
- Operation method signatures are in `authentication/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `authentication/models/`; enum types have `from_value()`.
