...
```go
flightoffersprice.WithOAuthCCGCredentials(
    flightoffersprice.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr flightoffersprice.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := flightoffersprice.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr flightoffersprice.OAuthCCGManager) models.OAuthToken {
...
```go
creds := flightoffersprice.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := flightoffersprice.NewClient(flightoffersprice.CreateConfiguration(flightoffersprice.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    flightoffersprice.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := flightoffersprice.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := flightoffersprice.NewClient(flightoffersprice.CreateConfiguration(flightoffersprice.WithOAuthROPCGCredentials(creds)))
...
```go
flightoffersprice.WithOAuthBearerTokenCredentials(flightoffersprice.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
flightoffersprice.WithBasicAuthCredentials(flightoffersprice.NewBasicAuthCredentials(username, password))
flightoffersprice.WithApiHeaderCredentials(flightoffersprice.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
flightoffersprice.WithApiKeyCredentials(flightoffersprice.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FLIGHTOFFERSPRICE_...` variables — e.g.
`FLIGHTOFFERSPRICE_USERNAME`, `FLIGHTOFFERSPRICE_PASSWORD`, `FLIGHTOFFERSPRICE_TOKEN`, `FLIGHTOFFERSPRICE_API_KEY`,
`FLIGHTOFFERSPRICE_O_AUTH_CLIENT_ID`, `FLIGHTOFFERSPRICE_O_AUTH_CLIENT_SECRET`, `FLIGHTOFFERSPRICE_ACCESS_TOKEN`. Grep the
