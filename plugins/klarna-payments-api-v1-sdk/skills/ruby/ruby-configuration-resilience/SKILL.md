...
```ruby
client = KlarnaPaymentsAPIV1::Client.new(
  max_retries:    3,
...
```ruby
client = KlarnaPaymentsAPIV1::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = KlarnaPaymentsAPIV1::Client.new(adapter: :net_http)
```
...
client = KlarnaPaymentsAPIV1::Client.new(connection: conn)
```
...
client = KlarnaPaymentsAPIV1::Client.new(environment: Environment::PRODUCTION)
client = KlarnaPaymentsAPIV1::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = KlarnaPaymentsAPIV1::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = KlarnaPaymentsAPIV1::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = KlarnaPaymentsAPIV1::ProxySettings.from_env
client = KlarnaPaymentsAPIV1::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < KlarnaPaymentsAPIV1::HttpCallBack
...
client = KlarnaPaymentsAPIV1::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = KlarnaPaymentsAPIV1::Client.from_env
# Overrides take precedence over env vars:
client = KlarnaPaymentsAPIV1::Client.from_env(timeout: 30, max_retries: 3)
```
