...
against the `ThePlaidAPIClient.Builder` setters, the `com.plaid.production.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ThePlaidAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ThePlaidAPIClient client = new ThePlaidAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeThePlaidAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
