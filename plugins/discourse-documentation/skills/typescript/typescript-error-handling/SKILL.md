...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `discourse-api-documentationlib`) — replace it with the concrete identifier from the source.
