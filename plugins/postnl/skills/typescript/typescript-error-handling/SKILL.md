...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `postnl-ecommercelib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'postnl-ecommercelib';
import { ApiError } from 'postnl-ecommercelib';
import { {Operation}Error } from 'postnl-ecommercelib/errors';
