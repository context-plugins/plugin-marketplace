...
| --- | --- |
| API | `PayPal API` |
| Project | `PayPalApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `PayPalApi` (the `using` namespace) |
| Client class | `PayPalApiClient` |
| Options class | `PayPalApiClientOptions` |
...
namespaces transitively, so `using PayPalApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/paypal-api-csharp
dotnet add reference paypal-api-csharp/PayPalApi.csproj
```

```csharp
using PayPalApi;          // client + options + ServerEnvironment
using PayPalApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/paypal-api-csharp /tmp/paypal-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/paypal-api-csharp "$env:TEMP\paypal-api-dotnet-src"
```
...
- `PayPalApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/paypal-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\paypal-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `PayPalApiClient`, build its
   `PayPalApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
