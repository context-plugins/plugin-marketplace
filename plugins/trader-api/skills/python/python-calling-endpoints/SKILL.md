...
Open `traderapi/traderapi_client.py` for the controller property names, then the relevant
`traderapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from traderapi.models.{request_model} import {RequestModel}
...
Open `traderapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from traderapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from traderapi.http.http_call_back import HttpCallBack
...
client = TraderapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `traderapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `TraderapiClient` in `traderapi/traderapi_client.py`.
- Operation method signatures are in `traderapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `traderapi/models/`; enum types have `from_value()`.
