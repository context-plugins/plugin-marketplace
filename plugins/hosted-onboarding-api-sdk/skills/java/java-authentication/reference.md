...
against the `HostedOnboardingAPIClient.Builder` setters, the `com.adyen.caltest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `HostedOnboardingAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
HostedOnboardingAPIClient client = new HostedOnboardingAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeHostedOnboardingAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new HostedOnboardingAPIClient.Builder()
