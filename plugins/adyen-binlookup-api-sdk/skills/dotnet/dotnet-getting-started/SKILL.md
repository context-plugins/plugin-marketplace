...
| Project | `AdyenBinLookupApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `AdyenBinLookupApi` (the `using` namespace) |
| Client class | `AdyenBinLookupApiClient` |
| Options class | `AdyenBinLookupApiClientOptions` |
...
namespaces transitively, so `using AdyenBinLookupApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/adyen-binlookup-api-csharp
dotnet add reference adyen-binlookup-api-csharp/AdyenBinLookupApi.csproj
```

```csharp
using AdyenBinLookupApi;          // client + options + ServerEnvironment
using AdyenBinLookupApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/adyen-binlookup-api-csharp /tmp/adyen-binlookup-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/adyen-binlookup-api-csharp "$env:TEMP\adyen-binlookup-api-dotnet-src"
```
...
- `AdyenBinLookupApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/adyen-binlookup-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\adyen-binlookup-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AdyenBinLookupApiClient`, build its
   `AdyenBinLookupApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
