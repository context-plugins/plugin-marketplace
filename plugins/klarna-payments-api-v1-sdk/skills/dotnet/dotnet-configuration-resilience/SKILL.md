...
All types below live under `KlarnaPaymentsApiV1.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using KlarnaPaymentsApiV1.Core.Configuration;
...
```csharp
using KlarnaPaymentsApiV1.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new KlarnaPaymentsApiV1ClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `KlarnaPaymentsApiV1.Core.Exceptions`):
...
With DI, the SDK's `AddKlarnaPaymentsApiV1Client` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddKlarnaPaymentsApiV1Client(options => { /* ... */ });   // resolves CreateClient() → the default client
```
