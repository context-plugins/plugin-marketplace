...
- `eventsapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/events-api-go` — the Go import path for the root package; `github.com/context-plugins/events-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/events-api-go"

client := eventsapi.NewClient(
    eventsapi.CreateConfiguration(
        eventsapi.WithEnvironment(eventsapi.PRODUCTION),
...
from environment variables named `EVENTSAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := eventsapi.NewClient(eventsapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `eventsapi.PRODUCTION`,
`eventsapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := eventsapi.NewClient(
    eventsapi.CreateConfiguration(eventsapi.WithEnvironment(eventsapi.PRODUCTION)),
)
```
...
client := eventsapi.NewClient(
    eventsapi.CreateConfiguration(
        eventsapi.WithHttpConfiguration(
            eventsapi.CreateHttpConfiguration(
                eventsapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                eventsapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = eventsapi.NewClient(eventsapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api eventsapi.ClientInterface
}

func NewService(api eventsapi.ClientInterface) *Service {
