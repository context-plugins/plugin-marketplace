...
- `AdyenDataProtectionAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.adyen.catest` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`AdyenDataProtectionAPIClient.Builder` class. A minimal initialization:

```java
import com.adyen.catest.AdyenDataProtectionAPIClient;
import com.adyen.catest.Environment;

AdyenDataProtectionAPIClient client = new AdyenDataProtectionAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `AdyenDataProtectionAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `AdyenDataProtectionAPIClient.java` |

Call `.build()` to produce an immutable `AdyenDataProtectionAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public AdyenDataProtectionAPIClient apiClient() {
        return new AdyenDataProtectionAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired AdyenDataProtectionAPIClient client) {
        client.shutdown();
    }
}
```

Inject `AdyenDataProtectionAPIClient` into your services as a normal Spring dependency. Build only one bean.
