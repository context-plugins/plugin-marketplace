...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `CellpointapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`cellpoint/configuration.py` in the cloned source.
...
```python
import requests
from cellpoint.configuration import Configuration
...
`cellpoint/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`cellpoint/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `cellpoint/configuration.py`
...
`NumberPagedResponse`) live in `cellpoint/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `cellpoint/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
