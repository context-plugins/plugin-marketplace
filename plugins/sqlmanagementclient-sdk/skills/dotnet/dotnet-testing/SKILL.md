...
`SqlManagementClientClient`/`SqlManagementClientClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `SqlManagementClientClient`,
...
static SqlManagementClientClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new SqlManagementClientClient(new HttpClient(handler), new SqlManagementClientClientOptions { /* auth not needed for stubs */ });
}
```
...
using SqlManagementClient.Errors;              // {Operation}Error types
...
```csharp
using SqlManagementClient.Core.Exceptions;
using SqlManagementClient.Core.ErrorResponse;
...
using SqlManagementClient.Core.ErrorResponse; // RawError (Case B)
...
var client = new SqlManagementClientClient(new HttpClient(handler), new SqlManagementClientClientOptions());
...
- For DI-based code, the SDK's `AddSqlManagementClientClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `SqlManagementClientClient` from the provider:
  ```csharp
  services.AddSqlManagementClientClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<SqlManagementClientClient>();
  ```
