...
`testing` + `testify/require` for reference; substitute the real `ipgeolocationapi` and names.
...
import (
    ...
    "github.com/context-plugins/ip-geolocation-api-go"   // root package ipgeolocationapi
)
...
func clientReturning(status int, body string) (ipgeolocationapi.ClientInterface, *stubTransport) {
    ...
    client := ipgeolocationapi.NewClient(
        ipgeolocationapi.CreateConfiguration(
            ipgeolocationapi.WithHttpConfiguration(
                ipgeolocationapi.CreateHttpConfiguration(ipgeolocationapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/ip-geolocation-api-go/errors"
...
client := ipgeolocationapi.NewClient(
    ipgeolocationapi.CreateConfiguration(
        ipgeolocationapi.WithHttpConfiguration(
            ipgeolocationapi.CreateHttpConfiguration(ipgeolocationapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
