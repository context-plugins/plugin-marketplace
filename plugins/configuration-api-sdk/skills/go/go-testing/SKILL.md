...
`testing` + `testify/require` for reference; substitute the real `configurationapi` and names.
...
import (
    ...
    "github.com/context-plugins/configuration-api-go"   // root package configurationapi
)
...
func clientReturning(status int, body string) (configurationapi.ClientInterface, *stubTransport) {
    ...
    client := configurationapi.NewClient(
        configurationapi.CreateConfiguration(
            configurationapi.WithHttpConfiguration(
                configurationapi.CreateHttpConfiguration(configurationapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/configuration-api-go/errors"
...
client := configurationapi.NewClient(
    configurationapi.CreateConfiguration(
        configurationapi.WithHttpConfiguration(
            configurationapi.CreateHttpConfiguration(configurationapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
