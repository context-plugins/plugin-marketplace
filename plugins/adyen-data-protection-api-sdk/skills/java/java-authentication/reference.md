...
against the `AdyenDataProtectionAPIClient.Builder` setters, the `com.adyen.catest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AdyenDataProtectionAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AdyenDataProtectionAPIClient client = new AdyenDataProtectionAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAdyenDataProtectionAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new AdyenDataProtectionAPIClient.Builder()
