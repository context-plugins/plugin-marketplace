...
Open `payquicker/payquicker_client.py` for the controller property names, then the relevant
`payquicker/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from payquicker.models.{request_model} import {RequestModel}
...
Open `payquicker/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from payquicker.models.suite_code_enum import SuiteCodeEnum
...
```python
from payquicker.http.http_call_back import HttpCallBack
...
client = Pqapiv2Client(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `payquicker/pagination/` in the source.

## Finding the right method

- Controller property names are on `Pqapiv2Client` in `payquicker/payquicker_client.py`.
- Operation method signatures are in `payquicker/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `payquicker/models/`; enum types have `from_value()`.
