...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new SubscriptionClientClient({
...
```typescript
import { {Operation}Error } from 'subscriptionclientlib/errors';
...
```typescript
import { ApiError } from 'subscriptionclientlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(SubscriptionClientClient)
    .useValue(new SubscriptionClientClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
