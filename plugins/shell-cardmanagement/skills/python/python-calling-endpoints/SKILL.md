...
Open `shellcardmanagement/shellcardmanagement_client.py` for the controller property names, then the relevant
`shellcardmanagement/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from shellcardmanagement.models.{request_model} import {RequestModel}
...
Open `shellcardmanagement/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from shellcardmanagement.models.suite_code_enum import SuiteCodeEnum
...
```python
from shellcardmanagement.http.http_call_back import HttpCallBack
...
client = ShellcardmanagementapisClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `shellcardmanagement/pagination/` in the source.

## Finding the right method

- Controller property names are on `ShellcardmanagementapisClient` in `shellcardmanagement/shellcardmanagement_client.py`.
- Operation method signatures are in `shellcardmanagement/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `shellcardmanagement/models/`; enum types have `from_value()`.
