...
`googleworkspace/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `googleworkspace/exceptions/` and operation docstrings.
...
```python
from googleworkspace.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `googleworkspace/exceptions/`.
...
```python
from googleworkspace.exceptions.api_exception import APIException
from googleworkspace.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`googleworkspace/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`googleworkspace/exceptions/` for the class.
...
`googleworkspace/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `googleworkspace/exceptions/`
  or a `ValueError` — check the auth handler in `googleworkspace/http/auth/` for the exact type.
