...
| --- | --- |
| API | `Notion` |
| Project | `Notion.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `Notion` (the `using` namespace) |
| Client class | `NotionClient` |
| Options class | `NotionClientOptions` |
...
namespaces transitively, so `using Notion.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/notion-csharp-sdk
dotnet add reference notion-csharp-sdk/Notion.csproj
```

```csharp
using Notion;          // client + options + ServerEnvironment
using Notion.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/notion-csharp-sdk /tmp/notion-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/notion-csharp-sdk "$env:TEMP\notion-dotnet-src"
```
...
- `NotionClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/notion-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\notion-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `NotionClient`, build its
   `NotionClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
