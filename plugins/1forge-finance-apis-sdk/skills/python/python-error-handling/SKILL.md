...
`1forgefinanceapis/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `1forgefinanceapis/exceptions/` and operation docstrings.
...
```python
from 1forgefinanceapis.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `1forgefinanceapis/exceptions/`.
...
```python
from 1forgefinanceapis.exceptions.api_exception import APIException
from 1forgefinanceapis.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`1forgefinanceapis/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`1forgefinanceapis/exceptions/` for the class.
...
`1forgefinanceapis/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `1forgefinanceapis/exceptions/`
  or a `ValueError` — check the auth handler in `1forgefinanceapis/http/auth/` for the exact type.
