...
against the `AkoyaClient.Builder` setters, the `com.akoya.ddp.sandboxproducts.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AkoyaClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AkoyaClient client = new AkoyaClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAkoyaEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
