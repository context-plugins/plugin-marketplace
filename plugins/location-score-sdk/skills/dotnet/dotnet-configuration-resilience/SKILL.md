...
All types below live under `LocationScore.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using LocationScore.Core.Configuration;
...
```csharp
using LocationScore.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new LocationScoreClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `LocationScore.Core.Exceptions`):
...
With DI, the SDK's `AddLocationScoreClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddLocationScoreClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
