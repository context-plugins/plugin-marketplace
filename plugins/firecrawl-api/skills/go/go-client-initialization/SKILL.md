...
- `firecrawlapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/firecrawl-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/firecrawl-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/firecrawl-api-go-sdk"

client := firecrawlapi.NewClient(
    firecrawlapi.CreateConfiguration(
        firecrawlapi.WithEnvironment(firecrawlapi.PRODUCTION),
...
from environment variables named `FIRECRAWLAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := firecrawlapi.NewClient(firecrawlapi.CreateConfigurationFromEnvironment())
```
...
```go
client := firecrawlapi.NewClient(
    firecrawlapi.CreateConfiguration(firecrawlapi.WithEnvironment(firecrawlapi.PRODUCTION)),
)
```
...
client := firecrawlapi.NewClient(
    firecrawlapi.CreateConfiguration(
        firecrawlapi.WithHttpConfiguration(
            firecrawlapi.CreateHttpConfiguration(
                firecrawlapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                firecrawlapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = firecrawlapi.NewClient(firecrawlapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api firecrawlapi.ClientInterface
}

func NewService(api firecrawlapi.ClientInterface) *Service {
