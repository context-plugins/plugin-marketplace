...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new SeatmapDisplayClient({
...
```typescript
import { {Operation}Error } from 'seatmap-displaylib/errors';
...
```typescript
import { ApiError } from 'seatmap-displaylib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(SeatmapDisplayClient)
    .useValue(new SeatmapDisplayClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
