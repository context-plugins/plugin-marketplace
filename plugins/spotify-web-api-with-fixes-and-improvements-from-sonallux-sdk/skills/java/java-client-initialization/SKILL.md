...
description: Construct and configure an APIMatic-generated Java SDK client — build via the inner SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder() class (never a public constructor), set httpClientConfig via a lambda (not a config object), choose an Environment constant, access controllers via client.get{Resource}Controller(), call client.shutdown() at process exit, clone a live client with client.newBuilder(), and wire the client into Spring or plain-constructor DI. Use the moment you call new SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder(), pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the arguments but not the builder pattern, the httpClientConfig lambda, or the lifetime/reuse rules.
---
...
- `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.spotify.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder` class. A minimal initialization:
...
The `Builder` is the only entry point — confirm its fluent methods in `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.java` |

Call `.build()` to produce an immutable `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient` instance.
...
public class ApiConfig {
    @Bean
    public SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient apiClient() {
        return new SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient client) {
        client.shutdown();
    }
}
```

Inject `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient` into your services as a normal Spring dependency. Build only one bean.
