...
> Throughout, `flightoffersprice` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := flightoffersprice.NewClient(
    flightoffersprice.CreateConfiguration(
        flightoffersprice.WithBasicAuthCredentials(
            flightoffersprice.NewBasicAuthCredentials(
...
flightoffersprice.WithApiKeyCredentials(flightoffersprice.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
flightoffersprice.WithApiHeaderCredentials(flightoffersprice.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
flightoffersprice.WithOAuthBearerTokenCredentials(
    flightoffersprice.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := flightoffersprice.NewClient(
    flightoffersprice.CreateConfiguration(
        flightoffersprice.WithOAuthCCGCredentials(
            flightoffersprice.NewOAuthCCGCredentials(
