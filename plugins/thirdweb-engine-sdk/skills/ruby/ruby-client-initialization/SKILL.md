...
- `ThirdwebEngine` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `thirdweb_engine` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'thirdweb_engine'
include ThirdwebEngine
...
The authoritative list is the `initialize` signature in `lib/thirdweb_engine/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/thirdweb_engine/configuration.rb`
(`build_default_config_from_env`) and `lib/thirdweb_engine/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/thirdweb_engine.rb`) and store it in a
