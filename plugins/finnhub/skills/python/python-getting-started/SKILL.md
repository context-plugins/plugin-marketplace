...
description: "Finnhub Python SDK identity and lookup layer (Python only) — install, import root, base URL/environments, the auth pattern, the SDK map that ships at the SDK root (`sdk-map.md` + `map/operations/`) and how to traverse it, and the module table naming the one file owning each fact the map leaves to the source. Load this before answering any Finnhub Python SDK contract question or writing any SDK code."
---

# Getting started with the Finnhub Python SDK
...
Verified against `finnhub/` and `pyproject.toml` of the generated package at version `1.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Finnhub |
| Distribution name (what you install) | `finnhub` — **not on any package index**; installed from source (see *Install*) |
| Import root (what you import) | `finnhub` — note the underscores; the two names differ |
| Source repository | https://github.com/context-plugins/finnhub-python-sdk |
| Source branch | `main` |
| Version | `1.0.0` |
| Sync client class | `FinnhubClient` (alias `Client`) |
| Async client class | `AsyncFinnhubClient` (alias `AsyncClient`) |
...
| Base-URL config | `ServerConfig` (`finnhub/server/server_config.py`), frozen, `extra="forbid"` |
...
| Surface | 117 operations across 0 controllers · 192 models · 0 enums · 0 per-operation error unions |
...
```bash
pip install "finnhub @ git+https://github.com/context-plugins/finnhub-python-sdk.git@main"
```
...
Python does not re-export child modules transitively, so `from finnhub import models` alone does **not** make enums, error unions, or runtime types reachable. Import each kind of type from the module that owns it.

`finnhub/__init__.py` exports exactly 5 names:
...
| --- | --- |
| Domain models, their `…Dict` companions | `finnhub.models` |
| Enums (and their open `…OrStr` aliases) | `finnhub.models.enums` |
| `ApiError` · `RawError` · `ApiResult` · `RequestOptions` · `HttpClient` · `SdkBaseModel` · `UNSET` · `Optional` | `finnhub.core` |
| Per-operation error *unions* | `finnhub.errors` — *this SDK declares none* |

`finnhub.core` re-exports its whole public surface (a curated `__all__`), so import from `…core` rather than from the private modules beneath it (`…core.results`, `…core.exceptions`, `…core.auth.schemes`).

## Environments — there is no environment enum

This SDK has **no environment type and no environment constants**. There is one knob: `base_url`, on `ServerConfig` (`finnhub/server/server_config.py`), and its default is **`https://finnhub.io/api/v1`**:
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `finnhub/` source directory, and these two entries:
...
Every `Source` path on the map is relative to that SDK root, so `finnhub/models/aichat_body.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
```bash
python -c "import finnhub, pathlib; print(pathlib.Path(finnhub.__file__).parent)"
```

Failing that, it is under the project's environment (`.venv/Lib/site-packages/finnhub` on Windows, `.venv/lib/python3.*/site-packages/finnhub` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
...
3. **The 1 operation that returns `None`** — `global_filings_download`. Its raw peer is `ApiResult[None, …]`, so `with_raw_response` is the only way to observe the status code.
