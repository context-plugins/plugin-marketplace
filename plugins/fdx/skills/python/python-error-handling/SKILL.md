...
`fdx/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `fdx/exceptions/` and operation docstrings.
...
```python
from fdx.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `fdx/exceptions/`.
...
```python
from fdx.exceptions.api_exception import APIException
from fdx.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`fdx/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`fdx/exceptions/` for the class.
...
`fdx/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `fdx/exceptions/`
  or a `ValueError` — check the auth handler in `fdx/http/auth/` for the exact type.
