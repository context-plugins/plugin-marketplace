...
kwargs to the `LocationScoreClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `locationscore/locationscore_client.py` and `locationscore/http/auth/` in
the cloned source.

## Basic auth

```python
from locationscore.http.auth.basic_auth import BasicAuthCredentials
...
```python
from locationscore.http.auth.api_key import ApiKeyCredentials
...
```python
from locationscore.http.auth.api_header import ApiHeaderCredentials
...
```python
from locationscore.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from locationscore.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from locationscore.http.auth.o_auth_acg import OAuthACGCredentials
from locationscore.models.o_auth_scope_locationscore_enum import OAuthScope{Pkg}Enum
...
client = LocationScoreClient(o_auth_acg_credentials=creds)
...
```python
from locationscore.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = LocationScoreClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `LocationScoreClient.from_environment()`) read each credential
...
Grep `from_environment` in `locationscore/configuration.py` and each `locationscore/http/auth/*.py` for the exact
