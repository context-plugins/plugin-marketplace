...
> `{ApiGroup}`, `KlarnaPaymentsApiV1`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `KlarnaPaymentsApiV1.Core.Exceptions`
- `ApiError` **and** `RawError` → `KlarnaPaymentsApiV1.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `KlarnaPaymentsApiV1.Errors`
...
```csharp
using KlarnaPaymentsApiV1.Core.Exceptions;     // SdkException<TError>
using KlarnaPaymentsApiV1.Core.ErrorResponse;  // ApiError, RawError
using KlarnaPaymentsApiV1.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`KlarnaPaymentsApiV1.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using KlarnaPaymentsApiV1.Core.Exceptions;     // SdkException<TError>
using KlarnaPaymentsApiV1.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `KlarnaPaymentsApiV1.Core.ErrorResponse`. Its public
...
`readonly struct` under `KlarnaPaymentsApiV1.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using KlarnaPaymentsApiV1.Core.Models;        // ApiResult<TResponse, TError>
using KlarnaPaymentsApiV1.Core.ErrorResponse; // RawError
using KlarnaPaymentsApiV1.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `KlarnaPaymentsApiV1.Core.Exceptions`) — an auth *application* failure, not an API error — when the
