...
kwargs to the `PaypalClient` constructor (or to `Configuration`). Exact credential class names and
...
client = PaypalClient(o_auth_acg_credentials=creds)
...
client = PaypalClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `PaypalClient.from_environment()`) read each credential
