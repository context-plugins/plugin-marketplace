    ...
    giteaapi.CreateConfiguration(
        giteaapi.WithHttpConfiguration(
            giteaapi.CreateHttpConfiguration(
                giteaapi.WithTimeout(30),
                giteaapi.WithRetryConfiguration(
                    giteaapi.CreateRetryConfiguration(
                        giteaapi.WithMaxRetryAttempts(3),
                        giteaapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := giteaapi.CreateRetryConfiguration(
    giteaapi.WithMaxRetryAttempts(5),
    giteaapi.WithRetryInterval(1),
    giteaapi.WithBackoffFactor(2),
    giteaapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    giteaapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := giteaapi.NewClient(
    giteaapi.CreateConfiguration(
        giteaapi.WithHttpConfiguration(
            giteaapi.CreateHttpConfiguration(
                giteaapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
