...
- `shellapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/shell-api-go` — the Go import path for the root package; `github.com/context-plugins/shell-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/shell-api-go"

client := shellapi.NewClient(
    shellapi.CreateConfiguration(
        shellapi.WithEnvironment(shellapi.PRODUCTION),
...
from environment variables named `SHELLAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := shellapi.NewClient(shellapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `shellapi.PRODUCTION`,
`shellapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := shellapi.NewClient(
    shellapi.CreateConfiguration(shellapi.WithEnvironment(shellapi.PRODUCTION)),
)
```
...
client := shellapi.NewClient(
    shellapi.CreateConfiguration(
        shellapi.WithHttpConfiguration(
            shellapi.CreateHttpConfiguration(
                shellapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                shellapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = shellapi.NewClient(shellapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api shellapi.ClientInterface
}

func NewService(api shellapi.ClientInterface) *Service {
