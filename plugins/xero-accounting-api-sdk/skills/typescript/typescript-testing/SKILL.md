...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new XeroAccountingAPIClient({
...
```typescript
import { {Operation}Error } from 'xero-accounting-apilib/errors';
...
```typescript
import { ApiError } from 'xero-accounting-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(XeroAccountingAPIClient)
    .useValue(new XeroAccountingAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
