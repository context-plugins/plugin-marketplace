...
kwarg names are generated per-API — confirm from `gitea/gitea_client.py` and `gitea/http/auth/` in
the cloned source.
...
```python
from gitea.http.auth.api_key import ApiKeyCredentials
...
```python
from gitea.http.auth.api_header import ApiHeaderCredentials
...
```python
from gitea.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from gitea.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from gitea.http.auth.o_auth_acg import OAuthACGCredentials
from gitea.models.o_auth_scope_gitea_enum import OAuthScope{Pkg}Enum
...
```python
from gitea.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `gitea/configuration.py` and each `gitea/http/auth/*.py` for the exact
