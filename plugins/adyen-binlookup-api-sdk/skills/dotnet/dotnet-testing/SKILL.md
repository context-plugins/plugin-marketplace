...
`AdyenBinLookupApiClient`/`AdyenBinLookupApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `AdyenBinLookupApiClient`,
...
static AdyenBinLookupApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new AdyenBinLookupApiClient(new HttpClient(handler), new AdyenBinLookupApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using AdyenBinLookupApi.Errors;              // {Operation}Error types
...
```csharp
using AdyenBinLookupApi.Core.Exceptions;
using AdyenBinLookupApi.Core.ErrorResponse;
...
using AdyenBinLookupApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new AdyenBinLookupApiClient(new HttpClient(handler), new AdyenBinLookupApiClientOptions());
...
- For DI-based code, the SDK's `AddAdyenBinLookupApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `AdyenBinLookupApiClient` from the provider:
  ```csharp
  services.AddAdyenBinLookupApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<AdyenBinLookupApiClient>();
  ```
