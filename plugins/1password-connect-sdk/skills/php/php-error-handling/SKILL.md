...
> `{operation}`, `{apiGroup}`, `M1PasswordConnectLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `M1PasswordConnectLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `M1PasswordConnectLib\Exceptions\` — read the source to
