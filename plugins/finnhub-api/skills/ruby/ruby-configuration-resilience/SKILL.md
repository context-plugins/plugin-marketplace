...
```ruby
client = FinnhubAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = FinnhubAPI::Client.new(adapter: :net_http)
```
...
client = FinnhubAPI::Client.new(connection: conn)
```
...
client = FinnhubAPI::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = FinnhubAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = FinnhubAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = FinnhubAPI::ProxySettings.from_env
client = FinnhubAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < FinnhubAPI::HttpCallBack
...
client = FinnhubAPI::Client.new(http_callback: LogCallback.new)
```
...
client = FinnhubAPI::Client.from_env(timeout: 30, max_retries: 3)
```
