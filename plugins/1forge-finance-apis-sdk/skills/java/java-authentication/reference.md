...
against the `1ForgeFinanceAPIsClient.Builder` setters, the `com.m1forge.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `1ForgeFinanceAPIsClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
1ForgeFinanceAPIsClient client = new 1ForgeFinanceAPIsClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScope1ForgeFinanceAPIsEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new 1ForgeFinanceAPIsClient.Builder()
