...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AuthentiqAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`authentiqapi/configuration.py` in the cloned source.
...
```python
import requests
from authentiqapi.configuration import Configuration
...
`authentiqapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`authentiqapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `authentiqapi/configuration.py`
...
`NumberPagedResponse`) live in `authentiqapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `authentiqapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
