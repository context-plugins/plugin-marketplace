...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new ArticleSearchAPIClient({
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(ArticleSearchAPIClient)
    .useValue(new ArticleSearchAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
