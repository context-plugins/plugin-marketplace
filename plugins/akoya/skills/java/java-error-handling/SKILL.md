...
│   └── com.akoya.ddp.sandboxproducts.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.akoya.ddp.sandboxproducts.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.akoya.ddp.sandboxproducts.http.client.HttpContext`, which carries:
