...
```ruby
client = XAPIV2::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = XAPIV2::Client.new(adapter: :net_http)
```
...
client = XAPIV2::Client.new(connection: conn)
```
...
client = XAPIV2::Client.new(environment: Environment::PRODUCTION)
client = XAPIV2::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = XAPIV2::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = XAPIV2::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = XAPIV2::ProxySettings.from_env
client = XAPIV2::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < XAPIV2::HttpCallBack
...
client = XAPIV2::Client.new(http_callback: LogCallback.new)
```
...
client = XAPIV2::Client.from_env(timeout: 30, max_retries: 3)
```
