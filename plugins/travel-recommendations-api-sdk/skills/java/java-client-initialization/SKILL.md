...
- `TravelRecommendationsAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.amadeus.api.test` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`TravelRecommendationsAPIClient.Builder` class. A minimal initialization:

```java
import com.amadeus.api.test.TravelRecommendationsAPIClient;
import com.amadeus.api.test.Environment;

TravelRecommendationsAPIClient client = new TravelRecommendationsAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `TravelRecommendationsAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `TravelRecommendationsAPIClient.java` |

Call `.build()` to produce an immutable `TravelRecommendationsAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public TravelRecommendationsAPIClient apiClient() {
        return new TravelRecommendationsAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired TravelRecommendationsAPIClient client) {
        client.shutdown();
    }
}
```

Inject `TravelRecommendationsAPIClient` into your services as a normal Spring dependency. Build only one bean.
