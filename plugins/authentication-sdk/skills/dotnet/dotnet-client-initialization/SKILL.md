...
- `AuthenticationClientOptions` — its options class.
- `Authentication` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public AuthenticationClient(HttpClient httpClient, AuthenticationClientOptions options)
```
...
var client = new AuthenticationClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddAuthenticationClient`, which registers the
...
builder.Services.AddAuthenticationClient(options =>
{
...
```csharp
public sealed class MyService(AuthenticationClient client)
{
