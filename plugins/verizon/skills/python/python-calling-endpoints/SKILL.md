...
Open `verizon/verizon_client.py` for the controller property names, then the relevant
`verizon/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from verizon.models.{request_model} import {RequestModel}
...
Open `verizon/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from verizon.models.suite_code_enum import SuiteCodeEnum
...
```python
from verizon.http.http_call_back import HttpCallBack
...
client = VerizonClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `verizon/pagination/` in the source.

## Finding the right method

- Controller property names are on `VerizonClient` in `verizon/verizon_client.py`.
- Operation method signatures are in `verizon/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `verizon/models/`; enum types have `from_value()`.
