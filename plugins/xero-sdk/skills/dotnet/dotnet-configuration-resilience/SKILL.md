...
All types below live under `Xero.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using Xero.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new XeroClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `Xero.Core.Exceptions`):
...
With DI, the SDK's `AddXeroClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddXeroClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
