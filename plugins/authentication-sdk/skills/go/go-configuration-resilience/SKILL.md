    ...
    authentication.CreateConfiguration(
        authentication.WithHttpConfiguration(
            authentication.CreateHttpConfiguration(
                authentication.WithTimeout(30),
                authentication.WithRetryConfiguration(
                    authentication.CreateRetryConfiguration(
                        authentication.WithMaxRetryAttempts(3),
                        authentication.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := authentication.CreateRetryConfiguration(
    authentication.WithMaxRetryAttempts(5),
    authentication.WithRetryInterval(1),
    authentication.WithBackoffFactor(2),
    authentication.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    authentication.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := authentication.NewClient(
    authentication.CreateConfiguration(
        authentication.WithHttpConfiguration(
            authentication.CreateHttpConfiguration(
                authentication.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
