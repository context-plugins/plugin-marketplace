    ...
    verizonapi.CreateConfiguration(
        verizonapi.WithHttpConfiguration(
            verizonapi.CreateHttpConfiguration(
                verizonapi.WithTimeout(30),
                verizonapi.WithRetryConfiguration(
                    verizonapi.CreateRetryConfiguration(
                        verizonapi.WithMaxRetryAttempts(3),
                        verizonapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := verizonapi.CreateRetryConfiguration(
    verizonapi.WithMaxRetryAttempts(5),
    verizonapi.WithRetryInterval(1),
    verizonapi.WithBackoffFactor(2),
    verizonapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    verizonapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := verizonapi.NewClient(
    verizonapi.CreateConfiguration(
        verizonapi.WithHttpConfiguration(
            verizonapi.CreateHttpConfiguration(
                verizonapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
