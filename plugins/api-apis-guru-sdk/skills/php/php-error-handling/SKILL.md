...
> `{operation}`, `{apiGroup}`, `PointsOfInterestLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `PointsOfInterestLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use PointsOfInterestLib\Exceptions\ApiException;
...
```php
use PointsOfInterestLib\Exceptions\ApiException;
use PointsOfInterestLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `PointsOfInterestLib\Exceptions\` — read the source to
