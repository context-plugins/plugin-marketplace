...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `firecrawl-apilib`) — replace it with the concrete identifier from the source.
...
import { ApiError } from 'firecrawl-apilib';
import { {Operation}Error } from 'firecrawl-apilib/errors';
