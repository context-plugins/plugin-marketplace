...
Confirm every name against `lib/hotel_search_api/client.rb`, `lib/hotel_search_api/configuration.rb`, the
`lib/hotel_search_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
