...
`cellpoint/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `cellpoint/exceptions/` and operation docstrings.
...
```python
from cellpoint.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `cellpoint/exceptions/`.
...
```python
from cellpoint.exceptions.api_exception import APIException
from cellpoint.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`cellpoint/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`cellpoint/exceptions/` for the class.
...
`cellpoint/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `cellpoint/exceptions/`
  or a `ValueError` — check the auth handler in `cellpoint/http/auth/` for the exact type.
