...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ShellcardmanagementapisClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`shellcardmanagement/configuration.py` in the cloned source.
...
```python
import requests
from shellcardmanagement.configuration import Configuration
...
`shellcardmanagement/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`shellcardmanagement/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `shellcardmanagement/configuration.py`
...
`NumberPagedResponse`) live in `shellcardmanagement/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `shellcardmanagement/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
