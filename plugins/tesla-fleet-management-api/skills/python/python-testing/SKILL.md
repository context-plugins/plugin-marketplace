...
reference; substitute your `teslafleetmanagementapi` and `TeslafleetmanagementapiClient` names.
...
- **Get the stub URL from the SDK, not from this page.** Every environment resolves to a real remote
  host — read the `environments` dict in `teslafleetmanagementapi/configuration.py`, or call
