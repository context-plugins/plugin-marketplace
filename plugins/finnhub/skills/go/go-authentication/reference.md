...
```go
finnhub.WithOAuthCCGCredentials(
    finnhub.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr finnhub.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := finnhub.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr finnhub.OAuthCCGManager) models.OAuthToken {
...
```go
creds := finnhub.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := finnhub.NewClient(finnhub.CreateConfiguration(finnhub.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    finnhub.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := finnhub.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := finnhub.NewClient(finnhub.CreateConfiguration(finnhub.WithOAuthROPCGCredentials(creds)))
...
```go
finnhub.WithOAuthBearerTokenCredentials(finnhub.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
finnhub.WithBasicAuthCredentials(finnhub.NewBasicAuthCredentials(username, password))
finnhub.WithApiHeaderCredentials(finnhub.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
finnhub.WithApiKeyCredentials(finnhub.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FINNHUB_...` variables — e.g.
`FINNHUB_USERNAME`, `FINNHUB_PASSWORD`, `FINNHUB_TOKEN`, `FINNHUB_API_KEY`,
`FINNHUB_O_AUTH_CLIENT_ID`, `FINNHUB_O_AUTH_CLIENT_SECRET`, `FINNHUB_ACCESS_TOKEN`. Grep the
