...
- `ShutterstockAPIExplorer` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `shutterstock_api_explorer` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'shutterstock_api_explorer'
include ShutterstockAPIExplorer
...
The authoritative list is the `initialize` signature in `lib/shutterstock_api_explorer/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/shutterstock_api_explorer/configuration.rb`
(`build_default_config_from_env`) and `lib/shutterstock_api_explorer/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/shutterstock_api_explorer.rb`) and store it in a
