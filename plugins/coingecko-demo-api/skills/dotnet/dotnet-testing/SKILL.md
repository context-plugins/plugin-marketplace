...
`CoinGeckoDemoApiClient`/`CoinGeckoDemoApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `CoinGeckoDemoApiClient`,
...
static CoinGeckoDemoApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new CoinGeckoDemoApiClient(new HttpClient(handler), new CoinGeckoDemoApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using CoinGeckoDemoApi.Errors;              // {Operation}Error types
...
using CoinGeckoDemoApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new CoinGeckoDemoApiClient(new HttpClient(handler), new CoinGeckoDemoApiClientOptions());
...
- For DI-based code, the SDK's `AddCoinGeckoDemoApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `CoinGeckoDemoApiClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<CoinGeckoDemoApiClient>();
  ```
