...
- `stax-fattmerchant-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'stax-fattmerchant-apilib';
...
```ts
import { {Resource}Controller } from 'stax-fattmerchant-apilib';
