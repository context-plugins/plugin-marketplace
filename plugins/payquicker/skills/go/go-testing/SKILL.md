...
`testing` + `testify/require` for reference; substitute the real `payquicker` and names.
...
import (
    ...
    "github.com/context-plugins/payquicker-go-sdk"   // root package payquicker
)
...
func clientReturning(status int, body string) (payquicker.ClientInterface, *stubTransport) {
    ...
    client := payquicker.NewClient(
        payquicker.CreateConfiguration(
            payquicker.WithHttpConfiguration(
                payquicker.CreateHttpConfiguration(payquicker.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/payquicker-go-sdk/errors"
...
client := payquicker.NewClient(
    payquicker.CreateConfiguration(
        payquicker.WithHttpConfiguration(
            payquicker.CreateHttpConfiguration(payquicker.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
