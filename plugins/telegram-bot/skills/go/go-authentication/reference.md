...
```go
telegrambotapi.WithOAuthCCGCredentials(
    telegrambotapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr telegrambotapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := telegrambotapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr telegrambotapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := telegrambotapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := telegrambotapi.NewClient(telegrambotapi.CreateConfiguration(telegrambotapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    telegrambotapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := telegrambotapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := telegrambotapi.NewClient(telegrambotapi.CreateConfiguration(telegrambotapi.WithOAuthROPCGCredentials(creds)))
...
```go
telegrambotapi.WithOAuthBearerTokenCredentials(telegrambotapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
telegrambotapi.WithBasicAuthCredentials(telegrambotapi.NewBasicAuthCredentials(username, password))
telegrambotapi.WithApiHeaderCredentials(telegrambotapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
telegrambotapi.WithApiKeyCredentials(telegrambotapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `TELEGRAMBOTAPI_...` variables — e.g.
`TELEGRAMBOTAPI_USERNAME`, `TELEGRAMBOTAPI_PASSWORD`, `TELEGRAMBOTAPI_TOKEN`, `TELEGRAMBOTAPI_API_KEY`,
`TELEGRAMBOTAPI_O_AUTH_CLIENT_ID`, `TELEGRAMBOTAPI_O_AUTH_CLIENT_SECRET`, `TELEGRAMBOTAPI_ACCESS_TOKEN`. Grep the
