...
kwargs to the `KlarnaPaymentsAPIV1Client` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `klarnapaymentsapiv1/klarnapaymentsapiv1_client.py` and `klarnapaymentsapiv1/http/auth/` in
the cloned source.

## Basic auth

```python
from klarnapaymentsapiv1.http.auth.basic_auth import BasicAuthCredentials
...
```python
from klarnapaymentsapiv1.http.auth.api_key import ApiKeyCredentials
...
```python
from klarnapaymentsapiv1.http.auth.api_header import ApiHeaderCredentials
...
```python
from klarnapaymentsapiv1.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from klarnapaymentsapiv1.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from klarnapaymentsapiv1.http.auth.o_auth_acg import OAuthACGCredentials
from klarnapaymentsapiv1.models.o_auth_scope_klarnapaymentsapiv1_enum import OAuthScope{Pkg}Enum
...
client = KlarnaPaymentsAPIV1Client(o_auth_acg_credentials=creds)
...
```python
from klarnapaymentsapiv1.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = KlarnaPaymentsAPIV1Client(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `KlarnaPaymentsAPIV1Client.from_environment()`) read each credential
...
Grep `from_environment` in `klarnapaymentsapiv1/configuration.py` and each `klarnapaymentsapiv1/http/auth/*.py` for the exact
