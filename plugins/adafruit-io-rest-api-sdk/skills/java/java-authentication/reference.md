...
against the `AdafruitIORESTAPIClient.Builder` setters, the `com.adafruit.io.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AdafruitIORESTAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AdafruitIORESTAPIClient client = new AdafruitIORESTAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAdafruitIORESTAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new AdafruitIORESTAPIClient.Builder()
