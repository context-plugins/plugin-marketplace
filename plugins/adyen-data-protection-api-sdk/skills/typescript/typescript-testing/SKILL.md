...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new AdyenDataProtectionAPIClient({
...
```typescript
import { {Operation}Error } from 'adyen-data-protection-apilib/errors';
...
```typescript
import { ApiError } from 'adyen-data-protection-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AdyenDataProtectionAPIClient)
    .useValue(new AdyenDataProtectionAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
