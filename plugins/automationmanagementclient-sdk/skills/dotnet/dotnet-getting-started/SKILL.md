...
| Project | `AutomationManagementClient.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `AutomationManagementClient` (the `using` namespace) |
| Client class | `AutomationManagementClientClient` |
| Options class | `AutomationManagementClientClientOptions` |
...
namespaces transitively, so `using AutomationManagementClient.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/automationmanagementclient-csharp
dotnet add reference automationmanagementclient-csharp/AutomationManagementClient.csproj
```

```csharp
using AutomationManagementClient;          // client + options + ServerEnvironment
using AutomationManagementClient.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/automationmanagementclient-csharp /tmp/automationmanagementclient-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/automationmanagementclient-csharp "$env:TEMP\automationmanagementclient-dotnet-src"
```
...
- `AutomationManagementClientClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/automationmanagementclient-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\automationmanagementclient-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AutomationManagementClientClient`, build its
   `AutomationManagementClientClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
