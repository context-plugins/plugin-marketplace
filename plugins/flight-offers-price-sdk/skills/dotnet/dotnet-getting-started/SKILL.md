...
| Project | `FlightOffersPrice.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `FlightOffersPrice` (the `using` namespace) |
| Client class | `FlightOffersPriceClient` |
| Options class | `FlightOffersPriceClientOptions` |
...
namespaces transitively, so `using FlightOffersPrice.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/flight-offers-price-csharp
dotnet add reference flight-offers-price-csharp/FlightOffersPrice.csproj
```

```csharp
using FlightOffersPrice;          // client + options + ServerEnvironment
using FlightOffersPrice.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/flight-offers-price-csharp /tmp/flight-offers-price-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/flight-offers-price-csharp "$env:TEMP\flight-offers-price-dotnet-src"
```
...
- `FlightOffersPriceClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/flight-offers-price-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\flight-offers-price-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `FlightOffersPriceClient`, build its
   `FlightOffersPriceClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
