...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AdyenDataProtectionAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`adyendataprotectionapi/configuration.py` in the cloned source.
...
```python
import requests
from adyendataprotectionapi.configuration import Configuration
...
`adyendataprotectionapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`adyendataprotectionapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `adyendataprotectionapi/configuration.py`
...
`NumberPagedResponse`) live in `adyendataprotectionapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `adyendataprotectionapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
