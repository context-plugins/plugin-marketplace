...
> Throughout, `pokapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := pokapi.NewClient(
    pokapi.CreateConfiguration(
        pokapi.WithBasicAuthCredentials(
            pokapi.NewBasicAuthCredentials(
...
pokapi.WithApiKeyCredentials(pokapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
pokapi.WithApiHeaderCredentials(pokapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
pokapi.WithOAuthBearerTokenCredentials(
    pokapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
