...
- `1passwordconnect` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/1password-connect-go` — the Go import path for the root package; `github.com/context-plugins/1password-connect-go/models` for request/response types.
...
```go
import "github.com/context-plugins/1password-connect-go"

client := 1passwordconnect.NewClient(
    1passwordconnect.CreateConfiguration(
        1passwordconnect.WithEnvironment(1passwordconnect.PRODUCTION),
...
from environment variables named `1PASSWORDCONNECT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := 1passwordconnect.NewClient(1passwordconnect.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `1passwordconnect.PRODUCTION`,
`1passwordconnect.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := 1passwordconnect.NewClient(
    1passwordconnect.CreateConfiguration(1passwordconnect.WithEnvironment(1passwordconnect.PRODUCTION)),
)
```
...
client := 1passwordconnect.NewClient(
    1passwordconnect.CreateConfiguration(
        1passwordconnect.WithHttpConfiguration(
            1passwordconnect.CreateHttpConfiguration(
                1passwordconnect.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                1passwordconnect.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = 1passwordconnect.NewClient(1passwordconnect.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api 1passwordconnect.ClientInterface
}

func NewService(api 1passwordconnect.ClientInterface) *Service {
