...
All types below live under `PointsOfInterest.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using PointsOfInterest.Core.Configuration;
...
```csharp
using PointsOfInterest.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new PointsOfInterestClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `PointsOfInterest.Core.Exceptions`):
...
With DI, the SDK's `AddPointsOfInterestClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddPointsOfInterestClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
