...
client = stub_client(stubs)
assert_raises FlightOffersPrice::APIException do
...
def build_test_client
  FlightOffersPrice::Client.new(
    environment: FlightOffersPrice::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error FlightOffersPrice::APIException
```
...
```ruby
expect { call }.to raise_error(FlightOffersPrice::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < FlightOffersPrice::HttpCallBack
...
client  = FlightOffersPrice::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
