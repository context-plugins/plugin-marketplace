...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `PazeCheckoutAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`pazecheckoutapi/configuration.py` in the cloned source.
...
```python
import requests
from pazecheckoutapi.configuration import Configuration
...
`pazecheckoutapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`pazecheckoutapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `pazecheckoutapi/configuration.py`
...
`NumberPagedResponse`) live in `pazecheckoutapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `pazecheckoutapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
