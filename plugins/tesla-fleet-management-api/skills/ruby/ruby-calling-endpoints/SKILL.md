...
Open `lib/tesla_fleet_management_api/client.rb` for the accessor names, then the relevant
`lib/tesla_fleet_management_api/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = TeslaFleetManagementAPI::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: TeslaFleetManagementAPI::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/tesla_fleet_management_api/models/`. Field names are defined as
...
Read the enum module in `lib/tesla_fleet_management_api/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/tesla_fleet_management_api/client.rb`.
- Operation methods are in `lib/tesla_fleet_management_api/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/tesla_fleet_management_api/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/tesla_fleet_management_api/exceptions/`.
