...
client = stub_client(stubs)
assert_raises FirecrawlAPI::APIException do
...
BASE = FirecrawlAPI::Configuration.new(environment: FirecrawlAPI::Environment::PRODUCTION).get_base_uri

def build_test_client
  FirecrawlAPI::Client.new(
    environment: FirecrawlAPI::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error FirecrawlAPI::APIException
```
...
```ruby
expect { call }.to raise_error(FirecrawlAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < FirecrawlAPI::HttpCallBack
...
client  = FirecrawlAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
