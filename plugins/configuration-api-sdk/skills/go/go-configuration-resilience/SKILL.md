    ...
    configurationapi.CreateConfiguration(
        configurationapi.WithHttpConfiguration(
            configurationapi.CreateHttpConfiguration(
                configurationapi.WithTimeout(30),
                configurationapi.WithRetryConfiguration(
                    configurationapi.CreateRetryConfiguration(
                        configurationapi.WithMaxRetryAttempts(3),
                        configurationapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := configurationapi.CreateRetryConfiguration(
    configurationapi.WithMaxRetryAttempts(5),
    configurationapi.WithRetryInterval(1),
    configurationapi.WithBackoffFactor(2),
    configurationapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    configurationapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := configurationapi.NewClient(
    configurationapi.CreateConfiguration(
        configurationapi.WithHttpConfiguration(
            configurationapi.CreateHttpConfiguration(
                configurationapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
