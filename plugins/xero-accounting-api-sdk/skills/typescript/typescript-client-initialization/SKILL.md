...
- `xero-accounting-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'xero-accounting-apilib';
...
```ts
import { {Resource}Controller } from 'xero-accounting-apilib';
