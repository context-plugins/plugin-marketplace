...
`testing` + `testify/require` for reference; substitute the real `pazecheckoutapi` and names.
...
import (
    ...
    "github.com/context-plugins/paze-checkout-api-go"   // root package pazecheckoutapi
)
...
func clientReturning(status int, body string) (pazecheckoutapi.ClientInterface, *stubTransport) {
    ...
    client := pazecheckoutapi.NewClient(
        pazecheckoutapi.CreateConfiguration(
            pazecheckoutapi.WithHttpConfiguration(
                pazecheckoutapi.CreateHttpConfiguration(pazecheckoutapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/paze-checkout-api-go/errors"
...
client := pazecheckoutapi.NewClient(
    pazecheckoutapi.CreateConfiguration(
        pazecheckoutapi.WithHttpConfiguration(
            pazecheckoutapi.CreateHttpConfiguration(pazecheckoutapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
