...
All types below live under `Authentication.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using Authentication.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new AuthenticationClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `Authentication.Core.Exceptions`):
...
With DI, the SDK's `AddAuthenticationClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAuthenticationClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
