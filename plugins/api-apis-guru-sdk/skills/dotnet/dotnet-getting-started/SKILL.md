...
| --- | --- |
| API | `api.apis.guru` |
| Project | `SafePlace.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `SafePlace` (the `using` namespace) |
| Client class | `SafePlaceClient` |
| Options class | `SafePlaceClientOptions` |
...
namespaces transitively, so `using SafePlace.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/api-apis-guru-csharp-ab984a
dotnet add reference api-apis-guru-csharp-ab984a/SafePlace.csproj
```

```csharp
using SafePlace;          // client + options + ServerEnvironment
using SafePlace.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-ab984a /tmp/api-apis-guru-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-ab984a "$env:TEMP\api-apis-guru-dotnet-src"
```
...
- `SafePlaceClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `SafePlaceClient`, build its
   `SafePlaceClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
