...
- `vimeo` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/vimeo-go` — the Go import path for the root package; `github.com/context-plugins/vimeo-go/models` for request/response types.
...
```go
import "github.com/context-plugins/vimeo-go"

client := vimeo.NewClient(
    vimeo.CreateConfiguration(
        vimeo.WithEnvironment(vimeo.PRODUCTION),
...
from environment variables named `VIMEO_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := vimeo.NewClient(vimeo.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `vimeo.PRODUCTION`,
`vimeo.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
    ...
    vimeo.CreateConfiguration(
        vimeo.WithHttpConfiguration(
            vimeo.CreateHttpConfiguration(
                vimeo.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                vimeo.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = vimeo.NewClient(vimeo.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api vimeo.ClientInterface
}

func NewService(api vimeo.ClientInterface) *Service {
