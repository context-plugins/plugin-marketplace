...
To see which schemes a specific SDK accepts, read the credential setter methods on `IPGeolocationAPIClient.Builder`
...
- Set credentials **on the `IPGeolocationAPIClient.Builder`** before calling `.build()` — the client is immutable
