...
> setting, so **confirm it** from the class names under `alpaca/apis/`.
...
| Client class | `TraderApiClient` in `alpaca/alpaca_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `alpaca/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Base exception | `APIException` in `alpaca/exceptions/api_exception.py` — raised on non-2xx |
...
- `alpaca/alpaca_client.py` — the `TraderApiClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `alpaca/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `alpaca/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `alpaca/controllers/` — one `{Resource}Api` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `alpaca/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `alpaca/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `alpaca/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `alpaca/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `alpaca/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
...
- `alpaca/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `alpaca/alpaca_client.py` — `TraderApiClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `alpaca/controllers/*.py` — operation method signatures, parameter names, and return types.
- `alpaca/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `alpaca/exceptions/` — `APIException` and typed subclasses.
