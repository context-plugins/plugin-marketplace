...
> `EbaySellClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): EbaySellClient
{
...
```php
use EbaySellLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use EbaySellLib\Exceptions\{OperationException};
use EbaySellLib\Exceptions\ApiException;
...
app()->instance(EbaySellClient::class, clientWithResponses([
