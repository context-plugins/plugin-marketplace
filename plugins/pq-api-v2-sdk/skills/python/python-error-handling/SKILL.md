...
`pqapiv2/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `pqapiv2/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `pqapiv2/exceptions/`.
...
```python
from pqapiv2.exceptions.api_exception import APIException
from pqapiv2.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`pqapiv2/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`pqapiv2/exceptions/` for the class.
...
`pqapiv2/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `pqapiv2/exceptions/`
  or a `ValueError` — check the auth handler in `pqapiv2/http/auth/` for the exact type.
