...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `EventsAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`eventsapi/configuration.py` in the cloned source.
...
```python
import requests
from eventsapi.configuration import Configuration
...
`eventsapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`eventsapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `eventsapi/configuration.py`
...
`NumberPagedResponse`) live in `eventsapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `eventsapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
