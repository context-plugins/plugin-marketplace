...
All types below live under `VimeoApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using VimeoApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new VimeoApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `VimeoApi.Core.Exceptions`):
...
With DI, the SDK's `AddVimeoApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddVimeoApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
