...
`testing` + `testify/require` for reference; substitute the real `telegrambotapi` and names.
...
import (
    ...
    "github.com/context-plugins/telegram-bot-api-go"   // root package telegrambotapi
)
...
func clientReturning(status int, body string) (telegrambotapi.ClientInterface, *stubTransport) {
    ...
    client := telegrambotapi.NewClient(
        telegrambotapi.CreateConfiguration(
            telegrambotapi.WithHttpConfiguration(
                telegrambotapi.CreateHttpConfiguration(telegrambotapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/telegram-bot-api-go/errors"
...
client := telegrambotapi.NewClient(
    telegrambotapi.CreateConfiguration(
        telegrambotapi.WithHttpConfiguration(
            telegrambotapi.CreateHttpConfiguration(telegrambotapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
