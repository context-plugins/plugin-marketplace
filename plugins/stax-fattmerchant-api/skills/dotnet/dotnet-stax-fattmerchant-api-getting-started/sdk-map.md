# SDK map — Stax (FattMerchant) API (.NET)

> A generated table-of-contents for this SDK. Consult this map and its sub-pages to learn signatures, error
> types, enum values, and server/auth wiring **by lookup** — open a source file only for a full method/model
> body the map doesn't carry. The compiler is the backstop: a wrong name fails to build.

| | |
|---|---|
| SDK display name | Stax (FattMerchant) API |
| Root namespace/module | `StaxFattMerchantApi` |
<!-- gen:stamp -->
| Target framework(s) | `netstandard2.0` (C# `LangVersion 14`, `Nullable enable`) |
| Source commit (spec stamp) | `9bdd181` (`9bdd18162cce9ecfcf5d622a1b5a8271a3a3d03f`, tagged `9bdd181`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
| Repo | https://github.com/context-plugins/stax-fattmerchant-api-csharp-sdk (branch `main`) |

Staleness check: if the SDK is regenerated, the source commit stamp above changes (the SDK repo and this
plugin regenerate together). If a lookup here fails to compile, trust the compiler and re-read the source
file linked in the row.

---

## Getting a client

```csharp
using StaxFattMerchantApi;
using StaxFattMerchantApi.Servers; // ServerEnvironment lives here

var options = new StaxFattMerchantApiClientOptions
{
    // Set the credentials properties for the scheme(s) this API uses — see Servers & auth below.
    // Environment selects the server environment; see Servers & auth below.
};
var client = new StaxFattMerchantApiClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddStaxFattMerchantApiClient(o =>
{
    // set credentials / environment on o here
});
```

Every API group is a property on the client (e.g. `client.Customers`). Source:
`StaxFattMerchantApiClient.cs`.

<!-- crawler:client-options -->
All `StaxFattMerchantApiClientOptions` properties (source: `StaxFattMerchantApiClientOptions.cs`):

| Property | Type |
|---|---|
| `Environment` | `ServerEnvironment` |
| `Retry` | `RetryOptions` |
| `Logging` | `LoggingOptions` |
| `Server` | `ServerOptions` |
| `BearerAuth` | `string?` |

`RetryOptions` members (source: `Core/Configuration/RetryOptions.cs`; build a full instance — all members are `required` — or start from `RetryOptions.Default()`):

| Member | Type |
|---|---|
| `StatusCodesToRetry` | `IReadOnlyList<HttpStatusCode>` |
| `HttpMethodsToRetry` | `IReadOnlyList<HttpMethod>` |
| `MaxRetries` | `int` |
| `Delay` | `TimeSpan` |
| `Timeout` | `TimeSpan?` |
| `BackOffFactor` | `int` |
| `UseExponentialBackoff` | `bool` |
| `MaxJitter` | `TimeSpan` |
| `OnRetry` | `Action<RetryAttempt>?` |

Client constructor(s):

- `StaxFattMerchantApiClient(HttpClient httpClient, StaxFattMerchantApiClientOptions options)`
<!-- /crawler:client-options -->

---

## Error-handling model (read once — applies to every operation)

Operations are **throw-based**. On an error status the SDK throws `SdkException<TError>`
(`Core/Exceptions/SdkException.cs`) exposing `.Error` of type `TError`. There are two cases:

- **Case A — typed error.** `TError` is a generated `…Error : ApiError` class with status-specific
  `TryGet…(out …)` accessors (returns `true` when that shape is present) plus the inherited
  `TryGetRawError(out RawError)` fallback. The per-operation rows name the exact `TryGet…` methods and the HTTP
  status each maps to.
- **Case B — raw error.** `TError` is `RawError` (`Core/ErrorResponse/RawError.cs`): `StatusCode`,
  `ReadAsString()`, `ReadAsJson<T>()`, `ReadAsBytes()`.

<!-- gen:error-core -->
Core error types (`Core/ErrorResponse/`) — public members with their **declared types**, verbatim from source:

| Type | Public members | Source |
|---|---|---|
| `ApiError` — abstract base of all 2 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
| `RawError` | `StatusCode: HttpStatusCode` · `ReadAsBytes(): ReadOnlyMemory<byte>` · `ReadAsString(): string` · `ReadAsJson<T>(): T?` | `Core/ErrorResponse/RawError.cs` |

Typed-error payload shapes (the `out` types in each operation page's error-accessor cells) are ordinary records/unions: field names, declared types, and JSON wire names live on the records pages / `unions.md` like any other model.
<!-- /gen:error-core -->

```csharp
try { var resp = await client.{ApiGroup}.{Operation}(body); }
catch (SdkException<{Operation}Error> ex)              // Case A
{
    if (ex.Error.TryGetSomeShape(out var typed))      { /* handle that status */ }
    else if (ex.Error.TryGetRawError(out var raw))    { /* other statuses */ }
}
catch (SdkException<RawError> ex)                     // Case B
{
    var status = ex.Error.StatusCode;
    var body   = ex.Error.ReadAsString();
}
```

<!-- crawler:op-stats -->
**No-throw ("`…Result`") variants: absent across this SDK** — every operation is throw-only.
Of **180 operations**, **2 are Case A (typed)** and **178 are Case B (raw)**.
<!-- /crawler:op-stats -->

---

## Operations — by controller (28 groups, 180 operations)

Each links to a sub-page with one row per operation (HTTP, signature with must-pass-explicitly params, return
type, error Case A/B + accessors, pagination).

<!-- crawler:ops-table -->
| Controller (`client.X`) | Ops | Page |
|---|---:|---|
| `Cache` | 1 | [map/operations/Cache.md](map/operations/Cache.md) |
| `Charge` | 1 | [map/operations/Charge.md](map/operations/Charge.md) |
| `Credit` | 1 | [map/operations/Credit.md](map/operations/Credit.md) |
| `Customer` | 14 | [map/operations/Customer.md](map/operations/Customer.md) |
| `DisputeFile` | 2 | [map/operations/DisputeFile.md](map/operations/DisputeFile.md) |
| `File` | 6 | [map/operations/File.md](map/operations/File.md) |
| `HelloSign` | 5 | [map/operations/HelloSign.md](map/operations/HelloSign.md) |
| `Integration` | 5 | [map/operations/Integration.md](map/operations/Integration.md) |
| `Invoice` | 11 | [map/operations/Invoice.md](map/operations/Invoice.md) |
| `InvoiceSchedule` | 5 | [map/operations/InvoiceSchedule.md](map/operations/InvoiceSchedule.md) |
| `Item` | 13 | [map/operations/Item.md](map/operations/Item.md) |
| `MerchantAdmin` | 15 | [map/operations/MerchantAdmin.md](map/operations/MerchantAdmin.md) |
| `PaymentMethod` | 6 | [map/operations/PaymentMethod.md](map/operations/PaymentMethod.md) |
| `Reporting` | 2 | [map/operations/Reporting.md](map/operations/Reporting.md) |
| `Sandbox` | 2 | [map/operations/Sandbox.md](map/operations/Sandbox.md) |
| `Self` | 10 | [map/operations/Self.md](map/operations/Self.md) |
| `Team` | 7 | [map/operations/Team.md](map/operations/Team.md) |
| `TeamApiKeys` | 7 | [map/operations/TeamApiKeys.md](map/operations/TeamApiKeys.md) |
| `TeamFundingAccounts` | 8 | [map/operations/TeamFundingAccounts.md](map/operations/TeamFundingAccounts.md) |
| `TeamOptions` | 7 | [map/operations/TeamOptions.md](map/operations/TeamOptions.md) |
| `TeamRegistration` | 8 | [map/operations/TeamRegistration.md](map/operations/TeamRegistration.md) |
| `TeamUsers` | 6 | [map/operations/TeamUsers.md](map/operations/TeamUsers.md) |
| `Terminal` | 3 | [map/operations/Terminal.md](map/operations/Terminal.md) |
| `Transaction` | 19 | [map/operations/Transaction.md](map/operations/Transaction.md) |
| `UserAdmin` | 8 | [map/operations/UserAdmin.md](map/operations/UserAdmin.md) |
| `Verify` | 2 | [map/operations/Verify.md](map/operations/Verify.md) |
| `Webhook` | 5 | [map/operations/Webhook.md](map/operations/Webhook.md) |
| `WebPayment` | 1 | [map/operations/WebPayment.md](map/operations/WebPayment.md) |
<!-- /crawler:ops-table -->

---

## Models

<!-- gen:models-table -->
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 41 | [`DeleteCustomerBulkRequestBodyJson` … `PutVerifyIntegrationTokenRequestBodyJson`](map/models/records-1-De-Pu.md) |
| Unions (`OneOf` / `AnyOf`) — variant factories + `TryGet…` | 0 + 0 | [map/models/unions.md](map/models/unions.md) |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — literal C# member names + wire values | 7 | [map/models/enums.md](map/models/enums.md) |
<!-- /gen:models-table -->

Model conventions: records are immutable with `init`-only setters; `required` properties must be set in the
object initializer; nullable (`T?`) properties are optional. Each record field is listed as
`CSharpName (wire_name): Type` — the parenthesized name is the JSON wire name (`[JsonPropertyName]`).
Unions wrap `Optional<T>` variants — construct via a static factory or implicit
conversion, read back via `TryGet…(out …)`. Enums are **not** C# enums — build with `Type.FromValue("wire")`
or the static members (enums.md lists the literal member names: `SomeEnum.SomeMember`, not
`SomeEnum.some_member`).

<!-- gen:namespaces -->
Namespaces by content type (add `using` accordingly):

| Contents | Namespace(s) |
|---|---|
| Client & options (root) | `StaxFattMerchantApi` |
| Operation controllers (`Api/`) | `StaxFattMerchantApi.Api` |
| Records (`Models/`) | `StaxFattMerchantApi.Models` |
| Enums (`Models/Enums/`) | `StaxFattMerchantApi.Models.Enums` |
| Error classes (`Errors/`) | `StaxFattMerchantApi.Errors` |
<!-- /gen:namespaces -->

---

## Servers & auth

<!-- crawler:servers-auth -->
**Auth.** The scheme(s) this API uses surface as credentials properties on `StaxFattMerchantApiClientOptions` (source: `StaxFattMerchantApiClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:

| Property | Type | Notes (from the source XML docs) |
|---|---|---|
| `BearerAuth` | `string?` | — |

**Environments.** `options.Environment` is a `ServerEnvironment` (`Servers/ServerEnvironment.cs`) with members: `ServerEnvironment.Production`, `ServerEnvironment.Environment2`. Base-URL templates and override points live under `Servers/` and `options.Server`.
<!-- /crawler:servers-auth -->
