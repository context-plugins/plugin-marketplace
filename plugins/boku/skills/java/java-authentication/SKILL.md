...
To see which schemes a specific SDK accepts, read the credential setter methods on `BokuDirectPaymentsApiClient.Builder`
...
- Set credentials **on the `BokuDirectPaymentsApiClient.Builder`** before calling `.build()` — the client is immutable
