...
kwargs to the `RestapiClient` constructor (or to `Configuration`). Exact credential class names and
...
client = RestapiClient(o_auth_acg_credentials=creds)
...
client = RestapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `RestapiClient.from_environment()`) read each credential
