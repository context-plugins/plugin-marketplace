...
`testing` + `testify/require` for reference; substitute the real `greenbyte` and names.
...
import (
    ...
    "github.com/context-plugins/greenbyte-go-sdk"   // root package greenbyte
)
...
func clientReturning(status int, body string) (greenbyte.ClientInterface, *stubTransport) {
    ...
    client := greenbyte.NewClient(
        greenbyte.CreateConfiguration(
            greenbyte.WithHttpConfiguration(
                greenbyte.CreateHttpConfiguration(greenbyte.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/greenbyte-go-sdk/errors"
...
client := greenbyte.NewClient(
    greenbyte.CreateConfiguration(
        greenbyte.WithHttpConfiguration(
            greenbyte.CreateHttpConfiguration(greenbyte.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
