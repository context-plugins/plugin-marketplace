...
Open `vimeoapi/vimeoapi_client.py` for the controller property names, then the relevant
`vimeoapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from vimeoapi.models.{request_model} import {RequestModel}
...
Open `vimeoapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from vimeoapi.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `vimeoapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `VimeoapiClient` in `vimeoapi/vimeoapi_client.py`.
- Operation method signatures are in `vimeoapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `vimeoapi/models/`; enum types have `from_value()`.
