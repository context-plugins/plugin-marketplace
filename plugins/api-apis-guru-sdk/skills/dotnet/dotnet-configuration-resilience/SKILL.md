...
All types below live under `FlightDelayPrediction.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FlightDelayPrediction.Core.Configuration;
...
```csharp
using FlightDelayPrediction.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FlightDelayPredictionClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FlightDelayPrediction.Core.Exceptions`):
...
With DI, the SDK's `AddFlightDelayPredictionClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFlightDelayPredictionClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
