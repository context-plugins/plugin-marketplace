...
```go
goe.WithOAuthCCGCredentials(
    goe.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr goe.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := goe.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr goe.OAuthCCGManager) models.OAuthToken {
...
```go
creds := goe.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := goe.NewClient(goe.CreateConfiguration(goe.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    goe.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := goe.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := goe.NewClient(goe.CreateConfiguration(goe.WithOAuthROPCGCredentials(creds)))
...
```go
goe.WithOAuthBearerTokenCredentials(goe.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
goe.WithBasicAuthCredentials(goe.NewBasicAuthCredentials(username, password))
goe.WithApiHeaderCredentials(goe.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
goe.WithApiKeyCredentials(goe.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `GOE_...` variables — e.g.
`GOE_USERNAME`, `GOE_PASSWORD`, `GOE_TOKEN`, `GOE_API_KEY`,
`GOE_O_AUTH_CLIENT_ID`, `GOE_O_AUTH_CLIENT_SECRET`, `GOE_ACCESS_TOKEN`. Grep the
