...
- `teslaapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/tesla-api-go` — the Go import path for the root package; `github.com/context-plugins/tesla-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/tesla-api-go"

client := teslaapi.NewClient(
    teslaapi.CreateConfiguration(
        teslaapi.WithEnvironment(teslaapi.PRODUCTION),
...
from environment variables named `TESLAAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := teslaapi.NewClient(teslaapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `teslaapi.PRODUCTION`,
`teslaapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := teslaapi.NewClient(
    teslaapi.CreateConfiguration(teslaapi.WithEnvironment(teslaapi.PRODUCTION)),
)
```
...
client := teslaapi.NewClient(
    teslaapi.CreateConfiguration(
        teslaapi.WithHttpConfiguration(
            teslaapi.CreateHttpConfiguration(
                teslaapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                teslaapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = teslaapi.NewClient(teslaapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api teslaapi.ClientInterface
}

func NewService(api teslaapi.ClientInterface) *Service {
