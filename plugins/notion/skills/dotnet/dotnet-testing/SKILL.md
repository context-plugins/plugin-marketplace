...
`NotionClient`/`NotionClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `NotionClient`,
...
static NotionClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new NotionClient(new HttpClient(handler), new NotionClientOptions { /* auth not needed for stubs */ });
}
```
...
using Notion.Errors;              // {Operation}Error types
...
```csharp
using Notion.Core.Exceptions;
using Notion.Core.ErrorResponse;
...
using Notion.Core.ErrorResponse; // RawError (Case B)
...
var client = new NotionClient(new HttpClient(handler), new NotionClientOptions());
...
- For DI-based code, the SDK's `AddNotionClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `NotionClient` from the provider:
  ```csharp
  services.AddNotionClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<NotionClient>();
  ```
