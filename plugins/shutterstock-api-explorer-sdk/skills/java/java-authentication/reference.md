...
against the `ShutterstockAPIExplorerClient.Builder` setters, the `com.shutterstock.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ShutterstockAPIExplorerClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ShutterstockAPIExplorerClient client = new ShutterstockAPIExplorerClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeShutterstockAPIExplorerEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new ShutterstockAPIExplorerClient.Builder()
