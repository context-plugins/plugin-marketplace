...
> `pokeapi/pokeapi_client.py` and from `doc/auth/*.md`.

To see which schemes a specific SDK accepts, read the credential kwargs in the client constructor
(`pokeapi/pokeapi_client.py`) — those are the source of truth. Each scheme's credentials class lives
under `pokeapi/http/auth/` and has its own `from_environment()` class method.
