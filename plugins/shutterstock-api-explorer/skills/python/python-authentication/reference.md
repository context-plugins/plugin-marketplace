...
kwargs to the `ShutterstockAPIExplorerClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `shutterstockapiexplorer/shutterstockapiexplorer_client.py` and `shutterstockapiexplorer/http/auth/` in
the cloned source.

## Basic auth

```python
from shutterstockapiexplorer.http.auth.basic_auth import BasicAuthCredentials
...
```python
from shutterstockapiexplorer.http.auth.api_key import ApiKeyCredentials
...
```python
from shutterstockapiexplorer.http.auth.api_header import ApiHeaderCredentials
...
```python
from shutterstockapiexplorer.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from shutterstockapiexplorer.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from shutterstockapiexplorer.http.auth.o_auth_acg import OAuthACGCredentials
from shutterstockapiexplorer.models.o_auth_scope_shutterstockapiexplorer_enum import OAuthScope{Pkg}Enum
...
client = ShutterstockAPIExplorerClient(o_auth_acg_credentials=creds)
...
```python
from shutterstockapiexplorer.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ShutterstockAPIExplorerClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ShutterstockAPIExplorerClient.from_environment()`) read each credential
...
Grep `from_environment` in `shutterstockapiexplorer/configuration.py` and each `shutterstockapiexplorer/http/auth/*.py` for the exact
