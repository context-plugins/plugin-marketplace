...
> Throughout, `accessanalyzer` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := accessanalyzer.NewClient(
    accessanalyzer.CreateConfiguration(
        accessanalyzer.WithBasicAuthCredentials(
            accessanalyzer.NewBasicAuthCredentials(
...
accessanalyzer.WithApiKeyCredentials(accessanalyzer.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
accessanalyzer.WithApiHeaderCredentials(accessanalyzer.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
accessanalyzer.WithOAuthBearerTokenCredentials(
    accessanalyzer.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := accessanalyzer.NewClient(
    accessanalyzer.CreateConfiguration(
        accessanalyzer.WithOAuthCCGCredentials(
            accessanalyzer.NewOAuthCCGCredentials(
