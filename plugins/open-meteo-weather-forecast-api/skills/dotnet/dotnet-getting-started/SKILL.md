...
| Project | `OpenMeteoWeatherForecastApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `OpenMeteoWeatherForecastApi` (the `using` namespace) |
| Client class | `OpenMeteoWeatherForecastApiClient` |
| Options class | `OpenMeteoWeatherForecastApiClientOptions` |
...
namespaces transitively, so `using OpenMeteoWeatherForecastApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/open-meteo-weather-forecast-api-csharp-sdk
dotnet add reference open-meteo-weather-forecast-api-csharp-sdk/OpenMeteoWeatherForecastApi.csproj
```

```csharp
using OpenMeteoWeatherForecastApi;          // client + options + ServerEnvironment
using OpenMeteoWeatherForecastApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/open-meteo-weather-forecast-api-csharp-sdk /tmp/open-meteo-weather-forecast-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/open-meteo-weather-forecast-api-csharp-sdk "$env:TEMP\open-meteo-weather-forecast-api-dotnet-src"
```
...
- `OpenMeteoWeatherForecastApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/open-meteo-weather-forecast-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\open-meteo-weather-forecast-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `OpenMeteoWeatherForecastApiClient`, build its
   `OpenMeteoWeatherForecastApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
