...
```go
locationscore.WithOAuthCCGCredentials(
    locationscore.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr locationscore.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := locationscore.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr locationscore.OAuthCCGManager) models.OAuthToken {
...
```go
creds := locationscore.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := locationscore.NewClient(locationscore.CreateConfiguration(locationscore.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    locationscore.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := locationscore.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := locationscore.NewClient(locationscore.CreateConfiguration(locationscore.WithOAuthROPCGCredentials(creds)))
...
```go
locationscore.WithOAuthBearerTokenCredentials(locationscore.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
locationscore.WithBasicAuthCredentials(locationscore.NewBasicAuthCredentials(username, password))
locationscore.WithApiHeaderCredentials(locationscore.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
locationscore.WithApiKeyCredentials(locationscore.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `LOCATIONSCORE_...` variables — e.g.
`LOCATIONSCORE_USERNAME`, `LOCATIONSCORE_PASSWORD`, `LOCATIONSCORE_TOKEN`, `LOCATIONSCORE_API_KEY`,
`LOCATIONSCORE_O_AUTH_CLIENT_ID`, `LOCATIONSCORE_O_AUTH_CLIENT_SECRET`, `LOCATIONSCORE_ACCESS_TOKEN`. Grep the
