...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `vimeo`) — replace it with the concrete identifier from the source.
...
import { ApiError } from 'vimeo';
import { {Operation}Error } from 'vimeo/errors';
