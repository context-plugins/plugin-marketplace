...
- `ApiApisGuruClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`ApiApisGuruClient.Builder` class. A minimal initialization:

```java
import com.amadeus.api.test.ApiApisGuruClient;
import com.amadeus.api.test.Environment;

ApiApisGuruClient client = new ApiApisGuruClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `ApiApisGuruClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `ApiApisGuruClient.java` |

Call `.build()` to produce an immutable `ApiApisGuruClient` instance.
...
public class ApiConfig {
    @Bean
    public ApiApisGuruClient apiClient() {
        return new ApiApisGuruClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired ApiApisGuruClient client) {
        client.shutdown();
    }
}
```

Inject `ApiApisGuruClient` into your services as a normal Spring dependency. Build only one bean.
