...
| Project | `HostedOnboardingApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `HostedOnboardingApi` (the `using` namespace) |
| Client class | `HostedOnboardingApiClient` |
| Options class | `HostedOnboardingApiClientOptions` |
...
namespaces transitively, so `using HostedOnboardingApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/hosted-onboarding-api-csharp
dotnet add reference hosted-onboarding-api-csharp/HostedOnboardingApi.csproj
```

```csharp
using HostedOnboardingApi;          // client + options + ServerEnvironment
using HostedOnboardingApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/hosted-onboarding-api-csharp /tmp/hosted-onboarding-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/hosted-onboarding-api-csharp "$env:TEMP\hosted-onboarding-api-dotnet-src"
```
...
- `HostedOnboardingApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/hosted-onboarding-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\hosted-onboarding-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `HostedOnboardingApiClient`, build its
   `HostedOnboardingApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
