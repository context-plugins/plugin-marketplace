...
Open `subscriptionclient/subscriptionclient_client.py` for the controller property names, then the relevant
`subscriptionclient/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from subscriptionclient.models.{request_model} import {RequestModel}
...
Open `subscriptionclient/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from subscriptionclient.models.suite_code_enum import SuiteCodeEnum
...
```python
from subscriptionclient.http.http_call_back import HttpCallBack
...
client = SubscriptionClientClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `subscriptionclient/pagination/` in the source.

## Finding the right method

- Controller property names are on `SubscriptionClientClient` in `subscriptionclient/subscriptionclient_client.py`.
- Operation method signatures are in `subscriptionclient/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `subscriptionclient/models/`; enum types have `from_value()`.
