...
> Throughout, `paypalapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := paypalapi.NewClient(
    paypalapi.CreateConfiguration(
        paypalapi.WithBasicAuthCredentials(
            paypalapi.NewBasicAuthCredentials(
...
paypalapi.WithApiKeyCredentials(paypalapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
paypalapi.WithApiHeaderCredentials(paypalapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
paypalapi.WithOAuthBearerTokenCredentials(
    paypalapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := paypalapi.NewClient(
    paypalapi.CreateConfiguration(
        paypalapi.WithOAuthCCGCredentials(
            paypalapi.NewOAuthCCGCredentials(
