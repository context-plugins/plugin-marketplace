...
- `geoapify` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/geoapify-go-sdk` — the Go import path for the root package; `github.com/context-plugins/geoapify-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/geoapify-go-sdk"

client := geoapify.NewClient(
    geoapify.CreateConfiguration(
        geoapify.WithEnvironment(geoapify.PRODUCTION),
...
from environment variables named `GEOAPIFY_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := geoapify.NewClient(geoapify.CreateConfigurationFromEnvironment())
```
...
```go
client := geoapify.NewClient(
    geoapify.CreateConfiguration(geoapify.WithEnvironment(geoapify.PRODUCTION)),
)
```
...
client := geoapify.NewClient(
    geoapify.CreateConfiguration(
        geoapify.WithHttpConfiguration(
            geoapify.CreateHttpConfiguration(
                geoapify.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                geoapify.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = geoapify.NewClient(geoapify.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api geoapify.ClientInterface
}

func NewService(api geoapify.ClientInterface) *Service {
