...
- `nytimeslib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'nytimeslib';
...
```ts
import { {Resource}Controller } from 'nytimeslib';
