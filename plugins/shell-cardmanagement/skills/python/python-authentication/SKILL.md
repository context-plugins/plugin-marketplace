...
> `shellcardmanagement/shellcardmanagement_client.py` and from `doc/auth/*.md`.

To see which schemes a specific SDK accepts, read the credential kwargs in the client constructor
(`shellcardmanagement/shellcardmanagement_client.py`) — those are the source of truth. Each scheme's credentials class lives
under `shellcardmanagement/http/auth/` and has its own `from_environment()` class method.
