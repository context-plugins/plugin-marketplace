...
| --- | --- |
| API | `Medium API` |
| Project | `MediumApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `MediumApi` (the `using` namespace) |
| Client class | `MediumApiClient` |
| Options class | `MediumApiClientOptions` |
...
namespaces transitively, so `using MediumApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/medium-api-csharp
dotnet add reference medium-api-csharp/MediumApi.csproj
```

```csharp
using MediumApi;          // client + options + ServerEnvironment
using MediumApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/medium-api-csharp /tmp/medium-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/medium-api-csharp "$env:TEMP\medium-api-dotnet-src"
```
...
- `MediumApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/medium-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\medium-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `MediumApiClient`, build its
   `MediumApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
