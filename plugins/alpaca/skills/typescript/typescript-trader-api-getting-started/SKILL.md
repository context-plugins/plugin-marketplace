---
name: "typescript-trader-api-getting-started"
description: "Trader API TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `trader-api`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any Trader API TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the Trader API TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Trader API TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-trader-api` for the workflow that wraps this skill.
...
Verified against `package.json` and `sdk-map.md` of the generated package at version `2.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Trader API |
| Package name (what you install, and what you import) | `trader-api` — **not on npm**; built from source (see *Install*) |
| Import specifier | `trader-api` — the package root is the **only** entry; deep imports do not resolve |
| Version | `2.0.0` (API spec version `2.0.0`) |
| Client class | `TraderApiClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new TraderApiClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new TraderApiClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `apiKey` · `apiSecret`. `timeout` defaults to `60_000` ms |
| Auth | **API key (header)** — set `ClientOptions.apiKey`<br>**API key (header)** — set `ClientOptions.apiSecret` |
| Environments | 2 environments (`ServerEnvironment.Paper` *(default)*, `ServerEnvironment.Live`) × 1 server group |
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://paper-api.alpaca.markets` |
...
| Surface | 29 operations · 9 resources · 16 models · 14 open enums · 2 unions · 4 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/alpaca-typescript-sdk#main
```

That fetches everything the package's `files` list packs — `src/`, `sdk-map.md` and the pages under `map/operations/` — so **every lookup on this page works as soon as the install finishes**. What it does *not* do is build `dist/`: the generated manifest declares no `prepare` script, and the `exports` map points into `dist/`, so the first `import` fails until something builds it. Run the SDK's own build once inside the installed package (`npm install && npm run build` in `node_modules/trader-api/`), or clone the repository, build there, and depend on that directory by path instead.
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 32 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { TraderApiClient, ServerEnvironment, ResponseError, TraderApiError } from "trader-api";
import type { ClientOptions, Account } from "trader-api";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `trader-api/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **From CommonJS**, the typed spelling is `import sdk = require("trader-api")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `paper` *(default)* | `https://paper-api.alpaca.markets` | `serverOptions.default.paper.baseUrl` |
| `default` | `live` | `https://api.alpaca.markets` | `serverOptions.default.live.baseUrl` |

Consequences to state on every contract sheet that touches configuration:

- Constructing the client with no options selects **`ServerEnvironment.Paper`**, silently.
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 29 of the 29 operations require a credential and 0 are public.

| `ClientOptions` field | Scheme kind | What the SDK sends |
| --- | --- | --- |
| `apiKey` | API key (header) | header `APCA-API-KEY-ID: <key>` |
| `apiSecret` | API key (header) | header `APCA-API-SECRET-KEY: <key>` |

```ts
const client = new TraderApiClient({
...
| --- | --- | --- | --- |
| `client.accounts` | `Accounts` | 1 | `getAccount` |
| `client.orders` | `Orders` | 6 | `deleteAllOrders` · `deleteOrderByOrderId` · `getAllOrders` · `getOrderByOrderId` · `patchOrderByOrderId` · `postOrder` |
| `client.positions` | `Positions` | 4 | `deleteAllOpenPositions` · `deleteOpenPosition` · `getAllOpenPositions` · `getOpenPosition` |
| `client.portfolioHistoryApi` | `PortfolioHistoryApi` | 1 | `getAccountPortfolioHistory` |
| `client.watchlists` | `Watchlists` | 11 | `addAssetToWatchlist` · `addAssetToWatchlistByName` · `deleteWatchlistById` · `deleteWatchlistByName` · `getWatchlistById` · `getWatchlistByName` · `getWatchlists` · `postWatchlist` · `removeAssetFromWatchlist` · `updateWatchlistById` · `updateWatchlistByName` |
| `client.accountConfigurationsApi` | `AccountConfigurationsApi` | 2 | `getAccountConfig` · `patchAccountConfig` |
| `client.accountActivities` | `AccountActivities` | 2 | `getAccountActivities` · `getAccountActivitiesByActivityType` |
| `client.calendarApi` | `CalendarApi` | 1 | `getCalendar` |
| `client.clockApi` | `ClockApi` | 1 | `getClock` |
...
⚠ **The request type name is not uniformly `<Operation>Request`.** 2 operations take `<Operation>RequestParams` instead: `addAssetToWatchlist`, `postWatchlist`. Take the name from the operation's **Signature** bullet on its map page; never construct it from the method name.
...
```bash
node -e "console.log(require.resolve('trader-api/package.json'))"
```

Failing that it is at `node_modules/trader-api/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/trader-api/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/trader-api/`:
...
Beyond the usual signatures and model members, a contract sheet for the Trader API TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.

1. **Which host each deployment talks to**, and where that is set. The members are `ServerEnvironment.Paper`, `ServerEnvironment.Live`, defaulting to `ServerEnvironment.Paper` when the field is unset.
2. **3 operations resolve to `undefined` (`orders.deleteOrderByOrderId`, `watchlists.deleteWatchlistById`, `watchlists.deleteWatchlistByName`)** — `await` gives you nothing to inspect, so **`.asApiResult()` is the only way to observe their status and headers** — decide the mode at write time, not by retrofit.
