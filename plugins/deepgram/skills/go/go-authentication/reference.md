...
```go
restapi.WithOAuthCCGCredentials(
    restapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr restapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := restapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr restapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := restapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := restapi.NewClient(restapi.CreateConfiguration(restapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    restapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := restapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := restapi.NewClient(restapi.CreateConfiguration(restapi.WithOAuthROPCGCredentials(creds)))
...
```go
restapi.WithOAuthBearerTokenCredentials(restapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
restapi.WithBasicAuthCredentials(restapi.NewBasicAuthCredentials(username, password))
restapi.WithApiHeaderCredentials(restapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
restapi.WithApiKeyCredentials(restapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `RESTAPI_...` variables — e.g.
`RESTAPI_USERNAME`, `RESTAPI_PASSWORD`, `RESTAPI_TOKEN`, `RESTAPI_API_KEY`,
`RESTAPI_O_AUTH_CLIENT_ID`, `RESTAPI_O_AUTH_CLIENT_SECRET`, `RESTAPI_ACCESS_TOKEN`. Grep the
