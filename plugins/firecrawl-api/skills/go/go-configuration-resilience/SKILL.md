    ...
    firecrawlapi.CreateConfiguration(
        firecrawlapi.WithHttpConfiguration(
            firecrawlapi.CreateHttpConfiguration(
                firecrawlapi.WithTimeout(30),
                firecrawlapi.WithRetryConfiguration(
                    firecrawlapi.CreateRetryConfiguration(
                        firecrawlapi.WithMaxRetryAttempts(3),
                        firecrawlapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := firecrawlapi.CreateRetryConfiguration(
    firecrawlapi.WithMaxRetryAttempts(5),
    firecrawlapi.WithRetryInterval(1),
    firecrawlapi.WithBackoffFactor(2),
    firecrawlapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    firecrawlapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := firecrawlapi.NewClient(
    firecrawlapi.CreateConfiguration(
        firecrawlapi.WithHttpConfiguration(
            firecrawlapi.CreateHttpConfiguration(
                firecrawlapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
