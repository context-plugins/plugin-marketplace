...
All types below live under `FlightChoicePrediction.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FlightChoicePrediction.Core.Configuration;
...
```csharp
using FlightChoicePrediction.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FlightChoicePredictionClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FlightChoicePrediction.Core.Exceptions`):
...
With DI, the SDK's `AddFlightChoicePredictionClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFlightChoicePredictionClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
