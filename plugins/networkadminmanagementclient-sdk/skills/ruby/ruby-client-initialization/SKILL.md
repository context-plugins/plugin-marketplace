...
- `NetworkAdminManagementClient` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `networkadminmanagementclient` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'networkadminmanagementclient'
include NetworkAdminManagementClient
...
The authoritative list is the `initialize` signature in `lib/networkadminmanagementclient/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/networkadminmanagementclient/configuration.rb`
(`build_default_config_from_env`) and `lib/networkadminmanagementclient/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/networkadminmanagementclient.rb`) and store it in a
