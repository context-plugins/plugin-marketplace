...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `VimeoapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`vimeoapi/configuration.py` in the cloned source.
...
```python
import requests
from vimeoapi.configuration import Configuration
...
`vimeoapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`vimeoapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `vimeoapi/configuration.py`
...
`NumberPagedResponse`) live in `vimeoapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `vimeoapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:

```python
import logging
from vimeoapi.http.http_call_back import HttpCallBack
from vimeoapi.configuration import Configuration
