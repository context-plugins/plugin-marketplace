...
- `pqapiv2` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/pq-api-v2-go-sdk` — the Go import path for the root package; `github.com/context-plugins/pq-api-v2-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/pq-api-v2-go-sdk"

client := pqapiv2.NewClient(
    pqapiv2.CreateConfiguration(
        pqapiv2.WithEnvironment(pqapiv2.PRODUCTION),
...
from environment variables named `PQAPIV2_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := pqapiv2.NewClient(pqapiv2.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `pqapiv2.PRODUCTION`,
`pqapiv2.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := pqapiv2.NewClient(
    pqapiv2.CreateConfiguration(pqapiv2.WithEnvironment(pqapiv2.PRODUCTION)),
)
```
...
client := pqapiv2.NewClient(
    pqapiv2.CreateConfiguration(
        pqapiv2.WithHttpConfiguration(
            pqapiv2.CreateHttpConfiguration(
                pqapiv2.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                pqapiv2.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = pqapiv2.NewClient(pqapiv2.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api pqapiv2.ClientInterface
}

func NewService(api pqapiv2.ClientInterface) *Service {
