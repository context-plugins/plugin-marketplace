...
│   └── com.splitit.sandbox.webapiv3.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.splitit.sandbox.webapiv3.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.splitit.sandbox.webapiv3.http.client.HttpContext`, which carries:
