...
> `{ApiGroup}`, `FlightOrderManagement`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FlightOrderManagement.Core.Exceptions`
- `ApiError` **and** `RawError` → `FlightOrderManagement.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FlightOrderManagement.Errors`
...
```csharp
using FlightOrderManagement.Core.Exceptions;     // SdkException<TError>
using FlightOrderManagement.Core.ErrorResponse;  // ApiError, RawError
using FlightOrderManagement.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FlightOrderManagement.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FlightOrderManagement.Core.Exceptions;     // SdkException<TError>
using FlightOrderManagement.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FlightOrderManagement.Core.ErrorResponse`. Its public
...
`readonly struct` under `FlightOrderManagement.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FlightOrderManagement.Core.Models;        // ApiResult<TResponse, TError>
using FlightOrderManagement.Core.ErrorResponse; // RawError
using FlightOrderManagement.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FlightOrderManagement.Core.Exceptions`) — an auth *application* failure, not an API error — when the
