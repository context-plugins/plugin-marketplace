...
All types below live under `AdyenApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using AdyenApi.Core.Configuration;
...
```csharp
using AdyenApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new AdyenApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `AdyenApi.Core.Exceptions`):
...
With DI, the SDK's `AddAdyenApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAdyenApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
