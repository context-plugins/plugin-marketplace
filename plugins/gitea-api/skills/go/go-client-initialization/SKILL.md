...
- `giteaapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/gitea-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/gitea-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/gitea-api-go-sdk"

client := giteaapi.NewClient(
    giteaapi.CreateConfiguration(
        giteaapi.WithEnvironment(giteaapi.PRODUCTION),
...
from environment variables named `GITEAAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := giteaapi.NewClient(giteaapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `giteaapi.PRODUCTION`,
`giteaapi.ENVIRONMENT2`) — only the first is named meaningfully, further servers are positional
...
```go
client := giteaapi.NewClient(
    giteaapi.CreateConfiguration(giteaapi.WithEnvironment(giteaapi.PRODUCTION)),
)
```
...
client := giteaapi.NewClient(
    giteaapi.CreateConfiguration(
        giteaapi.WithHttpConfiguration(
            giteaapi.CreateHttpConfiguration(
                giteaapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                giteaapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = giteaapi.NewClient(giteaapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api giteaapi.ClientInterface
}

func NewService(api giteaapi.ClientInterface) *Service {
