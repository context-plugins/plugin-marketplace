...
```go
mediumapi.WithOAuthCCGCredentials(
    mediumapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr mediumapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := mediumapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr mediumapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := mediumapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := mediumapi.NewClient(mediumapi.CreateConfiguration(mediumapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    mediumapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := mediumapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := mediumapi.NewClient(mediumapi.CreateConfiguration(mediumapi.WithOAuthROPCGCredentials(creds)))
...
```go
mediumapi.WithOAuthBearerTokenCredentials(mediumapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
mediumapi.WithBasicAuthCredentials(mediumapi.NewBasicAuthCredentials(username, password))
mediumapi.WithApiHeaderCredentials(mediumapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
mediumapi.WithApiKeyCredentials(mediumapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `MEDIUMAPI_...` variables — e.g.
`MEDIUMAPI_USERNAME`, `MEDIUMAPI_PASSWORD`, `MEDIUMAPI_TOKEN`, `MEDIUMAPI_API_KEY`,
`MEDIUMAPI_O_AUTH_CLIENT_ID`, `MEDIUMAPI_O_AUTH_CLIENT_SECRET`, `MEDIUMAPI_ACCESS_TOKEN`. Grep the
