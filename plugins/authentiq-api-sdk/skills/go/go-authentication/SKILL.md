...
> Throughout, `authentiqapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := authentiqapi.NewClient(
    authentiqapi.CreateConfiguration(
        authentiqapi.WithBasicAuthCredentials(
            authentiqapi.NewBasicAuthCredentials(
...
authentiqapi.WithApiKeyCredentials(authentiqapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
authentiqapi.WithApiHeaderCredentials(authentiqapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
authentiqapi.WithOAuthBearerTokenCredentials(
    authentiqapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := authentiqapi.NewClient(
    authentiqapi.CreateConfiguration(
        authentiqapi.WithOAuthCCGCredentials(
            authentiqapi.NewOAuthCCGCredentials(
