...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ThePlaidAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`theplaidapi/configuration.py` in the cloned source.
...
```python
import requests
from theplaidapi.configuration import Configuration
...
`theplaidapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`theplaidapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `theplaidapi/configuration.py`
...
`NumberPagedResponse`) live in `theplaidapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `theplaidapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
