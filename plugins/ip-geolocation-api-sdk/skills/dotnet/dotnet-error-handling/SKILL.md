...
> `{ApiGroup}`, `IPGeolocationAPI`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `IPGeolocationAPI.Core.Exceptions`
- `ApiError` **and** `RawError` → `IPGeolocationAPI.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `IPGeolocationAPI.Errors`
...
```csharp
using IPGeolocationAPI.Core.Exceptions;     // SdkException<TError>
using IPGeolocationAPI.Core.ErrorResponse;  // ApiError, RawError
using IPGeolocationAPI.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`IPGeolocationAPI.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using IPGeolocationAPI.Core.Exceptions;     // SdkException<TError>
using IPGeolocationAPI.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `IPGeolocationAPI.Core.ErrorResponse`. Its public
...
`readonly struct` under `IPGeolocationAPI.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using IPGeolocationAPI.Core.Models;        // ApiResult<TResponse, TError>
using IPGeolocationAPI.Core.ErrorResponse; // RawError
using IPGeolocationAPI.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `IPGeolocationAPI.Core.Exceptions`) — an auth *application* failure, not an API error — when the
