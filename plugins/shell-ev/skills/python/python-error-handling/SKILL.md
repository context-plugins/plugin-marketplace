...
`shellev/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `shellev/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `shellev/exceptions/`.
...
file (`shellev/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`shellev/exceptions/` for the class.
...
`shellev/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `shellev/exceptions/`
  or a `ValueError` — check the auth handler in `shellev/http/auth/` for the exact type.
