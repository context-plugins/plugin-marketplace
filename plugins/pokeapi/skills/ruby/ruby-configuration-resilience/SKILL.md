...
```ruby
client = PokeApi::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = PokeApi::Client.new(adapter: :net_http)
```
...
client = PokeApi::Client.new(connection: conn)
```
...
client = PokeApi::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = PokeApi::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = PokeApi::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = PokeApi::ProxySettings.from_env
client = PokeApi::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < PokeApi::HttpCallBack
...
client = PokeApi::Client.new(http_callback: LogCallback.new)
```
...
client = PokeApi::Client.from_env(timeout: 30, max_retries: 3)
```
