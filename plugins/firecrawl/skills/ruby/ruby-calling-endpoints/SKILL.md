...
Open `lib/firecrawl/client.rb` for the accessor names, then the relevant
`lib/firecrawl/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = Firecrawl::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: Firecrawl::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/firecrawl/models/`. Field names are defined as
...
Read the enum module in `lib/firecrawl/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/firecrawl/client.rb`.
- Operation methods are in `lib/firecrawl/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/firecrawl/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/firecrawl/exceptions/`.
