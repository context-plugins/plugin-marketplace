...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new KlarnaPaymentsAPIV1Client({
...
```typescript
import { {Operation}Error } from 'klarna-payments-api-v-1-lib/errors';
...
```typescript
import { ApiError } from 'klarna-payments-api-v-1-lib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(KlarnaPaymentsAPIV1Client)
    .useValue(new KlarnaPaymentsAPIV1Client({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
