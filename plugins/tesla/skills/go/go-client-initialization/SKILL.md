...
- `tesla` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/tesla-go-sdk` — the Go import path for the root package; `github.com/context-plugins/tesla-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/tesla-go-sdk"

client := tesla.NewClient(
    tesla.CreateConfiguration(
        tesla.WithEnvironment(tesla.PRODUCTION),
...
from environment variables named `TESLA_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := tesla.NewClient(tesla.CreateConfigurationFromEnvironment())
```
...
```go
client := tesla.NewClient(
    tesla.CreateConfiguration(tesla.WithEnvironment(tesla.PRODUCTION)),
)
```
...
client := tesla.NewClient(
    tesla.CreateConfiguration(
        tesla.WithHttpConfiguration(
            tesla.CreateHttpConfiguration(
                tesla.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                tesla.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = tesla.NewClient(tesla.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api tesla.ClientInterface
}

func NewService(api tesla.ClientInterface) *Service {
