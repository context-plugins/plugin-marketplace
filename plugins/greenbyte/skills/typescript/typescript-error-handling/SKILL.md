...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `greenbyte-apilib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'greenbyte-apilib';
import { ApiError } from 'greenbyte-apilib';
import { {Operation}Error } from 'greenbyte-apilib/errors';
