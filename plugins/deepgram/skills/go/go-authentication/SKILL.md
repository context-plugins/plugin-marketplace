...
> Throughout, `deepgram` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := deepgram.NewClient(
    deepgram.CreateConfiguration(
        deepgram.WithBasicAuthCredentials(
            deepgram.NewBasicAuthCredentials(
...
deepgram.WithApiKeyCredentials(deepgram.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
deepgram.WithApiHeaderCredentials(deepgram.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
deepgram.WithOAuthBearerTokenCredentials(
    deepgram.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := deepgram.NewClient(
    deepgram.CreateConfiguration(
        deepgram.WithOAuthCCGCredentials(
            deepgram.NewOAuthCCGCredentials(
