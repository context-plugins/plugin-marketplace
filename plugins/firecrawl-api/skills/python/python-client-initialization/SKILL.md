...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the FirecrawlapiClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write FirecrawlapiClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `FirecrawlapiClient` is the SDK's client class — **read the real name** from `firecrawlapi/firecrawlapi_client.py`
...
- `firecrawlapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `FirecrawlapiClient` — the generated client class (read it from the `class …Client` declaration in `firecrawlapi/firecrawlapi_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `FirecrawlapiClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from firecrawlapi.firecrawlapi_client import FirecrawlapiClient
from firecrawlapi.configuration import Environment
from firecrawlapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from firecrawlapi.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `firecrawlapi/firecrawlapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `FirecrawlapiClient` and `Configuration` that reads
...
client = FirecrawlapiClient.from_environment()
...
`from_environment` in `firecrawlapi/configuration.py` for the exact names.
...
Open `firecrawlapi/firecrawlapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `firecrawlapi/configuration.py`. **Read the enum in `configuration.py`
...
client = FirecrawlapiClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`firecrawlapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = FirecrawlapiClient(config=new_config)
```
