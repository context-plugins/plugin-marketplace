...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `TelegramBotAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`telegrambotapi/configuration.py` in the cloned source.
...
```python
import requests
from telegrambotapi.configuration import Configuration
...
`telegrambotapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`telegrambotapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `telegrambotapi/configuration.py`
...
`NumberPagedResponse`) live in `telegrambotapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `telegrambotapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
