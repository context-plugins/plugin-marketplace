...
```ruby
client = PokAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = PokAPI::Client.new(adapter: :net_http)
```
...
client = PokAPI::Client.new(connection: conn)
```
...
client = PokAPI::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = PokAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = PokAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = PokAPI::ProxySettings.from_env
client = PokAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < PokAPI::HttpCallBack
...
client = PokAPI::Client.new(http_callback: LogCallback.new)
```
...
client = PokAPI::Client.from_env(timeout: 30, max_retries: 3)
```
