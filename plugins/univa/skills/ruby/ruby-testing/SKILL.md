...
client = stub_client(stubs)
assert_raises Univa::APIException do
...
BASE = Univa::Configuration.new(environment: Univa::Environment::PRODUCTION).get_base_uri

def build_test_client
  Univa::Client.new(
    environment: Univa::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Univa::APIException
```
...
```ruby
expect { call }.to raise_error(Univa::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Univa::HttpCallBack
...
client  = Univa::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
