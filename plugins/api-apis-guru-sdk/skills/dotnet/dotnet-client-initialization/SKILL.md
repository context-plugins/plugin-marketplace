...
- `FlightMostBookedDestinationsClient` — the single public client class (e.g. `FooClient`).
- `FlightMostBookedDestinationsClientOptions` — its options class.
- `FlightMostBookedDestinations` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public FlightMostBookedDestinationsClient(HttpClient httpClient, FlightMostBookedDestinationsClientOptions options)
```
...
```csharp
public class FlightMostBookedDestinationsClientOptions
{
...
```csharp
using FlightMostBookedDestinations;
using FlightMostBookedDestinations.Servers;

var options = new FlightMostBookedDestinationsClientOptions
{
...
var client = new FlightMostBookedDestinationsClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddFlightMostBookedDestinationsClient`, which registers the
...
builder.Services.AddFlightMostBookedDestinationsClient(options =>
{
...
```csharp
public sealed class MyService(FlightMostBookedDestinationsClient client)
{
