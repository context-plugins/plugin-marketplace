...
> `{ApiGroup}`, `Klarna`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `Klarna.Core.Exceptions`
- `ApiError` **and** `RawError` → `Klarna.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `Klarna.Errors`
...
```csharp
using Klarna.Core.Exceptions;     // SdkException<TError>
using Klarna.Core.ErrorResponse;  // ApiError, RawError
using Klarna.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`Klarna.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using Klarna.Core.Exceptions;     // SdkException<TError>
using Klarna.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `Klarna.Core.ErrorResponse`. Its public
...
`readonly struct` under `Klarna.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using Klarna.Core.Models;        // ApiResult<TResponse, TError>
using Klarna.Core.ErrorResponse; // RawError
using Klarna.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `Klarna.Core.Exceptions`) — an auth *application* failure, not an API error — when the
