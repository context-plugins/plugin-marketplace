...
against the `ShellEVClient.Builder` setters, the `com.shell.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ShellEVClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ShellEVClient client = new ShellEVClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeShellEvEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
