...
kwarg names are generated per-API — confirm from `pokapi/pokapi_client.py` and `pokapi/http/auth/` in
the cloned source.

## Basic auth

```python
from pokapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from pokapi.http.auth.api_key import ApiKeyCredentials
...
```python
from pokapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from pokapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from pokapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from pokapi.http.auth.o_auth_acg import OAuthACGCredentials
from pokapi.models.o_auth_scope_pokapi_enum import OAuthScope{Pkg}Enum
...
```python
from pokapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `pokapi/configuration.py` and each `pokapi/http/auth/*.py` for the exact
