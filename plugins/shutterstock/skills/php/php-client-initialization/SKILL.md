...
description: Initialize an APIMatic-generated PHP API client — construct it from a config array (or options object) you supply, choose a server environment or override the base URL, and wire it into a PSR-11 container or plain instantiation. Use the moment you write `new ShutterstockApiExplorerClient(...)`, configure its options, pick an environment, or register the client in a DI container — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the Guzzle lifetime rules or environment constant names.
---

# Initializing an APIMatic PHP SDK client

> `ShutterstockApiExplorerClient` is the SDK's client class — **read the real name** from the `*Client.php` file in
> `src/` (it is derived from the API title with APIMatic's own casing, so do not guess it from the
> API name). `ShutterstockApiExplorerClientBuilder` sits beside it.

This applies to **any** APIMatic-generated PHP SDK. Replace placeholders with the real names from
the SDK you are using:

- `ShutterstockApiExplorerClient` — the single public client class (read it from the `*Client.php` file in `src/`).
- `ShutterstockApiExplorerLib` — the SDK's root namespace used in `use` directives.
...
```php
$client = new ShutterstockApiExplorerClient([
    'timeout'     => 30,
    'environment' => ShutterstockApiExplorerClient::ENVIRONMENT_PRODUCTION,
...
```php
use ShutterstockApiExplorerLib\ShutterstockApiExplorerClient;
...
a new `ShutterstockApiExplorerClient` per API call — Guzzle connection pooling is per-client-instance and per-request
construction leaks connections.

## Choosing the server / base URL

Environments are modeled as class constants on `ShutterstockApiExplorerClient` (e.g.
`ShutterstockApiExplorerClient::ENVIRONMENT_PRODUCTION`, `ShutterstockApiExplorerClient::ENVIRONMENT_SANDBOX`). Pass the constant in
...
$this->app->singleton(ShutterstockApiExplorerClient::class, function ($app) {
    return new ShutterstockApiExplorerClient([
        'timeout'     => 30,
        'environment' => ShutterstockApiExplorerClient::ENVIRONMENT_PRODUCTION,
...
class MyService
{
    public function __construct(private readonly ShutterstockApiExplorerClient $client) {}
