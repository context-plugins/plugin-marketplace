...
`testing` + `testify/require` for reference; substitute the real `automationmanagementclient` and names.
...
import (
    ...
    "github.com/context-plugins/automationmanagementclient-go"   // root package automationmanagementclient
)
...
func clientReturning(status int, body string) (automationmanagementclient.ClientInterface, *stubTransport) {
    ...
    client := automationmanagementclient.NewClient(
        automationmanagementclient.CreateConfiguration(
            automationmanagementclient.WithHttpConfiguration(
                automationmanagementclient.CreateHttpConfiguration(automationmanagementclient.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/automationmanagementclient-go/errors"
...
client := automationmanagementclient.NewClient(
    automationmanagementclient.CreateConfiguration(
        automationmanagementclient.WithHttpConfiguration(
            automationmanagementclient.CreateHttpConfiguration(automationmanagementclient.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
