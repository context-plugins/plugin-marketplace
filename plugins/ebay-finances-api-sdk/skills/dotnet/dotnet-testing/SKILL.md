...
`EBayFinancesApiClient`/`EBayFinancesApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `EBayFinancesApiClient`,
...
static EBayFinancesApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new EBayFinancesApiClient(new HttpClient(handler), new EBayFinancesApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using EBayFinancesApi.Errors;              // {Operation}Error types
...
using EBayFinancesApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new EBayFinancesApiClient(new HttpClient(handler), new EBayFinancesApiClientOptions());
...
- For DI-based code, the SDK's `AddEBayFinancesApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `EBayFinancesApiClient` from the provider:
  ```csharp
  services.AddEBayFinancesApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<EBayFinancesApiClient>();
  ```
