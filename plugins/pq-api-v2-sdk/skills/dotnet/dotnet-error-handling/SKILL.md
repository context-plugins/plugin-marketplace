...
> `{ApiGroup}`, `PqApiV2`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `PqApiV2.Core.Exceptions`
- `ApiError` **and** `RawError` → `PqApiV2.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `PqApiV2.Errors`
...
```csharp
using PqApiV2.Core.Exceptions;     // SdkException<TError>
using PqApiV2.Core.ErrorResponse;  // ApiError, RawError
using PqApiV2.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`PqApiV2.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using PqApiV2.Core.Exceptions;     // SdkException<TError>
using PqApiV2.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `PqApiV2.Core.ErrorResponse`. Its public
...
`readonly struct` under `PqApiV2.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using PqApiV2.Core.Models;        // ApiResult<TResponse, TError>
using PqApiV2.Core.ErrorResponse; // RawError
using PqApiV2.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `PqApiV2.Core.Exceptions`) — an auth *application* failure, not an API error — when the
