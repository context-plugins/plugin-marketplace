...
To see which schemes a specific SDK accepts, read the credential setter methods on `SlackClient.Builder`
...
- Set credentials **on the `SlackClient.Builder`** before calling `.build()` — the client is immutable
