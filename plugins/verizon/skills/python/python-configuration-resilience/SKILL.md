...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `VerizonClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`verizon/configuration.py` in the cloned source.
...
`verizon/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`verizon/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `verizon/configuration.py`
...
`NumberPagedResponse`) live in `verizon/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `verizon/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
