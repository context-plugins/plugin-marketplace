...
Open `vimeo/vimeo_client.py` for the controller property names, then the relevant
`vimeo/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from vimeo.models.{request_model} import {RequestModel}
...
Open `vimeo/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from vimeo.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `vimeo/pagination/` in the source.
