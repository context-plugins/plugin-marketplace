...
To see which schemes a specific SDK accepts, read the credential setter methods on `ApiApisGuruClient.Builder`
...
- Set credentials **on the `ApiApisGuruClient.Builder`** before calling `.build()` — the client is immutable
