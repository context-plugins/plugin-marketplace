...
your SDK source (`shellcardmanagement/models/`).
...
```python
from shellcardmanagement.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `shellcardmanagement/utilities/union_type_lookup.py`) and each candidate model
...
```python
from shellcardmanagement.models.animal import Cat, Dog
...
```python
from shellcardmanagement.models.animal import Animal, Cat, Dog
