...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `Tesserapiv1Client`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`tesserapiv1/configuration.py` in the cloned source.
...
```python
import requests
from tesserapiv1.configuration import Configuration
...
`tesserapiv1/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`tesserapiv1/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `tesserapiv1/configuration.py`
...
`NumberPagedResponse`) live in `tesserapiv1/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `tesserapiv1/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
