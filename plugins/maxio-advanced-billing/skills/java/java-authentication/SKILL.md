...
To see which schemes a specific SDK accepts, read the credential setter methods on `MaxioAdvancedBillingClient.Builder`
...
- Set credentials **on the `MaxioAdvancedBillingClient.Builder`** before calling `.build()` — the client is immutable
