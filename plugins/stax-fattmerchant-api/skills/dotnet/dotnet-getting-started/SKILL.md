...
| Project | `StaxFattMerchantApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using StaxFattMerchantApi.Models;` alone does **not** make enums, union types, or
...
```csharp
using StaxFattMerchantApi;          // client + options + ServerEnvironment
using StaxFattMerchantApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/stax-fattmerchant-api-csharp-sdk /tmp/stax-fattmerchant-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/stax-fattmerchant-api-csharp-sdk "$env:TEMP\stax-fattmerchant-api-dotnet-src"
```
...
- `StaxFattMerchantApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/stax-fattmerchant-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\stax-fattmerchant-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `StaxFattMerchantApiClient`, build its
   `StaxFattMerchantApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
