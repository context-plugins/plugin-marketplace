...
All types below live under `FinnhubApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FinnhubApi.Core.Configuration;
...
```csharp
using FinnhubApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FinnhubApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FinnhubApi.Core.Exceptions`):
...
With DI, the SDK's `AddFinnhubApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFinnhubApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
