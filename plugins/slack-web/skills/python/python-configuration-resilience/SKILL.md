...
`slackwebapi/configuration.py` in the cloned source.
...
```python
import requests
from slackwebapi.configuration import Configuration
...
`slackwebapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`slackwebapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `slackwebapi/configuration.py`
...
`NumberPagedResponse`) live in `slackwebapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `slackwebapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
