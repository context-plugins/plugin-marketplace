...
against the `FirecrawlClient.Builder` setters, the `dev.firecrawl.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FirecrawlClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
