...
`geoapify/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `geoapify/exceptions/` and operation docstrings.
...
```python
from geoapify.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `geoapify/exceptions/`.
...
```python
from geoapify.exceptions.api_exception import APIException
from geoapify.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`geoapify/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`geoapify/exceptions/` for the class.
...
`geoapify/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `geoapify/exceptions/`
  or a `ValueError` — check the auth handler in `geoapify/http/auth/` for the exact type.
