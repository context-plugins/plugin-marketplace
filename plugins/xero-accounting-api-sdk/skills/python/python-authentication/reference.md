...
kwargs to the `XeroAccountingAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `xeroaccountingapi/xeroaccountingapi_client.py` and `xeroaccountingapi/http/auth/` in
the cloned source.

## Basic auth

```python
from xeroaccountingapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from xeroaccountingapi.http.auth.api_key import ApiKeyCredentials
...
```python
from xeroaccountingapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from xeroaccountingapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from xeroaccountingapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from xeroaccountingapi.http.auth.o_auth_acg import OAuthACGCredentials
from xeroaccountingapi.models.o_auth_scope_xeroaccountingapi_enum import OAuthScope{Pkg}Enum
...
client = XeroAccountingAPIClient(o_auth_acg_credentials=creds)
...
```python
from xeroaccountingapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = XeroAccountingAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `XeroAccountingAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `xeroaccountingapi/configuration.py` and each `xeroaccountingapi/http/auth/*.py` for the exact
