    ...
    klarna.CreateConfiguration(
        klarna.WithHttpConfiguration(
            klarna.CreateHttpConfiguration(
                klarna.WithTimeout(30),
                klarna.WithRetryConfiguration(
                    klarna.CreateRetryConfiguration(
                        klarna.WithMaxRetryAttempts(3),
                        klarna.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := klarna.CreateRetryConfiguration(
    klarna.WithMaxRetryAttempts(5),
    klarna.WithRetryInterval(1),
    klarna.WithBackoffFactor(2),
    klarna.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    klarna.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := klarna.NewClient(
    klarna.CreateConfiguration(
        klarna.WithHttpConfiguration(
            klarna.CreateHttpConfiguration(
                klarna.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
