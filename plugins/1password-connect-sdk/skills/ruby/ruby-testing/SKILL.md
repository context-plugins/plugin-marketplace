...
client = stub_client(stubs)
assert_raises 1PasswordConnect::APIException do
...
def build_test_client
  1PasswordConnect::Client.new(
    environment: 1PasswordConnect::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error 1PasswordConnect::APIException
```
...
```ruby
expect { call }.to raise_error(1PasswordConnect::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < 1PasswordConnect::HttpCallBack
...
client  = 1PasswordConnect::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
