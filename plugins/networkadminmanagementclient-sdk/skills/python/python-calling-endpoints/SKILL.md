...
Open `networkadminmanagementclient/networkadminmanagementclient_client.py` for the controller property names, then the relevant
`networkadminmanagementclient/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from networkadminmanagementclient.models.{request_model} import {RequestModel}
...
Open `networkadminmanagementclient/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from networkadminmanagementclient.models.suite_code_enum import SuiteCodeEnum
...
```python
from networkadminmanagementclient.http.http_call_back import HttpCallBack
...
client = NetworkAdminManagementClientClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `networkadminmanagementclient/pagination/` in the source.

## Finding the right method

- Controller property names are on `NetworkAdminManagementClientClient` in `networkadminmanagementclient/networkadminmanagementclient_client.py`.
- Operation method signatures are in `networkadminmanagementclient/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `networkadminmanagementclient/models/`; enum types have `from_value()`.
