---
name: dotnet-klarna-getting-started
description: klarna .NET SDK identity and lookup layer for the dotnet-klarna-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the klarna .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-klarna-sdk` helper
...
| --- | --- |
| API | klarna |
| Source repo | https://github.com/context-plugins/klarna-csharp-sdk (branch `main`) |
| Root namespace | `Klarna` (the `using` namespace) |
| Client class | `KlarnaClient` |
| Options class | `KlarnaClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `KlarnaClientOptions`: `KlarnaApiKey: string?`, `BasicAuth: BasicAuthCredentials?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using Klarna.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/klarna-csharp-sdk
dotnet add reference klarna-csharp-sdk/Klarna.csproj
```
...
temp directory** (`<temp>/klarna-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/klarna-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/klarna-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/klarna-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\klarna-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/klarna-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/klarna-dotnet-src/`, **never** in the project repo, and its path never goes into
`klarna-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `KlarnaClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new KlarnaClient(...)`, build its options, or DI-register via
   `AddKlarnaClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `KlarnaClientOptions` — the map's *Servers & auth* section
