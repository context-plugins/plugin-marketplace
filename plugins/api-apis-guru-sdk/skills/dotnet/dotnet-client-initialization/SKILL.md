...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddSafePlaceClient extension. Use the moment you write `new SafePlaceClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `SafePlaceClient` — the single public client class (e.g. `FooClient`).
- `SafePlaceClientOptions` — its options class.
- `SafePlace` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public SafePlaceClient(HttpClient httpClient, SafePlaceClientOptions options)
```
...
```csharp
public class SafePlaceClientOptions
{
...
```csharp
using SafePlace;
using SafePlace.Servers;

var options = new SafePlaceClientOptions
{
...
var client = new SafePlaceClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddSafePlaceClient`, which registers the
...
builder.Services.AddSafePlaceClient(options =>
{
...
```csharp
public sealed class MyService(SafePlaceClient client)
{
