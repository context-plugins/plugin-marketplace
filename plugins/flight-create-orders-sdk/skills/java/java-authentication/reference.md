...
against the `FlightCreateOrdersClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FlightCreateOrdersClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FlightCreateOrdersClient client = new FlightCreateOrdersClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFlightCreateOrdersEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new FlightCreateOrdersClient.Builder()
