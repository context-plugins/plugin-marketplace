...
client = stub_client(stubs)
assert_raises Postnl::APIException do
...
BASE = Postnl::Configuration.new(environment: Postnl::Environment::PRODUCTION).get_base_uri

def build_test_client
  Postnl::Client.new(
    environment: Postnl::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Postnl::APIException
```
...
```ruby
expect { call }.to raise_error(Postnl::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Postnl::HttpCallBack
...
client  = Postnl::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
