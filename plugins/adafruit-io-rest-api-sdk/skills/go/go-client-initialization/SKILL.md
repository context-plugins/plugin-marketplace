...
- `adafruitiorestapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/adafruit-io-rest-api-go` — the Go import path for the root package; `github.com/context-plugins/adafruit-io-rest-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/adafruit-io-rest-api-go"

client := adafruitiorestapi.NewClient(
    adafruitiorestapi.CreateConfiguration(
        adafruitiorestapi.WithEnvironment(adafruitiorestapi.PRODUCTION),
...
from environment variables named `ADAFRUITIORESTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := adafruitiorestapi.NewClient(adafruitiorestapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `adafruitiorestapi.PRODUCTION`,
`adafruitiorestapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := adafruitiorestapi.NewClient(
    adafruitiorestapi.CreateConfiguration(adafruitiorestapi.WithEnvironment(adafruitiorestapi.PRODUCTION)),
)
```
...
client := adafruitiorestapi.NewClient(
    adafruitiorestapi.CreateConfiguration(
        adafruitiorestapi.WithHttpConfiguration(
            adafruitiorestapi.CreateHttpConfiguration(
                adafruitiorestapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                adafruitiorestapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = adafruitiorestapi.NewClient(adafruitiorestapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api adafruitiorestapi.ClientInterface
}

func NewService(api adafruitiorestapi.ClientInterface) *Service {
