...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `firecrawl`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'firecrawl';
import { ApiError } from 'firecrawl';
import { {Operation}Error } from 'firecrawl/errors';
