...
`twilioapis/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `twilioapis/exceptions/` and operation docstrings.
...
```python
from twilioapis.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `twilioapis/exceptions/`.
...
```python
from twilioapis.exceptions.api_exception import APIException
from twilioapis.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`twilioapis/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`twilioapis/exceptions/` for the class.
...
`twilioapis/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `twilioapis/exceptions/`
  or a `ValueError` — check the auth handler in `twilioapis/http/auth/` for the exact type.
