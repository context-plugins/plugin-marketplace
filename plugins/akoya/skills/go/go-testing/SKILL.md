...
`testing` + `testify/require` for reference; substitute the real `akoya` and names.
...
import (
    ...
    "github.com/context-plugins/akoya-go-sdk"   // root package akoya
)
...
func clientReturning(status int, body string) (akoya.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := akoya.NewClient(
        akoya.CreateConfiguration(
            akoya.WithHttpConfiguration(
                akoya.CreateHttpConfiguration(akoya.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/akoya-go-sdk/errors"
...
client := akoya.NewClient(
    akoya.CreateConfiguration(
        akoya.WithHttpConfiguration(
            akoya.CreateHttpConfiguration(akoya.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
