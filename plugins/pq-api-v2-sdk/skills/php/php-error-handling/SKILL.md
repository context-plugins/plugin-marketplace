...
> `{operation}`, `{apiGroup}`, `PayQuickerSDKLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `PayQuickerSDKLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use PayQuickerSDKLib\Exceptions\ApiException;
use PayQuickerSDKLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `PayQuickerSDKLib\Exceptions\` — read the source to
