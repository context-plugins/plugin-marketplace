...
`lib/tesla_fleet_management_api/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/tesla_fleet_management_api/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/tesla_fleet_management_api/http/auth/api_header.rb` and
