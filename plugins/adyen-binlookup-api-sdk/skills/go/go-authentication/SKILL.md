...
> Throughout, `adyenbinlookupapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := adyenbinlookupapi.NewClient(
    adyenbinlookupapi.CreateConfiguration(
        adyenbinlookupapi.WithBasicAuthCredentials(
            adyenbinlookupapi.NewBasicAuthCredentials(
...
adyenbinlookupapi.WithApiKeyCredentials(adyenbinlookupapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
adyenbinlookupapi.WithApiHeaderCredentials(adyenbinlookupapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
adyenbinlookupapi.WithOAuthBearerTokenCredentials(
    adyenbinlookupapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := adyenbinlookupapi.NewClient(
    adyenbinlookupapi.CreateConfiguration(
        adyenbinlookupapi.WithOAuthCCGCredentials(
            adyenbinlookupapi.NewOAuthCCGCredentials(
