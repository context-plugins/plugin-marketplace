...
def stub_client(stubs)
  ...
  DiscourseAPIDocumentation::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises DiscourseAPIDocumentation::APIException do
...
BASE = DiscourseAPIDocumentation::Configuration.new(environment: DiscourseAPIDocumentation::Environment::PRODUCTION).get_base_uri

def build_test_client
  DiscourseAPIDocumentation::Client.new(
    environment: DiscourseAPIDocumentation::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error DiscourseAPIDocumentation::APIException
```
...
```ruby
expect { call }.to raise_error(DiscourseAPIDocumentation::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < DiscourseAPIDocumentation::HttpCallBack
...
client  = DiscourseAPIDocumentation::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
...
- **Environment/base URL:** every environment constant resolves to a real remote host — no SDK has a
  localhost target, and many have only `PRODUCTION`. Get the prefix from the SDK itself
  (`get_base_uri`, or the `ENVIRONMENTS` hash in `configuration.rb`) and stub exactly that. A WebMock
  stub registered against a URL the SDK never calls silently never matches, and the request escapes to
  the real host.
