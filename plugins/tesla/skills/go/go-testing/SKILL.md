...
`testing` + `testify/require` for reference; substitute the real `teslaapi` and names.
...
import (
    ...
    "github.com/context-plugins/tesla-api-go"   // root package teslaapi
)
...
func clientReturning(status int, body string) (teslaapi.ClientInterface, *stubTransport) {
    ...
    client := teslaapi.NewClient(
        teslaapi.CreateConfiguration(
            teslaapi.WithHttpConfiguration(
                teslaapi.CreateHttpConfiguration(teslaapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/tesla-api-go/errors"
...
client := teslaapi.NewClient(
    teslaapi.CreateConfiguration(
        teslaapi.WithHttpConfiguration(
            teslaapi.CreateHttpConfiguration(teslaapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
