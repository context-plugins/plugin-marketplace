...
Open `azurecognitive/azurecognitive_client.py` for the controller property names, then the relevant
`azurecognitive/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from azurecognitive.models.{request_model} import {RequestModel}
...
Open `azurecognitive/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from azurecognitive.models.suite_code_enum import SuiteCodeEnum
...
```python
from azurecognitive.http.http_call_back import HttpCallBack
...
client = AzureCognitiveClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `azurecognitive/pagination/` in the source.

## Finding the right method

- Controller property names are on `AzureCognitiveClient` in `azurecognitive/azurecognitive_client.py`.
- Operation method signatures are in `azurecognitive/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `azurecognitive/models/`; enum types have `from_value()`.
