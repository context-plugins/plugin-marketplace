...
`testing` + `testify/require` for reference; substitute the real `mindbody` and names.
...
import (
    ...
    "github.com/context-plugins/mindbody-go-sdk"   // root package mindbody
)
...
func clientReturning(status int, body string) (mindbody.ClientInterface, *stubTransport) {
    ...
    client := mindbody.NewClient(
        mindbody.CreateConfiguration(
            mindbody.WithHttpConfiguration(
                mindbody.CreateHttpConfiguration(mindbody.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/mindbody-go-sdk/errors"
...
client := mindbody.NewClient(
    mindbody.CreateConfiguration(
        mindbody.WithHttpConfiguration(
            mindbody.CreateHttpConfiguration(mindbody.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
