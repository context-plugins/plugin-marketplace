...
- `open-meteo-weather-forecast-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'open-meteo-weather-forecast-apilib';
...
```ts
import { {Resource}Controller } from 'open-meteo-weather-forecast-apilib';
