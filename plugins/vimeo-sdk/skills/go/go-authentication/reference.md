...
```go
vimeo.WithOAuthCCGCredentials(
    vimeo.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr vimeo.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := vimeo.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr vimeo.OAuthCCGManager) models.OAuthToken {
...
```go
creds := vimeo.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := vimeo.NewClient(vimeo.CreateConfiguration(vimeo.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    vimeo.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := vimeo.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := vimeo.NewClient(vimeo.CreateConfiguration(vimeo.WithOAuthROPCGCredentials(creds)))
...
```go
vimeo.WithOAuthBearerTokenCredentials(vimeo.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
vimeo.WithBasicAuthCredentials(vimeo.NewBasicAuthCredentials(username, password))
vimeo.WithApiHeaderCredentials(vimeo.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
vimeo.WithApiKeyCredentials(vimeo.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `VIMEO_...` variables — e.g.
`VIMEO_USERNAME`, `VIMEO_PASSWORD`, `VIMEO_TOKEN`, `VIMEO_API_KEY`,
`VIMEO_O_AUTH_CLIENT_ID`, `VIMEO_O_AUTH_CLIENT_SECRET`, `VIMEO_ACCESS_TOKEN`. Grep the
