...
- `adyenbalancecontrolapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/adyen-balance-control-api-go` — the Go import path for the root package; `github.com/context-plugins/adyen-balance-control-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/adyen-balance-control-api-go"

client := adyenbalancecontrolapi.NewClient(
    adyenbalancecontrolapi.CreateConfiguration(
        adyenbalancecontrolapi.WithEnvironment(adyenbalancecontrolapi.PRODUCTION),
...
from environment variables named `ADYENBALANCECONTROLAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := adyenbalancecontrolapi.NewClient(adyenbalancecontrolapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `adyenbalancecontrolapi.PRODUCTION`,
`adyenbalancecontrolapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := adyenbalancecontrolapi.NewClient(
    adyenbalancecontrolapi.CreateConfiguration(adyenbalancecontrolapi.WithEnvironment(adyenbalancecontrolapi.PRODUCTION)),
)
```
...
client := adyenbalancecontrolapi.NewClient(
    adyenbalancecontrolapi.CreateConfiguration(
        adyenbalancecontrolapi.WithHttpConfiguration(
            adyenbalancecontrolapi.CreateHttpConfiguration(
                adyenbalancecontrolapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                adyenbalancecontrolapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = adyenbalancecontrolapi.NewClient(adyenbalancecontrolapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api adyenbalancecontrolapi.ClientInterface
}

func NewService(api adyenbalancecontrolapi.ClientInterface) *Service {
