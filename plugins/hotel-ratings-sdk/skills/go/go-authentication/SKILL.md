...
> Throughout, `hotelratings` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := hotelratings.NewClient(
    hotelratings.CreateConfiguration(
        hotelratings.WithBasicAuthCredentials(
            hotelratings.NewBasicAuthCredentials(
...
hotelratings.WithApiKeyCredentials(hotelratings.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
hotelratings.WithApiHeaderCredentials(hotelratings.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
hotelratings.WithOAuthBearerTokenCredentials(
    hotelratings.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := hotelratings.NewClient(
    hotelratings.CreateConfiguration(
        hotelratings.WithOAuthCCGCredentials(
            hotelratings.NewOAuthCCGCredentials(
