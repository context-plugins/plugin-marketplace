...
> `AuthenticationClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AuthenticationClient
{
...
```php
use AuthenticationLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AuthenticationLib\Exceptions\{OperationException};
use AuthenticationLib\Exceptions\ApiException;
...
app()->instance(AuthenticationClient::class, clientWithResponses([
