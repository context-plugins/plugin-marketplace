...
> Throughout, `pqapiv2` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := pqapiv2.NewClient(
    pqapiv2.CreateConfiguration(
        pqapiv2.WithBasicAuthCredentials(
            pqapiv2.NewBasicAuthCredentials(
...
pqapiv2.WithApiKeyCredentials(pqapiv2.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
pqapiv2.WithApiHeaderCredentials(pqapiv2.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
pqapiv2.WithOAuthBearerTokenCredentials(
    pqapiv2.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := pqapiv2.NewClient(
    pqapiv2.CreateConfiguration(
        pqapiv2.WithOAuthCCGCredentials(
            pqapiv2.NewOAuthCCGCredentials(
