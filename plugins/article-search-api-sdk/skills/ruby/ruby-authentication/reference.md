...
Confirm every name against `lib/article_search_api/client.rb`, `lib/article_search_api/configuration.rb`, the
`lib/article_search_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
