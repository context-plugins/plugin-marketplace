    ...
    discourse.CreateConfiguration(
        discourse.WithHttpConfiguration(
            discourse.CreateHttpConfiguration(
                discourse.WithTimeout(30),
                discourse.WithRetryConfiguration(
                    discourse.CreateRetryConfiguration(
                        discourse.WithMaxRetryAttempts(3),
                        discourse.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := discourse.CreateRetryConfiguration(
    discourse.WithMaxRetryAttempts(5),
    discourse.WithRetryInterval(1),
    discourse.WithBackoffFactor(2),
    discourse.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    discourse.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := discourse.NewClient(
    discourse.CreateConfiguration(
        discourse.WithHttpConfiguration(
            discourse.CreateHttpConfiguration(
                discourse.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
