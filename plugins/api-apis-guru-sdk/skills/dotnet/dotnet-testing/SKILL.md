...
`FlightDelayPredictionClient`/`FlightDelayPredictionClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `FlightDelayPredictionClient`,
...
static FlightDelayPredictionClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new FlightDelayPredictionClient(new HttpClient(handler), new FlightDelayPredictionClientOptions { /* auth not needed for stubs */ });
}
```
...
using FlightDelayPrediction.Errors;              // {Operation}Error types
...
```csharp
using FlightDelayPrediction.Core.Exceptions;
using FlightDelayPrediction.Core.ErrorResponse;
...
using FlightDelayPrediction.Core.ErrorResponse; // RawError (Case B)
...
var client = new FlightDelayPredictionClient(new HttpClient(handler), new FlightDelayPredictionClientOptions());
...
- For DI-based code, the SDK's `AddFlightDelayPredictionClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `FlightDelayPredictionClient` from the provider:
  ```csharp
  services.AddFlightDelayPredictionClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<FlightDelayPredictionClient>();
  ```
