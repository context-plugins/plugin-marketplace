...
> `GiteaClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): GiteaClient
{
...
```php
use GiteaLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use GiteaLib\Exceptions\{OperationException};
use GiteaLib\Exceptions\ApiException;
...
app()->instance(GiteaClient::class, clientWithResponses([
