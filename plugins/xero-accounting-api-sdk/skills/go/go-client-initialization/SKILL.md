...
- `xeroaccountingapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/xero-accounting-api-go` — the Go import path for the root package; `github.com/context-plugins/xero-accounting-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/xero-accounting-api-go"

client := xeroaccountingapi.NewClient(
    xeroaccountingapi.CreateConfiguration(
        xeroaccountingapi.WithEnvironment(xeroaccountingapi.PRODUCTION),
...
from environment variables named `XEROACCOUNTINGAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := xeroaccountingapi.NewClient(xeroaccountingapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `xeroaccountingapi.PRODUCTION`,
`xeroaccountingapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := xeroaccountingapi.NewClient(
    xeroaccountingapi.CreateConfiguration(xeroaccountingapi.WithEnvironment(xeroaccountingapi.PRODUCTION)),
)
```
...
client := xeroaccountingapi.NewClient(
    xeroaccountingapi.CreateConfiguration(
        xeroaccountingapi.WithHttpConfiguration(
            xeroaccountingapi.CreateHttpConfiguration(
                xeroaccountingapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                xeroaccountingapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = xeroaccountingapi.NewClient(xeroaccountingapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api xeroaccountingapi.ClientInterface
}

func NewService(api xeroaccountingapi.ClientInterface) *Service {
