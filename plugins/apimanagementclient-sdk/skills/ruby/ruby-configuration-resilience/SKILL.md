...
```ruby
client = ApiManagementClient::Client.new(
  max_retries:    3,
...
```ruby
client = ApiManagementClient::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ApiManagementClient::Client.new(adapter: :net_http)
```
...
client = ApiManagementClient::Client.new(connection: conn)
```
...
client = ApiManagementClient::Client.new(environment: Environment::PRODUCTION)
client = ApiManagementClient::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ApiManagementClient::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ApiManagementClient::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ApiManagementClient::ProxySettings.from_env
client = ApiManagementClient::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ApiManagementClient::HttpCallBack
...
client = ApiManagementClient::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ApiManagementClient::Client.from_env
# Overrides take precedence over env vars:
client = ApiManagementClient::Client.from_env(timeout: 30, max_retries: 3)
```
