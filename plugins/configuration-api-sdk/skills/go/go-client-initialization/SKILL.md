...
- `configurationapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/configuration-api-go` — the Go import path for the root package; `github.com/context-plugins/configuration-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/configuration-api-go"

client := configurationapi.NewClient(
    configurationapi.CreateConfiguration(
        configurationapi.WithEnvironment(configurationapi.PRODUCTION),
...
from environment variables named `CONFIGURATIONAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := configurationapi.NewClient(configurationapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `configurationapi.PRODUCTION`,
`configurationapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := configurationapi.NewClient(
    configurationapi.CreateConfiguration(configurationapi.WithEnvironment(configurationapi.PRODUCTION)),
)
```
...
client := configurationapi.NewClient(
    configurationapi.CreateConfiguration(
        configurationapi.WithHttpConfiguration(
            configurationapi.CreateHttpConfiguration(
                configurationapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                configurationapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = configurationapi.NewClient(configurationapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api configurationapi.ClientInterface
}

func NewService(api configurationapi.ClientInterface) *Service {
