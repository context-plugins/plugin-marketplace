...
`splititwebapiv3/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `splititwebapiv3/exceptions/` and operation docstrings.
...
```python
from splititwebapiv3.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `splititwebapiv3/exceptions/`.
...
```python
from splititwebapiv3.exceptions.api_exception import APIException
from splititwebapiv3.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`splititwebapiv3/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`splititwebapiv3/exceptions/` for the class.
...
`splititwebapiv3/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `splititwebapiv3/exceptions/`
  or a `ValueError` — check the auth handler in `splititwebapiv3/http/auth/` for the exact type.
