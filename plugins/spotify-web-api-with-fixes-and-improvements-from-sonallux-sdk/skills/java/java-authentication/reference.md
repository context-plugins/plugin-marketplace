...
against the `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder` setters, the `com.spotify.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient client = new SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeSpotifyWebAPIWithFixesAndImprovementsFromSonalluxEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder()
