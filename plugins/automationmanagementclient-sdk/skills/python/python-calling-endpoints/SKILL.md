...
Open `automationmanagementclient/automationmanagementclient_client.py` for the controller property names, then the relevant
`automationmanagementclient/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from automationmanagementclient.models.{request_model} import {RequestModel}
...
Open `automationmanagementclient/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from automationmanagementclient.models.suite_code_enum import SuiteCodeEnum
...
```python
from automationmanagementclient.http.http_call_back import HttpCallBack
...
client = AutomationManagementClientClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `automationmanagementclient/pagination/` in the source.

## Finding the right method

- Controller property names are on `AutomationManagementClientClient` in `automationmanagementclient/automationmanagementclient_client.py`.
- Operation method signatures are in `automationmanagementclient/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `automationmanagementclient/models/`; enum types have `from_value()`.
