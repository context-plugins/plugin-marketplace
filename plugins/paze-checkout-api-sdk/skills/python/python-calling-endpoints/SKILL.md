...
Open `pazecheckoutapi/pazecheckoutapi_client.py` for the controller property names, then the relevant
`pazecheckoutapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from pazecheckoutapi.models.{request_model} import {RequestModel}
...
Open `pazecheckoutapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from pazecheckoutapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from pazecheckoutapi.http.http_call_back import HttpCallBack
...
client = PazeCheckoutAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `pazecheckoutapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `PazeCheckoutAPIClient` in `pazecheckoutapi/pazecheckoutapi_client.py`.
- Operation method signatures are in `pazecheckoutapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `pazecheckoutapi/models/`; enum types have `from_value()`.
