...
To see which schemes a specific SDK accepts, read the credential setter methods on `MindbodyPushApiApiClient.Builder`
...
- Set credentials **on the `MindbodyPushApiApiClient.Builder`** before calling `.build()` — the client is immutable
