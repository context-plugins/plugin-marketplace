...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `PostnlecommerceClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`postnl/configuration.py` in the cloned source.
...
`postnl/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`postnl/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `postnl/configuration.py`
...
`NumberPagedResponse`) live in `postnl/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `postnl/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
