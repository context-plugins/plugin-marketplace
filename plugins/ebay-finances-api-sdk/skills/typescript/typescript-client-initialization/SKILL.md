...
- `ebay-finances-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'ebay-finances-apilib';
...
```ts
import { {Resource}Controller } from 'ebay-finances-apilib';
