...
```ruby
client = NetworkAdminManagementClient::Client.new(
  max_retries:    3,
...
```ruby
client = NetworkAdminManagementClient::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = NetworkAdminManagementClient::Client.new(adapter: :net_http)
```
...
client = NetworkAdminManagementClient::Client.new(connection: conn)
```
...
client = NetworkAdminManagementClient::Client.new(environment: Environment::PRODUCTION)
client = NetworkAdminManagementClient::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = NetworkAdminManagementClient::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = NetworkAdminManagementClient::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = NetworkAdminManagementClient::ProxySettings.from_env
client = NetworkAdminManagementClient::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < NetworkAdminManagementClient::HttpCallBack
...
client = NetworkAdminManagementClient::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = NetworkAdminManagementClient::Client.from_env
# Overrides take precedence over env vars:
client = NetworkAdminManagementClient::Client.from_env(timeout: 30, max_retries: 3)
```
