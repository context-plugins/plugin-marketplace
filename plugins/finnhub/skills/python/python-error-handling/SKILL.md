...
`finnhub/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `finnhub/exceptions/` and operation docstrings.
...
```python
from finnhub.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `finnhub/exceptions/`.
...
```python
from finnhub.exceptions.api_exception import APIException
from finnhub.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`finnhub/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`finnhub/exceptions/` for the class.
...
`finnhub/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `finnhub/exceptions/`
  or a `ValueError` — check the auth handler in `finnhub/http/auth/` for the exact type.
