...
`testing` + `testify/require` for reference; substitute the real `seatmapdisplay` and names.
...
import (
    ...
    "github.com/context-plugins/seatmap-display-go"   // root package seatmapdisplay
)
...
func clientReturning(status int, body string) (seatmapdisplay.ClientInterface, *stubTransport) {
    ...
    client := seatmapdisplay.NewClient(
        seatmapdisplay.CreateConfiguration(
            seatmapdisplay.WithHttpConfiguration(
                seatmapdisplay.CreateHttpConfiguration(seatmapdisplay.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/seatmap-display-go/errors"
...
client := seatmapdisplay.NewClient(
    seatmapdisplay.CreateConfiguration(
        seatmapdisplay.WithHttpConfiguration(
            seatmapdisplay.CreateHttpConfiguration(seatmapdisplay.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
