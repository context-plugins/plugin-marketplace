...
description: "Tesla Fleet Management API Python SDK identity and lookup layer (Python only) — install, import root, base URL/environments, the auth pattern, the SDK map that ships at the SDK root (`sdk-map.md` + `map/operations/`) and how to traverse it, and the module table naming the one file owning each fact the map leaves to the source. Load this before answering any Tesla Fleet Management API Python SDK contract question or writing any SDK code."
---

# Getting started with the Tesla Fleet Management API Python SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Tesla Fleet Management API Python SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source modules it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `python-integrate-tesla` for the workflow that wraps this skill.
...
Verified against `tesla_fleet_management_api/` and `pyproject.toml` of the generated package at version `1.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Tesla Fleet Management API |
| Distribution name (what you install) | `tesla-fleet-management-api` — **not on any package index**; installed from source (see *Install*) |
| Import root (what you import) | `tesla_fleet_management_api` — not the string you install |
| Source repository | https://github.com/context-plugins/tesla-python-sdk |
| Source branch | `main` |
| Version | `1.0.0` |
| Sync client class | `TeslaFleetManagementApiClient` (alias `Client`) |
| Async client class | `AsyncTeslaFleetManagementApiClient` (alias `AsyncClient`) |
| Client construction | **keyword-only**: `environment` · `base_url` · `timeout` (default `30.0`) · `bearer_auth` · `thirdpartytoken_authorization_code` · `thirdpartytoken_authorization_code_token_source` · `thirdpartytoken_client_credentials` · `thirdpartytoken_client_credentials_token_source`, and the transport override — `custom_http_client` on the sync client, `custom_async_http_client` on the async one (the names differ; see step 1) |
...
| Base-URL config | `ServerConfig` (`tesla_fleet_management_api/server/server_config.py`), frozen, `extra="forbid"` |
...
| Surface | 64 operations across 6 controllers · 84 models · 0 unions · 4 enums · 0 per-operation error unions |
...
```bash
pip install "tesla-fleet-management-api @ git+https://github.com/context-plugins/tesla-python-sdk.git@main"
```
...
Python does not re-export child modules transitively, so `from tesla_fleet_management_api import models` alone does **not** make enums, error unions, or runtime types reachable. Import each kind of type from the module that owns it.

`tesla_fleet_management_api/__init__.py` exports exactly 6 names beside the `tesla_fleet_management_api.models` subpackage it re-exports:

```python
from tesla_fleet_management_api import (
    AsyncClient,
...
| --- | --- |
| Domain models, their `…Dict` companions | `tesla_fleet_management_api.models` |
| Enums (and their open `…OrStr` aliases) | `tesla_fleet_management_api.models.enums` |
| `ApiError` · `RawError` · `ApiResult` · `RequestOptions` · `AuthorizationCodeCredentials` · `ClientCredentials` · `HttpClient` · `SdkBaseModel` · `UNSET` · `Optional` | `tesla_fleet_management_api.core` |
| Per-operation error *unions* | `tesla_fleet_management_api.errors` — *this SDK declares none* |

`tesla_fleet_management_api.core` re-exports its whole public surface (a curated `__all__`), so import from `…core` rather than from the private modules beneath it (`…core.results`, `…core.exceptions`, `…core.auth.schemes`).
...
```python
from tesla_fleet_management_api import Client
...
```python
from tesla_fleet_management_api import Client
from tesla_fleet_management_api.core import AuthorizationCodeCredentials

def prompt(url: str) -> str:
    return input(f"Open {url}, then paste the code: ")

client = Client(thirdpartytoken_authorization_code=AuthorizationCodeCredentials(client_id="…", client_secret="…", redirect_uri="…", prompt_for_authorization_code=prompt))
client = Client(thirdpartytoken_authorization_code={"client_id": "…", "client_secret": "…", "redirect_uri": "…", "prompt_for_authorization_code": prompt})   # equivalent
```
...
```python
from tesla_fleet_management_api import Client
from tesla_fleet_management_api.core import ClientCredentials
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `tesla_fleet_management_api/` source directory, and these two entries:
...
Every `Source` path on the map is relative to that SDK root, so `tesla_fleet_management_api/models/actuate_trunk_request.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
```bash
python -c "import tesla_fleet_management_api, pathlib; print(pathlib.Path(tesla_fleet_management_api.__file__).parent)"
```

Failing that, it is under the project's environment (`.venv/Lib/site-packages/tesla_fleet_management_api` on Windows, `.venv/lib/python3.*/site-packages/tesla_fleet_management_api` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
