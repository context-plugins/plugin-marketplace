...
To see which schemes a specific SDK accepts, read the credential setter methods on `GiteaClient.Builder`
...
- Set credentials **on the `GiteaClient.Builder`** before calling `.build()` — the client is immutable
