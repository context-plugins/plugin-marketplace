...
To see which schemes a specific SDK accepts, read the credential setter methods on `TeslaFleetManagementApiClient.Builder`
...
- Set credentials **on the `TeslaFleetManagementApiClient.Builder`** before calling `.build()` — the client is immutable
