...
> `{operation}`, `{apiGroup}`, `SafePlaceLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `SafePlaceLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use SafePlaceLib\Exceptions\ApiException;
...
```php
use SafePlaceLib\Exceptions\ApiException;
use SafePlaceLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `SafePlaceLib\Exceptions\` — read the source to
