...
│   └── com.adyen.caltest.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.adyen.caltest.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.adyen.caltest.http.client.HttpContext`, which carries:
