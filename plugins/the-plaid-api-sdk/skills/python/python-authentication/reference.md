...
kwargs to the `ThePlaidAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `theplaidapi/theplaidapi_client.py` and `theplaidapi/http/auth/` in
the cloned source.

## Basic auth

```python
from theplaidapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from theplaidapi.http.auth.api_key import ApiKeyCredentials
...
```python
from theplaidapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from theplaidapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from theplaidapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from theplaidapi.http.auth.o_auth_acg import OAuthACGCredentials
from theplaidapi.models.o_auth_scope_theplaidapi_enum import OAuthScope{Pkg}Enum
...
client = ThePlaidAPIClient(o_auth_acg_credentials=creds)
...
```python
from theplaidapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ThePlaidAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ThePlaidAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `theplaidapi/configuration.py` and each `theplaidapi/http/auth/*.py` for the exact
