...
> `{ApiGroup}`, `RestApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `RestApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `RestApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `RestApi.Errors`
...
```csharp
using RestApi.Core.Exceptions;     // SdkException<TError>
using RestApi.Core.ErrorResponse;  // ApiError, RawError
using RestApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`RestApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using RestApi.Core.Exceptions;     // SdkException<TError>
using RestApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `RestApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `RestApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using RestApi.Core.Models;        // ApiResult<TResponse, TError>
using RestApi.Core.ErrorResponse; // RawError
using RestApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `RestApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
