...
against the `AdyenBinLookupAPIClient.Builder` setters, the `com.adyen.paltest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AdyenBinLookupAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AdyenBinLookupAPIClient client = new AdyenBinLookupAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAdyenBinLookupAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new AdyenBinLookupAPIClient.Builder()
