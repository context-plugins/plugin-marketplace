...
`pokapi/configuration.py` in the cloned source.
...
```python
import requests
from pokapi.configuration import Configuration
...
`pokapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`pokapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `pokapi/configuration.py`
...
`NumberPagedResponse`) live in `pokapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `pokapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
