...
```go
googlemapsplatform.WithOAuthCCGCredentials(
    googlemapsplatform.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr googlemapsplatform.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := googlemapsplatform.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr googlemapsplatform.OAuthCCGManager) models.OAuthToken {
...
```go
creds := googlemapsplatform.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := googlemapsplatform.NewClient(googlemapsplatform.CreateConfiguration(googlemapsplatform.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    googlemapsplatform.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := googlemapsplatform.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := googlemapsplatform.NewClient(googlemapsplatform.CreateConfiguration(googlemapsplatform.WithOAuthROPCGCredentials(creds)))
...
```go
googlemapsplatform.WithOAuthBearerTokenCredentials(googlemapsplatform.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
googlemapsplatform.WithBasicAuthCredentials(googlemapsplatform.NewBasicAuthCredentials(username, password))
googlemapsplatform.WithApiHeaderCredentials(googlemapsplatform.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
googlemapsplatform.WithApiKeyCredentials(googlemapsplatform.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `GOOGLEMAPSPLATFORM_...` variables — e.g.
`GOOGLEMAPSPLATFORM_USERNAME`, `GOOGLEMAPSPLATFORM_PASSWORD`, `GOOGLEMAPSPLATFORM_TOKEN`, `GOOGLEMAPSPLATFORM_API_KEY`,
`GOOGLEMAPSPLATFORM_O_AUTH_CLIENT_ID`, `GOOGLEMAPSPLATFORM_O_AUTH_CLIENT_SECRET`, `GOOGLEMAPSPLATFORM_ACCESS_TOKEN`. Grep the
