...
| Project | `FlightPriceAnalysisApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `FlightPriceAnalysisApi` (the `using` namespace) |
| Client class | `FlightPriceAnalysisApiClient` |
| Options class | `FlightPriceAnalysisApiClientOptions` |
...
namespaces transitively, so `using FlightPriceAnalysisApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/flight-price-analysis-api-csharp
dotnet add reference flight-price-analysis-api-csharp/FlightPriceAnalysisApi.csproj
```

```csharp
using FlightPriceAnalysisApi;          // client + options + ServerEnvironment
using FlightPriceAnalysisApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/flight-price-analysis-api-csharp /tmp/flight-price-analysis-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/flight-price-analysis-api-csharp "$env:TEMP\flight-price-analysis-api-dotnet-src"
```
...
- `FlightPriceAnalysisApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/flight-price-analysis-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\flight-price-analysis-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `FlightPriceAnalysisApiClient`, build its
   `FlightPriceAnalysisApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
