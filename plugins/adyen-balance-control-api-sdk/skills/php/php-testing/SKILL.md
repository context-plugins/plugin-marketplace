...
> `AdyenBalanceControlAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AdyenBalanceControlAPIClient
{
...
```php
use AdyenBalanceControlAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AdyenBalanceControlAPILib\Exceptions\{OperationException};
use AdyenBalanceControlAPILib\Exceptions\ApiException;
...
app()->instance(AdyenBalanceControlAPIClient::class, clientWithResponses([
