...
- `XeroAccountingAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `xero_accounting_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'xero_accounting_api'
include XeroAccountingAPI
...
The authoritative list is the `initialize` signature in `lib/xero_accounting_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/xero_accounting_api/configuration.rb`
(`build_default_config_from_env`) and `lib/xero_accounting_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/xero_accounting_api.rb`) and store it in a
