...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `EbaysellClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`ebaysell/configuration.py` in the cloned source.
...
```python
import requests
from ebaysell.configuration import Configuration
...
`ebaysell/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`ebaysell/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `ebaysell/configuration.py`
...
`NumberPagedResponse`) live in `ebaysell/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `ebaysell/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
