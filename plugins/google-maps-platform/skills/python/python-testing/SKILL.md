...
reference; substitute your `googlemapsplatform` and `GooglemapsplatformClient` names.
...
- **Get the stub URL from the SDK, not from this page.** Every environment resolves to a real remote
  host — read the `environments` dict in `googlemapsplatform/configuration.py`, or call
