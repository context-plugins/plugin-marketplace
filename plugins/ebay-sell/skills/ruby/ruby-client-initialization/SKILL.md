...
- `EbaySell` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `ebay_sell` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/ebay_sell/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/ebay_sell/configuration.rb`
(`build_default_config_from_env`) and `lib/ebay_sell/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = EbaySell::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/ebay_sell.rb`) and store it in a
