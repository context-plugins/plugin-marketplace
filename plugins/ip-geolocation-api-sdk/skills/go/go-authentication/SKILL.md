...
> Throughout, `ipgeolocationapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := ipgeolocationapi.NewClient(
    ipgeolocationapi.CreateConfiguration(
        ipgeolocationapi.WithBasicAuthCredentials(
            ipgeolocationapi.NewBasicAuthCredentials(
...
ipgeolocationapi.WithApiKeyCredentials(ipgeolocationapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
ipgeolocationapi.WithApiHeaderCredentials(ipgeolocationapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
ipgeolocationapi.WithOAuthBearerTokenCredentials(
    ipgeolocationapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := ipgeolocationapi.NewClient(
    ipgeolocationapi.CreateConfiguration(
        ipgeolocationapi.WithOAuthCCGCredentials(
            ipgeolocationapi.NewOAuthCCGCredentials(
