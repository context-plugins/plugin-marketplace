...
kwargs to the `DeepgramClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `deepgram/deepgram_client.py` and `deepgram/http/auth/` in
the cloned source.

## Basic auth

```python
from deepgram.http.auth.basic_auth import BasicAuthCredentials
...
```python
from deepgram.http.auth.api_key import ApiKeyCredentials
...
```python
from deepgram.http.auth.api_header import ApiHeaderCredentials
...
```python
from deepgram.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from deepgram.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from deepgram.http.auth.o_auth_acg import OAuthACGCredentials
from deepgram.models.o_auth_scope_deepgram_enum import OAuthScope{Pkg}Enum
...
client = DeepgramClient(o_auth_acg_credentials=creds)
...
```python
from deepgram.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = DeepgramClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `DeepgramClient.from_environment()`) read each credential
...
Grep `from_environment` in `deepgram/configuration.py` and each `deepgram/http/auth/*.py` for the exact
