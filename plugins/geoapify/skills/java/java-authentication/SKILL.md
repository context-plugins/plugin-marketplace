...
To see which schemes a specific SDK accepts, read the credential setter methods on `GeoapifyClient.Builder`
...
- Set credentials **on the `GeoapifyClient.Builder`** before calling `.build()` — the client is immutable
