...
To see which schemes a specific SDK accepts, read the credential setter methods on `GreenbyteClient.Builder`
...
- Set credentials **on the `GreenbyteClient.Builder`** before calling `.build()` — the client is immutable
