...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddM3commasIoClient extension. Use the moment you write `new M3commasIoClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `M3commasIoClient` — the single public client class (e.g. `FooClient`).
- `M3commasIoClientOptions` — its options class.
- `M3commasIo` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public M3commasIoClient(HttpClient httpClient, M3commasIoClientOptions options)
```
...
```csharp
public class M3commasIoClientOptions
{
...
```csharp
using M3commasIo;
using M3commasIo.Servers;

var options = new M3commasIoClientOptions
{
...
var client = new M3commasIoClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddM3commasIoClient`, which registers the
...
builder.Services.AddM3commasIoClient(options =>
{
...
```csharp
public sealed class MyService(M3commasIoClient client)
{
