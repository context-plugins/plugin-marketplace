...
client = stub_client(stubs)
assert_raises GiteaAPI::APIException do
...
def build_test_client
  GiteaAPI::Client.new(
    environment: GiteaAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error GiteaAPI::APIException
```
...
```ruby
expect { call }.to raise_error(GiteaAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < GiteaAPI::HttpCallBack
...
client  = GiteaAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
