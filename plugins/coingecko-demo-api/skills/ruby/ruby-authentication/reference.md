...
Confirm every name against `lib/coingecko_demo_api/client.rb`, `lib/coingecko_demo_api/configuration.rb`, the
`lib/coingecko_demo_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
