...
Open `alpaca/alpaca_client.py` for the controller property names, then the relevant
`alpaca/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from alpaca.models.{request_model} import {RequestModel}
...
Open `alpaca/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from alpaca.models.suite_code_enum import SuiteCodeEnum
...
```python
from alpaca.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `alpaca/pagination/` in the source.

## Finding the right method

- Controller property names are on `TraderApiClient` in `alpaca/alpaca_client.py`.
- Operation method signatures are in `alpaca/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `alpaca/models/`; enum types have `from_value()`.
