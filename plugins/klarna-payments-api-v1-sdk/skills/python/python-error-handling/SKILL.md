...
`klarnapaymentsapiv1/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `klarnapaymentsapiv1/exceptions/` and operation docstrings.
...
```python
from klarnapaymentsapiv1.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `klarnapaymentsapiv1/exceptions/`.
...
```python
from klarnapaymentsapiv1.exceptions.api_exception import APIException
from klarnapaymentsapiv1.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`klarnapaymentsapiv1/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`klarnapaymentsapiv1/exceptions/` for the class.
...
`klarnapaymentsapiv1/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `klarnapaymentsapiv1/exceptions/`
  or a `ValueError` — check the auth handler in `klarnapaymentsapiv1/http/auth/` for the exact type.
