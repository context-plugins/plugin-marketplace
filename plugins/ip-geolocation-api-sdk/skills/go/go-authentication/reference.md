...
```go
ipgeolocationapi.WithOAuthCCGCredentials(
    ipgeolocationapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr ipgeolocationapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := ipgeolocationapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr ipgeolocationapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := ipgeolocationapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := ipgeolocationapi.NewClient(ipgeolocationapi.CreateConfiguration(ipgeolocationapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    ipgeolocationapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := ipgeolocationapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := ipgeolocationapi.NewClient(ipgeolocationapi.CreateConfiguration(ipgeolocationapi.WithOAuthROPCGCredentials(creds)))
...
```go
ipgeolocationapi.WithOAuthBearerTokenCredentials(ipgeolocationapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
ipgeolocationapi.WithBasicAuthCredentials(ipgeolocationapi.NewBasicAuthCredentials(username, password))
ipgeolocationapi.WithApiHeaderCredentials(ipgeolocationapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
ipgeolocationapi.WithApiKeyCredentials(ipgeolocationapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `IPGEOLOCATIONAPI_...` variables — e.g.
`IPGEOLOCATIONAPI_USERNAME`, `IPGEOLOCATIONAPI_PASSWORD`, `IPGEOLOCATIONAPI_TOKEN`, `IPGEOLOCATIONAPI_API_KEY`,
`IPGEOLOCATIONAPI_O_AUTH_CLIENT_ID`, `IPGEOLOCATIONAPI_O_AUTH_CLIENT_SECRET`, `IPGEOLOCATIONAPI_ACCESS_TOKEN`. Grep the
