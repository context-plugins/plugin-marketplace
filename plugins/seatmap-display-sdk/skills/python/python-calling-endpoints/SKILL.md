...
Open `seatmapdisplay/seatmapdisplay_client.py` for the controller property names, then the relevant
`seatmapdisplay/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from seatmapdisplay.models.{request_model} import {RequestModel}
...
Open `seatmapdisplay/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from seatmapdisplay.models.suite_code_enum import SuiteCodeEnum
...
```python
from seatmapdisplay.http.http_call_back import HttpCallBack
...
client = SeatmapDisplayClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `seatmapdisplay/pagination/` in the source.

## Finding the right method

- Controller property names are on `SeatmapDisplayClient` in `seatmapdisplay/seatmapdisplay_client.py`.
- Operation method signatures are in `seatmapdisplay/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `seatmapdisplay/models/`; enum types have `from_value()`.
