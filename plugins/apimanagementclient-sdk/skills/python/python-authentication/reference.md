...
kwargs to the `ApiManagementClientClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `apimanagementclient/apimanagementclient_client.py` and `apimanagementclient/http/auth/` in
the cloned source.

## Basic auth

```python
from apimanagementclient.http.auth.basic_auth import BasicAuthCredentials
...
```python
from apimanagementclient.http.auth.api_key import ApiKeyCredentials
...
```python
from apimanagementclient.http.auth.api_header import ApiHeaderCredentials
...
```python
from apimanagementclient.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from apimanagementclient.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from apimanagementclient.http.auth.o_auth_acg import OAuthACGCredentials
from apimanagementclient.models.o_auth_scope_apimanagementclient_enum import OAuthScope{Pkg}Enum
...
client = ApiManagementClientClient(o_auth_acg_credentials=creds)
...
```python
from apimanagementclient.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ApiManagementClientClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ApiManagementClientClient.from_environment()`) read each credential
...
Grep `from_environment` in `apimanagementclient/configuration.py` and each `apimanagementclient/http/auth/*.py` for the exact
