...
> Throughout, `klarna` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := klarna.NewClient(
    klarna.CreateConfiguration(
        klarna.WithBasicAuthCredentials(
            klarna.NewBasicAuthCredentials(
...
klarna.WithApiKeyCredentials(klarna.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
klarna.WithApiHeaderCredentials(klarna.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
klarna.WithOAuthBearerTokenCredentials(
    klarna.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
