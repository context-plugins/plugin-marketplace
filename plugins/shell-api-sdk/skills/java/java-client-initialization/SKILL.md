...
- `ShellAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`ShellAPIClient.Builder` class. A minimal initialization:

```java
import com.shell.apitest.ShellAPIClient;
import com.shell.apitest.Environment;

ShellAPIClient client = new ShellAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `ShellAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `ShellAPIClient.java` |

Call `.build()` to produce an immutable `ShellAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public ShellAPIClient apiClient() {
        return new ShellAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired ShellAPIClient client) {
        client.shutdown();
    }
}
```

Inject `ShellAPIClient` into your services as a normal Spring dependency. Build only one bean.
