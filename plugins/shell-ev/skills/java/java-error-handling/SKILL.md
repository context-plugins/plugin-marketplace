...
│   └── com.shell.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.shell.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.shell.api.http.client.HttpContext`, which carries:
