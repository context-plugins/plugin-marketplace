    ...
    finnhub.CreateConfiguration(
        finnhub.WithHttpConfiguration(
            finnhub.CreateHttpConfiguration(
                finnhub.WithTimeout(30),
                finnhub.WithRetryConfiguration(
                    finnhub.CreateRetryConfiguration(
                        finnhub.WithMaxRetryAttempts(3),
                        finnhub.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := finnhub.CreateRetryConfiguration(
    finnhub.WithMaxRetryAttempts(5),
    finnhub.WithRetryInterval(1),
    finnhub.WithBackoffFactor(2),
    finnhub.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    finnhub.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := finnhub.NewClient(
    finnhub.CreateConfiguration(
        finnhub.WithHttpConfiguration(
            finnhub.CreateHttpConfiguration(
                finnhub.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
