...
> setting, so **confirm it** from the class names under `shelldigitalpayments/apis/`.
...
| Install | `pip install "git+https://github.com/context-plugins/shell-digitalpayments-python-sdk@main"` |
| Client class | `ShellsmartpayapiClient` in `shelldigitalpayments/shelldigitalpayments_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `shelldigitalpayments/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `ShellsmartpayapiClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `shelldigitalpayments/exceptions/api_exception.py` — raised on non-2xx |
...
- `shelldigitalpayments/shelldigitalpayments_client.py` — the `ShellsmartpayapiClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `shelldigitalpayments/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `shelldigitalpayments/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `shelldigitalpayments/controllers/` — one `{Resource}Api` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `shelldigitalpayments/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `shelldigitalpayments/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `shelldigitalpayments/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `shelldigitalpayments/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `shelldigitalpayments/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/shell-digitalpayments-python-sdk@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/shell-digitalpayments-python-sdk /tmp/shell-digitalpayments-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/shell-digitalpayments-python-sdk "$env:TEMP\shell-digitalpayments-python-src"
```
...
- `shelldigitalpayments/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `shelldigitalpayments/shelldigitalpayments_client.py` — `ShellsmartpayapiClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `shelldigitalpayments/controllers/*.py` — operation method signatures, parameter names, and return types.
- `shelldigitalpayments/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `shelldigitalpayments/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/shell-digitalpayments-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\shell-digitalpayments-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `ShellsmartpayapiClient(...)`.
