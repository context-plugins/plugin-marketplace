...
Open `lib/splitit_web_api_v3/client.rb` for the accessor names, then the relevant
`lib/splitit_web_api_v3/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = SplititWebApiV3::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: SplititWebApiV3::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/splitit_web_api_v3/models/`. Field names are defined as
...
Read the enum module in `lib/splitit_web_api_v3/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/splitit_web_api_v3/client.rb`.
- Operation methods are in `lib/splitit_web_api_v3/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/splitit_web_api_v3/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/splitit_web_api_v3/exceptions/`.
