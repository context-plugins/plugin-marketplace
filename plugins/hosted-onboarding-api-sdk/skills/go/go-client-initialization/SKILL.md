...
- `hostedonboardingapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/hosted-onboarding-api-go` — the Go import path for the root package; `github.com/context-plugins/hosted-onboarding-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/hosted-onboarding-api-go"

client := hostedonboardingapi.NewClient(
    hostedonboardingapi.CreateConfiguration(
        hostedonboardingapi.WithEnvironment(hostedonboardingapi.PRODUCTION),
...
from environment variables named `HOSTEDONBOARDINGAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := hostedonboardingapi.NewClient(hostedonboardingapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `hostedonboardingapi.PRODUCTION`,
`hostedonboardingapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := hostedonboardingapi.NewClient(
    hostedonboardingapi.CreateConfiguration(hostedonboardingapi.WithEnvironment(hostedonboardingapi.PRODUCTION)),
)
```
...
client := hostedonboardingapi.NewClient(
    hostedonboardingapi.CreateConfiguration(
        hostedonboardingapi.WithHttpConfiguration(
            hostedonboardingapi.CreateHttpConfiguration(
                hostedonboardingapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                hostedonboardingapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = hostedonboardingapi.NewClient(hostedonboardingapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api hostedonboardingapi.ClientInterface
}

func NewService(api hostedonboardingapi.ClientInterface) *Service {
