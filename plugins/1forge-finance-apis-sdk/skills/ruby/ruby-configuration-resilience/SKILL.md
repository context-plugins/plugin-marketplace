...
```ruby
client = 1ForgeFinanceAPIs::Client.new(
  max_retries:    3,
...
```ruby
client = 1ForgeFinanceAPIs::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = 1ForgeFinanceAPIs::Client.new(adapter: :net_http)
```
...
client = 1ForgeFinanceAPIs::Client.new(connection: conn)
```
...
client = 1ForgeFinanceAPIs::Client.new(environment: Environment::PRODUCTION)
client = 1ForgeFinanceAPIs::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = 1ForgeFinanceAPIs::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = 1ForgeFinanceAPIs::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = 1ForgeFinanceAPIs::ProxySettings.from_env
client = 1ForgeFinanceAPIs::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < 1ForgeFinanceAPIs::HttpCallBack
...
client = 1ForgeFinanceAPIs::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = 1ForgeFinanceAPIs::Client.from_env
# Overrides take precedence over env vars:
client = 1ForgeFinanceAPIs::Client.from_env(timeout: 30, max_retries: 3)
```
