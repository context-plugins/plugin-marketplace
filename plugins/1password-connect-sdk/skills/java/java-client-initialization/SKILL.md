...
description: Construct and configure an APIMatic-generated Java SDK client — build via the inner 1PasswordConnectClient.Builder() class (never a public constructor), set httpClientConfig via a lambda (not a config object), choose an Environment constant, access controllers via client.get{Resource}Controller(), call client.shutdown() at process exit, clone a live client with client.newBuilder(), and wire the client into Spring or plain-constructor DI. Use the moment you call new 1PasswordConnectClient.Builder(), pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the arguments but not the builder pattern, the httpClientConfig lambda, or the lifetime/reuse rules.
---
...
- `1PasswordConnectClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `local.m1password` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`1PasswordConnectClient.Builder` class. A minimal initialization:

```java
import local.m1password.1PasswordConnectClient;
import local.m1password.Environment;

1PasswordConnectClient client = new 1PasswordConnectClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `1PasswordConnectClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `1PasswordConnectClient.java` |

Call `.build()` to produce an immutable `1PasswordConnectClient` instance.
...
public class ApiConfig {
    @Bean
    public 1PasswordConnectClient apiClient() {
        return new 1PasswordConnectClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired 1PasswordConnectClient client) {
        client.shutdown();
    }
}
```

Inject `1PasswordConnectClient` into your services as a normal Spring dependency. Build only one bean.
