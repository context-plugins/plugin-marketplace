...
> `{ApiGroup}`, `ToursAndActivities`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `ToursAndActivities.Core.Exceptions`
- `ApiError` **and** `RawError` → `ToursAndActivities.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `ToursAndActivities.Errors`
...
```csharp
using ToursAndActivities.Core.Exceptions;     // SdkException<TError>
using ToursAndActivities.Core.ErrorResponse;  // ApiError, RawError
using ToursAndActivities.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`ToursAndActivities.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using ToursAndActivities.Core.Exceptions;     // SdkException<TError>
using ToursAndActivities.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `ToursAndActivities.Core.ErrorResponse`. Its public
...
`readonly struct` under `ToursAndActivities.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using ToursAndActivities.Core.Models;        // ApiResult<TResponse, TError>
using ToursAndActivities.Core.ErrorResponse; // RawError
using ToursAndActivities.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `ToursAndActivities.Core.Exceptions`) — an auth *application* failure, not an API error — when the
