...
> Throughout, `restapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := restapi.NewClient(
    restapi.CreateConfiguration(
        restapi.WithBasicAuthCredentials(
            restapi.NewBasicAuthCredentials(
...
restapi.WithApiKeyCredentials(restapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
restapi.WithApiHeaderCredentials(restapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
restapi.WithOAuthBearerTokenCredentials(
    restapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := restapi.NewClient(
    restapi.CreateConfiguration(
        restapi.WithOAuthCCGCredentials(
            restapi.NewOAuthCCGCredentials(
