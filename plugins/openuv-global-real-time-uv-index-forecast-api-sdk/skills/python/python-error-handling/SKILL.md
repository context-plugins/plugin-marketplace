...
`openuvglobalrealtimeuvindexforecastapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `openuvglobalrealtimeuvindexforecastapi/exceptions/` and operation docstrings.
...
```python
from openuvglobalrealtimeuvindexforecastapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `openuvglobalrealtimeuvindexforecastapi/exceptions/`.
...
```python
from openuvglobalrealtimeuvindexforecastapi.exceptions.api_exception import APIException
from openuvglobalrealtimeuvindexforecastapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`openuvglobalrealtimeuvindexforecastapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`openuvglobalrealtimeuvindexforecastapi/exceptions/` for the class.
...
`openuvglobalrealtimeuvindexforecastapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `openuvglobalrealtimeuvindexforecastapi/exceptions/`
  or a `ValueError` — check the auth handler in `openuvglobalrealtimeuvindexforecastapi/http/auth/` for the exact type.
