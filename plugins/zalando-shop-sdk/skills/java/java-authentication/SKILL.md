...
To see which schemes a specific SDK accepts, read the credential setter methods on `ZalandoShopClient.Builder`
...
- Set credentials **on the `ZalandoShopClient.Builder`** before calling `.build()` — the client is immutable
