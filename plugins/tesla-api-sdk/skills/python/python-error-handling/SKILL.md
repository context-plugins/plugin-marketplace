...
`teslaapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `teslaapi/exceptions/` and operation docstrings.
...
```python
from teslaapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `teslaapi/exceptions/`.
...
```python
from teslaapi.exceptions.api_exception import APIException
from teslaapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`teslaapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`teslaapi/exceptions/` for the class.
...
`teslaapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `teslaapi/exceptions/`
  or a `ValueError` — check the auth handler in `teslaapi/http/auth/` for the exact type.
