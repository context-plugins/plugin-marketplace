...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new ZalandoShopClient({
...
```typescript
import { {Operation}Error } from 'zalando-shoplib/errors';
...
```typescript
import { ApiError } from 'zalando-shoplib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(ZalandoShopClient)
    .useValue(new ZalandoShopClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
