...
your SDK source (`ebaysell/models/`).
...
```python
from ebaysell.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `ebaysell/utilities/union_type_lookup.py`) and each candidate model
...
```python
from ebaysell.models.animal import Cat, Dog
...
```python
from ebaysell.models.animal import Animal, Cat, Dog
