...
All types below live under `FlightCreateOrders.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FlightCreateOrders.Core.Configuration;
...
```csharp
using FlightCreateOrders.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FlightCreateOrdersClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FlightCreateOrders.Core.Exceptions`):
...
With DI, the SDK's `AddFlightCreateOrdersClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFlightCreateOrdersClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
