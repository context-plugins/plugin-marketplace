...
> Throughout, `teslaapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := teslaapi.NewClient(
    teslaapi.CreateConfiguration(
        teslaapi.WithBasicAuthCredentials(
            teslaapi.NewBasicAuthCredentials(
...
teslaapi.WithApiKeyCredentials(teslaapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
teslaapi.WithApiHeaderCredentials(teslaapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
teslaapi.WithOAuthBearerTokenCredentials(
    teslaapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := teslaapi.NewClient(
    teslaapi.CreateConfiguration(
        teslaapi.WithOAuthCCGCredentials(
            teslaapi.NewOAuthCCGCredentials(
