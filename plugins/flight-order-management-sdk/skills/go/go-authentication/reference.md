...
```go
flightordermanagement.WithOAuthCCGCredentials(
    flightordermanagement.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr flightordermanagement.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := flightordermanagement.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr flightordermanagement.OAuthCCGManager) models.OAuthToken {
...
```go
creds := flightordermanagement.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := flightordermanagement.NewClient(flightordermanagement.CreateConfiguration(flightordermanagement.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    flightordermanagement.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := flightordermanagement.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := flightordermanagement.NewClient(flightordermanagement.CreateConfiguration(flightordermanagement.WithOAuthROPCGCredentials(creds)))
...
```go
flightordermanagement.WithOAuthBearerTokenCredentials(flightordermanagement.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
flightordermanagement.WithBasicAuthCredentials(flightordermanagement.NewBasicAuthCredentials(username, password))
flightordermanagement.WithApiHeaderCredentials(flightordermanagement.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
flightordermanagement.WithApiKeyCredentials(flightordermanagement.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FLIGHTORDERMANAGEMENT_...` variables — e.g.
`FLIGHTORDERMANAGEMENT_USERNAME`, `FLIGHTORDERMANAGEMENT_PASSWORD`, `FLIGHTORDERMANAGEMENT_TOKEN`, `FLIGHTORDERMANAGEMENT_API_KEY`,
`FLIGHTORDERMANAGEMENT_O_AUTH_CLIENT_ID`, `FLIGHTORDERMANAGEMENT_O_AUTH_CLIENT_SECRET`, `FLIGHTORDERMANAGEMENT_ACCESS_TOKEN`. Grep the
