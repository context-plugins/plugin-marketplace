...
Open `lib/aws_migration_hub/client.rb` for the accessor names, then the relevant
`lib/aws_migration_hub/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = AWSMigrationHub::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: AWSMigrationHub::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/aws_migration_hub/models/`. Field names are defined as
...
Read the enum module in `lib/aws_migration_hub/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/aws_migration_hub/client.rb`.
- Operation methods are in `lib/aws_migration_hub/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/aws_migration_hub/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/aws_migration_hub/exceptions/`.
