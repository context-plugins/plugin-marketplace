...
your SDK source (`deploymentadminclient/models/`).
...
```python
from deploymentadminclient.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `deploymentadminclient/utilities/union_type_lookup.py`) and each candidate model
...
```python
from deploymentadminclient.models.animal import Cat, Dog
...
```python
from deploymentadminclient.models.animal import Animal, Cat, Dog
