...
against the `TesserAPIV1Client.Builder` setters, the `app.zuplo.tesserplatformv1pull51me98e48a7.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `TesserAPIV1Client.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
TesserAPIV1Client client = new TesserAPIV1Client.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeTesserAPIV1Enum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
