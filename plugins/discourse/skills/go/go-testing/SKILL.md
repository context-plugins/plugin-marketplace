...
`testing` + `testify/require` for reference; substitute the real `discourse` and names.
...
import (
    ...
    "github.com/context-plugins/discourse-go-sdk"   // root package discourse
)
...
func clientReturning(status int, body string) (discourse.ClientInterface, *stubTransport) {
    ...
    client := discourse.NewClient(
        discourse.CreateConfiguration(
            discourse.WithHttpConfiguration(
                discourse.CreateHttpConfiguration(discourse.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/discourse-go-sdk/errors"
...
client := discourse.NewClient(
    discourse.CreateConfiguration(
        discourse.WithHttpConfiguration(
            discourse.CreateHttpConfiguration(discourse.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
