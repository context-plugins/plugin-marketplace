...
client = stub_client(stubs)
assert_raises Fdx::APIException do
...
BASE = Fdx::Configuration.new(environment: Fdx::Environment::PRODUCTION).get_base_uri

def build_test_client
  Fdx::Client.new(
    environment: Fdx::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Fdx::APIException
```
...
```ruby
expect { call }.to raise_error(Fdx::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Fdx::HttpCallBack
...
client  = Fdx::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
