...
`testing` + `testify/require` for reference; substitute the real `finnhubapi` and names.
...
import (
    ...
    "github.com/context-plugins/finnhub-api-go-sdk"   // root package finnhubapi
)
...
func clientReturning(status int, body string) (finnhubapi.ClientInterface, *stubTransport) {
    ...
    client := finnhubapi.NewClient(
        finnhubapi.CreateConfiguration(
            finnhubapi.WithHttpConfiguration(
                finnhubapi.CreateHttpConfiguration(finnhubapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/finnhub-api-go-sdk/errors"
...
client := finnhubapi.NewClient(
    finnhubapi.CreateConfiguration(
        finnhubapi.WithHttpConfiguration(
            finnhubapi.CreateHttpConfiguration(finnhubapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
