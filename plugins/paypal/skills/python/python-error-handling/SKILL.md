...
`paypal/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `paypal/exceptions/` and operation docstrings.
...
```python
from paypal.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `paypal/exceptions/`.
...
```python
from paypal.exceptions.api_exception import APIException
from paypal.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`paypal/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`paypal/exceptions/` for the class.
...
`paypal/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `paypal/exceptions/`
  or a `ValueError` — check the auth handler in `paypal/http/auth/` for the exact type.
