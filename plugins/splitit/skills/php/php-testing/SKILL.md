...
> `SplititClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SplititClient
{
...
```php
use SplititLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SplititLib\Exceptions\{OperationException};
use SplititLib\Exceptions\ApiException;
...
app()->instance(SplititClient::class, clientWithResponses([
