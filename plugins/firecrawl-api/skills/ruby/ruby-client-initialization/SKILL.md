...
- `FirecrawlAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `firecrawl_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'firecrawl_api'
include FirecrawlAPI
...
The authoritative list is the `initialize` signature in `lib/firecrawl_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/firecrawl_api/configuration.rb`
(`build_default_config_from_env`) and `lib/firecrawl_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = FirecrawlAPI::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/firecrawl_api.rb`) and store it in a
