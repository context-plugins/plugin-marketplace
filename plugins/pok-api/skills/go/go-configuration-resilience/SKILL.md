    ...
    pokapi.CreateConfiguration(
        pokapi.WithHttpConfiguration(
            pokapi.CreateHttpConfiguration(
                pokapi.WithTimeout(30),
                pokapi.WithRetryConfiguration(
                    pokapi.CreateRetryConfiguration(
                        pokapi.WithMaxRetryAttempts(3),
                        pokapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := pokapi.CreateRetryConfiguration(
    pokapi.WithMaxRetryAttempts(5),
    pokapi.WithRetryInterval(1),
    pokapi.WithBackoffFactor(2),
    pokapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    pokapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := pokapi.NewClient(
    pokapi.CreateConfiguration(
        pokapi.WithHttpConfiguration(
            pokapi.CreateHttpConfiguration(
                pokapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
