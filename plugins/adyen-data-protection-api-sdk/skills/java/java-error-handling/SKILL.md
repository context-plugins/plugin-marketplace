...
│   └── com.adyen.catest.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.adyen.catest.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.adyen.catest.http.client.HttpContext`, which carries:
