...
> `{ApiGroup}`, `ArticleSearchAPI`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `ArticleSearchAPI.Core.Exceptions`
- `ApiError` **and** `RawError` → `ArticleSearchAPI.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `ArticleSearchAPI.Errors`
...
```csharp
using ArticleSearchAPI.Core.Exceptions;     // SdkException<TError>
using ArticleSearchAPI.Core.ErrorResponse;  // ApiError, RawError
using ArticleSearchAPI.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`ArticleSearchAPI.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using ArticleSearchAPI.Core.Exceptions;     // SdkException<TError>
using ArticleSearchAPI.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `ArticleSearchAPI.Core.ErrorResponse`. Its public
...
`readonly struct` under `ArticleSearchAPI.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using ArticleSearchAPI.Core.Models;        // ApiResult<TResponse, TError>
using ArticleSearchAPI.Core.ErrorResponse; // RawError
using ArticleSearchAPI.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `ArticleSearchAPI.Core.Exceptions`) — an auth *application* failure, not an API error — when the
