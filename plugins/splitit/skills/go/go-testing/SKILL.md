...
`testing` + `testify/require` for reference; substitute the real `splitit` and names.
...
import (
    ...
    "github.com/context-plugins/splitit-go-sdk"   // root package splitit
)
...
func clientReturning(status int, body string) (splitit.ClientInterface, *stubTransport) {
    ...
    client := splitit.NewClient(
        splitit.CreateConfiguration(
            splitit.WithHttpConfiguration(
                splitit.CreateHttpConfiguration(splitit.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/splitit-go-sdk/errors"
...
client := splitit.NewClient(
    splitit.CreateConfiguration(
        splitit.WithHttpConfiguration(
            splitit.CreateHttpConfiguration(splitit.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
