...
- `ShellEv` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `shell_ev` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/shell_ev/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/shell_ev/configuration.rb`
(`build_default_config_from_env`) and `lib/shell_ev/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = ShellEv::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/shell_ev.rb`) and store it in a
