...
> `GreenbyteApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): GreenbyteApiClient
{
...
```php
use GreenbyteApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use GreenbyteApiLib\Exceptions\{OperationException};
use GreenbyteApiLib\Exceptions\ApiException;
...
app()->instance(GreenbyteApiClient::class, clientWithResponses([
