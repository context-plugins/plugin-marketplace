...
`univa/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `univa/exceptions/` and operation docstrings.
...
```python
from univa.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `univa/exceptions/`.
...
```python
from univa.exceptions.api_exception import APIException
from univa.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`univa/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`univa/exceptions/` for the class.
...
`univa/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `univa/exceptions/`
  or a `ValueError` — check the auth handler in `univa/http/auth/` for the exact type.
