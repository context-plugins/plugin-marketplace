...
- `ny_times` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/ny_times/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/ny_times/configuration.rb`
(`build_default_config_from_env`) and `lib/ny_times/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/ny_times.rb`) and store it in a
