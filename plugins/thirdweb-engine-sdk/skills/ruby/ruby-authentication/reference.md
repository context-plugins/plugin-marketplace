...
Confirm every name against `lib/thirdweb_engine/client.rb`, `lib/thirdweb_engine/configuration.rb`, the
`lib/thirdweb_engine/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
