...
Open `docker/docker_client.py` for the controller property names, then the relevant
`docker/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from docker.models.{request_model} import {RequestModel}
...
Open `docker/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from docker.models.suite_code_enum import SuiteCodeEnum
...
```python
from docker.http.http_call_back import HttpCallBack
...
client = DockerClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `docker/pagination/` in the source.

## Finding the right method

- Controller property names are on `DockerClient` in `docker/docker_client.py`.
- Operation method signatures are in `docker/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `docker/models/`; enum types have `from_value()`.
