...
To see which schemes a specific SDK accepts, read the credential setter methods on `SeatmapDisplayClient.Builder`
...
- Set credentials **on the `SeatmapDisplayClient.Builder`** before calling `.build()` — the client is immutable
