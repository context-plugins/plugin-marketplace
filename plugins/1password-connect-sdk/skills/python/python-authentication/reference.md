...
kwargs to the `1PasswordConnectClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `1passwordconnect/1passwordconnect_client.py` and `1passwordconnect/http/auth/` in
the cloned source.

## Basic auth

```python
from 1passwordconnect.http.auth.basic_auth import BasicAuthCredentials
...
```python
from 1passwordconnect.http.auth.api_key import ApiKeyCredentials
...
```python
from 1passwordconnect.http.auth.api_header import ApiHeaderCredentials
...
```python
from 1passwordconnect.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from 1passwordconnect.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from 1passwordconnect.http.auth.o_auth_acg import OAuthACGCredentials
from 1passwordconnect.models.o_auth_scope_1passwordconnect_enum import OAuthScope{Pkg}Enum
...
client = 1PasswordConnectClient(o_auth_acg_credentials=creds)
...
```python
from 1passwordconnect.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = 1PasswordConnectClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `1PasswordConnectClient.from_environment()`) read each credential
...
Grep `from_environment` in `1passwordconnect/configuration.py` and each `1passwordconnect/http/auth/*.py` for the exact
