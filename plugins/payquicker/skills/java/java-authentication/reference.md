...
against the `PqApiV2Client.Builder` setters, the `io.payquicker.sandbox.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `PqApiV2Client.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
PqApiV2Client client = new PqApiV2Client.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopePayquickerEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
