...
- `DiscourseApiDocumentationClient` — the single public client class (the generator reads this prefix from the SDK's
  `.csproj` name, so it is already the real one).
- `DiscourseApiDocumentationClientOptions` — its options class.
- `DiscourseApiDocumentation` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddDiscourseApiDocumentationClient`, which registers the
...
builder.Services.AddDiscourseApiDocumentationClient(options =>
{
...
```csharp
public sealed class MyService(DiscourseApiDocumentationClient client)
{
