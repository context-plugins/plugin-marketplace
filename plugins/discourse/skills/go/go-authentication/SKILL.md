...
> Throughout, `discourse` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := discourse.NewClient(
    discourse.CreateConfiguration(
        discourse.WithBasicAuthCredentials(
            discourse.NewBasicAuthCredentials(
...
discourse.WithApiKeyCredentials(discourse.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
discourse.WithApiHeaderCredentials(discourse.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
discourse.WithOAuthBearerTokenCredentials(
    discourse.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := discourse.NewClient(
    discourse.CreateConfiguration(
        discourse.WithOAuthCCGCredentials(
            discourse.NewOAuthCCGCredentials(
