...
To see which schemes a specific SDK accepts, read the credential setter methods on `NewStoreAPIClient.Builder`
...
- Set credentials **on the `NewStoreAPIClient.Builder`** before calling `.build()` — the client is immutable
