...
> `{ApiGroup}`, `AdyenApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `AdyenApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `AdyenApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `AdyenApi.Errors`
...
using AdyenApi.Core.ErrorResponse;  // ApiError, RawError
using AdyenApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`AdyenApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using AdyenApi.Core.Exceptions;     // SdkException<TError>
using AdyenApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `AdyenApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `AdyenApi.Core.Models`. If the controller has no `{Operation}Result`
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `AdyenApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
