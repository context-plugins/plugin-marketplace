...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `shell-evlib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'shell-evlib';
import { ApiError } from 'shell-evlib';
import { {Operation}Error } from 'shell-evlib/errors';
