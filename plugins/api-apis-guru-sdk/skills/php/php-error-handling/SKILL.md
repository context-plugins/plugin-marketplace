...
> `{operation}`, `{apiGroup}`, `OnDemandFlightStatusLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `OnDemandFlightStatusLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use OnDemandFlightStatusLib\Exceptions\ApiException;
...
```php
use OnDemandFlightStatusLib\Exceptions\ApiException;
use OnDemandFlightStatusLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `OnDemandFlightStatusLib\Exceptions\` — read the source to
