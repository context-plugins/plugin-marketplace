...
Open `slackwebapi/slackwebapi_client.py` for the controller property names, then the relevant
`slackwebapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from slackwebapi.models.{request_model} import {RequestModel}
...
Open `slackwebapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from slackwebapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from slackwebapi.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `slackwebapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `SlackWebAPIClient` in `slackwebapi/slackwebapi_client.py`.
- Operation method signatures are in `slackwebapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `slackwebapi/models/`; enum types have `from_value()`.
