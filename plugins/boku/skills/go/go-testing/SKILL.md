...
`testing` + `testify/require` for reference; substitute the real `boku` and names.
...
import (
    ...
    "github.com/context-plugins/boku-go-sdk"   // root package boku
)
...
func clientReturning(status int, body string) (boku.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := boku.NewClient(
        boku.CreateConfiguration(
            boku.WithHttpConfiguration(
                boku.CreateHttpConfiguration(boku.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/boku-go-sdk/errors"
...
client := boku.NewClient(
    boku.CreateConfiguration(
        boku.WithHttpConfiguration(
            boku.CreateHttpConfiguration(boku.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
