...
`theplaidapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `theplaidapi/exceptions/` and operation docstrings.
...
```python
from theplaidapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `theplaidapi/exceptions/`.
...
```python
from theplaidapi.exceptions.api_exception import APIException
from theplaidapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`theplaidapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`theplaidapi/exceptions/` for the class.
...
`theplaidapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `theplaidapi/exceptions/`
  or a `ValueError` — check the auth handler in `theplaidapi/http/auth/` for the exact type.
