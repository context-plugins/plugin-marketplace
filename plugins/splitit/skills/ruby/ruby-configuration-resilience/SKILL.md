...
```ruby
client = Splitit::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Splitit::Client.new(adapter: :net_http)
```
...
client = Splitit::Client.new(connection: conn)
```
...
client = Splitit::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Splitit::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Splitit::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Splitit::ProxySettings.from_env
client = Splitit::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Splitit::HttpCallBack
...
client = Splitit::Client.new(http_callback: LogCallback.new)
```
...
client = Splitit::Client.from_env(timeout: 30, max_retries: 3)
```
