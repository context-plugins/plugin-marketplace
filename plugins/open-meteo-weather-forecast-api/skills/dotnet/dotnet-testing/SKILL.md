...
`OpenMeteoWeatherForecastApiClient`/`OpenMeteoWeatherForecastApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `OpenMeteoWeatherForecastApiClient`,
...
static OpenMeteoWeatherForecastApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new OpenMeteoWeatherForecastApiClient(new HttpClient(handler), new OpenMeteoWeatherForecastApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using OpenMeteoWeatherForecastApi.Errors;              // {Operation}Error types
...
```csharp
using OpenMeteoWeatherForecastApi.Core.Exceptions;
using OpenMeteoWeatherForecastApi.Core.ErrorResponse;
...
using OpenMeteoWeatherForecastApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new OpenMeteoWeatherForecastApiClient(new HttpClient(handler), new OpenMeteoWeatherForecastApiClientOptions());
...
- For DI-based code, the SDK's `AddOpenMeteoWeatherForecastApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `OpenMeteoWeatherForecastApiClient` from the provider:
  ```csharp
  services.AddOpenMeteoWeatherForecastApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<OpenMeteoWeatherForecastApiClient>();
  ```
