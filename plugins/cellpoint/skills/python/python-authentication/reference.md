...
kwargs to the `CellPointAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `cellpointapi/cellpointapi_client.py` and `cellpointapi/http/auth/` in
the cloned source.

## Basic auth

```python
from cellpointapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from cellpointapi.http.auth.api_key import ApiKeyCredentials
...
```python
from cellpointapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from cellpointapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from cellpointapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from cellpointapi.http.auth.o_auth_acg import OAuthACGCredentials
from cellpointapi.models.o_auth_scope_cellpointapi_enum import OAuthScope{Pkg}Enum
...
client = CellPointAPIClient(o_auth_acg_credentials=creds)
...
```python
from cellpointapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = CellPointAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `CellPointAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `cellpointapi/configuration.py` and each `cellpointapi/http/auth/*.py` for the exact
