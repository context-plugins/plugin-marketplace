...
> `AdafruitIORESTAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AdafruitIORESTAPIClient
{
...
```php
use AdafruitIORESTAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AdafruitIORESTAPILib\Exceptions\{OperationException};
use AdafruitIORESTAPILib\Exceptions\ApiException;
...
app()->instance(AdafruitIORESTAPIClient::class, clientWithResponses([
