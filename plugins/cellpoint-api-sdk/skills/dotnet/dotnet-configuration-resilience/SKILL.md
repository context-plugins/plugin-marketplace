...
All types below live under `CellPointApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using CellPointApi.Core.Configuration;
...
```csharp
using CellPointApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new CellPointApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `CellPointApi.Core.Exceptions`):
...
With DI, the SDK's `AddCellPointApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddCellPointApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
