...
To see which schemes a specific SDK accepts, read the credential setter methods on `TeslaClient.Builder`
...
- Set credentials **on the `TeslaClient.Builder`** before calling `.build()` — the client is immutable
