...
`testing` + `testify/require` for reference; substitute the real `hostedonboardingapi` and names.
...
import (
    ...
    "github.com/context-plugins/hosted-onboarding-api-go"   // root package hostedonboardingapi
)
...
func clientReturning(status int, body string) (hostedonboardingapi.ClientInterface, *stubTransport) {
    ...
    client := hostedonboardingapi.NewClient(
        hostedonboardingapi.CreateConfiguration(
            hostedonboardingapi.WithHttpConfiguration(
                hostedonboardingapi.CreateHttpConfiguration(hostedonboardingapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/hosted-onboarding-api-go/errors"
...
client := hostedonboardingapi.NewClient(
    hostedonboardingapi.CreateConfiguration(
        hostedonboardingapi.WithHttpConfiguration(
            hostedonboardingapi.CreateHttpConfiguration(hostedonboardingapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
