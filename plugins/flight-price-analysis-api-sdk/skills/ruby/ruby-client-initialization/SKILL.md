...
- `FlightPriceAnalysisAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `flight_price_analysis_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'flight_price_analysis_api'
include FlightPriceAnalysisAPI
...
The authoritative list is the `initialize` signature in `lib/flight_price_analysis_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/flight_price_analysis_api/configuration.rb`
(`build_default_config_from_env`) and `lib/flight_price_analysis_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/flight_price_analysis_api.rb`) and store it in a
