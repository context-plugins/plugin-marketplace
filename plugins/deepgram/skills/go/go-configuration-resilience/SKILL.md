    ...
    restapi.CreateConfiguration(
        restapi.WithHttpConfiguration(
            restapi.CreateHttpConfiguration(
                restapi.WithTimeout(30),
                restapi.WithRetryConfiguration(
                    restapi.CreateRetryConfiguration(
                        restapi.WithMaxRetryAttempts(3),
                        restapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := restapi.CreateRetryConfiguration(
    restapi.WithMaxRetryAttempts(5),
    restapi.WithRetryInterval(1),
    restapi.WithBackoffFactor(2),
    restapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    restapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := restapi.NewClient(
    restapi.CreateConfiguration(
        restapi.WithHttpConfiguration(
            restapi.CreateHttpConfiguration(
                restapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
