...
kwargs to the `DeploymentAdminClientClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `deploymentadminclient/deploymentadminclient_client.py` and `deploymentadminclient/http/auth/` in
the cloned source.

## Basic auth

```python
from deploymentadminclient.http.auth.basic_auth import BasicAuthCredentials
...
```python
from deploymentadminclient.http.auth.api_key import ApiKeyCredentials
...
```python
from deploymentadminclient.http.auth.api_header import ApiHeaderCredentials
...
```python
from deploymentadminclient.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from deploymentadminclient.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from deploymentadminclient.http.auth.o_auth_acg import OAuthACGCredentials
from deploymentadminclient.models.o_auth_scope_deploymentadminclient_enum import OAuthScope{Pkg}Enum
...
client = DeploymentAdminClientClient(o_auth_acg_credentials=creds)
...
```python
from deploymentadminclient.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = DeploymentAdminClientClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `DeploymentAdminClientClient.from_environment()`) read each credential
...
Grep `from_environment` in `deploymentadminclient/configuration.py` and each `deploymentadminclient/http/auth/*.py` for the exact
