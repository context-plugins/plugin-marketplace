...
Open `lib/shell_digitalpayments/client.rb` for the accessor names, then the relevant
`lib/shell_digitalpayments/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = ShellDigitalpayments::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: ShellDigitalpayments::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/shell_digitalpayments/models/`. Field names are defined as
...
Read the enum module in `lib/shell_digitalpayments/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/shell_digitalpayments/client.rb`.
- Operation methods are in `lib/shell_digitalpayments/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/shell_digitalpayments/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/shell_digitalpayments/exceptions/`.
