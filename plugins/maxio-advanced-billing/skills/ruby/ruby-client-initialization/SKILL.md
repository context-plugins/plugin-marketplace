...
- `MaxioAdvancedBilling` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `maxio_advanced_billing` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'maxio_advanced_billing'
include MaxioAdvancedBilling
...
The authoritative list is the `initialize` signature in `lib/maxio_advanced_billing/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/maxio_advanced_billing/configuration.rb`
(`build_default_config_from_env`) and `lib/maxio_advanced_billing/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = MaxioAdvancedBilling::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/maxio_advanced_billing.rb`) and store it in a
