...
reference; substitute your `tesserapiv1` and `TesserAPIV1Client` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `tesserapiv1/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
