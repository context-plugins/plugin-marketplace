...
`testing` + `testify/require` for reference; substitute the real `coingeckodemoapi` and names.
...
import (
    ...
    "github.com/context-plugins/coingecko-demo-api-go-sdk"   // root package coingeckodemoapi
)
...
func clientReturning(status int, body string) (coingeckodemoapi.ClientInterface, *stubTransport) {
    ...
    client := coingeckodemoapi.NewClient(
        coingeckodemoapi.CreateConfiguration(
            coingeckodemoapi.WithHttpConfiguration(
                coingeckodemoapi.CreateHttpConfiguration(coingeckodemoapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/coingecko-demo-api-go-sdk/errors"
...
client := coingeckodemoapi.NewClient(
    coingeckodemoapi.CreateConfiguration(
        coingeckodemoapi.WithHttpConfiguration(
            coingeckodemoapi.CreateHttpConfiguration(coingeckodemoapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
