...
- `{package-id}` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from '{package-id}';
...
```ts
import { {Resource}Controller } from '{package-id}';
