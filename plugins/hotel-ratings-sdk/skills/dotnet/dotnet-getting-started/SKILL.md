...
| --- | --- |
| API | `Hotel Ratings` |
| Project | `HotelRatings.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `HotelRatings` (the `using` namespace) |
| Client class | `HotelRatingsClient` |
| Options class | `HotelRatingsClientOptions` |
...
namespaces transitively, so `using HotelRatings.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/hotel-ratings-csharp
dotnet add reference hotel-ratings-csharp/HotelRatings.csproj
```

```csharp
using HotelRatings;          // client + options + ServerEnvironment
using HotelRatings.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/hotel-ratings-csharp /tmp/hotel-ratings-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/hotel-ratings-csharp "$env:TEMP\hotel-ratings-dotnet-src"
```
...
- `HotelRatingsClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/hotel-ratings-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\hotel-ratings-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `HotelRatingsClient`, build its
   `HotelRatingsClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
