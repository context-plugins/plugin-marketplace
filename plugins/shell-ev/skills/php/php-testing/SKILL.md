...
> `ShellEVClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ShellEVClient
{
...
app()->instance(ShellEVClient::class, clientWithResponses([
