...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new HotelNameAutocompleteClient({
...
```typescript
import { {Operation}Error } from 'hotel-name-autocompletelib/errors';
...
```typescript
import { ApiError } from 'hotel-name-autocompletelib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(HotelNameAutocompleteClient)
    .useValue(new HotelNameAutocompleteClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
