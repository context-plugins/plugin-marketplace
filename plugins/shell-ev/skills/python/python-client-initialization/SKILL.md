...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the ShellevClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write ShellevClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `ShellevClient` is the SDK's client class — **read the real name** from `shellev/shellev_client.py`
...
- `shellev` — the root package name (e.g. `multiauthsample`, `batester`).
- `ShellevClient` — the generated client class (read it from the `class …Client` declaration in `shellev/shellev_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `ShellevClient` constructor accepts all configuration as **keyword arguments** — there is no
...
from shellev.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
client = ShellevClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `shellev/shellev_client.py` in the cloned source.
...
`from_environment()` is a class method on both `ShellevClient` and `Configuration` that reads
...
`from_environment` in `shellev/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `ShellevClient`. Access them as attributes —
...
Open `shellev/shellev_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `shellev/configuration.py`. **Read the enum in `configuration.py`
...
client = ShellevClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`shellev/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = ShellevClient(config=new_config)
```
