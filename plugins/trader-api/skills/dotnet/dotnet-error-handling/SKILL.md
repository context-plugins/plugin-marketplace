...
> `{ApiGroup}`, `TraderApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `TraderApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `TraderApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `TraderApi.Errors`
...
```csharp
using TraderApi.Core.Exceptions;     // SdkException<TError>
using TraderApi.Core.ErrorResponse;  // ApiError, RawError
using TraderApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`TraderApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using TraderApi.Core.Exceptions;     // SdkException<TError>
using TraderApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `TraderApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `TraderApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using TraderApi.Core.Models;        // ApiResult<TResponse, TError>
using TraderApi.Core.ErrorResponse; // RawError
using TraderApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `TraderApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
