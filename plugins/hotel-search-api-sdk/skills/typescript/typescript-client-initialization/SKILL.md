...
- `hotel-search-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'hotel-search-apilib';
...
```ts
import { {Resource}Controller } from 'hotel-search-apilib';
