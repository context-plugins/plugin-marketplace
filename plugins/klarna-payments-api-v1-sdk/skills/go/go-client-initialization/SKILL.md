...
- `klarnapaymentsapiv1` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/klarna-payments-api-v1-go` — the Go import path for the root package; `github.com/context-plugins/klarna-payments-api-v1-go/models` for request/response types.
...
```go
import "github.com/context-plugins/klarna-payments-api-v1-go"

client := klarnapaymentsapiv1.NewClient(
    klarnapaymentsapiv1.CreateConfiguration(
        klarnapaymentsapiv1.WithEnvironment(klarnapaymentsapiv1.PRODUCTION),
...
from environment variables named `KLARNAPAYMENTSAPIV1_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := klarnapaymentsapiv1.NewClient(klarnapaymentsapiv1.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `klarnapaymentsapiv1.PRODUCTION`,
`klarnapaymentsapiv1.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := klarnapaymentsapiv1.NewClient(
    klarnapaymentsapiv1.CreateConfiguration(klarnapaymentsapiv1.WithEnvironment(klarnapaymentsapiv1.PRODUCTION)),
)
```
...
client := klarnapaymentsapiv1.NewClient(
    klarnapaymentsapiv1.CreateConfiguration(
        klarnapaymentsapiv1.WithHttpConfiguration(
            klarnapaymentsapiv1.CreateHttpConfiguration(
                klarnapaymentsapiv1.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                klarnapaymentsapiv1.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = klarnapaymentsapiv1.NewClient(klarnapaymentsapiv1.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api klarnapaymentsapiv1.ClientInterface
}

func NewService(api klarnapaymentsapiv1.ClientInterface) *Service {
