...
client = stub_client(stubs)
assert_raises TeslaAPI::APIException do
...
def build_test_client
  TeslaAPI::Client.new(
    environment: TeslaAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error TeslaAPI::APIException
```
...
```ruby
expect { call }.to raise_error(TeslaAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < TeslaAPI::HttpCallBack
...
client  = TeslaAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
