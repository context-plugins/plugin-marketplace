    ...
    coingecko.CreateConfiguration(
        coingecko.WithHttpConfiguration(
            coingecko.CreateHttpConfiguration(
                coingecko.WithTimeout(30),
                coingecko.WithRetryConfiguration(
                    coingecko.CreateRetryConfiguration(
                        coingecko.WithMaxRetryAttempts(3),
                        coingecko.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := coingecko.CreateRetryConfiguration(
    coingecko.WithMaxRetryAttempts(5),
    coingecko.WithRetryInterval(1),
    coingecko.WithBackoffFactor(2),
    coingecko.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    coingecko.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := coingecko.NewClient(
    coingecko.CreateConfiguration(
        coingecko.WithHttpConfiguration(
            coingecko.CreateHttpConfiguration(
                coingecko.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
