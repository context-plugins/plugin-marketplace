...
```ruby
client = AuthentiqAPI::Client.new(
  max_retries:    3,
...
```ruby
client = AuthentiqAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = AuthentiqAPI::Client.new(adapter: :net_http)
```
...
client = AuthentiqAPI::Client.new(connection: conn)
```
...
client = AuthentiqAPI::Client.new(environment: Environment::PRODUCTION)
client = AuthentiqAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = AuthentiqAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = AuthentiqAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = AuthentiqAPI::ProxySettings.from_env
client = AuthentiqAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < AuthentiqAPI::HttpCallBack
...
client = AuthentiqAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = AuthentiqAPI::Client.from_env
# Overrides take precedence over env vars:
client = AuthentiqAPI::Client.from_env(timeout: 30, max_retries: 3)
```
