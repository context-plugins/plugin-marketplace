...
kwargs to the `SportsdataClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `sportsdata/sportsdata_client.py` and `sportsdata/http/auth/` in
the cloned source.

## Basic auth

```python
from sportsdata.http.auth.basic_auth import BasicAuthCredentials
...
```python
from sportsdata.http.auth.api_key import ApiKeyCredentials
...
```python
from sportsdata.http.auth.api_header import ApiHeaderCredentials
...
```python
from sportsdata.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from sportsdata.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from sportsdata.http.auth.o_auth_acg import OAuthACGCredentials
from sportsdata.models.o_auth_scope_sportsdata_enum import OAuthScope{Pkg}Enum
...
client = SportsdataClient(o_auth_acg_credentials=creds)
...
```python
from sportsdata.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SportsdataClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SportsdataClient.from_environment()`) read each credential
...
Grep `from_environment` in `sportsdata/configuration.py` and each `sportsdata/http/auth/*.py` for the exact
