...
All types below live under `SquareConnectApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using SquareConnectApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new SquareConnectApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `SquareConnectApi.Core.Exceptions`):
...
With DI, the SDK's `AddSquareConnectApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddSquareConnectApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
