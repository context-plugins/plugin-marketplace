...
> `{ApiGroup}`, `EventsApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `EventsApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `EventsApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `EventsApi.Errors`
...
```csharp
using EventsApi.Core.Exceptions;     // SdkException<TError>
using EventsApi.Core.ErrorResponse;  // ApiError, RawError
using EventsApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`EventsApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using EventsApi.Core.Exceptions;     // SdkException<TError>
using EventsApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `EventsApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `EventsApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using EventsApi.Core.Models;        // ApiResult<TResponse, TError>
using EventsApi.Core.ErrorResponse; // RawError
using EventsApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `EventsApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
