...
- `splitit` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/splitit-go-sdk` — the Go import path for the root package; `github.com/context-plugins/splitit-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/splitit-go-sdk"

client := splitit.NewClient(
    splitit.CreateConfiguration(
        splitit.WithEnvironment(splitit.PRODUCTION),
...
from environment variables named `SPLITIT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := splitit.NewClient(splitit.CreateConfigurationFromEnvironment())
```
...
```go
client := splitit.NewClient(
    splitit.CreateConfiguration(splitit.WithEnvironment(splitit.PRODUCTION)),
)
```
...
client := splitit.NewClient(
    splitit.CreateConfiguration(
        splitit.WithHttpConfiguration(
            splitit.CreateHttpConfiguration(
                splitit.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                splitit.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = splitit.NewClient(splitit.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api splitit.ClientInterface
}

func NewService(api splitit.ClientInterface) *Service {
