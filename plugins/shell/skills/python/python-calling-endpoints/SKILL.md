...
Open `shellapi/shellapi_client.py` for the controller property names, then the relevant
`shellapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from shellapi.models.{request_model} import {RequestModel}
...
Open `shellapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from shellapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from shellapi.http.http_call_back import HttpCallBack
...
client = ShellAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `shellapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `ShellAPIClient` in `shellapi/shellapi_client.py`.
- Operation method signatures are in `shellapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `shellapi/models/`; enum types have `from_value()`.
