...
> `PqApiV2Client`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): PqApiV2Client
{
...
```php
use PqApiV2Lib\Exceptions\{OperationException};
use PqApiV2Lib\Exceptions\ApiException;
...
app()->instance(PqApiV2Client::class, clientWithResponses([
