    ...
    eventsapi.CreateConfiguration(
        eventsapi.WithHttpConfiguration(
            eventsapi.CreateHttpConfiguration(
                eventsapi.WithTimeout(30),
                eventsapi.WithRetryConfiguration(
                    eventsapi.CreateRetryConfiguration(
                        eventsapi.WithMaxRetryAttempts(3),
                        eventsapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := eventsapi.CreateRetryConfiguration(
    eventsapi.WithMaxRetryAttempts(5),
    eventsapi.WithRetryInterval(1),
    eventsapi.WithBackoffFactor(2),
    eventsapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    eventsapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := eventsapi.NewClient(
    eventsapi.CreateConfiguration(
        eventsapi.WithHttpConfiguration(
            eventsapi.CreateHttpConfiguration(
                eventsapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
