...
- `TravelRecommendationsAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `travel_recommendations_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'travel_recommendations_api'
include TravelRecommendationsAPI
...
The authoritative list is the `initialize` signature in `lib/travel_recommendations_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/travel_recommendations_api/configuration.rb`
(`build_default_config_from_env`) and `lib/travel_recommendations_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/travel_recommendations_api.rb`) and store it in a
