...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `slacklib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'slacklib';
import { ApiError } from 'slacklib';
import { {Operation}Error } from 'slacklib/errors';
