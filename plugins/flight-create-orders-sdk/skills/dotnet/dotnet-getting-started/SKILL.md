...
| Project | `FlightCreateOrders.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `FlightCreateOrders` (the `using` namespace) |
| Client class | `FlightCreateOrdersClient` |
| Options class | `FlightCreateOrdersClientOptions` |
...
namespaces transitively, so `using FlightCreateOrders.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/flight-create-orders-csharp
dotnet add reference flight-create-orders-csharp/FlightCreateOrders.csproj
```

```csharp
using FlightCreateOrders;          // client + options + ServerEnvironment
using FlightCreateOrders.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/flight-create-orders-csharp /tmp/flight-create-orders-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/flight-create-orders-csharp "$env:TEMP\flight-create-orders-dotnet-src"
```
...
- `FlightCreateOrdersClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/flight-create-orders-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\flight-create-orders-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `FlightCreateOrdersClient`, build its
   `FlightCreateOrdersClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
