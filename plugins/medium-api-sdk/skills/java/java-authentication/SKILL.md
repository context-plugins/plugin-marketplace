...
To see which schemes a specific SDK accepts, read the credential setter methods on `MediumAPIClient.Builder`
...
- Set credentials **on the `MediumAPIClient.Builder`** before calling `.build()` — the client is immutable
