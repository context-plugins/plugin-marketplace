...
| Root namespace | `Alpaca` (the `using` namespace) |
| Client class | `AlpacaClient` |
| Options class | `AlpacaClientOptions` |
| Auth | **API key** in the `APCA-API-KEY-ID` header — set `ApiKey`<br>**API key** in the `APCA-API-SECRET-KEY` header — set `ApiSecret` |
| Environments | `ServerEnvironment.Production` *(default)* → `https://paper-api.alpaca.markets`<br>`ServerEnvironment.Environment2` → `https://api.alpaca.markets` |
| Log environment variable | `ALPACACLIENT_LOG` |
...
The SDK splits its public types across **separate child namespaces**. C# does **not** import child namespaces transitively, so `using Alpaca.Models;` alone does **not** make enums, union types, or error types visible — you get `CS0103`/`CS0246` ("name/type does not exist") on build. Add a separate `using` for each kind of type you reference:

| Type | Namespace |
| --- | --- |
| `AlpacaClient` | `Alpaca` |
| `AlpacaClientOptions` | `Alpaca` |
| `ServerEnvironment` | `Alpaca.Servers` |
...
- Collect the contracts for **every** in-scope operation in **one** pass — signatures and error accessors from the map pages, then required fields with wire names and enum values from the declaring files those pages name — into the **contract sheet** in `alpaca-plan.md`, then implement from the sheet. Do not re-open a map page or source file per field, and never re-look-up a fact the sheet already carries.
...
```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\alpaca-sdk-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --filter=blob:none --branch main https://github.com/context-plugins/alpaca-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```
...
The clone lives in `<temp>/alpaca-sdk-src/`, **never** in the project repo, and its path never goes into `alpaca-plan.md` — the plan must stay portable, and the clone is a session-local reference, not part of the project.
...
- `Servers/`, `AlpacaClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
The generator injects an `Idempotency-Key: Guid.NewGuid()` header on **every non-GET operation** — fresh on every call, invisible to you, and **not** an idempotency key in any meaningful sense: a value that changes per call deduplicates nothing, and Alpaca may not document reading that header at all. `dotnet-configuration-resilience` explains why a visible header like this is worse than an absent one.
