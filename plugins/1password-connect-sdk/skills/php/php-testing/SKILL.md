...
> `1PasswordConnectClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): 1PasswordConnectClient
{
...
```php
use M1PasswordConnectLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use M1PasswordConnectLib\Exceptions\{OperationException};
use M1PasswordConnectLib\Exceptions\ApiException;
...
app()->instance(1PasswordConnectClient::class, clientWithResponses([
