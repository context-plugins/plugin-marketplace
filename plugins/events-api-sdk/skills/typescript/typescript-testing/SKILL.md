...
```typescript
import { {Operation}Error } from 'events-apilib/errors';
...
```typescript
import { ApiError } from 'events-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(EventsAPIClient)
    .useValue(new EventsAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
