...
All types below live under `EBayFinancesApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using EBayFinancesApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new EBayFinancesApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `EBayFinancesApi.Core.Exceptions`):
...
With DI, the SDK's `AddEBayFinancesApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddEBayFinancesApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
