    ...
    fundapi.CreateConfiguration(
        fundapi.WithHttpConfiguration(
            fundapi.CreateHttpConfiguration(
                fundapi.WithTimeout(30),
                fundapi.WithRetryConfiguration(
                    fundapi.CreateRetryConfiguration(
                        fundapi.WithMaxRetryAttempts(3),
                        fundapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := fundapi.CreateRetryConfiguration(
    fundapi.WithMaxRetryAttempts(5),
    fundapi.WithRetryInterval(1),
    fundapi.WithBackoffFactor(2),
    fundapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    fundapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := fundapi.NewClient(
    fundapi.CreateConfiguration(
        fundapi.WithHttpConfiguration(
            fundapi.CreateHttpConfiguration(
                fundapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
