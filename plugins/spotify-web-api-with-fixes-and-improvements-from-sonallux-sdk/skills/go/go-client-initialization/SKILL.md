...
- `spotifywebapiwithfixesandimprovementsfromsonallux` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/spotify-web-api-with-fixes-and-improvements-from-sonallux-go` — the Go import path for the root package; `github.com/context-plugins/spotify-web-api-with-fixes-and-improvements-from-sonallux-go/models` for request/response types.
...
```go
import "github.com/context-plugins/spotify-web-api-with-fixes-and-improvements-from-sonallux-go"
...
from environment variables named `SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := spotifywebapiwithfixesandimprovementsfromsonallux.NewClient(spotifywebapiwithfixesandimprovementsfromsonallux.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `spotifywebapiwithfixesandimprovementsfromsonallux.PRODUCTION`,
`spotifywebapiwithfixesandimprovementsfromsonallux.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := spotifywebapiwithfixesandimprovementsfromsonallux.NewClient(
    spotifywebapiwithfixesandimprovementsfromsonallux.CreateConfiguration(spotifywebapiwithfixesandimprovementsfromsonallux.WithEnvironment(spotifywebapiwithfixesandimprovementsfromsonallux.PRODUCTION)),
)
```
    ...
    spotifywebapiwithfixesandimprovementsfromsonallux.CreateConfiguration(
        spotifywebapiwithfixesandimprovementsfromsonallux.WithHttpConfiguration(
            spotifywebapiwithfixesandimprovementsfromsonallux.CreateHttpConfiguration(
                spotifywebapiwithfixesandimprovementsfromsonallux.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                spotifywebapiwithfixesandimprovementsfromsonallux.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = spotifywebapiwithfixesandimprovementsfromsonallux.NewClient(spotifywebapiwithfixesandimprovementsfromsonallux.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api spotifywebapiwithfixesandimprovementsfromsonallux.ClientInterface
}

func NewService(api spotifywebapiwithfixesandimprovementsfromsonallux.ClientInterface) *Service {
