...
> Throughout, `adafruitiorestapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := adafruitiorestapi.NewClient(
    adafruitiorestapi.CreateConfiguration(
        adafruitiorestapi.WithBasicAuthCredentials(
            adafruitiorestapi.NewBasicAuthCredentials(
...
adafruitiorestapi.WithApiKeyCredentials(adafruitiorestapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
adafruitiorestapi.WithApiHeaderCredentials(adafruitiorestapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
adafruitiorestapi.WithOAuthBearerTokenCredentials(
    adafruitiorestapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := adafruitiorestapi.NewClient(
    adafruitiorestapi.CreateConfiguration(
        adafruitiorestapi.WithOAuthCCGCredentials(
            adafruitiorestapi.NewOAuthCCGCredentials(
