...
> Throughout, `frankfurter` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := frankfurter.NewClient(
    frankfurter.CreateConfiguration(
        frankfurter.WithBasicAuthCredentials(
            frankfurter.NewBasicAuthCredentials(
...
frankfurter.WithApiKeyCredentials(frankfurter.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
frankfurter.WithApiHeaderCredentials(frankfurter.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
frankfurter.WithOAuthBearerTokenCredentials(
    frankfurter.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := frankfurter.NewClient(
    frankfurter.CreateConfiguration(
        frankfurter.WithOAuthCCGCredentials(
            frankfurter.NewOAuthCCGCredentials(
