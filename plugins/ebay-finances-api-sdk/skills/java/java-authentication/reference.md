...
against the `EBayFinancesAPIClient.Builder` setters, the `com.ebay.apiz.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `EBayFinancesAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
EBayFinancesAPIClient client = new EBayFinancesAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeEBayFinancesAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new EBayFinancesAPIClient.Builder()
