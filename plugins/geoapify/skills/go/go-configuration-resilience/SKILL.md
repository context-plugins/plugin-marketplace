    ...
    geoapify.CreateConfiguration(
        geoapify.WithHttpConfiguration(
            geoapify.CreateHttpConfiguration(
                geoapify.WithTimeout(30),
                geoapify.WithRetryConfiguration(
                    geoapify.CreateRetryConfiguration(
                        geoapify.WithMaxRetryAttempts(3),
                        geoapify.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := geoapify.CreateRetryConfiguration(
    geoapify.WithMaxRetryAttempts(5),
    geoapify.WithRetryInterval(1),
    geoapify.WithBackoffFactor(2),
    geoapify.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    geoapify.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := geoapify.NewClient(
    geoapify.CreateConfiguration(
        geoapify.WithHttpConfiguration(
            geoapify.CreateHttpConfiguration(
                geoapify.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
