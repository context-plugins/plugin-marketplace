...
> `AdyenBinLookupAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AdyenBinLookupAPIClient
{
...
```php
use AdyenBinLookupAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AdyenBinLookupAPILib\Exceptions\{OperationException};
use AdyenBinLookupAPILib\Exceptions\ApiException;
...
app()->instance(AdyenBinLookupAPIClient::class, clientWithResponses([
