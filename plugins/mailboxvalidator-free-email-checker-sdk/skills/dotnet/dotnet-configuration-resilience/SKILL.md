...
All types below live under `MailboxValidatorFreeEmailChecker.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using MailboxValidatorFreeEmailChecker.Core.Configuration;
...
```csharp
using MailboxValidatorFreeEmailChecker.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new MailboxValidatorFreeEmailCheckerClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `MailboxValidatorFreeEmailChecker.Core.Exceptions`):
...
With DI, the SDK's `AddMailboxValidatorFreeEmailCheckerClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddMailboxValidatorFreeEmailCheckerClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
