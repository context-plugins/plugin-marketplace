...
| --- | --- |
| API | sportsdata |
| Source repo | https://github.com/context-plugins/sportsdata-csharp-sdk (branch `main`) |
| Root namespace | `Sportsdata` (the `using` namespace) |
| Client class | `SportsdataClient` |
| Options class | `SportsdataClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `SportsdataClientOptions`: `ApiKeyHeader: string?`, `ApiKeyQuery: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using Sportsdata.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/sportsdata-csharp-sdk
dotnet add reference sportsdata-csharp-sdk/Sportsdata.csproj
```
...
temp directory** (`<temp>/sportsdata-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/sportsdata-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/sportsdata-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/sportsdata-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\sportsdata-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/sportsdata-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/sportsdata-dotnet-src/`, **never** in the project repo, and its path never goes into
`sportsdata-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new SportsdataClient(...)`, build its options, or DI-register via
   `AddSportsdataClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `SportsdataClientOptions` — the map's *Servers & auth* section
