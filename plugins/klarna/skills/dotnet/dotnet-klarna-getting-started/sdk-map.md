...
```csharp
using Klarna;
using Klarna.Servers; // ServerEnvironment lives here

var options = new KlarnaClientOptions
{
...
var client = new KlarnaClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddKlarnaClient(o =>
{
...
All `KlarnaClientOptions` properties (source: `KlarnaClientOptions.cs`):
...
- `KlarnaClient(HttpClient httpClient, KlarnaClientOptions options)`
<!-- /crawler:client-options -->
...
|---|---|---|
| `ApiError` — abstract base of all 52 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
...
Of **27 operations**, **27 are Case A (typed)** and **0 are Case B (raw)**.
<!-- /crawler:op-stats -->

---

## Operations — by controller (4 groups, 27 operations)
...
|---|---:|---|
| `Captures` | 7 | [map/operations/Captures.md](map/operations/Captures.md) |
| `Orders` | 9 | [map/operations/Orders.md](map/operations/Orders.md) |
| `PaymentDisputeApi` | 9 | [map/operations/PaymentDisputeApi.md](map/operations/PaymentDisputeApi.md) |
| `Refunds` | 2 | [map/operations/Refunds.md](map/operations/Refunds.md) |
<!-- /crawler:ops-table -->
...
|---|---|
| Client & options (root) | `Klarna` |
| Operation controllers (`Api/`) | `Klarna.Api` |
| Records (`Models/`) | `Klarna.Models` |
| Enums (`Models/Enums/`) | `Klarna.Models.Enums` |
| Unions (`Models/AnyOf/`, `Models/OneOf/`) | `Klarna.Models.OneOf` |
| Error classes (`Errors/`) | `Klarna.Errors` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `KlarnaClientOptions` (source: `KlarnaClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:

| Property | Type | Notes (from the source XML docs) |
|---|---|---|
| `KlarnaApiKey` | `string?` | Use the Klarna API key as HTTP Basic Authentication credentials |
| `BasicAuth` | `BasicAuthCredentials?` | — |
