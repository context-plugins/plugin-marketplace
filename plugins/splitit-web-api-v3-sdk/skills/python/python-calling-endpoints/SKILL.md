...
Open `splititwebapiv3/splititwebapiv3_client.py` for the controller property names, then the relevant
`splititwebapiv3/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from splititwebapiv3.models.{request_model} import {RequestModel}
...
Open `splititwebapiv3/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from splititwebapiv3.models.suite_code_enum import SuiteCodeEnum
...
```python
from splititwebapiv3.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `splititwebapiv3/pagination/` in the source.

## Finding the right method

- Controller property names are on `SplititWebApiV3Client` in `splititwebapiv3/splititwebapiv3_client.py`.
- Operation method signatures are in `splititwebapiv3/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `splititwebapiv3/models/`; enum types have `from_value()`.
