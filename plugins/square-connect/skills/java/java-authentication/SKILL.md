...
To see which schemes a specific SDK accepts, read the credential setter methods on `SquareConnectAPIClient.Builder`
...
- Set credentials **on the `SquareConnectAPIClient.Builder`** before calling `.build()` — the client is immutable
