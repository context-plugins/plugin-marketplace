...
To see which schemes a specific SDK accepts, read the credential setter methods on `PazeCheckoutApiClient.Builder`
...
- Set credentials **on the `PazeCheckoutApiClient.Builder`** before calling `.build()` — the client is immutable
