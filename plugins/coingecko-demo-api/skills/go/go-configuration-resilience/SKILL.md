    ...
    coingeckodemoapi.CreateConfiguration(
        coingeckodemoapi.WithHttpConfiguration(
            coingeckodemoapi.CreateHttpConfiguration(
                coingeckodemoapi.WithTimeout(30),
                coingeckodemoapi.WithRetryConfiguration(
                    coingeckodemoapi.CreateRetryConfiguration(
                        coingeckodemoapi.WithMaxRetryAttempts(3),
                        coingeckodemoapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := coingeckodemoapi.CreateRetryConfiguration(
    coingeckodemoapi.WithMaxRetryAttempts(5),
    coingeckodemoapi.WithRetryInterval(1),
    coingeckodemoapi.WithBackoffFactor(2),
    coingeckodemoapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    coingeckodemoapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := coingeckodemoapi.NewClient(
    coingeckodemoapi.CreateConfiguration(
        coingeckodemoapi.WithHttpConfiguration(
            coingeckodemoapi.CreateHttpConfiguration(
                coingeckodemoapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
