...
| --- | --- |
| API | `Verizon API` |
| Project | `VerizonApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `VerizonApi` (the `using` namespace) |
| Client class | `VerizonApiClient` |
| Options class | `VerizonApiClientOptions` |
...
namespaces transitively, so `using VerizonApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/verizon-api-csharp
dotnet add reference verizon-api-csharp/VerizonApi.csproj
```

```csharp
using VerizonApi;          // client + options + ServerEnvironment
using VerizonApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/verizon-api-csharp /tmp/verizon-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/verizon-api-csharp "$env:TEMP\verizon-api-dotnet-src"
```
...
- `VerizonApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/verizon-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\verizon-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `VerizonApiClient`, build its
   `VerizonApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
