...
`testing` + `testify/require` for reference; substitute the real `adyenbalancecontrolapi` and names.
...
import (
    ...
    "github.com/context-plugins/adyen-balance-control-api-go"   // root package adyenbalancecontrolapi
)
...
func clientReturning(status int, body string) (adyenbalancecontrolapi.ClientInterface, *stubTransport) {
    ...
    client := adyenbalancecontrolapi.NewClient(
        adyenbalancecontrolapi.CreateConfiguration(
            adyenbalancecontrolapi.WithHttpConfiguration(
                adyenbalancecontrolapi.CreateHttpConfiguration(adyenbalancecontrolapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/adyen-balance-control-api-go/errors"
...
client := adyenbalancecontrolapi.NewClient(
    adyenbalancecontrolapi.CreateConfiguration(
        adyenbalancecontrolapi.WithHttpConfiguration(
            adyenbalancecontrolapi.CreateHttpConfiguration(adyenbalancecontrolapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
