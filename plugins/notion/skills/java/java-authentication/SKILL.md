...
To see which schemes a specific SDK accepts, read the credential setter methods on `NotionClient.Builder`
...
- Set credentials **on the `NotionClient.Builder`** before calling `.build()` — the client is immutable
