...
All types below live under `MediumApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using MediumApi.Core.Configuration;
...
```csharp
using MediumApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new MediumApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `MediumApi.Core.Exceptions`):
...
With DI, the SDK's `AddMediumApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddMediumApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
