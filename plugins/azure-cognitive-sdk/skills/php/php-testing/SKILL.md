...
> `AzureCognitiveClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AzureCognitiveClient
{
...
```php
use AzureCognitiveLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AzureCognitiveLib\Exceptions\{OperationException};
use AzureCognitiveLib\Exceptions\ApiException;
...
app()->instance(AzureCognitiveClient::class, clientWithResponses([
