...
- `telegram-bot-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'telegram-bot-apilib';
...
```ts
import { {Resource}Controller } from 'telegram-bot-apilib';
