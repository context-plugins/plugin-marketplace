...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `IPGeolocationAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`ipgeolocationapi/configuration.py` in the cloned source.
...
```python
import requests
from ipgeolocationapi.configuration import Configuration
...
`ipgeolocationapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`ipgeolocationapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `ipgeolocationapi/configuration.py`
...
`NumberPagedResponse`) live in `ipgeolocationapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `ipgeolocationapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
