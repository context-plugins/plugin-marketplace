...
│   └── com.googleapis.www.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.googleapis.www.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.googleapis.www.http.client.HttpContext`, which carries:
