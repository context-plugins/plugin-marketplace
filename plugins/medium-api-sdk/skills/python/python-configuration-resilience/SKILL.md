...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `MediumAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`mediumapi/configuration.py` in the cloned source.
...
`mediumapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`mediumapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `mediumapi/configuration.py`
...
`NumberPagedResponse`) live in `mediumapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `mediumapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
