---
name: dotnet-stax-fattmerchant-api-getting-started
description: Stax (FattMerchant) API .NET SDK identity and lookup layer for the dotnet-stax-fattmerchant-api-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Stax (FattMerchant) API .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-stax-fattmerchant-api-sdk` helper
...
| | |
| --- | --- |
| API | Stax (FattMerchant) API |
| Source repo | https://github.com/context-plugins/stax-fattmerchant-api-csharp-sdk (branch `main`) |
| Root namespace | `StaxFattMerchantApi` (the `using` namespace) |
| Client class | `StaxFattMerchantApiClient` |
| Options class | `StaxFattMerchantApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `StaxFattMerchantApiClientOptions`: `BearerAuth: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using StaxFattMerchantApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/stax-fattmerchant-api-csharp-sdk
dotnet add reference stax-fattmerchant-api-csharp-sdk/StaxFattMerchantApi.csproj
```
...
temp directory** (`<temp>/stax-fattmerchant-api-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/stax-fattmerchant-api-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/stax-fattmerchant-api-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/stax-fattmerchant-api-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\stax-fattmerchant-api-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/stax-fattmerchant-api-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/stax-fattmerchant-api-dotnet-src/`, **never** in the project repo, and its path never goes into
`stax-fattmerchant-api-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `StaxFattMerchantApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new StaxFattMerchantApiClient(...)`, build its options, or DI-register via
   `AddStaxFattMerchantApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `StaxFattMerchantApiClientOptions` — the map's *Servers & auth* section
