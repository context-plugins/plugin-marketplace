...
- `PointsOfInterestClient` — the single public client class (e.g. `FooClient`).
- `PointsOfInterestClientOptions` — its options class.
- `PointsOfInterest` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public PointsOfInterestClient(HttpClient httpClient, PointsOfInterestClientOptions options)
```
...
```csharp
public class PointsOfInterestClientOptions
{
...
using PointsOfInterest.Servers;

var options = new PointsOfInterestClientOptions
{
...
var client = new PointsOfInterestClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddPointsOfInterestClient`, which registers the
...
builder.Services.AddPointsOfInterestClient(options =>
{
...
```csharp
public sealed class MyService(PointsOfInterestClient client)
{
