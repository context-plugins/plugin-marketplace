...
against the `SpotifyWebApiClient.Builder` setters, the `com.spotify.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SpotifyWebApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
SpotifyWebApiClient client = new SpotifyWebApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeSpotifyWebAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new SpotifyWebApiClient.Builder()
