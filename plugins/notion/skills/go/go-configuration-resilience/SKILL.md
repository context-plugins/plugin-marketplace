    ...
    notion.CreateConfiguration(
        notion.WithHttpConfiguration(
            notion.CreateHttpConfiguration(
                ...
                notion.WithRetryConfiguration(
                    notion.CreateRetryConfiguration(
                        notion.WithMaxRetryAttempts(3),
                        notion.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := notion.CreateRetryConfiguration(
    notion.WithMaxRetryAttempts(5),
    notion.WithRetryInterval(1),
    notion.WithBackoffFactor(2),
    notion.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    notion.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := notion.NewClient(
    notion.CreateConfiguration(
        notion.WithHttpConfiguration(
            notion.CreateHttpConfiguration(
                notion.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
