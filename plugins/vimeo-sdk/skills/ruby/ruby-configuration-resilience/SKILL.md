...
```ruby
client = Vimeo::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Vimeo::Client.new(adapter: :net_http)
```
...
client = Vimeo::Client.new(connection: conn)
```
...
client = Vimeo::Client.new(environment: Environment::PRODUCTION)
client = Vimeo::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = Vimeo::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Vimeo::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Vimeo::ProxySettings.from_env
client = Vimeo::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Vimeo::HttpCallBack
...
client = Vimeo::Client.new(http_callback: LogCallback.new)
```
...
client = Vimeo::Client.from_env(timeout: 30, max_retries: 3)
```
