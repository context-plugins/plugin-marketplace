...
Verified against `package.json` and `sdk-map.md` of the generated package at version `2022-06-28`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Notion API |
| Package name (what you install, and what you import) | `notion-api` — **not on npm**; built from source (see *Install*) |
| Import specifier | `notion-api` — the package root is the **only** entry; deep imports do not resolve |
| Version | `2022-06-28` (API spec version `2022-06-28`) |
| Client class | `NotionApiClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new NotionApiClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new NotionApiClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `bearerAuth`. `timeout` defaults to `60_000` ms |
...
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://api.notion.com/v1` |
...
| Surface | 5 operations · 0 resources · 8 models · 2 open enums · 5 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/notion-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 10 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { NotionApiClient, ServerEnvironment, ResponseError, NotionApiError } from "notion-api";
import type { ClientOptions, Block } from "notion-api";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `notion-api/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **⚠ The SDK exports a model type literally named `Error`** (`src/models/error.ts`, schema `errorSchema`). Import it unaliased and it **shadows the global `Error`** for the rest of the file. Alias it: `import type { Error as SdkError } from "notion-api"`.
- **From CommonJS**, the typed spelling is `import sdk = require("notion-api")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://api.notion.com/v1` | `serverOptions.default.production.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 5 of the 5 operations require a credential and 0 are public.
...
5 operations are methods on the client itself: `client.appendBlockChildren` · `client.deleteBlock` · `client.retrieveBlock` · `client.retrieveBlockChildren` · `client.updateBlock`.
...
```bash
node -e "console.log(require.resolve('notion-api/package.json'))"
```

Failing that it is at `node_modules/notion-api/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/notion-api/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/notion-api/`:
...
Beyond the usual signatures and model members, a contract sheet for the Notion API TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
...
9. **That `Error` imported from this package is a model type, not the global of that name** — every sheet that references one should carry the alias it will be imported under. The error base is re-exported as `NotionApiError` for the same reason.
