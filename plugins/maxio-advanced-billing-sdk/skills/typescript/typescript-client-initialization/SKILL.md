...
- `maxio-advanced-billinglib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'maxio-advanced-billinglib';
...
```ts
import { {Resource}Controller } from 'maxio-advanced-billinglib';
