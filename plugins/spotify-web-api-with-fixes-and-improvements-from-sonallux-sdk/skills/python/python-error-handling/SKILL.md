...
`spotifywebapiwithfixesandimprovementsfromsonallux/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `spotifywebapiwithfixesandimprovementsfromsonallux/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `spotifywebapiwithfixesandimprovementsfromsonallux/exceptions/`.
...
file (`spotifywebapiwithfixesandimprovementsfromsonallux/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`spotifywebapiwithfixesandimprovementsfromsonallux/exceptions/` for the class.
...
`spotifywebapiwithfixesandimprovementsfromsonallux/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `spotifywebapiwithfixesandimprovementsfromsonallux/exceptions/`
  or a `ValueError` — check the auth handler in `spotifywebapiwithfixesandimprovementsfromsonallux/http/auth/` for the exact type.
