...
| --- | --- |
| API | `Seatmap Display` |
| Project | `SeatmapDisplay.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `SeatmapDisplay` (the `using` namespace) |
| Client class | `SeatmapDisplayClient` |
| Options class | `SeatmapDisplayClientOptions` |
...
namespaces transitively, so `using SeatmapDisplay.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/seatmap-display-csharp
dotnet add reference seatmap-display-csharp/SeatmapDisplay.csproj
```

```csharp
using SeatmapDisplay;          // client + options + ServerEnvironment
using SeatmapDisplay.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/seatmap-display-csharp /tmp/seatmap-display-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/seatmap-display-csharp "$env:TEMP\seatmap-display-dotnet-src"
```
...
- `SeatmapDisplayClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/seatmap-display-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\seatmap-display-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `SeatmapDisplayClient`, build its
   `SeatmapDisplayClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
