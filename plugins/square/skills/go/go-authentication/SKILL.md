...
> Throughout, `square` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := square.NewClient(
    square.CreateConfiguration(
        square.WithBasicAuthCredentials(
            square.NewBasicAuthCredentials(
...
square.WithApiKeyCredentials(square.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
square.WithApiHeaderCredentials(square.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
square.WithOAuthBearerTokenCredentials(
    square.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
