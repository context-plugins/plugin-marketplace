...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `HostedOnboardingAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`hostedonboardingapi/configuration.py` in the cloned source.
...
```python
import requests
from hostedonboardingapi.configuration import Configuration
...
`hostedonboardingapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`hostedonboardingapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `hostedonboardingapi/configuration.py`
...
`NumberPagedResponse`) live in `hostedonboardingapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `hostedonboardingapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
