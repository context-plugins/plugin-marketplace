...
description: "CoinGecko Python SDK identity and lookup layer (Python only) — install, import root, base URL/environments, the auth pattern, the SDK map that ships at the SDK root (`sdk-map.md` + `map/operations/`) and how to traverse it, and the module table naming the one file owning each fact the map leaves to the source. Load this before answering any CoinGecko Python SDK contract question or writing any SDK code."
---

# Getting started with the CoinGecko Python SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing CoinGecko Python SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source modules it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `python-integrate-coin-gecko` for the workflow that wraps this skill.
...
Verified against `coin_gecko/` and `pyproject.toml` of the generated package at version `3.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | CoinGecko |
| Distribution name (what you install) | `coin-gecko` — **not on any package index**; installed from source (see *Install*) |
| Import root (what you import) | `coin_gecko` — note the underscores; the two names differ |
| Source repository | https://github.com/context-plugins/coingecko-python-sdk |
| Source branch | `main` |
| Version | `3.0.0` |
| Sync client class | `CoinGeckoClient` (alias `Client`) |
| Async client class | `AsyncCoinGeckoClient` (alias `AsyncClient`) |
...
| Base-URL config | `ServerConfig` (`coin_gecko/server/server_config.py`), frozen, `extra="forbid"` |
...
| Surface | 61 operations across 0 controllers · 195 models · 28 enums · 0 per-operation error unions |
...
```bash
pip install "coin-gecko @ git+https://github.com/context-plugins/coingecko-python-sdk.git@main"
```
...
Python does not re-export child modules transitively, so `from coin_gecko import models` alone does **not** make enums, error unions, or runtime types reachable. Import each kind of type from the module that owns it.

`coin_gecko/__init__.py` exports exactly 5 names:
...
| --- | --- |
| Domain models, their `…Dict` companions | `coin_gecko.models` |
| Enums (and their open `…OrStr` aliases) | `coin_gecko.models.enums` |
| `ApiError` · `RawError` · `ApiResult` · `RequestOptions` · `HttpClient` · `SdkBaseModel` · `UNSET` · `Optional` | `coin_gecko.core` |
| Per-operation error *unions* | `coin_gecko.errors` — *this SDK declares none* |

`coin_gecko.core` re-exports its whole public surface (a curated `__all__`), so import from `…core` rather than from the private modules beneath it (`…core.results`, `…core.exceptions`, `…core.auth.schemes`).

## Environments — there is no environment enum

This SDK has **no environment type and no environment constants**. There is one knob: `base_url`, on `ServerConfig` (`coin_gecko/server/server_config.py`), and its default is **`https://api.coingecko.com/api/v3`**:
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `coin_gecko/` source directory, and these two entries:
...
Every `Source` path on the map is relative to that SDK root, so `coin_gecko/models/asset_platform.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
```bash
python -c "import coin_gecko, pathlib; print(pathlib.Path(coin_gecko.__file__).parent)"
```

Failing that, it is under the project's environment (`.venv/Lib/site-packages/coin_gecko` on Windows, `.venv/lib/python3.*/site-packages/coin_gecko` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
