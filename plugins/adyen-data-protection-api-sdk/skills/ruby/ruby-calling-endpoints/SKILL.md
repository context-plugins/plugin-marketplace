...
Open `lib/adyen_data_protection_api/client.rb` for the accessor names, then the relevant
`lib/adyen_data_protection_api/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = AdyenDataProtectionAPI::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: AdyenDataProtectionAPI::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/adyen_data_protection_api/models/`. Field names are defined as
...
Read the enum module in `lib/adyen_data_protection_api/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/adyen_data_protection_api/client.rb`.
- Operation methods are in `lib/adyen_data_protection_api/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/adyen_data_protection_api/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/adyen_data_protection_api/exceptions/`.
