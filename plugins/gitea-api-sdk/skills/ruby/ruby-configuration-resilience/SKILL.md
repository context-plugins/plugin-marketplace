...
```ruby
client = GiteaAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = GiteaAPI::Client.new(adapter: :net_http)
```
...
client = GiteaAPI::Client.new(connection: conn)
```
...
client = GiteaAPI::Client.new(environment: Environment::PRODUCTION)
client = GiteaAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = GiteaAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = GiteaAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = GiteaAPI::ProxySettings.from_env
client = GiteaAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < GiteaAPI::HttpCallBack
...
client = GiteaAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = GiteaAPI::Client.from_env
# Overrides take precedence over env vars:
client = GiteaAPI::Client.from_env(timeout: 30, max_retries: 3)
```
