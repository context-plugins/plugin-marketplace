...
- `APIsGuruClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `guru.apis.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`APIsGuruClient.Builder` class. A minimal initialization:

```java
import guru.apis.api.APIsGuruClient;
import guru.apis.api.Environment;

APIsGuruClient client = new APIsGuruClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `APIsGuruClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `APIsGuruClient.java` |

Call `.build()` to produce an immutable `APIsGuruClient` instance.
...
public class ApiConfig {
    @Bean
    public APIsGuruClient apiClient() {
        return new APIsGuruClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired APIsGuruClient client) {
        client.shutdown();
    }
}
```

Inject `APIsGuruClient` into your services as a normal Spring dependency. Build only one bean.
