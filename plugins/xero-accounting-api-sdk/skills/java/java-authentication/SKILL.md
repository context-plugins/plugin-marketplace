...
To see which schemes a specific SDK accepts, read the credential setter methods on `XeroAccountingAPIClient.Builder`
...
- Set credentials **on the `XeroAccountingAPIClient.Builder`** before calling `.build()` — the client is immutable
