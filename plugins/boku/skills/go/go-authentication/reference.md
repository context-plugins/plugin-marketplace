...
```go
boku.WithOAuthCCGCredentials(
    boku.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr boku.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := boku.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr boku.OAuthCCGManager) models.OAuthToken {
...
```go
creds := boku.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := boku.NewClient(boku.CreateConfiguration(boku.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    boku.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := boku.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := boku.NewClient(boku.CreateConfiguration(boku.WithOAuthROPCGCredentials(creds)))
...
```go
boku.WithOAuthBearerTokenCredentials(boku.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
boku.WithBasicAuthCredentials(boku.NewBasicAuthCredentials(username, password))
boku.WithApiHeaderCredentials(boku.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
boku.WithApiKeyCredentials(boku.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `BOKU_...` variables — e.g.
`BOKU_USERNAME`, `BOKU_PASSWORD`, `BOKU_TOKEN`, `BOKU_API_KEY`,
`BOKU_O_AUTH_CLIENT_ID`, `BOKU_O_AUTH_CLIENT_SECRET`, `BOKU_ACCESS_TOKEN`. Grep the
