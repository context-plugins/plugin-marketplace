...
`openmeteoweatherforecastapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `openmeteoweatherforecastapi/exceptions/` and operation docstrings.
...
```python
from openmeteoweatherforecastapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `openmeteoweatherforecastapi/exceptions/`.
...
```python
from openmeteoweatherforecastapi.exceptions.api_exception import APIException
from openmeteoweatherforecastapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`openmeteoweatherforecastapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`openmeteoweatherforecastapi/exceptions/` for the class.
...
`openmeteoweatherforecastapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `openmeteoweatherforecastapi/exceptions/`
  or a `ValueError` — check the auth handler in `openmeteoweatherforecastapi/http/auth/` for the exact type.
