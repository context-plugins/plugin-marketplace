    ...
    deepgram.CreateConfiguration(
        deepgram.WithHttpConfiguration(
            deepgram.CreateHttpConfiguration(
                deepgram.WithTimeout(30),
                deepgram.WithRetryConfiguration(
                    deepgram.CreateRetryConfiguration(
                        deepgram.WithMaxRetryAttempts(3),
                        deepgram.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := deepgram.CreateRetryConfiguration(
    deepgram.WithMaxRetryAttempts(5),
    deepgram.WithRetryInterval(1),
    deepgram.WithBackoffFactor(2),
    deepgram.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    deepgram.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := deepgram.NewClient(
    deepgram.CreateConfiguration(
        deepgram.WithHttpConfiguration(
            deepgram.CreateHttpConfiguration(
                deepgram.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
