...
`maxioadvancedbilling/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `maxioadvancedbilling/exceptions/` and operation docstrings.
...
```python
from maxioadvancedbilling.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `maxioadvancedbilling/exceptions/`.
...
```python
from maxioadvancedbilling.exceptions.api_exception import APIException
from maxioadvancedbilling.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`maxioadvancedbilling/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`maxioadvancedbilling/exceptions/` for the class.
...
`maxioadvancedbilling/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `maxioadvancedbilling/exceptions/`
  or a `ValueError` — check the auth handler in `maxioadvancedbilling/http/auth/` for the exact type.
