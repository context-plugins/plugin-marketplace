...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `Xapiv2Client`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`xapiv2/configuration.py` in the cloned source.
...
```python
import requests
from xapiv2.configuration import Configuration
...
`xapiv2/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`xapiv2/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `xapiv2/configuration.py`
...
`NumberPagedResponse`) live in `xapiv2/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `xapiv2/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
