...
```ruby
client = XeroAccountingAPI::Client.new(
  max_retries:    3,
...
```ruby
client = XeroAccountingAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = XeroAccountingAPI::Client.new(adapter: :net_http)
```
...
client = XeroAccountingAPI::Client.new(connection: conn)
```
...
client = XeroAccountingAPI::Client.new(environment: Environment::PRODUCTION)
client = XeroAccountingAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = XeroAccountingAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = XeroAccountingAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = XeroAccountingAPI::ProxySettings.from_env
client = XeroAccountingAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < XeroAccountingAPI::HttpCallBack
...
client = XeroAccountingAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = XeroAccountingAPI::Client.from_env
# Overrides take precedence over env vars:
client = XeroAccountingAPI::Client.from_env(timeout: 30, max_retries: 3)
```
