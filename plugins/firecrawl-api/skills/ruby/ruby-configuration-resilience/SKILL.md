...
```ruby
client = FirecrawlAPI::Client.new(
  max_retries:    3,
...
```ruby
client = FirecrawlAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = FirecrawlAPI::Client.new(adapter: :net_http)
```
...
client = FirecrawlAPI::Client.new(connection: conn)
```
...
client = FirecrawlAPI::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = FirecrawlAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = FirecrawlAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = FirecrawlAPI::ProxySettings.from_env
client = FirecrawlAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < FirecrawlAPI::HttpCallBack
...
client = FirecrawlAPI::Client.new(http_callback: LogCallback.new)
```
...
client = FirecrawlAPI::Client.from_env(timeout: 30, max_retries: 3)
```
