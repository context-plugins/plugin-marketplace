...
- `akoya` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/akoya-go-sdk` — the Go import path for the root package; `github.com/context-plugins/akoya-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/akoya-go-sdk"

client := akoya.NewClient(
    akoya.CreateConfiguration(
        akoya.WithEnvironment(akoya.PRODUCTION),
...
from environment variables named `AKOYA_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := akoya.NewClient(akoya.CreateConfigurationFromEnvironment())
```
...
```go
client := akoya.NewClient(
    akoya.CreateConfiguration(akoya.WithEnvironment(akoya.PRODUCTION)),
)
```
...
client := akoya.NewClient(
    akoya.CreateConfiguration(
        akoya.WithHttpConfiguration(
            akoya.CreateHttpConfiguration(
                akoya.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                akoya.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = akoya.NewClient(akoya.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api akoya.ClientInterface
}

func NewService(api akoya.ClientInterface) *Service {
