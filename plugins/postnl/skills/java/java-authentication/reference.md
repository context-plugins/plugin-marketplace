...
against the `PostnlEcommerceClient.Builder` setters, the `nl.postnl.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `PostnlEcommerceClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
PostnlEcommerceClient client = new PostnlEcommerceClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopePostnlEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new PostnlEcommerceClient.Builder()
