...
`testing` + `testify/require` for reference; substitute the real `adyenbinlookupapi` and names.
...
import (
    ...
    "github.com/context-plugins/adyen-binlookup-api-go"   // root package adyenbinlookupapi
)
...
func clientReturning(status int, body string) (adyenbinlookupapi.ClientInterface, *stubTransport) {
    ...
    client := adyenbinlookupapi.NewClient(
        adyenbinlookupapi.CreateConfiguration(
            adyenbinlookupapi.WithHttpConfiguration(
                adyenbinlookupapi.CreateHttpConfiguration(adyenbinlookupapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/adyen-binlookup-api-go/errors"
...
client := adyenbinlookupapi.NewClient(
    adyenbinlookupapi.CreateConfiguration(
        adyenbinlookupapi.WithHttpConfiguration(
            adyenbinlookupapi.CreateHttpConfiguration(adyenbinlookupapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
