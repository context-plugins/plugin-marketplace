...
`configurationapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `configurationapi/exceptions/` and operation docstrings.
...
```python
from configurationapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `configurationapi/exceptions/`.
...
```python
from configurationapi.exceptions.api_exception import APIException
from configurationapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`configurationapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`configurationapi/exceptions/` for the class.
...
`configurationapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `configurationapi/exceptions/`
  or a `ValueError` — check the auth handler in `configurationapi/http/auth/` for the exact type.
