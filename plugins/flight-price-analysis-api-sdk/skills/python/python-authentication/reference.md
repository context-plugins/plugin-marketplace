...
kwargs to the `FlightPriceAnalysisAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `flightpriceanalysisapi/flightpriceanalysisapi_client.py` and `flightpriceanalysisapi/http/auth/` in
the cloned source.

## Basic auth

```python
from flightpriceanalysisapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from flightpriceanalysisapi.http.auth.api_key import ApiKeyCredentials
...
```python
from flightpriceanalysisapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from flightpriceanalysisapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from flightpriceanalysisapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from flightpriceanalysisapi.http.auth.o_auth_acg import OAuthACGCredentials
from flightpriceanalysisapi.models.o_auth_scope_flightpriceanalysisapi_enum import OAuthScope{Pkg}Enum
...
client = FlightPriceAnalysisAPIClient(o_auth_acg_credentials=creds)
...
```python
from flightpriceanalysisapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FlightPriceAnalysisAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FlightPriceAnalysisAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `flightpriceanalysisapi/configuration.py` and each `flightpriceanalysisapi/http/auth/*.py` for the exact
