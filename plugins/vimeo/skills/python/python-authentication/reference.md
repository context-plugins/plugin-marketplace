...
kwargs to the `VimeoAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `vimeoapi/vimeoapi_client.py` and `vimeoapi/http/auth/` in
the cloned source.

## Basic auth

```python
from vimeoapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from vimeoapi.http.auth.api_key import ApiKeyCredentials
...
```python
from vimeoapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from vimeoapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from vimeoapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from vimeoapi.http.auth.o_auth_acg import OAuthACGCredentials
from vimeoapi.models.o_auth_scope_vimeoapi_enum import OAuthScope{Pkg}Enum
...
client = VimeoAPIClient(o_auth_acg_credentials=creds)
...
```python
from vimeoapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = VimeoAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `VimeoAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `vimeoapi/configuration.py` and each `vimeoapi/http/auth/*.py` for the exact
