...
| | |
| --- | --- |
| API | `OpenUV - Global Real-Time UV Index Forecast API` |
| Project | `OpenUvGlobalRealTimeUvIndexForecastApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `OpenUvGlobalRealTimeUvIndexForecastApi` (the `using` namespace) |
| Client class | `OpenUvGlobalRealTimeUvIndexForecastApiClient` |
| Options class | `OpenUvGlobalRealTimeUvIndexForecastApiClientOptions` |
...
namespaces transitively, so `using OpenUvGlobalRealTimeUvIndexForecastApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/openuv-global-real-time-uv-index-forecast-api-csharp
dotnet add reference openuv-global-real-time-uv-index-forecast-api-csharp/OpenUvGlobalRealTimeUvIndexForecastApi.csproj
```

```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi;          // client + options + ServerEnvironment
using OpenUvGlobalRealTimeUvIndexForecastApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/openuv-global-real-time-uv-index-forecast-api-csharp /tmp/openuv-global-real-time-uv-index-forecast-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/openuv-global-real-time-uv-index-forecast-api-csharp "$env:TEMP\openuv-global-real-time-uv-index-forecast-api-dotnet-src"
```
...
- `OpenUvGlobalRealTimeUvIndexForecastApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/openuv-global-real-time-uv-index-forecast-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\openuv-global-real-time-uv-index-forecast-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `OpenUvGlobalRealTimeUvIndexForecastApiClient`, build its
   `OpenUvGlobalRealTimeUvIndexForecastApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
