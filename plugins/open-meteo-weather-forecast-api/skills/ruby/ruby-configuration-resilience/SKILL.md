...
```ruby
client = OpenMeteoWeatherForecastAPI::Client.new(
  max_retries:    3,
...
```ruby
client = OpenMeteoWeatherForecastAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = OpenMeteoWeatherForecastAPI::Client.new(adapter: :net_http)
```
...
client = OpenMeteoWeatherForecastAPI::Client.new(connection: conn)
```
...
client = OpenMeteoWeatherForecastAPI::Client.new(environment: Environment::PRODUCTION)
client = OpenMeteoWeatherForecastAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = OpenMeteoWeatherForecastAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = OpenMeteoWeatherForecastAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = OpenMeteoWeatherForecastAPI::ProxySettings.from_env
client = OpenMeteoWeatherForecastAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < OpenMeteoWeatherForecastAPI::HttpCallBack
...
client = OpenMeteoWeatherForecastAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = OpenMeteoWeatherForecastAPI::Client.from_env
# Overrides take precedence over env vars:
client = OpenMeteoWeatherForecastAPI::Client.from_env(timeout: 30, max_retries: 3)
```
