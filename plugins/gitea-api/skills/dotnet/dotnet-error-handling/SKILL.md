...
> `{ApiGroup}`, `GiteaApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `GiteaApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `GiteaApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `GiteaApi.Errors`
...
```csharp
using GiteaApi.Core.Exceptions;     // SdkException<TError>
using GiteaApi.Core.ErrorResponse;  // ApiError, RawError
using GiteaApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`GiteaApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using GiteaApi.Core.Exceptions;     // SdkException<TError>
using GiteaApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `GiteaApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `GiteaApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using GiteaApi.Core.Models;        // ApiResult<TResponse, TError>
using GiteaApi.Core.ErrorResponse; // RawError
using GiteaApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `GiteaApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
