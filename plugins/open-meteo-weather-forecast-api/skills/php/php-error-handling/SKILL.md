...
> `{operation}`, `{apiGroup}`, `OpenMeteoWeatherForecastApiLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `OpenMeteoWeatherForecastApiLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `OpenMeteoWeatherForecastApiLib\Exceptions\` — read the source to
