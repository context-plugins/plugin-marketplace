...
```go
traderapi.WithOAuthCCGCredentials(
    traderapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr traderapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := traderapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr traderapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := traderapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := traderapi.NewClient(traderapi.CreateConfiguration(traderapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    traderapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := traderapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := traderapi.NewClient(traderapi.CreateConfiguration(traderapi.WithOAuthROPCGCredentials(creds)))
...
```go
traderapi.WithOAuthBearerTokenCredentials(traderapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
traderapi.WithBasicAuthCredentials(traderapi.NewBasicAuthCredentials(username, password))
traderapi.WithApiHeaderCredentials(traderapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
traderapi.WithApiKeyCredentials(traderapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `TRADERAPI_...` variables — e.g.
`TRADERAPI_USERNAME`, `TRADERAPI_PASSWORD`, `TRADERAPI_TOKEN`, `TRADERAPI_API_KEY`,
`TRADERAPI_O_AUTH_CLIENT_ID`, `TRADERAPI_O_AUTH_CLIENT_SECRET`, `TRADERAPI_ACCESS_TOKEN`. Grep the
