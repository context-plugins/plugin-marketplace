...
```go
mailboxvalidatorfreeemailchecker.WithOAuthCCGCredentials(
    mailboxvalidatorfreeemailchecker.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr mailboxvalidatorfreeemailchecker.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := mailboxvalidatorfreeemailchecker.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr mailboxvalidatorfreeemailchecker.OAuthCCGManager) models.OAuthToken {
...
```go
creds := mailboxvalidatorfreeemailchecker.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := mailboxvalidatorfreeemailchecker.NewClient(mailboxvalidatorfreeemailchecker.CreateConfiguration(mailboxvalidatorfreeemailchecker.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    mailboxvalidatorfreeemailchecker.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := mailboxvalidatorfreeemailchecker.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := mailboxvalidatorfreeemailchecker.NewClient(mailboxvalidatorfreeemailchecker.CreateConfiguration(mailboxvalidatorfreeemailchecker.WithOAuthROPCGCredentials(creds)))
...
```go
mailboxvalidatorfreeemailchecker.WithOAuthBearerTokenCredentials(mailboxvalidatorfreeemailchecker.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
mailboxvalidatorfreeemailchecker.WithBasicAuthCredentials(mailboxvalidatorfreeemailchecker.NewBasicAuthCredentials(username, password))
mailboxvalidatorfreeemailchecker.WithApiHeaderCredentials(mailboxvalidatorfreeemailchecker.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
mailboxvalidatorfreeemailchecker.WithApiKeyCredentials(mailboxvalidatorfreeemailchecker.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `MAILBOXVALIDATORFREEEMAILCHECKER_...` variables — e.g.
`MAILBOXVALIDATORFREEEMAILCHECKER_USERNAME`, `MAILBOXVALIDATORFREEEMAILCHECKER_PASSWORD`, `MAILBOXVALIDATORFREEEMAILCHECKER_TOKEN`, `MAILBOXVALIDATORFREEEMAILCHECKER_API_KEY`,
`MAILBOXVALIDATORFREEEMAILCHECKER_O_AUTH_CLIENT_ID`, `MAILBOXVALIDATORFREEEMAILCHECKER_O_AUTH_CLIENT_SECRET`, `MAILBOXVALIDATORFREEEMAILCHECKER_ACCESS_TOKEN`. Grep the
