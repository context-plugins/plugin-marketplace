...
def stub_client(stubs)
  ...
  ShellCardmanagement::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises ShellCardmanagement::APIException do
...
BASE = ShellCardmanagement::Configuration.new(environment: ShellCardmanagement::Environment::PRODUCTION).get_base_uri

def build_test_client
  ShellCardmanagement::Client.new(
    environment: ShellCardmanagement::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ShellCardmanagement::APIException
```
...
```ruby
expect { call }.to raise_error(ShellCardmanagement::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ShellCardmanagement::HttpCallBack
...
client  = ShellCardmanagement::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
