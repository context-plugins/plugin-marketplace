...
> `{ApiGroup}`, `M3commasIo`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `M3commasIo.Core.Exceptions`
- `ApiError` **and** `RawError` → `M3commasIo.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `M3commasIo.Errors`
...
```csharp
using M3commasIo.Core.Exceptions;     // SdkException<TError>
using M3commasIo.Core.ErrorResponse;  // ApiError, RawError
using M3commasIo.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`M3commasIo.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using M3commasIo.Core.Exceptions;     // SdkException<TError>
using M3commasIo.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `M3commasIo.Core.ErrorResponse`. Its public
...
`readonly struct` under `M3commasIo.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using M3commasIo.Core.Models;        // ApiResult<TResponse, TError>
using M3commasIo.Core.ErrorResponse; // RawError
using M3commasIo.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `M3commasIo.Core.Exceptions`) — an auth *application* failure, not an API error — when the
