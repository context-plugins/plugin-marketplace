...
against the `MediumAPIClient.Builder` setters, the `com.rapidapi.p.medium2.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `MediumAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
MediumAPIClient client = new MediumAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeMediumAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
