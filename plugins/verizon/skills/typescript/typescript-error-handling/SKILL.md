...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `verizonlib`) — replace it with the concrete identifier from the source.
...
import { ApiError } from 'verizonlib';
import { {Operation}Error } from 'verizonlib/errors';
