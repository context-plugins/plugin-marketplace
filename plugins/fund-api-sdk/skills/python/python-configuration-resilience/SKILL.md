...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `FundAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`fundapi/configuration.py` in the cloned source.
...
```python
import requests
from fundapi.configuration import Configuration
...
`fundapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`fundapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `fundapi/configuration.py`
...
`NumberPagedResponse`) live in `fundapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `fundapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
