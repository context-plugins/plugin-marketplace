...
def stub_client(stubs)
  ...
  TeslaFleetManagementAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises TeslaFleetManagementAPI::APIException do
...
BASE = TeslaFleetManagementAPI::Configuration.new(environment: TeslaFleetManagementAPI::Environment::PRODUCTION).get_base_uri

def build_test_client
  TeslaFleetManagementAPI::Client.new(
    environment: TeslaFleetManagementAPI::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error TeslaFleetManagementAPI::APIException
```
...
```ruby
expect { call }.to raise_error(TeslaFleetManagementAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < TeslaFleetManagementAPI::HttpCallBack
...
client  = TeslaFleetManagementAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
