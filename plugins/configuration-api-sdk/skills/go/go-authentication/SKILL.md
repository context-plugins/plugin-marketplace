...
> Throughout, `configurationapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := configurationapi.NewClient(
    configurationapi.CreateConfiguration(
        configurationapi.WithBasicAuthCredentials(
            configurationapi.NewBasicAuthCredentials(
...
configurationapi.WithApiKeyCredentials(configurationapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
configurationapi.WithApiHeaderCredentials(configurationapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
configurationapi.WithOAuthBearerTokenCredentials(
    configurationapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := configurationapi.NewClient(
    configurationapi.CreateConfiguration(
        configurationapi.WithOAuthCCGCredentials(
            configurationapi.NewOAuthCCGCredentials(
