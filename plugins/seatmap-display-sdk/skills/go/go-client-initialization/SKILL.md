...
- `seatmapdisplay` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/seatmap-display-go` — the Go import path for the root package; `github.com/context-plugins/seatmap-display-go/models` for request/response types.
...
```go
import "github.com/context-plugins/seatmap-display-go"

client := seatmapdisplay.NewClient(
    seatmapdisplay.CreateConfiguration(
        seatmapdisplay.WithEnvironment(seatmapdisplay.PRODUCTION),
...
from environment variables named `SEATMAPDISPLAY_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := seatmapdisplay.NewClient(seatmapdisplay.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `seatmapdisplay.PRODUCTION`,
`seatmapdisplay.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := seatmapdisplay.NewClient(
    seatmapdisplay.CreateConfiguration(seatmapdisplay.WithEnvironment(seatmapdisplay.PRODUCTION)),
)
```
...
client := seatmapdisplay.NewClient(
    seatmapdisplay.CreateConfiguration(
        seatmapdisplay.WithHttpConfiguration(
            seatmapdisplay.CreateHttpConfiguration(
                seatmapdisplay.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                seatmapdisplay.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = seatmapdisplay.NewClient(seatmapdisplay.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api seatmapdisplay.ClientInterface
}

func NewService(api seatmapdisplay.ClientInterface) *Service {
