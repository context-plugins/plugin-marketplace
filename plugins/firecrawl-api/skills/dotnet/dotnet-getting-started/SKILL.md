...
| --- | --- |
| API | `Firecrawl API` |
| Project | `FirecrawlApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using FirecrawlApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/firecrawl-api-csharp-sdk
dotnet add reference firecrawl-api-csharp-sdk/FirecrawlApi.csproj
```

```csharp
using FirecrawlApi;          // client + options + ServerEnvironment
using FirecrawlApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/firecrawl-api-csharp-sdk /tmp/firecrawl-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/firecrawl-api-csharp-sdk "$env:TEMP\firecrawl-api-dotnet-src"
```
...
- `FirecrawlApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/firecrawl-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\firecrawl-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `FirecrawlApiClient`, build its
   `FirecrawlApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
