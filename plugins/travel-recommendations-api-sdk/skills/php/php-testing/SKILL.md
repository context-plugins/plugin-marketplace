...
> `TravelRecommendationsAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): TravelRecommendationsAPIClient
{
...
```php
use TravelRecommendationsAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use TravelRecommendationsAPILib\Exceptions\{OperationException};
use TravelRecommendationsAPILib\Exceptions\ApiException;
...
app()->instance(TravelRecommendationsAPIClient::class, clientWithResponses([
