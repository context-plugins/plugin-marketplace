...
To see which schemes a specific SDK accepts, read the credential setter methods on `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder`
...
- Set credentials **on the `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.Builder`** before calling `.build()` — the client is immutable
