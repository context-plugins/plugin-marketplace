...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `XeroAccountingAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`xeroaccountingapi/configuration.py` in the cloned source.
...
```python
import requests
from xeroaccountingapi.configuration import Configuration
...
`xeroaccountingapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`xeroaccountingapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `xeroaccountingapi/configuration.py`
...
`NumberPagedResponse`) live in `xeroaccountingapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `xeroaccountingapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
