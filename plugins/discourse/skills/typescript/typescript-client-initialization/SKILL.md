...
- `discourse` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'discourse';
...
```ts
import { {Resource}Api } from 'discourse';
