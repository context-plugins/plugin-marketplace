---
name: dotnet-paze-getting-started
description: Paze .NET SDK identity and lookup layer for the dotnet-paze-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Paze .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-paze-sdk` helper
...
| --- | --- |
| API | Paze |
| Source repo | https://github.com/context-plugins/paze-csharp-sdk (branch `main`) |
| Root namespace | `PazeCheckoutApi` (the `using` namespace) |
| Client class | `PazeCheckoutApiClient` |
| Options class | `PazeCheckoutApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `PazeCheckoutApiClientOptions`: `Oauth2: OAuth2ClientCredentials?`, `Oauth2TokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using PazeCheckoutApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/paze-csharp-sdk
dotnet add reference paze-csharp-sdk/PazeCheckoutApi.csproj
```
...
temp directory** (`<temp>/paze-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/paze-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/paze-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/paze-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\paze-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/paze-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/paze-dotnet-src/`, **never** in the project repo, and its path never goes into
`paze-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `PazeCheckoutApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new PazeCheckoutApiClient(...)`, build its options, or DI-register via
   `AddPazeCheckoutApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `PazeCheckoutApiClientOptions` — the map's *Servers & auth* section
