...
kwargs to the `OpenAIAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `openaiapi/openaiapi_client.py` and `openaiapi/http/auth/` in
the cloned source.

## Basic auth

```python
from openaiapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from openaiapi.http.auth.api_key import ApiKeyCredentials
...
```python
from openaiapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from openaiapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from openaiapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from openaiapi.http.auth.o_auth_acg import OAuthACGCredentials
from openaiapi.models.o_auth_scope_openaiapi_enum import OAuthScope{Pkg}Enum
...
client = OpenAIAPIClient(o_auth_acg_credentials=creds)
...
```python
from openaiapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = OpenAIAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `OpenAIAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `openaiapi/configuration.py` and each `openaiapi/http/auth/*.py` for the exact
