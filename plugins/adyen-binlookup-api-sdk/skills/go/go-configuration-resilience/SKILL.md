    ...
    adyenbinlookupapi.CreateConfiguration(
        adyenbinlookupapi.WithHttpConfiguration(
            adyenbinlookupapi.CreateHttpConfiguration(
                adyenbinlookupapi.WithTimeout(30),
                adyenbinlookupapi.WithRetryConfiguration(
                    adyenbinlookupapi.CreateRetryConfiguration(
                        adyenbinlookupapi.WithMaxRetryAttempts(3),
                        adyenbinlookupapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := adyenbinlookupapi.CreateRetryConfiguration(
    adyenbinlookupapi.WithMaxRetryAttempts(5),
    adyenbinlookupapi.WithRetryInterval(1),
    adyenbinlookupapi.WithBackoffFactor(2),
    adyenbinlookupapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    adyenbinlookupapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := adyenbinlookupapi.NewClient(
    adyenbinlookupapi.CreateConfiguration(
        adyenbinlookupapi.WithHttpConfiguration(
            adyenbinlookupapi.CreateHttpConfiguration(
                adyenbinlookupapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
