...
> `{ApiGroup}`, `DeploymentAdminClient`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `DeploymentAdminClient.Core.Exceptions`
- `ApiError` **and** `RawError` → `DeploymentAdminClient.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `DeploymentAdminClient.Errors`
...
```csharp
using DeploymentAdminClient.Core.Exceptions;     // SdkException<TError>
using DeploymentAdminClient.Core.ErrorResponse;  // ApiError, RawError
using DeploymentAdminClient.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`DeploymentAdminClient.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using DeploymentAdminClient.Core.Exceptions;     // SdkException<TError>
using DeploymentAdminClient.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `DeploymentAdminClient.Core.ErrorResponse`. Its public
...
`readonly struct` under `DeploymentAdminClient.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using DeploymentAdminClient.Core.Models;        // ApiResult<TResponse, TError>
using DeploymentAdminClient.Core.ErrorResponse; // RawError
using DeploymentAdminClient.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `DeploymentAdminClient.Core.Exceptions`) — an auth *application* failure, not an API error — when the
