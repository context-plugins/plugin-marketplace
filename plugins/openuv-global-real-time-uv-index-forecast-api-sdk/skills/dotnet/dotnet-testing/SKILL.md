...
`OpenUvGlobalRealTimeUvIndexForecastApiClient`/`OpenUvGlobalRealTimeUvIndexForecastApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `OpenUvGlobalRealTimeUvIndexForecastApiClient`,
...
static OpenUvGlobalRealTimeUvIndexForecastApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new OpenUvGlobalRealTimeUvIndexForecastApiClient(new HttpClient(handler), new OpenUvGlobalRealTimeUvIndexForecastApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using OpenUvGlobalRealTimeUvIndexForecastApi.Errors;              // {Operation}Error types
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Exceptions;
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.ErrorResponse;
...
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new OpenUvGlobalRealTimeUvIndexForecastApiClient(new HttpClient(handler), new OpenUvGlobalRealTimeUvIndexForecastApiClientOptions());
...
- For DI-based code, the SDK's `AddOpenUvGlobalRealTimeUvIndexForecastApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `OpenUvGlobalRealTimeUvIndexForecastApiClient` from the provider:
  ```csharp
  services.AddOpenUvGlobalRealTimeUvIndexForecastApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<OpenUvGlobalRealTimeUvIndexForecastApiClient>();
  ```
