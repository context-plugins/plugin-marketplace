...
kwargs to the `PazeCheckoutAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `pazecheckoutapi/pazecheckoutapi_client.py` and `pazecheckoutapi/http/auth/` in
the cloned source.

## Basic auth

```python
from pazecheckoutapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from pazecheckoutapi.http.auth.api_key import ApiKeyCredentials
...
```python
from pazecheckoutapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from pazecheckoutapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from pazecheckoutapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from pazecheckoutapi.http.auth.o_auth_acg import OAuthACGCredentials
from pazecheckoutapi.models.o_auth_scope_pazecheckoutapi_enum import OAuthScope{Pkg}Enum
...
client = PazeCheckoutAPIClient(o_auth_acg_credentials=creds)
...
```python
from pazecheckoutapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = PazeCheckoutAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `PazeCheckoutAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `pazecheckoutapi/configuration.py` and each `pazecheckoutapi/http/auth/*.py` for the exact
