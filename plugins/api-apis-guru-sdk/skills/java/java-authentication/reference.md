...
against the `ApiApisGuruClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ApiApisGuruClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ApiApisGuruClient client = new ApiApisGuruClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeApiApisGuruEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
