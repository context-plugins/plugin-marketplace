...
│   └── local.discourse.exceptions.ApiException          ← base for all non-2xx API responses
│       └── local.discourse.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `local.discourse.http.client.HttpContext`, which carries:
