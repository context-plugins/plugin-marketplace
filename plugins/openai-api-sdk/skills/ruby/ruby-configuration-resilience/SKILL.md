...
```ruby
client = OpenAIAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = OpenAIAPI::Client.new(adapter: :net_http)
```
...
client = OpenAIAPI::Client.new(connection: conn)
```
...
client = OpenAIAPI::Client.new(environment: Environment::PRODUCTION)
client = OpenAIAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = OpenAIAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = OpenAIAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = OpenAIAPI::ProxySettings.from_env
client = OpenAIAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < OpenAIAPI::HttpCallBack
...
client = OpenAIAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = OpenAIAPI::Client.from_env
# Overrides take precedence over env vars:
client = OpenAIAPI::Client.from_env(timeout: 30, max_retries: 3)
```
