...
To see which schemes a specific SDK accepts, read the credential setter methods on `VimeoClient.Builder`
...
- Set credentials **on the `VimeoClient.Builder`** before calling `.build()` — the client is immutable
