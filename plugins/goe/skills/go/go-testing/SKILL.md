...
`testing` + `testify/require` for reference; substitute the real `goe` and names.
...
import (
    ...
    "github.com/context-plugins/goe-go-sdk"   // root package goe
)
...
func clientReturning(status int, body string) (goe.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := goe.NewClient(
        goe.CreateConfiguration(
            goe.WithHttpConfiguration(
                goe.CreateHttpConfiguration(goe.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/goe-go-sdk/errors"
...
client := goe.NewClient(
    goe.CreateConfiguration(
        goe.WithHttpConfiguration(
            goe.CreateHttpConfiguration(goe.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
