...
- `deploymentadminclient` — the root package name (e.g. `multiauthsample`, `batester`).
- `DeploymentAdminClientClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `DeploymentAdminClientClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from deploymentadminclient.deploymentadminclient_client import DeploymentAdminClientClient
from deploymentadminclient.configuration import Environment
from deploymentadminclient.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = DeploymentAdminClientClient(
...
```python
from deploymentadminclient.configuration import Configuration, Environment
...
client = DeploymentAdminClientClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `deploymentadminclient/deploymentadminclient_client.py` in the cloned source.
...
`from_environment()` is a class method on both `DeploymentAdminClientClient` and `Configuration` that reads
...
client = DeploymentAdminClientClient.from_environment()
...
`from_environment` in `deploymentadminclient/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `DeploymentAdminClientClient`. Access them as attributes —
...
Open `deploymentadminclient/deploymentadminclient_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `deploymentadminclient/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from deploymentadminclient.configuration import Environment

client = DeploymentAdminClientClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`deploymentadminclient/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = DeploymentAdminClientClient(config=new_config)
```
