...
> `ConfigurationAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ConfigurationAPIClient
{
...
```php
use ConfigurationAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use ConfigurationAPILib\Exceptions\{OperationException};
use ConfigurationAPILib\Exceptions\ApiException;
...
app()->instance(ConfigurationAPIClient::class, clientWithResponses([
