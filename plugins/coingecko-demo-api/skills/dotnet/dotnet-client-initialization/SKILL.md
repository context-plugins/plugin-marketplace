...
- `CoinGeckoDemoApiClientOptions` — its options class.
- `CoinGeckoDemoApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddCoinGeckoDemoApiClient`, which registers the
...
builder.Services.AddCoinGeckoDemoApiClient(options =>
{
...
```csharp
public sealed class MyService(CoinGeckoDemoApiClient client)
{
