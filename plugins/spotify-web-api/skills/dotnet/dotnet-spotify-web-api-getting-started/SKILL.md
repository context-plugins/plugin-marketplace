...
| --- | --- |
| API | Spotify Web API |
| Source repo | https://github.com/context-plugins/spotify-web-api-csharp-sdk (branch `main`) |
| Root namespace | `SpotifyWebApi` (the `using` namespace) |
| Client class | `SpotifyWebApiClient` |
| Options class | `SpotifyWebApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `SpotifyWebApiClientOptions`: `Oauth20: OAuth2AuthorizationCodeCredentials?`, `Oauth20TokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using SpotifyWebApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/spotify-web-api-csharp-sdk
dotnet add reference spotify-web-api-csharp-sdk/SpotifyWebApi.csproj
```
...
temp directory** (`<temp>/spotify-web-api-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/spotify-web-api-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/spotify-web-api-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/spotify-web-api-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\spotify-web-api-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/spotify-web-api-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/spotify-web-api-dotnet-src/`, **never** in the project repo, and its path never goes into
`spotify-web-api-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new SpotifyWebApiClient(...)`, build its options, or DI-register via
   `AddSpotifyWebApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `SpotifyWebApiClientOptions` — the map's *Servers & auth* section
