...
`mediumapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `mediumapi/exceptions/` and operation docstrings.
...
```python
from mediumapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `mediumapi/exceptions/`.
...
```python
from mediumapi.exceptions.api_exception import APIException
from mediumapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`mediumapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`mediumapi/exceptions/` for the class.
...
`mediumapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `mediumapi/exceptions/`
  or a `ValueError` — check the auth handler in `mediumapi/http/auth/` for the exact type.
