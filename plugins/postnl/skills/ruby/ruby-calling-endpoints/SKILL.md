...
Open `lib/postnl/client.rb` for the accessor names, then the relevant
`lib/postnl/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = Postnl::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: Postnl::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/postnl/models/`. Field names are defined as
...
Read the enum module in `lib/postnl/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/postnl/client.rb`.
- Operation methods are in `lib/postnl/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/postnl/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/postnl/exceptions/`.
