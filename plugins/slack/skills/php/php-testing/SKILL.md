...
> `SlackWebApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SlackWebApiClient
{
...
```php
use SlackWebApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SlackWebApiLib\Exceptions\{OperationException};
use SlackWebApiLib\Exceptions\ApiException;
...
app()->instance(SlackWebApiClient::class, clientWithResponses([
