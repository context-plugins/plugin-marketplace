    ...
    tesla.CreateConfiguration(
        tesla.WithHttpConfiguration(
            tesla.CreateHttpConfiguration(
                tesla.WithTimeout(30),
                tesla.WithRetryConfiguration(
                    tesla.CreateRetryConfiguration(
                        tesla.WithMaxRetryAttempts(3),
                        tesla.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := tesla.CreateRetryConfiguration(
    tesla.WithMaxRetryAttempts(5),
    tesla.WithRetryInterval(1),
    tesla.WithBackoffFactor(2),
    tesla.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    tesla.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := tesla.NewClient(
    tesla.CreateConfiguration(
        tesla.WithHttpConfiguration(
            tesla.CreateHttpConfiguration(
                tesla.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
