...
All types below live under `ShutterstockApiExplorer.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using ShutterstockApiExplorer.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new ShutterstockApiExplorerClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `ShutterstockApiExplorer.Core.Exceptions`):
...
With DI, the SDK's `AddShutterstockApiExplorerClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddShutterstockApiExplorerClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
