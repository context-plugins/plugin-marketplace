...
- `shutterstock` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/shutterstock-go-sdk` — the Go import path for the root package; `github.com/context-plugins/shutterstock-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/shutterstock-go-sdk"

client := shutterstock.NewClient(
    shutterstock.CreateConfiguration(
        shutterstock.WithEnvironment(shutterstock.PRODUCTION),
...
from environment variables named `SHUTTERSTOCK_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := shutterstock.NewClient(shutterstock.CreateConfigurationFromEnvironment())
```
...
```go
client := shutterstock.NewClient(
    shutterstock.CreateConfiguration(shutterstock.WithEnvironment(shutterstock.PRODUCTION)),
)
```
...
client := shutterstock.NewClient(
    shutterstock.CreateConfiguration(
        shutterstock.WithHttpConfiguration(
            shutterstock.CreateHttpConfiguration(
                shutterstock.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                shutterstock.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = shutterstock.NewClient(shutterstock.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api shutterstock.ClientInterface
}

func NewService(api shutterstock.ClientInterface) *Service {
