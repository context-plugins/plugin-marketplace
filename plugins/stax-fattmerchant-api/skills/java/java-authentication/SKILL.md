...
To see which schemes a specific SDK accepts, read the credential setter methods on `StaxFattMerchantApiClient.Builder`
...
- Set credentials **on the `StaxFattMerchantApiClient.Builder`** before calling `.build()` — the client is immutable
