...
> Throughout, `geoapify` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := geoapify.NewClient(
    geoapify.CreateConfiguration(
        geoapify.WithBasicAuthCredentials(
            geoapify.NewBasicAuthCredentials(
...
geoapify.WithApiKeyCredentials(geoapify.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
geoapify.WithApiHeaderCredentials(geoapify.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
geoapify.WithOAuthBearerTokenCredentials(
    geoapify.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := geoapify.NewClient(
    geoapify.CreateConfiguration(
        geoapify.WithOAuthCCGCredentials(
            geoapify.NewOAuthCCGCredentials(
