...
> Throughout, `kubernetes` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := kubernetes.NewClient(
    kubernetes.CreateConfiguration(
        kubernetes.WithBasicAuthCredentials(
            kubernetes.NewBasicAuthCredentials(
...
kubernetes.WithApiKeyCredentials(kubernetes.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
kubernetes.WithApiHeaderCredentials(kubernetes.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
kubernetes.WithOAuthBearerTokenCredentials(
    kubernetes.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := kubernetes.NewClient(
    kubernetes.CreateConfiguration(
        kubernetes.WithOAuthCCGCredentials(
            kubernetes.NewOAuthCCGCredentials(
