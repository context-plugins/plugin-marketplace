...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the 1PasswordConnectClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write 1PasswordConnectClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `1passwordconnect` — the root package name (e.g. `multiauthsample`, `batester`).
- `1PasswordConnectClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `1PasswordConnectClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from 1passwordconnect.1passwordconnect_client import 1PasswordConnectClient
from 1passwordconnect.configuration import Environment
from 1passwordconnect.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = 1PasswordConnectClient(
...
```python
from 1passwordconnect.configuration import Configuration, Environment
...
client = 1PasswordConnectClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `1passwordconnect/1passwordconnect_client.py` in the cloned source.
...
`from_environment()` is a class method on both `1PasswordConnectClient` and `Configuration` that reads
...
client = 1PasswordConnectClient.from_environment()
...
`from_environment` in `1passwordconnect/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `1PasswordConnectClient`. Access them as attributes —
...
Open `1passwordconnect/1passwordconnect_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `1passwordconnect/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from 1passwordconnect.configuration import Environment

client = 1PasswordConnectClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`1passwordconnect/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = 1PasswordConnectClient(config=new_config)
```
