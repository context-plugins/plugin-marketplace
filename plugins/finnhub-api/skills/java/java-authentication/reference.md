...
against the `FinnhubApiClient.Builder` setters, the `io.finnhub.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FinnhubApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FinnhubApiClient client = new FinnhubApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFinnhubAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
