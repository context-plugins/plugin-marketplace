...
client = stub_client(stubs)
assert_raises VimeoAPI::APIException do
...
BASE = VimeoAPI::Configuration.new(environment: VimeoAPI::Environment::PRODUCTION).get_base_uri

def build_test_client
  VimeoAPI::Client.new(
    environment: VimeoAPI::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error VimeoAPI::APIException
```
...
```ruby
expect { call }.to raise_error(VimeoAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < VimeoAPI::HttpCallBack
...
client  = VimeoAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
