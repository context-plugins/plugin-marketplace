...
```ruby
client = Coingecko::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Coingecko::Client.new(adapter: :net_http)
```
...
client = Coingecko::Client.new(connection: conn)
```
...
client = Coingecko::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Coingecko::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Coingecko::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Coingecko::ProxySettings.from_env
client = Coingecko::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Coingecko::HttpCallBack
...
client = Coingecko::Client.new(http_callback: LogCallback.new)
```
...
client = Coingecko::Client.from_env(timeout: 30, max_retries: 3)
```
