...
`lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/http/auth/api_header.rb` and
