...
│   └── com.chargify.subdomain.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.chargify.subdomain.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.chargify.subdomain.http.client.HttpContext`, which carries:
