...
> `{ApiGroup}`, `CoinGeckoDemoApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `CoinGeckoDemoApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `CoinGeckoDemoApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `CoinGeckoDemoApi.Errors`
...
```csharp
using CoinGeckoDemoApi.Core.Exceptions;     // SdkException<TError>
using CoinGeckoDemoApi.Core.ErrorResponse;  // ApiError, RawError
using CoinGeckoDemoApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`CoinGeckoDemoApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using CoinGeckoDemoApi.Core.Exceptions;     // SdkException<TError>
using CoinGeckoDemoApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `CoinGeckoDemoApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `CoinGeckoDemoApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using CoinGeckoDemoApi.Core.Models;        // ApiResult<TResponse, TError>
using CoinGeckoDemoApi.Core.ErrorResponse; // RawError
using CoinGeckoDemoApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `CoinGeckoDemoApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
