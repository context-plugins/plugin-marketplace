...
> `{ApiGroup}`, `Vimeo`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `Vimeo.Core.Exceptions`
- `ApiError` **and** `RawError` → `Vimeo.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `Vimeo.Errors`
...
```csharp
using Vimeo.Core.Exceptions;     // SdkException<TError>
using Vimeo.Core.ErrorResponse;  // ApiError, RawError
using Vimeo.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`Vimeo.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using Vimeo.Core.Exceptions;     // SdkException<TError>
using Vimeo.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `Vimeo.Core.ErrorResponse`. Its public
...
`readonly struct` under `Vimeo.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using Vimeo.Core.Models;        // ApiResult<TResponse, TError>
using Vimeo.Core.ErrorResponse; // RawError
using Vimeo.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `Vimeo.Core.Exceptions`) — an auth *application* failure, not an API error — when the
