...
against the `EventsAPIClient.Builder` setters, the `com.m1password.events.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `EventsAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
EventsAPIClient client = new EventsAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeEventsAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
