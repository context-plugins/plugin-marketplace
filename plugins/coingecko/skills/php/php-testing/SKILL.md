...
> `CoinGeckoDemoApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): CoinGeckoDemoApiClient
{
...
```php
use CoinGeckoDemoApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use CoinGeckoDemoApiLib\Exceptions\{OperationException};
use CoinGeckoDemoApiLib\Exceptions\ApiException;
...
app()->instance(CoinGeckoDemoApiClient::class, clientWithResponses([
