...
To see which schemes a specific SDK accepts, read the credential setter methods on `AWSMigrationHubClient.Builder`
...
- Set credentials **on the `AWSMigrationHubClient.Builder`** before calling `.build()` — the client is immutable
