...
To see which schemes a specific SDK accepts, read the credential setter methods on `AdafruitIORESTAPIClient.Builder`
...
- Set credentials **on the `AdafruitIORESTAPIClient.Builder`** before calling `.build()` — the client is immutable
