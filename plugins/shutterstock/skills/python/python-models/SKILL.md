...
your SDK source (`shutterstock/models/`).
...
```python
from shutterstock.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `shutterstock/utilities/union_type_lookup.py`) and each candidate model
...
```python
from shutterstock.models.animal import Cat, Dog
...
```python
from shutterstock.models.animal import Animal, Cat, Dog
