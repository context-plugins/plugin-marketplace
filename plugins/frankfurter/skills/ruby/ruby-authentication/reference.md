...
Confirm every name against `lib/frankfurter/client.rb`, `lib/frankfurter/configuration.rb`, the
`lib/frankfurter/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
