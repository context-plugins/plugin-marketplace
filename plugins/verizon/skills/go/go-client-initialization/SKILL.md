...
- `verizon` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/verizon-go-sdk` — the Go import path for the root package; `github.com/context-plugins/verizon-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/verizon-go-sdk"
...
from environment variables named `VERIZON_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := verizon.NewClient(verizon.CreateConfigurationFromEnvironment())
```
...
```go
client := verizon.NewClient(
    verizon.CreateConfiguration(verizon.WithEnvironment(verizon.PRODUCTION)),
)
```
    ...
    verizon.CreateConfiguration(
        verizon.WithHttpConfiguration(
            verizon.CreateHttpConfiguration(
                verizon.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                verizon.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = verizon.NewClient(verizon.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api verizon.ClientInterface
}

func NewService(api verizon.ClientInterface) *Service {
