...
against the `HotelRatingsClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `HotelRatingsClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
HotelRatingsClient client = new HotelRatingsClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeHotelRatingsEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new HotelRatingsClient.Builder()
