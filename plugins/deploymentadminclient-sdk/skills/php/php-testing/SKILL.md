...
> `DeploymentAdminClientClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): DeploymentAdminClientClient
{
...
```php
use DeploymentAdminClientLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use DeploymentAdminClientLib\Exceptions\{OperationException};
use DeploymentAdminClientLib\Exceptions\ApiException;
...
app()->instance(DeploymentAdminClientClient::class, clientWithResponses([
