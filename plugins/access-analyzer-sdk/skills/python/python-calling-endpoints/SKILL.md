...
Open `accessanalyzer/accessanalyzer_client.py` for the controller property names, then the relevant
`accessanalyzer/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from accessanalyzer.models.{request_model} import {RequestModel}
...
Open `accessanalyzer/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from accessanalyzer.models.suite_code_enum import SuiteCodeEnum
...
```python
from accessanalyzer.http.http_call_back import HttpCallBack
...
client = AccessAnalyzerClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `accessanalyzer/pagination/` in the source.

## Finding the right method

- Controller property names are on `AccessAnalyzerClient` in `accessanalyzer/accessanalyzer_client.py`.
- Operation method signatures are in `accessanalyzer/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `accessanalyzer/models/`; enum types have `from_value()`.
