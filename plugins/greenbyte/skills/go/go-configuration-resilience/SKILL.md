    ...
    greenbyte.CreateConfiguration(
        greenbyte.WithHttpConfiguration(
            greenbyte.CreateHttpConfiguration(
                greenbyte.WithTimeout(30),
                greenbyte.WithRetryConfiguration(
                    greenbyte.CreateRetryConfiguration(
                        greenbyte.WithMaxRetryAttempts(3),
                        greenbyte.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := greenbyte.CreateRetryConfiguration(
    greenbyte.WithMaxRetryAttempts(5),
    greenbyte.WithRetryInterval(1),
    greenbyte.WithBackoffFactor(2),
    greenbyte.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    greenbyte.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := greenbyte.NewClient(
    greenbyte.CreateConfiguration(
        greenbyte.WithHttpConfiguration(
            greenbyte.CreateHttpConfiguration(
                greenbyte.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
