...
| --- | --- |
| API | `Authentiq API` |
| Project | `AuthentiqApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `AuthentiqApi` (the `using` namespace) |
| Client class | `AuthentiqApiClient` |
| Options class | `AuthentiqApiClientOptions` |
...
namespaces transitively, so `using AuthentiqApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/authentiq-api-csharp
dotnet add reference authentiq-api-csharp/AuthentiqApi.csproj
```

```csharp
using AuthentiqApi;          // client + options + ServerEnvironment
using AuthentiqApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/authentiq-api-csharp /tmp/authentiq-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/authentiq-api-csharp "$env:TEMP\authentiq-api-dotnet-src"
```
...
- `AuthentiqApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/authentiq-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\authentiq-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AuthentiqApiClient`, build its
   `AuthentiqApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
