...
```go
sportsdata.WithOAuthCCGCredentials(
    sportsdata.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr sportsdata.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := sportsdata.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr sportsdata.OAuthCCGManager) models.OAuthToken {
...
```go
creds := sportsdata.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := sportsdata.NewClient(sportsdata.CreateConfiguration(sportsdata.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    sportsdata.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := sportsdata.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := sportsdata.NewClient(sportsdata.CreateConfiguration(sportsdata.WithOAuthROPCGCredentials(creds)))
...
```go
sportsdata.WithOAuthBearerTokenCredentials(sportsdata.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
sportsdata.WithBasicAuthCredentials(sportsdata.NewBasicAuthCredentials(username, password))
sportsdata.WithApiHeaderCredentials(sportsdata.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
sportsdata.WithApiKeyCredentials(sportsdata.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SPORTSDATA_...` variables — e.g.
`SPORTSDATA_USERNAME`, `SPORTSDATA_PASSWORD`, `SPORTSDATA_TOKEN`, `SPORTSDATA_API_KEY`,
`SPORTSDATA_O_AUTH_CLIENT_ID`, `SPORTSDATA_O_AUTH_CLIENT_SECRET`, `SPORTSDATA_ACCESS_TOKEN`. Grep the
