...
```go
openmeteoweatherforecastapi.WithOAuthCCGCredentials(
    openmeteoweatherforecastapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr openmeteoweatherforecastapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := openmeteoweatherforecastapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr openmeteoweatherforecastapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := openmeteoweatherforecastapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := openmeteoweatherforecastapi.NewClient(openmeteoweatherforecastapi.CreateConfiguration(openmeteoweatherforecastapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    openmeteoweatherforecastapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := openmeteoweatherforecastapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := openmeteoweatherforecastapi.NewClient(openmeteoweatherforecastapi.CreateConfiguration(openmeteoweatherforecastapi.WithOAuthROPCGCredentials(creds)))
...
```go
openmeteoweatherforecastapi.WithOAuthBearerTokenCredentials(openmeteoweatherforecastapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
openmeteoweatherforecastapi.WithBasicAuthCredentials(openmeteoweatherforecastapi.NewBasicAuthCredentials(username, password))
openmeteoweatherforecastapi.WithApiHeaderCredentials(openmeteoweatherforecastapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
openmeteoweatherforecastapi.WithApiKeyCredentials(openmeteoweatherforecastapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `OPENMETEOWEATHERFORECASTAPI_...` variables — e.g.
`OPENMETEOWEATHERFORECASTAPI_USERNAME`, `OPENMETEOWEATHERFORECASTAPI_PASSWORD`, `OPENMETEOWEATHERFORECASTAPI_TOKEN`, `OPENMETEOWEATHERFORECASTAPI_API_KEY`,
`OPENMETEOWEATHERFORECASTAPI_O_AUTH_CLIENT_ID`, `OPENMETEOWEATHERFORECASTAPI_O_AUTH_CLIENT_SECRET`, `OPENMETEOWEATHERFORECASTAPI_ACCESS_TOKEN`. Grep the
