...
```go
shutterstockapiexplorer.WithOAuthCCGCredentials(
    shutterstockapiexplorer.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr shutterstockapiexplorer.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := shutterstockapiexplorer.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr shutterstockapiexplorer.OAuthCCGManager) models.OAuthToken {
...
```go
creds := shutterstockapiexplorer.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := shutterstockapiexplorer.NewClient(shutterstockapiexplorer.CreateConfiguration(shutterstockapiexplorer.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    shutterstockapiexplorer.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := shutterstockapiexplorer.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := shutterstockapiexplorer.NewClient(shutterstockapiexplorer.CreateConfiguration(shutterstockapiexplorer.WithOAuthROPCGCredentials(creds)))
...
```go
shutterstockapiexplorer.WithOAuthBearerTokenCredentials(shutterstockapiexplorer.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
shutterstockapiexplorer.WithBasicAuthCredentials(shutterstockapiexplorer.NewBasicAuthCredentials(username, password))
shutterstockapiexplorer.WithApiHeaderCredentials(shutterstockapiexplorer.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
shutterstockapiexplorer.WithApiKeyCredentials(shutterstockapiexplorer.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SHUTTERSTOCKAPIEXPLORER_...` variables — e.g.
`SHUTTERSTOCKAPIEXPLORER_USERNAME`, `SHUTTERSTOCKAPIEXPLORER_PASSWORD`, `SHUTTERSTOCKAPIEXPLORER_TOKEN`, `SHUTTERSTOCKAPIEXPLORER_API_KEY`,
`SHUTTERSTOCKAPIEXPLORER_O_AUTH_CLIENT_ID`, `SHUTTERSTOCKAPIEXPLORER_O_AUTH_CLIENT_SECRET`, `SHUTTERSTOCKAPIEXPLORER_ACCESS_TOKEN`. Grep the
