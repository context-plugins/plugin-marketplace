...
```typescript
import { {Operation}Error } from 'configuration-apilib/errors';
...
```typescript
import { ApiError } from 'configuration-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(ConfigurationAPIClient)
    .useValue(new ConfigurationAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
