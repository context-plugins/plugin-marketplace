...
> `OpenUVGlobalRealTimeUVIndexForecastAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): OpenUVGlobalRealTimeUVIndexForecastAPIClient
{
...
```php
use OpenUVGlobalRealTimeUVIndexForecastAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use OpenUVGlobalRealTimeUVIndexForecastAPILib\Exceptions\{OperationException};
use OpenUVGlobalRealTimeUVIndexForecastAPILib\Exceptions\ApiException;
...
app()->instance(OpenUVGlobalRealTimeUVIndexForecastAPIClient::class, clientWithResponses([
