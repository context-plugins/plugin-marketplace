...
| --- | --- |
| API | verizon |
| Source repo | https://github.com/context-plugins/verizon-csharp-sdk (branch `main`) |
| Root namespace | `Verizon` (the `using` namespace) |
| Client class | `VerizonClient` |
| Options class | `VerizonClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `VerizonClientOptions`: `ThingspaceOauth: OAuth2ClientCredentials?`, `ThingspaceOauthTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?`, `VzM2MToken: string?`, `SessionToken: string?`, `ThingspaceOauth1: OAuth2ClientCredentials?`, `ThingspaceOauth1TokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `Production`, `Staging`, `Dev`, `Qa`, `MockServerForLimitedAvailabilitySeeQuickStart` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using Verizon.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/verizon-csharp-sdk
dotnet add reference verizon-csharp-sdk/Verizon.csproj
```
...
temp directory** (`<temp>/verizon-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/verizon-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/verizon-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/verizon-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\verizon-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/verizon-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/verizon-dotnet-src/`, **never** in the project repo, and its path never goes into
`verizon-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new VerizonClient(...)`, build its options, or DI-register via
   `AddVerizonClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `VerizonClientOptions` — the map's *Servers & auth* section
