...
Open `paypalapi/paypalapi_client.py` for the controller property names, then the relevant
`paypalapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from paypalapi.models.{request_model} import {RequestModel}
...
Open `paypalapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from paypalapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from paypalapi.http.http_call_back import HttpCallBack
...
client = PayPalAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `paypalapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `PayPalAPIClient` in `paypalapi/paypalapi_client.py`.
- Operation method signatures are in `paypalapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `paypalapi/models/`; enum types have `from_value()`.
