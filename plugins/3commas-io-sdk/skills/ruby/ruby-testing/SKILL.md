...
client = stub_client(stubs)
assert_raises 3commasIo::APIException do
...
def build_test_client
  3commasIo::Client.new(
    environment: 3commasIo::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error 3commasIo::APIException
```
...
```ruby
expect { call }.to raise_error(3commasIo::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < 3commasIo::HttpCallBack
...
client  = 3commasIo::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
