...
> Throughout, `xero` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := xero.NewClient(
    xero.CreateConfiguration(
        xero.WithBasicAuthCredentials(
            xero.NewBasicAuthCredentials(
...
xero.WithApiKeyCredentials(xero.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
xero.WithApiHeaderCredentials(xero.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
xero.WithOAuthBearerTokenCredentials(
    xero.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
