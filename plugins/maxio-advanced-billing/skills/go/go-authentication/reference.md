...
```go
maxioadvancedbilling.WithOAuthCCGCredentials(
    maxioadvancedbilling.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr maxioadvancedbilling.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := maxioadvancedbilling.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr maxioadvancedbilling.OAuthCCGManager) models.OAuthToken {
...
```go
creds := maxioadvancedbilling.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := maxioadvancedbilling.NewClient(maxioadvancedbilling.CreateConfiguration(maxioadvancedbilling.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    maxioadvancedbilling.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := maxioadvancedbilling.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := maxioadvancedbilling.NewClient(maxioadvancedbilling.CreateConfiguration(maxioadvancedbilling.WithOAuthROPCGCredentials(creds)))
...
```go
maxioadvancedbilling.WithOAuthBearerTokenCredentials(maxioadvancedbilling.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
maxioadvancedbilling.WithBasicAuthCredentials(maxioadvancedbilling.NewBasicAuthCredentials(username, password))
maxioadvancedbilling.WithApiHeaderCredentials(maxioadvancedbilling.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
maxioadvancedbilling.WithApiKeyCredentials(maxioadvancedbilling.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `MAXIOADVANCEDBILLING_...` variables — e.g.
`MAXIOADVANCEDBILLING_USERNAME`, `MAXIOADVANCEDBILLING_PASSWORD`, `MAXIOADVANCEDBILLING_TOKEN`, `MAXIOADVANCEDBILLING_API_KEY`,
`MAXIOADVANCEDBILLING_O_AUTH_CLIENT_ID`, `MAXIOADVANCEDBILLING_O_AUTH_CLIENT_SECRET`, `MAXIOADVANCEDBILLING_ACCESS_TOKEN`. Grep the
