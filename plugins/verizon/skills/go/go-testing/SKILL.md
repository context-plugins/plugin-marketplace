...
`testing` + `testify/require` for reference; substitute the real `verizon` and names.
...
import (
    ...
    "github.com/context-plugins/verizon-go-sdk"   // root package verizon
)
...
func clientReturning(status int, body string) (verizon.ClientInterface, *stubTransport) {
    ...
    client := verizon.NewClient(
        verizon.CreateConfiguration(
            verizon.WithHttpConfiguration(
                verizon.CreateHttpConfiguration(verizon.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/verizon-go-sdk/errors"
...
client := verizon.NewClient(
    verizon.CreateConfiguration(
        verizon.WithHttpConfiguration(
            verizon.CreateHttpConfiguration(verizon.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
