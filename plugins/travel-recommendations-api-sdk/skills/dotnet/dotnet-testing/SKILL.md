...
`TravelRecommendationsApiClient`/`TravelRecommendationsApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `TravelRecommendationsApiClient`,
...
static TravelRecommendationsApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new TravelRecommendationsApiClient(new HttpClient(handler), new TravelRecommendationsApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using TravelRecommendationsApi.Errors;              // {Operation}Error types
...
```csharp
using TravelRecommendationsApi.Core.Exceptions;
using TravelRecommendationsApi.Core.ErrorResponse;
...
using TravelRecommendationsApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new TravelRecommendationsApiClient(new HttpClient(handler), new TravelRecommendationsApiClientOptions());
...
- For DI-based code, the SDK's `AddTravelRecommendationsApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `TravelRecommendationsApiClient` from the provider:
  ```csharp
  services.AddTravelRecommendationsApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<TravelRecommendationsApiClient>();
  ```
