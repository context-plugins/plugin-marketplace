...
`sportsdata/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `sportsdata/exceptions/` and operation docstrings.
...
```python
from sportsdata.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `sportsdata/exceptions/`.
...
```python
from sportsdata.exceptions.api_exception import APIException
from sportsdata.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`sportsdata/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`sportsdata/exceptions/` for the class.
...
`sportsdata/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `sportsdata/exceptions/`
  or a `ValueError` — check the auth handler in `sportsdata/http/auth/` for the exact type.
