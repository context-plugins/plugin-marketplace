...
Open `lib/nytimes/client.rb` for the accessor names, then the relevant
`lib/nytimes/controllers/{resource}_controller.rb` for the operation's exact signature.
...
Model classes extend `BaseModel` and are in `lib/nytimes/models/`. Field names are defined as
...
Read the enum module in `lib/nytimes/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/nytimes/client.rb`.
- Operation methods are in `lib/nytimes/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/nytimes/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/nytimes/exceptions/`.
