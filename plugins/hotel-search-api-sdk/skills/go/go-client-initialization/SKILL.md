...
- `hotelsearchapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/hotel-search-api-go` — the Go import path for the root package; `github.com/context-plugins/hotel-search-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/hotel-search-api-go"

client := hotelsearchapi.NewClient(
    hotelsearchapi.CreateConfiguration(
        hotelsearchapi.WithEnvironment(hotelsearchapi.PRODUCTION),
...
from environment variables named `HOTELSEARCHAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := hotelsearchapi.NewClient(hotelsearchapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `hotelsearchapi.PRODUCTION`,
`hotelsearchapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := hotelsearchapi.NewClient(
    hotelsearchapi.CreateConfiguration(hotelsearchapi.WithEnvironment(hotelsearchapi.PRODUCTION)),
)
```
...
client := hotelsearchapi.NewClient(
    hotelsearchapi.CreateConfiguration(
        hotelsearchapi.WithHttpConfiguration(
            hotelsearchapi.CreateHttpConfiguration(
                hotelsearchapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                hotelsearchapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = hotelsearchapi.NewClient(hotelsearchapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api hotelsearchapi.ClientInterface
}

func NewService(api hotelsearchapi.ClientInterface) *Service {
