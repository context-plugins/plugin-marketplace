...
```go
squareconnectapi.WithOAuthCCGCredentials(
    squareconnectapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr squareconnectapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := squareconnectapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr squareconnectapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := squareconnectapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := squareconnectapi.NewClient(squareconnectapi.CreateConfiguration(squareconnectapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    squareconnectapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := squareconnectapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := squareconnectapi.NewClient(squareconnectapi.CreateConfiguration(squareconnectapi.WithOAuthROPCGCredentials(creds)))
...
```go
squareconnectapi.WithOAuthBearerTokenCredentials(squareconnectapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
squareconnectapi.WithBasicAuthCredentials(squareconnectapi.NewBasicAuthCredentials(username, password))
squareconnectapi.WithApiHeaderCredentials(squareconnectapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
squareconnectapi.WithApiKeyCredentials(squareconnectapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SQUARECONNECTAPI_...` variables — e.g.
`SQUARECONNECTAPI_USERNAME`, `SQUARECONNECTAPI_PASSWORD`, `SQUARECONNECTAPI_TOKEN`, `SQUARECONNECTAPI_API_KEY`,
`SQUARECONNECTAPI_O_AUTH_CLIENT_ID`, `SQUARECONNECTAPI_O_AUTH_CLIENT_SECRET`, `SQUARECONNECTAPI_ACCESS_TOKEN`. Grep the
