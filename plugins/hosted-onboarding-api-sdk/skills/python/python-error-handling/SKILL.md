...
`hostedonboardingapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `hostedonboardingapi/exceptions/` and operation docstrings.
...
```python
from hostedonboardingapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `hostedonboardingapi/exceptions/`.
...
```python
from hostedonboardingapi.exceptions.api_exception import APIException
from hostedonboardingapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`hostedonboardingapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`hostedonboardingapi/exceptions/` for the class.
...
`hostedonboardingapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `hostedonboardingapi/exceptions/`
  or a `ValueError` — check the auth handler in `hostedonboardingapi/http/auth/` for the exact type.
