...
`testing` + `testify/require` for reference; substitute the real `postnl` and names.
...
import (
    ...
    "github.com/context-plugins/postnl-go-sdk"   // root package postnl
)
...
func clientReturning(status int, body string) (postnl.ClientInterface, *stubTransport) {
    ...
    client := postnl.NewClient(
        postnl.CreateConfiguration(
            postnl.WithHttpConfiguration(
                postnl.CreateHttpConfiguration(postnl.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/postnl-go-sdk/errors"
...
client := postnl.NewClient(
    postnl.CreateConfiguration(
        postnl.WithHttpConfiguration(
            postnl.CreateHttpConfiguration(postnl.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
