...
`TeslaApiClient`/`TeslaApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `TeslaApiClient`,
...
static TeslaApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new TeslaApiClient(new HttpClient(handler), new TeslaApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using TeslaApi.Errors;              // {Operation}Error types
...
```csharp
using TeslaApi.Core.Exceptions;
using TeslaApi.Core.ErrorResponse;
...
using TeslaApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new TeslaApiClient(new HttpClient(handler), new TeslaApiClientOptions());
...
- For DI-based code, the SDK's `AddTeslaApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `TeslaApiClient` from the provider:
  ```csharp
  services.AddTeslaApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<TeslaApiClient>();
  ```
