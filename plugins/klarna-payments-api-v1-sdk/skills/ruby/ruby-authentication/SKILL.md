...
`lib/klarna_payments_api_v1/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/klarna_payments_api_v1/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/klarna_payments_api_v1/http/auth/api_header.rb` and
