...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `tesla`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'tesla';
import { ApiError } from 'tesla';
import { {Operation}Error } from 'tesla/errors';
