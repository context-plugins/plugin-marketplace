...
```go
flightpriceanalysisapi.WithOAuthCCGCredentials(
    flightpriceanalysisapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr flightpriceanalysisapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := flightpriceanalysisapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr flightpriceanalysisapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := flightpriceanalysisapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := flightpriceanalysisapi.NewClient(flightpriceanalysisapi.CreateConfiguration(flightpriceanalysisapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    flightpriceanalysisapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := flightpriceanalysisapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := flightpriceanalysisapi.NewClient(flightpriceanalysisapi.CreateConfiguration(flightpriceanalysisapi.WithOAuthROPCGCredentials(creds)))
...
```go
flightpriceanalysisapi.WithOAuthBearerTokenCredentials(flightpriceanalysisapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
flightpriceanalysisapi.WithBasicAuthCredentials(flightpriceanalysisapi.NewBasicAuthCredentials(username, password))
flightpriceanalysisapi.WithApiHeaderCredentials(flightpriceanalysisapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
flightpriceanalysisapi.WithApiKeyCredentials(flightpriceanalysisapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FLIGHTPRICEANALYSISAPI_...` variables — e.g.
`FLIGHTPRICEANALYSISAPI_USERNAME`, `FLIGHTPRICEANALYSISAPI_PASSWORD`, `FLIGHTPRICEANALYSISAPI_TOKEN`, `FLIGHTPRICEANALYSISAPI_API_KEY`,
`FLIGHTPRICEANALYSISAPI_O_AUTH_CLIENT_ID`, `FLIGHTPRICEANALYSISAPI_O_AUTH_CLIENT_SECRET`, `FLIGHTPRICEANALYSISAPI_ACCESS_TOKEN`. Grep the
