...
All types below live under `Nytimes.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using Nytimes.Core.Configuration;
...
```csharp
using Nytimes.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new NytimesClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `Nytimes.Core.Exceptions`):
...
With DI, the SDK's `AddNytimesClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddNytimesClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
