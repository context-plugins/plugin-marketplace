...
- `coingecko` — the root package name (e.g. `multiauthsample`, `batester`).
- `CoingeckodemoapiClient` — the generated client class (read it from the `class …Client` declaration in `coingecko/coingecko_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `CoingeckodemoapiClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from coingecko.coingecko_client import CoingeckodemoapiClient
from coingecko.configuration import Environment
from coingecko.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from coingecko.configuration import Configuration, Environment
...
client = CoingeckodemoapiClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `coingecko/coingecko_client.py` in the cloned source.
...
`from_environment()` is a class method on both `CoingeckodemoapiClient` and `Configuration` that reads
...
client = CoingeckodemoapiClient.from_environment()
...
`from_environment` in `coingecko/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `CoingeckodemoapiClient`. Access them as attributes —
...
Open `coingecko/coingecko_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `coingecko/configuration.py`. **Read the enum in `configuration.py`
...
client = CoingeckodemoapiClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`coingecko/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = CoingeckodemoapiClient(config=new_config)
```
