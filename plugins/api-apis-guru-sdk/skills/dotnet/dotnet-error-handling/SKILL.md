...
> `{ApiGroup}`, `FlightCheckInLinks`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FlightCheckInLinks.Core.Exceptions`
- `ApiError` **and** `RawError` → `FlightCheckInLinks.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FlightCheckInLinks.Errors`
...
using FlightCheckInLinks.Core.ErrorResponse;  // ApiError, RawError
using FlightCheckInLinks.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FlightCheckInLinks.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FlightCheckInLinks.Core.Exceptions;     // SdkException<TError>
using FlightCheckInLinks.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FlightCheckInLinks.Core.ErrorResponse`. Its public
...
`readonly struct` under `FlightCheckInLinks.Core.Models`. If the controller has no `{Operation}Result`
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FlightCheckInLinks.Core.Exceptions`) — an auth *application* failure, not an API error — when the
