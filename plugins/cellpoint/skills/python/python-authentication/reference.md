...
kwargs to the `CellpointClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `cellpoint/cellpoint_client.py` and `cellpoint/http/auth/` in
the cloned source.

## Basic auth

```python
from cellpoint.http.auth.basic_auth import BasicAuthCredentials
...
```python
from cellpoint.http.auth.api_key import ApiKeyCredentials
...
```python
from cellpoint.http.auth.api_header import ApiHeaderCredentials
...
```python
from cellpoint.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from cellpoint.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from cellpoint.http.auth.o_auth_acg import OAuthACGCredentials
from cellpoint.models.o_auth_scope_cellpoint_enum import OAuthScope{Pkg}Enum
...
client = CellpointClient(o_auth_acg_credentials=creds)
...
```python
from cellpoint.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = CellpointClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `CellpointClient.from_environment()`) read each credential
...
Grep `from_environment` in `cellpoint/configuration.py` and each `cellpoint/http/auth/*.py` for the exact
