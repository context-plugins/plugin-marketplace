...
reference; substitute your `shelldataandreporting` and `ShelldatareportingapisClient` names.
...
- **Get the stub URL from the SDK, not from this page.** Every environment resolves to a real remote
  host — read the `environments` dict in `shelldataandreporting/configuration.py`, or call
