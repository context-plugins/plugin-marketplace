    ...
    boku.CreateConfiguration(
        boku.WithHttpConfiguration(
            boku.CreateHttpConfiguration(
                boku.WithTimeout(30),
                boku.WithRetryConfiguration(
                    boku.CreateRetryConfiguration(
                        boku.WithMaxRetryAttempts(3),
                        boku.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := boku.CreateRetryConfiguration(
    boku.WithMaxRetryAttempts(5),
    boku.WithRetryInterval(1),
    boku.WithBackoffFactor(2),
    boku.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    boku.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := boku.NewClient(
    boku.CreateConfiguration(
        boku.WithHttpConfiguration(
            boku.CreateHttpConfiguration(
                boku.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
