...
To see which schemes a specific SDK accepts, read the credential setter methods on `AuthentiqAPIClient.Builder`
...
- Set credentials **on the `AuthentiqAPIClient.Builder`** before calling `.build()` — the client is immutable
