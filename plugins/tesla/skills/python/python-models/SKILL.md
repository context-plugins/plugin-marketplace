...
```python
from tesla.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `tesla/utilities/union_type_lookup.py`) and each candidate model
...
```python
from tesla.models.animal import Cat, Dog
...
```python
from tesla.models.animal import Animal, Cat, Dog
