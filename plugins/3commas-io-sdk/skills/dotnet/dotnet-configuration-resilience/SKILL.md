...
All types below live under `M3commasIo.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using M3commasIo.Core.Configuration;
...
```csharp
using M3commasIo.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new M3commasIoClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `M3commasIo.Core.Exceptions`):
...
With DI, the SDK's `AddM3commasIoClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddM3commasIoClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
