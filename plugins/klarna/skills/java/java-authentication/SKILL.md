...
To see which schemes a specific SDK accepts, read the credential setter methods on `KlarnaClient.Builder`
...
- Set credentials **on the `KlarnaClient.Builder`** before calling `.build()` — the client is immutable
