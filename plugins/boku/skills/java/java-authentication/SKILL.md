...
To see which schemes a specific SDK accepts, read the credential setter methods on `BokuClient.Builder`
...
- Set credentials **on the `BokuClient.Builder`** before calling `.build()` — the client is immutable
