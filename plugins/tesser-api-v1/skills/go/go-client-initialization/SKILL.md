...
- `tesserapiv1` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/tesser-api-v1-go-sdk` — the Go import path for the root package; `github.com/context-plugins/tesser-api-v1-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/tesser-api-v1-go-sdk"

client := tesserapiv1.NewClient(
    tesserapiv1.CreateConfiguration(
        tesserapiv1.WithEnvironment(tesserapiv1.PRODUCTION),
...
from environment variables named `TESSERAPIV1_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := tesserapiv1.NewClient(tesserapiv1.CreateConfigurationFromEnvironment())
```
...
```go
client := tesserapiv1.NewClient(
    tesserapiv1.CreateConfiguration(tesserapiv1.WithEnvironment(tesserapiv1.PRODUCTION)),
)
```
...
client := tesserapiv1.NewClient(
    tesserapiv1.CreateConfiguration(
        tesserapiv1.WithHttpConfiguration(
            tesserapiv1.CreateHttpConfiguration(
                tesserapiv1.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                tesserapiv1.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = tesserapiv1.NewClient(tesserapiv1.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api tesserapiv1.ClientInterface
}

func NewService(api tesserapiv1.ClientInterface) *Service {
