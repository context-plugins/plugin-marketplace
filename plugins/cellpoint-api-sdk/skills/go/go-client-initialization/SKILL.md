...
- `cellpointapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/cellpoint-api-go` — the Go import path for the root package; `github.com/context-plugins/cellpoint-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/cellpoint-api-go"

client := cellpointapi.NewClient(
    cellpointapi.CreateConfiguration(
        cellpointapi.WithEnvironment(cellpointapi.PRODUCTION),
...
from environment variables named `CELLPOINTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := cellpointapi.NewClient(cellpointapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `cellpointapi.PRODUCTION`,
`cellpointapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := cellpointapi.NewClient(
    cellpointapi.CreateConfiguration(cellpointapi.WithEnvironment(cellpointapi.PRODUCTION)),
)
```
...
client := cellpointapi.NewClient(
    cellpointapi.CreateConfiguration(
        cellpointapi.WithHttpConfiguration(
            cellpointapi.CreateHttpConfiguration(
                cellpointapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                cellpointapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = cellpointapi.NewClient(cellpointapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api cellpointapi.ClientInterface
}

func NewService(api cellpointapi.ClientInterface) *Service {
