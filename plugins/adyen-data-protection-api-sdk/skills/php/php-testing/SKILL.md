...
> `AdyenDataProtectionAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AdyenDataProtectionAPIClient
{
...
```php
use AdyenDataProtectionAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AdyenDataProtectionAPILib\Exceptions\{OperationException};
use AdyenDataProtectionAPILib\Exceptions\ApiException;
...
app()->instance(AdyenDataProtectionAPIClient::class, clientWithResponses([
