...
| Options class | `SplititWebApiV3ClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `SplititWebApiV3ClientOptions`: `Oauth2Sandbox: OAuth2ClientCredentials?`, `Oauth2SandboxTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?`, `Oauth2Production: OAuth2ClientCredentials?`, `Oauth2ProductionTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using SplititWebApiV3.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/splitit-csharp-sdk
dotnet add reference splitit-csharp-sdk/SplititWebApiV3.csproj
```
...
temp directory** (`<temp>/splitit-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/splitit-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/splitit-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/splitit-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\splitit-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/splitit-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/splitit-dotnet-src/`, **never** in the project repo, and its path never goes into
`splitit-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `SplititWebApiV3Client.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new SplititWebApiV3Client(...)`, build its options, or DI-register via
   `AddSplititWebApiV3Client`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `SplititWebApiV3ClientOptions` — the map's *Servers & auth* section
