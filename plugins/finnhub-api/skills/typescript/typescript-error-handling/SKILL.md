...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `finnhub-apilib`) — replace it with the concrete identifier from the source.
...
import { ApiError } from 'finnhub-apilib';
import { {Operation}Error } from 'finnhub-apilib/errors';
