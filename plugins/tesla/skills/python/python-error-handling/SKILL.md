...
`tesla/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `tesla/exceptions/` and operation docstrings.
...
```python
from tesla.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `tesla/exceptions/`.
...
```python
from tesla.exceptions.api_exception import APIException
from tesla.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`tesla/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`tesla/exceptions/` for the class.
...
`tesla/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `tesla/exceptions/`
  or a `ValueError` — check the auth handler in `tesla/http/auth/` for the exact type.
