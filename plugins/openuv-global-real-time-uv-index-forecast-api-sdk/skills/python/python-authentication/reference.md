...
kwargs to the `OpenUVGlobalRealTimeUVIndexForecastAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `openuvglobalrealtimeuvindexforecastapi/openuvglobalrealtimeuvindexforecastapi_client.py` and `openuvglobalrealtimeuvindexforecastapi/http/auth/` in
the cloned source.

## Basic auth

```python
from openuvglobalrealtimeuvindexforecastapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from openuvglobalrealtimeuvindexforecastapi.http.auth.api_key import ApiKeyCredentials
...
```python
from openuvglobalrealtimeuvindexforecastapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from openuvglobalrealtimeuvindexforecastapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from openuvglobalrealtimeuvindexforecastapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from openuvglobalrealtimeuvindexforecastapi.http.auth.o_auth_acg import OAuthACGCredentials
from openuvglobalrealtimeuvindexforecastapi.models.o_auth_scope_openuvglobalrealtimeuvindexforecastapi_enum import OAuthScope{Pkg}Enum
...
client = OpenUVGlobalRealTimeUVIndexForecastAPIClient(o_auth_acg_credentials=creds)
...
```python
from openuvglobalrealtimeuvindexforecastapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = OpenUVGlobalRealTimeUVIndexForecastAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `OpenUVGlobalRealTimeUVIndexForecastAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `openuvglobalrealtimeuvindexforecastapi/configuration.py` and each `openuvglobalrealtimeuvindexforecastapi/http/auth/*.py` for the exact
