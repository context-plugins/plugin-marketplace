...
- `spotifywebapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/spotify-web-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/spotify-web-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/spotify-web-api-go-sdk"

client := spotifywebapi.NewClient(
    spotifywebapi.CreateConfiguration(
        spotifywebapi.WithEnvironment(spotifywebapi.PRODUCTION),
...
from environment variables named `SPOTIFYWEBAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := spotifywebapi.NewClient(spotifywebapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `spotifywebapi.PRODUCTION`,
`spotifywebapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := spotifywebapi.NewClient(
    spotifywebapi.CreateConfiguration(spotifywebapi.WithEnvironment(spotifywebapi.PRODUCTION)),
)
```
...
client := spotifywebapi.NewClient(
    spotifywebapi.CreateConfiguration(
        spotifywebapi.WithHttpConfiguration(
            spotifywebapi.CreateHttpConfiguration(
                spotifywebapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                spotifywebapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = spotifywebapi.NewClient(spotifywebapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api spotifywebapi.ClientInterface
}

func NewService(api spotifywebapi.ClientInterface) *Service {
