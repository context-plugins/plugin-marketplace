...
```go
authentiqapi.WithOAuthCCGCredentials(
    authentiqapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr authentiqapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := authentiqapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr authentiqapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := authentiqapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := authentiqapi.NewClient(authentiqapi.CreateConfiguration(authentiqapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    authentiqapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := authentiqapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := authentiqapi.NewClient(authentiqapi.CreateConfiguration(authentiqapi.WithOAuthROPCGCredentials(creds)))
...
```go
authentiqapi.WithOAuthBearerTokenCredentials(authentiqapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
authentiqapi.WithBasicAuthCredentials(authentiqapi.NewBasicAuthCredentials(username, password))
authentiqapi.WithApiHeaderCredentials(authentiqapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
authentiqapi.WithApiKeyCredentials(authentiqapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `AUTHENTIQAPI_...` variables — e.g.
`AUTHENTIQAPI_USERNAME`, `AUTHENTIQAPI_PASSWORD`, `AUTHENTIQAPI_TOKEN`, `AUTHENTIQAPI_API_KEY`,
`AUTHENTIQAPI_O_AUTH_CLIENT_ID`, `AUTHENTIQAPI_O_AUTH_CLIENT_SECRET`, `AUTHENTIQAPI_ACCESS_TOKEN`. Grep the
