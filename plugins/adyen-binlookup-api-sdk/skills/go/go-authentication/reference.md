...
```go
adyenbinlookupapi.WithOAuthCCGCredentials(
    adyenbinlookupapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr adyenbinlookupapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := adyenbinlookupapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr adyenbinlookupapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := adyenbinlookupapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := adyenbinlookupapi.NewClient(adyenbinlookupapi.CreateConfiguration(adyenbinlookupapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    adyenbinlookupapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := adyenbinlookupapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := adyenbinlookupapi.NewClient(adyenbinlookupapi.CreateConfiguration(adyenbinlookupapi.WithOAuthROPCGCredentials(creds)))
...
```go
adyenbinlookupapi.WithOAuthBearerTokenCredentials(adyenbinlookupapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
adyenbinlookupapi.WithBasicAuthCredentials(adyenbinlookupapi.NewBasicAuthCredentials(username, password))
adyenbinlookupapi.WithApiHeaderCredentials(adyenbinlookupapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
adyenbinlookupapi.WithApiKeyCredentials(adyenbinlookupapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `ADYENBINLOOKUPAPI_...` variables — e.g.
`ADYENBINLOOKUPAPI_USERNAME`, `ADYENBINLOOKUPAPI_PASSWORD`, `ADYENBINLOOKUPAPI_TOKEN`, `ADYENBINLOOKUPAPI_API_KEY`,
`ADYENBINLOOKUPAPI_O_AUTH_CLIENT_ID`, `ADYENBINLOOKUPAPI_O_AUTH_CLIENT_SECRET`, `ADYENBINLOOKUPAPI_ACCESS_TOKEN`. Grep the
