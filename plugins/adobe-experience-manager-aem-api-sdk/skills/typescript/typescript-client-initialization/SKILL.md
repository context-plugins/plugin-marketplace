...
- `adobe-experience-manager-aem-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'adobe-experience-manager-aem-apilib';
...
```ts
import { {Resource}Controller } from 'adobe-experience-manager-aem-apilib';
