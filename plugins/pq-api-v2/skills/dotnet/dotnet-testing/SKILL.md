...
`PqApiV2Client`/`PqApiV2ClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `PqApiV2Client`,
...
static PqApiV2Client ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new PqApiV2Client(new HttpClient(handler), new PqApiV2ClientOptions { /* auth not needed for stubs */ });
}
```
...
using PqApiV2.Errors;              // {Operation}Error types
...
```csharp
using PqApiV2.Core.Exceptions;
using PqApiV2.Core.ErrorResponse;
...
using PqApiV2.Core.ErrorResponse; // RawError (Case B)
...
var client = new PqApiV2Client(new HttpClient(handler), new PqApiV2ClientOptions());
...
- For DI-based code, the SDK's `AddPqApiV2Client` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `PqApiV2Client` from the provider:
  ```csharp
  services.AddPqApiV2Client(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<PqApiV2Client>();
  ```
