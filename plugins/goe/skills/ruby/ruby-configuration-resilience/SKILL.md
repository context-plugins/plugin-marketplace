...
```ruby
client = GOE::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = GOE::Client.new(adapter: :net_http)
```
...
client = GOE::Client.new(connection: conn)
```
...
client = GOE::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = GOE::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = GOE::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = GOE::ProxySettings.from_env
client = GOE::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < GOE::HttpCallBack
...
client = GOE::Client.new(http_callback: LogCallback.new)
```
...
client = GOE::Client.from_env(timeout: 30, max_retries: 3)
```
