...
- `PokAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `pok_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/pok_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/pok_api/configuration.rb`
(`build_default_config_from_env`) and `lib/pok_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = PokAPI::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/pok_api.rb`) and store it in a
