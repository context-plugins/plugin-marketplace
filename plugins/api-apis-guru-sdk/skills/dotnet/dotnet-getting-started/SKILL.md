...
| --- | --- |
| API | `api.apis.guru` |
| Project | `OnDemandFlightStatus.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `OnDemandFlightStatus` (the `using` namespace) |
| Client class | `OnDemandFlightStatusClient` |
| Options class | `OnDemandFlightStatusClientOptions` |
...
namespaces transitively, so `using OnDemandFlightStatus.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/api-apis-guru-csharp-c869b2
dotnet add reference api-apis-guru-csharp-c869b2/OnDemandFlightStatus.csproj
```

```csharp
using OnDemandFlightStatus;          // client + options + ServerEnvironment
using OnDemandFlightStatus.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-c869b2 /tmp/api-apis-guru-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-c869b2 "$env:TEMP\api-apis-guru-dotnet-src"
```
...
- `OnDemandFlightStatusClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `OnDemandFlightStatusClient`, build its
   `OnDemandFlightStatusClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
