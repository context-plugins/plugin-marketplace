...
- `OpenMeteoWeatherForecastAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `open_meteo_weather_forecast_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'open_meteo_weather_forecast_api'
include OpenMeteoWeatherForecastAPI
...
The authoritative list is the `initialize` signature in `lib/open_meteo_weather_forecast_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/open_meteo_weather_forecast_api/configuration.rb`
(`build_default_config_from_env`) and `lib/open_meteo_weather_forecast_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/open_meteo_weather_forecast_api.rb`) and store it in a
