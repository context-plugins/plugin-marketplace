...
- `aws-migration-hublib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'aws-migration-hublib';
...
```ts
import { {Resource}Controller } from 'aws-migration-hublib';
