...
`testing` + `testify/require` for reference; substitute the real `xero` and names.
...
import (
    ...
    "github.com/context-plugins/xero-go-sdk"   // root package xero
)
...
func clientReturning(status int, body string) (xero.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := xero.NewClient(
        xero.CreateConfiguration(
            xero.WithHttpConfiguration(
                xero.CreateHttpConfiguration(xero.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/xero-go-sdk/errors"
...
client := xero.NewClient(
    xero.CreateConfiguration(
        xero.WithHttpConfiguration(
            xero.CreateHttpConfiguration(xero.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
