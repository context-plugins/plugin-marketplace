...
```ruby
client = ShellDigitalpayments::Client.new(
  max_retries:    3,
...
```ruby
client = ShellDigitalpayments::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ShellDigitalpayments::Client.new(adapter: :net_http)
```
...
client = ShellDigitalpayments::Client.new(connection: conn)
```
...
client = ShellDigitalpayments::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = ShellDigitalpayments::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ShellDigitalpayments::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ShellDigitalpayments::ProxySettings.from_env
client = ShellDigitalpayments::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ShellDigitalpayments::HttpCallBack
...
client = ShellDigitalpayments::Client.new(http_callback: LogCallback.new)
```
...
client = ShellDigitalpayments::Client.from_env(timeout: 30, max_retries: 3)
```
