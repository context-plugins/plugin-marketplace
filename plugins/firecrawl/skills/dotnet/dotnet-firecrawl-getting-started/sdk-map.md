...
| Source commit (spec stamp) | `feff43d` (`feff43dc1e7e12d7d6a8dcd006973cfcf8f77b52`, tagged `feff43d`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
...
|---|---|---|
| `ApiError` — abstract base of all 21 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
...
Of **23 operations**, **22 are Case A (typed)** and **1 are Case B (raw)**.
<!-- /crawler:op-stats -->
...
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 140 | [`Actions` … `WriteText`](map/models/records-1-Ac-Wr.md) |
