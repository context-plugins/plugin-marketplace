...
client = stub_client(stubs)
assert_raises Maxio::APIException do
...
BASE = Maxio::Configuration.new(environment: Maxio::Environment::PRODUCTION).get_base_uri

def build_test_client
  Maxio::Client.new(
    environment: Maxio::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Maxio::APIException
```
...
```ruby
expect { call }.to raise_error(Maxio::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Maxio::HttpCallBack
...
client  = Maxio::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
