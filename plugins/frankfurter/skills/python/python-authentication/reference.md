...
kwargs to the `FrankfurterapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `frankfurter/frankfurter_client.py` and `frankfurter/http/auth/` in
the cloned source.

## Basic auth

```python
from frankfurter.http.auth.basic_auth import BasicAuthCredentials
...
```python
from frankfurter.http.auth.api_key import ApiKeyCredentials
...
```python
from frankfurter.http.auth.api_header import ApiHeaderCredentials
...
```python
from frankfurter.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from frankfurter.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from frankfurter.http.auth.o_auth_acg import OAuthACGCredentials
from frankfurter.models.o_auth_scope_frankfurter_enum import OAuthScope{Pkg}Enum
...
client = FrankfurterapiClient(o_auth_acg_credentials=creds)
...
```python
from frankfurter.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FrankfurterapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FrankfurterapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `frankfurter/configuration.py` and each `frankfurter/http/auth/*.py` for the exact
