...
```python
from discourse.http.auth.o_auth_acg import OAuthACGCredentials
from discourse.models.o_auth_scope_discourse_enum import OAuthScope{Pkg}Enum
...
```python
from discourse.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `discourse/configuration.py` and each `discourse/http/auth/*.py` for the exact
