...
kwargs to the `GreenbyteClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `greenbyte/greenbyte_client.py` and `greenbyte/http/auth/` in
the cloned source.

## Basic auth

```python
from greenbyte.http.auth.basic_auth import BasicAuthCredentials
...
```python
from greenbyte.http.auth.api_key import ApiKeyCredentials
...
```python
from greenbyte.http.auth.api_header import ApiHeaderCredentials
...
```python
from greenbyte.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from greenbyte.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from greenbyte.http.auth.o_auth_acg import OAuthACGCredentials
from greenbyte.models.o_auth_scope_greenbyte_enum import OAuthScope{Pkg}Enum
...
client = GreenbyteClient(o_auth_acg_credentials=creds)
...
```python
from greenbyte.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = GreenbyteClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GreenbyteClient.from_environment()`) read each credential
...
Grep `from_environment` in `greenbyte/configuration.py` and each `greenbyte/http/auth/*.py` for the exact
