...
```ruby
client = Frankfurter::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Frankfurter::Client.new(adapter: :net_http)
```
...
client = Frankfurter::Client.new(connection: conn)
```
...
client = Frankfurter::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Frankfurter::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Frankfurter::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Frankfurter::ProxySettings.from_env
client = Frankfurter::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Frankfurter::HttpCallBack
...
client = Frankfurter::Client.new(http_callback: LogCallback.new)
```
...
client = Frankfurter::Client.from_env(timeout: 30, max_retries: 3)
```
