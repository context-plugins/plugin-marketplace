...
> `ShutterstockApiExplorerClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ShutterstockApiExplorerClient
{
...
app()->instance(ShutterstockApiExplorerClient::class, clientWithResponses([
