...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new DiscourseAPIDocumentationClient({
...
```typescript
import { {Operation}Error } from 'discourse-api-documentationlib/errors';
...
```typescript
import { ApiError } from 'discourse-api-documentationlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(DiscourseAPIDocumentationClient)
    .useValue(new DiscourseAPIDocumentationClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
