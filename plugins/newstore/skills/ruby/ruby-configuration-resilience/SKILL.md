...
```ruby
client = Newstore::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Newstore::Client.new(adapter: :net_http)
```
...
client = Newstore::Client.new(connection: conn)
```
...
client = Newstore::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Newstore::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Newstore::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Newstore::ProxySettings.from_env
client = Newstore::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Newstore::HttpCallBack
...
client = Newstore::Client.new(http_callback: LogCallback.new)
```
...
client = Newstore::Client.from_env(timeout: 30, max_retries: 3)
```
