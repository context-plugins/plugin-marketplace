...
> Throughout, `slack` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := slack.NewClient(
    slack.CreateConfiguration(
        slack.WithBasicAuthCredentials(
            slack.NewBasicAuthCredentials(
...
slack.WithApiKeyCredentials(slack.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
slack.WithApiHeaderCredentials(slack.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
slack.WithOAuthBearerTokenCredentials(
    slack.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
