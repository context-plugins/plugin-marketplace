...
- `EventsAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.m1password.events` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`EventsAPIClient.Builder` class. A minimal initialization:

```java
import com.m1password.events.EventsAPIClient;
import com.m1password.events.Environment;

EventsAPIClient client = new EventsAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `EventsAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `EventsAPIClient.java` |

Call `.build()` to produce an immutable `EventsAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public EventsAPIClient apiClient() {
        return new EventsAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired EventsAPIClient client) {
        client.shutdown();
    }
}
```

Inject `EventsAPIClient` into your services as a normal Spring dependency. Build only one bean.
