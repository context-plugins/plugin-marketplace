...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new TravelRecommendationsAPIClient({
...
```typescript
import { {Operation}Error } from 'travel-recommendations-apilib/errors';
...
```typescript
import { ApiError } from 'travel-recommendations-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(TravelRecommendationsAPIClient)
    .useValue(new TravelRecommendationsAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
