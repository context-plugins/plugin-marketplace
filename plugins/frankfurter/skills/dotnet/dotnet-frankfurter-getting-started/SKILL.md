...
| --- | --- |
| API | Frankfurter |
| Source repo | https://github.com/context-plugins/frankfurter-csharp-sdk (branch `main`) |
...
<!-- crawler:auth -->
| Auth | See the credentials properties on `FrankfurterApiClientOptions` — the SDK map's *Servers & auth* section lists the scheme(s) |
<!-- /crawler:auth -->
...
namespaces transitively, so `using FrankfurterApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/frankfurter-csharp-sdk
dotnet add reference frankfurter-csharp-sdk/FrankfurterApi.csproj
```
...
temp directory** (`<temp>/frankfurter-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/frankfurter-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/frankfurter-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/frankfurter-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\frankfurter-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/frankfurter-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/frankfurter-dotnet-src/`, **never** in the project repo, and its path never goes into
`frankfurter-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new FrankfurterApiClient(...)`, build its options, or DI-register via
   `AddFrankfurterApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `FrankfurterApiClientOptions` — the map's *Servers & auth* section
