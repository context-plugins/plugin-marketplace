...
- `AdobeExperienceManagerAemApiClientOptions` — its options class.
- `AdobeExperienceManagerAemApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public AdobeExperienceManagerAemApiClient(HttpClient httpClient, AdobeExperienceManagerAemApiClientOptions options)
```
...
```csharp
public class AdobeExperienceManagerAemApiClientOptions
{
...
```csharp
using AdobeExperienceManagerAemApi;
using AdobeExperienceManagerAemApi.Servers;

var options = new AdobeExperienceManagerAemApiClientOptions
{
...
var client = new AdobeExperienceManagerAemApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddAdobeExperienceManagerAemApiClient`, which registers the
...
builder.Services.AddAdobeExperienceManagerAemApiClient(options =>
{
...
```csharp
public sealed class MyService(AdobeExperienceManagerAemApiClient client)
{
