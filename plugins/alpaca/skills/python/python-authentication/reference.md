...
kwargs to the `TraderapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `traderapi/traderapi_client.py` and `traderapi/http/auth/` in
the cloned source.

## Basic auth

```python
from traderapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from traderapi.http.auth.api_key import ApiKeyCredentials
...
```python
from traderapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from traderapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from traderapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from traderapi.http.auth.o_auth_acg import OAuthACGCredentials
from traderapi.models.o_auth_scope_traderapi_enum import OAuthScope{Pkg}Enum
...
client = TraderapiClient(o_auth_acg_credentials=creds)
...
```python
from traderapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = TraderapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `TraderapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `traderapi/configuration.py` and each `traderapi/http/auth/*.py` for the exact
