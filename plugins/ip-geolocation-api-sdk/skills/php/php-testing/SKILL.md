...
> `IPGeolocationAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): IPGeolocationAPIClient
{
...
```php
use IPGeolocationAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use IPGeolocationAPILib\Exceptions\{OperationException};
use IPGeolocationAPILib\Exceptions\ApiException;
...
app()->instance(IPGeolocationAPIClient::class, clientWithResponses([
