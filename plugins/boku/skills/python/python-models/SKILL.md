...
```python
from boku.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `boku/utilities/union_type_lookup.py`) and each candidate model
...
```python
from boku.models.animal import Cat, Dog
...
```python
from boku.models.animal import Animal, Cat, Dog
