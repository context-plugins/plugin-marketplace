...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the HotelNameAutocompleteClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write HotelNameAutocompleteClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `hotelnameautocomplete` — the root package name (e.g. `multiauthsample`, `batester`).
- `HotelNameAutocompleteClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `HotelNameAutocompleteClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from hotelnameautocomplete.hotelnameautocomplete_client import HotelNameAutocompleteClient
from hotelnameautocomplete.configuration import Environment
from hotelnameautocomplete.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = HotelNameAutocompleteClient(
...
```python
from hotelnameautocomplete.configuration import Configuration, Environment
...
client = HotelNameAutocompleteClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `hotelnameautocomplete/hotelnameautocomplete_client.py` in the cloned source.
...
`from_environment()` is a class method on both `HotelNameAutocompleteClient` and `Configuration` that reads
...
client = HotelNameAutocompleteClient.from_environment()
...
`from_environment` in `hotelnameautocomplete/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `HotelNameAutocompleteClient`. Access them as attributes —
...
Open `hotelnameautocomplete/hotelnameautocomplete_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `hotelnameautocomplete/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from hotelnameautocomplete.configuration import Environment

client = HotelNameAutocompleteClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`hotelnameautocomplete/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = HotelNameAutocompleteClient(config=new_config)
```
