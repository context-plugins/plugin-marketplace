...
- `splititwebapiv3` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/splitit-web-api-v3-go` — the Go import path for the root package; `github.com/context-plugins/splitit-web-api-v3-go/models` for request/response types.
...
```go
import "github.com/context-plugins/splitit-web-api-v3-go"
...
from environment variables named `SPLITITWEBAPIV3_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := splititwebapiv3.NewClient(splititwebapiv3.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `splititwebapiv3.PRODUCTION`,
`splititwebapiv3.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := splititwebapiv3.NewClient(
    splititwebapiv3.CreateConfiguration(splititwebapiv3.WithEnvironment(splititwebapiv3.PRODUCTION)),
)
```
    ...
    splititwebapiv3.CreateConfiguration(
        splititwebapiv3.WithHttpConfiguration(
            splititwebapiv3.CreateHttpConfiguration(
                splititwebapiv3.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                splititwebapiv3.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = splititwebapiv3.NewClient(splititwebapiv3.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api splititwebapiv3.ClientInterface
}

func NewService(api splititwebapiv3.ClientInterface) *Service {
