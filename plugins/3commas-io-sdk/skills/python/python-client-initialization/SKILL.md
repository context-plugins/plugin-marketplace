...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the 3commasIoClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write 3commasIoClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `3commasio` — the root package name (e.g. `multiauthsample`, `batester`).
- `3commasIoClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `3commasIoClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from 3commasio.3commasio_client import 3commasIoClient
from 3commasio.configuration import Environment
from 3commasio.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = 3commasIoClient(
...
```python
from 3commasio.configuration import Configuration, Environment
...
client = 3commasIoClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `3commasio/3commasio_client.py` in the cloned source.
...
`from_environment()` is a class method on both `3commasIoClient` and `Configuration` that reads
...
client = 3commasIoClient.from_environment()
...
`from_environment` in `3commasio/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `3commasIoClient`. Access them as attributes —
...
Open `3commasio/3commasio_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `3commasio/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from 3commasio.configuration import Environment

client = 3commasIoClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`3commasio/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = 3commasIoClient(config=new_config)
```
