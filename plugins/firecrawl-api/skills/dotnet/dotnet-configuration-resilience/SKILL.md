...
All types below live under `FirecrawlApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FirecrawlApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FirecrawlApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FirecrawlApi.Core.Exceptions`):
...
With DI, the SDK's `AddFirecrawlApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFirecrawlApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
