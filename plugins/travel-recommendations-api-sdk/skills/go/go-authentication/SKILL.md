...
> Throughout, `travelrecommendationsapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := travelrecommendationsapi.NewClient(
    travelrecommendationsapi.CreateConfiguration(
        travelrecommendationsapi.WithBasicAuthCredentials(
            travelrecommendationsapi.NewBasicAuthCredentials(
...
travelrecommendationsapi.WithApiKeyCredentials(travelrecommendationsapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
travelrecommendationsapi.WithApiHeaderCredentials(travelrecommendationsapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
travelrecommendationsapi.WithOAuthBearerTokenCredentials(
    travelrecommendationsapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := travelrecommendationsapi.NewClient(
    travelrecommendationsapi.CreateConfiguration(
        travelrecommendationsapi.WithOAuthCCGCredentials(
            travelrecommendationsapi.NewOAuthCCGCredentials(
