...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `LocationScoreClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`locationscore/configuration.py` in the cloned source.
...
```python
import requests
from locationscore.configuration import Configuration
...
`locationscore/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`locationscore/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `locationscore/configuration.py`
...
`NumberPagedResponse`) live in `locationscore/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `locationscore/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
