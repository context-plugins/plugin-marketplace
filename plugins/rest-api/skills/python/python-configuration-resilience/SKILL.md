...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `RestapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`restapi/configuration.py` in the cloned source.
...
```python
import requests
from restapi.configuration import Configuration
...
`restapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`restapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `restapi/configuration.py`
...
`NumberPagedResponse`) live in `restapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `restapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:

```python
import logging
from restapi.http.http_call_back import HttpCallBack
from restapi.configuration import Configuration
