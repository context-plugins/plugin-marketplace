...
> Throughout, `flightordermanagement` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := flightordermanagement.NewClient(
    flightordermanagement.CreateConfiguration(
        flightordermanagement.WithBasicAuthCredentials(
            flightordermanagement.NewBasicAuthCredentials(
...
flightordermanagement.WithApiKeyCredentials(flightordermanagement.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
flightordermanagement.WithApiHeaderCredentials(flightordermanagement.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
flightordermanagement.WithOAuthBearerTokenCredentials(
    flightordermanagement.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := flightordermanagement.NewClient(
    flightordermanagement.CreateConfiguration(
        flightordermanagement.WithOAuthCCGCredentials(
            flightordermanagement.NewOAuthCCGCredentials(
