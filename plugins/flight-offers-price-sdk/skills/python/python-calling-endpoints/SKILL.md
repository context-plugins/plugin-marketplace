...
Open `flightoffersprice/flightoffersprice_client.py` for the controller property names, then the relevant
`flightoffersprice/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from flightoffersprice.models.{request_model} import {RequestModel}
...
Open `flightoffersprice/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from flightoffersprice.models.suite_code_enum import SuiteCodeEnum
...
```python
from flightoffersprice.http.http_call_back import HttpCallBack
...
client = FlightOffersPriceClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `flightoffersprice/pagination/` in the source.

## Finding the right method

- Controller property names are on `FlightOffersPriceClient` in `flightoffersprice/flightoffersprice_client.py`.
- Operation method signatures are in `flightoffersprice/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `flightoffersprice/models/`; enum types have `from_value()`.
