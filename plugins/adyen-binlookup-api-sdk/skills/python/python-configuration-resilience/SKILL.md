...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AdyenBinLookupAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`adyenbinlookupapi/configuration.py` in the cloned source.
...
```python
import requests
from adyenbinlookupapi.configuration import Configuration
...
`adyenbinlookupapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`adyenbinlookupapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `adyenbinlookupapi/configuration.py`
...
`NumberPagedResponse`) live in `adyenbinlookupapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `adyenbinlookupapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
