...
- `ny-timeslib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'ny-timeslib';
...
```ts
import { {Resource}Api } from 'ny-timeslib';
