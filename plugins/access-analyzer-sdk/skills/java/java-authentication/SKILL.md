...
To see which schemes a specific SDK accepts, read the credential setter methods on `AccessAnalyzerClient.Builder`
...
- Set credentials **on the `AccessAnalyzerClient.Builder`** before calling `.build()` — the client is immutable
