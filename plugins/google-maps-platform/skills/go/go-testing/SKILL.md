...
`testing` + `testify/require` for reference; substitute the real `googlemapsplatform` and names.
...
import (
    ...
    "github.com/context-plugins/google-maps-platform-go-sdk"   // root package googlemapsplatform
)
...
func clientReturning(status int, body string) (googlemapsplatform.ClientInterface, *stubTransport) {
    ...
    client := googlemapsplatform.NewClient(
        googlemapsplatform.CreateConfiguration(
            googlemapsplatform.WithHttpConfiguration(
                googlemapsplatform.CreateHttpConfiguration(googlemapsplatform.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/google-maps-platform-go-sdk/errors"
...
client := googlemapsplatform.NewClient(
    googlemapsplatform.CreateConfiguration(
        googlemapsplatform.WithHttpConfiguration(
            googlemapsplatform.CreateHttpConfiguration(googlemapsplatform.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
