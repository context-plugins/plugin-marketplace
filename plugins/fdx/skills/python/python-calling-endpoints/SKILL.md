...
Open `fdx/fdx_client.py` for the controller property names, then the relevant
`fdx/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from fdx.models.{request_model} import {RequestModel}
...
Open `fdx/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from fdx.models.suite_code_enum import SuiteCodeEnum
...
```python
from fdx.http.http_call_back import HttpCallBack
...
client = FdxapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `fdx/pagination/` in the source.

## Finding the right method

- Controller property names are on `FdxapiClient` in `fdx/fdx_client.py`.
- Operation method signatures are in `fdx/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `fdx/models/`; enum types have `from_value()`.
