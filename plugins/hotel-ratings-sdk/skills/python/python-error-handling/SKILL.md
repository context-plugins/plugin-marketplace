...
`hotelratings/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `hotelratings/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `hotelratings/exceptions/`.
...
```python
from hotelratings.exceptions.api_exception import APIException
from hotelratings.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`hotelratings/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`hotelratings/exceptions/` for the class.
...
`hotelratings/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `hotelratings/exceptions/`
  or a `ValueError` — check the auth handler in `hotelratings/http/auth/` for the exact type.
