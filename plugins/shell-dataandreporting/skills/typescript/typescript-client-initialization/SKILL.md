...
- `shell-data-reporting-apislib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'shell-data-reporting-apislib';
...
```ts
import { {Resource}Api } from 'shell-data-reporting-apislib';
