...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `routing-directorlib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'routing-directorlib';
import { ApiError } from 'routing-directorlib';
import { {Operation}Error } from 'routing-directorlib/errors';
