...
`nytimes/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `nytimes/exceptions/` and operation docstrings.
...
```python
from nytimes.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `nytimes/exceptions/`.
...
```python
from nytimes.exceptions.api_exception import APIException
from nytimes.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`nytimes/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`nytimes/exceptions/` for the class.
...
`nytimes/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `nytimes/exceptions/`
  or a `ValueError` — check the auth handler in `nytimes/http/auth/` for the exact type.
