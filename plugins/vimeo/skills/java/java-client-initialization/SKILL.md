...
- `VimeoApiClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
...
`VimeoApiClient.Builder` class. A minimal initialization:
...
The `Builder` is the only entry point — confirm its fluent methods in `VimeoApiClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `VimeoApiClient.java` |

Call `.build()` to produce an immutable `VimeoApiClient` instance.
...
public class ApiConfig {
    @Bean
    public VimeoApiClient apiClient() {
        return new VimeoApiClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired VimeoApiClient client) {
        client.shutdown();
    }
}
```

Inject `VimeoApiClient` into your services as a normal Spring dependency. Build only one bean.
