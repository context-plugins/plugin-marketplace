...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `FlightCreateOrdersClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`flightcreateorders/configuration.py` in the cloned source.
...
```python
import requests
from flightcreateorders.configuration import Configuration
...
`flightcreateorders/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`flightcreateorders/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `flightcreateorders/configuration.py`
...
`NumberPagedResponse`) live in `flightcreateorders/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `flightcreateorders/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
