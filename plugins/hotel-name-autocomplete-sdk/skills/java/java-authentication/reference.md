...
against the `HotelNameAutocompleteClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `HotelNameAutocompleteClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
HotelNameAutocompleteClient client = new HotelNameAutocompleteClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeHotelNameAutocompleteEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new HotelNameAutocompleteClient.Builder()
