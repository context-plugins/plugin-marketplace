...
client = stub_client(stubs)
assert_raises Geoapify::APIException do
...
BASE = Geoapify::Configuration.new(environment: Geoapify::Environment::PRODUCTION).get_base_uri

def build_test_client
  Geoapify::Client.new(
    environment: Geoapify::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Geoapify::APIException
```
...
```ruby
expect { call }.to raise_error(Geoapify::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Geoapify::HttpCallBack
...
client  = Geoapify::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
