...
Open `flightcreateorders/flightcreateorders_client.py` for the controller property names, then the relevant
`flightcreateorders/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from flightcreateorders.models.{request_model} import {RequestModel}
...
Open `flightcreateorders/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from flightcreateorders.models.suite_code_enum import SuiteCodeEnum
...
```python
from flightcreateorders.http.http_call_back import HttpCallBack
...
client = FlightCreateOrdersClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `flightcreateorders/pagination/` in the source.

## Finding the right method

- Controller property names are on `FlightCreateOrdersClient` in `flightcreateorders/flightcreateorders_client.py`.
- Operation method signatures are in `flightcreateorders/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `flightcreateorders/models/`; enum types have `from_value()`.
