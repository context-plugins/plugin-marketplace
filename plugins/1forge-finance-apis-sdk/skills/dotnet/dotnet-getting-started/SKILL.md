...
| Project | `ForgeFinanceApis1.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `ForgeFinanceApis1` (the `using` namespace) |
| Client class | `ForgeFinanceApis1Client` |
| Options class | `ForgeFinanceApis1ClientOptions` |
...
namespaces transitively, so `using ForgeFinanceApis1.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/1forge-finance-apis-csharp
dotnet add reference 1forge-finance-apis-csharp/ForgeFinanceApis1.csproj
```

```csharp
using ForgeFinanceApis1;          // client + options + ServerEnvironment
using ForgeFinanceApis1.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/1forge-finance-apis-csharp /tmp/1forge-finance-apis-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/1forge-finance-apis-csharp "$env:TEMP\1forge-finance-apis-dotnet-src"
```
...
- `ForgeFinanceApis1Client.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/1forge-finance-apis-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\1forge-finance-apis-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `ForgeFinanceApis1Client`, build its
   `ForgeFinanceApis1ClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
