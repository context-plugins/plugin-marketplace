...
`giteaapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `giteaapi/exceptions/` and operation docstrings.
...
```python
from giteaapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `giteaapi/exceptions/`.
...
```python
from giteaapi.exceptions.api_exception import APIException
from giteaapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`giteaapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`giteaapi/exceptions/` for the class.
...
`giteaapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `giteaapi/exceptions/`
  or a `ValueError` — check the auth handler in `giteaapi/http/auth/` for the exact type.
