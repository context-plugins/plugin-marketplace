...
reference; substitute your `accessanalyzer` and `AccessAnalyzerClient` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `accessanalyzer/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
