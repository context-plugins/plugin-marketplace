...
To see which schemes a specific SDK accepts, read the credential setter methods on `HotelNameAutocompleteClient.Builder`
...
- Set credentials **on the `HotelNameAutocompleteClient.Builder`** before calling `.build()` — the client is immutable
