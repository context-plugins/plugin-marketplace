...
def stub_client(stubs)
  ...
  DeploymentAdminClient::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises DeploymentAdminClient::APIException do
...
def build_test_client
  DeploymentAdminClient::Client.new(
    environment: DeploymentAdminClient::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error DeploymentAdminClient::APIException
```
...
```ruby
expect { call }.to raise_error(DeploymentAdminClient::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < DeploymentAdminClient::HttpCallBack
...
client  = DeploymentAdminClient::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
