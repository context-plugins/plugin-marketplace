...
- `authenticationlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'authenticationlib';
...
```ts
import { {Resource}Controller } from 'authenticationlib';
