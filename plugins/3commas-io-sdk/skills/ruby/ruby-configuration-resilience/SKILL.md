...
```ruby
client = 3commasIo::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = 3commasIo::Client.new(adapter: :net_http)
```
...
client = 3commasIo::Client.new(connection: conn)
```
...
client = 3commasIo::Client.new(environment: Environment::PRODUCTION)
client = 3commasIo::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = 3commasIo::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = 3commasIo::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = 3commasIo::ProxySettings.from_env
client = 3commasIo::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < 3commasIo::HttpCallBack
...
client = 3commasIo::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = 3commasIo::Client.from_env
# Overrides take precedence over env vars:
client = 3commasIo::Client.from_env(timeout: 30, max_retries: 3)
```
