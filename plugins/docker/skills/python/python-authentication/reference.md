...
kwargs to the `DockerClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `docker/docker_client.py` and `docker/http/auth/` in
the cloned source.

## Basic auth

```python
from docker.http.auth.basic_auth import BasicAuthCredentials
...
```python
from docker.http.auth.api_key import ApiKeyCredentials
...
```python
from docker.http.auth.api_header import ApiHeaderCredentials
...
```python
from docker.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from docker.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from docker.http.auth.o_auth_acg import OAuthACGCredentials
from docker.models.o_auth_scope_docker_enum import OAuthScope{Pkg}Enum
...
client = DockerClient(o_auth_acg_credentials=creds)
...
```python
from docker.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = DockerClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `DockerClient.from_environment()`) read each credential
...
Grep `from_environment` in `docker/configuration.py` and each `docker/http/auth/*.py` for the exact
