...
kwargs to the `OpenMeteoWeatherForecastAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `openmeteoweatherforecastapi/openmeteoweatherforecastapi_client.py` and `openmeteoweatherforecastapi/http/auth/` in
the cloned source.

## Basic auth

```python
from openmeteoweatherforecastapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from openmeteoweatherforecastapi.http.auth.api_key import ApiKeyCredentials
...
```python
from openmeteoweatherforecastapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from openmeteoweatherforecastapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from openmeteoweatherforecastapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from openmeteoweatherforecastapi.http.auth.o_auth_acg import OAuthACGCredentials
from openmeteoweatherforecastapi.models.o_auth_scope_openmeteoweatherforecastapi_enum import OAuthScope{Pkg}Enum
...
client = OpenMeteoWeatherForecastAPIClient(o_auth_acg_credentials=creds)
...
```python
from openmeteoweatherforecastapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = OpenMeteoWeatherForecastAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `OpenMeteoWeatherForecastAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `openmeteoweatherforecastapi/configuration.py` and each `openmeteoweatherforecastapi/http/auth/*.py` for the exact
