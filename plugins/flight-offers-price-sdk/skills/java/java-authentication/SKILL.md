...
To see which schemes a specific SDK accepts, read the credential setter methods on `FlightOffersPriceClient.Builder`
...
- Set credentials **on the `FlightOffersPriceClient.Builder`** before calling `.build()` — the client is immutable
