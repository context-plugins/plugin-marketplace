...
```python
from slack.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `slack/utilities/union_type_lookup.py`) and each candidate model
...
```python
from slack.models.animal import Cat, Dog
...
```python
from slack.models.animal import Animal, Cat, Dog
