...
your SDK source (`cellpoint/models/`).
...
```python
from cellpoint.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `cellpoint/utilities/union_type_lookup.py`) and each candidate model
...
```python
from cellpoint.models.animal import Cat, Dog
...
```python
from cellpoint.models.animal import Animal, Cat, Dog
