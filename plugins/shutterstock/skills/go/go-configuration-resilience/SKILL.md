    ...
    shutterstock.CreateConfiguration(
        shutterstock.WithHttpConfiguration(
            shutterstock.CreateHttpConfiguration(
                shutterstock.WithTimeout(30),
                shutterstock.WithRetryConfiguration(
                    shutterstock.CreateRetryConfiguration(
                        shutterstock.WithMaxRetryAttempts(3),
                        shutterstock.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := shutterstock.CreateRetryConfiguration(
    shutterstock.WithMaxRetryAttempts(5),
    shutterstock.WithRetryInterval(1),
    shutterstock.WithBackoffFactor(2),
    shutterstock.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    shutterstock.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := shutterstock.NewClient(
    shutterstock.CreateConfiguration(
        shutterstock.WithHttpConfiguration(
            shutterstock.CreateHttpConfiguration(
                shutterstock.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
