...
client = stub_client(stubs)
assert_raises GoogleWorkspace::APIException do
...
def build_test_client
  GoogleWorkspace::Client.new(
    environment: GoogleWorkspace::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error GoogleWorkspace::APIException
```
...
```ruby
expect { call }.to raise_error(GoogleWorkspace::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < GoogleWorkspace::HttpCallBack
...
client  = GoogleWorkspace::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
