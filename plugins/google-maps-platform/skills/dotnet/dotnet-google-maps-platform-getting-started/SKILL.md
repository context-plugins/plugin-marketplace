...
| Source repo | https://github.com/context-plugins/google-maps-platform-csharp-sdk (branch `main`) |
...
<!-- crawler:auth -->
| Auth | Credentials properties on `GoogleMapsPlatformClientOptions`: `ApiKeyAuth: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `Production`, `Environment2`, `Environment3` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using GoogleMapsPlatform.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/google-maps-platform-csharp-sdk
dotnet add reference google-maps-platform-csharp-sdk/GoogleMapsPlatform.csproj
```
...
temp directory** (`<temp>/google-maps-platform-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/google-maps-platform-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/google-maps-platform-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/google-maps-platform-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\google-maps-platform-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/google-maps-platform-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/google-maps-platform-dotnet-src/`, **never** in the project repo, and its path never goes into
`google-maps-platform-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new GoogleMapsPlatformClient(...)`, build its options, or DI-register via
   `AddGoogleMapsPlatformClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `GoogleMapsPlatformClientOptions` — the map's *Servers & auth* section
