...
To see which schemes a specific SDK accepts, read the credential setter methods on `PokApiClient.Builder`
...
- Set credentials **on the `PokApiClient.Builder`** before calling `.build()` — the client is immutable
