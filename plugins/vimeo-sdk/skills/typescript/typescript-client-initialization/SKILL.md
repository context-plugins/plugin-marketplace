...
- `vimeolib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'vimeolib';
...
```ts
import { {Resource}Controller } from 'vimeolib';
