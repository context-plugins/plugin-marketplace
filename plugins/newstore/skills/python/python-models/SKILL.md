...
your SDK source (`newstore/models/`).
...
```python
from newstore.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `newstore/utilities/union_type_lookup.py`) and each candidate model
...
```python
from newstore.models.animal import Cat, Dog
...
```python
from newstore.models.animal import Animal, Cat, Dog
