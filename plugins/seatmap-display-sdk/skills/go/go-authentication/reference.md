...
```go
seatmapdisplay.WithOAuthCCGCredentials(
    seatmapdisplay.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr seatmapdisplay.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := seatmapdisplay.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr seatmapdisplay.OAuthCCGManager) models.OAuthToken {
...
```go
creds := seatmapdisplay.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := seatmapdisplay.NewClient(seatmapdisplay.CreateConfiguration(seatmapdisplay.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    seatmapdisplay.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := seatmapdisplay.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := seatmapdisplay.NewClient(seatmapdisplay.CreateConfiguration(seatmapdisplay.WithOAuthROPCGCredentials(creds)))
...
```go
seatmapdisplay.WithOAuthBearerTokenCredentials(seatmapdisplay.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
seatmapdisplay.WithBasicAuthCredentials(seatmapdisplay.NewBasicAuthCredentials(username, password))
seatmapdisplay.WithApiHeaderCredentials(seatmapdisplay.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
seatmapdisplay.WithApiKeyCredentials(seatmapdisplay.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SEATMAPDISPLAY_...` variables — e.g.
`SEATMAPDISPLAY_USERNAME`, `SEATMAPDISPLAY_PASSWORD`, `SEATMAPDISPLAY_TOKEN`, `SEATMAPDISPLAY_API_KEY`,
`SEATMAPDISPLAY_O_AUTH_CLIENT_ID`, `SEATMAPDISPLAY_O_AUTH_CLIENT_SECRET`, `SEATMAPDISPLAY_ACCESS_TOKEN`. Grep the
