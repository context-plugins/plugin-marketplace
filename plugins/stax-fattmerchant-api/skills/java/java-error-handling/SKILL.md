...
│   └── com.fattlabs.apiprod.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.fattlabs.apiprod.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.fattlabs.apiprod.http.client.HttpContext`, which carries:
