...
kwargs to the `FundAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `fundapi/fundapi_client.py` and `fundapi/http/auth/` in
the cloned source.

## Basic auth

```python
from fundapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from fundapi.http.auth.api_key import ApiKeyCredentials
...
```python
from fundapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from fundapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from fundapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from fundapi.http.auth.o_auth_acg import OAuthACGCredentials
from fundapi.models.o_auth_scope_fundapi_enum import OAuthScope{Pkg}Enum
...
client = FundAPIClient(o_auth_acg_credentials=creds)
...
```python
from fundapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FundAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FundAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `fundapi/configuration.py` and each `fundapi/http/auth/*.py` for the exact
