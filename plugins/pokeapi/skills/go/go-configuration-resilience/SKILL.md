    ...
    pokeapi.CreateConfiguration(
        pokeapi.WithHttpConfiguration(
            pokeapi.CreateHttpConfiguration(
                pokeapi.WithTimeout(30),
                pokeapi.WithRetryConfiguration(
                    pokeapi.CreateRetryConfiguration(
                        pokeapi.WithMaxRetryAttempts(3),
                        pokeapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := pokeapi.CreateRetryConfiguration(
    pokeapi.WithMaxRetryAttempts(5),
    pokeapi.WithRetryInterval(1),
    pokeapi.WithBackoffFactor(2),
    pokeapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    pokeapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := pokeapi.NewClient(
    pokeapi.CreateConfiguration(
        pokeapi.WithHttpConfiguration(
            pokeapi.CreateHttpConfiguration(
                pokeapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
