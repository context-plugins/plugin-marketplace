...
```go
fundapi.WithOAuthCCGCredentials(
    fundapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr fundapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := fundapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr fundapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := fundapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := fundapi.NewClient(fundapi.CreateConfiguration(fundapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    fundapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := fundapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := fundapi.NewClient(fundapi.CreateConfiguration(fundapi.WithOAuthROPCGCredentials(creds)))
...
```go
fundapi.WithOAuthBearerTokenCredentials(fundapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
fundapi.WithBasicAuthCredentials(fundapi.NewBasicAuthCredentials(username, password))
fundapi.WithApiHeaderCredentials(fundapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
fundapi.WithApiKeyCredentials(fundapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FUNDAPI_...` variables — e.g.
`FUNDAPI_USERNAME`, `FUNDAPI_PASSWORD`, `FUNDAPI_TOKEN`, `FUNDAPI_API_KEY`,
`FUNDAPI_O_AUTH_CLIENT_ID`, `FUNDAPI_O_AUTH_CLIENT_SECRET`, `FUNDAPI_ACCESS_TOKEN`. Grep the
