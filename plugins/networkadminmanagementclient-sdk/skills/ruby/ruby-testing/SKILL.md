...
def stub_client(stubs)
  ...
  NetworkAdminManagementClient::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises NetworkAdminManagementClient::APIException do
...
def build_test_client
  NetworkAdminManagementClient::Client.new(
    environment: NetworkAdminManagementClient::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error NetworkAdminManagementClient::APIException
```
...
```ruby
# Base APIException:
rescue NetworkAdminManagementClient::APIException => e
  assert_equal 404, e.response_code
  assert_includes e.reason, 'Not Found'

# Typed subclass (e.g. OAuthProviderException):
rescue NetworkAdminManagementClient::OAuthProviderException => e
...
```ruby
expect { call }.to raise_error(NetworkAdminManagementClient::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < NetworkAdminManagementClient::HttpCallBack
...
client  = NetworkAdminManagementClient::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
