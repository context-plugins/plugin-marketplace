...
All types below live under `SeatmapDisplay.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using SeatmapDisplay.Core.Configuration;
...
```csharp
using SeatmapDisplay.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new SeatmapDisplayClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `SeatmapDisplay.Core.Exceptions`):
...
With DI, the SDK's `AddSeatmapDisplayClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddSeatmapDisplayClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
