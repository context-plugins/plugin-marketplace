...
- `openuv-global-real-time-uv-index-forecast-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'openuv-global-real-time-uv-index-forecast-apilib';
...
```ts
import { {Resource}Controller } from 'openuv-global-real-time-uv-index-forecast-apilib';
