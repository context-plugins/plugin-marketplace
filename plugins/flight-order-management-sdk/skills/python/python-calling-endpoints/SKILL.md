...
Open `flightordermanagement/flightordermanagement_client.py` for the controller property names, then the relevant
`flightordermanagement/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from flightordermanagement.models.{request_model} import {RequestModel}
...
Open `flightordermanagement/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from flightordermanagement.models.suite_code_enum import SuiteCodeEnum
...
```python
from flightordermanagement.http.http_call_back import HttpCallBack
...
client = FlightOrderManagementClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `flightordermanagement/pagination/` in the source.

## Finding the right method

- Controller property names are on `FlightOrderManagementClient` in `flightordermanagement/flightordermanagement_client.py`.
- Operation method signatures are in `flightordermanagement/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `flightordermanagement/models/`; enum types have `from_value()`.
