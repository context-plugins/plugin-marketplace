...
```go
pqapiv2.WithOAuthCCGCredentials(
    pqapiv2.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr pqapiv2.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := pqapiv2.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr pqapiv2.OAuthCCGManager) models.OAuthToken {
...
```go
creds := pqapiv2.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := pqapiv2.NewClient(pqapiv2.CreateConfiguration(pqapiv2.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    pqapiv2.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := pqapiv2.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := pqapiv2.NewClient(pqapiv2.CreateConfiguration(pqapiv2.WithOAuthROPCGCredentials(creds)))
...
```go
pqapiv2.WithOAuthBearerTokenCredentials(pqapiv2.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
pqapiv2.WithBasicAuthCredentials(pqapiv2.NewBasicAuthCredentials(username, password))
pqapiv2.WithApiHeaderCredentials(pqapiv2.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
pqapiv2.WithApiKeyCredentials(pqapiv2.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `PQAPIV2_...` variables — e.g.
`PQAPIV2_USERNAME`, `PQAPIV2_PASSWORD`, `PQAPIV2_TOKEN`, `PQAPIV2_API_KEY`,
`PQAPIV2_O_AUTH_CLIENT_ID`, `PQAPIV2_O_AUTH_CLIENT_SECRET`, `PQAPIV2_ACCESS_TOKEN`. Grep the
