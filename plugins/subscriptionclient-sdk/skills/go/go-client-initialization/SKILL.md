...
- `subscriptionclient` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/subscriptionclient-go` — the Go import path for the root package; `github.com/context-plugins/subscriptionclient-go/models` for request/response types.
...
```go
import "github.com/context-plugins/subscriptionclient-go"

client := subscriptionclient.NewClient(
    subscriptionclient.CreateConfiguration(
        subscriptionclient.WithEnvironment(subscriptionclient.PRODUCTION),
...
from environment variables named `SUBSCRIPTIONCLIENT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := subscriptionclient.NewClient(subscriptionclient.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `subscriptionclient.PRODUCTION`,
`subscriptionclient.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := subscriptionclient.NewClient(
    subscriptionclient.CreateConfiguration(subscriptionclient.WithEnvironment(subscriptionclient.PRODUCTION)),
)
```
...
client := subscriptionclient.NewClient(
    subscriptionclient.CreateConfiguration(
        subscriptionclient.WithHttpConfiguration(
            subscriptionclient.CreateHttpConfiguration(
                subscriptionclient.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                subscriptionclient.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = subscriptionclient.NewClient(subscriptionclient.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api subscriptionclient.ClientInterface
}

func NewService(api subscriptionclient.ClientInterface) *Service {
