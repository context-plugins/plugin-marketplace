...
`AuthentiqApiClient`/`AuthentiqApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `AuthentiqApiClient`,
...
static AuthentiqApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new AuthentiqApiClient(new HttpClient(handler), new AuthentiqApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using AuthentiqApi.Errors;              // {Operation}Error types
...
```csharp
using AuthentiqApi.Core.Exceptions;
using AuthentiqApi.Core.ErrorResponse;
...
using AuthentiqApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new AuthentiqApiClient(new HttpClient(handler), new AuthentiqApiClientOptions());
...
- For DI-based code, the SDK's `AddAuthentiqApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `AuthentiqApiClient` from the provider:
  ```csharp
  services.AddAuthentiqApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<AuthentiqApiClient>();
  ```
