...
against the `GeoapifyClient.Builder` setters, the `com.geoapify.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GeoapifyClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
