...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `StaxfattmerchantapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`staxfattmerchantapi/configuration.py` in the cloned source.
...
```python
import requests
from staxfattmerchantapi.configuration import Configuration
...
`staxfattmerchantapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`staxfattmerchantapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `staxfattmerchantapi/configuration.py`
...
`NumberPagedResponse`) live in `staxfattmerchantapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `staxfattmerchantapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
