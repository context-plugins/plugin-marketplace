...
Open `adafruitiorestapi/adafruitiorestapi_client.py` for the controller property names, then the relevant
`adafruitiorestapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from adafruitiorestapi.models.{request_model} import {RequestModel}
...
Open `adafruitiorestapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from adafruitiorestapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from adafruitiorestapi.http.http_call_back import HttpCallBack
...
client = AdafruitIORESTAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `adafruitiorestapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `AdafruitIORESTAPIClient` in `adafruitiorestapi/adafruitiorestapi_client.py`.
- Operation method signatures are in `adafruitiorestapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `adafruitiorestapi/models/`; enum types have `from_value()`.
