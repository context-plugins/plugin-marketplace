# SDK map — shell-dataandreporting (.NET)
...
| | |
|---|---|
| SDK display name | shell-dataandreporting |
| Root namespace/module | `ShellDataReportingApis` |
<!-- gen:stamp -->
| Target framework(s) | `netstandard2.0` (C# `LangVersion 14`, `Nullable enable`) |
| Source commit (spec stamp) | `9dc0ee5` (`9dc0ee5ed7a9a58a006eac378543c5f63bed9f8c`, tagged `9dc0ee5`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
| Repo | https://github.com/context-plugins/shell-dataandreporting-csharp-sdk (branch `main`) |
...
```csharp
using ShellDataReportingApis;
using ShellDataReportingApis.Servers; // ServerEnvironment lives here

var options = new ShellDataReportingApisClientOptions
{
...
var client = new ShellDataReportingApisClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddShellDataReportingApisClient(o =>
{
...
`ShellDataReportingApisClient.cs`.

<!-- crawler:client-options -->
All `ShellDataReportingApisClientOptions` properties (source: `ShellDataReportingApisClientOptions.cs`):
...
| `BearerToken` | `OAuth2ClientCredentials?` |
| `BearerTokenTokenStrategy` | `IOAuth2TokenStrategy<OAuth2ClientCredentials>?` |
...
- `ShellDataReportingApisClient(HttpClient httpClient, ShellDataReportingApisClientOptions options)`
<!-- /crawler:client-options -->
...
|---|---|---|
| `ApiError` — abstract base of all 30 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
...
Of **30 operations**, **30 are Case A (typed)** and **0 are Case B (raw)**.
<!-- /crawler:op-stats -->

---

## Operations — by controller (3 groups, 30 operations)
...
|---|---:|---|
| `Customer` | 8 | [map/operations/Customer.md](map/operations/Customer.md) |
| `Invoice` | 9 | [map/operations/Invoice.md](map/operations/Invoice.md) |
| `Transaction` | 13 | [map/operations/Transaction.md](map/operations/Transaction.md) |
<!-- /crawler:ops-table -->
...
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 170 | [`AccountAccess` … `MultiPricedTransactionRequest`](map/models/records-1-Ac-Mu.md) · [`MultiPricedTransactionRequestAccountsItems` … `StatementOfAccountRequest`](map/models/records-2-Mu-St.md) · [`StatementOfAccountRequestFilters` … `Warning`](map/models/records-3-St-Wa.md) |
...
|---|---|
| Client & options (root) | `ShellDataReportingApis` |
| Operation controllers (`Api/`) | `ShellDataReportingApis.Api` |
| Records (`Models/`) | `ShellDataReportingApis.Models` |
| Enums (`Models/Enums/`) | `ShellDataReportingApis.Models.Enums` |
| Unions (`Models/AnyOf/`, `Models/OneOf/`) | `ShellDataReportingApis.Models.AnyOf` |
| Error classes (`Errors/`) | `ShellDataReportingApis.Errors` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `ShellDataReportingApisClientOptions` (source: `ShellDataReportingApisClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:
...
**Environments.** `options.Environment` is a `ServerEnvironment` (`Servers/ServerEnvironment.cs`) with members: `ServerEnvironment.Sit`, `ServerEnvironment.Production`. Base-URL templates and override points live under `Servers/` and `options.Server`.
<!-- /crawler:servers-auth -->
