...
- `SeatmapDisplay` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `seatmap_display` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'seatmap_display'
include SeatmapDisplay
...
The authoritative list is the `initialize` signature in `lib/seatmap_display/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/seatmap_display/configuration.rb`
(`build_default_config_from_env`) and `lib/seatmap_display/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/seatmap_display.rb`) and store it in a
