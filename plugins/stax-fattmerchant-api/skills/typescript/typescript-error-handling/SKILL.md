...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `stax-fattmerchant-apilib`) — replace it with the concrete identifier from the source.
