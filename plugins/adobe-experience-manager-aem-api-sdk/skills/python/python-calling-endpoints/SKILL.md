...
Open `adobeexperiencemanageraemapi/adobeexperiencemanageraemapi_client.py` for the controller property names, then the relevant
`adobeexperiencemanageraemapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from adobeexperiencemanageraemapi.models.{request_model} import {RequestModel}
...
Open `adobeexperiencemanageraemapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from adobeexperiencemanageraemapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from adobeexperiencemanageraemapi.http.http_call_back import HttpCallBack
...
client = AdobeExperienceManagerAEMAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `adobeexperiencemanageraemapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `AdobeExperienceManagerAEMAPIClient` in `adobeexperiencemanageraemapi/adobeexperiencemanageraemapi_client.py`.
- Operation method signatures are in `adobeexperiencemanageraemapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `adobeexperiencemanageraemapi/models/`; enum types have `from_value()`.
