...
- `google-workspacelib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'google-workspacelib';
...
```ts
import { {Resource}Controller } from 'google-workspacelib';
