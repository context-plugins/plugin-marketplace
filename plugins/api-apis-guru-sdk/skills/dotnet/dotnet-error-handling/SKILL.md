...
> `{ApiGroup}`, `SafePlace`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `SafePlace.Core.Exceptions`
- `ApiError` **and** `RawError` → `SafePlace.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `SafePlace.Errors`
...
```csharp
using SafePlace.Core.Exceptions;     // SdkException<TError>
using SafePlace.Core.ErrorResponse;  // ApiError, RawError
using SafePlace.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`SafePlace.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using SafePlace.Core.Exceptions;     // SdkException<TError>
using SafePlace.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `SafePlace.Core.ErrorResponse`. Its public
...
`readonly struct` under `SafePlace.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using SafePlace.Core.Models;        // ApiResult<TResponse, TError>
using SafePlace.Core.ErrorResponse; // RawError
using SafePlace.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `SafePlace.Core.Exceptions`) — an auth *application* failure, not an API error — when the
