    ...
    firecrawl.CreateConfiguration(
        firecrawl.WithHttpConfiguration(
            firecrawl.CreateHttpConfiguration(
                firecrawl.WithTimeout(30),
                firecrawl.WithRetryConfiguration(
                    firecrawl.CreateRetryConfiguration(
                        firecrawl.WithMaxRetryAttempts(3),
                        firecrawl.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := firecrawl.CreateRetryConfiguration(
    firecrawl.WithMaxRetryAttempts(5),
    firecrawl.WithRetryInterval(1),
    firecrawl.WithBackoffFactor(2),
    firecrawl.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    firecrawl.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := firecrawl.NewClient(
    firecrawl.CreateConfiguration(
        firecrawl.WithHttpConfiguration(
            firecrawl.CreateHttpConfiguration(
                firecrawl.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
