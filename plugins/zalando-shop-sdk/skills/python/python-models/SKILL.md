...
your SDK source (`zalandoshop/models/`).
...
```python
from zalandoshop.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `zalandoshop/utilities/union_type_lookup.py`) and each candidate model
...
```python
from zalandoshop.models.animal import Cat, Dog
...
```python
from zalandoshop.models.animal import Animal, Cat, Dog
