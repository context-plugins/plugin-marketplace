...
```go
pokeapi.WithOAuthCCGCredentials(
    pokeapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr pokeapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := pokeapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr pokeapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := pokeapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := pokeapi.NewClient(pokeapi.CreateConfiguration(pokeapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    pokeapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := pokeapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := pokeapi.NewClient(pokeapi.CreateConfiguration(pokeapi.WithOAuthROPCGCredentials(creds)))
...
```go
pokeapi.WithOAuthBearerTokenCredentials(pokeapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
pokeapi.WithBasicAuthCredentials(pokeapi.NewBasicAuthCredentials(username, password))
pokeapi.WithApiHeaderCredentials(pokeapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
pokeapi.WithApiKeyCredentials(pokeapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `POKEAPI_...` variables — e.g.
`POKEAPI_USERNAME`, `POKEAPI_PASSWORD`, `POKEAPI_TOKEN`, `POKEAPI_API_KEY`,
`POKEAPI_O_AUTH_CLIENT_ID`, `POKEAPI_O_AUTH_CLIENT_SECRET`, `POKEAPI_ACCESS_TOKEN`. Grep the
