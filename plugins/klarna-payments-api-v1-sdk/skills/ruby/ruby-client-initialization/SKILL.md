...
- `KlarnaPaymentsAPIV1` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `klarna_payments_api_v1` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'klarna_payments_api_v1'
include KlarnaPaymentsAPIV1
...
The authoritative list is the `initialize` signature in `lib/klarna_payments_api_v1/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/klarna_payments_api_v1/configuration.rb`
(`build_default_config_from_env`) and `lib/klarna_payments_api_v1/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/klarna_payments_api_v1.rb`) and store it in a
