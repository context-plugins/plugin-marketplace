...
kwargs to the `AuthenticationClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `authentication/authentication_client.py` and `authentication/http/auth/` in
the cloned source.

## Basic auth

```python
from authentication.http.auth.basic_auth import BasicAuthCredentials
...
```python
from authentication.http.auth.api_key import ApiKeyCredentials
...
```python
from authentication.http.auth.api_header import ApiHeaderCredentials
...
```python
from authentication.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from authentication.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from authentication.http.auth.o_auth_acg import OAuthACGCredentials
from authentication.models.o_auth_scope_authentication_enum import OAuthScope{Pkg}Enum
...
client = AuthenticationClient(o_auth_acg_credentials=creds)
...
```python
from authentication.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AuthenticationClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AuthenticationClient.from_environment()`) read each credential
...
Grep `from_environment` in `authentication/configuration.py` and each `authentication/http/auth/*.py` for the exact
