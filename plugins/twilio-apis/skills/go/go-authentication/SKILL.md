...
> Throughout, `twilioapis` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := twilioapis.NewClient(
    twilioapis.CreateConfiguration(
        twilioapis.WithBasicAuthCredentials(
            twilioapis.NewBasicAuthCredentials(
...
twilioapis.WithApiKeyCredentials(twilioapis.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
twilioapis.WithApiHeaderCredentials(twilioapis.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
twilioapis.WithOAuthBearerTokenCredentials(
    twilioapis.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := twilioapis.NewClient(
    twilioapis.CreateConfiguration(
        twilioapis.WithOAuthCCGCredentials(
            twilioapis.NewOAuthCCGCredentials(
