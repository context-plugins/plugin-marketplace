...
`fundapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `fundapi/exceptions/` and operation docstrings.
...
```python
from fundapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `fundapi/exceptions/`.
...
```python
from fundapi.exceptions.api_exception import APIException
from fundapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`fundapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`fundapi/exceptions/` for the class.
...
`fundapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `fundapi/exceptions/`
  or a `ValueError` — check the auth handler in `fundapi/http/auth/` for the exact type.
