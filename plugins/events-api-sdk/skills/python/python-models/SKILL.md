...
your SDK source (`eventsapi/models/`).
...
```python
from eventsapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `eventsapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from eventsapi.models.animal import Cat, Dog
...
```python
from eventsapi.models.animal import Animal, Cat, Dog
