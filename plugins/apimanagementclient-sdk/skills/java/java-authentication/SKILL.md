...
To see which schemes a specific SDK accepts, read the credential setter methods on `ApiManagementClientClient.Builder`
...
- Set credentials **on the `ApiManagementClientClient.Builder`** before calling `.build()` — the client is immutable
