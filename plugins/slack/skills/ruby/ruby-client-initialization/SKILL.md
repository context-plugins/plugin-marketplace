...
- `Slack` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `slack` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/slack/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/slack/configuration.rb`
(`build_default_config_from_env`) and `lib/slack/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = Slack::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/slack.rb`) and store it in a
