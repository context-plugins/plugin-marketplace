...
Open `notion/notion_client.py` for the controller property names, then the relevant
`notion/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from notion.models.{request_model} import {RequestModel}
...
Open `notion/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from notion.models.suite_code_enum import SuiteCodeEnum
...
```python
from notion.http.http_call_back import HttpCallBack
...
client = NotionClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `notion/pagination/` in the source.

## Finding the right method

- Controller property names are on `NotionClient` in `notion/notion_client.py`.
- Operation method signatures are in `notion/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `notion/models/`; enum types have `from_value()`.
