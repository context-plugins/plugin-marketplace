...
client = stub_client(stubs)
assert_raises ApiApisGuru::APIException do
...
def build_test_client
  ApiApisGuru::Client.new(
    environment: ApiApisGuru::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ApiApisGuru::APIException
```
...
```ruby
expect { call }.to raise_error(ApiApisGuru::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ApiApisGuru::HttpCallBack
...
client  = ApiApisGuru::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
