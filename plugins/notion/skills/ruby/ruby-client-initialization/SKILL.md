...
- `Notion` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `notion` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/notion/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/notion/configuration.rb`
(`build_default_config_from_env`) and `lib/notion/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/notion.rb`) and store it in a
