...
```ruby
client = EbaySell::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = EbaySell::Client.new(adapter: :net_http)
```
...
client = EbaySell::Client.new(connection: conn)
```
...
client = EbaySell::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = EbaySell::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = EbaySell::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = EbaySell::ProxySettings.from_env
client = EbaySell::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < EbaySell::HttpCallBack
...
client = EbaySell::Client.new(http_callback: LogCallback.new)
```
...
client = EbaySell::Client.from_env(timeout: 30, max_retries: 3)
```
