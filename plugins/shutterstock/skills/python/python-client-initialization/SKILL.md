...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the ShutterstockapiexplorerClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write ShutterstockapiexplorerClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `ShutterstockapiexplorerClient` is the SDK's client class — **read the real name** from `shutterstock/shutterstock_client.py`
...
- `shutterstock` — the root package name (e.g. `multiauthsample`, `batester`).
- `ShutterstockapiexplorerClient` — the generated client class (read it from the `class …Client` declaration in `shutterstock/shutterstock_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `ShutterstockapiexplorerClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from shutterstock.shutterstock_client import ShutterstockapiexplorerClient
from shutterstock.configuration import Environment
from shutterstock.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from shutterstock.configuration import Configuration, Environment
...
client = ShutterstockapiexplorerClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `shutterstock/shutterstock_client.py` in the cloned source.
...
`from_environment()` is a class method on both `ShutterstockapiexplorerClient` and `Configuration` that reads
...
client = ShutterstockapiexplorerClient.from_environment()
...
`from_environment` in `shutterstock/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `ShutterstockapiexplorerClient`. Access them as attributes —
...
Open `shutterstock/shutterstock_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `shutterstock/configuration.py`. **Read the enum in `configuration.py`
...
client = ShutterstockapiexplorerClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`shutterstock/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = ShutterstockapiexplorerClient(config=new_config)
```
