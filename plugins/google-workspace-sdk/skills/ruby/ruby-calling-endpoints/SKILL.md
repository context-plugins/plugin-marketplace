...
Open `lib/google_workspace/client.rb` for the accessor names, then the relevant
`lib/google_workspace/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = GoogleWorkspace::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: GoogleWorkspace::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/google_workspace/models/`. Field names are defined as
...
Read the enum module in `lib/google_workspace/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/google_workspace/client.rb`.
- Operation methods are in `lib/google_workspace/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/google_workspace/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/google_workspace/exceptions/`.
