...
- `TesserAPIV1` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `tesser_api_v1` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/tesser_api_v1/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/tesser_api_v1/configuration.rb`
(`build_default_config_from_env`) and `lib/tesser_api_v1/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = TesserAPIV1::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/tesser_api_v1.rb`) and store it in a
