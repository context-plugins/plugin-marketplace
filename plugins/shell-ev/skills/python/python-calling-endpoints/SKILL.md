...
Open `shellev/shellev_client.py` for the controller property names, then the relevant
`shellev/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from shellev.models.{request_model} import {RequestModel}
...
Open `shellev/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from shellev.models.suite_code_enum import SuiteCodeEnum
...
client = ShellevClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `shellev/pagination/` in the source.

## Finding the right method

- Controller property names are on `ShellevClient` in `shellev/shellev_client.py`.
- Operation method signatures are in `shellev/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `shellev/models/`; enum types have `from_value()`.
