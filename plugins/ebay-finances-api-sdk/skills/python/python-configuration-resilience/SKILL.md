...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `EBayFinancesAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`ebayfinancesapi/configuration.py` in the cloned source.
...
```python
import requests
from ebayfinancesapi.configuration import Configuration
...
`ebayfinancesapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`ebayfinancesapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `ebayfinancesapi/configuration.py`
...
`NumberPagedResponse`) live in `ebayfinancesapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `ebayfinancesapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
