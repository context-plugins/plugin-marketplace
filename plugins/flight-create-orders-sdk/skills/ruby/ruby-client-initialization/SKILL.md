...
- `FlightCreateOrders` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `flight_create_orders` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'flight_create_orders'
include FlightCreateOrders
...
The authoritative list is the `initialize` signature in `lib/flight_create_orders/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/flight_create_orders/configuration.rb`
(`build_default_config_from_env`) and `lib/flight_create_orders/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/flight_create_orders.rb`) and store it in a
