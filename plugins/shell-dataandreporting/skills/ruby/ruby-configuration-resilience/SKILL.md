...
```ruby
client = ShellDataandreporting::Client.new(
  max_retries:    3,
...
```ruby
client = ShellDataandreporting::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ShellDataandreporting::Client.new(adapter: :net_http)
```
...
client = ShellDataandreporting::Client.new(connection: conn)
```
...
client = ShellDataandreporting::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = ShellDataandreporting::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ShellDataandreporting::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ShellDataandreporting::ProxySettings.from_env
client = ShellDataandreporting::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ShellDataandreporting::HttpCallBack
...
client = ShellDataandreporting::Client.new(http_callback: LogCallback.new)
```
...
client = ShellDataandreporting::Client.from_env(timeout: 30, max_retries: 3)
```
