...
> `SportsdataClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SportsdataClient
{
...
```php
use SportsdataLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SportsdataLib\Exceptions\{OperationException};
use SportsdataLib\Exceptions\ApiException;
...
app()->instance(SportsdataClient::class, clientWithResponses([
