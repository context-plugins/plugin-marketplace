...
> `{ApiGroup}`, `FinnhubApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FinnhubApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `FinnhubApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FinnhubApi.Errors`
...
```csharp
using FinnhubApi.Core.Exceptions;     // SdkException<TError>
using FinnhubApi.Core.ErrorResponse;  // ApiError, RawError
using FinnhubApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FinnhubApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FinnhubApi.Core.Exceptions;     // SdkException<TError>
using FinnhubApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FinnhubApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `FinnhubApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FinnhubApi.Core.Models;        // ApiResult<TResponse, TError>
using FinnhubApi.Core.ErrorResponse; // RawError
using FinnhubApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FinnhubApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
