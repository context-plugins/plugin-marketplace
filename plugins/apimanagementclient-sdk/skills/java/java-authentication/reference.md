...
against the `ApiManagementClientClient.Builder` setters, the `{rootpkg}.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ApiManagementClientClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ApiManagementClientClient client = new ApiManagementClientClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeApiManagementClientEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new ApiManagementClientClient.Builder()
