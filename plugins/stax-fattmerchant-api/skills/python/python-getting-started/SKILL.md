...
| Install | `pip install "git+https://github.com/context-plugins/stax-fattmerchant-api-python-sdk@main"` |
| Client class | `StaxFattMerchantAPIClient` in `staxfattmerchantapi/staxfattmerchantapi_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `staxfattmerchantapi/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `StaxFattMerchantAPIClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `staxfattmerchantapi/exceptions/api_exception.py` — raised on non-2xx |
...
- `staxfattmerchantapi/staxfattmerchantapi_client.py` — the `StaxFattMerchantAPIClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `staxfattmerchantapi/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `staxfattmerchantapi/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `staxfattmerchantapi/controllers/` — one `{Resource}Controller` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `staxfattmerchantapi/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `staxfattmerchantapi/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `staxfattmerchantapi/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `staxfattmerchantapi/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `staxfattmerchantapi/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/stax-fattmerchant-api-python-sdk@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/stax-fattmerchant-api-python-sdk /tmp/stax-fattmerchant-api-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/stax-fattmerchant-api-python-sdk "$env:TEMP\stax-fattmerchant-api-python-src"
```
...
- `staxfattmerchantapi/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `staxfattmerchantapi/staxfattmerchantapi_client.py` — `StaxFattMerchantAPIClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `staxfattmerchantapi/controllers/*.py` — operation method signatures, parameter names, and return types.
- `staxfattmerchantapi/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `staxfattmerchantapi/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/stax-fattmerchant-api-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\stax-fattmerchant-api-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `StaxFattMerchantAPIClient(...)`.
