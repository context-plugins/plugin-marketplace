...
def stub_client(stubs)
  ...
  AutomationManagementClient::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises AutomationManagementClient::APIException do
...
def build_test_client
  AutomationManagementClient::Client.new(
    environment: AutomationManagementClient::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error AutomationManagementClient::APIException
```
...
```ruby
expect { call }.to raise_error(AutomationManagementClient::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < AutomationManagementClient::HttpCallBack
...
client  = AutomationManagementClient::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
