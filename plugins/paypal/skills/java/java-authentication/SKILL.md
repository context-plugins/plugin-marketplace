...
To see which schemes a specific SDK accepts, read the credential setter methods on `PayPalAPIClient.Builder`
...
- Set credentials **on the `PayPalAPIClient.Builder`** before calling `.build()` — the client is immutable
