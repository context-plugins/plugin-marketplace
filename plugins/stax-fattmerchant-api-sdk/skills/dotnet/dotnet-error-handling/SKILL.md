...
> `{ApiGroup}`, `StaxFattMerchantApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `StaxFattMerchantApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `StaxFattMerchantApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `StaxFattMerchantApi.Errors`
...
```csharp
using StaxFattMerchantApi.Core.Exceptions;     // SdkException<TError>
using StaxFattMerchantApi.Core.ErrorResponse;  // ApiError, RawError
using StaxFattMerchantApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`StaxFattMerchantApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using StaxFattMerchantApi.Core.Exceptions;     // SdkException<TError>
using StaxFattMerchantApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `StaxFattMerchantApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `StaxFattMerchantApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using StaxFattMerchantApi.Core.Models;        // ApiResult<TResponse, TError>
using StaxFattMerchantApi.Core.ErrorResponse; // RawError
using StaxFattMerchantApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `StaxFattMerchantApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
