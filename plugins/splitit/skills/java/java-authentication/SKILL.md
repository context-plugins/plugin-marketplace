...
To see which schemes a specific SDK accepts, read the credential setter methods on `SplititWebApiV3Client.Builder`
...
- Set credentials **on the `SplititWebApiV3Client.Builder`** before calling `.build()` — the client is immutable
