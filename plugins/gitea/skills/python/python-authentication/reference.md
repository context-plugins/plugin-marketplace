...
kwargs to the `GiteaClient` constructor (or to `Configuration`). Exact credential class names and
...
client = GiteaClient(o_auth_acg_credentials=creds)
...
client = GiteaClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GiteaClient.from_environment()`) read each credential
