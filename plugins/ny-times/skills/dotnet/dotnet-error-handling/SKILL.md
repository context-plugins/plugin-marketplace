...
> `{ApiGroup}`, `NyTimes`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `NyTimes.Core.Exceptions`
- `ApiError` **and** `RawError` → `NyTimes.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `NyTimes.Errors`
...
```csharp
using NyTimes.Core.Exceptions;     // SdkException<TError>
using NyTimes.Core.ErrorResponse;  // ApiError, RawError
using NyTimes.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`NyTimes.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using NyTimes.Core.Exceptions;     // SdkException<TError>
using NyTimes.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `NyTimes.Core.ErrorResponse`. Its public
...
`readonly struct` under `NyTimes.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using NyTimes.Core.Models;        // ApiResult<TResponse, TError>
using NyTimes.Core.ErrorResponse; // RawError
using NyTimes.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `NyTimes.Core.Exceptions`) — an auth *application* failure, not an API error — when the
