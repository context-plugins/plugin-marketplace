...
All types below live under `SqlManagementClient.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using SqlManagementClient.Core.Configuration;
...
```csharp
using SqlManagementClient.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new SqlManagementClientClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `SqlManagementClient.Core.Exceptions`):
...
With DI, the SDK's `AddSqlManagementClientClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddSqlManagementClientClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
