...
- `hostedonboardingapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `HostedOnboardingAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `HostedOnboardingAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from hostedonboardingapi.hostedonboardingapi_client import HostedOnboardingAPIClient
from hostedonboardingapi.configuration import Environment
from hostedonboardingapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = HostedOnboardingAPIClient(
...
```python
from hostedonboardingapi.configuration import Configuration, Environment
...
client = HostedOnboardingAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `hostedonboardingapi/hostedonboardingapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `HostedOnboardingAPIClient` and `Configuration` that reads
...
client = HostedOnboardingAPIClient.from_environment()
...
`from_environment` in `hostedonboardingapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `HostedOnboardingAPIClient`. Access them as attributes —
...
Open `hostedonboardingapi/hostedonboardingapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `hostedonboardingapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from hostedonboardingapi.configuration import Environment

client = HostedOnboardingAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`hostedonboardingapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = HostedOnboardingAPIClient(config=new_config)
```
