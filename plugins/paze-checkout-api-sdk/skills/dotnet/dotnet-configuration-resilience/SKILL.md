...
All types below live under `PazeCheckoutApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using PazeCheckoutApi.Core.Configuration;
...
```csharp
using PazeCheckoutApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new PazeCheckoutApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `PazeCheckoutApi.Core.Exceptions`):
...
With DI, the SDK's `AddPazeCheckoutApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddPazeCheckoutApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
