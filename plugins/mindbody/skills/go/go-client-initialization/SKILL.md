...
- `mindbody` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/mindbody-go-sdk` — the Go import path for the root package; `github.com/context-plugins/mindbody-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/mindbody-go-sdk"

client := mindbody.NewClient(
    mindbody.CreateConfiguration(
        mindbody.WithEnvironment(mindbody.PRODUCTION),
...
from environment variables named `MINDBODY_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := mindbody.NewClient(mindbody.CreateConfigurationFromEnvironment())
```
...
```go
client := mindbody.NewClient(
    mindbody.CreateConfiguration(mindbody.WithEnvironment(mindbody.PRODUCTION)),
)
```
...
client := mindbody.NewClient(
    mindbody.CreateConfiguration(
        mindbody.WithHttpConfiguration(
            mindbody.CreateHttpConfiguration(
                mindbody.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                mindbody.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = mindbody.NewClient(mindbody.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api mindbody.ClientInterface
}

func NewService(api mindbody.ClientInterface) *Service {
