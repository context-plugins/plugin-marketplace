...
| Client class | `MindbodyClient` in `mindbody/mindbody_client.py`, constructed with keyword arguments |
...
| Controllers | `@LazyProperty` properties on `MindbodyClient` (e.g. `client.authentication`, `client.transaction`) |
...
- `mindbody/mindbody_client.py` — the `MindbodyClient` class: `__init__` keyword args, `from_environment()`
...
- `mindbody/mindbody_client.py` — `MindbodyClient.__init__` kwargs, `from_environment()`, and the
...
1. **Client construction** — load **python-client-initialization** before you call `MindbodyClient(...)`.
