...
- `paze` — the root package name (e.g. `multiauthsample`, `batester`).
- `PazecheckoutapiClient` — the generated client class (read it from the `class …Client` declaration in `paze/paze_client.py`).
...
from paze.configuration import Environment
from paze.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
Confirm the exact kwarg names from `paze/paze_client.py` in the cloned source.
...
`from_environment` in `paze/configuration.py` for the exact names.
...
Open `paze/paze_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `paze/configuration.py`. **Read the enum in `configuration.py`
...
`paze/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
