...
> `{ApiGroup}`, `AuthentiqApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `AuthentiqApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `AuthentiqApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `AuthentiqApi.Errors`
...
```csharp
using AuthentiqApi.Core.Exceptions;     // SdkException<TError>
using AuthentiqApi.Core.ErrorResponse;  // ApiError, RawError
using AuthentiqApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`AuthentiqApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using AuthentiqApi.Core.Exceptions;     // SdkException<TError>
using AuthentiqApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `AuthentiqApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `AuthentiqApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using AuthentiqApi.Core.Models;        // ApiResult<TResponse, TError>
using AuthentiqApi.Core.ErrorResponse; // RawError
using AuthentiqApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `AuthentiqApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
