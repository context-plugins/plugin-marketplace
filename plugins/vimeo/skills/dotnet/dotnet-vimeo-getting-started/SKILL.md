...
| --- | --- |
| API | vimeo |
| Source repo | https://github.com/context-plugins/vimeo-csharp-sdk (branch `main`) |
...
git clone --branch main https://github.com/context-plugins/vimeo-csharp-sdk
dotnet add reference vimeo-csharp-sdk/VimeoApi.csproj
```
...
temp directory** (`<temp>/vimeo-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/vimeo-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/vimeo-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\vimeo-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/vimeo-dotnet-src/`, **never** in the project repo, and its path never goes into
`vimeo-dotnet-plan.md` — the main agent must never see the clone or its path.
