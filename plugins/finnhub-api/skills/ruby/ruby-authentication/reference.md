...
Confirm every name against `lib/finnhub_api/client.rb`, `lib/finnhub_api/configuration.rb`, the
`lib/finnhub_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
