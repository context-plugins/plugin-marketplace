...
- `discourseapidocumentation` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/discourse-api-documentation-go-sdk` — the Go import path for the root package; `github.com/context-plugins/discourse-api-documentation-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/discourse-api-documentation-go-sdk"

client := discourseapidocumentation.NewClient(
    discourseapidocumentation.CreateConfiguration(
        discourseapidocumentation.WithEnvironment(discourseapidocumentation.PRODUCTION),
...
from environment variables named `DISCOURSEAPIDOCUMENTATION_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := discourseapidocumentation.NewClient(discourseapidocumentation.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `discourseapidocumentation.PRODUCTION`,
`discourseapidocumentation.ENVIRONMENT2`) — only the first is named meaningfully, further servers are positional
placeholders, and no SDK exports a `TESTING` constant; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := discourseapidocumentation.NewClient(
    discourseapidocumentation.CreateConfiguration(discourseapidocumentation.WithEnvironment(discourseapidocumentation.PRODUCTION)),
)
```
...
client := discourseapidocumentation.NewClient(
    discourseapidocumentation.CreateConfiguration(
        discourseapidocumentation.WithHttpConfiguration(
            discourseapidocumentation.CreateHttpConfiguration(
                discourseapidocumentation.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                discourseapidocumentation.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = discourseapidocumentation.NewClient(discourseapidocumentation.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api discourseapidocumentation.ClientInterface
}

func NewService(api discourseapidocumentation.ClientInterface) *Service {
