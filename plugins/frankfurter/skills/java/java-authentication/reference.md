...
against the `FrankfurterApiClient.Builder` setters, the `dev.frankfurter.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FrankfurterApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FrankfurterApiClient client = new FrankfurterApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFrankfurterEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new FrankfurterApiClient.Builder()
