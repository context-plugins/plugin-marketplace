    ...
    ipgeolocationapi.CreateConfiguration(
        ipgeolocationapi.WithHttpConfiguration(
            ipgeolocationapi.CreateHttpConfiguration(
                ipgeolocationapi.WithTimeout(30),
                ipgeolocationapi.WithRetryConfiguration(
                    ipgeolocationapi.CreateRetryConfiguration(
                        ipgeolocationapi.WithMaxRetryAttempts(3),
                        ipgeolocationapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := ipgeolocationapi.CreateRetryConfiguration(
    ipgeolocationapi.WithMaxRetryAttempts(5),
    ipgeolocationapi.WithRetryInterval(1),
    ipgeolocationapi.WithBackoffFactor(2),
    ipgeolocationapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    ipgeolocationapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := ipgeolocationapi.NewClient(
    ipgeolocationapi.CreateConfiguration(
        ipgeolocationapi.WithHttpConfiguration(
            ipgeolocationapi.CreateHttpConfiguration(
                ipgeolocationapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
