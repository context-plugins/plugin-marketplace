...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ShutterstockAPIExplorerClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`shutterstockapiexplorer/configuration.py` in the cloned source.
...
```python
import requests
from shutterstockapiexplorer.configuration import Configuration
...
`shutterstockapiexplorer/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`shutterstockapiexplorer/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `shutterstockapiexplorer/configuration.py`
...
`NumberPagedResponse`) live in `shutterstockapiexplorer/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `shutterstockapiexplorer/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
