...
`testing` + `testify/require` for reference; substitute the real `cellpoint` and names.
...
import (
    ...
    "github.com/context-plugins/cellpoint-go-sdk"   // root package cellpoint
)
...
func clientReturning(status int, body string) (cellpoint.ClientInterface, *stubTransport) {
    ...
    client := cellpoint.NewClient(
        cellpoint.CreateConfiguration(
            cellpoint.WithHttpConfiguration(
                cellpoint.CreateHttpConfiguration(cellpoint.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/cellpoint-go-sdk/errors"
...
client := cellpoint.NewClient(
    cellpoint.CreateConfiguration(
        cellpoint.WithHttpConfiguration(
            cellpoint.CreateHttpConfiguration(cellpoint.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
