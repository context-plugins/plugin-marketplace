...
> `GreenbyteClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): GreenbyteClient
{
...
```php
use GreenbyteLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use GreenbyteLib\Exceptions\{OperationException};
use GreenbyteLib\Exceptions\ApiException;
...
app()->instance(GreenbyteClient::class, clientWithResponses([
