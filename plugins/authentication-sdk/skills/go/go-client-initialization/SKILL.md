...
- `authentication` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/authentication-go` — the Go import path for the root package; `github.com/context-plugins/authentication-go/models` for request/response types.
...
```go
import "github.com/context-plugins/authentication-go"

client := authentication.NewClient(
    authentication.CreateConfiguration(
        authentication.WithEnvironment(authentication.PRODUCTION),
...
from environment variables named `AUTHENTICATION_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := authentication.NewClient(authentication.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `authentication.PRODUCTION`,
`authentication.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := authentication.NewClient(
    authentication.CreateConfiguration(authentication.WithEnvironment(authentication.PRODUCTION)),
)
```
...
client := authentication.NewClient(
    authentication.CreateConfiguration(
        authentication.WithHttpConfiguration(
            authentication.CreateHttpConfiguration(
                authentication.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                authentication.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = authentication.NewClient(authentication.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api authentication.ClientInterface
}

func NewService(api authentication.ClientInterface) *Service {
