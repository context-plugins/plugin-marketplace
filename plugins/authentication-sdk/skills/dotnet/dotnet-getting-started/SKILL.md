...
| --- | --- |
| API | `Authentication` |
| Project | `Authentication.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `Authentication` (the `using` namespace) |
| Client class | `AuthenticationClient` |
| Options class | `AuthenticationClientOptions` |
...
namespaces transitively, so `using Authentication.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/authentication-csharp
dotnet add reference authentication-csharp/Authentication.csproj
```

```csharp
using Authentication;          // client + options + ServerEnvironment
using Authentication.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/authentication-csharp /tmp/authentication-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/authentication-csharp "$env:TEMP\authentication-dotnet-src"
```
...
- `AuthenticationClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/authentication-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\authentication-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AuthenticationClient`, build its
   `AuthenticationClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
