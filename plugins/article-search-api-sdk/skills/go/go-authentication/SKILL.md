...
> Throughout, `articlesearchapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := articlesearchapi.NewClient(
    articlesearchapi.CreateConfiguration(
        articlesearchapi.WithBasicAuthCredentials(
            articlesearchapi.NewBasicAuthCredentials(
...
articlesearchapi.WithApiKeyCredentials(articlesearchapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
articlesearchapi.WithApiHeaderCredentials(articlesearchapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
articlesearchapi.WithOAuthBearerTokenCredentials(
    articlesearchapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := articlesearchapi.NewClient(
    articlesearchapi.CreateConfiguration(
        articlesearchapi.WithOAuthCCGCredentials(
            articlesearchapi.NewOAuthCCGCredentials(
