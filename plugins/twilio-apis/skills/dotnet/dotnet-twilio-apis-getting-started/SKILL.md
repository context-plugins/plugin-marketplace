---
name: dotnet-twilio-apis-getting-started
description: Twilio APIs .NET SDK identity and lookup layer for the dotnet-twilio-apis-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Twilio APIs .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-twilio-apis-sdk` helper
...
| --- | --- |
| API | Twilio APIs |
| Source repo | https://github.com/context-plugins/twilio-apis-csharp-sdk (branch `main`) |
| Root namespace | `TwilioApis` (the `using` namespace) |
| Client class | `TwilioApisClient` |
| Options class | `TwilioApisClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `TwilioApisClientOptions`: `AccountSidAuthToken: BasicAuthCredentials?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `Production`, `Environment2`, `Environment3`, `Environment4`, `Environment5`, `Environment6`, `Environment7` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using TwilioApis.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/twilio-apis-csharp-sdk
dotnet add reference twilio-apis-csharp-sdk/TwilioApis.csproj
```
...
temp directory** (`<temp>/twilio-apis-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/twilio-apis-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/twilio-apis-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/twilio-apis-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\twilio-apis-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/twilio-apis-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/twilio-apis-dotnet-src/`, **never** in the project repo, and its path never goes into
`twilio-apis-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `TwilioApisClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new TwilioApisClient(...)`, build its options, or DI-register via
   `AddTwilioApisClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `TwilioApisClientOptions` — the map's *Servers & auth* section
