...
Open `slack/slack_client.py` for the controller property names, then the relevant
`slack/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from slack.models.{request_model} import {RequestModel}
...
Open `slack/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from slack.models.suite_code_enum import SuiteCodeEnum
...
```python
from slack.http.http_call_back import HttpCallBack
...
client = SlackwebapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `slack/pagination/` in the source.

## Finding the right method

- Controller property names are on `SlackwebapiClient` in `slack/slack_client.py`.
- Operation method signatures are in `slack/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `slack/models/`; enum types have `from_value()`.
