...
`tesla/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`tesla/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `tesla/configuration.py`
...
`NumberPagedResponse`) live in `tesla/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `tesla/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
