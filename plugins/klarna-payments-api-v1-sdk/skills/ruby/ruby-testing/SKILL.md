...
def stub_client(stubs)
  ...
  KlarnaPaymentsAPIV1::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises KlarnaPaymentsAPIV1::APIException do
...
def build_test_client
  KlarnaPaymentsAPIV1::Client.new(
    environment: KlarnaPaymentsAPIV1::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error KlarnaPaymentsAPIV1::APIException
```
...
```ruby
expect { call }.to raise_error(KlarnaPaymentsAPIV1::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < KlarnaPaymentsAPIV1::HttpCallBack
...
client  = KlarnaPaymentsAPIV1::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
