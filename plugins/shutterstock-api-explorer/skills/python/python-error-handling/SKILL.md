...
`shutterstockapiexplorer/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `shutterstockapiexplorer/exceptions/` and operation docstrings.
...
```python
from shutterstockapiexplorer.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `shutterstockapiexplorer/exceptions/`.
...
```python
from shutterstockapiexplorer.exceptions.api_exception import APIException
from shutterstockapiexplorer.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`shutterstockapiexplorer/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`shutterstockapiexplorer/exceptions/` for the class.
...
`shutterstockapiexplorer/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `shutterstockapiexplorer/exceptions/`
  or a `ValueError` — check the auth handler in `shutterstockapiexplorer/http/auth/` for the exact type.
