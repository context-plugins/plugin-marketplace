...
| --- | --- |
| API | `Xero` |
| Project | `Xero.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using Xero.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/xero-csharp
dotnet add reference xero-csharp/Xero.csproj
```

```csharp
using Xero;          // client + options + ServerEnvironment
using Xero.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/xero-csharp /tmp/xero-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/xero-csharp "$env:TEMP\xero-dotnet-src"
```
...
- `XeroClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/xero-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\xero-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `XeroClient`, build its
   `XeroClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
