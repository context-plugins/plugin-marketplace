...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddAuthentiqApiClient extension. Use the moment you write `new AuthentiqApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `AuthentiqApiClientOptions` — its options class.
- `AuthentiqApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public AuthentiqApiClient(HttpClient httpClient, AuthentiqApiClientOptions options)
```
...
```csharp
public class AuthentiqApiClientOptions
{
...
```csharp
using AuthentiqApi;
using AuthentiqApi.Servers;

var options = new AuthentiqApiClientOptions
{
...
var client = new AuthentiqApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddAuthentiqApiClient`, which registers the
...
builder.Services.AddAuthentiqApiClient(options =>
{
...
```csharp
public sealed class MyService(AuthentiqApiClient client)
{
