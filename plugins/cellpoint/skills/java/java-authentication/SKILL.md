...
To see which schemes a specific SDK accepts, read the credential setter methods on `CellPointApiClient.Builder`
...
- Set credentials **on the `CellPointApiClient.Builder`** before calling `.build()` — the client is immutable
