...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddDeploymentAdminClientClient extension. Use the moment you write `new DeploymentAdminClientClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `DeploymentAdminClientClientOptions` — its options class.
- `DeploymentAdminClient` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public DeploymentAdminClientClient(HttpClient httpClient, DeploymentAdminClientClientOptions options)
```
...
```csharp
public class DeploymentAdminClientClientOptions
{
...
```csharp
using DeploymentAdminClient;
using DeploymentAdminClient.Servers;

var options = new DeploymentAdminClientClientOptions
{
...
var client = new DeploymentAdminClientClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddDeploymentAdminClientClient`, which registers the
...
builder.Services.AddDeploymentAdminClientClient(options =>
{
...
```csharp
public sealed class MyService(DeploymentAdminClientClient client)
{
