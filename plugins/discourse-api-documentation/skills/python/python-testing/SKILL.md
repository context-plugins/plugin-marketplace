...
reference; substitute your `discourseapidocumentation` and `DiscourseapidocumentationClient` names.
...
- **Get the stub URL from the SDK, not from this page.** Every environment resolves to a real remote
  host — read the `environments` dict in `discourseapidocumentation/configuration.py`, or call
