...
> Throughout, `ebayfinancesapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := ebayfinancesapi.NewClient(
    ebayfinancesapi.CreateConfiguration(
        ebayfinancesapi.WithBasicAuthCredentials(
            ebayfinancesapi.NewBasicAuthCredentials(
...
ebayfinancesapi.WithApiKeyCredentials(ebayfinancesapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
ebayfinancesapi.WithApiHeaderCredentials(ebayfinancesapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
ebayfinancesapi.WithOAuthBearerTokenCredentials(
    ebayfinancesapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := ebayfinancesapi.NewClient(
    ebayfinancesapi.CreateConfiguration(
        ebayfinancesapi.WithOAuthCCGCredentials(
            ebayfinancesapi.NewOAuthCCGCredentials(
