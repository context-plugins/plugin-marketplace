...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new TwilioAPIsClient({
...
```typescript
import { {Operation}Error } from 'twilio-apislib/errors';
...
```typescript
import { ApiError } from 'twilio-apislib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(TwilioAPIsClient)
    .useValue(new TwilioAPIsClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
