...
- `coingeckodemoapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/coingecko-demo-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/coingecko-demo-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/coingecko-demo-api-go-sdk"

client := coingeckodemoapi.NewClient(
    coingeckodemoapi.CreateConfiguration(
        coingeckodemoapi.WithEnvironment(coingeckodemoapi.PRODUCTION),
...
from environment variables named `COINGECKODEMOAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := coingeckodemoapi.NewClient(coingeckodemoapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `coingeckodemoapi.PRODUCTION`,
`coingeckodemoapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := coingeckodemoapi.NewClient(
    coingeckodemoapi.CreateConfiguration(coingeckodemoapi.WithEnvironment(coingeckodemoapi.PRODUCTION)),
)
```
...
client := coingeckodemoapi.NewClient(
    coingeckodemoapi.CreateConfiguration(
        coingeckodemoapi.WithHttpConfiguration(
            coingeckodemoapi.CreateHttpConfiguration(
                coingeckodemoapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                coingeckodemoapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = coingeckodemoapi.NewClient(coingeckodemoapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api coingeckodemoapi.ClientInterface
}

func NewService(api coingeckodemoapi.ClientInterface) *Service {
