...
| --- | --- |
| API | `Fund API` |
| Project | `FundApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `FundApi` (the `using` namespace) |
| Client class | `FundApiClient` |
| Options class | `FundApiClientOptions` |
...
namespaces transitively, so `using FundApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/fund-api-csharp
dotnet add reference fund-api-csharp/FundApi.csproj
```

```csharp
using FundApi;          // client + options + ServerEnvironment
using FundApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/fund-api-csharp /tmp/fund-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/fund-api-csharp "$env:TEMP\fund-api-dotnet-src"
```
...
- `FundApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/fund-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\fund-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `FundApiClient`, build its
   `FundApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
