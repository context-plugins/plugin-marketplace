...
> `{ApiGroup}`, `HostedOnboardingApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `HostedOnboardingApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `HostedOnboardingApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `HostedOnboardingApi.Errors`
...
```csharp
using HostedOnboardingApi.Core.Exceptions;     // SdkException<TError>
using HostedOnboardingApi.Core.ErrorResponse;  // ApiError, RawError
using HostedOnboardingApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`HostedOnboardingApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using HostedOnboardingApi.Core.Exceptions;     // SdkException<TError>
using HostedOnboardingApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `HostedOnboardingApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `HostedOnboardingApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using HostedOnboardingApi.Core.Models;        // ApiResult<TResponse, TError>
using HostedOnboardingApi.Core.ErrorResponse; // RawError
using HostedOnboardingApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `HostedOnboardingApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
