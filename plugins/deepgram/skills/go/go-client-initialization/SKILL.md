...
- `deepgram` — the root package name (e.g. `multiauthsample`).
...
client := deepgram.NewClient(
    deepgram.CreateConfiguration(
        deepgram.WithEnvironment(deepgram.PRODUCTION),
...
from environment variables named `DEEPGRAM_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := deepgram.NewClient(deepgram.CreateConfigurationFromEnvironment())
```
...
```go
client := deepgram.NewClient(
    deepgram.CreateConfiguration(deepgram.WithEnvironment(deepgram.PRODUCTION)),
)
```
...
client := deepgram.NewClient(
    deepgram.CreateConfiguration(
        deepgram.WithHttpConfiguration(
            deepgram.CreateHttpConfiguration(
                deepgram.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                deepgram.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = deepgram.NewClient(deepgram.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api deepgram.ClientInterface
}

func NewService(api deepgram.ClientInterface) *Service {
