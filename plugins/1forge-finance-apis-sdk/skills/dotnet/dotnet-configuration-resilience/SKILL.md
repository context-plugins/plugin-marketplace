...
All types below live under `ForgeFinanceApis1.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using ForgeFinanceApis1.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new ForgeFinanceApis1ClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `ForgeFinanceApis1.Core.Exceptions`):
...
With DI, the SDK's `AddForgeFinanceApis1Client` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddForgeFinanceApis1Client(options => { /* ... */ });   // resolves CreateClient() → the default client
```
