...
- `FundAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `fund_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/fund_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/fund_api/configuration.rb`
(`build_default_config_from_env`) and `lib/fund_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/fund_api.rb`) and store it in a
