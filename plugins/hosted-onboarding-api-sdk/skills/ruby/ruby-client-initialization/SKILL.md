...
- `HostedOnboardingAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `hosted_onboarding_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'hosted_onboarding_api'
include HostedOnboardingAPI
...
The authoritative list is the `initialize` signature in `lib/hosted_onboarding_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/hosted_onboarding_api/configuration.rb`
(`build_default_config_from_env`) and `lib/hosted_onboarding_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/hosted_onboarding_api.rb`) and store it in a
