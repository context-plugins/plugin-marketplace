...
your SDK source (`pokeapi/models/`).
...
```python
from pokeapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `pokeapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from pokeapi.models.animal import Cat, Dog
...
```python
from pokeapi.models.animal import Animal, Cat, Dog
