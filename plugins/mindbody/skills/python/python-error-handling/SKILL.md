...
`mindbody/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `mindbody/exceptions/` and operation docstrings.
...
```python
from mindbody.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `mindbody/exceptions/`.
...
```python
from mindbody.exceptions.api_exception import APIException
from mindbody.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`mindbody/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`mindbody/exceptions/` for the class.
...
`mindbody/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `mindbody/exceptions/`
  or a `ValueError` — check the auth handler in `mindbody/http/auth/` for the exact type.
