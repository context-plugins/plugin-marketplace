...
```typescript
import { {Operation}Error } from 'flight-most-traveled-destinationslib/errors';
...
```typescript
import { ApiError } from 'flight-most-traveled-destinationslib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(ApiApisGuruClient)
    .useValue(new ApiApisGuruClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
