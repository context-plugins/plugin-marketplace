...
- `shell-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'shell-apilib';
...
```ts
import { {Resource}Controller } from 'shell-apilib';
