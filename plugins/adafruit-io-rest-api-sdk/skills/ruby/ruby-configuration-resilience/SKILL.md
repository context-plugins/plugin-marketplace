...
```ruby
client = AdafruitIORESTAPI::Client.new(
  max_retries:    3,
...
```ruby
client = AdafruitIORESTAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = AdafruitIORESTAPI::Client.new(adapter: :net_http)
```
...
client = AdafruitIORESTAPI::Client.new(connection: conn)
```
...
client = AdafruitIORESTAPI::Client.new(environment: Environment::PRODUCTION)
client = AdafruitIORESTAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = AdafruitIORESTAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = AdafruitIORESTAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = AdafruitIORESTAPI::ProxySettings.from_env
client = AdafruitIORESTAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < AdafruitIORESTAPI::HttpCallBack
...
client = AdafruitIORESTAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = AdafruitIORESTAPI::Client.from_env
# Overrides take precedence over env vars:
client = AdafruitIORESTAPI::Client.from_env(timeout: 30, max_retries: 3)
```
