...
client = stub_client(stubs)
assert_raises PokeApi::APIException do
...
BASE = PokeApi::Configuration.new(environment: PokeApi::Environment::PRODUCTION).get_base_uri

def build_test_client
  PokeApi::Client.new(
    environment: PokeApi::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error PokeApi::APIException
```
...
```ruby
expect { call }.to raise_error(PokeApi::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < PokeApi::HttpCallBack
...
client  = PokeApi::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
