...
- `NotionClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `com.notion.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`NotionClient.Builder` class. A minimal initialization:

```java
import com.notion.api.NotionClient;
import com.notion.api.Environment;

NotionClient client = new NotionClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `NotionClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `NotionClient.java` |

Call `.build()` to produce an immutable `NotionClient` instance.
...
public class ApiConfig {
    @Bean
    public NotionClient apiClient() {
        return new NotionClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired NotionClient client) {
        client.shutdown();
    }
}
```

Inject `NotionClient` into your services as a normal Spring dependency. Build only one bean.
