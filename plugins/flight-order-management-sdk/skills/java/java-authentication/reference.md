...
against the `FlightOrderManagementClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FlightOrderManagementClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FlightOrderManagementClient client = new FlightOrderManagementClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFlightOrderManagementEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new FlightOrderManagementClient.Builder()
