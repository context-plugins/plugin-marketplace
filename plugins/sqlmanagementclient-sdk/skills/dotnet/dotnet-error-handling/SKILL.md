...
> `{ApiGroup}`, `SqlManagementClient`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `SqlManagementClient.Core.Exceptions`
- `ApiError` **and** `RawError` → `SqlManagementClient.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `SqlManagementClient.Errors`
...
```csharp
using SqlManagementClient.Core.Exceptions;     // SdkException<TError>
using SqlManagementClient.Core.ErrorResponse;  // ApiError, RawError
using SqlManagementClient.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`SqlManagementClient.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using SqlManagementClient.Core.Exceptions;     // SdkException<TError>
using SqlManagementClient.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `SqlManagementClient.Core.ErrorResponse`. Its public
...
`readonly struct` under `SqlManagementClient.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using SqlManagementClient.Core.Models;        // ApiResult<TResponse, TError>
using SqlManagementClient.Core.ErrorResponse; // RawError
using SqlManagementClient.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `SqlManagementClient.Core.Exceptions`) — an auth *application* failure, not an API error — when the
