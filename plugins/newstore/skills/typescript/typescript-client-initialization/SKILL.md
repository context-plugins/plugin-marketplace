...
- `newstore-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'newstore-apilib';
...
```ts
import { {Resource}Api } from 'newstore-apilib';
