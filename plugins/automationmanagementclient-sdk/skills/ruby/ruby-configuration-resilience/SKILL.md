...
```ruby
client = AutomationManagementClient::Client.new(
  max_retries:    3,
...
```ruby
client = AutomationManagementClient::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = AutomationManagementClient::Client.new(adapter: :net_http)
```
...
client = AutomationManagementClient::Client.new(connection: conn)
```
...
client = AutomationManagementClient::Client.new(environment: Environment::PRODUCTION)
client = AutomationManagementClient::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = AutomationManagementClient::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = AutomationManagementClient::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = AutomationManagementClient::ProxySettings.from_env
client = AutomationManagementClient::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < AutomationManagementClient::HttpCallBack
...
client = AutomationManagementClient::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = AutomationManagementClient::Client.from_env
# Overrides take precedence over env vars:
client = AutomationManagementClient::Client.from_env(timeout: 30, max_retries: 3)
```
