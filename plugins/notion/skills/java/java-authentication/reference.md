...
against the `NotionClient.Builder` setters, the `com.notion.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `NotionClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
NotionClient client = new NotionClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeNotionEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
