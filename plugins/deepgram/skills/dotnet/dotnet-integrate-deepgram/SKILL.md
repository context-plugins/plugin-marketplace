...
description: Entry point for Deepgram .NET SDK work in a C#/.NET project — this router is for the .NET/C# SDK ONLY; never load it for any other language. Load this first when asked to integrate the Deepgram API in C#, or when a Deepgram .NET SDK call errors or behaves unexpectedly. Defines the delegation contract, the agent reuse rules, and which companion skills are required reading before you write code.
---

# Deepgram .NET SDK — Router (map + one agent)

You (the main agent) orchestrate; the `dotnet-deepgram-sdk` agent carries the SDK knowledge. The
...
- **`dotnet-deepgram-sdk`** is the single agent for every SDK need. It grounds in a bundled SDK map
...
**Scope guard:** the APIMatic-generated Deepgram **.NET SDK** (root namespace
`RestApi`) in **C#/.NET projects only**. Unrelated API, or any language other than
C#/.NET — do nothing; this router and its agent do not apply.

## Workflow

**If the user opens with a reported SDK error or unexpected Deepgram behaviour** (not new
feature work), spawn **`dotnet-deepgram-sdk`** directly with the error output and the files involved,
...
Your FIRST action is to spawn **`dotnet-deepgram-sdk`** once, with the user's full request (all
features in scope — one spawn covers the whole implementation). Dictate the output path in
the brief: the absolute path where it writes the plan (`<project repo root>/deepgram-dotnet-plan.md`) —
do not let it pick its own location.

It writes `deepgram-dotnet-plan.md` (plan + contract sheet) and returns its path.

**Parallelize the wait — prerequisites only.** Repo reconnaissance (where the integration
lands in this codebase) is not the `dotnet-deepgram-sdk` agent's job: kick it off in the SAME message
...
- Blocker or major assumption → surface it to the user in plain language, get their answer,
  send the clarification to the EXISTING `dotnet-deepgram-sdk` agent (re-spawn only if it is gone). It
...
ask the warm `dotnet-deepgram-sdk` agent, never guess.

### Step 2 — Implement from the contract sheet

1. Read `deepgram-dotnet-plan.md` once. Treat its contracts as authoritative — do not re-derive or
...
2. Implement sequentially, following the repo's own conventions and layering. You loaded the
   ...
   contract sheet or the warm `dotnet-deepgram-sdk` agent — never re-derive one from a companion.
...
4. **Any compile or runtime error involving an SDK type or member** (`CS1061`, `CS0117`,
   `CS0234`, `CS0104`, `CS1503`, `CS7036`, … on `RestApi.*`, or a provider error
   at runtime) → send the exact error output and the files involved to your EXISTING
   `dotnet-deepgram-sdk` agent (a follow-up message, NOT a new spawn), and wait. It fixes the code in
...
A standalone Deepgram question with no code change: ask the warm `dotnet-deepgram-sdk` agent (narrow-
question mode) and relay its grounded answer. Never answer from memory, even for "easy"
questions.

## Wait for your agent

**Never create or edit a project file while `dotnet-deepgram-sdk` is running** — it edits files in
...
- **Get SDK knowledge from the `dotnet-deepgram-sdk` agent, not yourself.** Don't clone or decompile the
  SDK, don't fetch its source files, and don't web-search Deepgram topics to find an
...
- **Don't load `dotnet-deepgram-getting-started` or the SDK map pages** — the map is the agent's, and
  ...
  error accessors, and enum values come from the contract sheet (or you ask the warm `dotnet-deepgram-sdk`
  agent). Where a companion points at the SDK source or a clone, that path is the agent's, not
  yours — you have no local clone.
- **Never write a Deepgram/SDK fact from memory** — every signature, field name, enum value, and
...
- **Don't re-derive or double-check a sheet row from memory.** If you are unsure what a row
  said, batch your questions into ONE message to the warm `dotnet-deepgram-sdk` agent — it holds the map
  warm and answers in seconds.
- **Never spawn a second `dotnet-deepgram-sdk` agent.** One spawn per session; everything after is a
