...
```go
slack.WithOAuthCCGCredentials(
    slack.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr slack.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := slack.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr slack.OAuthCCGManager) models.OAuthToken {
...
```go
creds := slack.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := slack.NewClient(slack.CreateConfiguration(slack.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    slack.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := slack.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := slack.NewClient(slack.CreateConfiguration(slack.WithOAuthROPCGCredentials(creds)))
...
```go
slack.WithOAuthBearerTokenCredentials(slack.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
slack.WithBasicAuthCredentials(slack.NewBasicAuthCredentials(username, password))
slack.WithApiHeaderCredentials(slack.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
slack.WithApiKeyCredentials(slack.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SLACK_...` variables — e.g.
`SLACK_USERNAME`, `SLACK_PASSWORD`, `SLACK_TOKEN`, `SLACK_API_KEY`,
`SLACK_O_AUTH_CLIENT_ID`, `SLACK_O_AUTH_CLIENT_SECRET`, `SLACK_ACCESS_TOKEN`. Grep the
