...
> `{ApiGroup}`, `FlightOffersPrice`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FlightOffersPrice.Core.Exceptions`
- `ApiError` **and** `RawError` → `FlightOffersPrice.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FlightOffersPrice.Errors`
...
```csharp
using FlightOffersPrice.Core.Exceptions;     // SdkException<TError>
using FlightOffersPrice.Core.ErrorResponse;  // ApiError, RawError
using FlightOffersPrice.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FlightOffersPrice.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FlightOffersPrice.Core.Exceptions;     // SdkException<TError>
using FlightOffersPrice.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FlightOffersPrice.Core.ErrorResponse`. Its public
...
`readonly struct` under `FlightOffersPrice.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FlightOffersPrice.Core.Models;        // ApiResult<TResponse, TError>
using FlightOffersPrice.Core.ErrorResponse; // RawError
using FlightOffersPrice.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FlightOffersPrice.Core.Exceptions`) — an auth *application* failure, not an API error — when the
