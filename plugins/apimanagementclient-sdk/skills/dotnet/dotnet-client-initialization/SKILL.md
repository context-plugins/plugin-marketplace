...
- `ApiManagementClientClientOptions` — its options class.
- `ApiManagementClient` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public ApiManagementClientClient(HttpClient httpClient, ApiManagementClientClientOptions options)
```
...
```csharp
public class ApiManagementClientClientOptions
{
...
```csharp
using ApiManagementClient;
using ApiManagementClient.Servers;

var options = new ApiManagementClientClientOptions
{
...
var client = new ApiManagementClientClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddApiManagementClientClient`, which registers the
...
builder.Services.AddApiManagementClientClient(options =>
{
...
```csharp
public sealed class MyService(ApiManagementClientClient client)
{
