...
`apisguru/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `apisguru/exceptions/` and operation docstrings.
...
```python
from apisguru.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `apisguru/exceptions/`.
...
```python
from apisguru.exceptions.api_exception import APIException
from apisguru.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`apisguru/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`apisguru/exceptions/` for the class.
...
`apisguru/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `apisguru/exceptions/`
  or a `ValueError` — check the auth handler in `apisguru/http/auth/` for the exact type.
