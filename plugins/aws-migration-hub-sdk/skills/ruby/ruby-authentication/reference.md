...
Confirm every name against `lib/aws_migration_hub/client.rb`, `lib/aws_migration_hub/configuration.rb`, the
`lib/aws_migration_hub/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
