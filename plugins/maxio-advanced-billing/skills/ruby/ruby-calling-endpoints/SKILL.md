...
Open `lib/maxio_advanced_billing/client.rb` for the accessor names, then the relevant
`lib/maxio_advanced_billing/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = MaxioAdvancedBilling::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: MaxioAdvancedBilling::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/maxio_advanced_billing/models/`. Field names are defined as
...
Read the enum module in `lib/maxio_advanced_billing/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/maxio_advanced_billing/client.rb`.
- Operation methods are in `lib/maxio_advanced_billing/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/maxio_advanced_billing/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/maxio_advanced_billing/exceptions/`.
