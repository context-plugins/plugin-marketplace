...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `TraderapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`traderapi/configuration.py` in the cloned source.
...
```python
import requests
from traderapi.configuration import Configuration
...
`traderapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`traderapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `traderapi/configuration.py`
...
`NumberPagedResponse`) live in `traderapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `traderapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
