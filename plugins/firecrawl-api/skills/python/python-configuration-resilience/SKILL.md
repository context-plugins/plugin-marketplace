...
`firecrawlapi/configuration.py` in the cloned source.
...
```python
import requests
from firecrawlapi.configuration import Configuration
...
`firecrawlapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`firecrawlapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `firecrawlapi/configuration.py`
...
`NumberPagedResponse`) live in `firecrawlapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `firecrawlapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
