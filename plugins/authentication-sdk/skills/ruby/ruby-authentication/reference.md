...
Confirm every name against `lib/authentication/client.rb`, `lib/authentication/configuration.rb`, the
`lib/authentication/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
