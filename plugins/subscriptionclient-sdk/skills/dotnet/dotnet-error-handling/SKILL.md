...
> `{ApiGroup}`, `SubscriptionClient`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `SubscriptionClient.Core.Exceptions`
- `ApiError` **and** `RawError` → `SubscriptionClient.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `SubscriptionClient.Errors`
...
```csharp
using SubscriptionClient.Core.Exceptions;     // SdkException<TError>
using SubscriptionClient.Core.ErrorResponse;  // ApiError, RawError
using SubscriptionClient.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`SubscriptionClient.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using SubscriptionClient.Core.Exceptions;     // SdkException<TError>
using SubscriptionClient.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `SubscriptionClient.Core.ErrorResponse`. Its public
...
`readonly struct` under `SubscriptionClient.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using SubscriptionClient.Core.Models;        // ApiResult<TResponse, TError>
using SubscriptionClient.Core.ErrorResponse; // RawError
using SubscriptionClient.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `SubscriptionClient.Core.Exceptions`) — an auth *application* failure, not an API error — when the
