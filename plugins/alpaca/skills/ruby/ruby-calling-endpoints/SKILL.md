...
Open `lib/alpaca/client.rb` for the accessor names, then the relevant
`lib/alpaca/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = Alpaca::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: Alpaca::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/alpaca/models/`. Field names are defined as
...
Read the enum module in `lib/alpaca/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/alpaca/client.rb`.
- Operation methods are in `lib/alpaca/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/alpaca/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/alpaca/exceptions/`.
