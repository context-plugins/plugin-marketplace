...
Open `sportsdata/sportsdata_client.py` for the controller property names, then the relevant
`sportsdata/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from sportsdata.models.{request_model} import {RequestModel}
...
Open `sportsdata/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from sportsdata.models.suite_code_enum import SuiteCodeEnum
...
```python
from sportsdata.http.http_call_back import HttpCallBack
...
client = SportsdataClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `sportsdata/pagination/` in the source.

## Finding the right method

- Controller property names are on `SportsdataClient` in `sportsdata/sportsdata_client.py`.
- Operation method signatures are in `sportsdata/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `sportsdata/models/`; enum types have `from_value()`.
