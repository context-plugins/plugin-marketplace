...
`testing` + `testify/require` for reference; substitute the real `xeroaccountingapi` and names.
...
import (
    ...
    "github.com/context-plugins/xero-accounting-api-go"   // root package xeroaccountingapi
)
...
func clientReturning(status int, body string) (xeroaccountingapi.ClientInterface, *stubTransport) {
    ...
    client := xeroaccountingapi.NewClient(
        xeroaccountingapi.CreateConfiguration(
            xeroaccountingapi.WithHttpConfiguration(
                xeroaccountingapi.CreateHttpConfiguration(xeroaccountingapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/xero-accounting-api-go/errors"
...
client := xeroaccountingapi.NewClient(
    xeroaccountingapi.CreateConfiguration(
        xeroaccountingapi.WithHttpConfiguration(
            xeroaccountingapi.CreateHttpConfiguration(xeroaccountingapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
