...
```go
adyendataprotectionapi.WithOAuthCCGCredentials(
    adyendataprotectionapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr adyendataprotectionapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := adyendataprotectionapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr adyendataprotectionapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := adyendataprotectionapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := adyendataprotectionapi.NewClient(adyendataprotectionapi.CreateConfiguration(adyendataprotectionapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    adyendataprotectionapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := adyendataprotectionapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := adyendataprotectionapi.NewClient(adyendataprotectionapi.CreateConfiguration(adyendataprotectionapi.WithOAuthROPCGCredentials(creds)))
...
```go
adyendataprotectionapi.WithOAuthBearerTokenCredentials(adyendataprotectionapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
adyendataprotectionapi.WithBasicAuthCredentials(adyendataprotectionapi.NewBasicAuthCredentials(username, password))
adyendataprotectionapi.WithApiHeaderCredentials(adyendataprotectionapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
adyendataprotectionapi.WithApiKeyCredentials(adyendataprotectionapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `ADYENDATAPROTECTIONAPI_...` variables — e.g.
`ADYENDATAPROTECTIONAPI_USERNAME`, `ADYENDATAPROTECTIONAPI_PASSWORD`, `ADYENDATAPROTECTIONAPI_TOKEN`, `ADYENDATAPROTECTIONAPI_API_KEY`,
`ADYENDATAPROTECTIONAPI_O_AUTH_CLIENT_ID`, `ADYENDATAPROTECTIONAPI_O_AUTH_CLIENT_SECRET`, `ADYENDATAPROTECTIONAPI_ACCESS_TOKEN`. Grep the
