...
> `VerizonClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): VerizonClient
{
...
```php
use VerizonLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use VerizonLib\Exceptions\{OperationException};
use VerizonLib\Exceptions\ApiException;
...
app()->instance(VerizonClient::class, clientWithResponses([
