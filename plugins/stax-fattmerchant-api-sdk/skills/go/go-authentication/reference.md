...
```go
staxfattmerchantapi.WithOAuthCCGCredentials(
    staxfattmerchantapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr staxfattmerchantapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := staxfattmerchantapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr staxfattmerchantapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := staxfattmerchantapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := staxfattmerchantapi.NewClient(staxfattmerchantapi.CreateConfiguration(staxfattmerchantapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    staxfattmerchantapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := staxfattmerchantapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := staxfattmerchantapi.NewClient(staxfattmerchantapi.CreateConfiguration(staxfattmerchantapi.WithOAuthROPCGCredentials(creds)))
...
```go
staxfattmerchantapi.WithOAuthBearerTokenCredentials(staxfattmerchantapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
staxfattmerchantapi.WithBasicAuthCredentials(staxfattmerchantapi.NewBasicAuthCredentials(username, password))
staxfattmerchantapi.WithApiHeaderCredentials(staxfattmerchantapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
staxfattmerchantapi.WithApiKeyCredentials(staxfattmerchantapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `STAXFATTMERCHANTAPI_...` variables — e.g.
`STAXFATTMERCHANTAPI_USERNAME`, `STAXFATTMERCHANTAPI_PASSWORD`, `STAXFATTMERCHANTAPI_TOKEN`, `STAXFATTMERCHANTAPI_API_KEY`,
`STAXFATTMERCHANTAPI_O_AUTH_CLIENT_ID`, `STAXFATTMERCHANTAPI_O_AUTH_CLIENT_SECRET`, `STAXFATTMERCHANTAPI_ACCESS_TOKEN`. Grep the
