...
kwarg names are generated per-API — confirm from `paze/paze_client.py` and `paze/http/auth/` in
the cloned source.

## Basic auth

```python
from paze.http.auth.basic_auth import BasicAuthCredentials
...
```python
from paze.http.auth.api_key import ApiKeyCredentials
...
```python
from paze.http.auth.api_header import ApiHeaderCredentials
...
```python
from paze.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from paze.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from paze.http.auth.o_auth_acg import OAuthACGCredentials
from paze.models.o_auth_scope_paze_enum import OAuthScope{Pkg}Enum
...
```python
from paze.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `paze/configuration.py` and each `paze/http/auth/*.py` for the exact
