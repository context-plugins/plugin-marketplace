...
kwargs to the `EbaysellClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `ebaysell/ebaysell_client.py` and `ebaysell/http/auth/` in
the cloned source.

## Basic auth

```python
from ebaysell.http.auth.basic_auth import BasicAuthCredentials
...
```python
from ebaysell.http.auth.api_key import ApiKeyCredentials
...
```python
from ebaysell.http.auth.api_header import ApiHeaderCredentials
...
```python
from ebaysell.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from ebaysell.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from ebaysell.http.auth.o_auth_acg import OAuthACGCredentials
from ebaysell.models.o_auth_scope_ebaysell_enum import OAuthScope{Pkg}Enum
...
client = EbaysellClient(o_auth_acg_credentials=creds)
...
```python
from ebaysell.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = EbaysellClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `EbaysellClient.from_environment()`) read each credential
...
Grep `from_environment` in `ebaysell/configuration.py` and each `ebaysell/http/auth/*.py` for the exact
