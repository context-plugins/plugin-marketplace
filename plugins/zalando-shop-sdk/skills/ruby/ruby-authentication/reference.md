...
Confirm every name against `lib/zalando_shop/client.rb`, `lib/zalando_shop/configuration.rb`, the
`lib/zalando_shop/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
