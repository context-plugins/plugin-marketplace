...
> `EBayFinancesAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): EBayFinancesAPIClient
{
...
```php
use EBayFinancesAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use EBayFinancesAPILib\Exceptions\{OperationException};
use EBayFinancesAPILib\Exceptions\ApiException;
...
app()->instance(EBayFinancesAPIClient::class, clientWithResponses([
