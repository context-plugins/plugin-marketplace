...
- `TeslaAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`TeslaAPIClient.Builder` class. A minimal initialization:

```java
import com.tesla.cloud.vn.na.prd.fleetapi.TeslaAPIClient;
import com.tesla.cloud.vn.na.prd.fleetapi.Environment;

TeslaAPIClient client = new TeslaAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `TeslaAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `TeslaAPIClient.java` |

Call `.build()` to produce an immutable `TeslaAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public TeslaAPIClient apiClient() {
        return new TeslaAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired TeslaAPIClient client) {
        client.shutdown();
    }
}
```

Inject `TeslaAPIClient` into your services as a normal Spring dependency. Build only one bean.
