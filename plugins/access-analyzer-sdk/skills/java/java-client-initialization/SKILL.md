...
- `AccessAnalyzerClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.amazonaws.useast1.accessanalyzer` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`AccessAnalyzerClient.Builder` class. A minimal initialization:

```java
import com.amazonaws.useast1.accessanalyzer.AccessAnalyzerClient;
import com.amazonaws.useast1.accessanalyzer.Environment;

AccessAnalyzerClient client = new AccessAnalyzerClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `AccessAnalyzerClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `AccessAnalyzerClient.java` |

Call `.build()` to produce an immutable `AccessAnalyzerClient` instance.
...
public class ApiConfig {
    @Bean
    public AccessAnalyzerClient apiClient() {
        return new AccessAnalyzerClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired AccessAnalyzerClient client) {
        client.shutdown();
    }
}
```

Inject `AccessAnalyzerClient` into your services as a normal Spring dependency. Build only one bean.
