...
│   └── com.m1password.events.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.m1password.events.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.m1password.events.http.client.HttpContext`, which carries:
