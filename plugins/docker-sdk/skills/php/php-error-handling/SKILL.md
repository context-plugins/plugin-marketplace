...
> `{operation}`, `{apiGroup}`, `DockerLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `DockerLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use DockerLib\Exceptions\ApiException;
use DockerLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `DockerLib\Exceptions\` — read the source to
