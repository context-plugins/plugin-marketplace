...
`authentication/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `authentication/exceptions/` and operation docstrings.
...
```python
from authentication.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `authentication/exceptions/`.
...
```python
from authentication.exceptions.api_exception import APIException
from authentication.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`authentication/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`authentication/exceptions/` for the class.
...
`authentication/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `authentication/exceptions/`
  or a `ValueError` — check the auth handler in `authentication/http/auth/` for the exact type.
