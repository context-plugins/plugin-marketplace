...
> Throughout, `greenbyte` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := greenbyte.NewClient(
    greenbyte.CreateConfiguration(
        greenbyte.WithBasicAuthCredentials(
            greenbyte.NewBasicAuthCredentials(
...
greenbyte.WithApiKeyCredentials(greenbyte.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
greenbyte.WithApiHeaderCredentials(greenbyte.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
greenbyte.WithOAuthBearerTokenCredentials(
    greenbyte.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := greenbyte.NewClient(
    greenbyte.CreateConfiguration(
        greenbyte.WithOAuthCCGCredentials(
            greenbyte.NewOAuthCCGCredentials(
