...
kwargs to the `TeslafleetmanagementapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `teslafleetmanagementapi/teslafleetmanagementapi_client.py` and `teslafleetmanagementapi/http/auth/` in
the cloned source.

## Basic auth

```python
from teslafleetmanagementapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from teslafleetmanagementapi.http.auth.api_key import ApiKeyCredentials
...
```python
from teslafleetmanagementapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from teslafleetmanagementapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from teslafleetmanagementapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from teslafleetmanagementapi.http.auth.o_auth_acg import OAuthACGCredentials
from teslafleetmanagementapi.models.o_auth_scope_teslafleetmanagementapi_enum import OAuthScope{Pkg}Enum
...
client = TeslafleetmanagementapiClient(o_auth_acg_credentials=creds)
...
```python
from teslafleetmanagementapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = TeslafleetmanagementapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `TeslafleetmanagementapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `teslafleetmanagementapi/configuration.py` and each `teslafleetmanagementapi/http/auth/*.py` for the exact
