...
against the `FirecrawlApiClient.Builder` setters, the `dev.firecrawl.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FirecrawlApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FirecrawlApiClient client = new FirecrawlApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFirecrawlAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new FirecrawlApiClient.Builder()
