...
against the `TeslaAPIClient.Builder` setters, the `com.tesla.cloud.vn.na.prd.fleetapi.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `TeslaAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
TeslaAPIClient client = new TeslaAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeTeslaAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
