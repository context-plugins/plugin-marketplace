...
kwargs to the `3commasIoClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `3commasio/3commasio_client.py` and `3commasio/http/auth/` in
the cloned source.

## Basic auth

```python
from 3commasio.http.auth.basic_auth import BasicAuthCredentials
...
```python
from 3commasio.http.auth.api_key import ApiKeyCredentials
...
```python
from 3commasio.http.auth.api_header import ApiHeaderCredentials
...
```python
from 3commasio.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from 3commasio.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from 3commasio.http.auth.o_auth_acg import OAuthACGCredentials
from 3commasio.models.o_auth_scope_3commasio_enum import OAuthScope{Pkg}Enum
...
client = 3commasIoClient(o_auth_acg_credentials=creds)
...
```python
from 3commasio.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = 3commasIoClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `3commasIoClient.from_environment()`) read each credential
...
Grep `from_environment` in `3commasio/configuration.py` and each `3commasio/http/auth/*.py` for the exact
