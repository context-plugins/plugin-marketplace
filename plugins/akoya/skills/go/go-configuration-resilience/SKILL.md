    ...
    akoya.CreateConfiguration(
        akoya.WithHttpConfiguration(
            akoya.CreateHttpConfiguration(
                akoya.WithTimeout(30),
                akoya.WithRetryConfiguration(
                    akoya.CreateRetryConfiguration(
                        akoya.WithMaxRetryAttempts(3),
                        akoya.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := akoya.CreateRetryConfiguration(
    akoya.WithMaxRetryAttempts(5),
    akoya.WithRetryInterval(1),
    akoya.WithBackoffFactor(2),
    akoya.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    akoya.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := akoya.NewClient(
    akoya.CreateConfiguration(
        akoya.WithHttpConfiguration(
            akoya.CreateHttpConfiguration(
                akoya.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
