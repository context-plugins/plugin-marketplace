...
│   └── com.mypayquicker.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.mypayquicker.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.mypayquicker.api.http.client.HttpContext`, which carries:
