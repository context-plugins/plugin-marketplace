...
> Throughout, `cellpointapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := cellpointapi.NewClient(
    cellpointapi.CreateConfiguration(
        cellpointapi.WithBasicAuthCredentials(
            cellpointapi.NewBasicAuthCredentials(
...
cellpointapi.WithApiKeyCredentials(cellpointapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
cellpointapi.WithApiHeaderCredentials(cellpointapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
cellpointapi.WithOAuthBearerTokenCredentials(
    cellpointapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := cellpointapi.NewClient(
    cellpointapi.CreateConfiguration(
        cellpointapi.WithOAuthCCGCredentials(
            cellpointapi.NewOAuthCCGCredentials(
