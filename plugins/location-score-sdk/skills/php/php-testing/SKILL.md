...
> `LocationScoreClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): LocationScoreClient
{
...
```php
use LocationScoreLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use LocationScoreLib\Exceptions\{OperationException};
use LocationScoreLib\Exceptions\ApiException;
...
app()->instance(LocationScoreClient::class, clientWithResponses([
