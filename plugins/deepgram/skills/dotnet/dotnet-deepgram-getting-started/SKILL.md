...
<!-- crawler:auth -->
| Auth | Credentials properties on `RestApiClientOptions`: `ApiKeyAuth: string?`, `JwtAuth: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using RestApi.Models;` alone does **not** make enums, union
...
temp directory** (`<temp>/deepgram-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/deepgram-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/deepgram-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/deepgram-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\deepgram-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/deepgram-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/deepgram-dotnet-src/`, **never** in the project repo, and its path never goes into
`deepgram-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `RestApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new RestApiClient(...)`, build its options, or DI-register via
   `AddRestApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `RestApiClientOptions` — the map's *Servers & auth* section
