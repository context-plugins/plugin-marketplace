...
`adyenbalancecontrolapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `adyenbalancecontrolapi/exceptions/` and operation docstrings.
...
```python
from adyenbalancecontrolapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `adyenbalancecontrolapi/exceptions/`.
...
```python
from adyenbalancecontrolapi.exceptions.api_exception import APIException
from adyenbalancecontrolapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`adyenbalancecontrolapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`adyenbalancecontrolapi/exceptions/` for the class.
...
`adyenbalancecontrolapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `adyenbalancecontrolapi/exceptions/`
  or a `ValueError` — check the auth handler in `adyenbalancecontrolapi/http/auth/` for the exact type.
