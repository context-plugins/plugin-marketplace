...
- `Authentication` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `authentication` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/authentication/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/authentication/configuration.rb`
(`build_default_config_from_env`) and `lib/authentication/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/authentication.rb`) and store it in a
