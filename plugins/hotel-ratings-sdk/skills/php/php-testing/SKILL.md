...
> `HotelRatingsClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): HotelRatingsClient
{
...
```php
use HotelRatingsLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use HotelRatingsLib\Exceptions\{OperationException};
use HotelRatingsLib\Exceptions\ApiException;
...
app()->instance(HotelRatingsClient::class, clientWithResponses([
