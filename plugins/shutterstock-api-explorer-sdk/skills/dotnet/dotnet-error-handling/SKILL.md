...
> `{ApiGroup}`, `ShutterstockApiExplorer`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `ShutterstockApiExplorer.Core.Exceptions`
- `ApiError` **and** `RawError` → `ShutterstockApiExplorer.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `ShutterstockApiExplorer.Errors`
...
```csharp
using ShutterstockApiExplorer.Core.Exceptions;     // SdkException<TError>
using ShutterstockApiExplorer.Core.ErrorResponse;  // ApiError, RawError
using ShutterstockApiExplorer.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`ShutterstockApiExplorer.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using ShutterstockApiExplorer.Core.Exceptions;     // SdkException<TError>
using ShutterstockApiExplorer.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `ShutterstockApiExplorer.Core.ErrorResponse`. Its public
...
`readonly struct` under `ShutterstockApiExplorer.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using ShutterstockApiExplorer.Core.Models;        // ApiResult<TResponse, TError>
using ShutterstockApiExplorer.Core.ErrorResponse; // RawError
using ShutterstockApiExplorer.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `ShutterstockApiExplorer.Core.Exceptions`) — an auth *application* failure, not an API error — when the
