...
def stub_client(stubs)
  ...
  SqlManagementClient::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises SqlManagementClient::APIException do
...
def build_test_client
  SqlManagementClient::Client.new(
    environment: SqlManagementClient::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error SqlManagementClient::APIException
```
...
```ruby
expect { call }.to raise_error(SqlManagementClient::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < SqlManagementClient::HttpCallBack
...
client  = SqlManagementClient::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
