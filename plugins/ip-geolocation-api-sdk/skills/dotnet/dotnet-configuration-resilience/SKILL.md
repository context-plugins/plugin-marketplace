...
All types below live under `IPGeolocationAPI.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using IPGeolocationAPI.Core.Configuration;
...
```csharp
using IPGeolocationAPI.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new IPGeolocationAPIClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `IPGeolocationAPI.Core.Exceptions`):
...
With DI, the SDK's `AddIPGeolocationAPIClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddIPGeolocationAPIClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
