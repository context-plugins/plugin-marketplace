...
```go
awsmigrationhub.WithOAuthCCGCredentials(
    awsmigrationhub.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr awsmigrationhub.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := awsmigrationhub.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr awsmigrationhub.OAuthCCGManager) models.OAuthToken {
...
```go
creds := awsmigrationhub.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := awsmigrationhub.NewClient(awsmigrationhub.CreateConfiguration(awsmigrationhub.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    awsmigrationhub.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := awsmigrationhub.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := awsmigrationhub.NewClient(awsmigrationhub.CreateConfiguration(awsmigrationhub.WithOAuthROPCGCredentials(creds)))
...
```go
awsmigrationhub.WithOAuthBearerTokenCredentials(awsmigrationhub.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
awsmigrationhub.WithBasicAuthCredentials(awsmigrationhub.NewBasicAuthCredentials(username, password))
awsmigrationhub.WithApiHeaderCredentials(awsmigrationhub.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
awsmigrationhub.WithApiKeyCredentials(awsmigrationhub.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `AWSMIGRATIONHUB_...` variables — e.g.
`AWSMIGRATIONHUB_USERNAME`, `AWSMIGRATIONHUB_PASSWORD`, `AWSMIGRATIONHUB_TOKEN`, `AWSMIGRATIONHUB_API_KEY`,
`AWSMIGRATIONHUB_O_AUTH_CLIENT_ID`, `AWSMIGRATIONHUB_O_AUTH_CLIENT_SECRET`, `AWSMIGRATIONHUB_ACCESS_TOKEN`. Grep the
