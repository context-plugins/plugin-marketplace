...
```go
nytimes.WithOAuthCCGCredentials(
    nytimes.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr nytimes.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := nytimes.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr nytimes.OAuthCCGManager) models.OAuthToken {
...
```go
creds := nytimes.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := nytimes.NewClient(nytimes.CreateConfiguration(nytimes.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    nytimes.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := nytimes.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := nytimes.NewClient(nytimes.CreateConfiguration(nytimes.WithOAuthROPCGCredentials(creds)))
...
```go
nytimes.WithOAuthBearerTokenCredentials(nytimes.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
nytimes.WithBasicAuthCredentials(nytimes.NewBasicAuthCredentials(username, password))
nytimes.WithApiHeaderCredentials(nytimes.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
nytimes.WithApiKeyCredentials(nytimes.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `NYTIMES_...` variables — e.g.
`NYTIMES_USERNAME`, `NYTIMES_PASSWORD`, `NYTIMES_TOKEN`, `NYTIMES_API_KEY`,
`NYTIMES_O_AUTH_CLIENT_ID`, `NYTIMES_O_AUTH_CLIENT_SECRET`, `NYTIMES_ACCESS_TOKEN`. Grep the
