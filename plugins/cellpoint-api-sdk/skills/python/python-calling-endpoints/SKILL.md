...
Open `cellpointapi/cellpointapi_client.py` for the controller property names, then the relevant
`cellpointapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from cellpointapi.models.{request_model} import {RequestModel}
...
Open `cellpointapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from cellpointapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from cellpointapi.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `cellpointapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `CellPointAPIClient` in `cellpointapi/cellpointapi_client.py`.
- Operation method signatures are in `cellpointapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `cellpointapi/models/`; enum types have `from_value()`.
