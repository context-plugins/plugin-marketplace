...
```ruby
client = ZalandoShop::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ZalandoShop::Client.new(adapter: :net_http)
```
...
client = ZalandoShop::Client.new(connection: conn)
```
...
client = ZalandoShop::Client.new(environment: Environment::PRODUCTION)
client = ZalandoShop::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ZalandoShop::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ZalandoShop::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ZalandoShop::ProxySettings.from_env
client = ZalandoShop::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ZalandoShop::HttpCallBack
...
client = ZalandoShop::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ZalandoShop::Client.from_env
# Overrides take precedence over env vars:
client = ZalandoShop::Client.from_env(timeout: 30, max_retries: 3)
```
