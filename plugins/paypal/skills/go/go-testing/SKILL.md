...
`testing` + `testify/require` for reference; substitute the real `paypal` and names.
...
import (
    ...
    "github.com/context-plugins/paypal-go-sdk"   // root package paypal
)
...
func clientReturning(status int, body string) (paypal.ClientInterface, *stubTransport) {
    ...
    client := paypal.NewClient(
        paypal.CreateConfiguration(
            paypal.WithHttpConfiguration(
                paypal.CreateHttpConfiguration(paypal.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/paypal-go-sdk/errors"
...
client := paypal.NewClient(
    paypal.CreateConfiguration(
        paypal.WithHttpConfiguration(
            paypal.CreateHttpConfiguration(paypal.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
