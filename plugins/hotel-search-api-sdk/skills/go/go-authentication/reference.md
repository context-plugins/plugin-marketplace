...
```go
hotelsearchapi.WithOAuthCCGCredentials(
    hotelsearchapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr hotelsearchapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := hotelsearchapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr hotelsearchapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := hotelsearchapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := hotelsearchapi.NewClient(hotelsearchapi.CreateConfiguration(hotelsearchapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    hotelsearchapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := hotelsearchapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := hotelsearchapi.NewClient(hotelsearchapi.CreateConfiguration(hotelsearchapi.WithOAuthROPCGCredentials(creds)))
...
```go
hotelsearchapi.WithOAuthBearerTokenCredentials(hotelsearchapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
hotelsearchapi.WithBasicAuthCredentials(hotelsearchapi.NewBasicAuthCredentials(username, password))
hotelsearchapi.WithApiHeaderCredentials(hotelsearchapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
hotelsearchapi.WithApiKeyCredentials(hotelsearchapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `HOTELSEARCHAPI_...` variables — e.g.
`HOTELSEARCHAPI_USERNAME`, `HOTELSEARCHAPI_PASSWORD`, `HOTELSEARCHAPI_TOKEN`, `HOTELSEARCHAPI_API_KEY`,
`HOTELSEARCHAPI_O_AUTH_CLIENT_ID`, `HOTELSEARCHAPI_O_AUTH_CLIENT_SECRET`, `HOTELSEARCHAPI_ACCESS_TOKEN`. Grep the
