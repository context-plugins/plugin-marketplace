...
**Match the project's existing test stack — don't impose one.** Check the test project's `package.json` and existing tests, then mirror both its **test framework** (Jest / Vitest / Mocha) and its **assertion style**. The samples below use Jest `test` + `expect` **purely for reference** — they show the SDK testing seam and *what* to assert, not a mandated framework. Substitute your `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient`/config as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the source.

## A reusable stub helper

```typescript
import { SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient } from 'spotify-web-api-with-fixes-and-improvements-from-sonalluxlib';
...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient({
...
```typescript
import { {Operation}Error } from 'spotify-web-api-with-fixes-and-improvements-from-sonalluxlib/errors';
...
```typescript
import { ApiError } from 'spotify-web-api-with-fixes-and-improvements-from-sonalluxlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient)
    .useValue(new SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
