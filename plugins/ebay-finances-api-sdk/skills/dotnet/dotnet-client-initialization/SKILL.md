...
- `EBayFinancesApiClientOptions` — its options class.
- `EBayFinancesApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddEBayFinancesApiClient`, which registers the
...
builder.Services.AddEBayFinancesApiClient(options =>
{
...
```csharp
public sealed class MyService(EBayFinancesApiClient client)
{
