...
| Options class | `EbaySellClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `EbaySellClientOptions`: `ApiAuth: OAuth2AuthorizationCodeCredentials?`, `ApiAuthTokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?`, `BearerAuth: string?`, `ApiAuth1: OAuth2ClientCredentials?`, `ApiAuth1TokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using EbaySell.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/ebay-sell-csharp-sdk
dotnet add reference ebay-sell-csharp-sdk/EbaySell.csproj
```
...
temp directory** (`<temp>/ebay-sell-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/ebay-sell-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/ebay-sell-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/ebay-sell-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\ebay-sell-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/ebay-sell-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/ebay-sell-dotnet-src/`, **never** in the project repo, and its path never goes into
`ebay-sell-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `EbaySellClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new EbaySellClient(...)`, build its options, or DI-register via
   `AddEbaySellClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `EbaySellClientOptions` — the map's *Servers & auth* section
