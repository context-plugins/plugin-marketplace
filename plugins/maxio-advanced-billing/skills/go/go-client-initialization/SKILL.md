...
- `maxioadvancedbilling` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/maxio-advanced-billing-go-sdk` — the Go import path for the root package; `github.com/context-plugins/maxio-advanced-billing-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/maxio-advanced-billing-go-sdk"

client := maxioadvancedbilling.NewClient(
    maxioadvancedbilling.CreateConfiguration(
...
from environment variables named `MAXIOADVANCEDBILLING_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := maxioadvancedbilling.NewClient(maxioadvancedbilling.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `maxioadvancedbilling.PRODUCTION`,
`maxioadvancedbilling.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := maxioadvancedbilling.NewClient(
    maxioadvancedbilling.CreateConfiguration(maxioadvancedbilling.WithEnvironment(maxioadvancedbilling.PRODUCTION)),
)
```
    ...
    maxioadvancedbilling.CreateConfiguration(
        maxioadvancedbilling.WithHttpConfiguration(
            maxioadvancedbilling.CreateHttpConfiguration(
                maxioadvancedbilling.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                maxioadvancedbilling.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = maxioadvancedbilling.NewClient(maxioadvancedbilling.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api maxioadvancedbilling.ClientInterface
}

func NewService(api maxioadvancedbilling.ClientInterface) *Service {
