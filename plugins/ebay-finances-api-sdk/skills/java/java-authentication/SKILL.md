...
To see which schemes a specific SDK accepts, read the credential setter methods on `EBayFinancesAPIClient.Builder`
...
- Set credentials **on the `EBayFinancesAPIClient.Builder`** before calling `.build()` — the client is immutable
