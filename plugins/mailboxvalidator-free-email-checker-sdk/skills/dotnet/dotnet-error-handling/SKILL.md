...
> `{ApiGroup}`, `MailboxValidatorFreeEmailChecker`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `MailboxValidatorFreeEmailChecker.Core.Exceptions`
- `ApiError` **and** `RawError` → `MailboxValidatorFreeEmailChecker.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `MailboxValidatorFreeEmailChecker.Errors`
...
```csharp
using MailboxValidatorFreeEmailChecker.Core.Exceptions;     // SdkException<TError>
using MailboxValidatorFreeEmailChecker.Core.ErrorResponse;  // ApiError, RawError
using MailboxValidatorFreeEmailChecker.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`MailboxValidatorFreeEmailChecker.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using MailboxValidatorFreeEmailChecker.Core.Exceptions;     // SdkException<TError>
using MailboxValidatorFreeEmailChecker.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `MailboxValidatorFreeEmailChecker.Core.ErrorResponse`. Its public
...
`readonly struct` under `MailboxValidatorFreeEmailChecker.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using MailboxValidatorFreeEmailChecker.Core.Models;        // ApiResult<TResponse, TError>
using MailboxValidatorFreeEmailChecker.Core.ErrorResponse; // RawError
using MailboxValidatorFreeEmailChecker.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `MailboxValidatorFreeEmailChecker.Core.Exceptions`) — an auth *application* failure, not an API error — when the
