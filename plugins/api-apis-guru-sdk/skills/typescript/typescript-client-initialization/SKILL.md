...
- `flight-choice-predictionlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'flight-choice-predictionlib';
...
```ts
import { {Resource}Controller } from 'flight-choice-predictionlib';
