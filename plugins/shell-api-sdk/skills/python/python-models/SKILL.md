...
your SDK source (`shellapi/models/`).
...
```python
from shellapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `shellapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from shellapi.models.animal import Cat, Dog
...
```python
from shellapi.models.animal import Animal, Cat, Dog
