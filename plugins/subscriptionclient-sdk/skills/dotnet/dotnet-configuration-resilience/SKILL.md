...
All types below live under `SubscriptionClient.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using SubscriptionClient.Core.Configuration;
...
```csharp
using SubscriptionClient.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new SubscriptionClientClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `SubscriptionClient.Core.Exceptions`):
...
With DI, the SDK's `AddSubscriptionClientClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddSubscriptionClientClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
