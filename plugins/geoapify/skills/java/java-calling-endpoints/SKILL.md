...
Open `GeoapifyApiClient.java` for the full list of `get*Controller()` methods. Controllers are stateless;
...
- `GeoapifyApiClient.java` — controller accessor methods; each `get{Resource}Api()` returns a
  `{Resource}Api`.
