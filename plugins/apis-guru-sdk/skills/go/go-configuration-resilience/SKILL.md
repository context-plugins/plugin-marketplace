    ...
    apisguru.CreateConfiguration(
        apisguru.WithHttpConfiguration(
            apisguru.CreateHttpConfiguration(
                apisguru.WithTimeout(30),
                apisguru.WithRetryConfiguration(
                    apisguru.CreateRetryConfiguration(
                        apisguru.WithMaxRetryAttempts(3),
                        apisguru.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := apisguru.CreateRetryConfiguration(
    apisguru.WithMaxRetryAttempts(5),
    apisguru.WithRetryInterval(1),
    apisguru.WithBackoffFactor(2),
    apisguru.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    apisguru.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := apisguru.NewClient(
    apisguru.CreateConfiguration(
        apisguru.WithHttpConfiguration(
            apisguru.CreateHttpConfiguration(
                apisguru.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
