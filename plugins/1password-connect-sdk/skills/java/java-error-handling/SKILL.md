...
│   └── local.m1password.exceptions.ApiException          ← base for all non-2xx API responses
│       └── local.m1password.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `local.m1password.http.client.HttpContext`, which carries:
