...
> `{operation}`, `{apiGroup}`, `TeslaLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `TeslaLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use TeslaLib\Exceptions\ApiException;
...
```php
use TeslaLib\Exceptions\ApiException;
use TeslaLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `TeslaLib\Exceptions\` — read the source to
