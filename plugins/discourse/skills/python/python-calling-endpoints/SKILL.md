...
Open `discourse/discourse_client.py` for the controller property names, then the relevant
`discourse/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from discourse.models.{request_model} import {RequestModel}
...
Open `discourse/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from discourse.models.suite_code_enum import SuiteCodeEnum
...
```python
from discourse.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `discourse/pagination/` in the source.

## Finding the right method

- Controller property names are on `DiscourseapidocumentationClient` in `discourse/discourse_client.py`.
- Operation method signatures are in `discourse/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `discourse/models/`; enum types have `from_value()`.
