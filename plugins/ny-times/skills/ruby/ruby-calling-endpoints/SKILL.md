...
Open `lib/ny_times/client.rb` for the accessor names, then the relevant
`lib/ny_times/controllers/{resource}_controller.rb` for the operation's exact signature.
...
Model classes extend `BaseModel` and are in `lib/ny_times/models/`. Field names are defined as
...
Read the enum module in `lib/ny_times/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/ny_times/client.rb`.
- Operation methods are in `lib/ny_times/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/ny_times/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/ny_times/exceptions/`.
