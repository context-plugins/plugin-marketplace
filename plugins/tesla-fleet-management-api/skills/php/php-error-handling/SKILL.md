...
> `{operation}`, `{apiGroup}`, `TeslaFleetManagementApiLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `TeslaFleetManagementApiLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use TeslaFleetManagementApiLib\Exceptions\ApiException;
use TeslaFleetManagementApiLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `TeslaFleetManagementApiLib\Exceptions\` — read the source to
