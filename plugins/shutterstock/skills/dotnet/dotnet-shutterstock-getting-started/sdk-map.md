...
# SDK map — Shutterstock API Explorer (.NET)
...
| Root namespace | `ShutterstockApiExplorer` |
...
All `Source` paths on this map and its sub-pages are relative to the **SDK root** — the directory holding this file and `ShutterstockApiExplorer.csproj` — never to the page that carries them. Open them as-is from the SDK root, from any page; if the SDK sits under a subdirectory of a larger repo, prefix that subdirectory.
...
DI alternative (`services.AddShutterstockApiExplorerClient`):
...
Every API group is a property on the client (e.g. `client.AudioApi`). Source: `ShutterstockApiExplorerClient.cs`. The only constructor is `ShutterstockApiExplorerClient(HttpClient httpClient, ShutterstockApiExplorerClientOptions options)`.
...
`RetryOptions` members (namespace `ShutterstockApiExplorer.Core.Configuration` — add `using ShutterstockApiExplorer.Core.Configuration;`; source: `Core/Configuration/RetryOptions.cs`; all members are `required`, so build a full instance or start from `RetryOptions.Default()`):
...
| --- | --- | --- |
| `ApiError` — abstract base of the 100 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
...
try
{
    await client.AudioApi.AddTrackCollectionItems(id, body);
}
...
**No-throw (`…Result`) variants: absent across this SDK** — every operation is throw-only. Of **109 operations**, **100 are Case A (typed)** and **9 are Case B (raw)**.
...
| --- | --- | --- |
| Records (plain `record` data models) | 155 | `Models/` |
| Unions (`AnyOf`) — variant factories + `TryGet…` | 8 | `Models/AnyOf/` |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — C# member names + wire values | 66 | `Models/Enums/` |
| Typed error classes (`: ApiError`, one per Case A operation) | 100 | `Errors/` |
...
| AnyOf unions (`Models/AnyOf/`) | `ShutterstockApiExplorer.Models.AnyOf` |
...
**Basic auth.** Set `options.Basic = new BasicAuthCredentials { Username = …, Password = … }`.

**OAuth2 (authorization code).** Set `options.CustomerAccessCode` with client id, redirect URI and a `PromptForAuthorizationCode` callback; authorization at `https://accounts.shutterstock.com/oauth/authorize`, tokens from `https://api.shutterstock.com/v2/oauth/access_token`. `options.CustomerAccessCodeTokenStrategy` overrides how tokens are acquired and cached; leave it unset for the SDK's own handling.

**Environments.** `options.Environment` selects the target environment (`Servers/ServerEnvironment.cs`):

| Environment | Value | Hosting |
| --- | --- | --- |
| `ServerEnvironment.Production` *(default)* | `production` | Live server |
| `ServerEnvironment.Environment2` | `environment2` | Sandbox server |
...
| --- | --- | --- | --- |
| `Default` | `https://api.shutterstock.com` | `https://api-sandbox.shutterstock.com` | `options.Server.Default.Production.BaseUrl` (and the other environments) |
| `AuthServer` | `https://accounts.shutterstock.com/oauth` | `https://accounts.shutterstock.com/oauth` | `options.Server.AuthServer.Production.BaseUrl` (and the other environments) |
