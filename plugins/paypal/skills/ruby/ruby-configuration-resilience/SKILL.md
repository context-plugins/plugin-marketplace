...
```ruby
client = Paypal::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Paypal::Client.new(adapter: :net_http)
```
...
client = Paypal::Client.new(connection: conn)
```
...
client = Paypal::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Paypal::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Paypal::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Paypal::ProxySettings.from_env
client = Paypal::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Paypal::HttpCallBack
...
client = Paypal::Client.new(http_callback: LogCallback.new)
```
...
client = Paypal::Client.from_env(timeout: 30, max_retries: 3)
```
