...
```go
slackwebapi.WithOAuthCCGCredentials(
    slackwebapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr slackwebapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := slackwebapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr slackwebapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := slackwebapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := slackwebapi.NewClient(slackwebapi.CreateConfiguration(slackwebapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    slackwebapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := slackwebapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := slackwebapi.NewClient(slackwebapi.CreateConfiguration(slackwebapi.WithOAuthROPCGCredentials(creds)))
...
```go
slackwebapi.WithOAuthBearerTokenCredentials(slackwebapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
slackwebapi.WithBasicAuthCredentials(slackwebapi.NewBasicAuthCredentials(username, password))
slackwebapi.WithApiHeaderCredentials(slackwebapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
slackwebapi.WithApiKeyCredentials(slackwebapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SLACKWEBAPI_...` variables — e.g.
`SLACKWEBAPI_USERNAME`, `SLACKWEBAPI_PASSWORD`, `SLACKWEBAPI_TOKEN`, `SLACKWEBAPI_API_KEY`,
`SLACKWEBAPI_O_AUTH_CLIENT_ID`, `SLACKWEBAPI_O_AUTH_CLIENT_SECRET`, `SLACKWEBAPI_ACCESS_TOKEN`. Grep the
