...
```ruby
client = TraderAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = TraderAPI::Client.new(adapter: :net_http)
```
...
client = TraderAPI::Client.new(connection: conn)
```
...
client = TraderAPI::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = TraderAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = TraderAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = TraderAPI::ProxySettings.from_env
client = TraderAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < TraderAPI::HttpCallBack
...
client = TraderAPI::Client.new(http_callback: LogCallback.new)
```
...
client = TraderAPI::Client.from_env(timeout: 30, max_retries: 3)
```
