...
Open `lib/notion/client.rb` for the accessor names, then the relevant
`lib/notion/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = Notion::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: Notion::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/notion/models/`. Field names are defined as
...
Read the enum module in `lib/notion/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/notion/client.rb`.
- Operation methods are in `lib/notion/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/notion/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/notion/exceptions/`.
