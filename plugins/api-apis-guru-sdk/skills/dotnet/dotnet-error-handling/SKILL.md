...
> `{ApiGroup}`, `FlightDelayPrediction`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FlightDelayPrediction.Core.Exceptions`
- `ApiError` **and** `RawError` → `FlightDelayPrediction.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FlightDelayPrediction.Errors`
...
```csharp
using FlightDelayPrediction.Core.Exceptions;     // SdkException<TError>
using FlightDelayPrediction.Core.ErrorResponse;  // ApiError, RawError
using FlightDelayPrediction.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FlightDelayPrediction.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FlightDelayPrediction.Core.Exceptions;     // SdkException<TError>
using FlightDelayPrediction.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FlightDelayPrediction.Core.ErrorResponse`. Its public
...
`readonly struct` under `FlightDelayPrediction.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FlightDelayPrediction.Core.Models;        // ApiResult<TResponse, TError>
using FlightDelayPrediction.Core.ErrorResponse; // RawError
using FlightDelayPrediction.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FlightDelayPrediction.Core.Exceptions`) — an auth *application* failure, not an API error — when the
