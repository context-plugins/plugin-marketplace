...
kwargs to the `SlackWebAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `slackwebapi/slackwebapi_client.py` and `slackwebapi/http/auth/` in
the cloned source.

## Basic auth

```python
from slackwebapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from slackwebapi.http.auth.api_key import ApiKeyCredentials
...
```python
from slackwebapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from slackwebapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from slackwebapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from slackwebapi.http.auth.o_auth_acg import OAuthACGCredentials
from slackwebapi.models.o_auth_scope_slackwebapi_enum import OAuthScope{Pkg}Enum
...
client = SlackWebAPIClient(o_auth_acg_credentials=creds)
...
```python
from slackwebapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SlackWebAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SlackWebAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `slackwebapi/configuration.py` and each `slackwebapi/http/auth/*.py` for the exact
