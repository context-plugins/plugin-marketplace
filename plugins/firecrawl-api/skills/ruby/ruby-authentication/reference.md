...
Confirm every name against `lib/firecrawl_api/client.rb`, `lib/firecrawl_api/configuration.rb`, the
`lib/firecrawl_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
