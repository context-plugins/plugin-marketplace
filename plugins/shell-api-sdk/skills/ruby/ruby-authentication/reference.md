...
Confirm every name against `lib/shell_api/client.rb`, `lib/shell_api/configuration.rb`, the
`lib/shell_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
