...
`testing` + `testify/require` for reference; substitute the real `paze` and names.
...
import (
    ...
    "github.com/context-plugins/paze-go-sdk"   // root package paze
)
...
func clientReturning(status int, body string) (paze.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := paze.NewClient(
        paze.CreateConfiguration(
            paze.WithHttpConfiguration(
                paze.CreateHttpConfiguration(paze.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/paze-go-sdk/errors"
...
client := paze.NewClient(
    paze.CreateConfiguration(
        paze.WithHttpConfiguration(
            paze.CreateHttpConfiguration(paze.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
