...
> Throughout, `goe` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := goe.NewClient(
    goe.CreateConfiguration(
        goe.WithBasicAuthCredentials(
            goe.NewBasicAuthCredentials(
...
goe.WithApiKeyCredentials(goe.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
goe.WithApiHeaderCredentials(goe.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
goe.WithOAuthBearerTokenCredentials(
    goe.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
