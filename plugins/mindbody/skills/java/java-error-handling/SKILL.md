...
│   └── com.mindbodyonline.pushapi.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.mindbodyonline.pushapi.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.mindbodyonline.pushapi.http.client.HttpContext`, which carries:
