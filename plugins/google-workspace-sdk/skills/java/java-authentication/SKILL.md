...
To see which schemes a specific SDK accepts, read the credential setter methods on `GoogleWorkspaceClient.Builder`
...
- Set credentials **on the `GoogleWorkspaceClient.Builder`** before calling `.build()` — the client is immutable
