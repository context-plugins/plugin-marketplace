...
All types below live under `ConfigurationApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using ConfigurationApi.Core.Configuration;
...
```csharp
using ConfigurationApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new ConfigurationApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `ConfigurationApi.Core.Exceptions`):
...
With DI, the SDK's `AddConfigurationApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddConfigurationApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
