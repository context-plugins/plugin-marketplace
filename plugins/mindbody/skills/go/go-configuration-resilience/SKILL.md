    ...
    mindbody.CreateConfiguration(
        mindbody.WithHttpConfiguration(
            mindbody.CreateHttpConfiguration(
                mindbody.WithTimeout(30),
                mindbody.WithRetryConfiguration(
                    mindbody.CreateRetryConfiguration(
                        mindbody.WithMaxRetryAttempts(3),
                        mindbody.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := mindbody.CreateRetryConfiguration(
    mindbody.WithMaxRetryAttempts(5),
    mindbody.WithRetryInterval(1),
    mindbody.WithBackoffFactor(2),
    mindbody.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    mindbody.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := mindbody.NewClient(
    mindbody.CreateConfiguration(
        mindbody.WithHttpConfiguration(
            mindbody.CreateHttpConfiguration(
                mindbody.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
