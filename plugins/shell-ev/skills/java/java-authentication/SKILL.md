...
To see which schemes a specific SDK accepts, read the credential setter methods on `ShellEVClient.Builder`
...
- Set credentials **on the `ShellEVClient.Builder`** before calling `.build()` — the client is immutable
