...
```go
flightcreateorders.WithOAuthCCGCredentials(
    flightcreateorders.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr flightcreateorders.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := flightcreateorders.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr flightcreateorders.OAuthCCGManager) models.OAuthToken {
...
```go
creds := flightcreateorders.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := flightcreateorders.NewClient(flightcreateorders.CreateConfiguration(flightcreateorders.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    flightcreateorders.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := flightcreateorders.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := flightcreateorders.NewClient(flightcreateorders.CreateConfiguration(flightcreateorders.WithOAuthROPCGCredentials(creds)))
...
```go
flightcreateorders.WithOAuthBearerTokenCredentials(flightcreateorders.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
flightcreateorders.WithBasicAuthCredentials(flightcreateorders.NewBasicAuthCredentials(username, password))
flightcreateorders.WithApiHeaderCredentials(flightcreateorders.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
flightcreateorders.WithApiKeyCredentials(flightcreateorders.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FLIGHTCREATEORDERS_...` variables — e.g.
`FLIGHTCREATEORDERS_USERNAME`, `FLIGHTCREATEORDERS_PASSWORD`, `FLIGHTCREATEORDERS_TOKEN`, `FLIGHTCREATEORDERS_API_KEY`,
`FLIGHTCREATEORDERS_O_AUTH_CLIENT_ID`, `FLIGHTCREATEORDERS_O_AUTH_CLIENT_SECRET`, `FLIGHTCREATEORDERS_ACCESS_TOKEN`. Grep the
