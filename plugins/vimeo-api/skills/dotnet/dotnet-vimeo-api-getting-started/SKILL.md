...
| --- | --- |
| API | Vimeo API |
| Source repo | https://github.com/context-plugins/vimeo-api-csharp-sdk (branch `main`) |
| Root namespace | `VimeoApi` (the `using` namespace) |
| Client class | `VimeoApiClient` |
| Options class | `VimeoApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `VimeoApiClientOptions`: `Bearer: string?`, `Oauth2AuthorizationCode: OAuth2AuthorizationCodeCredentials?`, `Oauth2AuthorizationCodeTokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?`, `Oauth2ClientCredentials: OAuth2ClientCredentials?`, `Oauth2ClientCredentialsTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using VimeoApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/vimeo-api-csharp-sdk
dotnet add reference vimeo-api-csharp-sdk/VimeoApi.csproj
```
...
temp directory** (`<temp>/vimeo-api-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/vimeo-api-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/vimeo-api-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-api-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\vimeo-api-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-api-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/vimeo-api-dotnet-src/`, **never** in the project repo, and its path never goes into
`vimeo-api-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new VimeoApiClient(...)`, build its options, or DI-register via
   `AddVimeoApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `VimeoApiClientOptions` — the map's *Servers & auth* section
