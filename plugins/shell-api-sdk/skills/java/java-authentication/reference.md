...
against the `ShellAPIClient.Builder` setters, the `com.shell.apitest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ShellAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ShellAPIClient client = new ShellAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeShellAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
