...
def stub_client(stubs)
  ...
  FlightOrderManagement::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises FlightOrderManagement::APIException do
...
def build_test_client
  FlightOrderManagement::Client.new(
    environment: FlightOrderManagement::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error FlightOrderManagement::APIException
```
...
```ruby
expect { call }.to raise_error(FlightOrderManagement::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < FlightOrderManagement::HttpCallBack
...
client  = FlightOrderManagement::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
