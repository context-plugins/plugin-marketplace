...
│   └── com.ebay.apiz.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.ebay.apiz.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.ebay.apiz.http.client.HttpContext`, which carries:
