...
description: Construct and configure an APIMatic-generated Java SDK client — build via the inner MaxioAdvancedBillingClient.Builder() class (never a public constructor), set httpClientConfig via a lambda (not a config object), choose an Environment constant, access controllers via client.get{Resource}Api(), call client.shutdown() at process exit, clone a live client with client.newBuilder(), and wire the client into Spring or plain-constructor DI. Use the moment you call new MaxioAdvancedBillingClient.Builder(), pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the arguments but not the builder pattern, the httpClientConfig lambda, or the lifetime/reuse rules.
---

# Initializing an APIMatic Java SDK client

> `MaxioAdvancedBillingClient` is the SDK's client class — **read the real name** from the `*Client.java` file in
...
- `MaxioAdvancedBillingClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `com.chargify.subdomain` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`MaxioAdvancedBillingClient.Builder` class. A minimal initialization:

```java
import com.chargify.subdomain.MaxioAdvancedBillingClient;
import com.chargify.subdomain.Environment;

MaxioAdvancedBillingClient client = new MaxioAdvancedBillingClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `MaxioAdvancedBillingClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `MaxioAdvancedBillingClient.java` |

Call `.build()` to produce an immutable `MaxioAdvancedBillingClient` instance.
...
public class ApiConfig {
    @Bean
    public MaxioAdvancedBillingClient apiClient() {
        return new MaxioAdvancedBillingClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired MaxioAdvancedBillingClient client) {
        client.shutdown();
    }
}
```

Inject `MaxioAdvancedBillingClient` into your services as a normal Spring dependency. Build only one bean.
