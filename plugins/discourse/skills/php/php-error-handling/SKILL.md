...
> `{operation}`, `{apiGroup}`, `DiscourseLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `DiscourseLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use DiscourseLib\Exceptions\ApiException;
...
```php
use DiscourseLib\Exceptions\ApiException;
use DiscourseLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `DiscourseLib\Exceptions\` — read the source to
