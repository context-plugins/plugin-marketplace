...
kwargs to the `Xapiv2Client` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `xapiv2/xapiv2_client.py` and `xapiv2/http/auth/` in
the cloned source.

## Basic auth

```python
from xapiv2.http.auth.basic_auth import BasicAuthCredentials
...
```python
from xapiv2.http.auth.api_key import ApiKeyCredentials
...
```python
from xapiv2.http.auth.api_header import ApiHeaderCredentials
...
```python
from xapiv2.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from xapiv2.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from xapiv2.http.auth.o_auth_acg import OAuthACGCredentials
from xapiv2.models.o_auth_scope_xapiv2_enum import OAuthScope{Pkg}Enum
...
client = Xapiv2Client(o_auth_acg_credentials=creds)
...
```python
from xapiv2.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = Xapiv2Client(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `Xapiv2Client.from_environment()`) read each credential
...
Grep `from_environment` in `xapiv2/configuration.py` and each `xapiv2/http/auth/*.py` for the exact
