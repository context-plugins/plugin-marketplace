...
Confirm every name against `lib/fund_api/client.rb`, `lib/fund_api/configuration.rb`, the
`lib/fund_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
