...
│   └── com.ebay.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.ebay.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.ebay.api.http.client.HttpContext`, which carries:
