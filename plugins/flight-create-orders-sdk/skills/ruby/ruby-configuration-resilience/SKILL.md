...
```ruby
client = FlightCreateOrders::Client.new(
  max_retries:    3,
...
```ruby
client = FlightCreateOrders::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = FlightCreateOrders::Client.new(adapter: :net_http)
```
...
client = FlightCreateOrders::Client.new(connection: conn)
```
...
client = FlightCreateOrders::Client.new(environment: Environment::PRODUCTION)
client = FlightCreateOrders::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = FlightCreateOrders::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = FlightCreateOrders::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = FlightCreateOrders::ProxySettings.from_env
client = FlightCreateOrders::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < FlightCreateOrders::HttpCallBack
...
client = FlightCreateOrders::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = FlightCreateOrders::Client.from_env
# Overrides take precedence over env vars:
client = FlightCreateOrders::Client.from_env(timeout: 30, max_retries: 3)
```
