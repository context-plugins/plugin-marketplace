...
- `safe-placelib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'safe-placelib';
...
```ts
import { {Resource}Controller } from 'safe-placelib';
