...
`VimeoClient`/`VimeoClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `VimeoClient`,
...
static VimeoClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new VimeoClient(new HttpClient(handler), new VimeoClientOptions { /* auth not needed for stubs */ });
}
```
...
using Vimeo.Errors;              // {Operation}Error types
...
using Vimeo.Core.ErrorResponse; // RawError (Case B)
...
var client = new VimeoClient(new HttpClient(handler), new VimeoClientOptions());
...
- For DI-based code, the SDK's `AddVimeoClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `VimeoClient` from the provider:
  ```csharp
  services.AddVimeoClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<VimeoClient>();
  ```
