...
> `FlightOffersPriceClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FlightOffersPriceClient
{
...
```php
use FlightOffersPriceLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use FlightOffersPriceLib\Exceptions\{OperationException};
use FlightOffersPriceLib\Exceptions\ApiException;
...
app()->instance(FlightOffersPriceClient::class, clientWithResponses([
