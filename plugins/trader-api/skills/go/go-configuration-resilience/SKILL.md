    ...
    traderapi.CreateConfiguration(
        traderapi.WithHttpConfiguration(
            traderapi.CreateHttpConfiguration(
                traderapi.WithTimeout(30),
                traderapi.WithRetryConfiguration(
                    traderapi.CreateRetryConfiguration(
                        traderapi.WithMaxRetryAttempts(3),
                        traderapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := traderapi.CreateRetryConfiguration(
    traderapi.WithMaxRetryAttempts(5),
    traderapi.WithRetryInterval(1),
    traderapi.WithBackoffFactor(2),
    traderapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    traderapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := traderapi.NewClient(
    traderapi.CreateConfiguration(
        traderapi.WithHttpConfiguration(
            traderapi.CreateHttpConfiguration(
                traderapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
