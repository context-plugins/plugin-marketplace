...
Open `lib/ebay_sell/client.rb` for the accessor names, then the relevant
`lib/ebay_sell/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = EbaySell::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: EbaySell::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/ebay_sell/models/`. Field names are defined as
...
Read the enum module in `lib/ebay_sell/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/ebay_sell/client.rb`.
- Operation methods are in `lib/ebay_sell/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/ebay_sell/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/ebay_sell/exceptions/`.
