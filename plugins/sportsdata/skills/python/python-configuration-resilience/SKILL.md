...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `SportsdataClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`sportsdata/configuration.py` in the cloned source.
...
```python
import requests
from sportsdata.configuration import Configuration
...
`sportsdata/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`sportsdata/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `sportsdata/configuration.py`
...
`NumberPagedResponse`) live in `sportsdata/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `sportsdata/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
