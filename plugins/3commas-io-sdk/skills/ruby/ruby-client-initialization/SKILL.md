...
- `3commasIo` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `3commas_io` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/3commas_io/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/3commas_io/configuration.rb`
(`build_default_config_from_env`) and `lib/3commas_io/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/3commas_io.rb`) and store it in a
