...
```typescript
import { {Operation}Error } from 'dockerlib/errors';
...
```typescript
import { ApiError } from 'dockerlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(DockerClient)
    .useValue(new DockerClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
