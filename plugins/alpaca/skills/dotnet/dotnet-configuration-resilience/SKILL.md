...
All types below live under `TraderApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using TraderApi.Core.Configuration;
...
```csharp
using TraderApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new TraderApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `TraderApi.Core.Exceptions`):
...
With DI, the SDK's `AddTraderApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddTraderApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
