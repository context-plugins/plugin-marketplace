...
client = stub_client(stubs)
assert_raises EbaySell::APIException do
...
BASE = EbaySell::Configuration.new(environment: EbaySell::Environment::PRODUCTION).get_base_uri

def build_test_client
  EbaySell::Client.new(
    environment: EbaySell::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error EbaySell::APIException
```
...
```ruby
expect { call }.to raise_error(EbaySell::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < EbaySell::HttpCallBack
...
client  = EbaySell::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
