...
All types below live under `FlightOffersPrice.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FlightOffersPrice.Core.Configuration;
...
```csharp
using FlightOffersPrice.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FlightOffersPriceClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FlightOffersPrice.Core.Exceptions`):
...
With DI, the SDK's `AddFlightOffersPriceClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFlightOffersPriceClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
