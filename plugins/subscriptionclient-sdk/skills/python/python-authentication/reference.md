...
kwargs to the `SubscriptionClientClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `subscriptionclient/subscriptionclient_client.py` and `subscriptionclient/http/auth/` in
the cloned source.

## Basic auth

```python
from subscriptionclient.http.auth.basic_auth import BasicAuthCredentials
...
```python
from subscriptionclient.http.auth.api_key import ApiKeyCredentials
...
```python
from subscriptionclient.http.auth.api_header import ApiHeaderCredentials
...
```python
from subscriptionclient.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from subscriptionclient.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from subscriptionclient.http.auth.o_auth_acg import OAuthACGCredentials
from subscriptionclient.models.o_auth_scope_subscriptionclient_enum import OAuthScope{Pkg}Enum
...
client = SubscriptionClientClient(o_auth_acg_credentials=creds)
...
```python
from subscriptionclient.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SubscriptionClientClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SubscriptionClientClient.from_environment()`) read each credential
...
Grep `from_environment` in `subscriptionclient/configuration.py` and each `subscriptionclient/http/auth/*.py` for the exact
