...
Open `verizonapi/verizonapi_client.py` for the controller property names, then the relevant
`verizonapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from verizonapi.models.{request_model} import {RequestModel}
...
Open `verizonapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from verizonapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from verizonapi.http.http_call_back import HttpCallBack
...
client = VerizonAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `verizonapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `VerizonAPIClient` in `verizonapi/verizonapi_client.py`.
- Operation method signatures are in `verizonapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `verizonapi/models/`; enum types have `from_value()`.
