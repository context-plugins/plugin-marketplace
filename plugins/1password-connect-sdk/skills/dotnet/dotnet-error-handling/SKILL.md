...
> `{ApiGroup}`, `PasswordConnect1`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `PasswordConnect1.Core.Exceptions`
- `ApiError` **and** `RawError` → `PasswordConnect1.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `PasswordConnect1.Errors`
...
```csharp
using PasswordConnect1.Core.Exceptions;     // SdkException<TError>
using PasswordConnect1.Core.ErrorResponse;  // ApiError, RawError
using PasswordConnect1.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`PasswordConnect1.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using PasswordConnect1.Core.Exceptions;     // SdkException<TError>
using PasswordConnect1.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `PasswordConnect1.Core.ErrorResponse`. Its public
...
`readonly struct` under `PasswordConnect1.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using PasswordConnect1.Core.Models;        // ApiResult<TResponse, TError>
using PasswordConnect1.Core.ErrorResponse; // RawError
using PasswordConnect1.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `PasswordConnect1.Core.Exceptions`) — an auth *application* failure, not an API error — when the
