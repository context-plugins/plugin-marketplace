...
description: "Frankfurter API TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `frankfurter-api`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any Frankfurter API TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the Frankfurter API TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Frankfurter API TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-frankfurter` for the workflow that wraps this skill.
...
Verified against `package.json` and `sdk-map.md` of the generated package at version `2.1.1`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Frankfurter API |
| Package name (what you install, and what you import) | `frankfurter-api` — **not on npm**; built from source (see *Install*) |
| Import specifier | `frankfurter-api` — the package root is the **only** entry; deep imports do not resolve |
| Version | `2.1.1` (API spec version `2.1.1`) |
| Client class | `FrankfurterApiClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new FrankfurterApiClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new FrankfurterApiClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch`. `timeout` defaults to `60_000` ms |
...
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://api.frankfurter.dev/v2` |
...
| Surface | 5 operations · 0 resources · 12 models · 4 open enums · 3 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/frankfurter-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 16 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { FrankfurterApiClient, ServerEnvironment, ResponseError, FrankfurterApiError } from "frankfurter-api";
import type { ClientOptions, Currency } from "frankfurter-api";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `frankfurter-api/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **From CommonJS**, the typed spelling is `import sdk = require("frankfurter-api")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://api.frankfurter.dev/v2` | `serverOptions.default.production.baseUrl` |
...
5 operations are methods on the client itself: `client.getCurrencies` · `client.getCurrency` · `client.getProviders` · `client.getRate` · `client.getRates`.
...
```bash
node -e "console.log(require.resolve('frankfurter-api/package.json'))"
```

Failing that it is at `node_modules/frankfurter-api/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/frankfurter-api/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/frankfurter-api/`:
...
1. **Client construction & lifetime** — load **typescript-client-initialization** before you write `new FrankfurterApiClient(…)`. (*The signature won't tell you:* every option is optional, so a client built with no arguments compiles and talks to the default environment with no credential; the client must be **long-lived and app-scoped**, never rebuilt per request, because the resolved options and the server resolvers live on it; there is no `close()` or `dispose()` — it owns no pool, only a `fetch`; and when no `fetch` is reachable the **constructor** throws `SdkError`, not the first call.)
...
Beyond the usual signatures and model members, a contract sheet for the Frankfurter API TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
