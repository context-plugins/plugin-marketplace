...
> Throughout, `notion` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := notion.NewClient(
    notion.CreateConfiguration(
        notion.WithBasicAuthCredentials(
            notion.NewBasicAuthCredentials(
...
notion.WithApiKeyCredentials(notion.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
notion.WithApiHeaderCredentials(notion.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
notion.WithOAuthBearerTokenCredentials(
    notion.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
