...
> `APIsGuruClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): APIsGuruClient
{
...
```php
use APIsGuruLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use APIsGuruLib\Exceptions\{OperationException};
use APIsGuruLib\Exceptions\ApiException;
...
app()->instance(APIsGuruClient::class, clientWithResponses([
