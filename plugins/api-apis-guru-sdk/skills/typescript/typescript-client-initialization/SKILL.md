...
- `tours-and-activitieslib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'tours-and-activitieslib';
...
```ts
import { {Resource}Controller } from 'tours-and-activitieslib';
