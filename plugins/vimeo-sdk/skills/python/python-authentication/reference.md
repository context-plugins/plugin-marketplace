...
kwargs to the `VimeoClient` constructor (or to `Configuration`). Exact credential class names and
...
client = VimeoClient(o_auth_acg_credentials=creds)
...
client = VimeoClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `VimeoClient.from_environment()`) read each credential
