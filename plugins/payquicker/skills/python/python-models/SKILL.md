...
your SDK source (`payquicker/models/`).
...
```python
from payquicker.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `payquicker/utilities/union_type_lookup.py`) and each candidate model
...
```python
from payquicker.models.animal import Cat, Dog
...
```python
from payquicker.models.animal import Animal, Cat, Dog
