...
> `GoeapiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): GoeapiClient
{
...
```php
use GoeapiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use GoeapiLib\Exceptions\{OperationException};
use GoeapiLib\Exceptions\ApiException;
...
app()->instance(GoeapiClient::class, clientWithResponses([
