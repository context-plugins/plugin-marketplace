...
`MailboxValidatorFreeEmailCheckerClient`/`MailboxValidatorFreeEmailCheckerClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `MailboxValidatorFreeEmailCheckerClient`,
...
static MailboxValidatorFreeEmailCheckerClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new MailboxValidatorFreeEmailCheckerClient(new HttpClient(handler), new MailboxValidatorFreeEmailCheckerClientOptions { /* auth not needed for stubs */ });
}
```
...
using MailboxValidatorFreeEmailChecker.Errors;              // {Operation}Error types
...
```csharp
using MailboxValidatorFreeEmailChecker.Core.Exceptions;
using MailboxValidatorFreeEmailChecker.Core.ErrorResponse;
...
using MailboxValidatorFreeEmailChecker.Core.ErrorResponse; // RawError (Case B)
...
var client = new MailboxValidatorFreeEmailCheckerClient(new HttpClient(handler), new MailboxValidatorFreeEmailCheckerClientOptions());
...
- For DI-based code, the SDK's `AddMailboxValidatorFreeEmailCheckerClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `MailboxValidatorFreeEmailCheckerClient` from the provider:
  ```csharp
  services.AddMailboxValidatorFreeEmailCheckerClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<MailboxValidatorFreeEmailCheckerClient>();
  ```
