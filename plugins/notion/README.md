...
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, Java, Python, Ruby, Go.
...
(`dotnet-notion-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-notion`** router skill routes all
C#/.NET SDK work to the single **`dotnet-notion-sdk`** agent (.NET-only), which answers
