    ...
    paypalapi.CreateConfiguration(
        paypalapi.WithHttpConfiguration(
            paypalapi.CreateHttpConfiguration(
                paypalapi.WithTimeout(30),
                paypalapi.WithRetryConfiguration(
                    paypalapi.CreateRetryConfiguration(
                        paypalapi.WithMaxRetryAttempts(3),
                        paypalapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := paypalapi.CreateRetryConfiguration(
    paypalapi.WithMaxRetryAttempts(5),
    paypalapi.WithRetryInterval(1),
    paypalapi.WithBackoffFactor(2),
    paypalapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    paypalapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := paypalapi.NewClient(
    paypalapi.CreateConfiguration(
        paypalapi.WithHttpConfiguration(
            paypalapi.CreateHttpConfiguration(
                paypalapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
