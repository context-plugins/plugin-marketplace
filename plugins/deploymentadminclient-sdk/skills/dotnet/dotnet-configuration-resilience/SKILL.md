...
All types below live under `DeploymentAdminClient.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using DeploymentAdminClient.Core.Configuration;
...
```csharp
using DeploymentAdminClient.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new DeploymentAdminClientClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `DeploymentAdminClient.Core.Exceptions`):
...
With DI, the SDK's `AddDeploymentAdminClientClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddDeploymentAdminClientClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
