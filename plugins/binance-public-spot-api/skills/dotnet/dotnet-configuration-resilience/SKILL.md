...
All types below live under `BinancePublicSpotApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using BinancePublicSpotApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new BinancePublicSpotApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `BinancePublicSpotApi.Core.Exceptions`):
...
With DI, the SDK's `AddBinancePublicSpotApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddBinancePublicSpotApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
