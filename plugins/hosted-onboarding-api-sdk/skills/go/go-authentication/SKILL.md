...
> Throughout, `hostedonboardingapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := hostedonboardingapi.NewClient(
    hostedonboardingapi.CreateConfiguration(
        hostedonboardingapi.WithBasicAuthCredentials(
            hostedonboardingapi.NewBasicAuthCredentials(
...
hostedonboardingapi.WithApiKeyCredentials(hostedonboardingapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
hostedonboardingapi.WithApiHeaderCredentials(hostedonboardingapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
hostedonboardingapi.WithOAuthBearerTokenCredentials(
    hostedonboardingapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := hostedonboardingapi.NewClient(
    hostedonboardingapi.CreateConfiguration(
        hostedonboardingapi.WithOAuthCCGCredentials(
            hostedonboardingapi.NewOAuthCCGCredentials(
