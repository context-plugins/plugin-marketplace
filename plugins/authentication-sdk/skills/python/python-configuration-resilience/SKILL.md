...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AuthenticationClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`authentication/configuration.py` in the cloned source.
...
`authentication/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`authentication/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `authentication/configuration.py`
...
`NumberPagedResponse`) live in `authentication/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `authentication/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
