...
> `{ApiGroup}`, `LocationScore`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `LocationScore.Core.Exceptions`
- `ApiError` **and** `RawError` → `LocationScore.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `LocationScore.Errors`
...
```csharp
using LocationScore.Core.Exceptions;     // SdkException<TError>
using LocationScore.Core.ErrorResponse;  // ApiError, RawError
using LocationScore.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`LocationScore.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using LocationScore.Core.Exceptions;     // SdkException<TError>
using LocationScore.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `LocationScore.Core.ErrorResponse`. Its public
...
`readonly struct` under `LocationScore.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using LocationScore.Core.Models;        // ApiResult<TResponse, TError>
using LocationScore.Core.ErrorResponse; // RawError
using LocationScore.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `LocationScore.Core.Exceptions`) — an auth *application* failure, not an API error — when the
