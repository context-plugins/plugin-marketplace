...
| Root namespace | `Finnhub` (the `using` namespace) |
| Client class | `FinnhubClient` |
| Options class | `FinnhubClientOptions` |
...
| Log environment variable | `FINNHUBCLIENT_LOG` |
...
The SDK splits its public types across **separate child namespaces**. C# does **not** import child namespaces transitively, so `using Finnhub.Models;` alone does **not** make enums, union types, or error types visible — you get `CS0103`/`CS0246` ("name/type does not exist") on build. Add a separate `using` for each kind of type you reference:

| Type | Namespace |
| --- | --- |
| `FinnhubClient` | `Finnhub` |
| `FinnhubClientOptions` | `Finnhub` |
| `ServerEnvironment` | `Finnhub.Servers` |
...
```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\finnhub-sdk-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --filter=blob:none --branch main https://github.com/context-plugins/finnhub-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```
...
The clone lives in `<temp>/finnhub-sdk-src/`, **never** in the project repo, and its path never goes into `finnhub-plan.md` — the plan must stay portable, and the clone is a session-local reference, not part of the project.
...
- `Servers/`, `FinnhubClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
The generator injects an `Idempotency-Key: Guid.NewGuid()` header on **every non-GET operation** — fresh on every call, invisible to you, and **not** an idempotency key in any meaningful sense: a value that changes per call deduplicates nothing, and Finnhub may not document reading that header at all. `dotnet-configuration-resilience` explains why a visible header like this is worse than an absent one.
