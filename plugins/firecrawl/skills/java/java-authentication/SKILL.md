...
To see which schemes a specific SDK accepts, read the credential setter methods on `FirecrawlApiClient.Builder`
...
- Set credentials **on the `FirecrawlApiClient.Builder`** before calling `.build()` — the client is immutable
