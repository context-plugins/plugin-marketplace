...
- `squareconnectapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `SquareConnectAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `SquareConnectAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from squareconnectapi.squareconnectapi_client import SquareConnectAPIClient
from squareconnectapi.configuration import Environment
from squareconnectapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = SquareConnectAPIClient(
...
```python
from squareconnectapi.configuration import Configuration, Environment
...
client = SquareConnectAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `squareconnectapi/squareconnectapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `SquareConnectAPIClient` and `Configuration` that reads
...
client = SquareConnectAPIClient.from_environment()
...
`from_environment` in `squareconnectapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `SquareConnectAPIClient`. Access them as attributes —
...
Open `squareconnectapi/squareconnectapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `squareconnectapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from squareconnectapi.configuration import Environment

client = SquareConnectAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`squareconnectapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = SquareConnectAPIClient(config=new_config)
```
