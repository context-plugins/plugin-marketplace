...
To see which schemes a specific SDK accepts, read the credential setter methods on `AdyenClient.Builder`
...
- Set credentials **on the `AdyenClient.Builder`** before calling `.build()` — the client is immutable
