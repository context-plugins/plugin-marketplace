...
```go
discourseapidocumentation.WithOAuthCCGCredentials(
    discourseapidocumentation.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr discourseapidocumentation.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := discourseapidocumentation.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr discourseapidocumentation.OAuthCCGManager) models.OAuthToken {
...
```go
creds := discourseapidocumentation.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := discourseapidocumentation.NewClient(discourseapidocumentation.CreateConfiguration(discourseapidocumentation.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    discourseapidocumentation.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := discourseapidocumentation.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := discourseapidocumentation.NewClient(discourseapidocumentation.CreateConfiguration(discourseapidocumentation.WithOAuthROPCGCredentials(creds)))
...
```go
discourseapidocumentation.WithOAuthBearerTokenCredentials(discourseapidocumentation.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
discourseapidocumentation.WithBasicAuthCredentials(discourseapidocumentation.NewBasicAuthCredentials(username, password))
discourseapidocumentation.WithApiHeaderCredentials(discourseapidocumentation.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
discourseapidocumentation.WithApiKeyCredentials(discourseapidocumentation.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `DISCOURSEAPIDOCUMENTATION_...` variables — e.g.
`DISCOURSEAPIDOCUMENTATION_USERNAME`, `DISCOURSEAPIDOCUMENTATION_PASSWORD`, `DISCOURSEAPIDOCUMENTATION_TOKEN`, `DISCOURSEAPIDOCUMENTATION_API_KEY`,
`DISCOURSEAPIDOCUMENTATION_O_AUTH_CLIENT_ID`, `DISCOURSEAPIDOCUMENTATION_O_AUTH_CLIENT_SECRET`, `DISCOURSEAPIDOCUMENTATION_ACCESS_TOKEN`. Grep the
