...
```go
accessanalyzer.WithOAuthCCGCredentials(
    accessanalyzer.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr accessanalyzer.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := accessanalyzer.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr accessanalyzer.OAuthCCGManager) models.OAuthToken {
...
```go
creds := accessanalyzer.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := accessanalyzer.NewClient(accessanalyzer.CreateConfiguration(accessanalyzer.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    accessanalyzer.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := accessanalyzer.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := accessanalyzer.NewClient(accessanalyzer.CreateConfiguration(accessanalyzer.WithOAuthROPCGCredentials(creds)))
...
```go
accessanalyzer.WithOAuthBearerTokenCredentials(accessanalyzer.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
accessanalyzer.WithBasicAuthCredentials(accessanalyzer.NewBasicAuthCredentials(username, password))
accessanalyzer.WithApiHeaderCredentials(accessanalyzer.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
accessanalyzer.WithApiKeyCredentials(accessanalyzer.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `ACCESSANALYZER_...` variables — e.g.
`ACCESSANALYZER_USERNAME`, `ACCESSANALYZER_PASSWORD`, `ACCESSANALYZER_TOKEN`, `ACCESSANALYZER_API_KEY`,
`ACCESSANALYZER_O_AUTH_CLIENT_ID`, `ACCESSANALYZER_O_AUTH_CLIENT_SECRET`, `ACCESSANALYZER_ACCESS_TOKEN`. Grep the
