...
| --- | --- |
| API | `api.apis.guru` |
| Project | `HotelBooking.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `HotelBooking` (the `using` namespace) |
| Client class | `HotelBookingClient` |
| Options class | `HotelBookingClientOptions` |
...
namespaces transitively, so `using HotelBooking.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/api-apis-guru-csharp-6418e6
dotnet add reference api-apis-guru-csharp-6418e6/HotelBooking.csproj
```

```csharp
using HotelBooking;          // client + options + ServerEnvironment
using HotelBooking.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-6418e6 /tmp/api-apis-guru-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-6418e6 "$env:TEMP\api-apis-guru-dotnet-src"
```
...
- `HotelBookingClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `HotelBookingClient`, build its
   `HotelBookingClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
