...
- `firecrawl` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'firecrawl';
...
```ts
import { {Resource}Api } from 'firecrawl';
