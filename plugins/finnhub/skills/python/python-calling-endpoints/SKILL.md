...
Open `finnhub/finnhub_client.py` for the controller property names, then the relevant
`finnhub/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from finnhub.models.{request_model} import {RequestModel}
...
Open `finnhub/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from finnhub.models.suite_code_enum import SuiteCodeEnum
...
```python
from finnhub.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `finnhub/pagination/` in the source.

## Finding the right method

- Controller property names are on `FinnhubapiClient` in `finnhub/finnhub_client.py`.
- Operation method signatures are in `finnhub/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `finnhub/models/`; enum types have `from_value()`.
