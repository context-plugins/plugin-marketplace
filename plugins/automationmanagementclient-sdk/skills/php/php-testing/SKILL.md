...
> `AutomationManagementClientClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AutomationManagementClientClient
{
...
```php
use AutomationManagementClientLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AutomationManagementClientLib\Exceptions\{OperationException};
use AutomationManagementClientLib\Exceptions\ApiException;
...
app()->instance(AutomationManagementClientClient::class, clientWithResponses([
