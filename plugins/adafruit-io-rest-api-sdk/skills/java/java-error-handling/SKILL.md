...
│   └── com.adafruit.io.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.adafruit.io.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.adafruit.io.http.client.HttpContext`, which carries:
