...
To see which schemes a specific SDK accepts, read the credential setter methods on `PostnlEcommerceClient.Builder`
...
- Set credentials **on the `PostnlEcommerceClient.Builder`** before calling `.build()` — the client is immutable
