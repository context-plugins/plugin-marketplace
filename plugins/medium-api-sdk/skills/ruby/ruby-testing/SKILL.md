...
client = stub_client(stubs)
assert_raises MediumAPI::APIException do
...
def build_test_client
  MediumAPI::Client.new(
    environment: MediumAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error MediumAPI::APIException
```
...
```ruby
expect { call }.to raise_error(MediumAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < MediumAPI::HttpCallBack
...
client  = MediumAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
