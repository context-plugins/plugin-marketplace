...
│   └── co.pokeapi.exceptions.ApiException          ← base for all non-2xx API responses
│       └── co.pokeapi.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `co.pokeapi.http.client.HttpContext`, which carries:
