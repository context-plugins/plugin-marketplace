...
Confirm every name against `lib/hotel_ratings/client.rb`, `lib/hotel_ratings/configuration.rb`, the
`lib/hotel_ratings/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
