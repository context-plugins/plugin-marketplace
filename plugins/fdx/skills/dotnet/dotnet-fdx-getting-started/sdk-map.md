# SDK map — fdx (.NET)

> A generated table-of-contents for this SDK. Consult this map and its sub-pages to learn signatures, error
> types, enum values, and server/auth wiring **by lookup** — open a source file only for a full method/model
> body the map doesn't carry. The compiler is the backstop: a wrong name fails to build.

| | |
|---|---|
| SDK display name | fdx |
| Root namespace/module | `FdxApi` |
<!-- gen:stamp -->
| Target framework(s) | `netstandard2.0` (C# `LangVersion 14`, `Nullable enable`) |
| Source commit (spec stamp) | `3558446` (`3558446648f3fdcaa8dbd3feb5923f0ff6eb8268`, tagged `3558446`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
| Repo | https://github.com/context-plugins/fdx-csharp-sdk (branch `main`) |

Staleness check: if the SDK is regenerated, the source commit stamp above changes (the SDK repo and this
plugin regenerate together). If a lookup here fails to compile, trust the compiler and re-read the source
file linked in the row.

---

## Getting a client

```csharp
using FdxApi;
using FdxApi.Servers; // ServerEnvironment lives here

var options = new FdxApiClientOptions
{
    // Set the credentials properties for the scheme(s) this API uses — see Servers & auth below.
    // Environment selects the server environment; see Servers & auth below.
};
var client = new FdxApiClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddFdxApiClient(o =>
{
    // set credentials / environment on o here
});
```

Every API group is a property on the client (e.g. `client.Customers`). Source:
`FdxApiClient.cs`.

<!-- crawler:client-options -->
All `FdxApiClientOptions` properties (source: `FdxApiClientOptions.cs`):

| Property | Type |
|---|---|
| `Environment` | `ServerEnvironment` |
| `Retry` | `RetryOptions` |
| `Logging` | `LoggingOptions` |
| `Server` | `ServerOptions` |
| `BearerAuth` | `string?` |
| `TaxBasicAuth` | `BasicAuthCredentials?` |

`RetryOptions` members (source: `Core/Configuration/RetryOptions.cs`; build a full instance — all members are `required` — or start from `RetryOptions.Default()`):

| Member | Type |
|---|---|
| `StatusCodesToRetry` | `IReadOnlyList<HttpStatusCode>` |
| `HttpMethodsToRetry` | `IReadOnlyList<HttpMethod>` |
| `MaxRetries` | `int` |
| `Delay` | `TimeSpan` |
| `Timeout` | `TimeSpan?` |
| `BackOffFactor` | `int` |
| `UseExponentialBackoff` | `bool` |
| `MaxJitter` | `TimeSpan` |
| `OnRetry` | `Action<RetryAttempt>?` |

Client constructor(s):

- `FdxApiClient(HttpClient httpClient, FdxApiClientOptions options)`
<!-- /crawler:client-options -->

---

## Error-handling model (read once — applies to every operation)

Operations are **throw-based**. On an error status the SDK throws `SdkException<TError>`
(`Core/Exceptions/SdkException.cs`) exposing `.Error` of type `TError`. There are two cases:

- **Case A — typed error.** `TError` is a generated `…Error : ApiError` class with status-specific
  `TryGet…(out …)` accessors (returns `true` when that shape is present) plus the inherited
  `TryGetRawError(out RawError)` fallback. The per-operation rows name the exact `TryGet…` methods and the HTTP
  status each maps to.
- **Case B — raw error.** `TError` is `RawError` (`Core/ErrorResponse/RawError.cs`): `StatusCode`,
  `ReadAsString()`, `ReadAsJson<T>()`, `ReadAsBytes()`.

<!-- gen:error-core -->
Core error types (`Core/ErrorResponse/`) — public members with their **declared types**, verbatim from source:

| Type | Public members | Source |
|---|---|---|
| `ApiError` — abstract base of all 53 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
| `RawError` | `StatusCode: HttpStatusCode` · `ReadAsBytes(): ReadOnlyMemory<byte>` · `ReadAsString(): string` · `ReadAsJson<T>(): T?` | `Core/ErrorResponse/RawError.cs` |

Typed-error payload shapes (the `out` types in each operation page's error-accessor cells) are ordinary records/unions: field names, declared types, and JSON wire names live on the records pages / `unions.md` like any other model.
<!-- /gen:error-core -->

```csharp
try { var resp = await client.{ApiGroup}.{Operation}(body); }
catch (SdkException<{Operation}Error> ex)              // Case A
{
    if (ex.Error.TryGetSomeShape(out var typed))      { /* handle that status */ }
    else if (ex.Error.TryGetRawError(out var raw))    { /* other statuses */ }
}
catch (SdkException<RawError> ex)                     // Case B
{
    var status = ex.Error.StatusCode;
    var body   = ex.Error.ReadAsString();
}
```

<!-- crawler:op-stats -->
**No-throw ("`…Result`") variants: absent across this SDK** — every operation is throw-only.
Of **70 operations**, **53 are Case A (typed)** and **17 are Case B (raw)**.
<!-- /crawler:op-stats -->

---

## Operations — by controller (21 groups, 70 operations)

Each links to a sub-page with one row per operation (HTTP, signature with must-pass-explicitly params, return
type, error Case A/B + accessors, pagination).

<!-- crawler:ops-table -->
| Controller (`client.X`) | Ops | Page |
|---|---:|---|
| `AccountInformation` | 2 | [map/operations/AccountInformation.md](map/operations/AccountInformation.md) |
| `AccountStatements` | 2 | [map/operations/AccountStatements.md](map/operations/AccountStatements.md) |
| `AccountTransactions` | 2 | [map/operations/AccountTransactions.md](map/operations/AccountTransactions.md) |
| `EventNotifications` | 5 | [map/operations/EventNotifications.md](map/operations/EventNotifications.md) |
| `FraudNotification` | 1 | [map/operations/FraudNotification.md](map/operations/FraudNotification.md) |
| `InternalTransfers` | 4 | [map/operations/InternalTransfers.md](map/operations/InternalTransfers.md) |
| `Meta` | 3 | [map/operations/Meta.md](map/operations/Meta.md) |
| `MoneyMovement` | 2 | [map/operations/MoneyMovement.md](map/operations/MoneyMovement.md) |
| `PayeeManagement` | 5 | [map/operations/PayeeManagement.md](map/operations/PayeeManagement.md) |
| `PaymentInitiationParties` | 9 | [map/operations/PaymentInitiationParties.md](map/operations/PaymentInitiationParties.md) |
| `Payments` | 5 | [map/operations/Payments.md](map/operations/Payments.md) |
| `PayrollInformation` | 2 | [map/operations/PayrollInformation.md](map/operations/PayrollInformation.md) |
| `PersonalInformation` | 4 | [map/operations/PersonalInformation.md](map/operations/PersonalInformation.md) |
| `Recipients` | 6 | [map/operations/Recipients.md](map/operations/Recipients.md) |
| `RecurringPayments` | 6 | [map/operations/RecurringPayments.md](map/operations/RecurringPayments.md) |
| `ResourceInformation` | 2 | [map/operations/ResourceInformation.md](map/operations/ResourceInformation.md) |
| `RewardProgramCategories` | 1 | [map/operations/RewardProgramCategories.md](map/operations/RewardProgramCategories.md) |
| `RewardProgramInformation` | 2 | [map/operations/RewardProgramInformation.md](map/operations/RewardProgramInformation.md) |
| `SubmitTaxForms` | 2 | [map/operations/SubmitTaxForms.md](map/operations/SubmitTaxForms.md) |
| `TaxForms` | 2 | [map/operations/TaxForms.md](map/operations/TaxForms.md) |
| `UserConsent` | 3 | [map/operations/UserConsent.md](map/operations/UserConsent.md) |
<!-- /crawler:ops-table -->

---

## Models

<!-- gen:models-table -->
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 386 | [`AccountContactEntity` … `Form1042SRecipient1`](map/models/records-1-Ac-Fo.md) · [`Form1065K1` … `Form1099Int`](map/models/records-2-Fo-Fo.md) · [`Form1099Int1` … `Form5498Qa1`](map/models/records-3-Fo-Fo.md) · [`Form5498Sa` … `PaymentMethodDebitAccountEntity`](map/models/records-4-Fo-Pa.md) · [`PaymentMethodEntity` … `VestingEntity`](map/models/records-5-Pa-Ve.md) |
| Unions (`OneOf` / `AnyOf`) — variant factories + `TryGet…` | 4 + 8 | [map/models/unions.md](map/models/unions.md) |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — literal C# member names + wire values | 206 | [map/models/enums.md](map/models/enums.md) |
<!-- /gen:models-table -->

Model conventions: records are immutable with `init`-only setters; `required` properties must be set in the
object initializer; nullable (`T?`) properties are optional. Each record field is listed as
`CSharpName (wire_name): Type` — the parenthesized name is the JSON wire name (`[JsonPropertyName]`).
Unions wrap `Optional<T>` variants — construct via a static factory or implicit
conversion, read back via `TryGet…(out …)`. Enums are **not** C# enums — build with `Type.FromValue("wire")`
or the static members (enums.md lists the literal member names: `SomeEnum.SomeMember`, not
`SomeEnum.some_member`).

<!-- gen:namespaces -->
Namespaces by content type (add `using` accordingly):

| Contents | Namespace(s) |
|---|---|
| Client & options (root) | `FdxApi` |
| Operation controllers (`Api/`) | `FdxApi.Api` |
| Records (`Models/`) | `FdxApi.Models` |
| Enums (`Models/Enums/`) | `FdxApi.Models.Enums` |
| Unions (`Models/AnyOf/`, `Models/OneOf/`) | `FdxApi.Models.AnyOf` · `FdxApi.Models.OneOf` |
| Error classes (`Errors/`) | `FdxApi.Errors` |
<!-- /gen:namespaces -->

---

## Servers & auth

<!-- crawler:servers-auth -->
**Auth.** The scheme(s) this API uses surface as credentials properties on `FdxApiClientOptions` (source: `FdxApiClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:

| Property | Type | Notes (from the source XML docs) |
|---|---|---|
| `BearerAuth` | `string?` | The <c>access_token</c> should be used as the bearer token in this call. |
| `TaxBasicAuth` | `BasicAuthCredentials?` | — |

**Environments.** `options.Environment` is a `ServerEnvironment` (`Servers/ServerEnvironment.cs`) with members: `ServerEnvironment.Production`. Base-URL templates and override points live under `Servers/` and `options.Server`.
<!-- /crawler:servers-auth -->
