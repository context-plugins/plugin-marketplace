...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `NotionClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`notion/configuration.py` in the cloned source.
...
```python
import requests
from notion.configuration import Configuration
...
`notion/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`notion/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `notion/configuration.py`
...
`NumberPagedResponse`) live in `notion/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `notion/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
