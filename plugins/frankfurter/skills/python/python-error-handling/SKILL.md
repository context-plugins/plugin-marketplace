...
`frankfurter/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `frankfurter/exceptions/` and operation docstrings.
...
```python
from frankfurter.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `frankfurter/exceptions/`.
...
```python
from frankfurter.exceptions.api_exception import APIException
from frankfurter.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`frankfurter/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`frankfurter/exceptions/` for the class.
...
`frankfurter/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `frankfurter/exceptions/`
  or a `ValueError` — check the auth handler in `frankfurter/http/auth/` for the exact type.
