...
- `PasswordConnect1Client` — the single public client class (e.g. `FooClient`).
- `PasswordConnect1ClientOptions` — its options class.
- `PasswordConnect1` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public PasswordConnect1Client(HttpClient httpClient, PasswordConnect1ClientOptions options)
```
...
```csharp
public class PasswordConnect1ClientOptions
{
...
```csharp
using PasswordConnect1;
using PasswordConnect1.Servers;

var options = new PasswordConnect1ClientOptions
{
...
var client = new PasswordConnect1Client(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddPasswordConnect1Client`, which registers the
...
builder.Services.AddPasswordConnect1Client(options =>
{
...
```csharp
public sealed class MyService(PasswordConnect1Client client)
{
