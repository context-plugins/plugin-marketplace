...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `geoapify-apilib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'geoapify-apilib';
import { ApiError } from 'geoapify-apilib';
import { {Operation}Error } from 'geoapify-apilib/errors';
