...
<!-- crawler:auth -->
| Auth | Credentials properties on `FirecrawlApiClientOptions`: `BearerAuth: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using FirecrawlApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/firecrawl-csharp-sdk
dotnet add reference firecrawl-csharp-sdk/FirecrawlApi.csproj
```
...
temp directory** (`<temp>/firecrawl-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/firecrawl-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/firecrawl-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/firecrawl-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\firecrawl-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/firecrawl-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/firecrawl-dotnet-src/`, **never** in the project repo, and its path never goes into
`firecrawl-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new FirecrawlApiClient(...)`, build its options, or DI-register via
   `AddFirecrawlApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `FirecrawlApiClientOptions` — the map's *Servers & auth* section
