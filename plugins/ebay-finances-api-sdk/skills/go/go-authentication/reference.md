...
```go
ebayfinancesapi.WithOAuthCCGCredentials(
    ebayfinancesapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr ebayfinancesapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := ebayfinancesapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr ebayfinancesapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := ebayfinancesapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := ebayfinancesapi.NewClient(ebayfinancesapi.CreateConfiguration(ebayfinancesapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    ebayfinancesapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := ebayfinancesapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := ebayfinancesapi.NewClient(ebayfinancesapi.CreateConfiguration(ebayfinancesapi.WithOAuthROPCGCredentials(creds)))
...
```go
ebayfinancesapi.WithOAuthBearerTokenCredentials(ebayfinancesapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
ebayfinancesapi.WithBasicAuthCredentials(ebayfinancesapi.NewBasicAuthCredentials(username, password))
ebayfinancesapi.WithApiHeaderCredentials(ebayfinancesapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
ebayfinancesapi.WithApiKeyCredentials(ebayfinancesapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `EBAYFINANCESAPI_...` variables — e.g.
`EBAYFINANCESAPI_USERNAME`, `EBAYFINANCESAPI_PASSWORD`, `EBAYFINANCESAPI_TOKEN`, `EBAYFINANCESAPI_API_KEY`,
`EBAYFINANCESAPI_O_AUTH_CLIENT_ID`, `EBAYFINANCESAPI_O_AUTH_CLIENT_SECRET`, `EBAYFINANCESAPI_ACCESS_TOKEN`. Grep the
