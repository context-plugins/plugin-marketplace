...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new FlightPriceAnalysisAPIClient({
...
```typescript
import { {Operation}Error } from 'flight-price-analysis-apilib/errors';
...
```typescript
import { ApiError } from 'flight-price-analysis-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(FlightPriceAnalysisAPIClient)
    .useValue(new FlightPriceAnalysisAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
