...
To see which schemes a specific SDK accepts, read the credential setter methods on `TeslaAPIClient.Builder`
...
- Set credentials **on the `TeslaAPIClient.Builder`** before calling `.build()` — the client is immutable
