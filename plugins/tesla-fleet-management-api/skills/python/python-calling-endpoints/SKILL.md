...
Open `teslafleetmanagementapi/teslafleetmanagementapi_client.py` for the controller property names, then the relevant
`teslafleetmanagementapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from teslafleetmanagementapi.models.{request_model} import {RequestModel}
...
Open `teslafleetmanagementapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from teslafleetmanagementapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from teslafleetmanagementapi.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `teslafleetmanagementapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `TeslafleetmanagementapiClient` in `teslafleetmanagementapi/teslafleetmanagementapi_client.py`.
- Operation method signatures are in `teslafleetmanagementapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `teslafleetmanagementapi/models/`; enum types have `from_value()`.
