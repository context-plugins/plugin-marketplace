...
```go
hotelnameautocomplete.WithOAuthCCGCredentials(
    hotelnameautocomplete.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr hotelnameautocomplete.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := hotelnameautocomplete.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr hotelnameautocomplete.OAuthCCGManager) models.OAuthToken {
...
```go
creds := hotelnameautocomplete.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := hotelnameautocomplete.NewClient(hotelnameautocomplete.CreateConfiguration(hotelnameautocomplete.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    hotelnameautocomplete.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := hotelnameautocomplete.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := hotelnameautocomplete.NewClient(hotelnameautocomplete.CreateConfiguration(hotelnameautocomplete.WithOAuthROPCGCredentials(creds)))
...
```go
hotelnameautocomplete.WithOAuthBearerTokenCredentials(hotelnameautocomplete.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
hotelnameautocomplete.WithBasicAuthCredentials(hotelnameautocomplete.NewBasicAuthCredentials(username, password))
hotelnameautocomplete.WithApiHeaderCredentials(hotelnameautocomplete.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
hotelnameautocomplete.WithApiKeyCredentials(hotelnameautocomplete.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `HOTELNAMEAUTOCOMPLETE_...` variables — e.g.
`HOTELNAMEAUTOCOMPLETE_USERNAME`, `HOTELNAMEAUTOCOMPLETE_PASSWORD`, `HOTELNAMEAUTOCOMPLETE_TOKEN`, `HOTELNAMEAUTOCOMPLETE_API_KEY`,
`HOTELNAMEAUTOCOMPLETE_O_AUTH_CLIENT_ID`, `HOTELNAMEAUTOCOMPLETE_O_AUTH_CLIENT_SECRET`, `HOTELNAMEAUTOCOMPLETE_ACCESS_TOKEN`. Grep the
