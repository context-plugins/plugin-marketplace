...
# Getting started with the Adyen API .NET SDK
...
| --- | --- |
| API | `Adyen API` |
| Project | `AdyenApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `AdyenApi` (the `using` namespace) |
| Client class | `AdyenApiClient` |
| Options class | `AdyenApiClientOptions` |
...
namespaces transitively, so `using AdyenApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/adyen-api-csharp
dotnet add reference adyen-api-csharp/AdyenApi.csproj
```

```csharp
using AdyenApi;          // client + options + ServerEnvironment
using AdyenApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/adyen-api-csharp /tmp/adyen-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/adyen-api-csharp "$env:TEMP\adyen-api-dotnet-src"
```
...
- `AdyenApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/adyen-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\adyen-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AdyenApiClient`, build its
   `AdyenApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
