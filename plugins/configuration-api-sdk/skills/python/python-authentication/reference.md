...
kwargs to the `ConfigurationAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `configurationapi/configurationapi_client.py` and `configurationapi/http/auth/` in
the cloned source.

## Basic auth

```python
from configurationapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from configurationapi.http.auth.api_key import ApiKeyCredentials
...
```python
from configurationapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from configurationapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from configurationapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from configurationapi.http.auth.o_auth_acg import OAuthACGCredentials
from configurationapi.models.o_auth_scope_configurationapi_enum import OAuthScope{Pkg}Enum
...
client = ConfigurationAPIClient(o_auth_acg_credentials=creds)
...
```python
from configurationapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ConfigurationAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ConfigurationAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `configurationapi/configuration.py` and each `configurationapi/http/auth/*.py` for the exact
