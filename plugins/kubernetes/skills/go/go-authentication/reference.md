...
```go
kubernetes.WithOAuthCCGCredentials(
    kubernetes.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr kubernetes.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := kubernetes.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr kubernetes.OAuthCCGManager) models.OAuthToken {
...
```go
creds := kubernetes.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := kubernetes.NewClient(kubernetes.CreateConfiguration(kubernetes.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    kubernetes.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := kubernetes.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := kubernetes.NewClient(kubernetes.CreateConfiguration(kubernetes.WithOAuthROPCGCredentials(creds)))
...
```go
kubernetes.WithOAuthBearerTokenCredentials(kubernetes.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
kubernetes.WithBasicAuthCredentials(kubernetes.NewBasicAuthCredentials(username, password))
kubernetes.WithApiHeaderCredentials(kubernetes.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
kubernetes.WithApiKeyCredentials(kubernetes.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `KUBERNETES_...` variables — e.g.
`KUBERNETES_USERNAME`, `KUBERNETES_PASSWORD`, `KUBERNETES_TOKEN`, `KUBERNETES_API_KEY`,
`KUBERNETES_O_AUTH_CLIENT_ID`, `KUBERNETES_O_AUTH_CLIENT_SECRET`, `KUBERNETES_ACCESS_TOKEN`. Grep the
