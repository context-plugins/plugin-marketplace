...
`subscriptionclient/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `subscriptionclient/exceptions/` and operation docstrings.
...
```python
from subscriptionclient.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `subscriptionclient/exceptions/`.
...
```python
from subscriptionclient.exceptions.api_exception import APIException
from subscriptionclient.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`subscriptionclient/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`subscriptionclient/exceptions/` for the class.
...
`subscriptionclient/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `subscriptionclient/exceptions/`
  or a `ValueError` — check the auth handler in `subscriptionclient/http/auth/` for the exact type.
