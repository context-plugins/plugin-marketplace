...
Confirm every name against `lib/openai_api/client.rb`, `lib/openai_api/configuration.rb`, the
`lib/openai_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
