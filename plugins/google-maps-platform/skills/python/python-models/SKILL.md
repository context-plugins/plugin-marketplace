...
your SDK source (`googlemapsplatform/models/`).
...
```python
from googlemapsplatform.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `googlemapsplatform/utilities/union_type_lookup.py`) and each candidate model
...
```python
from googlemapsplatform.models.animal import Cat, Dog
...
```python
from googlemapsplatform.models.animal import Animal, Cat, Dog
