...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new IPGeolocationAPIClient({
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(IPGeolocationAPIClient)
    .useValue(new IPGeolocationAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
