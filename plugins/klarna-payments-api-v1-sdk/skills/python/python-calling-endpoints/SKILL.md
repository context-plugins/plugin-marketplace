...
Open `klarnapaymentsapiv1/klarnapaymentsapiv1_client.py` for the controller property names, then the relevant
`klarnapaymentsapiv1/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from klarnapaymentsapiv1.models.{request_model} import {RequestModel}
...
Open `klarnapaymentsapiv1/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from klarnapaymentsapiv1.models.suite_code_enum import SuiteCodeEnum
...
```python
from klarnapaymentsapiv1.http.http_call_back import HttpCallBack
...
client = KlarnaPaymentsAPIV1Client(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `klarnapaymentsapiv1/pagination/` in the source.

## Finding the right method

- Controller property names are on `KlarnaPaymentsAPIV1Client` in `klarnapaymentsapiv1/klarnapaymentsapiv1_client.py`.
- Operation method signatures are in `klarnapaymentsapiv1/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `klarnapaymentsapiv1/models/`; enum types have `from_value()`.
