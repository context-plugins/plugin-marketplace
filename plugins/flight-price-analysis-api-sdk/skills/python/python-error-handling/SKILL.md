...
`flightpriceanalysisapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `flightpriceanalysisapi/exceptions/` and operation docstrings.
...
```python
from flightpriceanalysisapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `flightpriceanalysisapi/exceptions/`.
...
```python
from flightpriceanalysisapi.exceptions.api_exception import APIException
from flightpriceanalysisapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`flightpriceanalysisapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`flightpriceanalysisapi/exceptions/` for the class.
...
`flightpriceanalysisapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `flightpriceanalysisapi/exceptions/`
  or a `ValueError` — check the auth handler in `flightpriceanalysisapi/http/auth/` for the exact type.
