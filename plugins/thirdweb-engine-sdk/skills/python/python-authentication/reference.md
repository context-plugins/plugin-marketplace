...
kwargs to the `ThirdwebEngineClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `thirdwebengine/thirdwebengine_client.py` and `thirdwebengine/http/auth/` in
the cloned source.

## Basic auth

```python
from thirdwebengine.http.auth.basic_auth import BasicAuthCredentials
...
```python
from thirdwebengine.http.auth.api_key import ApiKeyCredentials
...
```python
from thirdwebengine.http.auth.api_header import ApiHeaderCredentials
...
```python
from thirdwebengine.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from thirdwebengine.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from thirdwebengine.http.auth.o_auth_acg import OAuthACGCredentials
from thirdwebengine.models.o_auth_scope_thirdwebengine_enum import OAuthScope{Pkg}Enum
...
client = ThirdwebEngineClient(o_auth_acg_credentials=creds)
...
```python
from thirdwebengine.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ThirdwebEngineClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ThirdwebEngineClient.from_environment()`) read each credential
...
Grep `from_environment` in `thirdwebengine/configuration.py` and each `thirdwebengine/http/auth/*.py` for the exact
