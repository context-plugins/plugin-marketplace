...
| --- | --- |
| API | `Trader API` |
| Project | `TraderApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using TraderApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/trader-api-csharp-sdk
dotnet add reference trader-api-csharp-sdk/TraderApi.csproj
```

```csharp
using TraderApi;          // client + options + ServerEnvironment
using TraderApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/trader-api-csharp-sdk /tmp/trader-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/trader-api-csharp-sdk "$env:TEMP\trader-api-dotnet-src"
```
...
- `TraderApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/trader-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\trader-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `TraderApiClient`, build its
   `TraderApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
