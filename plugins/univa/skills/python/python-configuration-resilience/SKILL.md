...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `UnivapayClientSdkClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`univa/configuration.py` in the cloned source.
...
```python
import requests
from univa.configuration import Configuration
...
`univa/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`univa/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `univa/configuration.py`
...
`NumberPagedResponse`) live in `univa/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `univa/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
