...
`hotelnameautocomplete/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `hotelnameautocomplete/exceptions/` and operation docstrings.
...
```python
from hotelnameautocomplete.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `hotelnameautocomplete/exceptions/`.
...
```python
from hotelnameautocomplete.exceptions.api_exception import APIException
from hotelnameautocomplete.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`hotelnameautocomplete/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`hotelnameautocomplete/exceptions/` for the class.
...
`hotelnameautocomplete/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `hotelnameautocomplete/exceptions/`
  or a `ValueError` — check the auth handler in `hotelnameautocomplete/http/auth/` for the exact type.
