...
- `flightcreateorders` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/flight-create-orders-go` — the Go import path for the root package; `github.com/context-plugins/flight-create-orders-go/models` for request/response types.
...
```go
import "github.com/context-plugins/flight-create-orders-go"

client := flightcreateorders.NewClient(
    flightcreateorders.CreateConfiguration(
        flightcreateorders.WithEnvironment(flightcreateorders.PRODUCTION),
...
from environment variables named `FLIGHTCREATEORDERS_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := flightcreateorders.NewClient(flightcreateorders.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `flightcreateorders.PRODUCTION`,
`flightcreateorders.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := flightcreateorders.NewClient(
    flightcreateorders.CreateConfiguration(flightcreateorders.WithEnvironment(flightcreateorders.PRODUCTION)),
)
```
...
client := flightcreateorders.NewClient(
    flightcreateorders.CreateConfiguration(
        flightcreateorders.WithHttpConfiguration(
            flightcreateorders.CreateHttpConfiguration(
                flightcreateorders.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                flightcreateorders.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = flightcreateorders.NewClient(flightcreateorders.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api flightcreateorders.ClientInterface
}

func NewService(api flightcreateorders.ClientInterface) *Service {
