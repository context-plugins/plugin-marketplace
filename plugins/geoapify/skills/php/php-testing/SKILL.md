...
> `GeoapifyClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): GeoapifyClient
{
...
```php
use GeoapifyLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use GeoapifyLib\Exceptions\{OperationException};
use GeoapifyLib\Exceptions\ApiException;
...
app()->instance(GeoapifyClient::class, clientWithResponses([
