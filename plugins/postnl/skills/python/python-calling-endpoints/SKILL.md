...
Open `postnl/postnl_client.py` for the controller property names, then the relevant
`postnl/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from postnl.models.{request_model} import {RequestModel}
...
Open `postnl/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from postnl.models.suite_code_enum import SuiteCodeEnum
...
```python
from postnl.http.http_call_back import HttpCallBack
...
client = PostnlecommerceClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `postnl/pagination/` in the source.

## Finding the right method

- Controller property names are on `PostnlecommerceClient` in `postnl/postnl_client.py`.
- Operation method signatures are in `postnl/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `postnl/models/`; enum types have `from_value()`.
