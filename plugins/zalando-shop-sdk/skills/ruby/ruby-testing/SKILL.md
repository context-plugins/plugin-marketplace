...
client = stub_client(stubs)
assert_raises ZalandoShop::APIException do
...
def build_test_client
  ZalandoShop::Client.new(
    environment: ZalandoShop::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ZalandoShop::APIException
```
...
```ruby
expect { call }.to raise_error(ZalandoShop::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ZalandoShop::HttpCallBack
...
client  = ZalandoShop::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
