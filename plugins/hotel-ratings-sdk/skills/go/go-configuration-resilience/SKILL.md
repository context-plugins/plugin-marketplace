    ...
    hotelratings.CreateConfiguration(
        hotelratings.WithHttpConfiguration(
            hotelratings.CreateHttpConfiguration(
                hotelratings.WithTimeout(30),
                hotelratings.WithRetryConfiguration(
                    hotelratings.CreateRetryConfiguration(
                        hotelratings.WithMaxRetryAttempts(3),
                        hotelratings.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := hotelratings.CreateRetryConfiguration(
    hotelratings.WithMaxRetryAttempts(5),
    hotelratings.WithRetryInterval(1),
    hotelratings.WithBackoffFactor(2),
    hotelratings.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    hotelratings.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := hotelratings.NewClient(
    hotelratings.CreateConfiguration(
        hotelratings.WithHttpConfiguration(
            hotelratings.CreateHttpConfiguration(
                hotelratings.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
