...
kwargs to the `HotelSearchAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `hotelsearchapi/hotelsearchapi_client.py` and `hotelsearchapi/http/auth/` in
the cloned source.

## Basic auth

```python
from hotelsearchapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from hotelsearchapi.http.auth.api_key import ApiKeyCredentials
...
```python
from hotelsearchapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from hotelsearchapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from hotelsearchapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from hotelsearchapi.http.auth.o_auth_acg import OAuthACGCredentials
from hotelsearchapi.models.o_auth_scope_hotelsearchapi_enum import OAuthScope{Pkg}Enum
...
client = HotelSearchAPIClient(o_auth_acg_credentials=creds)
...
```python
from hotelsearchapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = HotelSearchAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `HotelSearchAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `hotelsearchapi/configuration.py` and each `hotelsearchapi/http/auth/*.py` for the exact
