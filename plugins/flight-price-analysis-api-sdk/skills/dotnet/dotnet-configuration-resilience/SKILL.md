...
All types below live under `FlightPriceAnalysisApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FlightPriceAnalysisApi.Core.Configuration;
...
```csharp
using FlightPriceAnalysisApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FlightPriceAnalysisApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FlightPriceAnalysisApi.Core.Exceptions`):
...
With DI, the SDK's `AddFlightPriceAnalysisApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFlightPriceAnalysisApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
