...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `1ForgeFinanceAPIsClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`1forgefinanceapis/configuration.py` in the cloned source.
...
```python
import requests
from 1forgefinanceapis.configuration import Configuration
...
`1forgefinanceapis/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`1forgefinanceapis/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `1forgefinanceapis/configuration.py`
...
`NumberPagedResponse`) live in `1forgefinanceapis/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `1forgefinanceapis/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
