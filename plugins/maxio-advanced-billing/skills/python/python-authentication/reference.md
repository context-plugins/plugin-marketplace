...
kwargs to the `MaxioadvancedbillingClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `maxioadvancedbilling/maxioadvancedbilling_client.py` and `maxioadvancedbilling/http/auth/` in
the cloned source.

## Basic auth

```python
from maxioadvancedbilling.http.auth.basic_auth import BasicAuthCredentials
...
```python
from maxioadvancedbilling.http.auth.api_key import ApiKeyCredentials
...
```python
from maxioadvancedbilling.http.auth.api_header import ApiHeaderCredentials
...
```python
from maxioadvancedbilling.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from maxioadvancedbilling.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from maxioadvancedbilling.http.auth.o_auth_acg import OAuthACGCredentials
from maxioadvancedbilling.models.o_auth_scope_maxioadvancedbilling_enum import OAuthScope{Pkg}Enum
...
client = MaxioadvancedbillingClient(o_auth_acg_credentials=creds)
...
```python
from maxioadvancedbilling.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = MaxioadvancedbillingClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `MaxioadvancedbillingClient.from_environment()`) read each credential
...
Grep `from_environment` in `maxioadvancedbilling/configuration.py` and each `maxioadvancedbilling/http/auth/*.py` for the exact
