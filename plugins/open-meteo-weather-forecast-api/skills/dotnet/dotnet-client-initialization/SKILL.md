...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddOpenMeteoWeatherForecastApiClient extension. Use the moment you write `new OpenMeteoWeatherForecastApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `OpenMeteoWeatherForecastApiClientOptions` — its options class.
- `OpenMeteoWeatherForecastApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public OpenMeteoWeatherForecastApiClient(HttpClient httpClient, OpenMeteoWeatherForecastApiClientOptions options)
```
...
```csharp
public class OpenMeteoWeatherForecastApiClientOptions
{
...
```csharp
using OpenMeteoWeatherForecastApi;
using OpenMeteoWeatherForecastApi.Servers;

var options = new OpenMeteoWeatherForecastApiClientOptions
{
...
var client = new OpenMeteoWeatherForecastApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddOpenMeteoWeatherForecastApiClient`, which registers the
...
builder.Services.AddOpenMeteoWeatherForecastApiClient(options =>
{
...
```csharp
public sealed class MyService(OpenMeteoWeatherForecastApiClient client)
{
