...
`testing` + `testify/require` for reference; substitute the real `adafruitiorestapi` and names.
...
import (
    ...
    "github.com/context-plugins/adafruit-io-rest-api-go"   // root package adafruitiorestapi
)
...
func clientReturning(status int, body string) (adafruitiorestapi.ClientInterface, *stubTransport) {
    ...
    client := adafruitiorestapi.NewClient(
        adafruitiorestapi.CreateConfiguration(
            adafruitiorestapi.WithHttpConfiguration(
                adafruitiorestapi.CreateHttpConfiguration(adafruitiorestapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/adafruit-io-rest-api-go/errors"
...
client := adafruitiorestapi.NewClient(
    adafruitiorestapi.CreateConfiguration(
        adafruitiorestapi.WithHttpConfiguration(
            adafruitiorestapi.CreateHttpConfiguration(adafruitiorestapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
