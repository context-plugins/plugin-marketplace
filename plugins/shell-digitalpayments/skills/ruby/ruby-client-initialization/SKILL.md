...
- `ShellDigitalpayments` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `shell_digitalpayments` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'shell_digitalpayments'
include ShellDigitalpayments
...
The authoritative list is the `initialize` signature in `lib/shell_digitalpayments/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/shell_digitalpayments/configuration.rb`
(`build_default_config_from_env`) and `lib/shell_digitalpayments/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = ShellDigitalpayments::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/shell_digitalpayments.rb`) and store it in a
