...
kwargs to the `FdxapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `fdx/fdx_client.py` and `fdx/http/auth/` in
the cloned source.

## Basic auth

```python
from fdx.http.auth.basic_auth import BasicAuthCredentials
...
```python
from fdx.http.auth.api_key import ApiKeyCredentials
...
```python
from fdx.http.auth.api_header import ApiHeaderCredentials
...
```python
from fdx.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from fdx.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from fdx.http.auth.o_auth_acg import OAuthACGCredentials
from fdx.models.o_auth_scope_fdx_enum import OAuthScope{Pkg}Enum
...
client = FdxapiClient(o_auth_acg_credentials=creds)
...
```python
from fdx.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FdxapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FdxapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `fdx/configuration.py` and each `fdx/http/auth/*.py` for the exact
