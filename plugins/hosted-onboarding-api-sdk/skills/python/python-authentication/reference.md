...
kwargs to the `HostedOnboardingAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `hostedonboardingapi/hostedonboardingapi_client.py` and `hostedonboardingapi/http/auth/` in
the cloned source.

## Basic auth

```python
from hostedonboardingapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from hostedonboardingapi.http.auth.api_key import ApiKeyCredentials
...
```python
from hostedonboardingapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from hostedonboardingapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from hostedonboardingapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from hostedonboardingapi.http.auth.o_auth_acg import OAuthACGCredentials
from hostedonboardingapi.models.o_auth_scope_hostedonboardingapi_enum import OAuthScope{Pkg}Enum
...
client = HostedOnboardingAPIClient(o_auth_acg_credentials=creds)
...
```python
from hostedonboardingapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = HostedOnboardingAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `HostedOnboardingAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `hostedonboardingapi/configuration.py` and each `hostedonboardingapi/http/auth/*.py` for the exact
