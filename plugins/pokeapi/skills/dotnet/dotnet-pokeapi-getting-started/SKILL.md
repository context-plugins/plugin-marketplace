---
name: dotnet-pokeapi-getting-started
description: PokeApi .NET SDK identity and lookup layer for the dotnet-pokeapi-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the PokeApi .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-pokeapi-sdk` helper
...
| --- | --- |
| API | PokeApi |
| Source repo | https://github.com/context-plugins/pokeapi-csharp-sdk (branch `main`) |
| Root namespace | `PokApi` (the `using` namespace) |
| Client class | `PokApiClient` |
| Options class | `PokApiClientOptions` |
<!-- crawler:auth -->
| Auth | See the credentials properties on `PokApiClientOptions` — the SDK map's *Servers & auth* section lists the scheme(s) |
<!-- /crawler:auth -->
...
namespaces transitively, so `using PokApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/pokeapi-csharp-sdk
dotnet add reference pokeapi-csharp-sdk/PokApi.csproj
```
...
temp directory** (`<temp>/pokeapi-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/pokeapi-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/pokeapi-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/pokeapi-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\pokeapi-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/pokeapi-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/pokeapi-dotnet-src/`, **never** in the project repo, and its path never goes into
`pokeapi-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `PokApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new PokApiClient(...)`, build its options, or DI-register via
   `AddPokApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `PokApiClientOptions` — the map's *Servers & auth* section
