...
│   └── io.finnhub.exceptions.ApiException          ← base for all non-2xx API responses
│       └── io.finnhub.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `io.finnhub.http.client.HttpContext`, which carries:
