...
Open `twilioapis/twilioapis_client.py` for the controller property names, then the relevant
`twilioapis/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from twilioapis.models.{request_model} import {RequestModel}
...
Open `twilioapis/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from twilioapis.models.suite_code_enum import SuiteCodeEnum
...
```python
from twilioapis.http.http_call_back import HttpCallBack
...
client = TwilioapisClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `twilioapis/pagination/` in the source.

## Finding the right method

- Controller property names are on `TwilioapisClient` in `twilioapis/twilioapis_client.py`.
- Operation method signatures are in `twilioapis/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `twilioapis/models/`; enum types have `from_value()`.
