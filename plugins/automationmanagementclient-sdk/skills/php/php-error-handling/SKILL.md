...
> `{operation}`, `{apiGroup}`, `AutomationManagementClientLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `AutomationManagementClientLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `AutomationManagementClientLib\Exceptions\` — read the source to
