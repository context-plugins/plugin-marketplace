...
`traderapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `traderapi/exceptions/` and operation docstrings.
...
```python
from traderapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `traderapi/exceptions/`.
...
```python
from traderapi.exceptions.api_exception import APIException
from traderapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`traderapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`traderapi/exceptions/` for the class.
...
`traderapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `traderapi/exceptions/`
  or a `ValueError` — check the auth handler in `traderapi/http/auth/` for the exact type.
