...
> `AdyenAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AdyenAPIClient
{
...
```php
use AdyenAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AdyenAPILib\Exceptions\{OperationException};
use AdyenAPILib\Exceptions\ApiException;
...
app()->instance(AdyenAPIClient::class, clientWithResponses([
