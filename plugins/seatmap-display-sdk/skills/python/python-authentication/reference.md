...
kwargs to the `SeatmapDisplayClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `seatmapdisplay/seatmapdisplay_client.py` and `seatmapdisplay/http/auth/` in
the cloned source.

## Basic auth

```python
from seatmapdisplay.http.auth.basic_auth import BasicAuthCredentials
...
```python
from seatmapdisplay.http.auth.api_key import ApiKeyCredentials
...
```python
from seatmapdisplay.http.auth.api_header import ApiHeaderCredentials
...
```python
from seatmapdisplay.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from seatmapdisplay.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from seatmapdisplay.http.auth.o_auth_acg import OAuthACGCredentials
from seatmapdisplay.models.o_auth_scope_seatmapdisplay_enum import OAuthScope{Pkg}Enum
...
client = SeatmapDisplayClient(o_auth_acg_credentials=creds)
...
```python
from seatmapdisplay.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SeatmapDisplayClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SeatmapDisplayClient.from_environment()`) read each credential
...
Grep `from_environment` in `seatmapdisplay/configuration.py` and each `seatmapdisplay/http/auth/*.py` for the exact
