...
All types below live under `HotelNameAutocomplete.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using HotelNameAutocomplete.Core.Configuration;
...
```csharp
using HotelNameAutocomplete.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new HotelNameAutocompleteClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `HotelNameAutocomplete.Core.Exceptions`):
...
With DI, the SDK's `AddHotelNameAutocompleteClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddHotelNameAutocompleteClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
