...
- **HTTP**: `GET /sessions/{sessionId}` (Default (checkout-test))
...
### PostAuthorise
- **HTTP**: `POST /authorise` (Default1 (pal-test))
...
### PostAuthorise3D
- **HTTP**: `POST /authorise3d` (Default1 (pal-test))
...
### PostAuthorise3Ds2
- **HTTP**: `POST /authorise3ds2` (Default1 (pal-test))
...
### PostCardDetails
- **HTTP**: `POST /cardDetails` (Default (checkout-test))
...
- **HTTP**: `POST /getAuthenticationResult` (Default1 (pal-test))
...
### PostPaymentMethods
- **HTTP**: `POST /paymentMethods` (Default (checkout-test))
...
### PostPayments
- **HTTP**: `POST /payments` (Default (checkout-test))
...
### PostPaymentsCancel
- **HTTP**: `POST /payments/cancel` (Default20 (balanceplatform-api-test))
...
### PostPaymentsConfirm
- **HTTP**: `POST /payments/confirm` (Default20 (balanceplatform-api-test))
...
### PostPaymentsDetails
- **HTTP**: `POST /payments/details` (Default (checkout-test))
- **Notes**: Submits details for a payment created using `/payments`. This step is only needed when no final state has been reached on the `/payments` request, for example when the shopper was redirected to another page to complete the payment.
...
- **HTTP**: `POST /payments/details` (Default20 (balanceplatform-api-test))
...
- **HTTP**: `POST /retrieve3ds2Result` (Default1 (pal-test))
...
### PostSessions
- **HTTP**: `POST /sessions` (Default (checkout-test))
