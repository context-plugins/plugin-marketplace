...
- `frankfurter` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/frankfurter-go-sdk` — the Go import path for the root package; `github.com/context-plugins/frankfurter-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/frankfurter-go-sdk"

client := frankfurter.NewClient(
    frankfurter.CreateConfiguration(
        frankfurter.WithEnvironment(frankfurter.PRODUCTION),
...
from environment variables named `FRANKFURTER_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := frankfurter.NewClient(frankfurter.CreateConfigurationFromEnvironment())
```
...
```go
client := frankfurter.NewClient(
    frankfurter.CreateConfiguration(frankfurter.WithEnvironment(frankfurter.PRODUCTION)),
)
```
...
client := frankfurter.NewClient(
    frankfurter.CreateConfiguration(
        frankfurter.WithHttpConfiguration(
            frankfurter.CreateHttpConfiguration(
                frankfurter.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                frankfurter.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = frankfurter.NewClient(frankfurter.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api frankfurter.ClientInterface
}

func NewService(api frankfurter.ClientInterface) *Service {
