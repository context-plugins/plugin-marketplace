...
`testing` + `testify/require` for reference; substitute the real `travelrecommendationsapi` and names.
...
import (
    ...
    "github.com/context-plugins/travel-recommendations-api-go"   // root package travelrecommendationsapi
)
...
func clientReturning(status int, body string) (travelrecommendationsapi.ClientInterface, *stubTransport) {
    ...
    client := travelrecommendationsapi.NewClient(
        travelrecommendationsapi.CreateConfiguration(
            travelrecommendationsapi.WithHttpConfiguration(
                travelrecommendationsapi.CreateHttpConfiguration(travelrecommendationsapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/travel-recommendations-api-go/errors"
...
client := travelrecommendationsapi.NewClient(
    travelrecommendationsapi.CreateConfiguration(
        travelrecommendationsapi.WithHttpConfiguration(
            travelrecommendationsapi.CreateHttpConfiguration(travelrecommendationsapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
