...
```go
xero.WithOAuthCCGCredentials(
    xero.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr xero.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := xero.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr xero.OAuthCCGManager) models.OAuthToken {
...
```go
creds := xero.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := xero.NewClient(xero.CreateConfiguration(xero.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    xero.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := xero.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := xero.NewClient(xero.CreateConfiguration(xero.WithOAuthROPCGCredentials(creds)))
...
```go
xero.WithOAuthBearerTokenCredentials(xero.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
xero.WithBasicAuthCredentials(xero.NewBasicAuthCredentials(username, password))
xero.WithApiHeaderCredentials(xero.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
xero.WithApiKeyCredentials(xero.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `XERO_...` variables — e.g.
`XERO_USERNAME`, `XERO_PASSWORD`, `XERO_TOKEN`, `XERO_API_KEY`,
`XERO_O_AUTH_CLIENT_ID`, `XERO_O_AUTH_CLIENT_SECRET`, `XERO_ACCESS_TOKEN`. Grep the
