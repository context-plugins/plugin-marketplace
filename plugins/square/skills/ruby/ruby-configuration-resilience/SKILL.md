...
```ruby
client = Square::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Square::Client.new(adapter: :net_http)
```
...
client = Square::Client.new(connection: conn)
```
...
client = Square::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Square::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Square::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Square::ProxySettings.from_env
client = Square::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Square::HttpCallBack
...
client = Square::Client.new(http_callback: LogCallback.new)
```
...
client = Square::Client.from_env(timeout: 30, max_retries: 3)
```
