...
client = stub_client(stubs)
assert_raises EBayFinancesAPI::APIException do
...
def build_test_client
  EBayFinancesAPI::Client.new(
    environment: EBayFinancesAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error EBayFinancesAPI::APIException
```
...
```ruby
expect { call }.to raise_error(EBayFinancesAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < EBayFinancesAPI::HttpCallBack
...
client  = EBayFinancesAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
