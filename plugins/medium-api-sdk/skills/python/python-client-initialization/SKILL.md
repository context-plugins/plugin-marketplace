...
- `mediumapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `MediumAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `MediumAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
from mediumapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from mediumapi.configuration import Configuration, Environment
...
client = MediumAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `mediumapi/mediumapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `MediumAPIClient` and `Configuration` that reads
...
`from_environment` in `mediumapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `MediumAPIClient`. Access them as attributes —
...
Open `mediumapi/mediumapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `mediumapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from mediumapi.configuration import Environment

client = MediumAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`mediumapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = MediumAPIClient(config=new_config)
```
