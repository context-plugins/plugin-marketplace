...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AWSMigrationHubClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`awsmigrationhub/configuration.py` in the cloned source.
...
```python
import requests
from awsmigrationhub.configuration import Configuration
...
`awsmigrationhub/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`awsmigrationhub/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `awsmigrationhub/configuration.py`
...
`NumberPagedResponse`) live in `awsmigrationhub/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `awsmigrationhub/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
