...
against the `EbaySellClient.Builder` setters, the `com.ebay.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `EbaySellClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
EbaySellClient client = new EbaySellClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeEbaySellEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
