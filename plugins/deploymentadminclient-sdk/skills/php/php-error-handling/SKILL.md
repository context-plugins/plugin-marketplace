...
> `{operation}`, `{apiGroup}`, `DeploymentAdminClientLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `DeploymentAdminClientLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `DeploymentAdminClientLib\Exceptions\` — read the source to
