...
```ruby
client = ThePlaidAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ThePlaidAPI::Client.new(adapter: :net_http)
```
...
client = ThePlaidAPI::Client.new(connection: conn)
```
...
client = ThePlaidAPI::Client.new(environment: Environment::PRODUCTION)
client = ThePlaidAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ThePlaidAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ThePlaidAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ThePlaidAPI::ProxySettings.from_env
client = ThePlaidAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ThePlaidAPI::HttpCallBack
...
client = ThePlaidAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ThePlaidAPI::Client.from_env
# Overrides take precedence over env vars:
client = ThePlaidAPI::Client.from_env(timeout: 30, max_retries: 3)
```
