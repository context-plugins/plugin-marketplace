...
kwargs to the `GeoapifyClient` constructor (or to `Configuration`). Exact credential class names and
...
client = GeoapifyClient(o_auth_acg_credentials=creds)
...
client = GeoapifyClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GeoapifyClient.from_environment()`) read each credential
