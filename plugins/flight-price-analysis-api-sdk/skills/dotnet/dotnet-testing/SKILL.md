...
`FlightPriceAnalysisApiClient`/`FlightPriceAnalysisApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `FlightPriceAnalysisApiClient`,
...
static FlightPriceAnalysisApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new FlightPriceAnalysisApiClient(new HttpClient(handler), new FlightPriceAnalysisApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using FlightPriceAnalysisApi.Errors;              // {Operation}Error types
...
```csharp
using FlightPriceAnalysisApi.Core.Exceptions;
using FlightPriceAnalysisApi.Core.ErrorResponse;
...
using FlightPriceAnalysisApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new FlightPriceAnalysisApiClient(new HttpClient(handler), new FlightPriceAnalysisApiClientOptions());
...
- For DI-based code, the SDK's `AddFlightPriceAnalysisApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `FlightPriceAnalysisApiClient` from the provider:
  ```csharp
  services.AddFlightPriceAnalysisApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<FlightPriceAnalysisApiClient>();
  ```
