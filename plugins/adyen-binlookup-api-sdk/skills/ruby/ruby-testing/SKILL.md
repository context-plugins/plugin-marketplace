...
client = stub_client(stubs)
assert_raises AdyenBinLookupAPI::APIException do
...
def build_test_client
  AdyenBinLookupAPI::Client.new(
    environment: AdyenBinLookupAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error AdyenBinLookupAPI::APIException
```
...
```ruby
expect { call }.to raise_error(AdyenBinLookupAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < AdyenBinLookupAPI::HttpCallBack
...
client  = AdyenBinLookupAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
