...
To see which schemes a specific SDK accepts, read the credential setter methods on `SquareClient.Builder`
...
- Set credentials **on the `SquareClient.Builder`** before calling `.build()` — the client is immutable
