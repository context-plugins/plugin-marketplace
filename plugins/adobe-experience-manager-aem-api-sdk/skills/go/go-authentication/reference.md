...
```go
adobeexperiencemanageraemapi.WithOAuthCCGCredentials(
    adobeexperiencemanageraemapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr adobeexperiencemanageraemapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := adobeexperiencemanageraemapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr adobeexperiencemanageraemapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := adobeexperiencemanageraemapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := adobeexperiencemanageraemapi.NewClient(adobeexperiencemanageraemapi.CreateConfiguration(adobeexperiencemanageraemapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    adobeexperiencemanageraemapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := adobeexperiencemanageraemapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := adobeexperiencemanageraemapi.NewClient(adobeexperiencemanageraemapi.CreateConfiguration(adobeexperiencemanageraemapi.WithOAuthROPCGCredentials(creds)))
...
```go
adobeexperiencemanageraemapi.WithOAuthBearerTokenCredentials(adobeexperiencemanageraemapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
adobeexperiencemanageraemapi.WithBasicAuthCredentials(adobeexperiencemanageraemapi.NewBasicAuthCredentials(username, password))
adobeexperiencemanageraemapi.WithApiHeaderCredentials(adobeexperiencemanageraemapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
adobeexperiencemanageraemapi.WithApiKeyCredentials(adobeexperiencemanageraemapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `ADOBEEXPERIENCEMANAGERAEMAPI_...` variables — e.g.
`ADOBEEXPERIENCEMANAGERAEMAPI_USERNAME`, `ADOBEEXPERIENCEMANAGERAEMAPI_PASSWORD`, `ADOBEEXPERIENCEMANAGERAEMAPI_TOKEN`, `ADOBEEXPERIENCEMANAGERAEMAPI_API_KEY`,
`ADOBEEXPERIENCEMANAGERAEMAPI_O_AUTH_CLIENT_ID`, `ADOBEEXPERIENCEMANAGERAEMAPI_O_AUTH_CLIENT_SECRET`, `ADOBEEXPERIENCEMANAGERAEMAPI_ACCESS_TOKEN`. Grep the
