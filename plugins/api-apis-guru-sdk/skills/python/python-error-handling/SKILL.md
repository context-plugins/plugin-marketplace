...
`apiapisguru/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `apiapisguru/exceptions/` and operation docstrings.
...
```python
from apiapisguru.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `apiapisguru/exceptions/`.
...
```python
from apiapisguru.exceptions.api_exception import APIException
from apiapisguru.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`apiapisguru/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`apiapisguru/exceptions/` for the class.
...
`apiapisguru/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `apiapisguru/exceptions/`
  or a `ValueError` — check the auth handler in `apiapisguru/http/auth/` for the exact type.
