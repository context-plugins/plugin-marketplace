...
`teslafleetmanagementapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `teslafleetmanagementapi/exceptions/` and operation docstrings.
...
```python
from teslafleetmanagementapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `teslafleetmanagementapi/exceptions/`.
...
```python
from teslafleetmanagementapi.exceptions.api_exception import APIException
from teslafleetmanagementapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`teslafleetmanagementapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`teslafleetmanagementapi/exceptions/` for the class.
...
`teslafleetmanagementapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `teslafleetmanagementapi/exceptions/`
  or a `ValueError` — check the auth handler in `teslafleetmanagementapi/http/auth/` for the exact type.
