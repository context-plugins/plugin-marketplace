...
```ruby
client = Shutterstock::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Shutterstock::Client.new(adapter: :net_http)
```
...
client = Shutterstock::Client.new(connection: conn)
```
...
client = Shutterstock::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Shutterstock::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Shutterstock::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Shutterstock::ProxySettings.from_env
client = Shutterstock::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Shutterstock::HttpCallBack
...
client = Shutterstock::Client.new(http_callback: LogCallback.new)
```
...
client = Shutterstock::Client.from_env(timeout: 30, max_retries: 3)
```
