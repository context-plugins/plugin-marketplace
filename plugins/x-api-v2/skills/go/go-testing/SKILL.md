...
`testing` + `testify/require` for reference; substitute the real `xapiv2` and names.
...
import (
    ...
    "github.com/context-plugins/x-api-v2-go-sdk"   // root package xapiv2
)
...
func clientReturning(status int, body string) (xapiv2.ClientInterface, *stubTransport) {
    ...
    client := xapiv2.NewClient(
        xapiv2.CreateConfiguration(
            xapiv2.WithHttpConfiguration(
                xapiv2.CreateHttpConfiguration(xapiv2.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/x-api-v2-go-sdk/errors"
...
client := xapiv2.NewClient(
    xapiv2.CreateConfiguration(
        xapiv2.WithHttpConfiguration(
            xapiv2.CreateHttpConfiguration(xapiv2.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
