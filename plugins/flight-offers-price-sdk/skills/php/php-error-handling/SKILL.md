...
> `{operation}`, `{apiGroup}`, `FlightOffersPriceLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `FlightOffersPriceLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `FlightOffersPriceLib\Exceptions\` — read the source to
