...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `geoapifylib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'geoapifylib';
import { ApiError } from 'geoapifylib';
import { {Operation}Error } from 'geoapifylib/errors';
