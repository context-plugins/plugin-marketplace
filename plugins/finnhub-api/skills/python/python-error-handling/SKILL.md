...
`finnhubapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `finnhubapi/exceptions/` and operation docstrings.
...
```python
from finnhubapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `finnhubapi/exceptions/`.
...
```python
from finnhubapi.exceptions.api_exception import APIException
from finnhubapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`finnhubapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`finnhubapi/exceptions/` for the class.
...
`finnhubapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `finnhubapi/exceptions/`
  or a `ValueError` — check the auth handler in `finnhubapi/http/auth/` for the exact type.
