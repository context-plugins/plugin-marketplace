...
```go
alpaca.WithOAuthCCGCredentials(
    alpaca.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr alpaca.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := alpaca.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr alpaca.OAuthCCGManager) models.OAuthToken {
...
```go
creds := alpaca.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := alpaca.NewClient(alpaca.CreateConfiguration(alpaca.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    alpaca.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := alpaca.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := alpaca.NewClient(alpaca.CreateConfiguration(alpaca.WithOAuthROPCGCredentials(creds)))
...
```go
alpaca.WithOAuthBearerTokenCredentials(alpaca.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
alpaca.WithBasicAuthCredentials(alpaca.NewBasicAuthCredentials(username, password))
alpaca.WithApiHeaderCredentials(alpaca.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
alpaca.WithApiKeyCredentials(alpaca.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `ALPACA_...` variables — e.g.
`ALPACA_USERNAME`, `ALPACA_PASSWORD`, `ALPACA_TOKEN`, `ALPACA_API_KEY`,
`ALPACA_O_AUTH_CLIENT_ID`, `ALPACA_O_AUTH_CLIENT_SECRET`, `ALPACA_ACCESS_TOKEN`. Grep the
