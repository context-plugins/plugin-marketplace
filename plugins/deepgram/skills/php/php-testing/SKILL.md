...
> `RestApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): RestApiClient
{
...
```php
use RestApiLib\Exceptions\{OperationException};
use RestApiLib\Exceptions\ApiException;
...
app()->instance(RestApiClient::class, clientWithResponses([
