...
Open `lib/1forge_finance_apis/client.rb` for the accessor names, then the relevant
`lib/1forge_finance_apis/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = 1ForgeFinanceAPIs::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: 1ForgeFinanceAPIs::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/1forge_finance_apis/models/`. Field names are defined as
...
Read the enum module in `lib/1forge_finance_apis/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/1forge_finance_apis/client.rb`.
- Operation methods are in `lib/1forge_finance_apis/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/1forge_finance_apis/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/1forge_finance_apis/exceptions/`.
