...
Open `flightpriceanalysisapi/flightpriceanalysisapi_client.py` for the controller property names, then the relevant
`flightpriceanalysisapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from flightpriceanalysisapi.models.{request_model} import {RequestModel}
...
Open `flightpriceanalysisapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from flightpriceanalysisapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from flightpriceanalysisapi.http.http_call_back import HttpCallBack
...
client = FlightPriceAnalysisAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `flightpriceanalysisapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `FlightPriceAnalysisAPIClient` in `flightpriceanalysisapi/flightpriceanalysisapi_client.py`.
- Operation method signatures are in `flightpriceanalysisapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `flightpriceanalysisapi/models/`; enum types have `from_value()`.
