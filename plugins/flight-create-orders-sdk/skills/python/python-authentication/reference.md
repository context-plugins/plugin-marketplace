...
kwargs to the `FlightCreateOrdersClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `flightcreateorders/flightcreateorders_client.py` and `flightcreateorders/http/auth/` in
the cloned source.

## Basic auth

```python
from flightcreateorders.http.auth.basic_auth import BasicAuthCredentials
...
```python
from flightcreateorders.http.auth.api_key import ApiKeyCredentials
...
```python
from flightcreateorders.http.auth.api_header import ApiHeaderCredentials
...
```python
from flightcreateorders.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from flightcreateorders.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from flightcreateorders.http.auth.o_auth_acg import OAuthACGCredentials
from flightcreateorders.models.o_auth_scope_flightcreateorders_enum import OAuthScope{Pkg}Enum
...
client = FlightCreateOrdersClient(o_auth_acg_credentials=creds)
...
```python
from flightcreateorders.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FlightCreateOrdersClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FlightCreateOrdersClient.from_environment()`) read each credential
...
Grep `from_environment` in `flightcreateorders/configuration.py` and each `flightcreateorders/http/auth/*.py` for the exact
