...
- `pq-api-v-2-lib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'pq-api-v-2-lib';
...
```ts
import { {Resource}Controller } from 'pq-api-v-2-lib';
