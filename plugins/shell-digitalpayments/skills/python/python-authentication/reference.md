...
kwargs to the `ShellsmartpayapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `shelldigitalpayments/shelldigitalpayments_client.py` and `shelldigitalpayments/http/auth/` in
the cloned source.

## Basic auth

```python
from shelldigitalpayments.http.auth.basic_auth import BasicAuthCredentials
...
```python
from shelldigitalpayments.http.auth.api_key import ApiKeyCredentials
...
```python
from shelldigitalpayments.http.auth.api_header import ApiHeaderCredentials
...
```python
from shelldigitalpayments.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from shelldigitalpayments.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from shelldigitalpayments.http.auth.o_auth_acg import OAuthACGCredentials
from shelldigitalpayments.models.o_auth_scope_shelldigitalpayments_enum import OAuthScope{Pkg}Enum
...
client = ShellsmartpayapiClient(o_auth_acg_credentials=creds)
...
```python
from shelldigitalpayments.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ShellsmartpayapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ShellsmartpayapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `shelldigitalpayments/configuration.py` and each `shelldigitalpayments/http/auth/*.py` for the exact
