...
`PayPalApiClient`/`PayPalApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `PayPalApiClient`,
...
static PayPalApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new PayPalApiClient(new HttpClient(handler), new PayPalApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using PayPalApi.Errors;              // {Operation}Error types
...
```csharp
using PayPalApi.Core.Exceptions;
using PayPalApi.Core.ErrorResponse;
...
using PayPalApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new PayPalApiClient(new HttpClient(handler), new PayPalApiClientOptions());
...
- For DI-based code, the SDK's `AddPayPalApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `PayPalApiClient` from the provider:
  ```csharp
  services.AddPayPalApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<PayPalApiClient>();
  ```
