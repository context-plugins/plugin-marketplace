...
> `{ApiGroup}`, `OnDemandFlightStatus`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `OnDemandFlightStatus.Core.Exceptions`
- `ApiError` **and** `RawError` → `OnDemandFlightStatus.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `OnDemandFlightStatus.Errors`
...
```csharp
using OnDemandFlightStatus.Core.Exceptions;     // SdkException<TError>
using OnDemandFlightStatus.Core.ErrorResponse;  // ApiError, RawError
using OnDemandFlightStatus.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`OnDemandFlightStatus.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using OnDemandFlightStatus.Core.Exceptions;     // SdkException<TError>
using OnDemandFlightStatus.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `OnDemandFlightStatus.Core.ErrorResponse`. Its public
...
`readonly struct` under `OnDemandFlightStatus.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using OnDemandFlightStatus.Core.Models;        // ApiResult<TResponse, TError>
using OnDemandFlightStatus.Core.ErrorResponse; // RawError
using OnDemandFlightStatus.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `OnDemandFlightStatus.Core.Exceptions`) — an auth *application* failure, not an API error — when the
