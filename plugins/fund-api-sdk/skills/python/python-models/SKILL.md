...
your SDK source (`fundapi/models/`).
...
```python
from fundapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `fundapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from fundapi.models.animal import Cat, Dog
...
```python
from fundapi.models.animal import Animal, Cat, Dog
