...
`HostedOnboardingApiClient`/`HostedOnboardingApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `HostedOnboardingApiClient`,
...
static HostedOnboardingApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new HostedOnboardingApiClient(new HttpClient(handler), new HostedOnboardingApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using HostedOnboardingApi.Errors;              // {Operation}Error types
...
```csharp
using HostedOnboardingApi.Core.Exceptions;
using HostedOnboardingApi.Core.ErrorResponse;
...
using HostedOnboardingApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new HostedOnboardingApiClient(new HttpClient(handler), new HostedOnboardingApiClientOptions());
...
- For DI-based code, the SDK's `AddHostedOnboardingApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `HostedOnboardingApiClient` from the provider:
  ```csharp
  services.AddHostedOnboardingApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<HostedOnboardingApiClient>();
  ```
