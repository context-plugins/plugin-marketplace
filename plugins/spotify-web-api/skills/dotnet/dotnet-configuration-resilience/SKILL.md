...
All types below live under `SpotifyWebApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using SpotifyWebApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new SpotifyWebApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `SpotifyWebApi.Core.Exceptions`):
...
With DI, the SDK's `AddSpotifyWebApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddSpotifyWebApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
