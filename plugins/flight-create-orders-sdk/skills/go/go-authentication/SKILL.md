...
> Throughout, `flightcreateorders` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := flightcreateorders.NewClient(
    flightcreateorders.CreateConfiguration(
        flightcreateorders.WithBasicAuthCredentials(
            flightcreateorders.NewBasicAuthCredentials(
...
flightcreateorders.WithApiKeyCredentials(flightcreateorders.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
flightcreateorders.WithApiHeaderCredentials(flightcreateorders.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
flightcreateorders.WithOAuthBearerTokenCredentials(
    flightcreateorders.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := flightcreateorders.NewClient(
    flightcreateorders.CreateConfiguration(
        flightcreateorders.WithOAuthCCGCredentials(
            flightcreateorders.NewOAuthCCGCredentials(
