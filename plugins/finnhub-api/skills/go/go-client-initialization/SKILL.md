...
- `finnhubapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/finnhub-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/finnhub-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/finnhub-api-go-sdk"

client := finnhubapi.NewClient(
    finnhubapi.CreateConfiguration(
        finnhubapi.WithEnvironment(finnhubapi.PRODUCTION),
...
from environment variables named `FINNHUBAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := finnhubapi.NewClient(finnhubapi.CreateConfigurationFromEnvironment())
```
...
```go
client := finnhubapi.NewClient(
    finnhubapi.CreateConfiguration(finnhubapi.WithEnvironment(finnhubapi.PRODUCTION)),
)
```
...
client := finnhubapi.NewClient(
    finnhubapi.CreateConfiguration(
        finnhubapi.WithHttpConfiguration(
            finnhubapi.CreateHttpConfiguration(
                finnhubapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                finnhubapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = finnhubapi.NewClient(finnhubapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api finnhubapi.ClientInterface
}

func NewService(api finnhubapi.ClientInterface) *Service {
