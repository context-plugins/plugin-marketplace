...
```ruby
client = SubscriptionClient::Client.new(
  max_retries:    3,
...
```ruby
client = SubscriptionClient::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = SubscriptionClient::Client.new(adapter: :net_http)
```
...
client = SubscriptionClient::Client.new(connection: conn)
```
...
client = SubscriptionClient::Client.new(environment: Environment::PRODUCTION)
client = SubscriptionClient::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = SubscriptionClient::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = SubscriptionClient::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = SubscriptionClient::ProxySettings.from_env
client = SubscriptionClient::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < SubscriptionClient::HttpCallBack
...
client = SubscriptionClient::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = SubscriptionClient::Client.from_env
# Overrides take precedence over env vars:
client = SubscriptionClient::Client.from_env(timeout: 30, max_retries: 3)
```
