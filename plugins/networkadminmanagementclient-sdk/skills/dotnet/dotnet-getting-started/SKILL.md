...
| Project | `NetworkAdminManagementClient.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `NetworkAdminManagementClient` (the `using` namespace) |
| Client class | `NetworkAdminManagementClientClient` |
| Options class | `NetworkAdminManagementClientClientOptions` |
...
namespaces transitively, so `using NetworkAdminManagementClient.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/networkadminmanagementclient-csharp
dotnet add reference networkadminmanagementclient-csharp/NetworkAdminManagementClient.csproj
```

```csharp
using NetworkAdminManagementClient;          // client + options + ServerEnvironment
using NetworkAdminManagementClient.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/networkadminmanagementclient-csharp /tmp/networkadminmanagementclient-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/networkadminmanagementclient-csharp "$env:TEMP\networkadminmanagementclient-dotnet-src"
```
...
- `NetworkAdminManagementClientClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/networkadminmanagementclient-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\networkadminmanagementclient-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `NetworkAdminManagementClientClient`, build its
   `NetworkAdminManagementClientClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
