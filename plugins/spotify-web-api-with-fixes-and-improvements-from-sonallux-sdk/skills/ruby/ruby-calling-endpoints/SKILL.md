...
Open `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/client.rb` for the accessor names, then the relevant
`lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: SpotifyWebAPIWithFixesAndImprovementsFromSonallux::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/models/`. Field names are defined as
...
Read the enum module in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/client.rb`.
- Operation methods are in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/exceptions/`.
