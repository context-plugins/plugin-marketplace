...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new StaxFattMerchantAPIClient({
...
```typescript
import { {Operation}Error } from 'stax-fattmerchant-apilib/errors';
...
```typescript
import { ApiError } from 'stax-fattmerchant-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(StaxFattMerchantAPIClient)
    .useValue(new StaxFattMerchantAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
