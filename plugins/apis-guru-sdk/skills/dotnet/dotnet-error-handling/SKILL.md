...
> `{ApiGroup}`, `ApisGuru`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `ApisGuru.Core.Exceptions`
- `ApiError` **and** `RawError` → `ApisGuru.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `ApisGuru.Errors`
...
```csharp
using ApisGuru.Core.Exceptions;     // SdkException<TError>
using ApisGuru.Core.ErrorResponse;  // ApiError, RawError
using ApisGuru.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`ApisGuru.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using ApisGuru.Core.Exceptions;     // SdkException<TError>
using ApisGuru.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `ApisGuru.Core.ErrorResponse`. Its public
...
`readonly struct` under `ApisGuru.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using ApisGuru.Core.Models;        // ApiResult<TResponse, TError>
using ApisGuru.Core.ErrorResponse; // RawError
using ApisGuru.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `ApisGuru.Core.Exceptions`) — an auth *application* failure, not an API error — when the
