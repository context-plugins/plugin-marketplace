...
against the `SquareClient.Builder` setters, the `com.squareup.connect.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SquareClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
SquareClient client = new SquareClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeSquareEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
