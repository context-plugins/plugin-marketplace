...
reference; substitute your `splititwebapiv3` and `SplititWebApiV3Client` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `splititwebapiv3/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
