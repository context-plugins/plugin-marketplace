...
- `adyendataprotectionapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/adyen-data-protection-api-go` — the Go import path for the root package; `github.com/context-plugins/adyen-data-protection-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/adyen-data-protection-api-go"

client := adyendataprotectionapi.NewClient(
    adyendataprotectionapi.CreateConfiguration(
        adyendataprotectionapi.WithEnvironment(adyendataprotectionapi.PRODUCTION),
...
from environment variables named `ADYENDATAPROTECTIONAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := adyendataprotectionapi.NewClient(adyendataprotectionapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `adyendataprotectionapi.PRODUCTION`,
`adyendataprotectionapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := adyendataprotectionapi.NewClient(
    adyendataprotectionapi.CreateConfiguration(adyendataprotectionapi.WithEnvironment(adyendataprotectionapi.PRODUCTION)),
)
```
...
client := adyendataprotectionapi.NewClient(
    adyendataprotectionapi.CreateConfiguration(
        adyendataprotectionapi.WithHttpConfiguration(
            adyendataprotectionapi.CreateHttpConfiguration(
                adyendataprotectionapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                adyendataprotectionapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = adyendataprotectionapi.NewClient(adyendataprotectionapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api adyendataprotectionapi.ClientInterface
}

func NewService(api adyendataprotectionapi.ClientInterface) *Service {
