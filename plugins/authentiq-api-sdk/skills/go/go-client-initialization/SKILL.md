...
- `authentiqapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/authentiq-api-go` — the Go import path for the root package; `github.com/context-plugins/authentiq-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/authentiq-api-go"

client := authentiqapi.NewClient(
    authentiqapi.CreateConfiguration(
        authentiqapi.WithEnvironment(authentiqapi.PRODUCTION),
...
from environment variables named `AUTHENTIQAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := authentiqapi.NewClient(authentiqapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `authentiqapi.PRODUCTION`,
`authentiqapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := authentiqapi.NewClient(
    authentiqapi.CreateConfiguration(authentiqapi.WithEnvironment(authentiqapi.PRODUCTION)),
)
```
...
client := authentiqapi.NewClient(
    authentiqapi.CreateConfiguration(
        authentiqapi.WithHttpConfiguration(
            authentiqapi.CreateHttpConfiguration(
                authentiqapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                authentiqapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = authentiqapi.NewClient(authentiqapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api authentiqapi.ClientInterface
}

func NewService(api authentiqapi.ClientInterface) *Service {
