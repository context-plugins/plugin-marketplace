...
All types below live under `ToursAndActivities.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using ToursAndActivities.Core.Configuration;
...
```csharp
using ToursAndActivities.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new ToursAndActivitiesClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `ToursAndActivities.Core.Exceptions`):
...
With DI, the SDK's `AddToursAndActivitiesClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddToursAndActivitiesClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
