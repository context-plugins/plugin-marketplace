...
- `klarnapaymentsapiv1` — the root package name (e.g. `multiauthsample`, `batester`).
- `KlarnaPaymentsAPIV1Client` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `KlarnaPaymentsAPIV1Client` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from klarnapaymentsapiv1.klarnapaymentsapiv1_client import KlarnaPaymentsAPIV1Client
from klarnapaymentsapiv1.configuration import Environment
from klarnapaymentsapiv1.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = KlarnaPaymentsAPIV1Client(
...
```python
from klarnapaymentsapiv1.configuration import Configuration, Environment
...
client = KlarnaPaymentsAPIV1Client(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `klarnapaymentsapiv1/klarnapaymentsapiv1_client.py` in the cloned source.
...
`from_environment()` is a class method on both `KlarnaPaymentsAPIV1Client` and `Configuration` that reads
...
client = KlarnaPaymentsAPIV1Client.from_environment()
...
`from_environment` in `klarnapaymentsapiv1/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `KlarnaPaymentsAPIV1Client`. Access them as attributes —
...
Open `klarnapaymentsapiv1/klarnapaymentsapiv1_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `klarnapaymentsapiv1/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from klarnapaymentsapiv1.configuration import Environment

client = KlarnaPaymentsAPIV1Client(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`klarnapaymentsapiv1/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = KlarnaPaymentsAPIV1Client(config=new_config)
```
