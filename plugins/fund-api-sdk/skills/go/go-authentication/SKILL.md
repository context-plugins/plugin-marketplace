...
> Throughout, `fundapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := fundapi.NewClient(
    fundapi.CreateConfiguration(
        fundapi.WithBasicAuthCredentials(
            fundapi.NewBasicAuthCredentials(
...
fundapi.WithApiKeyCredentials(fundapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
fundapi.WithApiHeaderCredentials(fundapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
fundapi.WithOAuthBearerTokenCredentials(
    fundapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := fundapi.NewClient(
    fundapi.CreateConfiguration(
        fundapi.WithOAuthCCGCredentials(
            fundapi.NewOAuthCCGCredentials(
