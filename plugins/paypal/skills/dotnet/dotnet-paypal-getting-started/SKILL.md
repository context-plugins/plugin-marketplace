...
dotnet add reference paypal-csharp-sdk/PayPalServerSdk.csproj
```
...
temp directory** (`<temp>/paypal-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/paypal-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/paypal-dotnet-src/$(date +%Y%m%d-%H%M%S)"
...
```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\paypal-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/paypal-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/paypal-dotnet-src/`, **never** in the project repo, and its path never goes into
`paypal-dotnet-plan.md` — the main agent must never see the clone or its path.
