...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ZalandoShopClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`zalandoshop/configuration.py` in the cloned source.
...
```python
import requests
from zalandoshop.configuration import Configuration
...
`zalandoshop/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`zalandoshop/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `zalandoshop/configuration.py`
...
`NumberPagedResponse`) live in `zalandoshop/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `zalandoshop/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
