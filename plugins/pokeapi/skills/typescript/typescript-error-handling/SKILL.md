...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `pok-apilib`) — replace it with the concrete identifier from the source.
...
import { ApiError } from 'pok-apilib';
import { {Operation}Error } from 'pok-apilib/errors';
