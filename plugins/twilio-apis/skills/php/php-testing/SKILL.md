...
> `TwilioApIsClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): TwilioApIsClient
{
...
```php
use TwilioApIsLib\Exceptions\{OperationException};
use TwilioApIsLib\Exceptions\ApiException;
...
app()->instance(TwilioApIsClient::class, clientWithResponses([
