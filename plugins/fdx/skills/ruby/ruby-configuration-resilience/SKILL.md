...
```ruby
client = Fdx::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Fdx::Client.new(adapter: :net_http)
```
...
client = Fdx::Client.new(connection: conn)
```
...
client = Fdx::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Fdx::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Fdx::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Fdx::ProxySettings.from_env
client = Fdx::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Fdx::HttpCallBack
...
client = Fdx::Client.new(http_callback: LogCallback.new)
```
...
client = Fdx::Client.from_env(timeout: 30, max_retries: 3)
```
