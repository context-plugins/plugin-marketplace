...
reference; substitute your `openaiapi` and `OpenAIAPIClient` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `openaiapi/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
