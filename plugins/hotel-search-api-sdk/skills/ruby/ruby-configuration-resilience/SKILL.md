...
```ruby
client = HotelSearchAPI::Client.new(
  max_retries:    3,
...
```ruby
client = HotelSearchAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = HotelSearchAPI::Client.new(adapter: :net_http)
```
...
client = HotelSearchAPI::Client.new(connection: conn)
```
...
client = HotelSearchAPI::Client.new(environment: Environment::PRODUCTION)
client = HotelSearchAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = HotelSearchAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = HotelSearchAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = HotelSearchAPI::ProxySettings.from_env
client = HotelSearchAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < HotelSearchAPI::HttpCallBack
...
client = HotelSearchAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = HotelSearchAPI::Client.from_env
# Overrides take precedence over env vars:
client = HotelSearchAPI::Client.from_env(timeout: 30, max_retries: 3)
```
