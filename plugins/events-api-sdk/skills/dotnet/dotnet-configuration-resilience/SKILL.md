...
All types below live under `EventsApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using EventsApi.Core.Configuration;
...
```csharp
using EventsApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new EventsApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `EventsApi.Core.Exceptions`):
...
With DI, the SDK's `AddEventsApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddEventsApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
