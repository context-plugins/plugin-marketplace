...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `OpenMeteoWeatherForecastAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`openmeteoweatherforecastapi/configuration.py` in the cloned source.
...
```python
import requests
from openmeteoweatherforecastapi.configuration import Configuration
...
`openmeteoweatherforecastapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`openmeteoweatherforecastapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `openmeteoweatherforecastapi/configuration.py`
...
`NumberPagedResponse`) live in `openmeteoweatherforecastapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `openmeteoweatherforecastapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
