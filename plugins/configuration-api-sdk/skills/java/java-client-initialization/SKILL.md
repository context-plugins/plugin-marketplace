...
- `ConfigurationAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`ConfigurationAPIClient.Builder` class. A minimal initialization:

```java
import com.adyen.balanceplatformapitest.ConfigurationAPIClient;
import com.adyen.balanceplatformapitest.Environment;

ConfigurationAPIClient client = new ConfigurationAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `ConfigurationAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `ConfigurationAPIClient.java` |

Call `.build()` to produce an immutable `ConfigurationAPIClient` instance.
...
public class ApiConfig {
    ...
    public ConfigurationAPIClient apiClient() {
        return new ConfigurationAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired ConfigurationAPIClient client) {
        client.shutdown();
    }
}
```

Inject `ConfigurationAPIClient` into your services as a normal Spring dependency. Build only one bean.
