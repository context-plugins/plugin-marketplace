...
| | |
| --- | --- |
| API | `Klarna Payments API V1` |
| Project | `KlarnaPaymentsApiV1.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `KlarnaPaymentsApiV1` (the `using` namespace) |
| Client class | `KlarnaPaymentsApiV1Client` |
| Options class | `KlarnaPaymentsApiV1ClientOptions` |
...
namespaces transitively, so `using KlarnaPaymentsApiV1.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/klarna-payments-api-v1-csharp
dotnet add reference klarna-payments-api-v1-csharp/KlarnaPaymentsApiV1.csproj
```

```csharp
using KlarnaPaymentsApiV1;          // client + options + ServerEnvironment
using KlarnaPaymentsApiV1.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/klarna-payments-api-v1-csharp /tmp/klarna-payments-api-v1-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/klarna-payments-api-v1-csharp "$env:TEMP\klarna-payments-api-v1-dotnet-src"
```
...
- `KlarnaPaymentsApiV1Client.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/klarna-payments-api-v1-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\klarna-payments-api-v1-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `KlarnaPaymentsApiV1Client`, build its
   `KlarnaPaymentsApiV1ClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
