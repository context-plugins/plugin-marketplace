...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new FlightOffersPriceClient({
...
```typescript
import { {Operation}Error } from 'flight-offers-pricelib/errors';
...
```typescript
import { ApiError } from 'flight-offers-pricelib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(FlightOffersPriceClient)
    .useValue(new FlightOffersPriceClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
