...
kwarg names are generated per-API — confirm from `alpaca/alpaca_client.py` and `alpaca/http/auth/` in
the cloned source.

## Basic auth

```python
from alpaca.http.auth.basic_auth import BasicAuthCredentials
...
```python
from alpaca.http.auth.api_key import ApiKeyCredentials
...
```python
from alpaca.http.auth.api_header import ApiHeaderCredentials
...
```python
from alpaca.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from alpaca.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from alpaca.http.auth.o_auth_acg import OAuthACGCredentials
from alpaca.models.o_auth_scope_alpaca_enum import OAuthScope{Pkg}Enum
...
```python
from alpaca.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `alpaca/configuration.py` and each `alpaca/http/auth/*.py` for the exact
