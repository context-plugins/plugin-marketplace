...
- `pqapiv2` — the root package name (e.g. `multiauthsample`, `batester`).
- `PQAPIV2Client` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
...
```python
from pqapiv2.pqapiv2_client import PQAPIV2Client
from pqapiv2.configuration import Environment
from pqapiv2.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from pqapiv2.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `pqapiv2/pqapiv2_client.py` in the cloned source.
...
`from_environment` in `pqapiv2/configuration.py` for the exact names.
...
Open `pqapiv2/pqapiv2_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `pqapiv2/configuration.py`. Use the constant for the desired
...
`pqapiv2/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
