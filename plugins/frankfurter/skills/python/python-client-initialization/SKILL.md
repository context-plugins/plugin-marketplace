...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the FrankfurterapiClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write FrankfurterapiClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `FrankfurterapiClient` is the SDK's client class — **read the real name** from `frankfurter/frankfurter_client.py`
...
- `frankfurter` — the root package name (e.g. `multiauthsample`, `batester`).
- `FrankfurterapiClient` — the generated client class (read it from the `class …Client` declaration in `frankfurter/frankfurter_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `FrankfurterapiClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from frankfurter.frankfurter_client import FrankfurterapiClient
from frankfurter.configuration import Environment
from frankfurter.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from frankfurter.configuration import Configuration, Environment
...
client = FrankfurterapiClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `frankfurter/frankfurter_client.py` in the cloned source.
...
`from_environment()` is a class method on both `FrankfurterapiClient` and `Configuration` that reads
...
client = FrankfurterapiClient.from_environment()
...
`from_environment` in `frankfurter/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `FrankfurterapiClient`. Access them as attributes —
...
Open `frankfurter/frankfurter_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `frankfurter/configuration.py`. **Read the enum in `configuration.py`
...
client = FrankfurterapiClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`frankfurter/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = FrankfurterapiClient(config=new_config)
```
