...
`FlightOrderManagementClient`/`FlightOrderManagementClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `FlightOrderManagementClient`,
...
static FlightOrderManagementClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new FlightOrderManagementClient(new HttpClient(handler), new FlightOrderManagementClientOptions { /* auth not needed for stubs */ });
}
```
...
using FlightOrderManagement.Errors;              // {Operation}Error types
...
```csharp
using FlightOrderManagement.Core.Exceptions;
using FlightOrderManagement.Core.ErrorResponse;
...
using FlightOrderManagement.Core.ErrorResponse; // RawError (Case B)
...
var client = new FlightOrderManagementClient(new HttpClient(handler), new FlightOrderManagementClientOptions());
...
- For DI-based code, the SDK's `AddFlightOrderManagementClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `FlightOrderManagementClient` from the provider:
  ```csharp
  services.AddFlightOrderManagementClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<FlightOrderManagementClient>();
  ```
