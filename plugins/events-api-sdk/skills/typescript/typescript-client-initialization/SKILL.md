...
- `events-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'events-apilib';
...
```ts
import { {Resource}Controller } from 'events-apilib';
