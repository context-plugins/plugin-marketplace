...
- `xero` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/xero-go` — the Go import path for the root package; `github.com/context-plugins/xero-go/models` for request/response types.
...
```go
import "github.com/context-plugins/xero-go"

client := xero.NewClient(
    xero.CreateConfiguration(
        xero.WithEnvironment(xero.PRODUCTION),
...
from environment variables named `XERO_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := xero.NewClient(xero.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `xero.PRODUCTION`,
`xero.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := xero.NewClient(
    xero.CreateConfiguration(xero.WithEnvironment(xero.PRODUCTION)),
)
```
...
client := xero.NewClient(
    xero.CreateConfiguration(
        xero.WithHttpConfiguration(
            xero.CreateHttpConfiguration(
                xero.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                xero.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = xero.NewClient(xero.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api xero.ClientInterface
}

func NewService(api xero.ClientInterface) *Service {
