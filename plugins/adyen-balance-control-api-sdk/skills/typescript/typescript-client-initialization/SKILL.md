...
- `adyen-balance-control-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'adyen-balance-control-apilib';
...
```ts
import { {Resource}Controller } from 'adyen-balance-control-apilib';
