...
To see which schemes a specific SDK accepts, read the credential setter methods on `HostedOnboardingAPIClient.Builder`
...
- Set credentials **on the `HostedOnboardingAPIClient.Builder`** before calling `.build()` — the client is immutable
