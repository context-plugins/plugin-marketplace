...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new HotelSearchAPIClient({
...
```typescript
import { {Operation}Error } from 'hotel-search-apilib/errors';
...
```typescript
import { ApiError } from 'hotel-search-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(HotelSearchAPIClient)
    .useValue(new HotelSearchAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
