...
client = stub_client(stubs)
assert_raises GoogleMapsPlatform::APIException do
...
BASE = GoogleMapsPlatform::Configuration.new(environment: GoogleMapsPlatform::Environment::PRODUCTION).get_base_uri

def build_test_client
  GoogleMapsPlatform::Client.new(
    environment: GoogleMapsPlatform::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error GoogleMapsPlatform::APIException
```
...
```ruby
expect { call }.to raise_error(GoogleMapsPlatform::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < GoogleMapsPlatform::HttpCallBack
...
client  = GoogleMapsPlatform::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
