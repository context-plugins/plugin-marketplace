...
<!-- crawler:auth -->
| Auth | Credentials properties on `CellPointApiClientOptions`: `CpdIssuedJwt: OAuth2ClientCredentials?`, `CpdIssuedJwtTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?`, `MerchantIssuedJwt: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using CellPointApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/cellpoint-csharp-sdk
dotnet add reference cellpoint-csharp-sdk/CellPointApi.csproj
```
...
temp directory** (`<temp>/cellpoint-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/cellpoint-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/cellpoint-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/cellpoint-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\cellpoint-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/cellpoint-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/cellpoint-dotnet-src/`, **never** in the project repo, and its path never goes into
`cellpoint-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `CellPointApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new CellPointApiClient(...)`, build its options, or DI-register via
   `AddCellPointApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `CellPointApiClientOptions` — the map's *Servers & auth* section
