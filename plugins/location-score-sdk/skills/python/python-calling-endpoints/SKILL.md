...
Open `locationscore/locationscore_client.py` for the controller property names, then the relevant
`locationscore/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from locationscore.models.{request_model} import {RequestModel}
...
Open `locationscore/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from locationscore.models.suite_code_enum import SuiteCodeEnum
...
```python
from locationscore.http.http_call_back import HttpCallBack
...
client = LocationScoreClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `locationscore/pagination/` in the source.

## Finding the right method

- Controller property names are on `LocationScoreClient` in `locationscore/locationscore_client.py`.
- Operation method signatures are in `locationscore/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `locationscore/models/`; enum types have `from_value()`.
