...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `shell-card-management-apislib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'shell-card-management-apislib';
import { ApiError } from 'shell-card-management-apislib';
import { {Operation}Error } from 'shell-card-management-apislib/errors';
