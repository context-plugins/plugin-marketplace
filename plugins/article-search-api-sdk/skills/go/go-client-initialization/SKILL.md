...
- `articlesearchapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/article-search-api-go` — the Go import path for the root package; `github.com/context-plugins/article-search-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/article-search-api-go"

client := articlesearchapi.NewClient(
    articlesearchapi.CreateConfiguration(
        articlesearchapi.WithEnvironment(articlesearchapi.PRODUCTION),
...
from environment variables named `ARTICLESEARCHAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := articlesearchapi.NewClient(articlesearchapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `articlesearchapi.PRODUCTION`,
`articlesearchapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := articlesearchapi.NewClient(
    articlesearchapi.CreateConfiguration(articlesearchapi.WithEnvironment(articlesearchapi.PRODUCTION)),
)
```
...
client := articlesearchapi.NewClient(
    articlesearchapi.CreateConfiguration(
        articlesearchapi.WithHttpConfiguration(
            articlesearchapi.CreateHttpConfiguration(
                articlesearchapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                articlesearchapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = articlesearchapi.NewClient(articlesearchapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api articlesearchapi.ClientInterface
}

func NewService(api articlesearchapi.ClientInterface) *Service {
