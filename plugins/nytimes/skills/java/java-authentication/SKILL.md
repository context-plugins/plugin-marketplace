...
To see which schemes a specific SDK accepts, read the credential setter methods on `NytimesClient.Builder`
...
- Set credentials **on the `NytimesClient.Builder`** before calling `.build()` — the client is immutable
