...
Open `paze/paze_client.py` for the controller property names, then the relevant
`paze/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from paze.models.{request_model} import {RequestModel}
...
Open `paze/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from paze.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `paze/pagination/` in the source.

## Finding the right method

- Controller property names are on `PazecheckoutapiClient` in `paze/paze_client.py`.
- Operation method signatures are in `paze/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `paze/models/`; enum types have `from_value()`.
