...
Open `staxfattmerchantapi/staxfattmerchantapi_client.py` for the controller property names, then the relevant
`staxfattmerchantapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from staxfattmerchantapi.models.{request_model} import {RequestModel}
...
Open `staxfattmerchantapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from staxfattmerchantapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from staxfattmerchantapi.http.http_call_back import HttpCallBack
...
client = StaxfattmerchantapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `staxfattmerchantapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `StaxfattmerchantapiClient` in `staxfattmerchantapi/staxfattmerchantapi_client.py`.
- Operation method signatures are in `staxfattmerchantapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `staxfattmerchantapi/models/`; enum types have `from_value()`.
