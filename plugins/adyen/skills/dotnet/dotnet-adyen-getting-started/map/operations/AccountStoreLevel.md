...
- **HTTP**: `GET /merchants/{merchantId}/stores` (Default (balanceplatform-api-test))
...
- **HTTP**: `GET /merchants/{merchantId}/stores/{storeId}` (Default (balanceplatform-api-test))
...
### GetStores
- **HTTP**: `GET /stores` (Default (balanceplatform-api-test))
...
### GetStoresStoreId
- **HTTP**: `GET /stores/{storeId}` (Default (balanceplatform-api-test))
- **Notes**: Returns the details of the store identified in the path. To make this request, your API credential must have one of the following roles : * Management API—Stores read * Management API—Stores read and write In the live environment, requests to this endpoint are subject to rate limits .
...
- **HTTP**: `PATCH /merchants/{merchantId}/stores/{storeId}` (Default (balanceplatform-api-test))
...
### PatchStoresStoreId
- **HTTP**: `PATCH /stores/{storeId}` (Default (balanceplatform-api-test))
- **Notes**: Updates the store identified in the path. You can only update some store parameters. To make this request, your API credential must have the following role : * Management API—Stores read and write In the live environment, requests to this endpoint are subject to rate limits .
...
- **HTTP**: `POST /merchants/{merchantId}/stores` (Default (balanceplatform-api-test))
...
### PostStores
- **HTTP**: `POST /stores` (Default (balanceplatform-api-test))
