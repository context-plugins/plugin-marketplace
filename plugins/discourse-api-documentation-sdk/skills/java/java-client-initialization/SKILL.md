...
- `DiscourseAPIDocumentationClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `local.discourse` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
```java
import local.discourse.DiscourseAPIDocumentationClient;
import local.discourse.Environment;
