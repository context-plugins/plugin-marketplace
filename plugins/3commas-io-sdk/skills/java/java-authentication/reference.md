...
against the `3commasIoClient.Builder` setters, the `io.m3commas.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `3commasIoClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
3commasIoClient client = new 3commasIoClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScope3commasIoEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
