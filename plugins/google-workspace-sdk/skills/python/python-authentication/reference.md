...
kwargs to the `GoogleWorkspaceClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `googleworkspace/googleworkspace_client.py` and `googleworkspace/http/auth/` in
the cloned source.

## Basic auth

```python
from googleworkspace.http.auth.basic_auth import BasicAuthCredentials
...
```python
from googleworkspace.http.auth.api_key import ApiKeyCredentials
...
```python
from googleworkspace.http.auth.api_header import ApiHeaderCredentials
...
```python
from googleworkspace.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from googleworkspace.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from googleworkspace.http.auth.o_auth_acg import OAuthACGCredentials
from googleworkspace.models.o_auth_scope_googleworkspace_enum import OAuthScope{Pkg}Enum
...
client = GoogleWorkspaceClient(o_auth_acg_credentials=creds)
...
```python
from googleworkspace.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = GoogleWorkspaceClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GoogleWorkspaceClient.from_environment()`) read each credential
...
Grep `from_environment` in `googleworkspace/configuration.py` and each `googleworkspace/http/auth/*.py` for the exact
