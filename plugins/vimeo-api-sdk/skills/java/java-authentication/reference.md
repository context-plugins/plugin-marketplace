...
against the `VimeoAPIClient.Builder` setters, the `com.vimeo.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `VimeoAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
VimeoAPIClient client = new VimeoAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeVimeoAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
