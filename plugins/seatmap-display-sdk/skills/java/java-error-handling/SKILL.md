...
│   └── com.amadeus.api.test.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.amadeus.api.test.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.amadeus.api.test.http.client.HttpContext`, which carries:
