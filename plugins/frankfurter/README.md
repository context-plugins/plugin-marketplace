# Frankfurter SDK Plugin

Integrate and consume the **APIMatic-generated Frankfurter SDK** (the "APIMATIC v3.0" generic
...
(`dotnet-frankfurter-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-frankfurter`** router skill routes all
C#/.NET SDK work to the single **`dotnet-frankfurter-sdk`** agent (.NET-only), which answers
