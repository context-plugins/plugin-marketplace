...
Verified against `package.json` and `sdk-map.md` of the generated package at version `2024.3.3`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Spotify Web API |
| Package name (what you install, and what you import) | `spotify-web-api` — **not on npm**; built from source (see *Install*) |
| Import specifier | `spotify-web-api` — the package root is the **only** entry; deep imports do not resolve |
| Version | `2024.3.3` (API spec version `2024.3.3`) |
| Client class | `SpotifyWebApiClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new SpotifyWebApiClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new SpotifyWebApiClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `oauth20` · `oauth20Strategy` · `oauth20ClientCredentials` · `oauth20ClientCredentialsStrategy`. `timeout` defaults to `60_000` ms |
| Auth | **OAuth 2 authorization code** — set `ClientOptions.oauth20`<br>**OAuth 2 client credentials** — set `ClientOptions.oauth20ClientCredentials` |
| Environments | 2 environments (`ServerEnvironment.Production` *(default)*, `ServerEnvironment.Environment2`) × 1 server group |
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://api.spotify.com/v1` |
...
| Surface | 138 operations · 15 resources · 129 models · 18 open enums · 4 unions · 138 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/spotify-web-api-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 151 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { SpotifyWebApiClient, ServerEnvironment, ResponseError, SpotifyWebApiError } from "spotify-web-api";
import type { ClientOptions, AlbumBase } from "spotify-web-api";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `spotify-web-api/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **From CommonJS**, the typed spelling is `import sdk = require("spotify-web-api")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://api.spotify.com/v1` | `serverOptions.default.production.baseUrl` |
| `default` | `environment2` | `https://accounts.spotify.com` | `serverOptions.default.environment2.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 138 of the 138 operations require a credential and 0 are public.

| `ClientOptions` field | Scheme kind | What the SDK sends |
| --- | --- | --- |
| `oauth20` | OAuth 2 authorization code | `Authorization: Bearer <access token>` |
| `oauth20ClientCredentials` | OAuth 2 client credentials | `Authorization: Bearer <access token>` |

```ts
const client = new SpotifyWebApiClient({
  oauth20: { clientId: process.env.CLIENT_ID!, redirectUri: process.env.REDIRECT_URI!, promptForAuthorizationCode: async (url) => { /* send the user to url */ return "YOUR_AUTHORIZATION_CODE"; } },
  oauth20ClientCredentials: { clientId: process.env.CLIENT_ID!, clientSecret: process.env.CLIENT_SECRET! },
});
```
...
| --- | --- | --- |
| `oauth20` | `default` + `/api/token` | as `Authorization: Basic` |
| `oauth20ClientCredentials` | `default` + `/api/token` | as `Authorization: Basic` |

`oauth20Strategy`, `oauth20ClientCredentialsStrategy` on `ClientOptions` substitute the whole token request (`getToken(credentials, signal)`, plus `tryRefreshToken(…)` where the grant is refreshable); the caching, expiry buffer and single-flight behaviour still apply.
...
| --- | --- | --- | --- |
| `client.albums` | `Albums` | 9 | `checkUsersSavedAlbums` · `getAnAlbum` · `getAnAlbumsTracks` · `getAnArtistsAlbums` · `getMultipleAlbums` · `getNewReleases` · `getUsersSavedAlbums` · `removeAlbumsUser` · `saveAlbumsUser` |
| `client.tracks` | `Tracks` | 17 | `addTracksToPlaylist` · `checkUsersSavedTracks` · `getAnAlbumsTracks` · `getAnArtistsTopTracks` · `getAudioAnalysis` · `getAudioFeatures` · `getPlaylistsTracks` · `getRecommendations` · `getSeveralAudioFeatures` · `getSeveralTracks` · `getTrack` · `getUsersSavedTracks` · `getUsersTopTracks` · `removeTracksPlaylist` · `removeTracksUser` · `reorderOrReplacePlaylistsTracks` · `saveTracksUser` |
| `client.library` | `Library` | 29 | `changePlaylistDetails` · `checkCurrentUserFollows` · `checkUsersSavedAlbums` · `checkUsersSavedAudiobooks` · `checkUsersSavedEpisodes` · `checkUsersSavedShows` · `checkUsersSavedTracks` · `createPlaylist` · `followArtistsUsers` · `getAListOfCurrentUsersPlaylists` · `getFollowed` · `getUsersSavedAlbums` · `getUsersSavedAudiobooks` · `getUsersSavedEpisodes` · `getUsersSavedShows` · `getUsersSavedTracks` · `getUsersTopArtists` · `getUsersTopTracks` · `removeAlbumsUser` · `removeAudiobooksUser` · `removeEpisodesUser` · `removeShowsUser` · `removeTracksUser` · `saveAlbumsUser` · `saveAudiobooksUser` · `saveEpisodesUser` · `saveShowsUser` · `saveTracksUser` · `unfollowArtistsUsers` |
| `client.artists` | `Artists` | 10 | `checkCurrentUserFollows` · `followArtistsUsers` · `getAnArtist` · `getAnArtistsAlbums` · `getAnArtistsRelatedArtists` · `getAnArtistsTopTracks` · `getFollowed` · `getMultipleArtists` · `getUsersTopArtists` · `unfollowArtistsUsers` |
| `client.audiobooks` | `Audiobooks` | 7 | `checkUsersSavedAudiobooks` · `getAnAudiobook` · `getAudiobookChapters` · `getMultipleAudiobooks` · `getUsersSavedAudiobooks` · `removeAudiobooksUser` · `saveAudiobooksUser` |
| `client.chapters` | `Chapters` | 3 | `getAChapter` · `getAudiobookChapters` · `getSeveralChapters` |
| `client.categoriesApi` | `CategoriesApi` | 3 | `getACategoriesPlaylists` · `getACategory` · `getCategories` |
| `client.episodes` | `Episodes` | 7 | `checkUsersSavedEpisodes` · `getAShowsEpisodes` · `getAnEpisode` · `getMultipleEpisodes` · `getUsersSavedEpisodes` · `removeEpisodesUser` · `saveEpisodesUser` |
| `client.genres` | `Genres` | 1 | `getRecommendationGenres` |
| `client.marketsApi` | `MarketsApi` | 1 | `getAvailableMarkets` |
| `client.player` | `Player` | 15 | `addToQueue` · `getAUsersAvailableDevices` · `getInformationAboutTheUsersCurrentPlayback` · `getQueue` · `getRecentlyPlayed` · `getTheUsersCurrentlyPlayingTrack` · `pauseAUsersPlayback` · `seekToPositionInCurrentlyPlayingTrack` · `setRepeatModeOnUsersPlayback` · `setVolumeForUsersPlayback` · `skipUsersPlaybackToNextTrack` · `skipUsersPlaybackToPreviousTrack` · `startAUsersPlayback` · `toggleShuffleForUsersPlayback` · `transferAUsersPlayback` |
| `client.playlists` | `Playlists` | 16 | `addTracksToPlaylist` · `changePlaylistDetails` · `checkIfUserFollowsPlaylist` · `createPlaylist` · `followPlaylist` · `getACategoriesPlaylists` · `getAListOfCurrentUsersPlaylists` · `getFeaturedPlaylists` · `getListUsersPlaylists` · `getPlaylist` · `getPlaylistCover` · `getPlaylistsTracks` · `removeTracksPlaylist` · `reorderOrReplacePlaylistsTracks` · `unfollowPlaylist` · `uploadCustomPlaylistCover` |
| `client.users` | `Users` | 12 | `checkCurrentUserFollows` · `checkIfUserFollowsPlaylist` · `followArtistsUsers` · `followPlaylist` · `getCurrentUsersProfile` · `getFollowed` · `getListUsersPlaylists` · `getUsersProfile` · `getUsersTopArtists` · `getUsersTopTracks` · `unfollowArtistsUsers` · `unfollowPlaylist` |
| `client.search` | `Search` | 1 | `search` |
| `client.shows` | `Shows` | 7 | `checkUsersSavedShows` · `getAShow` · `getAShowsEpisodes` · `getMultipleShows` · `getUsersSavedShows` · `removeShowsUser` · `saveShowsUser` |
...
```bash
node -e "console.log(require.resolve('spotify-web-api/package.json'))"
```

Failing that it is at `node_modules/spotify-web-api/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/spotify-web-api/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/spotify-web-api/`:
...
Beyond the usual signatures and model members, a contract sheet for the Spotify Web API TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
...
2. **43 operations resolve to `undefined`** — `await` gives you nothing to inspect, so **`.asApiResult()` is the only way to observe their status and headers** — decide the mode at write time, not by retrofit.
