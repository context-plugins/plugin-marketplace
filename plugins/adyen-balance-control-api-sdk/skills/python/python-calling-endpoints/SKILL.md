...
Open `adyenbalancecontrolapi/adyenbalancecontrolapi_client.py` for the controller property names, then the relevant
`adyenbalancecontrolapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from adyenbalancecontrolapi.models.{request_model} import {RequestModel}
...
Open `adyenbalancecontrolapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from adyenbalancecontrolapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from adyenbalancecontrolapi.http.http_call_back import HttpCallBack
...
client = AdyenBalanceControlAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `adyenbalancecontrolapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `AdyenBalanceControlAPIClient` in `adyenbalancecontrolapi/adyenbalancecontrolapi_client.py`.
- Operation method signatures are in `adyenbalancecontrolapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `adyenbalancecontrolapi/models/`; enum types have `from_value()`.
