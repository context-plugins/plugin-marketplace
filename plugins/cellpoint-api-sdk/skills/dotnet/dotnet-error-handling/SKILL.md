...
> `{ApiGroup}`, `CellPointApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `CellPointApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `CellPointApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `CellPointApi.Errors`
...
```csharp
using CellPointApi.Core.Exceptions;     // SdkException<TError>
using CellPointApi.Core.ErrorResponse;  // ApiError, RawError
using CellPointApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`CellPointApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using CellPointApi.Core.Exceptions;     // SdkException<TError>
using CellPointApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `CellPointApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `CellPointApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using CellPointApi.Core.Models;        // ApiResult<TResponse, TError>
using CellPointApi.Core.ErrorResponse; // RawError
using CellPointApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `CellPointApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
