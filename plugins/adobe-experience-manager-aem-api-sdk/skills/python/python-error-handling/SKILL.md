...
`adobeexperiencemanageraemapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `adobeexperiencemanageraemapi/exceptions/` and operation docstrings.
...
```python
from adobeexperiencemanageraemapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `adobeexperiencemanageraemapi/exceptions/`.
...
```python
from adobeexperiencemanageraemapi.exceptions.api_exception import APIException
from adobeexperiencemanageraemapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`adobeexperiencemanageraemapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`adobeexperiencemanageraemapi/exceptions/` for the class.
...
`adobeexperiencemanageraemapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `adobeexperiencemanageraemapi/exceptions/`
  or a `ValueError` — check the auth handler in `adobeexperiencemanageraemapi/http/auth/` for the exact type.
