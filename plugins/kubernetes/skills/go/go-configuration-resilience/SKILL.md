    ...
    kubernetes.CreateConfiguration(
        kubernetes.WithHttpConfiguration(
            kubernetes.CreateHttpConfiguration(
                kubernetes.WithTimeout(30),
                kubernetes.WithRetryConfiguration(
                    kubernetes.CreateRetryConfiguration(
                        kubernetes.WithMaxRetryAttempts(3),
                        kubernetes.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := kubernetes.CreateRetryConfiguration(
    kubernetes.WithMaxRetryAttempts(5),
    kubernetes.WithRetryInterval(1),
    kubernetes.WithBackoffFactor(2),
    kubernetes.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    kubernetes.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := kubernetes.NewClient(
    kubernetes.CreateConfiguration(
        kubernetes.WithHttpConfiguration(
            kubernetes.CreateHttpConfiguration(
                kubernetes.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
