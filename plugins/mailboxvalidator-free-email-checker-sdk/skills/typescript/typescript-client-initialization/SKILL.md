...
- `mailboxvalidator-free-email-checkerlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'mailboxvalidator-free-email-checkerlib';
...
```ts
import { {Resource}Controller } from 'mailboxvalidator-free-email-checkerlib';
