...
Confirm every name against `lib/ebay_finances_api/client.rb`, `lib/ebay_finances_api/configuration.rb`, the
`lib/ebay_finances_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
