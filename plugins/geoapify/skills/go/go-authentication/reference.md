...
```go
geoapify.WithOAuthCCGCredentials(
    geoapify.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr geoapify.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := geoapify.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr geoapify.OAuthCCGManager) models.OAuthToken {
...
```go
creds := geoapify.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := geoapify.NewClient(geoapify.CreateConfiguration(geoapify.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    geoapify.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := geoapify.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := geoapify.NewClient(geoapify.CreateConfiguration(geoapify.WithOAuthROPCGCredentials(creds)))
...
```go
geoapify.WithOAuthBearerTokenCredentials(geoapify.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
geoapify.WithBasicAuthCredentials(geoapify.NewBasicAuthCredentials(username, password))
geoapify.WithApiHeaderCredentials(geoapify.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
geoapify.WithApiKeyCredentials(geoapify.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `GEOAPIFY_...` variables — e.g.
`GEOAPIFY_USERNAME`, `GEOAPIFY_PASSWORD`, `GEOAPIFY_TOKEN`, `GEOAPIFY_API_KEY`,
`GEOAPIFY_O_AUTH_CLIENT_ID`, `GEOAPIFY_O_AUTH_CLIENT_SECRET`, `GEOAPIFY_ACCESS_TOKEN`. Grep the
