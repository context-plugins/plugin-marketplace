...
Confirm every name against `lib/spotify_web_api/client.rb`, `lib/spotify_web_api/configuration.rb`, the
`lib/spotify_web_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
