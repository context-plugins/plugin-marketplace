...
| Project | `DeploymentAdminClient.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `DeploymentAdminClient` (the `using` namespace) |
| Client class | `DeploymentAdminClientClient` |
| Options class | `DeploymentAdminClientClientOptions` |
...
namespaces transitively, so `using DeploymentAdminClient.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/deploymentadminclient-csharp
dotnet add reference deploymentadminclient-csharp/DeploymentAdminClient.csproj
```

```csharp
using DeploymentAdminClient;          // client + options + ServerEnvironment
using DeploymentAdminClient.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/deploymentadminclient-csharp /tmp/deploymentadminclient-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/deploymentadminclient-csharp "$env:TEMP\deploymentadminclient-dotnet-src"
```
...
- `DeploymentAdminClientClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/deploymentadminclient-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\deploymentadminclient-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `DeploymentAdminClientClient`, build its
   `DeploymentAdminClientClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
