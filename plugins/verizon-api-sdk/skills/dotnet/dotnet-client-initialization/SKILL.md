...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddVerizonApiClient extension. Use the moment you write `new VerizonApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `VerizonApiClientOptions` — its options class.
- `VerizonApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public VerizonApiClient(HttpClient httpClient, VerizonApiClientOptions options)
```
...
```csharp
public class VerizonApiClientOptions
{
...
```csharp
using VerizonApi;
using VerizonApi.Servers;

var options = new VerizonApiClientOptions
{
...
var client = new VerizonApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddVerizonApiClient`, which registers the
...
builder.Services.AddVerizonApiClient(options =>
{
...
```csharp
public sealed class MyService(VerizonApiClient client)
{
