...
`accessanalyzer/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `accessanalyzer/exceptions/` and operation docstrings.
...
```python
from accessanalyzer.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `accessanalyzer/exceptions/`.
...
```python
from accessanalyzer.exceptions.api_exception import APIException
from accessanalyzer.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`accessanalyzer/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`accessanalyzer/exceptions/` for the class.
...
`accessanalyzer/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `accessanalyzer/exceptions/`
  or a `ValueError` — check the auth handler in `accessanalyzer/http/auth/` for the exact type.
