...
| Project | `PasswordConnect1.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `PasswordConnect1` (the `using` namespace) |
| Client class | `PasswordConnect1Client` |
| Options class | `PasswordConnect1ClientOptions` |
...
namespaces transitively, so `using PasswordConnect1.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/1password-connect-csharp
dotnet add reference 1password-connect-csharp/PasswordConnect1.csproj
```

```csharp
using PasswordConnect1;          // client + options + ServerEnvironment
using PasswordConnect1.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/1password-connect-csharp /tmp/1password-connect-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/1password-connect-csharp "$env:TEMP\1password-connect-dotnet-src"
```
...
- `PasswordConnect1Client.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/1password-connect-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\1password-connect-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `PasswordConnect1Client`, build its
   `PasswordConnect1ClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
