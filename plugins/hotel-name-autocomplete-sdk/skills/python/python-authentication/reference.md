...
kwargs to the `HotelNameAutocompleteClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `hotelnameautocomplete/hotelnameautocomplete_client.py` and `hotelnameautocomplete/http/auth/` in
the cloned source.

## Basic auth

```python
from hotelnameautocomplete.http.auth.basic_auth import BasicAuthCredentials
...
```python
from hotelnameautocomplete.http.auth.api_key import ApiKeyCredentials
...
```python
from hotelnameautocomplete.http.auth.api_header import ApiHeaderCredentials
...
```python
from hotelnameautocomplete.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from hotelnameautocomplete.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from hotelnameautocomplete.http.auth.o_auth_acg import OAuthACGCredentials
from hotelnameautocomplete.models.o_auth_scope_hotelnameautocomplete_enum import OAuthScope{Pkg}Enum
...
client = HotelNameAutocompleteClient(o_auth_acg_credentials=creds)
...
```python
from hotelnameautocomplete.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = HotelNameAutocompleteClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `HotelNameAutocompleteClient.from_environment()`) read each credential
...
Grep `from_environment` in `hotelnameautocomplete/configuration.py` and each `hotelnameautocomplete/http/auth/*.py` for the exact
