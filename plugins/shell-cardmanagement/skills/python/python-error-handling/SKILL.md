...
`shellcardmanagement/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `shellcardmanagement/exceptions/` and operation docstrings.
...
```python
from shellcardmanagement.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `shellcardmanagement/exceptions/`.
...
```python
from shellcardmanagement.exceptions.api_exception import APIException
from shellcardmanagement.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`shellcardmanagement/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`shellcardmanagement/exceptions/` for the class.
...
`shellcardmanagement/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `shellcardmanagement/exceptions/`
  or a `ValueError` — check the auth handler in `shellcardmanagement/http/auth/` for the exact type.
