...
Open `3commasio/3commasio_client.py` for the controller property names, then the relevant
`3commasio/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from 3commasio.models.{request_model} import {RequestModel}
...
Open `3commasio/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from 3commasio.models.suite_code_enum import SuiteCodeEnum
...
```python
from 3commasio.http.http_call_back import HttpCallBack
...
client = 3commasIoClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `3commasio/pagination/` in the source.

## Finding the right method

- Controller property names are on `3commasIoClient` in `3commasio/3commasio_client.py`.
- Operation method signatures are in `3commasio/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `3commasio/models/`; enum types have `from_value()`.
