...
> Throughout, `coingeckodemoapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := coingeckodemoapi.NewClient(
    coingeckodemoapi.CreateConfiguration(
        coingeckodemoapi.WithBasicAuthCredentials(
            coingeckodemoapi.NewBasicAuthCredentials(
...
coingeckodemoapi.WithApiKeyCredentials(coingeckodemoapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
coingeckodemoapi.WithApiHeaderCredentials(coingeckodemoapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
coingeckodemoapi.WithOAuthBearerTokenCredentials(
    coingeckodemoapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := coingeckodemoapi.NewClient(
    coingeckodemoapi.CreateConfiguration(
        coingeckodemoapi.WithOAuthCCGCredentials(
            coingeckodemoapi.NewOAuthCCGCredentials(
