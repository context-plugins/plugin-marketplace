...
> `{ApiGroup}`, `AdyenBalanceControlApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `AdyenBalanceControlApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `AdyenBalanceControlApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `AdyenBalanceControlApi.Errors`
...
```csharp
using AdyenBalanceControlApi.Core.Exceptions;     // SdkException<TError>
using AdyenBalanceControlApi.Core.ErrorResponse;  // ApiError, RawError
using AdyenBalanceControlApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`AdyenBalanceControlApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using AdyenBalanceControlApi.Core.Exceptions;     // SdkException<TError>
using AdyenBalanceControlApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `AdyenBalanceControlApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `AdyenBalanceControlApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using AdyenBalanceControlApi.Core.Models;        // ApiResult<TResponse, TError>
using AdyenBalanceControlApi.Core.ErrorResponse; // RawError
using AdyenBalanceControlApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `AdyenBalanceControlApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
