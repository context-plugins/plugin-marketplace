...
All types below live under `Vimeo.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using Vimeo.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new VimeoClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `Vimeo.Core.Exceptions`):
...
With DI, the SDK's `AddVimeoClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddVimeoClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
