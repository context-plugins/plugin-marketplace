...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `boku-direct-payments-apilib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'boku-direct-payments-apilib';
import { ApiError } from 'boku-direct-payments-apilib';
import { {Operation}Error } from 'boku-direct-payments-apilib/errors';
