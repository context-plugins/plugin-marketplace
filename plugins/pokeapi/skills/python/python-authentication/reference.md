...
kwargs to the `PokapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `pokeapi/pokeapi_client.py` and `pokeapi/http/auth/` in
the cloned source.

## Basic auth

```python
from pokeapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from pokeapi.http.auth.api_key import ApiKeyCredentials
...
```python
from pokeapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from pokeapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from pokeapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from pokeapi.http.auth.o_auth_acg import OAuthACGCredentials
from pokeapi.models.o_auth_scope_pokeapi_enum import OAuthScope{Pkg}Enum
...
client = PokapiClient(o_auth_acg_credentials=creds)
...
```python
from pokeapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = PokapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `PokapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `pokeapi/configuration.py` and each `pokeapi/http/auth/*.py` for the exact
