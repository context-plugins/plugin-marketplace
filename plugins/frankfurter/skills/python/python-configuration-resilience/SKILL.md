...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `FrankfurterapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`frankfurter/configuration.py` in the cloned source.
...
```python
import requests
from frankfurter.configuration import Configuration
...
`frankfurter/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`frankfurter/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `frankfurter/configuration.py`
...
`NumberPagedResponse`) live in `frankfurter/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `frankfurter/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
