...
> `XApiV2Client`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): XApiV2Client
{
...
```php
use XApiV2Lib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use XApiV2Lib\Exceptions\{OperationException};
use XApiV2Lib\Exceptions\ApiException;
...
app()->instance(XApiV2Client::class, clientWithResponses([
