...
against the `KlarnaClient.Builder` setters, the `com.klarna.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `KlarnaClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
KlarnaClient client = new KlarnaClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeKlarnaEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
