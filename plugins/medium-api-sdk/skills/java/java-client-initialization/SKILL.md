...
- `MediumAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.rapidapi.p.medium2` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`MediumAPIClient.Builder` class. A minimal initialization:

```java
import com.rapidapi.p.medium2.MediumAPIClient;
import com.rapidapi.p.medium2.Environment;

MediumAPIClient client = new MediumAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `MediumAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `MediumAPIClient.java` |

Call `.build()` to produce an immutable `MediumAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public MediumAPIClient apiClient() {
        return new MediumAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired MediumAPIClient client) {
        client.shutdown();
    }
}
```

Inject `MediumAPIClient` into your services as a normal Spring dependency. Build only one bean.
