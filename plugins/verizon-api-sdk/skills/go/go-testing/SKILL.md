...
`testing` + `testify/require` for reference; substitute the real `verizonapi` and names.
...
import (
    ...
    "github.com/context-plugins/verizon-api-go"   // root package verizonapi
)
...
func clientReturning(status int, body string) (verizonapi.ClientInterface, *stubTransport) {
    ...
    client := verizonapi.NewClient(
        verizonapi.CreateConfiguration(
            verizonapi.WithHttpConfiguration(
                verizonapi.CreateHttpConfiguration(verizonapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/verizon-api-go/errors"
...
client := verizonapi.NewClient(
    verizonapi.CreateConfiguration(
        verizonapi.WithHttpConfiguration(
            verizonapi.CreateHttpConfiguration(verizonapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
