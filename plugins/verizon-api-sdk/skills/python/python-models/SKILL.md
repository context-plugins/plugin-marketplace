...
your SDK source (`verizonapi/models/`).
...
```python
from verizonapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `verizonapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from verizonapi.models.animal import Cat, Dog
...
```python
from verizonapi.models.animal import Animal, Cat, Dog
