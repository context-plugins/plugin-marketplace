...
```ruby
client = CoinGeckoDemoAPI::Client.new(
  max_retries:    3,
...
```ruby
client = CoinGeckoDemoAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = CoinGeckoDemoAPI::Client.new(adapter: :net_http)
```
...
client = CoinGeckoDemoAPI::Client.new(connection: conn)
```
...
client = CoinGeckoDemoAPI::Client.new(environment: Environment::PRODUCTION)
client = CoinGeckoDemoAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = CoinGeckoDemoAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = CoinGeckoDemoAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = CoinGeckoDemoAPI::ProxySettings.from_env
client = CoinGeckoDemoAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < CoinGeckoDemoAPI::HttpCallBack
...
client = CoinGeckoDemoAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = CoinGeckoDemoAPI::Client.from_env
# Overrides take precedence over env vars:
client = CoinGeckoDemoAPI::Client.from_env(timeout: 30, max_retries: 3)
```
