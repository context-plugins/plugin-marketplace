...
`testing` + `testify/require` for reference; substitute the real `accessanalyzer` and names.
...
import (
    ...
    "github.com/context-plugins/access-analyzer-go"   // root package accessanalyzer
)
...
func clientReturning(status int, body string) (accessanalyzer.ClientInterface, *stubTransport) {
    ...
    client := accessanalyzer.NewClient(
        accessanalyzer.CreateConfiguration(
            accessanalyzer.WithHttpConfiguration(
                accessanalyzer.CreateHttpConfiguration(accessanalyzer.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/access-analyzer-go/errors"
...
client := accessanalyzer.NewClient(
    accessanalyzer.CreateConfiguration(
        accessanalyzer.WithHttpConfiguration(
            accessanalyzer.CreateHttpConfiguration(accessanalyzer.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
