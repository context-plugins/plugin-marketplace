...
│   └── com.plaid.production.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.plaid.production.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.plaid.production.http.client.HttpContext`, which carries:
