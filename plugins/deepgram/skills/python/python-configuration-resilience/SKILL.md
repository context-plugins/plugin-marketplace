...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `DeepgramClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`deepgram/configuration.py` in the cloned source.
...
```python
import requests
from deepgram.configuration import Configuration
...
`deepgram/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`deepgram/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `deepgram/configuration.py`
...
`NumberPagedResponse`) live in `deepgram/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `deepgram/http/http_call_back.py`) and pass an instance
as `http_call_back=`:

```python
import logging
from deepgram.http.http_call_back import HttpCallBack
from deepgram.configuration import Configuration
