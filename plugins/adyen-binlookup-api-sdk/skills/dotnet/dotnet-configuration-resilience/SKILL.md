...
All types below live under `AdyenBinLookupApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using AdyenBinLookupApi.Core.Configuration;
...
```csharp
using AdyenBinLookupApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new AdyenBinLookupApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `AdyenBinLookupApi.Core.Exceptions`):
...
With DI, the SDK's `AddAdyenBinLookupApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAdyenBinLookupApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
