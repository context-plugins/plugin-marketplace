...
```ruby
client = TeslaAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = TeslaAPI::Client.new(adapter: :net_http)
```
...
client = TeslaAPI::Client.new(connection: conn)
```
...
client = TeslaAPI::Client.new(environment: Environment::PRODUCTION)
client = TeslaAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = TeslaAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = TeslaAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = TeslaAPI::ProxySettings.from_env
client = TeslaAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < TeslaAPI::HttpCallBack
...
client = TeslaAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = TeslaAPI::Client.from_env
# Overrides take precedence over env vars:
client = TeslaAPI::Client.from_env(timeout: 30, max_retries: 3)
```
