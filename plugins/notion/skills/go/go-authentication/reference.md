...
```go
notion.WithOAuthCCGCredentials(
    notion.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr notion.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := notion.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr notion.OAuthCCGManager) models.OAuthToken {
...
```go
creds := notion.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := notion.NewClient(notion.CreateConfiguration(notion.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    notion.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := notion.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := notion.NewClient(notion.CreateConfiguration(notion.WithOAuthROPCGCredentials(creds)))
...
```go
notion.WithOAuthBearerTokenCredentials(notion.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
notion.WithBasicAuthCredentials(notion.NewBasicAuthCredentials(username, password))
notion.WithApiHeaderCredentials(notion.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
notion.WithApiKeyCredentials(notion.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `NOTION_...` variables — e.g.
`NOTION_USERNAME`, `NOTION_PASSWORD`, `NOTION_TOKEN`, `NOTION_API_KEY`,
`NOTION_O_AUTH_CLIENT_ID`, `NOTION_O_AUTH_CLIENT_SECRET`, `NOTION_ACCESS_TOKEN`. Grep the
