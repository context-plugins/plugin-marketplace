...
| Install | `gem 'seatmap_display', git: 'https://github.com/context-plugins/seatmap-display-ruby', branch: 'main'` in a Gemfile |
| Client class | `SeatmapDisplay::Client` — one class; built with `Client.new(keyword_args...)` or `Client.from_env` |
| Configuration class | `SeatmapDisplay::Configuration` — extends `CoreLibrary::HttpClientConfiguration`; holds `Environment` and `Server` inner classes |
...
| Base exception | `SeatmapDisplay::APIException` (extends `CoreLibrary::ApiException`) |
...
APIMatic Ruby SDKs place everything under `lib/seatmap_display/`:

- `lib/seatmap_display.rb` — top-level require that loads `apimatic_core`, `apimatic_faraday_client_adapter`, then all internal files.
- `lib/seatmap_display/client.rb` — the `Client` class: `initialize` with all keyword args (http options + credentials), controller accessor methods (e.g. `def simple_calculator`), OAuth manager accessors (e.g. `def o_auth_ccg`), and `self.from_env`.
- `lib/seatmap_display/configuration.rb` — `Configuration` class (extends `CoreLibrary::HttpClientConfiguration`) with `Environment` and `Server` inner constant classes, the `ENVIRONMENTS` hash mapping environment+server to base URIs, `get_base_uri`, and `clone_with`.
- `lib/seatmap_display/controllers/` — one `{Resource}Controller` per API resource group, each extending `BaseController`. **This is where operation method signatures and required vs. optional params live.**
- `lib/seatmap_display/models/` — model classes (extend `BaseModel`) and enum modules with frozen string constants.
- `lib/seatmap_display/exceptions/` — `api_exception.rb` (`APIException`) plus any typed error subclasses.
- `lib/seatmap_display/http/auth/` — one file per auth scheme (`basic_auth.rb`, `o_auth_ccg.rb`, etc.), each containing both the handler class (extends `CoreLibrary::HeaderAuth` or `CoreLibrary::QueryAuth`) and the `{Scheme}Credentials` data class with `initialize`, `from_env`, and `clone_with`.
- `lib/seatmap_display/utilities/` — `file_wrapper.rb`, `date_time_helper.rb`.
...
```ruby
# In your Gemfile:
gem 'seatmap_display', git: 'https://github.com/context-plugins/seatmap-display-ruby', branch: 'main'
```
...
include SeatmapDisplay   # optional — lets you write Client.new instead of SeatmapDisplay::Client.new
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/seatmap-display-ruby /tmp/seatmap-display-ruby-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/seatmap-display-ruby "$env:TEMP\seatmap-display-ruby-src"
```
...
- `lib/seatmap_display/client.rb` — the full `initialize` keyword list (http kwargs + credentials kwargs), controller accessors, OAuth manager accessors, and `from_env`.
- `lib/seatmap_display/configuration.rb` — `Environment` and `Server` constants, the `ENVIRONMENTS` hash (maps environment+server strings to base URI templates), and `clone_with`.
- `lib/seatmap_display/http/auth/*.rb` — credentials classes for every scheme this API uses, including their `initialize` keyword args and `from_env` env-var names.
- `lib/seatmap_display/controllers/*.rb` — operation method signatures; **this is where required vs. optional params live**.
- `lib/seatmap_display/models/` — model classes and enum modules; **this is where field names and enum values live**.
- `lib/seatmap_display/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/seatmap-display-ruby-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\seatmap-display-ruby-src"   # Windows
```
