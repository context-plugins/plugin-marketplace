...
> `{operation}`, `{apiGroup}`, `ApiManagementClientLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `ApiManagementClientLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `ApiManagementClientLib\Exceptions\` — read the source to
