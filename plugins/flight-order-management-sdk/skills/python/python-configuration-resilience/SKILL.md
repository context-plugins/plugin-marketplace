...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `FlightOrderManagementClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`flightordermanagement/configuration.py` in the cloned source.
...
```python
import requests
from flightordermanagement.configuration import Configuration
...
`flightordermanagement/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`flightordermanagement/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `flightordermanagement/configuration.py`
...
`NumberPagedResponse`) live in `flightordermanagement/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `flightordermanagement/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
