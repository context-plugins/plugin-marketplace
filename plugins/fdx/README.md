...
(`dotnet-fdx-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-fdx`** router skill routes all
C#/.NET SDK work to the single **`dotnet-fdx-sdk`** agent (.NET-only), which answers
