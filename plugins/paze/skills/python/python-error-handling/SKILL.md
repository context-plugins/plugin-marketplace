...
`paze/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `paze/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `paze/exceptions/`.
...
file (`paze/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`paze/exceptions/` for the class.
...
`paze/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `paze/exceptions/`
  or a `ValueError` — check the auth handler in `paze/http/auth/` for the exact type.
