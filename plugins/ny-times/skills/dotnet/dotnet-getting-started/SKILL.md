...
| --- | --- |
| API | `NY TIMES` |
| Project | `NyTimes.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using NyTimes.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/ny-times-csharp-sdk
dotnet add reference ny-times-csharp-sdk/NyTimes.csproj
```

```csharp
using NyTimes;          // client + options + ServerEnvironment
using NyTimes.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/ny-times-csharp-sdk /tmp/ny-times-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/ny-times-csharp-sdk "$env:TEMP\ny-times-dotnet-src"
```
...
- `NyTimesClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/ny-times-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\ny-times-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `NyTimesClient`, build its
   `NyTimesClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
