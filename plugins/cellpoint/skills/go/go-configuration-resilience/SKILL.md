    ...
    cellpoint.CreateConfiguration(
        cellpoint.WithHttpConfiguration(
            cellpoint.CreateHttpConfiguration(
                cellpoint.WithTimeout(30),
                cellpoint.WithRetryConfiguration(
                    cellpoint.CreateRetryConfiguration(
                        cellpoint.WithMaxRetryAttempts(3),
                        cellpoint.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := cellpoint.CreateRetryConfiguration(
    cellpoint.WithMaxRetryAttempts(5),
    cellpoint.WithRetryInterval(1),
    cellpoint.WithBackoffFactor(2),
    cellpoint.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    cellpoint.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := cellpoint.NewClient(
    cellpoint.CreateConfiguration(
        cellpoint.WithHttpConfiguration(
            cellpoint.CreateHttpConfiguration(
                cellpoint.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
