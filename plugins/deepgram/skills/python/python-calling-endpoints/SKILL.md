...
Open `deepgram/deepgram_client.py` for the controller property names, then the relevant
`deepgram/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from deepgram.models.{request_model} import {RequestModel}
...
Open `deepgram/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from deepgram.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `deepgram/pagination/` in the source.
