...
To see which schemes a specific SDK accepts, read the credential setter methods on `FlightOrderManagementClient.Builder`
...
- Set credentials **on the `FlightOrderManagementClient.Builder`** before calling `.build()` — the client is immutable
