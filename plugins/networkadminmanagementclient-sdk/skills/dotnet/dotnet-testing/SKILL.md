...
`NetworkAdminManagementClientClient`/`NetworkAdminManagementClientClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `NetworkAdminManagementClientClient`,
...
static NetworkAdminManagementClientClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new NetworkAdminManagementClientClient(new HttpClient(handler), new NetworkAdminManagementClientClientOptions { /* auth not needed for stubs */ });
}
```
...
using NetworkAdminManagementClient.Errors;              // {Operation}Error types
...
```csharp
using NetworkAdminManagementClient.Core.Exceptions;
using NetworkAdminManagementClient.Core.ErrorResponse;
...
using NetworkAdminManagementClient.Core.ErrorResponse; // RawError (Case B)
...
var client = new NetworkAdminManagementClientClient(new HttpClient(handler), new NetworkAdminManagementClientClientOptions());
...
- For DI-based code, the SDK's `AddNetworkAdminManagementClientClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `NetworkAdminManagementClientClient` from the provider:
  ```csharp
  services.AddNetworkAdminManagementClientClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<NetworkAdminManagementClientClient>();
  ```
