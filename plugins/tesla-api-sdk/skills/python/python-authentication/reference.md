...
kwargs to the `TeslaAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `teslaapi/teslaapi_client.py` and `teslaapi/http/auth/` in
the cloned source.

## Basic auth

```python
from teslaapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from teslaapi.http.auth.api_key import ApiKeyCredentials
...
```python
from teslaapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from teslaapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from teslaapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from teslaapi.http.auth.o_auth_acg import OAuthACGCredentials
from teslaapi.models.o_auth_scope_teslaapi_enum import OAuthScope{Pkg}Enum
...
client = TeslaAPIClient(o_auth_acg_credentials=creds)
...
```python
from teslaapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = TeslaAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `TeslaAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `teslaapi/configuration.py` and each `teslaapi/http/auth/*.py` for the exact
