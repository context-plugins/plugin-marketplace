...
- `FlightOrderManagement` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `flight_order_management` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'flight_order_management'
include FlightOrderManagement
...
The authoritative list is the `initialize` signature in `lib/flight_order_management/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/flight_order_management/configuration.rb`
(`build_default_config_from_env`) and `lib/flight_order_management/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/flight_order_management.rb`) and store it in a
