...
```typescript
import { {Operation}Error } from 'slack-web-apilib/errors';
...
```typescript
import { ApiError } from 'slack-web-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(SlackWebAPIClient)
    .useValue(new SlackWebAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
