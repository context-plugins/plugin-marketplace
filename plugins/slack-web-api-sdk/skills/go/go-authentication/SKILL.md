...
> Throughout, `slackwebapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := slackwebapi.NewClient(
    slackwebapi.CreateConfiguration(
        slackwebapi.WithBasicAuthCredentials(
            slackwebapi.NewBasicAuthCredentials(
...
slackwebapi.WithApiKeyCredentials(slackwebapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
slackwebapi.WithApiHeaderCredentials(slackwebapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
slackwebapi.WithOAuthBearerTokenCredentials(
    slackwebapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := slackwebapi.NewClient(
    slackwebapi.CreateConfiguration(
        slackwebapi.WithOAuthCCGCredentials(
            slackwebapi.NewOAuthCCGCredentials(
