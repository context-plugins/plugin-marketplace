...
> `AuthentiqAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AuthentiqAPIClient
{
...
```php
use AuthentiqAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AuthentiqAPILib\Exceptions\{OperationException};
use AuthentiqAPILib\Exceptions\ApiException;
...
app()->instance(AuthentiqAPIClient::class, clientWithResponses([
