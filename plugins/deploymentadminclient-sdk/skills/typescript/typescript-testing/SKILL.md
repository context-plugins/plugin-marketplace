...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new DeploymentAdminClientClient({
...
```typescript
import { {Operation}Error } from 'deploymentadminclientlib/errors';
...
```typescript
import { ApiError } from 'deploymentadminclientlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(DeploymentAdminClientClient)
    .useValue(new DeploymentAdminClientClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
