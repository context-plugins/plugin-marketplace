...
kwargs to the `ShellAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `shellapi/shellapi_client.py` and `shellapi/http/auth/` in
the cloned source.

## Basic auth

```python
from shellapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from shellapi.http.auth.api_key import ApiKeyCredentials
...
```python
from shellapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from shellapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from shellapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from shellapi.http.auth.o_auth_acg import OAuthACGCredentials
from shellapi.models.o_auth_scope_shellapi_enum import OAuthScope{Pkg}Enum
...
client = ShellAPIClient(o_auth_acg_credentials=creds)
...
```python
from shellapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ShellAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ShellAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `shellapi/configuration.py` and each `shellapi/http/auth/*.py` for the exact
