...
kwargs to the `AutomationManagementClientClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `automationmanagementclient/automationmanagementclient_client.py` and `automationmanagementclient/http/auth/` in
the cloned source.

## Basic auth

```python
from automationmanagementclient.http.auth.basic_auth import BasicAuthCredentials
...
```python
from automationmanagementclient.http.auth.api_key import ApiKeyCredentials
...
```python
from automationmanagementclient.http.auth.api_header import ApiHeaderCredentials
...
```python
from automationmanagementclient.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from automationmanagementclient.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from automationmanagementclient.http.auth.o_auth_acg import OAuthACGCredentials
from automationmanagementclient.models.o_auth_scope_automationmanagementclient_enum import OAuthScope{Pkg}Enum
...
client = AutomationManagementClientClient(o_auth_acg_credentials=creds)
...
```python
from automationmanagementclient.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AutomationManagementClientClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AutomationManagementClientClient.from_environment()`) read each credential
...
Grep `from_environment` in `automationmanagementclient/configuration.py` and each `automationmanagementclient/http/auth/*.py` for the exact
