...
> `{ApiGroup}`, `TeslaApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `TeslaApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `TeslaApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `TeslaApi.Errors`
...
```csharp
using TeslaApi.Core.Exceptions;     // SdkException<TError>
using TeslaApi.Core.ErrorResponse;  // ApiError, RawError
using TeslaApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`TeslaApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using TeslaApi.Core.Exceptions;     // SdkException<TError>
using TeslaApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `TeslaApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `TeslaApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using TeslaApi.Core.Models;        // ApiResult<TResponse, TError>
using TeslaApi.Core.ErrorResponse; // RawError
using TeslaApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `TeslaApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
