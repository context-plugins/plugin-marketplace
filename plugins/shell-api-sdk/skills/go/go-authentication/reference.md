...
```go
shellapi.WithOAuthCCGCredentials(
    shellapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr shellapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := shellapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr shellapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := shellapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := shellapi.NewClient(shellapi.CreateConfiguration(shellapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    shellapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := shellapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := shellapi.NewClient(shellapi.CreateConfiguration(shellapi.WithOAuthROPCGCredentials(creds)))
...
```go
shellapi.WithOAuthBearerTokenCredentials(shellapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
shellapi.WithBasicAuthCredentials(shellapi.NewBasicAuthCredentials(username, password))
shellapi.WithApiHeaderCredentials(shellapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
shellapi.WithApiKeyCredentials(shellapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SHELLAPI_...` variables — e.g.
`SHELLAPI_USERNAME`, `SHELLAPI_PASSWORD`, `SHELLAPI_TOKEN`, `SHELLAPI_API_KEY`,
`SHELLAPI_O_AUTH_CLIENT_ID`, `SHELLAPI_O_AUTH_CLIENT_SECRET`, `SHELLAPI_ACCESS_TOKEN`. Grep the
