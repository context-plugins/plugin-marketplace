...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `BokudirectpaymentsapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`boku/configuration.py` in the cloned source.
...
```python
import requests
from boku.configuration import Configuration
...
`boku/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`boku/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `boku/configuration.py`
...
`NumberPagedResponse`) live in `boku/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `boku/http/http_call_back.py`) and pass an instance
as `http_call_back=`:

```python
import logging
from boku.http.http_call_back import HttpCallBack
from boku.configuration import Configuration
