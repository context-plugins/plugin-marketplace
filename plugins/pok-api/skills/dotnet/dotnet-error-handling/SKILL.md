...
> `{ApiGroup}`, `PokApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `PokApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `PokApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `PokApi.Errors`
...
```csharp
using PokApi.Core.Exceptions;     // SdkException<TError>
using PokApi.Core.ErrorResponse;  // ApiError, RawError
using PokApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`PokApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using PokApi.Core.Exceptions;     // SdkException<TError>
using PokApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `PokApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `PokApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using PokApi.Core.Models;        // ApiResult<TResponse, TError>
using PokApi.Core.ErrorResponse; // RawError
using PokApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `PokApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
