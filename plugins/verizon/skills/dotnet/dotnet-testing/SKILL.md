...
`VerizonApiClient`/`VerizonApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `VerizonApiClient`,
...
static VerizonApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new VerizonApiClient(new HttpClient(handler), new VerizonApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using VerizonApi.Errors;              // {Operation}Error types
...
```csharp
using VerizonApi.Core.Exceptions;
using VerizonApi.Core.ErrorResponse;
...
using VerizonApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new VerizonApiClient(new HttpClient(handler), new VerizonApiClientOptions());
...
- For DI-based code, the SDK's `AddVerizonApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `VerizonApiClient` from the provider:
  ```csharp
  services.AddVerizonApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<VerizonApiClient>();
  ```
