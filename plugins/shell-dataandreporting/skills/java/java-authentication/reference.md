...
against the `ShellDataReportingAPIsClient.Builder` setters, the `com.shell.apitest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ShellDataReportingAPIsClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ShellDataReportingAPIsClient client = new ShellDataReportingAPIsClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeShellDataandreportingEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new ShellDataReportingAPIsClient.Builder()
