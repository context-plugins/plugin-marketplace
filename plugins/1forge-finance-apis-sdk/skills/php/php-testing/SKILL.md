...
> `1ForgeFinanceAPIsClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): 1ForgeFinanceAPIsClient
{
...
```php
use M1ForgeFinanceAPIsLib\Exceptions\{OperationException};
use M1ForgeFinanceAPIsLib\Exceptions\ApiException;
...
app()->instance(1ForgeFinanceAPIsClient::class, clientWithResponses([
