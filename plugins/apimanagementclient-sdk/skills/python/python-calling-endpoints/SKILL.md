...
Open `apimanagementclient/apimanagementclient_client.py` for the controller property names, then the relevant
`apimanagementclient/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from apimanagementclient.models.{request_model} import {RequestModel}
...
Open `apimanagementclient/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from apimanagementclient.models.suite_code_enum import SuiteCodeEnum
...
```python
from apimanagementclient.http.http_call_back import HttpCallBack
...
client = ApiManagementClientClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `apimanagementclient/pagination/` in the source.

## Finding the right method

- Controller property names are on `ApiManagementClientClient` in `apimanagementclient/apimanagementclient_client.py`.
- Operation method signatures are in `apimanagementclient/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `apimanagementclient/models/`; enum types have `from_value()`.
