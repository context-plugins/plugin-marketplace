...
To see which schemes a specific SDK accepts, read the credential setter methods on `FlightPriceAnalysisAPIClient.Builder`
...
- Set credentials **on the `FlightPriceAnalysisAPIClient.Builder`** before calling `.build()` — the client is immutable
