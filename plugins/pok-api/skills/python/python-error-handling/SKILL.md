...
`pokapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `pokapi/exceptions/` and operation docstrings.
...
```python
from pokapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `pokapi/exceptions/`.
...
```python
from pokapi.exceptions.api_exception import APIException
from pokapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`pokapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`pokapi/exceptions/` for the class.
...
`pokapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `pokapi/exceptions/`
  or a `ValueError` — check the auth handler in `pokapi/http/auth/` for the exact type.
