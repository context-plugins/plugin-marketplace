...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the AdafruitIORESTAPIClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write AdafruitIORESTAPIClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `adafruitiorestapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `AdafruitIORESTAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `AdafruitIORESTAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from adafruitiorestapi.adafruitiorestapi_client import AdafruitIORESTAPIClient
from adafruitiorestapi.configuration import Environment
from adafruitiorestapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = AdafruitIORESTAPIClient(
...
```python
from adafruitiorestapi.configuration import Configuration, Environment
...
client = AdafruitIORESTAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `adafruitiorestapi/adafruitiorestapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `AdafruitIORESTAPIClient` and `Configuration` that reads
...
client = AdafruitIORESTAPIClient.from_environment()
...
`from_environment` in `adafruitiorestapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `AdafruitIORESTAPIClient`. Access them as attributes —
...
Open `adafruitiorestapi/adafruitiorestapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `adafruitiorestapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from adafruitiorestapi.configuration import Environment

client = AdafruitIORESTAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`adafruitiorestapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = AdafruitIORESTAPIClient(config=new_config)
```
