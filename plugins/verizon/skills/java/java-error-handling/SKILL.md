...
│   └── com.verizon.thingspace.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.verizon.thingspace.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.verizon.thingspace.http.client.HttpContext`, which carries:
