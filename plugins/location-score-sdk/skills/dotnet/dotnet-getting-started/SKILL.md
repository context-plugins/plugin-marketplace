...
| --- | --- |
| API | `Location Score` |
| Project | `LocationScore.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `LocationScore` (the `using` namespace) |
| Client class | `LocationScoreClient` |
| Options class | `LocationScoreClientOptions` |
...
namespaces transitively, so `using LocationScore.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/location-score-csharp
dotnet add reference location-score-csharp/LocationScore.csproj
```

```csharp
using LocationScore;          // client + options + ServerEnvironment
using LocationScore.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/location-score-csharp /tmp/location-score-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/location-score-csharp "$env:TEMP\location-score-dotnet-src"
```
...
- `LocationScoreClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/location-score-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\location-score-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `LocationScoreClient`, build its
   `LocationScoreClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
