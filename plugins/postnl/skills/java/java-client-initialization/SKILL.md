...
- `PostnlEcommerceClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `nl.postnl.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`PostnlEcommerceClient.Builder` class. A minimal initialization:

```java
import nl.postnl.api.PostnlEcommerceClient;
import nl.postnl.api.Environment;

PostnlEcommerceClient client = new PostnlEcommerceClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `PostnlEcommerceClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `PostnlEcommerceClient.java` |

Call `.build()` to produce an immutable `PostnlEcommerceClient` instance.
...
public class ApiConfig {
    @Bean
    public PostnlEcommerceClient apiClient() {
        return new PostnlEcommerceClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired PostnlEcommerceClient client) {
        client.shutdown();
    }
}
```

Inject `PostnlEcommerceClient` into your services as a normal Spring dependency. Build only one bean.
