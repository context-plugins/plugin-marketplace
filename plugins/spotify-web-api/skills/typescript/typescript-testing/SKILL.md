...
```typescript
import { {Operation}Error } from 'spotify-web-apilib/errors';
...
```typescript
import { ApiError } from 'spotify-web-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(SpotifyWebAPIClient)
    .useValue(new SpotifyWebAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
