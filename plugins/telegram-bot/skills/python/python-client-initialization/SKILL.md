...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the TelegramBotAPIClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write TelegramBotAPIClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `telegrambotapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `TelegramBotAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `TelegramBotAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from telegrambotapi.telegrambotapi_client import TelegramBotAPIClient
from telegrambotapi.configuration import Environment
from telegrambotapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from telegrambotapi.configuration import Configuration, Environment
...
client = TelegramBotAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `telegrambotapi/telegrambotapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `TelegramBotAPIClient` and `Configuration` that reads
...
client = TelegramBotAPIClient.from_environment()
...
`from_environment` in `telegrambotapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `TelegramBotAPIClient`. Access them as attributes —
...
Open `telegrambotapi/telegrambotapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `telegrambotapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from telegrambotapi.configuration import Environment

client = TelegramBotAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`telegrambotapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = TelegramBotAPIClient(config=new_config)
```
