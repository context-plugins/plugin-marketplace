...
`testing` + `testify/require` for reference; substitute the real `frankfurter` and names.
...
import (
    ...
    "github.com/context-plugins/frankfurter-go-sdk"   // root package frankfurter
)
...
func clientReturning(status int, body string) (frankfurter.ClientInterface, *stubTransport) {
    ...
    client := frankfurter.NewClient(
        frankfurter.CreateConfiguration(
            frankfurter.WithHttpConfiguration(
                frankfurter.CreateHttpConfiguration(frankfurter.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/frankfurter-go-sdk/errors"
...
client := frankfurter.NewClient(
    frankfurter.CreateConfiguration(
        frankfurter.WithHttpConfiguration(
            frankfurter.CreateHttpConfiguration(frankfurter.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
