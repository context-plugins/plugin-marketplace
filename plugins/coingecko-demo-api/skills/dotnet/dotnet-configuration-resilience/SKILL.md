...
All types below live under `CoinGeckoDemoApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using CoinGeckoDemoApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new CoinGeckoDemoApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `CoinGeckoDemoApi.Core.Exceptions`):
...
With DI, the SDK's `AddCoinGeckoDemoApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddCoinGeckoDemoApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
