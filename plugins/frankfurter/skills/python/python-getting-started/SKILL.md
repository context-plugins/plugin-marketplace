...
> setting, so **confirm it** from the class names under `frankfurter/apis/`.
...
| Install | `pip install "git+https://github.com/context-plugins/frankfurter-python-sdk@main"` |
| Client class | `FrankfurterapiClient` in `frankfurter/frankfurter_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `frankfurter/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `FrankfurterapiClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `frankfurter/exceptions/api_exception.py` — raised on non-2xx |
...
- `frankfurter/frankfurter_client.py` — the `FrankfurterapiClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `frankfurter/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `frankfurter/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `frankfurter/controllers/` — one `{Resource}Api` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `frankfurter/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `frankfurter/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `frankfurter/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `frankfurter/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `frankfurter/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/frankfurter-python-sdk@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/frankfurter-python-sdk /tmp/frankfurter-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/frankfurter-python-sdk "$env:TEMP\frankfurter-python-src"
```
...
- `frankfurter/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `frankfurter/frankfurter_client.py` — `FrankfurterapiClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `frankfurter/controllers/*.py` — operation method signatures, parameter names, and return types.
- `frankfurter/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `frankfurter/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/frankfurter-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\frankfurter-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `FrankfurterapiClient(...)`.
