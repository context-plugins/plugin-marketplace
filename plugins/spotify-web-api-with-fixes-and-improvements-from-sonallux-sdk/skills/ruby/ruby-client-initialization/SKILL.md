...
- `SpotifyWebAPIWithFixesAndImprovementsFromSonallux` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `spotify_web_api_with_fixes_and_improvements_from_sonallux` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/configuration.rb`
(`build_default_config_from_env`) and `lib/spotify_web_api_with_fixes_and_improvements_from_sonallux/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/spotify_web_api_with_fixes_and_improvements_from_sonallux.rb`) and store it in a
