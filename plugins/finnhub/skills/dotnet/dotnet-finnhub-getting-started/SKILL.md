...
| --- | --- |
| API | Finnhub |
| Source repo | https://github.com/context-plugins/finnhub-csharp-sdk (branch `main`) |
| Root namespace | `FinnhubApi` (the `using` namespace) |
| Client class | `FinnhubApiClient` |
| Options class | `FinnhubApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `FinnhubApiClientOptions`: `ApiKey: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using FinnhubApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/finnhub-csharp-sdk
dotnet add reference finnhub-csharp-sdk/FinnhubApi.csproj
```
...
temp directory** (`<temp>/finnhub-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/finnhub-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/finnhub-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/finnhub-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\finnhub-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/finnhub-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/finnhub-dotnet-src/`, **never** in the project repo, and its path never goes into
`finnhub-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new FinnhubApiClient(...)`, build its options, or DI-register via
   `AddFinnhubApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `FinnhubApiClientOptions` — the map's *Servers & auth* section
