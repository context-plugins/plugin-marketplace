...
| Project | `ApiManagementClient.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `ApiManagementClient` (the `using` namespace) |
| Client class | `ApiManagementClientClient` |
| Options class | `ApiManagementClientClientOptions` |
...
namespaces transitively, so `using ApiManagementClient.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/apimanagementclient-csharp
dotnet add reference apimanagementclient-csharp/ApiManagementClient.csproj
```

```csharp
using ApiManagementClient;          // client + options + ServerEnvironment
using ApiManagementClient.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/apimanagementclient-csharp /tmp/apimanagementclient-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/apimanagementclient-csharp "$env:TEMP\apimanagementclient-dotnet-src"
```
...
- `ApiManagementClientClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/apimanagementclient-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\apimanagementclient-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `ApiManagementClientClient`, build its
   `ApiManagementClientClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
