...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `adyen`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'adyen';
import { ApiError } from 'adyen';
import { {Operation}Error } from 'adyen/errors';
