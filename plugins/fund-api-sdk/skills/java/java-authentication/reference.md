...
against the `FundAPIClient.Builder` setters, the `com.adyen.caltest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FundAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FundAPIClient client = new FundAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFundAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
