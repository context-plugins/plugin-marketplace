...
your SDK source (`apisguru/models/`).
...
```python
from apisguru.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `apisguru/utilities/union_type_lookup.py`) and each candidate model
...
```python
from apisguru.models.animal import Cat, Dog
...
```python
from apisguru.models.animal import Animal, Cat, Dog
