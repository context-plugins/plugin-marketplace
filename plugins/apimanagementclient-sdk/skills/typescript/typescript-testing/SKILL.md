...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new ApiManagementClientClient({
...
```typescript
import { {Operation}Error } from 'apimanagementclientlib/errors';
...
```typescript
import { ApiError } from 'apimanagementclientlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(ApiManagementClientClient)
    .useValue(new ApiManagementClientClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
