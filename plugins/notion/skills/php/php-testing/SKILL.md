...
> `NotionClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): NotionClient
{
...
```php
use NotionLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use NotionLib\Exceptions\{OperationException};
use NotionLib\Exceptions\ApiException;
...
app()->instance(NotionClient::class, clientWithResponses([
