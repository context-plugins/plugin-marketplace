...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddFlightMostTraveledDestinationsClient extension. Use the moment you write `new FlightMostTraveledDestinationsClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `FlightMostTraveledDestinationsClient` — the single public client class (e.g. `FooClient`).
- `FlightMostTraveledDestinationsClientOptions` — its options class.
- `FlightMostTraveledDestinations` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public FlightMostTraveledDestinationsClient(HttpClient httpClient, FlightMostTraveledDestinationsClientOptions options)
```
...
```csharp
public class FlightMostTraveledDestinationsClientOptions
{
...
```csharp
using FlightMostTraveledDestinations;
using FlightMostTraveledDestinations.Servers;

var options = new FlightMostTraveledDestinationsClientOptions
{
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddFlightMostTraveledDestinationsClient`, which registers the
...
builder.Services.AddFlightMostTraveledDestinationsClient(options =>
{
...
```csharp
public sealed class MyService(FlightMostTraveledDestinationsClient client)
{
