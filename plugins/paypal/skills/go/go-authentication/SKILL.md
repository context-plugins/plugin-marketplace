...
> Throughout, `paypal` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := paypal.NewClient(
    paypal.CreateConfiguration(
        paypal.WithBasicAuthCredentials(
            paypal.NewBasicAuthCredentials(
...
paypal.WithApiKeyCredentials(paypal.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
paypal.WithApiHeaderCredentials(paypal.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
paypal.WithOAuthBearerTokenCredentials(
    paypal.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
