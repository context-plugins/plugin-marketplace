...
All types below live under `VerizonApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using VerizonApi.Core.Configuration;
...
```csharp
using VerizonApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new VerizonApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `VerizonApi.Core.Exceptions`):
...
With DI, the SDK's `AddVerizonApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddVerizonApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
