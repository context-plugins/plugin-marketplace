...
To see which schemes a specific SDK accepts, read the credential setter methods on `CoinGeckoDemoApiClient.Builder`
...
- Set credentials **on the `CoinGeckoDemoApiClient.Builder`** before calling `.build()` — the client is immutable
