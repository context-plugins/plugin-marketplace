...
client = stub_client(stubs)
assert_raises APIsGuru::APIException do
...
def build_test_client
  APIsGuru::Client.new(
    environment: APIsGuru::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error APIsGuru::APIException
```
...
```ruby
expect { call }.to raise_error(APIsGuru::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < APIsGuru::HttpCallBack
...
client  = APIsGuru::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
