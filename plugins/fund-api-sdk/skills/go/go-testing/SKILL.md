...
`testing` + `testify/require` for reference; substitute the real `fundapi` and names.
...
import (
    ...
    "github.com/context-plugins/fund-api-go"   // root package fundapi
)
...
func clientReturning(status int, body string) (fundapi.ClientInterface, *stubTransport) {
    ...
    client := fundapi.NewClient(
        fundapi.CreateConfiguration(
            fundapi.WithHttpConfiguration(
                fundapi.CreateHttpConfiguration(fundapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/fund-api-go/errors"
...
client := fundapi.NewClient(
    fundapi.CreateConfiguration(
        fundapi.WithHttpConfiguration(
            fundapi.CreateHttpConfiguration(fundapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
