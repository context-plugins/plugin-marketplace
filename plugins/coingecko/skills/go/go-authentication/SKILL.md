...
> Throughout, `coingecko` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := coingecko.NewClient(
    coingecko.CreateConfiguration(
        coingecko.WithBasicAuthCredentials(
            coingecko.NewBasicAuthCredentials(
...
coingecko.WithApiKeyCredentials(coingecko.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
coingecko.WithApiHeaderCredentials(coingecko.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
coingecko.WithOAuthBearerTokenCredentials(
    coingecko.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := coingecko.NewClient(
    coingecko.CreateConfiguration(
        coingecko.WithOAuthCCGCredentials(
            coingecko.NewOAuthCCGCredentials(
