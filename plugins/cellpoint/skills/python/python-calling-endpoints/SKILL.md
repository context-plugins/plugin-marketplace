...
Open `cellpoint/cellpoint_client.py` for the controller property names, then the relevant
`cellpoint/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from cellpoint.models.{request_model} import {RequestModel}
...
Open `cellpoint/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from cellpoint.models.suite_code_enum import SuiteCodeEnum
...
```python
from cellpoint.http.http_call_back import HttpCallBack
...
client = CellpointapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `cellpoint/pagination/` in the source.

## Finding the right method

- Controller property names are on `CellpointapiClient` in `cellpoint/cellpoint_client.py`.
- Operation method signatures are in `cellpoint/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `cellpoint/models/`; enum types have `from_value()`.
