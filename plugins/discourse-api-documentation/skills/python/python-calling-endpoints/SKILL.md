...
Open `discourseapidocumentation/discourseapidocumentation_client.py` for the controller property names, then the relevant
`discourseapidocumentation/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from discourseapidocumentation.models.{request_model} import {RequestModel}
...
Open `discourseapidocumentation/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from discourseapidocumentation.models.suite_code_enum import SuiteCodeEnum
...
```python
from discourseapidocumentation.http.http_call_back import HttpCallBack
...
client = DiscourseapidocumentationClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `discourseapidocumentation/pagination/` in the source.

## Finding the right method

- Controller property names are on `DiscourseapidocumentationClient` in `discourseapidocumentation/discourseapidocumentation_client.py`.
- Operation method signatures are in `discourseapidocumentation/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `discourseapidocumentation/models/`; enum types have `from_value()`.
