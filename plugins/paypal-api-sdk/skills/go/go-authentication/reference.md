...
```go
paypalapi.WithOAuthCCGCredentials(
    paypalapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr paypalapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := paypalapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr paypalapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := paypalapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := paypalapi.NewClient(paypalapi.CreateConfiguration(paypalapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    paypalapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := paypalapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := paypalapi.NewClient(paypalapi.CreateConfiguration(paypalapi.WithOAuthROPCGCredentials(creds)))
...
```go
paypalapi.WithOAuthBearerTokenCredentials(paypalapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
paypalapi.WithBasicAuthCredentials(paypalapi.NewBasicAuthCredentials(username, password))
paypalapi.WithApiHeaderCredentials(paypalapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
paypalapi.WithApiKeyCredentials(paypalapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `PAYPALAPI_...` variables — e.g.
`PAYPALAPI_USERNAME`, `PAYPALAPI_PASSWORD`, `PAYPALAPI_TOKEN`, `PAYPALAPI_API_KEY`,
`PAYPALAPI_O_AUTH_CLIENT_ID`, `PAYPALAPI_O_AUTH_CLIENT_SECRET`, `PAYPALAPI_ACCESS_TOKEN`. Grep the
