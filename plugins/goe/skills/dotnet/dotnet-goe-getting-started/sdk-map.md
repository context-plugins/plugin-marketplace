...
```csharp
using Goeapi;
using Goeapi.Servers; // ServerEnvironment lives here

var options = new GoeapiClientOptions
{
...
var client = new GoeapiClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddGoeapiClient(o =>
{
...
All `GoeapiClientOptions` properties (source: `GoeapiClientOptions.cs`):
...
- `GoeapiClient(HttpClient httpClient, GoeapiClientOptions options)`
<!-- /crawler:client-options -->
...
|---|---|---|
| `ApiError` — abstract base of all 14 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
...
Of **14 operations**, **14 are Case A (typed)** and **0 are Case B (raw)**.
<!-- /crawler:op-stats -->

---

## Operations — by controller (2 groups, 14 operations)
...
| `Goe` | 10 | [map/operations/Goe.md](map/operations/Goe.md) |
<!-- /crawler:ops-table -->
...
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 122 | [`Account` … `WealthSplitterOutputModel`](map/models/records-1-Ac-We.md) |
| Unions (`OneOf` / `AnyOf`) — variant factories + `TryGet…` | 0 + 13 | [map/models/unions.md](map/models/unions.md) |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — literal C# member names + wire values | 33 | [map/models/enums.md](map/models/enums.md) |
<!-- /gen:models-table -->
...
|---|---|
| Client & options (root) | `Goeapi` |
| Operation controllers (`Api/`) | `Goeapi.Api` |
| Records (`Models/`) | `Goeapi.Models` |
| Enums (`Models/Enums/`) | `Goeapi.Models.Enums` |
| Unions (`Models/AnyOf/`, `Models/OneOf/`) | `Goeapi.Models.AnyOf` |
| Error classes (`Errors/`) | `Goeapi.Errors` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `GoeapiClientOptions` (source: `GoeapiClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:
