...
```python
from pokapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `pokapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from pokapi.models.animal import Cat, Dog
...
```python
from pokapi.models.animal import Animal, Cat, Dog
