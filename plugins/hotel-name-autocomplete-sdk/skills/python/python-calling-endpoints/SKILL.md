...
Open `hotelnameautocomplete/hotelnameautocomplete_client.py` for the controller property names, then the relevant
`hotelnameautocomplete/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from hotelnameautocomplete.models.{request_model} import {RequestModel}
...
Open `hotelnameautocomplete/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from hotelnameautocomplete.models.suite_code_enum import SuiteCodeEnum
...
```python
from hotelnameautocomplete.http.http_call_back import HttpCallBack
...
client = HotelNameAutocompleteClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `hotelnameautocomplete/pagination/` in the source.

## Finding the right method

- Controller property names are on `HotelNameAutocompleteClient` in `hotelnameautocomplete/hotelnameautocomplete_client.py`.
- Operation method signatures are in `hotelnameautocomplete/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `hotelnameautocomplete/models/`; enum types have `from_value()`.
