...
All types below live under `FundApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FundApi.Core.Configuration;
...
```csharp
using FundApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FundApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FundApi.Core.Exceptions`):
...
With DI, the SDK's `AddFundApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFundApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
