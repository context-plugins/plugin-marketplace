...
> `{ApiGroup}`, `ForgeFinanceApis1`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `ForgeFinanceApis1.Core.Exceptions`
- `ApiError` **and** `RawError` → `ForgeFinanceApis1.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `ForgeFinanceApis1.Errors`
...
```csharp
using ForgeFinanceApis1.Core.Exceptions;     // SdkException<TError>
using ForgeFinanceApis1.Core.ErrorResponse;  // ApiError, RawError
using ForgeFinanceApis1.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`ForgeFinanceApis1.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using ForgeFinanceApis1.Core.Exceptions;     // SdkException<TError>
using ForgeFinanceApis1.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `ForgeFinanceApis1.Core.ErrorResponse`. Its public
...
`readonly struct` under `ForgeFinanceApis1.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using ForgeFinanceApis1.Core.Models;        // ApiResult<TResponse, TError>
using ForgeFinanceApis1.Core.ErrorResponse; // RawError
using ForgeFinanceApis1.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `ForgeFinanceApis1.Core.Exceptions`) — an auth *application* failure, not an API error — when the
