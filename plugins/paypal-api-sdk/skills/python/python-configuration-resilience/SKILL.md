...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `PayPalAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`paypalapi/configuration.py` in the cloned source.
...
```python
import requests
from paypalapi.configuration import Configuration
...
`paypalapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`paypalapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `paypalapi/configuration.py`
...
`NumberPagedResponse`) live in `paypalapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `paypalapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
