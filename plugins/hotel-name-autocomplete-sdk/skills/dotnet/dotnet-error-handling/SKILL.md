...
> `{ApiGroup}`, `HotelNameAutocomplete`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `HotelNameAutocomplete.Core.Exceptions`
- `ApiError` **and** `RawError` → `HotelNameAutocomplete.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `HotelNameAutocomplete.Errors`
...
```csharp
using HotelNameAutocomplete.Core.Exceptions;     // SdkException<TError>
using HotelNameAutocomplete.Core.ErrorResponse;  // ApiError, RawError
using HotelNameAutocomplete.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`HotelNameAutocomplete.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using HotelNameAutocomplete.Core.Exceptions;     // SdkException<TError>
using HotelNameAutocomplete.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `HotelNameAutocomplete.Core.ErrorResponse`. Its public
...
`readonly struct` under `HotelNameAutocomplete.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using HotelNameAutocomplete.Core.Models;        // ApiResult<TResponse, TError>
using HotelNameAutocomplete.Core.ErrorResponse; // RawError
using HotelNameAutocomplete.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `HotelNameAutocomplete.Core.Exceptions`) — an auth *application* failure, not an API error — when the
