...
kwargs to the `AccessAnalyzerClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `accessanalyzer/accessanalyzer_client.py` and `accessanalyzer/http/auth/` in
the cloned source.

## Basic auth

```python
from accessanalyzer.http.auth.basic_auth import BasicAuthCredentials
...
```python
from accessanalyzer.http.auth.api_key import ApiKeyCredentials
...
```python
from accessanalyzer.http.auth.api_header import ApiHeaderCredentials
...
```python
from accessanalyzer.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from accessanalyzer.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from accessanalyzer.http.auth.o_auth_acg import OAuthACGCredentials
from accessanalyzer.models.o_auth_scope_accessanalyzer_enum import OAuthScope{Pkg}Enum
...
client = AccessAnalyzerClient(o_auth_acg_credentials=creds)
...
```python
from accessanalyzer.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AccessAnalyzerClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AccessAnalyzerClient.from_environment()`) read each credential
...
Grep `from_environment` in `accessanalyzer/configuration.py` and each `accessanalyzer/http/auth/*.py` for the exact
