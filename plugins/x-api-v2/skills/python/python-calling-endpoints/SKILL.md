...
Open `xapiv2/xapiv2_client.py` for the controller property names, then the relevant
`xapiv2/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from xapiv2.models.{request_model} import {RequestModel}
...
Open `xapiv2/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from xapiv2.models.suite_code_enum import SuiteCodeEnum
...
```python
from xapiv2.http.http_call_back import HttpCallBack
...
client = Xapiv2Client(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `xapiv2/pagination/` in the source.

## Finding the right method

- Controller property names are on `Xapiv2Client` in `xapiv2/xapiv2_client.py`.
- Operation method signatures are in `xapiv2/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `xapiv2/models/`; enum types have `from_value()`.
