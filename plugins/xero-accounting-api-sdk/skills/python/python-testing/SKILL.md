...
reference; substitute your `xeroaccountingapi` and `XeroAccountingAPIClient` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `xeroaccountingapi/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
