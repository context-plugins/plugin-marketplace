...
kwargs to the `MailboxValidatorFreeEmailCheckerClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `mailboxvalidatorfreeemailchecker/mailboxvalidatorfreeemailchecker_client.py` and `mailboxvalidatorfreeemailchecker/http/auth/` in
the cloned source.

## Basic auth

```python
from mailboxvalidatorfreeemailchecker.http.auth.basic_auth import BasicAuthCredentials
...
```python
from mailboxvalidatorfreeemailchecker.http.auth.api_key import ApiKeyCredentials
...
```python
from mailboxvalidatorfreeemailchecker.http.auth.api_header import ApiHeaderCredentials
...
```python
from mailboxvalidatorfreeemailchecker.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from mailboxvalidatorfreeemailchecker.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from mailboxvalidatorfreeemailchecker.http.auth.o_auth_acg import OAuthACGCredentials
from mailboxvalidatorfreeemailchecker.models.o_auth_scope_mailboxvalidatorfreeemailchecker_enum import OAuthScope{Pkg}Enum
...
client = MailboxValidatorFreeEmailCheckerClient(o_auth_acg_credentials=creds)
...
```python
from mailboxvalidatorfreeemailchecker.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = MailboxValidatorFreeEmailCheckerClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `MailboxValidatorFreeEmailCheckerClient.from_environment()`) read each credential
...
Grep `from_environment` in `mailboxvalidatorfreeemailchecker/configuration.py` and each `mailboxvalidatorfreeemailchecker/http/auth/*.py` for the exact
