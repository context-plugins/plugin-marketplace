...
To see which schemes a specific SDK accepts, read the credential setter methods on `ShellCardManagementAPIsClient.Builder`
...
- Set credentials **on the `ShellCardManagementAPIsClient.Builder`** before calling `.build()` — the client is immutable
