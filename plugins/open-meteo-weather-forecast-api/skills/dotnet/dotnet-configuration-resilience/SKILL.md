...
All types below live under `OpenMeteoWeatherForecastApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using OpenMeteoWeatherForecastApi.Core.Configuration;
...
```csharp
using OpenMeteoWeatherForecastApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new OpenMeteoWeatherForecastApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `OpenMeteoWeatherForecastApi.Core.Exceptions`):
...
With DI, the SDK's `AddOpenMeteoWeatherForecastApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddOpenMeteoWeatherForecastApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
