...
> `{operation}`, `{apiGroup}`, `ShellEVLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `ShellEVLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use ShellEVLib\Exceptions\ApiException;
use ShellEVLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `ShellEVLib\Exceptions\` — read the source to
