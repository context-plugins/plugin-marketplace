...
To see which schemes a specific SDK accepts, read the credential setter methods on `MindbodyClient.Builder`
...
- Set credentials **on the `MindbodyClient.Builder`** before calling `.build()` — the client is immutable
