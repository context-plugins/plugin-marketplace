...
> `{ApiGroup}`, `TravelRecommendationsApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `TravelRecommendationsApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `TravelRecommendationsApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `TravelRecommendationsApi.Errors`
...
```csharp
using TravelRecommendationsApi.Core.Exceptions;     // SdkException<TError>
using TravelRecommendationsApi.Core.ErrorResponse;  // ApiError, RawError
using TravelRecommendationsApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`TravelRecommendationsApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using TravelRecommendationsApi.Core.Exceptions;     // SdkException<TError>
using TravelRecommendationsApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `TravelRecommendationsApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `TravelRecommendationsApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using TravelRecommendationsApi.Core.Models;        // ApiResult<TResponse, TError>
using TravelRecommendationsApi.Core.ErrorResponse; // RawError
using TravelRecommendationsApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `TravelRecommendationsApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
