...
your SDK source (`firecrawlapi/models/`).
...
```python
from firecrawlapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `firecrawlapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from firecrawlapi.models.animal import Cat, Dog
...
```python
from firecrawlapi.models.animal import Animal, Cat, Dog
