...
your SDK source (`vimeoapi/models/`).
...
```python
from vimeoapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `vimeoapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from vimeoapi.models.animal import Cat, Dog
...
```python
from vimeoapi.models.animal import Animal, Cat, Dog
