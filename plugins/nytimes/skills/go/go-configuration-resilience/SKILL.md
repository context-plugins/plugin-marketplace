    ...
    nytimes.CreateConfiguration(
        nytimes.WithHttpConfiguration(
            nytimes.CreateHttpConfiguration(
                nytimes.WithTimeout(30),
                nytimes.WithRetryConfiguration(
                    nytimes.CreateRetryConfiguration(
                        nytimes.WithMaxRetryAttempts(3),
                        nytimes.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := nytimes.CreateRetryConfiguration(
    nytimes.WithMaxRetryAttempts(5),
    nytimes.WithRetryInterval(1),
    nytimes.WithBackoffFactor(2),
    nytimes.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    nytimes.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := nytimes.NewClient(
    nytimes.CreateConfiguration(
        nytimes.WithHttpConfiguration(
            nytimes.CreateHttpConfiguration(
                nytimes.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
