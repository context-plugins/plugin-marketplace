...
your SDK source (`cellpointapi/models/`).
...
```python
from cellpointapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `cellpointapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from cellpointapi.models.animal import Cat, Dog
...
```python
from cellpointapi.models.animal import Animal, Cat, Dog
