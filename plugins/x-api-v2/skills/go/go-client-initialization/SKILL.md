...
- `xapiv2` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/x-api-v2-go-sdk` — the Go import path for the root package; `github.com/context-plugins/x-api-v2-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/x-api-v2-go-sdk"

client := xapiv2.NewClient(
    xapiv2.CreateConfiguration(
        xapiv2.WithEnvironment(xapiv2.PRODUCTION),
...
from environment variables named `XAPIV2_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := xapiv2.NewClient(xapiv2.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `xapiv2.PRODUCTION`,
`xapiv2.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := xapiv2.NewClient(
    xapiv2.CreateConfiguration(xapiv2.WithEnvironment(xapiv2.PRODUCTION)),
)
```
...
client := xapiv2.NewClient(
    xapiv2.CreateConfiguration(
        xapiv2.WithHttpConfiguration(
            xapiv2.CreateHttpConfiguration(
                xapiv2.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                xapiv2.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = xapiv2.NewClient(xapiv2.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api xapiv2.ClientInterface
}

func NewService(api xapiv2.ClientInterface) *Service {
