...
- `1forgefinanceapis` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/1forge-finance-apis-go` — the Go import path for the root package; `github.com/context-plugins/1forge-finance-apis-go/models` for request/response types.
...
```go
import "github.com/context-plugins/1forge-finance-apis-go"

client := 1forgefinanceapis.NewClient(
    1forgefinanceapis.CreateConfiguration(
        1forgefinanceapis.WithEnvironment(1forgefinanceapis.PRODUCTION),
...
from environment variables named `1FORGEFINANCEAPIS_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := 1forgefinanceapis.NewClient(1forgefinanceapis.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `1forgefinanceapis.PRODUCTION`,
`1forgefinanceapis.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := 1forgefinanceapis.NewClient(
    1forgefinanceapis.CreateConfiguration(1forgefinanceapis.WithEnvironment(1forgefinanceapis.PRODUCTION)),
)
```
...
client := 1forgefinanceapis.NewClient(
    1forgefinanceapis.CreateConfiguration(
        1forgefinanceapis.WithHttpConfiguration(
            1forgefinanceapis.CreateHttpConfiguration(
                1forgefinanceapis.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                1forgefinanceapis.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = 1forgefinanceapis.NewClient(1forgefinanceapis.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api 1forgefinanceapis.ClientInterface
}

func NewService(api 1forgefinanceapis.ClientInterface) *Service {
