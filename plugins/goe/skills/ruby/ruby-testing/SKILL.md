...
client = stub_client(stubs)
assert_raises GOE::APIException do
...
BASE = GOE::Configuration.new(environment: GOE::Environment::PRODUCTION).get_base_uri

def build_test_client
  GOE::Client.new(
    environment: GOE::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error GOE::APIException
```
...
```ruby
expect { call }.to raise_error(GOE::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < GOE::HttpCallBack
...
client  = GOE::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
