...
client = stub_client(stubs)
assert_raises Docker::APIException do
...
def build_test_client
  Docker::Client.new(
    environment: Docker::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Docker::APIException
```
...
```ruby
expect { call }.to raise_error(Docker::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Docker::HttpCallBack
...
client  = Docker::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
