...
| --- | --- |
| API | Finnhub API |
| Package name (what you install, and what you import) | `finnhub-api` — **not on npm**; built from source (see *Install*) |
| Import specifier | `finnhub-api` — the package root is the **only** entry; deep imports do not resolve |
| Version | `1.0.0` (API spec version `1.0.0`) |
| Client class | `FinnhubApiClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new FinnhubApiClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new FinnhubApiClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `apiKey`. `timeout` defaults to `60_000` ms |
| Auth | **API key (query)** — set `ClientOptions.apiKey` |
| Environments | 1 environment (`ServerEnvironment.Production` *(default)*) × 1 server group |
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://finnhub.io/api/v1` |
...
| Surface | 117 operations · 18 resources · 192 models · 0 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/finnhub-typescript-sdk#main
```

That fetches everything the package's `files` list packs — `src/`, `sdk-map.md` and the pages under `map/operations/` — so **every lookup on this page works as soon as the install finishes**. What it does *not* do is build `dist/`: the generated manifest declares no `prepare` script, and the `exports` map points into `dist/`, so the first `import` fails until something builds it. Run the SDK's own build once inside the installed package (`npm install && npm run build` in `node_modules/finnhub-api/`), or clone the repository, build there, and depend on that directory by path instead.
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 192 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { FinnhubApiClient, ServerEnvironment, ResponseError, FinnhubApiError } from "finnhub-api";
import type { ClientOptions, AiChatBody } from "finnhub-api";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `finnhub-api/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **From CommonJS**, the typed spelling is `import sdk = require("finnhub-api")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://finnhub.io/api/v1` | `serverOptions.default.production.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 117 of the 117 operations require a credential and 0 are public.

| `ClientOptions` field | Scheme kind | What the SDK sends |
| --- | --- | --- |
| `apiKey` | API key (query) | query `token=<key>` |

```ts
const client = new FinnhubApiClient({
...
| --- | --- | --- | --- |
| `client.bond` | `Bond` | 4 | `bondPrice` · `bondProfile` · `bondTick` · `bondYieldCurve` |
| `client.calendar` | `Calendar` | 3 | `earningsCalendar` · `economicCalendar` · `ipoCalendar` |
| `client.corporateActions` | `CorporateActions` | 2 | `isinChange` · `symbolChange` |
| `client.crypto` | `Crypto` | 4 | `cryptoCandles` · `cryptoExchanges` · `cryptoProfile` · `cryptoSymbols` |
| `client.economic` | `Economic` | 2 | `economicCode` · `economicData` |
| `client.etf` | `Etf` | 5 | `etfsAllocation` · `etfsCountryExposure` · `etfsHoldings` · `etfsProfile` · `etfsSectorExposure` |
| `client.forex` | `Forex` | 4 | `forexCandles` · `forexExchanges` · `forexRates` · `forexSymbols` |
| `client.globalFilings` | `GlobalFilings` | 4 | `globalFilingsDownload` · `globalFilingsSearch` · `globalFilingsSearchFilter` · `searchInFiling` |
| `client.index` | `Index` | 2 | `indicesConstituents` · `indicesHistoricalConstituents` |
| `client.institutional` | `Institutional` | 3 | `institutionalOwnership` · `institutionalPortfolio` · `institutionalProfile` |
| `client.misc` | `Misc` | 10 | `aiChat` · `airlinePriceIndex` · `bankBranch` · `country` · `covid19` · `fdaCommitteeMeetingCalendar` · `quote` · `sectorMetric` · `symbolSearch` · `technicalIndicator` |
| `client.mutualFund` | `MutualFund` | 6 | `mutualFundCountryExposure` · `mutualFundEet` · `mutualFundEetPai` · `mutualFundHoldings` · `mutualFundProfile` · `mutualFundSectorExposure` |
| `client.news` | `News` | 4 | `companyNews` · `marketNews` · `newsSentiment` · `pressReleases` |
| `client.scan` | `Scan` | 3 | `aggregateIndicator` · `patternRecognition` · `supportResistance` |
| `client.stockEstimates` | `StockEstimates` | 18 | `companyCapexEstimates` · `companyDpsEstimates` · `companyEarnings` · `companyEarningsQualityScore` · `companyEbitEstimates` · `companyEbitdaEstimates` · `companyEpsEstimates` · `companyFcfEstimates` · `companyGrossIncomeEstimates` · `companyNetIncomeEstimates` · `companyOcfEstimates` · `companyPretaxIncomeEstimates` · `companyRevenueEstimates` · `priceTarget` · `recommendationTrends` · `revenueBreakdown` · `revenueBreakdown2` · `upgradeDowngrade` |
| `client.stockFundamentals` | `StockFundamentals` | 18 | `companyBasicFinancials` · `companyEsgScore` · `companyExecutive` · `companyHistoricalEsgScore` · `companyProfile` · `companyProfile2` · `earningsCallLive` · `filings` · `filingsSentiment` · `financials` · `financialsReported` · `historicalEmployeeCount` · `internationalFilings` · `newsroom` · `similarityIndex` · `stockPresentation` · `transcripts` · `transcriptsList` |
| `client.stockOwnership` | `StockOwnership` | 12 | `congressionalTrading` · `fundOwnership` · `insiderSentiment` · `insiderTransactions` · `investmentThemes` · `ownership` · `socialSentiment` · `stockLobbying` · `stockUsaSpending` · `stockUsptoPatent` · `stockVisaApplication` · `supplyChainRelationships` |
| `client.stockPrices` | `StockPrices` | 13 | `companyPeers` · `historicalMarketCap` · `marketHoliday` · `marketStatus` · `priceMetrics` · `stockBasicDividends` · `stockBidask` · `stockCandles` · `stockDividends` · `stockNbbo` · `stockSplits` · `stockSymbols` · `stockTick` |
...
```bash
node -e "console.log(require.resolve('finnhub-api/package.json'))"
```

Failing that it is at `node_modules/finnhub-api/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/finnhub-api/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/finnhub-api/`:
...
Beyond the usual signatures and model members, a contract sheet for the Finnhub API TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
...
2. **One operation resolves to `undefined` (`globalFilings.globalFilingsDownload`)** — `await` gives you nothing to inspect, so **`.asApiResult()` is the only way to observe their status and headers** — decide the mode at write time, not by retrofit.
