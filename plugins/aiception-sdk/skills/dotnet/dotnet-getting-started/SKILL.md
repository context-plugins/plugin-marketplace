...
- `AIceptionInteractiveClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AIceptionInteractiveClient`, build its
   `AIceptionInteractiveClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
