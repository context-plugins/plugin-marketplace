...
| Project | `AdyenDataProtectionApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `AdyenDataProtectionApi` (the `using` namespace) |
| Client class | `AdyenDataProtectionApiClient` |
| Options class | `AdyenDataProtectionApiClientOptions` |
...
namespaces transitively, so `using AdyenDataProtectionApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/adyen-data-protection-api-csharp
dotnet add reference adyen-data-protection-api-csharp/AdyenDataProtectionApi.csproj
```

```csharp
using AdyenDataProtectionApi;          // client + options + ServerEnvironment
using AdyenDataProtectionApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/adyen-data-protection-api-csharp /tmp/adyen-data-protection-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/adyen-data-protection-api-csharp "$env:TEMP\adyen-data-protection-api-dotnet-src"
```
...
- `AdyenDataProtectionApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/adyen-data-protection-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\adyen-data-protection-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AdyenDataProtectionApiClient`, build its
   `AdyenDataProtectionApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
