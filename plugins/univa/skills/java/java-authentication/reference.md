...
against the `UnivapayClientSdkClient.Builder` setters, the `com.univapay.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `UnivapayClientSdkClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
UnivapayClientSdkClient client = new UnivapayClientSdkClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeUnivaEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new UnivapayClientSdkClient.Builder()
