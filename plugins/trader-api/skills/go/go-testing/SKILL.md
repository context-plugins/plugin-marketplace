...
`testing` + `testify/require` for reference; substitute the real `traderapi` and names.
...
import (
    ...
    "github.com/context-plugins/trader-api-go-sdk"   // root package traderapi
)
...
func clientReturning(status int, body string) (traderapi.ClientInterface, *stubTransport) {
    ...
    client := traderapi.NewClient(
        traderapi.CreateConfiguration(
            traderapi.WithHttpConfiguration(
                traderapi.CreateHttpConfiguration(traderapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/trader-api-go-sdk/errors"
...
client := traderapi.NewClient(
    traderapi.CreateConfiguration(
        traderapi.WithHttpConfiguration(
            traderapi.CreateHttpConfiguration(traderapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
