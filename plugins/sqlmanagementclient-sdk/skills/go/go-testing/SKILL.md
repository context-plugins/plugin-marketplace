...
`testing` + `testify/require` for reference; substitute the real `sqlmanagementclient` and names.
...
import (
    ...
    "github.com/context-plugins/sqlmanagementclient-go"   // root package sqlmanagementclient
)
...
func clientReturning(status int, body string) (sqlmanagementclient.ClientInterface, *stubTransport) {
    ...
    client := sqlmanagementclient.NewClient(
        sqlmanagementclient.CreateConfiguration(
            sqlmanagementclient.WithHttpConfiguration(
                sqlmanagementclient.CreateHttpConfiguration(sqlmanagementclient.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/sqlmanagementclient-go/errors"
...
client := sqlmanagementclient.NewClient(
    sqlmanagementclient.CreateConfiguration(
        sqlmanagementclient.WithHttpConfiguration(
            sqlmanagementclient.CreateHttpConfiguration(sqlmanagementclient.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
