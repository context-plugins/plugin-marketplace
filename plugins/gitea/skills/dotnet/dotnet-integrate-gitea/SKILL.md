---
name: dotnet-integrate-gitea
description: Entry point for gitea .NET SDK work in a C#/.NET project — this router is for the .NET/C# SDK ONLY; never load it for any other language. Load this first when asked to integrate the gitea API in C#, or when a gitea .NET SDK call errors or behaves unexpectedly. Defines the delegation contract, the agent reuse rules, and which companion skills are required reading before you write code.
---

# gitea .NET SDK — Router (map + one agent)

You (the main agent) orchestrate; the `dotnet-gitea-sdk` agent carries the SDK knowledge. The
...
- **`dotnet-gitea-sdk`** is the single agent for every SDK need. It grounds in a bundled SDK map
...
**Scope guard:** the APIMatic-generated gitea **.NET SDK** (root namespace
`GiteaApi`) in **C#/.NET projects only**. Unrelated API, or any language other than
C#/.NET — do nothing; this router and its agent do not apply.

## Workflow

**If the user opens with a reported SDK error or unexpected gitea behaviour** (not new
feature work), spawn **`dotnet-gitea-sdk`** directly with the error output and the files involved,
...
Your FIRST action is to spawn **`dotnet-gitea-sdk`** once, with the user's full request (all
features in scope — one spawn covers the whole implementation). Dictate the output path in
the brief: the absolute path where it writes the plan (`<project repo root>/gitea-dotnet-plan.md`) —
do not let it pick its own location.

It writes `gitea-dotnet-plan.md` (plan + contract sheet) and returns its path.

**Parallelize the wait — prerequisites only.** Repo reconnaissance (where the integration
lands in this codebase) is not the `dotnet-gitea-sdk` agent's job: kick it off in the SAME message
...
- Blocker or major assumption → surface it to the user in plain language, get their answer,
  send the clarification to the EXISTING `dotnet-gitea-sdk` agent (re-spawn only if it is gone). It
...
ask the warm `dotnet-gitea-sdk` agent, never guess.

### Step 2 — Implement from the contract sheet

1. Read `gitea-dotnet-plan.md` once. Treat its contracts as authoritative — do not re-derive or
...
2. Implement sequentially, following the repo's own conventions and layering. You loaded the
   ...
   contract sheet or the warm `dotnet-gitea-sdk` agent — never re-derive one from a companion.
...
4. **Any compile or runtime error involving an SDK type or member** (`CS1061`, `CS0117`,
   `CS0234`, `CS0104`, `CS1503`, `CS7036`, … on `GiteaApi.*`, or a provider error
   at runtime) → send the exact error output and the files involved to your EXISTING
   `dotnet-gitea-sdk` agent (a follow-up message, NOT a new spawn), and wait. It fixes the code in
...
A standalone gitea question with no code change: ask the warm `dotnet-gitea-sdk` agent (narrow-
question mode) and relay its grounded answer. Never answer from memory, even for "easy"
questions.

## Wait for your agent

**Never create or edit a project file while `dotnet-gitea-sdk` is running** — it edits files in
...
- **Get SDK knowledge from the `dotnet-gitea-sdk` agent, not yourself.** Don't clone or decompile the
  SDK, don't fetch its source files, and don't web-search gitea topics to find an
...
- **Don't load `dotnet-gitea-getting-started` or the SDK map pages** — the map is the agent's, and
  ...
  error accessors, and enum values come from the contract sheet (or you ask the warm `dotnet-gitea-sdk`
  agent). Where a companion points at the SDK source or a clone, that path is the agent's, not
  yours — you have no local clone.
- **Never write a gitea/SDK fact from memory** — every signature, field name, enum value, and
...
- **Don't re-derive or double-check a sheet row from memory.** If you are unsure what a row
  said, batch your questions into ONE message to the warm `dotnet-gitea-sdk` agent — it holds the map
  warm and answers in seconds.
- **Never spawn a second `dotnet-gitea-sdk` agent.** One spawn per session; everything after is a
