...
client = stub_client(stubs)
assert_raises PQAPIV2::APIException do
...
def build_test_client
  PQAPIV2::Client.new(
    environment: PQAPIV2::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error PQAPIV2::APIException
```
...
```ruby
expect { call }.to raise_error(PQAPIV2::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < PQAPIV2::HttpCallBack
...
client  = PQAPIV2::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
