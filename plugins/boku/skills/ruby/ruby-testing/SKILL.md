...
client = stub_client(stubs)
assert_raises Boku::APIException do
...
BASE = Boku::Configuration.new(environment: Boku::Environment::PRODUCTION).get_base_uri

def build_test_client
  Boku::Client.new(
    environment: Boku::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Boku::APIException
```
...
```ruby
expect { call }.to raise_error(Boku::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Boku::HttpCallBack
...
client  = Boku::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
