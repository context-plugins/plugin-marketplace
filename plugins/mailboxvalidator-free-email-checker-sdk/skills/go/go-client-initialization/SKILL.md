...
- `mailboxvalidatorfreeemailchecker` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/mailboxvalidator-free-email-checker-go` — the Go import path for the root package; `github.com/context-plugins/mailboxvalidator-free-email-checker-go/models` for request/response types.
...
```go
import "github.com/context-plugins/mailboxvalidator-free-email-checker-go"

client := mailboxvalidatorfreeemailchecker.NewClient(
    mailboxvalidatorfreeemailchecker.CreateConfiguration(
        mailboxvalidatorfreeemailchecker.WithEnvironment(mailboxvalidatorfreeemailchecker.PRODUCTION),
...
from environment variables named `MAILBOXVALIDATORFREEEMAILCHECKER_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := mailboxvalidatorfreeemailchecker.NewClient(mailboxvalidatorfreeemailchecker.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `mailboxvalidatorfreeemailchecker.PRODUCTION`,
`mailboxvalidatorfreeemailchecker.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := mailboxvalidatorfreeemailchecker.NewClient(
    mailboxvalidatorfreeemailchecker.CreateConfiguration(mailboxvalidatorfreeemailchecker.WithEnvironment(mailboxvalidatorfreeemailchecker.PRODUCTION)),
)
```
...
client := mailboxvalidatorfreeemailchecker.NewClient(
    mailboxvalidatorfreeemailchecker.CreateConfiguration(
        mailboxvalidatorfreeemailchecker.WithHttpConfiguration(
            mailboxvalidatorfreeemailchecker.CreateHttpConfiguration(
                mailboxvalidatorfreeemailchecker.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                mailboxvalidatorfreeemailchecker.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = mailboxvalidatorfreeemailchecker.NewClient(mailboxvalidatorfreeemailchecker.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api mailboxvalidatorfreeemailchecker.ClientInterface
}

func NewService(api mailboxvalidatorfreeemailchecker.ClientInterface) *Service {
