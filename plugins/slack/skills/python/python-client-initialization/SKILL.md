...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the SlackwebapiClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write SlackwebapiClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `SlackwebapiClient` is the SDK's client class — **read the real name** from `slack/slack_client.py`
...
- `slack` — the root package name (e.g. `multiauthsample`, `batester`).
- `SlackwebapiClient` — the generated client class (read it from the `class …Client` declaration in `slack/slack_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `SlackwebapiClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from slack.slack_client import SlackwebapiClient
from slack.configuration import Environment
from slack.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from slack.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `slack/slack_client.py` in the cloned source.
...
`from_environment()` is a class method on both `SlackwebapiClient` and `Configuration` that reads
...
client = SlackwebapiClient.from_environment()
...
`from_environment` in `slack/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `SlackwebapiClient`. Access them as attributes —
...
Open `slack/slack_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `slack/configuration.py`. **Read the enum in `configuration.py`
...
client = SlackwebapiClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`slack/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = SlackwebapiClient(config=new_config)
```
