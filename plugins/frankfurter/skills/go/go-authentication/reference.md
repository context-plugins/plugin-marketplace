...
```go
frankfurter.WithOAuthCCGCredentials(
    frankfurter.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr frankfurter.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := frankfurter.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr frankfurter.OAuthCCGManager) models.OAuthToken {
...
```go
creds := frankfurter.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := frankfurter.NewClient(frankfurter.CreateConfiguration(frankfurter.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    frankfurter.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := frankfurter.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := frankfurter.NewClient(frankfurter.CreateConfiguration(frankfurter.WithOAuthROPCGCredentials(creds)))
...
```go
frankfurter.WithOAuthBearerTokenCredentials(frankfurter.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
frankfurter.WithBasicAuthCredentials(frankfurter.NewBasicAuthCredentials(username, password))
frankfurter.WithApiHeaderCredentials(frankfurter.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
frankfurter.WithApiKeyCredentials(frankfurter.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FRANKFURTER_...` variables — e.g.
`FRANKFURTER_USERNAME`, `FRANKFURTER_PASSWORD`, `FRANKFURTER_TOKEN`, `FRANKFURTER_API_KEY`,
`FRANKFURTER_O_AUTH_CLIENT_ID`, `FRANKFURTER_O_AUTH_CLIENT_SECRET`, `FRANKFURTER_ACCESS_TOKEN`. Grep the
