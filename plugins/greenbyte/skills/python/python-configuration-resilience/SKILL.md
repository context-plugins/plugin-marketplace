...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `GreenbyteapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`greenbyte/configuration.py` in the cloned source.
...
```python
import requests
from greenbyte.configuration import Configuration
...
`greenbyte/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`greenbyte/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `greenbyte/configuration.py`
...
`NumberPagedResponse`) live in `greenbyte/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `greenbyte/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
