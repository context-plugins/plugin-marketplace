...
> Throughout, `3commasIo` and other `{...}` tokens are placeholders for names you take from your SDK —
...
`lib/3commas_io/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/3commas_io/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/3commas_io/http/auth/api_header.rb` and
