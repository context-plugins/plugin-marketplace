...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new PazeCheckoutAPIClient({
...
```typescript
import { {Operation}Error } from 'paze-checkout-apilib/errors';
...
```typescript
import { ApiError } from 'paze-checkout-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(PazeCheckoutAPIClient)
    .useValue(new PazeCheckoutAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
