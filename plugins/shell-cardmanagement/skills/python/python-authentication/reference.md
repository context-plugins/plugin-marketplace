...
kwargs to the `ShellcardmanagementapisClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `shellcardmanagement/shellcardmanagement_client.py` and `shellcardmanagement/http/auth/` in
the cloned source.

## Basic auth

```python
from shellcardmanagement.http.auth.basic_auth import BasicAuthCredentials
...
```python
from shellcardmanagement.http.auth.api_key import ApiKeyCredentials
...
```python
from shellcardmanagement.http.auth.api_header import ApiHeaderCredentials
...
```python
from shellcardmanagement.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from shellcardmanagement.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from shellcardmanagement.http.auth.o_auth_acg import OAuthACGCredentials
from shellcardmanagement.models.o_auth_scope_shellcardmanagement_enum import OAuthScope{Pkg}Enum
...
client = ShellcardmanagementapisClient(o_auth_acg_credentials=creds)
...
```python
from shellcardmanagement.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ShellcardmanagementapisClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ShellcardmanagementapisClient.from_environment()`) read each credential
...
Grep `from_environment` in `shellcardmanagement/configuration.py` and each `shellcardmanagement/http/auth/*.py` for the exact
