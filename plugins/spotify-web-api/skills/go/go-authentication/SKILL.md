...
> Throughout, `spotifywebapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := spotifywebapi.NewClient(
    spotifywebapi.CreateConfiguration(
        spotifywebapi.WithBasicAuthCredentials(
            spotifywebapi.NewBasicAuthCredentials(
...
spotifywebapi.WithApiKeyCredentials(spotifywebapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
spotifywebapi.WithApiHeaderCredentials(spotifywebapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
spotifywebapi.WithOAuthBearerTokenCredentials(
    spotifywebapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := spotifywebapi.NewClient(
    spotifywebapi.CreateConfiguration(
        spotifywebapi.WithOAuthCCGCredentials(
            spotifywebapi.NewOAuthCCGCredentials(
