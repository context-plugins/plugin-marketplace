...
| --- | --- |
| API | `klarna` |
| Project | `Klarna.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using Klarna.Models;` alone does **not** make enums, union types, or
...
```csharp
using Klarna;          // client + options + ServerEnvironment
using Klarna.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/klarna-csharp-sdk /tmp/klarna-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/klarna-csharp-sdk "$env:TEMP\klarna-dotnet-src"
```
...
- `KlarnaClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/klarna-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\klarna-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `KlarnaClient`, build its
   `KlarnaClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
