...
- `flightordermanagement` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/flight-order-management-go` — the Go import path for the root package; `github.com/context-plugins/flight-order-management-go/models` for request/response types.
...
```go
import "github.com/context-plugins/flight-order-management-go"

client := flightordermanagement.NewClient(
    flightordermanagement.CreateConfiguration(
        flightordermanagement.WithEnvironment(flightordermanagement.PRODUCTION),
...
from environment variables named `FLIGHTORDERMANAGEMENT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := flightordermanagement.NewClient(flightordermanagement.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `flightordermanagement.PRODUCTION`,
`flightordermanagement.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := flightordermanagement.NewClient(
    flightordermanagement.CreateConfiguration(flightordermanagement.WithEnvironment(flightordermanagement.PRODUCTION)),
)
```
...
client := flightordermanagement.NewClient(
    flightordermanagement.CreateConfiguration(
        flightordermanagement.WithHttpConfiguration(
            flightordermanagement.CreateHttpConfiguration(
                flightordermanagement.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                flightordermanagement.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = flightordermanagement.NewClient(flightordermanagement.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api flightordermanagement.ClientInterface
}

func NewService(api flightordermanagement.ClientInterface) *Service {
