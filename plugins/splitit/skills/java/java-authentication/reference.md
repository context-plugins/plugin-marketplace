...
against the `SplititClient.Builder` setters, the `com.splitit.sandbox.webapiv3.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SplititClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
