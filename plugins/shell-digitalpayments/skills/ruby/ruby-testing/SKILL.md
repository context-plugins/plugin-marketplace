...
def stub_client(stubs)
  ...
  ShellDigitalpayments::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises ShellDigitalpayments::APIException do
...
BASE = ShellDigitalpayments::Configuration.new(environment: ShellDigitalpayments::Environment::PRODUCTION).get_base_uri

def build_test_client
  ShellDigitalpayments::Client.new(
    environment: ShellDigitalpayments::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ShellDigitalpayments::APIException
```
...
```ruby
expect { call }.to raise_error(ShellDigitalpayments::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ShellDigitalpayments::HttpCallBack
...
client  = ShellDigitalpayments::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
