...
Open `tesserapiv1/tesserapiv1_client.py` for the controller property names, then the relevant
`tesserapiv1/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from tesserapiv1.models.{request_model} import {RequestModel}
...
Open `tesserapiv1/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from tesserapiv1.models.suite_code_enum import SuiteCodeEnum
...
```python
from tesserapiv1.http.http_call_back import HttpCallBack
...
client = TesserAPIV1Client(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `tesserapiv1/pagination/` in the source.

## Finding the right method

- Controller property names are on `TesserAPIV1Client` in `tesserapiv1/tesserapiv1_client.py`.
- Operation method signatures are in `tesserapiv1/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `tesserapiv1/models/`; enum types have `from_value()`.
