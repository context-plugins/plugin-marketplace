...
def stub_client(stubs)
  ...
  AdyenBalanceControlAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises AdyenBalanceControlAPI::APIException do
...
def build_test_client
  AdyenBalanceControlAPI::Client.new(
    environment: AdyenBalanceControlAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error AdyenBalanceControlAPI::APIException
```
...
```ruby
expect { call }.to raise_error(AdyenBalanceControlAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < AdyenBalanceControlAPI::HttpCallBack
...
client  = AdyenBalanceControlAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
