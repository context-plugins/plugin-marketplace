...
kwargs to the `EBayFinancesAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `ebayfinancesapi/ebayfinancesapi_client.py` and `ebayfinancesapi/http/auth/` in
the cloned source.

## Basic auth

```python
from ebayfinancesapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from ebayfinancesapi.http.auth.api_key import ApiKeyCredentials
...
```python
from ebayfinancesapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from ebayfinancesapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from ebayfinancesapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from ebayfinancesapi.http.auth.o_auth_acg import OAuthACGCredentials
from ebayfinancesapi.models.o_auth_scope_ebayfinancesapi_enum import OAuthScope{Pkg}Enum
...
client = EBayFinancesAPIClient(o_auth_acg_credentials=creds)
...
```python
from ebayfinancesapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = EBayFinancesAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `EBayFinancesAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `ebayfinancesapi/configuration.py` and each `ebayfinancesapi/http/auth/*.py` for the exact
