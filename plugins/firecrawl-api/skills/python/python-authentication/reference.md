...
kwargs to the `FirecrawlapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `firecrawlapi/firecrawlapi_client.py` and `firecrawlapi/http/auth/` in
the cloned source.

## Basic auth

```python
from firecrawlapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from firecrawlapi.http.auth.api_key import ApiKeyCredentials
...
```python
from firecrawlapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from firecrawlapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from firecrawlapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from firecrawlapi.http.auth.o_auth_acg import OAuthACGCredentials
from firecrawlapi.models.o_auth_scope_firecrawlapi_enum import OAuthScope{Pkg}Enum
...
client = FirecrawlapiClient(o_auth_acg_credentials=creds)
...
```python
from firecrawlapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FirecrawlapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FirecrawlapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `firecrawlapi/configuration.py` and each `firecrawlapi/http/auth/*.py` for the exact
