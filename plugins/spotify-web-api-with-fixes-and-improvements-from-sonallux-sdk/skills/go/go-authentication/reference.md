...
```go
spotifywebapiwithfixesandimprovementsfromsonallux.WithOAuthCCGCredentials(
    spotifywebapiwithfixesandimprovementsfromsonallux.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr spotifywebapiwithfixesandimprovementsfromsonallux.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := spotifywebapiwithfixesandimprovementsfromsonallux.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr spotifywebapiwithfixesandimprovementsfromsonallux.OAuthCCGManager) models.OAuthToken {
...
```go
creds := spotifywebapiwithfixesandimprovementsfromsonallux.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := spotifywebapiwithfixesandimprovementsfromsonallux.NewClient(spotifywebapiwithfixesandimprovementsfromsonallux.CreateConfiguration(spotifywebapiwithfixesandimprovementsfromsonallux.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    spotifywebapiwithfixesandimprovementsfromsonallux.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := spotifywebapiwithfixesandimprovementsfromsonallux.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := spotifywebapiwithfixesandimprovementsfromsonallux.NewClient(spotifywebapiwithfixesandimprovementsfromsonallux.CreateConfiguration(spotifywebapiwithfixesandimprovementsfromsonallux.WithOAuthROPCGCredentials(creds)))
...
```go
spotifywebapiwithfixesandimprovementsfromsonallux.WithOAuthBearerTokenCredentials(spotifywebapiwithfixesandimprovementsfromsonallux.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
spotifywebapiwithfixesandimprovementsfromsonallux.WithBasicAuthCredentials(spotifywebapiwithfixesandimprovementsfromsonallux.NewBasicAuthCredentials(username, password))
spotifywebapiwithfixesandimprovementsfromsonallux.WithApiHeaderCredentials(spotifywebapiwithfixesandimprovementsfromsonallux.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
spotifywebapiwithfixesandimprovementsfromsonallux.WithApiKeyCredentials(spotifywebapiwithfixesandimprovementsfromsonallux.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_...` variables — e.g.
`SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_USERNAME`, `SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_PASSWORD`, `SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_TOKEN`, `SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_API_KEY`,
`SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_O_AUTH_CLIENT_ID`, `SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_O_AUTH_CLIENT_SECRET`, `SPOTIFYWEBAPIWITHFIXESANDIMPROVEMENTSFROMSONALLUX_ACCESS_TOKEN`. Grep the
