...
client = stub_client(stubs)
assert_raises Slack::APIException do
...
BASE = Slack::Configuration.new(environment: Slack::Environment::PRODUCTION).get_base_uri

def build_test_client
  Slack::Client.new(
    environment: Slack::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Slack::APIException
```
...
```ruby
expect { call }.to raise_error(Slack::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Slack::HttpCallBack
...
client  = Slack::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
