...
`shellapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `shellapi/exceptions/` and operation docstrings.
...
```python
from shellapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `shellapi/exceptions/`.
...
```python
from shellapi.exceptions.api_exception import APIException
from shellapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`shellapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`shellapi/exceptions/` for the class.
...
`shellapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `shellapi/exceptions/`
  or a `ValueError` — check the auth handler in `shellapi/http/auth/` for the exact type.
