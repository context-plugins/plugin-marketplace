    ...
    finnhubapi.CreateConfiguration(
        finnhubapi.WithHttpConfiguration(
            finnhubapi.CreateHttpConfiguration(
                finnhubapi.WithTimeout(30),
                finnhubapi.WithRetryConfiguration(
                    finnhubapi.CreateRetryConfiguration(
                        finnhubapi.WithMaxRetryAttempts(3),
                        finnhubapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := finnhubapi.CreateRetryConfiguration(
    finnhubapi.WithMaxRetryAttempts(5),
    finnhubapi.WithRetryInterval(1),
    finnhubapi.WithBackoffFactor(2),
    finnhubapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    finnhubapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := finnhubapi.NewClient(
    finnhubapi.CreateConfiguration(
        finnhubapi.WithHttpConfiguration(
            finnhubapi.CreateHttpConfiguration(
                finnhubapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
