...
- `xeroaccountingapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `XeroAccountingAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `XeroAccountingAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from xeroaccountingapi.xeroaccountingapi_client import XeroAccountingAPIClient
from xeroaccountingapi.configuration import Environment
from xeroaccountingapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = XeroAccountingAPIClient(
...
```python
from xeroaccountingapi.configuration import Configuration, Environment
...
client = XeroAccountingAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `xeroaccountingapi/xeroaccountingapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `XeroAccountingAPIClient` and `Configuration` that reads
...
client = XeroAccountingAPIClient.from_environment()
...
`from_environment` in `xeroaccountingapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `XeroAccountingAPIClient`. Access them as attributes —
...
Open `xeroaccountingapi/xeroaccountingapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `xeroaccountingapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from xeroaccountingapi.configuration import Environment

client = XeroAccountingAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`xeroaccountingapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = XeroAccountingAPIClient(config=new_config)
```
