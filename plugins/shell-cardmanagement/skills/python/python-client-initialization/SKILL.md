...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the ShellcardmanagementapisClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write ShellcardmanagementapisClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `ShellcardmanagementapisClient` is the SDK's client class — **read the real name** from `shellcardmanagement/shellcardmanagement_client.py`
...
- `shellcardmanagement` — the root package name (e.g. `multiauthsample`, `batester`).
- `ShellcardmanagementapisClient` — the generated client class (read it from the `class …Client` declaration in `shellcardmanagement/shellcardmanagement_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `ShellcardmanagementapisClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from shellcardmanagement.shellcardmanagement_client import ShellcardmanagementapisClient
from shellcardmanagement.configuration import Environment
from shellcardmanagement.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from shellcardmanagement.configuration import Configuration, Environment
...
client = ShellcardmanagementapisClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `shellcardmanagement/shellcardmanagement_client.py` in the cloned source.
...
`from_environment()` is a class method on both `ShellcardmanagementapisClient` and `Configuration` that reads
...
`from_environment` in `shellcardmanagement/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `ShellcardmanagementapisClient`. Access them as attributes —
...
Open `shellcardmanagement/shellcardmanagement_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `shellcardmanagement/configuration.py`. **Read the enum in `configuration.py`
...
client = ShellcardmanagementapisClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`shellcardmanagement/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = ShellcardmanagementapisClient(config=new_config)
```
