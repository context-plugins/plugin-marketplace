...
against the `SplititWebApiV3Client.Builder` setters, the `com.splitit.sandbox.webapiv3.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SplititWebApiV3Client.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
SplititWebApiV3Client client = new SplititWebApiV3Client.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeSplititWebApiV3Enum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new SplititWebApiV3Client.Builder()
