...
To see which schemes a specific SDK accepts, read the credential setter methods on `APIsGuruClient.Builder`
...
- Set credentials **on the `APIsGuruClient.Builder`** before calling `.build()` — the client is immutable
