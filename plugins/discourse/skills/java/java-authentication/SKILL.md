...
To see which schemes a specific SDK accepts, read the credential setter methods on `DiscourseClient.Builder`
...
- Set credentials **on the `DiscourseClient.Builder`** before calling `.build()` — the client is immutable
