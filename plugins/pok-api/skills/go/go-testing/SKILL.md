...
`testing` + `testify/require` for reference; substitute the real `pokapi` and names.
...
import (
    ...
    "github.com/context-plugins/pok-api-go-sdk"   // root package pokapi
)
...
func clientReturning(status int, body string) (pokapi.ClientInterface, *stubTransport) {
    ...
    client := pokapi.NewClient(
        pokapi.CreateConfiguration(
            pokapi.WithHttpConfiguration(
                pokapi.CreateHttpConfiguration(pokapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/pok-api-go-sdk/errors"
...
client := pokapi.NewClient(
    pokapi.CreateConfiguration(
        pokapi.WithHttpConfiguration(
            pokapi.CreateHttpConfiguration(pokapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
