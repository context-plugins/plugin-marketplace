...
```ruby
client = Discourse::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Discourse::Client.new(adapter: :net_http)
```
...
client = Discourse::Client.new(connection: conn)
```
...
client = Discourse::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Discourse::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Discourse::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Discourse::ProxySettings.from_env
client = Discourse::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Discourse::HttpCallBack
...
client = Discourse::Client.new(http_callback: LogCallback.new)
```
...
client = Discourse::Client.from_env(timeout: 30, max_retries: 3)
```
