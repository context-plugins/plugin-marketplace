...
```ruby
client = Authentication::Client.new(
  max_retries:    3,
...
```ruby
client = Authentication::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Authentication::Client.new(adapter: :net_http)
```
...
client = Authentication::Client.new(connection: conn)
```
...
client = Authentication::Client.new(environment: Environment::PRODUCTION)
client = Authentication::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = Authentication::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Authentication::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Authentication::ProxySettings.from_env
client = Authentication::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Authentication::HttpCallBack
...
client = Authentication::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = Authentication::Client.from_env
# Overrides take precedence over env vars:
client = Authentication::Client.from_env(timeout: 30, max_retries: 3)
```
