...
- `1ForgeFinanceAPIs` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `1forge_finance_apis` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require '1forge_finance_apis'
include 1ForgeFinanceAPIs
...
The authoritative list is the `initialize` signature in `lib/1forge_finance_apis/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/1forge_finance_apis/configuration.rb`
(`build_default_config_from_env`) and `lib/1forge_finance_apis/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/1forge_finance_apis.rb`) and store it in a
