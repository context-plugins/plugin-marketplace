...
kwarg names are generated per-API — confirm from `firecrawl/firecrawl_client.py` and `firecrawl/http/auth/` in
the cloned source.

## Basic auth

```python
from firecrawl.http.auth.basic_auth import BasicAuthCredentials
...
```python
from firecrawl.http.auth.api_key import ApiKeyCredentials
...
```python
from firecrawl.http.auth.api_header import ApiHeaderCredentials
...
```python
from firecrawl.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from firecrawl.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from firecrawl.http.auth.o_auth_acg import OAuthACGCredentials
from firecrawl.models.o_auth_scope_firecrawl_enum import OAuthScope{Pkg}Enum
...
```python
from firecrawl.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `firecrawl/configuration.py` and each `firecrawl/http/auth/*.py` for the exact
