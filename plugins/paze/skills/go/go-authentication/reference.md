...
paze.WithOAuthCCGCredentials(
    paze.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr paze.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := paze.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr paze.OAuthCCGManager) models.OAuthToken {
...
```go
creds := paze.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := paze.NewClient(paze.CreateConfiguration(paze.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    paze.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := paze.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := paze.NewClient(paze.CreateConfiguration(paze.WithOAuthROPCGCredentials(creds)))
...
```go
paze.WithOAuthBearerTokenCredentials(paze.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
paze.WithBasicAuthCredentials(paze.NewBasicAuthCredentials(username, password))
paze.WithApiHeaderCredentials(paze.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
paze.WithApiKeyCredentials(paze.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `PAZE_...` variables — e.g.
`PAZE_USERNAME`, `PAZE_PASSWORD`, `PAZE_TOKEN`, `PAZE_API_KEY`,
`PAZE_O_AUTH_CLIENT_ID`, `PAZE_O_AUTH_CLIENT_SECRET`, `PAZE_ACCESS_TOKEN`. Grep the
