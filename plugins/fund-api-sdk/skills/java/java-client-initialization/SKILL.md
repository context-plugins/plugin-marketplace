...
- `FundAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`FundAPIClient.Builder` class. A minimal initialization:

```java
import com.adyen.caltest.FundAPIClient;
import com.adyen.caltest.Environment;

FundAPIClient client = new FundAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `FundAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `FundAPIClient.java` |

Call `.build()` to produce an immutable `FundAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public FundAPIClient apiClient() {
        return new FundAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired FundAPIClient client) {
        client.shutdown();
    }
}
```

Inject `FundAPIClient` into your services as a normal Spring dependency. Build only one bean.
