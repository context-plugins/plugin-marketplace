...
> `{ApiGroup}`, `EBayFinancesApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `EBayFinancesApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `EBayFinancesApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `EBayFinancesApi.Errors`
...
```csharp
using EBayFinancesApi.Core.Exceptions;     // SdkException<TError>
using EBayFinancesApi.Core.ErrorResponse;  // ApiError, RawError
using EBayFinancesApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`EBayFinancesApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using EBayFinancesApi.Core.Exceptions;     // SdkException<TError>
using EBayFinancesApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `EBayFinancesApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `EBayFinancesApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using EBayFinancesApi.Core.Models;        // ApiResult<TResponse, TError>
using EBayFinancesApi.Core.ErrorResponse; // RawError
using EBayFinancesApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `EBayFinancesApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
