...
```go
configurationapi.WithOAuthCCGCredentials(
    configurationapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr configurationapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := configurationapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr configurationapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := configurationapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := configurationapi.NewClient(configurationapi.CreateConfiguration(configurationapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    configurationapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := configurationapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := configurationapi.NewClient(configurationapi.CreateConfiguration(configurationapi.WithOAuthROPCGCredentials(creds)))
...
```go
configurationapi.WithOAuthBearerTokenCredentials(configurationapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
configurationapi.WithBasicAuthCredentials(configurationapi.NewBasicAuthCredentials(username, password))
configurationapi.WithApiHeaderCredentials(configurationapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
configurationapi.WithApiKeyCredentials(configurationapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `CONFIGURATIONAPI_...` variables — e.g.
`CONFIGURATIONAPI_USERNAME`, `CONFIGURATIONAPI_PASSWORD`, `CONFIGURATIONAPI_TOKEN`, `CONFIGURATIONAPI_API_KEY`,
`CONFIGURATIONAPI_O_AUTH_CLIENT_ID`, `CONFIGURATIONAPI_O_AUTH_CLIENT_SECRET`, `CONFIGURATIONAPI_ACCESS_TOKEN`. Grep the
