...
> `SquareClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SquareClient
{
...
```php
use SquareLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SquareLib\Exceptions\{OperationException};
use SquareLib\Exceptions\ApiException;
...
app()->instance(SquareClient::class, clientWithResponses([
