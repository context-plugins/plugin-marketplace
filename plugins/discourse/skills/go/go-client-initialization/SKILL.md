...
- `discourse` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/discourse-go-sdk` — the Go import path for the root package; `github.com/context-plugins/discourse-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/discourse-go-sdk"

client := discourse.NewClient(
    discourse.CreateConfiguration(
        discourse.WithEnvironment(discourse.PRODUCTION),
...
from environment variables named `DISCOURSE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := discourse.NewClient(discourse.CreateConfigurationFromEnvironment())
```
...
```go
client := discourse.NewClient(
    discourse.CreateConfiguration(discourse.WithEnvironment(discourse.PRODUCTION)),
)
```
...
client := discourse.NewClient(
    discourse.CreateConfiguration(
        discourse.WithHttpConfiguration(
            discourse.CreateHttpConfiguration(
                discourse.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                discourse.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = discourse.NewClient(discourse.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api discourse.ClientInterface
}

func NewService(api discourse.ClientInterface) *Service {
