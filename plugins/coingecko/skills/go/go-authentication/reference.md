...
```go
coingecko.WithOAuthCCGCredentials(
    coingecko.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr coingecko.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := coingecko.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr coingecko.OAuthCCGManager) models.OAuthToken {
...
```go
creds := coingecko.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := coingecko.NewClient(coingecko.CreateConfiguration(coingecko.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    coingecko.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := coingecko.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := coingecko.NewClient(coingecko.CreateConfiguration(coingecko.WithOAuthROPCGCredentials(creds)))
...
```go
coingecko.WithOAuthBearerTokenCredentials(coingecko.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
coingecko.WithBasicAuthCredentials(coingecko.NewBasicAuthCredentials(username, password))
coingecko.WithApiHeaderCredentials(coingecko.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
coingecko.WithApiKeyCredentials(coingecko.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `COINGECKO_...` variables — e.g.
`COINGECKO_USERNAME`, `COINGECKO_PASSWORD`, `COINGECKO_TOKEN`, `COINGECKO_API_KEY`,
`COINGECKO_O_AUTH_CLIENT_ID`, `COINGECKO_O_AUTH_CLIENT_SECRET`, `COINGECKO_ACCESS_TOKEN`. Grep the
