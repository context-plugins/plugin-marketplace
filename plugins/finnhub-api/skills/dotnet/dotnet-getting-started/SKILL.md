...
| --- | --- |
| API | `Finnhub API` |
| Project | `FinnhubApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using FinnhubApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/finnhub-api-csharp-sdk
dotnet add reference finnhub-api-csharp-sdk/FinnhubApi.csproj
```

```csharp
using FinnhubApi;          // client + options + ServerEnvironment
using FinnhubApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/finnhub-api-csharp-sdk /tmp/finnhub-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/finnhub-api-csharp-sdk "$env:TEMP\finnhub-api-dotnet-src"
```
...
- `FinnhubApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/finnhub-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\finnhub-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `FinnhubApiClient`, build its
   `FinnhubApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
