...
To see which schemes a specific SDK accepts, read the credential setter methods on `DiscourseApiDocumentationClient.Builder`
...
- Set credentials **on the `DiscourseApiDocumentationClient.Builder`** before calling `.build()` — the client is immutable
