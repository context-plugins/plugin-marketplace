...
kwarg names are generated per-API — confirm from `splitit/splitit_client.py` and `splitit/http/auth/` in
the cloned source.

## Basic auth

```python
from splitit.http.auth.basic_auth import BasicAuthCredentials
...
```python
from splitit.http.auth.api_key import ApiKeyCredentials
...
```python
from splitit.http.auth.api_header import ApiHeaderCredentials
...
```python
from splitit.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from splitit.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from splitit.http.auth.o_auth_acg import OAuthACGCredentials
from splitit.models.o_auth_scope_splitit_enum import OAuthScope{Pkg}Enum
...
```python
from splitit.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `splitit/configuration.py` and each `splitit/http/auth/*.py` for the exact
