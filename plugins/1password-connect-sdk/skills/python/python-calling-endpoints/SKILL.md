...
Open `1passwordconnect/1passwordconnect_client.py` for the controller property names, then the relevant
`1passwordconnect/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from 1passwordconnect.models.{request_model} import {RequestModel}
...
Open `1passwordconnect/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from 1passwordconnect.models.suite_code_enum import SuiteCodeEnum
...
```python
from 1passwordconnect.http.http_call_back import HttpCallBack
...
client = 1PasswordConnectClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `1passwordconnect/pagination/` in the source.

## Finding the right method

- Controller property names are on `1PasswordConnectClient` in `1passwordconnect/1passwordconnect_client.py`.
- Operation method signatures are in `1passwordconnect/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `1passwordconnect/models/`; enum types have `from_value()`.
