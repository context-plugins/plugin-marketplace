...
```go
hotelratings.WithOAuthCCGCredentials(
    hotelratings.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr hotelratings.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := hotelratings.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr hotelratings.OAuthCCGManager) models.OAuthToken {
...
```go
creds := hotelratings.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := hotelratings.NewClient(hotelratings.CreateConfiguration(hotelratings.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    hotelratings.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := hotelratings.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := hotelratings.NewClient(hotelratings.CreateConfiguration(hotelratings.WithOAuthROPCGCredentials(creds)))
...
```go
hotelratings.WithOAuthBearerTokenCredentials(hotelratings.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
hotelratings.WithBasicAuthCredentials(hotelratings.NewBasicAuthCredentials(username, password))
hotelratings.WithApiHeaderCredentials(hotelratings.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
hotelratings.WithApiKeyCredentials(hotelratings.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `HOTELRATINGS_...` variables — e.g.
`HOTELRATINGS_USERNAME`, `HOTELRATINGS_PASSWORD`, `HOTELRATINGS_TOKEN`, `HOTELRATINGS_API_KEY`,
`HOTELRATINGS_O_AUTH_CLIENT_ID`, `HOTELRATINGS_O_AUTH_CLIENT_SECRET`, `HOTELRATINGS_ACCESS_TOKEN`. Grep the
