...
> `PokapiClient` is the SDK's client class — **read the real name** from `pokapi/pokapi_client.py`
...
- `pokapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `PokapiClient` — the generated client class (read it from the `class …Client` declaration in `pokapi/pokapi_client.py`).
...
```python
from pokapi.pokapi_client import PokapiClient
from pokapi.configuration import Environment
from pokapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from pokapi.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `pokapi/pokapi_client.py` in the cloned source.
...
`from_environment` in `pokapi/configuration.py` for the exact names.
...
Open `pokapi/pokapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `pokapi/configuration.py`. **Read the enum in `configuration.py`
...
`pokapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
