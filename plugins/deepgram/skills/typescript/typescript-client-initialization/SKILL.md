...
- `deepgram` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'deepgram';
...
```ts
import { {Resource}Api } from 'deepgram';
