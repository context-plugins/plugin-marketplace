...
kwargs to the `TelegramBotAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `telegrambotapi/telegrambotapi_client.py` and `telegrambotapi/http/auth/` in
the cloned source.

## Basic auth

```python
from telegrambotapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from telegrambotapi.http.auth.api_key import ApiKeyCredentials
...
```python
from telegrambotapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from telegrambotapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from telegrambotapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from telegrambotapi.http.auth.o_auth_acg import OAuthACGCredentials
from telegrambotapi.models.o_auth_scope_telegrambotapi_enum import OAuthScope{Pkg}Enum
...
client = TelegramBotAPIClient(o_auth_acg_credentials=creds)
...
```python
from telegrambotapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = TelegramBotAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `TelegramBotAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `telegrambotapi/configuration.py` and each `telegrambotapi/http/auth/*.py` for the exact
