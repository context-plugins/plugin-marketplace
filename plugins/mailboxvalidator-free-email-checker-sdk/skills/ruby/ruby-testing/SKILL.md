...
def stub_client(stubs)
  ...
  MailboxValidatorFreeEmailChecker::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises MailboxValidatorFreeEmailChecker::APIException do
...
def build_test_client
  MailboxValidatorFreeEmailChecker::Client.new(
    environment: MailboxValidatorFreeEmailChecker::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error MailboxValidatorFreeEmailChecker::APIException
```
...
```ruby
# Base APIException:
rescue MailboxValidatorFreeEmailChecker::APIException => e
  assert_equal 404, e.response_code
  assert_includes e.reason, 'Not Found'

# Typed subclass (e.g. OAuthProviderException):
rescue MailboxValidatorFreeEmailChecker::OAuthProviderException => e
...
```ruby
expect { call }.to raise_error(MailboxValidatorFreeEmailChecker::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < MailboxValidatorFreeEmailChecker::HttpCallBack
...
client  = MailboxValidatorFreeEmailChecker::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
