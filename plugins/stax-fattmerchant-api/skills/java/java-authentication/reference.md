...
against the `StaxFattMerchantApiClient.Builder` setters, the `com.fattlabs.apiprod.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `StaxFattMerchantApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
StaxFattMerchantApiClient client = new StaxFattMerchantApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeStaxFattMerchantAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new StaxFattMerchantApiClient.Builder()
