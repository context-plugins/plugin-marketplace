...
`azurecognitive/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `azurecognitive/exceptions/` and operation docstrings.
...
```python
from azurecognitive.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `azurecognitive/exceptions/`.
...
```python
from azurecognitive.exceptions.api_exception import APIException
from azurecognitive.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`azurecognitive/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`azurecognitive/exceptions/` for the class.
...
`azurecognitive/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `azurecognitive/exceptions/`
  or a `ValueError` — check the auth handler in `azurecognitive/http/auth/` for the exact type.
