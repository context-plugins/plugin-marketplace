...
All types below live under `ApiManagementClient.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using ApiManagementClient.Core.Configuration;
...
```csharp
using ApiManagementClient.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new ApiManagementClientClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `ApiManagementClient.Core.Exceptions`):
...
With DI, the SDK's `AddApiManagementClientClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddApiManagementClientClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
