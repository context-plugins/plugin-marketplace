...
kwargs to the `GeoapifyapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `geoapify/geoapify_client.py` and `geoapify/http/auth/` in
the cloned source.

## Basic auth

```python
from geoapify.http.auth.basic_auth import BasicAuthCredentials
...
```python
from geoapify.http.auth.api_key import ApiKeyCredentials
...
```python
from geoapify.http.auth.api_header import ApiHeaderCredentials
...
```python
from geoapify.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from geoapify.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from geoapify.http.auth.o_auth_acg import OAuthACGCredentials
from geoapify.models.o_auth_scope_geoapify_enum import OAuthScope{Pkg}Enum
...
client = GeoapifyapiClient(o_auth_acg_credentials=creds)
...
```python
from geoapify.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = GeoapifyapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GeoapifyapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `geoapify/configuration.py` and each `geoapify/http/auth/*.py` for the exact
