...
- `paypal-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'paypal-apilib';
...
```ts
import { {Resource}Controller } from 'paypal-apilib';
