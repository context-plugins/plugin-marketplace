...
`discourseapidocumentation/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `discourseapidocumentation/exceptions/` and operation docstrings.
...
```python
from discourseapidocumentation.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `discourseapidocumentation/exceptions/`.
...
```python
from discourseapidocumentation.exceptions.api_exception import APIException
from discourseapidocumentation.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`discourseapidocumentation/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`discourseapidocumentation/exceptions/` for the class.
...
`discourseapidocumentation/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `discourseapidocumentation/exceptions/`
  or a `ValueError` — check the auth handler in `discourseapidocumentation/http/auth/` for the exact type.
