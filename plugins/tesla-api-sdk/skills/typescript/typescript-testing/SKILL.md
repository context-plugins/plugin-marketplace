...
```typescript
import { {Operation}Error } from 'tesla-apilib/errors';
...
```typescript
import { ApiError } from 'tesla-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(TeslaAPIClient)
    .useValue(new TeslaAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
