...
> Throughout, `subscriptionclient` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := subscriptionclient.NewClient(
    subscriptionclient.CreateConfiguration(
        subscriptionclient.WithBasicAuthCredentials(
            subscriptionclient.NewBasicAuthCredentials(
...
subscriptionclient.WithApiKeyCredentials(subscriptionclient.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
subscriptionclient.WithApiHeaderCredentials(subscriptionclient.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
subscriptionclient.WithOAuthBearerTokenCredentials(
    subscriptionclient.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := subscriptionclient.NewClient(
    subscriptionclient.CreateConfiguration(
        subscriptionclient.WithOAuthCCGCredentials(
            subscriptionclient.NewOAuthCCGCredentials(
