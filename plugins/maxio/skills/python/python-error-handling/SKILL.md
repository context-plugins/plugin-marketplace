...
`maxio/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `maxio/exceptions/` and operation docstrings.
...
```python
from maxio.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `maxio/exceptions/`.
...
```python
from maxio.exceptions.api_exception import APIException
from maxio.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`maxio/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`maxio/exceptions/` for the class.
...
`maxio/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `maxio/exceptions/`
  or a `ValueError` — check the auth handler in `maxio/http/auth/` for the exact type.
