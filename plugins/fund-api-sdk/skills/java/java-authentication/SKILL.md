...
To see which schemes a specific SDK accepts, read the credential setter methods on `FundAPIClient.Builder`
...
- Set credentials **on the `FundAPIClient.Builder`** before calling `.build()` — the client is immutable
