...
against the `MindbodyClient.Builder` setters, the `com.mindbodyonline.pushapi.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `MindbodyClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
MindbodyClient client = new MindbodyClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeMindbodyEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
