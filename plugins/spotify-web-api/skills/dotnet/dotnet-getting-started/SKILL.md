...
| --- | --- |
| API | `Spotify Web API` |
| Project | `SpotifyWebApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using SpotifyWebApi.Models;` alone does **not** make enums, union types, or
...
```csharp
using SpotifyWebApi;          // client + options + ServerEnvironment
using SpotifyWebApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/spotify-web-api-csharp-sdk /tmp/spotify-web-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/spotify-web-api-csharp-sdk "$env:TEMP\spotify-web-api-dotnet-src"
```
...
- `SpotifyWebApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/spotify-web-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\spotify-web-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `SpotifyWebApiClient`, build its
   `SpotifyWebApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
