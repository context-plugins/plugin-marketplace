...
against the `GoeapiClient.Builder` setters, the `com.frk.uip.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GoeapiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
GoeapiClient client = new GoeapiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeGOEEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
