...
client = stub_client(stubs)
assert_raises Payquicker::APIException do
...
BASE = Payquicker::Configuration.new(environment: Payquicker::Environment::PRODUCTION).get_base_uri

def build_test_client
  Payquicker::Client.new(
    environment: Payquicker::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Payquicker::APIException
```
...
```ruby
expect { call }.to raise_error(Payquicker::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Payquicker::HttpCallBack
...
client  = Payquicker::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
