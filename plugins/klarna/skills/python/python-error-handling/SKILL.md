...
`klarna/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `klarna/exceptions/` and operation docstrings.
...
```python
from klarna.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `klarna/exceptions/`.
...
```python
from klarna.exceptions.api_exception import APIException
from klarna.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`klarna/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`klarna/exceptions/` for the class.
...
`klarna/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `klarna/exceptions/`
  or a `ValueError` — check the auth handler in `klarna/http/auth/` for the exact type.
