...
```go
firecrawl.WithOAuthCCGCredentials(
    firecrawl.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr firecrawl.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := firecrawl.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr firecrawl.OAuthCCGManager) models.OAuthToken {
...
```go
creds := firecrawl.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := firecrawl.NewClient(firecrawl.CreateConfiguration(firecrawl.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    firecrawl.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := firecrawl.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := firecrawl.NewClient(firecrawl.CreateConfiguration(firecrawl.WithOAuthROPCGCredentials(creds)))
...
```go
firecrawl.WithOAuthBearerTokenCredentials(firecrawl.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
firecrawl.WithBasicAuthCredentials(firecrawl.NewBasicAuthCredentials(username, password))
firecrawl.WithApiHeaderCredentials(firecrawl.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
firecrawl.WithApiKeyCredentials(firecrawl.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FIRECRAWL_...` variables — e.g.
`FIRECRAWL_USERNAME`, `FIRECRAWL_PASSWORD`, `FIRECRAWL_TOKEN`, `FIRECRAWL_API_KEY`,
`FIRECRAWL_O_AUTH_CLIENT_ID`, `FIRECRAWL_O_AUTH_CLIENT_SECRET`, `FIRECRAWL_ACCESS_TOKEN`. Grep the
