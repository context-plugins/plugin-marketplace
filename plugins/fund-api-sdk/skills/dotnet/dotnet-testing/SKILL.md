...
`FundApiClient`/`FundApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `FundApiClient`,
...
static FundApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new FundApiClient(new HttpClient(handler), new FundApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using FundApi.Errors;              // {Operation}Error types
...
```csharp
using FundApi.Core.Exceptions;
using FundApi.Core.ErrorResponse;
...
using FundApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new FundApiClient(new HttpClient(handler), new FundApiClientOptions());
...
- For DI-based code, the SDK's `AddFundApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `FundApiClient` from the provider:
  ```csharp
  services.AddFundApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<FundApiClient>();
  ```
