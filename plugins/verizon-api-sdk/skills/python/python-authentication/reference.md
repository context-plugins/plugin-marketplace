...
kwargs to the `VerizonAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `verizonapi/verizonapi_client.py` and `verizonapi/http/auth/` in
the cloned source.

## Basic auth

```python
from verizonapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from verizonapi.http.auth.api_key import ApiKeyCredentials
...
```python
from verizonapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from verizonapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from verizonapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from verizonapi.http.auth.o_auth_acg import OAuthACGCredentials
from verizonapi.models.o_auth_scope_verizonapi_enum import OAuthScope{Pkg}Enum
...
client = VerizonAPIClient(o_auth_acg_credentials=creds)
...
```python
from verizonapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = VerizonAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `VerizonAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `verizonapi/configuration.py` and each `verizonapi/http/auth/*.py` for the exact
