...
To see which schemes a specific SDK accepts, read the credential setter methods on `GiteaApiClient.Builder`
...
- Set credentials **on the `GiteaApiClient.Builder`** before calling `.build()` — the client is immutable
