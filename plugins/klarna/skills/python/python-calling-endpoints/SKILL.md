...
Open `klarna/klarna_client.py` for the controller property names, then the relevant
`klarna/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from klarna.models.{request_model} import {RequestModel}
...
Open `klarna/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from klarna.models.suite_code_enum import SuiteCodeEnum
...
```python
from klarna.http.http_call_back import HttpCallBack
...
client = KlarnaClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `klarna/pagination/` in the source.

## Finding the right method

- Controller property names are on `KlarnaClient` in `klarna/klarna_client.py`.
- Operation method signatures are in `klarna/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `klarna/models/`; enum types have `from_value()`.
