...
- `paypal` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/paypal-go-sdk` — the Go import path for the root package; `github.com/context-plugins/paypal-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/paypal-go-sdk"

client := paypal.NewClient(
    paypal.CreateConfiguration(
        paypal.WithEnvironment(paypal.PRODUCTION),
...
from environment variables named `PAYPAL_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := paypal.NewClient(paypal.CreateConfigurationFromEnvironment())
```
...
```go
client := paypal.NewClient(
    paypal.CreateConfiguration(paypal.WithEnvironment(paypal.PRODUCTION)),
)
```
...
client := paypal.NewClient(
    paypal.CreateConfiguration(
        paypal.WithHttpConfiguration(
            paypal.CreateHttpConfiguration(
                paypal.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                paypal.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = paypal.NewClient(paypal.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api paypal.ClientInterface
}

func NewService(api paypal.ClientInterface) *Service {
