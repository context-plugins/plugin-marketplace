...
> `{operation}`, `{apiGroup}`, `ShellCardManagementAPIsLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `ShellCardManagementAPIsLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `ShellCardManagementAPIsLib\Exceptions\` — read the source to
