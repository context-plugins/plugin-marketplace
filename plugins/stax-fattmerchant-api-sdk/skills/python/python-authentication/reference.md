...
kwargs to the `StaxFattMerchantAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `staxfattmerchantapi/staxfattmerchantapi_client.py` and `staxfattmerchantapi/http/auth/` in
the cloned source.
...
```python
from staxfattmerchantapi.http.auth.o_auth_acg import OAuthACGCredentials
from staxfattmerchantapi.models.o_auth_scope_staxfattmerchantapi_enum import OAuthScope{Pkg}Enum
...
client = StaxFattMerchantAPIClient(o_auth_acg_credentials=creds)
...
```python
from staxfattmerchantapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = StaxFattMerchantAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `StaxFattMerchantAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `staxfattmerchantapi/configuration.py` and each `staxfattmerchantapi/http/auth/*.py` for the exact
