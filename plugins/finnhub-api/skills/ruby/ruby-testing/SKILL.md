...
client = stub_client(stubs)
assert_raises FinnhubAPI::APIException do
...
BASE = FinnhubAPI::Configuration.new(environment: FinnhubAPI::Environment::PRODUCTION).get_base_uri

def build_test_client
  FinnhubAPI::Client.new(
    environment: FinnhubAPI::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error FinnhubAPI::APIException
```
...
```ruby
expect { call }.to raise_error(FinnhubAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < FinnhubAPI::HttpCallBack
...
client  = FinnhubAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
