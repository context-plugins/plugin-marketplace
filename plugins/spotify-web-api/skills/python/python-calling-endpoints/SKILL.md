...
Open `spotifywebapi/spotifywebapi_client.py` for the controller property names, then the relevant
`spotifywebapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from spotifywebapi.models.{request_model} import {RequestModel}
...
Open `spotifywebapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from spotifywebapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from spotifywebapi.http.http_call_back import HttpCallBack
...
client = SpotifywebapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `spotifywebapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `SpotifywebapiClient` in `spotifywebapi/spotifywebapi_client.py`.
- Operation method signatures are in `spotifywebapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `spotifywebapi/models/`; enum types have `from_value()`.
