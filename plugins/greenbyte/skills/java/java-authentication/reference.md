...
against the `GreenbyteClient.Builder` setters, the `cloud.greenbyte.intro.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GreenbyteClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
