...
Open `ebaysell/ebaysell_client.py` for the controller property names, then the relevant
`ebaysell/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from ebaysell.models.{request_model} import {RequestModel}
...
Open `ebaysell/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from ebaysell.models.suite_code_enum import SuiteCodeEnum
...
```python
from ebaysell.http.http_call_back import HttpCallBack
...
client = EbaysellClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `ebaysell/pagination/` in the source.

## Finding the right method

- Controller property names are on `EbaysellClient` in `ebaysell/ebaysell_client.py`.
- Operation method signatures are in `ebaysell/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `ebaysell/models/`; enum types have `from_value()`.
