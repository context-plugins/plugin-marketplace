...
All types below live under `StaxFattMerchantApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using StaxFattMerchantApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new StaxFattMerchantApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `StaxFattMerchantApi.Core.Exceptions`):
...
With DI, the SDK's `AddStaxFattMerchantApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddStaxFattMerchantApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
