...
> `AWSMigrationHubClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AWSMigrationHubClient
{
...
```php
use AWSMigrationHubLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AWSMigrationHubLib\Exceptions\{OperationException};
use AWSMigrationHubLib\Exceptions\ApiException;
...
app()->instance(AWSMigrationHubClient::class, clientWithResponses([
