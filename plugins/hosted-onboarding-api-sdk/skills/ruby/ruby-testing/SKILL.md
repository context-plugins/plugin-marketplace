...
def stub_client(stubs)
  ...
  HostedOnboardingAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises HostedOnboardingAPI::APIException do
...
def build_test_client
  HostedOnboardingAPI::Client.new(
    environment: HostedOnboardingAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error HostedOnboardingAPI::APIException
```
...
```ruby
expect { call }.to raise_error(HostedOnboardingAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < HostedOnboardingAPI::HttpCallBack
...
client  = HostedOnboardingAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
