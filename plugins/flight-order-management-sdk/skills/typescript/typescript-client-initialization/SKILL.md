...
- `flight-order-managementlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'flight-order-managementlib';
...
```ts
import { {Resource}Controller } from 'flight-order-managementlib';
