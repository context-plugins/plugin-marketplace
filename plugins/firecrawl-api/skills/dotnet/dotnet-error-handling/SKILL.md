...
> `{ApiGroup}`, `FirecrawlApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FirecrawlApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `FirecrawlApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FirecrawlApi.Errors`
...
using FirecrawlApi.Core.ErrorResponse;  // ApiError, RawError
using FirecrawlApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FirecrawlApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
Case B needs no `.Errors` using — `RawError` lives under `FirecrawlApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `FirecrawlApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FirecrawlApi.Core.Models;        // ApiResult<TResponse, TError>
using FirecrawlApi.Core.ErrorResponse; // RawError
using FirecrawlApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FirecrawlApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
