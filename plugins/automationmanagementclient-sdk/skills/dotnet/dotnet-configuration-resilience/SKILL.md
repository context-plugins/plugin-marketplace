...
All types below live under `AutomationManagementClient.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using AutomationManagementClient.Core.Configuration;
...
```csharp
using AutomationManagementClient.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new AutomationManagementClientClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `AutomationManagementClient.Core.Exceptions`):
...
With DI, the SDK's `AddAutomationManagementClientClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAutomationManagementClientClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
