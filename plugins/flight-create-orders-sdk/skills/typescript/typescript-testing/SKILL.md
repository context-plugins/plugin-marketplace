...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new FlightCreateOrdersClient({
...
```typescript
import { {Operation}Error } from 'flight-create-orderslib/errors';
...
```typescript
import { ApiError } from 'flight-create-orderslib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(FlightCreateOrdersClient)
    .useValue(new FlightCreateOrdersClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
