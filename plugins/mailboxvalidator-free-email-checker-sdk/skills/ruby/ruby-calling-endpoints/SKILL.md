...
Open `lib/mailboxvalidator_free_email_checker/client.rb` for the accessor names, then the relevant
`lib/mailboxvalidator_free_email_checker/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = MailboxValidatorFreeEmailChecker::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: MailboxValidatorFreeEmailChecker::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/mailboxvalidator_free_email_checker/models/`. Field names are defined as
...
Read the enum module in `lib/mailboxvalidator_free_email_checker/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/mailboxvalidator_free_email_checker/client.rb`.
- Operation methods are in `lib/mailboxvalidator_free_email_checker/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/mailboxvalidator_free_email_checker/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/mailboxvalidator_free_email_checker/exceptions/`.
