...
> Throughout, `tesserapiv1` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := tesserapiv1.NewClient(
    tesserapiv1.CreateConfiguration(
        tesserapiv1.WithBasicAuthCredentials(
            tesserapiv1.NewBasicAuthCredentials(
...
tesserapiv1.WithApiKeyCredentials(tesserapiv1.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
tesserapiv1.WithApiHeaderCredentials(tesserapiv1.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
tesserapiv1.WithOAuthBearerTokenCredentials(
    tesserapiv1.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := tesserapiv1.NewClient(
    tesserapiv1.CreateConfiguration(
        tesserapiv1.WithOAuthCCGCredentials(
            tesserapiv1.NewOAuthCCGCredentials(
