...
To see which schemes a specific SDK accepts, read the credential setter methods on `ShellSmartPayAPIClient.Builder`
...
- Set credentials **on the `ShellSmartPayAPIClient.Builder`** before calling `.build()` — the client is immutable
