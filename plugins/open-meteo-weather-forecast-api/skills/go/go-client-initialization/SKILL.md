...
- `openmeteoweatherforecastapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/open-meteo-weather-forecast-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/open-meteo-weather-forecast-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/open-meteo-weather-forecast-api-go-sdk"

client := openmeteoweatherforecastapi.NewClient(
    openmeteoweatherforecastapi.CreateConfiguration(
        openmeteoweatherforecastapi.WithEnvironment(openmeteoweatherforecastapi.PRODUCTION),
...
from environment variables named `OPENMETEOWEATHERFORECASTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := openmeteoweatherforecastapi.NewClient(openmeteoweatherforecastapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `openmeteoweatherforecastapi.PRODUCTION`,
`openmeteoweatherforecastapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := openmeteoweatherforecastapi.NewClient(
    openmeteoweatherforecastapi.CreateConfiguration(openmeteoweatherforecastapi.WithEnvironment(openmeteoweatherforecastapi.PRODUCTION)),
)
```
...
client := openmeteoweatherforecastapi.NewClient(
    openmeteoweatherforecastapi.CreateConfiguration(
        openmeteoweatherforecastapi.WithHttpConfiguration(
            openmeteoweatherforecastapi.CreateHttpConfiguration(
                openmeteoweatherforecastapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                openmeteoweatherforecastapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = openmeteoweatherforecastapi.NewClient(openmeteoweatherforecastapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api openmeteoweatherforecastapi.ClientInterface
}

func NewService(api openmeteoweatherforecastapi.ClientInterface) *Service {
