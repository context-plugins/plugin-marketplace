...
```ruby
client = Slack::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Slack::Client.new(adapter: :net_http)
```
...
client = Slack::Client.new(connection: conn)
```
...
client = Slack::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Slack::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Slack::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Slack::ProxySettings.from_env
client = Slack::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Slack::HttpCallBack
...
client = Slack::Client.new(http_callback: LogCallback.new)
```
...
client = Slack::Client.from_env(timeout: 30, max_retries: 3)
```
