...
```go
networkadminmanagementclient.WithOAuthCCGCredentials(
    networkadminmanagementclient.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr networkadminmanagementclient.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := networkadminmanagementclient.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr networkadminmanagementclient.OAuthCCGManager) models.OAuthToken {
...
```go
creds := networkadminmanagementclient.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := networkadminmanagementclient.NewClient(networkadminmanagementclient.CreateConfiguration(networkadminmanagementclient.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    networkadminmanagementclient.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := networkadminmanagementclient.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := networkadminmanagementclient.NewClient(networkadminmanagementclient.CreateConfiguration(networkadminmanagementclient.WithOAuthROPCGCredentials(creds)))
...
```go
networkadminmanagementclient.WithOAuthBearerTokenCredentials(networkadminmanagementclient.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
networkadminmanagementclient.WithBasicAuthCredentials(networkadminmanagementclient.NewBasicAuthCredentials(username, password))
networkadminmanagementclient.WithApiHeaderCredentials(networkadminmanagementclient.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
networkadminmanagementclient.WithApiKeyCredentials(networkadminmanagementclient.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `NETWORKADMINMANAGEMENTCLIENT_...` variables — e.g.
`NETWORKADMINMANAGEMENTCLIENT_USERNAME`, `NETWORKADMINMANAGEMENTCLIENT_PASSWORD`, `NETWORKADMINMANAGEMENTCLIENT_TOKEN`, `NETWORKADMINMANAGEMENTCLIENT_API_KEY`,
`NETWORKADMINMANAGEMENTCLIENT_O_AUTH_CLIENT_ID`, `NETWORKADMINMANAGEMENTCLIENT_O_AUTH_CLIENT_SECRET`, `NETWORKADMINMANAGEMENTCLIENT_ACCESS_TOKEN`. Grep the
