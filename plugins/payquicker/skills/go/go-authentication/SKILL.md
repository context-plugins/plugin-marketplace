...
> Throughout, `payquicker` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := payquicker.NewClient(
    payquicker.CreateConfiguration(
        payquicker.WithBasicAuthCredentials(
            payquicker.NewBasicAuthCredentials(
...
payquicker.WithApiKeyCredentials(payquicker.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
payquicker.WithApiHeaderCredentials(payquicker.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
payquicker.WithOAuthBearerTokenCredentials(
    payquicker.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := payquicker.NewClient(
    payquicker.CreateConfiguration(
        payquicker.WithOAuthCCGCredentials(
            payquicker.NewOAuthCCGCredentials(
