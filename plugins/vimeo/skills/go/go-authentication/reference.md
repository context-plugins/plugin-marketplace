...
```go
vimeoapi.WithOAuthCCGCredentials(
    vimeoapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr vimeoapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := vimeoapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr vimeoapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := vimeoapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := vimeoapi.NewClient(vimeoapi.CreateConfiguration(vimeoapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    vimeoapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := vimeoapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := vimeoapi.NewClient(vimeoapi.CreateConfiguration(vimeoapi.WithOAuthROPCGCredentials(creds)))
...
```go
vimeoapi.WithOAuthBearerTokenCredentials(vimeoapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
vimeoapi.WithBasicAuthCredentials(vimeoapi.NewBasicAuthCredentials(username, password))
vimeoapi.WithApiHeaderCredentials(vimeoapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
vimeoapi.WithApiKeyCredentials(vimeoapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `VIMEOAPI_...` variables — e.g.
`VIMEOAPI_USERNAME`, `VIMEOAPI_PASSWORD`, `VIMEOAPI_TOKEN`, `VIMEOAPI_API_KEY`,
`VIMEOAPI_O_AUTH_CLIENT_ID`, `VIMEOAPI_O_AUTH_CLIENT_SECRET`, `VIMEOAPI_ACCESS_TOKEN`. Grep the
