...
> Throughout, `adobeexperiencemanageraemapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := adobeexperiencemanageraemapi.NewClient(
    adobeexperiencemanageraemapi.CreateConfiguration(
        adobeexperiencemanageraemapi.WithBasicAuthCredentials(
            adobeexperiencemanageraemapi.NewBasicAuthCredentials(
...
adobeexperiencemanageraemapi.WithApiKeyCredentials(adobeexperiencemanageraemapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
adobeexperiencemanageraemapi.WithApiHeaderCredentials(adobeexperiencemanageraemapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
adobeexperiencemanageraemapi.WithOAuthBearerTokenCredentials(
    adobeexperiencemanageraemapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := adobeexperiencemanageraemapi.NewClient(
    adobeexperiencemanageraemapi.CreateConfiguration(
        adobeexperiencemanageraemapi.WithOAuthCCGCredentials(
            adobeexperiencemanageraemapi.NewOAuthCCGCredentials(
