...
Open `mailboxvalidatorfreeemailchecker/mailboxvalidatorfreeemailchecker_client.py` for the controller property names, then the relevant
`mailboxvalidatorfreeemailchecker/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from mailboxvalidatorfreeemailchecker.models.{request_model} import {RequestModel}
...
Open `mailboxvalidatorfreeemailchecker/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from mailboxvalidatorfreeemailchecker.models.suite_code_enum import SuiteCodeEnum
...
```python
from mailboxvalidatorfreeemailchecker.http.http_call_back import HttpCallBack
...
client = MailboxValidatorFreeEmailCheckerClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `mailboxvalidatorfreeemailchecker/pagination/` in the source.

## Finding the right method

- Controller property names are on `MailboxValidatorFreeEmailCheckerClient` in `mailboxvalidatorfreeemailchecker/mailboxvalidatorfreeemailchecker_client.py`.
- Operation method signatures are in `mailboxvalidatorfreeemailchecker/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `mailboxvalidatorfreeemailchecker/models/`; enum types have `from_value()`.
