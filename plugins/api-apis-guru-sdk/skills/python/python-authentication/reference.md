...
kwargs to the `ApiApisGuruClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `apiapisguru/apiapisguru_client.py` and `apiapisguru/http/auth/` in
the cloned source.

## Basic auth

```python
from apiapisguru.http.auth.basic_auth import BasicAuthCredentials
...
```python
from apiapisguru.http.auth.api_key import ApiKeyCredentials
...
```python
from apiapisguru.http.auth.api_header import ApiHeaderCredentials
...
```python
from apiapisguru.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from apiapisguru.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from apiapisguru.http.auth.o_auth_acg import OAuthACGCredentials
from apiapisguru.models.o_auth_scope_apiapisguru_enum import OAuthScope{Pkg}Enum
...
client = ApiApisGuruClient(o_auth_acg_credentials=creds)
...
```python
from apiapisguru.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ApiApisGuruClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ApiApisGuruClient.from_environment()`) read each credential
...
Grep `from_environment` in `apiapisguru/configuration.py` and each `apiapisguru/http/auth/*.py` for the exact
