...
kwargs to the `FlightOffersPriceClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `flightoffersprice/flightoffersprice_client.py` and `flightoffersprice/http/auth/` in
the cloned source.

## Basic auth

```python
from flightoffersprice.http.auth.basic_auth import BasicAuthCredentials
...
```python
from flightoffersprice.http.auth.api_key import ApiKeyCredentials
...
```python
from flightoffersprice.http.auth.api_header import ApiHeaderCredentials
...
```python
from flightoffersprice.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from flightoffersprice.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from flightoffersprice.http.auth.o_auth_acg import OAuthACGCredentials
from flightoffersprice.models.o_auth_scope_flightoffersprice_enum import OAuthScope{Pkg}Enum
...
client = FlightOffersPriceClient(o_auth_acg_credentials=creds)
...
```python
from flightoffersprice.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FlightOffersPriceClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FlightOffersPriceClient.from_environment()`) read each credential
...
Grep `from_environment` in `flightoffersprice/configuration.py` and each `flightoffersprice/http/auth/*.py` for the exact
