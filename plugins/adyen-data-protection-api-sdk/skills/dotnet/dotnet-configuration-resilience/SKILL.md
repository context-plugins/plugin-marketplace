...
All types below live under `AdyenDataProtectionApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using AdyenDataProtectionApi.Core.Configuration;
...
```csharp
using AdyenDataProtectionApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new AdyenDataProtectionApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `AdyenDataProtectionApi.Core.Exceptions`):
...
With DI, the SDK's `AddAdyenDataProtectionApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAdyenDataProtectionApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
