...
| Install | `gem 'vimeo', git: 'https://github.com/context-plugins/vimeo-ruby-sdk', branch: 'main'` in a Gemfile |
...
- `lib/vimeo/controllers/` — one `{Resource}Api` per API resource group, each extending `BaseController`. **This is where operation method signatures and required vs. optional params live.**
...
```ruby
# In your Gemfile:
gem 'vimeo', git: 'https://github.com/context-plugins/vimeo-ruby-sdk', branch: 'main'
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-ruby-sdk /tmp/vimeo-ruby-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-ruby-sdk "$env:TEMP\vimeo-ruby-src"
```
