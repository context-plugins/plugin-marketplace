...
| Install | `gem 'splitit_web_api_v3', git: 'https://github.com/context-plugins/splitit-web-api-v3-ruby', branch: 'main'` in a Gemfile |
| Client class | `SplititWebApiV3::Client` — one class; built with `Client.new(keyword_args...)` or `Client.from_env` |
| Configuration class | `SplititWebApiV3::Configuration` — extends `CoreLibrary::HttpClientConfiguration`; holds `Environment` and `Server` inner classes |
...
| Base exception | `SplititWebApiV3::APIException` (extends `CoreLibrary::ApiException`) |
...
APIMatic Ruby SDKs place everything under `lib/splitit_web_api_v3/`:

- `lib/splitit_web_api_v3.rb` — top-level require that loads `apimatic_core`, `apimatic_faraday_client_adapter`, then all internal files.
- `lib/splitit_web_api_v3/client.rb` — the `Client` class: `initialize` with all keyword args (http options + credentials), controller accessor methods (e.g. `def simple_calculator`), OAuth manager accessors (e.g. `def o_auth_ccg`), and `self.from_env`.
- `lib/splitit_web_api_v3/configuration.rb` — `Configuration` class (extends `CoreLibrary::HttpClientConfiguration`) with `Environment` and `Server` inner constant classes, the `ENVIRONMENTS` hash mapping environment+server to base URIs, `get_base_uri`, and `clone_with`.
- `lib/splitit_web_api_v3/controllers/` — one `{Resource}Controller` per API resource group, each extending `BaseController`. **This is where operation method signatures and required vs. optional params live.**
- `lib/splitit_web_api_v3/models/` — model classes (extend `BaseModel`) and enum modules with frozen string constants.
- `lib/splitit_web_api_v3/exceptions/` — `api_exception.rb` (`APIException`) plus any typed error subclasses.
- `lib/splitit_web_api_v3/http/auth/` — one file per auth scheme (`basic_auth.rb`, `o_auth_ccg.rb`, etc.), each containing both the handler class (extends `CoreLibrary::HeaderAuth` or `CoreLibrary::QueryAuth`) and the `{Scheme}Credentials` data class with `initialize`, `from_env`, and `clone_with`.
- `lib/splitit_web_api_v3/utilities/` — `file_wrapper.rb`, `date_time_helper.rb`.
...
```ruby
# In your Gemfile:
gem 'splitit_web_api_v3', git: 'https://github.com/context-plugins/splitit-web-api-v3-ruby', branch: 'main'
```
...
include SplititWebApiV3   # optional — lets you write Client.new instead of SplititWebApiV3::Client.new
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/splitit-web-api-v3-ruby /tmp/splitit-web-api-v3-ruby-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/splitit-web-api-v3-ruby "$env:TEMP\splitit-web-api-v3-ruby-src"
```
...
- `lib/splitit_web_api_v3/client.rb` — the full `initialize` keyword list (http kwargs + credentials kwargs), controller accessors, OAuth manager accessors, and `from_env`.
- `lib/splitit_web_api_v3/configuration.rb` — `Environment` and `Server` constants, the `ENVIRONMENTS` hash (maps environment+server strings to base URI templates), and `clone_with`.
- `lib/splitit_web_api_v3/http/auth/*.rb` — credentials classes for every scheme this API uses, including their `initialize` keyword args and `from_env` env-var names.
- `lib/splitit_web_api_v3/controllers/*.rb` — operation method signatures; **this is where required vs. optional params live**.
- `lib/splitit_web_api_v3/models/` — model classes and enum modules; **this is where field names and enum values live**.
- `lib/splitit_web_api_v3/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/splitit-web-api-v3-ruby-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\splitit-web-api-v3-ruby-src"   # Windows
```
