...
> `{ApiGroup}`, `FlightPriceAnalysisApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FlightPriceAnalysisApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `FlightPriceAnalysisApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FlightPriceAnalysisApi.Errors`
...
```csharp
using FlightPriceAnalysisApi.Core.Exceptions;     // SdkException<TError>
using FlightPriceAnalysisApi.Core.ErrorResponse;  // ApiError, RawError
using FlightPriceAnalysisApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FlightPriceAnalysisApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FlightPriceAnalysisApi.Core.Exceptions;     // SdkException<TError>
using FlightPriceAnalysisApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FlightPriceAnalysisApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `FlightPriceAnalysisApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FlightPriceAnalysisApi.Core.Models;        // ApiResult<TResponse, TError>
using FlightPriceAnalysisApi.Core.ErrorResponse; // RawError
using FlightPriceAnalysisApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FlightPriceAnalysisApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
