...
`pazecheckoutapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `pazecheckoutapi/exceptions/` and operation docstrings.
...
```python
from pazecheckoutapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `pazecheckoutapi/exceptions/`.
...
```python
from pazecheckoutapi.exceptions.api_exception import APIException
from pazecheckoutapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`pazecheckoutapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`pazecheckoutapi/exceptions/` for the class.
...
`pazecheckoutapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `pazecheckoutapi/exceptions/`
  or a `ValueError` — check the auth handler in `pazecheckoutapi/http/auth/` for the exact type.
