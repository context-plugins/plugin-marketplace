...
> `{ApiGroup}`, `OpenAiApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `OpenAiApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `OpenAiApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `OpenAiApi.Errors`
...
```csharp
using OpenAiApi.Core.Exceptions;     // SdkException<TError>
using OpenAiApi.Core.ErrorResponse;  // ApiError, RawError
using OpenAiApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`OpenAiApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using OpenAiApi.Core.Exceptions;     // SdkException<TError>
using OpenAiApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `OpenAiApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `OpenAiApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using OpenAiApi.Core.Models;        // ApiResult<TResponse, TError>
using OpenAiApi.Core.ErrorResponse; // RawError
using OpenAiApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `OpenAiApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
