...
client = stub_client(stubs)
assert_raises Finnhub::APIException do
...
BASE = Finnhub::Configuration.new(environment: Finnhub::Environment::PRODUCTION).get_base_uri

def build_test_client
  Finnhub::Client.new(
    environment: Finnhub::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Finnhub::APIException
```
...
```ruby
expect { call }.to raise_error(Finnhub::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Finnhub::HttpCallBack
...
client  = Finnhub::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
