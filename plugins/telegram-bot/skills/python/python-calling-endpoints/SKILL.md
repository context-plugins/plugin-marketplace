...
Open `telegrambotapi/telegrambotapi_client.py` for the controller property names, then the relevant
`telegrambotapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from telegrambotapi.models.{request_model} import {RequestModel}
...
Open `telegrambotapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from telegrambotapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from telegrambotapi.http.http_call_back import HttpCallBack
...
client = TelegramBotAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `telegrambotapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `TelegramBotAPIClient` in `telegrambotapi/telegrambotapi_client.py`.
- Operation method signatures are in `telegrambotapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `telegrambotapi/models/`; enum types have `from_value()`.
