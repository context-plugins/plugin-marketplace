...
To see which schemes a specific SDK accepts, read the credential setter methods on `GoogleMapsPlatformClient.Builder`
...
- Set credentials **on the `GoogleMapsPlatformClient.Builder`** before calling `.build()` — the client is immutable
