    ...
    square.CreateConfiguration(
        square.WithHttpConfiguration(
            square.CreateHttpConfiguration(
                square.WithTimeout(30),
                square.WithRetryConfiguration(
                    square.CreateRetryConfiguration(
                        square.WithMaxRetryAttempts(3),
                        square.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := square.CreateRetryConfiguration(
    square.WithMaxRetryAttempts(5),
    square.WithRetryInterval(1),
    square.WithBackoffFactor(2),
    square.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    square.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := square.NewClient(
    square.CreateConfiguration(
        square.WithHttpConfiguration(
            square.CreateHttpConfiguration(
                square.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
