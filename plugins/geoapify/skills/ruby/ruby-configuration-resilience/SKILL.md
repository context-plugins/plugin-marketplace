...
```ruby
client = Geoapify::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Geoapify::Client.new(adapter: :net_http)
```
...
client = Geoapify::Client.new(connection: conn)
```
...
client = Geoapify::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Geoapify::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Geoapify::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Geoapify::ProxySettings.from_env
client = Geoapify::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Geoapify::HttpCallBack
...
client = Geoapify::Client.new(http_callback: LogCallback.new)
```
...
client = Geoapify::Client.from_env(timeout: 30, max_retries: 3)
```
