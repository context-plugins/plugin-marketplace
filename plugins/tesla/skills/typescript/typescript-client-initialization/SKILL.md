...
- `tesla-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'tesla-apilib';
...
```ts
import { {Resource}Controller } from 'tesla-apilib';
