...
`IPGeolocationAPIClient`/`IPGeolocationAPIClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `IPGeolocationAPIClient`,
...
static IPGeolocationAPIClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new IPGeolocationAPIClient(new HttpClient(handler), new IPGeolocationAPIClientOptions { /* auth not needed for stubs */ });
}
```
...
using IPGeolocationAPI.Errors;              // {Operation}Error types
...
```csharp
using IPGeolocationAPI.Core.Exceptions;
using IPGeolocationAPI.Core.ErrorResponse;
...
using IPGeolocationAPI.Core.ErrorResponse; // RawError (Case B)
...
var client = new IPGeolocationAPIClient(new HttpClient(handler), new IPGeolocationAPIClientOptions());
...
- For DI-based code, the SDK's `AddIPGeolocationAPIClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `IPGeolocationAPIClient` from the provider:
  ```csharp
  services.AddIPGeolocationAPIClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<IPGeolocationAPIClient>();
  ```
