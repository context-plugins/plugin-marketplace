...
> Throughout, `sportsdata` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := sportsdata.NewClient(
    sportsdata.CreateConfiguration(
        sportsdata.WithBasicAuthCredentials(
            sportsdata.NewBasicAuthCredentials(
...
sportsdata.WithApiKeyCredentials(sportsdata.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
sportsdata.WithApiHeaderCredentials(sportsdata.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
sportsdata.WithOAuthBearerTokenCredentials(
    sportsdata.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := sportsdata.NewClient(
    sportsdata.CreateConfiguration(
        sportsdata.WithOAuthCCGCredentials(
            sportsdata.NewOAuthCCGCredentials(
