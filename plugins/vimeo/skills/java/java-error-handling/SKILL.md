...
│   └── com.vimeo.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.vimeo.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.vimeo.api.http.client.HttpContext`, which carries:
