...
- `HotelNameAutocompleteClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`HotelNameAutocompleteClient.Builder` class. A minimal initialization:

```java
import com.amadeus.api.test.HotelNameAutocompleteClient;
import com.amadeus.api.test.Environment;

HotelNameAutocompleteClient client = new HotelNameAutocompleteClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `HotelNameAutocompleteClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `HotelNameAutocompleteClient.java` |

Call `.build()` to produce an immutable `HotelNameAutocompleteClient` instance.
...
public class ApiConfig {
    @Bean
    public HotelNameAutocompleteClient apiClient() {
        return new HotelNameAutocompleteClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired HotelNameAutocompleteClient client) {
        client.shutdown();
    }
}
```

Inject `HotelNameAutocompleteClient` into your services as a normal Spring dependency. Build only one bean.
