...
- `AkoyaClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `com.akoya.ddp.sandboxproducts` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`AkoyaClient.Builder` class. A minimal initialization:
...
The `Builder` is the only entry point — confirm its fluent methods in `AkoyaClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `AkoyaClient.java` |

Call `.build()` to produce an immutable `AkoyaClient` instance.
...
public class ApiConfig {
    @Bean
    public AkoyaClient apiClient() {
        return new AkoyaClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired AkoyaClient client) {
        client.shutdown();
    }
}
```

Inject `AkoyaClient` into your services as a normal Spring dependency. Build only one bean.
