...
Open `authentiqapi/authentiqapi_client.py` for the controller property names, then the relevant
`authentiqapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from authentiqapi.models.{request_model} import {RequestModel}
...
Open `authentiqapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from authentiqapi.models.suite_code_enum import SuiteCodeEnum
...
client = AuthentiqAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `authentiqapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `AuthentiqAPIClient` in `authentiqapi/authentiqapi_client.py`.
- Operation method signatures are in `authentiqapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `authentiqapi/models/`; enum types have `from_value()`.
