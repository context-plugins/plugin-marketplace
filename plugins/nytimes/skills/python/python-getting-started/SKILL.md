...
description: "The New York Times APIs Python SDK identity and lookup layer (Python only) — install, import root, base URL/environments, the auth pattern, the SDK map that ships at the SDK root (`sdk-map.md` + `map/operations/`) and how to traverse it, and the module table naming the one file owning each fact the map leaves to the source. Load this before answering any The New York Times APIs Python SDK contract question or writing any SDK code."
---

# Getting started with the The New York Times APIs Python SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing The New York Times APIs Python SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source modules it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `python-integrate-the-new-york-times-apis` for the workflow that wraps this skill.
...
Verified against `the_new_york_times_apis/` and `pyproject.toml` of the generated package at version `1.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | The New York Times APIs |
| Distribution name (what you install) | `the-new-york-times-apis` — **not on any package index**; installed from source (see *Install*) |
| Import root (what you import) | `the_new_york_times_apis` — not the string you install |
| Source repository | https://github.com/context-plugins/nytimes-python-sdk |
| Source branch | `main` |
| Version | `1.0.0` |
| Sync client class | `TheNewYorkTimesApisClient` (alias `Client`) |
| Async client class | `AsyncTheNewYorkTimesApisClient` (alias `AsyncClient`) |
| Client construction | **keyword-only**: `base_url` · `timeout` (default `30.0`) · `apikey`, and the transport override — `custom_http_client` on the sync client, `custom_async_http_client` on the async one (the names differ; see step 1) |
| Auth | **API key** in the `api-key` query parameter — set `apikey` |
| Environments | **no environment enum** — one `base_url` string, defaulting to `https://api.nytimes.com` |
| Base-URL config | `ServerConfig` (`the_new_york_times_apis/server/server_config.py`), frozen, `extra="forbid"` |
...
| Surface | 10 operations across 6 controllers · 45 models · 0 unions · 5 enums · 9 per-operation error unions |
...
This SDK is not published to a package index, so there is no `pip install` from PyPI for it. Install it from its repository — <https://github.com/context-plugins/nytimes-python-sdk> — into the same environment your project runs in:

```bash
pip install "the-new-york-times-apis @ git+https://github.com/context-plugins/nytimes-python-sdk.git@main"
```
...
Python does not re-export child modules transitively, so `from the_new_york_times_apis import models` alone does **not** make enums, error unions, or runtime types reachable. Import each kind of type from the module that owns it.

`the_new_york_times_apis/__init__.py` exports exactly 5 names beside the `the_new_york_times_apis.models` subpackage it re-exports:

```python
from the_new_york_times_apis import (
    AsyncClient,
    AsyncTheNewYorkTimesApisClient,
    Client,
    ServerConfig,
    TheNewYorkTimesApisClient,
)
```
...
| --- | --- |
| Domain models, their `…Dict` companions | `the_new_york_times_apis.models` |
| Enums (and their open `…OrInt`/`…OrStr` aliases) | `the_new_york_times_apis.models.enums` |
| `ApiError` · `RawError` · `ApiResult` · `RequestOptions` · `HttpClient` · `SdkBaseModel` · `UNSET` · `Optional` | `the_new_york_times_apis.core` |
| Per-operation error *unions* | `the_new_york_times_apis.errors` (`GetListsDateListJsonErrorBody`, …) |

`the_new_york_times_apis.core` re-exports its whole public surface (a curated `__all__`), so import from `…core` rather than from the private modules beneath it (`…core.results`, `…core.exceptions`, `…core.auth.schemes`).

## Environments — there is no environment enum

This SDK has **no environment type and no environment constants**. There is one knob: `base_url`, on `ServerConfig` (`the_new_york_times_apis/server/server_config.py`), and its default is **`https://api.nytimes.com`**:

```python
base_url: str = "https://api.nytimes.com"
```

Consequences to state on every contract sheet that touches configuration:

- Omitting `base_url` gives you **`https://api.nytimes.com`**, silently. A caller who believes they configured a different host and did not gets that host's behaviour with live credentials.
...
An API key sent as the `api-key` query parameter, exposed as the client's `apikey=` keyword taking a plain string.

```python
from the_new_york_times_apis import Client
...
| --- | --- | --- | --- |
| `client.archive` | `Archive` / `AsyncArchive` | 1 | `get_archive_by_month` |
| `client.books` | `Books` / `AsyncBooks` | 2 | `get_lists_date_list_json` · `get_lists_overview_json` |
| `client.most_popular` | `MostPopular` / `AsyncMostPopular` | 4 | `get_most_emailed_articles` · `get_most_shared_articles` · `get_most_shared_articles_by_type` · `get_most_viewed_articles` |
| `client.rss` | `Rss` / `AsyncRss` | 1 | `get_section_feed` |
| `client.search` | `Search` / `AsyncSearch` | 1 | `search_articles` |
| `client.stories` | `Stories` / `AsyncStories` | 1 | `get_top_stories_by_section` |
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `the_new_york_times_apis/` source directory, and these two entries:
...
**Installing the distribution does not give you the map.** `pyproject.toml` ships the `the_new_york_times_apis/` package only, so `sdk-map.md` and `map/operations/` are absent from the installed package — they live in the SDK's own source tree, at the root of its repository, <https://github.com/context-plugins/nytimes-python-sdk>. One clone therefore brings the map and the code it describes together, in lockstep by construction. Clone it to a temporary directory, outside the project repo:

```bash
git clone --depth 1 --branch main https://github.com/context-plugins/nytimes-python-sdk
```
...
Every `Source` path on the map is relative to that SDK root, so `the_new_york_times_apis/models/article.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
```bash
python -c "import the_new_york_times_apis, pathlib; print(pathlib.Path(the_new_york_times_apis.__file__).parent)"
```

Failing that, it is under the project's environment (`.venv/Lib/site-packages/the_new_york_times_apis` on Windows, `.venv/lib/python3.*/site-packages/the_new_york_times_apis` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
...
4. **Models** — load **python-models** the moment a request/response member is not a plain string or number. (*The signature won't tell you:* `Optional[T]` here is `T | UnsetType`, **not** `typing.Optional` — `None` is not a legal value for it; models are frozen pydantic instances with `…Dict` TypedDict companions; enums are **open** (`…OrInt`/`…OrStr`), so an unknown wire value passes through as a plain `int`/`str` rather than raising; wire aliases differ from Python member names; unknown response fields are **preserved**, not dropped; and serialize via `to_dict`/`to_json`.)
...
3. **The 1 operation that returns `None`** — `rss.get_section_feed`. Its raw peer is `ApiResult[None, …]`, so `with_raw_response` is the only way to observe the status code.
...
5. **The `ApiError.error` union** for each operation in scope — there is **one** typed error body in this SDK, so the union is never uniform. Every union is `<Typed> | RawError`:
   1. `SvcArchiveV1Json401Error1` — 1 operation (`archive.get_archive_by_month`); distinguishing members *none required*
   2. *(none)* — `books.get_lists_date_list_json` · `books.get_lists_overview_json` · `most_popular.get_most_emailed_articles` · `most_popular.get_most_shared_articles` · `most_popular.get_most_shared_articles_by_type` · `most_popular.get_most_viewed_articles` · `rss.get_section_feed` · `search.search_articles` · `stories.get_top_stories_by_section`: no typed arm, so `.error` is always `RawError`
   3. So `isinstance(e.error, SvcArchiveV1Json401Error1)` matches only 1 of 10 operations.
