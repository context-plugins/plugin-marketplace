...
- `ShellCardmanagement` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `shell_cardmanagement` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'shell_cardmanagement'
include ShellCardmanagement
...
The authoritative list is the `initialize` signature in `lib/shell_cardmanagement/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/shell_cardmanagement/configuration.rb`
(`build_default_config_from_env`) and `lib/shell_cardmanagement/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = ShellCardmanagement::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/shell_cardmanagement.rb`) and store it in a
