...
`testing` + `testify/require` for reference; substitute the real `flightpriceanalysisapi` and names.
...
import (
    ...
    "github.com/context-plugins/flight-price-analysis-api-go"   // root package flightpriceanalysisapi
)
...
func clientReturning(status int, body string) (flightpriceanalysisapi.ClientInterface, *stubTransport) {
    ...
    client := flightpriceanalysisapi.NewClient(
        flightpriceanalysisapi.CreateConfiguration(
            flightpriceanalysisapi.WithHttpConfiguration(
                flightpriceanalysisapi.CreateHttpConfiguration(flightpriceanalysisapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/flight-price-analysis-api-go/errors"
...
client := flightpriceanalysisapi.NewClient(
    flightpriceanalysisapi.CreateConfiguration(
        flightpriceanalysisapi.WithHttpConfiguration(
            flightpriceanalysisapi.CreateHttpConfiguration(flightpriceanalysisapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
