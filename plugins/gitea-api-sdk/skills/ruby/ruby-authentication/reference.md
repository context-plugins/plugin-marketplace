...
Confirm every name against `lib/gitea_api/client.rb`, `lib/gitea_api/configuration.rb`, the
`lib/gitea_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
