...
```ruby
client = Postnl::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Postnl::Client.new(adapter: :net_http)
```
...
client = Postnl::Client.new(connection: conn)
```
...
client = Postnl::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Postnl::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Postnl::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Postnl::ProxySettings.from_env
client = Postnl::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Postnl::HttpCallBack
...
client = Postnl::Client.new(http_callback: LogCallback.new)
```
...
client = Postnl::Client.from_env(timeout: 30, max_retries: 3)
```
