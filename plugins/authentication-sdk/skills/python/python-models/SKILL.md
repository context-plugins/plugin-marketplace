...
your SDK source (`authentication/models/`).
...
```python
from authentication.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `authentication/utilities/union_type_lookup.py`) and each candidate model
...
```python
from authentication.models.animal import Cat, Dog
...
```python
from authentication.models.animal import Animal, Cat, Dog
