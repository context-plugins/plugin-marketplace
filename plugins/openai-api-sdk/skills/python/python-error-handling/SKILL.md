...
`openaiapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `openaiapi/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `openaiapi/exceptions/`.
...
```python
from openaiapi.exceptions.api_exception import APIException
from openaiapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`openaiapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`openaiapi/exceptions/` for the class.
...
`openaiapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `openaiapi/exceptions/`
  or a `ValueError` — check the auth handler in `openaiapi/http/auth/` for the exact type.
