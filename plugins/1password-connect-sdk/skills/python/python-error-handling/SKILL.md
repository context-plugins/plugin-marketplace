...
`1passwordconnect/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `1passwordconnect/exceptions/` and operation docstrings.
...
```python
from 1passwordconnect.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `1passwordconnect/exceptions/`.
...
```python
from 1passwordconnect.exceptions.api_exception import APIException
from 1passwordconnect.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`1passwordconnect/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`1passwordconnect/exceptions/` for the class.
...
`1passwordconnect/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `1passwordconnect/exceptions/`
  or a `ValueError` — check the auth handler in `1passwordconnect/http/auth/` for the exact type.
