...
| Project | `MailboxValidatorFreeEmailChecker.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `MailboxValidatorFreeEmailChecker` (the `using` namespace) |
| Client class | `MailboxValidatorFreeEmailCheckerClient` |
| Options class | `MailboxValidatorFreeEmailCheckerClientOptions` |
...
namespaces transitively, so `using MailboxValidatorFreeEmailChecker.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/mailboxvalidator-free-email-checker-csharp
dotnet add reference mailboxvalidator-free-email-checker-csharp/MailboxValidatorFreeEmailChecker.csproj
```

```csharp
using MailboxValidatorFreeEmailChecker;          // client + options + ServerEnvironment
using MailboxValidatorFreeEmailChecker.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/mailboxvalidator-free-email-checker-csharp /tmp/mailboxvalidator-free-email-checker-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/mailboxvalidator-free-email-checker-csharp "$env:TEMP\mailboxvalidator-free-email-checker-dotnet-src"
```
...
- `MailboxValidatorFreeEmailCheckerClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/mailboxvalidator-free-email-checker-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\mailboxvalidator-free-email-checker-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `MailboxValidatorFreeEmailCheckerClient`, build its
   `MailboxValidatorFreeEmailCheckerClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
