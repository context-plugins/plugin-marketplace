    ...
    authentiqapi.CreateConfiguration(
        authentiqapi.WithHttpConfiguration(
            authentiqapi.CreateHttpConfiguration(
                authentiqapi.WithTimeout(30),
                authentiqapi.WithRetryConfiguration(
                    authentiqapi.CreateRetryConfiguration(
                        authentiqapi.WithMaxRetryAttempts(3),
                        authentiqapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := authentiqapi.CreateRetryConfiguration(
    authentiqapi.WithMaxRetryAttempts(5),
    authentiqapi.WithRetryInterval(1),
    authentiqapi.WithBackoffFactor(2),
    authentiqapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    authentiqapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := authentiqapi.NewClient(
    authentiqapi.CreateConfiguration(
        authentiqapi.WithHttpConfiguration(
            authentiqapi.CreateHttpConfiguration(
                authentiqapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
