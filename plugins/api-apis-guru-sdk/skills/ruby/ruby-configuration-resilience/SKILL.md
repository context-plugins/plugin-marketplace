...
```ruby
client = ApiApisGuru::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ApiApisGuru::Client.new(adapter: :net_http)
```
...
client = ApiApisGuru::Client.new(connection: conn)
```
...
client = ApiApisGuru::Client.new(environment: Environment::PRODUCTION)
client = ApiApisGuru::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ApiApisGuru::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ApiApisGuru::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ApiApisGuru::ProxySettings.from_env
client = ApiApisGuru::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ApiApisGuru::HttpCallBack
...
client = ApiApisGuru::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ApiApisGuru::Client.from_env
# Overrides take precedence over env vars:
client = ApiApisGuru::Client.from_env(timeout: 30, max_retries: 3)
```
