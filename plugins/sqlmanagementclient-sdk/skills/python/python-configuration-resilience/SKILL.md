...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `SqlManagementClientClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`sqlmanagementclient/configuration.py` in the cloned source.
...
```python
import requests
from sqlmanagementclient.configuration import Configuration
...
`sqlmanagementclient/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`sqlmanagementclient/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `sqlmanagementclient/configuration.py`
...
`NumberPagedResponse`) live in `sqlmanagementclient/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `sqlmanagementclient/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
