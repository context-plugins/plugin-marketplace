...
`ConfigurationApiClient`/`ConfigurationApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `ConfigurationApiClient`,
...
static ConfigurationApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new ConfigurationApiClient(new HttpClient(handler), new ConfigurationApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using ConfigurationApi.Errors;              // {Operation}Error types
...
```csharp
using ConfigurationApi.Core.Exceptions;
using ConfigurationApi.Core.ErrorResponse;
...
using ConfigurationApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new ConfigurationApiClient(new HttpClient(handler), new ConfigurationApiClientOptions());
...
- For DI-based code, the SDK's `AddConfigurationApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `ConfigurationApiClient` from the provider:
  ```csharp
  services.AddConfigurationApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<ConfigurationApiClient>();
  ```
