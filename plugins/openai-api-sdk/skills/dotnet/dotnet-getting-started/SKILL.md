...
| --- | --- |
| API | `OpenAI API` |
| Project | `OpenAiApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `OpenAiApi` (the `using` namespace) |
| Client class | `OpenAiApiClient` |
| Options class | `OpenAiApiClientOptions` |
...
namespaces transitively, so `using OpenAiApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/openai-api-csharp
dotnet add reference openai-api-csharp/OpenAiApi.csproj
```

```csharp
using OpenAiApi;          // client + options + ServerEnvironment
using OpenAiApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/openai-api-csharp /tmp/openai-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/openai-api-csharp "$env:TEMP\openai-api-dotnet-src"
```
...
- `OpenAiApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/openai-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\openai-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `OpenAiApiClient`, build its
   `OpenAiApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
