...
```go
twilioapis.WithOAuthCCGCredentials(
    twilioapis.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr twilioapis.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := twilioapis.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr twilioapis.OAuthCCGManager) models.OAuthToken {
...
```go
creds := twilioapis.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := twilioapis.NewClient(twilioapis.CreateConfiguration(twilioapis.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    twilioapis.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := twilioapis.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := twilioapis.NewClient(twilioapis.CreateConfiguration(twilioapis.WithOAuthROPCGCredentials(creds)))
...
```go
twilioapis.WithOAuthBearerTokenCredentials(twilioapis.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
twilioapis.WithBasicAuthCredentials(twilioapis.NewBasicAuthCredentials(username, password))
twilioapis.WithApiHeaderCredentials(twilioapis.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
twilioapis.WithApiKeyCredentials(twilioapis.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `TWILIOAPIS_...` variables — e.g.
`TWILIOAPIS_USERNAME`, `TWILIOAPIS_PASSWORD`, `TWILIOAPIS_TOKEN`, `TWILIOAPIS_API_KEY`,
`TWILIOAPIS_O_AUTH_CLIENT_ID`, `TWILIOAPIS_O_AUTH_CLIENT_SECRET`, `TWILIOAPIS_ACCESS_TOKEN`. Grep the
