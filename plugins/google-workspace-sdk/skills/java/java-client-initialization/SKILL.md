...
description: Construct and configure an APIMatic-generated Java SDK client — build via the inner GoogleWorkspaceClient.Builder() class (never a public constructor), set httpClientConfig via a lambda (not a config object), choose an Environment constant, access controllers via client.get{Resource}Controller(), call client.shutdown() at process exit, clone a live client with client.newBuilder(), and wire the client into Spring or plain-constructor DI. Use the moment you call new GoogleWorkspaceClient.Builder(), pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the arguments but not the builder pattern, the httpClientConfig lambda, or the lifetime/reuse rules.
---
...
- `GoogleWorkspaceClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`GoogleWorkspaceClient.Builder` class. A minimal initialization:

```java
import com.googleapis.www.GoogleWorkspaceClient;
import com.googleapis.www.Environment;

GoogleWorkspaceClient client = new GoogleWorkspaceClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `GoogleWorkspaceClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `GoogleWorkspaceClient.java` |

Call `.build()` to produce an immutable `GoogleWorkspaceClient` instance.
...
public class ApiConfig {
    @Bean
    public GoogleWorkspaceClient apiClient() {
        return new GoogleWorkspaceClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired GoogleWorkspaceClient client) {
        client.shutdown();
    }
}
```

Inject `GoogleWorkspaceClient` into your services as a normal Spring dependency. Build only one bean.
