...
your SDK source (`traderapi/models/`).
...
```python
from traderapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `traderapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from traderapi.models.animal import Cat, Dog
...
```python
from traderapi.models.animal import Animal, Cat, Dog
