...
`testing` + `testify/require` for reference; substitute the real `flightcreateorders` and names.
...
import (
    ...
    "github.com/context-plugins/flight-create-orders-go"   // root package flightcreateorders
)
...
func clientReturning(status int, body string) (flightcreateorders.ClientInterface, *stubTransport) {
    ...
    client := flightcreateorders.NewClient(
        flightcreateorders.CreateConfiguration(
            flightcreateorders.WithHttpConfiguration(
                flightcreateorders.CreateHttpConfiguration(flightcreateorders.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/flight-create-orders-go/errors"
...
client := flightcreateorders.NewClient(
    flightcreateorders.CreateConfiguration(
        flightcreateorders.WithHttpConfiguration(
            flightcreateorders.CreateHttpConfiguration(flightcreateorders.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
