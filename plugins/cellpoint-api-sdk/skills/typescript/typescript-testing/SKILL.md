...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new CellPointAPIClient({
...
```typescript
import { {Operation}Error } from 'cellpoint-apilib/errors';
...
```typescript
import { ApiError } from 'cellpoint-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(CellPointAPIClient)
    .useValue(new CellPointAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
