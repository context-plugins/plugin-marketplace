...
against the `HotelSearchAPIClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `HotelSearchAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
HotelSearchAPIClient client = new HotelSearchAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeHotelSearchAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new HotelSearchAPIClient.Builder()
