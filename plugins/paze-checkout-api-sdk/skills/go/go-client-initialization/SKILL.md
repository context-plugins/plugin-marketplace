...
- `pazecheckoutapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/paze-checkout-api-go` — the Go import path for the root package; `github.com/context-plugins/paze-checkout-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/paze-checkout-api-go"

client := pazecheckoutapi.NewClient(
    pazecheckoutapi.CreateConfiguration(
        pazecheckoutapi.WithEnvironment(pazecheckoutapi.PRODUCTION),
...
from environment variables named `PAZECHECKOUTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := pazecheckoutapi.NewClient(pazecheckoutapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `pazecheckoutapi.PRODUCTION`,
`pazecheckoutapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := pazecheckoutapi.NewClient(
    pazecheckoutapi.CreateConfiguration(pazecheckoutapi.WithEnvironment(pazecheckoutapi.PRODUCTION)),
)
```
...
client := pazecheckoutapi.NewClient(
    pazecheckoutapi.CreateConfiguration(
        pazecheckoutapi.WithHttpConfiguration(
            pazecheckoutapi.CreateHttpConfiguration(
                pazecheckoutapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                pazecheckoutapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = pazecheckoutapi.NewClient(pazecheckoutapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api pazecheckoutapi.ClientInterface
}

func NewService(api pazecheckoutapi.ClientInterface) *Service {
