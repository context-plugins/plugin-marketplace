...
client = stub_client(stubs)
assert_raises HotelSearchAPI::APIException do
...
def build_test_client
  HotelSearchAPI::Client.new(
    environment: HotelSearchAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error HotelSearchAPI::APIException
```
...
```ruby
expect { call }.to raise_error(HotelSearchAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < HotelSearchAPI::HttpCallBack
...
client  = HotelSearchAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
