...
│   └── com.notion.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.notion.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.notion.api.http.client.HttpContext`, which carries:
