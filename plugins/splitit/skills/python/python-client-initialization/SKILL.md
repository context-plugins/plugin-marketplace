...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the Splititwebapiv3Client constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write Splititwebapiv3Client(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `Splititwebapiv3Client` is the SDK's client class — **read the real name** from `splitit/splitit_client.py`
...
- `splitit` — the root package name (e.g. `multiauthsample`, `batester`).
- `Splititwebapiv3Client` — the generated client class (read it from the `class …Client` declaration in `splitit/splitit_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `Splititwebapiv3Client` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from splitit.splitit_client import Splititwebapiv3Client
from splitit.configuration import Environment
from splitit.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = Splititwebapiv3Client(
...
```python
from splitit.configuration import Configuration, Environment
...
client = Splititwebapiv3Client(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `splitit/splitit_client.py` in the cloned source.
...
`from_environment()` is a class method on both `Splititwebapiv3Client` and `Configuration` that reads
...
client = Splititwebapiv3Client.from_environment()
...
`from_environment` in `splitit/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `Splititwebapiv3Client`. Access them as attributes —
...
Open `splitit/splitit_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `splitit/configuration.py`. **Read the enum in `configuration.py`
...
client = Splititwebapiv3Client(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`splitit/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = Splititwebapiv3Client(config=new_config)
```
