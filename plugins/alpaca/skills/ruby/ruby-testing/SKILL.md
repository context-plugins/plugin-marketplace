...
client = stub_client(stubs)
assert_raises Alpaca::APIException do
...
BASE = Alpaca::Configuration.new(environment: Alpaca::Environment::PRODUCTION).get_base_uri

def build_test_client
  Alpaca::Client.new(
    environment: Alpaca::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Alpaca::APIException
```
...
```ruby
expect { call }.to raise_error(Alpaca::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Alpaca::HttpCallBack
...
client  = Alpaca::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
