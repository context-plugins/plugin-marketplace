...
your SDK source (`awsmigrationhub/models/`).
...
```python
from awsmigrationhub.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `awsmigrationhub/utilities/union_type_lookup.py`) and each candidate model
...
```python
from awsmigrationhub.models.animal import Cat, Dog
...
```python
from awsmigrationhub.models.animal import Animal, Cat, Dog
