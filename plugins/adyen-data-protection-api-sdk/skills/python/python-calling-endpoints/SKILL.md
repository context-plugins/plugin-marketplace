...
Open `adyendataprotectionapi/adyendataprotectionapi_client.py` for the controller property names, then the relevant
`adyendataprotectionapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from adyendataprotectionapi.models.{request_model} import {RequestModel}
...
Open `adyendataprotectionapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from adyendataprotectionapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from adyendataprotectionapi.http.http_call_back import HttpCallBack
...
client = AdyenDataProtectionAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `adyendataprotectionapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `AdyenDataProtectionAPIClient` in `adyendataprotectionapi/adyendataprotectionapi_client.py`.
- Operation method signatures are in `adyendataprotectionapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `adyendataprotectionapi/models/`; enum types have `from_value()`.
