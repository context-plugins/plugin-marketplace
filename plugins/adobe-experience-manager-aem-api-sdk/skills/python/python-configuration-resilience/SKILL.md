...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AdobeExperienceManagerAEMAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`adobeexperiencemanageraemapi/configuration.py` in the cloned source.
...
```python
import requests
from adobeexperiencemanageraemapi.configuration import Configuration
...
`adobeexperiencemanageraemapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`adobeexperiencemanageraemapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `adobeexperiencemanageraemapi/configuration.py`
...
`NumberPagedResponse`) live in `adobeexperiencemanageraemapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `adobeexperiencemanageraemapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
