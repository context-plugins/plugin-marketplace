...
> `{ApiGroup}`, `FlightInspirationSearch`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FlightInspirationSearch.Core.Exceptions`
- `ApiError` **and** `RawError` → `FlightInspirationSearch.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FlightInspirationSearch.Errors`
...
```csharp
using FlightInspirationSearch.Core.Exceptions;     // SdkException<TError>
using FlightInspirationSearch.Core.ErrorResponse;  // ApiError, RawError
using FlightInspirationSearch.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FlightInspirationSearch.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FlightInspirationSearch.Core.Exceptions;     // SdkException<TError>
using FlightInspirationSearch.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FlightInspirationSearch.Core.ErrorResponse`. Its public
...
`readonly struct` under `FlightInspirationSearch.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FlightInspirationSearch.Core.Models;        // ApiResult<TResponse, TError>
using FlightInspirationSearch.Core.ErrorResponse; // RawError
using FlightInspirationSearch.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FlightInspirationSearch.Core.Exceptions`) — an auth *application* failure, not an API error — when the
