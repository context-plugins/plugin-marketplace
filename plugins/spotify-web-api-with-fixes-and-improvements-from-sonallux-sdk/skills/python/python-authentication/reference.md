...
kwargs to the `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `spotifywebapiwithfixesandimprovementsfromsonallux/spotifywebapiwithfixesandimprovementsfromsonallux_client.py` and `spotifywebapiwithfixesandimprovementsfromsonallux/http/auth/` in
the cloned source.

## Basic auth

```python
from spotifywebapiwithfixesandimprovementsfromsonallux.http.auth.basic_auth import BasicAuthCredentials
...
```python
from spotifywebapiwithfixesandimprovementsfromsonallux.http.auth.api_key import ApiKeyCredentials
...
```python
from spotifywebapiwithfixesandimprovementsfromsonallux.http.auth.api_header import ApiHeaderCredentials
...
```python
from spotifywebapiwithfixesandimprovementsfromsonallux.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from spotifywebapiwithfixesandimprovementsfromsonallux.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from spotifywebapiwithfixesandimprovementsfromsonallux.http.auth.o_auth_acg import OAuthACGCredentials
from spotifywebapiwithfixesandimprovementsfromsonallux.models.o_auth_scope_spotifywebapiwithfixesandimprovementsfromsonallux_enum import OAuthScope{Pkg}Enum
...
client = SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient(o_auth_acg_credentials=creds)
...
```python
from spotifywebapiwithfixesandimprovementsfromsonallux.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient.from_environment()`) read each credential
...
Grep `from_environment` in `spotifywebapiwithfixesandimprovementsfromsonallux/configuration.py` and each `spotifywebapiwithfixesandimprovementsfromsonallux/http/auth/*.py` for the exact
