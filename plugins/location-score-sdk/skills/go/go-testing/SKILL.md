...
`testing` + `testify/require` for reference; substitute the real `locationscore` and names.
...
import (
    ...
    "github.com/context-plugins/location-score-go"   // root package locationscore
)
...
func clientReturning(status int, body string) (locationscore.ClientInterface, *stubTransport) {
    ...
    client := locationscore.NewClient(
        locationscore.CreateConfiguration(
            locationscore.WithHttpConfiguration(
                locationscore.CreateHttpConfiguration(locationscore.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/location-score-go/errors"
...
client := locationscore.NewClient(
    locationscore.CreateConfiguration(
        locationscore.WithHttpConfiguration(
            locationscore.CreateHttpConfiguration(locationscore.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
