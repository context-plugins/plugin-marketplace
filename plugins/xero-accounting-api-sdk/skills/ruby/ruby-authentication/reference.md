...
Confirm every name against `lib/xero_accounting_api/client.rb`, `lib/xero_accounting_api/configuration.rb`, the
`lib/xero_accounting_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
