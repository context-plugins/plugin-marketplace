...
> `{ApiGroup}`, `ApiManagementClient`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `ApiManagementClient.Core.Exceptions`
- `ApiError` **and** `RawError` → `ApiManagementClient.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `ApiManagementClient.Errors`
...
```csharp
using ApiManagementClient.Core.Exceptions;     // SdkException<TError>
using ApiManagementClient.Core.ErrorResponse;  // ApiError, RawError
using ApiManagementClient.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`ApiManagementClient.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using ApiManagementClient.Core.Exceptions;     // SdkException<TError>
using ApiManagementClient.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `ApiManagementClient.Core.ErrorResponse`. Its public
...
`readonly struct` under `ApiManagementClient.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using ApiManagementClient.Core.Models;        // ApiResult<TResponse, TError>
using ApiManagementClient.Core.ErrorResponse; // RawError
using ApiManagementClient.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `ApiManagementClient.Core.Exceptions`) — an auth *application* failure, not an API error — when the
