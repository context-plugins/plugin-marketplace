...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `SquareConnectAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`squareconnectapi/configuration.py` in the cloned source.
...
```python
import requests
from squareconnectapi.configuration import Configuration
...
`squareconnectapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`squareconnectapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `squareconnectapi/configuration.py`
...
`NumberPagedResponse`) live in `squareconnectapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `squareconnectapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
