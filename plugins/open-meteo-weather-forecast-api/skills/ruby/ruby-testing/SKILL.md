...
def stub_client(stubs)
  ...
  OpenMeteoWeatherForecastAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises OpenMeteoWeatherForecastAPI::APIException do
...
def build_test_client
  OpenMeteoWeatherForecastAPI::Client.new(
    environment: OpenMeteoWeatherForecastAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error OpenMeteoWeatherForecastAPI::APIException
```
...
```ruby
# Base APIException:
rescue OpenMeteoWeatherForecastAPI::APIException => e
  assert_equal 404, e.response_code
  assert_includes e.reason, 'Not Found'

# Typed subclass (e.g. OAuthProviderException):
rescue OpenMeteoWeatherForecastAPI::OAuthProviderException => e
...
```ruby
expect { call }.to raise_error(OpenMeteoWeatherForecastAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < OpenMeteoWeatherForecastAPI::HttpCallBack
...
client  = OpenMeteoWeatherForecastAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
