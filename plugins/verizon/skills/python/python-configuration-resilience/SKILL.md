...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `VerizonAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`verizonapi/configuration.py` in the cloned source.
...
```python
import requests
from verizonapi.configuration import Configuration
...
`verizonapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`verizonapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `verizonapi/configuration.py`
...
`NumberPagedResponse`) live in `verizonapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `verizonapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
