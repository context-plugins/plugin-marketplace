...
against the `AdyenBalanceControlAPIClient.Builder` setters, the `com.adyen.paltest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AdyenBalanceControlAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AdyenBalanceControlAPIClient client = new AdyenBalanceControlAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAdyenBalanceControlAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new AdyenBalanceControlAPIClient.Builder()
