...
`gitea/configuration.py` in the cloned source.
...
`gitea/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`gitea/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `gitea/configuration.py`
...
`NumberPagedResponse`) live in `gitea/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `gitea/http/http_call_back.py`) and pass an instance
as `http_call_back=`:

```python
import logging
from gitea.http.http_call_back import HttpCallBack
from gitea.configuration import Configuration
