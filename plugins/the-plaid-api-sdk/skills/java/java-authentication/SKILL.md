...
To see which schemes a specific SDK accepts, read the credential setter methods on `ThePlaidAPIClient.Builder`
...
- Set credentials **on the `ThePlaidAPIClient.Builder`** before calling `.build()` — the client is immutable
