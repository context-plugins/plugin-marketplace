...
```ruby
client = Finnhub::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Finnhub::Client.new(adapter: :net_http)
```
...
client = Finnhub::Client.new(connection: conn)
```
...
client = Finnhub::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Finnhub::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Finnhub::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Finnhub::ProxySettings.from_env
client = Finnhub::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Finnhub::HttpCallBack
...
client = Finnhub::Client.new(http_callback: LogCallback.new)
```
...
client = Finnhub::Client.from_env(timeout: 30, max_retries: 3)
```
