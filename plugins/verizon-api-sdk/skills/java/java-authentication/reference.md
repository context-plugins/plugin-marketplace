...
against the `VerizonAPIClient.Builder` setters, the `com.verizon.m5gedge.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `VerizonAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
VerizonAPIClient client = new VerizonAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeVerizonAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
