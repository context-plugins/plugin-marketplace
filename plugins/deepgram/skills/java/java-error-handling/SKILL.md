...
│   └── com.deepgram.agent.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.deepgram.agent.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.deepgram.agent.http.client.HttpContext`, which carries:
