...
`KlarnaPaymentsApiV1Client`/`KlarnaPaymentsApiV1ClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `KlarnaPaymentsApiV1Client`,
...
static KlarnaPaymentsApiV1Client ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new KlarnaPaymentsApiV1Client(new HttpClient(handler), new KlarnaPaymentsApiV1ClientOptions { /* auth not needed for stubs */ });
}
```
...
using KlarnaPaymentsApiV1.Errors;              // {Operation}Error types
...
```csharp
using KlarnaPaymentsApiV1.Core.Exceptions;
using KlarnaPaymentsApiV1.Core.ErrorResponse;
...
using KlarnaPaymentsApiV1.Core.ErrorResponse; // RawError (Case B)
...
var client = new KlarnaPaymentsApiV1Client(new HttpClient(handler), new KlarnaPaymentsApiV1ClientOptions());
...
- For DI-based code, the SDK's `AddKlarnaPaymentsApiV1Client` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `KlarnaPaymentsApiV1Client` from the provider:
  ```csharp
  services.AddKlarnaPaymentsApiV1Client(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<KlarnaPaymentsApiV1Client>();
  ```
