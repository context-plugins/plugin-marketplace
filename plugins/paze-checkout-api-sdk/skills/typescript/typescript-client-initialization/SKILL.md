...
- `paze-checkout-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'paze-checkout-apilib';
...
```ts
import { {Resource}Controller } from 'paze-checkout-apilib';
