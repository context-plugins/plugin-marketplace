...
- `adyen-binlookup-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'adyen-binlookup-apilib';
...
```ts
import { {Resource}Controller } from 'adyen-binlookup-apilib';
