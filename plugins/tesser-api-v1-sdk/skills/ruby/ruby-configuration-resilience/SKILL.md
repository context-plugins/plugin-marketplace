...
```ruby
client = TesserAPIV1::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = TesserAPIV1::Client.new(adapter: :net_http)
```
...
client = TesserAPIV1::Client.new(connection: conn)
```
...
client = TesserAPIV1::Client.new(environment: Environment::PRODUCTION)
client = TesserAPIV1::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = TesserAPIV1::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = TesserAPIV1::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = TesserAPIV1::ProxySettings.from_env
client = TesserAPIV1::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < TesserAPIV1::HttpCallBack
...
client = TesserAPIV1::Client.new(http_callback: LogCallback.new)
```
...
client = TesserAPIV1::Client.from_env(timeout: 30, max_retries: 3)
```
