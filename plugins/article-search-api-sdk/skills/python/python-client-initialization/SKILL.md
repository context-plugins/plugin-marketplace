...
- `articlesearchapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `ArticleSearchAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `ArticleSearchAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from articlesearchapi.articlesearchapi_client import ArticleSearchAPIClient
from articlesearchapi.configuration import Environment
from articlesearchapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = ArticleSearchAPIClient(
...
```python
from articlesearchapi.configuration import Configuration, Environment
...
client = ArticleSearchAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `articlesearchapi/articlesearchapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `ArticleSearchAPIClient` and `Configuration` that reads
...
client = ArticleSearchAPIClient.from_environment()
...
`from_environment` in `articlesearchapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `ArticleSearchAPIClient`. Access them as attributes —
...
Open `articlesearchapi/articlesearchapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `articlesearchapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from articlesearchapi.configuration import Environment

client = ArticleSearchAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`articlesearchapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = ArticleSearchAPIClient(config=new_config)
```
