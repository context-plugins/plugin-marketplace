...
- `newstore` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/newstore-go-sdk` — the Go import path for the root package; `github.com/context-plugins/newstore-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/newstore-go-sdk"

client := newstore.NewClient(
    newstore.CreateConfiguration(
        newstore.WithEnvironment(newstore.PRODUCTION),
...
from environment variables named `NEWSTORE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := newstore.NewClient(newstore.CreateConfigurationFromEnvironment())
```
...
```go
client := newstore.NewClient(
    newstore.CreateConfiguration(newstore.WithEnvironment(newstore.PRODUCTION)),
)
```
...
client := newstore.NewClient(
    newstore.CreateConfiguration(
        newstore.WithHttpConfiguration(
            newstore.CreateHttpConfiguration(
                newstore.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                newstore.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = newstore.NewClient(newstore.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api newstore.ClientInterface
}

func NewService(api newstore.ClientInterface) *Service {
