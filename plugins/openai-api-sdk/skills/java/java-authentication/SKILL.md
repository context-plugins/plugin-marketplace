...
To see which schemes a specific SDK accepts, read the credential setter methods on `OpenAIAPIClient.Builder`
...
- Set credentials **on the `OpenAIAPIClient.Builder`** before calling `.build()` — the client is immutable
