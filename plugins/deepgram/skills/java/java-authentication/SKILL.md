...
To see which schemes a specific SDK accepts, read the credential setter methods on `DeepgramClient.Builder`
...
- Set credentials **on the `DeepgramClient.Builder`** before calling `.build()` — the client is immutable
