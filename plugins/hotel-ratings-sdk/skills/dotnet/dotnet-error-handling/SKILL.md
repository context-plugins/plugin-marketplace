...
> `{ApiGroup}`, `HotelRatings`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `HotelRatings.Core.Exceptions`
- `ApiError` **and** `RawError` → `HotelRatings.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `HotelRatings.Errors`
...
```csharp
using HotelRatings.Core.Exceptions;     // SdkException<TError>
using HotelRatings.Core.ErrorResponse;  // ApiError, RawError
using HotelRatings.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`HotelRatings.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using HotelRatings.Core.Exceptions;     // SdkException<TError>
using HotelRatings.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `HotelRatings.Core.ErrorResponse`. Its public
...
`readonly struct` under `HotelRatings.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using HotelRatings.Core.Models;        // ApiResult<TResponse, TError>
using HotelRatings.Core.ErrorResponse; // RawError
using HotelRatings.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `HotelRatings.Core.Exceptions`) — an auth *application* failure, not an API error — when the
