...
To see which schemes a specific SDK accepts, read the credential setter methods on `AdvancedBillingClient.Builder`
...
- Set credentials **on the `AdvancedBillingClient.Builder`** before calling `.build()` — the client is immutable
