...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `paypal-server-sdklib`) — replace it with the concrete identifier from the source.
...
import { {Operation}Error } from 'paypal-server-sdklib/errors';
