...
- `cellpoint` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/cellpoint-go-sdk` — the Go import path for the root package; `github.com/context-plugins/cellpoint-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/cellpoint-go-sdk"

client := cellpoint.NewClient(
    cellpoint.CreateConfiguration(
        cellpoint.WithEnvironment(cellpoint.PRODUCTION),
...
from environment variables named `CELLPOINT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := cellpoint.NewClient(cellpoint.CreateConfigurationFromEnvironment())
```
...
```go
client := cellpoint.NewClient(
    cellpoint.CreateConfiguration(cellpoint.WithEnvironment(cellpoint.PRODUCTION)),
)
```
...
client := cellpoint.NewClient(
    cellpoint.CreateConfiguration(
        cellpoint.WithHttpConfiguration(
            cellpoint.CreateHttpConfiguration(
                cellpoint.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                cellpoint.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = cellpoint.NewClient(cellpoint.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api cellpoint.ClientInterface
}

func NewService(api cellpoint.ClientInterface) *Service {
