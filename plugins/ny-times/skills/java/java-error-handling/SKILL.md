...
│   └── com.nytimes.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.nytimes.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.nytimes.api.http.client.HttpContext`, which carries:
