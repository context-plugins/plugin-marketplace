...
- `seatmap-displaylib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'seatmap-displaylib';
...
```ts
import { {Resource}Controller } from 'seatmap-displaylib';
