...
`AdyenDataProtectionApiClient`/`AdyenDataProtectionApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `AdyenDataProtectionApiClient`,
...
static AdyenDataProtectionApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new AdyenDataProtectionApiClient(new HttpClient(handler), new AdyenDataProtectionApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using AdyenDataProtectionApi.Errors;              // {Operation}Error types
...
```csharp
using AdyenDataProtectionApi.Core.Exceptions;
using AdyenDataProtectionApi.Core.ErrorResponse;
...
using AdyenDataProtectionApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new AdyenDataProtectionApiClient(new HttpClient(handler), new AdyenDataProtectionApiClientOptions());
...
- For DI-based code, the SDK's `AddAdyenDataProtectionApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `AdyenDataProtectionApiClient` from the provider:
  ```csharp
  services.AddAdyenDataProtectionApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<AdyenDataProtectionApiClient>();
  ```
