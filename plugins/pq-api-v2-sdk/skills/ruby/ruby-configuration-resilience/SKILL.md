...
```ruby
client = PQAPIV2::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = PQAPIV2::Client.new(adapter: :net_http)
```
...
client = PQAPIV2::Client.new(connection: conn)
```
...
client = PQAPIV2::Client.new(environment: Environment::PRODUCTION)
client = PQAPIV2::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = PQAPIV2::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = PQAPIV2::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = PQAPIV2::ProxySettings.from_env
client = PQAPIV2::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < PQAPIV2::HttpCallBack
...
client = PQAPIV2::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = PQAPIV2::Client.from_env
# Overrides take precedence over env vars:
client = PQAPIV2::Client.from_env(timeout: 30, max_retries: 3)
```
