...
To see which schemes a specific SDK accepts, read the credential setter methods on `GoeapiClient.Builder`
...
- Set credentials **on the `GoeapiClient.Builder`** before calling `.build()` — the client is immutable
