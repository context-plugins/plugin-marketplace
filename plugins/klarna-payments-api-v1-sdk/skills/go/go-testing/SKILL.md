...
`testing` + `testify/require` for reference; substitute the real `klarnapaymentsapiv1` and names.
...
import (
    ...
    "github.com/context-plugins/klarna-payments-api-v1-go"   // root package klarnapaymentsapiv1
)
...
func clientReturning(status int, body string) (klarnapaymentsapiv1.ClientInterface, *stubTransport) {
    ...
    client := klarnapaymentsapiv1.NewClient(
        klarnapaymentsapiv1.CreateConfiguration(
            klarnapaymentsapiv1.WithHttpConfiguration(
                klarnapaymentsapiv1.CreateHttpConfiguration(klarnapaymentsapiv1.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/klarna-payments-api-v1-go/errors"
...
client := klarnapaymentsapiv1.NewClient(
    klarnapaymentsapiv1.CreateConfiguration(
        klarnapaymentsapiv1.WithHttpConfiguration(
            klarnapaymentsapiv1.CreateHttpConfiguration(klarnapaymentsapiv1.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
