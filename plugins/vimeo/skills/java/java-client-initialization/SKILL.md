...
- `VimeoClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `com.vimeo.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`VimeoClient.Builder` class. A minimal initialization:

```java
import com.vimeo.api.VimeoClient;
import com.vimeo.api.Environment;

VimeoClient client = new VimeoClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `VimeoClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `VimeoClient.java` |

Call `.build()` to produce an immutable `VimeoClient` instance.
...
public class ApiConfig {
    @Bean
    public VimeoClient apiClient() {
        return new VimeoClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired VimeoClient client) {
        client.shutdown();
    }
}
```

Inject `VimeoClient` into your services as a normal Spring dependency. Build only one bean.
