...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `GoogleWorkspaceClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`googleworkspace/configuration.py` in the cloned source.
...
```python
import requests
from googleworkspace.configuration import Configuration
...
`googleworkspace/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`googleworkspace/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `googleworkspace/configuration.py`
...
`NumberPagedResponse`) live in `googleworkspace/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `googleworkspace/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
