...
- `HotelSearchAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `hotel_search_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'hotel_search_api'
include HotelSearchAPI
...
The authoritative list is the `initialize` signature in `lib/hotel_search_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/hotel_search_api/configuration.rb`
(`build_default_config_from_env`) and `lib/hotel_search_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/hotel_search_api.rb`) and store it in a
