...
`testing` + `testify/require` for reference; substitute the real `firecrawl` and names.
...
import (
    ...
    "github.com/context-plugins/firecrawl-go-sdk"   // root package firecrawl
)
...
func clientReturning(status int, body string) (firecrawl.ClientInterface, *stubTransport) {
    ...
    client := firecrawl.NewClient(
        firecrawl.CreateConfiguration(
            firecrawl.WithHttpConfiguration(
                firecrawl.CreateHttpConfiguration(firecrawl.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/firecrawl-go-sdk/errors"
...
client := firecrawl.NewClient(
    firecrawl.CreateConfiguration(
        firecrawl.WithHttpConfiguration(
            firecrawl.CreateHttpConfiguration(firecrawl.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
