...
> Throughout, `newstore` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := newstore.NewClient(
    newstore.CreateConfiguration(
        newstore.WithBasicAuthCredentials(
            newstore.NewBasicAuthCredentials(
...
newstore.WithApiKeyCredentials(newstore.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
newstore.WithApiHeaderCredentials(newstore.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
newstore.WithOAuthBearerTokenCredentials(
    newstore.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := newstore.NewClient(
    newstore.CreateConfiguration(
        newstore.WithOAuthCCGCredentials(
            newstore.NewOAuthCCGCredentials(
