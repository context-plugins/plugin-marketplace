...
```ruby
client = AdyenDataProtectionAPI::Client.new(
  max_retries:    3,
...
```ruby
client = AdyenDataProtectionAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = AdyenDataProtectionAPI::Client.new(adapter: :net_http)
```
...
client = AdyenDataProtectionAPI::Client.new(connection: conn)
```
...
client = AdyenDataProtectionAPI::Client.new(environment: Environment::PRODUCTION)
client = AdyenDataProtectionAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = AdyenDataProtectionAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = AdyenDataProtectionAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = AdyenDataProtectionAPI::ProxySettings.from_env
client = AdyenDataProtectionAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < AdyenDataProtectionAPI::HttpCallBack
...
client = AdyenDataProtectionAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = AdyenDataProtectionAPI::Client.from_env
# Overrides take precedence over env vars:
client = AdyenDataProtectionAPI::Client.from_env(timeout: 30, max_retries: 3)
```
