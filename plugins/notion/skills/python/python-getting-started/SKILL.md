...
Verified against `notion_api/` and `pyproject.toml` of the generated package at version `0.1.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Notion API |
| Distribution name (what you install) | `notion-api` — **not on any package index**; installed from source (see *Install*) |
| Import root (what you import) | `notion_api` — not the string you install |
| Source repository | https://github.com/context-plugins/notion-python-sdk |
| Source branch | `main` |
| Version | `0.1.0` |
| Sync client class | `NotionApiClient` (alias `Client`) |
| Async client class | `AsyncNotionApiClient` (alias `AsyncClient`) |
...
| Environments | **no environment enum** — one `base_url` string, defaulting to `https://api.notion.com/v1` |
| Base-URL config | `ServerConfig` (`notion_api/server/server_config.py`), frozen, `extra="forbid"` |
...
| Surface | 5 operations, all directly on the client — no controller groups; the map's single page is `map/operations/client.md` · 8 models · 0 unions · 2 enums · 5 per-operation error unions |
...
This SDK is not published to a package index, so there is no `pip install` from PyPI for it. Install it from its repository — <https://github.com/context-plugins/notion-python-sdk> — into the same environment your project runs in:

```bash
pip install "notion-api @ git+https://github.com/context-plugins/notion-python-sdk.git@main"
```
...
Python does not re-export child modules transitively, so `from notion_api import models` alone does **not** make enums, error unions, or runtime types reachable. Import each kind of type from the module that owns it.

`notion_api/__init__.py` exports exactly 5 names beside the `notion_api.models` subpackage it re-exports:

```python
from notion_api import (
    AsyncClient,
    AsyncNotionApiClient,
    Client,
    NotionApiClient,
    ServerConfig,
)
...
| --- | --- |
| Domain models, their `…Dict` companions | `notion_api.models` |
| Enums (and their open `…OrStr` aliases) | `notion_api.models.enums` |
| `ApiError` · `RawError` · `ApiResult` · `RequestOptions` · `HttpClient` · `SdkBaseModel` · `UNSET` · `Optional` | `notion_api.core` |
| Per-operation error *unions* | `notion_api.errors` (`AppendBlockChildrenErrorBody`, …) |

`notion_api.core` re-exports its whole public surface (a curated `__all__`), so import from `…core` rather than from the private modules beneath it (`…core.results`, `…core.exceptions`, `…core.auth.schemes`).

## Environments — there is no environment enum

This SDK has **no environment type and no environment constants**. There is one knob: `base_url`, on `ServerConfig` (`notion_api/server/server_config.py`), and its default is **`https://api.notion.com/v1`**:
...
- Omitting `base_url` gives you **`https://api.notion.com/v1`**, silently. A caller who believes they configured a different host and did not gets that host's behaviour with live credentials.
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `notion_api/` source directory, and these two entries:
...
**Installing the distribution does not give you the map.** `pyproject.toml` ships the `notion_api/` package only, so `sdk-map.md` and `map/operations/` are absent from the installed package — they live in the SDK's own source tree, at the root of its repository, <https://github.com/context-plugins/notion-python-sdk>. One clone therefore brings the map and the code it describes together, in lockstep by construction. Clone it to a temporary directory, outside the project repo:

```bash
git clone --depth 1 --branch main https://github.com/context-plugins/notion-python-sdk
```
...
Every `Source` path on the map is relative to that SDK root, so `notion_api/models/block.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
```bash
python -c "import notion_api, pathlib; print(pathlib.Path(notion_api.__file__).parent)"
```

Failing that, it is under the project's environment (`.venv/Lib/site-packages/notion_api` on Windows, `.venv/lib/python3.*/site-packages/notion_api` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
...
5. **The `ApiError.error` union** for each operation in scope — there is **one** typed error body in this SDK, so the union is never uniform. Every union is `<Typed> | RawError`:
   1. `Error` — 5 operations (`append_block_children` · `delete_block` · `retrieve_block` · `retrieve_block_children` · `update_block`); distinguishing members `status` · `code` · `message`
   2. So `isinstance(e.error, Error)` matches only 5 of 5 operations.
