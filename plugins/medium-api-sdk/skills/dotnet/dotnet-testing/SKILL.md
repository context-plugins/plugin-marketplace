...
`MediumApiClient`/`MediumApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `MediumApiClient`,
...
static MediumApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new MediumApiClient(new HttpClient(handler), new MediumApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using MediumApi.Errors;              // {Operation}Error types
...
```csharp
using MediumApi.Core.Exceptions;
using MediumApi.Core.ErrorResponse;
...
using MediumApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new MediumApiClient(new HttpClient(handler), new MediumApiClientOptions());
...
- For DI-based code, the SDK's `AddMediumApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `MediumApiClient` from the provider:
  ```csharp
  services.AddMediumApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<MediumApiClient>();
  ```
