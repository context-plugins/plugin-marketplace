...
reference; substitute your `hostedonboardingapi` and `HostedOnboardingAPIClient` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `hostedonboardingapi/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
