...
> `KlarnaClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): KlarnaClient
{
...
```php
use KlarnaLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use KlarnaLib\Exceptions\{OperationException};
use KlarnaLib\Exceptions\ApiException;
...
app()->instance(KlarnaClient::class, clientWithResponses([
