...
kwargs to the `NetworkAdminManagementClientClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `networkadminmanagementclient/networkadminmanagementclient_client.py` and `networkadminmanagementclient/http/auth/` in
the cloned source.

## Basic auth

```python
from networkadminmanagementclient.http.auth.basic_auth import BasicAuthCredentials
...
```python
from networkadminmanagementclient.http.auth.api_key import ApiKeyCredentials
...
```python
from networkadminmanagementclient.http.auth.api_header import ApiHeaderCredentials
...
```python
from networkadminmanagementclient.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from networkadminmanagementclient.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from networkadminmanagementclient.http.auth.o_auth_acg import OAuthACGCredentials
from networkadminmanagementclient.models.o_auth_scope_networkadminmanagementclient_enum import OAuthScope{Pkg}Enum
...
client = NetworkAdminManagementClientClient(o_auth_acg_credentials=creds)
...
```python
from networkadminmanagementclient.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = NetworkAdminManagementClientClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `NetworkAdminManagementClientClient.from_environment()`) read each credential
...
Grep `from_environment` in `networkadminmanagementclient/configuration.py` and each `networkadminmanagementclient/http/auth/*.py` for the exact
