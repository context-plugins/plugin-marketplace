...
`testing` + `testify/require` for reference; substitute the real `nytimes` and names.
...
import (
    ...
    "github.com/context-plugins/ny-times-go-sdk"   // root package nytimes
)
...
func clientReturning(status int, body string) (nytimes.ClientInterface, *stubTransport) {
    ...
    client := nytimes.NewClient(
        nytimes.CreateConfiguration(
            nytimes.WithHttpConfiguration(
                nytimes.CreateHttpConfiguration(nytimes.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/ny-times-go-sdk/errors"
...
client := nytimes.NewClient(
    nytimes.CreateConfiguration(
        nytimes.WithHttpConfiguration(
            nytimes.CreateHttpConfiguration(nytimes.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
