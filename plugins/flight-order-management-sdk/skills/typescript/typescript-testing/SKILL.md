...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new FlightOrderManagementClient({
...
```typescript
import { {Operation}Error } from 'flight-order-managementlib/errors';
...
```typescript
import { ApiError } from 'flight-order-managementlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(FlightOrderManagementClient)
    .useValue(new FlightOrderManagementClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
