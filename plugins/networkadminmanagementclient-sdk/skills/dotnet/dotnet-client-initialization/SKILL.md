...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddNetworkAdminManagementClientClient extension. Use the moment you write `new NetworkAdminManagementClientClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `NetworkAdminManagementClientClientOptions` — its options class.
- `NetworkAdminManagementClient` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public NetworkAdminManagementClientClient(HttpClient httpClient, NetworkAdminManagementClientClientOptions options)
```
...
```csharp
public class NetworkAdminManagementClientClientOptions
{
...
```csharp
using NetworkAdminManagementClient;
using NetworkAdminManagementClient.Servers;

var options = new NetworkAdminManagementClientClientOptions
{
...
var client = new NetworkAdminManagementClientClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddNetworkAdminManagementClientClient`, which registers the
...
builder.Services.AddNetworkAdminManagementClientClient(options =>
{
...
```csharp
public sealed class MyService(NetworkAdminManagementClientClient client)
{
