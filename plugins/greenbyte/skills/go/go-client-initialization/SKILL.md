...
- `greenbyte` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/greenbyte-go-sdk` — the Go import path for the root package; `github.com/context-plugins/greenbyte-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/greenbyte-go-sdk"

client := greenbyte.NewClient(
    greenbyte.CreateConfiguration(
        greenbyte.WithEnvironment(greenbyte.PRODUCTION),
...
from environment variables named `GREENBYTE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := greenbyte.NewClient(greenbyte.CreateConfigurationFromEnvironment())
```
...
```go
client := greenbyte.NewClient(
    greenbyte.CreateConfiguration(greenbyte.WithEnvironment(greenbyte.PRODUCTION)),
)
```
...
client := greenbyte.NewClient(
    greenbyte.CreateConfiguration(
        greenbyte.WithHttpConfiguration(
            greenbyte.CreateHttpConfiguration(
                greenbyte.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                greenbyte.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = greenbyte.NewClient(greenbyte.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api greenbyte.ClientInterface
}

func NewService(api greenbyte.ClientInterface) *Service {
