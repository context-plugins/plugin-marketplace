...
Integrate and consume the **APIMatic-generated X API v2 SDK** (the "APIMATIC v3.0" generic
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, PHP, Python, Ruby, Go.
...
(`dotnet-x-api-v2-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-x-api-v2`** router skill routes all
C#/.NET SDK work to the single **`dotnet-x-api-v2-sdk`** agent (.NET-only), which answers
