...
> `{ApiGroup}`, `NetworkAdminManagementClient`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `NetworkAdminManagementClient.Core.Exceptions`
- `ApiError` **and** `RawError` → `NetworkAdminManagementClient.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `NetworkAdminManagementClient.Errors`
...
```csharp
using NetworkAdminManagementClient.Core.Exceptions;     // SdkException<TError>
using NetworkAdminManagementClient.Core.ErrorResponse;  // ApiError, RawError
using NetworkAdminManagementClient.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`NetworkAdminManagementClient.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using NetworkAdminManagementClient.Core.Exceptions;     // SdkException<TError>
using NetworkAdminManagementClient.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `NetworkAdminManagementClient.Core.ErrorResponse`. Its public
...
`readonly struct` under `NetworkAdminManagementClient.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using NetworkAdminManagementClient.Core.Models;        // ApiResult<TResponse, TError>
using NetworkAdminManagementClient.Core.ErrorResponse; // RawError
using NetworkAdminManagementClient.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `NetworkAdminManagementClient.Core.Exceptions`) — an auth *application* failure, not an API error — when the
