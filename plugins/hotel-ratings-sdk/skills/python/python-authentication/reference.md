...
kwargs to the `HotelRatingsClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `hotelratings/hotelratings_client.py` and `hotelratings/http/auth/` in
the cloned source.

## Basic auth

```python
from hotelratings.http.auth.basic_auth import BasicAuthCredentials
...
```python
from hotelratings.http.auth.api_key import ApiKeyCredentials
...
```python
from hotelratings.http.auth.api_header import ApiHeaderCredentials
...
```python
from hotelratings.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from hotelratings.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from hotelratings.http.auth.o_auth_acg import OAuthACGCredentials
from hotelratings.models.o_auth_scope_hotelratings_enum import OAuthScope{Pkg}Enum
...
client = HotelRatingsClient(o_auth_acg_credentials=creds)
...
```python
from hotelratings.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = HotelRatingsClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `HotelRatingsClient.from_environment()`) read each credential
...
Grep `from_environment` in `hotelratings/configuration.py` and each `hotelratings/http/auth/*.py` for the exact
