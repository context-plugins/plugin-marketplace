...
kwarg names are generated per-API — confirm from `vimeo/vimeo_client.py` and `vimeo/http/auth/` in
the cloned source.

## Basic auth

```python
from vimeo.http.auth.basic_auth import BasicAuthCredentials
...
```python
from vimeo.http.auth.api_key import ApiKeyCredentials
...
```python
from vimeo.http.auth.api_header import ApiHeaderCredentials
...
```python
from vimeo.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from vimeo.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from vimeo.http.auth.o_auth_acg import OAuthACGCredentials
from vimeo.models.o_auth_scope_vimeo_enum import OAuthScope{Pkg}Enum
...
```python
from vimeo.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `vimeo/configuration.py` and each `vimeo/http/auth/*.py` for the exact
