...
against the `VimeoClient.Builder` setters, the `com.vimeo.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `VimeoClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
VimeoClient client = new VimeoClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeVimeoEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
