...
- `SqlManagementClient` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `sqlmanagementclient` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'sqlmanagementclient'
include SqlManagementClient
...
The authoritative list is the `initialize` signature in `lib/sqlmanagementclient/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/sqlmanagementclient/configuration.rb`
(`build_default_config_from_env`) and `lib/sqlmanagementclient/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/sqlmanagementclient.rb`) and store it in a
