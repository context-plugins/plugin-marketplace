...
- `Payquicker` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `payquicker` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/payquicker/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/payquicker/configuration.rb`
(`build_default_config_from_env`) and `lib/payquicker/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = Payquicker::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/payquicker.rb`) and store it in a
