...
> `FirecrawlApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FirecrawlApiClient
{
...
app()->instance(FirecrawlApiClient::class, clientWithResponses([
