...
- `splitit-web-api-v-3-lib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'splitit-web-api-v-3-lib';
...
```ts
import { {Resource}Controller } from 'splitit-web-api-v-3-lib';
