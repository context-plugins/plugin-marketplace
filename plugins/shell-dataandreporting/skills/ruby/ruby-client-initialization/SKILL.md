...
- `ShellDataandreporting` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `shell_dataandreporting` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'shell_dataandreporting'
include ShellDataandreporting
...
The authoritative list is the `initialize` signature in `lib/shell_dataandreporting/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/shell_dataandreporting/configuration.rb`
(`build_default_config_from_env`) and `lib/shell_dataandreporting/http/auth/*.rb` (`{Scheme}Credentials.from_env`).

## Choosing the environment / base URL

`Environment` constants are frozen strings defined inside the `Configuration` class — read them for the
real names:
...
```ruby
# Application startup:
CLIENT = ShellDataandreporting::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/shell_dataandreporting.rb`) and store it in a
