...
`shelldataandreporting/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `shelldataandreporting/exceptions/` and operation docstrings.
...
```python
from shelldataandreporting.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `shelldataandreporting/exceptions/`.
...
```python
from shelldataandreporting.exceptions.api_exception import APIException
from shelldataandreporting.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`shelldataandreporting/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`shelldataandreporting/exceptions/` for the class.
...
`shelldataandreporting/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `shelldataandreporting/exceptions/`
  or a `ValueError` — check the auth handler in `shelldataandreporting/http/auth/` for the exact type.
