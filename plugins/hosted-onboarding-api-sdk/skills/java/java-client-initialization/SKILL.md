...
- `HostedOnboardingAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.adyen.caltest` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`HostedOnboardingAPIClient.Builder` class. A minimal initialization:

```java
import com.adyen.caltest.HostedOnboardingAPIClient;
import com.adyen.caltest.Environment;

HostedOnboardingAPIClient client = new HostedOnboardingAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `HostedOnboardingAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `HostedOnboardingAPIClient.java` |

Call `.build()` to produce an immutable `HostedOnboardingAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public HostedOnboardingAPIClient apiClient() {
        return new HostedOnboardingAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired HostedOnboardingAPIClient client) {
        client.shutdown();
    }
}
```

Inject `HostedOnboardingAPIClient` into your services as a normal Spring dependency. Build only one bean.
