...
`VimeoApiClient`/`VimeoApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `VimeoApiClient`,
...
static VimeoApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new VimeoApiClient(new HttpClient(handler), new VimeoApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using VimeoApi.Errors;              // {Operation}Error types
...
using VimeoApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new VimeoApiClient(new HttpClient(handler), new VimeoApiClientOptions());
...
- For DI-based code, the SDK's `AddVimeoApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `VimeoApiClient` from the provider:
  ```csharp
  services.AddVimeoApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<VimeoApiClient>();
  ```
