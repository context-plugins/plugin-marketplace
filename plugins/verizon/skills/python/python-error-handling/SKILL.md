...
`verizonapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `verizonapi/exceptions/` and operation docstrings.
...
```python
from verizonapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `verizonapi/exceptions/`.
...
```python
from verizonapi.exceptions.api_exception import APIException
from verizonapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`verizonapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`verizonapi/exceptions/` for the class.
...
`verizonapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `verizonapi/exceptions/`
  or a `ValueError` — check the auth handler in `verizonapi/http/auth/` for the exact type.
