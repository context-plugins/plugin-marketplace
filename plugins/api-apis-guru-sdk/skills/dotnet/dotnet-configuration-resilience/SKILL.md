...
All types below live under `OnDemandFlightStatus.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using OnDemandFlightStatus.Core.Configuration;
...
```csharp
using OnDemandFlightStatus.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new OnDemandFlightStatusClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `OnDemandFlightStatus.Core.Exceptions`):
...
With DI, the SDK's `AddOnDemandFlightStatusClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddOnDemandFlightStatusClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
