...
Open `shutterstockapiexplorer/shutterstockapiexplorer_client.py` for the controller property names, then the relevant
`shutterstockapiexplorer/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from shutterstockapiexplorer.models.{request_model} import {RequestModel}
...
Open `shutterstockapiexplorer/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from shutterstockapiexplorer.models.suite_code_enum import SuiteCodeEnum
...
```python
from shutterstockapiexplorer.http.http_call_back import HttpCallBack
...
client = ShutterstockAPIExplorerClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `shutterstockapiexplorer/pagination/` in the source.

## Finding the right method

- Controller property names are on `ShutterstockAPIExplorerClient` in `shutterstockapiexplorer/shutterstockapiexplorer_client.py`.
- Operation method signatures are in `shutterstockapiexplorer/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `shutterstockapiexplorer/models/`; enum types have `from_value()`.
