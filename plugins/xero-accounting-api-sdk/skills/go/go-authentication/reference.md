...
```go
xeroaccountingapi.WithOAuthCCGCredentials(
    xeroaccountingapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr xeroaccountingapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := xeroaccountingapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr xeroaccountingapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := xeroaccountingapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := xeroaccountingapi.NewClient(xeroaccountingapi.CreateConfiguration(xeroaccountingapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    xeroaccountingapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := xeroaccountingapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := xeroaccountingapi.NewClient(xeroaccountingapi.CreateConfiguration(xeroaccountingapi.WithOAuthROPCGCredentials(creds)))
...
```go
xeroaccountingapi.WithOAuthBearerTokenCredentials(xeroaccountingapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
xeroaccountingapi.WithBasicAuthCredentials(xeroaccountingapi.NewBasicAuthCredentials(username, password))
xeroaccountingapi.WithApiHeaderCredentials(xeroaccountingapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
xeroaccountingapi.WithApiKeyCredentials(xeroaccountingapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `XEROACCOUNTINGAPI_...` variables — e.g.
`XEROACCOUNTINGAPI_USERNAME`, `XEROACCOUNTINGAPI_PASSWORD`, `XEROACCOUNTINGAPI_TOKEN`, `XEROACCOUNTINGAPI_API_KEY`,
`XEROACCOUNTINGAPI_O_AUTH_CLIENT_ID`, `XEROACCOUNTINGAPI_O_AUTH_CLIENT_SECRET`, `XEROACCOUNTINGAPI_ACCESS_TOKEN`. Grep the
