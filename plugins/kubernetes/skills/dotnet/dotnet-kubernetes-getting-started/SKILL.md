---
name: dotnet-kubernetes-getting-started
description: Kubernetes .NET SDK identity and lookup layer for the dotnet-kubernetes-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Kubernetes .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-kubernetes-sdk` helper
...
| --- | --- |
| API | Kubernetes |
| Source repo | https://github.com/context-plugins/kubernetes-csharp-sdk (branch `main`) |
| Root namespace | `Kubernetes` (the `using` namespace) |
| Client class | `KubernetesClient` |
| Options class | `KubernetesClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `KubernetesClientOptions`: `BearerToken: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using Kubernetes.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/kubernetes-csharp-sdk
dotnet add reference kubernetes-csharp-sdk/Kubernetes.csproj
```
...
temp directory** (`<temp>/kubernetes-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/kubernetes-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/kubernetes-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/kubernetes-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\kubernetes-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/kubernetes-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/kubernetes-dotnet-src/`, **never** in the project repo, and its path never goes into
`kubernetes-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `KubernetesClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new KubernetesClient(...)`, build its options, or DI-register via
   `AddKubernetesClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `KubernetesClientOptions` — the map's *Servers & auth* section
