...
> setting, so **confirm it** from the class names under `tesla/apis/`.
...
| Install | `pip install "git+https://github.com/context-plugins/tesla-python-sdk@main"` |
| Client class | `TeslafleetmanagementapiClient` in `tesla/tesla_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `tesla/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Base exception | `APIException` in `tesla/exceptions/api_exception.py` — raised on non-2xx |
...
- `tesla/tesla_client.py` — the `TeslafleetmanagementapiClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `tesla/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `tesla/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `tesla/controllers/` — one `{Resource}Api` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `tesla/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `tesla/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `tesla/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `tesla/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `tesla/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/tesla-python-sdk@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/tesla-python-sdk /tmp/tesla-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/tesla-python-sdk "$env:TEMP\tesla-python-src"
```
...
- `tesla/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `tesla/tesla_client.py` — `TeslafleetmanagementapiClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `tesla/controllers/*.py` — operation method signatures, parameter names, and return types.
- `tesla/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `tesla/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/tesla-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\tesla-python-src"   # Windows
```
