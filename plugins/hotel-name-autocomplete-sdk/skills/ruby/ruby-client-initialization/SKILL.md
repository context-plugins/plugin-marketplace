...
- `HotelNameAutocomplete` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `hotel_name_autocomplete` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'hotel_name_autocomplete'
include HotelNameAutocomplete
...
The authoritative list is the `initialize` signature in `lib/hotel_name_autocomplete/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/hotel_name_autocomplete/configuration.rb`
(`build_default_config_from_env`) and `lib/hotel_name_autocomplete/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/hotel_name_autocomplete.rb`) and store it in a
