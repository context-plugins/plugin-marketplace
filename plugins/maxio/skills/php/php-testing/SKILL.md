...
> `AdvancedBillingClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AdvancedBillingClient
{
...
```php
use AdvancedBillingLib\Exceptions\{OperationException};
use AdvancedBillingLib\Exceptions\ApiException;
...
app()->instance(AdvancedBillingClient::class, clientWithResponses([
