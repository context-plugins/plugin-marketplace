...
kwargs to the `MindbodypushapiapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `mindbody/mindbody_client.py` and `mindbody/http/auth/` in
the cloned source.

## Basic auth

```python
from mindbody.http.auth.basic_auth import BasicAuthCredentials
...
```python
from mindbody.http.auth.api_key import ApiKeyCredentials
...
```python
from mindbody.http.auth.api_header import ApiHeaderCredentials
...
```python
from mindbody.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from mindbody.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from mindbody.http.auth.o_auth_acg import OAuthACGCredentials
from mindbody.models.o_auth_scope_mindbody_enum import OAuthScope{Pkg}Enum
...
client = MindbodypushapiapiClient(o_auth_acg_credentials=creds)
...
```python
from mindbody.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = MindbodypushapiapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `MindbodypushapiapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `mindbody/configuration.py` and each `mindbody/http/auth/*.py` for the exact
