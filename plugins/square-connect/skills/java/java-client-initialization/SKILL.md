...
- `SquareConnectAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.squareup.connect` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`SquareConnectAPIClient.Builder` class. A minimal initialization:

```java
import com.squareup.connect.SquareConnectAPIClient;
import com.squareup.connect.Environment;

SquareConnectAPIClient client = new SquareConnectAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `SquareConnectAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `SquareConnectAPIClient.java` |

Call `.build()` to produce an immutable `SquareConnectAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public SquareConnectAPIClient apiClient() {
        return new SquareConnectAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired SquareConnectAPIClient client) {
        client.shutdown();
    }
}
```

Inject `SquareConnectAPIClient` into your services as a normal Spring dependency. Build only one bean.
