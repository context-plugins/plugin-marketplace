    ...
    postnl.CreateConfiguration(
        postnl.WithHttpConfiguration(
            postnl.CreateHttpConfiguration(
                postnl.WithTimeout(30),
                postnl.WithRetryConfiguration(
                    postnl.CreateRetryConfiguration(
                        postnl.WithMaxRetryAttempts(3),
                        postnl.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := postnl.CreateRetryConfiguration(
    postnl.WithMaxRetryAttempts(5),
    postnl.WithRetryInterval(1),
    postnl.WithBackoffFactor(2),
    postnl.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    postnl.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := postnl.NewClient(
    postnl.CreateConfiguration(
        postnl.WithHttpConfiguration(
            postnl.CreateHttpConfiguration(
                postnl.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
