...
client = stub_client(stubs)
assert_raises ThirdwebEngine::APIException do
...
def build_test_client
  ThirdwebEngine::Client.new(
    environment: ThirdwebEngine::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ThirdwebEngine::APIException
```
...
```ruby
expect { call }.to raise_error(ThirdwebEngine::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ThirdwebEngine::HttpCallBack
...
client  = ThirdwebEngine::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
