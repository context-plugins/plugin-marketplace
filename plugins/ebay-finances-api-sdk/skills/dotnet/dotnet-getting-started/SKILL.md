...
| | |
| --- | --- |
| API | `eBay Finances API` |
| Project | `EBayFinancesApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `EBayFinancesApi` (the `using` namespace) |
| Client class | `EBayFinancesApiClient` |
| Options class | `EBayFinancesApiClientOptions` |
...
namespaces transitively, so `using EBayFinancesApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/ebay-finances-api-csharp
dotnet add reference ebay-finances-api-csharp/EBayFinancesApi.csproj
```

```csharp
using EBayFinancesApi;          // client + options + ServerEnvironment
using EBayFinancesApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/ebay-finances-api-csharp /tmp/ebay-finances-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/ebay-finances-api-csharp "$env:TEMP\ebay-finances-api-dotnet-src"
```
...
- `EBayFinancesApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/ebay-finances-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\ebay-finances-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `EBayFinancesApiClient`, build its
   `EBayFinancesApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
