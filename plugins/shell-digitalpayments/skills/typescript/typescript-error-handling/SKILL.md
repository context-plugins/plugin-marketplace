...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `shell-smartpay-apilib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'shell-smartpay-apilib';
import { ApiError } from 'shell-smartpay-apilib';
import { {Operation}Error } from 'shell-smartpay-apilib/errors';
