---
name: dotnet-greenbyte-getting-started
description: greenbyte .NET SDK identity and lookup layer for the dotnet-greenbyte-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the greenbyte .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-greenbyte-sdk` helper
...
| --- | --- |
| API | greenbyte |
| Source repo | https://github.com/context-plugins/greenbyte-csharp-sdk (branch `main`) |
| Root namespace | `GreenbyteApi` (the `using` namespace) |
| Client class | `GreenbyteApiClient` |
| Options class | `GreenbyteApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `GreenbyteApiClientOptions`: `ApiKeyHeaderAuth: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using GreenbyteApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/greenbyte-csharp-sdk
dotnet add reference greenbyte-csharp-sdk/GreenbyteApi.csproj
```
...
temp directory** (`<temp>/greenbyte-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/greenbyte-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/greenbyte-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/greenbyte-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\greenbyte-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/greenbyte-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/greenbyte-dotnet-src/`, **never** in the project repo, and its path never goes into
`greenbyte-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `GreenbyteApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new GreenbyteApiClient(...)`, build its options, or DI-register via
   `AddGreenbyteApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `GreenbyteApiClientOptions` — the map's *Servers & auth* section
