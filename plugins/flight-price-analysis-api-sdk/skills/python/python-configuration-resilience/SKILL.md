...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `FlightPriceAnalysisAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`flightpriceanalysisapi/configuration.py` in the cloned source.
...
```python
import requests
from flightpriceanalysisapi.configuration import Configuration
...
`flightpriceanalysisapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`flightpriceanalysisapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `flightpriceanalysisapi/configuration.py`
...
`NumberPagedResponse`) live in `flightpriceanalysisapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `flightpriceanalysisapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
