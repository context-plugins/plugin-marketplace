...
`vimeoapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `vimeoapi/exceptions/` and operation docstrings.
...
```python
from vimeoapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `vimeoapi/exceptions/`.
...
```python
from vimeoapi.exceptions.api_exception import APIException
from vimeoapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`vimeoapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`vimeoapi/exceptions/` for the class.
...
`vimeoapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `vimeoapi/exceptions/`
  or a `ValueError` — check the auth handler in `vimeoapi/http/auth/` for the exact type.
