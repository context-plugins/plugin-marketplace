...
> setting, so **confirm it** from the class names under `mindbody/apis/`.
...
| Install | `pip install "git+https://github.com/context-plugins/mindbody-python-sdk@main"` |
| Client class | `MindbodypushapiapiClient` in `mindbody/mindbody_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `mindbody/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `MindbodypushapiapiClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `mindbody/exceptions/api_exception.py` — raised on non-2xx |
...
- `mindbody/mindbody_client.py` — the `MindbodypushapiapiClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `mindbody/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `mindbody/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `mindbody/controllers/` — one `{Resource}Api` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `mindbody/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `mindbody/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `mindbody/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `mindbody/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `mindbody/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/mindbody-python-sdk@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/mindbody-python-sdk /tmp/mindbody-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/mindbody-python-sdk "$env:TEMP\mindbody-python-src"
```
...
- `mindbody/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `mindbody/mindbody_client.py` — `MindbodypushapiapiClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `mindbody/controllers/*.py` — operation method signatures, parameter names, and return types.
- `mindbody/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `mindbody/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/mindbody-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\mindbody-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `MindbodypushapiapiClient(...)`.
