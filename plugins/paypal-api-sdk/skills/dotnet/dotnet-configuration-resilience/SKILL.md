...
All types below live under `PayPalApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using PayPalApi.Core.Configuration;
...
```csharp
using PayPalApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new PayPalApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `PayPalApi.Core.Exceptions`):
...
With DI, the SDK's `AddPayPalApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddPayPalApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
