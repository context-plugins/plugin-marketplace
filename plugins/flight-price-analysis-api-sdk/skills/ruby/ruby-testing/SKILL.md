...
def stub_client(stubs)
  ...
  FlightPriceAnalysisAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises FlightPriceAnalysisAPI::APIException do
...
def build_test_client
  FlightPriceAnalysisAPI::Client.new(
    environment: FlightPriceAnalysisAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error FlightPriceAnalysisAPI::APIException
```
...
```ruby
expect { call }.to raise_error(FlightPriceAnalysisAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < FlightPriceAnalysisAPI::HttpCallBack
...
client  = FlightPriceAnalysisAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
