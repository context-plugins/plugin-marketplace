...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `TeslaAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`teslaapi/configuration.py` in the cloned source.
...
```python
import requests
from teslaapi.configuration import Configuration
...
`teslaapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`teslaapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `teslaapi/configuration.py`
...
`NumberPagedResponse`) live in `teslaapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `teslaapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
