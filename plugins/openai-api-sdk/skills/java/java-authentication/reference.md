...
against the `OpenAIAPIClient.Builder` setters, the `com.openai.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `OpenAIAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
OpenAIAPIClient client = new OpenAIAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeOpenAIAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
