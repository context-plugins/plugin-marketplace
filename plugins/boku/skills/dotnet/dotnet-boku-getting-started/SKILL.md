---
name: dotnet-boku-getting-started
description: boku .NET SDK identity and lookup layer for the dotnet-boku-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the boku .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-boku-sdk` helper
...
| --- | --- |
| API | boku |
| Source repo | https://github.com/context-plugins/boku-csharp-sdk (branch `main`) |
| Root namespace | `BokuDirectPaymentsApi` (the `using` namespace) |
| Client class | `BokuDirectPaymentsApiClient` |
| Options class | `BokuDirectPaymentsApiClientOptions` |
<!-- crawler:auth -->
| Auth | See the credentials properties on `BokuDirectPaymentsApiClientOptions` — the SDK map's *Servers & auth* section lists the scheme(s) |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `MerchantTestEnvironment`, `ProductionEnvironment` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using BokuDirectPaymentsApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/boku-csharp-sdk
dotnet add reference boku-csharp-sdk/BokuDirectPaymentsApi.csproj
```
...
temp directory** (`<temp>/boku-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/boku-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/boku-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/boku-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\boku-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/boku-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/boku-dotnet-src/`, **never** in the project repo, and its path never goes into
`boku-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `BokuDirectPaymentsApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new BokuDirectPaymentsApiClient(...)`, build its options, or DI-register via
   `AddBokuDirectPaymentsApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `BokuDirectPaymentsApiClientOptions` — the map's *Servers & auth* section
