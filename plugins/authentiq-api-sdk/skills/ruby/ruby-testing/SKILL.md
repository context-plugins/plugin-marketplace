...
client = stub_client(stubs)
assert_raises AuthentiqAPI::APIException do
...
def build_test_client
  AuthentiqAPI::Client.new(
    environment: AuthentiqAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error AuthentiqAPI::APIException
```
...
```ruby
expect { call }.to raise_error(AuthentiqAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < AuthentiqAPI::HttpCallBack
...
client  = AuthentiqAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
