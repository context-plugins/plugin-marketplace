...
- `flight-price-analysis-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'flight-price-analysis-apilib';
...
```ts
import { {Resource}Controller } from 'flight-price-analysis-apilib';
