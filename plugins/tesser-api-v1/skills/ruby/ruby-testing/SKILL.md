...
client = stub_client(stubs)
assert_raises TesserAPIV1::APIException do
...
BASE = TesserAPIV1::Configuration.new(environment: TesserAPIV1::Environment::PRODUCTION).get_base_uri

def build_test_client
  TesserAPIV1::Client.new(
    environment: TesserAPIV1::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error TesserAPIV1::APIException
```
...
```ruby
expect { call }.to raise_error(TesserAPIV1::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < TesserAPIV1::HttpCallBack
...
client  = TesserAPIV1::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
