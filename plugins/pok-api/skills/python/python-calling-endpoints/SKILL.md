...
Open `pokapi/pokapi_client.py` for the controller property names, then the relevant
`pokapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from pokapi.models.{request_model} import {RequestModel}
...
Open `pokapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from pokapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from pokapi.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `pokapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `PokapiClient` in `pokapi/pokapi_client.py`.
- Operation method signatures are in `pokapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `pokapi/models/`; enum types have `from_value()`.
