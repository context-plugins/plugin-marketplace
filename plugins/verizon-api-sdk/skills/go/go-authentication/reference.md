...
```go
verizonapi.WithOAuthCCGCredentials(
    verizonapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr verizonapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := verizonapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr verizonapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := verizonapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := verizonapi.NewClient(verizonapi.CreateConfiguration(verizonapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    verizonapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := verizonapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := verizonapi.NewClient(verizonapi.CreateConfiguration(verizonapi.WithOAuthROPCGCredentials(creds)))
...
```go
verizonapi.WithOAuthBearerTokenCredentials(verizonapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
verizonapi.WithBasicAuthCredentials(verizonapi.NewBasicAuthCredentials(username, password))
verizonapi.WithApiHeaderCredentials(verizonapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
verizonapi.WithApiKeyCredentials(verizonapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `VERIZONAPI_...` variables — e.g.
`VERIZONAPI_USERNAME`, `VERIZONAPI_PASSWORD`, `VERIZONAPI_TOKEN`, `VERIZONAPI_API_KEY`,
`VERIZONAPI_O_AUTH_CLIENT_ID`, `VERIZONAPI_O_AUTH_CLIENT_SECRET`, `VERIZONAPI_ACCESS_TOKEN`. Grep the
