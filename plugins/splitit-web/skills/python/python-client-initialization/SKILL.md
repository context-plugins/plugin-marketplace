...
- `splititwebapiv3` — the root package name (e.g. `multiauthsample`, `batester`).
- `SplititWebApiV3Client` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
...
```python
from splititwebapiv3.splititwebapiv3_client import SplititWebApiV3Client
from splititwebapiv3.configuration import Environment
from splititwebapiv3.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from splititwebapiv3.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `splititwebapiv3/splititwebapiv3_client.py` in the cloned source.
...
`from_environment` in `splititwebapiv3/configuration.py` for the exact names.
...
Open `splititwebapiv3/splititwebapiv3_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `splititwebapiv3/configuration.py`. Use the constant for the desired
...
`splititwebapiv3/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
