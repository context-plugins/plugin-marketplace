...
GiteaApiClient client = new GiteaApiClient.Builder()
    .environment(Environment.ENVIRONMENT2)   // switch environments; read the real member names
                                             // from Environment.java (PRODUCTION, ENVIRONMENT2, …)
...
```java
// Read the accessor off GiteaApiClient in the generated source.
...
Confirm `GiteaApiClient.shutdown()` in the generated source.
