...
| | |
|---|---|
| SDK display name | shell-ev |
| Root namespace/module | `ShellEv` |
<!-- gen:stamp -->
| Target framework(s) | `netstandard2.0` (C# `LangVersion 14`, `Nullable enable`) |
| Source commit (spec stamp) | `5906bb8` (`5906bb863416fa5537e3148dddfa644c5e861c70`, tagged `5906bb8`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
| Repo | https://github.com/context-plugins/shell-ev-csharp-sdk (branch `main`) |
...
```csharp
using ShellEv;
using ShellEv.Servers; // ServerEnvironment lives here

var options = new ShellEvClientOptions
{
...
var client = new ShellEvClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddShellEvClient(o =>
{
...
All `ShellEvClientOptions` properties (source: `ShellEvClientOptions.cs`):
...
| `BearerAuthTokenStrategy` | `IOAuth2TokenStrategy<OAuth2ClientCredentials>?` |
...
- `ShellEvClient(HttpClient httpClient, ShellEvClientOptions options)`
<!-- /crawler:client-options -->
...
|---|---|---|
| `ApiError` — abstract base of all 7 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
...
## Operations — by controller (2 groups, 8 operations)
...
|---|---:|---|
| `Charging` | 4 | [map/operations/Charging.md](map/operations/Charging.md) |
...
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 42 | [`Accessibility` … `UnauthorizedError`](map/models/records-1-Ac-Un.md) |
| Unions (`OneOf` / `AnyOf`) — variant factories + `TryGet…` | 2 + 0 | [map/models/unions.md](map/models/unions.md) |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — literal C# member names + wire values | 23 | [map/models/enums.md](map/models/enums.md) |
<!-- /gen:models-table -->
...
|---|---|
| Client & options (root) | `ShellEv` |
| Operation controllers (`Api/`) | `ShellEv.Api` |
| Records (`Models/`) | `ShellEv.Models` |
| Enums (`Models/Enums/`) | `ShellEv.Models.Enums` |
| Unions (`Models/AnyOf/`, `Models/OneOf/`) | `ShellEv.Models.OneOf` |
| Error classes (`Errors/`) | `ShellEv.Errors` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `ShellEvClientOptions` (source: `ShellEvClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:
