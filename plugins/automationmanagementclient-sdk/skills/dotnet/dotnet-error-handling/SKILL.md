...
> `{ApiGroup}`, `AutomationManagementClient`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `AutomationManagementClient.Core.Exceptions`
- `ApiError` **and** `RawError` → `AutomationManagementClient.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `AutomationManagementClient.Errors`
...
```csharp
using AutomationManagementClient.Core.Exceptions;     // SdkException<TError>
using AutomationManagementClient.Core.ErrorResponse;  // ApiError, RawError
using AutomationManagementClient.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`AutomationManagementClient.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using AutomationManagementClient.Core.Exceptions;     // SdkException<TError>
using AutomationManagementClient.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `AutomationManagementClient.Core.ErrorResponse`. Its public
...
`readonly struct` under `AutomationManagementClient.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using AutomationManagementClient.Core.Models;        // ApiResult<TResponse, TError>
using AutomationManagementClient.Core.ErrorResponse; // RawError
using AutomationManagementClient.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `AutomationManagementClient.Core.Exceptions`) — an auth *application* failure, not an API error — when the
