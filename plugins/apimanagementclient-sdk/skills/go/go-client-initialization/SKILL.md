...
- `apimanagementclient` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/apimanagementclient-go` — the Go import path for the root package; `github.com/context-plugins/apimanagementclient-go/models` for request/response types.
...
```go
import "github.com/context-plugins/apimanagementclient-go"

client := apimanagementclient.NewClient(
    apimanagementclient.CreateConfiguration(
        apimanagementclient.WithEnvironment(apimanagementclient.PRODUCTION),
...
from environment variables named `APIMANAGEMENTCLIENT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := apimanagementclient.NewClient(apimanagementclient.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `apimanagementclient.PRODUCTION`,
`apimanagementclient.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := apimanagementclient.NewClient(
    apimanagementclient.CreateConfiguration(apimanagementclient.WithEnvironment(apimanagementclient.PRODUCTION)),
)
```
...
client := apimanagementclient.NewClient(
    apimanagementclient.CreateConfiguration(
        apimanagementclient.WithHttpConfiguration(
            apimanagementclient.CreateHttpConfiguration(
                apimanagementclient.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                apimanagementclient.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = apimanagementclient.NewClient(apimanagementclient.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api apimanagementclient.ClientInterface
}

func NewService(api apimanagementclient.ClientInterface) *Service {
