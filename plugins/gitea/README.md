...
Integrate and consume the **APIMatic-generated Gitea SDK** (the "APIMATIC v3.0" generic
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, Java, PHP, Python, Go.
...
(`dotnet-gitea-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-gitea`** router skill routes all
C#/.NET SDK work to the single **`dotnet-gitea-sdk`** agent (.NET-only), which answers
