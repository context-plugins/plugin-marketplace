...
Open `zalandoshop/zalandoshop_client.py` for the controller property names, then the relevant
`zalandoshop/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from zalandoshop.models.{request_model} import {RequestModel}
...
Open `zalandoshop/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from zalandoshop.models.suite_code_enum import SuiteCodeEnum
...
```python
from zalandoshop.http.http_call_back import HttpCallBack
...
client = ZalandoShopClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `zalandoshop/pagination/` in the source.

## Finding the right method

- Controller property names are on `ZalandoShopClient` in `zalandoshop/zalandoshop_client.py`.
- Operation method signatures are in `zalandoshop/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `zalandoshop/models/`; enum types have `from_value()`.
