...
- `flight-most-traveled-destinationslib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'flight-most-traveled-destinationslib';
...
```ts
import { {Resource}Controller } from 'flight-most-traveled-destinationslib';
