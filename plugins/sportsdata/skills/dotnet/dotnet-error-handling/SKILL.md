...
> `{ApiGroup}`, `Sportsdata`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `Sportsdata.Core.Exceptions`
- `ApiError` **and** `RawError` → `Sportsdata.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `Sportsdata.Errors`
...
```csharp
using Sportsdata.Core.Exceptions;     // SdkException<TError>
using Sportsdata.Core.ErrorResponse;  // ApiError, RawError
using Sportsdata.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`Sportsdata.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using Sportsdata.Core.Exceptions;     // SdkException<TError>
using Sportsdata.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `Sportsdata.Core.ErrorResponse`. Its public
...
`readonly struct` under `Sportsdata.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using Sportsdata.Core.Models;        // ApiResult<TResponse, TError>
using Sportsdata.Core.ErrorResponse; // RawError
using Sportsdata.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `Sportsdata.Core.Exceptions`) — an auth *application* failure, not an API error — when the
