...
- `paypalapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/paypal-api-go` — the Go import path for the root package; `github.com/context-plugins/paypal-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/paypal-api-go"

client := paypalapi.NewClient(
    paypalapi.CreateConfiguration(
        paypalapi.WithEnvironment(paypalapi.PRODUCTION),
...
from environment variables named `PAYPALAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := paypalapi.NewClient(paypalapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `paypalapi.PRODUCTION`,
`paypalapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := paypalapi.NewClient(
    paypalapi.CreateConfiguration(paypalapi.WithEnvironment(paypalapi.PRODUCTION)),
)
```
...
client := paypalapi.NewClient(
    paypalapi.CreateConfiguration(
        paypalapi.WithHttpConfiguration(
            paypalapi.CreateHttpConfiguration(
                paypalapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                paypalapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = paypalapi.NewClient(paypalapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api paypalapi.ClientInterface
}

func NewService(api paypalapi.ClientInterface) *Service {
