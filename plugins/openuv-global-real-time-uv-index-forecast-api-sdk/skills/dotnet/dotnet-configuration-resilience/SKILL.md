...
All types below live under `OpenUvGlobalRealTimeUvIndexForecastApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Configuration;
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new OpenUvGlobalRealTimeUvIndexForecastApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `OpenUvGlobalRealTimeUvIndexForecastApi.Core.Exceptions`):
...
With DI, the SDK's `AddOpenUvGlobalRealTimeUvIndexForecastApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddOpenUvGlobalRealTimeUvIndexForecastApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
