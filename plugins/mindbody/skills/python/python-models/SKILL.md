...
your SDK source (`mindbody/models/`).
...
```python
from mindbody.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `mindbody/utilities/union_type_lookup.py`) and each candidate model
...
```python
from mindbody.models.animal import Cat, Dog
...
```python
from mindbody.models.animal import Animal, Cat, Dog
