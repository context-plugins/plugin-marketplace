...
`apimanagementclient/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `apimanagementclient/exceptions/` and operation docstrings.
...
```python
from apimanagementclient.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `apimanagementclient/exceptions/`.
...
```python
from apimanagementclient.exceptions.api_exception import APIException
from apimanagementclient.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`apimanagementclient/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`apimanagementclient/exceptions/` for the class.
...
`apimanagementclient/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `apimanagementclient/exceptions/`
  or a `ValueError` — check the auth handler in `apimanagementclient/http/auth/` for the exact type.
