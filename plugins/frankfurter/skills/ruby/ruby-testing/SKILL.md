...
client = stub_client(stubs)
assert_raises Frankfurter::APIException do
...
BASE = Frankfurter::Configuration.new(environment: Frankfurter::Environment::PRODUCTION).get_base_uri

def build_test_client
  Frankfurter::Client.new(
    environment: Frankfurter::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Frankfurter::APIException
```
...
```ruby
expect { call }.to raise_error(Frankfurter::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Frankfurter::HttpCallBack
...
client  = Frankfurter::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
