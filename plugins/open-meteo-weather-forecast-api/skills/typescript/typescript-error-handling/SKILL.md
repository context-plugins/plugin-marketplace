...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `open-meteo-weather-forecast-apilib`) — replace it with the concrete identifier from the source.
