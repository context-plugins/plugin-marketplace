...
description: Initialize an APIMatic-generated PHP API client — construct it from a config array (or options object) you supply, choose a server environment or override the base URL, and wire it into a PSR-11 container or plain instantiation. Use the moment you write `new NetworkAdminManagementClientClient(...)`, configure its options, pick an environment, or register the client in a DI container — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the Guzzle lifetime rules or environment constant names.
---
...
- `NetworkAdminManagementClientClient` — the single public client class (e.g. `FooClient`).
- `NetworkAdminManagementClientLib` — the SDK's root namespace used in `use` directives.
...
```php
$client = new NetworkAdminManagementClientClient([
    'timeout'     => 30,
    'environment' => NetworkAdminManagementClientClient::ENVIRONMENT_PRODUCTION,
...
```php
use NetworkAdminManagementClientLib\NetworkAdminManagementClientClient;
...
a new `NetworkAdminManagementClientClient` per API call — Guzzle connection pooling is per-client-instance and per-request
construction leaks connections.

## Choosing the server / base URL

Environments are modeled as class constants on `NetworkAdminManagementClientClient` (e.g.
`NetworkAdminManagementClientClient::ENVIRONMENT_PRODUCTION`, `NetworkAdminManagementClientClient::ENVIRONMENT_SANDBOX`). Pass the constant in
...
$this->app->singleton(NetworkAdminManagementClientClient::class, function ($app) {
    return new NetworkAdminManagementClientClient([
        'timeout'     => 30,
        'environment' => NetworkAdminManagementClientClient::ENVIRONMENT_PRODUCTION,
...
class MyService
{
    public function __construct(private readonly NetworkAdminManagementClientClient $client) {}
