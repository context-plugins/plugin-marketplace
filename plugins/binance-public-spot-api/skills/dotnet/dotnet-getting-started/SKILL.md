...
| Project | `BinancePublicSpotApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using BinancePublicSpotApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/binance-public-spot-api-csharp-sdk
dotnet add reference binance-public-spot-api-csharp-sdk/BinancePublicSpotApi.csproj
```

```csharp
using BinancePublicSpotApi;          // client + options + ServerEnvironment
using BinancePublicSpotApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/binance-public-spot-api-csharp-sdk /tmp/binance-public-spot-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/binance-public-spot-api-csharp-sdk "$env:TEMP\binance-public-spot-api-dotnet-src"
```
...
- `BinancePublicSpotApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/binance-public-spot-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\binance-public-spot-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `BinancePublicSpotApiClient`, build its
   `BinancePublicSpotApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
