...
To see which schemes a specific SDK accepts, read the credential setter methods on `HotelSearchAPIClient.Builder`
...
- Set credentials **on the `HotelSearchAPIClient.Builder`** before calling `.build()` — the client is immutable
