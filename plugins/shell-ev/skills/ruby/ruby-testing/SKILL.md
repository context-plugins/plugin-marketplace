...
client = stub_client(stubs)
assert_raises ShellEv::APIException do
...
BASE = ShellEv::Configuration.new(environment: ShellEv::Environment::PRODUCTION).get_base_uri

def build_test_client
  ShellEv::Client.new(
    environment: ShellEv::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ShellEv::APIException
```
...
```ruby
expect { call }.to raise_error(ShellEv::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ShellEv::HttpCallBack
...
client  = ShellEv::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
