...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddAdyenBalanceControlApiClient extension. Use the moment you write `new AdyenBalanceControlApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `AdyenBalanceControlApiClientOptions` — its options class.
- `AdyenBalanceControlApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public AdyenBalanceControlApiClient(HttpClient httpClient, AdyenBalanceControlApiClientOptions options)
```
...
```csharp
public class AdyenBalanceControlApiClientOptions
{
...
```csharp
using AdyenBalanceControlApi;
using AdyenBalanceControlApi.Servers;

var options = new AdyenBalanceControlApiClientOptions
{
...
var client = new AdyenBalanceControlApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddAdyenBalanceControlApiClient`, which registers the
...
builder.Services.AddAdyenBalanceControlApiClient(options =>
{
...
```csharp
public sealed class MyService(AdyenBalanceControlApiClient client)
{
