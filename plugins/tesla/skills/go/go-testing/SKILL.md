...
`testing` + `testify/require` for reference; substitute the real `tesla` and names.
...
import (
    ...
    "github.com/context-plugins/tesla-go-sdk"   // root package tesla
)
...
func clientReturning(status int, body string) (tesla.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := tesla.NewClient(
        tesla.CreateConfiguration(
            tesla.WithHttpConfiguration(
                tesla.CreateHttpConfiguration(tesla.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/tesla-go-sdk/errors"
...
client := tesla.NewClient(
    tesla.CreateConfiguration(
        tesla.WithHttpConfiguration(
            tesla.CreateHttpConfiguration(tesla.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
