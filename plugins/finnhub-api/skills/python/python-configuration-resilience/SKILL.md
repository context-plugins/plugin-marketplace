...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `FinnhubapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`finnhubapi/configuration.py` in the cloned source.
...
```python
import requests
from finnhubapi.configuration import Configuration
...
`finnhubapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`finnhubapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `finnhubapi/configuration.py`
...
`NumberPagedResponse`) live in `finnhubapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `finnhubapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
