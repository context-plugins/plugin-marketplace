...
To see which schemes a specific SDK accepts, read the credential setter methods on `VerizonAPIClient.Builder`
...
- Set credentials **on the `VerizonAPIClient.Builder`** before calling `.build()` — the client is immutable
