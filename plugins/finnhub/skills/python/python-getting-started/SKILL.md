...
Verified against `finnhub_api/` and `pyproject.toml` of the generated package at version `1.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Finnhub API |
| Distribution name (what you install) | `finnhub-api` — **not on any package index**; installed from source (see *Install*) |
| Import root (what you import) | `finnhub_api` — not the string you install |
| Source repository | https://github.com/context-plugins/finnhub-python-sdk |
| Source branch | `main` |
| Version | `1.0.0` |
| Sync client class | `FinnhubApiClient` (alias `Client`) |
| Async client class | `AsyncFinnhubApiClient` (alias `AsyncClient`) |
| Client construction | **keyword-only**: `base_url` · `timeout` (default `30.0`) · `api_key`, and the transport override — `custom_http_client` on the sync client, `custom_async_http_client` on the async one (the names differ; see step 1) |
| Auth | **API key** in the `token` query parameter — set `api_key` |
| Environments | **no environment enum** — one `base_url` string, defaulting to `https://finnhub.io/api/v1` |
| Base-URL config | `ServerConfig` (`finnhub_api/server/server_config.py`), frozen, `extra="forbid"` |
...
| Surface | 117 operations across 18 controllers · 192 models · 0 unions · 0 enums · 0 per-operation error unions |
...
This SDK is not published to a package index, so there is no `pip install` from PyPI for it. Install it from its repository — <https://github.com/context-plugins/finnhub-python-sdk> — into the same environment your project runs in:

```bash
pip install "finnhub-api @ git+https://github.com/context-plugins/finnhub-python-sdk.git@main"
```
...
Python does not re-export child modules transitively, so `from finnhub_api import models` alone does **not** make enums, error unions, or runtime types reachable. Import each kind of type from the module that owns it.

`finnhub_api/__init__.py` exports exactly 5 names beside the `finnhub_api.models` subpackage it re-exports:

```python
from finnhub_api import (
    AsyncClient,
    AsyncFinnhubApiClient,
    Client,
    FinnhubApiClient,
    ServerConfig,
)
...
| --- | --- |
| Domain models, their `…Dict` companions | `finnhub_api.models` |
| `ApiError` · `RawError` · `ApiResult` · `RequestOptions` · `HttpClient` · `SdkBaseModel` · `UNSET` · `Optional` | `finnhub_api.core` |
| Per-operation error *unions* | `finnhub_api.errors` — *this SDK declares none* |

`finnhub_api.core` re-exports its whole public surface (a curated `__all__`), so import from `…core` rather than from the private modules beneath it (`…core.results`, `…core.exceptions`, `…core.auth.schemes`).

## Environments — there is no environment enum

This SDK has **no environment type and no environment constants**. There is one knob: `base_url`, on `ServerConfig` (`finnhub_api/server/server_config.py`), and its default is **`https://finnhub.io/api/v1`**:
...
- Omitting `base_url` gives you **`https://finnhub.io/api/v1`**, silently. A caller who believes they configured a different host and did not gets that host's behaviour with live credentials.
...
An API key sent as the `token` query parameter, exposed as the client's `api_key=` keyword taking a plain string.

```python
from finnhub_api import Client
...
| --- | --- | --- | --- |
| `client.bond` | `Bond` / `AsyncBond` | 4 | `bond_price` · `bond_profile` · `bond_tick` · `bond_yield_curve` |
| `client.calendar` | `Calendar` / `AsyncCalendar` | 3 | `earnings_calendar` · `economic_calendar` · `ipo_calendar` |
| `client.corporate_actions` | `CorporateActions` / `AsyncCorporateActions` | 2 | `isin_change` · `symbol_change` |
| `client.crypto` | `Crypto` / `AsyncCrypto` | 4 | `crypto_candles` · `crypto_exchanges` · `crypto_profile` · `crypto_symbols` |
| `client.economic` | `Economic` / `AsyncEconomic` | 2 | `economic_code` · `economic_data` |
| `client.etf` | `Etf` / `AsyncEtf` | 5 | `etfs_allocation` · `etfs_country_exposure` · `etfs_holdings` · `etfs_profile` · `etfs_sector_exposure` |
| `client.forex` | `Forex` / `AsyncForex` | 4 | `forex_candles` · `forex_exchanges` · `forex_rates` · `forex_symbols` |
| `client.global_filings` | `GlobalFilings` / `AsyncGlobalFilings` | 4 | `global_filings_download` · `global_filings_search` · `global_filings_search_filter` · `search_in_filing` |
| `client.index` | `Index` / `AsyncIndex` | 2 | `indices_constituents` · `indices_historical_constituents` |
| `client.institutional` | `Institutional` / `AsyncInstitutional` | 3 | `institutional_ownership` · `institutional_portfolio` · `institutional_profile` |
| `client.misc` | `Misc` / `AsyncMisc` | 10 | `ai_chat` · `airline_price_index` · `bank_branch` · `country` · `covid_19` · `fda_committee_meeting_calendar` · … |
| `client.mutual_fund` | `MutualFund` / `AsyncMutualFund` | 6 | `mutual_fund_country_exposure` · `mutual_fund_eet` · `mutual_fund_eet_pai` · `mutual_fund_holdings` · `mutual_fund_profile` · `mutual_fund_sector_exposure` |
| `client.news` | `News` / `AsyncNews` | 4 | `company_news` · `market_news` · `news_sentiment` · `press_releases` |
| `client.scan` | `Scan` / `AsyncScan` | 3 | `aggregate_indicator` · `pattern_recognition` · `support_resistance` |
| `client.stock_estimates` | `StockEstimates` / `AsyncStockEstimates` | 18 | `company_capex_estimates` · `company_dps_estimates` · `company_earnings` · `company_earnings_quality_score` · `company_ebit_estimates` · `company_ebitda_estimates` · … |
| `client.stock_fundamentals` | `StockFundamentals` / `AsyncStockFundamentals` | 18 | `company_basic_financials` · `company_esg_score` · `company_executive` · `company_historical_esg_score` · `company_profile` · `company_profile2` · … |
| `client.stock_ownership` | `StockOwnership` / `AsyncStockOwnership` | 12 | `congressional_trading` · `fund_ownership` · `insider_sentiment` · `insider_transactions` · `investment_themes` · `ownership` · … |
| `client.stock_prices` | `StockPrices` / `AsyncStockPrices` | 13 | `company_peers` · `historical_market_cap` · `market_holiday` · `market_status` · `price_metrics` · `stock_basic_dividends` · … |
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `finnhub_api/` source directory, and these two entries:
...
**Installing the distribution does not give you the map.** `pyproject.toml` ships the `finnhub_api/` package only, so `sdk-map.md` and `map/operations/` are absent from the installed package — they live in the SDK's own source tree, at the root of its repository, <https://github.com/context-plugins/finnhub-python-sdk>. One clone therefore brings the map and the code it describes together, in lockstep by construction. Clone it to a temporary directory, outside the project repo:

```bash
git clone --depth 1 --branch main https://github.com/context-plugins/finnhub-python-sdk
```
...
Every `Source` path on the map is relative to that SDK root, so `finnhub_api/models/aichat_body.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
```bash
python -c "import finnhub_api, pathlib; print(pathlib.Path(finnhub_api.__file__).parent)"
```

Failing that, it is under the project's environment (`.venv/Lib/site-packages/finnhub_api` on Windows, `.venv/lib/python3.*/site-packages/finnhub_api` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
...
4. **Models** — load **python-models** the moment a request/response member is not a plain string or number. (*The signature won't tell you:* `Optional[T]` here is `T | UnsetType`, **not** `typing.Optional` — `None` is not a legal value for it; models are frozen pydantic instances with `…Dict` TypedDict companions; wire aliases differ from Python member names; unknown response fields are **preserved**, not dropped; and serialize via `to_dict`/`to_json`.)
...
3. **The 1 operation that returns `None`** — `global_filings.global_filings_download`. Its raw peer is `ApiResult[None, …]`, so `with_raw_response` is the only way to observe the status code.
