...
`restapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `restapi/exceptions/` and operation docstrings.
...
```python
from restapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `restapi/exceptions/`.
...
```python
from restapi.exceptions.api_exception import APIException
from restapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`restapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`restapi/exceptions/` for the class.
...
`restapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `restapi/exceptions/`
  or a `ValueError` — check the auth handler in `restapi/http/auth/` for the exact type.
