...
```ruby
client = HotelNameAutocomplete::Client.new(
  max_retries:    3,
...
```ruby
client = HotelNameAutocomplete::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = HotelNameAutocomplete::Client.new(adapter: :net_http)
```
...
client = HotelNameAutocomplete::Client.new(connection: conn)
```
...
client = HotelNameAutocomplete::Client.new(environment: Environment::PRODUCTION)
client = HotelNameAutocomplete::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = HotelNameAutocomplete::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = HotelNameAutocomplete::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = HotelNameAutocomplete::ProxySettings.from_env
client = HotelNameAutocomplete::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < HotelNameAutocomplete::HttpCallBack
...
client = HotelNameAutocomplete::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = HotelNameAutocomplete::Client.from_env
# Overrides take precedence over env vars:
client = HotelNameAutocomplete::Client.from_env(timeout: 30, max_retries: 3)
```
