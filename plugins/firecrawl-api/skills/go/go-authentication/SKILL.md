...
> Throughout, `firecrawlapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := firecrawlapi.NewClient(
    firecrawlapi.CreateConfiguration(
        firecrawlapi.WithBasicAuthCredentials(
            firecrawlapi.NewBasicAuthCredentials(
...
firecrawlapi.WithApiKeyCredentials(firecrawlapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
firecrawlapi.WithApiHeaderCredentials(firecrawlapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
firecrawlapi.WithOAuthBearerTokenCredentials(
    firecrawlapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := firecrawlapi.NewClient(
    firecrawlapi.CreateConfiguration(
        firecrawlapi.WithOAuthCCGCredentials(
            firecrawlapi.NewOAuthCCGCredentials(
