...
`travelrecommendationsapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `travelrecommendationsapi/exceptions/` and operation docstrings.
...
```python
from travelrecommendationsapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `travelrecommendationsapi/exceptions/`.
...
```python
from travelrecommendationsapi.exceptions.api_exception import APIException
from travelrecommendationsapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`travelrecommendationsapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`travelrecommendationsapi/exceptions/` for the class.
...
`travelrecommendationsapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `travelrecommendationsapi/exceptions/`
  or a `ValueError` — check the auth handler in `travelrecommendationsapi/http/auth/` for the exact type.
