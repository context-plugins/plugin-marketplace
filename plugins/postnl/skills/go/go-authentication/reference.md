...
```go
postnl.WithOAuthCCGCredentials(
    postnl.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr postnl.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := postnl.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr postnl.OAuthCCGManager) models.OAuthToken {
...
```go
creds := postnl.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := postnl.NewClient(postnl.CreateConfiguration(postnl.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    postnl.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := postnl.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := postnl.NewClient(postnl.CreateConfiguration(postnl.WithOAuthROPCGCredentials(creds)))
...
```go
postnl.WithOAuthBearerTokenCredentials(postnl.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
postnl.WithBasicAuthCredentials(postnl.NewBasicAuthCredentials(username, password))
postnl.WithApiHeaderCredentials(postnl.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
postnl.WithApiKeyCredentials(postnl.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `POSTNL_...` variables — e.g.
`POSTNL_USERNAME`, `POSTNL_PASSWORD`, `POSTNL_TOKEN`, `POSTNL_API_KEY`,
`POSTNL_O_AUTH_CLIENT_ID`, `POSTNL_O_AUTH_CLIENT_SECRET`, `POSTNL_ACCESS_TOKEN`. Grep the
