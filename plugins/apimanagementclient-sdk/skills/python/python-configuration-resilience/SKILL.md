...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ApiManagementClientClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`apimanagementclient/configuration.py` in the cloned source.
...
```python
import requests
from apimanagementclient.configuration import Configuration
...
`apimanagementclient/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`apimanagementclient/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `apimanagementclient/configuration.py`
...
`NumberPagedResponse`) live in `apimanagementclient/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `apimanagementclient/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
