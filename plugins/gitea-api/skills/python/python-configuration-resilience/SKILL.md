...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `GiteaapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`giteaapi/configuration.py` in the cloned source.
...
```python
import requests
from giteaapi.configuration import Configuration
...
`giteaapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`giteaapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `giteaapi/configuration.py`
...
`NumberPagedResponse`) live in `giteaapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `giteaapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:

```python
import logging
from giteaapi.http.http_call_back import HttpCallBack
from giteaapi.configuration import Configuration
