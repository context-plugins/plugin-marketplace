...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddPayPalApiClient extension. Use the moment you write `new PayPalApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `PayPalApiClientOptions` — its options class.
- `PayPalApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public PayPalApiClient(HttpClient httpClient, PayPalApiClientOptions options)
```
...
```csharp
public class PayPalApiClientOptions
{
...
```csharp
using PayPalApi;
using PayPalApi.Servers;

var options = new PayPalApiClientOptions
{
...
var client = new PayPalApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddPayPalApiClient`, which registers the
...
builder.Services.AddPayPalApiClient(options =>
{
...
```csharp
public sealed class MyService(PayPalApiClient client)
{
