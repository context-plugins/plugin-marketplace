...
- `Paypal` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `paypal` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/paypal/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/paypal/configuration.rb`
(`build_default_config_from_env`) and `lib/paypal/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = Paypal::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/paypal.rb`) and store it in a
