...
kwargs to the `Pqapiv2Client` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `payquicker/payquicker_client.py` and `payquicker/http/auth/` in
the cloned source.

## Basic auth

```python
from payquicker.http.auth.basic_auth import BasicAuthCredentials
...
```python
from payquicker.http.auth.api_key import ApiKeyCredentials
...
```python
from payquicker.http.auth.api_header import ApiHeaderCredentials
...
```python
from payquicker.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from payquicker.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from payquicker.http.auth.o_auth_acg import OAuthACGCredentials
from payquicker.models.o_auth_scope_payquicker_enum import OAuthScope{Pkg}Enum
...
client = Pqapiv2Client(o_auth_acg_credentials=creds)
...
```python
from payquicker.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = Pqapiv2Client(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `Pqapiv2Client.from_environment()`) read each credential
...
Grep `from_environment` in `payquicker/configuration.py` and each `payquicker/http/auth/*.py` for the exact
