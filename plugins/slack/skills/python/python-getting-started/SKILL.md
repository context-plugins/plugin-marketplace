...
description: "Slack Python SDK identity and lookup layer (Python only) — install, import root, base URL/environments, the auth pattern, the SDK map that ships at the SDK root (`sdk-map.md` + `map/operations/`) and how to traverse it, and the module table naming the one file owning each fact the map leaves to the source. Load this before answering any Slack Python SDK contract question or writing any SDK code."
---

# Getting started with the Slack Python SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Slack Python SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source modules it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `python-integrate-slack` for the workflow that wraps this skill.
...
Verified against `slack/` and `pyproject.toml` of the generated package at version `1.7.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Slack |
| Distribution name (what you install) | `slack` — **not on any package index**; installed from source (see *Install*) |
| Import root (what you import) | `slack` — note the underscores; the two names differ |
| Source repository | https://github.com/context-plugins/slack-python-sdk |
| Source branch | `main` |
| Version | `1.7.0` |
| Sync client class | `SlackClient` (alias `Client`) |
| Async client class | `AsyncSlackClient` (alias `AsyncClient`) |
| Client construction | **keyword-only**: `environment` · `base_url` · `timeout` (default `30.0`) · `slack_auth`, and the transport override — `custom_http_client` on the sync client, `custom_async_http_client` on the async one (the names differ; see step 1) |
...
| Base-URL config | `ServerConfig` (`slack/server/server_config.py`), frozen, `extra="forbid"` |
...
| Surface | 253 operations across 56 controllers · 357 models · 89 enums · 0 per-operation error unions |
...
This SDK is not published to a package index, so there is no `pip install` from PyPI for it. Install it from its repository — <https://github.com/context-plugins/slack-python-sdk> — into the same environment your project runs in:

```bash
pip install "slack @ git+https://github.com/context-plugins/slack-python-sdk.git@main"
```
...
Python does not re-export child modules transitively, so `from slack import models` alone does **not** make enums, error unions, or runtime types reachable. Import each kind of type from the module that owns it.

`slack/__init__.py` exports exactly 6 names:
...
| --- | --- |
| Domain models, their `…Dict` companions | `slack.models` |
| Enums (and their open `…OrStr` aliases) | `slack.models.enums` |
| `ApiError` · `RawError` · `ApiResult` · `RequestOptions` · `AuthorizationCodeCredentials` · `HttpClient` · `SdkBaseModel` · `UNSET` · `Optional` | `slack.core` |
| Per-operation error *unions* | `slack.errors` — *this SDK declares none* |

`slack.core` re-exports its whole public surface (a curated `__all__`), so import from `…core` rather than from the private modules beneath it (`…core.results`, `…core.exceptions`, `…core.auth.schemes`).
...
from slack.core import AuthorizationCodeCredentials

client = Client(slack_auth=AuthorizationCodeCredentials(client_id="…", client_secret="…", redirect_uri="…"))
client = Client(slack_auth={"client_id": "…", "client_secret": "…", "redirect_uri": "…"})   # equivalent
```
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `slack/` source directory, and these two entries:
...
Every `Source` path on the map is relative to that SDK root, so `slack/models/apimethodusers_get_presence.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
```bash
python -c "import slack, pathlib; print(pathlib.Path(slack.__file__).parent)"
```

Failing that, it is under the project's environment (`.venv/Lib/site-packages/slack` on Windows, `.venv/lib/python3.*/site-packages/slack` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
