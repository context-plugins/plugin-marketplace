...
`sqlmanagementclient/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `sqlmanagementclient/exceptions/` and operation docstrings.
...
```python
from sqlmanagementclient.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `sqlmanagementclient/exceptions/`.
...
```python
from sqlmanagementclient.exceptions.api_exception import APIException
from sqlmanagementclient.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`sqlmanagementclient/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`sqlmanagementclient/exceptions/` for the class.
...
`sqlmanagementclient/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `sqlmanagementclient/exceptions/`
  or a `ValueError` — check the auth handler in `sqlmanagementclient/http/auth/` for the exact type.
