...
kwargs to the `ShellevClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `shellev/shellev_client.py` and `shellev/http/auth/` in
the cloned source.

## Basic auth

```python
from shellev.http.auth.basic_auth import BasicAuthCredentials
...
```python
from shellev.http.auth.api_key import ApiKeyCredentials
...
```python
from shellev.http.auth.api_header import ApiHeaderCredentials
...
```python
from shellev.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from shellev.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from shellev.http.auth.o_auth_acg import OAuthACGCredentials
from shellev.models.o_auth_scope_shellev_enum import OAuthScope{Pkg}Enum
...
client = ShellevClient(o_auth_acg_credentials=creds)
...
```python
from shellev.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ShellevClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ShellevClient.from_environment()`) read each credential
...
Grep `from_environment` in `shellev/configuration.py` and each `shellev/http/auth/*.py` for the exact
