...
`testing` + `testify/require` for reference; substitute the real `shutterstock` and names.
...
import (
    ...
    "github.com/context-plugins/shutterstock-go-sdk"   // root package shutterstock
)
...
func clientReturning(status int, body string) (shutterstock.ClientInterface, *stubTransport) {
    ...
    client := shutterstock.NewClient(
        shutterstock.CreateConfiguration(
            shutterstock.WithHttpConfiguration(
                shutterstock.CreateHttpConfiguration(shutterstock.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/shutterstock-go-sdk/errors"
...
client := shutterstock.NewClient(
    shutterstock.CreateConfiguration(
        shutterstock.WithHttpConfiguration(
            shutterstock.CreateHttpConfiguration(shutterstock.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
