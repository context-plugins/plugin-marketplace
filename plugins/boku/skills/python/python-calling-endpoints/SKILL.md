...
Open `boku/boku_client.py` for the controller property names, then the relevant
`boku/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from boku.models.{request_model} import {RequestModel}
...
Open `boku/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from boku.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `boku/pagination/` in the source.
