...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddTeslaApiClient extension. Use the moment you write `new TeslaApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `TeslaApiClientOptions` — its options class.
- `TeslaApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public TeslaApiClient(HttpClient httpClient, TeslaApiClientOptions options)
```
...
```csharp
public class TeslaApiClientOptions
{
...
```csharp
using TeslaApi;
using TeslaApi.Servers;

var options = new TeslaApiClientOptions
{
...
var client = new TeslaApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddTeslaApiClient`, which registers the
...
builder.Services.AddTeslaApiClient(options =>
{
...
```csharp
public sealed class MyService(TeslaApiClient client)
{
