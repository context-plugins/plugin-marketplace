...
`ShutterstockApiExplorerClient`/`ShutterstockApiExplorerClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `ShutterstockApiExplorerClient`,
...
static ShutterstockApiExplorerClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new ShutterstockApiExplorerClient(new HttpClient(handler), new ShutterstockApiExplorerClientOptions { /* auth not needed for stubs */ });
}
```
...
using ShutterstockApiExplorer.Errors;              // {Operation}Error types
...
using ShutterstockApiExplorer.Core.ErrorResponse; // RawError (Case B)
...
var client = new ShutterstockApiExplorerClient(new HttpClient(handler), new ShutterstockApiExplorerClientOptions());
...
- For DI-based code, the SDK's `AddShutterstockApiExplorerClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `ShutterstockApiExplorerClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<ShutterstockApiExplorerClient>();
  ```
