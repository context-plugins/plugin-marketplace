...
| --- | --- |
| API | `Vimeo` |
| Project | `Vimeo.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using Vimeo.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/vimeo-csharp
dotnet add reference vimeo-csharp/Vimeo.csproj
```

```csharp
using Vimeo;          // client + options + ServerEnvironment
using Vimeo.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-csharp /tmp/vimeo-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-csharp "$env:TEMP\vimeo-dotnet-src"
```
...
- `VimeoClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/vimeo-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\vimeo-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `VimeoClient`, build its
   `VimeoClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
