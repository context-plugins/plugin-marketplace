...
`FlightCreateOrdersClient`/`FlightCreateOrdersClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `FlightCreateOrdersClient`,
...
static FlightCreateOrdersClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new FlightCreateOrdersClient(new HttpClient(handler), new FlightCreateOrdersClientOptions { /* auth not needed for stubs */ });
}
```
...
using FlightCreateOrders.Errors;              // {Operation}Error types
...
```csharp
using FlightCreateOrders.Core.Exceptions;
using FlightCreateOrders.Core.ErrorResponse;
...
using FlightCreateOrders.Core.ErrorResponse; // RawError (Case B)
...
var client = new FlightCreateOrdersClient(new HttpClient(handler), new FlightCreateOrdersClientOptions());
...
- For DI-based code, the SDK's `AddFlightCreateOrdersClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `FlightCreateOrdersClient` from the provider:
  ```csharp
  services.AddFlightCreateOrdersClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<FlightCreateOrdersClient>();
  ```
