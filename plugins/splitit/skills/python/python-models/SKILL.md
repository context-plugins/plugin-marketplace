...
your SDK source (`splitit/models/`).
...
```python
from splitit.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `splitit/utilities/union_type_lookup.py`) and each candidate model
...
```python
from splitit.models.animal import Cat, Dog
...
```python
from splitit.models.animal import Animal, Cat, Dog
