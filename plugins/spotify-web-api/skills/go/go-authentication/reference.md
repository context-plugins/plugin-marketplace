...
```go
spotifywebapi.WithOAuthCCGCredentials(
    spotifywebapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr spotifywebapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := spotifywebapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr spotifywebapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := spotifywebapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := spotifywebapi.NewClient(spotifywebapi.CreateConfiguration(spotifywebapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    spotifywebapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := spotifywebapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := spotifywebapi.NewClient(spotifywebapi.CreateConfiguration(spotifywebapi.WithOAuthROPCGCredentials(creds)))
...
```go
spotifywebapi.WithOAuthBearerTokenCredentials(spotifywebapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
spotifywebapi.WithBasicAuthCredentials(spotifywebapi.NewBasicAuthCredentials(username, password))
spotifywebapi.WithApiHeaderCredentials(spotifywebapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
spotifywebapi.WithApiKeyCredentials(spotifywebapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SPOTIFYWEBAPI_...` variables — e.g.
`SPOTIFYWEBAPI_USERNAME`, `SPOTIFYWEBAPI_PASSWORD`, `SPOTIFYWEBAPI_TOKEN`, `SPOTIFYWEBAPI_API_KEY`,
`SPOTIFYWEBAPI_O_AUTH_CLIENT_ID`, `SPOTIFYWEBAPI_O_AUTH_CLIENT_SECRET`, `SPOTIFYWEBAPI_ACCESS_TOKEN`. Grep the
