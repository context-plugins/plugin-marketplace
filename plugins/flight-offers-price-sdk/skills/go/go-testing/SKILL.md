...
`testing` + `testify/require` for reference; substitute the real `flightoffersprice` and names.
...
import (
    ...
    "github.com/context-plugins/flight-offers-price-go"   // root package flightoffersprice
)
...
func clientReturning(status int, body string) (flightoffersprice.ClientInterface, *stubTransport) {
    ...
    client := flightoffersprice.NewClient(
        flightoffersprice.CreateConfiguration(
            flightoffersprice.WithHttpConfiguration(
                flightoffersprice.CreateHttpConfiguration(flightoffersprice.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/flight-offers-price-go/errors"
...
client := flightoffersprice.NewClient(
    flightoffersprice.CreateConfiguration(
        flightoffersprice.WithHttpConfiguration(
            flightoffersprice.CreateHttpConfiguration(flightoffersprice.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
