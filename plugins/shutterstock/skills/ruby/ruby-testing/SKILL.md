...
client = stub_client(stubs)
assert_raises Shutterstock::APIException do
...
BASE = Shutterstock::Configuration.new(environment: Shutterstock::Environment::PRODUCTION).get_base_uri

def build_test_client
  Shutterstock::Client.new(
    environment: Shutterstock::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Shutterstock::APIException
```
...
```ruby
expect { call }.to raise_error(Shutterstock::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Shutterstock::HttpCallBack
...
client  = Shutterstock::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
