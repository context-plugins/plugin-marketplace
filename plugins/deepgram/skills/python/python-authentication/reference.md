...
kwarg names are generated per-API — confirm from `restapi/restapi_client.py` and `restapi/http/auth/` in
the cloned source.

## Basic auth

```python
from restapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from restapi.http.auth.api_key import ApiKeyCredentials
...
```python
from restapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from restapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from restapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from restapi.http.auth.o_auth_acg import OAuthACGCredentials
from restapi.models.o_auth_scope_restapi_enum import OAuthScope{Pkg}Enum
...
```python
from restapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `restapi/configuration.py` and each `restapi/http/auth/*.py` for the exact
