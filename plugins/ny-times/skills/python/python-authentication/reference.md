...
kwargs to the `NytimesClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `nytimes/nytimes_client.py` and `nytimes/http/auth/` in
the cloned source.

## Basic auth

```python
from nytimes.http.auth.basic_auth import BasicAuthCredentials
...
```python
from nytimes.http.auth.api_key import ApiKeyCredentials
...
```python
from nytimes.http.auth.api_header import ApiHeaderCredentials
...
```python
from nytimes.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from nytimes.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from nytimes.http.auth.o_auth_acg import OAuthACGCredentials
from nytimes.models.o_auth_scope_nytimes_enum import OAuthScope{Pkg}Enum
...
client = NytimesClient(o_auth_acg_credentials=creds)
...
```python
from nytimes.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = NytimesClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `NytimesClient.from_environment()`) read each credential
...
Grep `from_environment` in `nytimes/configuration.py` and each `nytimes/http/auth/*.py` for the exact
