...
Open `ipgeolocationapi/ipgeolocationapi_client.py` for the controller property names, then the relevant
`ipgeolocationapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from ipgeolocationapi.models.{request_model} import {RequestModel}
...
Open `ipgeolocationapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from ipgeolocationapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from ipgeolocationapi.http.http_call_back import HttpCallBack
...
client = IPGeolocationAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `ipgeolocationapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `IPGeolocationAPIClient` in `ipgeolocationapi/ipgeolocationapi_client.py`.
- Operation method signatures are in `ipgeolocationapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `ipgeolocationapi/models/`; enum types have `from_value()`.
