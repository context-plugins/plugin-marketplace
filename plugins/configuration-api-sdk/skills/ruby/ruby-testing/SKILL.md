...
client = stub_client(stubs)
assert_raises ConfigurationAPI::APIException do
...
def build_test_client
  ConfigurationAPI::Client.new(
    environment: ConfigurationAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ConfigurationAPI::APIException
```
...
```ruby
expect { call }.to raise_error(ConfigurationAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ConfigurationAPI::HttpCallBack
...
client  = ConfigurationAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
