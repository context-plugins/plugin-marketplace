...
reference; substitute your `shutterstock` and `ShutterstockapiexplorerClient` names.
...
- **Get the stub URL from the SDK, not from this page.** Every environment resolves to a real remote
  host — read the `environments` dict in `shutterstock/configuration.py`, or call
