...
> Throughout, `sqlmanagementclient` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := sqlmanagementclient.NewClient(
    sqlmanagementclient.CreateConfiguration(
        sqlmanagementclient.WithBasicAuthCredentials(
            sqlmanagementclient.NewBasicAuthCredentials(
...
sqlmanagementclient.WithApiKeyCredentials(sqlmanagementclient.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
sqlmanagementclient.WithApiHeaderCredentials(sqlmanagementclient.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
sqlmanagementclient.WithOAuthBearerTokenCredentials(
    sqlmanagementclient.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := sqlmanagementclient.NewClient(
    sqlmanagementclient.CreateConfiguration(
        sqlmanagementclient.WithOAuthCCGCredentials(
            sqlmanagementclient.NewOAuthCCGCredentials(
