...
> `SqlManagementClientClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SqlManagementClientClient
{
...
```php
use SqlManagementClientLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SqlManagementClientLib\Exceptions\{OperationException};
use SqlManagementClientLib\Exceptions\ApiException;
...
app()->instance(SqlManagementClientClient::class, clientWithResponses([
