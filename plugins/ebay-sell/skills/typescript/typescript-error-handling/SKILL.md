...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `ebay-selllib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'ebay-selllib';
import { ApiError } from 'ebay-selllib';
import { {Operation}Error } from 'ebay-selllib/errors';
