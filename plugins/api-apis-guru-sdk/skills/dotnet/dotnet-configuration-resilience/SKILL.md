...
All types below live under `SafePlace.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using SafePlace.Core.Configuration;
...
```csharp
using SafePlace.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new SafePlaceClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `SafePlace.Core.Exceptions`):
...
With DI, the SDK's `AddSafePlaceClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddSafePlaceClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
