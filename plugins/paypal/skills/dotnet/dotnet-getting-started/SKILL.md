...
> **Who this skill is for.** This is the **map layer**: load it to ground every Paypal SDK fact. `dotnet-integrate-paypal` is the workflow that drives it — you collect the contracts for the work in scope into `paypal-plan.md`, implement from that sheet, and come back to the one map page a row cites for any fact the sheet is missing. Every contract fact comes from here, never from memory.
...
| Environments | `ServerEnvironment.Production` *(default)* → `https://api-m.sandbox.paypal.com` |
| Log environment variable | `PAYPALCLIENT_LOG` |
...
The SDK splits its public types across **separate child namespaces**. C# does **not** import child namespaces transitively, so `using Paypal.Models;` alone does **not** make enums, union types, or error types visible — you get `CS0103`/`CS0246` ("name/type does not exist") on build. Add a separate `using` for each kind of type you reference:

| Type | Namespace |
| --- | --- |
| `PaypalClient` | `Paypal` |
| `PaypalClientOptions` | `Paypal` |
| `ServerEnvironment` | `Paypal.Servers` |
...
The clone lives in `<temp>/paypal-sdk-src/`, **never** in the project repo, and its path never goes into `paypal-plan.md` — the plan must stay portable, and the clone is a session-local reference, not part of the project.
...
The generator injects an `Idempotency-Key: Guid.NewGuid()` header on **every non-GET operation** — fresh on every call, invisible to you, and **not** an idempotency key in any meaningful sense: a value that changes per call deduplicates nothing, and Paypal may not document reading that header at all. `dotnet-configuration-resilience` explains why a visible header like this is worse than an absent one.
