...
│   └── com.squareup.connect.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.squareup.connect.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.squareup.connect.http.client.HttpContext`, which carries:
