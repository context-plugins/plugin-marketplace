...
| --- | --- |
| API | `Tesla API` |
| Project | `TeslaApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `TeslaApi` (the `using` namespace) |
| Client class | `TeslaApiClient` |
| Options class | `TeslaApiClientOptions` |
...
namespaces transitively, so `using TeslaApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/tesla-api-csharp
dotnet add reference tesla-api-csharp/TeslaApi.csproj
```

```csharp
using TeslaApi;          // client + options + ServerEnvironment
using TeslaApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/tesla-api-csharp /tmp/tesla-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/tesla-api-csharp "$env:TEMP\tesla-api-dotnet-src"
```
...
- `TeslaApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/tesla-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\tesla-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `TeslaApiClient`, build its
   `TeslaApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
