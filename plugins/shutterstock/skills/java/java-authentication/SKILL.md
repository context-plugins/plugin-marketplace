...
To see which schemes a specific SDK accepts, read the credential setter methods on `ShutterstockApiExplorerClient.Builder`
...
- Set credentials **on the `ShutterstockApiExplorerClient.Builder`** before calling `.build()` — the client is immutable
