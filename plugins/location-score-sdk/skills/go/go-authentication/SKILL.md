...
> Throughout, `locationscore` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := locationscore.NewClient(
    locationscore.CreateConfiguration(
        locationscore.WithBasicAuthCredentials(
            locationscore.NewBasicAuthCredentials(
...
locationscore.WithApiKeyCredentials(locationscore.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
locationscore.WithApiHeaderCredentials(locationscore.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
locationscore.WithOAuthBearerTokenCredentials(
    locationscore.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := locationscore.NewClient(
    locationscore.CreateConfiguration(
        locationscore.WithOAuthCCGCredentials(
            locationscore.NewOAuthCCGCredentials(
