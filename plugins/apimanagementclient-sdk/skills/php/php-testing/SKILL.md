...
> `ApiManagementClientClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ApiManagementClientClient
{
...
```php
use ApiManagementClientLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use ApiManagementClientLib\Exceptions\{OperationException};
use ApiManagementClientLib\Exceptions\ApiException;
...
app()->instance(ApiManagementClientClient::class, clientWithResponses([
