...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new AdobeExperienceManagerAEMAPIClient({
...
```typescript
import { {Operation}Error } from 'adobe-experience-manager-aem-apilib/errors';
...
```typescript
import { ApiError } from 'adobe-experience-manager-aem-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AdobeExperienceManagerAEMAPIClient)
    .useValue(new AdobeExperienceManagerAEMAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
