...
Confirm every name against `lib/adafruit_io_rest_api/client.rb`, `lib/adafruit_io_rest_api/configuration.rb`, the
`lib/adafruit_io_rest_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
