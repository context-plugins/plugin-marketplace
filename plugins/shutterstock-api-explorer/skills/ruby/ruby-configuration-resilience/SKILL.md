...
```ruby
client = ShutterstockAPIExplorer::Client.new(
  max_retries:    3,
...
```ruby
client = ShutterstockAPIExplorer::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ShutterstockAPIExplorer::Client.new(adapter: :net_http)
```
...
client = ShutterstockAPIExplorer::Client.new(connection: conn)
```
...
client = ShutterstockAPIExplorer::Client.new(environment: Environment::PRODUCTION)
client = ShutterstockAPIExplorer::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ShutterstockAPIExplorer::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ShutterstockAPIExplorer::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ShutterstockAPIExplorer::ProxySettings.from_env
client = ShutterstockAPIExplorer::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ShutterstockAPIExplorer::HttpCallBack
...
client = ShutterstockAPIExplorer::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ShutterstockAPIExplorer::Client.from_env
# Overrides take precedence over env vars:
client = ShutterstockAPIExplorer::Client.from_env(timeout: 30, max_retries: 3)
```
