...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ShellevClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`shellev/configuration.py` in the cloned source.
...
`shellev/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`shellev/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `shellev/configuration.py`
...
`NumberPagedResponse`) live in `shellev/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `shellev/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
