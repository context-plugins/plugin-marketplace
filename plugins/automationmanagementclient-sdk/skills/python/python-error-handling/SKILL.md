...
`automationmanagementclient/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `automationmanagementclient/exceptions/` and operation docstrings.
...
```python
from automationmanagementclient.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `automationmanagementclient/exceptions/`.
...
```python
from automationmanagementclient.exceptions.api_exception import APIException
from automationmanagementclient.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`automationmanagementclient/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`automationmanagementclient/exceptions/` for the class.
...
`automationmanagementclient/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `automationmanagementclient/exceptions/`
  or a `ValueError` — check the auth handler in `automationmanagementclient/http/auth/` for the exact type.
