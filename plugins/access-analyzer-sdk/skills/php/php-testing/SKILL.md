...
> `AccessAnalyzerClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AccessAnalyzerClient
{
...
```php
use AccessAnalyzerLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AccessAnalyzerLib\Exceptions\{OperationException};
use AccessAnalyzerLib\Exceptions\ApiException;
...
app()->instance(AccessAnalyzerClient::class, clientWithResponses([
