...
kwargs to the `SplititWebApiV3Client` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `splititwebapiv3/splititwebapiv3_client.py` and `splititwebapiv3/http/auth/` in
the cloned source.

## Basic auth

```python
from splititwebapiv3.http.auth.basic_auth import BasicAuthCredentials
...
```python
from splititwebapiv3.http.auth.api_key import ApiKeyCredentials
...
```python
from splititwebapiv3.http.auth.api_header import ApiHeaderCredentials
...
```python
from splititwebapiv3.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from splititwebapiv3.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from splititwebapiv3.http.auth.o_auth_acg import OAuthACGCredentials
from splititwebapiv3.models.o_auth_scope_splititwebapiv3_enum import OAuthScope{Pkg}Enum
...
client = SplititWebApiV3Client(o_auth_acg_credentials=creds)
...
```python
from splititwebapiv3.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SplititWebApiV3Client(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SplititWebApiV3Client.from_environment()`) read each credential
...
Grep `from_environment` in `splititwebapiv3/configuration.py` and each `splititwebapiv3/http/auth/*.py` for the exact
