...
Open `shelldataandreporting/shelldataandreporting_client.py` for the controller property names, then the relevant
`shelldataandreporting/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from shelldataandreporting.models.{request_model} import {RequestModel}
...
Open `shelldataandreporting/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from shelldataandreporting.models.suite_code_enum import SuiteCodeEnum
...
```python
from shelldataandreporting.http.http_call_back import HttpCallBack
...
client = ShelldatareportingapisClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `shelldataandreporting/pagination/` in the source.

## Finding the right method

- Controller property names are on `ShelldatareportingapisClient` in `shelldataandreporting/shelldataandreporting_client.py`.
- Operation method signatures are in `shelldataandreporting/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `shelldataandreporting/models/`; enum types have `from_value()`.
