...
```ruby
client = ArticleSearchAPI::Client.new(
  max_retries:    3,
...
```ruby
client = ArticleSearchAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ArticleSearchAPI::Client.new(adapter: :net_http)
```
...
client = ArticleSearchAPI::Client.new(connection: conn)
```
...
client = ArticleSearchAPI::Client.new(environment: Environment::PRODUCTION)
client = ArticleSearchAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ArticleSearchAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ArticleSearchAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ArticleSearchAPI::ProxySettings.from_env
client = ArticleSearchAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ArticleSearchAPI::HttpCallBack
...
client = ArticleSearchAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ArticleSearchAPI::Client.from_env
# Overrides take precedence over env vars:
client = ArticleSearchAPI::Client.from_env(timeout: 30, max_retries: 3)
```
