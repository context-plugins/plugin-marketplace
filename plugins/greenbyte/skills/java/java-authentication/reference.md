...
against the `GreenbyteApiClient.Builder` setters, the `cloud.greenbyte.intro.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GreenbyteApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
GreenbyteApiClient client = new GreenbyteApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeGreenbyteEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new GreenbyteApiClient.Builder()
