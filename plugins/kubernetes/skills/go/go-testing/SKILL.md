...
`testing` + `testify/require` for reference; substitute the real `kubernetes` and names.
...
import (
    ...
    "github.com/context-plugins/kubernetes-go-sdk"   // root package kubernetes
)
...
func clientReturning(status int, body string) (kubernetes.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := kubernetes.NewClient(
        kubernetes.CreateConfiguration(
            kubernetes.WithHttpConfiguration(
                kubernetes.CreateHttpConfiguration(kubernetes.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/kubernetes-go-sdk/errors"
...
client := kubernetes.NewClient(
    kubernetes.CreateConfiguration(
        kubernetes.WithHttpConfiguration(
            kubernetes.CreateHttpConfiguration(kubernetes.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
