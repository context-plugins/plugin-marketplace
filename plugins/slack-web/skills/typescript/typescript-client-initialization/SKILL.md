...
- `slack-web-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'slack-web-apilib';
...
```ts
import { {Resource}Controller } from 'slack-web-apilib';
