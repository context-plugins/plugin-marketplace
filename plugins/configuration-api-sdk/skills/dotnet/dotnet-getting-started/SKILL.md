...
| Project | `ConfigurationApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `ConfigurationApi` (the `using` namespace) |
| Client class | `ConfigurationApiClient` |
| Options class | `ConfigurationApiClientOptions` |
...
namespaces transitively, so `using ConfigurationApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/configuration-api-csharp
dotnet add reference configuration-api-csharp/ConfigurationApi.csproj
```

```csharp
using ConfigurationApi;          // client + options + ServerEnvironment
using ConfigurationApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/configuration-api-csharp /tmp/configuration-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/configuration-api-csharp "$env:TEMP\configuration-api-dotnet-src"
```
...
- `ConfigurationApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/configuration-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\configuration-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `ConfigurationApiClient`, build its
   `ConfigurationApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
