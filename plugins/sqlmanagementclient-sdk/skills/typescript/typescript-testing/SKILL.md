...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new SqlManagementClientClient({
...
```typescript
import { {Operation}Error } from 'sqlmanagementclientlib/errors';
...
```typescript
import { ApiError } from 'sqlmanagementclientlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(SqlManagementClientClient)
    .useValue(new SqlManagementClientClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
