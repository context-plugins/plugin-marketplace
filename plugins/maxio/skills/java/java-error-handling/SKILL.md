...
│   └── com.maxio.advancedbilling.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.maxio.advancedbilling.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.maxio.advancedbilling.http.client.HttpContext`, which carries:
