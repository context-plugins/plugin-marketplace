...
Confirm every name against `lib/medium_api/client.rb`, `lib/medium_api/configuration.rb`, the
`lib/medium_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
