...
> `NetworkAdminManagementClientClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): NetworkAdminManagementClientClient
{
...
```php
use NetworkAdminManagementClientLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use NetworkAdminManagementClientLib\Exceptions\{OperationException};
use NetworkAdminManagementClientLib\Exceptions\ApiException;
...
app()->instance(NetworkAdminManagementClientClient::class, clientWithResponses([
