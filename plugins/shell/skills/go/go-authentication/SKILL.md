...
> Throughout, `shellapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := shellapi.NewClient(
    shellapi.CreateConfiguration(
        shellapi.WithBasicAuthCredentials(
            shellapi.NewBasicAuthCredentials(
...
shellapi.WithApiKeyCredentials(shellapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
shellapi.WithApiHeaderCredentials(shellapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
shellapi.WithOAuthBearerTokenCredentials(
    shellapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := shellapi.NewClient(
    shellapi.CreateConfiguration(
        shellapi.WithOAuthCCGCredentials(
            shellapi.NewOAuthCCGCredentials(
