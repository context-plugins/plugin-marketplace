...
```go
firecrawlapi.WithOAuthCCGCredentials(
    firecrawlapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr firecrawlapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := firecrawlapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr firecrawlapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := firecrawlapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := firecrawlapi.NewClient(firecrawlapi.CreateConfiguration(firecrawlapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    firecrawlapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := firecrawlapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := firecrawlapi.NewClient(firecrawlapi.CreateConfiguration(firecrawlapi.WithOAuthROPCGCredentials(creds)))
...
```go
firecrawlapi.WithOAuthBearerTokenCredentials(firecrawlapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
firecrawlapi.WithBasicAuthCredentials(firecrawlapi.NewBasicAuthCredentials(username, password))
firecrawlapi.WithApiHeaderCredentials(firecrawlapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
firecrawlapi.WithApiKeyCredentials(firecrawlapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FIRECRAWLAPI_...` variables — e.g.
`FIRECRAWLAPI_USERNAME`, `FIRECRAWLAPI_PASSWORD`, `FIRECRAWLAPI_TOKEN`, `FIRECRAWLAPI_API_KEY`,
`FIRECRAWLAPI_O_AUTH_CLIENT_ID`, `FIRECRAWLAPI_O_AUTH_CLIENT_SECRET`, `FIRECRAWLAPI_ACCESS_TOKEN`. Grep the
