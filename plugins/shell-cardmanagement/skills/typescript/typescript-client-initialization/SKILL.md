...
- `shell-card-management-apislib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'shell-card-management-apislib';
...
```ts
import { {Resource}Api } from 'shell-card-management-apislib';
