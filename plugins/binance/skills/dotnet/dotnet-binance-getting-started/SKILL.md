...
| --- | --- |
| API | Binance |
| Source repo | https://github.com/context-plugins/binance-csharp-sdk (branch `main`) |
...
<!-- crawler:auth -->
| Auth | Credentials properties on `BinancePublicSpotApiClientOptions`: `ApiKeyAuth: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using BinancePublicSpotApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/binance-csharp-sdk
dotnet add reference binance-csharp-sdk/BinancePublicSpotApi.csproj
```
...
temp directory** (`<temp>/binance-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/binance-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/binance-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/binance-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\binance-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/binance-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/binance-dotnet-src/`, **never** in the project repo, and its path never goes into
`binance-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new BinancePublicSpotApiClient(...)`, build its options, or DI-register via
   `AddBinancePublicSpotApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `BinancePublicSpotApiClientOptions` — the map's *Servers & auth* section
