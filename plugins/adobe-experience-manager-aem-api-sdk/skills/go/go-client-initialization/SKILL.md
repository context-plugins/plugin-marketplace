...
- `adobeexperiencemanageraemapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/adobe-experience-manager-aem-api-go` — the Go import path for the root package; `github.com/context-plugins/adobe-experience-manager-aem-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/adobe-experience-manager-aem-api-go"

client := adobeexperiencemanageraemapi.NewClient(
    adobeexperiencemanageraemapi.CreateConfiguration(
        adobeexperiencemanageraemapi.WithEnvironment(adobeexperiencemanageraemapi.PRODUCTION),
...
from environment variables named `ADOBEEXPERIENCEMANAGERAEMAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := adobeexperiencemanageraemapi.NewClient(adobeexperiencemanageraemapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `adobeexperiencemanageraemapi.PRODUCTION`,
`adobeexperiencemanageraemapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := adobeexperiencemanageraemapi.NewClient(
    adobeexperiencemanageraemapi.CreateConfiguration(adobeexperiencemanageraemapi.WithEnvironment(adobeexperiencemanageraemapi.PRODUCTION)),
)
```
...
client := adobeexperiencemanageraemapi.NewClient(
    adobeexperiencemanageraemapi.CreateConfiguration(
        adobeexperiencemanageraemapi.WithHttpConfiguration(
            adobeexperiencemanageraemapi.CreateHttpConfiguration(
                adobeexperiencemanageraemapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                adobeexperiencemanageraemapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = adobeexperiencemanageraemapi.NewClient(adobeexperiencemanageraemapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api adobeexperiencemanageraemapi.ClientInterface
}

func NewService(api adobeexperiencemanageraemapi.ClientInterface) *Service {
