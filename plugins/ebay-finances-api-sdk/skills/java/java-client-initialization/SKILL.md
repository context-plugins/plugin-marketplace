...
- `EBayFinancesAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.ebay.apiz` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`EBayFinancesAPIClient.Builder` class. A minimal initialization:

```java
import com.ebay.apiz.EBayFinancesAPIClient;
import com.ebay.apiz.Environment;

EBayFinancesAPIClient client = new EBayFinancesAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `EBayFinancesAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `EBayFinancesAPIClient.java` |

Call `.build()` to produce an immutable `EBayFinancesAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public EBayFinancesAPIClient apiClient() {
        return new EBayFinancesAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired EBayFinancesAPIClient client) {
        client.shutdown();
    }
}
```

Inject `EBayFinancesAPIClient` into your services as a normal Spring dependency. Build only one bean.
