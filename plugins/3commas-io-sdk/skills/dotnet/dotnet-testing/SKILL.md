...
`M3commasIoClient`/`M3commasIoClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `M3commasIoClient`,
...
static M3commasIoClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new M3commasIoClient(new HttpClient(handler), new M3commasIoClientOptions { /* auth not needed for stubs */ });
}
```
...
using M3commasIo.Errors;              // {Operation}Error types
...
```csharp
using M3commasIo.Core.Exceptions;
using M3commasIo.Core.ErrorResponse;
...
using M3commasIo.Core.ErrorResponse; // RawError (Case B)
...
var client = new M3commasIoClient(new HttpClient(handler), new M3commasIoClientOptions());
...
- For DI-based code, the SDK's `AddM3commasIoClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `M3commasIoClient` from the provider:
  ```csharp
  services.AddM3commasIoClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<M3commasIoClient>();
  ```
