...
```ruby
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(
  max_retries:    3,
...
```ruby
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(adapter: :net_http)
```
...
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(connection: conn)
```
...
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(environment: Environment::PRODUCTION)
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = OpenUVGlobalRealTimeUVIndexForecastAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = OpenUVGlobalRealTimeUVIndexForecastAPI::ProxySettings.from_env
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < OpenUVGlobalRealTimeUVIndexForecastAPI::HttpCallBack
...
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.from_env
# Overrides take precedence over env vars:
client = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.from_env(timeout: 30, max_retries: 3)
```
