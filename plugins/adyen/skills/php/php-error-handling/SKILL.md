...
> `{operation}`, `{apiGroup}`, `AdyenLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `AdyenLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
- All `ApiException` subclasses live under `AdyenLib\Exceptions\` — read the source to
