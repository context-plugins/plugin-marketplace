...
```typescript
import { {Operation}Error } from 'sportsdatalib/errors';
...
```typescript
import { ApiError } from 'sportsdatalib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .useValue(new SportsdataClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
