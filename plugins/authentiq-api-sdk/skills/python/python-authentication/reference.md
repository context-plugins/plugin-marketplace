...
kwargs to the `AuthentiqAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `authentiqapi/authentiqapi_client.py` and `authentiqapi/http/auth/` in
the cloned source.

## Basic auth

```python
from authentiqapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from authentiqapi.http.auth.api_key import ApiKeyCredentials
...
```python
from authentiqapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from authentiqapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from authentiqapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from authentiqapi.http.auth.o_auth_acg import OAuthACGCredentials
from authentiqapi.models.o_auth_scope_authentiqapi_enum import OAuthScope{Pkg}Enum
...
client = AuthentiqAPIClient(o_auth_acg_credentials=creds)
...
```python
from authentiqapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AuthentiqAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AuthentiqAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `authentiqapi/configuration.py` and each `authentiqapi/http/auth/*.py` for the exact
