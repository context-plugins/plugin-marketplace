...
> `{ApiGroup}`, `SquareConnectApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `SquareConnectApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `SquareConnectApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `SquareConnectApi.Errors`
...
```csharp
using SquareConnectApi.Core.Exceptions;     // SdkException<TError>
using SquareConnectApi.Core.ErrorResponse;  // ApiError, RawError
using SquareConnectApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`SquareConnectApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using SquareConnectApi.Core.Exceptions;     // SdkException<TError>
using SquareConnectApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `SquareConnectApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `SquareConnectApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using SquareConnectApi.Core.Models;        // ApiResult<TResponse, TError>
using SquareConnectApi.Core.ErrorResponse; // RawError
using SquareConnectApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `SquareConnectApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
