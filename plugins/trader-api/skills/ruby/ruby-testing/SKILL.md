...
client = stub_client(stubs)
assert_raises TraderAPI::APIException do
...
BASE = TraderAPI::Configuration.new(environment: TraderAPI::Environment::PRODUCTION).get_base_uri

def build_test_client
  TraderAPI::Client.new(
    environment: TraderAPI::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error TraderAPI::APIException
```
...
```ruby
expect { call }.to raise_error(TraderAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < TraderAPI::HttpCallBack
...
client  = TraderAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
