...
- `payquicker` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/payquicker-go-sdk` — the Go import path for the root package; `github.com/context-plugins/payquicker-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/payquicker-go-sdk"

client := payquicker.NewClient(
    payquicker.CreateConfiguration(
        payquicker.WithEnvironment(payquicker.PRODUCTION),
...
from environment variables named `PAYQUICKER_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := payquicker.NewClient(payquicker.CreateConfigurationFromEnvironment())
```
...
```go
client := payquicker.NewClient(
    payquicker.CreateConfiguration(payquicker.WithEnvironment(payquicker.PRODUCTION)),
)
```
...
client := payquicker.NewClient(
    payquicker.CreateConfiguration(
        payquicker.WithHttpConfiguration(
            payquicker.CreateHttpConfiguration(
                payquicker.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                payquicker.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = payquicker.NewClient(payquicker.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api payquicker.ClientInterface
}

func NewService(api payquicker.ClientInterface) *Service {
