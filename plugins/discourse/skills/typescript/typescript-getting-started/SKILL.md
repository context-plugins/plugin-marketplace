...
description: "Discourse TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `discourse`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any Discourse TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the Discourse TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Discourse TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-discourse` for the workflow that wraps this skill.
...
Verified against `package.json` and `sdk-map.md` of the generated package at version `latest`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Discourse |
| Package name (what you install, and what you import) | `discourse` — **not on npm**; built from source (see *Install*) |
| Import specifier | `discourse` — the package root is the **only** entry; deep imports do not resolve |
| Version | `latest` (API spec version `latest`) |
| Client class | `DiscourseClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new DiscourseClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new DiscourseClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch`. `timeout` defaults to `60_000` ms |
| Auth | None — the spec declares no security schemes |
| Environments | 1 environment (`ServerEnvironment.Production` *(default)*) × 1 server group |
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://{defaultHost}` |
...
| Surface | 110 operations · 16 resources · 250 models · 18 open enums · 1 per-operation error subclass |
...
```bash
npm install git+https://github.com/context-plugins/discourse-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 268 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { DiscourseClient, ServerEnvironment, ResponseError, DiscourseError } from "discourse";
import type { ClientOptions, AccessControl } from "discourse";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `discourse/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **⚠ The SDK exports a model type literally named `Event`** (`src/models/event.ts`, schema `eventSchema`). Import it unaliased and it **shadows the global `Event`** for the rest of the file. Alias it: `import type { Event as SdkEvent } from "discourse"`.
- **From CommonJS**, the typed spelling is `import sdk = require("discourse")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://{defaultHost}` | `serverOptions.default.production.baseUrl` |
...
## Auth pattern (no schemes)

The spec declares no security schemes, so every operation is public, the client sends no credential, and there is nothing to configure. An operation that answers `401` here is telling you the spec and the deployment disagree — not that a field is unset.
...
| --- | --- | --- | --- |
| `client.discourseCalendarEvents` | `DiscourseCalendarEvents` | 2 | `exportEventsIcs` · `listEvents` |
| `client.backups` | `Backups` | 4 | `createBackup` · `downloadBackup` · `getBackups` · `sendDownloadBackupEmail` |
| `client.badges` | `Badges` | 5 | `adminListBadges` · `createBadge` · `deleteBadge` · `listUserBadges` · `updateBadge` |
| `client.categories` | `Categories` | 6 | `createCategory` · `getCategory` · `getSite` · `listCategories` · `listCategoryTopics` · `updateCategory` |
| `client.groups` | `Groups` | 9 | `addGroupMembers` · `createGroup` · `deleteGroup` · `getGroup` · `getGroupById` · `listGroupMembers` · `listGroups` · `removeGroupMembers` · `updateGroup` |
| `client.invites` | `Invites` | 4 | `createInvite` · `createMultipleInvites` · `inviteGroupToTopic` · `inviteToTopic` |
| `client.notifications` | `Notifications` | 2 | `getNotifications` · `markNotificationsAsRead` |
| `client.posts` | `Posts` | 8 | `createTopicPostPm` · `deletePost` · `getPost` · `listPosts` · `lockPost` · `performPostAction` · `postReplies` · `updatePost` |
| `client.topics` | `Topics` | 15 | `bookmarkTopic` · `createTopicPostPm` · `createTopicTimer` · `getSpecificPostsFromTopic` · `getTopic` · `getTopicByExternalId` · `inviteGroupToTopic` · `inviteToTopic` · `listLatestTopics` · `listTopTopics` · `removeTopic` · `setNotificationLevel` · `updateTopic` · `updateTopicStatus` · `updateTopicTimestamp` |
| `client.privateMessages` | `PrivateMessages` | 3 | `createTopicPostPm` · `getUserSentPrivateMessages` · `listUserPrivateMessages` |
| `client.search` | `Search` | 1 | `search` |
| `client.site` | `Site` | 2 | `getSite` · `getSiteBasicInfo` |
| `client.tags` | `Tags` | 6 | `createTagGroup` · `getTag` · `getTagGroup` · `listTagGroups` · `listTags` · `updateTagGroup` |
| `client.uploads` | `Uploads` | 7 | `abortMultipart` · `batchPresignMultipartParts` · `completeExternalUpload` · `completeMultipart` · `createMultipartUpload` · `createUpload` · `generatePresignedPut` |
| `client.users` | `Users` | 25 | `activateUser` · `adminGetUser` · `adminListUsers` · `adminListUsersFlag` · `anonymizeUser` · `changePassword` · `createUser` · `deactivateUser` · `deleteUser` · `getUser` · `getUserEmails` · `getUserExternalId` · `getUserIdentiyProviderExternalId` · `listUserActions` · `listUserBadges` · `listUsersPublic` · `logOutUser` · `refreshGravatar` · `sendPasswordResetEmail` · `silenceUser` · `suspendUser` · `updateAvatar` · `updateEmail` · `updateUser` · `updateUsername` |
| `client.admin` | `Admin` | 11 | `activateUser` · `adminGetUser` · `adminListUsers` · `adminListUsersFlag` · `anonymizeUser` · `deactivateUser` · `deleteUser` · `logOutUser` · `refreshGravatar` · `silenceUser` · `suspendUser` |
...
```bash
node -e "console.log(require.resolve('discourse/package.json'))"
```

Failing that it is at `node_modules/discourse/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/discourse/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/discourse/`:
...
| Auth scheme wiring, token endpoint, credential placement | *no scheme is declared, so no auth module is emitted* |
...
Beyond the usual signatures and model members, a contract sheet for the Discourse TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
...
9. **That `Event` imported from this package is a model type, not the global of that name** — every sheet that references one should carry the alias it will be imported under. The error base is re-exported as `DiscourseError` for the same reason.
