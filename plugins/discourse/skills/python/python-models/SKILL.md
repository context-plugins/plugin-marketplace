...
your SDK source (`discourse/models/`).
...
```python
from discourse.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `discourse/utilities/union_type_lookup.py`) and each candidate model
...
```python
from discourse.models.animal import Cat, Dog
...
```python
from discourse.models.animal import Animal, Cat, Dog
