...
```ruby
client = AWSMigrationHub::Client.new(
  max_retries:    3,
...
```ruby
client = AWSMigrationHub::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = AWSMigrationHub::Client.new(adapter: :net_http)
```
...
client = AWSMigrationHub::Client.new(connection: conn)
```
...
client = AWSMigrationHub::Client.new(environment: Environment::PRODUCTION)
client = AWSMigrationHub::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = AWSMigrationHub::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = AWSMigrationHub::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = AWSMigrationHub::ProxySettings.from_env
client = AWSMigrationHub::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < AWSMigrationHub::HttpCallBack
...
client = AWSMigrationHub::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = AWSMigrationHub::Client.from_env
# Overrides take precedence over env vars:
client = AWSMigrationHub::Client.from_env(timeout: 30, max_retries: 3)
```
