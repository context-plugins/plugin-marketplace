...
`CellPointApiClient`/`CellPointApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `CellPointApiClient`,
...
static CellPointApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new CellPointApiClient(new HttpClient(handler), new CellPointApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using CellPointApi.Errors;              // {Operation}Error types
...
```csharp
using CellPointApi.Core.Exceptions;
using CellPointApi.Core.ErrorResponse;
...
using CellPointApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new CellPointApiClient(new HttpClient(handler), new CellPointApiClientOptions());
...
- For DI-based code, the SDK's `AddCellPointApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `CellPointApiClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<CellPointApiClient>();
  ```
