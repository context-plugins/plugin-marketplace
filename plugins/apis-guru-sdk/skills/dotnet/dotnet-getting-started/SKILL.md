...
| --- | --- |
| API | `APIs.guru` |
| Project | `ApisGuru.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `ApisGuru` (the `using` namespace) |
| Client class | `ApisGuruClient` |
| Options class | `ApisGuruClientOptions` |
...
namespaces transitively, so `using ApisGuru.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/apis-guru-csharp
dotnet add reference apis-guru-csharp/ApisGuru.csproj
```

```csharp
using ApisGuru;          // client + options + ServerEnvironment
using ApisGuru.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/apis-guru-csharp /tmp/apis-guru-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/apis-guru-csharp "$env:TEMP\apis-guru-dotnet-src"
```
...
- `ApisGuruClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/apis-guru-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\apis-guru-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `ApisGuruClient`, build its
   `ApisGuruClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
