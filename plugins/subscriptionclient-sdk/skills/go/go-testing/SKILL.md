...
`testing` + `testify/require` for reference; substitute the real `subscriptionclient` and names.
...
import (
    ...
    "github.com/context-plugins/subscriptionclient-go"   // root package subscriptionclient
)
...
func clientReturning(status int, body string) (subscriptionclient.ClientInterface, *stubTransport) {
    ...
    client := subscriptionclient.NewClient(
        subscriptionclient.CreateConfiguration(
            subscriptionclient.WithHttpConfiguration(
                subscriptionclient.CreateHttpConfiguration(subscriptionclient.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/subscriptionclient-go/errors"
...
client := subscriptionclient.NewClient(
    subscriptionclient.CreateConfiguration(
        subscriptionclient.WithHttpConfiguration(
            subscriptionclient.CreateHttpConfiguration(subscriptionclient.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
