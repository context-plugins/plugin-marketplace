...
- `coingecko` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/coingecko-go-sdk` — the Go import path for the root package; `github.com/context-plugins/coingecko-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/coingecko-go-sdk"

client := coingecko.NewClient(
    coingecko.CreateConfiguration(
        coingecko.WithEnvironment(coingecko.PRODUCTION),
...
from environment variables named `COINGECKO_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := coingecko.NewClient(coingecko.CreateConfigurationFromEnvironment())
```
...
```go
client := coingecko.NewClient(
    coingecko.CreateConfiguration(coingecko.WithEnvironment(coingecko.PRODUCTION)),
)
```
...
client := coingecko.NewClient(
    coingecko.CreateConfiguration(
        coingecko.WithHttpConfiguration(
            coingecko.CreateHttpConfiguration(
                coingecko.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                coingecko.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = coingecko.NewClient(coingecko.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api coingecko.ClientInterface
}

func NewService(api coingecko.ClientInterface) *Service {
