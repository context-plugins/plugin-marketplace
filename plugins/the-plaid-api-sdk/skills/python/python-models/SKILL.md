...
your SDK source (`theplaidapi/models/`).
...
```python
from theplaidapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `theplaidapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from theplaidapi.models.animal import Cat, Dog
...
```python
from theplaidapi.models.animal import Animal, Cat, Dog
