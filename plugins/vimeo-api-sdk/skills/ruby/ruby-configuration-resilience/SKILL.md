...
```ruby
client = VimeoAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = VimeoAPI::Client.new(adapter: :net_http)
```
...
client = VimeoAPI::Client.new(connection: conn)
```
...
client = VimeoAPI::Client.new(environment: Environment::PRODUCTION)
client = VimeoAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = VimeoAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = VimeoAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = VimeoAPI::ProxySettings.from_env
client = VimeoAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < VimeoAPI::HttpCallBack
...
client = VimeoAPI::Client.new(http_callback: LogCallback.new)
```
...
client = VimeoAPI::Client.from_env(timeout: 30, max_retries: 3)
```
