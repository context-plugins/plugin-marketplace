...
`BinancePublicSpotApiClient`/`BinancePublicSpotApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `BinancePublicSpotApiClient`,
...
static BinancePublicSpotApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new BinancePublicSpotApiClient(new HttpClient(handler), new BinancePublicSpotApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using BinancePublicSpotApi.Errors;              // {Operation}Error types
...
using BinancePublicSpotApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new BinancePublicSpotApiClient(new HttpClient(handler), new BinancePublicSpotApiClientOptions());
...
- For DI-based code, the SDK's `AddBinancePublicSpotApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `BinancePublicSpotApiClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<BinancePublicSpotApiClient>();
  ```
