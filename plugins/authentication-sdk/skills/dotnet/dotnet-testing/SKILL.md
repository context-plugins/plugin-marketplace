...
`AuthenticationClient`/`AuthenticationClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `AuthenticationClient`,
...
static AuthenticationClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new AuthenticationClient(new HttpClient(handler), new AuthenticationClientOptions { /* auth not needed for stubs */ });
}
```
...
using Authentication.Errors;              // {Operation}Error types
...
using Authentication.Core.ErrorResponse;
...
using Authentication.Core.ErrorResponse; // RawError (Case B)
...
var client = new AuthenticationClient(new HttpClient(handler), new AuthenticationClientOptions());
...
- For DI-based code, the SDK's `AddAuthenticationClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `AuthenticationClient` from the provider:
  ```csharp
  services.AddAuthenticationClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<AuthenticationClient>();
  ```
