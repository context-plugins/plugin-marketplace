...
against the `GeoapifyApiClient.Builder` setters, the `com.geoapify.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GeoapifyApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
GeoapifyApiClient client = new GeoapifyApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeGeoapifyEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
