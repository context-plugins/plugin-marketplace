...
kwargs to the `FlightOrderManagementClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `flightordermanagement/flightordermanagement_client.py` and `flightordermanagement/http/auth/` in
the cloned source.

## Basic auth

```python
from flightordermanagement.http.auth.basic_auth import BasicAuthCredentials
...
```python
from flightordermanagement.http.auth.api_key import ApiKeyCredentials
...
```python
from flightordermanagement.http.auth.api_header import ApiHeaderCredentials
...
```python
from flightordermanagement.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from flightordermanagement.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from flightordermanagement.http.auth.o_auth_acg import OAuthACGCredentials
from flightordermanagement.models.o_auth_scope_flightordermanagement_enum import OAuthScope{Pkg}Enum
...
client = FlightOrderManagementClient(o_auth_acg_credentials=creds)
...
```python
from flightordermanagement.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FlightOrderManagementClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FlightOrderManagementClient.from_environment()`) read each credential
...
Grep `from_environment` in `flightordermanagement/configuration.py` and each `flightordermanagement/http/auth/*.py` for the exact
