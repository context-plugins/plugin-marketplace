...
│   └── com.xero.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.xero.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.xero.api.http.client.HttpContext`, which carries:
