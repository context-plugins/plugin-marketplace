...
```go
newstore.WithOAuthCCGCredentials(
    newstore.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr newstore.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := newstore.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr newstore.OAuthCCGManager) models.OAuthToken {
...
```go
creds := newstore.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := newstore.NewClient(newstore.CreateConfiguration(newstore.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    newstore.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := newstore.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := newstore.NewClient(newstore.CreateConfiguration(newstore.WithOAuthROPCGCredentials(creds)))
...
```go
newstore.WithOAuthBearerTokenCredentials(newstore.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
newstore.WithBasicAuthCredentials(newstore.NewBasicAuthCredentials(username, password))
newstore.WithApiHeaderCredentials(newstore.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
newstore.WithApiKeyCredentials(newstore.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `NEWSTORE_...` variables — e.g.
`NEWSTORE_USERNAME`, `NEWSTORE_PASSWORD`, `NEWSTORE_TOKEN`, `NEWSTORE_API_KEY`,
`NEWSTORE_O_AUTH_CLIENT_ID`, `NEWSTORE_O_AUTH_CLIENT_SECRET`, `NEWSTORE_ACCESS_TOKEN`. Grep the
