...
All types below live under `PasswordConnect1.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using PasswordConnect1.Core.Configuration;
...
```csharp
using PasswordConnect1.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new PasswordConnect1ClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `PasswordConnect1.Core.Exceptions`):
...
With DI, the SDK's `AddPasswordConnect1Client` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddPasswordConnect1Client(options => { /* ... */ });   // resolves CreateClient() → the default client
```
