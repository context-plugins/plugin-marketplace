...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new TesserAPIV1Client({
...
```typescript
import { {Operation}Error } from 'tesser-api-v-1-lib/errors';
...
```typescript
import { ApiError } from 'tesser-api-v-1-lib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(TesserAPIV1Client)
    .useValue(new TesserAPIV1Client({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
