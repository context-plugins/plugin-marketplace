...
def stub_client(stubs)
  ...
  ShellDataandreporting::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises ShellDataandreporting::APIException do
...
BASE = ShellDataandreporting::Configuration.new(environment: ShellDataandreporting::Environment::PRODUCTION).get_base_uri

def build_test_client
  ShellDataandreporting::Client.new(
    environment: ShellDataandreporting::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ShellDataandreporting::APIException
```
...
```ruby
expect { call }.to raise_error(ShellDataandreporting::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ShellDataandreporting::HttpCallBack
...
client  = ShellDataandreporting::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
