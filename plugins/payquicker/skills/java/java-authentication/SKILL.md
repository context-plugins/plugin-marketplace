...
To see which schemes a specific SDK accepts, read the credential setter methods on `PqApiV2Client.Builder`
...
- Set credentials **on the `PqApiV2Client.Builder`** before calling `.build()` — the client is immutable
