...
kwargs to the `TwilioapisClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `twilioapis/twilioapis_client.py` and `twilioapis/http/auth/` in
the cloned source.
...
```python
from twilioapis.http.auth.api_key import ApiKeyCredentials
...
```python
from twilioapis.http.auth.api_header import ApiHeaderCredentials
...
```python
from twilioapis.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from twilioapis.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from twilioapis.http.auth.o_auth_acg import OAuthACGCredentials
from twilioapis.models.o_auth_scope_twilioapis_enum import OAuthScope{Pkg}Enum
...
client = TwilioapisClient(o_auth_acg_credentials=creds)
...
```python
from twilioapis.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = TwilioapisClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `TwilioapisClient.from_environment()`) read each credential
...
Grep `from_environment` in `twilioapis/configuration.py` and each `twilioapis/http/auth/*.py` for the exact
