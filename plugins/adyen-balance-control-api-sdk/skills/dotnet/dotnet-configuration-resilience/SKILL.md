...
All types below live under `AdyenBalanceControlApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using AdyenBalanceControlApi.Core.Configuration;
...
```csharp
using AdyenBalanceControlApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new AdyenBalanceControlApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `AdyenBalanceControlApi.Core.Exceptions`):
...
With DI, the SDK's `AddAdyenBalanceControlApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAdyenBalanceControlApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
