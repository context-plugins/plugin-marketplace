...
- `traderapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/trader-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/trader-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/trader-api-go-sdk"

client := traderapi.NewClient(
    traderapi.CreateConfiguration(
        traderapi.WithEnvironment(traderapi.PRODUCTION),
...
from environment variables named `TRADERAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := traderapi.NewClient(traderapi.CreateConfigurationFromEnvironment())
```
...
```go
client := traderapi.NewClient(
    traderapi.CreateConfiguration(traderapi.WithEnvironment(traderapi.PRODUCTION)),
)
```
...
client := traderapi.NewClient(
    traderapi.CreateConfiguration(
        traderapi.WithHttpConfiguration(
            traderapi.CreateHttpConfiguration(
                traderapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                traderapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = traderapi.NewClient(traderapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api traderapi.ClientInterface
}

func NewService(api traderapi.ClientInterface) *Service {
