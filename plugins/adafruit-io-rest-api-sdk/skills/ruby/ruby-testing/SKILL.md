...
client = stub_client(stubs)
assert_raises AdafruitIORESTAPI::APIException do
...
def build_test_client
  AdafruitIORESTAPI::Client.new(
    environment: AdafruitIORESTAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error AdafruitIORESTAPI::APIException
```
...
```ruby
expect { call }.to raise_error(AdafruitIORESTAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < AdafruitIORESTAPI::HttpCallBack
...
client  = AdafruitIORESTAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
