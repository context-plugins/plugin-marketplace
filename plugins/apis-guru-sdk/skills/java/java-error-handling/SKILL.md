...
│   └── guru.apis.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── guru.apis.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `guru.apis.api.http.client.HttpContext`, which carries:
