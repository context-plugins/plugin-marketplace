...
against the `SeatmapDisplayClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SeatmapDisplayClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
SeatmapDisplayClient client = new SeatmapDisplayClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeSeatmapDisplayEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new SeatmapDisplayClient.Builder()
