...
`testing` + `testify/require` for reference; substitute the real `square` and names.
...
import (
    ...
    "github.com/context-plugins/square-go-sdk"   // root package square
)
...
func clientReturning(status int, body string) (square.ClientInterface, *stubTransport) {
    ...
    client := square.NewClient(
        square.CreateConfiguration(
            square.WithHttpConfiguration(
                square.CreateHttpConfiguration(square.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/square-go-sdk/errors"
...
client := square.NewClient(
    square.CreateConfiguration(
        square.WithHttpConfiguration(
            square.CreateHttpConfiguration(square.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
