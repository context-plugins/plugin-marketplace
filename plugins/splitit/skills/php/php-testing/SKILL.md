...
> `SplititWebApiV3Client`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SplititWebApiV3Client
{
...
```php
use SplititWebApiV3Lib\Exceptions\{OperationException};
use SplititWebApiV3Lib\Exceptions\ApiException;
...
app()->instance(SplititWebApiV3Client::class, clientWithResponses([
