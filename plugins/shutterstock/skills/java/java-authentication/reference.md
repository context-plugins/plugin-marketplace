...
against the `ShutterstockClient.Builder` setters, the `com.shutterstock.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ShutterstockClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
new ShutterstockClient.Builder()
