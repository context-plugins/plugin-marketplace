...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `FlightOffersPriceClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`flightoffersprice/configuration.py` in the cloned source.
...
```python
import requests
from flightoffersprice.configuration import Configuration
...
`flightoffersprice/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`flightoffersprice/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `flightoffersprice/configuration.py`
...
`NumberPagedResponse`) live in `flightoffersprice/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `flightoffersprice/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
