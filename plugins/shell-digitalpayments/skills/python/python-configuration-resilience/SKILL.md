...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ShellsmartpayapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`shelldigitalpayments/configuration.py` in the cloned source.
...
`shelldigitalpayments/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`shelldigitalpayments/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `shelldigitalpayments/configuration.py`
...
`NumberPagedResponse`) live in `shelldigitalpayments/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `shelldigitalpayments/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
