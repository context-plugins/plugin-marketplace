...
def stub_client(stubs)
  ...
  AdyenDataProtectionAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises AdyenDataProtectionAPI::APIException do
...
def build_test_client
  AdyenDataProtectionAPI::Client.new(
    environment: AdyenDataProtectionAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error AdyenDataProtectionAPI::APIException
```
...
```ruby
expect { call }.to raise_error(AdyenDataProtectionAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < AdyenDataProtectionAPI::HttpCallBack
...
client  = AdyenDataProtectionAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
