...
`testing` + `testify/require` for reference; substitute the real `apiapisguru` and names.
...
import (
    ...
    "github.com/context-plugins/api-apis-guru-go-832e4b"   // root package apiapisguru
)
...
func clientReturning(status int, body string) (apiapisguru.ClientInterface, *stubTransport) {
    ...
    client := apiapisguru.NewClient(
        apiapisguru.CreateConfiguration(
            apiapisguru.WithHttpConfiguration(
                apiapisguru.CreateHttpConfiguration(apiapisguru.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/api-apis-guru-go-832e4b/errors"
...
client := apiapisguru.NewClient(
    apiapisguru.CreateConfiguration(
        apiapisguru.WithHttpConfiguration(
            apiapisguru.CreateHttpConfiguration(apiapisguru.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
