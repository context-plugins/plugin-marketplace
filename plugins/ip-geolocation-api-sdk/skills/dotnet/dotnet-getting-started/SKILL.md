...
| Project | `IPGeolocationAPI.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `IPGeolocationAPI` (the `using` namespace) |
| Client class | `IPGeolocationAPIClient` |
| Options class | `IPGeolocationAPIClientOptions` |
...
namespaces transitively, so `using IPGeolocationAPI.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/ip-geolocation-api-csharp
dotnet add reference ip-geolocation-api-csharp/IPGeolocationAPI.csproj
```

```csharp
using IPGeolocationAPI;          // client + options + ServerEnvironment
using IPGeolocationAPI.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/ip-geolocation-api-csharp /tmp/ip-geolocation-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/ip-geolocation-api-csharp "$env:TEMP\ip-geolocation-api-dotnet-src"
```
...
- `IPGeolocationAPIClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/ip-geolocation-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\ip-geolocation-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `IPGeolocationAPIClient`, build its
   `IPGeolocationAPIClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
