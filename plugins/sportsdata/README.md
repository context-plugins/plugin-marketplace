# sportsdata SDK Plugin

Integrate and consume the **APIMatic-generated sportsdata SDK** (the "APIMATIC v3.0" generic
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, TypeScript, PHP, Python, Ruby, Go.
...
(`dotnet-sportsdata-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-sportsdata`** router skill routes all
C#/.NET SDK work to the single **`dotnet-sportsdata-sdk`** agent (.NET-only), which answers
