...
client = stub_client(stubs)
assert_raises Notion::APIException do
...
BASE = Notion::Configuration.new(environment: Notion::Environment::PRODUCTION).get_base_uri

def build_test_client
  Notion::Client.new(
    environment: Notion::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Notion::APIException
```
...
```ruby
expect { call }.to raise_error(Notion::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Notion::HttpCallBack
...
client  = Notion::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
