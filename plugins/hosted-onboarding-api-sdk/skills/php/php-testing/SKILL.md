...
> `HostedOnboardingAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): HostedOnboardingAPIClient
{
...
```php
use HostedOnboardingAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use HostedOnboardingAPILib\Exceptions\{OperationException};
use HostedOnboardingAPILib\Exceptions\ApiException;
...
app()->instance(HostedOnboardingAPIClient::class, clientWithResponses([
