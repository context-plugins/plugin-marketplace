...
`testing` + `testify/require` for reference; substitute the real `awsmigrationhub` and names.
...
import (
    ...
    "github.com/context-plugins/aws-migration-hub-go"   // root package awsmigrationhub
)
...
func clientReturning(status int, body string) (awsmigrationhub.ClientInterface, *stubTransport) {
    ...
    client := awsmigrationhub.NewClient(
        awsmigrationhub.CreateConfiguration(
            awsmigrationhub.WithHttpConfiguration(
                awsmigrationhub.CreateHttpConfiguration(awsmigrationhub.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/aws-migration-hub-go/errors"
...
client := awsmigrationhub.NewClient(
    awsmigrationhub.CreateConfiguration(
        awsmigrationhub.WithHttpConfiguration(
            awsmigrationhub.CreateHttpConfiguration(awsmigrationhub.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
