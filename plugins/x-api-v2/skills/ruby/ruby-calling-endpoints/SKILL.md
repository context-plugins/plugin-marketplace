...
Open `lib/x_api_v2/client.rb` for the accessor names, then the relevant
`lib/x_api_v2/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = XAPIV2::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: XAPIV2::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/x_api_v2/models/`. Field names are defined as
...
Read the enum module in `lib/x_api_v2/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/x_api_v2/client.rb`.
- Operation methods are in `lib/x_api_v2/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/x_api_v2/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/x_api_v2/exceptions/`.
