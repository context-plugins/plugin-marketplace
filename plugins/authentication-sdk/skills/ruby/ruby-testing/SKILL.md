...
client = stub_client(stubs)
assert_raises Authentication::APIException do
...
def build_test_client
  Authentication::Client.new(
    environment: Authentication::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Authentication::APIException
```
...
```ruby
expect { call }.to raise_error(Authentication::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Authentication::HttpCallBack
...
client  = Authentication::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
