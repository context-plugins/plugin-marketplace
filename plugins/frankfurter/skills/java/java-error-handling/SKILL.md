...
│   └── dev.frankfurter.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── dev.frankfurter.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `dev.frankfurter.api.http.client.HttpContext`, which carries:
