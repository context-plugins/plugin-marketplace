...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddHotelRatingsClient extension. Use the moment you write `new HotelRatingsClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `HotelRatingsClientOptions` — its options class.
- `HotelRatings` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public HotelRatingsClient(HttpClient httpClient, HotelRatingsClientOptions options)
```
...
```csharp
public class HotelRatingsClientOptions
{
...
```csharp
using HotelRatings;
using HotelRatings.Servers;

var options = new HotelRatingsClientOptions
{
...
var client = new HotelRatingsClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddHotelRatingsClient`, which registers the
...
builder.Services.AddHotelRatingsClient(options =>
{
...
```csharp
public sealed class MyService(HotelRatingsClient client)
{
