...
Open `shelldigitalpayments/shelldigitalpayments_client.py` for the controller property names, then the relevant
`shelldigitalpayments/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from shelldigitalpayments.models.{request_model} import {RequestModel}
...
Open `shelldigitalpayments/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from shelldigitalpayments.models.suite_code_enum import SuiteCodeEnum
...
```python
from shelldigitalpayments.http.http_call_back import HttpCallBack
...
client = ShellsmartpayapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `shelldigitalpayments/pagination/` in the source.

## Finding the right method

- Controller property names are on `ShellsmartpayapiClient` in `shelldigitalpayments/shelldigitalpayments_client.py`.
- Operation method signatures are in `shelldigitalpayments/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `shelldigitalpayments/models/`; enum types have `from_value()`.
