...
│   └── nl.postnl.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── nl.postnl.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `nl.postnl.api.http.client.HttpContext`, which carries:
