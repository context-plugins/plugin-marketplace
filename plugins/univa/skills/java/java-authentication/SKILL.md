...
To see which schemes a specific SDK accepts, read the credential setter methods on `UnivapayClientSdkClient.Builder`
...
- Set credentials **on the `UnivapayClientSdkClient.Builder`** before calling `.build()` — the client is immutable
