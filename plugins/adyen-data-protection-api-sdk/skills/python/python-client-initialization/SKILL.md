...
- `adyendataprotectionapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `AdyenDataProtectionAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `AdyenDataProtectionAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from adyendataprotectionapi.adyendataprotectionapi_client import AdyenDataProtectionAPIClient
from adyendataprotectionapi.configuration import Environment
from adyendataprotectionapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = AdyenDataProtectionAPIClient(
...
```python
from adyendataprotectionapi.configuration import Configuration, Environment
...
client = AdyenDataProtectionAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `adyendataprotectionapi/adyendataprotectionapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `AdyenDataProtectionAPIClient` and `Configuration` that reads
...
client = AdyenDataProtectionAPIClient.from_environment()
...
`from_environment` in `adyendataprotectionapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `AdyenDataProtectionAPIClient`. Access them as attributes —
...
Open `adyendataprotectionapi/adyendataprotectionapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `adyendataprotectionapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from adyendataprotectionapi.configuration import Environment

client = AdyenDataProtectionAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`adyendataprotectionapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = AdyenDataProtectionAPIClient(config=new_config)
```
