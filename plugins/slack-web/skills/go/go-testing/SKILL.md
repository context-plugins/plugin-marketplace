...
`testing` + `testify/require` for reference; substitute the real `slackwebapi` and names.
...
import (
    ...
    "github.com/context-plugins/slack-web-api-go"   // root package slackwebapi
)
...
func clientReturning(status int, body string) (slackwebapi.ClientInterface, *stubTransport) {
    ...
    client := slackwebapi.NewClient(
        slackwebapi.CreateConfiguration(
            slackwebapi.WithHttpConfiguration(
                slackwebapi.CreateHttpConfiguration(slackwebapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/slack-web-api-go/errors"
...
client := slackwebapi.NewClient(
    slackwebapi.CreateConfiguration(
        slackwebapi.WithHttpConfiguration(
            slackwebapi.CreateHttpConfiguration(slackwebapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
