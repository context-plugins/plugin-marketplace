...
against the `CoinGeckoDemoAPIClient.Builder` setters, the `com.coingecko.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `CoinGeckoDemoAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
CoinGeckoDemoAPIClient client = new CoinGeckoDemoAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeCoinGeckoDemoAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new CoinGeckoDemoAPIClient.Builder()
