    ...
    shellapi.CreateConfiguration(
        shellapi.WithHttpConfiguration(
            shellapi.CreateHttpConfiguration(
                shellapi.WithTimeout(30),
                shellapi.WithRetryConfiguration(
                    shellapi.CreateRetryConfiguration(
                        shellapi.WithMaxRetryAttempts(3),
                        shellapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := shellapi.CreateRetryConfiguration(
    shellapi.WithMaxRetryAttempts(5),
    shellapi.WithRetryInterval(1),
    shellapi.WithBackoffFactor(2),
    shellapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    shellapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := shellapi.NewClient(
    shellapi.CreateConfiguration(
        shellapi.WithHttpConfiguration(
            shellapi.CreateHttpConfiguration(
                shellapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
