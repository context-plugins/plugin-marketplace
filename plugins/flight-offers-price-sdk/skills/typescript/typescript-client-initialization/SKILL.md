...
- `flight-offers-pricelib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'flight-offers-pricelib';
...
```ts
import { {Resource}Controller } from 'flight-offers-pricelib';
