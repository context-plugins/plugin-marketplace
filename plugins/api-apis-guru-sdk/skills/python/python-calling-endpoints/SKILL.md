...
Open `apiapisguru/apiapisguru_client.py` for the controller property names, then the relevant
`apiapisguru/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from apiapisguru.models.{request_model} import {RequestModel}
...
Open `apiapisguru/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from apiapisguru.models.suite_code_enum import SuiteCodeEnum
...
```python
from apiapisguru.http.http_call_back import HttpCallBack
...
client = ApiApisGuruClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `apiapisguru/pagination/` in the source.

## Finding the right method

- Controller property names are on `ApiApisGuruClient` in `apiapisguru/apiapisguru_client.py`.
- Operation method signatures are in `apiapisguru/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `apiapisguru/models/`; enum types have `from_value()`.
