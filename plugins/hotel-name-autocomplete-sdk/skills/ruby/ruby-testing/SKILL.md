...
def stub_client(stubs)
  ...
  HotelNameAutocomplete::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises HotelNameAutocomplete::APIException do
...
def build_test_client
  HotelNameAutocomplete::Client.new(
    environment: HotelNameAutocomplete::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error HotelNameAutocomplete::APIException
```
...
```ruby
expect { call }.to raise_error(HotelNameAutocomplete::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < HotelNameAutocomplete::HttpCallBack
...
client  = HotelNameAutocomplete::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
