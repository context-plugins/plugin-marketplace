...
To see which schemes a specific SDK accepts, read the credential setter methods on `RestApiClient.Builder`
...
- Set credentials **on the `RestApiClient.Builder`** before calling `.build()` — the client is immutable
