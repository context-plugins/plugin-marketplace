...
- `flightoffersprice` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/flight-offers-price-go` — the Go import path for the root package; `github.com/context-plugins/flight-offers-price-go/models` for request/response types.
...
```go
import "github.com/context-plugins/flight-offers-price-go"

client := flightoffersprice.NewClient(
    flightoffersprice.CreateConfiguration(
        flightoffersprice.WithEnvironment(flightoffersprice.PRODUCTION),
...
from environment variables named `FLIGHTOFFERSPRICE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := flightoffersprice.NewClient(flightoffersprice.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `flightoffersprice.PRODUCTION`,
`flightoffersprice.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := flightoffersprice.NewClient(
    flightoffersprice.CreateConfiguration(flightoffersprice.WithEnvironment(flightoffersprice.PRODUCTION)),
)
```
...
client := flightoffersprice.NewClient(
    flightoffersprice.CreateConfiguration(
        flightoffersprice.WithHttpConfiguration(
            flightoffersprice.CreateHttpConfiguration(
                flightoffersprice.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                flightoffersprice.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = flightoffersprice.NewClient(flightoffersprice.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api flightoffersprice.ClientInterface
}

func NewService(api flightoffersprice.ClientInterface) *Service {
