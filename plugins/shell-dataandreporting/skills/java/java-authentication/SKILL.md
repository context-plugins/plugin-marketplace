...
To see which schemes a specific SDK accepts, read the credential setter methods on `ShellDataReportingAPIsClient.Builder`
...
- Set credentials **on the `ShellDataReportingAPIsClient.Builder`** before calling `.build()` — the client is immutable
