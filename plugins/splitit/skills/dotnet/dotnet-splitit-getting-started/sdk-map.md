...
```csharp
using SplititWebApiV3;
using SplititWebApiV3.Servers; // ServerEnvironment lives here

var options = new SplititWebApiV3ClientOptions
{
...
var client = new SplititWebApiV3Client(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddSplititWebApiV3Client(o =>
{
...
`SplititWebApiV3Client.cs`.

<!-- crawler:client-options -->
All `SplititWebApiV3ClientOptions` properties (source: `SplititWebApiV3ClientOptions.cs`):
...
| `Oauth2Sandbox` | `OAuth2ClientCredentials?` |
| `Oauth2SandboxTokenStrategy` | `IOAuth2TokenStrategy<OAuth2ClientCredentials>?` |
| `Oauth2Production` | `OAuth2ClientCredentials?` |
| `Oauth2ProductionTokenStrategy` | `IOAuth2TokenStrategy<OAuth2ClientCredentials>?` |
...
- `SplititWebApiV3Client(HttpClient httpClient, SplititWebApiV3ClientOptions options)`
<!-- /crawler:client-options -->
...
Of **10 operations**, **10 are Case A (typed)** and **0 are Case B (raw)**.
<!-- /crawler:op-stats -->

---

## Operations — by controller (1 groups, 10 operations)
...
|---|---:|---|
| `SplititWebApiV3Client` | 10 | [map/operations/SplititWebApiV3Client.md](map/operations/SplititWebApiV3Client.md) |
<!-- /crawler:ops-table -->
...
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 49 | [`AddressData` … `VerifyAuthorizationResponse`](map/models/records-1-Ad-Ve.md) |
| Unions (`OneOf` / `AnyOf`) — variant factories + `TryGet…` | 0 + 0 | [map/models/unions.md](map/models/unions.md) |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — literal C# member names + wire values | 11 | [map/models/enums.md](map/models/enums.md) |
<!-- /gen:models-table -->
...
|---|---|
| Client & options (root) | `SplititWebApiV3` |
| Records (`Models/`) | `SplititWebApiV3.Models` |
| Enums (`Models/Enums/`) | `SplititWebApiV3.Models.Enums` |
| Error classes (`Errors/`) | `SplititWebApiV3.Errors` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `SplititWebApiV3ClientOptions` (source: `SplititWebApiV3ClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:

| Property | Type | Notes (from the source XML docs) |
|---|---|---|
| `Oauth2Sandbox` | `OAuth2ClientCredentials?` | OAuth 2.0 Client Credentials flow for sandbox environment |
| `Oauth2SandboxTokenStrategy` | `IOAuth2TokenStrategy<OAuth2ClientCredentials>?` | — |
| `Oauth2Production` | `OAuth2ClientCredentials?` | OAuth 2.0 Client Credentials flow for production environment |
| `Oauth2ProductionTokenStrategy` | `IOAuth2TokenStrategy<OAuth2ClientCredentials>?` | — |
