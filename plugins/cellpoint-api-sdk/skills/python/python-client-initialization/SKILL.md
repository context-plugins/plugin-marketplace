...
- `cellpointapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `CellPointAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
...
from cellpointapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from cellpointapi.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `cellpointapi/cellpointapi_client.py` in the cloned source.
...
`from_environment` in `cellpointapi/configuration.py` for the exact names.
...
Open `cellpointapi/cellpointapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `cellpointapi/configuration.py`. Use the constant for the desired
...
`cellpointapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
