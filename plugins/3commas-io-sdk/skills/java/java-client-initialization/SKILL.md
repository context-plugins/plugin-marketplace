...
- `3commasIoClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `io.m3commas.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`3commasIoClient.Builder` class. A minimal initialization:

```java
import io.m3commas.api.3commasIoClient;
import io.m3commas.api.Environment;

3commasIoClient client = new 3commasIoClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `3commasIoClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `3commasIoClient.java` |

Call `.build()` to produce an immutable `3commasIoClient` instance.
...
public class ApiConfig {
    @Bean
    public 3commasIoClient apiClient() {
        return new 3commasIoClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired 3commasIoClient client) {
        client.shutdown();
    }
}
```

Inject `3commasIoClient` into your services as a normal Spring dependency. Build only one bean.
