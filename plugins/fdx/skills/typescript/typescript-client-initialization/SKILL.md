...
- `fdx-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'fdx-apilib';
...
```ts
import { {Resource}Api } from 'fdx-apilib';
