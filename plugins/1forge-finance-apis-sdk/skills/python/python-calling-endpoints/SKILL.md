...
Open `1forgefinanceapis/1forgefinanceapis_client.py` for the controller property names, then the relevant
`1forgefinanceapis/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from 1forgefinanceapis.models.{request_model} import {RequestModel}
...
Open `1forgefinanceapis/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from 1forgefinanceapis.models.suite_code_enum import SuiteCodeEnum
...
```python
from 1forgefinanceapis.http.http_call_back import HttpCallBack
...
client = 1ForgeFinanceAPIsClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `1forgefinanceapis/pagination/` in the source.

## Finding the right method

- Controller property names are on `1ForgeFinanceAPIsClient` in `1forgefinanceapis/1forgefinanceapis_client.py`.
- Operation method signatures are in `1forgefinanceapis/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `1forgefinanceapis/models/`; enum types have `from_value()`.
