...
- `awsmigrationhub` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/aws-migration-hub-go` — the Go import path for the root package; `github.com/context-plugins/aws-migration-hub-go/models` for request/response types.
...
```go
import "github.com/context-plugins/aws-migration-hub-go"

client := awsmigrationhub.NewClient(
    awsmigrationhub.CreateConfiguration(
        awsmigrationhub.WithEnvironment(awsmigrationhub.PRODUCTION),
...
from environment variables named `AWSMIGRATIONHUB_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := awsmigrationhub.NewClient(awsmigrationhub.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `awsmigrationhub.PRODUCTION`,
`awsmigrationhub.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := awsmigrationhub.NewClient(
    awsmigrationhub.CreateConfiguration(awsmigrationhub.WithEnvironment(awsmigrationhub.PRODUCTION)),
)
```
...
client := awsmigrationhub.NewClient(
    awsmigrationhub.CreateConfiguration(
        awsmigrationhub.WithHttpConfiguration(
            awsmigrationhub.CreateHttpConfiguration(
                awsmigrationhub.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                awsmigrationhub.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = awsmigrationhub.NewClient(awsmigrationhub.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api awsmigrationhub.ClientInterface
}

func NewService(api awsmigrationhub.ClientInterface) *Service {
