...
`coingeckodemoapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `coingeckodemoapi/exceptions/` and operation docstrings.
...
```python
from coingeckodemoapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `coingeckodemoapi/exceptions/`.
...
```python
from coingeckodemoapi.exceptions.api_exception import APIException
from coingeckodemoapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`coingeckodemoapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`coingeckodemoapi/exceptions/` for the class.
...
`coingeckodemoapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `coingeckodemoapi/exceptions/`
  or a `ValueError` — check the auth handler in `coingeckodemoapi/http/auth/` for the exact type.
