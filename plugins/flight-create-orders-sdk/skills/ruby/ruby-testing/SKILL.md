...
client = stub_client(stubs)
assert_raises FlightCreateOrders::APIException do
...
def build_test_client
  FlightCreateOrders::Client.new(
    environment: FlightCreateOrders::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error FlightCreateOrders::APIException
```
...
```ruby
expect { call }.to raise_error(FlightCreateOrders::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < FlightCreateOrders::HttpCallBack
...
client  = FlightCreateOrders::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
