...
- `AWSMigrationHub` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `aws_migration_hub` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'aws_migration_hub'
include AWSMigrationHub
...
The authoritative list is the `initialize` signature in `lib/aws_migration_hub/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/aws_migration_hub/configuration.rb`
(`build_default_config_from_env`) and `lib/aws_migration_hub/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/aws_migration_hub.rb`) and store it in a
