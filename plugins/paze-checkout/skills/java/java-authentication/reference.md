...
against the `PazeCheckoutAPIClient.Builder` setters, the `com.paze.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `PazeCheckoutAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
PazeCheckoutAPIClient client = new PazeCheckoutAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopePazeCheckoutAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new PazeCheckoutAPIClient.Builder()
