...
Open `coingeckodemoapi/coingeckodemoapi_client.py` for the controller property names, then the relevant
`coingeckodemoapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from coingeckodemoapi.models.{request_model} import {RequestModel}
...
Open `coingeckodemoapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from coingeckodemoapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from coingeckodemoapi.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `coingeckodemoapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `CoinGeckoDemoAPIClient` in `coingeckodemoapi/coingeckodemoapi_client.py`.
- Operation method signatures are in `coingeckodemoapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `coingeckodemoapi/models/`; enum types have `from_value()`.
