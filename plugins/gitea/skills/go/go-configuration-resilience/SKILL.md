    ...
    gitea.CreateConfiguration(
        gitea.WithHttpConfiguration(
            gitea.CreateHttpConfiguration(
                gitea.WithTimeout(30),
                gitea.WithRetryConfiguration(
                    gitea.CreateRetryConfiguration(
                        gitea.WithMaxRetryAttempts(3),
                        gitea.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := gitea.CreateRetryConfiguration(
    gitea.WithMaxRetryAttempts(5),
    gitea.WithRetryInterval(1),
    gitea.WithBackoffFactor(2),
    gitea.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    gitea.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := gitea.NewClient(
    gitea.CreateConfiguration(
        gitea.WithHttpConfiguration(
            gitea.CreateHttpConfiguration(
                gitea.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
