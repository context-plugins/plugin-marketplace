...
│   └── com.adyen.paltest.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.adyen.paltest.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.adyen.paltest.http.client.HttpContext`, which carries:
