...
- `openmeteoweatherforecastapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `OpenMeteoWeatherForecastAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `OpenMeteoWeatherForecastAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from openmeteoweatherforecastapi.openmeteoweatherforecastapi_client import OpenMeteoWeatherForecastAPIClient
from openmeteoweatherforecastapi.configuration import Environment
from openmeteoweatherforecastapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from openmeteoweatherforecastapi.configuration import Configuration, Environment
...
client = OpenMeteoWeatherForecastAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `openmeteoweatherforecastapi/openmeteoweatherforecastapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `OpenMeteoWeatherForecastAPIClient` and `Configuration` that reads
...
client = OpenMeteoWeatherForecastAPIClient.from_environment()
...
`from_environment` in `openmeteoweatherforecastapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `OpenMeteoWeatherForecastAPIClient`. Access them as attributes —
...
Open `openmeteoweatherforecastapi/openmeteoweatherforecastapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `openmeteoweatherforecastapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from openmeteoweatherforecastapi.configuration import Environment

client = OpenMeteoWeatherForecastAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`openmeteoweatherforecastapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = OpenMeteoWeatherForecastAPIClient(config=new_config)
```
