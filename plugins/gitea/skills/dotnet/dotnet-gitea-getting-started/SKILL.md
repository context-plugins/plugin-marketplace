...
<!-- crawler:auth -->
| Auth | Credentials properties on `GiteaApiClientOptions`: `AccessToken: string?`, `AuthorizationHeaderToken: string?`, `BasicAuth: BasicAuthCredentials?`, `SudoHeader: string?`, `SudoParam: string?`, `Totpheader: string?`, `Token: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using GiteaApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/gitea-csharp-sdk
dotnet add reference gitea-csharp-sdk/GiteaApi.csproj
```
...
temp directory** (`<temp>/gitea-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/gitea-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/gitea-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/gitea-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\gitea-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/gitea-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/gitea-dotnet-src/`, **never** in the project repo, and its path never goes into
`gitea-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `GiteaApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new GiteaApiClient(...)`, build its options, or DI-register via
   `AddGiteaApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `GiteaApiClientOptions` — the map's *Servers & auth* section
