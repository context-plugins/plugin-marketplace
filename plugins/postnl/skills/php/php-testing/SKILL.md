...
> `PostnlEcommerceClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): PostnlEcommerceClient
{
...
```php
use PostnlEcommerceLib\Exceptions\{OperationException};
use PostnlEcommerceLib\Exceptions\ApiException;
...
app()->instance(PostnlEcommerceClient::class, clientWithResponses([
