...
> Throughout, `boku` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := boku.NewClient(
    boku.CreateConfiguration(
        boku.WithBasicAuthCredentials(
            boku.NewBasicAuthCredentials(
...
boku.WithApiKeyCredentials(boku.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
boku.WithApiHeaderCredentials(boku.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
boku.WithOAuthBearerTokenCredentials(
    boku.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
