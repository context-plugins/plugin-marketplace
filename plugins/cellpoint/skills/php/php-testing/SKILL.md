...
> `CellpointClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): CellpointClient
{
...
```php
use CellpointLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use CellpointLib\Exceptions\{OperationException};
use CellpointLib\Exceptions\ApiException;
...
app()->instance(CellpointClient::class, clientWithResponses([
