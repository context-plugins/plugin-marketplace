...
> `TesserApiV1Client`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): TesserApiV1Client
{
...
```php
use TesserApiV1Lib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use TesserApiV1Lib\Exceptions\{OperationException};
use TesserApiV1Lib\Exceptions\ApiException;
...
app()->instance(TesserApiV1Client::class, clientWithResponses([
