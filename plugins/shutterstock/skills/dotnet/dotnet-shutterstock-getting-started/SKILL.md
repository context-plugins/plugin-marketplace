...
<!-- crawler:auth -->
| Auth | Credentials properties on `ShutterstockApiExplorerClientOptions`: `Basic: BasicAuthCredentials?`, `CustomerAccessCode: OAuth2AuthorizationCodeCredentials?`, `CustomerAccessCodeTokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using ShutterstockApiExplorer.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/shutterstock-csharp-sdk
dotnet add reference shutterstock-csharp-sdk/ShutterstockApiExplorer.csproj
```
...
temp directory** (`<temp>/shutterstock-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/shutterstock-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/shutterstock-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/shutterstock-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\shutterstock-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/shutterstock-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/shutterstock-dotnet-src/`, **never** in the project repo, and its path never goes into
`shutterstock-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new ShutterstockApiExplorerClient(...)`, build its options, or DI-register via
   `AddShutterstockApiExplorerClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `ShutterstockApiExplorerClientOptions` — the map's *Servers & auth* section
