---
name: dotnet-payquicker-getting-started
description: Payquicker .NET SDK identity and lookup layer for the dotnet-payquicker-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Payquicker .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-payquicker-sdk` helper
...
| --- | --- |
| API | Payquicker |
| Source repo | https://github.com/context-plugins/payquicker-csharp-sdk (branch `main`) |
| Root namespace | `PqApiV2` (the `using` namespace) |
| Client class | `PqApiV2Client` |
| Options class | `PqApiV2ClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `PqApiV2ClientOptions`: `ServerCredentials: OAuth2ClientCredentials?`, `ServerTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?`, `Clientside: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `Production`, `Sandbox`, `Uat`, `Development` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using PqApiV2.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/payquicker-csharp-sdk
dotnet add reference payquicker-csharp-sdk/PqApiV2.csproj
```
...
temp directory** (`<temp>/payquicker-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/payquicker-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/payquicker-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/payquicker-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\payquicker-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/payquicker-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/payquicker-dotnet-src/`, **never** in the project repo, and its path never goes into
`payquicker-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `PqApiV2Client.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new PqApiV2Client(...)`, build its options, or DI-register via
   `AddPqApiV2Client`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `PqApiV2ClientOptions` — the map's *Servers & auth* section
