...
description: "CoinGecko Demo API TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `coin-gecko-demo-api`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any CoinGecko Demo API TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the CoinGecko Demo API TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing CoinGecko Demo API TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-coin-gecko` for the workflow that wraps this skill.
...
| Package name (what you install, and what you import) | `coin-gecko-demo-api` — **not on npm**; built from source (see *Install*) |
| Import specifier | `coin-gecko-demo-api` — the package root is the **only** entry; deep imports do not resolve |
| Version | `3.0.0` (API spec version `3.0.0`) |
| Client class | `CoinGeckoDemoApiClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new CoinGeckoDemoApiClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new CoinGeckoDemoApiClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `headerAuth` · `queryAuth`. `timeout` defaults to `60_000` ms |
...
```ts
import { CoinGeckoDemoApiClient, ServerEnvironment, ResponseError, CoinGeckoDemoApiError } from "coin-gecko-demo-api";
import type { ClientOptions, AssetPlatform } from "coin-gecko-demo-api";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `coin-gecko-demo-api/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **From CommonJS**, the typed spelling is `import sdk = require("coin-gecko-demo-api")`. A plain `require` destructure runs but yields `any`.
...
```ts
const client = new CoinGeckoDemoApiClient({
...
```bash
node -e "console.log(require.resolve('coin-gecko-demo-api/package.json'))"
```

Failing that it is at `node_modules/coin-gecko-demo-api/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/coin-gecko-demo-api/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/coin-gecko-demo-api/`:
...
Beyond the usual signatures and model members, a contract sheet for the CoinGecko Demo API TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
