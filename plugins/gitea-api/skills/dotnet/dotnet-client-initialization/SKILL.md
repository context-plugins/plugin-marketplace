...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddGiteaApiClient extension. Use the moment you write `new GiteaApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `GiteaApiClient` — the single public client class (the generator reads this prefix from the SDK's
  `.csproj` name, so it is already the real one).
- `GiteaApiClientOptions` — its options class.
- `GiteaApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public GiteaApiClient(HttpClient httpClient, GiteaApiClientOptions options)
```
...
```csharp
public class GiteaApiClientOptions
{
...
var client = new GiteaApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddGiteaApiClient`, which registers the
...
builder.Services.AddGiteaApiClient(options =>
{
...
```csharp
public sealed class MyService(GiteaApiClient client)
{
