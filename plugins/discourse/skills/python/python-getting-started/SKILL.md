...
- Omitting `base_url` gives you **`https://{defaultHost}`**, silently. A caller who believes they configured a different host and did not gets that host's behaviour with live credentials.
...
| --- | --- | --- | --- |
| `client.admin` | `Admin` / `AsyncAdmin` | 11 | `activate_user` · `admin_get_user` · `admin_list_users` · `admin_list_users_flag` · `anonymize_user` · `deactivate_user` · … |
| `client.backups` | `Backups` / `AsyncBackups` | 4 | `create_backup` · `download_backup` · `get_backups` · `send_download_backup_email` |
| `client.badges` | `Badges` / `AsyncBadges` | 5 | `admin_list_badges` · `create_badge` · `delete_badge` · `list_user_badges` · `update_badge` |
| `client.categories` | `Categories` / `AsyncCategories` | 6 | `create_category` · `get_category` · `get_site` · `list_categories` · `list_category_topics` · `update_category` |
| `client.discourse_calendar_events` | `DiscourseCalendarEvents` / `AsyncDiscourseCalendarEvents` | 2 | `export_events_ics` · `list_events` |
| `client.groups` | `Groups` / `AsyncGroups` | 9 | `add_group_members` · `create_group` · `delete_group` · `get_group` · `get_group_by_id` · `list_group_members` · … |
| `client.invites` | `Invites` / `AsyncInvites` | 4 | `create_invite` · `create_multiple_invites` · `invite_group_to_topic` · `invite_to_topic` |
| `client.notifications` | `Notifications` / `AsyncNotifications` | 2 | `get_notifications` · `mark_notifications_as_read` |
| `client.posts` | `Posts` / `AsyncPosts` | 8 | `create_topic_post_pm` · `delete_post` · `get_post` · `list_posts` · `lock_post` · `perform_post_action` · … |
| `client.private_messages` | `PrivateMessages` / `AsyncPrivateMessages` | 3 | `create_topic_post_pm` · `get_user_sent_private_messages` · `list_user_private_messages` |
| `client.search` | `Search` / `AsyncSearch` | 1 | `search` |
| `client.site` | `Site` / `AsyncSite` | 2 | `get_site` · `get_site_basic_info` |
| `client.tags` | `Tags` / `AsyncTags` | 6 | `create_tag_group` · `get_tag` · `get_tag_group` · `list_tag_groups` · `list_tags` · `update_tag_group` |
| `client.topics` | `Topics` / `AsyncTopics` | 15 | `bookmark_topic` · `create_topic_post_pm` · `create_topic_timer` · `get_specific_posts_from_topic` · `get_topic` · `get_topic_by_external_id` · … |
| `client.uploads` | `Uploads` / `AsyncUploads` | 7 | `abort_multipart` · `batch_presign_multipart_parts` · `complete_external_upload` · `complete_multipart` · `create_multipart_upload` · `create_upload` · … |
| `client.users` | `Users` / `AsyncUsers` | 25 | `activate_user` · `admin_get_user` · `admin_list_users` · `admin_list_users_flag` · `anonymize_user` · `change_password` · … |
