...
against the `GiteaClient.Builder` setters, the `com.example.www.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GiteaClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
