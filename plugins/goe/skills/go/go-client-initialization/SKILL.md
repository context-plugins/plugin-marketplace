...
- `goe` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/goe-go-sdk` — the Go import path for the root package; `github.com/context-plugins/goe-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/goe-go-sdk"

client := goe.NewClient(
    goe.CreateConfiguration(
        goe.WithEnvironment(goe.PRODUCTION),
...
from environment variables named `GOE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := goe.NewClient(goe.CreateConfigurationFromEnvironment())
```
...
```go
client := goe.NewClient(
    goe.CreateConfiguration(goe.WithEnvironment(goe.PRODUCTION)),
)
```
...
client := goe.NewClient(
    goe.CreateConfiguration(
        goe.WithHttpConfiguration(
            goe.CreateHttpConfiguration(
                goe.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                goe.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = goe.NewClient(goe.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api goe.ClientInterface
}

func NewService(api goe.ClientInterface) *Service {
