...
`zalandoshop/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `zalandoshop/exceptions/` and operation docstrings.
...
```python
from zalandoshop.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `zalandoshop/exceptions/`.
...
```python
from zalandoshop.exceptions.api_exception import APIException
from zalandoshop.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`zalandoshop/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`zalandoshop/exceptions/` for the class.
...
`zalandoshop/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `zalandoshop/exceptions/`
  or a `ValueError` — check the auth handler in `zalandoshop/http/auth/` for the exact type.
