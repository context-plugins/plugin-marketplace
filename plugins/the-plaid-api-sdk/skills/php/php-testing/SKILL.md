...
> `ThePlaidAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ThePlaidAPIClient
{
...
```php
use ThePlaidAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use ThePlaidAPILib\Exceptions\{OperationException};
use ThePlaidAPILib\Exceptions\ApiException;
...
app()->instance(ThePlaidAPIClient::class, clientWithResponses([
