...
`testing` + `testify/require` for reference; substitute the real `slack` and names.
...
import (
    ...
    "github.com/context-plugins/slack-go-sdk"   // root package slack
)
...
func clientReturning(status int, body string) (slack.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := slack.NewClient(
        slack.CreateConfiguration(
            slack.WithHttpConfiguration(
                slack.CreateHttpConfiguration(slack.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/slack-go-sdk/errors"
...
client := slack.NewClient(
    slack.CreateConfiguration(
        slack.WithHttpConfiguration(
            slack.CreateHttpConfiguration(slack.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
