...
- `FrankfurterApiClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `dev.frankfurter.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`FrankfurterApiClient.Builder` class. A minimal initialization:

```java
import dev.frankfurter.api.FrankfurterApiClient;
import dev.frankfurter.api.Environment;

FrankfurterApiClient client = new FrankfurterApiClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `FrankfurterApiClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `FrankfurterApiClient.java` |

Call `.build()` to produce an immutable `FrankfurterApiClient` instance.
...
public class ApiConfig {
    @Bean
    public FrankfurterApiClient apiClient() {
        return new FrankfurterApiClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired FrankfurterApiClient client) {
        client.shutdown();
    }
}
```

Inject `FrankfurterApiClient` into your services as a normal Spring dependency. Build only one bean.
