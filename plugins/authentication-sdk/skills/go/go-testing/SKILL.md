...
`testing` + `testify/require` for reference; substitute the real `authentication` and names.
...
import (
    ...
    "github.com/context-plugins/authentication-go"   // root package authentication
)
...
func clientReturning(status int, body string) (authentication.ClientInterface, *stubTransport) {
    ...
    client := authentication.NewClient(
        authentication.CreateConfiguration(
            authentication.WithHttpConfiguration(
                authentication.CreateHttpConfiguration(authentication.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/authentication-go/errors"
...
client := authentication.NewClient(
    authentication.CreateConfiguration(
        authentication.WithHttpConfiguration(
            authentication.CreateHttpConfiguration(authentication.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
