...
kwargs to the `AdvancedBillingClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `maxio/maxio_client.py` and `maxio/http/auth/` in
the cloned source.

## Basic auth

```python
from maxio.http.auth.basic_auth import BasicAuthCredentials
...
```python
from maxio.http.auth.api_key import ApiKeyCredentials
...
```python
from maxio.http.auth.api_header import ApiHeaderCredentials
...
```python
from maxio.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from maxio.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from maxio.http.auth.o_auth_acg import OAuthACGCredentials
from maxio.models.o_auth_scope_maxio_enum import OAuthScope{Pkg}Enum
...
client = AdvancedBillingClient(o_auth_acg_credentials=creds)
...
```python
from maxio.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AdvancedBillingClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AdvancedBillingClient.from_environment()`) read each credential
...
Grep `from_environment` in `maxio/configuration.py` and each `maxio/http/auth/*.py` for the exact
