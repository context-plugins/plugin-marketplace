...
`PazeCheckoutApiClient`/`PazeCheckoutApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `PazeCheckoutApiClient`,
...
static PazeCheckoutApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new PazeCheckoutApiClient(new HttpClient(handler), new PazeCheckoutApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using PazeCheckoutApi.Errors;              // {Operation}Error types
...
```csharp
using PazeCheckoutApi.Core.Exceptions;
using PazeCheckoutApi.Core.ErrorResponse;
...
using PazeCheckoutApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new PazeCheckoutApiClient(new HttpClient(handler), new PazeCheckoutApiClientOptions());
...
- For DI-based code, the SDK's `AddPazeCheckoutApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `PazeCheckoutApiClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<PazeCheckoutApiClient>();
  ```
