...
```python
from goe.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `goe/utilities/union_type_lookup.py`) and each candidate model
...
```python
from goe.models.animal import Cat, Dog
...
```python
from goe.models.animal import Animal, Cat, Dog
