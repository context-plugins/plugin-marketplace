...
your SDK source (`deepgram/models/`).
...
```python
from deepgram.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `deepgram/utilities/union_type_lookup.py`) and each candidate model
...
```python
from deepgram.models.animal import Cat, Dog
...
```python
from deepgram.models.animal import Animal, Cat, Dog
