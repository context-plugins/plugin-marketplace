...
- `FlightCheckInLinksClient` — the single public client class (e.g. `FooClient`).
- `FlightCheckInLinksClientOptions` — its options class.
- `FlightCheckInLinks` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public FlightCheckInLinksClient(HttpClient httpClient, FlightCheckInLinksClientOptions options)
```
...
```csharp
public class FlightCheckInLinksClientOptions
{
...
```csharp
using FlightCheckInLinks;
using FlightCheckInLinks.Servers;

var options = new FlightCheckInLinksClientOptions
{
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddFlightCheckInLinksClient`, which registers the
...
builder.Services.AddFlightCheckInLinksClient(options =>
{
...
```csharp
public sealed class MyService(FlightCheckInLinksClient client)
{
