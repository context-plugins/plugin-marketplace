...
- `finnhub-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'finnhub-apilib';
...
```ts
import { {Resource}Api } from 'finnhub-apilib';
