...
`greenbyte/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `greenbyte/exceptions/` and operation docstrings.
...
```python
from greenbyte.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `greenbyte/exceptions/`.
...
```python
from greenbyte.exceptions.api_exception import APIException
from greenbyte.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`greenbyte/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`greenbyte/exceptions/` for the class.
...
`greenbyte/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `greenbyte/exceptions/`
  or a `ValueError` — check the auth handler in `greenbyte/http/auth/` for the exact type.
