...
against the `ConfigurationAPIClient.Builder` setters, the `com.adyen.balanceplatformapitest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ConfigurationAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ConfigurationAPIClient client = new ConfigurationAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeConfigurationAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new ConfigurationAPIClient.Builder()
