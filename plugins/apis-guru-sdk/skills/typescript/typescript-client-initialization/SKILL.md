...
- `apis-gurulib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'apis-gurulib';
...
```ts
import { {Resource}Controller } from 'apis-gurulib';
