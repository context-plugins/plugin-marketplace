...
- `apisguru` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/apis-guru-go` — the Go import path for the root package; `github.com/context-plugins/apis-guru-go/models` for request/response types.
...
```go
import "github.com/context-plugins/apis-guru-go"

client := apisguru.NewClient(
    apisguru.CreateConfiguration(
        apisguru.WithEnvironment(apisguru.PRODUCTION),
...
from environment variables named `APISGURU_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := apisguru.NewClient(apisguru.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `apisguru.PRODUCTION`,
`apisguru.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := apisguru.NewClient(
    apisguru.CreateConfiguration(apisguru.WithEnvironment(apisguru.PRODUCTION)),
)
```
...
client := apisguru.NewClient(
    apisguru.CreateConfiguration(
        apisguru.WithHttpConfiguration(
            apisguru.CreateHttpConfiguration(
                apisguru.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                apisguru.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = apisguru.NewClient(apisguru.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api apisguru.ClientInterface
}

func NewService(api apisguru.ClientInterface) *Service {
