...
All types below live under `HostedOnboardingApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using HostedOnboardingApi.Core.Configuration;
...
```csharp
using HostedOnboardingApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new HostedOnboardingApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `HostedOnboardingApi.Core.Exceptions`):
...
With DI, the SDK's `AddHostedOnboardingApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddHostedOnboardingApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
