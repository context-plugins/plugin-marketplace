...
```go
klarnapaymentsapiv1.WithOAuthCCGCredentials(
    klarnapaymentsapiv1.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr klarnapaymentsapiv1.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := klarnapaymentsapiv1.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr klarnapaymentsapiv1.OAuthCCGManager) models.OAuthToken {
...
```go
creds := klarnapaymentsapiv1.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := klarnapaymentsapiv1.NewClient(klarnapaymentsapiv1.CreateConfiguration(klarnapaymentsapiv1.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    klarnapaymentsapiv1.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := klarnapaymentsapiv1.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := klarnapaymentsapiv1.NewClient(klarnapaymentsapiv1.CreateConfiguration(klarnapaymentsapiv1.WithOAuthROPCGCredentials(creds)))
...
```go
klarnapaymentsapiv1.WithOAuthBearerTokenCredentials(klarnapaymentsapiv1.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
klarnapaymentsapiv1.WithBasicAuthCredentials(klarnapaymentsapiv1.NewBasicAuthCredentials(username, password))
klarnapaymentsapiv1.WithApiHeaderCredentials(klarnapaymentsapiv1.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
klarnapaymentsapiv1.WithApiKeyCredentials(klarnapaymentsapiv1.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `KLARNAPAYMENTSAPIV1_...` variables — e.g.
`KLARNAPAYMENTSAPIV1_USERNAME`, `KLARNAPAYMENTSAPIV1_PASSWORD`, `KLARNAPAYMENTSAPIV1_TOKEN`, `KLARNAPAYMENTSAPIV1_API_KEY`,
`KLARNAPAYMENTSAPIV1_O_AUTH_CLIENT_ID`, `KLARNAPAYMENTSAPIV1_O_AUTH_CLIENT_SECRET`, `KLARNAPAYMENTSAPIV1_ACCESS_TOKEN`. Grep the
