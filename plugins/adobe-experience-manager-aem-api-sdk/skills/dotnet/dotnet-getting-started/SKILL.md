...
| Project | `AdobeExperienceManagerAemApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `AdobeExperienceManagerAemApi` (the `using` namespace) |
| Client class | `AdobeExperienceManagerAemApiClient` |
| Options class | `AdobeExperienceManagerAemApiClientOptions` |
...
namespaces transitively, so `using AdobeExperienceManagerAemApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/adobe-experience-manager-aem-api-csharp
dotnet add reference adobe-experience-manager-aem-api-csharp/AdobeExperienceManagerAemApi.csproj
```

```csharp
using AdobeExperienceManagerAemApi;          // client + options + ServerEnvironment
using AdobeExperienceManagerAemApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/adobe-experience-manager-aem-api-csharp /tmp/adobe-experience-manager-aem-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/adobe-experience-manager-aem-api-csharp "$env:TEMP\adobe-experience-manager-aem-api-dotnet-src"
```
...
- `AdobeExperienceManagerAemApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/adobe-experience-manager-aem-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\adobe-experience-manager-aem-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `AdobeExperienceManagerAemApiClient`, build its
   `AdobeExperienceManagerAemApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
