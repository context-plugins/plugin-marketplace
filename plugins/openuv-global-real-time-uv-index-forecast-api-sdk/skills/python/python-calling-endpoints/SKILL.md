...
Open `openuvglobalrealtimeuvindexforecastapi/openuvglobalrealtimeuvindexforecastapi_client.py` for the controller property names, then the relevant
`openuvglobalrealtimeuvindexforecastapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from openuvglobalrealtimeuvindexforecastapi.models.{request_model} import {RequestModel}
...
Open `openuvglobalrealtimeuvindexforecastapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from openuvglobalrealtimeuvindexforecastapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from openuvglobalrealtimeuvindexforecastapi.http.http_call_back import HttpCallBack
...
client = OpenUVGlobalRealTimeUVIndexForecastAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `openuvglobalrealtimeuvindexforecastapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `OpenUVGlobalRealTimeUVIndexForecastAPIClient` in `openuvglobalrealtimeuvindexforecastapi/openuvglobalrealtimeuvindexforecastapi_client.py`.
- Operation method signatures are in `openuvglobalrealtimeuvindexforecastapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `openuvglobalrealtimeuvindexforecastapi/models/`; enum types have `from_value()`.
