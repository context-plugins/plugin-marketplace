...
def stub_client(stubs)
  ...
  ShutterstockAPIExplorer::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises ShutterstockAPIExplorer::APIException do
...
def build_test_client
  ShutterstockAPIExplorer::Client.new(
    environment: ShutterstockAPIExplorer::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ShutterstockAPIExplorer::APIException
```
...
```ruby
expect { call }.to raise_error(ShutterstockAPIExplorer::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ShutterstockAPIExplorer::HttpCallBack
...
client  = ShutterstockAPIExplorer::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
