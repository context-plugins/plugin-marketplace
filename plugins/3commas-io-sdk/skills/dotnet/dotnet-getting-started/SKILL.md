...
| --- | --- |
| API | `3commas.io` |
| Project | `M3commasIo.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `M3commasIo` (the `using` namespace) |
| Client class | `M3commasIoClient` |
| Options class | `M3commasIoClientOptions` |
...
namespaces transitively, so `using M3commasIo.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/3commas-io-csharp
dotnet add reference 3commas-io-csharp/M3commasIo.csproj
```

```csharp
using M3commasIo;          // client + options + ServerEnvironment
using M3commasIo.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/3commas-io-csharp /tmp/3commas-io-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/3commas-io-csharp "$env:TEMP\3commas-io-dotnet-src"
```
...
- `M3commasIoClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/3commas-io-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\3commas-io-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `M3commasIoClient`, build its
   `M3commasIoClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
