...
- `fundapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/fund-api-go` — the Go import path for the root package; `github.com/context-plugins/fund-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/fund-api-go"

client := fundapi.NewClient(
    fundapi.CreateConfiguration(
        fundapi.WithEnvironment(fundapi.PRODUCTION),
...
from environment variables named `FUNDAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := fundapi.NewClient(fundapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `fundapi.PRODUCTION`,
`fundapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := fundapi.NewClient(
    fundapi.CreateConfiguration(fundapi.WithEnvironment(fundapi.PRODUCTION)),
)
```
...
client := fundapi.NewClient(
    fundapi.CreateConfiguration(
        fundapi.WithHttpConfiguration(
            fundapi.CreateHttpConfiguration(
                fundapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                fundapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = fundapi.NewClient(fundapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api fundapi.ClientInterface
}

func NewService(api fundapi.ClientInterface) *Service {
