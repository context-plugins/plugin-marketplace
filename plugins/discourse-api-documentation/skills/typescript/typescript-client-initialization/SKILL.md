...
- `discourse-api-documentationlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'discourse-api-documentationlib';
...
Environments are members of an `enum Environment` in `src/configuration.ts`. **Read the enum before you
name a member.** The generator names only the first one meaningfully — `Environment.Production` — and
every additional server becomes a positional placeholder: `Environment.Environment2`,
`Environment.Environment3`, … There is no `Environment.Testing`, `Environment.Test`, or
`Environment.Sandbox`, whatever the API's own docs call its hosts, and an API with one server gets
...
```ts
import { {Resource}Api } from 'discourse-api-documentationlib';
