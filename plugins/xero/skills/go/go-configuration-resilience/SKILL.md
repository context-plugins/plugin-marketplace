    ...
    xero.CreateConfiguration(
        xero.WithHttpConfiguration(
            xero.CreateHttpConfiguration(
                xero.WithTimeout(30),
                xero.WithRetryConfiguration(
                    xero.CreateRetryConfiguration(
                        xero.WithMaxRetryAttempts(3),
                        xero.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := xero.CreateRetryConfiguration(
    xero.WithMaxRetryAttempts(5),
    xero.WithRetryInterval(1),
    xero.WithBackoffFactor(2),
    xero.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    xero.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := xero.NewClient(
    xero.CreateConfiguration(
        xero.WithHttpConfiguration(
            xero.CreateHttpConfiguration(
                xero.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
