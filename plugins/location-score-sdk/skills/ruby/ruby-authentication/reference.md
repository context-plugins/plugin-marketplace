...
Confirm every name against `lib/location_score/client.rb`, `lib/location_score/configuration.rb`, the
`lib/location_score/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
