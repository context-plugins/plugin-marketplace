...
```ruby
client = TravelRecommendationsAPI::Client.new(
  max_retries:    3,
...
```ruby
client = TravelRecommendationsAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = TravelRecommendationsAPI::Client.new(adapter: :net_http)
```
...
client = TravelRecommendationsAPI::Client.new(connection: conn)
```
...
client = TravelRecommendationsAPI::Client.new(environment: Environment::PRODUCTION)
client = TravelRecommendationsAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = TravelRecommendationsAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = TravelRecommendationsAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = TravelRecommendationsAPI::ProxySettings.from_env
client = TravelRecommendationsAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < TravelRecommendationsAPI::HttpCallBack
...
client = TravelRecommendationsAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = TravelRecommendationsAPI::Client.from_env
# Overrides take precedence over env vars:
client = TravelRecommendationsAPI::Client.from_env(timeout: 30, max_retries: 3)
```
