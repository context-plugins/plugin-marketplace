...
client = stub_client(stubs)
assert_raises SpotifyWebAPI::APIException do
...
BASE = SpotifyWebAPI::Configuration.new(environment: SpotifyWebAPI::Environment::PRODUCTION).get_base_uri

def build_test_client
  SpotifyWebAPI::Client.new(
    environment: SpotifyWebAPI::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error SpotifyWebAPI::APIException
```
...
```ruby
expect { call }.to raise_error(SpotifyWebAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < SpotifyWebAPI::HttpCallBack
...
client  = SpotifyWebAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
