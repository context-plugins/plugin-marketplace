...
Confirm every name against `lib/flight_offers_price/client.rb`, `lib/flight_offers_price/configuration.rb`, the
`lib/flight_offers_price/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
