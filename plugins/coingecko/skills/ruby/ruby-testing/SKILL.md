...
client = stub_client(stubs)
assert_raises Coingecko::APIException do
...
BASE = Coingecko::Configuration.new(environment: Coingecko::Environment::PRODUCTION).get_base_uri

def build_test_client
  Coingecko::Client.new(
    environment: Coingecko::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Coingecko::APIException
```
...
```ruby
expect { call }.to raise_error(Coingecko::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Coingecko::HttpCallBack
...
client  = Coingecko::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
