...
against the `TwilioAPIsClient.Builder` setters, the `com.twilio.accounts.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `TwilioAPIsClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
TwilioAPIsClient client = new TwilioAPIsClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeTwilioAPIsEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
