...
All types below live under `NetworkAdminManagementClient.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using NetworkAdminManagementClient.Core.Configuration;
...
```csharp
using NetworkAdminManagementClient.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new NetworkAdminManagementClientClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `NetworkAdminManagementClient.Core.Exceptions`):
...
With DI, the SDK's `AddNetworkAdminManagementClientClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddNetworkAdminManagementClientClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
