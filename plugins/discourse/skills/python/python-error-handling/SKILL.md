...
`discourse/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `discourse/exceptions/` and operation docstrings.
...
```python
from discourse.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `discourse/exceptions/`.
...
```python
from discourse.exceptions.api_exception import APIException
from discourse.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`discourse/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`discourse/exceptions/` for the class.
...
`discourse/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `discourse/exceptions/`
  or a `ValueError` — check the auth handler in `discourse/http/auth/` for the exact type.
