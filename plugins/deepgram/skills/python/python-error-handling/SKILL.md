...
`deepgram/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `deepgram/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `deepgram/exceptions/`.
...
```python
from deepgram.exceptions.api_exception import APIException
from deepgram.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`deepgram/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`deepgram/exceptions/` for the class.
...
`deepgram/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `deepgram/exceptions/`
  or a `ValueError` — check the auth handler in `deepgram/http/auth/` for the exact type.
