...
> `FirecrawlClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FirecrawlClient
{
...
app()->instance(FirecrawlClient::class, clientWithResponses([
