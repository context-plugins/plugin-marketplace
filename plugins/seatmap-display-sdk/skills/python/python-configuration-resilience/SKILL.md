...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `SeatmapDisplayClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`seatmapdisplay/configuration.py` in the cloned source.
...
```python
import requests
from seatmapdisplay.configuration import Configuration
...
`seatmapdisplay/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`seatmapdisplay/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `seatmapdisplay/configuration.py`
...
`NumberPagedResponse`) live in `seatmapdisplay/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `seatmapdisplay/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
