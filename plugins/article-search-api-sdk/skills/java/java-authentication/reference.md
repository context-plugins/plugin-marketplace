...
against the `ArticleSearchAPIClient.Builder` setters, the `{rootpkg}.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ArticleSearchAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ArticleSearchAPIClient client = new ArticleSearchAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeArticleSearchAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new ArticleSearchAPIClient.Builder()
