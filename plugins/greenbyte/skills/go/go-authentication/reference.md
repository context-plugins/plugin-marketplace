...
```go
greenbyte.WithOAuthCCGCredentials(
    greenbyte.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr greenbyte.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := greenbyte.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr greenbyte.OAuthCCGManager) models.OAuthToken {
...
```go
creds := greenbyte.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := greenbyte.NewClient(greenbyte.CreateConfiguration(greenbyte.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    greenbyte.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := greenbyte.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := greenbyte.NewClient(greenbyte.CreateConfiguration(greenbyte.WithOAuthROPCGCredentials(creds)))
...
```go
greenbyte.WithOAuthBearerTokenCredentials(greenbyte.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
greenbyte.WithBasicAuthCredentials(greenbyte.NewBasicAuthCredentials(username, password))
greenbyte.WithApiHeaderCredentials(greenbyte.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
greenbyte.WithApiKeyCredentials(greenbyte.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `GREENBYTE_...` variables — e.g.
`GREENBYTE_USERNAME`, `GREENBYTE_PASSWORD`, `GREENBYTE_TOKEN`, `GREENBYTE_API_KEY`,
`GREENBYTE_O_AUTH_CLIENT_ID`, `GREENBYTE_O_AUTH_CLIENT_SECRET`, `GREENBYTE_ACCESS_TOKEN`. Grep the
