...
To see which schemes a specific SDK accepts, read the credential setter methods on `SlackWebAPIClient.Builder`
...
- Set credentials **on the `SlackWebAPIClient.Builder`** before calling `.build()` — the client is immutable
