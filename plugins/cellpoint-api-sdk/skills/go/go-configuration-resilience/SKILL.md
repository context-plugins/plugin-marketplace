    ...
    cellpointapi.CreateConfiguration(
        cellpointapi.WithHttpConfiguration(
            cellpointapi.CreateHttpConfiguration(
                cellpointapi.WithTimeout(30),
                cellpointapi.WithRetryConfiguration(
                    cellpointapi.CreateRetryConfiguration(
                        cellpointapi.WithMaxRetryAttempts(3),
                        cellpointapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := cellpointapi.CreateRetryConfiguration(
    cellpointapi.WithMaxRetryAttempts(5),
    cellpointapi.WithRetryInterval(1),
    cellpointapi.WithBackoffFactor(2),
    cellpointapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    cellpointapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := cellpointapi.NewClient(
    cellpointapi.CreateConfiguration(
        cellpointapi.WithHttpConfiguration(
            cellpointapi.CreateHttpConfiguration(
                cellpointapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
