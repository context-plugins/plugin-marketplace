...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `boku`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'boku';
import { ApiError } from 'boku';
import { {Operation}Error } from 'boku/errors';
