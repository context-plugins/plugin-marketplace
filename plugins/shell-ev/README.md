...
Integrate and consume the **APIMatic-generated shell-ev SDK** (the "APIMATIC v3.0" generic
...
(`dotnet-shell-ev-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-shell-ev`** router skill routes all
C#/.NET SDK work to the single **`dotnet-shell-ev-sdk`** agent (.NET-only), which answers
