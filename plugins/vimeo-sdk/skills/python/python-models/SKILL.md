...
```python
from vimeo.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `vimeo/utilities/union_type_lookup.py`) and each candidate model
...
```python
from vimeo.models.animal import Cat, Dog
...
```python
from vimeo.models.animal import Animal, Cat, Dog
