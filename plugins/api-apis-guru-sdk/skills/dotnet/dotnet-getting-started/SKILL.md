...
| --- | --- |
| API | `api.apis.guru` |
| Project | `FlightMostTraveledDestinations.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `FlightMostTraveledDestinations` (the `using` namespace) |
| Client class | `FlightMostTraveledDestinationsClient` |
| Options class | `FlightMostTraveledDestinationsClientOptions` |
...
namespaces transitively, so `using FlightMostTraveledDestinations.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/api-apis-guru-csharp-f09838
dotnet add reference api-apis-guru-csharp-f09838/FlightMostTraveledDestinations.csproj
```

```csharp
using FlightMostTraveledDestinations;          // client + options + ServerEnvironment
using FlightMostTraveledDestinations.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-f09838 /tmp/api-apis-guru-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-f09838 "$env:TEMP\api-apis-guru-dotnet-src"
```
...
- `FlightMostTraveledDestinationsClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/api-apis-guru-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\api-apis-guru-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `FlightMostTraveledDestinationsClient`, build its
   `FlightMostTraveledDestinationsClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
