...
- `apimanagementclient` — the root package name (e.g. `multiauthsample`, `batester`).
- `ApiManagementClientClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `ApiManagementClientClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from apimanagementclient.apimanagementclient_client import ApiManagementClientClient
from apimanagementclient.configuration import Environment
from apimanagementclient.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = ApiManagementClientClient(
...
```python
from apimanagementclient.configuration import Configuration, Environment
...
client = ApiManagementClientClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `apimanagementclient/apimanagementclient_client.py` in the cloned source.
...
`from_environment()` is a class method on both `ApiManagementClientClient` and `Configuration` that reads
...
client = ApiManagementClientClient.from_environment()
...
`from_environment` in `apimanagementclient/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `ApiManagementClientClient`. Access them as attributes —
...
Open `apimanagementclient/apimanagementclient_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `apimanagementclient/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from apimanagementclient.configuration import Environment

client = ApiManagementClientClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`apimanagementclient/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = ApiManagementClientClient(config=new_config)
```
