...
kwarg names are generated per-API — confirm from `slack/slack_client.py` and `slack/http/auth/` in
the cloned source.

## Basic auth

```python
from slack.http.auth.basic_auth import BasicAuthCredentials
...
```python
from slack.http.auth.api_key import ApiKeyCredentials
...
```python
from slack.http.auth.api_header import ApiHeaderCredentials
...
```python
from slack.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from slack.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from slack.http.auth.o_auth_acg import OAuthACGCredentials
from slack.models.o_auth_scope_slack_enum import OAuthScope{Pkg}Enum
...
```python
from slack.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `slack/configuration.py` and each `slack/http/auth/*.py` for the exact
