...
def stub_client(stubs)
  ...
  OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises OpenUVGlobalRealTimeUVIndexForecastAPI::APIException do
...
def build_test_client
  OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(
    environment: OpenUVGlobalRealTimeUVIndexForecastAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error OpenUVGlobalRealTimeUVIndexForecastAPI::APIException
```
...
```ruby
# Base APIException:
rescue OpenUVGlobalRealTimeUVIndexForecastAPI::APIException => e
  assert_equal 404, e.response_code
  assert_includes e.reason, 'Not Found'

# Typed subclass (e.g. OAuthProviderException):
rescue OpenUVGlobalRealTimeUVIndexForecastAPI::OAuthProviderException => e
...
```ruby
expect { call }.to raise_error(OpenUVGlobalRealTimeUVIndexForecastAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < OpenUVGlobalRealTimeUVIndexForecastAPI::HttpCallBack
...
client  = OpenUVGlobalRealTimeUVIndexForecastAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
