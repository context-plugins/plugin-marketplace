...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `HotelSearchAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`hotelsearchapi/configuration.py` in the cloned source.
...
```python
import requests
from hotelsearchapi.configuration import Configuration
...
`hotelsearchapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`hotelsearchapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `hotelsearchapi/configuration.py`
...
`NumberPagedResponse`) live in `hotelsearchapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `hotelsearchapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
