...
- `twilioapis` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/twilio-apis-go-sdk` — the Go import path for the root package; `github.com/context-plugins/twilio-apis-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/twilio-apis-go-sdk"

client := twilioapis.NewClient(
    twilioapis.CreateConfiguration(
        twilioapis.WithEnvironment(twilioapis.PRODUCTION),
...
from environment variables named `TWILIOAPIS_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := twilioapis.NewClient(twilioapis.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `twilioapis.PRODUCTION`,
`twilioapis.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := twilioapis.NewClient(
    twilioapis.CreateConfiguration(twilioapis.WithEnvironment(twilioapis.PRODUCTION)),
)
```
...
client := twilioapis.NewClient(
    twilioapis.CreateConfiguration(
        twilioapis.WithHttpConfiguration(
            twilioapis.CreateHttpConfiguration(
                twilioapis.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                twilioapis.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = twilioapis.NewClient(twilioapis.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api twilioapis.ClientInterface
}

func NewService(api twilioapis.ClientInterface) *Service {
