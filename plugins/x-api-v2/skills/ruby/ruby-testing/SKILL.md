...
client = stub_client(stubs)
assert_raises XAPIV2::APIException do
...
BASE = XAPIV2::Configuration.new(environment: XAPIV2::Environment::PRODUCTION).get_base_uri

def build_test_client
  XAPIV2::Client.new(
    environment: XAPIV2::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error XAPIV2::APIException
```
...
```ruby
expect { call }.to raise_error(XAPIV2::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < XAPIV2::HttpCallBack
...
client  = XAPIV2::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
