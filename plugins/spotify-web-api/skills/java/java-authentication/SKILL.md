...
To see which schemes a specific SDK accepts, read the credential setter methods on `SpotifyWebApiClient.Builder`
...
- Set credentials **on the `SpotifyWebApiClient.Builder`** before calling `.build()` — the client is immutable
