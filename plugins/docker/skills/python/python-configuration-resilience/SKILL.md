...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `DockerClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`docker/configuration.py` in the cloned source.
...
```python
import requests
from docker.configuration import Configuration
...
`docker/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`docker/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `docker/configuration.py`
...
`NumberPagedResponse`) live in `docker/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `docker/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
