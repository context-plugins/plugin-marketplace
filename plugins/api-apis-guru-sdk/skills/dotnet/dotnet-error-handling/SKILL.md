...
> `{ApiGroup}`, `PointsOfInterest`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `PointsOfInterest.Core.Exceptions`
- `ApiError` **and** `RawError` → `PointsOfInterest.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `PointsOfInterest.Errors`
...
```csharp
using PointsOfInterest.Core.Exceptions;     // SdkException<TError>
using PointsOfInterest.Core.ErrorResponse;  // ApiError, RawError
using PointsOfInterest.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`PointsOfInterest.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using PointsOfInterest.Core.Exceptions;     // SdkException<TError>
using PointsOfInterest.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `PointsOfInterest.Core.ErrorResponse`. Its public
...
`readonly struct` under `PointsOfInterest.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using PointsOfInterest.Core.Models;        // ApiResult<TResponse, TError>
using PointsOfInterest.Core.ErrorResponse; // RawError
using PointsOfInterest.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `PointsOfInterest.Core.Exceptions`) — an auth *application* failure, not an API error — when the
