...
> `AdobeExperienceManagerAEMAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AdobeExperienceManagerAEMAPIClient
{
...
```php
use AdobeExperienceManagerAEMAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AdobeExperienceManagerAEMAPILib\Exceptions\{OperationException};
use AdobeExperienceManagerAEMAPILib\Exceptions\ApiException;
...
app()->instance(AdobeExperienceManagerAEMAPIClient::class, clientWithResponses([
