---
name: dotnet-mist-getting-started
description: mist .NET SDK identity and lookup layer for the dotnet-mist-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the mist .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-mist-sdk` helper
...
| --- | --- |
| API | mist |
| Source repo | https://github.com/context-plugins/mist-csharp-sdk (branch `main`) |
| Root namespace | `MistApi` (the `using` namespace) |
| Client class | `MistApiClient` |
| Options class | `MistApiClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `MistApiClientOptions`: `ApiToken: string?`, `BasicAuth: BasicAuthCredentials?`, `CsrfToken: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `MistGlobal01`, `MistGlobal02`, `MistGlobal03`, `MistGlobal04`, `MistGlobal05`, `MistEmea01`, `MistEmea02`, `MistEmea03`, `MistEmea04`, `MistApac01`, `MistApac02`, `MistApac03` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using MistApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/mist-csharp-sdk
dotnet add reference mist-csharp-sdk/MistApi.csproj
```
...
temp directory** (`<temp>/mist-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/mist-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/mist-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/mist-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\mist-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/mist-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/mist-dotnet-src/`, **never** in the project repo, and its path never goes into
`mist-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `MistApiClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new MistApiClient(...)`, build its options, or DI-register via
   `AddMistApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `MistApiClientOptions` — the map's *Servers & auth* section
