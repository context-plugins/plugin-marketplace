...
> Throughout, `finnhubapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := finnhubapi.NewClient(
    finnhubapi.CreateConfiguration(
        finnhubapi.WithBasicAuthCredentials(
            finnhubapi.NewBasicAuthCredentials(
...
finnhubapi.WithApiKeyCredentials(finnhubapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
finnhubapi.WithApiHeaderCredentials(finnhubapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
finnhubapi.WithOAuthBearerTokenCredentials(
    finnhubapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := finnhubapi.NewClient(
    finnhubapi.CreateConfiguration(
        finnhubapi.WithOAuthCCGCredentials(
            finnhubapi.NewOAuthCCGCredentials(
