...
```python
from fdx.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `fdx/utilities/union_type_lookup.py`) and each candidate model
...
```python
from fdx.models.animal import Cat, Dog
...
```python
from fdx.models.animal import Animal, Cat, Dog
