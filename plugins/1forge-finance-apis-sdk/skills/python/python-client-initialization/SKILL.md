...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the 1ForgeFinanceAPIsClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write 1ForgeFinanceAPIsClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `1forgefinanceapis` — the root package name (e.g. `multiauthsample`, `batester`).
- `1ForgeFinanceAPIsClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `1ForgeFinanceAPIsClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from 1forgefinanceapis.1forgefinanceapis_client import 1ForgeFinanceAPIsClient
from 1forgefinanceapis.configuration import Environment
from 1forgefinanceapis.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = 1ForgeFinanceAPIsClient(
...
```python
from 1forgefinanceapis.configuration import Configuration, Environment
...
client = 1ForgeFinanceAPIsClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `1forgefinanceapis/1forgefinanceapis_client.py` in the cloned source.
...
`from_environment()` is a class method on both `1ForgeFinanceAPIsClient` and `Configuration` that reads
...
client = 1ForgeFinanceAPIsClient.from_environment()
...
`from_environment` in `1forgefinanceapis/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `1ForgeFinanceAPIsClient`. Access them as attributes —
...
Open `1forgefinanceapis/1forgefinanceapis_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `1forgefinanceapis/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from 1forgefinanceapis.configuration import Environment

client = 1ForgeFinanceAPIsClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`1forgefinanceapis/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = 1ForgeFinanceAPIsClient(config=new_config)
```
