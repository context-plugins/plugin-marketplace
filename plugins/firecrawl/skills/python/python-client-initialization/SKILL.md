...
- `firecrawl` — the root package name (e.g. `multiauthsample`, `batester`).
- `FirecrawlapiClient` — the generated client class (read it from the `class …Client` declaration in `firecrawl/firecrawl_client.py`).
...
from firecrawl.configuration import Environment
from firecrawl.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from firecrawl.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `firecrawl/firecrawl_client.py` in the cloned source.
...
`from_environment` in `firecrawl/configuration.py` for the exact names.
...
Open `firecrawl/firecrawl_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `firecrawl/configuration.py`. **Read the enum in `configuration.py`
...
`firecrawl/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
