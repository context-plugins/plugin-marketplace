...
kwargs to the `PostnlecommerceClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `postnl/postnl_client.py` and `postnl/http/auth/` in
the cloned source.

## Basic auth

```python
from postnl.http.auth.basic_auth import BasicAuthCredentials
...
```python
from postnl.http.auth.api_key import ApiKeyCredentials
...
```python
from postnl.http.auth.api_header import ApiHeaderCredentials
...
```python
from postnl.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from postnl.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from postnl.http.auth.o_auth_acg import OAuthACGCredentials
from postnl.models.o_auth_scope_postnl_enum import OAuthScope{Pkg}Enum
...
client = PostnlecommerceClient(o_auth_acg_credentials=creds)
...
```python
from postnl.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = PostnlecommerceClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `PostnlecommerceClient.from_environment()`) read each credential
...
Grep `from_environment` in `postnl/configuration.py` and each `postnl/http/auth/*.py` for the exact
