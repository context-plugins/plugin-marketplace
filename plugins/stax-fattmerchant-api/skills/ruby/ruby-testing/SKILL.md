...
def stub_client(stubs)
  ...
  StaxFattMerchantAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises StaxFattMerchantAPI::APIException do
...
BASE = StaxFattMerchantAPI::Configuration.new(environment: StaxFattMerchantAPI::Environment::PRODUCTION).get_base_uri

def build_test_client
  StaxFattMerchantAPI::Client.new(
    environment: StaxFattMerchantAPI::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error StaxFattMerchantAPI::APIException
```
...
```ruby
expect { call }.to raise_error(StaxFattMerchantAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < StaxFattMerchantAPI::HttpCallBack
...
client  = StaxFattMerchantAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
