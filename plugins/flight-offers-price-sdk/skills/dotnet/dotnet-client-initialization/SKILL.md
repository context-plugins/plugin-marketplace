...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddFlightOffersPriceClient extension. Use the moment you write `new FlightOffersPriceClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `FlightOffersPriceClientOptions` — its options class.
- `FlightOffersPrice` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public FlightOffersPriceClient(HttpClient httpClient, FlightOffersPriceClientOptions options)
```
...
```csharp
public class FlightOffersPriceClientOptions
{
...
```csharp
using FlightOffersPrice;
using FlightOffersPrice.Servers;

var options = new FlightOffersPriceClientOptions
{
...
var client = new FlightOffersPriceClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddFlightOffersPriceClient`, which registers the
...
builder.Services.AddFlightOffersPriceClient(options =>
{
...
```csharp
public sealed class MyService(FlightOffersPriceClient client)
{
