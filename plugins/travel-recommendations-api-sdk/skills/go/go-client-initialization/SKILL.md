...
- `travelrecommendationsapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/travel-recommendations-api-go` — the Go import path for the root package; `github.com/context-plugins/travel-recommendations-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/travel-recommendations-api-go"

client := travelrecommendationsapi.NewClient(
    travelrecommendationsapi.CreateConfiguration(
        travelrecommendationsapi.WithEnvironment(travelrecommendationsapi.PRODUCTION),
...
from environment variables named `TRAVELRECOMMENDATIONSAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := travelrecommendationsapi.NewClient(travelrecommendationsapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `travelrecommendationsapi.PRODUCTION`,
`travelrecommendationsapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := travelrecommendationsapi.NewClient(
    travelrecommendationsapi.CreateConfiguration(travelrecommendationsapi.WithEnvironment(travelrecommendationsapi.PRODUCTION)),
)
```
...
client := travelrecommendationsapi.NewClient(
    travelrecommendationsapi.CreateConfiguration(
        travelrecommendationsapi.WithHttpConfiguration(
            travelrecommendationsapi.CreateHttpConfiguration(
                travelrecommendationsapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                travelrecommendationsapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = travelrecommendationsapi.NewClient(travelrecommendationsapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api travelrecommendationsapi.ClientInterface
}

func NewService(api travelrecommendationsapi.ClientInterface) *Service {
