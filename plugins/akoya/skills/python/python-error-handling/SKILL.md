...
`akoya/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `akoya/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `akoya/exceptions/`.
...
```python
from akoya.exceptions.api_exception import APIException
from akoya.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`akoya/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`akoya/exceptions/` for the class.
...
`akoya/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `akoya/exceptions/`
  or a `ValueError` — check the auth handler in `akoya/http/auth/` for the exact type.
