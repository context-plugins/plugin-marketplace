...
kwargs to the `IPGeolocationAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `ipgeolocationapi/ipgeolocationapi_client.py` and `ipgeolocationapi/http/auth/` in
the cloned source.

## Basic auth

```python
from ipgeolocationapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from ipgeolocationapi.http.auth.api_key import ApiKeyCredentials
...
```python
from ipgeolocationapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from ipgeolocationapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from ipgeolocationapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from ipgeolocationapi.http.auth.o_auth_acg import OAuthACGCredentials
from ipgeolocationapi.models.o_auth_scope_ipgeolocationapi_enum import OAuthScope{Pkg}Enum
...
client = IPGeolocationAPIClient(o_auth_acg_credentials=creds)
...
```python
from ipgeolocationapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = IPGeolocationAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `IPGeolocationAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `ipgeolocationapi/configuration.py` and each `ipgeolocationapi/http/auth/*.py` for the exact
