...
kwargs to the `SquareConnectAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `squareconnectapi/squareconnectapi_client.py` and `squareconnectapi/http/auth/` in
the cloned source.

## Basic auth

```python
from squareconnectapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from squareconnectapi.http.auth.api_key import ApiKeyCredentials
...
```python
from squareconnectapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from squareconnectapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from squareconnectapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from squareconnectapi.http.auth.o_auth_acg import OAuthACGCredentials
from squareconnectapi.models.o_auth_scope_squareconnectapi_enum import OAuthScope{Pkg}Enum
...
client = SquareConnectAPIClient(o_auth_acg_credentials=creds)
...
```python
from squareconnectapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SquareConnectAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SquareConnectAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `squareconnectapi/configuration.py` and each `squareconnectapi/http/auth/*.py` for the exact
