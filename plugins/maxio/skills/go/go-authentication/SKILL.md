...
> Throughout, `maxio` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := maxio.NewClient(
    maxio.CreateConfiguration(
        maxio.WithBasicAuthCredentials(
            maxio.NewBasicAuthCredentials(
...
maxio.WithApiKeyCredentials(maxio.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
maxio.WithApiHeaderCredentials(maxio.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
maxio.WithOAuthBearerTokenCredentials(
    maxio.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
