...
│   └── com.shell.apitest.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.shell.apitest.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.shell.apitest.http.client.HttpContext`, which carries:
