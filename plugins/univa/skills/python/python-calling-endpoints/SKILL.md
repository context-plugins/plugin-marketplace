...
Open `univa/univa_client.py` for the controller property names, then the relevant
`univa/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from univa.models.{request_model} import {RequestModel}
...
Open `univa/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from univa.models.suite_code_enum import SuiteCodeEnum
...
```python
from univa.http.http_call_back import HttpCallBack
...
client = UnivapayClientSdkClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `univa/pagination/` in the source.

## Finding the right method

- Controller property names are on `UnivapayClientSdkClient` in `univa/univa_client.py`.
- Operation method signatures are in `univa/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `univa/models/`; enum types have `from_value()`.
