...
`testing` + `testify/require` for reference; substitute the real `newstore` and names.
...
import (
    ...
    "github.com/context-plugins/newstore-go-sdk"   // root package newstore
)
...
func clientReturning(status int, body string) (newstore.ClientInterface, *stubTransport) {
    ...
    client := newstore.NewClient(
        newstore.CreateConfiguration(
            newstore.WithHttpConfiguration(
                newstore.CreateHttpConfiguration(newstore.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/newstore-go-sdk/errors"
...
client := newstore.NewClient(
    newstore.CreateConfiguration(
        newstore.WithHttpConfiguration(
            newstore.CreateHttpConfiguration(newstore.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
