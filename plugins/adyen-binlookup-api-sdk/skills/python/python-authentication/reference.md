...
kwargs to the `AdyenBinLookupAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `adyenbinlookupapi/adyenbinlookupapi_client.py` and `adyenbinlookupapi/http/auth/` in
the cloned source.

## Basic auth

```python
from adyenbinlookupapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from adyenbinlookupapi.http.auth.api_key import ApiKeyCredentials
...
```python
from adyenbinlookupapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from adyenbinlookupapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from adyenbinlookupapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from adyenbinlookupapi.http.auth.o_auth_acg import OAuthACGCredentials
from adyenbinlookupapi.models.o_auth_scope_adyenbinlookupapi_enum import OAuthScope{Pkg}Enum
...
client = AdyenBinLookupAPIClient(o_auth_acg_credentials=creds)
...
```python
from adyenbinlookupapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AdyenBinLookupAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AdyenBinLookupAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `adyenbinlookupapi/configuration.py` and each `adyenbinlookupapi/http/auth/*.py` for the exact
