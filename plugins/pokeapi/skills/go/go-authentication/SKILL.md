...
> Throughout, `pokeapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := pokeapi.NewClient(
    pokeapi.CreateConfiguration(
        pokeapi.WithBasicAuthCredentials(
            pokeapi.NewBasicAuthCredentials(
...
pokeapi.WithApiKeyCredentials(pokeapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
pokeapi.WithApiHeaderCredentials(pokeapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
pokeapi.WithOAuthBearerTokenCredentials(
    pokeapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := pokeapi.NewClient(
    pokeapi.CreateConfiguration(
        pokeapi.WithOAuthCCGCredentials(
            pokeapi.NewOAuthCCGCredentials(
