...
against the `BokuClient.Builder` setters, the `com.boku.gbapi4stage.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `BokuClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
