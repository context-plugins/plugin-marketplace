...
- `finnhub` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/finnhub-go-sdk` — the Go import path for the root package; `github.com/context-plugins/finnhub-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/finnhub-go-sdk"

client := finnhub.NewClient(
    finnhub.CreateConfiguration(
        finnhub.WithEnvironment(finnhub.PRODUCTION),
...
from environment variables named `FINNHUB_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := finnhub.NewClient(finnhub.CreateConfigurationFromEnvironment())
```
...
```go
client := finnhub.NewClient(
    finnhub.CreateConfiguration(finnhub.WithEnvironment(finnhub.PRODUCTION)),
)
```
...
client := finnhub.NewClient(
    finnhub.CreateConfiguration(
        finnhub.WithHttpConfiguration(
            finnhub.CreateHttpConfiguration(
                finnhub.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                finnhub.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = finnhub.NewClient(finnhub.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api finnhub.ClientInterface
}

func NewService(api finnhub.ClientInterface) *Service {
