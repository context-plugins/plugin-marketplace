...
All types below live under `Sportsdata.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using Sportsdata.Core.Configuration;
...
```csharp
using Sportsdata.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new SportsdataClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `Sportsdata.Core.Exceptions`):
...
With DI, the SDK's `AddSportsdataClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddSportsdataClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
