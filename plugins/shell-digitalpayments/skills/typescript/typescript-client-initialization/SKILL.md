...
- `shell-smartpay-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'shell-smartpay-apilib';
...
```ts
import { {Resource}Api } from 'shell-smartpay-apilib';
