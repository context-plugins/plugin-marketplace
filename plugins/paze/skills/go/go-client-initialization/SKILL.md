...
- `paze` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/paze-go-sdk` — the Go import path for the root package; `github.com/context-plugins/paze-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/paze-go-sdk"

client := paze.NewClient(
    paze.CreateConfiguration(
        paze.WithEnvironment(paze.PRODUCTION),
...
from environment variables named `PAZE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := paze.NewClient(paze.CreateConfigurationFromEnvironment())
```
...
```go
client := paze.NewClient(
    paze.CreateConfiguration(paze.WithEnvironment(paze.PRODUCTION)),
)
```
...
client := paze.NewClient(
    paze.CreateConfiguration(
        paze.WithHttpConfiguration(
            paze.CreateHttpConfiguration(
                paze.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                paze.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = paze.NewClient(paze.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api paze.ClientInterface
}

func NewService(api paze.ClientInterface) *Service {
