...
| --- | --- |
| API | `api.apis.guru` |
| Project | `ToursAndActivities.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `ToursAndActivities` (the `using` namespace) |
| Client class | `ToursAndActivitiesClient` |
| Options class | `ToursAndActivitiesClientOptions` |
...
namespaces transitively, so `using ToursAndActivities.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/api-apis-guru-csharp-64002b
dotnet add reference api-apis-guru-csharp-64002b/ToursAndActivities.csproj
```

```csharp
using ToursAndActivities;          // client + options + ServerEnvironment
using ToursAndActivities.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-64002b /tmp/api-apis-guru-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-64002b "$env:TEMP\api-apis-guru-dotnet-src"
```
...
- `ToursAndActivitiesClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `ToursAndActivitiesClient`, build its
   `ToursAndActivitiesClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
