...
```ruby
client = TeslaFleetManagementAPI::Client.new(
  max_retries:    3,
...
```ruby
client = TeslaFleetManagementAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = TeslaFleetManagementAPI::Client.new(adapter: :net_http)
```
...
client = TeslaFleetManagementAPI::Client.new(connection: conn)
```
...
client = TeslaFleetManagementAPI::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = TeslaFleetManagementAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = TeslaFleetManagementAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = TeslaFleetManagementAPI::ProxySettings.from_env
client = TeslaFleetManagementAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < TeslaFleetManagementAPI::HttpCallBack
...
client = TeslaFleetManagementAPI::Client.new(http_callback: LogCallback.new)
```
...
client = TeslaFleetManagementAPI::Client.from_env(timeout: 30, max_retries: 3)
```
