...
against the `ZalandoShopClient.Builder` setters, the `com.zalando.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ZalandoShopClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ZalandoShopClient client = new ZalandoShopClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeZalandoShopEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
