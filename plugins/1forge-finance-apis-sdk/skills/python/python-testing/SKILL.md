...
reference; substitute your `1forgefinanceapis` and `1ForgeFinanceAPIsClient` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `1forgefinanceapis/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
