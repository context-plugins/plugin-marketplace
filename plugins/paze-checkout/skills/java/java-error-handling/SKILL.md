...
│   └── com.paze.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.paze.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.paze.api.http.client.HttpContext`, which carries:
