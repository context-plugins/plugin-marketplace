...
> `TraderApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): TraderApiClient
{
...
```php
use TraderApiLib\Exceptions\{OperationException};
use TraderApiLib\Exceptions\ApiException;
...
app()->instance(TraderApiClient::class, clientWithResponses([
