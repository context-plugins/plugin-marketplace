...
client = stub_client(stubs)
assert_raises AWSMigrationHub::APIException do
...
def build_test_client
  AWSMigrationHub::Client.new(
    environment: AWSMigrationHub::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error AWSMigrationHub::APIException
```
...
```ruby
expect { call }.to raise_error(AWSMigrationHub::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < AWSMigrationHub::HttpCallBack
...
client  = AWSMigrationHub::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
