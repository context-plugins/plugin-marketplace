# Stax (FattMerchant) API SDK Plugin

Integrate and consume the **APIMatic-generated Stax (FattMerchant) API SDK** (the "APIMATIC v3.0" generic
...
(`dotnet-stax-fattmerchant-api-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-stax-fattmerchant-api`** router skill routes all
C#/.NET SDK work to the single **`dotnet-stax-fattmerchant-api-sdk`** agent (.NET-only), which answers
