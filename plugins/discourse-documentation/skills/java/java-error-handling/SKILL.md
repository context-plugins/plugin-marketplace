...
│   └── com.example.discourse.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.example.discourse.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.example.discourse.http.client.HttpContext`, which carries:
