...
> Throughout, `apisguru` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := apisguru.NewClient(
    apisguru.CreateConfiguration(
        apisguru.WithBasicAuthCredentials(
            apisguru.NewBasicAuthCredentials(
...
apisguru.WithApiKeyCredentials(apisguru.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
apisguru.WithApiHeaderCredentials(apisguru.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
apisguru.WithOAuthBearerTokenCredentials(
    apisguru.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := apisguru.NewClient(
    apisguru.CreateConfiguration(
        apisguru.WithOAuthCCGCredentials(
            apisguru.NewOAuthCCGCredentials(
