...
> Throughout, `mediumapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := mediumapi.NewClient(
    mediumapi.CreateConfiguration(
        mediumapi.WithBasicAuthCredentials(
            mediumapi.NewBasicAuthCredentials(
...
mediumapi.WithApiKeyCredentials(mediumapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
mediumapi.WithApiHeaderCredentials(mediumapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
mediumapi.WithOAuthBearerTokenCredentials(
    mediumapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := mediumapi.NewClient(
    mediumapi.CreateConfiguration(
        mediumapi.WithOAuthCCGCredentials(
            mediumapi.NewOAuthCCGCredentials(
