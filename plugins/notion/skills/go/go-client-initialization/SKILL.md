...
- `notion` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/notion-go-sdk` — the Go import path for the root package; `github.com/context-plugins/notion-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/notion-go-sdk"

client := notion.NewClient(
    notion.CreateConfiguration(
        notion.WithEnvironment(notion.PRODUCTION),
...
from environment variables named `NOTION_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := notion.NewClient(notion.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `notion.PRODUCTION`,
`notion.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := notion.NewClient(
    notion.CreateConfiguration(notion.WithEnvironment(notion.PRODUCTION)),
)
```
    ...
    notion.CreateConfiguration(
        notion.WithHttpConfiguration(
            notion.CreateHttpConfiguration(
                notion.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                notion.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = notion.NewClient(notion.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api notion.ClientInterface
}

func NewService(api notion.ClientInterface) *Service {
