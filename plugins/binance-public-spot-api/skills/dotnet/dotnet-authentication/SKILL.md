...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `BinancePublicSpotApi`,
> `BinancePublicSpotApiClientOptions`, `{BasicAuthProperty}`) — replace it with the concrete identifier from the source.

To see which schemes a specific SDK accepts, read the **credentials properties on its `BinancePublicSpotApiClientOptions`
class** — those are the source of truth (read the class in the SDK source, not a decompiled or reflected
view of the installed package). The `BinancePublicSpotApi.Core.Authentication` folder ships *every*
...
The credential classes below live under `BinancePublicSpotApi.Core.Authentication.*` and are the **same across
...
```csharp
using BinancePublicSpotApi.Core.Authentication.Basic;
...
```csharp
using BinancePublicSpotApi.Core.Authentication.OAuth2.ClientCredentials;
...
```csharp
using BinancePublicSpotApi.Core.Authentication.OAuth2.AuthorizationCode;
...
```csharp
using BinancePublicSpotApi.Core.Authentication.OAuth2.Password;
...
- Set credentials **before** constructing the client, or inside the `AddBinancePublicSpotApiClient(options => ...)`
...
- Keep secrets out of source — load them from configuration (environment variables, a secret store, or any
  other `IConfiguration` source) instead of hardcoding, either inside the `AddBinancePublicSpotApiClient(options => ...)`
