...
`shelldigitalpayments/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `shelldigitalpayments/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `shelldigitalpayments/exceptions/`.
...
```python
from shelldigitalpayments.exceptions.api_exception import APIException
from shelldigitalpayments.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`shelldigitalpayments/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`shelldigitalpayments/exceptions/` for the class.
...
`shelldigitalpayments/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `shelldigitalpayments/exceptions/`
  or a `ValueError` — check the auth handler in `shelldigitalpayments/http/auth/` for the exact type.
