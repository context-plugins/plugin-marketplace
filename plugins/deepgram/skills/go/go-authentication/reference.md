...
```go
deepgram.WithOAuthCCGCredentials(
    deepgram.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr deepgram.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := deepgram.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr deepgram.OAuthCCGManager) models.OAuthToken {
...
```go
creds := deepgram.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := deepgram.NewClient(deepgram.CreateConfiguration(deepgram.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    deepgram.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := deepgram.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := deepgram.NewClient(deepgram.CreateConfiguration(deepgram.WithOAuthROPCGCredentials(creds)))
...
```go
deepgram.WithOAuthBearerTokenCredentials(deepgram.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
deepgram.WithBasicAuthCredentials(deepgram.NewBasicAuthCredentials(username, password))
deepgram.WithApiHeaderCredentials(deepgram.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
deepgram.WithApiKeyCredentials(deepgram.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `DEEPGRAM_...` variables — e.g.
`DEEPGRAM_USERNAME`, `DEEPGRAM_PASSWORD`, `DEEPGRAM_TOKEN`, `DEEPGRAM_API_KEY`,
`DEEPGRAM_O_AUTH_CLIENT_ID`, `DEEPGRAM_O_AUTH_CLIENT_SECRET`, `DEEPGRAM_ACCESS_TOKEN`. Grep the
