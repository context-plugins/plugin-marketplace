...
```go
splitit.WithOAuthCCGCredentials(
    splitit.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr splitit.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := splitit.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr splitit.OAuthCCGManager) models.OAuthToken {
...
```go
creds := splitit.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := splitit.NewClient(splitit.CreateConfiguration(splitit.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    splitit.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := splitit.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := splitit.NewClient(splitit.CreateConfiguration(splitit.WithOAuthROPCGCredentials(creds)))
...
```go
splitit.WithOAuthBearerTokenCredentials(splitit.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
splitit.WithBasicAuthCredentials(splitit.NewBasicAuthCredentials(username, password))
splitit.WithApiHeaderCredentials(splitit.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
splitit.WithApiKeyCredentials(splitit.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SPLITIT_...` variables — e.g.
`SPLITIT_USERNAME`, `SPLITIT_PASSWORD`, `SPLITIT_TOKEN`, `SPLITIT_API_KEY`,
`SPLITIT_O_AUTH_CLIENT_ID`, `SPLITIT_O_AUTH_CLIENT_SECRET`, `SPLITIT_ACCESS_TOKEN`. Grep the
