...
> `AkoyaClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AkoyaClient
{
...
```php
use AkoyaLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use AkoyaLib\Exceptions\{OperationException};
use AkoyaLib\Exceptions\ApiException;
...
app()->instance(AkoyaClient::class, clientWithResponses([
