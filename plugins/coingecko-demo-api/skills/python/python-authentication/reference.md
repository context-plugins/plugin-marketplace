...
kwargs to the `CoinGeckoDemoAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `coingeckodemoapi/coingeckodemoapi_client.py` and `coingeckodemoapi/http/auth/` in
the cloned source.

## Basic auth

```python
from coingeckodemoapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from coingeckodemoapi.http.auth.api_key import ApiKeyCredentials
...
```python
from coingeckodemoapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from coingeckodemoapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from coingeckodemoapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from coingeckodemoapi.http.auth.o_auth_acg import OAuthACGCredentials
from coingeckodemoapi.models.o_auth_scope_coingeckodemoapi_enum import OAuthScope{Pkg}Enum
...
client = CoinGeckoDemoAPIClient(o_auth_acg_credentials=creds)
...
```python
from coingeckodemoapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = CoinGeckoDemoAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `CoinGeckoDemoAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `coingeckodemoapi/configuration.py` and each `coingeckodemoapi/http/auth/*.py` for the exact
