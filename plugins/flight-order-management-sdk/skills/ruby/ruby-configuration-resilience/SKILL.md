...
```ruby
client = FlightOrderManagement::Client.new(
  max_retries:    3,
...
```ruby
client = FlightOrderManagement::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = FlightOrderManagement::Client.new(adapter: :net_http)
```
...
client = FlightOrderManagement::Client.new(connection: conn)
```
...
client = FlightOrderManagement::Client.new(environment: Environment::PRODUCTION)
client = FlightOrderManagement::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = FlightOrderManagement::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = FlightOrderManagement::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = FlightOrderManagement::ProxySettings.from_env
client = FlightOrderManagement::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < FlightOrderManagement::HttpCallBack
...
client = FlightOrderManagement::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = FlightOrderManagement::Client.from_env
# Overrides take precedence over env vars:
client = FlightOrderManagement::Client.from_env(timeout: 30, max_retries: 3)
```
