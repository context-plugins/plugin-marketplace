...
against the `KlarnaPaymentsAPIV1Client.Builder` setters, the `com.klarna.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `KlarnaPaymentsAPIV1Client.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
KlarnaPaymentsAPIV1Client client = new KlarnaPaymentsAPIV1Client.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeKlarnaPaymentsAPIV1Enum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new KlarnaPaymentsAPIV1Client.Builder()
