...
kwarg names are generated per-API — confirm from `shutterstock/shutterstock_client.py` and `shutterstock/http/auth/` in
the cloned source.

## Basic auth

```python
from shutterstock.http.auth.basic_auth import BasicAuthCredentials
...
```python
from shutterstock.http.auth.api_key import ApiKeyCredentials
...
```python
from shutterstock.http.auth.api_header import ApiHeaderCredentials
...
```python
from shutterstock.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from shutterstock.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from shutterstock.http.auth.o_auth_acg import OAuthACGCredentials
from shutterstock.models.o_auth_scope_shutterstock_enum import OAuthScope{Pkg}Enum
...
```python
from shutterstock.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `shutterstock/configuration.py` and each `shutterstock/http/auth/*.py` for the exact
