...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new AutomationManagementClientClient({
...
```typescript
import { {Operation}Error } from '{RootModule}/errors';
...
```typescript
import { ApiError } from '{RootModule}';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AutomationManagementClientClient)
    .useValue(new AutomationManagementClientClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
