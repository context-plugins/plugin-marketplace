...
│   └── cloud.greenbyte.intro.exceptions.ApiException          ← base for all non-2xx API responses
│       └── cloud.greenbyte.intro.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `cloud.greenbyte.intro.http.client.HttpContext`, which carries:
