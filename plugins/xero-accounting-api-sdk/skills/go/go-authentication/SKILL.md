...
> Throughout, `xeroaccountingapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := xeroaccountingapi.NewClient(
    xeroaccountingapi.CreateConfiguration(
        xeroaccountingapi.WithBasicAuthCredentials(
            xeroaccountingapi.NewBasicAuthCredentials(
...
xeroaccountingapi.WithApiKeyCredentials(xeroaccountingapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
xeroaccountingapi.WithApiHeaderCredentials(xeroaccountingapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
xeroaccountingapi.WithOAuthBearerTokenCredentials(
    xeroaccountingapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := xeroaccountingapi.NewClient(
    xeroaccountingapi.CreateConfiguration(
        xeroaccountingapi.WithOAuthCCGCredentials(
            xeroaccountingapi.NewOAuthCCGCredentials(
