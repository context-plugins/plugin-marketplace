...
reference; substitute your `1passwordconnect` and `1PasswordConnectClient` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `1passwordconnect/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
