...
`ebaysell/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `ebaysell/exceptions/` and operation docstrings.
...
```python
from ebaysell.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `ebaysell/exceptions/`.
...
```python
from ebaysell.exceptions.api_exception import APIException
from ebaysell.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`ebaysell/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`ebaysell/exceptions/` for the class.
...
`ebaysell/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `ebaysell/exceptions/`
  or a `ValueError` — check the auth handler in `ebaysell/http/auth/` for the exact type.
