...
```go
subscriptionclient.WithOAuthCCGCredentials(
    subscriptionclient.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr subscriptionclient.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := subscriptionclient.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr subscriptionclient.OAuthCCGManager) models.OAuthToken {
...
```go
creds := subscriptionclient.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := subscriptionclient.NewClient(subscriptionclient.CreateConfiguration(subscriptionclient.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    subscriptionclient.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := subscriptionclient.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := subscriptionclient.NewClient(subscriptionclient.CreateConfiguration(subscriptionclient.WithOAuthROPCGCredentials(creds)))
...
```go
subscriptionclient.WithOAuthBearerTokenCredentials(subscriptionclient.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
subscriptionclient.WithBasicAuthCredentials(subscriptionclient.NewBasicAuthCredentials(username, password))
subscriptionclient.WithApiHeaderCredentials(subscriptionclient.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
subscriptionclient.WithApiKeyCredentials(subscriptionclient.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SUBSCRIPTIONCLIENT_...` variables — e.g.
`SUBSCRIPTIONCLIENT_USERNAME`, `SUBSCRIPTIONCLIENT_PASSWORD`, `SUBSCRIPTIONCLIENT_TOKEN`, `SUBSCRIPTIONCLIENT_API_KEY`,
`SUBSCRIPTIONCLIENT_O_AUTH_CLIENT_ID`, `SUBSCRIPTIONCLIENT_O_AUTH_CLIENT_SECRET`, `SUBSCRIPTIONCLIENT_ACCESS_TOKEN`. Grep the
