    ...
    splitit.CreateConfiguration(
        splitit.WithHttpConfiguration(
            splitit.CreateHttpConfiguration(
                splitit.WithTimeout(30),
                splitit.WithRetryConfiguration(
                    splitit.CreateRetryConfiguration(
                        splitit.WithMaxRetryAttempts(3),
                        splitit.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := splitit.CreateRetryConfiguration(
    splitit.WithMaxRetryAttempts(5),
    splitit.WithRetryInterval(1),
    splitit.WithBackoffFactor(2),
    splitit.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    splitit.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := splitit.NewClient(
    splitit.CreateConfiguration(
        splitit.WithHttpConfiguration(
            splitit.CreateHttpConfiguration(
                splitit.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
