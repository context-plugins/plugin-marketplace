...
Open `lib/shutterstock_api_explorer/client.rb` for the accessor names, then the relevant
`lib/shutterstock_api_explorer/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = ShutterstockAPIExplorer::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: ShutterstockAPIExplorer::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/shutterstock_api_explorer/models/`. Field names are defined as
...
Read the enum module in `lib/shutterstock_api_explorer/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/shutterstock_api_explorer/client.rb`.
- Operation methods are in `lib/shutterstock_api_explorer/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/shutterstock_api_explorer/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/shutterstock_api_explorer/exceptions/`.
