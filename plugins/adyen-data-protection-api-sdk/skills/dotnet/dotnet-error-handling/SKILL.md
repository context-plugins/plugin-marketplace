...
> `{ApiGroup}`, `AdyenDataProtectionApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `AdyenDataProtectionApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `AdyenDataProtectionApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `AdyenDataProtectionApi.Errors`
...
```csharp
using AdyenDataProtectionApi.Core.Exceptions;     // SdkException<TError>
using AdyenDataProtectionApi.Core.ErrorResponse;  // ApiError, RawError
using AdyenDataProtectionApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`AdyenDataProtectionApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using AdyenDataProtectionApi.Core.Exceptions;     // SdkException<TError>
using AdyenDataProtectionApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `AdyenDataProtectionApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `AdyenDataProtectionApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using AdyenDataProtectionApi.Core.Models;        // ApiResult<TResponse, TError>
using AdyenDataProtectionApi.Core.ErrorResponse; // RawError
using AdyenDataProtectionApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `AdyenDataProtectionApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
