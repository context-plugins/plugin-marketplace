...
Open `squareconnectapi/squareconnectapi_client.py` for the controller property names, then the relevant
`squareconnectapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from squareconnectapi.models.{request_model} import {RequestModel}
...
Open `squareconnectapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from squareconnectapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from squareconnectapi.http.http_call_back import HttpCallBack
...
client = SquareConnectAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `squareconnectapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `SquareConnectAPIClient` in `squareconnectapi/squareconnectapi_client.py`.
- Operation method signatures are in `squareconnectapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `squareconnectapi/models/`; enum types have `from_value()`.
