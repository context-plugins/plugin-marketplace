...
`NyTimesClient`/`NyTimesClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `NyTimesClient`,
...
static NyTimesClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new NyTimesClient(new HttpClient(handler), new NyTimesClientOptions { /* auth not needed for stubs */ });
}
```
...
using NyTimes.Errors;              // {Operation}Error types
...
using NyTimes.Core.ErrorResponse;
...
using NyTimes.Core.ErrorResponse; // RawError (Case B)
...
var client = new NyTimesClient(new HttpClient(handler), new NyTimesClientOptions());
...
- For DI-based code, the SDK's `AddNyTimesClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `NyTimesClient` from the provider:
  ```csharp
  services.AddNyTimesClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<NyTimesClient>();
  ```
