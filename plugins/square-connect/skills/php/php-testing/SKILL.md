...
> `SquareConnectAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SquareConnectAPIClient
{
...
```php
use SquareConnectAPILib\Exceptions\{OperationException};
use SquareConnectAPILib\Exceptions\ApiException;
...
app()->instance(SquareConnectAPIClient::class, clientWithResponses([
