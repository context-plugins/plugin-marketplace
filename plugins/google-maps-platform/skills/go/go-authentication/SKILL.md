...
> Throughout, `googlemapsplatform` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := googlemapsplatform.NewClient(
    googlemapsplatform.CreateConfiguration(
        googlemapsplatform.WithBasicAuthCredentials(
            googlemapsplatform.NewBasicAuthCredentials(
...
googlemapsplatform.WithApiKeyCredentials(googlemapsplatform.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
googlemapsplatform.WithApiHeaderCredentials(googlemapsplatform.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
googlemapsplatform.WithOAuthBearerTokenCredentials(
    googlemapsplatform.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := googlemapsplatform.NewClient(
    googlemapsplatform.CreateConfiguration(
        googlemapsplatform.WithOAuthCCGCredentials(
            googlemapsplatform.NewOAuthCCGCredentials(
