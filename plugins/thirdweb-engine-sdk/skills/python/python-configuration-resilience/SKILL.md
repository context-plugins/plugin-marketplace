...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ThirdwebEngineClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`thirdwebengine/configuration.py` in the cloned source.
...
```python
import requests
from thirdwebengine.configuration import Configuration
...
`thirdwebengine/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`thirdwebengine/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `thirdwebengine/configuration.py`
...
`NumberPagedResponse`) live in `thirdwebengine/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `thirdwebengine/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
