...
> `GeoapifyApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): GeoapifyApiClient
{
...
```php
use GeoapifyApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use GeoapifyApiLib\Exceptions\{OperationException};
use GeoapifyApiLib\Exceptions\ApiException;
...
app()->instance(GeoapifyApiClient::class, clientWithResponses([
