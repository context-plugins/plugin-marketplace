...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `SplititWebApiV3Client`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`splititwebapiv3/configuration.py` in the cloned source.
...
```python
import requests
from splititwebapiv3.configuration import Configuration
...
`splititwebapiv3/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`splititwebapiv3/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `splititwebapiv3/configuration.py`
...
`NumberPagedResponse`) live in `splititwebapiv3/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `splititwebapiv3/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
