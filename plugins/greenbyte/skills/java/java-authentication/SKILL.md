...
To see which schemes a specific SDK accepts, read the credential setter methods on `GreenbyteApiClient.Builder`
...
- Set credentials **on the `GreenbyteApiClient.Builder`** before calling `.build()` — the client is immutable
