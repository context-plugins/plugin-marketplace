...
- `MailboxValidatorFreeEmailChecker` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `mailboxvalidator_free_email_checker` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'mailboxvalidator_free_email_checker'
include MailboxValidatorFreeEmailChecker
...
The authoritative list is the `initialize` signature in `lib/mailboxvalidator_free_email_checker/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/mailboxvalidator_free_email_checker/configuration.rb`
(`build_default_config_from_env`) and `lib/mailboxvalidator_free_email_checker/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/mailboxvalidator_free_email_checker.rb`) and store it in a
