...
Open `splitit/splitit_client.py` for the controller property names, then the relevant
`splitit/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from splitit.models.{request_model} import {RequestModel}
...
Open `splitit/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from splitit.models.suite_code_enum import SuiteCodeEnum
...
```python
from splitit.http.http_call_back import HttpCallBack
...
client = Splititwebapiv3Client(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `splitit/pagination/` in the source.

## Finding the right method

- Controller property names are on `Splititwebapiv3Client` in `splitit/splitit_client.py`.
- Operation method signatures are in `splitit/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `splitit/models/`; enum types have `from_value()`.
