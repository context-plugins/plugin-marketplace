...
> `ShutterstockClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ShutterstockClient
{
...
```php
use ShutterstockLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use ShutterstockLib\Exceptions\{OperationException};
use ShutterstockLib\Exceptions\ApiException;
...
app()->instance(ShutterstockClient::class, clientWithResponses([
