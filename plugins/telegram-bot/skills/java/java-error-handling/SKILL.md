...
│   └── org.telegram.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── org.telegram.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `org.telegram.api.http.client.HttpContext`, which carries:
