...
Open `frankfurter/frankfurter_client.py` for the controller property names, then the relevant
`frankfurter/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from frankfurter.models.{request_model} import {RequestModel}
...
Open `frankfurter/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from frankfurter.models.suite_code_enum import SuiteCodeEnum
...
```python
from frankfurter.http.http_call_back import HttpCallBack
...
client = FrankfurterapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `frankfurter/pagination/` in the source.

## Finding the right method

- Controller property names are on `FrankfurterapiClient` in `frankfurter/frankfurter_client.py`.
- Operation method signatures are in `frankfurter/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `frankfurter/models/`; enum types have `from_value()`.
