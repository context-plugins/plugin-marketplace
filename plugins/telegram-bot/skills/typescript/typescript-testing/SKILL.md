...
```typescript
import { {Operation}Error } from 'telegram-bot-apilib/errors';
...
```typescript
import { ApiError } from 'telegram-bot-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .useValue(new TelegramBotAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
