...
- `OpenAIAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.openai.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`OpenAIAPIClient.Builder` class. A minimal initialization:
...
The `Builder` is the only entry point — confirm its fluent methods in `OpenAIAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `OpenAIAPIClient.java` |

Call `.build()` to produce an immutable `OpenAIAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public OpenAIAPIClient apiClient() {
        return new OpenAIAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired OpenAIAPIClient client) {
        client.shutdown();
    }
}
```

Inject `OpenAIAPIClient` into your services as a normal Spring dependency. Build only one bean.
