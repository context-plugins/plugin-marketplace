...
- `ApiManagementClientClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `{rootpkg}` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`ApiManagementClientClient.Builder` class. A minimal initialization:

```java
import {rootpkg}.ApiManagementClientClient;
import {rootpkg}.Environment;

ApiManagementClientClient client = new ApiManagementClientClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `ApiManagementClientClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `ApiManagementClientClient.java` |

Call `.build()` to produce an immutable `ApiManagementClientClient` instance.
...
public class ApiConfig {
    @Bean
    public ApiManagementClientClient apiClient() {
        return new ApiManagementClientClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired ApiManagementClientClient client) {
        client.shutdown();
    }
}
```

Inject `ApiManagementClientClient` into your services as a normal Spring dependency. Build only one bean.
