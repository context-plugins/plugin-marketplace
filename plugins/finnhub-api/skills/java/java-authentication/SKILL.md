...
To see which schemes a specific SDK accepts, read the credential setter methods on `FinnhubApiClient.Builder`
...
- Set credentials **on the `FinnhubApiClient.Builder`** before calling `.build()` — the client is immutable
