...
kwargs to the `ShutterstockClient` constructor (or to `Configuration`). Exact credential class names and
...
client = ShutterstockClient(o_auth_acg_credentials=creds)
...
client = ShutterstockClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ShutterstockClient.from_environment()`) read each credential
