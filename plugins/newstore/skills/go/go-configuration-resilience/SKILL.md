    ...
    newstore.CreateConfiguration(
        newstore.WithHttpConfiguration(
            newstore.CreateHttpConfiguration(
                newstore.WithTimeout(30),
                newstore.WithRetryConfiguration(
                    newstore.CreateRetryConfiguration(
                        newstore.WithMaxRetryAttempts(3),
                        newstore.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := newstore.CreateRetryConfiguration(
    newstore.WithMaxRetryAttempts(5),
    newstore.WithRetryInterval(1),
    newstore.WithBackoffFactor(2),
    newstore.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    newstore.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := newstore.NewClient(
    newstore.CreateConfiguration(
        newstore.WithHttpConfiguration(
            newstore.CreateHttpConfiguration(
                newstore.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
