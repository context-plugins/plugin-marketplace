...
Open `googlemapsplatform/googlemapsplatform_client.py` for the controller property names, then the relevant
`googlemapsplatform/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from googlemapsplatform.models.{request_model} import {RequestModel}
...
Open `googlemapsplatform/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from googlemapsplatform.models.suite_code_enum import SuiteCodeEnum
...
client = GoogleMapsPlatformClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `googlemapsplatform/pagination/` in the source.

## Finding the right method

- Controller property names are on `GoogleMapsPlatformClient` in `googlemapsplatform/googlemapsplatform_client.py`.
- Operation method signatures are in `googlemapsplatform/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `googlemapsplatform/models/`; enum types have `from_value()`.
