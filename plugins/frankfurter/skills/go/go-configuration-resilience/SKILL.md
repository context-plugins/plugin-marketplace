    ...
    frankfurter.CreateConfiguration(
        frankfurter.WithHttpConfiguration(
            frankfurter.CreateHttpConfiguration(
                frankfurter.WithTimeout(30),
                frankfurter.WithRetryConfiguration(
                    frankfurter.CreateRetryConfiguration(
                        frankfurter.WithMaxRetryAttempts(3),
                        frankfurter.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := frankfurter.CreateRetryConfiguration(
    frankfurter.WithMaxRetryAttempts(5),
    frankfurter.WithRetryInterval(1),
    frankfurter.WithBackoffFactor(2),
    frankfurter.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    frankfurter.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := frankfurter.NewClient(
    frankfurter.CreateConfiguration(
        frankfurter.WithHttpConfiguration(
            frankfurter.CreateHttpConfiguration(
                frankfurter.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
