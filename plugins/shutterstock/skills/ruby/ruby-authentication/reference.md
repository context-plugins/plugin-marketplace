...
Confirm every name against `lib/shutterstock/client.rb`, `lib/shutterstock/configuration.rb`, the
`lib/shutterstock/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
