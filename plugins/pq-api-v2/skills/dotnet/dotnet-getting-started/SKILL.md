...
| --- | --- |
| API | `PQ API v2` |
| Project | `PqApiV2.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using PqApiV2.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/pq-api-v2-csharp-sdk
dotnet add reference pq-api-v2-csharp-sdk/PqApiV2.csproj
```

```csharp
using PqApiV2;          // client + options + ServerEnvironment
using PqApiV2.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/pq-api-v2-csharp-sdk /tmp/pq-api-v2-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/pq-api-v2-csharp-sdk "$env:TEMP\pq-api-v2-dotnet-src"
```
...
- `PqApiV2Client.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/pq-api-v2-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\pq-api-v2-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `PqApiV2Client`, build its
   `PqApiV2ClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
