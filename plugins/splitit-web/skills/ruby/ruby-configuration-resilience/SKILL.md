...
```ruby
client = SplititWebApiV3::Client.new(
  max_retries:    3,
...
```ruby
client = SplititWebApiV3::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = SplititWebApiV3::Client.new(adapter: :net_http)
```
...
client = SplititWebApiV3::Client.new(connection: conn)
```
...
client = SplititWebApiV3::Client.new(environment: Environment::PRODUCTION)
client = SplititWebApiV3::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = SplititWebApiV3::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = SplititWebApiV3::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = SplititWebApiV3::ProxySettings.from_env
client = SplititWebApiV3::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < SplititWebApiV3::HttpCallBack
...
client = SplititWebApiV3::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = SplititWebApiV3::Client.from_env
# Overrides take precedence over env vars:
client = SplititWebApiV3::Client.from_env(timeout: 30, max_retries: 3)
```
