...
- `fdx` — the root package name (e.g. `multiauthsample`, `batester`).
- `FdxapiClient` — the generated client class (read it from the `class …Client` declaration in `fdx/fdx_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `FdxapiClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from fdx.fdx_client import FdxapiClient
from fdx.configuration import Environment
from fdx.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from fdx.configuration import Configuration, Environment
...
client = FdxapiClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `fdx/fdx_client.py` in the cloned source.
...
`from_environment()` is a class method on both `FdxapiClient` and `Configuration` that reads
...
client = FdxapiClient.from_environment()
...
`from_environment` in `fdx/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `FdxapiClient`. Access them as attributes —
...
Open `fdx/fdx_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `fdx/configuration.py`. **Read the enum in `configuration.py`
...
client = FdxapiClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`fdx/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = FdxapiClient(config=new_config)
```
