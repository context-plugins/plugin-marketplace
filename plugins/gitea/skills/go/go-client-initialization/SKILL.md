...
- `gitea` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/gitea-go-sdk` — the Go import path for the root package; `github.com/context-plugins/gitea-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/gitea-go-sdk"

client := gitea.NewClient(
    gitea.CreateConfiguration(
        gitea.WithEnvironment(gitea.PRODUCTION),
...
from environment variables named `GITEA_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := gitea.NewClient(gitea.CreateConfigurationFromEnvironment())
```
...
```go
client := gitea.NewClient(
    gitea.CreateConfiguration(gitea.WithEnvironment(gitea.PRODUCTION)),
)
```
...
client := gitea.NewClient(
    gitea.CreateConfiguration(
        gitea.WithHttpConfiguration(
            gitea.CreateHttpConfiguration(
                gitea.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                gitea.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = gitea.NewClient(gitea.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api gitea.ClientInterface
}

func NewService(api gitea.ClientInterface) *Service {
