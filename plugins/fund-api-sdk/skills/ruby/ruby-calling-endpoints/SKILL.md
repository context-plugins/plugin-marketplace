...
Open `lib/fund_api/client.rb` for the accessor names, then the relevant
`lib/fund_api/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = FundAPI::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: FundAPI::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/fund_api/models/`. Field names are defined as
...
Read the enum module in `lib/fund_api/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/fund_api/client.rb`.
- Operation methods are in `lib/fund_api/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/fund_api/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/fund_api/exceptions/`.
