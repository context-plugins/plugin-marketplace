...
- `shutterstockapiexplorer` — the root package name (e.g. `multiauthsample`, `batester`).
- `ShutterstockAPIExplorerClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
...
```python
from shutterstockapiexplorer.shutterstockapiexplorer_client import ShutterstockAPIExplorerClient
from shutterstockapiexplorer.configuration import Environment
from shutterstockapiexplorer.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from shutterstockapiexplorer.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `shutterstockapiexplorer/shutterstockapiexplorer_client.py` in the cloned source.
...
`from_environment` in `shutterstockapiexplorer/configuration.py` for the exact names.
...
Open `shutterstockapiexplorer/shutterstockapiexplorer_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `shutterstockapiexplorer/configuration.py`. Use the constant for the desired
...
`shutterstockapiexplorer/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
