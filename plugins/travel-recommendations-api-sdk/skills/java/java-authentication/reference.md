...
against the `TravelRecommendationsAPIClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `TravelRecommendationsAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
TravelRecommendationsAPIClient client = new TravelRecommendationsAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeTravelRecommendationsAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new TravelRecommendationsAPIClient.Builder()
