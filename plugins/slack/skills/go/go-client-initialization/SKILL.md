...
- `slack` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/slack-go-sdk` — the Go import path for the root package; `github.com/context-plugins/slack-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/slack-go-sdk"

client := slack.NewClient(
    slack.CreateConfiguration(
        slack.WithEnvironment(slack.PRODUCTION),
...
from environment variables named `SLACK_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := slack.NewClient(slack.CreateConfigurationFromEnvironment())
```
...
```go
client := slack.NewClient(
    slack.CreateConfiguration(slack.WithEnvironment(slack.PRODUCTION)),
)
```
...
client := slack.NewClient(
    slack.CreateConfiguration(
        slack.WithHttpConfiguration(
            slack.CreateHttpConfiguration(
                slack.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                slack.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = slack.NewClient(slack.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api slack.ClientInterface
}

func NewService(api slack.ClientInterface) *Service {
