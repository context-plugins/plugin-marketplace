...
`testing` + `testify/require` for reference; substitute the real `hotelratings` and names.
...
import (
    ...
    "github.com/context-plugins/hotel-ratings-go"   // root package hotelratings
)
...
func clientReturning(status int, body string) (hotelratings.ClientInterface, *stubTransport) {
    ...
    client := hotelratings.NewClient(
        hotelratings.CreateConfiguration(
            hotelratings.WithHttpConfiguration(
                hotelratings.CreateHttpConfiguration(hotelratings.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/hotel-ratings-go/errors"
...
client := hotelratings.NewClient(
    hotelratings.CreateConfiguration(
        hotelratings.WithHttpConfiguration(
            hotelratings.CreateHttpConfiguration(hotelratings.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
