...
- `SplititWebApiV3` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `splitit_web_api_v3` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'splitit_web_api_v3'
include SplititWebApiV3
...
The authoritative list is the `initialize` signature in `lib/splitit_web_api_v3/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/splitit_web_api_v3/configuration.rb`
(`build_default_config_from_env`) and `lib/splitit_web_api_v3/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/splitit_web_api_v3.rb`) and store it in a
