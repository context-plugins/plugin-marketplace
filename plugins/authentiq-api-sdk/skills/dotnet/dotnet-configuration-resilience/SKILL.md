...
All types below live under `AuthentiqApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using AuthentiqApi.Core.Configuration;
...
```csharp
using AuthentiqApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new AuthentiqApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `AuthentiqApi.Core.Exceptions`):
...
With DI, the SDK's `AddAuthentiqApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAuthentiqApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
