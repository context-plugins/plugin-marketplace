...
│   └── com.slack.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.slack.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.slack.http.client.HttpContext`, which carries:
