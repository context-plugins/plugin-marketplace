...
| --- | --- |
| API | `Gitea API` |
| Project | `GiteaApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `GiteaApi` (the `using` namespace) |
| Client class | `GiteaApiClient` |
| Options class | `GiteaApiClientOptions` |
...
namespaces transitively, so `using GiteaApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/gitea-api-csharp-sdk
dotnet add reference gitea-api-csharp-sdk/GiteaApi.csproj
```

```csharp
using GiteaApi;          // client + options + ServerEnvironment
using GiteaApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/gitea-api-csharp-sdk /tmp/gitea-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/gitea-api-csharp-sdk "$env:TEMP\gitea-api-dotnet-src"
```
...
- `GiteaApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/gitea-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\gitea-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `GiteaApiClient`, build its
   `GiteaApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
