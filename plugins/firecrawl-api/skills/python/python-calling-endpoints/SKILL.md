...
Open `firecrawlapi/firecrawlapi_client.py` for the controller property names, then the relevant
`firecrawlapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from firecrawlapi.models.{request_model} import {RequestModel}
...
Open `firecrawlapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from firecrawlapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from firecrawlapi.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `firecrawlapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `FirecrawlapiClient` in `firecrawlapi/firecrawlapi_client.py`.
- Operation method signatures are in `firecrawlapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `firecrawlapi/models/`; enum types have `from_value()`.
