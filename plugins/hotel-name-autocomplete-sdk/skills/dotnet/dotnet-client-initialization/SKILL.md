...
- `HotelNameAutocompleteClientOptions` — its options class.
- `HotelNameAutocomplete` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public HotelNameAutocompleteClient(HttpClient httpClient, HotelNameAutocompleteClientOptions options)
```
...
```csharp
public class HotelNameAutocompleteClientOptions
{
...
```csharp
using HotelNameAutocomplete;
using HotelNameAutocomplete.Servers;

var options = new HotelNameAutocompleteClientOptions
{
...
var client = new HotelNameAutocompleteClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddHotelNameAutocompleteClient`, which registers the
...
builder.Services.AddHotelNameAutocompleteClient(options =>
{
...
```csharp
public sealed class MyService(HotelNameAutocompleteClient client)
{
