...
All types below live under `ApisGuru.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using ApisGuru.Core.Configuration;
...
```csharp
using ApisGuru.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new ApisGuruClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `ApisGuru.Core.Exceptions`):
...
With DI, the SDK's `AddApisGuruClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddApisGuruClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
