...
- `github.com/context-plugins/discourse-api-documentation-go` — the Go import path for the root package; `github.com/context-plugins/discourse-api-documentation-go/models` for request/response types.
...
```go
import "github.com/context-plugins/discourse-api-documentation-go"
...
`discourseapidocumentation.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
