...
Integrate and consume the **APIMatic-generated boku SDK** (the "APIMATIC v3.0" generic
...
(`dotnet-boku-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-boku`** router skill routes all
C#/.NET SDK work to the single **`dotnet-boku-sdk`** agent (.NET-only), which answers
