...
> `VerizonAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): VerizonAPIClient
{
...
```php
use VerizonAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use VerizonAPILib\Exceptions\{OperationException};
use VerizonAPILib\Exceptions\ApiException;
...
app()->instance(VerizonAPIClient::class, clientWithResponses([
