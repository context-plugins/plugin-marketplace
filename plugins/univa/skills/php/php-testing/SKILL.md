...
> `UnivapayClientSdkClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): UnivapayClientSdkClient
{
...
```php
use UnivaPay\Exceptions\{OperationException};
use UnivaPay\Exceptions\ApiException;
...
app()->instance(UnivapayClientSdkClient::class, clientWithResponses([
