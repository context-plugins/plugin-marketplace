...
```ruby
client = Docker::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Docker::Client.new(adapter: :net_http)
```
...
client = Docker::Client.new(connection: conn)
```
...
client = Docker::Client.new(environment: Environment::PRODUCTION)
client = Docker::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = Docker::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Docker::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Docker::ProxySettings.from_env
client = Docker::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Docker::HttpCallBack
...
client = Docker::Client.new(http_callback: LogCallback.new)
```
...
client = Docker::Client.from_env(timeout: 30, max_retries: 3)
```
