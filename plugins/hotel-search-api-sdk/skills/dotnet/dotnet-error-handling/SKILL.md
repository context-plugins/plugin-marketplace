...
> `{ApiGroup}`, `HotelSearchApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `HotelSearchApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `HotelSearchApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `HotelSearchApi.Errors`
...
```csharp
using HotelSearchApi.Core.Exceptions;     // SdkException<TError>
using HotelSearchApi.Core.ErrorResponse;  // ApiError, RawError
using HotelSearchApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`HotelSearchApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using HotelSearchApi.Core.Exceptions;     // SdkException<TError>
using HotelSearchApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `HotelSearchApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `HotelSearchApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using HotelSearchApi.Core.Models;        // ApiResult<TResponse, TError>
using HotelSearchApi.Core.ErrorResponse; // RawError
using HotelSearchApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `HotelSearchApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
