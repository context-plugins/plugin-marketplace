...
- `firecrawl` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/firecrawl-go-sdk` — the Go import path for the root package; `github.com/context-plugins/firecrawl-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/firecrawl-go-sdk"

client := firecrawl.NewClient(
    firecrawl.CreateConfiguration(
        firecrawl.WithEnvironment(firecrawl.PRODUCTION),
...
from environment variables named `FIRECRAWL_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := firecrawl.NewClient(firecrawl.CreateConfigurationFromEnvironment())
```
...
```go
client := firecrawl.NewClient(
    firecrawl.CreateConfiguration(firecrawl.WithEnvironment(firecrawl.PRODUCTION)),
)
```
...
client := firecrawl.NewClient(
    firecrawl.CreateConfiguration(
        firecrawl.WithHttpConfiguration(
            firecrawl.CreateHttpConfiguration(
                firecrawl.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                firecrawl.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = firecrawl.NewClient(firecrawl.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api firecrawl.ClientInterface
}

func NewService(api firecrawl.ClientInterface) *Service {
