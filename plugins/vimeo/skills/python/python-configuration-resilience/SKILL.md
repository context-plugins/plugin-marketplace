...
`vimeo/configuration.py` in the cloned source.
...
```python
import requests
from vimeo.configuration import Configuration
...
`vimeo/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`vimeo/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `vimeo/configuration.py`
...
`NumberPagedResponse`) live in `vimeo/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `vimeo/http/http_call_back.py`) and pass an instance
as `http_call_back=`:

```python
import logging
from vimeo.http.http_call_back import HttpCallBack
from vimeo.configuration import Configuration
