...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `ConfigurationAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`configurationapi/configuration.py` in the cloned source.
...
```python
import requests
from configurationapi.configuration import Configuration
...
`configurationapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`configurationapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `configurationapi/configuration.py`
...
`NumberPagedResponse`) live in `configurationapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `configurationapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
