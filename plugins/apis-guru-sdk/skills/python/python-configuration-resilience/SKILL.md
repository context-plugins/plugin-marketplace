...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `APIsGuruClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`apisguru/configuration.py` in the cloned source.
...
```python
import requests
from apisguru.configuration import Configuration
...
`apisguru/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`apisguru/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `apisguru/configuration.py`
...
`NumberPagedResponse`) live in `apisguru/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `apisguru/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
