...
`coingecko/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `coingecko/exceptions/` and operation docstrings.
...
```python
from coingecko.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `coingecko/exceptions/`.
...
```python
from coingecko.exceptions.api_exception import APIException
from coingecko.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`coingecko/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`coingecko/exceptions/` for the class.
...
`coingecko/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `coingecko/exceptions/`
  or a `ValueError` — check the auth handler in `coingecko/http/auth/` for the exact type.
