...
Open `apisguru/apisguru_client.py` for the controller property names, then the relevant
`apisguru/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from apisguru.models.{request_model} import {RequestModel}
...
Open `apisguru/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from apisguru.models.suite_code_enum import SuiteCodeEnum
...
```python
from apisguru.http.http_call_back import HttpCallBack
...
client = APIsGuruClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `apisguru/pagination/` in the source.

## Finding the right method

- Controller property names are on `APIsGuruClient` in `apisguru/apisguru_client.py`.
- Operation method signatures are in `apisguru/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `apisguru/models/`; enum types have `from_value()`.
