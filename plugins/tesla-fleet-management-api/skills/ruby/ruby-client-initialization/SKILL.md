...
- `TeslaFleetManagementAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `tesla_fleet_management_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/tesla_fleet_management_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/tesla_fleet_management_api/configuration.rb`
(`build_default_config_from_env`) and `lib/tesla_fleet_management_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = TeslaFleetManagementAPI::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/tesla_fleet_management_api.rb`) and store it in a
