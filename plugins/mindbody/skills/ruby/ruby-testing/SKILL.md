...
client = stub_client(stubs)
assert_raises Mindbody::APIException do
...
BASE = Mindbody::Configuration.new(environment: Mindbody::Environment::PRODUCTION).get_base_uri

def build_test_client
  Mindbody::Client.new(
    environment: Mindbody::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Mindbody::APIException
```
...
```ruby
expect { call }.to raise_error(Mindbody::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Mindbody::HttpCallBack
...
client  = Mindbody::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
