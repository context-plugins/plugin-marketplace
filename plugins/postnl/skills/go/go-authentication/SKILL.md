...
> Throughout, `postnl` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := postnl.NewClient(
    postnl.CreateConfiguration(
        postnl.WithBasicAuthCredentials(
            postnl.NewBasicAuthCredentials(
...
postnl.WithApiKeyCredentials(postnl.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
postnl.WithApiHeaderCredentials(postnl.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
postnl.WithOAuthBearerTokenCredentials(
    postnl.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
