...
- `the-plaid-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'the-plaid-apilib';
...
```ts
import { {Resource}Controller } from 'the-plaid-apilib';
