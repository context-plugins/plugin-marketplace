...
```go
travelrecommendationsapi.WithOAuthCCGCredentials(
    travelrecommendationsapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr travelrecommendationsapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := travelrecommendationsapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr travelrecommendationsapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := travelrecommendationsapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := travelrecommendationsapi.NewClient(travelrecommendationsapi.CreateConfiguration(travelrecommendationsapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    travelrecommendationsapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := travelrecommendationsapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := travelrecommendationsapi.NewClient(travelrecommendationsapi.CreateConfiguration(travelrecommendationsapi.WithOAuthROPCGCredentials(creds)))
...
```go
travelrecommendationsapi.WithOAuthBearerTokenCredentials(travelrecommendationsapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
travelrecommendationsapi.WithBasicAuthCredentials(travelrecommendationsapi.NewBasicAuthCredentials(username, password))
travelrecommendationsapi.WithApiHeaderCredentials(travelrecommendationsapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
travelrecommendationsapi.WithApiKeyCredentials(travelrecommendationsapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `TRAVELRECOMMENDATIONSAPI_...` variables — e.g.
`TRAVELRECOMMENDATIONSAPI_USERNAME`, `TRAVELRECOMMENDATIONSAPI_PASSWORD`, `TRAVELRECOMMENDATIONSAPI_TOKEN`, `TRAVELRECOMMENDATIONSAPI_API_KEY`,
`TRAVELRECOMMENDATIONSAPI_O_AUTH_CLIENT_ID`, `TRAVELRECOMMENDATIONSAPI_O_AUTH_CLIENT_SECRET`, `TRAVELRECOMMENDATIONSAPI_ACCESS_TOKEN`. Grep the
