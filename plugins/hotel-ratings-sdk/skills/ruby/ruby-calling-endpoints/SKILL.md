...
Open `lib/hotel_ratings/client.rb` for the accessor names, then the relevant
`lib/hotel_ratings/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = HotelRatings::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: HotelRatings::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/hotel_ratings/models/`. Field names are defined as
...
Read the enum module in `lib/hotel_ratings/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/hotel_ratings/client.rb`.
- Operation methods are in `lib/hotel_ratings/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/hotel_ratings/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/hotel_ratings/exceptions/`.
