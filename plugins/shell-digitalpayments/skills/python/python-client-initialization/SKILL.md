...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the ShellsmartpayapiClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write ShellsmartpayapiClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `ShellsmartpayapiClient` is the SDK's client class — **read the real name** from `shelldigitalpayments/shelldigitalpayments_client.py`
...
- `shelldigitalpayments` — the root package name (e.g. `multiauthsample`, `batester`).
- `ShellsmartpayapiClient` — the generated client class (read it from the `class …Client` declaration in `shelldigitalpayments/shelldigitalpayments_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `ShellsmartpayapiClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from shelldigitalpayments.shelldigitalpayments_client import ShellsmartpayapiClient
from shelldigitalpayments.configuration import Environment
from shelldigitalpayments.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from shelldigitalpayments.configuration import Configuration, Environment
...
client = ShellsmartpayapiClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `shelldigitalpayments/shelldigitalpayments_client.py` in the cloned source.
...
`from_environment()` is a class method on both `ShellsmartpayapiClient` and `Configuration` that reads
...
client = ShellsmartpayapiClient.from_environment()
...
`from_environment` in `shelldigitalpayments/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `ShellsmartpayapiClient`. Access them as attributes —
...
Open `shelldigitalpayments/shelldigitalpayments_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `shelldigitalpayments/configuration.py`. **Read the enum in `configuration.py`
...
client = ShellsmartpayapiClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`shelldigitalpayments/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = ShellsmartpayapiClient(config=new_config)
```
