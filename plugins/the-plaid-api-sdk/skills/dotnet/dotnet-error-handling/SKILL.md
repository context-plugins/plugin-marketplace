...
> `{ApiGroup}`, `ThePlaidApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `ThePlaidApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `ThePlaidApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `ThePlaidApi.Errors`
...
```csharp
using ThePlaidApi.Core.Exceptions;     // SdkException<TError>
using ThePlaidApi.Core.ErrorResponse;  // ApiError, RawError
using ThePlaidApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`ThePlaidApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using ThePlaidApi.Core.Exceptions;     // SdkException<TError>
using ThePlaidApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `ThePlaidApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `ThePlaidApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using ThePlaidApi.Core.Models;        // ApiResult<TResponse, TError>
using ThePlaidApi.Core.ErrorResponse; // RawError
using ThePlaidApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `ThePlaidApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
