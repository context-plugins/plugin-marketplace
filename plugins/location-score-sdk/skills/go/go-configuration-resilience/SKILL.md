    ...
    locationscore.CreateConfiguration(
        locationscore.WithHttpConfiguration(
            locationscore.CreateHttpConfiguration(
                locationscore.WithTimeout(30),
                locationscore.WithRetryConfiguration(
                    locationscore.CreateRetryConfiguration(
                        locationscore.WithMaxRetryAttempts(3),
                        locationscore.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := locationscore.CreateRetryConfiguration(
    locationscore.WithMaxRetryAttempts(5),
    locationscore.WithRetryInterval(1),
    locationscore.WithBackoffFactor(2),
    locationscore.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    locationscore.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := locationscore.NewClient(
    locationscore.CreateConfiguration(
        locationscore.WithHttpConfiguration(
            locationscore.CreateHttpConfiguration(
                locationscore.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
