...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `OpenUVGlobalRealTimeUVIndexForecastAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`openuvglobalrealtimeuvindexforecastapi/configuration.py` in the cloned source.
...
```python
import requests
from openuvglobalrealtimeuvindexforecastapi.configuration import Configuration
...
`openuvglobalrealtimeuvindexforecastapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`openuvglobalrealtimeuvindexforecastapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `openuvglobalrealtimeuvindexforecastapi/configuration.py`
...
`NumberPagedResponse`) live in `openuvglobalrealtimeuvindexforecastapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `openuvglobalrealtimeuvindexforecastapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
