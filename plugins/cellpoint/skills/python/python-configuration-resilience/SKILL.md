...
`cellpointapi/configuration.py` in the cloned source.
...
```python
import requests
from cellpointapi.configuration import Configuration
...
`cellpointapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`cellpointapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `cellpointapi/configuration.py`
...
`NumberPagedResponse`) live in `cellpointapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `cellpointapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
