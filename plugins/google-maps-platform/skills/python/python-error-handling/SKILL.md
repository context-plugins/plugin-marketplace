...
`googlemapsplatform/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `googlemapsplatform/exceptions/` and operation docstrings.
...
```python
from googlemapsplatform.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `googlemapsplatform/exceptions/`.
...
```python
from googlemapsplatform.exceptions.api_exception import APIException
from googlemapsplatform.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`googlemapsplatform/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`googlemapsplatform/exceptions/` for the class.
...
`googlemapsplatform/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `googlemapsplatform/exceptions/`
  or a `ValueError` — check the auth handler in `googlemapsplatform/http/auth/` for the exact type.
