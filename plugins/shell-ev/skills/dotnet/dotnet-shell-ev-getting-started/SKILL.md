---
name: dotnet-shell-ev-getting-started
description: shell-ev .NET SDK identity and lookup layer for the dotnet-shell-ev-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the shell-ev .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-shell-ev-sdk` helper
...
| --- | --- |
| API | shell-ev |
| Source repo | https://github.com/context-plugins/shell-ev-csharp-sdk (branch `main`) |
| Root namespace | `ShellEv` (the `using` namespace) |
| Client class | `ShellEvClient` |
| Options class | `ShellEvClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `ShellEvClientOptions`: `BearerAuth: OAuth2ClientCredentials?`, `BearerAuthTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using ShellEv.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/shell-ev-csharp-sdk
dotnet add reference shell-ev-csharp-sdk/ShellEv.csproj
```
...
temp directory** (`<temp>/shell-ev-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/shell-ev-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/shell-ev-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/shell-ev-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\shell-ev-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/shell-ev-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/shell-ev-dotnet-src/`, **never** in the project repo, and its path never goes into
`shell-ev-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `ShellEvClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new ShellEvClient(...)`, build its options, or DI-register via
   `AddShellEvClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `ShellEvClientOptions` — the map's *Servers & auth* section
