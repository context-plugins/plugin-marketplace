...
> `TelegramBotAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): TelegramBotAPIClient
{
...
```php
use TelegramBotAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use TelegramBotAPILib\Exceptions\{OperationException};
use TelegramBotAPILib\Exceptions\ApiException;
...
app()->instance(TelegramBotAPIClient::class, clientWithResponses([
