...
kwargs to the `FirecrawlClient` constructor (or to `Configuration`). Exact credential class names and
...
client = FirecrawlClient(o_auth_acg_credentials=creds)
...
client = FirecrawlClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FirecrawlClient.from_environment()`) read each credential
