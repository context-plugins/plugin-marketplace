...
`testing` + `testify/require` for reference; substitute the real `maxio` and names.
...
import (
    ...
    "github.com/context-plugins/maxio-go-sdk"   // root package maxio
)
...
func clientReturning(status int, body string) (maxio.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := maxio.NewClient(
        maxio.CreateConfiguration(
            maxio.WithHttpConfiguration(
                maxio.CreateHttpConfiguration(maxio.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/maxio-go-sdk/errors"
...
client := maxio.NewClient(
    maxio.CreateConfiguration(
        maxio.WithHttpConfiguration(
            maxio.CreateHttpConfiguration(maxio.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
