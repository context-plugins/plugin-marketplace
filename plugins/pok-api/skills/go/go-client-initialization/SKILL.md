...
- `pokapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/pok-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/pok-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/pok-api-go-sdk"

client := pokapi.NewClient(
    pokapi.CreateConfiguration(
        pokapi.WithEnvironment(pokapi.PRODUCTION),
...
from environment variables named `POKAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := pokapi.NewClient(pokapi.CreateConfigurationFromEnvironment())
```
...
```go
client := pokapi.NewClient(
    pokapi.CreateConfiguration(pokapi.WithEnvironment(pokapi.PRODUCTION)),
)
```
...
client := pokapi.NewClient(
    pokapi.CreateConfiguration(
        pokapi.WithHttpConfiguration(
            pokapi.CreateHttpConfiguration(
                pokapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                pokapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = pokapi.NewClient(pokapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api pokapi.ClientInterface
}

func NewService(api pokapi.ClientInterface) *Service {
