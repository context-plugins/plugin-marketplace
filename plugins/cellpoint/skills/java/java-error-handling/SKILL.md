...
│   └── app.cellpoint.us.api.payments.exceptions.ApiException          ← base for all non-2xx API responses
│       └── app.cellpoint.us.api.payments.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `app.cellpoint.us.api.payments.http.client.HttpContext`, which carries:
