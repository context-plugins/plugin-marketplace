...
- `postnl-ecommercelib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'postnl-ecommercelib';
...
```ts
import { {Resource}Api } from 'postnl-ecommercelib';
