...
`SquareConnectApiClient`/`SquareConnectApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `SquareConnectApiClient`,
...
static SquareConnectApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new SquareConnectApiClient(new HttpClient(handler), new SquareConnectApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using SquareConnectApi.Errors;              // {Operation}Error types
...
using SquareConnectApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new SquareConnectApiClient(new HttpClient(handler), new SquareConnectApiClientOptions());
...
- For DI-based code, the SDK's `AddSquareConnectApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `SquareConnectApiClient` from the provider:
  ```csharp
  services.AddSquareConnectApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<SquareConnectApiClient>();
  ```
