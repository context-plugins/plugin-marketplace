...
> `{ApiGroup}`, `AIceptionInteractive.Standard`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `AIceptionInteractive.Standard.Core.Exceptions`
- `ApiError` **and** `RawError` → `AIceptionInteractive.Standard.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `AIceptionInteractive.Standard.Errors`
...
using AIceptionInteractive.Standard.Core.ErrorResponse;  // ApiError, RawError
using AIceptionInteractive.Standard.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`AIceptionInteractive.Standard.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using AIceptionInteractive.Standard.Core.Exceptions;     // SdkException<TError>
using AIceptionInteractive.Standard.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `AIceptionInteractive.Standard.Core.ErrorResponse`. Its public
...
`readonly struct` under `AIceptionInteractive.Standard.Core.Models`. If the controller has no `{Operation}Result`
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `AIceptionInteractive.Standard.Core.Exceptions`) — an auth *application* failure, not an API error — when the
