...
| --- | --- |
| API | Coingecko |
| Source repo | https://github.com/context-plugins/coingecko-csharp-sdk (branch `main`) |
...
<!-- crawler:auth -->
| Auth | Credentials properties on `CoinGeckoDemoApiClientOptions`: `HeaderAuth: string?`, `QueryAuth: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using CoinGeckoDemoApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/coingecko-csharp-sdk
dotnet add reference coingecko-csharp-sdk/CoinGeckoDemoApi.csproj
```
...
temp directory** (`<temp>/coingecko-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/coingecko-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/coingecko-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/coingecko-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\coingecko-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/coingecko-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/coingecko-dotnet-src/`, **never** in the project repo, and its path never goes into
`coingecko-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new CoinGeckoDemoApiClient(...)`, build its options, or DI-register via
   `AddCoinGeckoDemoApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `CoinGeckoDemoApiClientOptions` — the map's *Servers & auth* section
