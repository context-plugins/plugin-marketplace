...
description: APIMatic-generated Python SDK (APIMATIC v3.0, built on apimatic-core + apimatic-requests-client-adapter) reference and grounding layer — the PyPI package name from pyproject.toml, the single SqlManagementClientClient class in sqlmanagementclient_client.py constructed with keyword args (credentials objects + environment= + http/config kwargs), the Configuration class with Environment/Server enums, controllers accessed via @LazyProperty on the client, the APIException base, the HttpCallBack test seam, the from_environment() class method, and how to navigate the generated source. Entry point into the companion python-* skills (client-initialization, authentication, calling-endpoints, models, error-handling, configuration-resilience, testing) — gates loading each at its integration step, since the source shows signatures but not the usage gotchas these skills carry.
---
...
| Install | `pip install "git+https://github.com/context-plugins/sqlmanagementclient-python@main"` |
| Client class | `SqlManagementClientClient` in `sqlmanagementclient/sqlmanagementclient_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `sqlmanagementclient/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `SqlManagementClientClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `sqlmanagementclient/exceptions/api_exception.py` — raised on non-2xx |
...
- `sqlmanagementclient/sqlmanagementclient_client.py` — the `SqlManagementClientClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `sqlmanagementclient/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `sqlmanagementclient/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `sqlmanagementclient/controllers/` — one `{Resource}Controller` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `sqlmanagementclient/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `sqlmanagementclient/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `sqlmanagementclient/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `sqlmanagementclient/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `sqlmanagementclient/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/sqlmanagementclient-python@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/sqlmanagementclient-python /tmp/sqlmanagementclient-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/sqlmanagementclient-python "$env:TEMP\sqlmanagementclient-python-src"
```
...
- `sqlmanagementclient/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `sqlmanagementclient/sqlmanagementclient_client.py` — `SqlManagementClientClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `sqlmanagementclient/controllers/*.py` — operation method signatures, parameter names, and return types.
- `sqlmanagementclient/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `sqlmanagementclient/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/sqlmanagementclient-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\sqlmanagementclient-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `SqlManagementClientClient(...)`.
