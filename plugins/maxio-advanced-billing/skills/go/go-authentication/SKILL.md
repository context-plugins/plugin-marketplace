...
> Throughout, `maxioadvancedbilling` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := maxioadvancedbilling.NewClient(
    maxioadvancedbilling.CreateConfiguration(
        maxioadvancedbilling.WithBasicAuthCredentials(
            maxioadvancedbilling.NewBasicAuthCredentials(
...
maxioadvancedbilling.WithApiKeyCredentials(maxioadvancedbilling.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
maxioadvancedbilling.WithApiHeaderCredentials(maxioadvancedbilling.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
maxioadvancedbilling.WithOAuthBearerTokenCredentials(
    maxioadvancedbilling.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := maxioadvancedbilling.NewClient(
    maxioadvancedbilling.CreateConfiguration(
        maxioadvancedbilling.WithOAuthCCGCredentials(
            maxioadvancedbilling.NewOAuthCCGCredentials(
