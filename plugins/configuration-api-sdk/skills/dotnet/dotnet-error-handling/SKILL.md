...
> `{ApiGroup}`, `ConfigurationApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `ConfigurationApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `ConfigurationApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `ConfigurationApi.Errors`
...
```csharp
using ConfigurationApi.Core.Exceptions;     // SdkException<TError>
using ConfigurationApi.Core.ErrorResponse;  // ApiError, RawError
using ConfigurationApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`ConfigurationApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using ConfigurationApi.Core.Exceptions;     // SdkException<TError>
using ConfigurationApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `ConfigurationApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `ConfigurationApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using ConfigurationApi.Core.Models;        // ApiResult<TResponse, TError>
using ConfigurationApi.Core.ErrorResponse; // RawError
using ConfigurationApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `ConfigurationApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
