...
- `m1-password-connectlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'm1-password-connectlib';
...
```ts
import { {Resource}Controller } from 'm1-password-connectlib';
