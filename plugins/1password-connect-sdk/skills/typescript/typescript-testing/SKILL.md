...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new 1PasswordConnectClient({
...
```typescript
import { {Operation}Error } from 'm1-password-connectlib/errors';
...
```typescript
import { ApiError } from 'm1-password-connectlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(1PasswordConnectClient)
    .useValue(new 1PasswordConnectClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
