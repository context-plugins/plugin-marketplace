...
Open `travelrecommendationsapi/travelrecommendationsapi_client.py` for the controller property names, then the relevant
`travelrecommendationsapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from travelrecommendationsapi.models.{request_model} import {RequestModel}
...
Open `travelrecommendationsapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from travelrecommendationsapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from travelrecommendationsapi.http.http_call_back import HttpCallBack
...
client = TravelRecommendationsAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `travelrecommendationsapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `TravelRecommendationsAPIClient` in `travelrecommendationsapi/travelrecommendationsapi_client.py`.
- Operation method signatures are in `travelrecommendationsapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `travelrecommendationsapi/models/`; enum types have `from_value()`.
