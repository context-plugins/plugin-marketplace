...
`TripPurposePredictionClient`/`TripPurposePredictionClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `TripPurposePredictionClient`,
...
static TripPurposePredictionClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new TripPurposePredictionClient(new HttpClient(handler), new TripPurposePredictionClientOptions { /* auth not needed for stubs */ });
}
```
...
using TripPurposePrediction.Errors;              // {Operation}Error types
...
```csharp
using TripPurposePrediction.Core.Exceptions;
using TripPurposePrediction.Core.ErrorResponse;
...
using TripPurposePrediction.Core.ErrorResponse; // RawError (Case B)
...
var client = new TripPurposePredictionClient(new HttpClient(handler), new TripPurposePredictionClientOptions());
...
- For DI-based code, the SDK's `AddTripPurposePredictionClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `TripPurposePredictionClient` from the provider:
  ```csharp
  services.AddTripPurposePredictionClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<TripPurposePredictionClient>();
  ```
