...
> `VimeoClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): VimeoClient
{
...
```php
use VimeoLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use VimeoLib\Exceptions\{OperationException};
use VimeoLib\Exceptions\ApiException;
...
app()->instance(VimeoClient::class, clientWithResponses([
