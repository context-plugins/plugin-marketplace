...
```ruby
client = Akoya::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Akoya::Client.new(adapter: :net_http)
```
...
client = Akoya::Client.new(connection: conn)
```
...
client = Akoya::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Akoya::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Akoya::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Akoya::ProxySettings.from_env
client = Akoya::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Akoya::HttpCallBack
...
client = Akoya::Client.new(http_callback: LogCallback.new)
```
...
client = Akoya::Client.from_env(timeout: 30, max_retries: 3)
```
