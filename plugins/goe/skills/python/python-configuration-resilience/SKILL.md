...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `GoeapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`goe/configuration.py` in the cloned source.
...
```python
import requests
from goe.configuration import Configuration
...
`goe/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`goe/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `goe/configuration.py`
...
`NumberPagedResponse`) live in `goe/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `goe/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
