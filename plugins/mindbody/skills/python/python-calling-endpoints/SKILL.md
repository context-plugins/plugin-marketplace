...
Open `mindbody/mindbody_client.py` for the controller property names, then the relevant
`mindbody/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from mindbody.models.{request_model} import {RequestModel}
...
Open `mindbody/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from mindbody.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `mindbody/pagination/` in the source.
