...
All types below live under `FlightOrderManagement.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using FlightOrderManagement.Core.Configuration;
...
```csharp
using FlightOrderManagement.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new FlightOrderManagementClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `FlightOrderManagement.Core.Exceptions`):
...
With DI, the SDK's `AddFlightOrderManagementClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddFlightOrderManagementClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
