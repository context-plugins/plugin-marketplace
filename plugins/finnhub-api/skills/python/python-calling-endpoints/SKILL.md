...
Open `finnhubapi/finnhubapi_client.py` for the controller property names, then the relevant
`finnhubapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from finnhubapi.models.{request_model} import {RequestModel}
...
Open `finnhubapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from finnhubapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from finnhubapi.http.http_call_back import HttpCallBack
...
client = FinnhubapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `finnhubapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `FinnhubapiClient` in `finnhubapi/finnhubapi_client.py`.
- Operation method signatures are in `finnhubapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `finnhubapi/models/`; enum types have `from_value()`.
