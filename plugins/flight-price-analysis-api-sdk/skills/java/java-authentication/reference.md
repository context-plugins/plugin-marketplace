...
against the `FlightPriceAnalysisAPIClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FlightPriceAnalysisAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FlightPriceAnalysisAPIClient client = new FlightPriceAnalysisAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFlightPriceAnalysisAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new FlightPriceAnalysisAPIClient.Builder()
