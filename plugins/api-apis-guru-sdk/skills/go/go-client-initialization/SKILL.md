...
- `github.com/context-plugins/api-apis-guru-go-832e4b` — the Go import path for the root package; `github.com/context-plugins/api-apis-guru-go-832e4b/models` for request/response types.
...
```go
import "github.com/context-plugins/api-apis-guru-go-832e4b"

client := apiapisguru.NewClient(
    apiapisguru.CreateConfiguration(
        apiapisguru.WithEnvironment(apiapisguru.PRODUCTION),
...
from environment variables named `APIAPISGURU_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := apiapisguru.NewClient(apiapisguru.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `apiapisguru.PRODUCTION`,
`apiapisguru.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := apiapisguru.NewClient(
    apiapisguru.CreateConfiguration(apiapisguru.WithEnvironment(apiapisguru.PRODUCTION)),
)
```
...
client := apiapisguru.NewClient(
    apiapisguru.CreateConfiguration(
        apiapisguru.WithHttpConfiguration(
            apiapisguru.CreateHttpConfiguration(
                apiapisguru.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                apiapisguru.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = apiapisguru.NewClient(apiapisguru.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api apiapisguru.ClientInterface
}

func NewService(api apiapisguru.ClientInterface) *Service {
