...
against the `PaypalClient.Builder` setters, the `com.paypal.sandbox.apim.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `PaypalClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
PaypalClient client = new PaypalClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopePaypalEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
