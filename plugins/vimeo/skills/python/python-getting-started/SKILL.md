...
> setting, so **confirm it** from the class names under `vimeo/apis/`.
...
| Install | `pip install "git+https://github.com/context-plugins/vimeo-python-sdk@main"` |
| Client class | `VimeoapiClient` in `vimeo/vimeo_client.py`, constructed with keyword arguments |
...
- `vimeo/vimeo_client.py` — the `VimeoapiClient` class: `__init__` keyword args, `from_environment()`
...
- `vimeo/controllers/` — one `{Resource}Api` per API resource group, all inheriting from
...
```bash
pip install "git+https://github.com/context-plugins/vimeo-python-sdk@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-python-sdk /tmp/vimeo-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/vimeo-python-sdk "$env:TEMP\vimeo-python-src"
```
...
- `vimeo/vimeo_client.py` — `VimeoapiClient.__init__` kwargs, `from_environment()`, and the
