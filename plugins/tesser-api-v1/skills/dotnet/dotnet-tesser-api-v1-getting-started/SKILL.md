---
name: dotnet-tesser-api-v1-getting-started
description: Tesser API (v1) .NET SDK identity and lookup layer for the dotnet-tesser-api-v1-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Tesser API (v1) .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-tesser-api-v1-sdk` helper
...
| --- | --- |
| API | Tesser API (v1) |
| Source repo | https://github.com/context-plugins/tesser-api-v1-csharp-sdk (branch `main`) |
| Root namespace | `TesserApiV1` (the `using` namespace) |
| Client class | `TesserApiV1Client` |
| Options class | `TesserApiV1ClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `TesserApiV1ClientOptions`: `Bearer: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using TesserApiV1.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/tesser-api-v1-csharp-sdk
dotnet add reference tesser-api-v1-csharp-sdk/TesserApiV1.csproj
```
...
temp directory** (`<temp>/tesser-api-v1-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/tesser-api-v1-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/tesser-api-v1-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/tesser-api-v1-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\tesser-api-v1-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/tesser-api-v1-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/tesser-api-v1-dotnet-src/`, **never** in the project repo, and its path never goes into
`tesser-api-v1-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `TesserApiV1Client.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new TesserApiV1Client(...)`, build its options, or DI-register via
   `AddTesserApiV1Client`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `TesserApiV1ClientOptions` — the map's *Servers & auth* section
