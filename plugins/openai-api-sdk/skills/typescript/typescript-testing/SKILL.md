...
```typescript
import { {Operation}Error } from 'openai-apilib/errors';
...
```typescript
import { ApiError } from 'openai-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(OpenAIAPIClient)
    .useValue(new OpenAIAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
