...
`paze/configuration.py` in the cloned source.
...
```python
import requests
from paze.configuration import Configuration
...
`paze/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`paze/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `paze/configuration.py`
...
`NumberPagedResponse`) live in `paze/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `paze/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
