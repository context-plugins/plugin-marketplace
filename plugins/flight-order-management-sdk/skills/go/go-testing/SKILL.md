...
`testing` + `testify/require` for reference; substitute the real `flightordermanagement` and names.
...
import (
    ...
    "github.com/context-plugins/flight-order-management-go"   // root package flightordermanagement
)
...
func clientReturning(status int, body string) (flightordermanagement.ClientInterface, *stubTransport) {
    ...
    client := flightordermanagement.NewClient(
        flightordermanagement.CreateConfiguration(
            flightordermanagement.WithHttpConfiguration(
                flightordermanagement.CreateHttpConfiguration(flightordermanagement.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/flight-order-management-go/errors"
...
client := flightordermanagement.NewClient(
    flightordermanagement.CreateConfiguration(
        flightordermanagement.WithHttpConfiguration(
            flightordermanagement.CreateHttpConfiguration(flightordermanagement.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
