...
kwargs to the `AdafruitIORESTAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `adafruitiorestapi/adafruitiorestapi_client.py` and `adafruitiorestapi/http/auth/` in
the cloned source.

## Basic auth

```python
from adafruitiorestapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from adafruitiorestapi.http.auth.api_key import ApiKeyCredentials
...
```python
from adafruitiorestapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from adafruitiorestapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from adafruitiorestapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from adafruitiorestapi.http.auth.o_auth_acg import OAuthACGCredentials
from adafruitiorestapi.models.o_auth_scope_adafruitiorestapi_enum import OAuthScope{Pkg}Enum
...
client = AdafruitIORESTAPIClient(o_auth_acg_credentials=creds)
...
```python
from adafruitiorestapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AdafruitIORESTAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AdafruitIORESTAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `adafruitiorestapi/configuration.py` and each `adafruitiorestapi/http/auth/*.py` for the exact
