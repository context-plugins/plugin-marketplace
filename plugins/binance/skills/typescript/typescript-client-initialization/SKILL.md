...
- `binance-public-spot-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'binance-public-spot-apilib';
...
```ts
import { {Resource}Api } from 'binance-public-spot-apilib';
