...
`docker/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `docker/exceptions/` and operation docstrings.
...
```python
from docker.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `docker/exceptions/`.
...
```python
from docker.exceptions.api_exception import APIException
from docker.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`docker/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`docker/exceptions/` for the class.
...
`docker/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `docker/exceptions/`
  or a `ValueError` — check the auth handler in `docker/http/auth/` for the exact type.
