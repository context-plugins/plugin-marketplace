...
> `CellPointApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): CellPointApiClient
{
...
```php
use CellPointApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use CellPointApiLib\Exceptions\{OperationException};
use CellPointApiLib\Exceptions\ApiException;
...
app()->instance(CellPointApiClient::class, clientWithResponses([
