...
| --- | --- |
| API | Square |
| Source repo | https://github.com/context-plugins/square-csharp-sdk (branch `main`) |
| Root namespace | `Square` (the `using` namespace) |
| Client class | `SquareClient` |
| Options class | `SquareClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `SquareClientOptions`: `Oauth2: OAuth2AuthorizationCodeCredentials?`, `Oauth2TokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?`, `Oauth2ClientSecret: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `Production`, `Sandbox`, `Custom` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using Square.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/square-csharp-sdk
dotnet add reference square-csharp-sdk/Square.csproj
```
...
temp directory** (`<temp>/square-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/square-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/square-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/square-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\square-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/square-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/square-dotnet-src/`, **never** in the project repo, and its path never goes into
`square-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new SquareClient(...)`, build its options, or DI-register via
   `AddSquareClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `SquareClientOptions` — the map's *Servers & auth* section
