...
- `AuthentiqAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `authentiq_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'authentiq_api'
include AuthentiqAPI
...
The authoritative list is the `initialize` signature in `lib/authentiq_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/authentiq_api/configuration.rb`
(`build_default_config_from_env`) and `lib/authentiq_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/authentiq_api.rb`) and store it in a
