...
`networkadminmanagementclient/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `networkadminmanagementclient/exceptions/` and operation docstrings.
...
```python
from networkadminmanagementclient.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `networkadminmanagementclient/exceptions/`.
...
```python
from networkadminmanagementclient.exceptions.api_exception import APIException
from networkadminmanagementclient.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`networkadminmanagementclient/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`networkadminmanagementclient/exceptions/` for the class.
...
`networkadminmanagementclient/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `networkadminmanagementclient/exceptions/`
  or a `ValueError` — check the auth handler in `networkadminmanagementclient/http/auth/` for the exact type.
