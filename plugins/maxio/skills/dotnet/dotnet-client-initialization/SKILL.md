...
Tuning these knobs — `Retry` (retries, backoff, per-attempt timeout), `Server` / `Environment` (server
selection and **overriding the base URL**), and `Hooks` (request/response interception), plus pagination
and logging — is covered in **dotnet-configuration-resilience**.
