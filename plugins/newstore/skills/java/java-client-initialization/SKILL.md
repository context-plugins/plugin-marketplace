...
- `NewStoreAPIClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `net.newstore.p.dodicidemo` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`NewStoreAPIClient.Builder` class. A minimal initialization:

```java
import net.newstore.p.dodicidemo.NewStoreAPIClient;
import net.newstore.p.dodicidemo.Environment;

NewStoreAPIClient client = new NewStoreAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `NewStoreAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `NewStoreAPIClient.java` |

Call `.build()` to produce an immutable `NewStoreAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public NewStoreAPIClient apiClient() {
        return new NewStoreAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired NewStoreAPIClient client) {
        client.shutdown();
    }
}
```

Inject `NewStoreAPIClient` into your services as a normal Spring dependency. Build only one bean.
