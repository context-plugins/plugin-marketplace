...
```go
authentication.WithOAuthCCGCredentials(
    authentication.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr authentication.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := authentication.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr authentication.OAuthCCGManager) models.OAuthToken {
...
```go
creds := authentication.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := authentication.NewClient(authentication.CreateConfiguration(authentication.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    authentication.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := authentication.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := authentication.NewClient(authentication.CreateConfiguration(authentication.WithOAuthROPCGCredentials(creds)))
...
```go
authentication.WithOAuthBearerTokenCredentials(authentication.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
authentication.WithBasicAuthCredentials(authentication.NewBasicAuthCredentials(username, password))
authentication.WithApiHeaderCredentials(authentication.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
authentication.WithApiKeyCredentials(authentication.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `AUTHENTICATION_...` variables — e.g.
`AUTHENTICATION_USERNAME`, `AUTHENTICATION_PASSWORD`, `AUTHENTICATION_TOKEN`, `AUTHENTICATION_API_KEY`,
`AUTHENTICATION_O_AUTH_CLIENT_ID`, `AUTHENTICATION_O_AUTH_CLIENT_SECRET`, `AUTHENTICATION_ACCESS_TOKEN`. Grep the
