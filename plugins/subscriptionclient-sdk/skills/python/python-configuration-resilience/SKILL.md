...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `SubscriptionClientClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`subscriptionclient/configuration.py` in the cloned source.
...
```python
import requests
from subscriptionclient.configuration import Configuration
...
`subscriptionclient/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`subscriptionclient/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `subscriptionclient/configuration.py`
...
`NumberPagedResponse`) live in `subscriptionclient/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `subscriptionclient/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
