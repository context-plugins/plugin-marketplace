...
> `{ApiGroup}`, `OpenMeteoWeatherForecastApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `OpenMeteoWeatherForecastApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `OpenMeteoWeatherForecastApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `OpenMeteoWeatherForecastApi.Errors`
...
```csharp
using OpenMeteoWeatherForecastApi.Core.Exceptions;     // SdkException<TError>
using OpenMeteoWeatherForecastApi.Core.ErrorResponse;  // ApiError, RawError
using OpenMeteoWeatherForecastApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`OpenMeteoWeatherForecastApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using OpenMeteoWeatherForecastApi.Core.Exceptions;     // SdkException<TError>
using OpenMeteoWeatherForecastApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `OpenMeteoWeatherForecastApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `OpenMeteoWeatherForecastApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using OpenMeteoWeatherForecastApi.Core.Models;        // ApiResult<TResponse, TError>
using OpenMeteoWeatherForecastApi.Core.ErrorResponse; // RawError
using OpenMeteoWeatherForecastApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `OpenMeteoWeatherForecastApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
