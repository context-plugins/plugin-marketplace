...
`FlightInspirationSearchClient`/`FlightInspirationSearchClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `FlightInspirationSearchClient`,
...
static FlightInspirationSearchClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new FlightInspirationSearchClient(new HttpClient(handler), new FlightInspirationSearchClientOptions { /* auth not needed for stubs */ });
}
```
...
using FlightInspirationSearch.Errors;              // {Operation}Error types
...
```csharp
using FlightInspirationSearch.Core.Exceptions;
using FlightInspirationSearch.Core.ErrorResponse;
...
using FlightInspirationSearch.Core.ErrorResponse; // RawError (Case B)
...
var client = new FlightInspirationSearchClient(new HttpClient(handler), new FlightInspirationSearchClientOptions());
...
- For DI-based code, the SDK's `AddFlightInspirationSearchClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `FlightInspirationSearchClient` from the provider:
  ```csharp
  services.AddFlightInspirationSearchClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<FlightInspirationSearchClient>();
  ```
