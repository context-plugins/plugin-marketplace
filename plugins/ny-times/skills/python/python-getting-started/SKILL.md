...
> setting, so **confirm it** from the class names under `nytimes/apis/`.
...
| Install | `pip install "git+https://github.com/context-plugins/ny-times-python-sdk@main"` |
| Client class | `NytimesClient` in `nytimes/nytimes_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `nytimes/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `NytimesClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `nytimes/exceptions/api_exception.py` — raised on non-2xx |
...
- `nytimes/nytimes_client.py` — the `NytimesClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `nytimes/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `nytimes/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `nytimes/controllers/` — one `{Resource}Api` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `nytimes/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `nytimes/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `nytimes/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `nytimes/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `nytimes/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/ny-times-python-sdk@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/ny-times-python-sdk /tmp/ny-times-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/ny-times-python-sdk "$env:TEMP\ny-times-python-src"
```
...
- `nytimes/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `nytimes/nytimes_client.py` — `NytimesClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `nytimes/controllers/*.py` — operation method signatures, parameter names, and return types.
- `nytimes/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `nytimes/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/ny-times-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\ny-times-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `NytimesClient(...)`.
