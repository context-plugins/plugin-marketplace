...
`testing` + `testify/require` for reference; substitute the real `coingecko` and names.
...
import (
    ...
    "github.com/context-plugins/coingecko-go-sdk"   // root package coingecko
)
...
func clientReturning(status int, body string) (coingecko.ClientInterface, *stubTransport) {
    ...
    client := coingecko.NewClient(
        coingecko.CreateConfiguration(
            coingecko.WithHttpConfiguration(
                coingecko.CreateHttpConfiguration(coingecko.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/coingecko-go-sdk/errors"
...
client := coingecko.NewClient(
    coingecko.CreateConfiguration(
        coingecko.WithHttpConfiguration(
            coingecko.CreateHttpConfiguration(coingecko.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
