...
> `PazeCheckoutApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): PazeCheckoutApiClient
{
...
```php
use PazeCheckoutApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use PazeCheckoutApiLib\Exceptions\{OperationException};
use PazeCheckoutApiLib\Exceptions\ApiException;
...
app()->instance(PazeCheckoutApiClient::class, clientWithResponses([
