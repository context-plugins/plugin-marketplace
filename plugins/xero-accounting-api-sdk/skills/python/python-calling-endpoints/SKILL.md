...
Open `xeroaccountingapi/xeroaccountingapi_client.py` for the controller property names, then the relevant
`xeroaccountingapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from xeroaccountingapi.models.{request_model} import {RequestModel}
...
Open `xeroaccountingapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from xeroaccountingapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from xeroaccountingapi.http.http_call_back import HttpCallBack
...
client = XeroAccountingAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `xeroaccountingapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `XeroAccountingAPIClient` in `xeroaccountingapi/xeroaccountingapi_client.py`.
- Operation method signatures are in `xeroaccountingapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `xeroaccountingapi/models/`; enum types have `from_value()`.
