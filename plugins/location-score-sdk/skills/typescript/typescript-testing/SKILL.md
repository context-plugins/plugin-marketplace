...
```typescript
import { {Operation}Error } from 'location-scorelib/errors';
...
```typescript
import { ApiError } from 'location-scorelib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(LocationScoreClient)
    .useValue(new LocationScoreClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
