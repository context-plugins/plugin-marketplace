...
kwarg names are generated per-API — confirm from `pqapiv2/pqapiv2_client.py` and `pqapiv2/http/auth/` in
the cloned source.

## Basic auth

```python
from pqapiv2.http.auth.basic_auth import BasicAuthCredentials
...
```python
from pqapiv2.http.auth.api_key import ApiKeyCredentials
...
```python
from pqapiv2.http.auth.api_header import ApiHeaderCredentials
...
```python
from pqapiv2.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from pqapiv2.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from pqapiv2.http.auth.o_auth_acg import OAuthACGCredentials
from pqapiv2.models.o_auth_scope_pqapiv2_enum import OAuthScope{Pkg}Enum
...
```python
from pqapiv2.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `pqapiv2/configuration.py` and each `pqapiv2/http/auth/*.py` for the exact
