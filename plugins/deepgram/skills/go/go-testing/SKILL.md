...
`testing` + `testify/require` for reference; substitute the real `restapi` and names.
...
import (
    ...
    "github.com/context-plugins/deepgram-go-sdk"   // root package restapi
)
...
func clientReturning(status int, body string) (restapi.ClientInterface, *stubTransport) {
    ...
    client := restapi.NewClient(
        restapi.CreateConfiguration(
            restapi.WithHttpConfiguration(
                restapi.CreateHttpConfiguration(restapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/deepgram-go-sdk/errors"
...
client := restapi.NewClient(
    restapi.CreateConfiguration(
        restapi.WithHttpConfiguration(
            restapi.CreateHttpConfiguration(restapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
