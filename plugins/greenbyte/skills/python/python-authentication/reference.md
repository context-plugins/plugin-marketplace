...
kwargs to the `GreenbyteapiClient` constructor (or to `Configuration`). Exact credential class names and
...
client = GreenbyteapiClient(o_auth_acg_credentials=creds)
...
client = GreenbyteapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GreenbyteapiClient.from_environment()`) read each credential
