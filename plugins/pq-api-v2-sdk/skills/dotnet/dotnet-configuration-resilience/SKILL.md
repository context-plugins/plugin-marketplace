...
All types below live under `PqApiV2.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using PqApiV2.Core.Configuration;
...
```csharp
using PqApiV2.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new PqApiV2ClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `PqApiV2.Core.Exceptions`):
...
With DI, the SDK's `AddPqApiV2Client` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddPqApiV2Client(options => { /* ... */ });   // resolves CreateClient() → the default client
```
