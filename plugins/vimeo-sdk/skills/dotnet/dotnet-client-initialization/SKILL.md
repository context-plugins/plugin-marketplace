...
- `VimeoClientOptions` — its options class.
- `Vimeo` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddVimeoClient`, which registers the
...
builder.Services.AddVimeoClient(options =>
{
...
```csharp
public sealed class MyService(VimeoClient client)
{
