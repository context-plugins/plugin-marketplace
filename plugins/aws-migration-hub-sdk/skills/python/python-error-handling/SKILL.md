...
`awsmigrationhub/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `awsmigrationhub/exceptions/` and operation docstrings.
...
```python
from awsmigrationhub.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `awsmigrationhub/exceptions/`.
...
```python
from awsmigrationhub.exceptions.api_exception import APIException
from awsmigrationhub.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`awsmigrationhub/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`awsmigrationhub/exceptions/` for the class.
...
`awsmigrationhub/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `awsmigrationhub/exceptions/`
  or a `ValueError` — check the auth handler in `awsmigrationhub/http/auth/` for the exact type.
