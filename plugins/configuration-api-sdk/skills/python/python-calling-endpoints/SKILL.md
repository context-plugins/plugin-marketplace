...
Open `configurationapi/configurationapi_client.py` for the controller property names, then the relevant
`configurationapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from configurationapi.models.{request_model} import {RequestModel}
...
Open `configurationapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from configurationapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from configurationapi.http.http_call_back import HttpCallBack
...
client = ConfigurationAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `configurationapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `ConfigurationAPIClient` in `configurationapi/configurationapi_client.py`.
- Operation method signatures are in `configurationapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `configurationapi/models/`; enum types have `from_value()`.
