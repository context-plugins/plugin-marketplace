...
- `CoinGeckoDemoApiClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `com.coingecko.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`CoinGeckoDemoApiClient.Builder` class. A minimal initialization:

```java
import com.coingecko.api.CoinGeckoDemoApiClient;
import com.coingecko.api.Environment;
...
The `Builder` is the only entry point — confirm its fluent methods in `CoinGeckoDemoApiClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `CoinGeckoDemoApiClient.java` |

Call `.build()` to produce an immutable `CoinGeckoDemoApiClient` instance.
...
public class ApiConfig {
    @Bean
    public CoinGeckoDemoApiClient apiClient() {
        return new CoinGeckoDemoApiClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired CoinGeckoDemoApiClient client) {
        client.shutdown();
    }
}
```

Inject `CoinGeckoDemoApiClient` into your services as a normal Spring dependency. Build only one bean.
