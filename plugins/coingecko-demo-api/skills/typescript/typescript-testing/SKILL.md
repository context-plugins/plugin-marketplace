...
```typescript
import { {Operation}Error } from 'coingecko-demo-apilib/errors';
...
```typescript
import { ApiError } from 'coingecko-demo-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(CoinGeckoDemoAPIClient)
    .useValue(new CoinGeckoDemoAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
