...
Confirm every name against `lib/tesla_api/client.rb`, `lib/tesla_api/configuration.rb`, the
`lib/tesla_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
