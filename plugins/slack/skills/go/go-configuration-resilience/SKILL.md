    ...
    slack.CreateConfiguration(
        slack.WithHttpConfiguration(
            slack.CreateHttpConfiguration(
                slack.WithTimeout(30),
                slack.WithRetryConfiguration(
                    slack.CreateRetryConfiguration(
                        slack.WithMaxRetryAttempts(3),
                        slack.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := slack.CreateRetryConfiguration(
    slack.WithMaxRetryAttempts(5),
    slack.WithRetryInterval(1),
    slack.WithBackoffFactor(2),
    slack.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    slack.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := slack.NewClient(
    slack.CreateConfiguration(
        slack.WithHttpConfiguration(
            slack.CreateHttpConfiguration(
                slack.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
