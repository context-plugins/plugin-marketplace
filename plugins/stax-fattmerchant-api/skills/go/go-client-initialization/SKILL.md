...
- `staxfattmerchantapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/stax-fattmerchant-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/stax-fattmerchant-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/stax-fattmerchant-api-go-sdk"

client := staxfattmerchantapi.NewClient(
    staxfattmerchantapi.CreateConfiguration(
        staxfattmerchantapi.WithEnvironment(staxfattmerchantapi.PRODUCTION),
...
from environment variables named `STAXFATTMERCHANTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := staxfattmerchantapi.NewClient(staxfattmerchantapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `staxfattmerchantapi.PRODUCTION`,
`staxfattmerchantapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := staxfattmerchantapi.NewClient(
    staxfattmerchantapi.CreateConfiguration(staxfattmerchantapi.WithEnvironment(staxfattmerchantapi.PRODUCTION)),
)
```
...
client := staxfattmerchantapi.NewClient(
    staxfattmerchantapi.CreateConfiguration(
        staxfattmerchantapi.WithHttpConfiguration(
            staxfattmerchantapi.CreateHttpConfiguration(
                staxfattmerchantapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                staxfattmerchantapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = staxfattmerchantapi.NewClient(staxfattmerchantapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api staxfattmerchantapi.ClientInterface
}

func NewService(api staxfattmerchantapi.ClientInterface) *Service {
