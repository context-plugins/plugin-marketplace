...
All types below live under `HotelBooking.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using HotelBooking.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new HotelBookingClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `HotelBooking.Core.Exceptions`):
...
With DI, the SDK's `AddHotelBookingClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddHotelBookingClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
