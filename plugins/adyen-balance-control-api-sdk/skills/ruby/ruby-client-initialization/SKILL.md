...
- `AdyenBalanceControlAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `adyen_balance_control_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'adyen_balance_control_api'
include AdyenBalanceControlAPI
...
The authoritative list is the `initialize` signature in `lib/adyen_balance_control_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/adyen_balance_control_api/configuration.rb`
(`build_default_config_from_env`) and `lib/adyen_balance_control_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/adyen_balance_control_api.rb`) and store it in a
