...
description: "Google Maps Platform TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `google-maps-platform`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any Google Maps Platform TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the Google Maps Platform TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Google Maps Platform TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-google-maps-platform` for the workflow that wraps this skill.
...
Verified against `package.json` and `sdk-map.md` of the generated package at version `1.22.5`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Google Maps Platform |
| Package name (what you install, and what you import) | `google-maps-platform` — **not on npm**; built from source (see *Install*) |
| Import specifier | `google-maps-platform` — the package root is the **only** entry; deep imports do not resolve |
| Version | `1.22.5` (API spec version `1.22.5`) |
| Client class | `GoogleMapsPlatformClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new GoogleMapsPlatformClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new GoogleMapsPlatformClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `apiKeyAuth`. `timeout` defaults to `60_000` ms |
| Auth | **API key (query)** — set `ClientOptions.apiKeyAuth` |
| Environments | 3 environments (`ServerEnvironment.Production` *(default)*, `ServerEnvironment.Environment2`, `ServerEnvironment.Environment3`) × 1 server group |
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://www.googleapis.com` |
...
| Surface | 17 operations · 9 resources · 96 models · 41 open enums · 2 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/google-maps-platform-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 137 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { GoogleMapsPlatformClient, ServerEnvironment, ResponseError, GoogleMapsPlatformError } from "google-maps-platform";
import type { ClientOptions, AddressComponent } from "google-maps-platform";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `google-maps-platform/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **From CommonJS**, the typed spelling is `import sdk = require("google-maps-platform")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://www.googleapis.com` | `serverOptions.default.production.baseUrl` |
| `default` | `environment2` | `https://maps.googleapis.com` | `serverOptions.default.environment2.baseUrl` |
| `default` | `environment3` | `https://roads.googleapis.com` | `serverOptions.default.environment3.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 17 of the 17 operations require a credential and 0 are public.

| `ClientOptions` field | Scheme kind | What the SDK sends |
| --- | --- | --- |
| `apiKeyAuth` | API key (query) | query `key=<key>` |
...
| --- | --- | --- | --- |
| `client.geolocationApi` | `GeolocationApi` | 1 | `geolocate` |
| `client.directionsApi` | `DirectionsApi` | 1 | `directions` |
| `client.elevationApi` | `ElevationApi` | 1 | `elevation` |
| `client.geocodingApi` | `GeocodingApi` | 1 | `geocode` |
| `client.timeZoneApi` | `TimeZoneApi` | 1 | `timezone` |
| `client.roadsApi` | `RoadsApi` | 2 | `nearestRoads` · `snapToRoads` |
| `client.distanceMatrixApi` | `DistanceMatrixApi` | 1 | `distanceMatrix` |
| `client.placesApi` | `PlacesApi` | 7 | `autocomplete` · `findPlaceFromText` · `nearbySearch` · `placeDetails` · `placePhoto` · `queryAutocomplete` · `textSearch` |
| `client.streetViewApi` | `StreetViewApi` | 2 | `streetView` · `streetViewMetadata` |
...
```bash
node -e "console.log(require.resolve('google-maps-platform/package.json'))"
```

Failing that it is at `node_modules/google-maps-platform/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/google-maps-platform/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/google-maps-platform/`:
...
Beyond the usual signatures and model members, a contract sheet for the Google Maps Platform TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.

1. **Which host each deployment talks to**, and where that is set. The members are `ServerEnvironment.Production`, `ServerEnvironment.Environment2`, `ServerEnvironment.Environment3`, defaulting to `ServerEnvironment.Production` when the field is unset.
2. **2 operations resolve to `undefined` (`placesApi.placePhoto`, `streetViewApi.streetView`)** — `await` gives you nothing to inspect, so **`.asApiResult()` is the only way to observe their status and headers** — decide the mode at write time, not by retrofit.
