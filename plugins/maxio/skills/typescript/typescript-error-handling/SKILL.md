...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `advancedbilling`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'advancedbilling';
import { ApiError } from 'advancedbilling';
import { {Operation}Error } from 'advancedbilling/errors';
