...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `akoyalib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'akoyalib';
import { ApiError } from 'akoyalib';
import { {Operation}Error } from 'akoyalib/errors';
