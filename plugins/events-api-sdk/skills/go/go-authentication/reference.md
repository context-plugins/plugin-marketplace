...
```go
eventsapi.WithOAuthCCGCredentials(
    eventsapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr eventsapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := eventsapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr eventsapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := eventsapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := eventsapi.NewClient(eventsapi.CreateConfiguration(eventsapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    eventsapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := eventsapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := eventsapi.NewClient(eventsapi.CreateConfiguration(eventsapi.WithOAuthROPCGCredentials(creds)))
...
```go
eventsapi.WithOAuthBearerTokenCredentials(eventsapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
eventsapi.WithBasicAuthCredentials(eventsapi.NewBasicAuthCredentials(username, password))
eventsapi.WithApiHeaderCredentials(eventsapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
eventsapi.WithApiKeyCredentials(eventsapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `EVENTSAPI_...` variables — e.g.
`EVENTSAPI_USERNAME`, `EVENTSAPI_PASSWORD`, `EVENTSAPI_TOKEN`, `EVENTSAPI_API_KEY`,
`EVENTSAPI_O_AUTH_CLIENT_ID`, `EVENTSAPI_O_AUTH_CLIENT_SECRET`, `EVENTSAPI_ACCESS_TOKEN`. Grep the
