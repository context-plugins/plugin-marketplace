...
your SDK source (`paypalapi/models/`).
...
```python
from paypalapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `paypalapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from paypalapi.models.animal import Cat, Dog
...
```python
from paypalapi.models.animal import Animal, Cat, Dog
