...
Open `lib/flight_order_management/client.rb` for the accessor names, then the relevant
`lib/flight_order_management/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = FlightOrderManagement::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: FlightOrderManagement::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/flight_order_management/models/`. Field names are defined as
...
Read the enum module in `lib/flight_order_management/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/flight_order_management/client.rb`.
- Operation methods are in `lib/flight_order_management/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/flight_order_management/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/flight_order_management/exceptions/`.
