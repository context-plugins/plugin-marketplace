...
> `KlarnaPaymentsAPIV1Client`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): KlarnaPaymentsAPIV1Client
{
...
```php
use KlarnaPaymentsAPIV1Lib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use KlarnaPaymentsAPIV1Lib\Exceptions\{OperationException};
use KlarnaPaymentsAPIV1Lib\Exceptions\ApiException;
...
app()->instance(KlarnaPaymentsAPIV1Client::class, clientWithResponses([
