...
Open `sqlmanagementclient/sqlmanagementclient_client.py` for the controller property names, then the relevant
`sqlmanagementclient/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from sqlmanagementclient.models.{request_model} import {RequestModel}
...
Open `sqlmanagementclient/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from sqlmanagementclient.models.suite_code_enum import SuiteCodeEnum
...
```python
from sqlmanagementclient.http.http_call_back import HttpCallBack
...
client = SqlManagementClientClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `sqlmanagementclient/pagination/` in the source.

## Finding the right method

- Controller property names are on `SqlManagementClientClient` in `sqlmanagementclient/sqlmanagementclient_client.py`.
- Operation method signatures are in `sqlmanagementclient/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `sqlmanagementclient/models/`; enum types have `from_value()`.
