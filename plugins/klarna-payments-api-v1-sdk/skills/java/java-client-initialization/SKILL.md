...
description: Construct and configure an APIMatic-generated Java SDK client — build via the inner KlarnaPaymentsAPIV1Client.Builder() class (never a public constructor), set httpClientConfig via a lambda (not a config object), choose an Environment constant, access controllers via client.get{Resource}Controller(), call client.shutdown() at process exit, clone a live client with client.newBuilder(), and wire the client into Spring or plain-constructor DI. Use the moment you call new KlarnaPaymentsAPIV1Client.Builder(), pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the arguments but not the builder pattern, the httpClientConfig lambda, or the lifetime/reuse rules.
---
...
- `KlarnaPaymentsAPIV1Client` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.klarna.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`KlarnaPaymentsAPIV1Client.Builder` class. A minimal initialization:

```java
import com.klarna.api.KlarnaPaymentsAPIV1Client;
import com.klarna.api.Environment;

KlarnaPaymentsAPIV1Client client = new KlarnaPaymentsAPIV1Client.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `KlarnaPaymentsAPIV1Client.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `KlarnaPaymentsAPIV1Client.java` |

Call `.build()` to produce an immutable `KlarnaPaymentsAPIV1Client` instance.
...
public class ApiConfig {
    @Bean
    public KlarnaPaymentsAPIV1Client apiClient() {
        return new KlarnaPaymentsAPIV1Client.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired KlarnaPaymentsAPIV1Client client) {
        client.shutdown();
    }
}
```

Inject `KlarnaPaymentsAPIV1Client` into your services as a normal Spring dependency. Build only one bean.
