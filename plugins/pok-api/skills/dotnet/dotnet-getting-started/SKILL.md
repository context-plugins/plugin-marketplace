...
| --- | --- |
| API | `PokéAPI` |
| Project | `PokApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using PokApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/pok-api-csharp-sdk
dotnet add reference pok-api-csharp-sdk/PokApi.csproj
```

```csharp
using PokApi;          // client + options + ServerEnvironment
using PokApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/pok-api-csharp-sdk /tmp/pok-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/pok-api-csharp-sdk "$env:TEMP\pok-api-dotnet-src"
```
...
- `PokApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/pok-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\pok-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `PokApiClient`, build its
   `PokApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
