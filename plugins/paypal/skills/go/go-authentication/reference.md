...
```go
paypal.WithOAuthCCGCredentials(
    paypal.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr paypal.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := paypal.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr paypal.OAuthCCGManager) models.OAuthToken {
...
```go
creds := paypal.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := paypal.NewClient(paypal.CreateConfiguration(paypal.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    paypal.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := paypal.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := paypal.NewClient(paypal.CreateConfiguration(paypal.WithOAuthROPCGCredentials(creds)))
...
```go
paypal.WithOAuthBearerTokenCredentials(paypal.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
paypal.WithBasicAuthCredentials(paypal.NewBasicAuthCredentials(username, password))
paypal.WithApiHeaderCredentials(paypal.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
paypal.WithApiKeyCredentials(paypal.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `PAYPAL_...` variables — e.g.
`PAYPAL_USERNAME`, `PAYPAL_PASSWORD`, `PAYPAL_TOKEN`, `PAYPAL_API_KEY`,
`PAYPAL_O_AUTH_CLIENT_ID`, `PAYPAL_O_AUTH_CLIENT_SECRET`, `PAYPAL_ACCESS_TOKEN`. Grep the
