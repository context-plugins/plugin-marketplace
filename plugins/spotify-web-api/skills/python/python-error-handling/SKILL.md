...
`spotifywebapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `spotifywebapi/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `spotifywebapi/exceptions/`.
...
file (`spotifywebapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`spotifywebapi/exceptions/` for the class.
...
`spotifywebapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `spotifywebapi/exceptions/`
  or a `ValueError` — check the auth handler in `spotifywebapi/http/auth/` for the exact type.
