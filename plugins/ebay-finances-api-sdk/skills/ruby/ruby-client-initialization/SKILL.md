...
- `EBayFinancesAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `ebay_finances_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'ebay_finances_api'
include EBayFinancesAPI
...
The authoritative list is the `initialize` signature in `lib/ebay_finances_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/ebay_finances_api/configuration.rb`
(`build_default_config_from_env`) and `lib/ebay_finances_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/ebay_finances_api.rb`) and store it in a
