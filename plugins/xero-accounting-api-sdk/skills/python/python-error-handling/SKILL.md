...
`xeroaccountingapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `xeroaccountingapi/exceptions/` and operation docstrings.
...
```python
from xeroaccountingapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `xeroaccountingapi/exceptions/`.
...
```python
from xeroaccountingapi.exceptions.api_exception import APIException
from xeroaccountingapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`xeroaccountingapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`xeroaccountingapi/exceptions/` for the class.
...
`xeroaccountingapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `xeroaccountingapi/exceptions/`
  or a `ValueError` — check the auth handler in `xeroaccountingapi/http/auth/` for the exact type.
