...
client = stub_client(stubs)
assert_raises Firecrawl::APIException do
...
BASE = Firecrawl::Configuration.new(environment: Firecrawl::Environment::PRODUCTION).get_base_uri

def build_test_client
  Firecrawl::Client.new(
    environment: Firecrawl::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Firecrawl::APIException
```
...
```ruby
expect { call }.to raise_error(Firecrawl::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Firecrawl::HttpCallBack
...
client  = Firecrawl::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
