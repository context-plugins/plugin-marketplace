...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new HostedOnboardingAPIClient({
...
```typescript
import { {Operation}Error } from 'hosted-onboarding-apilib/errors';
...
```typescript
import { ApiError } from 'hosted-onboarding-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(HostedOnboardingAPIClient)
    .useValue(new HostedOnboardingAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
