...
> `StaxFattMerchantApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): StaxFattMerchantApiClient
{
...
```php
use StaxFattMerchantApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use StaxFattMerchantApiLib\Exceptions\{OperationException};
use StaxFattMerchantApiLib\Exceptions\ApiException;
...
app()->instance(StaxFattMerchantApiClient::class, clientWithResponses([
