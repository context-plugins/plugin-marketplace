...
your SDK source (`shutterstockapiexplorer/models/`).
...
```python
from shutterstockapiexplorer.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `shutterstockapiexplorer/utilities/union_type_lookup.py`) and each candidate model
...
```python
from shutterstockapiexplorer.models.animal import Cat, Dog
...
```python
from shutterstockapiexplorer.models.animal import Animal, Cat, Dog
