...
| Project | `DiscourseApiDocumentation.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using DiscourseApiDocumentation.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/discourse-api-documentation-csharp-sdk
dotnet add reference discourse-api-documentation-csharp-sdk/DiscourseApiDocumentation.csproj
```

```csharp
using DiscourseApiDocumentation;          // client + options + ServerEnvironment
using DiscourseApiDocumentation.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/discourse-api-documentation-csharp-sdk /tmp/discourse-api-documentation-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/discourse-api-documentation-csharp-sdk "$env:TEMP\discourse-api-documentation-dotnet-src"
```
...
- `DiscourseApiDocumentationClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/discourse-api-documentation-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\discourse-api-documentation-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `DiscourseApiDocumentationClient`, build its
   `DiscourseApiDocumentationClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
