...
│   └── com.zalando.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.zalando.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.zalando.api.http.client.HttpContext`, which carries:
