...
- `hotelnameautocomplete` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/hotel-name-autocomplete-go` — the Go import path for the root package; `github.com/context-plugins/hotel-name-autocomplete-go/models` for request/response types.
...
```go
import "github.com/context-plugins/hotel-name-autocomplete-go"

client := hotelnameautocomplete.NewClient(
    hotelnameautocomplete.CreateConfiguration(
        hotelnameautocomplete.WithEnvironment(hotelnameautocomplete.PRODUCTION),
...
from environment variables named `HOTELNAMEAUTOCOMPLETE_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := hotelnameautocomplete.NewClient(hotelnameautocomplete.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `hotelnameautocomplete.PRODUCTION`,
`hotelnameautocomplete.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := hotelnameautocomplete.NewClient(
    hotelnameautocomplete.CreateConfiguration(hotelnameautocomplete.WithEnvironment(hotelnameautocomplete.PRODUCTION)),
)
```
...
client := hotelnameautocomplete.NewClient(
    hotelnameautocomplete.CreateConfiguration(
        hotelnameautocomplete.WithHttpConfiguration(
            hotelnameautocomplete.CreateHttpConfiguration(
                hotelnameautocomplete.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                hotelnameautocomplete.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = hotelnameautocomplete.NewClient(hotelnameautocomplete.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api hotelnameautocomplete.ClientInterface
}

func NewService(api hotelnameautocomplete.ClientInterface) *Service {
