...
To see which schemes a specific SDK accepts, read the credential setter methods on `ConfigurationAPIClient.Builder`
...
- Set credentials **on the `ConfigurationAPIClient.Builder`** before calling `.build()` — the client is immutable
