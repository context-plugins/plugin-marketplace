...
- `APIsGuru` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `apis_guru` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/apis_guru/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/apis_guru/configuration.rb`
(`build_default_config_from_env`) and `lib/apis_guru/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/apis_guru.rb`) and store it in a
