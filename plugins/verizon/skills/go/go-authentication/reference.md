...
```go
verizon.WithOAuthCCGCredentials(
    verizon.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr verizon.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := verizon.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr verizon.OAuthCCGManager) models.OAuthToken {
...
```go
creds := verizon.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := verizon.NewClient(verizon.CreateConfiguration(verizon.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    verizon.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := verizon.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := verizon.NewClient(verizon.CreateConfiguration(verizon.WithOAuthROPCGCredentials(creds)))
...
```go
verizon.WithOAuthBearerTokenCredentials(verizon.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
verizon.WithBasicAuthCredentials(verizon.NewBasicAuthCredentials(username, password))
verizon.WithApiHeaderCredentials(verizon.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
verizon.WithApiKeyCredentials(verizon.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `VERIZON_...` variables — e.g.
`VERIZON_USERNAME`, `VERIZON_PASSWORD`, `VERIZON_TOKEN`, `VERIZON_API_KEY`,
`VERIZON_O_AUTH_CLIENT_ID`, `VERIZON_O_AUTH_CLIENT_SECRET`, `VERIZON_ACCESS_TOKEN`. Grep the
