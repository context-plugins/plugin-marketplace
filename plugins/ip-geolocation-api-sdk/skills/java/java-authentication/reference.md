...
against the `IPGeolocationAPIClient.Builder` setters, the `{rootpkg}.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `IPGeolocationAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
IPGeolocationAPIClient client = new IPGeolocationAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeIPGeolocationAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new IPGeolocationAPIClient.Builder()
