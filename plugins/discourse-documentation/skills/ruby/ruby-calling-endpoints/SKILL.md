...
Open `lib/discourse_api_documentation/client.rb` for the accessor names, then the relevant
`lib/discourse_api_documentation/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = DiscourseAPIDocumentation::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: DiscourseAPIDocumentation::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/discourse_api_documentation/models/`. Field names are defined as
...
Read the enum module in `lib/discourse_api_documentation/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/discourse_api_documentation/client.rb`.
- Operation methods are in `lib/discourse_api_documentation/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/discourse_api_documentation/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/discourse_api_documentation/exceptions/`.
