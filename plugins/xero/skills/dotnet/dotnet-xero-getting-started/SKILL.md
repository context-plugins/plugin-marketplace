---
name: dotnet-xero-getting-started
description: Xero .NET SDK identity and lookup layer for the dotnet-xero-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Xero .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-xero-sdk` helper
...
| --- | --- |
| API | Xero |
| Source repo | https://github.com/context-plugins/xero-csharp-sdk (branch `main`) |
| Root namespace | `Xero` (the `using` namespace) |
| Client class | `XeroClient` |
| Options class | `XeroClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `XeroClientOptions`: `Oauth2: OAuth2AuthorizationCodeCredentials?`, `Oauth2TokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?`, `Oauth21: OAuth2ClientCredentials?`, `Oauth21TokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?`, `BasicAuth: BasicAuthCredentials?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using Xero.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/xero-csharp-sdk
dotnet add reference xero-csharp-sdk/Xero.csproj
```
...
temp directory** (`<temp>/xero-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/xero-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/xero-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/xero-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\xero-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/xero-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/xero-dotnet-src/`, **never** in the project repo, and its path never goes into
`xero-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `XeroClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new XeroClient(...)`, build its options, or DI-register via
   `AddXeroClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `XeroClientOptions` — the map's *Servers & auth* section
