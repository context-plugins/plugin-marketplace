...
All types below live under `PokApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using PokApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new PokApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `PokApi.Core.Exceptions`):
...
With DI, the SDK's `AddPokApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddPokApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
