...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AutomationManagementClientClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`automationmanagementclient/configuration.py` in the cloned source.
...
```python
import requests
from automationmanagementclient.configuration import Configuration
...
`automationmanagementclient/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`automationmanagementclient/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `automationmanagementclient/configuration.py`
...
`NumberPagedResponse`) live in `automationmanagementclient/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `automationmanagementclient/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
