...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new ShellAPIClient({
...
```typescript
import { {Operation}Error } from 'shell-apilib/errors';
...
```typescript
import { ApiError } from 'shell-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(ShellAPIClient)
    .useValue(new ShellAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
