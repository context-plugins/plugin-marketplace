...
- `github.com/context-plugins/nytimes-go-sdk` — the Go import path for the root package; `github.com/context-plugins/nytimes-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/nytimes-go-sdk"
...
`nytimes.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
