...
> `FrankfurterApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FrankfurterApiClient
{
...
```php
use FrankfurterApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use FrankfurterApiLib\Exceptions\{OperationException};
use FrankfurterApiLib\Exceptions\ApiException;
...
app()->instance(FrankfurterApiClient::class, clientWithResponses([
