...
- `HotelRatings` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `hotel_ratings` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'hotel_ratings'
include HotelRatings
...
The authoritative list is the `initialize` signature in `lib/hotel_ratings/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/hotel_ratings/configuration.rb`
(`build_default_config_from_env`) and `lib/hotel_ratings/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/hotel_ratings.rb`) and store it in a
