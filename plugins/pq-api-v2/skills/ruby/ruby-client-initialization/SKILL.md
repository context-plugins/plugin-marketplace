...
- `PQAPIV2` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `pq_api_v2` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/pq_api_v2/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/pq_api_v2/configuration.rb`
(`build_default_config_from_env`) and `lib/pq_api_v2/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/pq_api_v2.rb`) and store it in a
