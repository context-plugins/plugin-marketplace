...
`HotelSearchApiClient`/`HotelSearchApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `HotelSearchApiClient`,
...
static HotelSearchApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new HotelSearchApiClient(new HttpClient(handler), new HotelSearchApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using HotelSearchApi.Errors;              // {Operation}Error types
...
```csharp
using HotelSearchApi.Core.Exceptions;
using HotelSearchApi.Core.ErrorResponse;
...
using HotelSearchApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new HotelSearchApiClient(new HttpClient(handler), new HotelSearchApiClientOptions());
...
- For DI-based code, the SDK's `AddHotelSearchApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `HotelSearchApiClient` from the provider:
  ```csharp
  services.AddHotelSearchApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<HotelSearchApiClient>();
  ```
