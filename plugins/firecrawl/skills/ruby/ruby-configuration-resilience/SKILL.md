...
```ruby
client = Firecrawl::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Firecrawl::Client.new(adapter: :net_http)
```
...
client = Firecrawl::Client.new(connection: conn)
```
...
client = Firecrawl::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Firecrawl::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Firecrawl::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Firecrawl::ProxySettings.from_env
client = Firecrawl::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Firecrawl::HttpCallBack
...
client = Firecrawl::Client.new(http_callback: LogCallback.new)
```
...
client = Firecrawl::Client.from_env(timeout: 30, max_retries: 3)
```
