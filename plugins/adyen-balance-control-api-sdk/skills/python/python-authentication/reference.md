...
kwargs to the `AdyenBalanceControlAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `adyenbalancecontrolapi/adyenbalancecontrolapi_client.py` and `adyenbalancecontrolapi/http/auth/` in
the cloned source.

## Basic auth

```python
from adyenbalancecontrolapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from adyenbalancecontrolapi.http.auth.api_key import ApiKeyCredentials
...
```python
from adyenbalancecontrolapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from adyenbalancecontrolapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from adyenbalancecontrolapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from adyenbalancecontrolapi.http.auth.o_auth_acg import OAuthACGCredentials
from adyenbalancecontrolapi.models.o_auth_scope_adyenbalancecontrolapi_enum import OAuthScope{Pkg}Enum
...
client = AdyenBalanceControlAPIClient(o_auth_acg_credentials=creds)
...
```python
from adyenbalancecontrolapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AdyenBalanceControlAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AdyenBalanceControlAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `adyenbalancecontrolapi/configuration.py` and each `adyenbalancecontrolapi/http/auth/*.py` for the exact
