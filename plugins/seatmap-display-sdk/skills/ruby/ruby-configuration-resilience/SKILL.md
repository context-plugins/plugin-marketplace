...
```ruby
client = SeatmapDisplay::Client.new(
  max_retries:    3,
...
```ruby
client = SeatmapDisplay::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = SeatmapDisplay::Client.new(adapter: :net_http)
```
...
client = SeatmapDisplay::Client.new(connection: conn)
```
...
client = SeatmapDisplay::Client.new(environment: Environment::PRODUCTION)
client = SeatmapDisplay::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = SeatmapDisplay::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = SeatmapDisplay::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = SeatmapDisplay::ProxySettings.from_env
client = SeatmapDisplay::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < SeatmapDisplay::HttpCallBack
...
client = SeatmapDisplay::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = SeatmapDisplay::Client.from_env
# Overrides take precedence over env vars:
client = SeatmapDisplay::Client.from_env(timeout: 30, max_retries: 3)
```
