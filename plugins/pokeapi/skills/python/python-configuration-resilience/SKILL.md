...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `PokapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`pokeapi/configuration.py` in the cloned source.
...
```python
import requests
from pokeapi.configuration import Configuration
...
`pokeapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`pokeapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `pokeapi/configuration.py`
...
`NumberPagedResponse`) live in `pokeapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `pokeapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
