...
> `{ApiGroup}`, `MaxioAdvancedBilling`) — replace it with the concrete identifier from the source.
...
- `ApiError` **and** `RawError` → `MaxioAdvancedBilling.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `MaxioAdvancedBilling.Errors`
...
using MaxioAdvancedBilling.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`MaxioAdvancedBilling.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
Case B needs no `.Errors` using — `RawError` lives under `MaxioAdvancedBilling.Core.ErrorResponse`. Its public
...
`readonly struct` under `MaxioAdvancedBilling.Core.Models`. If the controller has no `{Operation}Result`
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `MaxioAdvancedBilling.Core.Exceptions`) — an auth *application* failure, not an API error — when the
