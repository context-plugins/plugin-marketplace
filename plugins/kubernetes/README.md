# Kubernetes SDK Plugin

Integrate and consume the **APIMatic-generated Kubernetes SDK** (the "APIMATIC v3.0" generic
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, Go.
...
(`dotnet-kubernetes-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-kubernetes`** router skill routes all
C#/.NET SDK work to the single **`dotnet-kubernetes-sdk`** agent (.NET-only), which answers
