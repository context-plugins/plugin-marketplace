...
> `{ApiGroup}`, `OpenUvGlobalRealTimeUvIndexForecastApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `OpenUvGlobalRealTimeUvIndexForecastApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `OpenUvGlobalRealTimeUvIndexForecastApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `OpenUvGlobalRealTimeUvIndexForecastApi.Errors`
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Exceptions;     // SdkException<TError>
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.ErrorResponse;  // ApiError, RawError
using OpenUvGlobalRealTimeUvIndexForecastApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`OpenUvGlobalRealTimeUvIndexForecastApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Exceptions;     // SdkException<TError>
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `OpenUvGlobalRealTimeUvIndexForecastApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `OpenUvGlobalRealTimeUvIndexForecastApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Models;        // ApiResult<TResponse, TError>
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.ErrorResponse; // RawError
using OpenUvGlobalRealTimeUvIndexForecastApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `OpenUvGlobalRealTimeUvIndexForecastApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
