...
```go
teslaapi.WithOAuthCCGCredentials(
    teslaapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr teslaapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := teslaapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr teslaapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := teslaapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := teslaapi.NewClient(teslaapi.CreateConfiguration(teslaapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    teslaapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := teslaapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := teslaapi.NewClient(teslaapi.CreateConfiguration(teslaapi.WithOAuthROPCGCredentials(creds)))
...
```go
teslaapi.WithOAuthBearerTokenCredentials(teslaapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
teslaapi.WithBasicAuthCredentials(teslaapi.NewBasicAuthCredentials(username, password))
teslaapi.WithApiHeaderCredentials(teslaapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
teslaapi.WithApiKeyCredentials(teslaapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `TESLAAPI_...` variables — e.g.
`TESLAAPI_USERNAME`, `TESLAAPI_PASSWORD`, `TESLAAPI_TOKEN`, `TESLAAPI_API_KEY`,
`TESLAAPI_O_AUTH_CLIENT_ID`, `TESLAAPI_O_AUTH_CLIENT_SECRET`, `TESLAAPI_ACCESS_TOKEN`. Grep the
