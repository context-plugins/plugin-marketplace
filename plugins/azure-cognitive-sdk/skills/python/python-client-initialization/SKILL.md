...
- `azurecognitive` — the root package name (e.g. `multiauthsample`, `batester`).
- `AzureCognitiveClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `AzureCognitiveClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from azurecognitive.azurecognitive_client import AzureCognitiveClient
from azurecognitive.configuration import Environment
from azurecognitive.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = AzureCognitiveClient(
...
```python
from azurecognitive.configuration import Configuration, Environment
...
client = AzureCognitiveClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `azurecognitive/azurecognitive_client.py` in the cloned source.
...
`from_environment()` is a class method on both `AzureCognitiveClient` and `Configuration` that reads
...
client = AzureCognitiveClient.from_environment()
...
`from_environment` in `azurecognitive/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `AzureCognitiveClient`. Access them as attributes —
...
Open `azurecognitive/azurecognitive_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `azurecognitive/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from azurecognitive.configuration import Environment

client = AzureCognitiveClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`azurecognitive/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = AzureCognitiveClient(config=new_config)
```
