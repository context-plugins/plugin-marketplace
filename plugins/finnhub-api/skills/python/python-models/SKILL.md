...
your SDK source (`finnhubapi/models/`).
...
```python
from finnhubapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `finnhubapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from finnhubapi.models.animal import Cat, Dog
...
```python
from finnhubapi.models.animal import Animal, Cat, Dog
