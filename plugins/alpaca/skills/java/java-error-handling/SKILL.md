...
│   └── markets.alpaca.paperapi.exceptions.ApiException          ← base for all non-2xx API responses
│       └── markets.alpaca.paperapi.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `markets.alpaca.paperapi.http.client.HttpContext`, which carries:
