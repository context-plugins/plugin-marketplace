...
- `GiteaApiClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `com.gitea` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`GiteaApiClient.Builder` class. A minimal initialization:

```java
import com.gitea.GiteaApiClient;
import com.gitea.Environment;

GiteaApiClient client = new GiteaApiClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `GiteaApiClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `GiteaApiClient.java` |

Call `.build()` to produce an immutable `GiteaApiClient` instance.
...
public class ApiConfig {
    @Bean
    public GiteaApiClient apiClient() {
        return new GiteaApiClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired GiteaApiClient client) {
        client.shutdown();
    }
}
```

Inject `GiteaApiClient` into your services as a normal Spring dependency. Build only one bean.
