...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `OpenAIAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`openaiapi/configuration.py` in the cloned source.
...
`openaiapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`openaiapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `openaiapi/configuration.py`
...
`NumberPagedResponse`) live in `openaiapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `openaiapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
