...
```typescript
import { {Operation}Error } from 'google-workspacelib/errors';
...
```typescript
import { ApiError } from 'google-workspacelib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(GoogleWorkspaceClient)
    .useValue(new GoogleWorkspaceClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
