...
against the `SlackWebAPIClient.Builder` setters, the `com.slack.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SlackWebAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
SlackWebAPIClient client = new SlackWebAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeSlackWebAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
