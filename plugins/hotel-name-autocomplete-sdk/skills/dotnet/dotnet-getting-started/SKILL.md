...
| Project | `HotelNameAutocomplete.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `HotelNameAutocomplete` (the `using` namespace) |
| Client class | `HotelNameAutocompleteClient` |
| Options class | `HotelNameAutocompleteClientOptions` |
...
namespaces transitively, so `using HotelNameAutocomplete.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/hotel-name-autocomplete-csharp
dotnet add reference hotel-name-autocomplete-csharp/HotelNameAutocomplete.csproj
```

```csharp
using HotelNameAutocomplete;          // client + options + ServerEnvironment
using HotelNameAutocomplete.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/hotel-name-autocomplete-csharp /tmp/hotel-name-autocomplete-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/hotel-name-autocomplete-csharp "$env:TEMP\hotel-name-autocomplete-dotnet-src"
```
...
- `HotelNameAutocompleteClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/hotel-name-autocomplete-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\hotel-name-autocomplete-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `HotelNameAutocompleteClient`, build its
   `HotelNameAutocompleteClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
