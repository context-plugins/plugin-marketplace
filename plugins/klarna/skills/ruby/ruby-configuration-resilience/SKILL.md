...
```ruby
client = Klarna::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Klarna::Client.new(adapter: :net_http)
```
...
client = Klarna::Client.new(connection: conn)
```
...
client = Klarna::Client.new(environment: Environment::PRODUCTION)
client = Klarna::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = Klarna::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Klarna::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Klarna::ProxySettings.from_env
client = Klarna::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Klarna::HttpCallBack
...
client = Klarna::Client.new(http_callback: LogCallback.new)
```
...
client = Klarna::Client.from_env(timeout: 30, max_retries: 3)
```
