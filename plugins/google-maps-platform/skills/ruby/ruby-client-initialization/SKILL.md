...
- `GoogleMapsPlatform` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `google_maps_platform` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/google_maps_platform/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/google_maps_platform/configuration.rb`
(`build_default_config_from_env`) and `lib/google_maps_platform/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = GoogleMapsPlatform::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/google_maps_platform.rb`) and store it in a
