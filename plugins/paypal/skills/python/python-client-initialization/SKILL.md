...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the PaypalServersdkClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write PaypalServersdkClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---
...
- `paypal` — the root package name (e.g. `multiauthsample`, `batester`).
- `PaypalServersdkClient` — the generated client class (read it from the `class …Client` declaration in `paypal/paypal_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `PaypalServersdkClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from paypal.paypal_client import PaypalServersdkClient
from paypal.configuration import Environment
from paypal.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from paypal.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `paypal/paypal_client.py` in the cloned source.
...
`from_environment()` is a class method on both `PaypalServersdkClient` and `Configuration` that reads
...
`from_environment` in `paypal/configuration.py` for the exact names.
...
Open `paypal/paypal_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `paypal/configuration.py`. **Read the enum in `configuration.py`
...
client = PaypalServersdkClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`paypal/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = PaypalServersdkClient(config=new_config)
```
