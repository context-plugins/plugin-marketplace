...
`flightoffersprice/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `flightoffersprice/exceptions/` and operation docstrings.
...
```python
from flightoffersprice.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `flightoffersprice/exceptions/`.
...
```python
from flightoffersprice.exceptions.api_exception import APIException
from flightoffersprice.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`flightoffersprice/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`flightoffersprice/exceptions/` for the class.
...
`flightoffersprice/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `flightoffersprice/exceptions/`
  or a `ValueError` — check the auth handler in `flightoffersprice/http/auth/` for the exact type.
