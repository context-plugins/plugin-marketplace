...
`flightordermanagement/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `flightordermanagement/exceptions/` and operation docstrings.
...
```python
from flightordermanagement.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `flightordermanagement/exceptions/`.
...
```python
from flightordermanagement.exceptions.api_exception import APIException
from flightordermanagement.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`flightordermanagement/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`flightordermanagement/exceptions/` for the class.
...
`flightordermanagement/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `flightordermanagement/exceptions/`
  or a `ValueError` — check the auth handler in `flightordermanagement/http/auth/` for the exact type.
