...
- `squareconnectapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/square-connect-api-go` — the Go import path for the root package; `github.com/context-plugins/square-connect-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/square-connect-api-go"

client := squareconnectapi.NewClient(
    squareconnectapi.CreateConfiguration(
        squareconnectapi.WithEnvironment(squareconnectapi.PRODUCTION),
...
from environment variables named `SQUARECONNECTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := squareconnectapi.NewClient(squareconnectapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `squareconnectapi.PRODUCTION`,
`squareconnectapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := squareconnectapi.NewClient(
    squareconnectapi.CreateConfiguration(squareconnectapi.WithEnvironment(squareconnectapi.PRODUCTION)),
)
```
...
client := squareconnectapi.NewClient(
    squareconnectapi.CreateConfiguration(
        squareconnectapi.WithHttpConfiguration(
            squareconnectapi.CreateHttpConfiguration(
                squareconnectapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                squareconnectapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = squareconnectapi.NewClient(squareconnectapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api squareconnectapi.ClientInterface
}

func NewService(api squareconnectapi.ClientInterface) *Service {
