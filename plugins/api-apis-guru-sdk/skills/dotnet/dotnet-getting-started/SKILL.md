...
| --- | --- |
| API | `api.apis.guru` |
| Project | `PointsOfInterest.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `PointsOfInterest` (the `using` namespace) |
| Client class | `PointsOfInterestClient` |
| Options class | `PointsOfInterestClientOptions` |
...
namespaces transitively, so `using PointsOfInterest.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/api-apis-guru-csharp-013bf4
dotnet add reference api-apis-guru-csharp-013bf4/PointsOfInterest.csproj
```

```csharp
using PointsOfInterest;          // client + options + ServerEnvironment
using PointsOfInterest.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-013bf4 /tmp/api-apis-guru-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/api-apis-guru-csharp-013bf4 "$env:TEMP\api-apis-guru-dotnet-src"
```
...
- `PointsOfInterestClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `PointsOfInterestClient`, build its
   `PointsOfInterestClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
