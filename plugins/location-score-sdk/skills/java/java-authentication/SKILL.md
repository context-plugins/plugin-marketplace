...
To see which schemes a specific SDK accepts, read the credential setter methods on `LocationScoreClient.Builder`
...
- Set credentials **on the `LocationScoreClient.Builder`** before calling `.build()` — the client is immutable
