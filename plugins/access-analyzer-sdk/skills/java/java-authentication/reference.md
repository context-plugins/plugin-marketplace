...
against the `AccessAnalyzerClient.Builder` setters, the `com.amazonaws.useast1.accessanalyzer.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AccessAnalyzerClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AccessAnalyzerClient client = new AccessAnalyzerClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAccessAnalyzerEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new AccessAnalyzerClient.Builder()
