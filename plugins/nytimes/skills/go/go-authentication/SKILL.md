...
client := nytimes.NewClient(
    nytimes.CreateConfiguration(
        nytimes.WithBasicAuthCredentials(
            nytimes.NewBasicAuthCredentials(
...
nytimes.WithApiKeyCredentials(nytimes.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
nytimes.WithApiHeaderCredentials(nytimes.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
nytimes.WithOAuthBearerTokenCredentials(
    nytimes.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := nytimes.NewClient(
    nytimes.CreateConfiguration(
        nytimes.WithOAuthCCGCredentials(
            nytimes.NewOAuthCCGCredentials(
