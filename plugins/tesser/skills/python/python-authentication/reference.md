...
kwargs to the `TesserAPIV1Client` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `tesserapiv1/tesserapiv1_client.py` and `tesserapiv1/http/auth/` in
the cloned source.

## Basic auth

```python
from tesserapiv1.http.auth.basic_auth import BasicAuthCredentials
...
```python
from tesserapiv1.http.auth.api_key import ApiKeyCredentials
...
```python
from tesserapiv1.http.auth.api_header import ApiHeaderCredentials
...
```python
from tesserapiv1.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from tesserapiv1.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from tesserapiv1.http.auth.o_auth_acg import OAuthACGCredentials
from tesserapiv1.models.o_auth_scope_tesserapiv1_enum import OAuthScope{Pkg}Enum
...
client = TesserAPIV1Client(o_auth_acg_credentials=creds)
...
```python
from tesserapiv1.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = TesserAPIV1Client(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `TesserAPIV1Client.from_environment()`) read each credential
...
Grep `from_environment` in `tesserapiv1/configuration.py` and each `tesserapiv1/http/auth/*.py` for the exact
