...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the NytimesClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write NytimesClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `NytimesClient` is the SDK's client class — **read the real name** from `nytimes/nytimes_client.py`
...
- `nytimes` — the root package name (e.g. `multiauthsample`, `batester`).
- `NytimesClient` — the generated client class (read it from the `class …Client` declaration in `nytimes/nytimes_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `NytimesClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from nytimes.nytimes_client import NytimesClient
from nytimes.configuration import Environment
from nytimes.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from nytimes.configuration import Configuration, Environment
...
client = NytimesClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `nytimes/nytimes_client.py` in the cloned source.
...
`from_environment()` is a class method on both `NytimesClient` and `Configuration` that reads
...
client = NytimesClient.from_environment()
...
`from_environment` in `nytimes/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `NytimesClient`. Access them as attributes —
...
Open `nytimes/nytimes_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `nytimes/configuration.py`. Use the constant for the desired
...
client = NytimesClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`nytimes/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = NytimesClient(config=new_config)
```
