...
> `{ApiGroup}`, `FlightCreateOrders`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FlightCreateOrders.Core.Exceptions`
- `ApiError` **and** `RawError` → `FlightCreateOrders.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FlightCreateOrders.Errors`
...
```csharp
using FlightCreateOrders.Core.Exceptions;     // SdkException<TError>
using FlightCreateOrders.Core.ErrorResponse;  // ApiError, RawError
using FlightCreateOrders.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FlightCreateOrders.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FlightCreateOrders.Core.Exceptions;     // SdkException<TError>
using FlightCreateOrders.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FlightCreateOrders.Core.ErrorResponse`. Its public
...
`readonly struct` under `FlightCreateOrders.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FlightCreateOrders.Core.Models;        // ApiResult<TResponse, TError>
using FlightCreateOrders.Core.ErrorResponse; // RawError
using FlightCreateOrders.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FlightCreateOrders.Core.Exceptions`) — an auth *application* failure, not an API error — when the
