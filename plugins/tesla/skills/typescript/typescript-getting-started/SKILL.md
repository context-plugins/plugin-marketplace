...
description: "Tesla Fleet Management API TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `tesla-fleet-management-api`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any Tesla Fleet Management API TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the Tesla Fleet Management API TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Tesla Fleet Management API TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-tesla` for the workflow that wraps this skill.
...
| Fact | Value |
| --- | --- |
| API | Tesla Fleet Management API |
| Package name (what you install, and what you import) | `tesla-fleet-management-api` — **not on npm**; built from source (see *Install*) |
| Import specifier | `tesla-fleet-management-api` — the package root is the **only** entry; deep imports do not resolve |
| Version | `1.0.0` (API spec version `1.0.0`) |
| Client class | `TeslaFleetManagementApiClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new TeslaFleetManagementApiClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new TeslaFleetManagementApiClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `bearerAuth` · `thirdpartytokenAuthorizationCode` · `thirdpartytokenAuthorizationCodeStrategy` · `thirdpartytokenClientCredentials` · `thirdpartytokenClientCredentialsStrategy`. `timeout` defaults to `60_000` ms |
| Auth | **Bearer token** — set `ClientOptions.bearerAuth`<br>**OAuth 2 authorization code** — set `ClientOptions.thirdpartytokenAuthorizationCode`<br>**OAuth 2 client credentials** — set `ClientOptions.thirdpartytokenClientCredentials` |
| Environments | 2 environments (`ServerEnvironment.Production` *(default)*, `ServerEnvironment.Environment2`) × 1 server group |
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://fleet-api.prd.na.vn.cloud.tesla.com` |
...
| Surface | 64 operations · 6 resources · 84 models · 4 open enums · 0 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/tesla-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 88 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { TeslaFleetManagementApiClient, ServerEnvironment, ResponseError, TeslaFleetManagementApiError } from "tesla-fleet-management-api";
import type { ClientOptions, ActuateTrunkRequest } from "tesla-fleet-management-api";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `tesla-fleet-management-api/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **⚠ The SDK exports a model type literally named `Event`** (`src/models/event.ts`, schema `eventSchema`). Import it unaliased and it **shadows the global `Event`** for the rest of the file. Alias it: `import type { Event as SdkEvent } from "tesla-fleet-management-api"`.
- **⚠ The SDK exports a model type literally named `Response`** (`src/models/response.ts`, schema `responseSchema`). Import it unaliased and it **shadows the global `Response`** for the rest of the file. Alias it: `import type { Response as SdkResponse } from "tesla-fleet-management-api"`.
- **From CommonJS**, the typed spelling is `import sdk = require("tesla-fleet-management-api")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://fleet-api.prd.na.vn.cloud.tesla.com` | `serverOptions.default.production.baseUrl` |
| `default` | `environment2` | `https://auth.tesla.com/oauth2/v3` | `serverOptions.default.environment2.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 64 of the 64 operations require a credential and 0 are public.

| `ClientOptions` field | Scheme kind | What the SDK sends |
| --- | --- | --- |
| `bearerAuth` | Bearer token | `Authorization: Bearer <token>` |
| `thirdpartytokenAuthorizationCode` | OAuth 2 authorization code | `Authorization: Bearer <access token>` |
| `thirdpartytokenClientCredentials` | OAuth 2 client credentials | `Authorization: Bearer <access token>` |

```ts
const client = new TeslaFleetManagementApiClient({
  bearerAuth: process.env.API_TOKEN!,
  thirdpartytokenAuthorizationCode: { clientId: process.env.CLIENT_ID!, redirectUri: process.env.REDIRECT_URI!, promptForAuthorizationCode: async (url) => { /* send the user to url */ return "YOUR_AUTHORIZATION_CODE"; } },
  thirdpartytokenClientCredentials: { clientId: process.env.CLIENT_ID!, clientSecret: process.env.CLIENT_SECRET! },
});
```
...
`thirdpartytokenAuthorizationCodeStrategy`, `thirdpartytokenClientCredentialsStrategy` on `ClientOptions` substitute the whole token request (`getToken(credentials, signal)`, plus `tryRefreshToken(…)` where the grant is refreshable); the caching, expiry buffer and single-flight behaviour still apply.
...
| --- | --- | --- | --- |
| `client.charging` | `Charging` | 3 | `getChargingHistory` · `getChargingInvoice` · `getChargingSessions` |
| `client.energy` | `Energy` | 11 | `adjustSiteSBackupReserve` · `adjustSiteSOffGridVehicleChargingReserve` · `allowDisallowChargingFromTheGridAndExportingEnergyToTheGrid` · `getBackupOrEnergyHistory` · `getLiveSiteStatus` · `getSiteInformationAssetsSettingsFeatures` · `getUserProductsVehiclesEnergySites` · `getWallConnectorChargingHistory` · `setSiteModeAutonomousOrSelfConsumption` · `updateStormWatchParticipation` · `updateTimeOfUseTouSettings` |
| `client.partner` | `Partner` | 4 | `getPublicKeyForADomain` · `getRecentFleetTelemetryErrors` · `getVinsWithFleetTelemetryErrors` · `registerAPartnerAccount` |
| `client.user` | `User` | 4 | `getActiveOrdersForAUser` · `getCustomFeatureFlagsForAUser` · `getSummaryOfAUserSAccount` · `getUserSRegionAndFleetApiBaseUrl` |
| `client.vehicles` | `Vehicles` | 21 | `configureFleetTelemetryUsingSignedJwsToken` · `createOrUpdateFleetTelemetryConfiguration` · `deleteFleetTelemetryConfiguration` · `getAllowedDriversForAVehicle` · `getEligibleVehicleSubscriptions` · `getEligibleVehicleUpgrades` · `getEnterpriseRolesForAVehicle` · `getFleetStatusForVehicles` · `getFleetTelemetryConfiguration` · `getFleetTelemetryErrorsForAVehicle` · `getVehicle` · `listVehicles` · `mobileEnabled` · `nearbyChargingSites` · `removeDriverAccessFromAVehicle` · `setEnterprisePayerRoles` · `vehicleLiveData` · `vehicleOptions` · `vehicleSpecs` · `wakeUpVehicle` · `warrantyDetails` |
| `client.vehicleCommands` | `VehicleCommands` | 21 | `actuatetrunk` · `addchargeschedule` · `addpreconditionschedule` · `adjustmediavolume` · `cancelsoftwareupdate` · `chargemaxrange` · `chargestandard` · `clearPiNtoDriveAdmin` · `closechargeportdoor` · `enableordisableGuestMode` · `eraseuserdata` · `flashlights` · `honkhorn` · `lockdoors` · `nextfavoritemediatrack` · `openchargeportdoor` · `startcharging` · `startclimatepreconditioning` · `stopcharging` · `stopclimatepreconditioning` · `unlockdoors` |
...
```bash
node -e "console.log(require.resolve('tesla-fleet-management-api/package.json'))"
```

Failing that it is at `node_modules/tesla-fleet-management-api/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/tesla-fleet-management-api/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/tesla-fleet-management-api/`:
...
Beyond the usual signatures and model members, a contract sheet for the Tesla Fleet Management API TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
...
2. **2 operations resolve to `undefined` (`charging.getChargingInvoice`, `vehicles.setEnterprisePayerRoles`)** — `await` gives you nothing to inspect, so **`.asApiResult()` is the only way to observe their status and headers** — decide the mode at write time, not by retrofit.
...
9. **That `Event`, `Response` imported from this package are model types, not the globals of those names** — every sheet that references one should carry the alias it will be imported under. The error base is re-exported as `TeslaFleetManagementApiError` for the same reason.
