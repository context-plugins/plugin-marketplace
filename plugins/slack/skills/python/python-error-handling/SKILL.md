...
`slack/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `slack/exceptions/` and operation docstrings.
...
```python
from slack.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `slack/exceptions/`.
...
```python
from slack.exceptions.api_exception import APIException
from slack.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`slack/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`slack/exceptions/` for the class.
...
`slack/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `slack/exceptions/`
  or a `ValueError` — check the auth handler in `slack/http/auth/` for the exact type.
