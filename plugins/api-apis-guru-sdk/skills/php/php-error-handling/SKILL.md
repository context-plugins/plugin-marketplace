...
> `{operation}`, `{apiGroup}`, `HotelBookingLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `HotelBookingLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use HotelBookingLib\Exceptions\ApiException;
...
```php
use HotelBookingLib\Exceptions\ApiException;
use HotelBookingLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `HotelBookingLib\Exceptions\` — read the source to
