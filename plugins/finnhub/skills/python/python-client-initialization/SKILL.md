...
> `FinnhubapiClient` is the SDK's client class — **read the real name** from `finnhub/finnhub_client.py`
...
- `finnhub` — the root package name (e.g. `multiauthsample`, `batester`).
- `FinnhubapiClient` — the generated client class (read it from the `class …Client` declaration in `finnhub/finnhub_client.py`).
...
```python
from finnhub.finnhub_client import FinnhubapiClient
from finnhub.configuration import Environment
from finnhub.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from finnhub.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `finnhub/finnhub_client.py` in the cloned source.
...
`from_environment` in `finnhub/configuration.py` for the exact names.
...
Open `finnhub/finnhub_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `finnhub/configuration.py`. **Read the enum in `configuration.py`
...
`finnhub/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
