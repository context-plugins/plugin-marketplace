...
`authentiqapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `authentiqapi/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `authentiqapi/exceptions/`.
...
file (`authentiqapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`authentiqapi/exceptions/` for the class.
...
`authentiqapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `authentiqapi/exceptions/`
  or a `ValueError` — check the auth handler in `authentiqapi/http/auth/` for the exact type.
