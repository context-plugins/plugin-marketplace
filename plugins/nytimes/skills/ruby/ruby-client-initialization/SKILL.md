...
The authoritative list is the `initialize` signature in `lib/nytimes/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/nytimes/configuration.rb`
(`build_default_config_from_env`) and `lib/nytimes/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/nytimes.rb`) and store it in a
