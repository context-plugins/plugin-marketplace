...
```ruby
client = Sportsdata::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Sportsdata::Client.new(adapter: :net_http)
```
...
client = Sportsdata::Client.new(connection: conn)
```
...
client = Sportsdata::Client.new(environment: Environment::PRODUCTION)
client = Sportsdata::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = Sportsdata::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Sportsdata::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Sportsdata::ProxySettings.from_env
client = Sportsdata::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Sportsdata::HttpCallBack
...
client = Sportsdata::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = Sportsdata::Client.from_env
# Overrides take precedence over env vars:
client = Sportsdata::Client.from_env(timeout: 30, max_retries: 3)
```
