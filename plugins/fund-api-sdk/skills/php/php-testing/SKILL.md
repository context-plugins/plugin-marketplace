...
> `FundAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FundAPIClient
{
...
```php
use FundAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use FundAPILib\Exceptions\{OperationException};
use FundAPILib\Exceptions\ApiException;
...
app()->instance(FundAPIClient::class, clientWithResponses([
