...
against the `AdvancedBillingClient.Builder` setters, the `com.maxio.advancedbilling.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AdvancedBillingClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AdvancedBillingClient client = new AdvancedBillingClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeMaxioEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new AdvancedBillingClient.Builder()
