...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new MediumAPIClient({
...
```typescript
import { {Operation}Error } from 'medium-apilib/errors';
...
```typescript
import { ApiError } from 'medium-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(MediumAPIClient)
    .useValue(new MediumAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
