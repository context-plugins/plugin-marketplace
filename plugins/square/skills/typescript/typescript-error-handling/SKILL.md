...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `squarelib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'squarelib';
import { ApiError } from 'squarelib';
import { {Operation}Error } from 'squarelib/errors';
