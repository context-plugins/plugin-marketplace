...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `fdx-apilib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'fdx-apilib';
import { ApiError } from 'fdx-apilib';
import { {Operation}Error } from 'fdx-apilib/errors';
