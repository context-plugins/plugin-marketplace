...
> `{ApiGroup}`, `PayPalApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `PayPalApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `PayPalApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `PayPalApi.Errors`
...
```csharp
using PayPalApi.Core.Exceptions;     // SdkException<TError>
using PayPalApi.Core.ErrorResponse;  // ApiError, RawError
using PayPalApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`PayPalApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using PayPalApi.Core.Exceptions;     // SdkException<TError>
using PayPalApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `PayPalApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `PayPalApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using PayPalApi.Core.Models;        // ApiResult<TResponse, TError>
using PayPalApi.Core.ErrorResponse; // RawError
using PayPalApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `PayPalApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
