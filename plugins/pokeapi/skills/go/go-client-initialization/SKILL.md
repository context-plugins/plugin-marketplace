...
- `pokeapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/pokeapi-go-sdk` — the Go import path for the root package; `github.com/context-plugins/pokeapi-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/pokeapi-go-sdk"

client := pokeapi.NewClient(
    pokeapi.CreateConfiguration(
        pokeapi.WithEnvironment(pokeapi.PRODUCTION),
...
from environment variables named `POKEAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := pokeapi.NewClient(pokeapi.CreateConfigurationFromEnvironment())
```
...
```go
client := pokeapi.NewClient(
    pokeapi.CreateConfiguration(pokeapi.WithEnvironment(pokeapi.PRODUCTION)),
)
```
...
client := pokeapi.NewClient(
    pokeapi.CreateConfiguration(
        pokeapi.WithHttpConfiguration(
            pokeapi.CreateHttpConfiguration(
                pokeapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                pokeapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = pokeapi.NewClient(pokeapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api pokeapi.ClientInterface
}

func NewService(api pokeapi.ClientInterface) *Service {
