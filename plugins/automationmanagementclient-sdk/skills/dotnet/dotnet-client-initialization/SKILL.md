...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddAutomationManagementClientClient extension. Use the moment you write `new AutomationManagementClientClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `AutomationManagementClientClientOptions` — its options class.
- `AutomationManagementClient` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public AutomationManagementClientClient(HttpClient httpClient, AutomationManagementClientClientOptions options)
```
...
```csharp
public class AutomationManagementClientClientOptions
{
...
```csharp
using AutomationManagementClient;
using AutomationManagementClient.Servers;

var options = new AutomationManagementClientClientOptions
{
...
var client = new AutomationManagementClientClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddAutomationManagementClientClient`, which registers the
...
builder.Services.AddAutomationManagementClientClient(options =>
{
...
```csharp
public sealed class MyService(AutomationManagementClientClient client)
{
