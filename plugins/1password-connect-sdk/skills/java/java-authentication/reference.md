...
against the `1PasswordConnectClient.Builder` setters, the `local.m1password.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `1PasswordConnectClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
1PasswordConnectClient client = new 1PasswordConnectClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScope1PasswordConnectEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new 1PasswordConnectClient.Builder()
