...
│   └── com.klarna.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.klarna.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.klarna.api.http.client.HttpContext`, which carries:
