...
kwargs to the `AdobeExperienceManagerAEMAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `adobeexperiencemanageraemapi/adobeexperiencemanageraemapi_client.py` and `adobeexperiencemanageraemapi/http/auth/` in
the cloned source.

## Basic auth

```python
from adobeexperiencemanageraemapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from adobeexperiencemanageraemapi.http.auth.api_key import ApiKeyCredentials
...
```python
from adobeexperiencemanageraemapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from adobeexperiencemanageraemapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from adobeexperiencemanageraemapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from adobeexperiencemanageraemapi.http.auth.o_auth_acg import OAuthACGCredentials
from adobeexperiencemanageraemapi.models.o_auth_scope_adobeexperiencemanageraemapi_enum import OAuthScope{Pkg}Enum
...
client = AdobeExperienceManagerAEMAPIClient(o_auth_acg_credentials=creds)
...
```python
from adobeexperiencemanageraemapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AdobeExperienceManagerAEMAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AdobeExperienceManagerAEMAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `adobeexperiencemanageraemapi/configuration.py` and each `adobeexperiencemanageraemapi/http/auth/*.py` for the exact
