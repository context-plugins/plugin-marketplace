...
client = stub_client(stubs)
assert_raises SubscriptionClient::APIException do
...
def build_test_client
  SubscriptionClient::Client.new(
    environment: SubscriptionClient::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error SubscriptionClient::APIException
```
...
```ruby
expect { call }.to raise_error(SubscriptionClient::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < SubscriptionClient::HttpCallBack
...
client  = SubscriptionClient::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
