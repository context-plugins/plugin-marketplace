...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the GiteaapiClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write GiteaapiClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `GiteaapiClient` is the SDK's client class — **read the real name** from `giteaapi/giteaapi_client.py`
...
- `giteaapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `GiteaapiClient` — the generated client class (read it from the `class …Client` declaration in `giteaapi/giteaapi_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `GiteaapiClient` constructor accepts all configuration as **keyword arguments** — there is no
...
from giteaapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from giteaapi.configuration import Configuration, Environment
...
client = GiteaapiClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `giteaapi/giteaapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `GiteaapiClient` and `Configuration` that reads
...
`from_environment` in `giteaapi/configuration.py` for the exact names.
...
Open `giteaapi/giteaapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `giteaapi/configuration.py`. Use the constant for the desired
...
client = GiteaapiClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`giteaapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = GiteaapiClient(config=new_config)
```
