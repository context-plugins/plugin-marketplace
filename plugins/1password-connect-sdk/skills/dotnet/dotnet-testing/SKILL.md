...
`PasswordConnect1Client`/`PasswordConnect1ClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `PasswordConnect1Client`,
...
static PasswordConnect1Client ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new PasswordConnect1Client(new HttpClient(handler), new PasswordConnect1ClientOptions { /* auth not needed for stubs */ });
}
```
...
using PasswordConnect1.Errors;              // {Operation}Error types
...
```csharp
using PasswordConnect1.Core.Exceptions;
using PasswordConnect1.Core.ErrorResponse;
...
using PasswordConnect1.Core.ErrorResponse; // RawError (Case B)
...
var client = new PasswordConnect1Client(new HttpClient(handler), new PasswordConnect1ClientOptions());
...
- For DI-based code, the SDK's `AddPasswordConnect1Client` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `PasswordConnect1Client` from the provider:
  ```csharp
  services.AddPasswordConnect1Client(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<PasswordConnect1Client>();
  ```
