...
against the `TraderApiClient.Builder` setters, the `markets.alpaca.paperapi.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `TraderApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
TraderApiClient client = new TraderApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeTraderAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
