...
your SDK source (`greenbyte/models/`).
...
```python
from greenbyte.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `greenbyte/utilities/union_type_lookup.py`) and each candidate model
...
```python
from greenbyte.models.animal import Cat, Dog
...
```python
from greenbyte.models.animal import Animal, Cat, Dog
