...
> `{ApiGroup}`, `AdobeExperienceManagerAemApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `AdobeExperienceManagerAemApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `AdobeExperienceManagerAemApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `AdobeExperienceManagerAemApi.Errors`
...
```csharp
using AdobeExperienceManagerAemApi.Core.Exceptions;     // SdkException<TError>
using AdobeExperienceManagerAemApi.Core.ErrorResponse;  // ApiError, RawError
using AdobeExperienceManagerAemApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`AdobeExperienceManagerAemApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using AdobeExperienceManagerAemApi.Core.Exceptions;     // SdkException<TError>
using AdobeExperienceManagerAemApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `AdobeExperienceManagerAemApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `AdobeExperienceManagerAemApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using AdobeExperienceManagerAemApi.Core.Models;        // ApiResult<TResponse, TError>
using AdobeExperienceManagerAemApi.Core.ErrorResponse; // RawError
using AdobeExperienceManagerAemApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `AdobeExperienceManagerAemApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
