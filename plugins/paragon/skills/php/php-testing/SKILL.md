...
> `RoutingDirectorClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): RoutingDirectorClient
{
...
```php
use RoutingDirectorLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use RoutingDirectorLib\Exceptions\{OperationException};
use RoutingDirectorLib\Exceptions\ApiException;
...
app()->instance(RoutingDirectorClient::class, clientWithResponses([
