...
```go
square.WithOAuthCCGCredentials(
    square.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr square.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := square.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr square.OAuthCCGManager) models.OAuthToken {
...
```go
creds := square.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := square.NewClient(square.CreateConfiguration(square.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    square.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := square.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := square.NewClient(square.CreateConfiguration(square.WithOAuthROPCGCredentials(creds)))
...
```go
square.WithOAuthBearerTokenCredentials(square.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
square.WithBasicAuthCredentials(square.NewBasicAuthCredentials(username, password))
square.WithApiHeaderCredentials(square.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
square.WithApiKeyCredentials(square.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `SQUARE_...` variables — e.g.
`SQUARE_USERNAME`, `SQUARE_PASSWORD`, `SQUARE_TOKEN`, `SQUARE_API_KEY`,
`SQUARE_O_AUTH_CLIENT_ID`, `SQUARE_O_AUTH_CLIENT_SECRET`, `SQUARE_ACCESS_TOKEN`. Grep the
