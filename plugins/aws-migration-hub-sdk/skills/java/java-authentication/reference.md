...
against the `AWSMigrationHubClient.Builder` setters, the `com.amazonaws.useast1.mgh.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AWSMigrationHubClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AWSMigrationHubClient client = new AWSMigrationHubClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAWSMigrationHubEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new AWSMigrationHubClient.Builder()
