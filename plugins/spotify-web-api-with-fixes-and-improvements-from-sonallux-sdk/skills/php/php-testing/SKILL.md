...
> `SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient
{
...
```php
use SpotifyWebAPIWithFixesAndImprovementsFromSonalluxLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SpotifyWebAPIWithFixesAndImprovementsFromSonalluxLib\Exceptions\{OperationException};
use SpotifyWebAPIWithFixesAndImprovementsFromSonalluxLib\Exceptions\ApiException;
...
app()->instance(SpotifyWebAPIWithFixesAndImprovementsFromSonalluxClient::class, clientWithResponses([
