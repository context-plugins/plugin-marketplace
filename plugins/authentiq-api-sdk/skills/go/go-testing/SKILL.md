...
`testing` + `testify/require` for reference; substitute the real `authentiqapi` and names.
...
import (
    ...
    "github.com/context-plugins/authentiq-api-go"   // root package authentiqapi
)
...
func clientReturning(status int, body string) (authentiqapi.ClientInterface, *stubTransport) {
    ...
    client := authentiqapi.NewClient(
        authentiqapi.CreateConfiguration(
            authentiqapi.WithHttpConfiguration(
                authentiqapi.CreateHttpConfiguration(authentiqapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/authentiq-api-go/errors"
...
client := authentiqapi.NewClient(
    authentiqapi.CreateConfiguration(
        authentiqapi.WithHttpConfiguration(
            authentiqapi.CreateHttpConfiguration(authentiqapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
