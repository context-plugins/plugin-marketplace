...
`hotelsearchapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `hotelsearchapi/exceptions/` and operation docstrings.
...
```python
from hotelsearchapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `hotelsearchapi/exceptions/`.
...
```python
from hotelsearchapi.exceptions.api_exception import APIException
from hotelsearchapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`hotelsearchapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`hotelsearchapi/exceptions/` for the class.
...
`hotelsearchapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `hotelsearchapi/exceptions/`
  or a `ValueError` — check the auth handler in `hotelsearchapi/http/auth/` for the exact type.
