...
`alpaca/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `alpaca/exceptions/` and operation docstrings.
...
```python
from alpaca.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `alpaca/exceptions/`.
...
```python
from alpaca.exceptions.api_exception import APIException
from alpaca.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`alpaca/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`alpaca/exceptions/` for the class.
...
`alpaca/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `alpaca/exceptions/`
  or a `ValueError` — check the auth handler in `alpaca/http/auth/` for the exact type.
