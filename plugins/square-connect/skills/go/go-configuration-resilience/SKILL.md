    ...
    squareconnectapi.CreateConfiguration(
        squareconnectapi.WithHttpConfiguration(
            squareconnectapi.CreateHttpConfiguration(
                squareconnectapi.WithTimeout(30),
                squareconnectapi.WithRetryConfiguration(
                    squareconnectapi.CreateRetryConfiguration(
                        squareconnectapi.WithMaxRetryAttempts(3),
                        squareconnectapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := squareconnectapi.CreateRetryConfiguration(
    squareconnectapi.WithMaxRetryAttempts(5),
    squareconnectapi.WithRetryInterval(1),
    squareconnectapi.WithBackoffFactor(2),
    squareconnectapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    squareconnectapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := squareconnectapi.NewClient(
    squareconnectapi.CreateConfiguration(
        squareconnectapi.WithHttpConfiguration(
            squareconnectapi.CreateHttpConfiguration(
                squareconnectapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
