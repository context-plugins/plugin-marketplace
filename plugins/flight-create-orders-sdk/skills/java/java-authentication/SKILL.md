...
To see which schemes a specific SDK accepts, read the credential setter methods on `FlightCreateOrdersClient.Builder`
...
- Set credentials **on the `FlightCreateOrdersClient.Builder`** before calling `.build()` — the client is immutable
