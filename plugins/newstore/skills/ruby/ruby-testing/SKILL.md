...
client = stub_client(stubs)
assert_raises Newstore::APIException do
...
BASE = Newstore::Configuration.new(environment: Newstore::Environment::PRODUCTION).get_base_uri

def build_test_client
  Newstore::Client.new(
    environment: Newstore::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Newstore::APIException
```
...
```ruby
expect { call }.to raise_error(Newstore::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Newstore::HttpCallBack
...
client  = Newstore::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
