...
- `hotelratings` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/hotel-ratings-go` — the Go import path for the root package; `github.com/context-plugins/hotel-ratings-go/models` for request/response types.
...
```go
import "github.com/context-plugins/hotel-ratings-go"

client := hotelratings.NewClient(
    hotelratings.CreateConfiguration(
        hotelratings.WithEnvironment(hotelratings.PRODUCTION),
...
from environment variables named `HOTELRATINGS_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := hotelratings.NewClient(hotelratings.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `hotelratings.PRODUCTION`,
`hotelratings.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := hotelratings.NewClient(
    hotelratings.CreateConfiguration(hotelratings.WithEnvironment(hotelratings.PRODUCTION)),
)
```
...
client := hotelratings.NewClient(
    hotelratings.CreateConfiguration(
        hotelratings.WithHttpConfiguration(
            hotelratings.CreateHttpConfiguration(
                hotelratings.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                hotelratings.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = hotelratings.NewClient(hotelratings.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api hotelratings.ClientInterface
}

func NewService(api hotelratings.ClientInterface) *Service {
