...
> Throughout, `cellpoint` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := cellpoint.NewClient(
    cellpoint.CreateConfiguration(
        cellpoint.WithBasicAuthCredentials(
            cellpoint.NewBasicAuthCredentials(
...
cellpoint.WithApiKeyCredentials(cellpoint.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
cellpoint.WithApiHeaderCredentials(cellpoint.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
cellpoint.WithOAuthBearerTokenCredentials(
    cellpoint.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := cellpoint.NewClient(
    cellpoint.CreateConfiguration(
        cellpoint.WithOAuthCCGCredentials(
            cellpoint.NewOAuthCCGCredentials(
