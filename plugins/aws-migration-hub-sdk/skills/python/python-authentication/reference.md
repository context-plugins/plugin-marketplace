...
kwargs to the `AWSMigrationHubClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `awsmigrationhub/awsmigrationhub_client.py` and `awsmigrationhub/http/auth/` in
the cloned source.

## Basic auth

```python
from awsmigrationhub.http.auth.basic_auth import BasicAuthCredentials
...
```python
from awsmigrationhub.http.auth.api_key import ApiKeyCredentials
...
```python
from awsmigrationhub.http.auth.api_header import ApiHeaderCredentials
...
```python
from awsmigrationhub.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from awsmigrationhub.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from awsmigrationhub.http.auth.o_auth_acg import OAuthACGCredentials
from awsmigrationhub.models.o_auth_scope_awsmigrationhub_enum import OAuthScope{Pkg}Enum
...
client = AWSMigrationHubClient(o_auth_acg_credentials=creds)
...
```python
from awsmigrationhub.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AWSMigrationHubClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AWSMigrationHubClient.from_environment()`) read each credential
...
Grep `from_environment` in `awsmigrationhub/configuration.py` and each `awsmigrationhub/http/auth/*.py` for the exact
