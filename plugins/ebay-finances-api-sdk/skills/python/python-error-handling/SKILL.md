...
`ebayfinancesapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `ebayfinancesapi/exceptions/` and operation docstrings.
...
```python
from ebayfinancesapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `ebayfinancesapi/exceptions/`.
...
```python
from ebayfinancesapi.exceptions.api_exception import APIException
from ebayfinancesapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`ebayfinancesapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`ebayfinancesapi/exceptions/` for the class.
...
`ebayfinancesapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `ebayfinancesapi/exceptions/`
  or a `ValueError` — check the auth handler in `ebayfinancesapi/http/auth/` for the exact type.
