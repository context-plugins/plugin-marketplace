...
kwargs to the `AzureCognitiveClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `azurecognitive/azurecognitive_client.py` and `azurecognitive/http/auth/` in
the cloned source.

## Basic auth

```python
from azurecognitive.http.auth.basic_auth import BasicAuthCredentials
...
```python
from azurecognitive.http.auth.api_key import ApiKeyCredentials
...
```python
from azurecognitive.http.auth.api_header import ApiHeaderCredentials
...
```python
from azurecognitive.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from azurecognitive.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from azurecognitive.http.auth.o_auth_acg import OAuthACGCredentials
from azurecognitive.models.o_auth_scope_azurecognitive_enum import OAuthScope{Pkg}Enum
...
client = AzureCognitiveClient(o_auth_acg_credentials=creds)
...
```python
from azurecognitive.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AzureCognitiveClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AzureCognitiveClient.from_environment()`) read each credential
...
Grep `from_environment` in `azurecognitive/configuration.py` and each `azurecognitive/http/auth/*.py` for the exact
