...
```typescript
import { {Operation}Error } from 'the-plaid-apilib/errors';
...
```typescript
import { ApiError } from 'the-plaid-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(ThePlaidAPIClient)
    .useValue(new ThePlaidAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
