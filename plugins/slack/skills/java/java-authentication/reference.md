...
against the `SlackClient.Builder` setters, the `com.slack.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SlackClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
