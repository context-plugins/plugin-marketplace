...
Integrate and consume the **APIMatic-generated paragon SDK** (the "APIMATIC v3.0" generic
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, TypeScript, PHP.
...
(`dotnet-paragon-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-paragon`** router skill routes all
C#/.NET SDK work to the single **`dotnet-paragon-sdk`** agent (.NET-only), which answers
