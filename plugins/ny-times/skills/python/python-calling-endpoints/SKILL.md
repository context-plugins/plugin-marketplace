...
Open `nytimes/nytimes_client.py` for the controller property names, then the relevant
`nytimes/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from nytimes.models.{request_model} import {RequestModel}
...
Open `nytimes/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from nytimes.models.suite_code_enum import SuiteCodeEnum
...
```python
from nytimes.http.http_call_back import HttpCallBack
...
client = NytimesClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `nytimes/pagination/` in the source.

## Finding the right method

- Controller property names are on `NytimesClient` in `nytimes/nytimes_client.py`.
- Operation method signatures are in `nytimes/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `nytimes/models/`; enum types have `from_value()`.
