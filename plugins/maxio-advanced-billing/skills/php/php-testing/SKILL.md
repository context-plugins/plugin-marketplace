...
> `MaxioAdvancedBillingClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): MaxioAdvancedBillingClient
{
...
```php
use MaxioAdvancedBillingLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use MaxioAdvancedBillingLib\Exceptions\{OperationException};
use MaxioAdvancedBillingLib\Exceptions\ApiException;
...
app()->instance(MaxioAdvancedBillingClient::class, clientWithResponses([
