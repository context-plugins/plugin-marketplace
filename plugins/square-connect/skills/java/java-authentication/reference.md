...
against the `SquareConnectAPIClient.Builder` setters, the `com.squareup.connect.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `SquareConnectAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
SquareConnectAPIClient client = new SquareConnectAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeSquareConnectAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new SquareConnectAPIClient.Builder()
