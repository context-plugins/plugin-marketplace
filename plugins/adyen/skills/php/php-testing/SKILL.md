...
> `AdyenClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): AdyenClient
{
...
```php
use AdyenLib\Exceptions\{OperationException};
use AdyenLib\Exceptions\ApiException;
...
app()->instance(AdyenClient::class, clientWithResponses([
