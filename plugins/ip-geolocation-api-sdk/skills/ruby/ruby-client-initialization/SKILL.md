...
- `IPGeolocationAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `ip_geolocation_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'ip_geolocation_api'
include IPGeolocationAPI
...
The authoritative list is the `initialize` signature in `lib/ip_geolocation_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/ip_geolocation_api/configuration.rb`
(`build_default_config_from_env`) and `lib/ip_geolocation_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/ip_geolocation_api.rb`) and store it in a
