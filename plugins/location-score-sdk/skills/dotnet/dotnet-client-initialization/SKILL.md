...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddLocationScoreClient extension. Use the moment you write `new LocationScoreClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `LocationScoreClientOptions` — its options class.
- `LocationScore` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public LocationScoreClient(HttpClient httpClient, LocationScoreClientOptions options)
```
...
```csharp
public class LocationScoreClientOptions
{
...
```csharp
using LocationScore;
using LocationScore.Servers;

var options = new LocationScoreClientOptions
{
...
var client = new LocationScoreClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddLocationScoreClient`, which registers the
...
builder.Services.AddLocationScoreClient(options =>
{
...
```csharp
public sealed class MyService(LocationScoreClient client)
{
