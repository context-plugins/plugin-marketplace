...
Open `eventsapi/eventsapi_client.py` for the controller property names, then the relevant
`eventsapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from eventsapi.models.{request_model} import {RequestModel}
...
Open `eventsapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from eventsapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from eventsapi.http.http_call_back import HttpCallBack
...
client = EventsAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `eventsapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `EventsAPIClient` in `eventsapi/eventsapi_client.py`.
- Operation method signatures are in `eventsapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `eventsapi/models/`; enum types have `from_value()`.
