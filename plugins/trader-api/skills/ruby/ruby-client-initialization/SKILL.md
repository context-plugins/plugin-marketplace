...
- `TraderAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `trader_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/trader_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/trader_api/configuration.rb`
(`build_default_config_from_env`) and `lib/trader_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = TraderAPI::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/trader_api.rb`) and store it in a
