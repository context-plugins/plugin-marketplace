...
To see which schemes a specific SDK accepts, read the credential setter methods on `ArticleSearchAPIClient.Builder`
...
- Set credentials **on the `ArticleSearchAPIClient.Builder`** before calling `.build()` — the client is immutable
