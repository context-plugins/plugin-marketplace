...
```ruby
client = StaxFattMerchantAPI::Client.new(
  max_retries:    3,
...
```ruby
client = StaxFattMerchantAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = StaxFattMerchantAPI::Client.new(adapter: :net_http)
```
...
client = StaxFattMerchantAPI::Client.new(connection: conn)
```
...
client = StaxFattMerchantAPI::Client.new(environment: Environment::PRODUCTION)
client = StaxFattMerchantAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = StaxFattMerchantAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = StaxFattMerchantAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = StaxFattMerchantAPI::ProxySettings.from_env
client = StaxFattMerchantAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < StaxFattMerchantAPI::HttpCallBack
...
client = StaxFattMerchantAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = StaxFattMerchantAPI::Client.from_env
# Overrides take precedence over env vars:
client = StaxFattMerchantAPI::Client.from_env(timeout: 30, max_retries: 3)
```
