...
Open `shutterstock/shutterstock_client.py` for the controller property names, then the relevant
`shutterstock/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from shutterstock.models.{request_model} import {RequestModel}
...
Open `shutterstock/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from shutterstock.models.suite_code_enum import SuiteCodeEnum
...
```python
from shutterstock.http.http_call_back import HttpCallBack
...
an iterator over the items on that page. Inspect `shutterstock/pagination/` in the source.

## Finding the right method

- Controller property names are on `ShutterstockapiexplorerClient` in `shutterstock/shutterstock_client.py`.
- Operation method signatures are in `shutterstock/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `shutterstock/models/`; enum types have `from_value()`.
