...
> `ArticleSearchAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ArticleSearchAPIClient
{
...
```php
use ArticleSearchAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use ArticleSearchAPILib\Exceptions\{OperationException};
use ArticleSearchAPILib\Exceptions\ApiException;
...
app()->instance(ArticleSearchAPIClient::class, clientWithResponses([
