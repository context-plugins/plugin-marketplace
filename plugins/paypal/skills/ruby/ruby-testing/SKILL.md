...
client = stub_client(stubs)
assert_raises PayPalAPI::APIException do
...
def build_test_client
  PayPalAPI::Client.new(
    environment: PayPalAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error PayPalAPI::APIException
```
...
```ruby
expect { call }.to raise_error(PayPalAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < PayPalAPI::HttpCallBack
...
client  = PayPalAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
