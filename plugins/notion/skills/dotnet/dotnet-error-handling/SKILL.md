...
> `{ApiGroup}`, `Notion`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `Notion.Core.Exceptions`
- `ApiError` **and** `RawError` → `Notion.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `Notion.Errors`
...
```csharp
using Notion.Core.Exceptions;     // SdkException<TError>
using Notion.Core.ErrorResponse;  // ApiError, RawError
using Notion.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`Notion.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using Notion.Core.Exceptions;     // SdkException<TError>
using Notion.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `Notion.Core.ErrorResponse`. Its public
...
`readonly struct` under `Notion.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using Notion.Core.Models;        // ApiResult<TResponse, TError>
using Notion.Core.ErrorResponse; // RawError
using Notion.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `Notion.Core.Exceptions`) — an auth *application* failure, not an API error — when the
