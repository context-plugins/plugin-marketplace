    ...
    xapiv2.CreateConfiguration(
        xapiv2.WithHttpConfiguration(
            xapiv2.CreateHttpConfiguration(
                xapiv2.WithTimeout(30),
                xapiv2.WithRetryConfiguration(
                    xapiv2.CreateRetryConfiguration(
                        xapiv2.WithMaxRetryAttempts(3),
                        xapiv2.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := xapiv2.CreateRetryConfiguration(
    xapiv2.WithMaxRetryAttempts(5),
    xapiv2.WithRetryInterval(1),
    xapiv2.WithBackoffFactor(2),
    xapiv2.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    xapiv2.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := xapiv2.NewClient(
    xapiv2.CreateConfiguration(
        xapiv2.WithHttpConfiguration(
            xapiv2.CreateHttpConfiguration(
                xapiv2.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
