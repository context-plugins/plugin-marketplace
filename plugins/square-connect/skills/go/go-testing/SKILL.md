...
`testing` + `testify/require` for reference; substitute the real `squareconnectapi` and names.
...
import (
    ...
    "github.com/context-plugins/square-connect-api-go"   // root package squareconnectapi
)
...
func clientReturning(status int, body string) (squareconnectapi.ClientInterface, *stubTransport) {
    ...
    client := squareconnectapi.NewClient(
        squareconnectapi.CreateConfiguration(
            squareconnectapi.WithHttpConfiguration(
                squareconnectapi.CreateHttpConfiguration(squareconnectapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/square-connect-api-go/errors"
...
client := squareconnectapi.NewClient(
    squareconnectapi.CreateConfiguration(
        squareconnectapi.WithHttpConfiguration(
            squareconnectapi.CreateHttpConfiguration(squareconnectapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
