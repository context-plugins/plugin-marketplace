...
`staxfattmerchantapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `staxfattmerchantapi/exceptions/` and operation docstrings.
...
```python
from staxfattmerchantapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `staxfattmerchantapi/exceptions/`.
...
```python
from staxfattmerchantapi.exceptions.api_exception import APIException
from staxfattmerchantapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`staxfattmerchantapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`staxfattmerchantapi/exceptions/` for the class.
...
`staxfattmerchantapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `staxfattmerchantapi/exceptions/`
  or a `ValueError` — check the auth handler in `staxfattmerchantapi/http/auth/` for the exact type.
