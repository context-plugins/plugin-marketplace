...
> Throughout, `theplaidapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := theplaidapi.NewClient(
    theplaidapi.CreateConfiguration(
        theplaidapi.WithBasicAuthCredentials(
            theplaidapi.NewBasicAuthCredentials(
...
theplaidapi.WithApiKeyCredentials(theplaidapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
theplaidapi.WithApiHeaderCredentials(theplaidapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
theplaidapi.WithOAuthBearerTokenCredentials(
    theplaidapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := theplaidapi.NewClient(
    theplaidapi.CreateConfiguration(
        theplaidapi.WithOAuthCCGCredentials(
            theplaidapi.NewOAuthCCGCredentials(
