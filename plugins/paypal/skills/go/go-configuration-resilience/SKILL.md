    ...
    paypal.CreateConfiguration(
        paypal.WithHttpConfiguration(
            paypal.CreateHttpConfiguration(
                paypal.WithTimeout(30),
                paypal.WithRetryConfiguration(
                    paypal.CreateRetryConfiguration(
                        paypal.WithMaxRetryAttempts(3),
                        paypal.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := paypal.CreateRetryConfiguration(
    paypal.WithMaxRetryAttempts(5),
    paypal.WithRetryInterval(1),
    paypal.WithBackoffFactor(2),
    paypal.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    paypal.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := paypal.NewClient(
    paypal.CreateConfiguration(
        paypal.WithHttpConfiguration(
            paypal.CreateHttpConfiguration(
                paypal.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
