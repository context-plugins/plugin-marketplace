...
- `verizonapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/verizon-api-go` — the Go import path for the root package; `github.com/context-plugins/verizon-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/verizon-api-go"

client := verizonapi.NewClient(
    verizonapi.CreateConfiguration(
        verizonapi.WithEnvironment(verizonapi.PRODUCTION),
...
from environment variables named `VERIZONAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := verizonapi.NewClient(verizonapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `verizonapi.PRODUCTION`,
`verizonapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := verizonapi.NewClient(
    verizonapi.CreateConfiguration(verizonapi.WithEnvironment(verizonapi.PRODUCTION)),
)
```
...
client := verizonapi.NewClient(
    verizonapi.CreateConfiguration(
        verizonapi.WithHttpConfiguration(
            verizonapi.CreateHttpConfiguration(
                verizonapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                verizonapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = verizonapi.NewClient(verizonapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api verizonapi.ClientInterface
}

func NewService(api verizonapi.ClientInterface) *Service {
