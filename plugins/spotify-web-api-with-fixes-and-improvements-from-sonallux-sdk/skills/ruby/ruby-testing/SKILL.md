...
def stub_client(stubs)
  ...
  SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises SpotifyWebAPIWithFixesAndImprovementsFromSonallux::APIException do
...
def build_test_client
  SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(
    environment: SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error SpotifyWebAPIWithFixesAndImprovementsFromSonallux::APIException
```
...
```ruby
# Base APIException:
rescue SpotifyWebAPIWithFixesAndImprovementsFromSonallux::APIException => e
  assert_equal 404, e.response_code
  assert_includes e.reason, 'Not Found'

# Typed subclass (e.g. OAuthProviderException):
rescue SpotifyWebAPIWithFixesAndImprovementsFromSonallux::OAuthProviderException => e
...
```ruby
expect { call }.to raise_error(SpotifyWebAPIWithFixesAndImprovementsFromSonallux::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < SpotifyWebAPIWithFixesAndImprovementsFromSonallux::HttpCallBack
...
client  = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
