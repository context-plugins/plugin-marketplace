...
```go
automationmanagementclient.WithOAuthCCGCredentials(
    automationmanagementclient.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr automationmanagementclient.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := automationmanagementclient.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr automationmanagementclient.OAuthCCGManager) models.OAuthToken {
...
```go
creds := automationmanagementclient.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := automationmanagementclient.NewClient(automationmanagementclient.CreateConfiguration(automationmanagementclient.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    automationmanagementclient.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := automationmanagementclient.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := automationmanagementclient.NewClient(automationmanagementclient.CreateConfiguration(automationmanagementclient.WithOAuthROPCGCredentials(creds)))
...
```go
automationmanagementclient.WithOAuthBearerTokenCredentials(automationmanagementclient.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
automationmanagementclient.WithBasicAuthCredentials(automationmanagementclient.NewBasicAuthCredentials(username, password))
automationmanagementclient.WithApiHeaderCredentials(automationmanagementclient.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
automationmanagementclient.WithApiKeyCredentials(automationmanagementclient.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `AUTOMATIONMANAGEMENTCLIENT_...` variables — e.g.
`AUTOMATIONMANAGEMENTCLIENT_USERNAME`, `AUTOMATIONMANAGEMENTCLIENT_PASSWORD`, `AUTOMATIONMANAGEMENTCLIENT_TOKEN`, `AUTOMATIONMANAGEMENTCLIENT_API_KEY`,
`AUTOMATIONMANAGEMENTCLIENT_O_AUTH_CLIENT_ID`, `AUTOMATIONMANAGEMENTCLIENT_O_AUTH_CLIENT_SECRET`, `AUTOMATIONMANAGEMENTCLIENT_ACCESS_TOKEN`. Grep the
