...
client = stub_client(stubs)
assert_raises ArticleSearchAPI::APIException do
...
def build_test_client
  ArticleSearchAPI::Client.new(
    environment: ArticleSearchAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ArticleSearchAPI::APIException
```
...
```ruby
expect { call }.to raise_error(ArticleSearchAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ArticleSearchAPI::HttpCallBack
...
client  = ArticleSearchAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
