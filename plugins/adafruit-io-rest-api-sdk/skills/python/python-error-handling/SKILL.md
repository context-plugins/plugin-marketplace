...
`adafruitiorestapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `adafruitiorestapi/exceptions/` and operation docstrings.
...
```python
from adafruitiorestapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `adafruitiorestapi/exceptions/`.
...
```python
from adafruitiorestapi.exceptions.api_exception import APIException
from adafruitiorestapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`adafruitiorestapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`adafruitiorestapi/exceptions/` for the class.
...
`adafruitiorestapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `adafruitiorestapi/exceptions/`
  or a `ValueError` — check the auth handler in `adafruitiorestapi/http/auth/` for the exact type.
