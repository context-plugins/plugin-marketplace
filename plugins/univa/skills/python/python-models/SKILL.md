...
```python
from univa.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `univa/utilities/union_type_lookup.py`) and each candidate model
...
```python
from univa.models.animal import Cat, Dog
...
```python
from univa.models.animal import Animal, Cat, Dog
