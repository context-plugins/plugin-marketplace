...
reference; substitute your `maxioadvancedbilling` and `MaxioadvancedbillingClient` names.
...
- **Get the stub URL from the SDK, not from this page.** Every environment resolves to a real remote
  host — read the `environments` dict in `maxioadvancedbilling/configuration.py`, or call
