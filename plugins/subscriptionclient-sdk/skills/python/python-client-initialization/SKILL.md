...
- `subscriptionclient` — the root package name (e.g. `multiauthsample`, `batester`).
- `SubscriptionClientClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `SubscriptionClientClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from subscriptionclient.subscriptionclient_client import SubscriptionClientClient
from subscriptionclient.configuration import Environment
from subscriptionclient.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = SubscriptionClientClient(
...
```python
from subscriptionclient.configuration import Configuration, Environment
...
client = SubscriptionClientClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `subscriptionclient/subscriptionclient_client.py` in the cloned source.
...
`from_environment()` is a class method on both `SubscriptionClientClient` and `Configuration` that reads
...
client = SubscriptionClientClient.from_environment()
...
`from_environment` in `subscriptionclient/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `SubscriptionClientClient`. Access them as attributes —
...
Open `subscriptionclient/subscriptionclient_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `subscriptionclient/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from subscriptionclient.configuration import Environment

client = SubscriptionClientClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`subscriptionclient/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = SubscriptionClientClient(config=new_config)
```
