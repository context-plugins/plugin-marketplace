...
> `{ApiGroup}`, `SpotifyWebApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `SpotifyWebApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `SpotifyWebApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `SpotifyWebApi.Errors`
...
```csharp
using SpotifyWebApi.Core.Exceptions;     // SdkException<TError>
using SpotifyWebApi.Core.ErrorResponse;  // ApiError, RawError
using SpotifyWebApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`SpotifyWebApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using SpotifyWebApi.Core.Exceptions;     // SdkException<TError>
using SpotifyWebApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `SpotifyWebApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `SpotifyWebApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using SpotifyWebApi.Core.Models;        // ApiResult<TResponse, TError>
using SpotifyWebApi.Core.ErrorResponse; // RawError
using SpotifyWebApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `SpotifyWebApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
