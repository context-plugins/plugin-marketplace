...
```ruby
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(adapter: :net_http)
```
...
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(connection: conn)
```
...
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(environment: Environment::PRODUCTION)
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::ProxySettings.from_env
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < SpotifyWebAPIWithFixesAndImprovementsFromSonallux::HttpCallBack
...
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.from_env
# Overrides take precedence over env vars:
client = SpotifyWebAPIWithFixesAndImprovementsFromSonallux::Client.from_env(timeout: 30, max_retries: 3)
```
