...
Open `lib/flight_offers_price/client.rb` for the accessor names, then the relevant
`lib/flight_offers_price/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = FlightOffersPrice::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: FlightOffersPrice::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/flight_offers_price/models/`. Field names are defined as
...
Read the enum module in `lib/flight_offers_price/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/flight_offers_price/client.rb`.
- Operation methods are in `lib/flight_offers_price/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/flight_offers_price/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/flight_offers_price/exceptions/`.
