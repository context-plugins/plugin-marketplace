...
- `coingeckodemoapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `CoinGeckoDemoAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
...
```python
from coingeckodemoapi.coingeckodemoapi_client import CoinGeckoDemoAPIClient
from coingeckodemoapi.configuration import Environment
from coingeckodemoapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from coingeckodemoapi.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `coingeckodemoapi/coingeckodemoapi_client.py` in the cloned source.
...
`from_environment` in `coingeckodemoapi/configuration.py` for the exact names.
...
Open `coingeckodemoapi/coingeckodemoapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `coingeckodemoapi/configuration.py`. Use the constant for the desired
...
`coingeckodemoapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
