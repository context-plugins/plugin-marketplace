...
kwargs to the `APIsGuruClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `apisguru/apisguru_client.py` and `apisguru/http/auth/` in
the cloned source.

## Basic auth

```python
from apisguru.http.auth.basic_auth import BasicAuthCredentials
...
```python
from apisguru.http.auth.api_key import ApiKeyCredentials
...
```python
from apisguru.http.auth.api_header import ApiHeaderCredentials
...
```python
from apisguru.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from apisguru.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from apisguru.http.auth.o_auth_acg import OAuthACGCredentials
from apisguru.models.o_auth_scope_apisguru_enum import OAuthScope{Pkg}Enum
...
client = APIsGuruClient(o_auth_acg_credentials=creds)
...
```python
from apisguru.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = APIsGuruClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `APIsGuruClient.from_environment()`) read each credential
...
Grep `from_environment` in `apisguru/configuration.py` and each `apisguru/http/auth/*.py` for the exact
