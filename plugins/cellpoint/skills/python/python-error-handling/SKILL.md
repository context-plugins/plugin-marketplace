...
`cellpointapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `cellpointapi/exceptions/` and operation docstrings.
...
```python
from cellpointapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `cellpointapi/exceptions/`.
...
```python
from cellpointapi.exceptions.api_exception import APIException
from cellpointapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`cellpointapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`cellpointapi/exceptions/` for the class.
...
`cellpointapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `cellpointapi/exceptions/`
  or a `ValueError` — check the auth handler in `cellpointapi/http/auth/` for the exact type.
