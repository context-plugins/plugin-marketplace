...
description: Construct and configure an APIMatic-generated Java SDK client — build via the inner AdyenBalanceControlAPIClient.Builder() class (never a public constructor), set httpClientConfig via a lambda (not a config object), choose an Environment constant, access controllers via client.get{Resource}Controller(), call client.shutdown() at process exit, clone a live client with client.newBuilder(), and wire the client into Spring or plain-constructor DI. Use the moment you call new AdyenBalanceControlAPIClient.Builder(), pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the arguments but not the builder pattern, the httpClientConfig lambda, or the lifetime/reuse rules.
---
...
- `AdyenBalanceControlAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.adyen.paltest` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`AdyenBalanceControlAPIClient.Builder` class. A minimal initialization:

```java
import com.adyen.paltest.AdyenBalanceControlAPIClient;
import com.adyen.paltest.Environment;

AdyenBalanceControlAPIClient client = new AdyenBalanceControlAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `AdyenBalanceControlAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `AdyenBalanceControlAPIClient.java` |

Call `.build()` to produce an immutable `AdyenBalanceControlAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public AdyenBalanceControlAPIClient apiClient() {
        return new AdyenBalanceControlAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired AdyenBalanceControlAPIClient client) {
        client.shutdown();
    }
}
```

Inject `AdyenBalanceControlAPIClient` into your services as a normal Spring dependency. Build only one bean.
