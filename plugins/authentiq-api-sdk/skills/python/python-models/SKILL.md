...
your SDK source (`authentiqapi/models/`).
...
```python
from authentiqapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `authentiqapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from authentiqapi.models.animal import Cat, Dog
...
```python
from authentiqapi.models.animal import Animal, Cat, Dog
