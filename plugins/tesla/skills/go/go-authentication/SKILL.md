...
> Throughout, `tesla` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := tesla.NewClient(
    tesla.CreateConfiguration(
        tesla.WithBasicAuthCredentials(
            tesla.NewBasicAuthCredentials(
...
tesla.WithApiKeyCredentials(tesla.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
tesla.WithApiHeaderCredentials(tesla.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
tesla.WithOAuthBearerTokenCredentials(
    tesla.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
