...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the SplititClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write SplititClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `SplititClient` is the SDK's client class — **read the real name** from `splitit/splitit_client.py`
...
- `SplititClient` — the generated client class (read it from the `class …Client` declaration in `splitit/splitit_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `SplititClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from splitit.splitit_client import SplititClient
...
client = SplititClient(config=config)
```
...
`from_environment()` is a class method on both `SplititClient` and `Configuration` that reads
...
client = SplititClient.from_environment()
...
Controllers are `@LazyProperty` properties on `SplititClient`. Access them as attributes —
...
client = SplititClient(environment=Environment.PRODUCTION)
```
...
client = SplititClient(config=new_config)
```
