...
```ruby
client = Mindbody::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Mindbody::Client.new(adapter: :net_http)
```
...
client = Mindbody::Client.new(connection: conn)
```
...
client = Mindbody::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Mindbody::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Mindbody::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Mindbody::ProxySettings.from_env
client = Mindbody::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Mindbody::HttpCallBack
...
client = Mindbody::Client.new(http_callback: LogCallback.new)
```
...
client = Mindbody::Client.from_env(timeout: 30, max_retries: 3)
```
