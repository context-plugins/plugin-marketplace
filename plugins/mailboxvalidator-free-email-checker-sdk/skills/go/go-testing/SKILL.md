...
`testing` + `testify/require` for reference; substitute the real `mailboxvalidatorfreeemailchecker` and names.
...
import (
    ...
    "github.com/context-plugins/mailboxvalidator-free-email-checker-go"   // root package mailboxvalidatorfreeemailchecker
)
...
func clientReturning(status int, body string) (mailboxvalidatorfreeemailchecker.ClientInterface, *stubTransport) {
    ...
    client := mailboxvalidatorfreeemailchecker.NewClient(
        mailboxvalidatorfreeemailchecker.CreateConfiguration(
            mailboxvalidatorfreeemailchecker.WithHttpConfiguration(
                mailboxvalidatorfreeemailchecker.CreateHttpConfiguration(mailboxvalidatorfreeemailchecker.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/mailboxvalidator-free-email-checker-go/errors"
...
client := mailboxvalidatorfreeemailchecker.NewClient(
    mailboxvalidatorfreeemailchecker.CreateConfiguration(
        mailboxvalidatorfreeemailchecker.WithHttpConfiguration(
            mailboxvalidatorfreeemailchecker.CreateHttpConfiguration(mailboxvalidatorfreeemailchecker.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
