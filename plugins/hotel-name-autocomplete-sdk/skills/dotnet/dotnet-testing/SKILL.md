...
`HotelNameAutocompleteClient`/`HotelNameAutocompleteClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `HotelNameAutocompleteClient`,
...
static HotelNameAutocompleteClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new HotelNameAutocompleteClient(new HttpClient(handler), new HotelNameAutocompleteClientOptions { /* auth not needed for stubs */ });
}
```
...
using HotelNameAutocomplete.Errors;              // {Operation}Error types
...
```csharp
using HotelNameAutocomplete.Core.Exceptions;
using HotelNameAutocomplete.Core.ErrorResponse;
...
using HotelNameAutocomplete.Core.ErrorResponse; // RawError (Case B)
...
var client = new HotelNameAutocompleteClient(new HttpClient(handler), new HotelNameAutocompleteClientOptions());
...
- For DI-based code, the SDK's `AddHotelNameAutocompleteClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `HotelNameAutocompleteClient` from the provider:
  ```csharp
  services.AddHotelNameAutocompleteClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<HotelNameAutocompleteClient>();
  ```
