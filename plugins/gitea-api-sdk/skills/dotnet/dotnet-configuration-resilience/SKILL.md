...
All types below live under `GiteaApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using GiteaApi.Core.Configuration;
...
```csharp
using GiteaApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new GiteaApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `GiteaApi.Core.Exceptions`):
...
With DI, the SDK's `AddGiteaApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddGiteaApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
