...
- `nytimes` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/ny-times-go-sdk` — the Go import path for the root package; `github.com/context-plugins/ny-times-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/ny-times-go-sdk"

client := nytimes.NewClient(
    nytimes.CreateConfiguration(
        nytimes.WithEnvironment(nytimes.PRODUCTION),
...
from environment variables named `NYTIMES_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := nytimes.NewClient(nytimes.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `nytimes.PRODUCTION`,
`nytimes.ENVIRONMENT2`) — only the first is named meaningfully, further servers are positional
...
```go
client := nytimes.NewClient(
    nytimes.CreateConfiguration(nytimes.WithEnvironment(nytimes.PRODUCTION)),
)
```
...
client := nytimes.NewClient(
    nytimes.CreateConfiguration(
        nytimes.WithHttpConfiguration(
            nytimes.CreateHttpConfiguration(
                nytimes.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                nytimes.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = nytimes.NewClient(nytimes.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api nytimes.ClientInterface
}

func NewService(api nytimes.ClientInterface) *Service {
