...
```go
maxio.WithOAuthCCGCredentials(
    maxio.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr maxio.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := maxio.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr maxio.OAuthCCGManager) models.OAuthToken {
...
```go
creds := maxio.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := maxio.NewClient(maxio.CreateConfiguration(maxio.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    maxio.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := maxio.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := maxio.NewClient(maxio.CreateConfiguration(maxio.WithOAuthROPCGCredentials(creds)))
...
```go
maxio.WithOAuthBearerTokenCredentials(maxio.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
maxio.WithBasicAuthCredentials(maxio.NewBasicAuthCredentials(username, password))
maxio.WithApiHeaderCredentials(maxio.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
maxio.WithApiKeyCredentials(maxio.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `MAXIO_...` variables — e.g.
`MAXIO_USERNAME`, `MAXIO_PASSWORD`, `MAXIO_TOKEN`, `MAXIO_API_KEY`,
`MAXIO_O_AUTH_CLIENT_ID`, `MAXIO_O_AUTH_CLIENT_SECRET`, `MAXIO_ACCESS_TOKEN`. Grep the
