...
your SDK source (`nytimes/models/`).
...
```python
from nytimes.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `nytimes/utilities/union_type_lookup.py`) and each candidate model
...
```python
from nytimes.models.animal import Cat, Dog
...
```python
from nytimes.models.animal import Animal, Cat, Dog
