...
`3commasio/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `3commasio/exceptions/` and operation docstrings.
...
```python
from 3commasio.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `3commasio/exceptions/`.
...
```python
from 3commasio.exceptions.api_exception import APIException
from 3commasio.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`3commasio/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`3commasio/exceptions/` for the class.
...
`3commasio/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `3commasio/exceptions/`
  or a `ValueError` — check the auth handler in `3commasio/http/auth/` for the exact type.
