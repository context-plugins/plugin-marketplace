...
> Throughout, `gitea` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := gitea.NewClient(
    gitea.CreateConfiguration(
        gitea.WithBasicAuthCredentials(
            gitea.NewBasicAuthCredentials(
...
gitea.WithApiKeyCredentials(gitea.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
gitea.WithApiHeaderCredentials(gitea.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
gitea.WithOAuthBearerTokenCredentials(
    gitea.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
