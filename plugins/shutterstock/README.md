# Shutterstock SDK Plugin

A plugin whose skills teach a coding agent to install and use the APIMatic-generated **Shutterstock SDK**, in C#/.NET, TypeScript, Python. Every SDK fact the skills state is grounded in the SDK's own source and generated documentation, not in what a model remembers about this API.
...
Once your team publishes it to a plugin marketplace, install it from there by name instead — `/plugin install shutterstock@<marketplace>` — and drop the local `marketplace add` step.
