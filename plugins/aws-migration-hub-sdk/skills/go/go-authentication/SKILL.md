...
> Throughout, `awsmigrationhub` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := awsmigrationhub.NewClient(
    awsmigrationhub.CreateConfiguration(
        awsmigrationhub.WithBasicAuthCredentials(
            awsmigrationhub.NewBasicAuthCredentials(
...
awsmigrationhub.WithApiKeyCredentials(awsmigrationhub.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
awsmigrationhub.WithApiHeaderCredentials(awsmigrationhub.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
awsmigrationhub.WithOAuthBearerTokenCredentials(
    awsmigrationhub.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := awsmigrationhub.NewClient(
    awsmigrationhub.CreateConfiguration(
        awsmigrationhub.WithOAuthCCGCredentials(
            awsmigrationhub.NewOAuthCCGCredentials(
