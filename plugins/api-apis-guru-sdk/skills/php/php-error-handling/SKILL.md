...
> `{operation}`, `{apiGroup}`, `ToursAndActivitiesLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `ToursAndActivitiesLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use ToursAndActivitiesLib\Exceptions\ApiException;
...
```php
use ToursAndActivitiesLib\Exceptions\ApiException;
use ToursAndActivitiesLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `ToursAndActivitiesLib\Exceptions\` — read the source to
