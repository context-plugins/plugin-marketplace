...
kwargs to the `SqlManagementClientClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `sqlmanagementclient/sqlmanagementclient_client.py` and `sqlmanagementclient/http/auth/` in
the cloned source.

## Basic auth

```python
from sqlmanagementclient.http.auth.basic_auth import BasicAuthCredentials
...
```python
from sqlmanagementclient.http.auth.api_key import ApiKeyCredentials
...
```python
from sqlmanagementclient.http.auth.api_header import ApiHeaderCredentials
...
```python
from sqlmanagementclient.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from sqlmanagementclient.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from sqlmanagementclient.http.auth.o_auth_acg import OAuthACGCredentials
from sqlmanagementclient.models.o_auth_scope_sqlmanagementclient_enum import OAuthScope{Pkg}Enum
...
client = SqlManagementClientClient(o_auth_acg_credentials=creds)
...
```python
from sqlmanagementclient.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = SqlManagementClientClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SqlManagementClientClient.from_environment()`) read each credential
...
Grep `from_environment` in `sqlmanagementclient/configuration.py` and each `sqlmanagementclient/http/auth/*.py` for the exact
