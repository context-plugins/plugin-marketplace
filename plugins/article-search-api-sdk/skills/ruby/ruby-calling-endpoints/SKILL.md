...
Open `lib/article_search_api/client.rb` for the accessor names, then the relevant
`lib/article_search_api/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = ArticleSearchAPI::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: ArticleSearchAPI::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/article_search_api/models/`. Field names are defined as
...
Read the enum module in `lib/article_search_api/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/article_search_api/client.rb`.
- Operation methods are in `lib/article_search_api/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/article_search_api/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/article_search_api/exceptions/`.
