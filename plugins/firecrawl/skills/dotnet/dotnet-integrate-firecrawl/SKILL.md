...
description: "MANDATORY FIRST STEP for Firecrawl .NET SDK work in a C#/.NET project — load this BEFORE opening the SDK map or touching any project file; .NET/C# SDK ONLY, never load it for any other language. Applies when asked to integrate Firecrawl in C# — Account, Agent, Billing, Crawling, Developer, Extraction, Feedback, Interact, Mapping, Miscellaneous, Monitoring, ResearchApi, Scraping, Search, Support, ThreatProtection — or when a Firecrawl .NET SDK call errors or behaves unexpectedly. It carries four binding rules stated NOWHERE else — (1) for implementation work, the plan file firecrawl-plan.md is written at the project repo root BEFORE any project file is created or edited; (2) dotnet-getting-started and the SDK map it carries are loaded and used for every contract fact, never memory; (3) every dotnet-* skill the contract sheet's REQUIRED READING names is loaded before implementation starts; (4) every sheet row carries one of three labels — a map page, UNVERIFIED, or YOUR CALL — not in the map. Skip it and SDK facts get written from stale memory."
---

# Firecrawl .NET SDK — Workflow (map + skills)
...
**If the user opens with a reported SDK error or unexpected Firecrawl behaviour** (not new feature work), go straight to *Step 4 — Fixing SDK errors* below — do not run the plan-first flow for a bug report. Otherwise, for implementation work:

### Step 1 — Plan first: write `firecrawl-plan.md` (always, for any implementation work)

Your FIRST deliverable is `firecrawl-plan.md` at `<project repo root>/firecrawl-plan.md` — that exact path, always; never pick another location. It carries the plan and the contract sheet in the format below, and it is the only project-repo file you write before the HARD GATE further down.
...
**No project-file creation or edits until `firecrawl-plan.md` exists at that path with every section below filled in.** Starting to code before the sheet exists defeats the plan-first design: the sheet is what keeps SDK facts out of your memory and in the map. The gate bars coding before the sheet exists; it does not bar the read-only prerequisite work above.

#### The `firecrawl-plan.md` format (keep it tight — tables, not prose)
...
- Blocker or major assumption → surface it to the user in plain language, get their answer, then revise `firecrawl-plan.md` in place with targeted edits — edit the changed rows, append the new section. Rewriting the whole file to change a few rows is a defect.
...
4. **Any compile or runtime error involving an SDK type or member** (`CS1061`, `CS0117`, `CS0234`, `CS0104`, `CS1503`, `CS7036`, … on `Firecrawl.*`, or a provider error at runtime) → *Step 4* below. Do not attempt more than one self-fix of an SDK-name error before switching to that procedure — rewriting from the same knowledge that produced the error is guessing.
...
A compile or runtime error involving an SDK type or member (`CS1061`, `CS0117`, `CS0234`, `CS0104`, `CS1503`, `CS7036`, … on `Firecrawl.*`, or a provider error). Resolve it map-first — never patch it by guessing. Load `dotnet-getting-started` first if you have not already — the map is where every fix below comes from.
...
If a fix corrects a row in `firecrawl-plan.md`, correct the row in the file too — the sheet stays the record of what was verified.

### Step 5 — Pure questions

A standalone Firecrawl question with no code change: load `dotnet-getting-started`, look it up in the map (open the one source file the map names only on a real gap), and answer with the map page (or source file) it came from. No plan file — just the grounded answers; when several questions arrive batched, answer them all in one pass. Never answer from memory, even for "easy" questions.

## Anti-patterns — never do these

- **Never write a Firecrawl/SDK fact from memory** — every signature, field name, enum value, and error type in your code must come from the contract sheet or a map lookup. And **never write a call from memory "to fix later".**
...
- **Do not web-search Firecrawl for an implementation detail** — the map and the pinned source are the ground truth for THIS SDK version, and `Firecrawl` is what the generated source declares, not what public docs describe.
