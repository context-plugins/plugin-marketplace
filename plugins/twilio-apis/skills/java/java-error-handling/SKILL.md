...
│   └── com.twilio.accounts.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.twilio.accounts.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.twilio.accounts.http.client.HttpContext`, which carries:
