...
- `trip-purpose-predictionlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'trip-purpose-predictionlib';
...
```ts
import { {Resource}Controller } from 'trip-purpose-predictionlib';
