...
description: APIMatic-generated Python SDK (APIMATIC v3.0, built on apimatic-core + apimatic-requests-client-adapter) reference and grounding layer — the PyPI package name from pyproject.toml, the single OpenAIAPIClient class in openaiapi_client.py constructed with keyword args (credentials objects + environment= + http/config kwargs), the Configuration class with Environment/Server enums, controllers accessed via @LazyProperty on the client, the APIException base, the HttpCallBack test seam, the from_environment() class method, and how to navigate the generated source. Entry point into the companion python-* skills (client-initialization, authentication, calling-endpoints, models, error-handling, configuration-resilience, testing) — gates loading each at its integration step, since the source shows signatures but not the usage gotchas these skills carry.
---
...
| Install | `pip install "git+https://github.com/context-plugins/openai-api-python@main"` |
| Client class | `OpenAIAPIClient` in `openaiapi/openaiapi_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `openaiapi/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `OpenAIAPIClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `openaiapi/exceptions/api_exception.py` — raised on non-2xx |
...
- `openaiapi/openaiapi_client.py` — the `OpenAIAPIClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `openaiapi/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `openaiapi/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `openaiapi/controllers/` — one `{Resource}Controller` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `openaiapi/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `openaiapi/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `openaiapi/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `openaiapi/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `openaiapi/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/openai-api-python@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/openai-api-python /tmp/openai-api-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/openai-api-python "$env:TEMP\openai-api-python-src"
```
...
- `openaiapi/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `openaiapi/openaiapi_client.py` — `OpenAIAPIClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `openaiapi/controllers/*.py` — operation method signatures, parameter names, and return types.
- `openaiapi/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `openaiapi/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/openai-api-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\openai-api-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `OpenAIAPIClient(...)`.
