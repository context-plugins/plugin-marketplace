...
> `{ApiGroup}`, `VimeoApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `VimeoApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `VimeoApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `VimeoApi.Errors`
...
```csharp
using VimeoApi.Core.Exceptions;     // SdkException<TError>
using VimeoApi.Core.ErrorResponse;  // ApiError, RawError
using VimeoApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`VimeoApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using VimeoApi.Core.Exceptions;     // SdkException<TError>
using VimeoApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `VimeoApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `VimeoApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using VimeoApi.Core.Models;        // ApiResult<TResponse, TError>
using VimeoApi.Core.ErrorResponse; // RawError
using VimeoApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `VimeoApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
