...
`MaxioAdvancedBillingClient`/`MaxioAdvancedBillingClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `MaxioAdvancedBillingClient`,
...
static MaxioAdvancedBillingClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new MaxioAdvancedBillingClient(new HttpClient(handler), new MaxioAdvancedBillingClientOptions { /* auth not needed for stubs */ });
}
```
...
using MaxioAdvancedBilling.Errors;              // {Operation}Error types
...
```csharp
using MaxioAdvancedBilling.Core.Models;        // ApiResult<TResponse, TError>
using MaxioAdvancedBilling.Core.ErrorResponse; // RawError (Case B)
using MaxioAdvancedBilling.Errors;             // {Operation}Error (Case A only)
...
- For DI-based code, the SDK's `AddMaxioAdvancedBillingClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `MaxioAdvancedBillingClient` from the provider:
  ```csharp
  ...
  var client = services.BuildServiceProvider().GetRequiredService<MaxioAdvancedBillingClient>();
  ```
