...
description: "Alpaca Python SDK identity and lookup layer (Python only) — install, import root, base URL/environments, the auth pattern, the SDK map that ships at the SDK root (`sdk-map.md` + `map/operations/`) and how to traverse it, and the module table naming the one file owning each fact the map leaves to the source. Load this before answering any Alpaca Python SDK contract question or writing any SDK code."
---

# Getting started with the Alpaca Python SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Alpaca Python SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source modules it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `python-integrate-alpaca` for the workflow that wraps this skill.
...
Verified against `alpaca/` and `pyproject.toml` of the generated package at version `2.0.0`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Alpaca |
| Distribution name (what you install) | `alpaca` — **not on any package index**; installed from source (see *Install*) |
| Import root (what you import) | `alpaca` — note the underscores; the two names differ |
| Source repository | https://github.com/context-plugins/alpaca-python-sdk |
| Source branch | `main` |
| Version | `2.0.0` |
| Sync client class | `AlpacaClient` (alias `Client`) |
| Async client class | `AsyncAlpacaClient` (alias `AsyncClient`) |
...
| Base-URL config | `ServerConfig` (`alpaca/server/server_config.py`), frozen, `extra="forbid"` |
...
| Surface | 29 operations across 9 controllers · 18 models · 14 enums · 4 per-operation error unions |
...
This SDK is not published to a package index, so there is no `pip install` from PyPI for it. Install it from its repository — <https://github.com/context-plugins/alpaca-python-sdk> — into the same environment your project runs in:

```bash
pip install "alpaca @ git+https://github.com/context-plugins/alpaca-python-sdk.git@main"
```
...
Python does not re-export child modules transitively, so `from alpaca import models` alone does **not** make enums, error unions, or runtime types reachable. Import each kind of type from the module that owns it.

`alpaca/__init__.py` exports exactly 6 names:
...
| --- | --- |
| Domain models, their `…Dict` companions | `alpaca.models` |
| Enums (and their open `…OrStr` aliases) | `alpaca.models.enums` |
| `ApiError` · `RawError` · `ApiResult` · `RequestOptions` · `HttpClient` · `SdkBaseModel` · `UNSET` · `Optional` | `alpaca.core` |
| Per-operation error *unions* | `alpaca.errors` (`DeleteAllOpenPositionsErrorBody`, …) |

`alpaca.core` re-exports its whole public surface (a curated `__all__`), so import from `…core` rather than from the private modules beneath it (`…core.results`, `…core.exceptions`, `…core.auth.schemes`).
...
| --- | --- | --- |
| `"production"` *(default)* | `https://paper-api.alpaca.markets` | Paper |
| `"environment2"` | `https://api.alpaca.markets` | Live |
...
The SDK ships a generated map at its **root** — the directory holding `pyproject.toml`, the `alpaca/` source directory, and these two entries:
...
Every `Source` path on the map is relative to that SDK root, so `alpaca/models/account.py` opens as written from there — and the same path resolves inside the installed package, which is where you read a module's body once the map has named it.
...
```bash
python -c "import alpaca, pathlib; print(pathlib.Path(alpaca.__file__).parent)"
```

Failing that, it is under the project's environment (`.venv/Lib/site-packages/alpaca` on Windows, `.venv/lib/python3.*/site-packages/alpaca` elsewhere). **If the package is not installed, there is no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory. Paths below are relative to that package root:
