...
description: Construct and configure an APIMatic-generated Java SDK client — build via the inner LocationScoreClient.Builder() class (never a public constructor), set httpClientConfig via a lambda (not a config object), choose an Environment constant, access controllers via client.get{Resource}Controller(), call client.shutdown() at process exit, clone a live client with client.newBuilder(), and wire the client into Spring or plain-constructor DI. Use the moment you call new LocationScoreClient.Builder(), pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the arguments but not the builder pattern, the httpClientConfig lambda, or the lifetime/reuse rules.
---
...
- `LocationScoreClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`LocationScoreClient.Builder` class. A minimal initialization:

```java
import com.amadeus.api.test.LocationScoreClient;
import com.amadeus.api.test.Environment;

LocationScoreClient client = new LocationScoreClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `LocationScoreClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `LocationScoreClient.java` |

Call `.build()` to produce an immutable `LocationScoreClient` instance.
...
public class ApiConfig {
    @Bean
    public LocationScoreClient apiClient() {
        return new LocationScoreClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired LocationScoreClient client) {
        client.shutdown();
    }
}
```

Inject `LocationScoreClient` into your services as a normal Spring dependency. Build only one bean.
