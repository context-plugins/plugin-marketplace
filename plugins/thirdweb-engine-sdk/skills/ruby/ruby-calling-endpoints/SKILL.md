...
Open `lib/thirdweb_engine/client.rb` for the accessor names, then the relevant
`lib/thirdweb_engine/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = ThirdwebEngine::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: ThirdwebEngine::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/thirdweb_engine/models/`. Field names are defined as
...
Read the enum module in `lib/thirdweb_engine/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/thirdweb_engine/client.rb`.
- Operation methods are in `lib/thirdweb_engine/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/thirdweb_engine/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/thirdweb_engine/exceptions/`.
