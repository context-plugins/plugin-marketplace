...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddFinnhubApiClient extension. Use the moment you write `new FinnhubApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `FinnhubApiClient` — the single public client class (the generator reads this prefix from the SDK's
  `.csproj` name, so it is already the real one).
- `FinnhubApiClientOptions` — its options class.
- `FinnhubApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public FinnhubApiClient(HttpClient httpClient, FinnhubApiClientOptions options)
```
...
```csharp
public class FinnhubApiClientOptions
{
...
var client = new FinnhubApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddFinnhubApiClient`, which registers the
...
builder.Services.AddFinnhubApiClient(options =>
{
...
```csharp
public sealed class MyService(FinnhubApiClient client)
{
