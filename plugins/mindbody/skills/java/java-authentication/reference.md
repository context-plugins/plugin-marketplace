...
against the `MindbodyPushApiApiClient.Builder` setters, the `com.mindbodyonline.pushapi.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `MindbodyPushApiApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
new MindbodyPushApiApiClient.Builder()
