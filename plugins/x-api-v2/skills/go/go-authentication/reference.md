...
```go
xapiv2.WithOAuthCCGCredentials(
    xapiv2.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr xapiv2.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := xapiv2.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr xapiv2.OAuthCCGManager) models.OAuthToken {
...
```go
creds := xapiv2.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := xapiv2.NewClient(xapiv2.CreateConfiguration(xapiv2.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    xapiv2.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := xapiv2.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := xapiv2.NewClient(xapiv2.CreateConfiguration(xapiv2.WithOAuthROPCGCredentials(creds)))
...
```go
xapiv2.WithOAuthBearerTokenCredentials(xapiv2.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
xapiv2.WithBasicAuthCredentials(xapiv2.NewBasicAuthCredentials(username, password))
xapiv2.WithApiHeaderCredentials(xapiv2.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
xapiv2.WithApiKeyCredentials(xapiv2.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `XAPIV2_...` variables — e.g.
`XAPIV2_USERNAME`, `XAPIV2_PASSWORD`, `XAPIV2_TOKEN`, `XAPIV2_API_KEY`,
`XAPIV2_O_AUTH_CLIENT_ID`, `XAPIV2_O_AUTH_CLIENT_SECRET`, `XAPIV2_ACCESS_TOKEN`. Grep the
