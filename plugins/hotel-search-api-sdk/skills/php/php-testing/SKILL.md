...
> `HotelSearchAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): HotelSearchAPIClient
{
...
```php
use HotelSearchAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use HotelSearchAPILib\Exceptions\{OperationException};
use HotelSearchAPILib\Exceptions\ApiException;
...
app()->instance(HotelSearchAPIClient::class, clientWithResponses([
