...
```typescript
import { {Operation}Error } from 'nytimeslib/errors';
...
```typescript
import { ApiError } from 'nytimeslib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(NytimesClient)
    .useValue(new NytimesClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
