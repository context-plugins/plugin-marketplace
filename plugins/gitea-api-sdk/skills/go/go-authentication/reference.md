...
```go
giteaapi.WithOAuthCCGCredentials(
    giteaapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr giteaapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := giteaapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr giteaapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := giteaapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := giteaapi.NewClient(giteaapi.CreateConfiguration(giteaapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    giteaapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := giteaapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := giteaapi.NewClient(giteaapi.CreateConfiguration(giteaapi.WithOAuthROPCGCredentials(creds)))
...
```go
giteaapi.WithOAuthBearerTokenCredentials(giteaapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
giteaapi.WithBasicAuthCredentials(giteaapi.NewBasicAuthCredentials(username, password))
giteaapi.WithApiHeaderCredentials(giteaapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
giteaapi.WithApiKeyCredentials(giteaapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `GITEAAPI_...` variables — e.g.
`GITEAAPI_USERNAME`, `GITEAAPI_PASSWORD`, `GITEAAPI_TOKEN`, `GITEAAPI_API_KEY`,
`GITEAAPI_O_AUTH_CLIENT_ID`, `GITEAAPI_O_AUTH_CLIENT_SECRET`, `GITEAAPI_ACCESS_TOKEN`. Grep the
