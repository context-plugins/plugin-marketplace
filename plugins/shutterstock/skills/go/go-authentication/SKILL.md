...
> Throughout, `shutterstock` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := shutterstock.NewClient(
    shutterstock.CreateConfiguration(
        shutterstock.WithBasicAuthCredentials(
            shutterstock.NewBasicAuthCredentials(
...
shutterstock.WithApiKeyCredentials(shutterstock.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
shutterstock.WithApiHeaderCredentials(shutterstock.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
shutterstock.WithOAuthBearerTokenCredentials(
    shutterstock.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := shutterstock.NewClient(
    shutterstock.CreateConfiguration(
        shutterstock.WithOAuthCCGCredentials(
            shutterstock.NewOAuthCCGCredentials(
