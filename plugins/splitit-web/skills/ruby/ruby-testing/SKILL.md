...
client = stub_client(stubs)
assert_raises SplititWebApiV3::APIException do
...
def build_test_client
  SplititWebApiV3::Client.new(
    environment: SplititWebApiV3::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error SplititWebApiV3::APIException
```
...
```ruby
expect { call }.to raise_error(SplititWebApiV3::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < SplititWebApiV3::HttpCallBack
...
client  = SplititWebApiV3::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
