...
client = stub_client(stubs)
assert_raises ThePlaidAPI::APIException do
...
def build_test_client
  ThePlaidAPI::Client.new(
    environment: ThePlaidAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ThePlaidAPI::APIException
```
...
```ruby
expect { call }.to raise_error(ThePlaidAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ThePlaidAPI::HttpCallBack
...
client  = ThePlaidAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
