...
kwargs to the `ZalandoShopClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `zalandoshop/zalandoshop_client.py` and `zalandoshop/http/auth/` in
the cloned source.

## Basic auth

```python
from zalandoshop.http.auth.basic_auth import BasicAuthCredentials
...
```python
from zalandoshop.http.auth.api_key import ApiKeyCredentials
...
```python
from zalandoshop.http.auth.api_header import ApiHeaderCredentials
...
```python
from zalandoshop.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from zalandoshop.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from zalandoshop.http.auth.o_auth_acg import OAuthACGCredentials
from zalandoshop.models.o_auth_scope_zalandoshop_enum import OAuthScope{Pkg}Enum
...
client = ZalandoShopClient(o_auth_acg_credentials=creds)
...
```python
from zalandoshop.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ZalandoShopClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ZalandoShopClient.from_environment()`) read each credential
...
Grep `from_environment` in `zalandoshop/configuration.py` and each `zalandoshop/http/auth/*.py` for the exact
