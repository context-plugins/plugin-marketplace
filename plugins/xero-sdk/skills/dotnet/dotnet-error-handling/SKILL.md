...
> `{ApiGroup}`, `Xero`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `Xero.Core.Exceptions`
- `ApiError` **and** `RawError` → `Xero.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `Xero.Errors`
...
```csharp
using Xero.Core.Exceptions;     // SdkException<TError>
using Xero.Core.ErrorResponse;  // ApiError, RawError
using Xero.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`Xero.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using Xero.Core.Exceptions;     // SdkException<TError>
using Xero.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `Xero.Core.ErrorResponse`. Its public
...
`readonly struct` under `Xero.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using Xero.Core.Models;        // ApiResult<TResponse, TError>
using Xero.Core.ErrorResponse; // RawError
using Xero.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `Xero.Core.Exceptions`) — an auth *application* failure, not an API error — when the
