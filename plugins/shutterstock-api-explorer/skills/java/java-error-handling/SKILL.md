...
│   └── com.shutterstock.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.shutterstock.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.shutterstock.api.http.client.HttpContext`, which carries:
