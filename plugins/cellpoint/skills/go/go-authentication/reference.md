...
```go
cellpointapi.WithOAuthCCGCredentials(
    cellpointapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr cellpointapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := cellpointapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr cellpointapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := cellpointapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := cellpointapi.NewClient(cellpointapi.CreateConfiguration(cellpointapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    cellpointapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := cellpointapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := cellpointapi.NewClient(cellpointapi.CreateConfiguration(cellpointapi.WithOAuthROPCGCredentials(creds)))
...
```go
cellpointapi.WithOAuthBearerTokenCredentials(cellpointapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
cellpointapi.WithBasicAuthCredentials(cellpointapi.NewBasicAuthCredentials(username, password))
cellpointapi.WithApiHeaderCredentials(cellpointapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
cellpointapi.WithApiKeyCredentials(cellpointapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `CELLPOINTAPI_...` variables — e.g.
`CELLPOINTAPI_USERNAME`, `CELLPOINTAPI_PASSWORD`, `CELLPOINTAPI_TOKEN`, `CELLPOINTAPI_API_KEY`,
`CELLPOINTAPI_O_AUTH_CLIENT_ID`, `CELLPOINTAPI_O_AUTH_CLIENT_SECRET`, `CELLPOINTAPI_ACCESS_TOKEN`. Grep the
