...
> Throughout, `verizon` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := verizon.NewClient(
    verizon.CreateConfiguration(
        verizon.WithBasicAuthCredentials(
            verizon.NewBasicAuthCredentials(
...
verizon.WithApiKeyCredentials(verizon.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
verizon.WithApiHeaderCredentials(verizon.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
verizon.WithOAuthBearerTokenCredentials(
    verizon.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := verizon.NewClient(
    verizon.CreateConfiguration(
        verizon.WithOAuthCCGCredentials(
            verizon.NewOAuthCCGCredentials(
