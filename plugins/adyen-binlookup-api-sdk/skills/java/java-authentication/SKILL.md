...
To see which schemes a specific SDK accepts, read the credential setter methods on `AdyenBinLookupAPIClient.Builder`
...
- Set credentials **on the `AdyenBinLookupAPIClient.Builder`** before calling `.build()` — the client is immutable
