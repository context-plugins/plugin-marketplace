...
| Install | `gem 'subscriptionclient', git: 'https://github.com/context-plugins/subscriptionclient-ruby', branch: 'main'` in a Gemfile |
| Client class | `SubscriptionClient::Client` — one class; built with `Client.new(keyword_args...)` or `Client.from_env` |
| Configuration class | `SubscriptionClient::Configuration` — extends `CoreLibrary::HttpClientConfiguration`; holds `Environment` and `Server` inner classes |
...
| Base exception | `SubscriptionClient::APIException` (extends `CoreLibrary::ApiException`) |
...
APIMatic Ruby SDKs place everything under `lib/subscriptionclient/`:

- `lib/subscriptionclient.rb` — top-level require that loads `apimatic_core`, `apimatic_faraday_client_adapter`, then all internal files.
- `lib/subscriptionclient/client.rb` — the `Client` class: `initialize` with all keyword args (http options + credentials), controller accessor methods (e.g. `def simple_calculator`), OAuth manager accessors (e.g. `def o_auth_ccg`), and `self.from_env`.
- `lib/subscriptionclient/configuration.rb` — `Configuration` class (extends `CoreLibrary::HttpClientConfiguration`) with `Environment` and `Server` inner constant classes, the `ENVIRONMENTS` hash mapping environment+server to base URIs, `get_base_uri`, and `clone_with`.
- `lib/subscriptionclient/controllers/` — one `{Resource}Controller` per API resource group, each extending `BaseController`. **This is where operation method signatures and required vs. optional params live.**
- `lib/subscriptionclient/models/` — model classes (extend `BaseModel`) and enum modules with frozen string constants.
- `lib/subscriptionclient/exceptions/` — `api_exception.rb` (`APIException`) plus any typed error subclasses.
- `lib/subscriptionclient/http/auth/` — one file per auth scheme (`basic_auth.rb`, `o_auth_ccg.rb`, etc.), each containing both the handler class (extends `CoreLibrary::HeaderAuth` or `CoreLibrary::QueryAuth`) and the `{Scheme}Credentials` data class with `initialize`, `from_env`, and `clone_with`.
- `lib/subscriptionclient/utilities/` — `file_wrapper.rb`, `date_time_helper.rb`.
...
```ruby
# In your Gemfile:
gem 'subscriptionclient', git: 'https://github.com/context-plugins/subscriptionclient-ruby', branch: 'main'
```
...
include SubscriptionClient   # optional — lets you write Client.new instead of SubscriptionClient::Client.new
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/subscriptionclient-ruby /tmp/subscriptionclient-ruby-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/subscriptionclient-ruby "$env:TEMP\subscriptionclient-ruby-src"
```
...
- `lib/subscriptionclient/client.rb` — the full `initialize` keyword list (http kwargs + credentials kwargs), controller accessors, OAuth manager accessors, and `from_env`.
- `lib/subscriptionclient/configuration.rb` — `Environment` and `Server` constants, the `ENVIRONMENTS` hash (maps environment+server strings to base URI templates), and `clone_with`.
- `lib/subscriptionclient/http/auth/*.rb` — credentials classes for every scheme this API uses, including their `initialize` keyword args and `from_env` env-var names.
- `lib/subscriptionclient/controllers/*.rb` — operation method signatures; **this is where required vs. optional params live**.
- `lib/subscriptionclient/models/` — model classes and enum modules; **this is where field names and enum values live**.
- `lib/subscriptionclient/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/subscriptionclient-ruby-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\subscriptionclient-ruby-src"   # Windows
```
