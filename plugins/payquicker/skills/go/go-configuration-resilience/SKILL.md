    ...
    payquicker.CreateConfiguration(
        payquicker.WithHttpConfiguration(
            payquicker.CreateHttpConfiguration(
                payquicker.WithTimeout(30),
                payquicker.WithRetryConfiguration(
                    payquicker.CreateRetryConfiguration(
                        payquicker.WithMaxRetryAttempts(3),
                        payquicker.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := payquicker.CreateRetryConfiguration(
    payquicker.WithMaxRetryAttempts(5),
    payquicker.WithRetryInterval(1),
    payquicker.WithBackoffFactor(2),
    payquicker.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    payquicker.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := payquicker.NewClient(
    payquicker.CreateConfiguration(
        payquicker.WithHttpConfiguration(
            payquicker.CreateHttpConfiguration(
                payquicker.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
