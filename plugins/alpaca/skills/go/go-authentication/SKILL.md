...
> Throughout, `alpaca` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := alpaca.NewClient(
    alpaca.CreateConfiguration(
        alpaca.WithBasicAuthCredentials(
            alpaca.NewBasicAuthCredentials(
...
alpaca.WithApiKeyCredentials(alpaca.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
alpaca.WithApiHeaderCredentials(alpaca.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
alpaca.WithOAuthBearerTokenCredentials(
    alpaca.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
