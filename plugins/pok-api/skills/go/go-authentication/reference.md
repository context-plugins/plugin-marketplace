...
```go
pokapi.WithOAuthCCGCredentials(
    pokapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr pokapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := pokapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr pokapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := pokapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := pokapi.NewClient(pokapi.CreateConfiguration(pokapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    pokapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := pokapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := pokapi.NewClient(pokapi.CreateConfiguration(pokapi.WithOAuthROPCGCredentials(creds)))
...
```go
pokapi.WithOAuthBearerTokenCredentials(pokapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
pokapi.WithBasicAuthCredentials(pokapi.NewBasicAuthCredentials(username, password))
pokapi.WithApiHeaderCredentials(pokapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
pokapi.WithApiKeyCredentials(pokapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `POKAPI_...` variables — e.g.
`POKAPI_USERNAME`, `POKAPI_PASSWORD`, `POKAPI_TOKEN`, `POKAPI_API_KEY`,
`POKAPI_O_AUTH_CLIENT_ID`, `POKAPI_O_AUTH_CLIENT_SECRET`, `POKAPI_ACCESS_TOKEN`. Grep the
