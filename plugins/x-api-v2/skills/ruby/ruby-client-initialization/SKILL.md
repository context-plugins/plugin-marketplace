...
- `XAPIV2` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `x_api_v2` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/x_api_v2/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/x_api_v2/configuration.rb`
(`build_default_config_from_env`) and `lib/x_api_v2/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = XAPIV2::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/x_api_v2.rb`) and store it in a
