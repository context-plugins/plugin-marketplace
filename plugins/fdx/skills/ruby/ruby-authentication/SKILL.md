...
> Throughout, `Fdx` and other `{...}` tokens are placeholders for names you take from your SDK —
...
`lib/fdx/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/fdx/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/fdx/http/auth/api_header.rb` and
