...
- `fund-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'fund-apilib';
...
```ts
import { {Resource}Controller } from 'fund-apilib';
