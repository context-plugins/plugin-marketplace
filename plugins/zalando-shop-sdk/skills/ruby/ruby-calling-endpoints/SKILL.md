...
Open `lib/zalando_shop/client.rb` for the accessor names, then the relevant
`lib/zalando_shop/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = ZalandoShop::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: ZalandoShop::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/zalando_shop/models/`. Field names are defined as
...
Read the enum module in `lib/zalando_shop/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/zalando_shop/client.rb`.
- Operation methods are in `lib/zalando_shop/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/zalando_shop/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/zalando_shop/exceptions/`.
