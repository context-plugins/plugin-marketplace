...
your SDK source (`teslaapi/models/`).
...
```python
from teslaapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `teslaapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from teslaapi.models.animal import Cat, Dog
...
```python
from teslaapi.models.animal import Animal, Cat, Dog
