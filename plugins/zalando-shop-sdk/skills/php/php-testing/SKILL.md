...
> `ZalandoShopClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ZalandoShopClient
{
...
```php
use ZalandoShopLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use ZalandoShopLib\Exceptions\{OperationException};
use ZalandoShopLib\Exceptions\ApiException;
...
app()->instance(ZalandoShopClient::class, clientWithResponses([
