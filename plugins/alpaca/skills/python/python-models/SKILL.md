...
```python
from alpaca.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `alpaca/utilities/union_type_lookup.py`) and each candidate model
...
```python
from alpaca.models.animal import Cat, Dog
...
```python
from alpaca.models.animal import Animal, Cat, Dog
