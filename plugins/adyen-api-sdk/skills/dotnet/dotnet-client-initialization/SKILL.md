...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddAdyenApiClient extension. Use the moment you write `new AdyenApiClient(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `AdyenApiClientOptions` — its options class.
- `AdyenApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public AdyenApiClient(HttpClient httpClient, AdyenApiClientOptions options)
```
...
```csharp
public class AdyenApiClientOptions
{
...
```csharp
using AdyenApi;
using AdyenApi.Servers;

var options = new AdyenApiClientOptions
{
...
var client = new AdyenApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddAdyenApiClient`, which registers the
...
builder.Services.AddAdyenApiClient(options =>
{
...
```csharp
public sealed class MyService(AdyenApiClient client)
{
