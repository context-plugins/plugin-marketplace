...
Open `awsmigrationhub/awsmigrationhub_client.py` for the controller property names, then the relevant
`awsmigrationhub/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from awsmigrationhub.models.{request_model} import {RequestModel}
...
Open `awsmigrationhub/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from awsmigrationhub.models.suite_code_enum import SuiteCodeEnum
...
```python
from awsmigrationhub.http.http_call_back import HttpCallBack
...
client = AWSMigrationHubClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `awsmigrationhub/pagination/` in the source.

## Finding the right method

- Controller property names are on `AWSMigrationHubClient` in `awsmigrationhub/awsmigrationhub_client.py`.
- Operation method signatures are in `awsmigrationhub/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `awsmigrationhub/models/`; enum types have `from_value()`.
