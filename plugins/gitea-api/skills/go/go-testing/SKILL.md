...
`testing` + `testify/require` for reference; substitute the real `giteaapi` and names.
...
import (
    ...
    "github.com/context-plugins/gitea-api-go-sdk"   // root package giteaapi
)
...
func clientReturning(status int, body string) (giteaapi.ClientInterface, *stubTransport) {
    ...
    client := giteaapi.NewClient(
        giteaapi.CreateConfiguration(
            giteaapi.WithHttpConfiguration(
                giteaapi.CreateHttpConfiguration(giteaapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/gitea-api-go-sdk/errors"
...
client := giteaapi.NewClient(
    giteaapi.CreateConfiguration(
        giteaapi.WithHttpConfiguration(
            giteaapi.CreateHttpConfiguration(giteaapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
