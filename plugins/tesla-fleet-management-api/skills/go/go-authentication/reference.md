...
```go
teslafleetmanagementapi.WithOAuthCCGCredentials(
    teslafleetmanagementapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr teslafleetmanagementapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := teslafleetmanagementapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr teslafleetmanagementapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := teslafleetmanagementapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := teslafleetmanagementapi.NewClient(teslafleetmanagementapi.CreateConfiguration(teslafleetmanagementapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    teslafleetmanagementapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := teslafleetmanagementapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := teslafleetmanagementapi.NewClient(teslafleetmanagementapi.CreateConfiguration(teslafleetmanagementapi.WithOAuthROPCGCredentials(creds)))
...
```go
teslafleetmanagementapi.WithOAuthBearerTokenCredentials(teslafleetmanagementapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
teslafleetmanagementapi.WithBasicAuthCredentials(teslafleetmanagementapi.NewBasicAuthCredentials(username, password))
teslafleetmanagementapi.WithApiHeaderCredentials(teslafleetmanagementapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
teslafleetmanagementapi.WithApiKeyCredentials(teslafleetmanagementapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `TESLAFLEETMANAGEMENTAPI_...` variables — e.g.
`TESLAFLEETMANAGEMENTAPI_USERNAME`, `TESLAFLEETMANAGEMENTAPI_PASSWORD`, `TESLAFLEETMANAGEMENTAPI_TOKEN`, `TESLAFLEETMANAGEMENTAPI_API_KEY`,
`TESLAFLEETMANAGEMENTAPI_O_AUTH_CLIENT_ID`, `TESLAFLEETMANAGEMENTAPI_O_AUTH_CLIENT_SECRET`, `TESLAFLEETMANAGEMENTAPI_ACCESS_TOKEN`. Grep the
