...
Open `teslaapi/teslaapi_client.py` for the controller property names, then the relevant
`teslaapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from teslaapi.models.{request_model} import {RequestModel}
...
Open `teslaapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from teslaapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from teslaapi.http.http_call_back import HttpCallBack
...
client = TeslaAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `teslaapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `TeslaAPIClient` in `teslaapi/teslaapi_client.py`.
- Operation method signatures are in `teslaapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `teslaapi/models/`; enum types have `from_value()`.
