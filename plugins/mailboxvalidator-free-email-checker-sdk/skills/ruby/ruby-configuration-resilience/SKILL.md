...
```ruby
client = MailboxValidatorFreeEmailChecker::Client.new(
  max_retries:    3,
...
```ruby
client = MailboxValidatorFreeEmailChecker::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = MailboxValidatorFreeEmailChecker::Client.new(adapter: :net_http)
```
...
client = MailboxValidatorFreeEmailChecker::Client.new(connection: conn)
```
...
client = MailboxValidatorFreeEmailChecker::Client.new(environment: Environment::PRODUCTION)
client = MailboxValidatorFreeEmailChecker::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = MailboxValidatorFreeEmailChecker::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = MailboxValidatorFreeEmailChecker::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = MailboxValidatorFreeEmailChecker::ProxySettings.from_env
client = MailboxValidatorFreeEmailChecker::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < MailboxValidatorFreeEmailChecker::HttpCallBack
...
client = MailboxValidatorFreeEmailChecker::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = MailboxValidatorFreeEmailChecker::Client.from_env
# Overrides take precedence over env vars:
client = MailboxValidatorFreeEmailChecker::Client.from_env(timeout: 30, max_retries: 3)
```
