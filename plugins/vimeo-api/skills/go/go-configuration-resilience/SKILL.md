    ...
    vimeoapi.CreateConfiguration(
        vimeoapi.WithHttpConfiguration(
            vimeoapi.CreateHttpConfiguration(
                ...
                vimeoapi.WithRetryConfiguration(
                    vimeoapi.CreateRetryConfiguration(
                        vimeoapi.WithMaxRetryAttempts(3),
                        vimeoapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := vimeoapi.CreateRetryConfiguration(
    vimeoapi.WithMaxRetryAttempts(5),
    vimeoapi.WithRetryInterval(1),
    vimeoapi.WithBackoffFactor(2),
    vimeoapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    vimeoapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := vimeoapi.NewClient(
    vimeoapi.CreateConfiguration(
        vimeoapi.WithHttpConfiguration(
            vimeoapi.CreateHttpConfiguration(
                vimeoapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
