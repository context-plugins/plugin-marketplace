...
To see which schemes a specific SDK accepts, read the credential setter methods on `VerizonClient.Builder`
...
- Set credentials **on the `VerizonClient.Builder`** before calling `.build()` — the client is immutable
