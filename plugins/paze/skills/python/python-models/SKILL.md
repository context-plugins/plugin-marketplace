...
```python
from paze.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `paze/utilities/union_type_lookup.py`) and each candidate model
...
```python
from paze.models.animal import Cat, Dog
...
```python
from paze.models.animal import Animal, Cat, Dog
