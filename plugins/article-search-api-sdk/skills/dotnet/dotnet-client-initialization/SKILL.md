...
- `ArticleSearchAPIClientOptions` — its options class.
- `ArticleSearchAPI` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public ArticleSearchAPIClient(HttpClient httpClient, ArticleSearchAPIClientOptions options)
```
...
```csharp
public class ArticleSearchAPIClientOptions
{
...
```csharp
using ArticleSearchAPI;
using ArticleSearchAPI.Servers;

var options = new ArticleSearchAPIClientOptions
{
...
var client = new ArticleSearchAPIClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddArticleSearchAPIClient`, which registers the
...
builder.Services.AddArticleSearchAPIClient(options =>
{
...
```csharp
public sealed class MyService(ArticleSearchAPIClient client)
{
