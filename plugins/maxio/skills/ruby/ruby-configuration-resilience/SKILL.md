...
```ruby
client = Maxio::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Maxio::Client.new(adapter: :net_http)
```
...
client = Maxio::Client.new(connection: conn)
```
...
client = Maxio::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Maxio::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Maxio::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Maxio::ProxySettings.from_env
client = Maxio::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Maxio::HttpCallBack
...
client = Maxio::Client.new(http_callback: LogCallback.new)
```
...
client = Maxio::Client.from_env(timeout: 30, max_retries: 3)
```
