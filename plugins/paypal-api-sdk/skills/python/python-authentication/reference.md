...
kwargs to the `PayPalAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `paypalapi/paypalapi_client.py` and `paypalapi/http/auth/` in
the cloned source.

## Basic auth

```python
from paypalapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from paypalapi.http.auth.api_key import ApiKeyCredentials
...
```python
from paypalapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from paypalapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from paypalapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from paypalapi.http.auth.o_auth_acg import OAuthACGCredentials
from paypalapi.models.o_auth_scope_paypalapi_enum import OAuthScope{Pkg}Enum
...
client = PayPalAPIClient(o_auth_acg_credentials=creds)
...
```python
from paypalapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = PayPalAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `PayPalAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `paypalapi/configuration.py` and each `paypalapi/http/auth/*.py` for the exact
