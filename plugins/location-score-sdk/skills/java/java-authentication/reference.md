...
against the `LocationScoreClient.Builder` setters, the `com.amadeus.api.test.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `LocationScoreClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
LocationScoreClient client = new LocationScoreClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeLocationScoreEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new LocationScoreClient.Builder()
