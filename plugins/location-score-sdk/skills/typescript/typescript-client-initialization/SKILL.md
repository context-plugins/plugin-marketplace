...
- `location-scorelib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'location-scorelib';
...
```ts
import { {Resource}Controller } from 'location-scorelib';
