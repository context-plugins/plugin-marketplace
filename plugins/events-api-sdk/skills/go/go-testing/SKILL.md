...
`testing` + `testify/require` for reference; substitute the real `eventsapi` and names.
...
import (
    ...
    "github.com/context-plugins/events-api-go"   // root package eventsapi
)
...
func clientReturning(status int, body string) (eventsapi.ClientInterface, *stubTransport) {
    ...
    client := eventsapi.NewClient(
        eventsapi.CreateConfiguration(
            eventsapi.WithHttpConfiguration(
                eventsapi.CreateHttpConfiguration(eventsapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/events-api-go/errors"
...
client := eventsapi.NewClient(
    eventsapi.CreateConfiguration(
        eventsapi.WithHttpConfiguration(
            eventsapi.CreateHttpConfiguration(eventsapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
