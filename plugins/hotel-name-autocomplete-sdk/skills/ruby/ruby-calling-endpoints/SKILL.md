...
Open `lib/hotel_name_autocomplete/client.rb` for the accessor names, then the relevant
`lib/hotel_name_autocomplete/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = HotelNameAutocomplete::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: HotelNameAutocomplete::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/hotel_name_autocomplete/models/`. Field names are defined as
...
Read the enum module in `lib/hotel_name_autocomplete/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/hotel_name_autocomplete/client.rb`.
- Operation methods are in `lib/hotel_name_autocomplete/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/hotel_name_autocomplete/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/hotel_name_autocomplete/exceptions/`.
