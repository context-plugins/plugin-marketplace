...
Open `hostedonboardingapi/hostedonboardingapi_client.py` for the controller property names, then the relevant
`hostedonboardingapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from hostedonboardingapi.models.{request_model} import {RequestModel}
...
Open `hostedonboardingapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from hostedonboardingapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from hostedonboardingapi.http.http_call_back import HttpCallBack
...
client = HostedOnboardingAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `hostedonboardingapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `HostedOnboardingAPIClient` in `hostedonboardingapi/hostedonboardingapi_client.py`.
- Operation method signatures are in `hostedonboardingapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `hostedonboardingapi/models/`; enum types have `from_value()`.
