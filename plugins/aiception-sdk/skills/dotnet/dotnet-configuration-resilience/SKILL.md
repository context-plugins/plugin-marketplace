...
With DI, the SDK's `AddAIceptionInteractiveClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAIceptionInteractiveClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
