...
`splitit/configuration.py` in the cloned source.
...
`splitit/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`splitit/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `splitit/configuration.py`
...
`NumberPagedResponse`) live in `splitit/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `splitit/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
