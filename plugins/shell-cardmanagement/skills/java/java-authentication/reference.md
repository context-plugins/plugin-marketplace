...
against the `ShellCardManagementAPIsClient.Builder` setters, the `com.shell.apitest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ShellCardManagementAPIsClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ShellCardManagementAPIsClient client = new ShellCardManagementAPIsClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeShellCardmanagementEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new ShellCardManagementAPIsClient.Builder()
