...
- `slackwebapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/slack-web-api-go` — the Go import path for the root package; `github.com/context-plugins/slack-web-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/slack-web-api-go"

client := slackwebapi.NewClient(
    slackwebapi.CreateConfiguration(
        slackwebapi.WithEnvironment(slackwebapi.PRODUCTION),
...
from environment variables named `SLACKWEBAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := slackwebapi.NewClient(slackwebapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `slackwebapi.PRODUCTION`,
`slackwebapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := slackwebapi.NewClient(
    slackwebapi.CreateConfiguration(slackwebapi.WithEnvironment(slackwebapi.PRODUCTION)),
)
```
...
client := slackwebapi.NewClient(
    slackwebapi.CreateConfiguration(
        slackwebapi.WithHttpConfiguration(
            slackwebapi.CreateHttpConfiguration(
                slackwebapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                slackwebapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = slackwebapi.NewClient(slackwebapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api slackwebapi.ClientInterface
}

func NewService(api slackwebapi.ClientInterface) *Service {
