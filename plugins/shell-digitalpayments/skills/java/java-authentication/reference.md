...
against the `ShellSmartPayAPIClient.Builder` setters, the `com.shell.apitest.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `ShellSmartPayAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
ShellSmartPayAPIClient client = new ShellSmartPayAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeShellDigitalpaymentsEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new ShellSmartPayAPIClient.Builder()
