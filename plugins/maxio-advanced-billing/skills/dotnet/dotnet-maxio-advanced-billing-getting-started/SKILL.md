...
| Source repo | https://github.com/context-plugins/maxio-advanced-billing-csharp-sdk (branch `main`) |
...
<!-- crawler:auth -->
| Auth | Credentials properties on `MaxioAdvancedBillingClientOptions`: `BasicAuth: BasicAuthCredentials?`, `BearerAuth: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `Us`, `Eu`, `MaxioApiGateway` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
git clone --branch main https://github.com/context-plugins/maxio-advanced-billing-csharp-sdk
dotnet add reference maxio-advanced-billing-csharp-sdk/MaxioAdvancedBilling.csproj
```
...
temp directory** (`<temp>/maxio-advanced-billing-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/maxio-advanced-billing-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/maxio-advanced-billing-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/maxio-advanced-billing-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\maxio-advanced-billing-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/maxio-advanced-billing-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/maxio-advanced-billing-dotnet-src/`, **never** in the project repo, and its path never goes into
`maxio-advanced-billing-dotnet-plan.md` — the main agent must never see the clone or its path.
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `MaxioAdvancedBillingClientOptions` — the map's *Servers & auth* section
