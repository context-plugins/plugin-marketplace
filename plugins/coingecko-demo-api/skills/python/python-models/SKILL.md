...
your SDK source (`coingeckodemoapi/models/`).
...
```python
from coingeckodemoapi.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `coingeckodemoapi/utilities/union_type_lookup.py`) and each candidate model
...
```python
from coingeckodemoapi.models.animal import Cat, Dog
...
```python
from coingeckodemoapi.models.animal import Animal, Cat, Dog
