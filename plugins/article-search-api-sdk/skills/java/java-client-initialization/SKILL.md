...
- `ArticleSearchAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
...
`ArticleSearchAPIClient.Builder` class. A minimal initialization:

```java
import {rootpkg}.ArticleSearchAPIClient;
import {rootpkg}.Environment;

ArticleSearchAPIClient client = new ArticleSearchAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `ArticleSearchAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `ArticleSearchAPIClient.java` |

Call `.build()` to produce an immutable `ArticleSearchAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public ArticleSearchAPIClient apiClient() {
        return new ArticleSearchAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired ArticleSearchAPIClient client) {
        client.shutdown();
    }
}
```

Inject `ArticleSearchAPIClient` into your services as a normal Spring dependency. Build only one bean.
