...
> `ShellCardManagementAPIsClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ShellCardManagementAPIsClient
{
...
```php
use ShellCardManagementAPIsLib\Exceptions\{OperationException};
use ShellCardManagementAPIsLib\Exceptions\ApiException;
...
app()->instance(ShellCardManagementAPIsClient::class, clientWithResponses([
