...
against the `FdxApiClient.Builder` setters, the `net.apigee.financialdataexchangeprod.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `FdxApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
FdxApiClient client = new FdxApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeFdxEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
