...
`eventsapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `eventsapi/exceptions/` and operation docstrings.
...
```python
from eventsapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `eventsapi/exceptions/`.
...
```python
from eventsapi.exceptions.api_exception import APIException
from eventsapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`eventsapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`eventsapi/exceptions/` for the class.
...
`eventsapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `eventsapi/exceptions/`
  or a `ValueError` — check the auth handler in `eventsapi/http/auth/` for the exact type.
