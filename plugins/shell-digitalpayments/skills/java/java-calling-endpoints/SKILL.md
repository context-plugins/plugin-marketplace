...
Open `ShellSmartPayAPIClient.java` for the full list of `get*Controller()` methods. Controllers are stateless;
...
- `ShellSmartPayAPIClient.java` — controller accessor methods; each `get{Resource}Api()` returns a
  `{Resource}Api`.
