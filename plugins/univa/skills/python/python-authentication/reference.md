...
kwargs to the `UnivapayClientSdkClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `univa/univa_client.py` and `univa/http/auth/` in
the cloned source.

## Basic auth

```python
from univa.http.auth.basic_auth import BasicAuthCredentials
...
```python
from univa.http.auth.api_key import ApiKeyCredentials
...
```python
from univa.http.auth.api_header import ApiHeaderCredentials
...
```python
from univa.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from univa.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from univa.http.auth.o_auth_acg import OAuthACGCredentials
from univa.models.o_auth_scope_univa_enum import OAuthScope{Pkg}Enum
...
client = UnivapayClientSdkClient(o_auth_acg_credentials=creds)
...
```python
from univa.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = UnivapayClientSdkClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `UnivapayClientSdkClient.from_environment()`) read each credential
...
Grep `from_environment` in `univa/configuration.py` and each `univa/http/auth/*.py` for the exact
