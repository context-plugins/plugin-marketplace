...
Open `lib/location_score/client.rb` for the accessor names, then the relevant
`lib/location_score/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = LocationScore::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: LocationScore::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/location_score/models/`. Field names are defined as
...
Read the enum module in `lib/location_score/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/location_score/client.rb`.
- Operation methods are in `lib/location_score/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/location_score/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/location_score/exceptions/`.
