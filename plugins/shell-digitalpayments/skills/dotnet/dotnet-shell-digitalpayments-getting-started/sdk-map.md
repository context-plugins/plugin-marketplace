# SDK map — shell-digitalpayments (.NET)
...
| | |
|---|---|
| SDK display name | shell-digitalpayments |
| Root namespace/module | `ShellSmartPayApi` |
<!-- gen:stamp -->
| Target framework(s) | `netstandard2.0` (C# `LangVersion 14`, `Nullable enable`) |
| Source commit (spec stamp) | `a212ada` (`a212adaca63c35d6673aaff93a8bcf36fba3c876`, tagged `a212ada`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
| Repo | https://github.com/context-plugins/shell-digitalpayments-csharp-sdk (branch `main`) |
...
```csharp
using ShellSmartPayApi;
using ShellSmartPayApi.Servers; // ServerEnvironment lives here

var options = new ShellSmartPayApiClientOptions
{
...
var client = new ShellSmartPayApiClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddShellSmartPayApiClient(o =>
{
...
`ShellSmartPayApiClient.cs`.

<!-- crawler:client-options -->
All `ShellSmartPayApiClientOptions` properties (source: `ShellSmartPayApiClientOptions.cs`):
...
- `ShellSmartPayApiClient(HttpClient httpClient, ShellSmartPayApiClientOptions options)`
<!-- /crawler:client-options -->
...
Of **9 operations**, **9 are Case A (typed)** and **0 are Case B (raw)**.
<!-- /crawler:op-stats -->
...
|---|---:|---|
| `DigitalPaymentEnablement` | 1 | [map/operations/DigitalPaymentEnablement.md](map/operations/DigitalPaymentEnablement.md) |
| `Fueling` | 3 | [map/operations/Fueling.md](map/operations/Fueling.md) |
| `PartnerNotification` | 3 | [map/operations/PartnerNotification.md](map/operations/PartnerNotification.md) |
| `ShellApiPlatformSecurityAuthentication` | 1 | [map/operations/ShellApiPlatformSecurityAuthentication.md](map/operations/ShellApiPlatformSecurityAuthentication.md) |
| `StationLocator` | 1 | [map/operations/StationLocator.md](map/operations/StationLocator.md) |
<!-- /crawler:ops-table -->
...
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 41 | [`AccessTokenError` … `StationLocatorUnauthorizedError`](map/models/records-1-Ac-St.md) |
| Unions (`OneOf` / `AnyOf`) — variant factories + `TryGet…` | 0 + 0 | [map/models/unions.md](map/models/unions.md) |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — literal C# member names + wire values | 1 | [map/models/enums.md](map/models/enums.md) |
<!-- /gen:models-table -->
...
|---|---|
| Client & options (root) | `ShellSmartPayApi` |
| Operation controllers (`Api/`) | `ShellSmartPayApi.Api` |
| Records (`Models/`) | `ShellSmartPayApi.Models` |
| Enums (`Models/Enums/`) | `ShellSmartPayApi.Models.Enums` |
| Error classes (`Errors/`) | `ShellSmartPayApi.Errors` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `ShellSmartPayApiClientOptions` (source: `ShellSmartPayApiClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:

| Property | Type | Notes (from the source XML docs) |
|---|---|---|
| `MppToken` | `string?` | Access token to be passed for Mobile Payment Platform Authentication (<see href="page:guided-walkthrough/walkthrough1">How to obtain Digital Payments access token?</see>) |
| `OAuthTokenPost` | `string?` | API Gateway Access token to be passed for Authentiction. The calling party’s OAuth 2.0/bearer token that’s required for using the Shell API Platform. (<see href="page:guided-walkthrough/walkthrough1">How to obtain APIGEE access token?</see>) |

**Environments.** `options.Environment` is a `ServerEnvironment` (`Servers/ServerEnvironment.cs`) with members: `ServerEnvironment.Test`, `ServerEnvironment.Production`. Base-URL templates and override points live under `Servers/` and `options.Server`.
<!-- /crawler:servers-auth -->
