...
- `hotel-name-autocompletelib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'hotel-name-autocompletelib';
...
```ts
import { {Resource}Controller } from 'hotel-name-autocompletelib';
