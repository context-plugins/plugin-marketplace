...
```go
discourse.WithOAuthCCGCredentials(
    discourse.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr discourse.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := discourse.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr discourse.OAuthCCGManager) models.OAuthToken {
...
```go
creds := discourse.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := discourse.NewClient(discourse.CreateConfiguration(discourse.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    discourse.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := discourse.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := discourse.NewClient(discourse.CreateConfiguration(discourse.WithOAuthROPCGCredentials(creds)))
...
```go
discourse.WithOAuthBearerTokenCredentials(discourse.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
discourse.WithBasicAuthCredentials(discourse.NewBasicAuthCredentials(username, password))
discourse.WithApiHeaderCredentials(discourse.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
discourse.WithApiKeyCredentials(discourse.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `DISCOURSE_...` variables — e.g.
`DISCOURSE_USERNAME`, `DISCOURSE_PASSWORD`, `DISCOURSE_TOKEN`, `DISCOURSE_API_KEY`,
`DISCOURSE_O_AUTH_CLIENT_ID`, `DISCOURSE_O_AUTH_CLIENT_SECRET`, `DISCOURSE_ACCESS_TOKEN`. Grep the
