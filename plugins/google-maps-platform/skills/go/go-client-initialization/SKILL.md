...
- `googlemapsplatform` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/google-maps-platform-go-sdk` — the Go import path for the root package; `github.com/context-plugins/google-maps-platform-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/google-maps-platform-go-sdk"

client := googlemapsplatform.NewClient(
    googlemapsplatform.CreateConfiguration(
        googlemapsplatform.WithEnvironment(googlemapsplatform.PRODUCTION),
...
from environment variables named `GOOGLEMAPSPLATFORM_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := googlemapsplatform.NewClient(googlemapsplatform.CreateConfigurationFromEnvironment())
```
...
```go
client := googlemapsplatform.NewClient(
    googlemapsplatform.CreateConfiguration(googlemapsplatform.WithEnvironment(googlemapsplatform.PRODUCTION)),
)
```
...
client := googlemapsplatform.NewClient(
    googlemapsplatform.CreateConfiguration(
        googlemapsplatform.WithHttpConfiguration(
            googlemapsplatform.CreateHttpConfiguration(
                googlemapsplatform.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                googlemapsplatform.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = googlemapsplatform.NewClient(googlemapsplatform.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api googlemapsplatform.ClientInterface
}

func NewService(api googlemapsplatform.ClientInterface) *Service {
