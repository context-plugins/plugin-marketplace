...
against the `MaxioAdvancedBillingClient.Builder` setters, the `com.chargify.subdomain.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `MaxioAdvancedBillingClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
MaxioAdvancedBillingClient client = new MaxioAdvancedBillingClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeMaxioAdvancedBillingEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new MaxioAdvancedBillingClient.Builder()
