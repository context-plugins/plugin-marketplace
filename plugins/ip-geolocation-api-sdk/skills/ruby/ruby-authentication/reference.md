...
Confirm every name against `lib/ip_geolocation_api/client.rb`, `lib/ip_geolocation_api/configuration.rb`, the
`lib/ip_geolocation_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
