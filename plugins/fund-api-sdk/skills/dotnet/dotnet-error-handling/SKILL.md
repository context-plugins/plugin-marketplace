...
> `{ApiGroup}`, `FundApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `FundApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `FundApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `FundApi.Errors`
...
```csharp
using FundApi.Core.Exceptions;     // SdkException<TError>
using FundApi.Core.ErrorResponse;  // ApiError, RawError
using FundApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`FundApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using FundApi.Core.Exceptions;     // SdkException<TError>
using FundApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `FundApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `FundApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using FundApi.Core.Models;        // ApiResult<TResponse, TError>
using FundApi.Core.ErrorResponse; // RawError
using FundApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `FundApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
