...
> `{operation}`, `{apiGroup}`, `FlightChoicePredictionLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `FlightChoicePredictionLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use FlightChoicePredictionLib\Exceptions\ApiException;
...
```php
use FlightChoicePredictionLib\Exceptions\ApiException;
use FlightChoicePredictionLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `FlightChoicePredictionLib\Exceptions\` — read the source to
