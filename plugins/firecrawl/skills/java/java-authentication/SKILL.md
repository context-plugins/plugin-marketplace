...
To see which schemes a specific SDK accepts, read the credential setter methods on `FirecrawlClient.Builder`
...
- Set credentials **on the `FirecrawlClient.Builder`** before calling `.build()` — the client is immutable
