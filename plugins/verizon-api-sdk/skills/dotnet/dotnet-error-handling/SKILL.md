...
> `{ApiGroup}`, `VerizonApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `VerizonApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `VerizonApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `VerizonApi.Errors`
...
```csharp
using VerizonApi.Core.Exceptions;     // SdkException<TError>
using VerizonApi.Core.ErrorResponse;  // ApiError, RawError
using VerizonApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`VerizonApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using VerizonApi.Core.Exceptions;     // SdkException<TError>
using VerizonApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `VerizonApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `VerizonApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using VerizonApi.Core.Models;        // ApiResult<TResponse, TError>
using VerizonApi.Core.ErrorResponse; // RawError
using VerizonApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `VerizonApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
