...
Open `lib/seatmap_display/client.rb` for the accessor names, then the relevant
`lib/seatmap_display/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = SeatmapDisplay::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: SeatmapDisplay::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/seatmap_display/models/`. Field names are defined as
...
Read the enum module in `lib/seatmap_display/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/seatmap_display/client.rb`.
- Operation methods are in `lib/seatmap_display/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/seatmap_display/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/seatmap_display/exceptions/`.
