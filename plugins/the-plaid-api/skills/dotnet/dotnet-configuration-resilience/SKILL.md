...
All types below live under `ThePlaidApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using ThePlaidApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new ThePlaidApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `ThePlaidApi.Core.Exceptions`):
...
With DI, the SDK's `AddThePlaidApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddThePlaidApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
