...
`ForgeFinanceApis1Client`/`ForgeFinanceApis1ClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `ForgeFinanceApis1Client`,
...
static ForgeFinanceApis1Client ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new ForgeFinanceApis1Client(new HttpClient(handler), new ForgeFinanceApis1ClientOptions { /* auth not needed for stubs */ });
}
```
...
using ForgeFinanceApis1.Errors;              // {Operation}Error types
...
using ForgeFinanceApis1.Core.ErrorResponse; // RawError (Case B)
...
var client = new ForgeFinanceApis1Client(new HttpClient(handler), new ForgeFinanceApis1ClientOptions());
...
- For DI-based code, the SDK's `AddForgeFinanceApis1Client` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `ForgeFinanceApis1Client` from the provider:
  ```csharp
  services.AddForgeFinanceApis1Client(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<ForgeFinanceApis1Client>();
  ```
