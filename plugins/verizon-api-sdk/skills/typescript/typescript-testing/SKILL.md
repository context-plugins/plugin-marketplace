...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new VerizonAPIClient({
...
```typescript
import { {Operation}Error } from 'verizon-apilib/errors';
...
```typescript
import { ApiError } from 'verizon-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(VerizonAPIClient)
    .useValue(new VerizonAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
