...
> Throughout, `eventsapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := eventsapi.NewClient(
    eventsapi.CreateConfiguration(
        eventsapi.WithBasicAuthCredentials(
            eventsapi.NewBasicAuthCredentials(
...
eventsapi.WithApiKeyCredentials(eventsapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
eventsapi.WithApiHeaderCredentials(eventsapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
eventsapi.WithOAuthBearerTokenCredentials(
    eventsapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := eventsapi.NewClient(
    eventsapi.CreateConfiguration(
        eventsapi.WithOAuthCCGCredentials(
            eventsapi.NewOAuthCCGCredentials(
