...
Open `hotelsearchapi/hotelsearchapi_client.py` for the controller property names, then the relevant
`hotelsearchapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from hotelsearchapi.models.{request_model} import {RequestModel}
...
Open `hotelsearchapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from hotelsearchapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from hotelsearchapi.http.http_call_back import HttpCallBack
...
client = HotelSearchAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `hotelsearchapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `HotelSearchAPIClient` in `hotelsearchapi/hotelsearchapi_client.py`.
- Operation method signatures are in `hotelsearchapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `hotelsearchapi/models/`; enum types have `from_value()`.
