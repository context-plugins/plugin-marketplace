...
kwargs to the `EventsAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `eventsapi/eventsapi_client.py` and `eventsapi/http/auth/` in
the cloned source.

## Basic auth

```python
from eventsapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from eventsapi.http.auth.api_key import ApiKeyCredentials
...
```python
from eventsapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from eventsapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from eventsapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from eventsapi.http.auth.o_auth_acg import OAuthACGCredentials
from eventsapi.models.o_auth_scope_eventsapi_enum import OAuthScope{Pkg}Enum
...
client = EventsAPIClient(o_auth_acg_credentials=creds)
...
```python
from eventsapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = EventsAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `EventsAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `eventsapi/configuration.py` and each `eventsapi/http/auth/*.py` for the exact
