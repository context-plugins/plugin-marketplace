...
Open `lib/apimanagementclient/client.rb` for the accessor names, then the relevant
`lib/apimanagementclient/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = ApiManagementClient::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: ApiManagementClient::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/apimanagementclient/models/`. Field names are defined as
...
Read the enum module in `lib/apimanagementclient/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/apimanagementclient/client.rb`.
- Operation methods are in `lib/apimanagementclient/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/apimanagementclient/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/apimanagementclient/exceptions/`.
