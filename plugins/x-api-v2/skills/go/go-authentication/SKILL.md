...
> Throughout, `xapiv2` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := xapiv2.NewClient(
    xapiv2.CreateConfiguration(
        xapiv2.WithBasicAuthCredentials(
            xapiv2.NewBasicAuthCredentials(
...
xapiv2.WithApiKeyCredentials(xapiv2.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
xapiv2.WithApiHeaderCredentials(xapiv2.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
xapiv2.WithOAuthBearerTokenCredentials(
    xapiv2.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
