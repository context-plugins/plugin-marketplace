...
| Project | `ArticleSearchAPI.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `ArticleSearchAPI` (the `using` namespace) |
| Client class | `ArticleSearchAPIClient` |
| Options class | `ArticleSearchAPIClientOptions` |
...
namespaces transitively, so `using ArticleSearchAPI.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/article-search-api-csharp
dotnet add reference article-search-api-csharp/ArticleSearchAPI.csproj
```

```csharp
using ArticleSearchAPI;          // client + options + ServerEnvironment
using ArticleSearchAPI.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/article-search-api-csharp /tmp/article-search-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/article-search-api-csharp "$env:TEMP\article-search-api-dotnet-src"
```
...
- `ArticleSearchAPIClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/article-search-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\article-search-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `ArticleSearchAPIClient`, build its
   `ArticleSearchAPIClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
