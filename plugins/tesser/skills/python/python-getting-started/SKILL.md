...
| Install | `pip install "git+https://github.com/context-plugins/tesser-api-v1-python@main"` |
| Client class | `TesserAPIV1Client` in `tesserapiv1/tesserapiv1_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `tesserapiv1/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `TesserAPIV1Client` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `tesserapiv1/exceptions/api_exception.py` — raised on non-2xx |
...
- `tesserapiv1/tesserapiv1_client.py` — the `TesserAPIV1Client` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `tesserapiv1/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `tesserapiv1/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `tesserapiv1/controllers/` — one `{Resource}Controller` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `tesserapiv1/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `tesserapiv1/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `tesserapiv1/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `tesserapiv1/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `tesserapiv1/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/tesser-api-v1-python@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/tesser-api-v1-python /tmp/tesser-api-v1-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/tesser-api-v1-python "$env:TEMP\tesser-api-v1-python-src"
```
...
- `tesserapiv1/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `tesserapiv1/tesserapiv1_client.py` — `TesserAPIV1Client.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `tesserapiv1/controllers/*.py` — operation method signatures, parameter names, and return types.
- `tesserapiv1/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `tesserapiv1/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/tesser-api-v1-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\tesser-api-v1-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `TesserAPIV1Client(...)`.
