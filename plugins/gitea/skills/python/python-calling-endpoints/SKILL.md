...
Open `gitea/gitea_client.py` for the controller property names, then the relevant
`gitea/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from gitea.models.{request_model} import {RequestModel}
...
Open `gitea/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from gitea.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `gitea/pagination/` in the source.
