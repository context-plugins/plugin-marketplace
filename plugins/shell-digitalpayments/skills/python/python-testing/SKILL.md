...
reference; substitute your `shelldigitalpayments` and `ShellsmartpayapiClient` names.
...
- **Get the stub URL from the SDK, not from this page.** Every environment resolves to a real remote
  host — read the `environments` dict in `shelldigitalpayments/configuration.py`, or call
