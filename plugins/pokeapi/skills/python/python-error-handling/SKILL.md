...
`pokeapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `pokeapi/exceptions/` and operation docstrings.
...
```python
from pokeapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `pokeapi/exceptions/`.
...
```python
from pokeapi.exceptions.api_exception import APIException
from pokeapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`pokeapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`pokeapi/exceptions/` for the class.
...
`pokeapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `pokeapi/exceptions/`
  or a `ValueError` — check the auth handler in `pokeapi/http/auth/` for the exact type.
