...
Open `maxioadvancedbilling/maxioadvancedbilling_client.py` for the controller property names, then the relevant
`maxioadvancedbilling/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from maxioadvancedbilling.models.{request_model} import {RequestModel}
...
Open `maxioadvancedbilling/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from maxioadvancedbilling.models.suite_code_enum import SuiteCodeEnum
...
```python
from maxioadvancedbilling.http.http_call_back import HttpCallBack
...
client = MaxioadvancedbillingClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `maxioadvancedbilling/pagination/` in the source.

## Finding the right method

- Controller property names are on `MaxioadvancedbillingClient` in `maxioadvancedbilling/maxioadvancedbilling_client.py`.
- Operation method signatures are in `maxioadvancedbilling/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `maxioadvancedbilling/models/`; enum types have `from_value()`.
