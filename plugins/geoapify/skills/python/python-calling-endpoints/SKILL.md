...
Open `geoapify/geoapify_client.py` for the controller property names, then the relevant
`geoapify/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from geoapify.models.{request_model} import {RequestModel}
...
Open `geoapify/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from geoapify.models.suite_code_enum import SuiteCodeEnum
...
```python
from geoapify.http.http_call_back import HttpCallBack
...
client = GeoapifyapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `geoapify/pagination/` in the source.

## Finding the right method

- Controller property names are on `GeoapifyapiClient` in `geoapify/geoapify_client.py`.
- Operation method signatures are in `geoapify/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `geoapify/models/`; enum types have `from_value()`.
