...
reference; substitute your `flightpriceanalysisapi` and `FlightPriceAnalysisAPIClient` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `flightpriceanalysisapi/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
