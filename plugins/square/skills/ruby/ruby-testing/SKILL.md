...
client = stub_client(stubs)
assert_raises Square::APIException do
...
BASE = Square::Configuration.new(environment: Square::Environment::PRODUCTION).get_base_uri

def build_test_client
  Square::Client.new(
    environment: Square::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Square::APIException
```
...
rescue Square::OAuthProviderException => e
...
```ruby
expect { call }.to raise_error(Square::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Square::HttpCallBack
...
client  = Square::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
