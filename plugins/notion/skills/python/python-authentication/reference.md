...
kwargs to the `NotionClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `notion/notion_client.py` and `notion/http/auth/` in
the cloned source.

## Basic auth

```python
from notion.http.auth.basic_auth import BasicAuthCredentials
...
```python
from notion.http.auth.api_key import ApiKeyCredentials
...
```python
from notion.http.auth.api_header import ApiHeaderCredentials
...
```python
from notion.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from notion.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from notion.http.auth.o_auth_acg import OAuthACGCredentials
from notion.models.o_auth_scope_notion_enum import OAuthScope{Pkg}Enum
...
client = NotionClient(o_auth_acg_credentials=creds)
...
```python
from notion.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = NotionClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `NotionClient.from_environment()`) read each credential
...
Grep `from_environment` in `notion/configuration.py` and each `notion/http/auth/*.py` for the exact
