...
- `paypallib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'paypallib';
...
```ts
import { {Resource}Api } from 'paypallib';
