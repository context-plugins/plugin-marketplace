...
- `mediumapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/medium-api-go` — the Go import path for the root package; `github.com/context-plugins/medium-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/medium-api-go"

client := mediumapi.NewClient(
    mediumapi.CreateConfiguration(
        mediumapi.WithEnvironment(mediumapi.PRODUCTION),
...
from environment variables named `MEDIUMAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := mediumapi.NewClient(mediumapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `mediumapi.PRODUCTION`,
`mediumapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := mediumapi.NewClient(
    mediumapi.CreateConfiguration(mediumapi.WithEnvironment(mediumapi.PRODUCTION)),
)
```
...
client := mediumapi.NewClient(
    mediumapi.CreateConfiguration(
        mediumapi.WithHttpConfiguration(
            mediumapi.CreateHttpConfiguration(
                mediumapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                mediumapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = mediumapi.NewClient(mediumapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api mediumapi.ClientInterface
}

func NewService(api mediumapi.ClientInterface) *Service {
