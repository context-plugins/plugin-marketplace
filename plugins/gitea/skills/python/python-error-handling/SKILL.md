...
`gitea/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `gitea/exceptions/` and operation docstrings.
...
For status codes the API documents, the generator creates a typed subclass in `gitea/exceptions/`.
...
```python
from gitea.exceptions.api_exception import APIException
from gitea.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`gitea/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`gitea/exceptions/` for the class.
...
`gitea/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `gitea/exceptions/`
  or a `ValueError` — check the auth handler in `gitea/http/auth/` for the exact type.
