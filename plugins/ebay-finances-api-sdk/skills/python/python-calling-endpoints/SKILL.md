...
Open `ebayfinancesapi/ebayfinancesapi_client.py` for the controller property names, then the relevant
`ebayfinancesapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from ebayfinancesapi.models.{request_model} import {RequestModel}
...
Open `ebayfinancesapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from ebayfinancesapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from ebayfinancesapi.http.http_call_back import HttpCallBack
...
client = EBayFinancesAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `ebayfinancesapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `EBayFinancesAPIClient` in `ebayfinancesapi/ebayfinancesapi_client.py`.
- Operation method signatures are in `ebayfinancesapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `ebayfinancesapi/models/`; enum types have `from_value()`.
