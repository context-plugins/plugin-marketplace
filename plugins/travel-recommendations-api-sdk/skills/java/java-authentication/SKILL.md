...
To see which schemes a specific SDK accepts, read the credential setter methods on `TravelRecommendationsAPIClient.Builder`
...
- Set credentials **on the `TravelRecommendationsAPIClient.Builder`** before calling `.build()` — the client is immutable
