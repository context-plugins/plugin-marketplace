---
name: dotnet-tesla-fleet-management-api-getting-started
description: Tesla Fleet Management API .NET SDK identity and lookup layer for the dotnet-tesla-fleet-management-api-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the Tesla Fleet Management API .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-tesla-fleet-management-api-sdk` helper
...
| Source repo | https://github.com/context-plugins/tesla-fleet-management-api-csharp-sdk (branch `main`) |
...
<!-- crawler:auth -->
| Auth | Credentials properties on `TeslaFleetManagementApiClientOptions`: `BearerAuth: string?`, `Thirdpartytoken: OAuth2AuthorizationCodeCredentials?`, `ThirdpartytokenTokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using TeslaFleetManagementApi.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/tesla-fleet-management-api-csharp-sdk
dotnet add reference tesla-fleet-management-api-csharp-sdk/TeslaFleetManagementApi.csproj
```
...
temp directory** (`<temp>/tesla-fleet-management-api-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/tesla-fleet-management-api-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/tesla-fleet-management-api-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/tesla-fleet-management-api-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\tesla-fleet-management-api-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/tesla-fleet-management-api-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/tesla-fleet-management-api-dotnet-src/`, **never** in the project repo, and its path never goes into
`tesla-fleet-management-api-dotnet-plan.md` — the main agent must never see the clone or its path.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new TeslaFleetManagementApiClient(...)`, build its options, or DI-register via
   `AddTeslaFleetManagementApiClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `TeslaFleetManagementApiClientOptions` — the map's *Servers & auth* section
