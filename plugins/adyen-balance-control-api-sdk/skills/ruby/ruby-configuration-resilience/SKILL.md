...
```ruby
client = AdyenBalanceControlAPI::Client.new(
  max_retries:    3,
...
```ruby
client = AdyenBalanceControlAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = AdyenBalanceControlAPI::Client.new(adapter: :net_http)
```
...
client = AdyenBalanceControlAPI::Client.new(connection: conn)
```
...
client = AdyenBalanceControlAPI::Client.new(environment: Environment::PRODUCTION)
client = AdyenBalanceControlAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = AdyenBalanceControlAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = AdyenBalanceControlAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = AdyenBalanceControlAPI::ProxySettings.from_env
client = AdyenBalanceControlAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < AdyenBalanceControlAPI::HttpCallBack
...
client = AdyenBalanceControlAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = AdyenBalanceControlAPI::Client.from_env
# Overrides take precedence over env vars:
client = AdyenBalanceControlAPI::Client.from_env(timeout: 30, max_retries: 3)
```
