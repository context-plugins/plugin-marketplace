    ...
    apiapisguru.CreateConfiguration(
        apiapisguru.WithHttpConfiguration(
            apiapisguru.CreateHttpConfiguration(
                apiapisguru.WithTimeout(30),
                apiapisguru.WithRetryConfiguration(
                    apiapisguru.CreateRetryConfiguration(
                        apiapisguru.WithMaxRetryAttempts(3),
                        apiapisguru.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := apiapisguru.CreateRetryConfiguration(
    apiapisguru.WithMaxRetryAttempts(5),
    apiapisguru.WithRetryInterval(1),
    apiapisguru.WithBackoffFactor(2),
    apiapisguru.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    apiapisguru.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := apiapisguru.NewClient(
    apiapisguru.CreateConfiguration(
        apiapisguru.WithHttpConfiguration(
            apiapisguru.CreateHttpConfiguration(
                apiapisguru.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
