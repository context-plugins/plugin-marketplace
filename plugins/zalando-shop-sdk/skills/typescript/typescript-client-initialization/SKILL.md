...
- `zalando-shoplib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'zalando-shoplib';
...
```ts
import { {Resource}Controller } from 'zalando-shoplib';
