...
```go
cellpoint.WithOAuthCCGCredentials(
    cellpoint.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr cellpoint.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := cellpoint.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr cellpoint.OAuthCCGManager) models.OAuthToken {
...
```go
creds := cellpoint.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := cellpoint.NewClient(cellpoint.CreateConfiguration(cellpoint.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    cellpoint.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := cellpoint.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := cellpoint.NewClient(cellpoint.CreateConfiguration(cellpoint.WithOAuthROPCGCredentials(creds)))
...
```go
cellpoint.WithOAuthBearerTokenCredentials(cellpoint.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
cellpoint.WithBasicAuthCredentials(cellpoint.NewBasicAuthCredentials(username, password))
cellpoint.WithApiHeaderCredentials(cellpoint.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
cellpoint.WithApiKeyCredentials(cellpoint.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `CELLPOINT_...` variables — e.g.
`CELLPOINT_USERNAME`, `CELLPOINT_PASSWORD`, `CELLPOINT_TOKEN`, `CELLPOINT_API_KEY`,
`CELLPOINT_O_AUTH_CLIENT_ID`, `CELLPOINT_O_AUTH_CLIENT_SECRET`, `CELLPOINT_ACCESS_TOKEN`. Grep the
