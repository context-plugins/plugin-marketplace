...
`firecrawlapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `firecrawlapi/exceptions/` and operation docstrings.
...
```python
from firecrawlapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `firecrawlapi/exceptions/`.
...
```python
from firecrawlapi.exceptions.api_exception import APIException
from firecrawlapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`firecrawlapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`firecrawlapi/exceptions/` for the class.
...
`firecrawlapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `firecrawlapi/exceptions/`
  or a `ValueError` — check the auth handler in `firecrawlapi/http/auth/` for the exact type.
