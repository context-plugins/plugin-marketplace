...
`GiteaApiClient`/`GiteaApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `GiteaApiClient`,
...
static GiteaApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new GiteaApiClient(new HttpClient(handler), new GiteaApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using GiteaApi.Errors;              // {Operation}Error types
...
```csharp
using GiteaApi.Core.Exceptions;
using GiteaApi.Core.ErrorResponse;
...
using GiteaApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new GiteaApiClient(new HttpClient(handler), new GiteaApiClientOptions());
...
- For DI-based code, the SDK's `AddGiteaApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `GiteaApiClient` from the provider:
  ```csharp
  services.AddGiteaApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<GiteaApiClient>();
  ```
