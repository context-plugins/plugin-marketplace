...
```ruby
client = AdyenBinLookupAPI::Client.new(
  max_retries:    3,
...
```ruby
client = AdyenBinLookupAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = AdyenBinLookupAPI::Client.new(adapter: :net_http)
```
...
client = AdyenBinLookupAPI::Client.new(connection: conn)
```
...
client = AdyenBinLookupAPI::Client.new(environment: Environment::PRODUCTION)
client = AdyenBinLookupAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = AdyenBinLookupAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = AdyenBinLookupAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = AdyenBinLookupAPI::ProxySettings.from_env
client = AdyenBinLookupAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < AdyenBinLookupAPI::HttpCallBack
...
client = AdyenBinLookupAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = AdyenBinLookupAPI::Client.from_env
# Overrides take precedence over env vars:
client = AdyenBinLookupAPI::Client.from_env(timeout: 30, max_retries: 3)
```
