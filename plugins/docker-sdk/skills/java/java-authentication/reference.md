...
against the `DockerClient.Builder` setters, the `v133.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `DockerClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
DockerClient client = new DockerClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeDockerEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
