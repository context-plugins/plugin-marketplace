...
reference; substitute your `klarnapaymentsapiv1` and `KlarnaPaymentsAPIV1Client` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `klarnapaymentsapiv1/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
