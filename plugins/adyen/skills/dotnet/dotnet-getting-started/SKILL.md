...
description: "Adyen .NET SDK identity and lookup layer (.NET/C# only) you load directly — install, root namespace, environments, auth pattern, and the SDK map (shipped inside the SDK source repo): every operation signature and error case by lookup, plus the source file declaring each model, enum and error type. Load it for every contract fact, never memory; dotnet-integrate-adyen is the workflow that uses it."
---

# Getting started with the Adyen .NET SDK

> **Who this skill is for.** This is the **map layer**: load it to ground every Adyen SDK fact. `dotnet-integrate-adyen` is the workflow that drives it — you collect the contracts for the work in scope into `adyen-plan.md`, implement from that sheet, and come back to the one map page a row cites for any fact the sheet is missing. Every contract fact comes from here, never from memory.
...
| Source repo | https://github.com/context-plugins/adyen-csharp-sdk (branch `main` — the ref this map documents) |
| Root namespace | `Adyen` (the `using` namespace) |
| Client class | `AdyenClient` |
| Options class | `AdyenClientOptions` |
| Auth | **API key** in the `X-API-Key` header — set `ApiKeyAuth`<br>HTTP **Basic** — set `BasicAuth`<br>**API key** in the `clientKey` query parameter — set `ClientKey` |
| Environments | `ServerEnvironment.Test` *(default)* → `Default` `https://checkout-test.adyen.com/v72`, `Default1` `https://pal-test.adyen.com/pal/servlet/Payment/v68`, `Default2` `https://pal-test.adyen.com/pal/servlet/Recurring/v68`, `Default3` `https://pal-test.adyen.com/pal/servlet/Payout/v68`, `Default4` `https://pal-test.adyen.com/pal/servlet/BinLookup/v54`, `Default5` `https://pal-test.adyen.com/pal/servlet/StoredValue/v46`, `Default6` `https://ca-test.adyen.com/ca/services/DataProtectionService/v1`, `Default7` `https://balanceplatform-api-test.adyen.com/fx/api/v1`, `Default8` `https://pal-test.adyen.com/pal/services/TestCard/v1`, `Default9` `https://management-test.adyen.com/v3`, `Default10` `https://cal-test.adyen.com/cal/services/Account/v6`, `Default11` `https://balance-control-test.adyen.com/balance-control/api/v2`, `Default12` `https://cal-test.adyen.com/cal/services/Notification/v6`, `Default13` `https://balanceplatform-api-test.adyen.com/bcl/v2`, `Default14` `https://balanceplatform-api-test.adyen.com/btl/v4`, `Default15` `https://balanceplatform-api-test.adyen.com/capital/v1`, `Default16` `https://cal-test.adyen.com/cal/services/Fund/v6`, `Default17` `https://test.adyen.com/authe/api/v1`, `Default18` `https://kyc-test.adyen.com/lem/v4`, `Default19` `https://cal-test.adyen.com/cal/services/Hop/v6`, `Default20` `https://balanceplatform-api-test.adyen.com/a2aissuer-api/v1`, `Default21` `https://obgateway-test.adyen.com/obgateway/v1`, `Default22` `https://ca-test.adyen.com/ca/services/DisputeService/v30`, `Default23` `https://balanceplatform-api-test.adyen.com/btl/api/v4`, `Default24` `https://device-api-test.adyen.com/v1`, `Default25` `https://softposconfig-test.adyen.com/softposconfig/v3`, `Default26` `https://management-test.adyen.com/v1`, `Default27` `https://checkout-test.adyen.com/checkout/possdk/v68`, `Default28` `https://postfmapi-test.adyen.com/postfmapi/terminal/v1` |
| Log environment variable | `ADYENCLIENT_LOG` |
| Target framework | `netstandard2.0` |

The table above is **orientation, not a copy-paste recipe** — it gives you the names and facts (install, namespaces, the auth *pattern*, the environments), while the actual integration code comes from the companion skills. Load every one the contract sheet's REQUIRED READING names before you start implementing (`dotnet-integrate-adyen` makes that binding); the **Integration workflow** below says which governs which step.

## Namespaces (using-directives)

The SDK splits its public types across **separate child namespaces**. C# does **not** import child namespaces transitively, so `using Adyen.Models;` alone does **not** make enums, union types, or error types visible — you get `CS0103`/`CS0246` ("name/type does not exist") on build. Add a separate `using` for each kind of type you reference:

| Type | Namespace |
| --- | --- |
| `AdyenClient` | `Adyen` |
| `AdyenClientOptions` | `Adyen` |
| `ServerEnvironment` | `Adyen.Servers` |
...
- Collect the contracts for **every** in-scope operation in **one** pass — signatures and error accessors from the map pages, then required fields with wire names and enum values from the declaring files those pages name — into the **contract sheet** in `adyen-plan.md`, then implement from the sheet. Do not re-open a map page or source file per field, and never re-look-up a fact the sheet already carries.
...
The clone lives in `<temp>/adyen-sdk-src/`, **never** in the project repo, and its path never goes into `adyen-plan.md` — the plan must stay portable, and the clone is a session-local reference, not part of the project.
...
- `Servers/`, `AdyenClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
The generator injects an `Idempotency-Key: Guid.NewGuid()` header on **every non-GET operation that does not already declare that header itself** — fresh on every call, invisible to you, and **not** an idempotency key in any meaningful sense: a value that changes per call deduplicates nothing, and Adyen may not document reading that header at all. `dotnet-configuration-resilience` explains why a visible header like this is worse than an absent one.
