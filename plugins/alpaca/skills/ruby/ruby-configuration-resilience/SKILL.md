...
```ruby
client = Alpaca::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Alpaca::Client.new(adapter: :net_http)
```
...
client = Alpaca::Client.new(connection: conn)
```
...
client = Alpaca::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Alpaca::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Alpaca::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Alpaca::ProxySettings.from_env
client = Alpaca::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Alpaca::HttpCallBack
...
client = Alpaca::Client.new(http_callback: LogCallback.new)
```
...
client = Alpaca::Client.from_env(timeout: 30, max_retries: 3)
```
