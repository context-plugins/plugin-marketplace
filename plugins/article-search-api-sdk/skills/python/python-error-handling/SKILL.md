...
`articlesearchapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `articlesearchapi/exceptions/` and operation docstrings.
...
```python
from articlesearchapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `articlesearchapi/exceptions/`.
...
```python
from articlesearchapi.exceptions.api_exception import APIException
from articlesearchapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`articlesearchapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`articlesearchapi/exceptions/` for the class.
...
`articlesearchapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `articlesearchapi/exceptions/`
  or a `ValueError` — check the auth handler in `articlesearchapi/http/auth/` for the exact type.
