...
```ruby
client = Payquicker::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Payquicker::Client.new(adapter: :net_http)
```
...
client = Payquicker::Client.new(connection: conn)
```
...
client = Payquicker::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Payquicker::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Payquicker::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Payquicker::ProxySettings.from_env
client = Payquicker::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Payquicker::HttpCallBack
...
client = Payquicker::Client.new(http_callback: LogCallback.new)
```
...
client = Payquicker::Client.from_env(timeout: 30, max_retries: 3)
```
