...
kwargs to the `BokudirectpaymentsapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `boku/boku_client.py` and `boku/http/auth/` in
the cloned source.

## Basic auth

```python
from boku.http.auth.basic_auth import BasicAuthCredentials
...
```python
from boku.http.auth.api_key import ApiKeyCredentials
...
```python
from boku.http.auth.api_header import ApiHeaderCredentials
...
```python
from boku.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from boku.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from boku.http.auth.o_auth_acg import OAuthACGCredentials
from boku.models.o_auth_scope_boku_enum import OAuthScope{Pkg}Enum
...
client = BokudirectpaymentsapiClient(o_auth_acg_credentials=creds)
...
```python
from boku.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = BokudirectpaymentsapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `BokudirectpaymentsapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `boku/configuration.py` and each `boku/http/auth/*.py` for the exact
