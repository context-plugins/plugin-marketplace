...
description: Initialize an APIMatic-generated C#/.NET API client — you construct it from an HttpClient you supply (the SDK doesn't own it; reuse one long-lived instance or an IHttpClientFactory, not one per request) plus an options object, choose a server environment/base URL, and DI-register via the generated AddKlarnaPaymentsApiV1Client extension. Use the moment you write `new KlarnaPaymentsApiV1Client(...)`, build its options, pick an environment, set up the HttpClient/client lifetime, or register the client in dependency injection — load it even after reading the constructor in the SDK source, since the signature shows the arguments but not the lifetime/reuse rules or DI wiring.
---
...
- `KlarnaPaymentsApiV1ClientOptions` — its options class.
- `KlarnaPaymentsApiV1` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public KlarnaPaymentsApiV1Client(HttpClient httpClient, KlarnaPaymentsApiV1ClientOptions options)
```
...
```csharp
public class KlarnaPaymentsApiV1ClientOptions
{
...
```csharp
using KlarnaPaymentsApiV1;
using KlarnaPaymentsApiV1.Servers;

var options = new KlarnaPaymentsApiV1ClientOptions
{
...
var client = new KlarnaPaymentsApiV1Client(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddKlarnaPaymentsApiV1Client`, which registers the
...
builder.Services.AddKlarnaPaymentsApiV1Client(options =>
{
...
```csharp
public sealed class MyService(KlarnaPaymentsApiV1Client client)
{
