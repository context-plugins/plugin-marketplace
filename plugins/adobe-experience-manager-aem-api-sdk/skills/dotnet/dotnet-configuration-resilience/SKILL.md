...
All types below live under `AdobeExperienceManagerAemApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using AdobeExperienceManagerAemApi.Core.Configuration;
...
```csharp
using AdobeExperienceManagerAemApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new AdobeExperienceManagerAemApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `AdobeExperienceManagerAemApi.Core.Exceptions`):
...
With DI, the SDK's `AddAdobeExperienceManagerAemApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddAdobeExperienceManagerAemApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
