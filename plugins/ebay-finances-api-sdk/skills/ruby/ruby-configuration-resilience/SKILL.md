...
```ruby
client = EBayFinancesAPI::Client.new(
  max_retries:    3,
...
```ruby
client = EBayFinancesAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = EBayFinancesAPI::Client.new(adapter: :net_http)
```
...
client = EBayFinancesAPI::Client.new(connection: conn)
```
...
client = EBayFinancesAPI::Client.new(environment: Environment::PRODUCTION)
client = EBayFinancesAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = EBayFinancesAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = EBayFinancesAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = EBayFinancesAPI::ProxySettings.from_env
client = EBayFinancesAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < EBayFinancesAPI::HttpCallBack
...
client = EBayFinancesAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = EBayFinancesAPI::Client.from_env
# Overrides take precedence over env vars:
client = EBayFinancesAPI::Client.from_env(timeout: 30, max_retries: 3)
```
