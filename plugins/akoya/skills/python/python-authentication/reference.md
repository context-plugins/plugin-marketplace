...
kwargs to the `AkoyaClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `akoya/akoya_client.py` and `akoya/http/auth/` in
the cloned source.
...
```python
from akoya.http.auth.api_key import ApiKeyCredentials
...
```python
from akoya.http.auth.api_header import ApiHeaderCredentials
...
```python
from akoya.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from akoya.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from akoya.http.auth.o_auth_acg import OAuthACGCredentials
from akoya.models.o_auth_scope_akoya_enum import OAuthScope{Pkg}Enum
...
client = AkoyaClient(o_auth_acg_credentials=creds)
...
```python
from akoya.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = AkoyaClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `AkoyaClient.from_environment()`) read each credential
...
Grep `from_environment` in `akoya/configuration.py` and each `akoya/http/auth/*.py` for the exact
