...
All types below live under `RestApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using RestApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new RestApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `RestApi.Core.Exceptions`):
...
With DI, the SDK's `AddRestApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddRestApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
