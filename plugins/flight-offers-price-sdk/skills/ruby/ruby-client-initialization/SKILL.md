...
- `FlightOffersPrice` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `flight_offers_price` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'flight_offers_price'
include FlightOffersPrice
...
The authoritative list is the `initialize` signature in `lib/flight_offers_price/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/flight_offers_price/configuration.rb`
(`build_default_config_from_env`) and `lib/flight_offers_price/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/flight_offers_price.rb`) and store it in a
