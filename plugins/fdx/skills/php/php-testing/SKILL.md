...
> `FdxApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): FdxApiClient
{
...
```php
use FdxApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use FdxApiLib\Exceptions\{OperationException};
use FdxApiLib\Exceptions\ApiException;
...
app()->instance(FdxApiClient::class, clientWithResponses([
