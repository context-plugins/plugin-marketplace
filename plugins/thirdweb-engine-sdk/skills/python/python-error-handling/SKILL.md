...
`thirdwebengine/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `thirdwebengine/exceptions/` and operation docstrings.
...
```python
from thirdwebengine.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `thirdwebengine/exceptions/`.
...
```python
from thirdwebengine.exceptions.api_exception import APIException
from thirdwebengine.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`thirdwebengine/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`thirdwebengine/exceptions/` for the class.
...
`thirdwebengine/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `thirdwebengine/exceptions/`
  or a `ValueError` — check the auth handler in `thirdwebengine/http/auth/` for the exact type.
