...
```ruby
client = MaxioAdvancedBilling::Client.new(
  max_retries:    3,
...
```ruby
client = MaxioAdvancedBilling::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = MaxioAdvancedBilling::Client.new(adapter: :net_http)
```
...
client = MaxioAdvancedBilling::Client.new(connection: conn)
```
...
client = MaxioAdvancedBilling::Client.new(environment: Environment::PRODUCTION)
client = MaxioAdvancedBilling::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = MaxioAdvancedBilling::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = MaxioAdvancedBilling::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = MaxioAdvancedBilling::ProxySettings.from_env
client = MaxioAdvancedBilling::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < MaxioAdvancedBilling::HttpCallBack
...
client = MaxioAdvancedBilling::Client.new(http_callback: LogCallback.new)
```
...
client = MaxioAdvancedBilling::Client.from_env(timeout: 30, max_retries: 3)
```
