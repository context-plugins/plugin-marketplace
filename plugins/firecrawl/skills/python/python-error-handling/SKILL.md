...
`firecrawl/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `firecrawl/exceptions/` and operation docstrings.
...
```python
from firecrawl.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `firecrawl/exceptions/`.
...
```python
from firecrawl.exceptions.api_exception import APIException
from firecrawl.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`firecrawl/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`firecrawl/exceptions/` for the class.
...
`firecrawl/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `firecrawl/exceptions/`
  or a `ValueError` — check the auth handler in `firecrawl/http/auth/` for the exact type.
