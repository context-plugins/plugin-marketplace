...
`coingeckodemoapi/configuration.py` in the cloned source.
...
```python
import requests
from coingeckodemoapi.configuration import Configuration
...
`coingeckodemoapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`coingeckodemoapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `coingeckodemoapi/configuration.py`
...
`NumberPagedResponse`) live in `coingeckodemoapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `coingeckodemoapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
