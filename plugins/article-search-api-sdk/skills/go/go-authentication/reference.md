...
```go
articlesearchapi.WithOAuthCCGCredentials(
    articlesearchapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr articlesearchapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := articlesearchapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr articlesearchapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := articlesearchapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := articlesearchapi.NewClient(articlesearchapi.CreateConfiguration(articlesearchapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    articlesearchapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := articlesearchapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := articlesearchapi.NewClient(articlesearchapi.CreateConfiguration(articlesearchapi.WithOAuthROPCGCredentials(creds)))
...
```go
articlesearchapi.WithOAuthBearerTokenCredentials(articlesearchapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
articlesearchapi.WithBasicAuthCredentials(articlesearchapi.NewBasicAuthCredentials(username, password))
articlesearchapi.WithApiHeaderCredentials(articlesearchapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
articlesearchapi.WithApiKeyCredentials(articlesearchapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `ARTICLESEARCHAPI_...` variables — e.g.
`ARTICLESEARCHAPI_USERNAME`, `ARTICLESEARCHAPI_PASSWORD`, `ARTICLESEARCHAPI_TOKEN`, `ARTICLESEARCHAPI_API_KEY`,
`ARTICLESEARCHAPI_O_AUTH_CLIENT_ID`, `ARTICLESEARCHAPI_O_AUTH_CLIENT_SECRET`, `ARTICLESEARCHAPI_ACCESS_TOKEN`. Grep the
