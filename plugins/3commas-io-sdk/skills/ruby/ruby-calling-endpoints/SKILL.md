...
Open `lib/3commas_io/client.rb` for the accessor names, then the relevant
`lib/3commas_io/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = 3commasIo::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: 3commasIo::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/3commas_io/models/`. Field names are defined as
...
Read the enum module in `lib/3commas_io/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/3commas_io/client.rb`.
- Operation methods are in `lib/3commas_io/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/3commas_io/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/3commas_io/exceptions/`.
