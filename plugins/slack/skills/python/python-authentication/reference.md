...
kwargs to the `SlackClient` constructor (or to `Configuration`). Exact credential class names and
...
client = SlackClient(o_auth_acg_credentials=creds)
...
client = SlackClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `SlackClient.from_environment()`) read each credential
