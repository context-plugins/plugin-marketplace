...
kwargs to the `BokuClient` constructor (or to `Configuration`). Exact credential class names and
...
client = BokuClient(o_auth_acg_credentials=creds)
...
client = BokuClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `BokuClient.from_environment()`) read each credential
