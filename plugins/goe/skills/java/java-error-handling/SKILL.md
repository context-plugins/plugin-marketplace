...
│   └── com.frk.uip.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.frk.uip.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.frk.uip.api.http.client.HttpContext`, which carries:
