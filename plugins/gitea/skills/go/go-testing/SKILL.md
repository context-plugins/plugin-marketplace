...
`testing` + `testify/require` for reference; substitute the real `gitea` and names.
...
import (
    ...
    "github.com/context-plugins/gitea-go-sdk"   // root package gitea
)
...
func clientReturning(status int, body string) (gitea.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := gitea.NewClient(
        gitea.CreateConfiguration(
            gitea.WithHttpConfiguration(
                gitea.CreateHttpConfiguration(gitea.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/gitea-go-sdk/errors"
...
client := gitea.NewClient(
    gitea.CreateConfiguration(
        gitea.WithHttpConfiguration(
            gitea.CreateHttpConfiguration(gitea.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
