...
> `PokApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): PokApiClient
{
...
```php
use PokApiLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use PokApiLib\Exceptions\{OperationException};
use PokApiLib\Exceptions\ApiException;
...
app()->instance(PokApiClient::class, clientWithResponses([
