...
```ruby
client = ShellEv::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ShellEv::Client.new(adapter: :net_http)
```
...
client = ShellEv::Client.new(connection: conn)
```
...
client = ShellEv::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = ShellEv::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ShellEv::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ShellEv::ProxySettings.from_env
client = ShellEv::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ShellEv::HttpCallBack
...
client = ShellEv::Client.new(http_callback: LogCallback.new)
```
...
client = ShellEv::Client.from_env(timeout: 30, max_retries: 3)
```
