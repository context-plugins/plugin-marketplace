...
`ipgeolocationapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `ipgeolocationapi/exceptions/` and operation docstrings.
...
```python
from ipgeolocationapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `ipgeolocationapi/exceptions/`.
...
```python
from ipgeolocationapi.exceptions.api_exception import APIException
from ipgeolocationapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`ipgeolocationapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`ipgeolocationapi/exceptions/` for the class.
...
`ipgeolocationapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `ipgeolocationapi/exceptions/`
  or a `ValueError` — check the auth handler in `ipgeolocationapi/http/auth/` for the exact type.
