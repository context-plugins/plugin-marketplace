...
- **`dotnet-cellpoint-sdk`** is the single agent for every SDK need. It grounds in a bundled SDK map
...
Your FIRST action is to spawn **`dotnet-cellpoint-sdk`** once, with the user's full request (all
features in scope — one spawn covers the whole implementation). Dictate the output path in
the brief: the absolute path where it writes the plan (`<project repo root>/cellpoint-dotnet-plan.md`) —
do not let it pick its own location.

It writes `cellpoint-dotnet-plan.md` (plan + contract sheet) and returns its path.

**Parallelize the wait — prerequisites only.** Repo reconnaissance (where the integration
lands in this codebase) is not the `dotnet-cellpoint-sdk` agent's job: kick it off in the SAME message
...
- Blocker or major assumption → surface it to the user in plain language, get their answer,
  send the clarification to the EXISTING `dotnet-cellpoint-sdk` agent (re-spawn only if it is gone). It
...
ask the warm `dotnet-cellpoint-sdk` agent, never guess.

### Step 2 — Implement from the contract sheet

1. Read `cellpoint-dotnet-plan.md` once. Treat its contracts as authoritative — do not re-derive or
...
2. Implement sequentially, following the repo's own conventions and layering. You loaded the
   ...
   contract sheet or the warm `dotnet-cellpoint-sdk` agent — never re-derive one from a companion.
...
4. **Any compile or runtime error involving an SDK type or member** (`CS1061`, `CS0117`,
   `CS0234`, `CS0104`, `CS1503`, `CS7036`, … on `CellPointApi.*`, or a provider error
   at runtime) → send the exact error output and the files involved to your EXISTING
   `dotnet-cellpoint-sdk` agent (a follow-up message, NOT a new spawn), and wait. It fixes the code in
...
**Never create or edit a project file while `dotnet-cellpoint-sdk` is running** — it edits files in
...
- **Don't re-derive or double-check a sheet row from memory.** If you are unsure what a row
  said, batch your questions into ONE message to the warm `dotnet-cellpoint-sdk` agent — it holds the map
  warm and answers in seconds.
- **Never spawn a second `dotnet-cellpoint-sdk` agent.** One spawn per session; everything after is a
