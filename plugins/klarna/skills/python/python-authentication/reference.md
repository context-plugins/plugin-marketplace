...
kwargs to the `KlarnaClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `klarna/klarna_client.py` and `klarna/http/auth/` in
the cloned source.
...
```python
from klarna.http.auth.api_key import ApiKeyCredentials
...
```python
from klarna.http.auth.api_header import ApiHeaderCredentials
...
```python
from klarna.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from klarna.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from klarna.http.auth.o_auth_acg import OAuthACGCredentials
from klarna.models.o_auth_scope_klarna_enum import OAuthScope{Pkg}Enum
...
client = KlarnaClient(o_auth_acg_credentials=creds)
...
```python
from klarna.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = KlarnaClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `KlarnaClient.from_environment()`) read each credential
...
Grep `from_environment` in `klarna/configuration.py` and each `klarna/http/auth/*.py` for the exact
