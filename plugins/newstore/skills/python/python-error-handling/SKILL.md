...
`newstore/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `newstore/exceptions/` and operation docstrings.
...
```python
from newstore.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `newstore/exceptions/`.
...
```python
from newstore.exceptions.api_exception import APIException
from newstore.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`newstore/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`newstore/exceptions/` for the class.
...
`newstore/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `newstore/exceptions/`
  or a `ValueError` — check the auth handler in `newstore/http/auth/` for the exact type.
