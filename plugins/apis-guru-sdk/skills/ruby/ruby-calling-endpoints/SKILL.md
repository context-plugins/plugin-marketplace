...
Open `lib/apis_guru/client.rb` for the accessor names, then the relevant
`lib/apis_guru/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = APIsGuru::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: APIsGuru::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/apis_guru/models/`. Field names are defined as
...
Read the enum module in `lib/apis_guru/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/apis_guru/client.rb`.
- Operation methods are in `lib/apis_guru/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/apis_guru/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/apis_guru/exceptions/`.
