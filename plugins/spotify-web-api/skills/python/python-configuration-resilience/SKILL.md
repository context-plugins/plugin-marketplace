...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `SpotifywebapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`spotifywebapi/configuration.py` in the cloned source.
...
`spotifywebapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`spotifywebapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `spotifywebapi/configuration.py`
...
`NumberPagedResponse`) live in `spotifywebapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `spotifywebapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
