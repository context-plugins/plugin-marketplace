...
> `{operation}`, `{apiGroup}`, `FirecrawlLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `FirecrawlLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use FirecrawlLib\Exceptions\ApiException;
...
```php
use FirecrawlLib\Exceptions\ApiException;
use FirecrawlLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `FirecrawlLib\Exceptions\` — read the source to
