...
client = stub_client(stubs)
assert_raises 1ForgeFinanceAPIs::APIException do
...
def build_test_client
  1ForgeFinanceAPIs::Client.new(
    environment: 1ForgeFinanceAPIs::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error 1ForgeFinanceAPIs::APIException
```
...
```ruby
expect { call }.to raise_error(1ForgeFinanceAPIs::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < 1ForgeFinanceAPIs::HttpCallBack
...
client  = 1ForgeFinanceAPIs::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
