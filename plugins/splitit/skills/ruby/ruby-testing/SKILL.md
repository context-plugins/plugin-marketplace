...
client = stub_client(stubs)
assert_raises Splitit::APIException do
...
BASE = Splitit::Configuration.new(environment: Splitit::Environment::PRODUCTION).get_base_uri

def build_test_client
  Splitit::Client.new(
    environment: Splitit::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Splitit::APIException
```
...
```ruby
expect { call }.to raise_error(Splitit::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Splitit::HttpCallBack
...
client  = Splitit::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
