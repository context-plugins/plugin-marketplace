...
> `{ApiGroup}`, `PazeCheckoutApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `PazeCheckoutApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `PazeCheckoutApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `PazeCheckoutApi.Errors`
...
```csharp
using PazeCheckoutApi.Core.Exceptions;     // SdkException<TError>
using PazeCheckoutApi.Core.ErrorResponse;  // ApiError, RawError
using PazeCheckoutApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`PazeCheckoutApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using PazeCheckoutApi.Core.Exceptions;     // SdkException<TError>
using PazeCheckoutApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `PazeCheckoutApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `PazeCheckoutApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using PazeCheckoutApi.Core.Models;        // ApiResult<TResponse, TError>
using PazeCheckoutApi.Core.ErrorResponse; // RawError
using PazeCheckoutApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `PazeCheckoutApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
