    ...
    tesserapiv1.CreateConfiguration(
        tesserapiv1.WithHttpConfiguration(
            tesserapiv1.CreateHttpConfiguration(
                tesserapiv1.WithTimeout(30),
                tesserapiv1.WithRetryConfiguration(
                    tesserapiv1.CreateRetryConfiguration(
                        tesserapiv1.WithMaxRetryAttempts(3),
                        tesserapiv1.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := tesserapiv1.CreateRetryConfiguration(
    tesserapiv1.WithMaxRetryAttempts(5),
    tesserapiv1.WithRetryInterval(1),
    tesserapiv1.WithBackoffFactor(2),
    tesserapiv1.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    tesserapiv1.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := tesserapiv1.NewClient(
    tesserapiv1.CreateConfiguration(
        tesserapiv1.WithHttpConfiguration(
            tesserapiv1.CreateHttpConfiguration(
                tesserapiv1.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
