...
kwargs to the `VerizonClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `verizon/verizon_client.py` and `verizon/http/auth/` in
the cloned source.

## Basic auth

```python
from verizon.http.auth.basic_auth import BasicAuthCredentials
...
```python
from verizon.http.auth.api_key import ApiKeyCredentials
...
```python
from verizon.http.auth.api_header import ApiHeaderCredentials
...
```python
from verizon.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from verizon.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from verizon.http.auth.o_auth_acg import OAuthACGCredentials
from verizon.models.o_auth_scope_verizon_enum import OAuthScope{Pkg}Enum
...
client = VerizonClient(o_auth_acg_credentials=creds)
...
```python
from verizon.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = VerizonClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `VerizonClient.from_environment()`) read each credential
...
Grep `from_environment` in `verizon/configuration.py` and each `verizon/http/auth/*.py` for the exact
