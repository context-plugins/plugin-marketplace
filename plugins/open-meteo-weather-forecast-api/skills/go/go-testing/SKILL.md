...
`testing` + `testify/require` for reference; substitute the real `openmeteoweatherforecastapi` and names.
...
import (
    ...
    "github.com/context-plugins/open-meteo-weather-forecast-api-go-sdk"   // root package openmeteoweatherforecastapi
)
...
func clientReturning(status int, body string) (openmeteoweatherforecastapi.ClientInterface, *stubTransport) {
    ...
    client := openmeteoweatherforecastapi.NewClient(
        openmeteoweatherforecastapi.CreateConfiguration(
            openmeteoweatherforecastapi.WithHttpConfiguration(
                openmeteoweatherforecastapi.CreateHttpConfiguration(openmeteoweatherforecastapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/open-meteo-weather-forecast-api-go-sdk/errors"
...
client := openmeteoweatherforecastapi.NewClient(
    openmeteoweatherforecastapi.CreateConfiguration(
        openmeteoweatherforecastapi.WithHttpConfiguration(
            openmeteoweatherforecastapi.CreateHttpConfiguration(openmeteoweatherforecastapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
