...
- `gitea` — the root package name (e.g. `multiauthsample`, `batester`).
- `GiteaapiClient` — the generated client class (read it from the `class …Client` declaration in `gitea/gitea_client.py`).
...
```python
from gitea.gitea_client import GiteaapiClient
from gitea.configuration import Environment
from gitea.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from gitea.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `gitea/gitea_client.py` in the cloned source.
...
`from_environment` in `gitea/configuration.py` for the exact names.
...
Open `gitea/gitea_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `gitea/configuration.py`. **Read the enum in `configuration.py`
...
`gitea/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
