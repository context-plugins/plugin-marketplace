---
name: dotnet-x-api-v2-getting-started
description: X API v2 .NET SDK identity and lookup layer for the dotnet-x-api-v2-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the X API v2 .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-x-api-v2-sdk` helper
...
| --- | --- |
| API | X API v2 |
| Source repo | https://github.com/context-plugins/x-api-v2-csharp-sdk (branch `main`) |
| Root namespace | `XApiV2` (the `using` namespace) |
| Client class | `XApiV2Client` |
| Options class | `XApiV2ClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `XApiV2ClientOptions`: `BearerToken: string?`, `Oauth2UserToken: OAuth2AuthorizationCodeCredentials?`, `Oauth2UserTokenTokenStrategy: IOAuth2RefreshableTokenStrategy<OAuth2AuthorizationCodeCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using XApiV2.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/x-api-v2-csharp-sdk
dotnet add reference x-api-v2-csharp-sdk/XApiV2.csproj
```
...
temp directory** (`<temp>/x-api-v2-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/x-api-v2-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/x-api-v2-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/x-api-v2-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\x-api-v2-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/x-api-v2-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/x-api-v2-dotnet-src/`, **never** in the project repo, and its path never goes into
`x-api-v2-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `XApiV2Client.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new XApiV2Client(...)`, build its options, or DI-register via
   `AddXApiV2Client`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `XApiV2ClientOptions` — the map's *Servers & auth* section
