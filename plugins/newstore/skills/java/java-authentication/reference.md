...
against the `NewStoreAPIClient.Builder` setters, the `net.newstore.p.dodicidemo.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `NewStoreAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
NewStoreAPIClient client = new NewStoreAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeNewstoreEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
