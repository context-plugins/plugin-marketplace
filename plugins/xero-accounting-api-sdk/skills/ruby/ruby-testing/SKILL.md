...
client = stub_client(stubs)
assert_raises XeroAccountingAPI::APIException do
...
def build_test_client
  XeroAccountingAPI::Client.new(
    environment: XeroAccountingAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error XeroAccountingAPI::APIException
```
...
```ruby
expect { call }.to raise_error(XeroAccountingAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < XeroAccountingAPI::HttpCallBack
...
client  = XeroAccountingAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
