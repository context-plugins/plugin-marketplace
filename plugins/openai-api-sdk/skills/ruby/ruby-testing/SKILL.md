...
client = stub_client(stubs)
assert_raises OpenAIAPI::APIException do
...
def build_test_client
  OpenAIAPI::Client.new(
    environment: OpenAIAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error OpenAIAPI::APIException
```
...
```ruby
expect { call }.to raise_error(OpenAIAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < OpenAIAPI::HttpCallBack
...
client  = OpenAIAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
