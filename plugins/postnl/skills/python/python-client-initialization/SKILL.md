...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the PostnlecommerceClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write PostnlecommerceClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `PostnlecommerceClient` is the SDK's client class — **read the real name** from `postnl/postnl_client.py`
...
- `postnl` — the root package name (e.g. `multiauthsample`, `batester`).
- `PostnlecommerceClient` — the generated client class (read it from the `class …Client` declaration in `postnl/postnl_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `PostnlecommerceClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from postnl.postnl_client import PostnlecommerceClient
from postnl.configuration import Environment
from postnl.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from postnl.configuration import Configuration, Environment
...
client = PostnlecommerceClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `postnl/postnl_client.py` in the cloned source.
...
`from_environment()` is a class method on both `PostnlecommerceClient` and `Configuration` that reads
...
`from_environment` in `postnl/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `PostnlecommerceClient`. Access them as attributes —
...
Open `postnl/postnl_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `postnl/configuration.py`. **Read the enum in `configuration.py`
...
client = PostnlecommerceClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`postnl/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = PostnlecommerceClient(config=new_config)
```
