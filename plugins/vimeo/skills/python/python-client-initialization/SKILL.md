...
- `vimeo` — the root package name (e.g. `multiauthsample`, `batester`).
- `VimeoapiClient` — the generated client class (read it from the `class …Client` declaration in `vimeo/vimeo_client.py`).
...
```python
from vimeo.vimeo_client import VimeoapiClient
from vimeo.configuration import Environment
from vimeo.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from vimeo.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `vimeo/vimeo_client.py` in the cloned source.
...
`from_environment` in `vimeo/configuration.py` for the exact names.
...
Open `vimeo/vimeo_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `vimeo/configuration.py`. **Read the enum in `configuration.py`
...
`vimeo/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
