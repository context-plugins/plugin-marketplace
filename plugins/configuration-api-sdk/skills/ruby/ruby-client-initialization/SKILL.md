...
- `ConfigurationAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `configuration_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'configuration_api'
include ConfigurationAPI
...
The authoritative list is the `initialize` signature in `lib/configuration_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/configuration_api/configuration.rb`
(`build_default_config_from_env`) and `lib/configuration_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/configuration_api.rb`) and store it in a
