...
def stub_client(stubs)
  ...
  AdobeExperienceManagerAEMAPI::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises AdobeExperienceManagerAEMAPI::APIException do
...
def build_test_client
  AdobeExperienceManagerAEMAPI::Client.new(
    environment: AdobeExperienceManagerAEMAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error AdobeExperienceManagerAEMAPI::APIException
```
...
```ruby
# Base APIException:
rescue AdobeExperienceManagerAEMAPI::APIException => e
  assert_equal 404, e.response_code
  assert_includes e.reason, 'Not Found'

# Typed subclass (e.g. OAuthProviderException):
rescue AdobeExperienceManagerAEMAPI::OAuthProviderException => e
...
```ruby
expect { call }.to raise_error(AdobeExperienceManagerAEMAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < AdobeExperienceManagerAEMAPI::HttpCallBack
...
client  = AdobeExperienceManagerAEMAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
