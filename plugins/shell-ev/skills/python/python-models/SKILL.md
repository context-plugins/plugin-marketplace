...
your SDK source (`shellev/models/`).
...
`UnionTypeLookUp` registry (in `shellev/utilities/union_type_lookup.py`) and each candidate model
...
```python
from shellev.models.animal import Cat, Dog
...
```python
from shellev.models.animal import Animal, Cat, Dog
