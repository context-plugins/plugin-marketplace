...
│   └── com.rapidapi.p.medium2.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.rapidapi.p.medium2.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.rapidapi.p.medium2.http.client.HttpContext`, which carries:
