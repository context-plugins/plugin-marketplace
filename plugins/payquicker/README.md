# Payquicker SDK Plugin

Integrate and consume the **APIMatic-generated Payquicker SDK** (the "APIMATIC v3.0" generic
libraries, built on `apimatic-*-core` / `apimatic/*-core-runtime` runtimes) across C#/.NET, Java, PHP, Python, Ruby, Go.
...
(`dotnet-payquicker-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-payquicker`** router skill routes all
C#/.NET SDK work to the single **`dotnet-payquicker-sdk`** agent (.NET-only), which answers
