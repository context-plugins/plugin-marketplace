...
> Throughout, `vimeo` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := vimeo.NewClient(
    vimeo.CreateConfiguration(
        vimeo.WithBasicAuthCredentials(
            vimeo.NewBasicAuthCredentials(
...
vimeo.WithApiKeyCredentials(vimeo.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
vimeo.WithApiHeaderCredentials(vimeo.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
vimeo.WithOAuthBearerTokenCredentials(
    vimeo.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
