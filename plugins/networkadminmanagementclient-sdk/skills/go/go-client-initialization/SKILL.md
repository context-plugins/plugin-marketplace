...
- `networkadminmanagementclient` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/networkadminmanagementclient-go` — the Go import path for the root package; `github.com/context-plugins/networkadminmanagementclient-go/models` for request/response types.
...
```go
import "github.com/context-plugins/networkadminmanagementclient-go"

client := networkadminmanagementclient.NewClient(
    networkadminmanagementclient.CreateConfiguration(
        networkadminmanagementclient.WithEnvironment(networkadminmanagementclient.PRODUCTION),
...
from environment variables named `NETWORKADMINMANAGEMENTCLIENT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := networkadminmanagementclient.NewClient(networkadminmanagementclient.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `networkadminmanagementclient.PRODUCTION`,
`networkadminmanagementclient.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := networkadminmanagementclient.NewClient(
    networkadminmanagementclient.CreateConfiguration(networkadminmanagementclient.WithEnvironment(networkadminmanagementclient.PRODUCTION)),
)
```
...
client := networkadminmanagementclient.NewClient(
    networkadminmanagementclient.CreateConfiguration(
        networkadminmanagementclient.WithHttpConfiguration(
            networkadminmanagementclient.CreateHttpConfiguration(
                networkadminmanagementclient.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                networkadminmanagementclient.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = networkadminmanagementclient.NewClient(networkadminmanagementclient.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api networkadminmanagementclient.ClientInterface
}

func NewService(api networkadminmanagementclient.ClientInterface) *Service {
