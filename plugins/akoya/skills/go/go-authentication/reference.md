...
```go
akoya.WithOAuthCCGCredentials(
    akoya.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr akoya.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := akoya.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr akoya.OAuthCCGManager) models.OAuthToken {
...
```go
creds := akoya.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := akoya.NewClient(akoya.CreateConfiguration(akoya.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    akoya.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := akoya.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := akoya.NewClient(akoya.CreateConfiguration(akoya.WithOAuthROPCGCredentials(creds)))
...
```go
akoya.WithOAuthBearerTokenCredentials(akoya.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
akoya.WithBasicAuthCredentials(akoya.NewBasicAuthCredentials(username, password))
akoya.WithApiHeaderCredentials(akoya.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
akoya.WithApiKeyCredentials(akoya.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `AKOYA_...` variables — e.g.
`AKOYA_USERNAME`, `AKOYA_PASSWORD`, `AKOYA_TOKEN`, `AKOYA_API_KEY`,
`AKOYA_O_AUTH_CLIENT_ID`, `AKOYA_O_AUTH_CLIENT_SECRET`, `AKOYA_ACCESS_TOKEN`. Grep the
