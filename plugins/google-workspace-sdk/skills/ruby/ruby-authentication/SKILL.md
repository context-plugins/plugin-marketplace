...
> Throughout, `GoogleWorkspace` and other `{...}` tokens are placeholders for names you take from your SDK —
...
`lib/google_workspace/client.rb` and the `{scheme}_credentials:` kwargs — those are the source of truth. An
...
`initialize` signature in `lib/google_workspace/http/auth/api_key.rb` and the `doc/auth/custom-query-parameter.md`
...
Sends the values as custom request headers. Check `lib/google_workspace/http/auth/api_header.rb` and
