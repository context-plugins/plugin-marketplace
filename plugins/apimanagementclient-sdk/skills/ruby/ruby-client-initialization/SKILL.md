...
- `ApiManagementClient` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `apimanagementclient` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'apimanagementclient'
include ApiManagementClient
...
The authoritative list is the `initialize` signature in `lib/apimanagementclient/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/apimanagementclient/configuration.rb`
(`build_default_config_from_env`) and `lib/apimanagementclient/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/apimanagementclient.rb`) and store it in a
