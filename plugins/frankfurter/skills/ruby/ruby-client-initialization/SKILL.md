...
- `Frankfurter` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `frankfurter` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/frankfurter/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/frankfurter/configuration.rb`
(`build_default_config_from_env`) and `lib/frankfurter/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = Frankfurter::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/frankfurter.rb`) and store it in a
