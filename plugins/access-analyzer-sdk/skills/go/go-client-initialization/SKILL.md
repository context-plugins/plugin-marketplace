...
- `accessanalyzer` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/access-analyzer-go` — the Go import path for the root package; `github.com/context-plugins/access-analyzer-go/models` for request/response types.
...
```go
import "github.com/context-plugins/access-analyzer-go"

client := accessanalyzer.NewClient(
    accessanalyzer.CreateConfiguration(
        accessanalyzer.WithEnvironment(accessanalyzer.PRODUCTION),
...
from environment variables named `ACCESSANALYZER_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := accessanalyzer.NewClient(accessanalyzer.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `accessanalyzer.PRODUCTION`,
`accessanalyzer.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := accessanalyzer.NewClient(
    accessanalyzer.CreateConfiguration(accessanalyzer.WithEnvironment(accessanalyzer.PRODUCTION)),
)
```
...
client := accessanalyzer.NewClient(
    accessanalyzer.CreateConfiguration(
        accessanalyzer.WithHttpConfiguration(
            accessanalyzer.CreateHttpConfiguration(
                accessanalyzer.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                accessanalyzer.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = accessanalyzer.NewClient(accessanalyzer.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api accessanalyzer.ClientInterface
}

func NewService(api accessanalyzer.ClientInterface) *Service {
