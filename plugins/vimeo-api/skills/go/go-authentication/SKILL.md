...
> Throughout, `vimeoapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := vimeoapi.NewClient(
    vimeoapi.CreateConfiguration(
        vimeoapi.WithBasicAuthCredentials(
            vimeoapi.NewBasicAuthCredentials(
...
vimeoapi.WithApiKeyCredentials(vimeoapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
vimeoapi.WithApiHeaderCredentials(vimeoapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
vimeoapi.WithOAuthBearerTokenCredentials(
    vimeoapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := vimeoapi.NewClient(
    vimeoapi.CreateConfiguration(
        vimeoapi.WithOAuthCCGCredentials(
            vimeoapi.NewOAuthCCGCredentials(
