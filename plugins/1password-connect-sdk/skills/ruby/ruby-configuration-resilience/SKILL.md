...
```ruby
client = 1PasswordConnect::Client.new(
  max_retries:    3,
...
```ruby
client = 1PasswordConnect::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = 1PasswordConnect::Client.new(adapter: :net_http)
```
...
client = 1PasswordConnect::Client.new(connection: conn)
```
...
client = 1PasswordConnect::Client.new(environment: Environment::PRODUCTION)
client = 1PasswordConnect::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = 1PasswordConnect::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = 1PasswordConnect::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = 1PasswordConnect::ProxySettings.from_env
client = 1PasswordConnect::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < 1PasswordConnect::HttpCallBack
...
client = 1PasswordConnect::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = 1PasswordConnect::Client.from_env
# Overrides take precedence over env vars:
client = 1PasswordConnect::Client.from_env(timeout: 30, max_retries: 3)
```
