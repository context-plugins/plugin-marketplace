---
name: dotnet-shell-cardmanagement-getting-started
description: shell-cardmanagement .NET SDK identity and lookup layer for the dotnet-shell-cardmanagement-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the shell-cardmanagement .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-shell-cardmanagement-sdk` helper
...
| | |
| --- | --- |
| API | shell-cardmanagement |
| Source repo | https://github.com/context-plugins/shell-cardmanagement-csharp-sdk (branch `main`) |
| Root namespace | `ShellCardManagementApis` (the `using` namespace) |
| Client class | `ShellCardManagementApisClient` |
| Options class | `ShellCardManagementApisClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `ShellCardManagementApisClientOptions`: `BearerToken: OAuth2ClientCredentials?`, `BearerTokenTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
...
namespaces transitively, so `using ShellCardManagementApis.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/shell-cardmanagement-csharp-sdk
dotnet add reference shell-cardmanagement-csharp-sdk/ShellCardManagementApis.csproj
```
...
temp directory** (`<temp>/shell-cardmanagement-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/shell-cardmanagement-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/shell-cardmanagement-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/shell-cardmanagement-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\shell-cardmanagement-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/shell-cardmanagement-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/shell-cardmanagement-dotnet-src/`, **never** in the project repo, and its path never goes into
`shell-cardmanagement-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `ShellCardManagementApisClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new ShellCardManagementApisClient(...)`, build its options, or DI-register via
   `AddShellCardManagementApisClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `ShellCardManagementApisClientOptions` — the map's *Servers & auth* section
