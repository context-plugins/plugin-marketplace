...
`seatmapdisplay/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `seatmapdisplay/exceptions/` and operation docstrings.
...
```python
from seatmapdisplay.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `seatmapdisplay/exceptions/`.
...
```python
from seatmapdisplay.exceptions.api_exception import APIException
from seatmapdisplay.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`seatmapdisplay/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`seatmapdisplay/exceptions/` for the class.
...
`seatmapdisplay/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `seatmapdisplay/exceptions/`
  or a `ValueError` — check the auth handler in `seatmapdisplay/http/auth/` for the exact type.
