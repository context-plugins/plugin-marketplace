...
`boku/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `boku/exceptions/` and operation docstrings.
...
```python
from boku.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `boku/exceptions/`.
...
```python
from boku.exceptions.api_exception import APIException
from boku.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`boku/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`boku/exceptions/` for the class.
...
`boku/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `boku/exceptions/`
  or a `ValueError` — check the auth handler in `boku/http/auth/` for the exact type.
