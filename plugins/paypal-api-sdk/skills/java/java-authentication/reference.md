...
against the `PayPalAPIClient.Builder` setters, the `com.paypal.sandbox.apim.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `PayPalAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
PayPalAPIClient client = new PayPalAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopePayPalAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
