...
Open `restapi/restapi_client.py` for the controller property names, then the relevant
`restapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from restapi.models.{request_model} import {RequestModel}
...
Open `restapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
an iterator over the items on that page. Inspect `restapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `RestapiClient` in `restapi/restapi_client.py`.
- Operation method signatures are in `restapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `restapi/models/`; enum types have `from_value()`.
