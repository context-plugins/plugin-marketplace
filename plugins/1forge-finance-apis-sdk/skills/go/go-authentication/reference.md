...
```go
1forgefinanceapis.WithOAuthCCGCredentials(
    1forgefinanceapis.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr 1forgefinanceapis.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := 1forgefinanceapis.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr 1forgefinanceapis.OAuthCCGManager) models.OAuthToken {
...
```go
creds := 1forgefinanceapis.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := 1forgefinanceapis.NewClient(1forgefinanceapis.CreateConfiguration(1forgefinanceapis.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    1forgefinanceapis.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := 1forgefinanceapis.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := 1forgefinanceapis.NewClient(1forgefinanceapis.CreateConfiguration(1forgefinanceapis.WithOAuthROPCGCredentials(creds)))
...
```go
1forgefinanceapis.WithOAuthBearerTokenCredentials(1forgefinanceapis.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
1forgefinanceapis.WithBasicAuthCredentials(1forgefinanceapis.NewBasicAuthCredentials(username, password))
1forgefinanceapis.WithApiHeaderCredentials(1forgefinanceapis.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
1forgefinanceapis.WithApiKeyCredentials(1forgefinanceapis.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `1FORGEFINANCEAPIS_...` variables — e.g.
`1FORGEFINANCEAPIS_USERNAME`, `1FORGEFINANCEAPIS_PASSWORD`, `1FORGEFINANCEAPIS_TOKEN`, `1FORGEFINANCEAPIS_API_KEY`,
`1FORGEFINANCEAPIS_O_AUTH_CLIENT_ID`, `1FORGEFINANCEAPIS_O_AUTH_CLIENT_SECRET`, `1FORGEFINANCEAPIS_ACCESS_TOKEN`. Grep the
