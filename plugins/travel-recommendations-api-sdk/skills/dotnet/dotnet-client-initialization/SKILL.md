...
- `TravelRecommendationsApiClientOptions` — its options class.
- `TravelRecommendationsApi` — the SDK's root namespace, used in `using` directives. This can differ from the NuGet
...
```csharp
public TravelRecommendationsApiClient(HttpClient httpClient, TravelRecommendationsApiClientOptions options)
```
...
```csharp
public class TravelRecommendationsApiClientOptions
{
...
```csharp
using TravelRecommendationsApi;
using TravelRecommendationsApi.Servers;

var options = new TravelRecommendationsApiClientOptions
{
...
var client = new TravelRecommendationsApiClient(httpClient, options);
```
...
Every APIMatic .NET SDK ships a `ServiceCollection` extension named `AddTravelRecommendationsApiClient`, which registers the
...
builder.Services.AddTravelRecommendationsApiClient(options =>
{
...
```csharp
public sealed class MyService(TravelRecommendationsApiClient client)
{
