...
- `telegrambotapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/telegram-bot-api-go` — the Go import path for the root package; `github.com/context-plugins/telegram-bot-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/telegram-bot-api-go"

client := telegrambotapi.NewClient(
    telegrambotapi.CreateConfiguration(
        telegrambotapi.WithEnvironment(telegrambotapi.PRODUCTION),
...
from environment variables named `TELEGRAMBOTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := telegrambotapi.NewClient(telegrambotapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `telegrambotapi.PRODUCTION`,
`telegrambotapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := telegrambotapi.NewClient(
    telegrambotapi.CreateConfiguration(telegrambotapi.WithEnvironment(telegrambotapi.PRODUCTION)),
)
```
...
client := telegrambotapi.NewClient(
    telegrambotapi.CreateConfiguration(
        telegrambotapi.WithHttpConfiguration(
            telegrambotapi.CreateHttpConfiguration(
                telegrambotapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                telegrambotapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = telegrambotapi.NewClient(telegrambotapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api telegrambotapi.ClientInterface
}

func NewService(api telegrambotapi.ClientInterface) *Service {
