...
- `sqlmanagementclient` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/sqlmanagementclient-go` — the Go import path for the root package; `github.com/context-plugins/sqlmanagementclient-go/models` for request/response types.
...
```go
import "github.com/context-plugins/sqlmanagementclient-go"

client := sqlmanagementclient.NewClient(
    sqlmanagementclient.CreateConfiguration(
        sqlmanagementclient.WithEnvironment(sqlmanagementclient.PRODUCTION),
...
from environment variables named `SQLMANAGEMENTCLIENT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := sqlmanagementclient.NewClient(sqlmanagementclient.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `sqlmanagementclient.PRODUCTION`,
`sqlmanagementclient.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := sqlmanagementclient.NewClient(
    sqlmanagementclient.CreateConfiguration(sqlmanagementclient.WithEnvironment(sqlmanagementclient.PRODUCTION)),
)
```
...
client := sqlmanagementclient.NewClient(
    sqlmanagementclient.CreateConfiguration(
        sqlmanagementclient.WithHttpConfiguration(
            sqlmanagementclient.CreateHttpConfiguration(
                sqlmanagementclient.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                sqlmanagementclient.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = sqlmanagementclient.NewClient(sqlmanagementclient.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api sqlmanagementclient.ClientInterface
}

func NewService(api sqlmanagementclient.ClientInterface) *Service {
