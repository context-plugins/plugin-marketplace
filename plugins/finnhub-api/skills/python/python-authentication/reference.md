...
kwargs to the `FinnhubapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `finnhubapi/finnhubapi_client.py` and `finnhubapi/http/auth/` in
the cloned source.

## Basic auth

```python
from finnhubapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from finnhubapi.http.auth.api_key import ApiKeyCredentials
...
```python
from finnhubapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from finnhubapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from finnhubapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from finnhubapi.http.auth.o_auth_acg import OAuthACGCredentials
from finnhubapi.models.o_auth_scope_finnhubapi_enum import OAuthScope{Pkg}Enum
...
client = FinnhubapiClient(o_auth_acg_credentials=creds)
...
```python
from finnhubapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = FinnhubapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `FinnhubapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `finnhubapi/configuration.py` and each `finnhubapi/http/auth/*.py` for the exact
