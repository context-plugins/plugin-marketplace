...
```typescript
import { {Operation}Error } from 'fund-apilib/errors';
...
```typescript
import { ApiError } from 'fund-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(FundAPIClient)
    .useValue(new FundAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
