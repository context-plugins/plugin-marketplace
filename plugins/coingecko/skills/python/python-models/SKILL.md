...
your SDK source (`coingecko/models/`).
...
```python
from coingecko.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `coingecko/utilities/union_type_lookup.py`) and each candidate model
...
```python
from coingecko.models.animal import Cat, Dog
...
```python
from coingecko.models.animal import Animal, Cat, Dog
