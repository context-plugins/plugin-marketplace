...
- `univa` — the root package name (e.g. `multiauthsample`, `batester`).
- `UnivapayClientSdkClient` — the generated client class (read it from the `class …Client` declaration in `univa/univa_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `UnivapayClientSdkClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from univa.univa_client import UnivapayClientSdkClient
from univa.configuration import Environment
from univa.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from univa.configuration import Configuration, Environment
...
client = UnivapayClientSdkClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `univa/univa_client.py` in the cloned source.
...
`from_environment()` is a class method on both `UnivapayClientSdkClient` and `Configuration` that reads
...
`from_environment` in `univa/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `UnivapayClientSdkClient`. Access them as attributes —
...
Open `univa/univa_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `univa/configuration.py`. **Read the enum in `configuration.py`
...
client = UnivapayClientSdkClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`univa/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = UnivapayClientSdkClient(config=new_config)
```
