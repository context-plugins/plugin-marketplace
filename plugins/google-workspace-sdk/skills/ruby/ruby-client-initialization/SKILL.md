...
- `GoogleWorkspace` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `google_workspace` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/google_workspace/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/google_workspace/configuration.rb`
(`build_default_config_from_env`) and `lib/google_workspace/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/google_workspace.rb`) and store it in a
