...
```ruby
client = FlightPriceAnalysisAPI::Client.new(
  max_retries:    3,
...
```ruby
client = FlightPriceAnalysisAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = FlightPriceAnalysisAPI::Client.new(adapter: :net_http)
```
...
client = FlightPriceAnalysisAPI::Client.new(connection: conn)
```
...
client = FlightPriceAnalysisAPI::Client.new(environment: Environment::PRODUCTION)
client = FlightPriceAnalysisAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = FlightPriceAnalysisAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = FlightPriceAnalysisAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = FlightPriceAnalysisAPI::ProxySettings.from_env
client = FlightPriceAnalysisAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < FlightPriceAnalysisAPI::HttpCallBack
...
client = FlightPriceAnalysisAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = FlightPriceAnalysisAPI::Client.from_env
# Overrides take precedence over env vars:
client = FlightPriceAnalysisAPI::Client.from_env(timeout: 30, max_retries: 3)
```
