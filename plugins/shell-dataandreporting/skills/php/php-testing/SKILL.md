...
> `ShellDataReportingAPIsClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): ShellDataReportingAPIsClient
{
...
```php
use ShellDataReportingAPIsLib\Exceptions\{OperationException};
use ShellDataReportingAPIsLib\Exceptions\ApiException;
...
app()->instance(ShellDataReportingAPIsClient::class, clientWithResponses([
