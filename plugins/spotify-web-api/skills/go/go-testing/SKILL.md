...
`testing` + `testify/require` for reference; substitute the real `spotifywebapi` and names.
...
import (
    ...
    "github.com/context-plugins/spotify-web-api-go-sdk"   // root package spotifywebapi
)
...
func clientReturning(status int, body string) (spotifywebapi.ClientInterface, *stubTransport) {
    ...
    client := spotifywebapi.NewClient(
        spotifywebapi.CreateConfiguration(
            spotifywebapi.WithHttpConfiguration(
                spotifywebapi.CreateHttpConfiguration(spotifywebapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/spotify-web-api-go-sdk/errors"
...
client := spotifywebapi.NewClient(
    spotifywebapi.CreateConfiguration(
        spotifywebapi.WithHttpConfiguration(
            spotifywebapi.CreateHttpConfiguration(spotifywebapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
