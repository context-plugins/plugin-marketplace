...
description: Construct and configure an APIMatic-generated Java SDK client — build via the inner ShutterstockApiExplorerClient.Builder() class (never a public constructor), set httpClientConfig via a lambda (not a config object), choose an Environment constant, access controllers via client.get{Resource}Api(), call client.shutdown() at process exit, clone a live client with client.newBuilder(), and wire the client into Spring or plain-constructor DI. Use the moment you call new ShutterstockApiExplorerClient.Builder(), pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the arguments but not the builder pattern, the httpClientConfig lambda, or the lifetime/reuse rules.
---

# Initializing an APIMatic Java SDK client

> `ShutterstockApiExplorerClient` is the SDK's client class — **read the real name** from the `*Client.java` file in
...
- `ShutterstockApiExplorerClient` — the gateway class, named after the API (read it from the `*Client.java` file in the root package).
- `com.shutterstock.api` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`ShutterstockApiExplorerClient.Builder` class. A minimal initialization:
...
The `Builder` is the only entry point — confirm its fluent methods in `ShutterstockApiExplorerClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `ShutterstockApiExplorerClient.java` |

Call `.build()` to produce an immutable `ShutterstockApiExplorerClient` instance.
...
public class ApiConfig {
    @Bean
    public ShutterstockApiExplorerClient apiClient() {
        return new ShutterstockApiExplorerClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired ShutterstockApiExplorerClient client) {
        client.shutdown();
    }
}
```

Inject `ShutterstockApiExplorerClient` into your services as a normal Spring dependency. Build only one bean.
