...
against the `AuthentiqAPIClient.Builder` setters, the `com.appspot.m6dotauthentiqio.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `AuthentiqAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
AuthentiqAPIClient client = new AuthentiqAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeAuthentiqAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new AuthentiqAPIClient.Builder()
