...
| Install | `pip install "git+https://github.com/context-plugins/1password-connect-python@main"` |
| Client class | `1PasswordConnectClient` in `1passwordconnect/1passwordconnect_client.py`, constructed with keyword arguments |
| Configuration class | `Configuration` in `1passwordconnect/configuration.py` (subclasses `HttpClientConfiguration`) |
...
| Controllers | `@LazyProperty` properties on `1PasswordConnectClient` (e.g. `client.authentication`, `client.transaction`) |
| Return type | operations return a typed model or primitive directly; paginated operations return `PagedIterable` |
| Base exception | `APIException` in `1passwordconnect/exceptions/api_exception.py` — raised on non-2xx |
...
- `1passwordconnect/1passwordconnect_client.py` — the `1PasswordConnectClient` class: `__init__` keyword args, `from_environment()`
  class method, `@LazyProperty` controller property accessors, and (for OAuth) auth manager properties.
- `1passwordconnect/configuration.py` — `Configuration` class (subclasses `HttpClientConfiguration`),
  `Environment` enum, `Server` enum, `get_base_uri()`, `from_environment()`, and `clone_with()`.
- `1passwordconnect/api_helper.py` — `APIHelper` (subclasses `CoreApiHelper`): `json_deserialize`,
  `json_serialize`, `SKIP` sentinel for optional model fields, datetime helpers.
- `1passwordconnect/controllers/` — one `{Resource}Controller` per API resource group, all inheriting from
  `base_controller.py`; operations are synchronous methods returning typed values or raising `APIException`.
- `1passwordconnect/models/` — model classes with `from_dictionary(cls, dict)`, `_names` / `_optionals` class
  attributes, and `additional_properties`; enum classes with `MEMBER = value` attributes.
- `1passwordconnect/exceptions/` — `APIException` base and typed subclasses (when the API documents error
  schemas); subclasses parse their body fields in `unbox()`.
- `1passwordconnect/http/` — `HttpCallBack` (the test seam), `HttpResponse`, `HttpRequest`, `HttpMethodEnum`,
  auth handlers under `auth/`, and `proxy_settings.py`.
- `1passwordconnect/utilities/` — `FileWrapper` for multipart uploads; `union_type_lookup.py` (when the API
  uses oneOf/anyOf).
- `1passwordconnect/logging/` — `LoggingConfiguration`, `RequestLoggingConfiguration`,
  `ResponseLoggingConfiguration` (generated when the API enables SDK logging).

## Install

```bash
pip install "git+https://github.com/context-plugins/1password-connect-python@main"
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/1password-connect-python /tmp/1password-connect-python-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/1password-connect-python "$env:TEMP\1password-connect-python-src"
```
...
- `1passwordconnect/configuration.py` — `Configuration.__init__` kwargs, `Environment` enum values, `Server`
  enum, env-var names in `from_environment()`, and the base-URL template in `environments`.
- `1passwordconnect/1passwordconnect_client.py` — `1PasswordConnectClient.__init__` kwargs, `from_environment()`, and the
  `@LazyProperty` controller property names.
- `1passwordconnect/controllers/*.py` — operation method signatures, parameter names, and return types.
- `1passwordconnect/models/` — model `__init__` args, `_names`, `_optionals`, `from_dictionary()`, enum values.
- `1passwordconnect/exceptions/` — `APIException` and typed subclasses.
...
```bash
rm -rf /tmp/1password-connect-python-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\1password-connect-python-src"   # Windows
```
...
1. **Client construction** — load **python-client-initialization** before you call `1PasswordConnectClient(...)`.
