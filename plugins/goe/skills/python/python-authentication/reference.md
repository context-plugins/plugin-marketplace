...
kwargs to the `GoeapiClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `goe/goe_client.py` and `goe/http/auth/` in
the cloned source.

## Basic auth

```python
from goe.http.auth.basic_auth import BasicAuthCredentials
...
```python
from goe.http.auth.api_key import ApiKeyCredentials
...
```python
from goe.http.auth.api_header import ApiHeaderCredentials
...
```python
from goe.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from goe.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from goe.http.auth.o_auth_acg import OAuthACGCredentials
from goe.models.o_auth_scope_goe_enum import OAuthScope{Pkg}Enum
...
client = GoeapiClient(o_auth_acg_credentials=creds)
...
```python
from goe.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = GoeapiClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GoeapiClient.from_environment()`) read each credential
...
Grep `from_environment` in `goe/configuration.py` and each `goe/http/auth/*.py` for the exact
