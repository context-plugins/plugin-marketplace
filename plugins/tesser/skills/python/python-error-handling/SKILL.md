...
`tesserapiv1/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `tesserapiv1/exceptions/` and operation docstrings.
...
```python
from tesserapiv1.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `tesserapiv1/exceptions/`.
...
```python
from tesserapiv1.exceptions.api_exception import APIException
from tesserapiv1.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`tesserapiv1/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`tesserapiv1/exceptions/` for the class.
...
`tesserapiv1/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `tesserapiv1/exceptions/`
  or a `ValueError` — check the auth handler in `tesserapiv1/http/auth/` for the exact type.
