...
- `LocationScore` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `location_score` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'location_score'
include LocationScore
...
The authoritative list is the `initialize` signature in `lib/location_score/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/location_score/configuration.rb`
(`build_default_config_from_env`) and `lib/location_score/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/location_score.rb`) and store it in a
