...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `shell-data-reporting-apislib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'shell-data-reporting-apislib';
import { ApiError } from 'shell-data-reporting-apislib';
import { {Operation}Error } from 'shell-data-reporting-apislib/errors';
