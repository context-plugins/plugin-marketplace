...
description: "The New York Times APIs TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `the-new-york-times-apis`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any The New York Times APIs TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the The New York Times APIs TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing The New York Times APIs TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-the-new-york-times-apis` for the workflow that wraps this skill.
...
| Package name (what you install, and what you import) | `the-new-york-times-apis` — **not on npm**; built from source (see *Install*) |
| Import specifier | `the-new-york-times-apis` — the package root is the **only** entry; deep imports do not resolve |
| Version | `1.0.0` (API spec version `1.0.0`) |
| Client class | `TheNewYorkTimesApIsClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new TheNewYorkTimesApIsClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new TheNewYorkTimesApIsClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `apikey`. `timeout` defaults to `60_000` ms |
...
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://api.nytimes.com` |
...
| Surface | 10 operations · 6 resources · 45 models · 5 open enums · 9 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/nytimes-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 50 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { TheNewYorkTimesApIsClient, ServerEnvironment, ResponseError, TheNewYorkTimesApIsError } from "the-new-york-times-apis";
import type { ClientOptions, Article } from "the-new-york-times-apis";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `the-new-york-times-apis/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **⚠ The SDK exports a model type literally named `Response`** (`src/models/response.ts`, schema `responseSchema`). Import it unaliased and it **shadows the global `Response`** for the rest of the file. Alias it: `import type { Response as SdkResponse } from "the-new-york-times-apis"`.
- **From CommonJS**, the typed spelling is `import sdk = require("the-new-york-times-apis")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://api.nytimes.com` | `serverOptions.default.production.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 9 of the 10 operations require a credential and 1 is public.

| `ClientOptions` field | Scheme kind | What the SDK sends |
| --- | --- | --- |
| `apikey` | API key (query) | query `api-key=<key>` |

```ts
const client = new TheNewYorkTimesApIsClient({
...
| --- | --- | --- | --- |
| `client.search` | `Search` | 1 | `searchArticles` |
| `client.archive` | `Archive` | 1 | `getArchiveByMonth` |
| `client.stories` | `Stories` | 1 | `getTopStoriesBySection` |
| `client.mostPopular` | `MostPopular` | 4 | `getMostEmailedArticles` · `getMostSharedArticles` · `getMostSharedArticlesByType` · `getMostViewedArticles` |
| `client.books` | `Books` | 2 | `getListsDateListJson` · `getListsOverviewJson` |
| `client.rss` | `Rss` | 1 | `getSectionFeed` |
...
```bash
node -e "console.log(require.resolve('the-new-york-times-apis/package.json'))"
```

Failing that it is at `node_modules/the-new-york-times-apis/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/the-new-york-times-apis/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/the-new-york-times-apis/`:
...
Beyond the usual signatures and model members, a contract sheet for the The New York Times APIs TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
...
2. **One operation resolves to `undefined` (`rss.getSectionFeed`)** — `await` gives you nothing to inspect, so **`.asApiResult()` is the only way to observe their status and headers** — decide the mode at write time, not by retrofit.
...
9. **That `Response` imported from this package is a model type, not the global of that name** — every sheet that references one should carry the alias it will be imported under. The error base is re-exported as `TheNewYorkTimesApIsError` for the same reason.
