...
- `restapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/deepgram-go-sdk` — the Go import path for the root package; `github.com/context-plugins/deepgram-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/deepgram-go-sdk"

client := restapi.NewClient(
    restapi.CreateConfiguration(
        restapi.WithEnvironment(restapi.PRODUCTION),
...
from environment variables named `RESTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := restapi.NewClient(restapi.CreateConfigurationFromEnvironment())
```
...
```go
client := restapi.NewClient(
    restapi.CreateConfiguration(restapi.WithEnvironment(restapi.PRODUCTION)),
)
```
...
client := restapi.NewClient(
    restapi.CreateConfiguration(
        restapi.WithHttpConfiguration(
            restapi.CreateHttpConfiguration(
                restapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                restapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = restapi.NewClient(restapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api restapi.ClientInterface
}

func NewService(api restapi.ClientInterface) *Service {
