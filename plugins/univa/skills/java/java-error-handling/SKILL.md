...
│   └── com.univapay.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.univapay.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.univapay.api.http.client.HttpContext`, which carries:
