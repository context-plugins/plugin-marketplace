...
> Throughout, `traderapi` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := traderapi.NewClient(
    traderapi.CreateConfiguration(
        traderapi.WithBasicAuthCredentials(
            traderapi.NewBasicAuthCredentials(
...
traderapi.WithApiKeyCredentials(traderapi.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
traderapi.WithApiHeaderCredentials(traderapi.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
traderapi.WithOAuthBearerTokenCredentials(
    traderapi.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := traderapi.NewClient(
    traderapi.CreateConfiguration(
        traderapi.WithOAuthCCGCredentials(
            traderapi.NewOAuthCCGCredentials(
