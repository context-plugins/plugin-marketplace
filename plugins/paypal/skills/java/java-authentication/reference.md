...
against the `PaypalServerSdkClient.Builder` setters, the `com.paypal.sdk.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `PaypalServerSdkClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
