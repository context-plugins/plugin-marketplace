...
> `{operation}`, `{apiGroup}`, `DiscourseApiDocumentationLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `DiscourseApiDocumentationLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use DiscourseApiDocumentationLib\Exceptions\ApiException;
...
```php
use DiscourseApiDocumentationLib\Exceptions\ApiException;
use DiscourseApiDocumentationLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `DiscourseApiDocumentationLib\Exceptions\` — read the source to
