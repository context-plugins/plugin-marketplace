...
client = stub_client(stubs)
assert_raises Klarna::APIException do
...
BASE = Klarna::Configuration.new(environment: Klarna::Environment::PRODUCTION).get_base_uri

def build_test_client
  Klarna::Client.new(
    environment: Klarna::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Klarna::APIException
```
...
```ruby
expect { call }.to raise_error(Klarna::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Klarna::HttpCallBack
...
client  = Klarna::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
