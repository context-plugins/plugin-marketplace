...
client = stub_client(stubs)
assert_raises SlackWebAPI::APIException do
...
def build_test_client
  SlackWebAPI::Client.new(
    environment: SlackWebAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error SlackWebAPI::APIException
```
...
```ruby
expect { call }.to raise_error(SlackWebAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < SlackWebAPI::HttpCallBack
...
client  = SlackWebAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
