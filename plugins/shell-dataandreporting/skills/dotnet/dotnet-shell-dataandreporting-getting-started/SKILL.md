---
name: dotnet-shell-dataandreporting-getting-started
description: shell-dataandreporting .NET SDK identity and lookup layer for the dotnet-shell-dataandreporting-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the shell-dataandreporting .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-shell-dataandreporting-sdk` helper
...
| | |
| --- | --- |
| API | shell-dataandreporting |
| Source repo | https://github.com/context-plugins/shell-dataandreporting-csharp-sdk (branch `main`) |
| Root namespace | `ShellDataReportingApis` (the `using` namespace) |
| Client class | `ShellDataReportingApisClient` |
| Options class | `ShellDataReportingApisClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `ShellDataReportingApisClientOptions`: `BearerToken: OAuth2ClientCredentials?`, `BearerTokenTokenStrategy: IOAuth2TokenStrategy<OAuth2ClientCredentials>?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `Sit`, `Production` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using ShellDataReportingApis.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/shell-dataandreporting-csharp-sdk
dotnet add reference shell-dataandreporting-csharp-sdk/ShellDataReportingApis.csproj
```
...
temp directory** (`<temp>/shell-dataandreporting-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/shell-dataandreporting-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/shell-dataandreporting-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/shell-dataandreporting-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\shell-dataandreporting-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/shell-dataandreporting-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/shell-dataandreporting-dotnet-src/`, **never** in the project repo, and its path never goes into
`shell-dataandreporting-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `ShellDataReportingApisClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new ShellDataReportingApisClient(...)`, build its options, or DI-register via
   `AddShellDataReportingApisClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `ShellDataReportingApisClientOptions` — the map's *Servers & auth* section
