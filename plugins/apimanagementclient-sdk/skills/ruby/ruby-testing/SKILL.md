...
def stub_client(stubs)
  ...
  ApiManagementClient::Client.new(
    connection:  conn,
...
client = stub_client(stubs)
assert_raises ApiManagementClient::APIException do
...
def build_test_client
  ApiManagementClient::Client.new(
    environment: ApiManagementClient::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error ApiManagementClient::APIException
```
...
```ruby
expect { call }.to raise_error(ApiManagementClient::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < ApiManagementClient::HttpCallBack
...
client  = ApiManagementClient::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
