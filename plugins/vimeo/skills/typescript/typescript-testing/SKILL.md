...
```typescript
import { {Operation}Error } from 'vimeo-apilib/errors';
...
```typescript
import { ApiError } from 'vimeo-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(VimeoAPIClient)
    .useValue(new VimeoAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
