...
> `{operation}`, `{apiGroup}`, `FlightMostTraveledDestinationsLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `FlightMostTraveledDestinationsLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use FlightMostTraveledDestinationsLib\Exceptions\ApiException;
...
```php
use FlightMostTraveledDestinationsLib\Exceptions\ApiException;
use FlightMostTraveledDestinationsLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `FlightMostTraveledDestinationsLib\Exceptions\` — read the source to
