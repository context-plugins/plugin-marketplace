...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new AdafruitIORESTAPIClient({
...
```typescript
import { {Operation}Error } from 'adafruit-io-rest-apilib/errors';
...
```typescript
import { ApiError } from 'adafruit-io-rest-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AdafruitIORESTAPIClient)
    .useValue(new AdafruitIORESTAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
