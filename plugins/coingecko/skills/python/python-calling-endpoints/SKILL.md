...
Open `coingecko/coingecko_client.py` for the controller property names, then the relevant
`coingecko/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from coingecko.models.{request_model} import {RequestModel}
...
Open `coingecko/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from coingecko.models.suite_code_enum import SuiteCodeEnum
...
```python
from coingecko.http.http_call_back import HttpCallBack
...
client = CoingeckodemoapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `coingecko/pagination/` in the source.

## Finding the right method

- Controller property names are on `CoingeckodemoapiClient` in `coingecko/coingecko_client.py`.
- Operation method signatures are in `coingecko/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `coingecko/models/`; enum types have `from_value()`.
