...
- `DockerClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `v133` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`DockerClient.Builder` class. A minimal initialization:

```java
import v133.DockerClient;
import v133.Environment;

DockerClient client = new DockerClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `DockerClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `DockerClient.java` |

Call `.build()` to produce an immutable `DockerClient` instance.
...
public class ApiConfig {
    ...
    public DockerClient apiClient() {
        return new DockerClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired DockerClient client) {
        client.shutdown();
    }
}
```

Inject `DockerClient` into your services as a normal Spring dependency. Build only one bean.
