...
- `boku-direct-payments-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'boku-direct-payments-apilib';
...
```ts
import { {Resource}Api } from 'boku-direct-payments-apilib';
