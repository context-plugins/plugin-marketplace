...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new MailboxValidatorFreeEmailCheckerClient({
...
```typescript
import { {Operation}Error } from 'mailboxvalidator-free-email-checkerlib/errors';
...
```typescript
import { ApiError } from 'mailboxvalidator-free-email-checkerlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(MailboxValidatorFreeEmailCheckerClient)
    .useValue(new MailboxValidatorFreeEmailCheckerClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
