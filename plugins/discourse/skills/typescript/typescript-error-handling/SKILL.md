...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `discourse`) — replace it with the concrete identifier from the source.
...
import { ApiError } from 'discourse';
import { {Operation}Error } from 'discourse/errors';
