    ...
    alpaca.CreateConfiguration(
        alpaca.WithHttpConfiguration(
            alpaca.CreateHttpConfiguration(
                alpaca.WithTimeout(30),
                alpaca.WithRetryConfiguration(
                    alpaca.CreateRetryConfiguration(
                        alpaca.WithMaxRetryAttempts(3),
                        alpaca.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := alpaca.CreateRetryConfiguration(
    alpaca.WithMaxRetryAttempts(5),
    alpaca.WithRetryInterval(1),
    alpaca.WithBackoffFactor(2),
    alpaca.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    alpaca.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := alpaca.NewClient(
    alpaca.CreateConfiguration(
        alpaca.WithHttpConfiguration(
            alpaca.CreateHttpConfiguration(
                alpaca.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
