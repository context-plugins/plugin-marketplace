...
- `spotify-web-api-with-fixes-and-improvements-from-sonalluxlib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'spotify-web-api-with-fixes-and-improvements-from-sonalluxlib';
...
```ts
import { {Resource}Controller } from 'spotify-web-api-with-fixes-and-improvements-from-sonalluxlib';
