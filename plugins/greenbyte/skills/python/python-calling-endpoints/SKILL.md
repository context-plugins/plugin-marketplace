...
Open `greenbyte/greenbyte_client.py` for the controller property names, then the relevant
`greenbyte/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from greenbyte.models.{request_model} import {RequestModel}
...
Open `greenbyte/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from greenbyte.models.suite_code_enum import SuiteCodeEnum
...
```python
from greenbyte.http.http_call_back import HttpCallBack
...
client = GreenbyteapiClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `greenbyte/pagination/` in the source.

## Finding the right method

- Controller property names are on `GreenbyteapiClient` in `greenbyte/greenbyte_client.py`.
- Operation method signatures are in `greenbyte/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `greenbyte/models/`; enum types have `from_value()`.
