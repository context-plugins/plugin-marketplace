...
> `{ApiGroup}`, `MediumApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `MediumApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `MediumApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `MediumApi.Errors`
...
```csharp
using MediumApi.Core.Exceptions;     // SdkException<TError>
using MediumApi.Core.ErrorResponse;  // ApiError, RawError
using MediumApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`MediumApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using MediumApi.Core.Exceptions;     // SdkException<TError>
using MediumApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `MediumApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `MediumApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using MediumApi.Core.Models;        // ApiResult<TResponse, TError>
using MediumApi.Core.ErrorResponse; // RawError
using MediumApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `MediumApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
