...
```ruby
client = HotelRatings::Client.new(
  max_retries:    3,
...
```ruby
client = HotelRatings::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = HotelRatings::Client.new(adapter: :net_http)
```
...
client = HotelRatings::Client.new(connection: conn)
```
...
client = HotelRatings::Client.new(environment: Environment::PRODUCTION)
client = HotelRatings::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = HotelRatings::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = HotelRatings::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = HotelRatings::ProxySettings.from_env
client = HotelRatings::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < HotelRatings::HttpCallBack
...
client = HotelRatings::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = HotelRatings::Client.from_env
# Overrides take precedence over env vars:
client = HotelRatings::Client.from_env(timeout: 30, max_retries: 3)
```
