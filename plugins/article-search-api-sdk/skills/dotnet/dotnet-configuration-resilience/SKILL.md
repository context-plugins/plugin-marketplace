...
All types below live under `ArticleSearchAPI.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using ArticleSearchAPI.Core.Configuration;
...
```csharp
using ArticleSearchAPI.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new ArticleSearchAPIClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `ArticleSearchAPI.Core.Exceptions`):
...
With DI, the SDK's `AddArticleSearchAPIClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddArticleSearchAPIClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
