...
- `flight-check-in-linkslib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'flight-check-in-linkslib';
...
```ts
import { {Resource}Controller } from 'flight-check-in-linkslib';
