...
- `coingecko-demo-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'coingecko-demo-apilib';
...
```ts
import { {Resource}Controller } from 'coingecko-demo-apilib';
