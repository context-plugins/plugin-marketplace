...
```go
theplaidapi.WithOAuthCCGCredentials(
    theplaidapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr theplaidapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := theplaidapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr theplaidapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := theplaidapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := theplaidapi.NewClient(theplaidapi.CreateConfiguration(theplaidapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    theplaidapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := theplaidapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := theplaidapi.NewClient(theplaidapi.CreateConfiguration(theplaidapi.WithOAuthROPCGCredentials(creds)))
...
```go
theplaidapi.WithOAuthBearerTokenCredentials(theplaidapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
theplaidapi.WithBasicAuthCredentials(theplaidapi.NewBasicAuthCredentials(username, password))
theplaidapi.WithApiHeaderCredentials(theplaidapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
theplaidapi.WithApiKeyCredentials(theplaidapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `THEPLAIDAPI_...` variables — e.g.
`THEPLAIDAPI_USERNAME`, `THEPLAIDAPI_PASSWORD`, `THEPLAIDAPI_TOKEN`, `THEPLAIDAPI_API_KEY`,
`THEPLAIDAPI_O_AUTH_CLIENT_ID`, `THEPLAIDAPI_O_AUTH_CLIENT_SECRET`, `THEPLAIDAPI_ACCESS_TOKEN`. Grep the
