...
against the `GoogleWorkspaceClient.Builder` setters, the `com.googleapis.www.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GoogleWorkspaceClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
GoogleWorkspaceClient client = new GoogleWorkspaceClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeGoogleWorkspaceEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new GoogleWorkspaceClient.Builder()
