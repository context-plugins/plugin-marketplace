...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new AdyenBinLookupAPIClient({
...
```typescript
import { {Operation}Error } from 'adyen-binlookup-apilib/errors';
...
```typescript
import { ApiError } from 'adyen-binlookup-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AdyenBinLookupAPIClient)
    .useValue(new AdyenBinLookupAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
