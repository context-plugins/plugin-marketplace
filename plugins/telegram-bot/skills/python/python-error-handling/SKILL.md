...
`telegrambotapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `telegrambotapi/exceptions/` and operation docstrings.
...
```python
from telegrambotapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `telegrambotapi/exceptions/`.
...
```python
from telegrambotapi.exceptions.api_exception import APIException
from telegrambotapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`telegrambotapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`telegrambotapi/exceptions/` for the class.
...
`telegrambotapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `telegrambotapi/exceptions/`
  or a `ValueError` — check the auth handler in `telegrambotapi/http/auth/` for the exact type.
