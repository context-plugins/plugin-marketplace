...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `CoingeckodemoapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`coingecko/configuration.py` in the cloned source.
...
```python
import requests
from coingecko.configuration import Configuration
...
`coingecko/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`coingecko/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `coingecko/configuration.py`
...
`NumberPagedResponse`) live in `coingecko/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `coingecko/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
