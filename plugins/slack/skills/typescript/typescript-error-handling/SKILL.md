...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `slack`) — replace it with the concrete identifier from the source.
...
import { ApiError } from 'slack';
import { {Operation}Error } from 'slack/errors';
