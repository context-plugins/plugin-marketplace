...
> `HotelNameAutocompleteClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): HotelNameAutocompleteClient
{
...
```php
use HotelNameAutocompleteLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use HotelNameAutocompleteLib\Exceptions\{OperationException};
use HotelNameAutocompleteLib\Exceptions\ApiException;
...
app()->instance(HotelNameAutocompleteClient::class, clientWithResponses([
