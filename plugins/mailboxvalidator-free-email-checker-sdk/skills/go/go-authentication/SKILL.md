...
> Throughout, `mailboxvalidatorfreeemailchecker` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := mailboxvalidatorfreeemailchecker.NewClient(
    mailboxvalidatorfreeemailchecker.CreateConfiguration(
        mailboxvalidatorfreeemailchecker.WithBasicAuthCredentials(
            mailboxvalidatorfreeemailchecker.NewBasicAuthCredentials(
...
mailboxvalidatorfreeemailchecker.WithApiKeyCredentials(mailboxvalidatorfreeemailchecker.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
mailboxvalidatorfreeemailchecker.WithApiHeaderCredentials(mailboxvalidatorfreeemailchecker.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
mailboxvalidatorfreeemailchecker.WithOAuthBearerTokenCredentials(
    mailboxvalidatorfreeemailchecker.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := mailboxvalidatorfreeemailchecker.NewClient(
    mailboxvalidatorfreeemailchecker.CreateConfiguration(
        mailboxvalidatorfreeemailchecker.WithOAuthCCGCredentials(
            mailboxvalidatorfreeemailchecker.NewOAuthCCGCredentials(
