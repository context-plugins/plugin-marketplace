...
| Project | `FlightOrderManagement.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `FlightOrderManagement` (the `using` namespace) |
| Client class | `FlightOrderManagementClient` |
| Options class | `FlightOrderManagementClientOptions` |
...
namespaces transitively, so `using FlightOrderManagement.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/flight-order-management-csharp
dotnet add reference flight-order-management-csharp/FlightOrderManagement.csproj
```

```csharp
using FlightOrderManagement;          // client + options + ServerEnvironment
using FlightOrderManagement.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/flight-order-management-csharp /tmp/flight-order-management-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/flight-order-management-csharp "$env:TEMP\flight-order-management-dotnet-src"
```
...
- `FlightOrderManagementClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/flight-order-management-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\flight-order-management-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `FlightOrderManagementClient`, build its
   `FlightOrderManagementClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
