...
- `flightpriceanalysisapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/flight-price-analysis-api-go` — the Go import path for the root package; `github.com/context-plugins/flight-price-analysis-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/flight-price-analysis-api-go"

client := flightpriceanalysisapi.NewClient(
    flightpriceanalysisapi.CreateConfiguration(
        flightpriceanalysisapi.WithEnvironment(flightpriceanalysisapi.PRODUCTION),
...
from environment variables named `FLIGHTPRICEANALYSISAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := flightpriceanalysisapi.NewClient(flightpriceanalysisapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `flightpriceanalysisapi.PRODUCTION`,
`flightpriceanalysisapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := flightpriceanalysisapi.NewClient(
    flightpriceanalysisapi.CreateConfiguration(flightpriceanalysisapi.WithEnvironment(flightpriceanalysisapi.PRODUCTION)),
)
```
...
client := flightpriceanalysisapi.NewClient(
    flightpriceanalysisapi.CreateConfiguration(
        flightpriceanalysisapi.WithHttpConfiguration(
            flightpriceanalysisapi.CreateHttpConfiguration(
                flightpriceanalysisapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                flightpriceanalysisapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = flightpriceanalysisapi.NewClient(flightpriceanalysisapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api flightpriceanalysisapi.ClientInterface
}

func NewService(api flightpriceanalysisapi.ClientInterface) *Service {
