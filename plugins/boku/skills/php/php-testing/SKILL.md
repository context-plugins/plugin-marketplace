...
> `BokuDirectPaymentsApiClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): BokuDirectPaymentsApiClient
{
...
app()->instance(BokuDirectPaymentsApiClient::class, clientWithResponses([
