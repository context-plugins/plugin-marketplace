...
| | |
| --- | --- |
| API | `Paze Checkout API` |
| Project | `PazeCheckoutApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
...
namespaces transitively, so `using PazeCheckoutApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/paze-checkout-api-csharp
dotnet add reference paze-checkout-api-csharp/PazeCheckoutApi.csproj
```

```csharp
using PazeCheckoutApi;          // client + options + ServerEnvironment
using PazeCheckoutApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/paze-checkout-api-csharp /tmp/paze-checkout-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/paze-checkout-api-csharp "$env:TEMP\paze-checkout-api-dotnet-src"
```
...
- `PazeCheckoutApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/paze-checkout-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\paze-checkout-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `PazeCheckoutApiClient`, build its
   `PazeCheckoutApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
