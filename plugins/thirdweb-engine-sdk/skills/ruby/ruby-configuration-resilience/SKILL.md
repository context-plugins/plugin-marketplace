...
```ruby
client = ThirdwebEngine::Client.new(
  max_retries:    3,
...
```ruby
client = ThirdwebEngine::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ThirdwebEngine::Client.new(adapter: :net_http)
```
...
client = ThirdwebEngine::Client.new(connection: conn)
```
...
client = ThirdwebEngine::Client.new(environment: Environment::PRODUCTION)
client = ThirdwebEngine::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ThirdwebEngine::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ThirdwebEngine::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ThirdwebEngine::ProxySettings.from_env
client = ThirdwebEngine::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ThirdwebEngine::HttpCallBack
...
client = ThirdwebEngine::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ThirdwebEngine::Client.from_env
# Overrides take precedence over env vars:
client = ThirdwebEngine::Client.from_env(timeout: 30, max_retries: 3)
```
