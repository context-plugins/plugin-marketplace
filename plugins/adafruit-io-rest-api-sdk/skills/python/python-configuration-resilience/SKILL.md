...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `AdafruitIORESTAPIClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`adafruitiorestapi/configuration.py` in the cloned source.
...
```python
import requests
from adafruitiorestapi.configuration import Configuration
...
`adafruitiorestapi/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`adafruitiorestapi/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `adafruitiorestapi/configuration.py`
...
`NumberPagedResponse`) live in `adafruitiorestapi/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `adafruitiorestapi/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
