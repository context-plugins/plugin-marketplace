...
```ruby
client = Univa::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = Univa::Client.new(adapter: :net_http)
```
...
client = Univa::Client.new(connection: conn)
```
...
client = Univa::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = Univa::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = Univa::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = Univa::ProxySettings.from_env
client = Univa::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < Univa::HttpCallBack
...
client = Univa::Client.new(http_callback: LogCallback.new)
```
...
client = Univa::Client.from_env(timeout: 30, max_retries: 3)
```
