...
- `PayPalAPIClient` — the gateway class (e.g. `APIMATICCalculatorClient`, `MultiAuthSampleClient`).
- `com.paypal.sandbox.apim` — the root Java package (e.g. `io.apimatic.examples`, `localhost3000`).
...
`PayPalAPIClient.Builder` class. A minimal initialization:

```java
import com.paypal.sandbox.apim.PayPalAPIClient;
import com.paypal.sandbox.apim.Environment;

PayPalAPIClient client = new PayPalAPIClient.Builder()
...
The `Builder` is the only entry point — confirm its fluent methods in `PayPalAPIClient.java`. The common
...
| other per-API methods | API-specific parameters (e.g. `.port("80")`, `.suites(SuiteCodeEnum.HEARTS)`) — check `PayPalAPIClient.java` |

Call `.build()` to produce an immutable `PayPalAPIClient` instance.
...
public class ApiConfig {
    @Bean
    public PayPalAPIClient apiClient() {
        return new PayPalAPIClient.Builder()
            .environment(Environment.PRODUCTION)
    ...
    @PreDestroy
    public void shutdown(@Autowired PayPalAPIClient client) {
        client.shutdown();
    }
}
```

Inject `PayPalAPIClient` into your services as a normal Spring dependency. Build only one bean.
