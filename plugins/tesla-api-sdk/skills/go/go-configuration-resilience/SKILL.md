    ...
    teslaapi.CreateConfiguration(
        teslaapi.WithHttpConfiguration(
            teslaapi.CreateHttpConfiguration(
                teslaapi.WithTimeout(30),
                teslaapi.WithRetryConfiguration(
                    teslaapi.CreateRetryConfiguration(
                        teslaapi.WithMaxRetryAttempts(3),
                        teslaapi.WithBackoffFactor(2),
                    ),
                ),
            ),
        ),
...
```go
rc := teslaapi.CreateRetryConfiguration(
    teslaapi.WithMaxRetryAttempts(5),
    teslaapi.WithRetryInterval(1),
    teslaapi.WithBackoffFactor(2),
    teslaapi.WithHttpStatusCodesToRetry([]int64{429, 500, 503}),
    teslaapi.WithHttpMethodsToRetry([]string{"GET", "PUT"}),
)
```
...
client := teslaapi.NewClient(
    teslaapi.CreateConfiguration(
        teslaapi.WithHttpConfiguration(
            teslaapi.CreateHttpConfiguration(
                teslaapi.WithTransport(&loggingTransport{inner: http.DefaultTransport}),
            ),
        ),
    ),
)
