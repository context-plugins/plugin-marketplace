...
To see which schemes a specific SDK accepts, read the credential setter methods on `FrankfurterApiClient.Builder`
...
- Set credentials **on the `FrankfurterApiClient.Builder`** before calling `.build()` — the client is immutable
