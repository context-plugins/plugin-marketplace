...
Open `giteaapi/giteaapi_client.py` for the controller property names, then the relevant
`giteaapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from giteaapi.models.{request_model} import {RequestModel}
...
Open `giteaapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from giteaapi.models.suite_code_enum import SuiteCodeEnum
...
an iterator over the items on that page. Inspect `giteaapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `GiteaapiClient` in `giteaapi/giteaapi_client.py`.
- Operation method signatures are in `giteaapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `giteaapi/models/`; enum types have `from_value()`.
