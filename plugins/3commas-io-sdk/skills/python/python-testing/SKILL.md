...
reference; substitute your `3commasio` and `3commasIoClient` names.
...
- The `Environment.TESTING` base URL is `http://localhost:3000` (confirm from the `environments`
  dict in `3commasio/configuration.py`) — use this URL in `responses` stubs so the URL prefix matches.
