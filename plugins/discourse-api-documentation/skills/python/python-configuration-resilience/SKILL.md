...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `DiscourseapidocumentationClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`discourseapidocumentation/configuration.py` in the cloned source.
...
```python
import requests
from discourseapidocumentation.configuration import Configuration
...
`discourseapidocumentation/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`discourseapidocumentation/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `discourseapidocumentation/configuration.py`
...
`NumberPagedResponse`) live in `discourseapidocumentation/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `discourseapidocumentation/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
