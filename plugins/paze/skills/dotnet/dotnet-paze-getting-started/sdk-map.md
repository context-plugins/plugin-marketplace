...
| | |
|---|---|
| SDK display name | paze |
| Root namespace/module | `PazeCheckoutApi` |
<!-- gen:stamp -->
| Target framework(s) | `netstandard2.0` (C# `LangVersion 14`, `Nullable enable`) |
| Source commit (spec stamp) | `ecba157` (`ecba1571d773ee2e7802fff3b5677f1f38d28089`, tagged `ecba157`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
| Repo | https://github.com/context-plugins/paze-csharp-sdk (branch `main`) |
...
```csharp
using PazeCheckoutApi;
using PazeCheckoutApi.Servers; // ServerEnvironment lives here

var options = new PazeCheckoutApiClientOptions
{
...
var client = new PazeCheckoutApiClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddPazeCheckoutApiClient(o =>
{
...
`PazeCheckoutApiClient.cs`.

<!-- crawler:client-options -->
All `PazeCheckoutApiClientOptions` properties (source: `PazeCheckoutApiClientOptions.cs`):
...
- `PazeCheckoutApiClient(HttpClient httpClient, PazeCheckoutApiClientOptions options)`
<!-- /crawler:client-options -->
...
|---|---|---|
| `ApiError` — abstract base of all 5 typed error classes in `Errors/` | `TryGetRawError(out RawError error): bool` | `Core/ErrorResponse/ApiError.cs` |
...
Of **5 operations**, **5 are Case A (typed)** and **0 are Case B (raw)**.
<!-- /crawler:op-stats -->

---

## Operations — by controller (3 groups, 5 operations)
...
|---|---:|---|
| `CheckoutSession` | 3 | [map/operations/CheckoutSession.md](map/operations/CheckoutSession.md) |
| `Identity` | 1 | [map/operations/Identity.md](map/operations/Identity.md) |
| `MerchantOnboarding` | 1 | [map/operations/MerchantOnboarding.md](map/operations/MerchantOnboarding.md) |
<!-- /crawler:ops-table -->
...
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 41 | [`ApiErrorResponse` … `TravelData`](map/models/records-1-Ap-Tr.md) |
| Unions (`OneOf` / `AnyOf`) — variant factories + `TryGet…` | 0 + 0 | [map/models/unions.md](map/models/unions.md) |
| Enums (`StringEnum<T>` / `IntEnum<T>`) — literal C# member names + wire values | 9 | [map/models/enums.md](map/models/enums.md) |
<!-- /gen:models-table -->
...
|---|---|
| Client & options (root) | `PazeCheckoutApi` |
| Operation controllers (`Api/`) | `PazeCheckoutApi.Api` |
| Records (`Models/`) | `PazeCheckoutApi.Models` |
| Enums (`Models/Enums/`) | `PazeCheckoutApi.Models.Enums` |
| Error classes (`Errors/`) | `PazeCheckoutApi.Errors` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `PazeCheckoutApiClientOptions` (source: `PazeCheckoutApiClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:
