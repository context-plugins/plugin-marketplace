...
```python
from mindbody.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `mindbody/mindbody_client.py` in the cloned source.
...
`from_environment` in `mindbody/configuration.py` for the exact names.
...
Open `mindbody/mindbody_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `mindbody/configuration.py`. **Read the enum in `configuration.py`
...
`mindbody/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
