...
```typescript
import { {Operation}Error } from 'access-analyzerlib/errors';
...
```typescript
import { ApiError } from 'access-analyzerlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AccessAnalyzerClient)
    .useValue(new AccessAnalyzerClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
