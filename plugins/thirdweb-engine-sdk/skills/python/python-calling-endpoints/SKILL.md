...
Open `thirdwebengine/thirdwebengine_client.py` for the controller property names, then the relevant
`thirdwebengine/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from thirdwebengine.models.{request_model} import {RequestModel}
...
Open `thirdwebengine/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from thirdwebengine.models.suite_code_enum import SuiteCodeEnum
...
```python
from thirdwebengine.http.http_call_back import HttpCallBack
...
client = ThirdwebEngineClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `thirdwebengine/pagination/` in the source.

## Finding the right method

- Controller property names are on `ThirdwebEngineClient` in `thirdwebengine/thirdwebengine_client.py`.
- Operation method signatures are in `thirdwebengine/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `thirdwebengine/models/`; enum types have `from_value()`.
