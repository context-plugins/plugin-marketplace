...
- `AdafruitIORESTAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `adafruit_io_rest_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'adafruit_io_rest_api'
include AdafruitIORESTAPI
...
The authoritative list is the `initialize` signature in `lib/adafruit_io_rest_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/adafruit_io_rest_api/configuration.rb`
(`build_default_config_from_env`) and `lib/adafruit_io_rest_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/adafruit_io_rest_api.rb`) and store it in a
