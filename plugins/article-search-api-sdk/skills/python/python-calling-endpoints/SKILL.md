...
Open `articlesearchapi/articlesearchapi_client.py` for the controller property names, then the relevant
`articlesearchapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from articlesearchapi.models.{request_model} import {RequestModel}
...
Open `articlesearchapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from articlesearchapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from articlesearchapi.http.http_call_back import HttpCallBack
...
client = ArticleSearchAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `articlesearchapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `ArticleSearchAPIClient` in `articlesearchapi/articlesearchapi_client.py`.
- Operation method signatures are in `articlesearchapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `articlesearchapi/models/`; enum types have `from_value()`.
