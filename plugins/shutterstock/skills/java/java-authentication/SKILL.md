...
To see which schemes a specific SDK accepts, read the credential setter methods on `ShutterstockClient.Builder`
...
- Set credentials **on the `ShutterstockClient.Builder`** before calling `.build()` — the client is immutable
