...
client = stub_client(stubs)
assert_raises FundAPI::APIException do
...
def build_test_client
  FundAPI::Client.new(
    environment: FundAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error FundAPI::APIException
```
...
```ruby
expect { call }.to raise_error(FundAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < FundAPI::HttpCallBack
...
client  = FundAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
