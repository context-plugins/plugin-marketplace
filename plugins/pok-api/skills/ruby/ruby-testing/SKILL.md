...
client = stub_client(stubs)
assert_raises PokAPI::APIException do
...
BASE = PokAPI::Configuration.new(environment: PokAPI::Environment::PRODUCTION).get_base_uri

def build_test_client
  PokAPI::Client.new(
    environment: PokAPI::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error PokAPI::APIException
```
...
```ruby
expect { call }.to raise_error(PokAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < PokAPI::HttpCallBack
...
client  = PokAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
