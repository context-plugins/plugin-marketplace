...
> `GoogleWorkspaceClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): GoogleWorkspaceClient
{
...
```php
use GoogleWorkspaceLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use GoogleWorkspaceLib\Exceptions\{OperationException};
use GoogleWorkspaceLib\Exceptions\ApiException;
...
app()->instance(GoogleWorkspaceClient::class, clientWithResponses([
