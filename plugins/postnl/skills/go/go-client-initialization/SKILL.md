...
- `postnl` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/postnl-go-sdk` — the Go import path for the root package; `github.com/context-plugins/postnl-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/postnl-go-sdk"

client := postnl.NewClient(
    postnl.CreateConfiguration(
        postnl.WithEnvironment(postnl.PRODUCTION),
...
from environment variables named `POSTNL_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := postnl.NewClient(postnl.CreateConfigurationFromEnvironment())
```
...
```go
client := postnl.NewClient(
    postnl.CreateConfiguration(postnl.WithEnvironment(postnl.PRODUCTION)),
)
```
...
client := postnl.NewClient(
    postnl.CreateConfiguration(
        postnl.WithHttpConfiguration(
            postnl.CreateHttpConfiguration(
                postnl.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                postnl.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = postnl.NewClient(postnl.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api postnl.ClientInterface
}

func NewService(api postnl.ClientInterface) *Service {
