...
`testing` + `testify/require` for reference; substitute the real `notion` and names.
...
import (
    ...
    "github.com/context-plugins/notion-go-sdk"   // root package notion
)
...
func clientReturning(status int, body string) (notion.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := notion.NewClient(
        notion.CreateConfiguration(
            notion.WithHttpConfiguration(
                notion.CreateHttpConfiguration(notion.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/notion-go-sdk/errors"
...
client := notion.NewClient(
    notion.CreateConfiguration(
        notion.WithHttpConfiguration(
            notion.CreateHttpConfiguration(notion.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
