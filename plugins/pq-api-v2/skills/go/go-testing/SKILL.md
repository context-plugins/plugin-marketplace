...
`testing` + `testify/require` for reference; substitute the real `pqapiv2` and names.
...
import (
    ...
    "github.com/context-plugins/pq-api-v2-go-sdk"   // root package pqapiv2
)
...
func clientReturning(status int, body string) (pqapiv2.ClientInterface, *stubTransport) {
    ...
    client := pqapiv2.NewClient(
        pqapiv2.CreateConfiguration(
            pqapiv2.WithHttpConfiguration(
                pqapiv2.CreateHttpConfiguration(pqapiv2.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/pq-api-v2-go-sdk/errors"
...
client := pqapiv2.NewClient(
    pqapiv2.CreateConfiguration(
        pqapiv2.WithHttpConfiguration(
            pqapiv2.CreateHttpConfiguration(pqapiv2.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
