...
kwargs to the `1ForgeFinanceAPIsClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `1forgefinanceapis/1forgefinanceapis_client.py` and `1forgefinanceapis/http/auth/` in
the cloned source.

## Basic auth

```python
from 1forgefinanceapis.http.auth.basic_auth import BasicAuthCredentials
...
```python
from 1forgefinanceapis.http.auth.api_key import ApiKeyCredentials
...
```python
from 1forgefinanceapis.http.auth.api_header import ApiHeaderCredentials
...
```python
from 1forgefinanceapis.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from 1forgefinanceapis.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from 1forgefinanceapis.http.auth.o_auth_acg import OAuthACGCredentials
from 1forgefinanceapis.models.o_auth_scope_1forgefinanceapis_enum import OAuthScope{Pkg}Enum
...
client = 1ForgeFinanceAPIsClient(o_auth_acg_credentials=creds)
...
```python
from 1forgefinanceapis.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = 1ForgeFinanceAPIsClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `1ForgeFinanceAPIsClient.from_environment()`) read each credential
...
Grep `from_environment` in `1forgefinanceapis/configuration.py` and each `1forgefinanceapis/http/auth/*.py` for the exact
