...
`xapiv2/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `xapiv2/exceptions/` and operation docstrings.
...
```python
from xapiv2.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `xapiv2/exceptions/`.
...
```python
from xapiv2.exceptions.api_exception import APIException
from xapiv2.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`xapiv2/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`xapiv2/exceptions/` for the class.
...
`xapiv2/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `xapiv2/exceptions/`
  or a `ValueError` — check the auth handler in `xapiv2/http/auth/` for the exact type.
