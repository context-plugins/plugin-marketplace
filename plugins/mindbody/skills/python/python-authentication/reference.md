...
kwargs to the `MindbodyClient` constructor (or to `Configuration`). Exact credential class names and
...
client = MindbodyClient(o_auth_acg_credentials=creds)
...
client = MindbodyClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `MindbodyClient.from_environment()`) read each credential
