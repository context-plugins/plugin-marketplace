...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `FdxapiClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`fdx/configuration.py` in the cloned source.
...
```python
import requests
from fdx.configuration import Configuration
...
`fdx/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`fdx/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `fdx/configuration.py`
...
`NumberPagedResponse`) live in `fdx/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `fdx/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
