...
against the `GoogleMapsPlatformClient.Builder` setters, the `com.googleapis.www.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `GoogleMapsPlatformClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
GoogleMapsPlatformClient client = new GoogleMapsPlatformClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeGoogleMapsPlatformEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new GoogleMapsPlatformClient.Builder()
