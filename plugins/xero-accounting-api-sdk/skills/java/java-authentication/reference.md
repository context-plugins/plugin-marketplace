...
against the `XeroAccountingAPIClient.Builder` setters, the `com.xero.api.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `XeroAccountingAPIClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
XeroAccountingAPIClient client = new XeroAccountingAPIClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopeXeroAccountingAPIEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
...
new XeroAccountingAPIClient.Builder()
