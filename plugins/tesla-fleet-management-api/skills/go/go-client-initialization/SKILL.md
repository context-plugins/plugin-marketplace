...
- `teslafleetmanagementapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/tesla-fleet-management-api-go-sdk` — the Go import path for the root package; `github.com/context-plugins/tesla-fleet-management-api-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/tesla-fleet-management-api-go-sdk"

client := teslafleetmanagementapi.NewClient(
    teslafleetmanagementapi.CreateConfiguration(
        teslafleetmanagementapi.WithEnvironment(teslafleetmanagementapi.PRODUCTION),
...
from environment variables named `TESLAFLEETMANAGEMENTAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := teslafleetmanagementapi.NewClient(teslafleetmanagementapi.CreateConfigurationFromEnvironment())
```
...
```go
client := teslafleetmanagementapi.NewClient(
    teslafleetmanagementapi.CreateConfiguration(teslafleetmanagementapi.WithEnvironment(teslafleetmanagementapi.PRODUCTION)),
)
```
...
client := teslafleetmanagementapi.NewClient(
    teslafleetmanagementapi.CreateConfiguration(
        teslafleetmanagementapi.WithHttpConfiguration(
            teslafleetmanagementapi.CreateHttpConfiguration(
                teslafleetmanagementapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                teslafleetmanagementapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = teslafleetmanagementapi.NewClient(teslafleetmanagementapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api teslafleetmanagementapi.ClientInterface
}

func NewService(api teslafleetmanagementapi.ClientInterface) *Service {
