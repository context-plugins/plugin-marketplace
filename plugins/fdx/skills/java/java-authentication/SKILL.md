...
To see which schemes a specific SDK accepts, read the credential setter methods on `FdxApiClient.Builder`
...
- Set credentials **on the `FdxApiClient.Builder`** before calling `.build()` — the client is immutable
