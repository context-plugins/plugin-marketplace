...
> `PayPalAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): PayPalAPIClient
{
...
```php
use PayPalAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use PayPalAPILib\Exceptions\{OperationException};
use PayPalAPILib\Exceptions\ApiException;
...
app()->instance(PayPalAPIClient::class, clientWithResponses([
