...
- `ebayfinancesapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `EBayFinancesAPIClient` — the generated client class (e.g. `MultiauthsampleClient`, `BatesterClient`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `EBayFinancesAPIClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from ebayfinancesapi.ebayfinancesapi_client import EBayFinancesAPIClient
from ebayfinancesapi.configuration import Environment
from ebayfinancesapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import

# Flat kwargs — the client wraps them in a Configuration internally:
client = EBayFinancesAPIClient(
...
```python
from ebayfinancesapi.configuration import Configuration, Environment
...
client = EBayFinancesAPIClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `ebayfinancesapi/ebayfinancesapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `EBayFinancesAPIClient` and `Configuration` that reads
...
client = EBayFinancesAPIClient.from_environment()
...
`from_environment` in `ebayfinancesapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `EBayFinancesAPIClient`. Access them as attributes —
...
Open `ebayfinancesapi/ebayfinancesapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `ebayfinancesapi/configuration.py`. Use the constant for the desired
environment — the default is per-API (usually `Environment.TESTING`):

```python
from ebayfinancesapi.configuration import Environment

client = EBayFinancesAPIClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`ebayfinancesapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = EBayFinancesAPIClient(config=new_config)
```
