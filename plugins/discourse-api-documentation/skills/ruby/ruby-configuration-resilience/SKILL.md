...
```ruby
client = DiscourseAPIDocumentation::Client.new(
  max_retries:    3,
...
```ruby
client = DiscourseAPIDocumentation::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = DiscourseAPIDocumentation::Client.new(adapter: :net_http)
```
...
client = DiscourseAPIDocumentation::Client.new(connection: conn)
```
...
client = DiscourseAPIDocumentation::Client.new(environment: Environment::PRODUCTION)
client = DiscourseAPIDocumentation::Client.new(environment: Environment::ENVIRONMENT2)
```
...
```ruby
proxy = DiscourseAPIDocumentation::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = DiscourseAPIDocumentation::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = DiscourseAPIDocumentation::ProxySettings.from_env
client = DiscourseAPIDocumentation::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < DiscourseAPIDocumentation::HttpCallBack
...
client = DiscourseAPIDocumentation::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = DiscourseAPIDocumentation::Client.from_env
# Overrides take precedence over env vars:
client = DiscourseAPIDocumentation::Client.from_env(timeout: 30, max_retries: 3)
```
