...
`shutterstock/configuration.py` in the cloned source.
...
```python
import requests
from shutterstock.configuration import Configuration
...
`shutterstock/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`shutterstock/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `shutterstock/configuration.py`
...
`NumberPagedResponse`) live in `shutterstock/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `shutterstock/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
