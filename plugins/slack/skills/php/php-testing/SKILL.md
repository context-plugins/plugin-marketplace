...
> `SlackClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): SlackClient
{
...
```php
use SlackLib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use SlackLib\Exceptions\{OperationException};
use SlackLib\Exceptions\ApiException;
...
app()->instance(SlackClient::class, clientWithResponses([
