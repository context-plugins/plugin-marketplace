...
Confirm every name against `lib/coingecko/client.rb`, `lib/coingecko/configuration.rb`, the
`lib/coingecko/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
