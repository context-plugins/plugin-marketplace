...
```go
finnhubapi.WithOAuthCCGCredentials(
    finnhubapi.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr finnhubapi.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := finnhubapi.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr finnhubapi.OAuthCCGManager) models.OAuthToken {
...
```go
creds := finnhubapi.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := finnhubapi.NewClient(finnhubapi.CreateConfiguration(finnhubapi.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    finnhubapi.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := finnhubapi.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := finnhubapi.NewClient(finnhubapi.CreateConfiguration(finnhubapi.WithOAuthROPCGCredentials(creds)))
...
```go
finnhubapi.WithOAuthBearerTokenCredentials(finnhubapi.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
finnhubapi.WithBasicAuthCredentials(finnhubapi.NewBasicAuthCredentials(username, password))
finnhubapi.WithApiHeaderCredentials(finnhubapi.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
finnhubapi.WithApiKeyCredentials(finnhubapi.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `FINNHUBAPI_...` variables — e.g.
`FINNHUBAPI_USERNAME`, `FINNHUBAPI_PASSWORD`, `FINNHUBAPI_TOKEN`, `FINNHUBAPI_API_KEY`,
`FINNHUBAPI_O_AUTH_CLIENT_ID`, `FINNHUBAPI_O_AUTH_CLIENT_SECRET`, `FINNHUBAPI_ACCESS_TOKEN`. Grep the
