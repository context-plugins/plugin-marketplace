...
`DeploymentAdminClientClient`/`DeploymentAdminClientClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `DeploymentAdminClientClient`,
...
static DeploymentAdminClientClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new DeploymentAdminClientClient(new HttpClient(handler), new DeploymentAdminClientClientOptions { /* auth not needed for stubs */ });
}
```
...
using DeploymentAdminClient.Errors;              // {Operation}Error types
...
```csharp
using DeploymentAdminClient.Core.Exceptions;
using DeploymentAdminClient.Core.ErrorResponse;
...
using DeploymentAdminClient.Core.ErrorResponse; // RawError (Case B)
...
var client = new DeploymentAdminClientClient(new HttpClient(handler), new DeploymentAdminClientClientOptions());
...
- For DI-based code, the SDK's `AddDeploymentAdminClientClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `DeploymentAdminClientClient` from the provider:
  ```csharp
  services.AddDeploymentAdminClientClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<DeploymentAdminClientClient>();
  ```
