...
- `finnhubapi` — the root package name (e.g. `multiauthsample`, `batester`).
- `FinnhubapiClient` — the generated client class (read it from the `class …Client` declaration in `finnhubapi/finnhubapi_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `FinnhubapiClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from finnhubapi.finnhubapi_client import FinnhubapiClient
from finnhubapi.configuration import Environment
from finnhubapi.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from finnhubapi.configuration import Configuration, Environment
...
client = FinnhubapiClient(config=config)
```

Both patterns are equivalent — the client uses `config or Configuration(...)` in its `__init__`.
Confirm the exact kwarg names from `finnhubapi/finnhubapi_client.py` in the cloned source.
...
`from_environment()` is a class method on both `FinnhubapiClient` and `Configuration` that reads
...
client = FinnhubapiClient.from_environment()
...
`from_environment` in `finnhubapi/configuration.py` for the exact names.

## Accessing controllers

Controllers are `@LazyProperty` properties on `FinnhubapiClient`. Access them as attributes —
...
Open `finnhubapi/finnhubapi_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `finnhubapi/configuration.py`. **Read the enum in `configuration.py`
...
client = FinnhubapiClient(environment=Environment.PRODUCTION)
```

To inspect the URL each environment resolves to, read the `environments` dict in
`finnhubapi/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
...
client = FinnhubapiClient(config=new_config)
```
