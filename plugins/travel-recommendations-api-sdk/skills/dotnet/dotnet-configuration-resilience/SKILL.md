...
All types below live under `TravelRecommendationsApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using TravelRecommendationsApi.Core.Configuration;
...
```csharp
using TravelRecommendationsApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new TravelRecommendationsApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `TravelRecommendationsApi.Core.Exceptions`):
...
With DI, the SDK's `AddTravelRecommendationsApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddTravelRecommendationsApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
