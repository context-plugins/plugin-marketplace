...
```python
from docker.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `docker/utilities/union_type_lookup.py`) and each candidate model
...
```python
from docker.models.animal import Cat, Dog
...
```python
from docker.models.animal import Animal, Cat, Dog
