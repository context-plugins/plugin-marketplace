...
> Throughout, `seatmapdisplay` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := seatmapdisplay.NewClient(
    seatmapdisplay.CreateConfiguration(
        seatmapdisplay.WithBasicAuthCredentials(
            seatmapdisplay.NewBasicAuthCredentials(
...
seatmapdisplay.WithApiKeyCredentials(seatmapdisplay.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
seatmapdisplay.WithApiHeaderCredentials(seatmapdisplay.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
seatmapdisplay.WithOAuthBearerTokenCredentials(
    seatmapdisplay.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := seatmapdisplay.NewClient(
    seatmapdisplay.CreateConfiguration(
        seatmapdisplay.WithOAuthCCGCredentials(
            seatmapdisplay.NewOAuthCCGCredentials(
