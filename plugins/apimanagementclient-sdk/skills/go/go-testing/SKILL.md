...
`testing` + `testify/require` for reference; substitute the real `apimanagementclient` and names.
...
import (
    ...
    "github.com/context-plugins/apimanagementclient-go"   // root package apimanagementclient
)
...
func clientReturning(status int, body string) (apimanagementclient.ClientInterface, *stubTransport) {
    ...
    client := apimanagementclient.NewClient(
        apimanagementclient.CreateConfiguration(
            apimanagementclient.WithHttpConfiguration(
                apimanagementclient.CreateHttpConfiguration(apimanagementclient.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/apimanagementclient-go/errors"
...
client := apimanagementclient.NewClient(
    apimanagementclient.CreateConfiguration(
        apimanagementclient.WithHttpConfiguration(
            apimanagementclient.CreateHttpConfiguration(apimanagementclient.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
