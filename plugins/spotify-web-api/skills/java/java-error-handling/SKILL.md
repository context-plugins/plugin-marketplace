...
│   └── com.spotify.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.spotify.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.spotify.api.http.client.HttpContext`, which carries:
