...
```go
1passwordconnect.WithOAuthCCGCredentials(
    1passwordconnect.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr 1passwordconnect.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := 1passwordconnect.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr 1passwordconnect.OAuthCCGManager) models.OAuthToken {
...
```go
creds := 1passwordconnect.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := 1passwordconnect.NewClient(1passwordconnect.CreateConfiguration(1passwordconnect.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    1passwordconnect.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := 1passwordconnect.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := 1passwordconnect.NewClient(1passwordconnect.CreateConfiguration(1passwordconnect.WithOAuthROPCGCredentials(creds)))
...
```go
1passwordconnect.WithOAuthBearerTokenCredentials(1passwordconnect.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
1passwordconnect.WithBasicAuthCredentials(1passwordconnect.NewBasicAuthCredentials(username, password))
1passwordconnect.WithApiHeaderCredentials(1passwordconnect.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
1passwordconnect.WithApiKeyCredentials(1passwordconnect.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `1PASSWORDCONNECT_...` variables — e.g.
`1PASSWORDCONNECT_USERNAME`, `1PASSWORDCONNECT_PASSWORD`, `1PASSWORDCONNECT_TOKEN`, `1PASSWORDCONNECT_API_KEY`,
`1PASSWORDCONNECT_O_AUTH_CLIENT_ID`, `1PASSWORDCONNECT_O_AUTH_CLIENT_SECRET`, `1PASSWORDCONNECT_ACCESS_TOKEN`. Grep the
