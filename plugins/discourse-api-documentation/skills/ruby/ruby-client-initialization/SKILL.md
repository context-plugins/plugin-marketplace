...
- `DiscourseAPIDocumentation` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `discourse_api_documentation` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'discourse_api_documentation'
include DiscourseAPIDocumentation
...
The authoritative list is the `initialize` signature in `lib/discourse_api_documentation/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/discourse_api_documentation/configuration.rb`
(`build_default_config_from_env`) and `lib/discourse_api_documentation/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
Environment::ENVIRONMENT2 # => 'environment2'   (further servers are numbered, not named)
```
...
In Rails, create the client in an initializer (`config/initializers/discourse_api_documentation.rb`) and store it in a
