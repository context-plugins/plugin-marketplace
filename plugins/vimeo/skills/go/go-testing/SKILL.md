...
`testing` + `testify/require` for reference; substitute the real `vimeo` and names.
...
import (
    ...
    "github.com/context-plugins/vimeo-go-sdk"   // root package vimeo
)
...
func clientReturning(status int, body string) (vimeo.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := vimeo.NewClient(
        vimeo.CreateConfiguration(
            vimeo.WithHttpConfiguration(
                vimeo.CreateHttpConfiguration(vimeo.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/vimeo-go-sdk/errors"
...
client := vimeo.NewClient(
    vimeo.CreateConfiguration(
        vimeo.WithHttpConfiguration(
            vimeo.CreateHttpConfiguration(vimeo.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
