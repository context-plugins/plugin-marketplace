...
kwarg names are generated per-API — confirm from `tesla/tesla_client.py` and `tesla/http/auth/` in
the cloned source.

## Basic auth

```python
from tesla.http.auth.basic_auth import BasicAuthCredentials
...
```python
from tesla.http.auth.api_key import ApiKeyCredentials
...
```python
from tesla.http.auth.api_header import ApiHeaderCredentials
...
```python
from tesla.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from tesla.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from tesla.http.auth.o_auth_acg import OAuthACGCredentials
from tesla.models.o_auth_scope_tesla_enum import OAuthScope{Pkg}Enum
...
```python
from tesla.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
Grep `from_environment` in `tesla/configuration.py` and each `tesla/http/auth/*.py` for the exact
