...
`testing` + `testify/require` for reference; substitute the real `apisguru` and names.
...
import (
    ...
    "github.com/context-plugins/apis-guru-go"   // root package apisguru
)
...
func clientReturning(status int, body string) (apisguru.ClientInterface, *stubTransport) {
    ...
    client := apisguru.NewClient(
        apisguru.CreateConfiguration(
            apisguru.WithHttpConfiguration(
                apisguru.CreateHttpConfiguration(apisguru.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/apis-guru-go/errors"
...
client := apisguru.NewClient(
    apisguru.CreateConfiguration(
        apisguru.WithHttpConfiguration(
            apisguru.CreateHttpConfiguration(apisguru.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
