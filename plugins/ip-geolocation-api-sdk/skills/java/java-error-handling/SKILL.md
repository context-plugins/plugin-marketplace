...
│   └── {rootpkg}.exceptions.ApiException          ← base for all non-2xx API responses
│       └── {rootpkg}.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `{rootpkg}.http.client.HttpContext`, which carries:
