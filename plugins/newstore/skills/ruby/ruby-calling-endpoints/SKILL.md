...
Open `lib/newstore/client.rb` for the accessor names, then the relevant
`lib/newstore/controllers/{resource}_controller.rb` for the operation's exact signature.
...
```ruby
body = Newstore::{RequestModel}.new(
  field_one: 'value',
  field_two: 42,
  nested: Newstore::{NestedModel}.new(sub_field: true)
)
result = client.{resource}.{create_operation}(body)
```

Model classes extend `BaseModel` and are in `lib/newstore/models/`. Field names are defined as
...
Read the enum module in `lib/newstore/models/{enum}_enum.rb` for the available constants.
...
- Controller accessors are on `Client` in `lib/newstore/client.rb`.
- Operation methods are in `lib/newstore/controllers/{resource}_controller.rb`.
...
- Request/response model classes are in `lib/newstore/models/`; enum modules end in `_enum.rb`.
- Typed error classes are in `lib/newstore/exceptions/`.
