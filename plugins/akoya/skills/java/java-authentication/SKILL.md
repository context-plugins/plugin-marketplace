...
To see which schemes a specific SDK accepts, read the credential setter methods on `AkoyaClient.Builder`
...
- Set credentials **on the `AkoyaClient.Builder`** before calling `.build()` — the client is immutable
