...
Open `hotelratings/hotelratings_client.py` for the controller property names, then the relevant
`hotelratings/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from hotelratings.models.{request_model} import {RequestModel}
...
Open `hotelratings/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from hotelratings.models.suite_code_enum import SuiteCodeEnum
...
client = HotelRatingsClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `hotelratings/pagination/` in the source.

## Finding the right method

- Controller property names are on `HotelRatingsClient` in `hotelratings/hotelratings_client.py`.
- Operation method signatures are in `hotelratings/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `hotelratings/models/`; enum types have `from_value()`.
