...
```ruby
client = ShellAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = ShellAPI::Client.new(adapter: :net_http)
```
...
client = ShellAPI::Client.new(connection: conn)
```
...
client = ShellAPI::Client.new(environment: Environment::PRODUCTION)
client = ShellAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = ShellAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = ShellAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = ShellAPI::ProxySettings.from_env
client = ShellAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < ShellAPI::HttpCallBack
...
client = ShellAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = ShellAPI::Client.from_env
# Overrides take precedence over env vars:
client = ShellAPI::Client.from_env(timeout: 30, max_retries: 3)
```
