...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new NetworkAdminManagementClientClient({
...
```typescript
import { {Operation}Error } from 'networkadminmanagementclientlib/errors';
...
```typescript
import { ApiError } from 'networkadminmanagementclientlib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(NetworkAdminManagementClientClient)
    .useValue(new NetworkAdminManagementClientClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
