...
| Source commit (spec stamp) | `c40b869` (`c40b86985b4a5496150095f906542ba923033d41`, tagged `c40b869`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
| Repo | https://github.com/context-plugins/finnhub-csharp-sdk (branch `main`) |
...
```csharp
using FinnhubApi;
using FinnhubApi.Servers; // ServerEnvironment lives here

var options = new FinnhubApiClientOptions
{
...
var client = new FinnhubApiClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```

DI alternative (`ServiceCollectionExtensions.cs`):

```csharp
services.AddFinnhubApiClient(o =>
{
...
All `FinnhubApiClientOptions` properties (source: `FinnhubApiClientOptions.cs`):
...
- `FinnhubApiClient(HttpClient httpClient, FinnhubApiClientOptions options)`
<!-- /crawler:client-options -->
...
Of **117 operations**, **0 are Case A (typed)** and **117 are Case B (raw)**.
<!-- /crawler:op-stats -->

---

## Operations — by controller (1 groups, 117 operations)
...
|---|---:|---|
| `FinnhubApiClient` | 117 | [map/operations/FinnhubApiClient.md](map/operations/FinnhubApiClient.md) |
<!-- /crawler:ops-table -->
...
| Group | Count | Page |
|---|---:|---|
| Records (plain `record` data models) | 192 | [`AggregateIndicators` … `VisaApplicationResult`](map/models/records-1-Ag-Vi.md) |
...
|---|---|
| Client & options (root) | `FinnhubApi` |
| Records (`Models/`) | `FinnhubApi.Models` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `FinnhubApiClientOptions` (source: `FinnhubApiClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:
