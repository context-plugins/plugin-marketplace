...
| Source commit (spec stamp) | `e2d0abc` (`e2d0abc6a8ffabe4edcbb497a9a408a83b127984`, tagged `e2d0abc`) |
<!-- /gen:stamp -->
| Generator | APIMatic |
...
```csharp
using Nytimes;
using Nytimes.Servers; // ServerEnvironment lives here
...
var client = new NytimesClient(httpClient, options); // httpClient: System.Net.Http.HttpClient
```
...
All `NytimesClientOptions` properties (source: `NytimesClientOptions.cs`):
...
- `NytimesClient(HttpClient httpClient, NytimesClientOptions options)`
<!-- /crawler:client-options -->
...
|---|---|
| Client & options (root) | `Nytimes` |
| Operation controllers (`Api/`) | `Nytimes.Api` |
| Records (`Models/`) | `Nytimes.Models` |
| Enums (`Models/Enums/`) | `Nytimes.Models.Enums` |
| Error classes (`Errors/`) | `Nytimes.Errors` |
<!-- /gen:namespaces -->
...
**Auth.** The scheme(s) this API uses surface as credentials properties on `NytimesClientOptions` (source: `NytimesClientOptions.cs`) — set them before constructing the client; load `dotnet-authentication` for the wiring:
