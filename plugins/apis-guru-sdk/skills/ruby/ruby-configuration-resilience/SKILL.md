...
```ruby
client = APIsGuru::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = APIsGuru::Client.new(adapter: :net_http)
```
...
client = APIsGuru::Client.new(connection: conn)
```
...
client = APIsGuru::Client.new(environment: Environment::PRODUCTION)
client = APIsGuru::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = APIsGuru::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = APIsGuru::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = APIsGuru::ProxySettings.from_env
client = APIsGuru::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < APIsGuru::HttpCallBack
...
client = APIsGuru::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = APIsGuru::Client.from_env
# Overrides take precedence over env vars:
client = APIsGuru::Client.from_env(timeout: 30, max_retries: 3)
```
