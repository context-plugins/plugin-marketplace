...
> Throughout, `firecrawl` and other `{...}` tokens are placeholders for names you take from your SDK —
...
client := firecrawl.NewClient(
    firecrawl.CreateConfiguration(
        firecrawl.WithBasicAuthCredentials(
            firecrawl.NewBasicAuthCredentials(
...
firecrawl.WithApiKeyCredentials(firecrawl.NewApiKeyCredentials("token", "api-key"))

// custom header scheme (often surfaced as ApiHeaderCredentials):
firecrawl.WithApiHeaderCredentials(firecrawl.NewApiHeaderCredentials("token", "api-key"))
```
...
```go
firecrawl.WithOAuthBearerTokenCredentials(
    firecrawl.NewOAuthBearerTokenCredentials(os.Getenv("{ACCESS_TOKEN_ENV}")),
)
```
...
client := firecrawl.NewClient(
    firecrawl.CreateConfiguration(
        firecrawl.WithOAuthCCGCredentials(
            firecrawl.NewOAuthCCGCredentials(
