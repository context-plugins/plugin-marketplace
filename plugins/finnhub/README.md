...
Integrate and consume the **APIMatic-generated Finnhub SDK** (the "APIMATIC v3.0" generic
...
(`dotnet-finnhub-getting-started/sdk-map.md` + `map/`) indexing every operation signature,
model, enum, union, and error type. The **`dotnet-integrate-finnhub`** router skill routes all
C#/.NET SDK work to the single **`dotnet-finnhub-sdk`** agent (.NET-only), which answers
