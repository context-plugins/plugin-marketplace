...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `OpenUvGlobalRealTimeUvIndexForecastApi`,
> `OpenUvGlobalRealTimeUvIndexForecastApiClientOptions`, `{BasicAuthProperty}`) — replace it with the concrete identifier from the source.

To see which schemes a specific SDK accepts, read the **credentials properties on its `OpenUvGlobalRealTimeUvIndexForecastApiClientOptions`
class** — those are the source of truth (read the class in the SDK source, not a decompiled or reflected
view of the installed package). The `OpenUvGlobalRealTimeUvIndexForecastApi.Core.Authentication` folder ships *every*
...
The credential classes below live under `OpenUvGlobalRealTimeUvIndexForecastApi.Core.Authentication.*` and are the **same across
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Authentication.Basic;
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Authentication.OAuth2.ClientCredentials;
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Authentication.OAuth2.AuthorizationCode;
...
```csharp
using OpenUvGlobalRealTimeUvIndexForecastApi.Core.Authentication.OAuth2.Password;
...
- Set credentials **before** constructing the client, or inside the `AddOpenUvGlobalRealTimeUvIndexForecastApiClient(options => ...)`
...
- Keep secrets out of source — load them from configuration (environment variables, a secret store, or any
  other `IConfiguration` source) instead of hardcoding, either inside the `AddOpenUvGlobalRealTimeUvIndexForecastApiClient(options => ...)`
