...
```go
tesserapiv1.WithOAuthCCGCredentials(
    tesserapiv1.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr tesserapiv1.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := tesserapiv1.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr tesserapiv1.OAuthCCGManager) models.OAuthToken {
...
```go
creds := tesserapiv1.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := tesserapiv1.NewClient(tesserapiv1.CreateConfiguration(tesserapiv1.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    tesserapiv1.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := tesserapiv1.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := tesserapiv1.NewClient(tesserapiv1.CreateConfiguration(tesserapiv1.WithOAuthROPCGCredentials(creds)))
...
```go
tesserapiv1.WithOAuthBearerTokenCredentials(tesserapiv1.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
tesserapiv1.WithBasicAuthCredentials(tesserapiv1.NewBasicAuthCredentials(username, password))
tesserapiv1.WithApiHeaderCredentials(tesserapiv1.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
tesserapiv1.WithApiKeyCredentials(tesserapiv1.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `TESSERAPIV1_...` variables — e.g.
`TESSERAPIV1_USERNAME`, `TESSERAPIV1_PASSWORD`, `TESSERAPIV1_TOKEN`, `TESSERAPIV1_API_KEY`,
`TESSERAPIV1_O_AUTH_CLIENT_ID`, `TESSERAPIV1_O_AUTH_CLIENT_SECRET`, `TESSERAPIV1_ACCESS_TOKEN`. Grep the
