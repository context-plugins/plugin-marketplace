...
> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `{operation}`, `{apiGroup}`, `goeapilib`) — replace it with the concrete identifier from the source.
...
```typescript
import { Client } from 'goeapilib';
import { ApiError } from 'goeapilib';
import { {Operation}Error } from 'goeapilib/errors';
