...
│   └── com.openai.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.openai.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.openai.api.http.client.HttpContext`, which carries:
