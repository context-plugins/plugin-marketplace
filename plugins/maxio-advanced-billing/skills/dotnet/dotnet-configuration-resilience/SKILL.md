...
All types below live under `MaxioAdvancedBilling.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using MaxioAdvancedBilling.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new MaxioAdvancedBillingClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `MaxioAdvancedBilling.Core.Exceptions`):
...
With DI, the SDK's `AddMaxioAdvancedBillingClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddMaxioAdvancedBillingClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
