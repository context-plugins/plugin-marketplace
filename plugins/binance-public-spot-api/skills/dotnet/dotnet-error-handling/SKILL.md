...
> `{ApiGroup}`, `BinancePublicSpotApi`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `BinancePublicSpotApi.Core.Exceptions`
- `ApiError` **and** `RawError` → `BinancePublicSpotApi.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `BinancePublicSpotApi.Errors`
...
```csharp
using BinancePublicSpotApi.Core.Exceptions;     // SdkException<TError>
using BinancePublicSpotApi.Core.ErrorResponse;  // ApiError, RawError
using BinancePublicSpotApi.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`BinancePublicSpotApi.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using BinancePublicSpotApi.Core.Exceptions;     // SdkException<TError>
using BinancePublicSpotApi.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `BinancePublicSpotApi.Core.ErrorResponse`. Its public
...
`readonly struct` under `BinancePublicSpotApi.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using BinancePublicSpotApi.Core.Models;        // ApiResult<TResponse, TError>
using BinancePublicSpotApi.Core.ErrorResponse; // RawError
using BinancePublicSpotApi.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `BinancePublicSpotApi.Core.Exceptions`) — an auth *application* failure, not an API error — when the
...
- **A scheme's own request to obtain credentials failing is not `AuthSchemeException` — it's an ordinary
  `SdkException<RawError>`.** Some auth schemes (e.g. an OAuth2 grant) make their own request to acquire or
  refresh a token before the real call goes out; if that request itself is rejected (bad credentials,
  expired refresh token, etc.), the SDK does not wrap it in a dedicated auth exception — it surfaces on the
  call you made as `SdkException<RawError>`, the same shape as any Case-B operation, **regardless of whether
  that operation is Case A or Case B**. `AuthSchemeException` only covers *scheme-selection* failures
  (composite AND/OR, above), never a single scheme's own credential request failing. So a `catch` written
  only for the endpoint's documented `{Operation}Error` still misses this — also catch
  `SdkException<RawError>` on any call guarded by a scheme that performs its own request. See
