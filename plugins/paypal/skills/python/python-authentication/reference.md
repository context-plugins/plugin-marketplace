...
kwargs to the `PaypalServersdkClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `paypal/paypal_client.py` and `paypal/http/auth/` in
the cloned source.

## Basic auth

```python
from paypal.http.auth.basic_auth import BasicAuthCredentials
...
```python
from paypal.http.auth.api_key import ApiKeyCredentials
...
```python
from paypal.http.auth.api_header import ApiHeaderCredentials
...
```python
from paypal.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from paypal.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from paypal.http.auth.o_auth_acg import OAuthACGCredentials
from paypal.models.o_auth_scope_paypal_enum import OAuthScope{Pkg}Enum
...
client = PaypalServersdkClient(o_auth_acg_credentials=creds)
...
```python
from paypal.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = PaypalServersdkClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `PaypalServersdkClient.from_environment()`) read each credential
...
Grep `from_environment` in `paypal/configuration.py` and each `paypal/http/auth/*.py` for the exact
