...
client = stub_client(stubs)
assert_raises Akoya::APIException do
...
BASE = Akoya::Configuration.new(environment: Akoya::Environment::PRODUCTION).get_base_uri

def build_test_client
  Akoya::Client.new(
    environment: Akoya::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Akoya::APIException
```
...
```ruby
expect { call }.to raise_error(Akoya::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Akoya::HttpCallBack
...
client  = Akoya::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
