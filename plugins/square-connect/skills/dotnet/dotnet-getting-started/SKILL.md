...
| Project | `SquareConnectApi.csproj` at the repo root — referenced from the cloned source (see **Install**) |
| Root namespace | `SquareConnectApi` (the `using` namespace) |
| Client class | `SquareConnectApiClient` |
| Options class | `SquareConnectApiClientOptions` |
...
namespaces transitively, so `using SquareConnectApi.Models;` alone does **not** make enums, union types, or
...
git clone --branch main https://github.com/context-plugins/square-connect-api-csharp
dotnet add reference square-connect-api-csharp/SquareConnectApi.csproj
```

```csharp
using SquareConnectApi;          // client + options + ServerEnvironment
using SquareConnectApi.Models;   // request/response types, enums, unions
```
...
```bash
# Linux / macOS:
git clone --depth 1 --branch main https://github.com/context-plugins/square-connect-api-csharp /tmp/square-connect-api-dotnet-src
```

```powershell
# Windows (PowerShell):
git clone --depth 1 --branch main https://github.com/context-plugins/square-connect-api-csharp "$env:TEMP\square-connect-api-dotnet-src"
```
...
- `SquareConnectApiClient.cs`, the `ServerEnvironment` values, and the DI registration (`ServiceCollectionExtensions`).
...
```bash
rm -rf /tmp/square-connect-api-dotnet-src                               # Linux / macOS
```
```powershell
Remove-Item -Recurse -Force "$env:TEMP\square-connect-api-dotnet-src"   # Windows
```
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you construct `SquareConnectApiClient`, build its
   `SquareConnectApiClientOptions`, or DI-register it. (*The signature won't tell you:* the `HttpClient` and client must
