...
kwargs to the `TravelRecommendationsAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `travelrecommendationsapi/travelrecommendationsapi_client.py` and `travelrecommendationsapi/http/auth/` in
the cloned source.

## Basic auth

```python
from travelrecommendationsapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from travelrecommendationsapi.http.auth.api_key import ApiKeyCredentials
...
```python
from travelrecommendationsapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from travelrecommendationsapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from travelrecommendationsapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from travelrecommendationsapi.http.auth.o_auth_acg import OAuthACGCredentials
from travelrecommendationsapi.models.o_auth_scope_travelrecommendationsapi_enum import OAuthScope{Pkg}Enum
...
client = TravelRecommendationsAPIClient(o_auth_acg_credentials=creds)
...
```python
from travelrecommendationsapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = TravelRecommendationsAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `TravelRecommendationsAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `travelrecommendationsapi/configuration.py` and each `travelrecommendationsapi/http/auth/*.py` for the exact
