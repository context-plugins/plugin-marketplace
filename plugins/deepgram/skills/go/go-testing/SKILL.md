...
`testing` + `testify/require` for reference; substitute the real `deepgram` and names.
...
import (
    ...
    "github.com/context-plugins/deepgram-go-sdk"   // root package deepgram
)
...
func clientReturning(status int, body string) (deepgram.ClientInterface, *stubTransport) {
    ...
    client := deepgram.NewClient(
        deepgram.CreateConfiguration(
            deepgram.WithHttpConfiguration(
                deepgram.CreateHttpConfiguration(deepgram.WithTransport(stub)),
            ),
...
client := deepgram.NewClient(
    deepgram.CreateConfiguration(
        deepgram.WithHttpConfiguration(
            deepgram.CreateHttpConfiguration(deepgram.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
