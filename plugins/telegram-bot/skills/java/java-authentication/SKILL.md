...
To see which schemes a specific SDK accepts, read the credential setter methods on `TelegramBotAPIClient.Builder`
...
- Set credentials **on the `TelegramBotAPIClient.Builder`** before calling `.build()` — the client is immutable
