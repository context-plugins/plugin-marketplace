...
- `Postnl` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `postnl` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/postnl/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/postnl/configuration.rb`
(`build_default_config_from_env`) and `lib/postnl/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
```ruby
# Application startup:
CLIENT = Postnl::Client.from_env

# In handlers or services:
result = CLIENT.{resource}.{operation}(...)
```

In Rails, create the client in an initializer (`config/initializers/postnl.rb`) and store it in a
