...
│   └── io.m3commas.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── io.m3commas.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `io.m3commas.api.http.client.HttpContext`, which carries:
