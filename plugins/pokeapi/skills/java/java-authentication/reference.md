...
against the `PokApiClient.Builder` setters, the `co.pokeapi.authentication/` source, and `doc/auth/*.md`
in the cloned SDK.
...
on `PokApiClient.Builder` before calling `.build()`. The client is **immutable** after construction. To
...
PokApiClient client = new PokApiClient.Builder()
    .oAuthACGCredentials(new OAuthACGModel.Builder(
        ...
        .oAuthScopes(Arrays.asList(OAuthScopePokeApiEnum.READ_SCOPE))  // per-API scope enum
        .build())
    .build();
```
