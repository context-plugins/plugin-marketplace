...
- `klarna` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/klarna-go-sdk` — the Go import path for the root package; `github.com/context-plugins/klarna-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/klarna-go-sdk"

client := klarna.NewClient(
    klarna.CreateConfiguration(
        klarna.WithEnvironment(klarna.PRODUCTION),
...
from environment variables named `KLARNA_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := klarna.NewClient(klarna.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `klarna.PRODUCTION`,
`klarna.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := klarna.NewClient(
    klarna.CreateConfiguration(klarna.WithEnvironment(klarna.PRODUCTION)),
)
```
...
client := klarna.NewClient(
    klarna.CreateConfiguration(
        klarna.WithHttpConfiguration(
            klarna.CreateHttpConfiguration(
                klarna.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                klarna.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = klarna.NewClient(klarna.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api klarna.ClientInterface
}

func NewService(api klarna.ClientInterface) *Service {
