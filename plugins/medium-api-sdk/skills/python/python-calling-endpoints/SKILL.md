...
Open `mediumapi/mediumapi_client.py` for the controller property names, then the relevant
`mediumapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from mediumapi.models.{request_model} import {RequestModel}
...
Open `mediumapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from mediumapi.models.suite_code_enum import SuiteCodeEnum
...
client = MediumAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `mediumapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `MediumAPIClient` in `mediumapi/mediumapi_client.py`.
- Operation method signatures are in `mediumapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `mediumapi/models/`; enum types have `from_value()`.
