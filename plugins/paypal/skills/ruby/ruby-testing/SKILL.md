...
client = stub_client(stubs)
assert_raises Paypal::APIException do
...
BASE = Paypal::Configuration.new(environment: Paypal::Environment::PRODUCTION).get_base_uri

def build_test_client
  Paypal::Client.new(
    environment: Paypal::Environment::PRODUCTION,  # read the real base URL from configuration.rb
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error Paypal::APIException
```
...
```ruby
expect { call }.to raise_error(Paypal::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < Paypal::HttpCallBack
...
client  = Paypal::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
