...
> `{operation}`, `{apiGroup}`, `FirecrawlApiLib`) — replace it with the concrete identifier from
> the source.

Endpoint methods **throw on non-success responses**. The base thrown type is always `ApiException`
(under `FirecrawlApiLib\Exceptions\ApiException`, which extends `\Exception`). Depending on
...
```php
use FirecrawlApiLib\Exceptions\ApiException;
...
```php
use FirecrawlApiLib\Exceptions\ApiException;
use FirecrawlApiLib\Exceptions\{OperationException};
...
- All `ApiException` subclasses live under `FirecrawlApiLib\Exceptions\` — read the source to
