...
All types below live under `OpenAiApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using OpenAiApi.Core.Configuration;
...
```csharp
using OpenAiApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new OpenAiApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `OpenAiApi.Core.Exceptions`):
...
With DI, the SDK's `AddOpenAiApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddOpenAiApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
