...
All types below live under `HotelSearchApi.Core.Configuration` / `.Servers` and are generic across
APIMatic .NET SDKs.
...
```csharp
using HotelSearchApi.Core.Configuration;
...
```csharp
using HotelSearchApi.Core.Exceptions;   // SseException, SseTimeoutException, SseDeserializationException
...
the client:
```csharp
var options = new HotelSearchApiClientOptions { StreamReadTimeout = TimeSpan.FromSeconds(30) };
```

**Errors** (all under `HotelSearchApi.Core.Exceptions`):
...
With DI, the SDK's `AddHotelSearchApiClient` resolves the **default (unnamed)** `IHttpClientFactory` client, so attach
...
services.AddHotelSearchApiClient(options => { /* ... */ });   // resolves CreateClient() → the default client
```
