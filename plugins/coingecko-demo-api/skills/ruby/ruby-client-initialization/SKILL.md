...
- `CoinGeckoDemoAPI` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `coingecko_demo_api` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
```ruby
require 'coingecko_demo_api'
include CoinGeckoDemoAPI
...
The authoritative list is the `initialize` signature in `lib/coingecko_demo_api/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/coingecko_demo_api/configuration.rb`
(`build_default_config_from_env`) and `lib/coingecko_demo_api/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/coingecko_demo_api.rb`) and store it in a
