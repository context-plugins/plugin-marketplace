...
Confirm every name against `lib/paypal_api/client.rb`, `lib/paypal_api/configuration.rb`, the
`lib/paypal_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
