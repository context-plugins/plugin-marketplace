...
`locationscore/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `locationscore/exceptions/` and operation docstrings.
...
```python
from locationscore.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `locationscore/exceptions/`.
...
```python
from locationscore.exceptions.api_exception import APIException
from locationscore.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`locationscore/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`locationscore/exceptions/` for the class.
...
`locationscore/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `locationscore/exceptions/`
  or a `ValueError` — check the auth handler in `locationscore/http/auth/` for the exact type.
