...
> `EventsAPIClient`, `{apiGroup}`, `{operation}`) — replace it with the concrete identifier from the
> source.
 ...
 */
function clientWithResponses(array $responses, array &$container = []): EventsAPIClient
{
...
```php
use EventsAPILib\Exceptions\ApiException;
use GuzzleHttp\Psr7\Response;
use PHPUnit\Framework\TestCase;
...
```php
use EventsAPILib\Exceptions\{OperationException};
use EventsAPILib\Exceptions\ApiException;
...
app()->instance(EventsAPIClient::class, clientWithResponses([
