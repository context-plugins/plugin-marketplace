...
│   └── io.payquicker.sandbox.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── io.payquicker.sandbox.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `io.payquicker.sandbox.api.http.client.HttpContext`, which carries:
