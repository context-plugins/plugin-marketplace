...
`notion/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `notion/exceptions/` and operation docstrings.
...
```python
from notion.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `notion/exceptions/`.
...
```python
from notion.exceptions.api_exception import APIException
from notion.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`notion/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`notion/exceptions/` for the class.
...
`notion/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `notion/exceptions/`
  or a `ValueError` — check the auth handler in `notion/http/auth/` for the exact type.
