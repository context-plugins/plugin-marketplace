...
To see which schemes a specific SDK accepts, read the credential setter methods on `DockerClient.Builder`
...
- Set credentials **on the `DockerClient.Builder`** before calling `.build()` — the client is immutable
