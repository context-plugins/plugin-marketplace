...
client = stub_client(stubs)
assert_raises HotelRatings::APIException do
...
def build_test_client
  HotelRatings::Client.new(
    environment: HotelRatings::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error HotelRatings::APIException
```
...
```ruby
expect { call }.to raise_error(HotelRatings::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < HotelRatings::HttpCallBack
...
client  = HotelRatings::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
