...
```ruby
client = GoogleMapsPlatform::Client.new(
  max_retries:    3,
...
```ruby
client = GoogleMapsPlatform::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = GoogleMapsPlatform::Client.new(adapter: :net_http)
```
...
client = GoogleMapsPlatform::Client.new(connection: conn)
```
...
client = GoogleMapsPlatform::Client.new(environment: Environment::{MEMBER})
```
...
```ruby
proxy = GoogleMapsPlatform::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = GoogleMapsPlatform::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = GoogleMapsPlatform::ProxySettings.from_env
client = GoogleMapsPlatform::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < GoogleMapsPlatform::HttpCallBack
...
client = GoogleMapsPlatform::Client.new(http_callback: LogCallback.new)
```
...
client = GoogleMapsPlatform::Client.from_env(timeout: 30, max_retries: 3)
```
