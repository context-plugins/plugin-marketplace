...
- `automationmanagementclient` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/automationmanagementclient-go` — the Go import path for the root package; `github.com/context-plugins/automationmanagementclient-go/models` for request/response types.
...
```go
import "github.com/context-plugins/automationmanagementclient-go"

client := automationmanagementclient.NewClient(
    automationmanagementclient.CreateConfiguration(
        automationmanagementclient.WithEnvironment(automationmanagementclient.PRODUCTION),
...
from environment variables named `AUTOMATIONMANAGEMENTCLIENT_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := automationmanagementclient.NewClient(automationmanagementclient.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `automationmanagementclient.PRODUCTION`,
`automationmanagementclient.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := automationmanagementclient.NewClient(
    automationmanagementclient.CreateConfiguration(automationmanagementclient.WithEnvironment(automationmanagementclient.PRODUCTION)),
)
```
...
client := automationmanagementclient.NewClient(
    automationmanagementclient.CreateConfiguration(
        automationmanagementclient.WithHttpConfiguration(
            automationmanagementclient.CreateHttpConfiguration(
                automationmanagementclient.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                automationmanagementclient.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = automationmanagementclient.NewClient(automationmanagementclient.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api automationmanagementclient.ClientInterface
}

func NewService(api automationmanagementclient.ClientInterface) *Service {
