...
`adyenbinlookupapi/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `adyenbinlookupapi/exceptions/` and operation docstrings.
...
```python
from adyenbinlookupapi.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `adyenbinlookupapi/exceptions/`.
...
```python
from adyenbinlookupapi.exceptions.api_exception import APIException
from adyenbinlookupapi.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`adyenbinlookupapi/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`adyenbinlookupapi/exceptions/` for the class.
...
`adyenbinlookupapi/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `adyenbinlookupapi/exceptions/`
  or a `ValueError` — check the auth handler in `adyenbinlookupapi/http/auth/` for the exact type.
