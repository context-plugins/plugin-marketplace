...
description: "Firecrawl API TypeScript SDK identity and lookup layer (TypeScript/JavaScript only) — install, the single import specifier `firecrawl-api`, the server environments and the base-URL knob, the auth pattern, the SDK map that ships inside the installed package (`sdk-map.md` + `map/operations/`) and how to traverse it, and the file table naming the one source file owning each fact the map leaves to the source. Load this before answering any Firecrawl API TypeScript SDK contract question or writing any SDK code."
---

# Getting started with the Firecrawl API TypeScript SDK

> **Who this skill is for.** This is the **lookup layer** for anyone writing Firecrawl API TypeScript SDK code — it is yours to follow directly and fully. Ground every contract fact here (in the SDK map, and in the source files it names) rather than in recall, and carry those facts onto a contract sheet before you implement. Load `typescript-integrate-firecrawl` for the workflow that wraps this skill.
...
Verified against `package.json` and `sdk-map.md` of the generated package at version `v2`. **Re-verify after a version bump** — this page is a snapshot, not a live read.

| Fact | Value |
| --- | --- |
| API | Firecrawl API |
| Package name (what you install, and what you import) | `firecrawl-api` — **not on npm**; built from source (see *Install*) |
| Import specifier | `firecrawl-api` — the package root is the **only** entry; deep imports do not resolve |
| Version | `v2` (API spec version `v2`) |
| Client class | `FirecrawlApiClient` (`src/client.ts`) — one class, no sync/async split |
| Options type | `ClientOptions`, with `DEFAULT_CLIENT_OPTIONS` beside it (`src/client-options.ts`) |
| Client construction | `new FirecrawlApiClient(clientOptions: Partial<ClientOptions> = {})` — **every** field is optional, so `new FirecrawlApiClient()` compiles. Fields: `serverEnvironment` · `serverOptions` · `timeout` · `fetch` · `bearerAuth`. `timeout` defaults to `60_000` ms |
...
| Base-URL config | `serverOptions.<group>.<environment>.baseUrl` (`src/servers.ts`), defaulting to `https://api.firecrawl.dev/v2` |
...
| Surface | 52 operations · 16 resources · 303 models · 51 open enums · 16 unions · 45 per-operation error subclasses |
...
```bash
npm install git+https://github.com/context-plugins/firecrawl-typescript-sdk#main
```
...
**Every** public name is re-exported from the package root — the client, `ClientOptions`, `ServerEnvironment`, 370 model types with the schema value beside each, the error classes, and the runtime types (`ApiPromise`, `ApiResult`, `RequestOptions`, `ErrorPayload`, `Declared`, `Schema`, `EnumSchema`, `Encoded`).

```ts
import { FirecrawlApiClient, ServerEnvironment, ResponseError, FirecrawlApiError } from "firecrawl-api";
import type { ClientOptions, Action } from "firecrawl-api";
```

Things the specifier alone will not tell you:

- **Deep imports do not resolve.** The `exports` map exposes `.` and `./package.json` and nothing else, so `firecrawl-api/models/…` fails (`TS2307`) even though the file exists in the shipped `src/`. Every `Source` path on the SDK map is where to **read** a shape, never what to import.
- **⚠ The SDK exports a model type literally named `Error`** (`src/models/error.ts`, schema `errorSchema`). Import it unaliased and it **shadows the global `Error`** for the rest of the file. Alias it: `import type { Error as SdkError } from "firecrawl-api"`.
- **⚠ The SDK exports a model type literally named `Event`** (`src/models/event.ts`, schema `eventSchema`). Import it unaliased and it **shadows the global `Event`** for the rest of the file. Alias it: `import type { Event as SdkEvent } from "firecrawl-api"`.
- **From CommonJS**, the typed spelling is `import sdk = require("firecrawl-api")`. A plain `require` destructure runs but yields `any`.
...
| --- | --- | --- | --- |
| `default` | `production` *(default)* | `https://api.firecrawl.dev/v2` | `serverOptions.default.production.baseUrl` |
...
Authentication is **per operation**: every operation declares the requirement it enforces and the SDK sends exactly that. Each block on a map page carries an **Auth** bullet, `none` included. There is no client-global switch and no per-call override. 52 of the 52 operations require a credential and 0 are public.
...
```ts
const client = new FirecrawlApiClient({
...
| --- | --- | --- | --- |
| `client.monitoring` | `Monitoring` | 8 | `createMonitor` · `deleteMonitor` · `getMonitor` · `getMonitorCheck` · `listMonitorChecks` · `listMonitors` · `runMonitor` · `updateMonitor` |
| `client.scraping` | `Scraping` | 9 | `cancelBatchScrape` · `getBatchScrapeErrors` · `getBatchScrapeStatus` · `getScrapeStatus` · `interactWithScrapeBrowserSession` · `parseFile` · `scrapeAndExtractFromUrl` · `scrapeAndExtractFromUrls` · `stopInteractiveScrapeBrowserSession` |
| `client.crawling` | `Crawling` | 6 | `cancelCrawl` · `crawlParamsPreview` · `crawlUrls` · `getActiveCrawls` · `getCrawlErrors` · `getCrawlStatus` |
| `client.mapping` | `Mapping` | 1 | `mapUrls` |
| `client.extraction` | `Extraction` | 2 | `extractData` · `getExtractStatus` |
| `client.agent` | `Agent` | 3 | `cancelAgent` · `getAgentStatus` · `startAgent` |
| `client.billing` | `Billing` | 4 | `getCreditUsage` · `getHistoricalCreditUsage` · `getHistoricalTokenUsage` · `getTokenUsage` |
| `client.miscellaneous` | `Miscellaneous` | 1 | `getQueueStatus` |
| `client.account` | `Account` | 1 | `getActivity` |
| `client.search` | `Search` | 2 | `searchAndScrape` · `submitSearchFeedback` |
| `client.interact` | `Interact` | 4 | `createBrowserSession` · `deleteBrowserSession` · `executeBrowserCode` · `listBrowserSessions` |
| `client.feedback` | `Feedback` | 2 | `submitEndpointFeedback` · `submitSearchFeedback` |
| `client.support` | `Support` | 2 | `askSupportAgent` · `searchSupportDocs` |
| `client.researchApi` | `ResearchApi` | 3 | `researchGetPaper` · `researchRelatedPapers` · `researchSearchPapers` |
| `client.developer` | `Developer` | 2 | `developerSearch` · `developerSearchPost` |
| `client.threatProtection` | `ThreatProtection` | 2 | `getThreatProtection` · `updateThreatProtection` |
...
⚠ **The request type name is not uniformly `<Operation>Request`.** One operation takes `<Operation>RequestParams` instead: `crawlParamsPreview`. Take the name from the operation's **Signature** bullet on its map page; never construct it from the method name.
...
```bash
node -e "console.log(require.resolve('firecrawl-api/package.json'))"
```

Failing that it is at `node_modules/firecrawl-api/`. **If the package is not installed, there is no map and no source to read** — mark the fact `UNVERIFIED` and say what would settle it rather than answering from memory.

Every `Source` path on the map is relative to that package root, so `src/models/<file>.ts` opens as written from there — the package ships its `src/` tree, so the path resolves inside `node_modules/firecrawl-api/` exactly as the map writes it. An import specifier ending `.js` inside that source is the NodeNext spelling of the sibling `.ts` file.
...
The table below covers everything else, and the full body behind a map row. Paths are relative to `node_modules/firecrawl-api/`:
...
Beyond the usual signatures and model members, a contract sheet for the Firecrawl API TypeScript SDK is incomplete without these, because each one is a decision the implementer cannot make correctly from the signature alone.
...
3. **The exact request type name per operation**, taken from the **Signature** bullet — One operation takes `<Operation>RequestParams`, not `<Operation>Request`.
...
9. **That `Error`, `Event` imported from this package are model types, not the globals of those names** — every sheet that references one should carry the alias it will be imported under. The error base is re-exported as `FirecrawlApiError` for the same reason.
