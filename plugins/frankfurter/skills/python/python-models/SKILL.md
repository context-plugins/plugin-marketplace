...
your SDK source (`frankfurter/models/`).
...
```python
from frankfurter.api_helper import APIHelper
...
`UnionTypeLookUp` registry (in `frankfurter/utilities/union_type_lookup.py`) and each candidate model
...
```python
from frankfurter.models.animal import Cat, Dog
...
```python
from frankfurter.models.animal import Animal, Cat, Dog
