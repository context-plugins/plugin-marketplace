...
All settings are passed as keyword arguments to `Configuration.__init__` (or to `HotelNameAutocompleteClient`
directly, which wraps them in a `Configuration` internally). Confirm defaults from
`hotelnameautocomplete/configuration.py` in the cloned source.
...
```python
import requests
from hotelnameautocomplete.configuration import Configuration
...
`hotelnameautocomplete/configuration.py`) and any API-specific server parameters (e.g. `port`, `suites`):
...
`hotelnameautocomplete/configuration.py` and call `config.get_base_uri()` (or
...
from environment variables or a `.env` file — grep `from_environment` in `hotelnameautocomplete/configuration.py`
...
`NumberPagedResponse`) live in `hotelnameautocomplete/pagination/` — inspect that directory for the exact type
...
and response, subclass `HttpCallBack` (from `hotelnameautocomplete/http/http_call_back.py`) and pass an instance
as `http_call_back=`:
