...
- `hosted-onboarding-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'hosted-onboarding-apilib';
...
```ts
import { {Resource}Controller } from 'hosted-onboarding-apilib';
