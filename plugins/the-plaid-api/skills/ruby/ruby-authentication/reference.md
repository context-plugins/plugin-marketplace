...
Confirm every name against `lib/the_plaid_api/client.rb`, `lib/the_plaid_api/configuration.rb`, the
`lib/the_plaid_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
