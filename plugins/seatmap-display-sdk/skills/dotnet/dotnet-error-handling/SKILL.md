...
> `{ApiGroup}`, `SeatmapDisplay`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `SeatmapDisplay.Core.Exceptions`
- `ApiError` **and** `RawError` → `SeatmapDisplay.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `SeatmapDisplay.Errors`
...
```csharp
using SeatmapDisplay.Core.Exceptions;     // SdkException<TError>
using SeatmapDisplay.Core.ErrorResponse;  // ApiError, RawError
using SeatmapDisplay.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`SeatmapDisplay.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using SeatmapDisplay.Core.Exceptions;     // SdkException<TError>
using SeatmapDisplay.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `SeatmapDisplay.Core.ErrorResponse`. Its public
...
`readonly struct` under `SeatmapDisplay.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using SeatmapDisplay.Core.Models;        // ApiResult<TResponse, TError>
using SeatmapDisplay.Core.ErrorResponse; // RawError
using SeatmapDisplay.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `SeatmapDisplay.Core.Exceptions`) — an auth *application* failure, not an API error — when the
