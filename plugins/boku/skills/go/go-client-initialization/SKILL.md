...
- `boku` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/boku-go-sdk` — the Go import path for the root package; `github.com/context-plugins/boku-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/boku-go-sdk"

client := boku.NewClient(
    boku.CreateConfiguration(
        boku.WithEnvironment(boku.PRODUCTION),
...
from environment variables named `BOKU_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := boku.NewClient(boku.CreateConfigurationFromEnvironment())
```
...
```go
client := boku.NewClient(
    boku.CreateConfiguration(boku.WithEnvironment(boku.PRODUCTION)),
)
```
...
client := boku.NewClient(
    boku.CreateConfiguration(
        boku.WithHttpConfiguration(
            boku.CreateHttpConfiguration(
                boku.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                boku.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = boku.NewClient(boku.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api boku.ClientInterface
}

func NewService(api boku.ClientInterface) *Service {
