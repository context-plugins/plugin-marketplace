---
name: dotnet-postnl-getting-started
description: postnl .NET SDK identity and lookup layer for the dotnet-postnl-sdk helper agent (.NET/C# only) — install, root namespace, environments, auth pattern, and the bundled SDK map of every operation signature, model, enum, union and error type. The helper agent loads this to answer contract questions; other agents work from the contract sheet it produces.
---

# Getting started with the postnl .NET SDK

> **Who this skill is for.** This is the **map layer**, preloaded for the `dotnet-postnl-sdk` helper
...
| --- | --- |
| API | postnl |
| Source repo | https://github.com/context-plugins/postnl-csharp-sdk (branch `main`) |
| Root namespace | `PostnlEcommerce` (the `using` namespace) |
| Client class | `PostnlEcommerceClient` |
| Options class | `PostnlEcommerceClientOptions` |
<!-- crawler:auth -->
| Auth | Credentials properties on `PostnlEcommerceClientOptions`: `ApikeyHeader: string?` — see the SDK map's *Servers & auth* section |
<!-- /crawler:auth -->
<!-- crawler:environments -->
| Environments | `options.Environment` — `ServerEnvironment` members: `ProductionServer`, `NonProductionServer` (see the SDK map's *Servers & auth* section) |
<!-- /crawler:environments -->
...
namespaces transitively, so `using PostnlEcommerce.Models;` alone does **not** make enums, union
...
git clone --branch main https://github.com/context-plugins/postnl-csharp-sdk
dotnet add reference postnl-csharp-sdk/PostnlEcommerce.csproj
```
...
temp directory** (`<temp>/postnl-dotnet-src/`), never in the project
...
branch the map was generated from) — into a fresh timestamped folder under `<temp>/postnl-dotnet-src/`, and reuse
that folder for the rest of your session:

```bash
# Linux/macOS:
dir="${TMPDIR:-/tmp}/postnl-dotnet-src/$(date +%Y%m%d-%H%M%S)"
git clone --depth 1 --branch main https://github.com/context-plugins/postnl-csharp-sdk "$dir"
# Reuse "$dir" for the rest of your session (it is your clone path).
```

```powershell
# Windows (PowerShell):
$dir = "$env:TEMP\postnl-dotnet-src\$(Get-Date -Format yyyyMMdd-HHmmss)"
git clone --depth 1 --branch main https://github.com/context-plugins/postnl-csharp-sdk $dir
# Reuse $dir for the rest of your session (it is your clone path).
```

The clone lives in `<temp>/postnl-dotnet-src/`, **never** in the project repo, and its path never goes into
`postnl-dotnet-plan.md` — the main agent must never see the clone or its path.
...
- `Servers/`, `PostnlEcommerceClient.cs`, `ServiceCollectionExtensions.cs` — environments, the client, DI.
...
1. **Client & DI setup** — load **dotnet-client-initialization** before you write
   `new PostnlEcommerceClient(...)`, build its options, or DI-register via
   `AddPostnlEcommerceClient`. (*The signature won't tell you:* the `HttpClient`/handler
...
2. **Authentication** — load **dotnet-authentication** before you set credentials. The scheme(s) this SDK
   accepts are the credentials properties on `PostnlEcommerceClientOptions` — the map's *Servers & auth* section
