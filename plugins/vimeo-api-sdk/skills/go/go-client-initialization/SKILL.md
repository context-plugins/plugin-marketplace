...
- `vimeoapi` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/vimeo-api-go` — the Go import path for the root package; `github.com/context-plugins/vimeo-api-go/models` for request/response types.
...
```go
import "github.com/context-plugins/vimeo-api-go"

client := vimeoapi.NewClient(
    vimeoapi.CreateConfiguration(
        vimeoapi.WithEnvironment(vimeoapi.PRODUCTION),
...
from environment variables named `VIMEOAPI_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := vimeoapi.NewClient(vimeoapi.CreateConfigurationFromEnvironment())
```
...
Environments are constants of the `Environment` string type in the root package (e.g. `vimeoapi.PRODUCTION`,
`vimeoapi.TESTING`) — the names and the default are per-API; the default is whatever `DefaultConfiguration()`
sets. Select one with `WithEnvironment`:

```go
client := vimeoapi.NewClient(
    vimeoapi.CreateConfiguration(vimeoapi.WithEnvironment(vimeoapi.PRODUCTION)),
)
```
    ...
    vimeoapi.CreateConfiguration(
        vimeoapi.WithHttpConfiguration(
            vimeoapi.CreateHttpConfiguration(
                vimeoapi.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                vimeoapi.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = vimeoapi.NewClient(vimeoapi.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api vimeoapi.ClientInterface
}

func NewService(api vimeoapi.ClientInterface) *Service {
