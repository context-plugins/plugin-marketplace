...
> `{ApiGroup}`, `TripPurposePrediction`) — replace it with the concrete identifier from the source.
...
- `SdkException<T>` → `TripPurposePrediction.Core.Exceptions`
- `ApiError` **and** `RawError` → `TripPurposePrediction.Core.ErrorResponse`
- the per-operation `{Operation}Error` models (e.g. `CreateWidgetError`) → `TripPurposePrediction.Errors`
...
```csharp
using TripPurposePrediction.Core.Exceptions;     // SdkException<TError>
using TripPurposePrediction.Core.ErrorResponse;  // ApiError, RawError
using TripPurposePrediction.Errors;              // {Operation}Error types, e.g. CreateWidgetError
...
(`TripPurposePrediction.Errors.{Operation}Error`), not the actual message. Read the typed accessors **inside the
...
```csharp
using TripPurposePrediction.Core.Exceptions;     // SdkException<TError>
using TripPurposePrediction.Core.ErrorResponse;  // RawError
...
Case B needs no `.Errors` using — `RawError` lives under `TripPurposePrediction.Core.ErrorResponse`. Its public
...
`readonly struct` under `TripPurposePrediction.Core.Models`. If the controller has no `{Operation}Result`
...
```csharp
using TripPurposePrediction.Core.Models;        // ApiResult<TResponse, TError>
using TripPurposePrediction.Core.ErrorResponse; // RawError
using TripPurposePrediction.Errors;             // {Operation}Error (Case A only)
...
- On an SDK with **multiple/composite auth schemes**, a call can also throw `AuthSchemeException`
  (under `TripPurposePrediction.Core.Exceptions`) — an auth *application* failure, not an API error — when the
