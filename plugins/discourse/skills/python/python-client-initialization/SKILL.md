...
- `discourse` — the root package name (e.g. `multiauthsample`, `batester`).
- `DiscourseapidocumentationClient` — the generated client class (read it from the `class …Client` declaration in `discourse/discourse_client.py`).
...
from discourse.http.auth.basic_auth import BasicAuthCredentials  # per-scheme import
...
```python
from discourse.configuration import Configuration, Environment
...
Confirm the exact kwarg names from `discourse/discourse_client.py` in the cloned source.
...
`from_environment` in `discourse/configuration.py` for the exact names.
...
Open `discourse/discourse_client.py` to see all available `@LazyProperty` controller names. Each one is
...
`Environment` is an `Enum` subclass in `discourse/configuration.py`. **Read the enum in `configuration.py`
...
`discourse/configuration.py`. There is no free-form base-URL override; to target a custom host (mock
