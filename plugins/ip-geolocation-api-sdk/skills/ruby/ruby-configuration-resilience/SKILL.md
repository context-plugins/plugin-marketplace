...
```ruby
client = IPGeolocationAPI::Client.new(
  max_retries:    3,
...
```ruby
client = IPGeolocationAPI::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = IPGeolocationAPI::Client.new(adapter: :net_http)
```
...
client = IPGeolocationAPI::Client.new(connection: conn)
```
...
client = IPGeolocationAPI::Client.new(environment: Environment::PRODUCTION)
client = IPGeolocationAPI::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = IPGeolocationAPI::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = IPGeolocationAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = IPGeolocationAPI::ProxySettings.from_env
client = IPGeolocationAPI::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < IPGeolocationAPI::HttpCallBack
...
client = IPGeolocationAPI::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = IPGeolocationAPI::Client.from_env
# Overrides take precedence over env vars:
client = IPGeolocationAPI::Client.from_env(timeout: 30, max_retries: 3)
```
