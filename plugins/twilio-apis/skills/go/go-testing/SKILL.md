...
`testing` + `testify/require` for reference; substitute the real `twilioapis` and names.
...
import (
    ...
    "github.com/context-plugins/twilio-apis-go-sdk"   // root package twilioapis
)
...
func clientReturning(status int, body string) (twilioapis.ClientInterface, *stubTransport) {
    ...
    client := twilioapis.NewClient(
        twilioapis.CreateConfiguration(
            twilioapis.WithHttpConfiguration(
                twilioapis.CreateHttpConfiguration(twilioapis.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/twilio-apis-go-sdk/errors"
...
client := twilioapis.NewClient(
    twilioapis.CreateConfiguration(
        twilioapis.WithHttpConfiguration(
            twilioapis.CreateHttpConfiguration(twilioapis.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
