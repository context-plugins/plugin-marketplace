...
- `adyen-data-protection-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'adyen-data-protection-apilib';
...
```ts
import { {Resource}Controller } from 'adyen-data-protection-apilib';
