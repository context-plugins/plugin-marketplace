...
│   └── com.coingecko.api.exceptions.ApiException          ← base for all non-2xx API responses
│       └── com.coingecko.api.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `com.coingecko.api.http.client.HttpContext`, which carries:
