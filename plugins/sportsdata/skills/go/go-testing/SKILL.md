...
`testing` + `testify/require` for reference; substitute the real `sportsdata` and names.
...
import (
    ...
    "github.com/context-plugins/sportsdata-go-sdk"   // root package sportsdata
)
...
func clientReturning(status int, body string) (sportsdata.ClientInterface, *stubTransport) {
    ...
    client := sportsdata.NewClient(
        sportsdata.CreateConfiguration(
            sportsdata.WithHttpConfiguration(
                sportsdata.CreateHttpConfiguration(sportsdata.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/sportsdata-go-sdk/errors"
...
client := sportsdata.NewClient(
    sportsdata.CreateConfiguration(
        sportsdata.WithHttpConfiguration(
            sportsdata.CreateHttpConfiguration(sportsdata.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
