...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new AdyenBalanceControlAPIClient({
...
```typescript
import { {Operation}Error } from 'adyen-balance-control-apilib/errors';
...
```typescript
import { ApiError } from 'adyen-balance-control-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AdyenBalanceControlAPIClient)
    .useValue(new AdyenBalanceControlAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
