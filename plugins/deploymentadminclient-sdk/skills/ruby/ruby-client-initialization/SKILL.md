...
- `DeploymentAdminClient` — the root Ruby module (e.g. `MultiAuthSample`, `ApimaticCalculatorByClient`).
- `deploymentadminclient` — the gem name from the `.gemspec` (e.g. `multi_auth_sample`).
...
The authoritative list is the `initialize` signature in `lib/deploymentadminclient/client.rb`. Common kwargs:
...
Confirm the exact variable names in `lib/deploymentadminclient/configuration.rb`
(`build_default_config_from_env`) and `lib/deploymentadminclient/http/auth/*.rb` (`{Scheme}Credentials.from_env`).
...
In Rails, create the client in an initializer (`config/initializers/deploymentadminclient.rb`) and store it in a
