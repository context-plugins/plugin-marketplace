...
`testing` + `testify/require` for reference; substitute the real `tesserapiv1` and names.
...
import (
    ...
    "github.com/context-plugins/tesser-api-v1-go-sdk"   // root package tesserapiv1
)
...
func clientReturning(status int, body string) (tesserapiv1.ClientInterface, *stubTransport) {
    ...
    client := tesserapiv1.NewClient(
        tesserapiv1.CreateConfiguration(
            tesserapiv1.WithHttpConfiguration(
                tesserapiv1.CreateHttpConfiguration(tesserapiv1.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/tesser-api-v1-go-sdk/errors"
...
client := tesserapiv1.NewClient(
    tesserapiv1.CreateConfiguration(
        tesserapiv1.WithHttpConfiguration(
            tesserapiv1.CreateHttpConfiguration(tesserapiv1.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
