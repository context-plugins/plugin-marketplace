...
│   └── v133.exceptions.ApiException          ← base for all non-2xx API responses
│       └── v133.exceptions.{Name}Exception   ← typed subclass with payload fields
...
`getHttpContext()` returns the SDK's `v133.http.client.HttpContext`, which carries:
