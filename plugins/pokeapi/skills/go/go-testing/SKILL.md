...
`testing` + `testify/require` for reference; substitute the real `pokeapi` and names.
...
import (
    ...
    "github.com/context-plugins/pokeapi-go-sdk"   // root package pokeapi
)
...
func clientReturning(status int, body string) (pokeapi.ClientInterface, *stubTransport) {
    stub := &stubTransport{statusCode: status, body: body}
    client := pokeapi.NewClient(
        pokeapi.CreateConfiguration(
            pokeapi.WithHttpConfiguration(
                pokeapi.CreateHttpConfiguration(pokeapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/pokeapi-go-sdk/errors"
...
client := pokeapi.NewClient(
    pokeapi.CreateConfiguration(
        pokeapi.WithHttpConfiguration(
            pokeapi.CreateHttpConfiguration(pokeapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
