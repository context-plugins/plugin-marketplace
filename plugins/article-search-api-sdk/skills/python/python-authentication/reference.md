...
kwargs to the `ArticleSearchAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `articlesearchapi/articlesearchapi_client.py` and `articlesearchapi/http/auth/` in
the cloned source.

## Basic auth

```python
from articlesearchapi.http.auth.basic_auth import BasicAuthCredentials
...
```python
from articlesearchapi.http.auth.api_key import ApiKeyCredentials
...
```python
from articlesearchapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from articlesearchapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from articlesearchapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from articlesearchapi.http.auth.o_auth_acg import OAuthACGCredentials
from articlesearchapi.models.o_auth_scope_articlesearchapi_enum import OAuthScope{Pkg}Enum
...
client = ArticleSearchAPIClient(o_auth_acg_credentials=creds)
...
```python
from articlesearchapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ArticleSearchAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ArticleSearchAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `articlesearchapi/configuration.py` and each `articlesearchapi/http/auth/*.py` for the exact
