...
function clientReturning(status: number, body: unknown): {
  ...
  const client = new AWSMigrationHubClient({
...
```typescript
import { {Operation}Error } from 'aws-migration-hublib/errors';
...
```typescript
import { ApiError } from 'aws-migration-hublib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(AWSMigrationHubClient)
    .useValue(new AWSMigrationHubClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
