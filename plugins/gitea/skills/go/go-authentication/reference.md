...
```go
gitea.WithOAuthCCGCredentials(
    gitea.NewOAuthCCGCredentials(clientId, clientSecret),
)
```
...
| `.WithOAuthTokenProvider(func(last models.OAuthToken, mgr gitea.OAuthCCGManager) models.OAuthToken)` | supply/refresh the token yourself (e.g. load from DB; call `mgr.FetchToken(ctx)` to mint a new one) |
...
```go
creds := gitea.NewOAuthCCGCredentials(id, secret).
    WithOAuthOnTokenUpdate(func(t models.OAuthToken) { saveToDB(t) }).
    WithOAuthTokenProvider(func(last models.OAuthToken, mgr gitea.OAuthCCGManager) models.OAuthToken {
...
```go
creds := gitea.NewOAuthACGCredentials(clientId, clientSecret, redirectUri).
    WithOAuthScopes([]models.{OAuthScopeEnum}{ models.{OAuthScopeEnum}_{SCOPE} })  // scope enum is per-API

client := gitea.NewClient(gitea.CreateConfiguration(gitea.WithOAuthACGCredentials(creds)))
...
client = client.CloneWithConfiguration(
    gitea.WithOAuthACGCredentials(client.Configuration().OAuthACGCredentials().WithOAuthToken(token)),
)
```
...
```go
creds := gitea.NewOAuthROPCGCredentials(clientId, clientSecret, username, password)
client := gitea.NewClient(gitea.CreateConfiguration(gitea.WithOAuthROPCGCredentials(creds)))
...
```go
gitea.WithOAuthBearerTokenCredentials(gitea.NewOAuthBearerTokenCredentials(accessToken))
```

## Basic / custom header / custom query parameter

```go
gitea.WithBasicAuthCredentials(gitea.NewBasicAuthCredentials(username, password))
gitea.WithApiHeaderCredentials(gitea.NewApiHeaderCredentials(token, apiKey))   // custom header scheme
gitea.WithApiKeyCredentials(gitea.NewApiKeyCredentials(token, apiKey))         // custom query-param scheme
```
...
`CreateConfigurationFromEnvironment(...)` reads credentials from `GITEA_...` variables — e.g.
`GITEA_USERNAME`, `GITEA_PASSWORD`, `GITEA_TOKEN`, `GITEA_API_KEY`,
`GITEA_O_AUTH_CLIENT_ID`, `GITEA_O_AUTH_CLIENT_SECRET`, `GITEA_ACCESS_TOKEN`. Grep the
