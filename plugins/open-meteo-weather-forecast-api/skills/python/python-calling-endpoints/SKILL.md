...
Open `openmeteoweatherforecastapi/openmeteoweatherforecastapi_client.py` for the controller property names, then the relevant
`openmeteoweatherforecastapi/controllers/{resource}_controller.py` for the operation's exact signature.
...
```python
from openmeteoweatherforecastapi.models.{request_model} import {RequestModel}
...
Open `openmeteoweatherforecastapi/models/{model}.py` to confirm the `_names`, `_optionals`, and constructor args.
...
```python
from openmeteoweatherforecastapi.models.suite_code_enum import SuiteCodeEnum
...
```python
from openmeteoweatherforecastapi.http.http_call_back import HttpCallBack
...
client = OpenMeteoWeatherForecastAPIClient(http_call_back=capture, ...)
result = client.authentication.custom_authentication()
...
an iterator over the items on that page. Inspect `openmeteoweatherforecastapi/pagination/` in the source.

## Finding the right method

- Controller property names are on `OpenMeteoWeatherForecastAPIClient` in `openmeteoweatherforecastapi/openmeteoweatherforecastapi_client.py`.
- Operation method signatures are in `openmeteoweatherforecastapi/controllers/{resource}_controller.py`.
- `doc/controllers/*.md` lists every operation with parameters and usage snippets — grep it first.
- Request/response model types are under `openmeteoweatherforecastapi/models/`; enum types have `from_value()`.
