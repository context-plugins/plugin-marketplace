...
kwargs to the `GiteaAPIClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `giteaapi/giteaapi_client.py` and `giteaapi/http/auth/` in
the cloned source.
...
```python
from giteaapi.http.auth.api_key import ApiKeyCredentials
...
```python
from giteaapi.http.auth.api_header import ApiHeaderCredentials
...
```python
from giteaapi.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from giteaapi.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from giteaapi.http.auth.o_auth_acg import OAuthACGCredentials
from giteaapi.models.o_auth_scope_giteaapi_enum import OAuthScope{Pkg}Enum
...
client = GiteaAPIClient(o_auth_acg_credentials=creds)
...
```python
from giteaapi.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = GiteaAPIClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `GiteaAPIClient.from_environment()`) read each credential
...
Grep `from_environment` in `giteaapi/configuration.py` and each `giteaapi/http/auth/*.py` for the exact
