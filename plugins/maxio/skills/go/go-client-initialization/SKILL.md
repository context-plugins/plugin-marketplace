...
- `maxio` — the root package name (e.g. `multiauthsample`).
- `github.com/context-plugins/maxio-go-sdk` — the Go import path for the root package; `github.com/context-plugins/maxio-go-sdk/models` for request/response types.
...
```go
import "github.com/context-plugins/maxio-go-sdk"

client := maxio.NewClient(
    maxio.CreateConfiguration(
        maxio.WithEnvironment(maxio.PRODUCTION),
...
from environment variables named `MAXIO_...` (e.g. `MULTIAUTHSAMPLE_ENVIRONMENT`,
...
```go
client := maxio.NewClient(maxio.CreateConfigurationFromEnvironment())
```
...
```go
client := maxio.NewClient(
    maxio.CreateConfiguration(maxio.WithEnvironment(maxio.PRODUCTION)),
)
```
...
client := maxio.NewClient(
    maxio.CreateConfiguration(
        maxio.WithHttpConfiguration(
            maxio.CreateHttpConfiguration(
                maxio.WithTimeout(30),                  // seconds; 0 = no timeout (the default)
                maxio.WithTransport(myRoundTripper),    // default: http.DefaultTransport
            ),
        ),
    ),
)
...
var apiClient = maxio.NewClient(maxio.CreateConfigurationFromEnvironment())
...
```go
type Service struct {
    api maxio.ClientInterface
}

func NewService(api maxio.ClientInterface) *Service {
