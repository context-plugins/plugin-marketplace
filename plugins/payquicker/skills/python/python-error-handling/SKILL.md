...
`payquicker/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `payquicker/exceptions/` and operation docstrings.
...
```python
from payquicker.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `payquicker/exceptions/`.
...
```python
from payquicker.exceptions.api_exception import APIException
from payquicker.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`payquicker/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`payquicker/exceptions/` for the class.
...
`payquicker/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `payquicker/exceptions/`
  or a `ValueError` — check the auth handler in `payquicker/http/auth/` for the exact type.
