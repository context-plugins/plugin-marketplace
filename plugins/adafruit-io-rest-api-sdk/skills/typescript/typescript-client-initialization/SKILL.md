...
- `adafruit-io-rest-apilib` — the npm package name (the `"name"` in `package.json`, e.g. `multiauth-samplelib`).
...
```ts
import { Client, Environment } from 'adafruit-io-rest-apilib';
...
```ts
import { {Resource}Controller } from 'adafruit-io-rest-apilib';
