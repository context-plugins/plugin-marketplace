...
`TraderApiClient`/`TraderApiClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `TraderApiClient`,
...
static TraderApiClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new TraderApiClient(new HttpClient(handler), new TraderApiClientOptions { /* auth not needed for stubs */ });
}
```
...
using TraderApi.Errors;              // {Operation}Error types
...
```csharp
using TraderApi.Core.Exceptions;
using TraderApi.Core.ErrorResponse;
...
using TraderApi.Core.ErrorResponse; // RawError (Case B)
...
var client = new TraderApiClient(new HttpClient(handler), new TraderApiClientOptions());
...
- For DI-based code, the SDK's `AddTraderApiClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `TraderApiClient` from the provider:
  ```csharp
  services.AddTraderApiClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<TraderApiClient>();
  ```
