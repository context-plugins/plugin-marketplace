...
`testing` + `testify/require` for reference; substitute the real `geoapify` and names.
...
import (
    ...
    "github.com/context-plugins/geoapify-go-sdk"   // root package geoapify
)
...
func clientReturning(status int, body string) (geoapify.ClientInterface, *stubTransport) {
    ...
    client := geoapify.NewClient(
        geoapify.CreateConfiguration(
            geoapify.WithHttpConfiguration(
                geoapify.CreateHttpConfiguration(geoapify.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/geoapify-go-sdk/errors"
...
client := geoapify.NewClient(
    geoapify.CreateConfiguration(
        geoapify.WithHttpConfiguration(
            geoapify.CreateHttpConfiguration(geoapify.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
