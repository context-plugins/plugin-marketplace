...
`mailboxvalidatorfreeemailchecker/exceptions/api_exception.py`. Transport failures (`requests.exceptions.Timeout`,
...
> class names from `mailboxvalidatorfreeemailchecker/exceptions/` and operation docstrings.
...
```python
from mailboxvalidatorfreeemailchecker.exceptions.api_exception import APIException
...
For status codes the API documents, the generator creates a typed subclass in `mailboxvalidatorfreeemailchecker/exceptions/`.
...
```python
from mailboxvalidatorfreeemailchecker.exceptions.api_exception import APIException
from mailboxvalidatorfreeemailchecker.exceptions.o_auth_provider_exception import OAuthProviderException
...
file (`mailboxvalidatorfreeemailchecker/controllers/{resource}_controller.py`) or `doc/controllers/*.md`, and check
`mailboxvalidatorfreeemailchecker/exceptions/` for the class.
...
`mailboxvalidatorfreeemailchecker/http/` in the source if you are unsure of the transport library.
...
- On OAuth flows, a failed token exchange may raise a specific exception from `mailboxvalidatorfreeemailchecker/exceptions/`
  or a `ValueError` — check the auth handler in `mailboxvalidatorfreeemailchecker/http/auth/` for the exact type.
