...
client = stub_client(stubs)
assert_raises IPGeolocationAPI::APIException do
...
def build_test_client
  IPGeolocationAPI::Client.new(
    environment: IPGeolocationAPI::Environment::TESTING,  # resolves to http://localhost:3000
    max_retries: 0
  )
end
...
expect { client.authentication.custom_authentication }
  .to raise_error IPGeolocationAPI::APIException
```
...
```ruby
expect { call }.to raise_error(IPGeolocationAPI::OAuthProviderException) do |e|
...
```ruby
class ResponseCatcher < IPGeolocationAPI::HttpCallBack
...
client  = IPGeolocationAPI::Client.new(connection: conn, max_retries: 0, http_callback: catcher)
