...
`testing` + `testify/require` for reference; substitute the real `paypalapi` and names.
...
import (
    ...
    "github.com/context-plugins/paypal-api-go"   // root package paypalapi
)
...
func clientReturning(status int, body string) (paypalapi.ClientInterface, *stubTransport) {
    ...
    client := paypalapi.NewClient(
        paypalapi.CreateConfiguration(
            paypalapi.WithHttpConfiguration(
                paypalapi.CreateHttpConfiguration(paypalapi.WithTransport(stub)),
            ),
...
```go
import sdkerrors "github.com/context-plugins/paypal-api-go/errors"
...
client := paypalapi.NewClient(
    paypalapi.CreateConfiguration(
        paypalapi.WithHttpConfiguration(
            paypalapi.CreateHttpConfiguration(paypalapi.WithTransport(redirectTransport{host: target.Host})),
        ),
    ),
)
```
