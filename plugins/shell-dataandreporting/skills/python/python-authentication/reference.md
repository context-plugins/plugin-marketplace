...
kwargs to the `ShelldatareportingapisClient` constructor (or to `Configuration`). Exact credential class names and
kwarg names are generated per-API — confirm from `shelldataandreporting/shelldataandreporting_client.py` and `shelldataandreporting/http/auth/` in
the cloned source.

## Basic auth

```python
from shelldataandreporting.http.auth.basic_auth import BasicAuthCredentials
...
```python
from shelldataandreporting.http.auth.api_key import ApiKeyCredentials
...
```python
from shelldataandreporting.http.auth.api_header import ApiHeaderCredentials
...
```python
from shelldataandreporting.http.auth.o_auth_bearer_token import OAuthBearerTokenCredentials
...
```python
from shelldataandreporting.http.auth.o_auth_ccg import OAuthCCGCredentials
...
```python
from shelldataandreporting.http.auth.o_auth_acg import OAuthACGCredentials
from shelldataandreporting.models.o_auth_scope_shelldataandreporting_enum import OAuthScope{Pkg}Enum
...
client = ShelldatareportingapisClient(o_auth_acg_credentials=creds)
...
```python
from shelldataandreporting.http.auth.o_auth_ropcg import OAuthROPCGCredentials
...
client = ShelldatareportingapisClient(o_auth_ropcg_credentials=creds)
...
`Configuration.from_environment()` (and `ShelldatareportingapisClient.from_environment()`) read each credential
...
Grep `from_environment` in `shelldataandreporting/configuration.py` and each `shelldataandreporting/http/auth/*.py` for the exact
