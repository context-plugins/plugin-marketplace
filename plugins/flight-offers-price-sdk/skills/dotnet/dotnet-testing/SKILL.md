...
`FlightOffersPriceClient`/`FlightOffersPriceClientOptions` as well.

> Throughout this skill, `{...}` is a placeholder for a name you take from your SDK (e.g. `FlightOffersPriceClient`,
...
static FlightOffersPriceClient ClientReturning(HttpStatusCode status, string json)
{
    var handler = new StubHandler(_ => new HttpResponseMessage(status)
    {
        Content = new StringContent(json, System.Text.Encoding.UTF8, "application/json")
    });
    return new FlightOffersPriceClient(new HttpClient(handler), new FlightOffersPriceClientOptions { /* auth not needed for stubs */ });
}
```
...
using FlightOffersPrice.Errors;              // {Operation}Error types
...
```csharp
using FlightOffersPrice.Core.Exceptions;
using FlightOffersPrice.Core.ErrorResponse;
...
using FlightOffersPrice.Core.ErrorResponse; // RawError (Case B)
...
var client = new FlightOffersPriceClient(new HttpClient(handler), new FlightOffersPriceClientOptions());
...
- For DI-based code, the SDK's `AddFlightOffersPriceClient` resolves the **default (unnamed)** `IHttpClientFactory`
  client, so register your stub on that one, then resolve `FlightOffersPriceClient` from the provider:
  ```csharp
  services.AddFlightOffersPriceClient(o => { /* ... */ });
  services.AddHttpClient(Options.DefaultName).ConfigurePrimaryHttpMessageHandler(() => stubHandler);
  var client = services.BuildServiceProvider().GetRequiredService<FlightOffersPriceClient>();
  ```
