...
```ruby
client = GoogleWorkspace::Client.new(
  max_retries:    3,
...
```ruby
client = GoogleWorkspace::Client.new(timeout: 30)  # seconds per attempt; default 60
```
...
```ruby
client = GoogleWorkspace::Client.new(adapter: :net_http)
```
...
client = GoogleWorkspace::Client.new(connection: conn)
```
...
client = GoogleWorkspace::Client.new(environment: Environment::PRODUCTION)
client = GoogleWorkspace::Client.new(environment: Environment::TESTING)
```
...
```ruby
proxy = GoogleWorkspace::ProxySettings.new(
  address:  'http://proxy.example.com',
  port:     8080,
  username: 'user',        # optional
  password: 'pass'         # optional
)
client = GoogleWorkspace::Client.new(proxy_settings: proxy)
```
...
```ruby
proxy = GoogleWorkspace::ProxySettings.from_env
client = GoogleWorkspace::Client.new(proxy_settings: proxy)
```
...
```ruby
class LogCallback < GoogleWorkspace::HttpCallBack
...
client = GoogleWorkspace::Client.new(http_callback: LogCallback.new)
```
...
```ruby
client = GoogleWorkspace::Client.from_env
# Overrides take precedence over env vars:
client = GoogleWorkspace::Client.from_env(timeout: 30, max_retries: 3)
```
