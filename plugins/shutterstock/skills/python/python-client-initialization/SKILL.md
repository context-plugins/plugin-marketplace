...
description: Construct and configure an APIMatic-generated Python SDK client — pass keyword args (credentials objects + environment= + timeout/retry/proxy kwargs) directly to the ShutterstockClient constructor or to a separate Configuration object, use from_environment() to read from a .env file and env vars, access controllers via @LazyProperty properties (not constructors), pass a custom requests.Session via http_client_instance, and reuse the long-lived client. Use the moment you write ShutterstockClient(...), build a Configuration, pick an environment, or wire the client into your application — load it even after reading the constructor in the source, since the signature shows the kwargs but not the credential-objects pattern, the @LazyProperty controller access, or the lifetime/reuse rules.
---

# Initializing an APIMatic Python SDK client

> `ShutterstockClient` is the SDK's client class — **read the real name** from `shutterstock/shutterstock_client.py`
...
- `ShutterstockClient` — the generated client class (read it from the `class …Client` declaration in `shutterstock/shutterstock_client.py`).
- `{resource}` — a controller property name on the client (e.g. `authentication`, `transaction`).

## The constructor shape

The `ShutterstockClient` constructor accepts all configuration as **keyword arguments** — there is no
...
```python
from shutterstock.shutterstock_client import ShutterstockClient
...
client = ShutterstockClient(config=config)
```
...
`from_environment()` is a class method on both `ShutterstockClient` and `Configuration` that reads
...
client = ShutterstockClient.from_environment()
...
Controllers are `@LazyProperty` properties on `ShutterstockClient`. Access them as attributes —
...
client = ShutterstockClient(environment=Environment.PRODUCTION)
```
...
client = ShutterstockClient(config=new_config)
```
