...
```typescript
import { {Operation}Error } from 'paypal-apilib/errors';
...
```typescript
import { ApiError } from 'paypal-apilib';
...
- For DI-based code (e.g. NestJS), override the provider in your test module:
  ...
  const moduleRef = await Test.createTestingModule({
    ...
    .overrideProvider(PayPalAPIClient)
    .useValue(new PayPalAPIClient({ customFetch: stubFetch, retryConfig: { maxNumberOfRetries: 0 } }))
    .compile();
  ```
