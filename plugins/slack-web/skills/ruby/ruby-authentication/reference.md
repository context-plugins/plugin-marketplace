...
Confirm every name against `lib/slack_web_api/client.rb`, `lib/slack_web_api/configuration.rb`, the
`lib/slack_web_api/http/auth/*.rb` files, and `doc/auth/*.md` in the cloned source.
